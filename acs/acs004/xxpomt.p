...
/* Revision: 1.13.2.30.1.1  BY: Tejasvi Kulkarni  DATE: 04/28/05 ECO: *P3JJ* */
/* Revision: 1.13.2.30.1.2  BY: Robin McCarthy    DATE: 08/11/05 ECO: *P2PJ* */
/* Revision: 1.13.2.30.1.6  BY: Prerita Joshi     DATE: 07/21/06 ECO: *P4XY* */
/* $Revision: 1.13.2.30.1.7 $            BY: Ajit     DATE: 06/07/07 ECO: *Q15G*  */
/*-Revision end---------------------------------------------------------------*/
...
define variable old_chr01      like po__chr01.
...
repeat:
   ...
   {xxpopomt02.i}  /* Shared frames a and b */

   /* ADDRESS FORMS */
   /* VENDOR ADDRESS */
   {mfadform.i "vend" 1 SHIP_FROM}
   /* SHIP TO ADDRESS */
   ...
   do transaction on error undo, retry:
     ...
     if {gpisapi.i} then
        {pxmsg.i &MSGNUM=1 &ERRORLEVEL={&INFORMATION-RESULT} &MSGARG1=ponbr}
     else
     ...
     .
      ...
      do on error undo, retry:
         ...
         */
         ...
         if c-application-mode <> "API" then
            /* DISPLAY VENDOR ADDRESS */
            {mfaddisp.i "input po__chr01" vend}
      ...
      if new_po then do:
         assign po__chr01 = input po_vend.
         display po__chr01 with frame a.
          {mfaddisp.i po__chr01 vend}
         if c-application-mode <> "API" then do:
...
/**0914***/
 do on error undo, retry: /* 0914 */
          if c-application-mode <> "API" then do:
            prompt-for po_mstr.po__chr01 with frame a
            editing:
               /* FIND NEXT/PREVIOUS  RECORD */
           {mfnp01.i ad_mstr po__chr01 ad_addr ad_ref " ad_mstr.ad_domain =
              global_domain and po_vend " ad_ref}
               if recno <> ? then do:
                  po__chr01 = ad_addr.
                  display po__chr01 with frame a.
                  /* DISPLAY SHIP TO ADDRESS */
            ...
            assign po__chr01 = input po__chr01.
            {mfaddisp.i po__chr01 vend}
            if not can-find (first ad_mstr no-lock where
                   ad_domain = global_domain and (po__chr01 = po_vend or
                  (ad_addr = po__chr01 and ad_ref = po_vend)))
            then do:
                {pxmsg.i &MSGNUM=80401 &ERRORLEVEL=3}
                undo, retry.
               /*** next-prompt po__chr01 with frame a.
                undo,retry.                 **/
            end.
         end.
         else    /*if c-application-mode = "API"*/
...
end. /* do on error undo, retry: 0914*/
      /* PURCHASE ORDER MAINTENANCE -- ORDER HEADER subroutine */
      {gprun.i ""xxpopomtb.p"" "(input using_supplier_consignment)"}
...
------------------------------------------------------------------------------*/
   ...
   {gpbrparm.i &browse=gplu908.p &parm=c-brparm1 &val=poNbr}
END PROCEDURE.
