/* xxrcshwb.p - Shipper Workbench                                            */
/* REVISION: 1.0 LAST MODIFIED: 09/14/10   BY: apple                         */
/* REVISION: 1.1 LAST MODIFIED: 09/26/10   BY: zy 出货量不可以大于订单量     */
/*-Revision end--------------------------------------------------------------*/
...
define variable bIsError as logical no-undo.
define variable iErrorID as integer no-undo.
define variable sNrm as character no-undo.
define variable sPrefix as character no-undo.
define variable sChannel as character no-undo.
/*SS-20100913 end *********/
...
/*SS-20100913 add***************************/
   recgp = input frame a inv_mov.
  /*
   if recgp = ""
   then do:
      /* Invalid Number Range */
      {pxmsg.i &MSGNUM=80201 &ERRORLEVEL=3}
            next-prompt
               inv_mov
            with frame a.
            undo mainloop, retry mainloop.
   ...
   */
   ...
   find first im_mstr where im_domain = global_domain and im_inv_mov = recgp no-lock no-error.
   if not available im_mstr then do:
      /* Invalid Number Range */
      {pxmsg.i &MSGNUM=80201 &ERRORLEVEL=3}
            next-prompt
               inv_mov
   ...
   find first nr_mstr where nr_domain = global_domain
      and nr_dataset = recgp no-lock no-error.
   if available nr_mstr
   then do:
       recgp = nr_seqid.
   end.
   else do:
