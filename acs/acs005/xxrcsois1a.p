/* xxrcsois1a.p - Customer Schedules Confirm Shipper                         */
/* REVISION: 1.0      LAST MODIFIED: 09/25/10   BY: zy                       */
/*-Revision end--------------------------------------------------------------*/

{mfdeclre.i}
{gplabel.i}
{cxcustom.i "xxRCSOIS1A.P"}
...
do on error undo, leave:
   ...
   do transaction:
     ...
     for first pt_mstr
        fields (pt_domain pt_part pt_cfg_type)
   ...
   do transaction on error undo, leave:
      ...
      do on error undo, retry:
        ...
        if (absolute(so_trl1_amt) + absolute(so_trl2_amt) +
            absolute(so_trl3_amt) + l_cur_tax_amt) <> 0 then
                l_increment = yes.

        if use_shipper and not l_increment then
            {pxmsg.i &MSGNUM=6900 &ERRORLEVEL=2}

        if not l_increment then
            undo_stat = yes.
           end. /* If so_consignment then do: */
           ...
           if undo_stat then do:
            ...
            if not l_increment then do:
                auto_post = no.
            do i = 1 to order_ct:
                order_num = if (i <= 30) then order_nbrs[i]                             else
                        entry(i - 30,order_nbr_list).
                ...
                if available tt_somstr then
                    so_to_inv = tt_sotoinv.

            end. /* do i = 1 to order_ct */
            end. /*if not l_increment then do*/
        end.
         end.
         end.
   ...
   then do transaction:
      ...
      do i = 1 to inv_ct:
         ...
         {gprun.i ""xxsoivpst1.p""
                  "(input abs_recid,
                    input l_po_schd_nbr)"}
/*zy*/ if available so_mstr then do:                           
