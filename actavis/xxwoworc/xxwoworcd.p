...
then do:
   ...
   assign
    ...
    /* ss - 100517.1 -b */
   v_logical = NO .
   FIND FIRST pod_det WHERE pod_domain = GLOBAL_domain AND pod_type = "s" AND  pod_wo_lot = wo_lot NO-LOCK NO-ERROR .
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   IF v_logical  THEN DO:
       ...
       /* ss - 100602.1 -b */
            ...
            FIND LAST  prh_hist  WHERE prh_domain = GLOBAL_domain  AND  prh_lot =   wo_lot  NO-LOCK NO-ERROR .
...
/*GUI*/ if global-beam-me-up then undo, leave.
 /*FOR EACH sr_wkfl*/
        ...
        /* ss - 100526.1 -e */
             for last wr_route no-lock where wr_domain = global_domain and wr_lot = wo_lot break by wr_op:
         	 end.
         	 if avail wr_route then do: /* 有工艺记录 */
         	 	 ...
         	 	 if not avail pod_det then do:  /* 不是委外加工单 */
          	 	 ...
          	 	 if tot_units <> wr_qty_comp then do:
                     ...
                     /* ss - 100528.1 -b */
                        v_qty =  lotserial_qty .
                       message "入库数量不等于回报数量:"  + string(wr_qty_comp) .
