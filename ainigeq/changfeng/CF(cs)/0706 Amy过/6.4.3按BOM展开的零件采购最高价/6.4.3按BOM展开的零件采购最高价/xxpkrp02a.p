...
/*SS 20090923 - B*/
                   define variable allcost as decimal.
/*SS 20090923 - E*/
...
/*K10Y*/    {wbrp06.i &command = update &fields = " part part1  op op1
...
/*SS 20090928 - B*/
		   allcost = 0.
                   ...
                   for each tr_hist where tr_type = "RCT-PO" and tr_part = pkpart use-index tr_part_eff no-lock:
		       
		       if tr_price > allcost then allcost = tr_price.

		   end.

		   if allcost = 0 then do:
			   for each spt_det where spt_site = site and spt_sim = "standard" and spt_part = pkpart  no-lock:
			       allcost   = allcost +  spt_cst_tl + spt_cst_ll.
			   end.
                   end.		
...
/*G234*                    if qty <> 0 then qty else pt_mstr.pt_ord_qty */
                           @ qty
/*SS 20090923 - B*/
													allcost @ pt_mstr.pt_price 
/*SS 20090923 - E*/                                                     
/*SS 20080623 - B*/
													(accum total by pkop pkqty) * allcost @ ar_amt
/*
