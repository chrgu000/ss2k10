...
/* ***********************Kaine B Add********************** */
FORM HEADER
	FILL("-", 78) FORMAT "x(78)" AT 1
	"编制:                 审核:           生产部签收:" AT 1
	SKIP(3)
	"备注:" AT 1
	SKIP(1)
WITH FRAME frmbottom WIDTH 80 PAGE-BOTTOM.
VIEW FRAME frmbottom.
/* ***********************Kaine E Add********************** */


form with frame ceuro width 132 down.
...
/*Kaine*  {so05f01.i}               /* INCLUDE FILE FOR FRAME C */  */
/*Kaine*/ {xxso05f01.i}               /* INCLUDE FILE FOR FRAME C */

/* SET PREPAID OLD TO PREPAID:FORMAT AND EXT_PRICE:FORMAT */
/*Kaine*  assign  */
   /*Kaine*  prepaid_old = so_prepaid:format  */
   /*Kaine*  ext_price_old = ext_price:format.  */
...
no-lock by so_nbr with frame b no-box:
   ...
   no-lock by sod_nbr by sod_line:
      ...
      /*Kaine*  ext_price:format = ext_price_fmt.  */
      {etdcrd.i so_curr 75 91}
      /*Kaine*  {so05b01.i} /*Display Line*/  */
      /*Kaine*/ {xxso05b01.i}

      /*Kaine*  if page-size - line-counter < 1 then do:  */
      /*Kaine*/ if page-size - line-counter < 2 then do:
         page.
         /*Kaine*  {so05e01.i}  */
         /*Kaine*/ {xxso05e01.i}
      end.
      /*Kaine*  put desc1 at 5.  */
      /*Kaine*/ put (desc1 + pt_desc2) FORMAT "x(48)" at 5.
      ...
      if using_cust_consignment then do:
         ...
         if consign_flag then do:
            ...
            put "Contract: Consignment Inventory" at 5.
         end.
      ...
      if using_line_charges and print_line_charges then do:
         ...
         no-lock break by sodlc_order:
            ...
            if sodlc_one_time and sodlc_times_charged > 0 then
               ...
               /*Kaine*  if page-size - line-count < 1 then do:  */
               /*Kaine*/ if page-size - line-count < 2 then do:
                  page.
                  /*Kaine*  {so05e01.i}  */
                  /*Kaine*/ {xxso05e01.i}
               end.
      ...
      *******************************************/
      ...
      then do:
         ...
         {sopiprn1.i &doctype="1" &nbr=sod_nbr &line=sod_line
            ...
            &confg_disc="no"
            &command="~{xxso05e01.i~}"}
              /*Kaine last line so05e01.i --> xxso05e01.i */
