...
define variable iBottomUseLine as integer initial 1 no-undo.
...
REPEAT ON ENDKEY UNDO, LEAVE:
    ...
    :
        ...
        if available(sct_det) then
            assign
                decCost = sct_cst_tot
                decAmount = prh_rcvd * sct_cst_tot
                .
        else
            assign
                decCost = 0
        ...
        if first-of(prh_receiver) and last-of(prh_receiver) then do:
            if line-counter + iHeadUseLine + iLastUseLine + iBottomUseLine > page-size then do:
                {xxgotobottomline.i iBottomUseLine}
                run xxputbottom.
                page.
            end.
            run xxputhead(
        ...
        else if first-of(prh_receiver) then do:
            if line-counter + iHeadUseLine + iCommonUseLine + iBottomUseLine > page-size then do:
                {xxgotobottomline.i iBottomUseLine}
                run xxputbottom.
                page.
            end.
