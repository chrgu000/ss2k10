...
usection = TRIM ( string(year(TODAY)) + string(MONTH(TODAY)) + string(DAY(TODAY)))  + trim(STRING(TIME)) + trim(string(RANDOM(1,100))) + "inv41" .
output to value( trim(usection) + ".i") .
        display 
"R" skip
trim ( V1300) format "x(50)" skip
trim(V1002) + " " + trim ( V1320 ) + " " format "X(50)"  if trim( V1500 ) = "" then " - " else """" + trim(V1500) + """"  format "X(50)" skip
