...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv83wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
If AVAILABLE ( pt_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find first upd_det where upd_nbr = "INV83" and upd_select = 99 no-lock no-error.
If AVAILABLE ( upd_det ) then
   ...
   /* Additional Labels Format */
     ...
     INPUT FROM VALUE(LabelsPath + "inv83" + trim ( wtm_fm ) ).
