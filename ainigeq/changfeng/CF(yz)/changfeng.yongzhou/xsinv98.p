...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[FIFO批号审核查询]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[FIFO批号审核查询]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
...
tr_type = "ISS-WO" and tr_effdate > today - 30 no-lock no-error.
If AVAILABLE ( tr_hist ) then
                L14004 = "料品:" + trim(tr_part) + "/" + string( - tr_qty_loc ) .
                else L14004 = "" . 
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[FIFO批号审核查询]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[FIFO批号审核查询]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1800 no-box.
...
If AVAILABLE ( tr_hist ) then
                ...
                find first tr_hist where tr_trnbr = integer(V1600) no-lock no-error.
If AVAILABLE ( tr_hist ) then
                L18004 = "料品:" + trim(tr_part) + "/" + string( - tr_qty_loc ) .
                else L18004 = "" . 
