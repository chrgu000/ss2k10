...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xswor04wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1103 then
        leave V1103L.
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1103 no-box.
...
If AVAILABLE ( wo_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1104 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1105 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
...
If AVAILABLE ( code_mstr ) then
        leave V1203L.
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1203 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1310 no-box.
...
If AVAILABLE ( code_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1320 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1330 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1335 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
...
If AVAILABLE ( wo_mstr ) then
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
                def var ttqty0 as dec.
 Ttqty0=0.
for each wo_mstr where wo_nbr=V1100 and wo_lot=V1103 no-lock:
   ttqty0=wo_qty_comp.
End.
 
Def var tflag1 as char.
Def var tmppart like pt_part.
Def var ttqty1 as dec.
Tflag1="1". 
Tmppart ="".
Ttqty1=0.
for each wod_det where wod_nbr=V1100 and wod_lot=V1103 and
(wod_bom_qty * (Dec(V1600) + ttqty0) - wod_qty_iss - wod_bom_qty * Dec(V1600) * 0.05 >0) no-lock:
/*
  disp  wod_nbr wod_part   wod_bom_qty * (ttqty + ttqty0) - wod_qty_iss
wod_bom_qty wod_qty_req wod_qty_iss.

*/
/* disp (wod_bom_qty * (ttqty1 + ttqty0) - wod_qty_iss).*/
  tflag1="2".
  Tmppart=wod_part.
  Ttqty1=(wod_bom_qty * (Dec(V1600) + ttqty0) - wod_qty_iss). 
End.

If tflag1="2" then do:
 display skip tmppart + "祇ぃì!,祏" + string(ttqty1) @ WMESSAGE NO-LABEL with fram F1600.
                pause 0 before-hide.
                Undo, retry.
End.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
...
If NOT AVAILABLE ( tr_hist ) then
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[VI自制标签入库]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
   ...
   /* Additional Labels Format */
     ...
     INPUT FROM VALUE(LabelsPath + "wor04" + trim ( wtm_fm ) ).
...
If AVAILABLE ( pt_mstr )  then
       ...
       find first pt_mstr where pt_part = V1104  no-lock no-error.
If AVAILABLE ( pt_mstr )  then
        av9130 = pt_um + ( if V1105 <> "" THEN "    虫:" + trim ( V1105 ) else "" ).
