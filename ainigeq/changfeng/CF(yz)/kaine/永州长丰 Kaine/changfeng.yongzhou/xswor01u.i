...
usection = TRIM ( string(year(TODAY)) + string(MONTH(TODAY)) + string(DAY(TODAY)))  + trim(STRING(TIME)) + trim(string(RANDOM(1,100))) + "wor01" .
output to value( trim(usection) + ".i") .
        display 
"- "  + trim ( V1110 )  + " " + V1203 format "x(50)" skip
trim ( V1600 ) + " - - - - - " + trim ( V1002 ) + " " +  trim( V1400 ) format "X(50)"  if trim( V1500 ) = "" then " - " else """" + trim(V1500) + """"  format "X(50)" skip
trim ( V1005 )  format "X(50)" skip
"Y" skip
"Y" skip
"."
