...
repeat:
    ...
    FOR EACH tt BREAK BY tt_wo_nbr by tt_wo_lot :
        IF FIRST-OF(tt_wo_lot) THEN DO:
            PUT UNFORMATTED "工单: " + tt_wo_nbr + ";" + "标志: " + tt_wo_lot + ";" +  "数量: " + STRING(v_qty) SKIP.
            PUT UNFORMATTED "父件号" ';' '件号' ';' '名称' ';' '需求量' ';' '库存数量' ';' '欠料量' SKIP .
        END.

        PUT UNFORMATTED tt_wo_part ';' tt_wod_part ';' tt_pt_desc1 ';' tt_req_qty ';' tt_qty_oh ';' tt_wr_qty SKIP .
    END.
