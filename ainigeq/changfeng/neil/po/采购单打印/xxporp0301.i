...
{po03b01.i}
/* ss 20061128.1 */
{xxpo03d01.i} /* DEFINE FOR FRAME C */
...
/* ss 20061128.1 - b */
      ...
      with frame c.
/* ss 20061219.1 - b */
      find first vp_mstr where vp_part = pod_part and vp_vend = po_vend no-lock no-error.
      if avail vp_mstr then disp vp_vend_part @ pod__qad11 with frame c.
/* ss 20061219.1 - e */
      if pod_qty_ord = int(pod_qty_ord) then 
         disp pod_qty_ord format "->,>>>,>>9" @ pod_qty_ord with frame c.
      if pt_desc2 <> "" then do:
         down 1 with frame c.
         disp pt_desc2 @ pt_desc1 with frame c . end.
      end.
      else do:
       disp 
         pod_line
