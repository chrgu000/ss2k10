...
/* $Revision: 1.11.2.7 $    BY: Micho Yang     DATE: 03/20/06 ECO: *SS - 20060320*   */
/* By: Neil Gao Date: 20061128.1 ECO: *ss 20061128.1 * */
/* By: Neil GAo Date: 20061219.1 ECO: *ss 20061219.1 * */
...
/******************************************************************************/
/* ss 20061219.1 
删除采购单中打印中的版本,并让备注不留空,而是带上1.19供应商零件维护得vp_vend_part
 */
{mfdeclre.i}
...
DEFINE NEW SHARED VAR vship_phone LIKE ad_phone.
DEFINE NEW SHARED VAR vship_attn  LIKE ad_attn.
...
/* ss 20061128.1 ext_cost_old       = ext_cost:format */
   prepaid_old        = "->,>>>,>>>,>>9.99".
oldcurr = "".
...
for each po_mstr where (po_nbr >= nbr) and (po_nbr <= nbr1)
   ...
   if (oldcurr <> po_curr) or (oldcurr = "") then do:
      ...
      {gprun.i ""gpcurfmt.p"" "(input-output ext_cost_fmt,
                                         input rndmthd)"}
/* ss 20061128.1 ext_cost:format = ext_cost_fmt. */
      {xxpocurfmt.i}
      oldcurr = po_curr.
   end.
   ...
   if available ad_mstr and print_bill then do:
      assign
         ...
         addr[5] = ad_fax 
          .
         IF ad_ext <> "" THEN addr[4] = addr[4] + "-" + ad_ext .
         IF ad_phone2 <> "" THEN addr[4] = addr[4] + "/" + ad_phone2 .
         IF ad_ext2 <> "" THEN addr[4] = addr[4] + "-" + ad_ext2  .

         IF ad_fax2 <> ""  THEN addr[5] = addr[5] + "/" + ad_fax2.
       /*
...
/* ss 20061128.1 - b */
   ...
   assign billto[4] = ad_phone
          billto[5] = ad_fax.
   else assign billto[4] = ""
               billto[5] = "".
...
/* ss 20061128.1 - b */
      ...
      view frame phead1.
      put "电话:" at 1   billto[4]  ad_ext    "传真:" at 50  billto[5] skip.
      put "订单号:" at 1  po_nbr  /* ss 20061219 "版本:" po_rev*/ skip.
      put "订货日期:" at 1 po_ord_date skip.
      put "供应商:" at 1 po_vend  "货物发往:" at 50 po_ship skip.
      put "供应商全称:" at 1 ad_name   "湖南长丰汽车沙发有限公司" at 59 skip.
      put "供应商地址:" at 1 ad_line1  "湖南永州冷水滩工业区"     at 59 skip.
      if ad_line2 <> "" then put ad_line2 at 12 skip.
      if ad_line3 <> "" then put ad_line3 at 12 skip.
      put "联系人:" at 1 ad_attn  "物管员:" at 50 .
      if avail code_mstr then put code_cmmt skip.
      else put po_buyer skip.
      put "电话:" at 1 vend_phone skip.
...
/*ss 20061128.1 - e */
      ...
      PUT "备注:"  AT 1.
/* ss 20061128.1 - b *
      PUT "1. 贵方送达我公司之产品合格数应达我公司订货数,不合格产品由贵方直接负责处理; "  AT 1.
      PUT "2. 如未按时送达和合格产品数量不够等贵方原因造成我公司生产待料和损失的,责任  " AT 1.
      PUT  "   由贵方负担,罚款在货款中扣除;"   AT 1.
      PUT "3. 技术沟通请与我公司技术部相关人员联系：13787645883 或 0746-8456019-3304;  "  AT 1.
      PUT "4. 所有货物应有批次及供应商标示,按我公司要求包装,到货时须包装完好;          "           AT 1.
      PUT  "5. 请随货附采购订单号,货物清单,材质报告和质量检验报告,否则不予报检入库.     "  AT 1 SKIP(1).
      PUT "______________________________" TO 70.
      PUT "            授权签字(章)      " TO 70.
 * ss 20061128.1 - e */
/* ss 20061128.1 - b */
      put "1、请随货附采购订单号、货物清单、材质报告和质量检验报告，否则不予报检入库." at 1.
      put "2、请按照实际生产计划需求提前送货." at 1.
      if page-size - line-counter < 10 then do:
      ...
      put "--------------"  at 60.
      put "  授权签字（章）" at 60.
/*    do while page-size  > line-counter :
         put skip(1).
      end.
      put "结  束" at 35 . */
