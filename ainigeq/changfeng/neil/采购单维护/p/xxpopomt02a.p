...
/* $Revision: 1.72.1.3 $ BY: Gaurav Kerkar        DATE: 09/21/04  ECO: *P2L7*  */
...
define variable l_old_db         as   character                     no-undo.
...
*/
                  pod_need     = w_pod_need
                  pod_due_date = w_pod_need
                  pod_per_date = w_pod_due_date
/*SS 20080630 - E*/
                  pod_um       = w_pod_um
...
*/
                  pod_so_job   = w_pod_project
                  pod_sod_line  = w_pod_req_line
/*SS 20080630 - E*/
                  pod_acct     = w_pod_acct
                  pod_sub      = w_pod_sub
                  pod_cc       = w_pod_cc
            ...
            /* SWITCH TO THE REQUISITION DATABASE */
            l_old_db = global_db.
            if siteDB <> global_db
            then do:
               {gprun.i ""gpalias3.p"" "(siteDB, output l_err_flag)"}
            ...
            /*SWITCH BACK TO CENTRAL PO DATABASE IF REQUIRED */
            if l_old_db <> global_db
            then do:
               {gprun.i ""gpalias3.p"" "(l_old_db, output l_err_flag)"}
