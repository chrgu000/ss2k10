...
view frame dtitle .
  ...
  {mfselprt.i "printer" 132}
	...
	for each wo_mstr where wo_status = "R" and wo_due_date >= duedate
		and wo_due_date <= duedate1 and wo_lot >= lot and wo_lot <= lot1 no-lock,
		each wod_det where wod_nbr = wo_nbr and wod_lot = wo_lot 
			and (wod_loc = loc or loc = "") 
			and wod_part >= part and wod_part <= part1
			and (wo_qty_comp * wod_bom_qty > wod_qty_iss or not open_only )no-lock,
		each pt_mstr where pt_part = wod_part and pt_iss_pol no-lock
		...
		/*
		xxqty = 0 .
		for each ld_det where ld_part = wod_part and ld_status begins "Y" no-lock:
			xxqty = xxqty + ld_qty_oh.
		end.*/
		
		xxqty = wo_qty_comp * wod_bom_qty - wod_qty_iss.
		
		disp 	wo_due_date wo_lot wod_part pt_desc1 wo_qty_comp wo_qty_comp * wod_bom_qty @ wod_qty_req
					wod_qty_iss xxqty label "短缺量"
		with frame c.
		down with frame c.
