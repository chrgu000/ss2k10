/* xxicintr37.p - COMMON PROGRAM FOR MISC INVENTORY TRANSACTIONS             */
...
/* $Revision: 1.40.1.4 $   BY: Paul Dreslinski   DATE: 09/10/04  ECO: *M1LL* */
/*-Revision end--------------------------------------------------------------*/
...
repeat:
   ...
   if c-application-mode = "API" then
       find pt_mstr where pt_part = ttInventoryTrans.part
        no-lock no-error.
   else
...
/* SS - 100625.1 - B */
         if can-find (first code_mstr where code_fldname = "ISS-UNP-TYPE")
            and not can-find (code_mstr where code_fldname = "ISS-UNP-TYPE"
                          and code_value = rmks )
         then do:
