...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv03wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[庫位庫存查詢]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[庫位庫存查詢]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
        ...
        IF NOT AVAILABLE PT_MSTR then do:
                display skip "錯誤,重試!" @ WMESSAGE NO-LABEL with fram F1300.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[庫位庫存查詢]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
...
If AVAILABLE ( in_mstr ) then
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LOC_MSTR where LOC_LOC = V1500 AND LOC_SITE = V1002  no-lock no-error.
     ...
     PV1500 = V1500.
     /* END    LINE :1500  庫位[LOC]  */


   /* Additional Labels Format */
     /* START  LINE :1510  明細資料[Detail]  */
     V1510L:
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[庫位庫存查詢]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1510 no-box.
                ...
                /* LABEL 2 - START */ 
                find first ld_det where ld_part = V1300 and ld_qty_oh <> 0  and ld_loc = V1500 and ld_site = V1002 no-lock no-error.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1510.
            ...
            THEN DO:
                  IF recid(LD_DET) = ? THEN find first LD_DET where 
                              LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_LOC = V1500 AND LD_SITE = V1002 AND  
                              LD_LOT >=  INPUT V1510
                               no-lock no-error.
                  ELSE find next LD_DET where 
                              LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_LOC = V1500 AND LD_SITE = V1002  
                               no-lock no-error.
