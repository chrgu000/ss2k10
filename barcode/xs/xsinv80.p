...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv80wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first SO_MSTR where SO_NBR = V1100 AND (SO_SITE = "" OR SO_SITE = V1002 )  no-lock no-error.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find last so_mstr where so_nbr = V1100  no-lock no-error.
If AVAILABLE (so_mstr) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
...
If AVAILABLE (sod_det) and string ( sod_line ) = V1205 then
        leave V1205L.
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1205 no-box.
        ...
        IF NOT AVAILABLE SOD_DET then do:
                display skip "赣兜ΩΤ粇!" @ WMESSAGE NO-LABEL with fram F1205.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find first sod_det where sod_nbr = V1100  and sod_site = V1002 and string ( sod_line ) = V1205 no-lock no-error.
If AVAILABLE (sod_det) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE (pt_mstr) then
                ...
                /* LABEL 3 - START */ 
                find first cp_mstr where cp_cust = V1110 and cp_part = V1300 no-lock no-error.
If AVAILABLE (cp_mstr) then
                L13003 = ":" + Trim(cp_cust_part) .
                else L13003 = "" . 
...
If AVAILABLE (cp_mstr) then
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first SOD_DET where SOD_LINE = integer( V1205 ) and SOD_NBR = V1100 and sod_site = V1002 and SOD_PART = ENTRY(1, V1300, "@")  no-lock no-error.
        IF NOT AVAILABLE SOD_DET then do:
                display skip "珇ぃ癸莱" @ WMESSAGE NO-LABEL with fram F1300.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
...
If AVAILABLE (cp_mstr) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
        ...
        IF not INDEX("AME",V9010) <> 0 THEN DO:
                display skip "块A M E" @ WMESSAGE NO-LABEL with fram F9010.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   END.
   pause 0 before-hide.
     /* START  LINE :9109  输出到打印机的条码上数量  */
     V9109L:
     REPEAT:

        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V9109           as char format "x(50)".
        define variable PV9109          as char format "x(50)".
        ...
        leave V9109L.
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9109 no-box.

                /* LABEL 1 - START */ 
                  L91091 = "" . 
                display L91091          format "x(40)" skip with fram F9109 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                  L91092 = "" . 
                display L91092          format "x(40)" skip with fram F9109 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                  L91093 = "" . 
                display L91093          format "x(40)" skip with fram F9109 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                  L91094 = "" . 
                display L91094          format "x(40)" skip with fram F9109 no-box.
                /* LABEL 4 - END */ 
                display "块┪E癶 "      format "x(40)" skip
        skip with fram F9109 no-box.
        Update V9109
   ...
   IF NOT (V9010 = "A" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.

                /* LABEL 1 - START */ 
                find first sod_det where sod_nbr = V1100 and string ( sod_line ) = V1205  no-lock no-error.
If AVAILABLE ( pt_mstr ) then
                L91101 = sod_um + " 夹帽计秖[笆]?" .
                else L91101 = "" . 
...
If AVAILABLE (so_mstr) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V9120L.
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
...
IF 1 <> 1 THEN
        leave V9130L.
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
   ...
   /* Additional Labels Format */
     ...
     INPUT FROM VALUE(LabelsPath + "inv80" + trim ( wtm_fm ) ).
...
If AVAILABLE ( pt_mstr )  then
       ...
       END.
       find first ad_mstr where ad_addr = V1110  no-lock no-error.
...
If AVAILABLE ( so_mstr )  then
       ...
       END.
       find first cp_mstr where cp_cust = V1110 and cp_cust_part <> "" and cp_part = V1300  no-lock no-error.
...
If AVAILABLE ( pt_mstr )  then
   ...
   IF NOT (V9010 = "A" AND ( decimal ( V9110 ) * decimal ( V9120 )  <> decimal ( V1600 ) ) ) THEN LEAVE V9140LMAINLOOP.
     ...
     REPEAT:
        ...
        leave V9140L.
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9140 no-box.
...
If AVAILABLE (so_mstr) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V9150L.
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9150 no-box.
...
IF 1 <> 1 THEN
        leave V9160L.
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9160 no-box.
        ...
        IF NOT AVAILABLE PRD_DET then do:
                display skip "ゴ诀Τ粇 " @ WMESSAGE NO-LABEL with fram F9160.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
...
If AVAILABLE ( pt_mstr )  then
   ...
   IF NOT (V9010 = "M" ) THEN LEAVE V9170LMAINLOOP.
     ...
     REPEAT:
        ...
        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V9170           as char format "x(50)".
        define variable PV9170          as char format "x(50)".
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9170 no-box.

                /* LABEL 1 - START */ 
                find first sod_det where sod_nbr = V1100 and string ( sod_line ) = V1205  no-lock no-error.
If AVAILABLE ( pt_mstr ) then
                L91701 = sod_um + " 夹帽计秖[も]?" .
                else L91701 = "" . 
                display L91701          format "x(40)" skip with fram F9170 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                L91702 = "MFG腹:" + trim ( V1300 ) .
                display L91702          format "x(40)" skip with fram F9170 no-box.
                /* LABEL 2 - END */ 
                ...
                find first cp_mstr where cp_cust = V1110 and cp_part = V1300 no-lock no-error.
If AVAILABLE (cp_mstr) then
                L91703 = ":" + Trim(cp_cust_part) .
                else L91703 = "" . 
                display L91703          format "x(40)" skip with fram F9170 no-box.
                /* LABEL 3 - END */ 
...
If AVAILABLE (so_mstr) then
                ...
                display L91704          format "x(40)" skip with fram F9170 no-box.
                /* LABEL 4 - END */ 
                display "块┪E癶 "      format "x(40)" skip
        skip with fram F9170 no-box.
        Update V9170
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V9180           as char format "x(50)".
        define variable PV9180          as char format "x(50)".
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9180 no-box.

                /* LABEL 1 - START */ 
                L91801 = "夹帽计?" .
                display L91801          format "x(40)" skip with fram F9180 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                L91802 = "珇:" + trim( V1300 ) .
                display L91802          format "x(40)" skip with fram F9180 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                L91803 = "у腹:" + Trim(V1500) .
                display L91803          format "x(40)" skip with fram F9180 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                  L91804 = "" . 
                display L91804          format "x(40)" skip with fram F9180 no-box.
                /* LABEL 4 - END */ 
                display "块┪E癶 "      format "x(40)" skip
        skip with fram F9180 no-box.
        Update V9180
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V9190           as char format "x(50)".
        define variable PV9190          as char format "x(50)".
...
IF 1 <> 1 THEN
        leave V9190L.
        /* LOGICAL SKIP END */
                display "[綪扳夹帽干ゴ]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9190 no-box.

                /* LABEL 1 - START */ 
                L91901 = "ゴ诀?" .
                display L91901          format "x(40)" skip with fram F9190 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                  L91902 = "" . 
                display L91902          format "x(40)" skip with fram F9190 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                  L91903 = "" . 
                display L91903          format "x(40)" skip with fram F9190 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                  L91904 = "" . 
                display L91904          format "x(40)" skip with fram F9190 no-box.
                /* LABEL 4 - END */ 
                display "块┪E癶 "      format "x(40)" skip
        skip with fram F9190 no-box.
        Update V9190
        ...
        IF NOT AVAILABLE PRD_DET then do:
                display skip "ゴ诀Τ粇 " @ WMESSAGE NO-LABEL with fram F9190.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
