...
/* BARCODE SYSTEM */
/* Generate date / time  2006-6-16 15:53:39 */
...
IF OkToRun = yes THEN  RUN    xsinv99.p.
   ...
   IF NOT (V1170 = "71" OR V1100 = "71" ) THEN LEAVE V1171LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        V1170 = "".
RUN CheckSecurity (INPUT "xsinv80.p" , INPUT global_userid , OUTPUT okToRun , OUTPUT Execname ).
IF OkToRun = yes THEN  RUN    xsinv80.p.
        leave V1171L.
        /* LOGICAL SKIP END */
                display "#兵絏# *" + ( if length(DBNAME) < 5 then trim( DBNAME ) else trim(substring(DBNAME,length(DBNAME) - 4,5)) )
