...
psid = trim ( userid(sdbname('qaddb')) ).
    ...
    if available usrw_wkfl then do:
	  ...
	  display skip (16) "** 條碼軟件注冊 **"				at 48 no-label skip
			    "公司:" + temp1 	    format "x(32)"   		at 48 no-label skip
			    "系統:" + trim ( opsys )  format "x(28)"      	at 48 no-label skip
			    "類型:" + trim ( dbtype(1) ) + trim ( dbversion(1) )  format "x(28)" at 48 no-label skip
    ...
    */
     ...
     */
     if index(summaryusr,psid) = 0 then do:
        display skip(16)
	             "**BARCODE 用戶有誤**" at 60 no-label skip
	             " * 請輸入用戶資料* "  at 60 no-label skip
		     "通用代碼:(.36.2.13)"  at 60 no-label skip
	...
	quit.
     ...
     for last usr_mstr where usr_userid <> ""  and usr_lang = "us" no-lock :
         psid = usr_userid.
     end.


     for last mon_mstr  no-lock , each usr_mstr where  mon_userid = usr_userid and usr_lang = "us" no-lock by mon_sid:
         psid = mon_userid.
     end.
