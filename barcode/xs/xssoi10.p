...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xssoi10wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
...
If AVAILABLE ( so_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1105 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find first sod_det where sod_nbr = V1100 and sod_site = V1002 and sod_confirm no-lock no-error.
If AVAILABLE (sod_det) and string ( sod_line ) <> V1110 then
        ...
        /* LOGICAL SKIP START */
        find last sod_det where sod_nbr = V1100 and sod_site = V1002 and sod_confirm no-lock no-error.
If AVAILABLE (sod_det) and string ( sod_line ) = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1110.
            ...
            THEN DO:
                  IF recid(SOD_DET) = ? THEN find first SOD_DET where 
                              sod_nbr = V1100 and sod_site = V1002 and sod_confirm AND  
                              string ( SOD_LINE ) >=  INPUT V1110
                               no-lock no-error.
                  ELSE find next SOD_DET where 
                              sod_nbr = V1100 and sod_site = V1002 and sod_confirm  
                               no-lock no-error.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first SOD_DET where SOD_NBR = V1100 AND string ( sod_line ) = V1110 and SOD_SITE = V1002 and sod_confirm  NO-ERROR NO-WAIT.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.

                /* LABEL 1 - START */ 
                find first sod_det where sod_nbr = V1100 and sod_line = integer(V1110)  no-lock no-error.
If AVAILABLE ( sod_det ) then
                L13001 = "成品:" + trim(sod_part) .
                else L13001 = "" . 
...
If AVAILABLE ( so_mstr ) then
        ...
        /* PRESS e EXIST CYCLE */
        IF V1300 = "e" THEN  LEAVE V1110LMAINLOOP.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first SOD_DET where SOD_NBR = V1100 AND SOD_LINE = integer ( V1110 ) AND SOD_PART = ENTRY(1, V1300, "@")  no-lock no-error.
        IF NOT AVAILABLE SOD_DET then do:
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* PRESS e EXIST CYCLE */
        IF V1400 = "e" THEN  LEAVE V1110LMAINLOOP.
        display  skip WMESSAGE NO-LABEL with fram F1400.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
        ...
        /* PRESS e EXIST CYCLE */
        IF V1410 = "e" THEN  LEAVE V1110LMAINLOOP.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
...
If AVAILABLE ( ld_det ) then
        ...
        /* PRESS e EXIST CYCLE */
        IF V1500 = "e" THEN  LEAVE V1110LMAINLOOP.
        display  skip WMESSAGE NO-LABEL with fram F1500.
...
If AVAILABLE ( ld_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1550 no-box.
        ...
        /* PRESS e EXIST CYCLE */
        IF V1550 = "e" THEN  LEAVE V1110LMAINLOOP.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
...
If AVAILABLE ( ld_det ) then
                L16004 = "庫存:" + trim(V1400) + "/" + string ( ld_qty_oh ) .
                else L16004 = "" . 
        ...
        /* PRESS e EXIST CYCLE */
        IF V1600 = "e" THEN  LEAVE V1110LMAINLOOP.
...
ld_site = V1002 and ld_ref = "" and  ld_lot = V1500 and  ld_QTY_oh >= DECIMAL ( V1600 )  no-lock no-error.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
...
If AVAILABLE ( sod_det ) then
        ...
        /* PRESS e EXIST CYCLE */
        IF V1700 = "e" THEN  LEAVE V1110LMAINLOOP.
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
        ...
        /* PRESS e EXIST CYCLE */
        IF V9000 = "e" THEN  LEAVE V1110LMAINLOOP.
...
tr_nbr  = V1100     and  tr_type = "ISS-SO"  and
...
If 1 = 1 then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9005 no-box.
        ...
        /* PRESS e EXIST CYCLE */
        IF V9005 = "e" THEN  LEAVE V1110LMAINLOOP.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
...
tr_date = today     and 
tr_trnbr > integer ( V9000 ) and 
tr_nbr  = V1100     and  tr_type = "ISS-SO"  and  tr_site = V1002     and tr_part = V1300     and tr_serial = V1500   and 
tr_time  + 15 >= TIME 
use-index tr_date_trn no-lock no-error.
If AVAILABLE ( tr_hist ) then
...
If NOT AVAILABLE ( tr_hist ) then
        ...
        /* PRESS e EXIST CYCLE */
        IF V9010 = "e" THEN  LEAVE V1110LMAINLOOP.
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find first upd_det where upd_nbr = "SOI10" and upd_select = 99 no-lock no-error.
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售訂單發貨]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
   ...
   /* Additional Labels Format */
     ...
     INPUT FROM VALUE(LabelsPath + "soi10" + trim ( wtm_fm ) ).
...
If AVAILABLE ( pt_mstr )  then
       ...
       END.
       find first cp_mstr where cp_cust = V1105 and cp_cust_part <> "" and cp_part = V1300 no-lock no-error.
