...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xswoi09wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
                ...
                /* LABEL 2 - START */ 
                find first wo_mstr where wo_nbr  = V1100 and wo_site = V1002 and      no-lock no-error.
If AVAILABLE ( wo_mstr ) then
                L11102 = wo_lot .
                else L11102 = "" . 
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE (wod_det) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
...
ld_lot = V1500  and ld_qty_oh <> 0  and ld_ref = ""  no-lock no-error.
        IF NOT AVAILABLE LD_DET then do:
                display skip "批號有誤" @ WMESSAGE NO-LABEL with fram F1500.
                pause 0 before-hide.
                undo, retry.
        end.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
...
ld_site = V1002 AND ld_ref = "" AND  ld_loc = V1400 and ld_lot = V1500 and  ld_QTY_oh >= DECIMAL ( V1600 )  no-lock no-error.
        IF NOT AVAILABLE LD_DET then do:
                display skip "在庫數 <: " + string( V1600 ) + "不可超量" @ WMESSAGE NO-LABEL with fram F1600.
                pause 0 before-hide.
                undo, retry.
        end.
        IF not CAN-FIND(FIRST wod_det where wod_lot = V1110 and wod_part = V1300 and wod_qty_req - wod_qty_iss >= DECIMAL ( V1600 ) ) THEN DO:
                display skip "在庫數 <: " + string( V1600 ) + "不可超量" @ WMESSAGE NO-LABEL with fram F1600.
                pause 0 before-hide.
                undo, retry.
        end.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
                ...
                find first wod_det where wod_lot = V1110 and wod_part = V1300  no-lock no-error.
If AVAILABLE ( wod_det ) then
                L17004 = "工單短缺數量:" + ( if wod_qty_req <= wod_qty_iss then "0" else ( string ( wod_qty_req - wod_qty_iss ) ) ) .
                else L17004 = "" . 
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
...
If NOT AVAILABLE ( tr_hist ) then
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單限量發料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
   ...
   /* Additional Labels Format */
     ...
     INPUT FROM VALUE(LabelsPath + "woi09" + trim ( wtm_fm ) ).
