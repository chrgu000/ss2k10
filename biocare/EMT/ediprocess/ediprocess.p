...
/* *SS-20130107.1* */  define variable v_sonbr  like so_nbr.
...
*/
	...
	for each edi_hist where  edi_targetdomain = global_domain and edi_type = "ORD-PO" and edi_sucess  = ""  by edi_group :
		...
		*/
		...
		/* SS - 121211.1 - E */
/* *SS-20130107.1* -b */ 
		find first po_mstr where po_domain = edi_targetdomain  
		               and po_nbr = edi_nbr no-lock no-error.
		if AVAILABLE(po_mstr) and po_so_nbr <> "" then
                   v_sonbr = soprefix +  substring(po_so_nbr,3,6). 
		else 
		   v_sonbr = soprefix +  substring(edi_nbr,3,6). 
...
/* *SS-20130107.1* -e*/
		...
		find first so_mstr where so_domain = global_domain and so_nbr = /* *SS-20130107.1* -b */ v_sonbr /* soprefix +  substring(edi_nbr,3,6) */  no-lock no-error .
		...
		find first sod_det where sod_domain = global_domain and sod_nbr =  /* *SS-20130107.1* -b */ v_sonbr /* soprefix +  substring(edi_nbr,3,6) */
		...
		put unformat   /* *SS-20130107.1* -b */ v_sonbr /* soprefix +  substring(edi_nbr,3,6) */   format "x(8)" skip .  /* Sales Order */
