...
repeat:
   ...
   if c-application-mode <> 'web' then
      update nbr nbr1 addr addr1  with frame a.

   {wbrp06.i &command = update &fields = "  nbr nbr1 addr addr1 " &frm = "a"}
   ...
   for each edi_hist no-lock
	    ...
	    and ( edi_type = "ISS-SO" OR  edi_type = "RCT-PO"  )
   ...
   with frame b width 400 no-attr-space:
     ...
     if wedigroup <> edigroup then down 1.
	  ...
	  edi_TransLoc     column-label "!库位"
          edi_Translot     column-label "!批号"      
          edi_Transref     column-label "!参考号"
	  edi_TransQty     column-label "!数量"
          edi_targetdomain column-label "!目标域"
