...
/* $Revision: 1.20 $ BY: Bill Jiang DATE: 08/15/07 ECO: *SS - 20070815.1* */
/* $Revision: 1.20 $ BY: Bill Jiang DATE: 02/18/08 ECO: *SS - 20080218.1* */
/* SS - 120620.1 By: Randy Li */
...
【 120620.1 】
1.更改为EXCEREPORT输出方式。
2.更改显示会计单位名称为ad_name + ad_line1
【 120620.1 】
SS - 120620.1 - RNE */

/* SS - 121224.1 RNB
【 121224.1 】
1.更改会计单位名称为ad_mstr中的名称。
【 121224.1 】
SS - 121224.1 - RNE */
/*-Revision end---------------------------------------------------------------*/
...
/* ********** End Translatable Strings Definitions ********* */
         ...
         define new shared variable cname like glname format "x(35)" no-undo.
...
/*J241*/          for first glc_cal fields( glc_domain glc_end glc_per
glc_start glc_year)
/*J241*/           where glc_cal.glc_domain = global_domain and  glc_year =
fiscal_yr[i] and
...
/*J241*/          if available glc_cal then assign
                  ...
                  if budgflag[i] = yes then do:
                     ...
                     global_domain and  bg_code = budgetcode[i])
                     then do:
...
/*N0QH*/    if budgflag[2] then assign
            ...
            */
            ...
            /* SS - 20090224.1 - B */
            rptdt[2] = date(12,31,year(rptdt [1]) - 1)  .
            budgflag[2] = budgflag[1] .
            budgetcode[2] = budgetcode[1].
            /* SS - 20090224.1 - E */
			/* SS - 120620.1 - B */
			/*
			...
			*/
			   PUT UNFORMATTED "ExcelFile;ssglbsr1" SKIP.
				PUT UNFORMATTED "SaveFile;ssglbsr1" + "-" + string(today, "99999999") + "-" + replace(string(time, "HH:MM:SS"), ":", "") SKIP.
				/*
				PUT UNFORMATTED "exclemacro;excelformat" SKIP.
				*/
				PUT UNFORMATTED "BeginRow;2" SKIP.
			/* SS - 120620.1 - E */
			
            EMPTY TEMP-TABLE ttssglcbsrp01.
            ...
            FOR EACH ttssglcbsrp01
               ,EACH ac_mstr NO-LOCK
               WHERE ac_domain = GLOBAL_domain
               AND ac_code = ttssglcbsrp01_ac_code
               ...
               /*
                EXPORT DELIMITER ";" 
                   ...
                   ac_code
                   ac_desc
                   ttssglcbsrp01_et_balance[1]
                   ttssglcbsrp01_et_balance[2]
                   .
                */
               IF AVAILABLE mfc_ctrl THEN DO:
                  EXPORT DELIMITER ";" 
                     cname
                     ssDate(rptdt[1],mfc_char)
                     ssDate(rptdt[2],mfc_char)
                     fm_fpos
                     fm_desc
                     ac_code
                     ac_desc                    
...
/*J241*/    fields( en_domain en_name en_entity)
/*J241*/    no-lock  where en_mstr.en_domain = global_domain and  en_entity =
current_entity: end.
            if not available en_mstr then do:
               ...
               leave.
				...
				/* SS - 121224.1 - B */
				find first ad_mstr where ad_domain = global_domain and ad_type = "company" and ad_addr = en_entity no-lock no-error.
				if avail ad_mstr then glname = trim(ad_name) + trim(ad_line1) . else glname = en_name.
...
/*J241*/    fields( co_domain co_enty_bal co_pl co_ret co_use_cc co_use_sub)
...
/*J241*/    fields( ac_domain ac_code ac_fpos ac_desc ac_type)
/*J241*/    no-lock  where ac_mstr.ac_domain = global_domain and  ac_code=pl:
end.
/*J241*/    if not available ac_mstr then do:
               ...
               if not can-find(first asc_mstr  where asc_mstr.asc_domain =
               global_domain and  asc_acc = pl) then do:
...
/*J241*/          if can-find ( first glt_det  where glt_det.glt_domain =
global_domain and
            ...
            do i = 1 to 2:
               if not budgflag[i] then do:
                  ...
                  {glplbg.i &yr=fiscal_yr[i] &plamt=pl_amt[i] &per1=per_end[i]
                            &bcode=budgetcode[i]}
                  for each bg_mstr
/*J241*/          fields( bg_domain bg_acc bg_code bg_fpos)
...
/*J241*/             for first fm_mstr fields( fm_domain fm_fpos fm_type)
/*J241*/              where fm_mstr.fm_domain = global_domain and  fm_fpos =
bg_fpos no-lock: end.
