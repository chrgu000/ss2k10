...
/* $Revision: 1.20 $ BY: Bill Jiang DATE: 02/24/09 ECO: *SS - 20090224.1* */
/* SS - 121225.1 By: Randy Li */
...
if not available en_mstr then do:
   ...
   leave.
	...
	if avail ad_mstr then glname = trim(ad_name) + trim(ad_line1) . else 
	/* SS - 121225.1 - E */ 
   glname = en_name. 
   release en_mstr.
end.
...
*/
   PUT UNFORMATTED "ExcelFile;ssglinr1" SKIP.
	PUT UNFORMATTED "SaveFile;ssglinr1" + "-" + string(today, "99999999") + "-" + replace(string(time, "HH:MM:SS"), ":", "") SKIP.
	/*
