...
/* Revision: 1.71.1.24  BY: Sarita Gonsalves   DATE: 07/12/06 ECO: *P4X7* */
/* Revision: 1.71.1.25  BY: Ashwini G.         DATE: 08/31/06 ECO: *P544* */
/* Revision: 1.71.1.26  BY: Dilip Manawat      DATE: 04/09/07 ECO: *P5SS* */
/* Revision: 1.71.1.26.1.1  BY: Dilip Manawat  DATE: 08/29/07 ECO: *P660* */
/* $Revision: 1.71.1.26.1.2 $ BY: Amandeep Saini  DATE: 10/25/07 ECO: *P6BX* */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
...
define new shared temp-table tt_cm_mstr
   fields t_cm_recid    as   recid
   fields t_cm_balance  like cm_balance
   fields t_cm_upd_date as   logical
   fields t_cm_pay_date as   date.
      ...
      do transaction:
         ...
         do while true:
                     ...
                     if new ar_mstr then
                        ...
                        do on error undo, retry:
                           ...
                           if auto_apply then do:
                              ...
                              /* ALSO ar_applied and ar_base_applied */
                              /* ARE RESET TO ZERO                  */
            ...
            for first cm_mstr
               ...
               and   cm_mstr.cm_addr   = ar_mstr.ar_bill
               no-lock:
            end.
            for first tt_cm_mstr
               where t_cm_recid = recid(cm_mstr):
            end.
            if not available tt_cm_mstr
            then do:
               create tt_cm_mstr.
               t_cm_recid = recid(cm_mstr).
            end. /*IF NOT AVAILABLE tt_cm_mstr*/
            ...
            then do:
               ...
               assign
                  t_cm_balance = t_cm_balance + cust_bal
                  cust_bal   = 0.
...
/* PROCEDURE TO UPDATE cm_mstr FROM TEMP-TABLE tt_cm_mstr*/
PROCEDURE update_cm_mstr:
   for each tt_cm_mstr:
      for first cm_mstr
         where recid(cm_mstr) = t_cm_recid
         exclusive-lock:
      end.
      if available cm_mstr
      then do:
         cm_balance = cm_balance + t_cm_balance.

         if t_cm_upd_date
         then
            cm_pay_date = t_cm_pay_date.

         if recid(cm_mstr) = -1 then .
         release cm_mstr.
      end.

      delete tt_cm_mstr.
   end. /*FOR EACH tt_cm_mstr*/
END PROCEDURE. /*update_cm_mstr*/
