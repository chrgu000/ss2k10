     ...
     .
		...
		/* SS - YYMMDD.1 - B */
                find code_mstr  where code_domain = global_domain
				    and code_fldname = "PO-Jurisdiction" 
					and code_value = global_userid NO-LOCK no-error.
		    if available(code_mstr) and lookup(po_user_id, code_cmmt) = 0 then do:
       		   /*用户无写入权限*/
			   {pxmsg.i &MSGNUM=4401 &ERRORLEVEL=3}
        		next-prompt po_nbr with frame a.
        		undo mainloop, retry .
             end. 
			if not available code_mstr and po_user_id <> global_userid  then do:
       		   /*用户无写入权限*/
			   {pxmsg.i &MSGNUM=4401 &ERRORLEVEL=3}
        		next-prompt po_nbr with frame a.
        		undo mainloop , retry .
