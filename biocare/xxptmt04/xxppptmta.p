...
/* $Revision: 1.61.2.4 $ BY: Kumar Desai   DATE: 10/19/07   ECO: *Q1DC*     */
...
/*V8!
on    'entry':U of pt_desc1 in frame a
   or 'entry':U of pt_desc2 in frame a
do:
   if last-event:event-type <> "MOUSE"
   then
      self:set-selection(1,25).
end. /* ON 'ENTRY' OF pt_desc1... */
*/
         ...
         {pxmsg.i &MSGNUM     = 1021
           ...
           if  ppform = ""
			...
			/* SS - YYMMDD.1 - B */
						...
						{gprun.i ""xxgpcmmt.p"" "(input pt_part,input 'CH',input 'PT')"}
/*						
						find first cd_det where cd_det.cd_domain = global_domain and cd_det.cd_ref = pt_part 
							and cd_det.cd_lang = "CH" and cd_det.cd_type = "SC" no-lock no-error.
						if avail cd_det then do:
							{gprunp.i "xxproced" "p" "getstring" 
									"( input cd_det.cd_cmmt[1],
										 input '24',
									   output cmmt1 )"}
							pt_desc1 = entry(1,cmmt1,"^") no-error.
							pt_desc2 = entry(2,cmmt1,"^") no-error.
						end.
*/						
						view frame a .
						view frame a1.
						disp pt_desc1 pt_desc2 						
						with frame a.
