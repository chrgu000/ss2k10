...
/* Revision: 1.19.1.31.1.3  BY: Preeti Sattur     DATE: 10/30/06  ECO: *P5CN* */
...
/* SS - 130626.1 RNB
【 130627.1 】
1.货运单出货的ENTITY与SOD的ENTITY不符不允许继续(用site确认)
【 130627.1 】
SS - 130627.1 - RNE */
...
repeat with frame a:
   ...
   if not available si_mstr then do:
      ...
      run ip_msg (input 708 , input 3, input '').
   ...
   if return_int = 0 then do:
      /* USER DOES NOT HAVE */
      run ip_msg (input 725 , input 3, input '').
      /* ACCESS TO THIS SITE*/
      next-prompt abs_shipfrom with frame a.
      undo MAINLOOP, retry MAINLOOP.
   end.
   ...
   do transaction:
      ...
      if available abs_mstr and
         ...
         run ip_msg (input 5814 , input 3, input '').
      ...
      then do:
         ...
         if({gpiswrap.i}) and (using_container_charges or using_line_charges)
         then do:
      ...
      if not available abs_mstr then do:
         ...
         if not authorized then do:
            ...
            MOVEMENT CODE */
            run ip_msg (input 5990 , input 4, input '').
            undo mainloop, retry mainloop.
         end.
         ...
         if errorst then do:
            run ip_msg (input errornum , input 3, input '').
            next-prompt abs_id with frame a.
            undo mainloop, retry mainloop.
         end.
         ...
         /* ADDING NEW RECORD */
         run ip_msg (input 1 , input 1, input '').
      ...
      then do:
         ...
         run ip_msg (input 5967 , input 2, input '').
      end.
      ...
      then do:
         ...
         run ip_msg (input 5965 , input 2, input '').
      end.
      ...
      if v_confirmed then do:
         ...
         run ip_msg (input 8146 , input 2, input '').
      ...
      then do:
         ...
         run ip_msg (input 8103 , input 1, input '').
      end.
   ...
   do transaction:
      ...
      repeat:
         ...
         if not available df_mstr and abs_format <> "" then do:
            run ip_msg (input 5900 , input 3, input '').
         ...
         if available df_mstr and df_inv = true and v_so_ship then do:
            ...
            if abs_id begins "s" then do:
               ...
               if length(abs_id) > 9 then do:
                  ...
                  run ip_msg (input 5982 , input 3, input '').
                  next-prompt abs_format with frame c.
                  undo merge_loop, retry merge_loop.
            ...
            run chk_abs_inv_cons(input abs_id,
                                 input abs_shipfrom,
                                 input "",
                                 input "",
                                 input "",
         ...
         if old_abs_format <> abs_format and
            ...
            run ip_msg (input 8124 , input 2, input '').
         end.
         ...
         if not multi_carrier then do:
            ...
            if carrier <> "" then do:
               ...
               then do:
                  ...
                  run ip_msg (input 980 , input 3, input '').
               ...
               then do:
                  ...
                  run ip_msg (input 5905 , input 3, input '').
                  next-prompt carrier with frame c.
                  undo merge_loop, retry merge_loop.
         ...
         if abs_lang <> ad_lang then do:
            ...
            then do:
               run ip_msg (input 5050 , input 3, input '').
               /* LANGUAGE MUST EXIST */
         ...
         if abs_format = "" then do:
            run ip_msg (input 5817 , input 2, input '').
...
PROCEDURE get_lngd:
   ...
   if not available lngd_det then do:
      /* Invalid Option */
      run ip_msg (input 712 , input 3, input '').
   end.
   else
...
PROCEDURE ip_msg:
   ...
   define input parameter i_msg  as character no-undo.
   {pxmsg.i &MSGNUM=i_num
            &ERRORLEVEL=i_stat
            &MSGARG1=i_msg}
END PROCEDURE.
