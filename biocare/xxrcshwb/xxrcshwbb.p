...
/* Revision: 1.127.1.8  BY: Ashwini G.             DATE: 07/11/06 ECO: *P40K* */
/* Revision: 1.127.1.9  BY: Munira Savai           DATE: 10/27/06 ECO: *Q0ZH* */
/* Revision: 1.127.1.12 BY: Tejasvi Kulkarni       DATE: 12/27/06 ECO: *P5HV* */
/* Revision: 1.127.1.15 BY: Masroor Alam           DATE: 02/10/07 ECO: *P5Q4* */
/* Revision: 1.127.1.17 BY: Ambrose Almeida        DATE: 07/24/07 ECO: *P62D* */
...
define variable l_cons_req like mfc_logical no-undo.
define variable l_recid1   as recid no-undo.
define variable l_recid2   as recid no-undo.
define variable l_recid3   as recid no-undo.
...
/*6*/     assign v_si_entity = "".
/*6*/     if input ship_line.abs_site <> sod_site and not batchrun then do:
/*6*/        find first si_mstr no-lock where si_domain = global_domain and
/*6*/             si_site = input ship_line.abs_site no-error.
/*6*/        if available si_mstr then do:
/*6*/           assign v_si_entity = si_entity.
/*6*/        end.
/*6*/        find first si_mstr no-lock where si_domain = global_domain and
/*6*/             si_site = sod_site no-error.
/*6*/        if available si_mstr then do:
/*6*/           if si_entity <> v_si_entity then do:
/*6*/              assign v_si_entity = '会计单位[' + v_si_entity + ']与订单项[' + si_entity + ']不符'.
/*6*/              {pxmsg.i &MSGTEXT=v_si_entity &ERRORLEVEL=3}
/*6*/              next-prompt ship_line.abs_site with frame sample.
/*6*/              undo setdet, retry setdet.
/*6*/           end.
/*6*/        end.
/*6*/     end.
/*6*/ end. /* do transaction: */
         ...
         assign
            l_recid1 = recid(ship_line)
            l_recid2 = absship_recid.
         ...
         then do:
            ...
            if ship_line.abs_order <> "" and
               ship_line.abs_id begins "i"
            then
               assign l_recid3 = recid(so_mstr) l_recid4 = recid(sod_det).
...
if l_recid1 <> ?
then
   run itemLinePostProcessing.

PROCEDURE itemLinePostProcessing:

   define buffer ship_line for abs_mstr.
   define buffer absbuffer for abs_mstr.

      for first ship_line
         where ship_line.abs_domain = global_domain
         and   recid(ship_line)     = l_recid1
      no-lock:
         if cmmts then
         do:
            cmmts = no.
            ...
            if ship_line.abs_cmtindx <> cmtindx
            then do transaction:
               for first absbuffer
                  where absbuffer.abs_domain = global_domain
                  and   recid(absbuffer)     = l_recid1
               exclusive-lock:
                  absbuffer.abs_cmtindx = cmtindx.
               end. /*FOR FIRST absbuffer*/
            end. /*THEN DO TRANSACTION*/
         end. /*IF cmmts THEN */

         if l_cons_req then
         do:
            {gprun.i ""rcshwbb2.p"" "(input l_recid1, input l_recid2)"}
         end. /*IF l_cons_req THEN */
         ...
         if ship_line.abs_order <> ""
            and using_line_charges
            and ship_line.abs_id begins "i"
            then
               run getLineCharges
                  (input ship_line.abs_shipfrom,
