...
/* $Revision: 1.115.1.6 $   BY: Dan Herman            DATE: 09/07/05 ECO: *P40X* */
/* Revision: 1.115.1.5       BY: Xiao Liu     DATE: 01/11/07   ECO: *Q11H*  */
/* $Revision: 1.115.1.6 $ BY: Abhijit Gupta    DATE: 10/30/07  ECO: *P6CJ* */
/*-Revision end---------------------------------------------------------------*/
...
/* Added on 7/22/05 for credit card validation */
...
do on endkey undo main, leave main
   ...
   no-lock:
      ...
      if can-find (first df_mstr
         ...
         and   df_inv = true)
      then do:
         run chk_abs_inv_cons(input absid,
                              input ship_from,
                              input "",
                              input "",
                              input part_order,
   ...
   no-lock.
   /*
   * Added 07/12/05 fzt.Determine if this order will be processed with a credit card
...
/**/     assign v_si_entity = "".
/**/     if part_site <> sod_site and not batchrun then do:
/**/        find first si_mstr no-lock where si_domain = global_domain and
/**/             si_site = part_site no-error.
...
/**/              assign v_si_entity = '会计单位[' + v_si_entity + ']与订单项[' + si_entity + ']不符'.
/**/              {pxmsg.i &MSGTEXT=v_si_entity &ERRORLEVEL=3}
/**/              next-prompt part_site with frame a.
/**/              undo,retry.
/**/           end.
/**/        end.
/**/     end.
...
/* up_yn */
   ...
   abnormal_exit = false.
   /* Added for credit card enhancement on 22/07/05 */
   if lIfRelease = true then release so_mstr.
   /* Added for credit card enhancement on 22/07/05 end */
end. /* main */
