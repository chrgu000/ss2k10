...
/* Revision: 1.52.1.1      BY: Gaurav Kerkar      DATE: 07/18/05  ECO: *P3T8* */
/* Revision: 1.52.1.2      BY: Deepak Taneja      DATE: 07/27/07  ECO: *P62Q* */
/* $Revision: 1.52.1.3 $   BY: Anju Dubey      DATE: 08/13/07  ECO: *P64F* */
/*-Revision end---------------------------------------------------------------*/
...
for first so_mstr
   fields (so_domain so_curr so_cust so_due_date so_ex_rate so_ex_rate2
   ...
   with frame c down:
      ...
      if not lgData then do:
         ...
         editing:
            ...
            /* FIND NEXT RECORD */
            {mfnp01.i sod_det line sod_line so_nbr  " sod_domain =
               global_domain and sod_nbr "  sod_nbrln}
      ...
      if not available sod_det then do:
         ...
         if not lgData then do:
            ...
            editing:
               ...
               {mfnp.i pt_mstr sod_part  " pt_domain = global_domain
      ...
      if not lgData then do:
         ...
         assign
            sod_det.sod_qty_inv = sod_det.sod_qty_chg
...
PROCEDURE p_get_lotref:
   ...
   for last tr_hist
      field (tr_domain tr_nbr tr_line tr_type tr_part tr_rmks tr_ref)
