/* soivmtea.p - PENDING INVOICE MAINTENANCE                                   */
...
/* Revision: 1.94       BY: Wojciech Palczynski DATE: 12/17/02  ECO: *P0LB*   */
/* Revision: 1.96       BY: Paul Donnelly (SB)  DATE: 06/28/03  ECO: *Q00L*   */
/* Revision: 1.97       BY: Patrick de Jong     DATE: 07/10/03  ECO: *Q00Y*   */
/* Revision: 1.98       BY: Mercy Chittilapilly DATE: 08/04/03  ECO: *N2DJ*   */
/* Revision: 1.99       BY: Rajesh Kini         DATE: 08/13/03  ECO: *N2HN*   */
/* Revision: 1.100      BY: Dorota Hohol        DATE: 09/03/03  ECO: *P112*   */
/* Revision: 1.102      BY: Kirti Desai         DATE: 01/30/04  ECO: *P1JH*   */
/* Revision: 1.103      BY: Shoma Salgaonkar    DATE: 02/04/04  ECO: *P1M5*   */
/* Revision: 1.104      BY: Shoma Salgaonkar    DATE: 02/23/04  ECO: *P1PY*   */
/* Revision: 1.106      BY: Pankaj Goswami      DATE: 08/05/04  ECO: *P2BD*   */
/* Revision: 1.107      BY: Priya Idnani        DATE: 11/25/04  ECO: *P2WH*   */
/* Revision: 1.108      BY: Shivanand H         DATE: 01/20/05  ECO: *P34X*   */
/* Revision: 1.109      BY: Bharath Kumar       DATE: 02/23/05  ECO: *P39C*   */
/* Revision: 1.109.3.1  BY: Jean Miller         DATE: 06/26/06  ECO: *Q066*   */
/* Revision: 1.109.3.2  BY: Munira Savai     DATE: 07/13/06 ECO: *P4VS* */
/* Revision: 1.109.3.3   BY: Dilip Manawat    DATE: 03/05/07 ECO: *P5QK* */
/* $Revision: 1.109.3.4 $  BY: Anju Dubey    DATE: 08/13/07 ECO: *P64F* */
/*-Revision end---------------------------------------------------------------*/
...
define variable l_qty_cons    like sod_qty_chg Initial 0 no-undo.
...
do on error undo, leave:
   ...
   if so_fsm_type = "RMA" then do:
      ...
      find rmd_det  where rmd_det.rmd_domain = global_domain and  rmd_nbr  =
      so_nbr and
                         rmd_prefix = "C" and
                         rmd_line = sod_line
   ...
   then do:
      ...
      then do:
         ...
         if available pih_hist then
            ...
            save_parent_list = 0.
            for each sob_det where sob_domain = global_domain and
                                   sob_nbr  = sod_nbr and
                                   sob_line = sod_line
            no-lock:
   ...
   do on error undo, leave loopc:
      ...
      */
      ...
      end. /* NOT DEL-YN AND SOC_APM AND AVAILABLE PT_MSTR */
   ...
   then do:
      ...
      if l_consigned_line_item
      then
         if l_qty_cons <> sod_qty_chg
         then do:
             /*INVOICE QUANTITY CANNOT BE CHANGED FOR CONSIGNED LINE ITEM */
             {pxmsg.i &MSGNUM=7087 &ERRORLEVEL=4}
             return.
         end. /*IF l_qty_cons <> sod_qty_chg */
      ...
      if not lgData then do:
                  ...
                  if sod_list_pr entered or discount entered then do:
                     ...
                     if sod_list_pr entered then do:
                        ...
                        if line_pricing or not new_order then do:
                           ...
                           /* CHANGED FOURTH PARAMETER FROM sod_qty_ord */
                           /* TO sod_qty_chg.                           */
                           {gprun.i ""gppiqty2.p""
                              "(sod_line,
                                sod_part,
                                0,
                                sod_qty_chg * (sod_list_pr - last_list_price),
                                sod_um,
                                yes,
                                yes,
                                yes)"}
         ...
         end. /* do on error undo, retry (update sod_list_pr) */
      ...
      do on error undo, retry on endkey undo, leave loopc:
         form
            ...
            sod_slspsn[2]     colon 15 label "Salesperson 2"
            sod_comm_pct[2]   colon 26 no-label
            sort[2]           colon 35 no-label
            sod_slspsn[3]     colon 15 label "Salesperson 3"
            sod_comm_pct[3]   colon 26 no-label
            sort[3]           colon 35 no-label
            sod_slspsn[4]     colon 15 label "Salesperson 4"
         ...
         do counter = 1 to 4:
            find sp_mstr where sp_domain = global_domain
                           and sp_addr = sod_slspsn[counter]
            no-lock no-error.
      ...
      if (so_fsm_type = " " and soc_returns_isb and
         ...
         can-find (first isb_mstr where isb_domain = global_domain
...
--------------------------------------------------------------------------*/
   ...
   with frame c_site overlay row frame-row(c) column 16.
...
/**/     end.
         ...
         then do:
            ...
            find si_mstr where si_domain = global_domain
               and si_site = old_sod_site
            no-lock no-error.

            old_db = si_db .

            if new_db <> old_db  then do:
               /*New site must be in the same domain as the old site */
               {pxmsg.i &MSGNUM=6255 &ERRORLEVEL=3}
               if not batchrun then pause.
               else undo, leave.
               undo setsite, retry.
            end.
   ...
   end. /* SETSITE: DO ON ERROR UNDO, RETRY */
...
--------------------------------------------------------------------------*/
   ...
   if sod_type = "" then do:
      ...
      find mfc_ctrl where mfc_domain = global_domain and
         mfc_field = "frc_returns"
      no-lock no-error.
   ...
   then do:
      ...
      {gprun.i ""gppiqty2.p""
         "(sod_det.sod_line,
           sod_det.sod_part,
           0,
           sod_det.sod_qty_chg * sod_det.sod_price,
           sod_det.sod_um,
           yes,
           yes,
           yes)"}
   ...
   for each wkpi_wkfl
      ...
      if wkpi_amt_type = "3" then do:
         if available pt_mstr then do:
            ...
            find pi_mstr where pi_domain = global_domain and
                               pi_list_id = wkpi_list_id no-lock.
