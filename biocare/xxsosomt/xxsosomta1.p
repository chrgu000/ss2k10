...
/* Revision: 1.71.1.1    BY: Swati Sharma         DATE: 03/23/05  ECO: *P3DF* */
/* Revision: 1.71.1.2    BY: Patrick Weng         DATE: 04/19/05  ECO: *P3GG* */
/* Revision: 1.71.1.3    BY: Alok Gupta           DATE: 05/04/05  ECO: *P3K4* */
/* Revision: 1.71.1.4    BY: Amit Singh           DATE: 11/22/05  ECO: *P48V* */
/* Revision: 1.71.1.8    BY: Preeti Sattur        DATE: 03/21/06  ECO: *Q0SL* */
/* Revision: 1.71.1.9    BY: Ajit                 DATE: 06/07/07  ECO: *Q15G* */
/* Revision: 1.71.1.10   BY: Ajit                 DATE: 06/12/07  ECO: *Q16P* */
/* $Revision: 1.71.1.13 $  BY: Sanat Paul       DATE: 08/13/07  ECO: *P64D* */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
...
define shared variable confirm       like mfc_logical
                                     format "yes/no" initial yes label "Confirmed".
...
for first gl_ctrl
   fields(gl_domain gl_ar_acct gl_ar_cc gl_ar_sub)
   where gl_domain = global_domain
no-lock: end.

for first pic_ctrl
   fields(pic_domain pic_so_date pic_so_linpri)
...
for first soc_ctrl
   fields(soc_domain soc_all_days soc_confirm soc_cr_hold soc_det_all
...
if this-is-rma then do:
   ...
   for first rmc_ctrl
      fields(rmc_domain rmc_all_days rmc_consume rmc_det_all rmc_hcmmts
...
for first mfc_ctrl
   fields(mfc_field mfc_logical mfc_domain)
...
editing:
   ...
   if recno <> ? then do with frame b:
      ...
      for first sod_det
         fields(sod_domain sod_btb_po sod_btb_pod_line sod_btb_type
                sod_btb_vend sod_line sod_nbr sod_per_date sod_qty_inv)
...
/* SS - 121206.1 - B */
 if input so_nbr <> "" then do:
    ...
    find so_mstr  where so_domain= global_domain and so_nbr = input so_nbr no-lock no-error.
         if avail so_mstr then do:
                  find code_mstr  where code_domain = global_domain
				    and code_fldname = "SO-Jurisdiction" 
					and code_value = global_userid NO-LOCK no-error.
		    if available(code_mstr) and lookup(so_userid, code_cmmt) = 0 then do:
       		   /*用户无写入权限*/
			   {pxmsg.i &MSGNUM=4401 &ERRORLEVEL=3}
        		next-prompt so_nbr with frame a.
        		undo, retry .
             end. 
			if not available code_mstr and so_userid <> global_userid then do:
       		   /*用户无写入权限*/
			   {pxmsg.i &MSGNUM=4401 &ERRORLEVEL=3}
        		next-prompt so_nbr with frame a.
        		undo, retry .
...
do transaction on error undo, leave on endkey undo, return:
   ...
   if input so_nbr = "" then do:
      ...
      if this-is-rma then do:
         ...
         {fsnctrl2.i "rmc_domain = global_domain"
...
do transaction on error undo, retry:
   ...
   if not available so_mstr then do:
      ...
      /* ADDING NEW RECORD */
      if {gpisapi.i} then
         run qxi-msg (input 1, input 1, input sonbr).
      else
   ...
   if soc_use_btb and so_primary then do:
      ...
      if not so_secondary then do :
         for first mfc_ctrl
            ...
            where mfc_domain = global_domain
            and   mfc_field = "soc_emt_ship"
         no-lock:
         end. /* FOR FIRST mfc_ctrl */

         if available mfc_ctrl and not mfc_logical then do :
            for first sod_det
               fields(sod_btb_po sod_btb_pod_line sod_btb_type sod_btb_vend
               sod_line sod_nbr sod_qty_ship sod_qty_inv)
               where sod_domain = global_domain
               and   sod_nbr = so_nbr
            no-lock:
            end. /* FOR FIRST SOD_DET */

            if available sod_det and sod_qty_ship <> 0
            then do:
               run p-mfmsg (input 2864, input 4).
               /* Modification is not allowed for shipped Secondary SO */
               if not batchrun then pause.
               not_okay = 3.
               return.
            end.
   ...
   for first sod_det
      fields(sod_domain sod_btb_po sod_btb_pod_line sod_btb_type sod_btb_vend
   ...
   then do:
      ...
      then do:
         ...
         then do:
            ...
            for first ad_mstr
               fields (ad_domain ad_addr ad_ctry)
      ...
      if l_edittax then do:
         ...
         for first ad_mstr
            fields(ad_domain ad_addr ad_city ad_country ad_edi_ctrl
                   ad_inv_mthd ad_lang ad_line1 ad_line2 ad_name ad_pst_id
                   ad_ctry ad_state ad_taxable ad_taxc ad_tax_in
                   ad_tax_usage ad_zip)
            where ad_domain = global_domain
              and ad_addr   = so_ship
         no-lock:
   ...
   for first cm_mstr
      fields(cm_domain cm_addr cm_ar_acct cm_ar_cc cm_ar_sub cm_balance
             cm_cr_hold cm_cr_limit cm_cr_terms cm_curr cm_disc_pct cm_fix_pr
   ...
   for first ad_mstr
      fields(ad_domain ad_addr ad_city ad_country ad_edi_ctrl
             ...
             ad_pst_id ad_state ad_taxable ad_taxc ad_tax_in
             ad_tax_usage ad_zip)
       where ad_domain = global_domain and ad_addr = so_bill
   no-lock: end.
   ...
   if ad_inv_mthd = "" then do:
      ...
      for first ad_mstr
         fields(ad_domain ad_addr ad_city ad_country ad_edi_ctrl
                ...
                ad_pst_id ad_state ad_taxable ad_taxc ad_tax_in
                ad_tax_usage ad_zip)
          where ad_domain = global_domain and ad_addr = so_ship
      no-lock: end.
      ...
      if ad_inv_mthd = "" then
         for first ad_mstr
            fields(ad_domain ad_addr ad_city ad_country ad_edi_ctrl
                   ...
                   ad_pst_id ad_state ad_taxable ad_taxc ad_tax_in
                   ad_tax_usage ad_zip)
             where ad_domain = global_domain and ad_addr = so_cust
         no-lock: end.
   ...
   do on error undo, retry with frame b:
      ...
      if new so_mstr then do:
         ...
         /* LOAD DEFAULT TAX CLASS & USAGE */
         for first ad_mstr
            fields(ad_domain ad_addr ad_city ad_country ad_edi_ctrl ad_inv_mthd
         ...
         do counter = 1 to 4:
            ...
            if cm_mstr.cm_slspsn[counter] <> "" then do:
               ...
               for first spd_det
                  fields(spd_domain spd_addr spd_comm_pct spd_cust spd_part
                         spd_prod_ln)
               ...
               if not available spd_det then
                  for first sp_mstr
                     fields(sp_domain sp_addr sp_comm_pct)
      ...
      if available ad_mstr and not so_sched then do:
   ...
   for last sod_det
      fields(sod_domain sod_btb_po sod_btb_pod_line sod_btb_type sod_btb_vend
             sod_line sod_nbr sod_per_date sod_qty_inv)
      where sod_domain = global_domain and sod_nbr = so_mstr.so_nbr
...
PROCEDURE qxi-msg:
/* ------------------------------------------------------------------
   Purpose:     Display Adding New Record Message for Qxtend
   Parameters:  l_num  = Message Number
                l_stat = Severity
                c_var  = Optional Message text
   Notes:
   ------------------------------------------------------------------*/
   define input parameter l_num  as integer no-undo.
   define input parameter l_stat as integer no-undo.
   define input parameter c_var as  character no-undo.

   {pxmsg.i &MSGNUM=l_num &ERRORLEVEL=l_stat &MSGARG1=c_var}

END PROCEDURE. /* PROCEDURE QXI-MSG */
...
/* ------------------------------------------------------------------
   ...
   find ad_mstr where ad_domain = global_domain
                  and ad_addr = "qadtemp" + mfguser
   exclusive-lock.

   /* Get Next Record Number */
   {mfactrl.i "cmc_domain = global_domain" "cmc_domain"
    "ad_domain = global_domain" cmc_ctrl cmc_nbr ad_mstr ad_addr p-shipto}
