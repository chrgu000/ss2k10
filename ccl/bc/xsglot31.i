...
if available(pod_det) and available(po_mstr) then do: 
	  ...
	  vv1500 = substring(trim(pod_rev),1,2).
       ...
       vv1500 = vv1500 + substring(string(year(Date ( V1309 ))),4,1).   /* year - 1 character*/

   

    if  length(string(month(Date ( v1309 )))) = 1 then 
     vv1500 = vv1500 +  string(month(Date ( v1309 ))).   /* month - 1 character*/
 ELSE DO:
     v_month = string(month(Date ( v1309 ))) .
 ...
 END.
	...
	if  length(string(day(Date ( V1309 )))) = 1 then 
	     vv1500 = vv1500 + "0" + string(day(Date ( V1309 ))).     /* day - 2 character*/
	else
	    vv1500 = vv1500 + string(day(Date ( V1309 ))) .
