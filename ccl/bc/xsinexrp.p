...
form
   ...
   includingErrorFormat   colon 25 label "包括不规则的 LOT"
...
repeat:
   ...
   {mfphead.i}
	...
	for each ld_det where ld_site >= site and ld_site <= site1  and 
       ...
       display ld_site		label "地点"  
                    ld_loc		column-label "!库位"
		    ld_part		column-label "!料品"
		    ld_lot           column-label  "批号"
		    ld_qty_oh    column-label "库存数"
		    wptavgint     column-label "保质期(M)"
		    expireinv      column-label "LOT到期日"
		    if expireinv - today = ? then  0 else expireinv - today
		    column-label "过N天后过期"  WITH STREAM-IO width 200.
