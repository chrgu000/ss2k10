...
/* UNPLANNED ISSUE [NORMAL MATERIAL] */
...
7. 出/入库时的成本中心要填部门代码
*****************************************************************************/
define variable version as character format "x(2)" initial ".1".
...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv24wtimeout" no-lock no-error. /* Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
 ...
 sectionid = sectionid + 1 .
  ...
  leave V1002L.
  /* LOGICAL SKIP END */
   display "[计划外出正常料]"+ version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1002 no-box.
 ...
 /* Additional Labels Format */
  ...
  /* --CYCLE TIME SKIP -- END */
   ...
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1100 no-box.
...
/*20140109.1 -start
 *  /* --FIRST TIME DEFAULT VALUE -- START */
 *  find first code_mstr where code_fldname = "ISS-UNPREASON" and V1100 begins code_value no-lock no-error.
 ...
 *20140109.1 -End *******/
  ...
  /* --FIRST TIME SKIP -- END */
   ...
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1200 no-box.
...
/*****20140109.1 -start */
  ...
  /* --DEFINE VARIABLE -- START */
  hide all.
  define variable V1210   as char format "x(50)".
  define variable PV1210  as char format "x(50)".
  define variable account  as char format "x(50)".
  define variable sub   as char format "x(50)".
  define variable L12101  as char format "x(40)".
  define variable L12102  as char format "x(40)".
  define variable L12103  as char format "x(40)".
  define variable L12104  as char format "x(40)".
  define variable L12105  as char format "x(40)".
  define variable L12106  as char format "x(40)".
  /* --DEFINE VARIABLE -- END */
  ...
  /* --CYCLE TIME SKIP -- START */
  if sectionid >= 1 then leave V1210L.
  /* --CYCLE TIME SKIP -- END */

   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1210 no-box.

   /* LABEL 1 - START */
   L12101 = "部门?" .
   display L12101  format "x(40)" skip with fram F1210 no-box.
   /* LABEL 1 - END */


   /* LABEL 2 - START */
   L12102 = "" .
   display L12102  format "x(40)" skip with fram F1210 no-box.
   /* LABEL 2 - END */


   /* LABEL 3 - START */
   L12103 = "" .
   display L12103  format "x(40)" skip with fram F1210 no-box.
   /* LABEL 3 - END */


   /* LABEL 4 - START */
   L12104 = "" .
   display L12104  format "x(40)" skip with fram F1210 no-box.
   /* LABEL 4 - END */
   display "输入或按E退出 " format "x(40)" skip
  skip with fram F1210 no-box.
  /* 部门  */
  Update V1210 WITH fram F1210 NO-LABEL
  EDITING:
  readkey pause wtimeout.
  if lastkey = -1 Then quit.
  if LASTKEY = 404 Then Do: /* DISABLE F4 */
  ...
  /* PRESS e EXIST CYCLE */
  IF V1210 = "e" THEN LEAVE MAINLOOP.
  display skip WMESSAGE NO-LABEL with fram F1210.

  /* ---- Valid Check ---- START */

  display "...PROCESSING... " @ WMESSAGE NO-LABEL with fram F1210.
  pause 0.
  /* CHECK FOR NUMBER VARIABLE START */
  /* CHECK FOR NUMBER VARIABLE END */
  /* ---- Valid Check ---- END */
...
/******************
 *  find first dpt_mstr where dpt_dept = V1210.
 *  IF NOT AVAILABLE dpt_mstr then do:
 *      display skip "Error , Retry." @ WMESSAGE NO-LABEL with fram F1210.
 *      pause 0 before-hide.
 *      undo, retry.
 *  end.
********************/
     find first rsn_ref where rsn_type = V1210 and rsn_code = V1110 no-lock no-error.
     IF NOT AVAILABLE rsn_ref then do:
      display skip "Error , Retry." @ WMESSAGE NO-LABEL with fram F1210.
      pause 0 before-hide.
      undo, retry.
  end.
     find first yyrsn_ref where yyrsn_type = rsn_type and yyrsn_code = rsn_code no-lock no-error.
     IF NOT AVAILABLE yyrsn_ref then do:
      display skip "Error , Retry." @ WMESSAGE NO-LABEL with fram F1210.
      pause 0 before-hide.
      undo, retry.
  end.
  ...
  display "" @ WMESSAGE NO-LABEL with fram F1210.
  pause 0.
  leave V1210L.
  END.
  /* END LINE :1210 部门 */
/*****20140109.1 -end */
 ...
 REPEAT:
  ...
  /* LOGICAL SKIP START */
  /* LOGICAL SKIP END */
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE ( pt_mstr ) then
  ...
  /* LOGICAL SKIP START */
  /* LOGICAL SKIP END */
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1400 no-box.
...
If AVAILABLE ( pt_mstr ) then
  ...
  /* --CYCLE TIME SKIP -- END */
   ...
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1410 no-box.
 ...
 /* Additional Labels Format */
  ...
  leave V1500L.
  /* LOGICAL SKIP END */
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1500 no-box.
   ...
   /* LABEL 2 - START */
   find first ld_det where ld_part = V1300 and ld_site = V1002 and ld_loc = V1400 and
...
ld_ref = ""  and
ld_loc = V1400 and ld_qty_oh <> 0 use-index ld_part_lot no-lock no-error.
If AVAILABLE ( ld_det ) then
  ...
  /*20121127 START */
   find first ld_det where ld_part = V1300 and ld_site = V1002 and ld_loc = V1400 and
  ld_ref = ""  and ld_qty_oh <> 0 use-index ld_part_lot no-lock no-error.
  If AVAILABLE ( ld_det ) then L15002 = ld_lot.
   else L15002 = "".
  /*20121127 END */
 ...
 /* Additional Labels Format */
  ...
  leave V1503L.
  /* LOGICAL SKIP END */
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1503 no-box.
...
If AVAILABLE ( pt_mstr ) then
 ...
 /* Additional Labels Format */
  ...
  /* LOGICAL SKIP START */
  /* LOGICAL SKIP END */
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1600 no-box.
...
If AVAILABLE ( ld_det ) then
  ...
  /* CHECK FOR NUMBER VARIABLE END */
  find first LD_DET where ( decimal(V1600) < 0 and ld_part = "BARCODEXZYIU" ) OR ( ld_part = V1300 AND Ld_site = V1002 AND
ld_ref = ""  and ld_loc = V1400 and ld_lot = V1500 and ld_QTY_oh >= DECIMAL ( V1600 ) ) no-lock no-error.
 ...
 /* Additional Labels Format */
  ...
  /* LOGICAL SKIP START */
  /* LOGICAL SKIP END */
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F1700 no-box.
...
If AVAILABLE ( tr_hist ) then
  ...
  /* --CYCLE TIME SKIP -- END */
   ...
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F9000 no-box.
 ...
 /* Additional Labels Format */
  ...
  /* LOGICAL SKIP START */
  /* LOGICAL SKIP END */
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F9010 no-box.
...
/*******************/
  ...
  If AVAILABLE ( ld_det ) then do:
    if ld_lot <> V1500 and wtr__chr04 <> "" then
...
tr_date = today  and
tr_trnbr > integer ( V9000 ) and
tr_nbr = V1100  and tr_type = "ISS-UNP" and ld_site = V1002 and tr_part = V1300  and tr_serial = V1500 and
tr_time + 15 >= TIME
use-index tr_date_trn no-lock no-error.
If NOT AVAILABLE ( tr_hist ) then
 ...
 IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
  ...
  /* LOGICAL SKIP START */
  /* LOGICAL SKIP END */
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F9110 no-box.
...
If AVAILABLE ( pt_mstr ) then
 ...
 /* Additional Labels Format */
  ...
  /* LOGICAL SKIP START */
  /* LOGICAL SKIP END */
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F9120 no-box.
 ...
 /* Additional Labels Format */
  ...
  /* --FIRST TIME DEFAULT VALUE -- START */
  find first upd_det where upd_nbr = "INV24" and upd_select = 99 no-lock no-error.
If AVAILABLE ( upd_det ) then
  ...
  /* --CYCLE TIME SKIP -- END */
   ...
   display "[计划外出正常料]" + version + "*" + TRIM ( V1002 ) format "x(40)" skip with fram F9130 no-box.
 ...
 /* Additional Labels Format */
  ...
  INPUT FROM VALUE(LabelsPath + "inv24" + trim ( wtm_fm ) ).
