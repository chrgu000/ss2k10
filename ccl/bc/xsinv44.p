...
/* Generate date / time  2007-11-27 15:56:57 */
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1205 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1205.
            ...
            THEN DO:
                  IF recid(TAG_MSTR) = ? THEN find first TAG_MSTR where 
                              NOT TAG_VOID AND NOT TAG_POSTED AND ( TAG_LOC = V1200 OR V1200 = "" ) AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = "" AND  
                              TAG_NBR >=  INTEGER ( INPUT V1205)
                               no-lock no-error.
                  ELSE find next TAG_MSTR where 
                              NOT TAG_VOID AND NOT TAG_POSTED AND ( TAG_LOC = V1200 OR V1200 = "" ) AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = ""  
                               no-lock no-error.
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first TAG_MSTR where NOT TAG_VOID AND NOT TAG_POSTED AND TAG_NBR = integer ( V1205 ) AND ( TAG_LOC = V1200 OR V1200 = "" )  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = ""  NO-ERROR NO-WAIT.
        IF NOT AVAILABLE TAG_MSTR then do:
                display skip "該標簽無效" @ WMESSAGE NO-LABEL with fram F1205.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE ( tag_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.

                /* LABEL 1 - START */ 
                find first tag_mstr where tag_nbr = integer(V1205)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
                L16001 = "復盤數量?[" + trim (tag_cnt_um) + "]" .
                else L16001 = "" . 
...
If AVAILABLE ( tag_mstr ) then
                ...
                find first tag_mstr where tag_nbr = integer(V1205)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
                L16004 = "標簽/庫位:" + trim(V1205) + "/" + trim ( tag_loc ) .
                else L16004 = "" . 
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
...
If AVAILABLE ( tag_mstr ) then
                ...
                find first tag_mstr where tag_nbr = integer(V1205)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
                L17004 = "庫位/數量:" + trim ( tag_loc ) + "/" + trim( V1600 ) + "[" + trim ( tag_cnt_um ) + "]" .
                else L17004 = "" . 
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.

                /* LABEL 1 - START */ 
                find first tag_mstr where tag_nbr = integer(V1205) and tag_rcnt_qty = decimal (V1600)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
                L90101 = "已提交 TAG:" + string ( tag_nbr ) .
                else L90101 = "" . 
...
If AVAILABLE ( tag_mstr ) then
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.
...
If AVAILABLE ( tag_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[復盤(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
