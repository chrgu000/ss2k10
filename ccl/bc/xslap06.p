...
/* SO-WO LABLE */
/* Generate date / time  10/04/07 16:32:59 */
/* ss - 121029.1 by: jack */  /* 增加类型选择 */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* ss - 121029.1 -b
           ...
           ss - 121029.1 -e */
           /* ss - 121029.1 -b */
            display "[销售工单标签121029.1]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
           /* ss - 121029.1 -e */
                /* LABEL 1 - START */ 
                L11001 = "工单号码?" .
...
If AVAILABLE ( wo_mstr ) then
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1103.
            ...
            THEN DO:
                  IF recid(wo_mstr) = ? THEN find first wo_mstr where 
                              wo_nbr = V1100 AND wo_site = V1002 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" OR substring ( WO_SO_JOB ,1,2) = "SR" OR substring(WO_SO_JOB,1,3)="OBS" OR substring ( WO_SO_JOB ,1,2) = "RM"  OR substring ( WO_SO_JOB ,1,1) = "R"  ) AND  
                              wo_lot >=  INPUT V1103
                               no-lock no-error.
                  ELSE find next wo_mstr where 
                              wo_nbr = V1100 AND wo_site = V1002 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA"  OR substring ( WO_SO_JOB ,1,2) = "SR" OR substring(WO_SO_JOB,1,3)="OBS" OR substring ( WO_SO_JOB ,1,2) = "RM" OR substring ( WO_SO_JOB ,1,1) = "R"   )   
                               no-lock no-error.
                  IF AVAILABLE wo_mstr then display skip 
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first wo_mstr where wo_lot = V1103 AND wo_site = V1002 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" OR substring ( WO_SO_JOB ,1,2) = "SR"  OR substring(WO_SO_JOB,1,3)="OBS"  OR substring ( WO_SO_JOB ,1,2) = "RM" OR substring ( WO_SO_JOB ,1,1) = "R"  )   no-lock no-error.
        IF NOT AVAILABLE wo_mstr then do:
...
If AVAILABLE ( wo_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        find first wo_mstr where wo_lot  = V1103 no-lock no-error.
If AVAILABLE ( wo_mstr ) then
        V1105 = if substring( wo_so_job,1,2 ) = "RA" then substring(wo_so_job,1,7) else substring(wo_so_job,1,7).
        V1105 = ENTRY(1,V1105,"@").
        /* --FIRST TIME DEFAULT  VALUE -- END  */
        ...
        IF NOT AVAILABLE SOD_DET then do:
                display skip "订单有误,请查询1" @ WMESSAGE NO-LABEL with fram F1105.
                pause 0 before-hide.
                undo, retry.
        end.
        IF not (substring(V1105,1,2) = "SA" or substring(V1105,1,2) = "RA" OR substring ( WO_SO_JOB ,1,2) = "SR" OR substring(WO_SO_JOB,1,3)="OBS"  OR substring ( WO_SO_JOB ,1,2) = "RM" OR substring ( WO_SO_JOB ,1,1) = "R"  ) THEN DO:
                display skip "订单有误,请查询2" @ WMESSAGE NO-LABEL with fram F1105.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
...
If V1111<>"" then
        ...
        /* --FIRST TIME SKIP -- END  */
                ...
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1111 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     /* ss - 121029.1 -b */
      /* START  LINE :9026  选择打印条码类型  */
     V9026L:
     REPEAT:

        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V9026           as char format "x(50)".
        define variable PV9026          as char format "x(50)".
        define variable L90261          as char format "x(40)".
        define variable L90262          as char format "x(40)".
        define variable L90263          as char format "x(40)".
        define variable L90264          as char format "x(40)".
        define variable L90265          as char format "x(40)".
        define variable L90266          as char format "x(40)".
        /* --DEFINE VARIABLE -- END */
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9026 no-box.

                /* LABEL 1 - START */ 
                L90261 = "选择条码格式" .
                display L90261          format "x(40)" skip with fram F9026 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                  L90262 = "1旧标签格式 2 亚伦标签格式(含检验规格)" . 
                display L90262          format "x(40)" skip with fram F9026 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                  L90263 = "" . 
                display L90263          format "x(40)" skip with fram F9026 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                  L90264 = "" . 
                display L90264          format "x(40)" skip with fram F9026 no-box.
                /* LABEL 4 - END */ 
                display "输入或按E退出 "      format "x(40)" skip
        skip with fram F9026 no-box.
        Update V9026
        WITH  fram F9026 NO-LABEL
        EDITING:
          readkey pause wtimeout.
          if lastkey = -1 Then quit.
        if LASTKEY = 404 Then Do: /* DISABLE F4 */
        ...
        /* PRESS e EXIST CYCLE */
        IF V9026 = "e" THEN  LEAVE V9010LMAINLOOP.
        display  skip WMESSAGE NO-LABEL with fram F9026.

         /*  ---- Valid Check ---- START */

        display "...PROCESSING...  " @ WMESSAGE NO-LABEL with fram F9026.
        pause 0.
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        IF not ( length(V9026) = 1 AND index("12",V9026) <> 0 ) THEN DO:
                display skip "Error , Retry " @ WMESSAGE NO-LABEL with fram F9026.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */

        display  "" @ WMESSAGE NO-LABEL with fram F9026.
        pause 0.
        leave V9026L.
     END.
     PV9026 = V9026.
     /* END    LINE :9026  选择条码格式类型  */
     
     
      /* START  LINE :9027  选择检验类型  */
     if v9026 = "2" then do:
     V9027L:
     REPEAT:
        ...
        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V9027           as char format "x(50)".
        define variable PV9027          as char format "x(50)".
        define variable L90271          as char format "x(40)".
        define variable L90272          as char format "x(40)".
        define variable L90273          as char format "x(40)".
        define variable L90274          as char format "x(40)".
        define variable L90275          as char format "x(40)".
        define variable L90276          as char format "x(40)".
        define var v_i as int .
        define var v_error as logical .
        /* --DEFINE VARIABLE -- END */


        /* --FIRST TIME DEFAULT  VALUE -- START  */
        V9027 = "ROHS,REACH,HF,FC".
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9027 no-box.

                /* LABEL 1 - START */ 
                L90271 = "选择检验规格" .
                display L90271          format "x(40)" skip with fram F9027 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                  L90272 = "逗号分隔保留新规则就可以" . 
                display L90272          format "x(40)" skip with fram F9027 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                  L90273 = "" . 
                display L90273          format "x(40)" skip with fram F9027 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                  L90274 = "" . 
                display L90274          format "x(40)" skip with fram F9027 no-box.
                /* LABEL 4 - END */ 
                display "输入或按E退出 "      format "x(40)" skip
        skip with fram F9027 no-box.
        Update V9027
        WITH  fram F9027 NO-LABEL
        EDITING:
          readkey pause wtimeout.
          if lastkey = -1 Then quit.
        if LASTKEY = 404 Then Do: /* DISABLE F4 */
        ...
        /* PRESS e EXIST CYCLE */
        IF V9027 = "e" THEN  LEAVE V9010LMAINLOOP.
        display  skip WMESSAGE NO-LABEL with fram F9027.
        
        if  v9027 = "" then do:
		     	display skip "Error , Retry " @ WMESSAGE NO-LABEL with fram F9027.
                pause 0 before-hide.
                undo, retry.
		    end .
         ...
         /*  ---- Valid Check ---- START */
         v_error = no .
         DO v_i = 1 TO   NUM-ENTRIES(v9027,",") :
			     if   INDEX("ROHS,REACH,HF,FC",   ENTRY(v_i,v9027,",")) = 0  then do:
			     	v_error = yes .
			     leave .
			     end .
        ...
        display  "" @ WMESSAGE NO-LABEL with fram F9027.
        pause 0.
        leave V9027L.
     END.
     PV9027 = V9027.
   end . /* 2 新格式 */
     /* END    LINE :9027  选择检验类型  */
...
If AVAILABLE ( upd_det ) then
   ...
   /* Additional Labels Format */
     ...
     ss - 121029.1 -e */
     /* ss - 121029.1 -b */
      INPUT FROM VALUE(LabelsPath + v_lap ).
     /* ss - 121029.1 -e */
     ...
     Do While True:
       ...
       /* ss - 121029.1 -b */
       if v9026 = "2" then do:
       	/* ROHS,REACH,HF,FC $j rohs $k reach $l hf $m fc */
       	   if index (v9027,"rohs") > 0 then 
		       	  av9030 = "V".
		       	else av9030 = "" .
		       IF INDEX(ts9030,"$j") <> 0  THEN DO:
		       TS9030 = substring(TS9030, 1, Index(TS9030 , "$j") - 1) + av9030 
		       + SUBSTRING( ts9030 , index(ts9030 ,"$j") + length("$j"), LENGTH(ts9030) - ( index(ts9030 ,"$j" ) + length("$j") - 1 ) ).
		       END.
		       if index (v9027,"REACH") > 0 then 
		       	  av9030 = "V".
		       	else av9030 = "" .
