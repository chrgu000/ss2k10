...
/* ss - 111224.1 by: jack */  /* add new label style */
/* ss - 111226.1 by: jack */
 /* code_fldname = "shiptorev" and code_value = so_ship */
...
display skip "工单不能为空" @ WMESSAGE NO-LABEL with fram F1100.
                pause 0 before-hide.
                Undo, retry.
...
display skip "料号不存在" @ WMESSAGE NO-LABEL with fram F1104.
                pause 0 before-hide.
                Undo, retry.
...
End.
   ...
   /* Additional Labels Format */
     /* START  LINE :1105  销售订单[SalesOrder]  */
     V1105L:
     REPEAT:
...
If not AVAILABLE ( so_mstr )  then do:
 display skip "订单不存在" @ WMESSAGE NO-LABEL with fram F1105.
                pause 0 before-hide.
                Undo, retry.
...
display skip "PO不能为空" @ WMESSAGE NO-LABEL with fram F1106.
                pause 0 before-hide.
                Undo, retry.
...
display skip "料号版本不能为空" @ WMESSAGE NO-LABEL with fram F1110.
                pause 0 before-hide.
                Undo, retry.
...
End.
   ...
   /* Additional Labels Format */
     /* START  LINE :1111  客户料号[CUST PART]  */
     V1111L:
     REPEAT:
...
If V1111<>"" then
        ...
        IF not V1111<>"" THEN DO:
                display skip "客户料号不能为空" @ WMESSAGE NO-LABEL with fram F1111.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
...
display skip "不能为空" @ WMESSAGE NO-LABEL with fram F1114.
                pause 0 before-hide.
                Undo, retry.
...
End. */
        /* ss - 111224.1 -b */
        FIND FIRST so_mstr WHERE so_nbr = V1105 NO-LOCK NO-ERROR .
        FIND FIRST CODE_mstr WHERE  Code_fldname = "shitptorev" AND 
             code_value = so_ship  NO-LOCK NO-ERROR .

        /* ss - 111224.1 -e */
        /* ss - 111224.1 -b
        IF not V1114<>"" THEN DO:
                display skip "客户料号版本不能为空" @ WMESSAGE NO-LABEL with fram F1114.
                pause 0 before-hide.
                undo, retry.
        end.
        ss - 11124.1 -e */
        /* ss - 111224.1 -b */
        IF AVAILABLE CODE_mstr AND  V1114 = "" THEN DO:
               display skip "客户料号版本不能为空" @ WMESSAGE NO-LABEL with fram F1114.
               pause 0 before-hide.
               undo, retry.
       end.
   ...
   /* Additional Labels Format */
     /* START  LINE :1115  生产日期[Prod Date]  */
     V1115L:
     REPEAT:
     ...
     END.
     PV1115 = V1115.
     /* END    LINE :1115  生产日期[Prod Date]  */


   /* Additional Labels Format */
     /* START  LINE :1116  生产线[prod line]  */
     V1116L:
     REPEAT:
...
display skip "长度不能超过2位" @ WMESSAGE NO-LABEL with fram F1116.
                pause 0 before-hide.
                Undo, retry.

End.
        IF not V1116<>"" THEN DO:
                display skip "生产线不能为空" @ WMESSAGE NO-LABEL with fram F1116.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
...
If V1117<>"" then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        IF not (V1118="d" or V1118="n") THEN DO:
                display skip "班次只能是D或N" @ WMESSAGE NO-LABEL with fram F1118.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     /* START  LINE :1120  标签类型[label type]  */
     V1120L:
     REPEAT:
      ...
      FIND FIRST so_mstr WHERE so_nbr = V1105 NO-LOCK NO-ERROR .
        ...
        /* ss - 111226.1 -b */
        If search(LabelsPath1 + "lap87" + trim ( wtm_fm ) ) = ? Then do:
	                 display skip "文件不存在，请重新输入." @ WMESSAGE NO-LABEL with fram F1120.
                         pause 0 before-hide.
                         Undo, retry.
	 End.
        /* ss - 111226.1 -e */
...
display skip "Serial不能为空" @ WMESSAGE NO-LABEL with fram F1122.
                pause 0 before-hide.
                Undo, retry.
...
End.
   ...
   /* Additional Labels Format */
     /* START  LINE :1124  发货日期[Ship Date]  */
     V1124L:
     REPEAT:
...
display skip "Ship Date不能为空" @ WMESSAGE NO-LABEL with fram F1124.
                pause 0 before-hide.
                Undo, retry.
...
End.
 ...
 {xsgetsoseqno.i}
	V1335 = fill( "0" , 4 - length (  trim (V1110) ) ) + trim (V1110) + trim ( WeekResult ). 
	V1335 = V1335 + Get_SO_SEQ_NO(weekResult,V1104,V1105).

	/* ( if substring ( trim (V1105) ,1,1) = "R" then "R" else "*" ) */ 
	
	V1335 = V1335 + trim(V1116) + trim(V1118).
...
If V1335<>"" then
   ...
   wtm_num = V9020.
   /* Additional Labels Format */
     /* START  LINE :9025  标签额外格式[Label Format]  */
     V9025L:
     REPEAT:
...
*/
     ...
     END.
     PV9025 = V9025.
     /* END    LINE :9025  标签额外格式[Label Format]  */
...
If AVAILABLE ( upd_det ) then
   ...
   /* Additional Labels Format */
     ...
     PROCEDURE lap879030l.
       ...
       DEFINE VAR v1123 AS CHAR .
       
         FIND FIRST so_mstr WHERE so_nbr = V1105 NO-LOCK NO-ERROR .
        FIND FIRST CODE_mstr WHERE  Code_fldname = "shitptorev" AND 
             code_value = so_ship  NO-LOCK NO-ERROR .
        IF AVAILABLE CODE_mstr THEN DO:
            IF v1120 = "w"  THEN DO:  /* 4s 外包装 两位 */
                FIND FIRST usrw_wkfl WHERE usrw_key1 = "4s" + STRING(TODAY , "99/99/99" ) 
                    + v1118  NO-ERROR .
                IF AVAILABLE usrw_wkfl  THEN DO:
                    usrw_key2 = string(INT(usrw_key2) + 1 , "99" ) .

                     
                   v1123 =  STRING(usrw_key2,"99") .
                END.
                ELSE DO:
                    CREATE usrw_wkfl .
                    ASSIGN
                        usrw_key1 = "4s" + STRING(TODAY , "99/99/99" ) + v1118
                ...
                END.
            END.
            ELSE IF v1120 = "n" THEN DO:

                 FIND FIRST usrw_wkfl WHERE usrw_key1 = "s" + STRING(TODAY , "99/99/99" ) 
                    + v1118  NO-ERROR .
                IF AVAILABLE usrw_wkfl  THEN DO:
                    usrw_key2 = string(INT(usrw_key2) + 1  , "999" ) .
                    
                   

                   v1123 =  STRING(usrw_key2,"999") .
                END.
                ELSE DO:
                   CREATE usrw_wkfl .
                    ASSIGN
                        usrw_key1 = "s" + STRING(TODAY , "99/99/99" ) + v1118
...
If AVAILABLE ( pt_mstr )  then
       ...
       END.
 /* ss - 111224.1 -b */
       DEFINE VAR v11151 AS CHAR .
       DEFINE VAR v11152 AS CHAR .
       /* ss - 111226.1 -b
       DEFINE VAR v1123 AS CHAR .
       ss - 111226.1 -e */
        FIND FIRST so_mstr WHERE so_nbr = V1105 NO-LOCK NO-ERROR .
        ...
        IF AVAILABLE CODE_mstr THEN DO:
            ...
            v11151 = STRING(DAY(DATE(v1115)) , "99" ) + STRING( MONTH(DATE(v1115) ) , "99") 
                +  STRING(YEAR(DATE(v1115)) , "9999" ) .
           ...
           v11152 = entry(month(date(V1115)),"JAN,FEB,MAR,APR,MAY,JUN,JUL,AUG,SEP,OCT,NOV,DEC") .

           /* ss - 111226.1 -b 
            av9030 = TRIM(STRING(DAY(DATE(v1115)) , "99" )) + TRIM(v11152) + STRING(YEAR(DATE(v1115)) , "9999" )  .
           IF INDEX(ts9030,"$C1") <> 0  THEN DO:
           TS9030 = substring(TS9030, 1, Index(TS9030 , "$C1") - 1) + av9030 
           + SUBSTRING( ts9030 , index(ts9030 ,"$C1") + length("$C1"), LENGTH(ts9030) - ( index(ts9030 ,"$C1" ) + length("$C1") - 1 ) ).
           END.
           ss  - 111226.1 -e */
           /* ss - 111226.1 -b */
           av9030 = TRIM(STRING(DAY(DATE(v1115)) , "99" )) + TRIM(v11152) + STRING(YEAR(DATE(v1115)) , "9999" )  .
       ...
       /* ss - 111224.1 -b */  /* new shipto add serial  */
        /*
        类型（外包装or内包装） + 日期 + 班次
        "s" 内包装三位序号
        "4s" 外包装两位序号

        Usrw_key1 =  ((s) or (4s)) + string(today,"99/99/99") + shift 
        Usrw_key2 = 序号
        */
        /* ss - 111226.1 -b 
         FIND FIRST so_mstr WHERE so_nbr = V1105 NO-LOCK NO-ERROR .
        ...
        /* ss - 111224.1 -b */
         av9030 = trim(trim(V1122) + TRIM(v1123)) .
        /* ss - 111224.1 -e */
