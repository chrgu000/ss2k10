...
/* SO-WO LABLE */
/* Generate date / time  04/10/07 10:59:23 */
...
If avail so_mstr and so_po<>"" then
do:
do ii=1 to length(so_po):
    if substring(so_po,ii,1)="/" then do:
V1106=substring(so_po,1,ii - 1).
Ttstr="A".
End.
End.
If ttstr<>"A" then
...
do:
do ii=1 to length(so_po):
    if substring(so_po,ii,1)="/" then V1112=substring(so_po,ii + 1).
End.
End.
If V1112<>"" then
        V1112 = V1112.
