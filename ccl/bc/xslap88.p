...
/* SO-WO LABLE */
/* Generate date / time  12/11/07 14:37:54 */
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1103 then
        leave V1103L.
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1103 no-box.
...
If AVAILABLE ( wo_mstr ) then
   ...
   /* Additional Labels Format */
     /* START  LINE :1104  銷售訂單[SalesOrder]  */
     V1104L:
     REPEAT:
...
If V1104<>"" then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1104 no-box.
...
If not AVAILABLE ( so_mstr )  then do:
 display skip "訂單不存在" @ WMESSAGE NO-LABEL with fram F1104.
                pause 0 before-hide.
                Undo, retry.
...
If V1105<>"" then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1105 no-box.
...
If avail so_mstr and so_po<>"" then do:
	V1106=so_po.
	do iii=1 to length(so_po):
	    if substring(so_po,iii,1)="/" then  V1106=substring(so_po,1,iii - 1).
...
If V1106<>"" then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1106 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1108 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1109 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1112 no-box.
...
If avail wo_mstr then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1114 no-box.
...
If  AVAILABLE ( so_mstr )  then
        ...
        leave V1116L.
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1116 no-box.
     ...
     END.
     PV1116 = V1116.
     /* END    LINE :1116  客戶[CUST]  */


   /* Additional Labels Format */
     /* START  LINE :1118  客戶料號[CUST PART]  */
     V1118L:
     REPEAT:
...
End.
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1118 no-box.
     ...
     END.
     PV1118 = V1118.
     /* END    LINE :1118  客戶料號[CUST PART]  */


   /* Additional Labels Format */
     /* START  LINE :1120  批號[Part Lot]  */
     V1120L:
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1120 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1121 no-box.
...
display skip "版本不能為空" @ WMESSAGE NO-LABEL with fram F1114.
                pause 0 before-hide.
                Undo, retry.
...
do kkb=1 to length(so_rmks):
    if substring(so_rmks,kkb,1)="/" then V1122=substring(so_rmks,kkb + 1).
End.
End.
If length(V1122)>4 then assign V1122=substring(V1122,5).
If V1122<>"" then
        V1122 = V1122.
        V1122 = ENTRY(1,V1122,"@").
        /* --FIRST TIME DEFAULT  VALUE -- END  */
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1122 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1124 no-box.
     ...
     END.
     PV1124 = V1124.
     /* END    LINE :1124  生產日期[MF Date]  */


   /* Additional Labels Format */
     /* START  LINE :1126  生產線[prod line]  */
     V1126L:
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1126 no-box.
...
display skip "長度不能超過2位" @ WMESSAGE NO-LABEL with fram F1126.
                pause 0 before-hide.
                Undo, retry.

End.
        IF not V1116<>"" THEN DO:
                display skip "生產線不能為空" @ WMESSAGE NO-LABEL with fram F1126.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1128 no-box.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        IF not (V1128="d" or V1128="n") THEN DO:
                display skip "班次隻能是D或N" @ WMESSAGE NO-LABEL with fram F1128.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1129L.
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1129 no-box.
   ...
   IF NOT (V1129="Y" ) THEN LEAVE V1130LMAINLOOP.
     /* START  LINE :1130  標簽類型[label type]  */
     V1130L:
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1130 no-box.
                ...
                /* LABEL 4 - START */ 
                L11304 = "n-內箱標簽w-外箱標簽p-packing list" .
	 ...
	 If search(LabelsPath + "lap88" + trim ( V1130 ) ) = ? Then do:
	                 display skip "文件不存在,請重新輸入." @ WMESSAGE NO-LABEL with fram F1130.
                         pause 0 before-hide.
                         Undo, retry.
	 End.
Wtm_fm = V1130.
        IF not (V1130="n" or V1130="w" or V1130="p") THEN DO:
     ...
     END.
     PV1130 = V1130.
     /* END    LINE :1130  標簽類型[label type]  */


   /* Additional Labels Format */
     /* START  LINE :1131  ship_wt  */
     V1131L:
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        if trim(V1130)="N" then do:
	find first pt_mstr where pt_part=V1114 no-lock no-error.
	If avail pt_mstr then assign V1131=string(pt_ship_wt).
end.
if trim(V1130)="W" then do:
	find first pt_mstr where pt_part=V1114 no-lock no-error.
	If avail pt_mstr then assign V1131=string(pt_size).
end.
if V1131<>"" then
        V1131 = V1131.
        V1131 = ENTRY(1,V1131,"@").
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1131 no-box.
        ...
        /* PRESS e EXIST CYCLE */
        IF V1131 = "e" THEN  LEAVE V1130LMAINLOOP.
...
If avail so_mstr then
        ...
        leave V1133L.
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1133 no-box.
...
If V1134<>"" then
        ...
        leave V1134L.
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1134 no-box.
...
display skip "Serial不能為空" @ WMESSAGE NO-LABEL with fram F1134.
                pause 0 before-hide.
                Undo, retry.
...
Else V1135="".
        ...
        leave V1135L.
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1135 no-box.
...
Else V1136="".
        ...
        leave V1136L.
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1136 no-box.
...
Else V1137=V1109.
        ...
        leave V1137L.
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1137 no-box.
   ...
   /* Additional Labels Format */
	...
	V1338 = fill( "0" , 4 - length (  trim (V1121) ) ) + trim (V1121) + trim ( WeekResult ) + ( if substring ( trim (V1104) ,1,1) = "R" then "R" else "*" ) + trim(V1126) + trim(V1128).
       ...
       {xsgetsoseqno.i}
	V1338 = fill( "0" , 4 - length (  trim (V1121) ) ) + trim (V1121) + trim ( WeekResult ). 
	V1338 = V1338 + Get_SO_SEQ_NO(weekResult,V1114,V1104).

	/* ( if substring ( trim (V1105) ,1,1) = "R" then "R" else "*" ) */ 
	
	V1338 = V1338 + trim(V1126) + trim(V1128).
...
If V1338<>"" then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1338 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9020 no-box.
   ...
   wtm_num = V9020.
   /* Additional Labels Format */
     /* START  LINE :9025  標簽額外客式[Label Format]  */
     V9025L:
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9025 no-box.
     ...
     END.
     PV9025 = V9025.
     /* END    LINE :9025  標簽額外客式[Label Format]  */
...
If AVAILABLE ( upd_det ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[NEW成品標簽打印]"   + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9030 no-box.
