...
/* ss - 110531.1 by: jack */ /* shced po not control */
/* ss - 110926.1 by: jack */  /* modify lot and location 修改批号生成逻辑，是否检验获取零件判断，自动覹入库位判断 */
/* ss - 111104.1 by: jack */  /* 修改条码打印内容 */
...
/* ss - 111013.1 -b */
  DEFINE VAR v_vat AS LOGICAL INITIAL NO. /* tax_in  */
/* ss - 111013.1 -e */
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
...
If AVAILABLE (po_mstr) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1101 no-box.
...
If AVAILABLE (gl_ctrl) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1102 no-box.
...
If AVAILABLE (po_mstr) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1103 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1203 no-box.
   ...
   V1205LMAINLOOP:
   REPEAT:
     /* START  LINE :1205  粒划砐棒 SKIP ONLY ONE  */
     V1205L:
     REPEAT:
...
If AVAILABLE (pod_det) and string ( pod_line ) = V1205 then
        leave V1205L.
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1205 no-box.
            ...
            string ( POD_LINE ) @ V1205 trim(POD_PART) + "*" + String( POD_DUE_DATE) + "*" + string ( if ( pod_qty_ord - pod_qty_rcvd ) > 0 then ( pod_qty_ord - pod_qty_rcvd ) else 0 ) @ WMESSAGE NO-LABEL with fram F1205.
                ...
                ss - 110531.1 -e */
                  ...
                  IF AVAILABLE pod_Det THEN DO: 
                      IF pod_sched = NO then display skip 
                  string ( POD_LINE ) @ V1205 trim(POD_PART) + "*" + String( POD_DUE_DATE) + "*" + string ( if ( pod_qty_ord - pod_qty_rcvd ) > 0 then ( pod_qty_ord - pod_qty_rcvd ) else 0 ) @ WMESSAGE NO-LABEL with fram F1205.
              
                     ELSE
                         display skip 
                  string ( POD_LINE ) @ V1205 trim(POD_PART)  @ WMESSAGE NO-LABEL with fram F1205.
...
pod_site = V1002 AND string( POD_LINE ) = Trim(V1205) and index ( "XC",pod_status ) = 0  and pod_type = "S"  NO-ERROR NO-WAIT.
     ...
     END.
     PV1205 = V1205.
     /* END    LINE :1205  粒划砐棒 SKIP ONLY ONE  */


   /* Additional Labels Format */
     /* START  LINE :1300  蹋?[Raw Material]  */
     V1300L:
     REPEAT:
...
If AVAILABLE (pod_det) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE (po_mstr) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1301 no-box.
...
If AVAILABLE (pod_det) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1302 no-box.
...
If AVAILABLE (pod_det) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1303 no-box.
...
If NOT AVAILABLE (vp_mstr) THEN
        leave V1304L.
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1304 no-box.
   ...
   /* Additional Labels Format */
     /* START  LINE :1305  INSP_LOC剒溃蹋?苏?踱弇-偌莉?盄  */
     V1305L:
     REPEAT:
...
If AVAILABLE (poc_ctrl) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1305 no-box.
     ...
     END.
     PV1305 = V1305.
     /* END    LINE :1305  INSP_LOC剒溃蹋?苏?踱弇-偌莉?盄  */


   /* Additional Labels Format */
     /* START  LINE :1306  PT_LOC 轿溃蹋?苏?踱弇-偌莉?盄  */
     V1306L:
     REPEAT:
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1306 no-box.
     ...
     END.
     PV1306 = V1306.
     /* END    LINE :1306  PT_LOC 轿溃蹋?苏?踱弇-偌莉?盄  */
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1307 no-box.
   ...
   /* Additional Labels Format */
     /* START  LINE :1309  汜莉??(Create LOT) - CANCEL  */
     V1309L:
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1309 no-box.
     ...
     END.
     PV1309 = V1309.
     /* END    LINE :1309  汜莉??(Create LOT) - CANCEL  */
...
If AVAILABLE (CODE_MSTR) then V1305 = "".
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
...
If AVAILABLE (CODE_MSTR) then
     ...
     REPEAT:
        ...
        /* --DEFINE VARIABLE -- END */
        /* ss - 110926.1 -b */
        DEFINE VAR v_insp AS LOGICAL INITIAL NO .
        /* ss - 110926.1 -e */
        find first POD_DET where POD_NBR = V1100  AND string( POD_LINE ) = Trim(V1205) NO-LOCK NO-ERROR NO-WAIT.
         FIND FIRST pt_mstr WHERE pt_part = v1300 NO-LOCK NO-ERROR .

        FIND FIRST ptp_det WHERE ptp_site = v1002 AND ptp_part = v1300 NO-LOCK NO-ERROR .
        ...
        IF v_insp = NO  THEN DO:
            /* tax_in */
           FIND FIRST CODE_mstr WHERE CODE_fldname = "codevat" + pt_loc   NO-LOCK NO-ERROR .
            IF pod_taxable  THEN DO:
              /* ss - 111013.1 -b */
                ...
                ELSE DO:
                    v1400 = "" .
                      display skip "库位错误,重试." @ WMESSAGE NO-LABEL with fram F1400.
                        pause 0 before-hide.
                        LEAVE MAINLOOP .
                END.
            END.
        ...
        ELSE DO: /* v_insp = yes */
           /* ss - 110926.2 -b */
            ...
            DEFINE VAR v_vat AS LOGICAL INITIAL NO. /* tax_in  */
            ss - 111013.1 -e */
            FIND FIRST CODE_mstr WHERE CODE_fldname = "vatsite" AND CODE_value = v1002 NO-LOCK NO-ERROR .
            IF AVAILABLE CODE_mstr THEN DO:
                v_vat = YES .
            END.
            ELSE DO:
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
     ...
     /* END    LINE :1410  L Control  */
      ...
      /* ss - 110926.1 -b */
      /* Additional Labels Format */
     /* START  LINE :1411 L Control  */
     V1411L:
     REPEAT:

        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V1411           as LOGICAL INITIAL NO .
        define variable PV1411           as LOGICAL.
        define variable L14111          as char format "x(40)".
        define variable L14112          as char format "x(40)".
        define variable L14113          as char format "x(40)".
        define variable L14114          as char format "x(40)".
        define variable L14115          as char format "x(40)".
        define variable L14116          as char format "x(40)".
        /* --DEFINE VARIABLE -- END */
     ...
     /*
        ...
        /* --FIRST TIME SKIP -- START  */
         if sectionid = 1 then leave V1411L.
        /* --FIRST TIME SKIP -- END  */
         ...
         */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1411 no-box.

                /* LABEL 1 - START */ 
                  L14111 = "是否输入批号?" . 
                display L14111          format "x(40)" skip with fram F1411 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                  L14112 = "" . 
                display L14112          format "x(40)" skip with fram F1411 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                  L14113 = "" . 
                display L14113          format "x(40)" skip with fram F1411 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                  L14114 = "" . 
                display L14114          format "x(40)" skip with fram F1411 no-box.
                /* LABEL 4 - END */ 
                display "输入或按E退出 "      format "x(40)" skip
        skip with fram F1411 no-box.
        /* DISPLAY ONLY */
        define variable X1411            as LOGICAL.
        X1411 = V1411.
       
        /* DISPLAY ONLY */
        Update V1411
        WITH  fram F1411 NO-LABEL
        EDITING:
          readkey pause wtimeout.
          if lastkey = -1 Then quit.
        if LASTKEY = 404 Then Do: /* DISABLE F4 */
        ...
        /* PRESS e EXIST CYCLE */
      /*  IF V1411 = "e" THEN  LEAVE V1205LMAINLOOP. */
        display  skip WMESSAGE NO-LABEL with fram F1411.

         /*  ---- Valid Check ---- START */

        display "...PROCESSING...  " @ WMESSAGE NO-LABEL with fram F1411.
        pause 0.
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
         /*  ---- Valid Check ---- END */

        display  "" @ WMESSAGE NO-LABEL with fram F1411.
        pause 0.
        leave V1411L.
     END.
     PV1411 = V1411.
     /* END    LINE :1411  L Control  */
     /* ss - 110926.1 -e */

/* ss - 110926.1 -b
   /* Additional Labels Format */
     /* START  LINE :1500  蚕疡[LOT]  */
     V1500L:
     REPEAT:
...
IF 1 = 1 then
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
...
\        IF V1411  = NO then do:
        ...
        /* ss - 110926.2 -b */
                 {xsglot21.i}
                 IF v_go = NO THEN DO:
                      display skip "错误,重试." @ WMESSAGE NO-LABEL with fram F1500.
                        pause 0 before-hide.
                       LEAVE MAINLOOP .
                 END.
                 /* ss - 110926.2 -e */
...
If AVAILABLE (wo_mstr) and V1410 = "L" AND V1306 <> "" THEN
        leave V1501L.
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1501 no-box.
   ...
   /* Additional Labels Format */
     /* START  LINE :1550  等弇遥呾掀瞰[UM FACTOR]  */
     V1550L:
     REPEAT:
...
If AVAILABLE (pod_det) and pod_um_conv = 1 then
        leave V1550L.
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1550 no-box.
...
If AVAILABLE (pt_mstr) then
     ...
     END.
     PV1550 = V1550.
     /* END    LINE :1550  等弇遥呾掀瞰[UM FACTOR]  */


   /* Additional Labels Format */
     /* START  LINE :1558  ?船缀郔嗣彶亿杅 (硐讽秶杅讲?船,踢涂?船=郔湮)  */
     V1558L:
     REPEAT:
...
If AVAILABLE (poc_ctrl) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1558 no-box.
...
If AVAILABLE (poc_ctrl) then
     ...
     END.
     PV1558 = V1558.
     /* END    LINE :1558  ?船缀郔嗣彶亿杅 (硐讽秶杅讲?船,踢涂?船=郔湮)  */


   /* Additional Labels Format */
     /* START  LINE :1600  杅讲[QTY](粒划等UM)  */
     V1600L:
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
...
If AVAILABLE (pod_det) and pod_um_conv = 1 then
        leave V1610L.
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1610 no-box.
...
If AVAILABLE (pt_mstr) THEN
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
...
If AVAILABLE (wo_mstr) and V1410 = "L" AND V1306 <> "" THEN
        leave V1702L.
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1702 no-box.
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
...
IF 1 = 1 THEN
        leave V9005L.
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9005 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
...
If AVAILABLE (pod_det) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.
...
If AVAILABLE (pod_det) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        leave V9130L.
        /* LOGICAL SKIP END */
                display "[采购收货(委外)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
...
If AVAILABLE ( pt_mstr )  then
       ...
       av9130 = IF SUBSTRING(v1500,LENGTH(v1500) , 1) = "v" THEN "vat" ELSE "" .
       IF INDEX(ts9130,"$v") <> 0  THEN DO:
       TS9130 = substring(TS9130, 1, Index(TS9130 , "$v") - 1) + av9130 
       + SUBSTRING( ts9130 , index(ts9130 ,"$v") + length("$v"), LENGTH(ts9130) - ( index(ts9130 ,"$v" ) + length("$v") - 1 ) ).
       END.
       ...
       /* ss - 111104.1 -e */
        ...
        ss - 111104.1 -e */
        /* ss - 111104.1 -b */
        av9130 = if length( trim ( V1500 ) ) >= 8 then substring ( trim ( V1500 ),4,1) else "0".
       /* ss - 111104.1 -e */
