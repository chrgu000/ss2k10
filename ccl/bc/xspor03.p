...
If AVAILABLE (pt_mstr) then
                ...
                find first pt_mstr where pt_part = V1300 no-lock no-error.
If AVAILABLE (pt_mstr) then
                L13003 = "保质期:" + trim ( string ( if pt_avg_int = 90 then 0 else pt_avg_int ) ) + "月" .
                else L13003 = "" . 
...
If AVAILABLE (po_mstr) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[采购收货(严控)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1301 no-box.
...
If NOT AVAILABLE (vp_mstr) THEN
        leave V1304L.
        /* LOGICAL SKIP END */
                display "[采购收货(严控)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1304 no-box.
...
If AVAILABLE ( pt_mstr ) then
     ...
     /*
         ...
         */
                ...
                display "[采购收货(严控)]"    + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1411 no-box.
