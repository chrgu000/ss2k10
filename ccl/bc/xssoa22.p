...
/* SO SHIPMENT */
/* Generate date / time  2007-1-9 11:00:55 */
...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xssoa22wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
...
If AVAILABLE ( so_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1105 no-box.
...
If AVAILABLE (sod_det) and string ( sod_line ) = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1400.
            ...
            THEN DO:
                  IF recid(LD_DET) = ? THEN find first LD_DET where 
                              LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_SITE = V1002 AND LD_REF = V1100 AND  
                              LD_LOC >=  INPUT V1400
                               no-lock no-error.
                  ELSE find next LD_DET where 
                              LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_SITE = V1002 AND LD_REF = V1100  
                               no-lock no-error.
                  IF AVAILABLE LD_DET then display skip 
            LD_LOC @ V1400 ( if ld_status = "SO-CHECK" then "已檢查:" else "未檢查:" ) + LD_LOT + "/" + trim(string(LD_QTY_OH)) @ WMESSAGE NO-LABEL with fram F1400.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1500.
            ...
            THEN DO:
                  IF recid(LD_DET) = ? THEN find first LD_DET where 
                              LD_PART = V1300 AND LD_LOC = V1400 AND LD_QTY_OH <> 0 AND LD_SITE = V1002 AND LD_REF = V1100 AND  
                              LD_LOT >=  INPUT V1500
                               no-lock no-error.
                  ELSE find next LD_DET where 
                              LD_PART = V1300 AND LD_LOC = V1400 AND LD_QTY_OH <> 0 AND LD_SITE = V1002 AND LD_REF = V1100  
                               no-lock no-error.
                  IF AVAILABLE LD_DET then display skip 
            LD_LOT @ V1500 ( if ld_status = "SO-CHECK" then "已檢查:" else "未檢查:" ) + LD_LOT + "/" + trim(string(LD_QTY_OH)) @ WMESSAGE NO-LABEL with fram F1500.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where LD_SITE = V1002 AND LD_LOC  = V1400 AND LD_PART = V1300 AND
LD_LOT  = ( IF INDEX(V1500,"@" ) <> 0 then ENTRY(2,V1500,"@") else V1500 ) AND
LD_REF  = V1100  no-lock no-error.
        IF NOT AVAILABLE LD_DET then do:
                display skip "該LOT備料有誤" @ WMESSAGE NO-LABEL with fram F1500.
                pause 0 before-hide.
                undo, retry.
        end.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.

                /* LABEL 1 - START */ 
                L17001 = "訂單/項:" + trim ( V1100 )  + "/" + trim ( V1110 ) .
...
If AVAILABLE ( ld_det ) then
                L17004 = "數量:" + trim ( V1400 ) + "/" + string ( ld_qty_oh ) + ( if ld_status = "SO-CHECK" then "**" else "" ) .
                else L17004 = "" . 
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品檢查]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
...
tr_date = today     and 
tr_trnbr > integer ( V9000 ) and 
tr_type = "RCT-CHL"  and  tr_site = V1002     and tr_part = V1300     and tr_serial = V1500   and 
tr_time  + 15 >= TIME 
use-index tr_date_trn no-lock no-error.
If AVAILABLE ( tr_hist ) then
