...
/* Generate date / time  2007-2-27 15:44:28 */
...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xssoa23wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
END FUNCTION.
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
...
If AVAILABLE ( so_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1105 no-box.
   ...
   /* Additional Labels Format */
     /* START  LINE :1203  預計發貨日期[SHIPPMENT DATE]  */
     V1203L:
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1203 no-box.
     ...
     END.
     PV1203 = V1203.
     /* END    LINE :1203  預計發貨日期[SHIPPMENT DATE]  */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE (sod_det) and string ( sod_line ) = V1305 then
        leave V1305L.
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1305 no-box.
...
If NOT AVAILABLE (lad_det) THEN
        leave V1309L.
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1309 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
...
If MinLot <> ""  then
                L15003 = "庫存/SO:" + trim(string(MinQty)) + "/" +  (If AVAILABLE ( sod_det ) then string ( sod_qty_ord - sod_qty_ship ) else "0"  ) .
                else L15003 = "" . 
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1500.
			...
			find first LD_DET where 
                              LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_SITE = V1002 AND ( LD_REF = "" OR LD_REF = V1100) AND  
                              LD_LOT >=  INPUT V1500
                               no-lock no-error.
                  ELSE find next LD_DET where 
                              LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_SITE = V1002 AND ( LD_REF = "" OR LD_REF = V1100)  
                               no-lock no-error.
                  IF AVAILABLE LD_DET then display skip 
            LD_LOT @ V1500 (IF LD_REF = "" THEN "可" ELSE "已" ) + "備SO/Q:" + GETSONBR(LD_LOT,V1300) + "/" + trim(string(LD_QTY_OH)) + "/" + trim ( LD_LOC ) @ WMESSAGE NO-LABEL with fram F1500.
                  else   do :
			display skip "" @ WMESSAGE with fram F1500.
			if recno = ? then do:
				find last ld_det where LD_QTY_OH <> 0 AND ld_site = V1002 AND (LD_REF = "" OR LD_REF = V1100) AND
				LD_LOT <= INPUT V1500
				no-lock no-error.
			end.
			else
			if recno <> ? then
				find ld_det where recno = recid(ld_det) no-lock no-error.
			if available ld_det then display skip
			LD_LOT @ V1500 (IF LD_REF = "" THEN "可" ELSE "已" ) + "備SO/Q:" + GETSONBR(LD_LOT,V1300) + "/" + trim(string(LD_QTY_OH)) + "/" + trim ( LD_LOC ) @ WMESSAGE NO-LABEL with fram F1500.
			input clear.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1505 no-box.
                ...
                /* LABEL 3 - START */ 
                L15053 = "訂單/項:" + trim ( V1100 ) + "/" + trim ( V1305 ) .
            ...
            THEN DO:
                  ...
                  ELSE find next LD_DET where 
                              LD_PART = V1300 AND LD_LOT =  V1500 AND LD_QTY_OH <> 0  AND LD_SITE = V1002 AND ( LD_REF = "" OR LD_REF = V1100)  
                               no-lock no-error.
                  IF AVAILABLE LD_DET then display skip 
            LD_LOC @ V1505 (IF LD_REF = "" THEN "可" ELSE "已" ) + "備L/Q:" + LD_LOT + "/" + trim(string(LD_QTY_OH)) @ WMESSAGE NO-LABEL with fram F1505.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LOC_MSTR where LOC_LOC = V1505 AND LOC_SITE = V1002  no-lock no-error.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
  ...
  DECIMAL ( V1600 ) > 0 and ld_ref = "" and ld_qty_oh >= DECIMAL ( V1600 )
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
                ...
                /* LABEL 3 - START */ 
                L17003 = "庫位:" + trim ( V1505 ) + "/" + trim( V1600 ) .
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
...
IF 1 = 1 THEN
        leave V9005L.
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9005 no-box.
...
IF 1 = 2 THEN
        leave V9010L.
        /* LOGICAL SKIP END */
                display "[銷售備料LOT]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
