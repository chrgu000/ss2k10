...
/* Generate date / time  2007-6-19 14:41:38        */
/* 140109.1  - Start
 * 单据号码（规则为WS+年份的最后2个数字+5位数字的流水号+原因代码，
 * 例如：WS1300001-01，需要判断单据号后两位是否在原因代码中存在，
 * 原因代码会在36.2.17中原因代码中维护）
**/
...
define variable Lchk as logical initial no.
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
 ...
 if lastkey = -1 Then quit.
  ...
  apply lastkey.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
       ...
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
  ...
  undo, retry.
   ...
   APPLY LASTKEY.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first WO_MSTR no-lock where WO_NBR = V1100 AND INDEX("AR",WO_STATUS) <> 0 AND WO_SITE = V1002  NO-ERROR NO-WAIT.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
...
If AVAILABLE ( wo_mstr ) then
  ...
  undo, retry.
   ...
   APPLY LASTKEY.
    ...
    assign V1100 = wo_nbr when V1100 = "".
        end.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- START  */
         if sectionid > 1 then leave V1190L .
        /* --CYCLE TIME SKIP -- END  */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1190 no-box.
 ...
 *           CODE_FLDNAME = "RCT-UNPREASON" AND
 *           CODE_VALUE >=  INPUT V1190
 *   no-lock no-error.
 *        ELSE find next CODE_MSTR where
 *           CODE_FLDNAME = "RCT-UNPREASON"
 *   no-lock no-error.
 *        IF AVAILABLE CODE_MSTR then display skip
 *  CODE_VALUE @ V1190 TRIM(CODE_CMMT) @ WMESSAGE NO-LABEL with fram F1190.
 *        else   display skip "" @ WMESSAGE with fram F1190.
 *  END.
 *  IF LASTKEY = keycode("F9") or keyfunction(lastkey) = "CURSOR-UP"
 *  THEN DO:
 *        IF recid(CODE_MSTR) = ? THEN find first CODE_MSTR where
 ...
 * 140109.1  - End */
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
/* 140109.1  - Start
 *  find first CODE_MSTR where CODE_FLDNAME = "RCT-UNPREASON" AND index ( V1190,CODE_VALUE ) <> 0  no-lock no-error.
 *  IF NOT AVAILABLE CODE_MSTR then do:
 * display skip "原因代码有误" @ WMESSAGE NO-LABEL with fram F1190.
 * pause 0 before-hide.
 * undo, retry.
 *  end.
 * 140109.1  - End */
 /* 140109.1  - Start  */
        if substring(V1190,1,2) <> "WS" then do:
           display skip "单据号码有误非WS" @ WMESSAGE NO-LABEL with fram F1190.
           pause 0 before-hide.
           undo, retry.
        end.
        if substring(V1190,3,2) <> substring(string(year(today),"9999"),3,2) then do:
           display skip "单据号中年份有误" @ WMESSAGE NO-LABEL with fram F1190.
           pause 0 before-hide.
           undo, retry.
        end.
        assign Lchk = no.
        DO i = 1 to 5.
           If index("0987654321.", substring(V1190, 4 + i ,1)) = 0 then do:
              assign Lchk = yes.
              leave.
           end.
        end.
        if Lchk then do:
           display skip "单据号流水号有误" @ WMESSAGE NO-LABEL with fram F1190.
           pause 0 before-hide.
           undo, retry.
        end.
        find first rsn_ref where rsn_type = "WOISSUE" and
                   rsn_code = trim(substring(V1190,length(trim(v1190)) - 1))
                   no-lock no-error.
        ...
        IF not length(trim(V1190)) <= 18 THEN DO:
           display skip "原因代码长度有误" @ WMESSAGE NO-LABEL with fram F1190.
           pause 0 before-hide.
           undo, retry.
        end.
/* 140109.1  - End  */        
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
       ...
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 no-box.
...
If AVAILABLE ( code_mstr ) then
        leave V1203L.
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1203 no-box.
 ...
 if lastkey = -1 Then quit.
  ...
  apply lastkey.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
  ...
  undo, retry.
   ...
   APPLY LASTKEY.
       ...
       display skip "该料品不在WO清单内!" @ WMESSAGE NO-LABEL with fram F1300.
       pause 0 before-hide.
       undo, retry.
        end.
         /*  ---- Valid Check ---- END */
...
If AVAILABLE ( pt_mstr ) then
       ...
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
       ...
       /* LABEL 2 - START */
       find first pt_mstr where pt_part = V1300  no-lock no-error.
         If AVAILABLE ( pt_mstr ) then L14002 = pt_loc .
                                  else L14002 = "" .
...
If AVAILABLE ( pt_mstr ) then
       ...
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
  ...
  apply lastkey.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
  ...
  undo, retry.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1503L.
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1503 no-box.
...
If AVAILABLE ( ld_det ) then
       ...
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1550 no-box.
  ...
  apply lastkey.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
 ...
 if lastkey = -1 Then quit.
  ...
  apply lastkey.
       ...
       undo, retry.
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where ld_part = V1300 AND ld_site = V1002 AND ld_ref = "" AND   ld_loc = V1400 and
ld_lot = V1500 and  ld_QTY_oh >= DECIMAL ( V1600 )  no-lock no-error.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
...
/*         V9000 = ENTRY(1,V9000,"@").           */
        V9000 = string(current-value(tr_sq01)).
        /* --FIRST TIME DEFAULT  VALUE -- END  */
       ...
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
...
tr_site = V1002 and tr_loc = v1400 and  tr_lot  = V1110     and tr_part = V1300 and tr_serial = V1500
use-index tr_trnbr exclusive-lock no-error.
If AVAILABLE ( tr_hist ) then do:
  assign tr__chr01 = V1550
         tr__chr02 = V1190.
end.
If 1 = 1 then
        leave V9005L.
       ...
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9005 no-box.
...
IF 1 = 2 THEN
        leave V9010L.
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
...
tr_nbr  = V1100     and  tr_type = "ISS-WO" and
tr_site = V1002     and tr_loc = v1400   and tr_lot  = V1110 and tr_part = V1300 and tr_serial = V1500
use-index tr_trnbr no-lock no-error.
If AVAILABLE ( tr_hist ) then
       do:
          L90101 = "交易已提交" .
  ...
  apply lastkey.
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.
  ...
  apply lastkey.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
...
If AVAILABLE ( upd_det ) then
       ...
       display "[工单超发]" + version + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
  ...
  undo, retry.
   ...
   /* Additional Labels Format */
     ...
     INPUT FROM VALUE(LabelsPath + "woi10" + trim ( wtm_fm ) ).
