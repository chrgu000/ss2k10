...
/* Generate date / time  2007-2-27 15:43:20 */
...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xswor01wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1005 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1100.
            ...
            THEN DO:
                  IF recid(WO_MSTR) = ? THEN find first WO_MSTR where 
                              WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0 AND  ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" OR substring ( WO_SO_JOB ,1,3) = "OBS" OR substring ( WO_SO_JOB ,1,2) = "RM"  ) AND  
                              WO_NBR >=  INPUT V1100
                               no-lock no-error.
                  ELSE find next WO_MSTR where 
                              WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0 AND  ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" OR substring ( WO_SO_JOB ,1,3) = "OBS" OR substring ( WO_SO_JOB ,1,2) = "RM"   )  
                               no-lock no-error.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first WO_MSTR where WO_NBR = V1100 AND WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" OR substring ( WO_SO_JOB ,1,3) = "OBS" OR substring ( WO_SO_JOB ,1,2) = "RM"   )  NO-ERROR NO-WAIT.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
...
If AVAILABLE ( wo_mstr ) then
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1110.
            ...
            THEN DO:
                  IF recid(wo_mstr) = ? THEN find first wo_mstr where 
                              wo_site = V1002 AND wo_nbr = V1100 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" OR substring ( WO_SO_JOB ,1,3) = "OBS" OR substring ( WO_SO_JOB ,1,2) = "RM"   ) AND  
                              wo_lot >=  INPUT V1110
                               no-lock no-error.
                  ELSE find next wo_mstr where 
                              wo_site = V1002 AND wo_nbr = V1100 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" OR substring ( WO_SO_JOB ,1,3) = "OBS" OR substring ( WO_SO_JOB ,1,2) = "RM"   )  
                               no-lock no-error.
                  IF AVAILABLE wo_mstr then display skip 
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first wo_mstr where wo_site = V1002 AND wo_lot = V1110 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" OR substring ( WO_SO_JOB ,1,3) = "OBS" OR substring ( WO_SO_JOB ,1,2) = "RM"  )  NO-ERROR NO-WAIT.
        IF NOT AVAILABLE wo_mstr then do:
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        find first code_mstr where code_fldname = "BARCODEFIELDDISABLE" and code_value = "WOR01_V1203" no-error.
If AVAILABLE ( code_mstr ) then
        leave V1203L.
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1203 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first WO_MSTR where WO_LOT = V1110 AND WO_PART = ENTRY(1, V1300, "@")  no-lock no-error.
        IF NOT AVAILABLE WO_MSTR then do:
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
...
If AVAILABLE ( wo_mstr ) then
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where ( decimal(V1600) > 0 ) OR ( decimal(V1600) < 0 AND (ld_site = V1002 and ld_ref = "" AND ld_part  = V1300 AND ld_loc = V1400 and ld_lot = V1500 and  ld_QTY_oh + DECIMAL ( V1600 ) >= 0 ) )  no-lock no-error.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
