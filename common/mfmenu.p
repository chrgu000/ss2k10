...
define buffer qadwkfl0 for qad_wkfl.
assign
      ...
      repeat on endkey undo block1 , leave block1:
         ...
         do on error undo with frame bb:
            ...
            then do:
               ...
               /* See if they are trying to run something only avail in AppShell */
               if not available mnd_det and cselection <> "" then
               for first mnd_det
                  where mnd_det.mnd_uri = cselection
               no-lock :
                  /* Menu Option Only available thru App Shell */
                  {pxmsg.i &MSGNUM=1621 &ERRORLEVEL=3}
            ...
            then do:
               ...
               if dtitle <> "" then do:
                  find first qadwkfl0 exclusive-lock
                       where qadwkfl0.qad_domain = "mfmenu.p.version.reference"
                         and qadwkfl0.qad_key1 = "mfmenu.p.version.reference"
                         and qadwkfl0.qad_key2 = trun_file no-error.
                  if not available qadwkfl0 then do:
                     create qadwkfl0.
                     assign qadwkfl0.qad_domain = "mfmenu.p.version.reference"
                            qadwkfl0.qad_key1 = "mfmenu.p.version.reference"
                            qadwkfl0.qad_key2 = trun_file.
                  end.
                  assign qadwkfl0.qad_key3 = entry(2,dtitle," ")
                         qadwkfl0.qad_key4 = global_userid
                         qadwkfl0.qad_charfld[1] = entry(1,dtitle," ")
                         qadwkfl0.qad_charfld[2] = entry(2,dtitle," ")
                         qadwkfl0.qad_charfld[3] = c-menu-label
                         qadwkfl0.qad_datefld[1] = today
                         qadwkfl0.qad_intfld[1] = time.
                 release qadwkfl0.
               end.
               pause before-hide.
