...
form
   ...
   to_id               label {t001.i}
   skip(1)
   repeat_process      colon 30 view-as toggle-box
   skip(1)
   pause_seconds       colon 30
   skip(1)
   process_file        colon 30 view-as toggle-box
   skip(1)
   file_name           colon 30
   skip(1)
...
find first code_mstr no-lock where
           code_fldname = "mgbdpro" and code_value = global_userid no-error.
if avail code_mstr and not locked(qad_wkfl) then do:
  assign code__qadc01 = string(from_id) + ";"
                      + string(to_id) + ";"
                      + string(repeat_process) + ";"
                      + string(pause_seconds) + ";"
                      + string(process_file) + ";"
                      + file_name.
end.
else do:
  create code_mstr.
  assign code_fldname = "mgbdpro"
         code_value   = global_userid
         code_cmmt    = "CIM_LOAD×°Èë¼ÇÂ¼"
         code__qadc01 = string(from_id) + ";"
                      + string(to_id) + ";"
                      + string(repeat_process) + ";"
                      + string(pause_seconds) + ";"
                      + string(process_file) + ";"
...
do transaction:
   ...
   find first qad_wkfl exclusive-lock where
              qad_key1 = "mgbdpro" and qad_key2 = global_userid no-error.
   if avail qad_wkfl then do:
     assign qad_intfld[1] = vfrom_id when qad_intfld[1] <> vfrom_id
            qad_intfld[2] = vto_id when qad_intfld[2] <> vto_id
            qad_logfld[1] = vrepeat_process
                when (qad_logfld[1] <> vrepeat_process and vrepeat_process)
            qad_intfld[3] = vpause_seconds
                when qad_intfld[3] <> vpause_seconds
            qad_logfld[2] = vprocess_file
                when (qad_logfld[2] <> vprocess_file and vprocess_file)
            qad_charfld[1] = vfile_name
                when (qad_charfld[1] <> vfile_name and vfile_name <> "").
   end.
   else do:
     create qad_wkfl.
     assign qad_key1 = "mgbdpro"
            qad_key2 = global_userid
            qad_intfld[1] = vfrom_id
            qad_intfld[2] = vto_id
            qad_logfld[1] = vrepeat_process
            qad_intfld[3] = vpause_seconds
            qad_logfld[2] = vprocess_file
