/* GUI CONVERTED from pomt.p (converter v1.77) Mon Jan 12 01:49:06 2004 */
...
/*                                                                            */
/* Last Modified by apple on 06/09/03 *Z001* */
/* Last Modified by Hanjia on 07/11/22 *hj--* */
/* Revision: eB2.1SP3  BY: Apple Tam         DATE: 07/26/12  ECO: *SS -20120726.1  */
/* Revision: eB2.1SP3  BY: mel zhao         DATE: 12/05/13  ECO: *SS - 131205.1  */
...
{mfdtitle.i "1+ "}
*/
/*{mfdtitle.i "20120726.1"}*/
{mfdtitle.i "131205.1 "} /* ss - 131205.1 */
/**SS -20120726.1 -E*/
/* Clear anything displayed by mftitle if api mode.*/
{mfaititl.i}
{cxcustom.i "xxPOMT_e.P"}
{pxmaint.i}
...
define new shared variable l_include_retain like mfc_logical
   initial yes no-undo.
/* ss - 131205.1 - b */
define new shared variable v_count as integer initial 0. 
define new shared variable v_old as character.
define new shared variable v_yn like mfc_logical initial no.
define new shared variable v_new as integer initial 0. 
/* ss - 131205.1 - e */
...
/*hj--*/
/*Variables added by Hanjia*/
DEFINE VARIABLE xxponbr LIKE po_nbr . 
DEFINE VARIABLE cimf AS CHAR FORMAT "x(15)" .
cimf = "po" + SUBSTRING(STRING(TODAY),1,2) + SUBSTRING(STRING(TODAY),4,2) + SUBSTRING(string(TIME),1,4)  .

DEFINE VARIABLE jk AS INTEGER INIT 0 .
DEFINE VARIABLE bcurr AS LOGICAL INIT YES .
DEFINE STREAM cim .
DEFINE VARIABLE cim-yn AS LOG INITIAL YES .
def var chk as log initial no.
def var cust as char.
/*******************************/


/*Added by Hanjia--temparary table*/
DEFINE TEMP-TABLE xxpod_cont
    FIELD xxpod_part LIKE pod_part
    FIELD xxpod_um LIKE pod_um
    FIELD xxpod_count AS INTEGER
    FIELD xxpod_total LIKE pod_qty_ord LABEL "TOTAL"
    FIELD xxpod_price LIKE pod_pur_cost LABEL "Cost"
    FIELD xxpod_disc LIKE pod_disc_pct LABEL "Disc"
    FIELD xxpod_um_conv LIKE pod_um_conv 
    FIELD xxpod_user as char label "UserID"
    INDEX xxpod_id xxpod_part .

DEFINE TEMP-TABLE xxdpo_mstr
    FIELD xxdpo_line LIKE pod_line
    FIELD xxdpo_part LIKE pod_part
    FIELD xxdpo_number AS INT label "数量"
    FIELD xxdpo_idiscount LIKE pod_disc_pct LABEL "原折扣"
    FIELD xxdpo_discount LIKE pod_disc_pct LABEL "折扣"
    FIELD xxdpo_iprice LIKE pod_pur_cost LABEL "原价格"
    FIELD xxdpo_price LIKE pod_pur_cost LABEL "价格"
    FIELD xxdpo_taxable LIKE pod_taxable LABEL "taxable"
    FIELD xxdpo_user as char label "UserID"
    INDEX xxdpo_id xxdpo_line .
/**************************************/
/*--hj*/
  ...
  view global-tool-bar-handle. /*GUI*/
   ...
   do transaction on error undo, retry:
      ...
      if not available poc_ctrl then
          do: create poc_ctrl. poc_ctrl.poc_domain = global_domain. end.
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         assign v_old = po_mstr.po_vend. /* ss - 131205.1 */
            ...
            editing:
               ...
               do:
                  ...
                  {mfaddisp.i po_vend vend}
						...
						/* ss - 131205.1 */
						if po_mstr.po_vend <> v_old then
							assign v_count = v_count + 1.

					/*Z001*/

					def var iii as inte.
					def var intvend as char.
					 iii = 0.
					 intvend = "".
					for each xinad_mstr no-lock where xinad_domain = global_domain and xinad_type = "supplier":
						 iii = iii + 1.
						 if iii = 1 then intvend = intvend + xinad_int_code.
						 if iii > 1 then intvend = intvend + "," + xinad_int_code.
					end.
						 if lookup(input po_vend,intvend) <> 0 then do:
								 message "该程序只能维护外部供应商".
								 undo,retry.
						 end.
					/*Z001*/
         ...
         do: /* if c-application-mode ="API" */
            ...
            for first vd_mstr no-lock
                 where vd_mstr.vd_domain = global_domain and  vd_addr = vend:
            end.
...
/*GUI*/ RECT-FRAME-LABEL:SCREEN-VALUE in frame vend = F-vend-title.
      ...
      /* BELOW CODE ADDED FOR DEFAULTING TAX IN       */
      /* FIELD, IN THE PO HEADER FROM SUPPLIER MASTER */
      if not new_po
      then do:
         tax_in = no.
         for first ad_mstr
         fields (ad_domain ad_addr ad_tax_in)
...
/*B001*/     /* {gprun.i ""popomtb.p"" "(input using_supplier_consignment)"}*/ /* ss - 131205.1 */
/*B001*/ /*run D:\gtc\po_mt\5.42\xxpopomtb_e.p (input using_supplier_consignment).*/
        {gprun.i ""xxpopomtb.p"" "(input using_supplier_consignment)"} /* ss - 131205.1 */
/*GUI*/ if global-beam-me-up then undo, leave.
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         /*{gprun.i ""gpcmmt01.p"" "(input ""po_mstr"")"}*/ /* ss - 131205.1 */
				 {gprun.i ""xxgpcmmt01.p"" "(input ""po_mstr"")"} /* ss - 131205.1 */
...
/*GUI*/ if global-beam-me-up then undo, leave.
    ...
    if v_count > 0 then do:
			if po_mstr.po__chr01 = "" then
				assign v_new = 1000 + 001.
			else
				assign v_new = 1000 + int(po_mstr.po__chr01)  + 1.

			update po_mstr.po__chr01 = substring(string(v_new),2,3).
      assign v_yn = yes.
    end.
    /* ss - 131205.1 - e */
   ...
   /* LINE ITEMS */
/*Z001   {gprun.i ""popomta.p""}*/
/*Z001*/ {gprun.i ""xxpopomta_e.p""}
...
IF global-tool-bar AND global-tool-bar-handle ne ? THEN
  ...
  /* {gprun.i ""popomtf.p""} */ /* ss - 131205.1 */
	{gprun.i ""xxpopomtf.p""} /* ss - 131205.1 */

	 
/*GUI*/ if global-beam-me-up then undo, leave.
          

/*B001*/ {gprun.i ""xxpodbak.p"" "(input global_domain,input po_nbr)"}                             
...
/*hj--*/
/*
Operations should be here.
*/

                     
xxponbr = po_nbr .
/*cleaning the temp tables*/
FOR EACH xxpod_cont where xxpod_user = global_userid :
    DELETE xxpod_cont .
END.

FOR EACH xxdpo_mstr where xxdpo_user = global_userid :
    DELETE xxdpo_mstr .
END.

FOR EACH pod_det NO-LOCK WHERE pod_domain = po_domain AND pod_nbr = xxponbr:
   find first pt_mstr where pt_domain =po_domain and pt_part = pod_part no-lock no-error.
   if not avail pt_mstr then next.
    FIND FIRST xxpod_cont WHERE xxpod_part = pod_part NO-ERROR.
    IF AVAIL xxpod_cont THEN DO:
        xxpod_count = xxpod_count + 1 .
    END.
    ELSE DO:
        CREATE xxpod_cont .
        ASSIGN		
            ...
            xxpod_um = pod_um
            xxpod_total = 0
	    xxpod_user = global_userid .
    END.
END.

FOR EACH xxpod_cont 
	WHERE xxpod_count = 1 
	and xxpod_user = global_userid:
    DELETE xxpod_cont .
END.
...
FOR EACH xxpod_cont where xxpod_user = global_userid :
    FOR EACH pod_det WHERE pod_domain = po_domain 
        AND pod_nbr = xxponbr
        AND pod_part = xxpod_part:        
        CREATE xxdpo_mstr.
        ASSIGN 
            xxdpo_line = pod_line
            xxdpo_part = pod_part
            xxdpo_number = pod_qty_ord
            xxdpo_idiscount = pod_disc_pct
            xxdpo_iprice = pod_pur_cost 
            xxdpo_price = xxdpo_iprice 
	    xxdpo_discount = xxdpo_idiscount
            xxdpo_taxable = pod_taxable
	    xxdpo_user = global_userid .
        xxpod_total = xxpod_total + pod_qty_ord .
        xxpod_um_conv = pod_um_conv .
    END.
END.

FOR EACH xxpod_cont where xxpod_user = global_userid :
     for last pc_mstr
            fields( pc_domain pc_amt pc_amt_type pc_curr pc_expire pc_list
                     pc_part pc_start pc_um)
            where pc_mstr.pc_domain = global_domain and (  pc_list      =
                    po_pr_list2                                  AND
                    pc_part      = xxpod_part                       and
                   (pc_um = ""  OR pc_um = xxpod_um)                and
                   (pc_start    <= po_ord_date or pc_start = ?)  and
                   (pc_expire   >= po_ord_date or pc_expire = ?) and
                   (pc_curr      = po_curr OR pc_curr = "")       and
                    pc_amt_type  = "l" ) no-lock:
    end. /* FOR LAST PC_MSTR */

    if available pc_mstr then do:
        xxpod_price = pc_amt[1] .
    END.
    ELSE DO:
        MESSAGE "没有找到L类型的价格单!"  .
    END.
    ...
    for last pc_mstr
	fields( pc_domain pc_amt pc_amt_type pc_curr pc_expire pc_list
                    pc_min_qty pc_part pc_start pc_um)
            where pc_mstr.pc_domain = global_domain and (  pc_list      =
                    po_pr_list                                  AND
                    pc_part      = xxpod_part                       and
                   (pc_um = ""  OR pc_um = xxpod_um)                and
    ...
    if available pc_mstr then do:
        if xxpod_total < 0 then xxpod_total = xxpod_total * -1.

        /* Convert to stock pod_um if we don't have a pod_um match */
         if pc_um <> xxpod_um then xxpod_total = xxpod_total * xxpod_um_conv. 

            do jk = 1 to 15:
               if pc_min_qty[jk] > xxpod_total
            ...
            if jk > 0 then do:
                    xxpod_disc = INT(abs(xxpod_price - pc_amt[jk]) / xxpod_price  * 100000 ) / 1000  .
            END.
    END.
END.

FOR EACH xxpod_cont 
	where xxpod_user = global_userid :
    FOR EACH xxdpo_mstr WHERE xxdpo_part = xxpod_part:
        xxdpo_discount = xxpod_disc .
	xxdpo_price = xxpod_price .
    END.
END.

FOR EACH xxdpo_mstr 
	WHERE xxdpo_idiscount = xxdpo_discount 
	AND xxdpo_iprice = xxdpo_price
	and xxdpo_user = global_userid :
    DELETE xxdpo_mstr .
END.

FOR EACH xxdpo_mstr where xxdpo_user = global_userid:
    DISP xxdpo_line xxdpo_part xxdpo_number 
	xxdpo_iprice xxdpo_price xxdpo_idiscount 
	xxdpo_discount with stream-io width 90.
END.


FIND FIRST gl_ctrl WHERE gl_domain = po_domain NO-ERROR.
IF AVAIL gl_ctrl THEN DO:
    IF gl_base_curr <> po_curr THEN
        ASSIGN bcurr = NO .
	else bcurr = yes .
END.
cust = po_vend .
...
/*hj--*/
/*Added */
FIND FIRST xxdpo_mstr where xxdpo_user = global_userid NO-ERROR.
IF AVAIL xxdpo_mstr and execname<>'xxposp.p' THEN DO:
  if batchrun=no then 
    MESSAGE "此定单有多项零件相同,确认要更新价格和折扣吗?"  
	VIEW-AS ALERT-BOX INFORMAT BUTTONS YES-NO UPDATE cim-yn .
    if cim-yn or batchrun then do:
    OS-DELETE VALUE(cimf + ".cim") . 
    OUTPUT STREAM cim TO VALUE(cimf + ".cim") NO-ECHO .
    PUT STREAM cim UNFORMATTED  '@@BATCHLOAD popomt.p' SKIP. 
    PUT STREAM cim UNFORMATTED """" xxponbr """" SKIP.
    PUT STREAM cim UNFORMATTED "-" SKIP.
    PUT STREAM cim UNFORMATTED "-" SKIP.
    PUT STREAM cim UNFORMATTED "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "no" SKIP.
    for first pod_det fields(pod_nbr pod_qty_rcvd) 
		where pod_domain=global_domain and  pod_nbr = xxponbr and pod_qty_rcvd <> 0 :
	end .
	if avail pod_det then bcurr = yes .
    IF bcurr = NO THEN DO:
        PUT STREAM cim UNFORMATTED "- "  SKIP .
    END.
    PUT STREAM cim UNFORMATTED "- " SKIP.
    ...
    FOR EACH xxdpo_mstr where xxdpo_user = global_userid
        BY xxdpo_line: 
        PUT STREAM cim UNFORMATTED """" xxdpo_line """" SKIP.
	for first pod_det fields(pod_nbr pod_line pod_qty_rcvd) 
		where pod_domain=global_domain and pod_nbr = xxponbr and pod_line = xxdpo_line 
		and pod_qty_rcvd <> 0 :
	end .
	if not avail pod_det then 
		PUT STREAM cim UNFORMATTED "-" SKIP.
	PUT STREAM cim UNFORMATTED "-" SKIP.
        PUT STREAM cim UNFORMATTED """" xxdpo_price """" " " .
        PUT STREAM cim UNFORMATTED """" xxdpo_discount """" SKIP .
	PUT STREAM cim UNFORMATTED "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "- " "no" SKIP. 
        IF xxdpo_taxable = YES THEN DO:
            PUT STREAM cim UNFORMATTED "- " SKIP .
        END.
	find first vp_mstr no-lock where vp_domain=global_domain and vp_part = xxdpo_part and vp_vend = cust no-error .
	if avail vp_mstr then PUT STREAM cim UNFORMATTED "no" SKIP.
    END.
    ...
    define var m_data as char format "x(80)" no-undo. 
    define var m_prg as char format "x(80)" no-undo.
    ...
    for each xxdpo_mstr where xxdpo_user = global_userid :
    	find first pod_det 
    		...
    		and pod_nbr = xxponbr
    		and pod_line = xxdpo_line 
    		and pod_part = xxdpo_part
    		and pod_pur_cost = xxdpo_price
    		and pod_disc_pct = xxdpo_discount no-error.
    	if not avail pod_det then chk = yes .
    end.
    if chk then do: 
    	MESSAGE "价格更新过程有错误!请检查" + cimf + ".log文件!"
    		VIEW-AS ALERT-BOX INFORMAT  . 
    end.
    else do:
         OS-DELETE VALUE(cimf + ".cim") .    
         OS-DELETE VALUE(cimf + ".log") .
    end.


    end.  /* if cim-yn end */

END.  /*if avail xxdpo_mstr*For cimloading*/
   ...
   then do:
      ...
      editing:
         ...
         if blanket = false then do:
            ...
            {mfnp01.i po_mstr po_nbr po_nbr po_type  " po_mstr.po_domain =
...
------------------------------------------------------------------------------*/
   ...
   do transaction on error undo, retry:
      ...
      for first mfc_ctrl  where mfc_ctrl.mfc_domain = global_domain and
      mfc_field = "poc_pc_line"
      no-lock:
      end.
