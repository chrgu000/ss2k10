...
FOR EACH acd_det NO-LOCK
    ...
    FIND FIRST ac_mstr WHERE ac_domain = "dcec" AND ac_code = acd_acc NO-LOCK NO-ERROR.
         IF AVAIL ac_mstr THEN
         DISP acd_acc ac_desc acd_sub acd_cc acd_amt with width 200 stream-io.      
    END.
END.
OUTPUT CLOSE.
