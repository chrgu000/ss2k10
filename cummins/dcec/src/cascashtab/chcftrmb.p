/* GUI CONVERTED from chcftrmb.p (converter v1.71) Sun Oct 21 21:39:26 2007 */
/* chcftrmb.p -- CASH FLOW ENTRY TRANSACTION MAINT - CAS                   */
...
define buffer glt_b1 for glt_det.
    ...
    find glt_det where recid(glt_det) = w-recid and glt_domain = global_domain
    no-lock no-error.
         find first gl_ctrl no-lock no-error.
    
    for first xcfp_ctrl where xcfp_domain = global_domain no-lock: end.
      if glt_tr_type = "AR" or glt_tr_type = "AP" then do:   

        linenum = 0.

    find first glt_b1 where glt_b1.glt_ref = glt_det.glt_ref
...
/*XXLY*ADD END*/      and xcf1_domain = global_domain)
                      no-lock no-error.
        for each glt_b where glt_b.glt_ref = glt_det.glt_ref
                         and recid(glt_b) <> w-recid
                         /*and glt_b.glt_acct <> glt_det.glt_acct*/ 
...
/*XXLY*ADD*/            and ( if xcf1_mfg_sub <> "*" then xcf1_mfg_sub = glt_b.glt_sub else true )
                        and ( if xcf1_mfg_cc <> "*" then xcf1_mfg_cc = glt_b.glt_cc else true )
                        and ( if xcf1_mfg_pro <> "*" then xcf1_mfg_pro = glt_b.glt_project else true )
                        and ( if xcf1_mfgc_sub <> "*" then xcf1_mfgc_sub = glt_b1.glt_sub else true )
                        and ( if xcf1_mfgc_cc <> "*" then xcf1_mfgc_cc = glt_b1.glt_cc else true )
                        and ( if xcf1_mfgc_pro <> "*" then xcf1_mfgc_pro = glt_b1.glt_project else true )
/*XXLY*ADD END*/        and xcf1_active = yes 
                        and xcf1_domain = global_domain no-lock no-error.
        if not available xcf1_mstr then do:
        find first xcf1_mstr where glt_b.glt_acct = xcf1_mfg_ac_code
/*XXLY*ADD*/ 
                        and ( if xcf1_mfg_sub <> "*" then xcf1_mfg_sub = glt_b.glt_sub else true )
                        and ( if xcf1_mfg_cc <> "*" then xcf1_mfg_cc = glt_b.glt_cc else true )
                        and ( if xcf1_mfg_pro <> "*" then xcf1_mfg_pro = glt_b.glt_project else true )
/*XXLY*ADD END*/       
                        and xcf1_cf_acc = yes 
        ...
        if available xcf1_mstr then do:        
           ...
           if available xcft_det then 
            ...
            find ac_mstr where ac_code = xcft_acct no-lock no-error.
...
/* when xcfp_dsp_mt = no, Cash Flow maintenance screen does not display */
  if (xcfp_dsp_mt = yes and execname = "chgltrmt.p") or 
     ...
     (execname = "arpamt.p" or execname = "apmcmt.p" or execname = "apvomt.p"
      or execname = "ardrmt.p" or execname = "apcrmt.p") )
...
/*GUI*/ if global-beam-me-up then undo, leave.
    ...
    linenum = 1.
/*
    for each xcft_det where xcft_ref = glt_det.glt_ref
...
/*GUI*/ if global-beam-me-up then undo, leave.
	    ...
	    update xcft_acct
            ...
            find ac_mstr where ac_code = xcft_acct and ac_domain = global_domain no-lock no-error.
