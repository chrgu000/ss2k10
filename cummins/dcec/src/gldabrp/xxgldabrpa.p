...
/*fm268 */
DEFINE SHARED VAR i AS INT.
DEFINE SHARED TEMP-TABLE wkacctdt
    FIELD sosum AS INT
    FIELD account AS CHAR
    FIELD detail AS CHAR
    FIELD frsum AS DECIMAL
    FIELD perriod AS CHAR
    FIELD dodate AS DATE
    FIELD pzhsum AS CHAR
    FIELD des1 AS CHAR
    FIELD des2 AS CHAR
    FIELD des3 AS CHAR
    FIELD fdr1 AS DECIMAL
    FIELD fcr1 AS DECIMAL
    FIELD ljactive AS DECIMAL.
/*fm268 */
...
for first asc_mstr fields (asc_acc asc_sub asc_cc)
   where asc_domain = global_domain and recid(asc_mstr) = asc_recno no-lock:
end.

for first ac_mstr fields (ac_active ac_code ac_type ac_curr ac_desc )
   where ac_domain = global_domain and ac_code = asc_acc no-lock:
end.
assign
   act_to_dt    = 0
   beg_bal      = 0
...
for each en_mstr
   ...
   where en_domain = global_domain and en_entity >= entity and en_entity <= entity1
no-lock:

   if can-find (first acd_det
      where acd_domain = global_domain and acd_acc  = asc_acc and acd_sub = asc_sub
...
then do:
   ...
   if first_acct then DO:
      /*put asc_acc at 2 ac_desc at 26 "一" AT 60. fm268*/
      i = i + 1.
      CREATE wkacctdt. /*fm268*/
      ASSIGN wkacctdt.sosum = i.
             wkacctdt.account = ASC_acc.
             wkacctdt.detail = ac_desc.
   END.
   ...
   then do:
      ...
      /*put substring(account,1,(length(trim(asc_acc))+ 1
         ...
         "*" at 26 sb_desc at 27 "二" AT 60. fm268*/
      i = i + 1.
      CREATE wkacctdt. /*fm268*/
      ASSIGN wkacctdt.sosum = i.
             wkacctdt.account = substring(account,1,(length(trim(asc_acc))+ 1 + length(asc_sub))).
             wkacctdt.detail = sb_desc.
   ...
   if asc_cc <> "" and first_cc then do:
      ...
      /*put account at 2
         "**" at 26 cc_desc at 28 "三" AT 60. fm268*/
      i = i + 1.
      CREATE wkacctdt. /*fm268*/
      ASSIGN wkacctdt.sosum = i.
             wkacctdt.account = account.
             wkacctdt.detail = cc_desc.
   end. /* cc descrp printed */

   /*if ac_curr <> et_report_curr then put ac_curr at 53 "四" AT 60. fm268*/
   ...
   if ac_type = "S" then
      put {gplblfmt.i
          ...
          }  at 53 "六" AT 60. /*fm268*/ fm268*/

   {&GLDABRPA-P-TAG30}
   /*put string(et_beg_bal, prtfmt) format "x(19)" to 74 SKIP. fm268*/
   
   /* PUT et_beg_bal AT 74  "七" AT 90. 期初余额fm268 */
   ASSIGN wkacctdt.frsum = et_beg_bal.
   {&GLDABRPA-P-TAG31} 
   ...
   no-lock:
      ...
      for each gltr_hist
         ...
         {&GLDABRPA-P-TAG3}
         where gltr_domain = global_domain and 
         ...
         /*fm268   page.
            if first_acct then
               put asc_acc at 2 ac_desc + " (" + getTermLabel("CONT",4) + ".)"
                   format "x(32)" at 26  "八" AT 60 skip. /* 分页帐户fm268 */
            ...
            then
               ...
               format "x(32)" at 27  "九" AT 60 skip.  /* 分页帐户fm268 */   */
            /*fm268
            if asc_cc <> "" and first_cc then
               put account at 2 "**" at 26
               cc_desc + " (" + getTermLabel("CONT",4) + ".)"
               format "x(32)" at 28  "十" AT 60 skip.  fm268 */
            /*fm268
            put getTermLabel("PERIOD",6) + " " + string(glc_per) + "/" +
               string(glc_year) format "x(15)" at 3 "十一" AT 60 skip.  期间 fm268 */

         end. /* new page processed */
         else if first(gltr_tr_type) then  /* ignore if new page */ DO:
           /*put getTermLabel("PERIOD",6) + " " + string(glc_per) + "/" +
               string(glc_year) format "x(15)" at 3 "十二" AT 60 skip.  期间fm268 */
            CREATE wkacctdt. /*fm268*/
            i = i + 1. 
            ASSIGN wkacctdt.perriod = getTermLabel("PERIOD",6) + " " + string(glc_per) + "/" +  string(glc_year).
                   wkacctdt.sosum = i .
         END.
         if transflag = no
         then do:
               ...
               then do:
                 ...
                 then
                    put
                       account at 2 "**" at 26
                       cc_desc + " (" + getTermLabel("CONT",4) + ".)"
                       format "x(32)" at 28 skip. fm268*/
               /*fm268
                 put
                    getTermLabel("PERIOD",6) + " " + string(glc_per) + "/" +
                    string(glc_year) format "x(15)" at 3 skip. fm268*/
               end. /* new page processed */
               {&GLDABRPA-P-TAG6}
            end. /* IF NOT l_skip_process */
            ...
            then do:
               ...
               then do:
                 /* put
                     gltr_eff_dt  at 4   /*fm268 日期*/
                     gltr_ref     at 13  /*fm268凭证号*/
                     gltr_project at 28 . fm268项目为空值 */
                  i = i + 1.
                  CREATE wkacctdt. 
                  ASSIGN wkacctdt.sosum = i.
                         wkacctdt.dodate = gltr_eff_dt.
                         wkacctdt.pzhsum = gltr_ref.
                  {&GLDABRPA-P-TAG7}
                  ...
                  THEN DO:
                    ...
                    /* put
                        ...
                        gltr_addr    at 37    fm268 描述一*/
                        /*gltr_batch   at 46   fm268 描述二*/
/*judy 07/06/05*/   /*  gltr_doc_typ at 55*/
/*judy 07/06/05*/   /*    gltr_doc     at 57.*/
/*judy 07/06/05*/   /*gltr_desc at 55 .  fm268 描述三*/
                    ASSIGN wkacctdt.des1 = gltr_addr.
                           wkacctdt.des2 = gltr_batch.
                           wkacctdt.des3 = gltr_desc.
                     {&GLDABRPA-P-TAG33}
                  END.
                  ELSE DO:
                     /*put gltr_desc at 37 . fm268 描述三*/
                     ASSIGN wkacctdt.des3 = gltr_desc.
                  END.
                  if daybooks-in-use
                  THEN DO:
                     {&GLDABRPA-P-TAG34}
                     /*put gltr_dy_code at 75. fm268 描述三*/
                     ASSIGN wkacctdt.des3 = gltr_desc.
                     {&GLDABRPA-P-TAG35}
                  END.
                  if l_flag_debit
                  THEN DO:
                    ...
                    /* put
                        et_base_amt to 102  
                        0.00 to 124  skip.  fm268 借方发生额 ，贷方发生额0*/
                        ASSIGN wkacctdt.fdr1 = et_base_amt.
                               wkacctdt.fcr1 = 0.
                  ...
                  ELSE DO:
                     ...
                     /*put
                        0.00 to 102
                        (et_base_amt * -1)
                        format "->>>,>>>,>>>,>>9.99" to 124  skip. fm268 借方发生额0*/
                      ASSIGN wkacctdt.fdr1 = 0.
                             wkacctdt.fcr1 = (et_base_amt * -1).
                     {&GLDABRPA-P-TAG39}
                  END.
               end. /* IF NOT l_skip_process */
               ...
               then do:
                 ...
                 /* put "-------------------" to 102   
                      ...
                      {gplblfmt.i
                         ...
                         &CONCAT = "':  '"
                      }  to 74 "十七"  AT 80         /*fm268*/
                      gltr_dy_code  "十三" /* fm268*/
                      et_dy-total-dr to 102  "十四" AT 110 /*fm268*/
                      et_dy-total-cr to 124  "十五" AT 130 /*fm268*/
                      {&GLDABRPA-P-TAG9}
                      skip(1).

                  {&GLDABRPA-P-TAG41} fm268 合计线 */
                  assign
                     et_dy-total-dr = 0
                     et_dy-total-cr = 0.
            ...
            then do:
               ...
               then
                  /* put
                     gltr_tr_type at 16  "十八" AT 20
                     gltr_dy_code at 75 "十九" AT 80. fm268*/
               ...
               then do:
                  ...
                  {&GLDABRPA-P-TAG42}
                  put
                     et_trans-total-dr to 102 "二十" AT 110
                     et_trans-total-cr to 124 "二十一" AT 120 skip.  m268*/
                  {&GLDABRPA-P-TAG43}
                  {&GLDABRPA-P-TAG11}
                  assign
                     et_trans-total-cr = 0
      ...
      and l_other_daybook)
      then do:
        /* put "-------------------" to 102  "二十二" AT 110
             "-------------------" to 124  "二十三" AT 130  /*fm268*/
             skip
             {gplblfmt.i
              &FUNC=getTermLabel(""OTHER_DAYBOOKS"",35)
              &CONCAT = "':  '"
             }  to 74
              l_et_oth-total-dr to 102   "二十四" AT 110
              l_et_oth-total-cr to 124   "二十五" AT 130
              skip(1). */
      ...
      if knt > 0 then do:
         ...
         {&GLDABRPA-P-TAG12}
         {&GLDABRPA-P-TAG44}
         put "-------------------" to 102 "二十六" AT 110 /*fm268 合计线*/
             "-------------------" to 124 "二十七" AT 130
             skip
             et_per-total-dr to 102 "二十八" AT 110
             et_per-total-cr to 124 "二十九" AT 130 skip. fm268 借方和贷方活动汇总*/
         i = i + 1.
         CREATE wkacctdt.
         ASSIGN wkacctdt.sosum = i.
                wkacctdt.des1 = "期间小计".
                wkacctdt.des2 = "借方:".
                wkacctdt.des3 = "贷方:".
                wkacctdt.fdr1 = et_per-total-dr.
                wkacctdt.fcr1 = et_per-total-cr.
         {&GLDABRPA-P-TAG45}
         {&GLDABRPA-P-TAG13}
   ...
   /* put
      ...
      {gplblfmt.i
         ...
         &CONCAT = "': '"
      } at 2 begdt " - " enddt1 /*fm268累计活动金额：: 01/01/05 - 28/02/05  */
      et_act_to_dt to 74 SKIP  /*fm268  累计活动金额 */
      {gplblfmt.i
      ...
      ASSIGN wkacctdt.sosum = i.
             wkacctdt.des1 = "分帐户合计".
             wkacctdt.des2 = "累计活动金额:".
             wkacctdt.des3 = "期末余额:".
             wkacctdt.fdr1 = et_act_to_dt.
             wkacctdt.fcr1 = et_end_bal.
