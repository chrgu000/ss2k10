...
mfile  = "SQ01\SQ01_" + STRING(YEAR(TODAY),"9999") + STRING(MONTH(TODAY),"99") + STRING(DAY(TODAY),"99") + STRING(TIME,"999999") + ".txt".

OUTPUT TO VALUE(CODE_value + mfile) CONVERT TARGET "UTF-8" SOURCE 'GB2312'.

FOR EACH pod_det NO-LOCK WHERE pod_domain = global_domain and pod_site >= "DCEC-B" AND pod_site <= "DCEC-C",
    ...
    EACH schd_det NO-LOCK WHERE schd_domain = global_domain and schd_type = 4 AND schd_rlse_id = pod_curr_rlse_id[1] AND schd_nbr = pod_nbr
         AND schd_line = pod_line AND schd_discr_qty > 0 AND schd_date >= TODAY  AND schd_date <= TODAY + 320.
   ...
   v_date_string = STRING(YEAR(schd_date),"9999") + "-" + STRING(MONTH(schd_date),"99") + "-" +  STRING(DAY(schd_date),"99").
    ...
    PUT UNFORMATTED UPPER(pt_status) "|".
    PUT UNFORMATTED schd_discr_qty "|".
    PUT UNFORMATTED v_date_string  SKIP.
END.
QUIT.
