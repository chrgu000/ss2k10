...
FORM /*GUI*/
 ...
 SKIP(.1)  /*GUI*/
   ...
   usrw_key3 colon 25 format "x(2)" skip
   usrw_key4 colon 25 format "x(4)" skip
   usrw_key5 colon 25 format "x(8)" skip
   usrw_charfld[1] colon 25 format "x(120)" view-as fill-in size 40 by 1
   usrw_charfld[2] colon 25 format "x(120)" view-as fill-in size 40 by 1
...
/*GUI*/ if global-beam-me-up then undo, leave.
        ...
        do i = 1 to 4:
           ...
           if substring(usrw_charfld[i],length(usrw_charfld[i]),1) <> "~\"
           then do:
              {pxmsg.i &MSGTEXT=""目录结尾必须是~\"" &ERRORLEVEL=3}
              undo,retry.
           end.
        end.
        leave.
