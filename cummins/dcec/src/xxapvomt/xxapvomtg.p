...
/* Revision: 1.12.2.12     BY: Gnanasekar     DATE: 09/11/02 ECO: *N1PG* */
/* Revision: 1.12.2.13     BY: Anitha Gopal   DATE: 01/29/03 ECO: *N25J* */
/* Revision: 1.12.2.15     BY: Piotr Witkowicz DATE: 03/17/03 ECO: *P0NP* */
/* Revision: 1.12.2.16  BY: Orawan S. DATE: 04/21/03 ECO: *P0Q8* */
/* Revision: 1.12.2.18  BY: Paul Donnelly (SB) DATE: 06/26/03 ECO: *Q00B* */
/* $Revision: 1.12.2.19 $ BY: W.Palczynski DATE: 09/01/03 ECO: *P0YC* */
/*-Revision end---------------------------------------------------------------*/
...
do transaction:
   ...
   {&APVOMTG-P-TAG10}
   for first apc_ctrl
      fields( apc_domain apc_vchr_all apc_voucher)
   ...
   do on error undo, retry:
      ...
      editing:
         ...
         {mfnp01.i ap_mstr ap_ref ap_ref batch  " ap_mstr.ap_domain =
         global_domain and ap_batch "  ap_batch}
         if recno <> ?
         then do:
            ...
            with frame vohdr1.
            for first vo_mstr
               fields( vo_domain vo_cr_terms vo_curr vo_disc_date vo_due_date
               vo_hold_amt
                       {&APVOMTG-P-TAG12}
            ...
            then do:
               ...
               for first vd_mstr
                  fields( vd_domain vd_addr vd_remit vd_sort)
                   where vd_mstr.vd_domain = global_domain and  vd_addr =
                   ap_vend
               no-lock:
            ...
            then
               for first ad_mstr
                  fields( ad_domain ad_addr ad_city ad_country ad_county
                  ad_format
            ...
            for first ad_mstr
               fields( ad_domain ad_addr ad_city ad_country ad_county ad_format
                       ad_line1 ad_line2 ad_line3 ad_name ad_pst_id
                       ad_state ad_zip)
      ...
      if input ap_ref = ""
      then do:
         {mfnctrl.i "apc_ctrl.apc_domain = global_domain" "apc_ctrl.apc_domain"
         "vo_mstr.vo_domain = global_domain" apc_ctrl apc_voucher vo_mstr
         vo_ref apref}
         {&APVOMTG-P-TAG16}
      ...
      for first ap_mstr
         fields( ap_domain ap_acct ap_amt ap_bank ap_batch ap_cc ap_ckfrm
         ap_curr ap_date
                 ap_disc_acct ap_disc_cc ap_disc_sub ap_dy_code ap_effdate
                 ap_entity ap_expt_date ap_open ap_ref ap_rmk ap_sub ap_type
                 ap_vend)
          where ap_mstr.ap_domain = global_domain and  ap_ref  = input ap_ref
           and ap_type = "RV"
      no-lock:
      ...
      if available ap_mstr
         and ap_open = no
      then do:
         /* VOUCHER PAID IN FULL OR CLOSED */
      ...
      then do:
         ...
         for first mfc_ctrl
            fields( mfc_domain mfc_field mfc_label mfc_logical mfc_module
            mfc_seq mfc_type)
         ...
         then do:
            create mfc_ctrl. mfc_ctrl.mfc_domain = global_domain.
         ...
         if can-find(bk_mstr
                        where bk_mstr.bk_domain = global_domain and  bk_code =
                        substring(apref,1,2))
         then do:
      ...
      then do:
         {&APVOMTG-P-TAG7}
         for first ba_mstr
            fields( ba_domain ba_batch ba_ctrl ba_module ba_total)
