...
/* Revision: 1.26.2.49     BY: Paul Donnelly (SB) DATE: 06/26/03 ECO: *Q00B* */
/* Revision: 1.26.2.50     BY: Rajiv Ramaiah      DATE: 08/14/03 ECO: *N2JV* */
/* Revision: 1.26.2.51     BY: Gnanasekar         DATE: 09/17/03 ECO: *P139* */
/* Revision: 1.26.2.52     BY: Jean Miller        DATE: 10/06/03 ECO: *Q03S* */
/* Revision: 1.26.2.53     BY: Manjusha Inglay    DATE: 10/27/03 ECO: *P175* */
/* Revision: 1.26.2.54     BY: Ed van de Gevel    DATE: 12/01/03 ECO: *N2N7* */
/* Revision: 1.26.2.55     BY: Vandna Rohira      DATE: 01/30/04 ECO: *P1LT* */
/* Revision: 1.26.2.56     BY: Nishit V           DATE: 02/26/04 ECO: *P1Q9* */
/* Revision: 1.26.2.57     BY: Vandna Rohira      DATE: 09/06/04 ECO: *P2J0* */
/* Revision: 1.26.2.58     BY: Vinay Nayak-Sujir  DATE: 10/07/04 ECO: *P2NL* */
/* Revision: 1.26.2.59     BY: Ajay Nair          DATE: 01/06/05 ECO: *P32L* */
/* Revision: 1.26.2.60 BY: SurenderSingh Nihalani DATE: 06/29/05 ECO: *P3PL* */
/* Revision: 1.26.2.61     BY: S. Nugent          DATE: 07/26/05 ECO: *P2PJ* */
/* Revision: 1.26.2.61.1.1 BY: Ashwini G.         DATE: 05/30/06 ECO: *P4RS* */
/* Revision: 1.26.2.61.1.2 BY: Antony LejoS       DATE: 11/09/06 ECO: *P53Y* */
/* Revision: 1.26.2.61.1.3 BY: Gaurav Kerkar      DATE: 11/20/06 ECO: *P58T* */
/* Revision: 1.26.2.61.1.4 BY: Sundeep Kalla      DATE: 11/22/06 ECO: *P4YS* */
/* Revision: 1.26.2.61.1.5 BY: Dilip Manawat      DATE: 04/12/07 ECO: *P5T4* */
/* Revision: 1.26.2.61.1.6 BY: Sarita Gonsalves   DATE: 07/06/07 ECO: *P61H* */
/* $Revision: 1.26.2.61.1.7 $ BY: Sarita Gonsalves      DATE: 08/17/07 ECO: *P650*  */
/*-Revision end---------------------------------------------------------------*/
...
define buffer tx2ddet1          for  tx2d_det.
...
define variable l_vph_total     like vph_curr_amt  no-undo.
define variable l_ratio         like vph_curr_amt  no-undo.
...
define variable l_transaction_qty like pvo_vouchered_qty    no-undo.
define variable pvod-trx-cost     like pvod_pur_cost        no-undo.
define variable l_rcvtax          like ap_amt               no-undo.
define variable l_rcvrec          like ap_amt               no-undo.
define variable l_ppvpor          like ap_amt               no-undo.
define variable l_varamt          like ap_amt               no-undo.
define variable l_prh_rcvd        like prh_rcvd             no-undo.
...
define temp-table tt-vod_det
   fields tt-vod_ln        like vod_ln
   fields tt-vod_entity    like vod_entity
   fields tt-vod_acct      like vod_acct
   fields tt-vod_sub       like vod_sub
   fields tt-vod_cc        like vod_cc
   fields tt-vod_exp_acct  like vod_exp_acct
   fields tt-vod_exp_sub   like vod_exp_sub
   fields tt-vod_exp_cc    like vod_exp_cc
   fields tt-vod_project   like vod_project
   fields tt-vod_type      like vod_type
   fields tt-vod_desc      like vod_desc
   index idx-vod_det
      tt-vod_entity
      tt-vod_acct
      tt-vod_sub
...
define temp-table tt-vod_tax
   fields tt-recid         as   recid
   fields tt-vod_ln        like vod_ln
   fields tt-vod_entity    like vod_entity
   fields tt-vod_acct      like vod_acct
   fields tt-vod_sub       like vod_sub
   fields tt-vod_cc        like vod_cc
   fields tt-vod_project   like vod_project
   fields tt-vod_tax       like vod_tax
   index idx-vod_tax
      tt-vod_entity
      tt-vod_acct
      tt-vod_sub
...
find last vod_det where vod_domain = global_domain and
                        vod_ref = vo_ref
no-lock no-error.
...
for each vod_det
   ...
   assign
      tt-vod_ln       = vod_ln
      tt-vod_entity   = vod_entity
      tt-vod_acct     = vod_acct
      tt-vod_sub      = vod_sub
      tt-vod_cc       = vod_cc
      tt-vod_exp_acct = vod_exp_acct
      tt-vod_exp_sub  = vod_exp_sub
      tt-vod_exp_cc   = vod_exp_cc
      tt-vod_project  = vod_project
      tt-vod_type     = vod_type
      tt-vod_desc     = vod_desc.
...
for each vph_hist where vph_domain = global_domain
     and vph_ref = vo_ref
     and vph_pvo_id <> 0
...
/* GET THE EXCHANGE RATE VALUES FROM THE PENDING VOUCHER */
/* DETAIL RECORD.                                        */
    run get_pvod_exchange_rate
        (input pvod_id,
         input pvod_id_line,
         output pvod_ex_rate,
    ...
    assign
       l_transaction_qty = pvod_trans_qty
       vouchered_qty = pvod_vouchered_qty
   ...
   for each tx2d_det where
            ...
            tx2d_det.tx2d_ref = pvo_internal_ref and
            tx2d_det.tx2d_tr_type = rcpt_tr_type /* PO Receipts/Returns */
        and tx2d_det.tx2d_line = pvo_line
   no-lock:
      ...
      find tx2_mstr where tx2_domain = global_domain and
                          tx2_tax_code = tx2d_det.tx2d_tax_code
      no-lock no-error.

      find txed_det where txed_domain = global_domain and
      ...
      if available txed_det and txed_round <> "" then
         ...
         find first txc_ctrl where txc_domain = global_domain
         no-lock no-error.
      ...
      /* ("23" TYPE) */
      find first rcpt_det where
                 rcpt_det.tx2d_domain = global_domain
             and rcpt_det.tx2d_ref = pvo_internal_ref
      ...
      then do:
         ...
         for first work_tx2d_det
            ...
            where work_tx2d_det.work_ref      = pvo_mstr.pvo_internal_ref and
      ...
      then do:
         ...
         assign
            work_tx2d_det.work_rcv_tax = - rcpt_det.tx2d_cur_tax_amt
            work_tx2d_det.work_rcv_rec = - rcpt_det.tx2d_cur_recov_amt
      ...
      if (rcpt_det.tx2d_rcpt_tax_point and not pvo_mstr.pvo_consignment)
         or (pvo_mstr.pvo_consignment and op_usage_point)
      then do:

         find tx2_mstr where tx2_domain = global_domain and
                             tx2_tax_code = rcpt_det.tx2d_tax_code
         no-lock no-error.
      ...
      if (tx2d_det.tx2d_rcpt_tax_point and not pvo_mstr.pvo_consignment)
         or (pvo_mstr.pvo_consignment and op_usage_point)
      then do:
...
/* "22" TYPE */
for each tx2d_det where tx2d_det.tx2d_domain = global_domain and
                        tx2d_det.tx2d_ref = vo_ref and
   ...
   if available txed_det and txed_round <> "" then
      ...
      /* TRY TAX CONTROL FILE NEXT */
      find first txc_ctrl where txc_domain = global_domain no-lock no-error.
   ...
   if tx2d_det.tx2d_nbr > ""
   then do:

      /* CALCULATE THE TOTAL AMT BEING VOUCHERED */
      assign
         l_vph_total  = 0.00
         l_ratio      = 1 .

      for each  vph_hist
         fields (vph_domain vph_ref vph_pvo_id vph_pvod_id_line
                 vph_inv_qty vph_curr_amt )
         where vph_domain     = global_domain
           and vph_ref        = vo_ref
           and vph_pvo_id     <> 0
           and vph_pvod_id_line > 0 no-lock ,
          first pvod_det
             fields (pvod_domain pvod_id pvod_id_line)
             where pvod_domain  = global_domain
               and pvod_id      = vph_pvo_id
               and pvod_id_line = vph_pvod_id_line  no-lock,
          first pvo_mstr
             fields(pvo_domain pvo_id pvo_internal_ref pvo_line)
             where pvo_domain          = global_domain
               and   pvo_id            = vph_pvo_id
               and   pvo_internal_ref  = tx2d_det.tx2d_nbr
               and   pvo_line          = tx2d_det.tx2d_line no-lock ,
          first prh_hist
             fields (prh_domain prh_receiver prh_line )
             where prh_domain   = global_domain
             and   prh_receiver = pvo_internal_ref
             and   prh_line     = pvo_line no-lock :

          l_vph_total =  l_vph_total + ( vph_inv_qty * vph_curr_amt) .
      ...
      for each vph_hist exclusive-lock
         where vph_domain = global_domain
            ...
            and vph_pvo_id <> 0
            and vph_pvod_id_line > 0,
         first pvod_det no-lock
            where pvod_domain  = global_domain
              and pvod_id      = vph_pvo_id
              and pvod_id_line = vph_pvod_id_line,
         first pvo_mstr no-lock
            where pvo_domain             = global_domain
              and pvo_id                 = vph_pvo_id
              and pvo_internal_ref       = tx2d_det.tx2d_nbr
              and (pvo_line              = tx2d_det.tx2d_line
                    or tx2d_det.tx2d_line = 0),
         first prh_hist no-lock
             where prh_domain   = global_domain
             and   prh_receiver = pvo_internal_ref
      ...
      then do:
         ...
         find pld_det where pld_domain = global_domain
      ...
      then do:
         ...
         /* AFFECT AP RATE VARIANCE ACCOUNT. */
            assign
               ...
               ppv_discrep_acct = dftAPVRAcct
               ppv_discrep_sub  = dftAPVRSubAcct
               ppv_discrep_cc   = dftAPVRCostCenter.
         ...
         qty_po = l_transaction_qty * prh_um_conv.
         if qty_po <= 0 then
            pct_po = 0.
         else
      ...
      assign
         open_amt = l_transaction_qty * pvod-trx-cost * prh_um_conv
         rndamt   = vouchered_qty * pvod-trx-cost * prh_um_conv
         rcpt_amt = vph_inv_qty * pvod-trx-cost * prh_um_conv.
      ...
      /* CALCULATED THE RATIO SO AS TO APPORATION THE TAX AMT TO THE */
      /* TO THE AMT BEING PROCESSED                                  */

      if l_vph_total <> 0.00
      then
         l_ratio  = ppv_amt / l_vph_total .
      ...
      then do:
         ...
         then do:
            ...
            if last_voucher > "" and
               l_transaction_qty <> vouchered_qty
            then
               assign
      ...
      if not tx2d_det.tx2d_tax_in
         or ((not tx2d_det.tx2d_rcpt_tax_point and not pvo_mstr.pvo_consignment)
      ...
      if tx2d_det.tx2d_tax_in
         and ((tx2d_det.tx2d_rcpt_tax_point and not pvo_mstr.pvo_consignment)
              or (pvo_mstr.pvo_consignment and op_usage_point))
         then
      /* TAX INCLUDED = YES. */
      ...
      /*UPDATE PPV HERE ONLY IF TAX IS NOT EDITED*/
      if not tx2d_edited then
         run update_vod_det
            (input  ((rcv_tax - rcv_rec - rcv_rec_discrep) * l_ratio) ) .
     ...
     tax_project  = "".
      ...
      if ppv_tax <> 0 then do:
         ...
         then do:
            ...
            run update_vod_det
               ( input (( - ppv_rec - ppv_rec_discrep) * l_ratio) ) .
      ...
      if tx2d_det.tx2d_edited
      then do:
         assign
            tax_acct = por_acct
            tax_sub  = por_sub
            tax_cc   = por_cc
            l_rcvtax = decimal(entry(2, tx2d__qad02))
            l_rcvtax = (l_rcvtax * rcv_amt) / (rcv_amt + ppv_amt + usg_amt)
            l_rcvrec = decimal(entry(1, tx2d__qad02))
            l_rcvrec = (l_rcvrec * rcv_amt) / (rcv_amt + ppv_amt + usg_amt).
         ...
         /*UPDATING PPV/POR WHEN TAX IS EDITED*/
         run update_vod_det ((l_rcvtax
                             - l_rcvrec
                             -  rcv_rec_discrep)
                             * l_ratio).
         ...
         if tx2d_det.tx2d_tax_in = yes
         then
            assign

            /* WHEN TAX IS INCLUDED ANY RECEOVERABLE AMOUNT SHOULD */
            /* BE REDUCED FROM AP RATE VARIANCE ALREDY BOOKED      */
            ...
            /* ANY ADJUSTMENT MADE TO TOTAL TAX AMOUNT SHOULD BE */
            /* PASSED TO AP RATE VARIANCE                        */

            l_varamt = l_varamt + tx2d_det.tx2d_cur_tax_amt
                       - decimal(entry(2, tx2d__qad02)).
         else

            /* WHEN TAX IS NOT INCLUDED, ANY ADJUSTMENT TO  NON-RECOVERABLE */
            /* TAX AMOUNT SHOULD BE BOOKED AS AP RATE VARIANCE.             */

            l_varamt = tx2d_det.tx2d_cur_tax_amt
                       - tx2d_det.tx2d_cur_recov_amt
                       - (l_rcvtax - l_rcvrec) .

         run update_vod_det (l_varamt).
      end. /*IF tx2d_det.tx2d_edited THEN DO*/
   ...
   /* OR RECEIVER LINES TAXED BY TOTAL */
      ...
      then do:
         ...
         then do:
            ...
            for first vod_det
               fields(vod_domain vod_ref vod_ln vod_entity vod_project)
                where vod_domain = global_domain
                  and vod_ref = vo_ref
         ...
         /* CREATE/MODIFY VOD_DET FOR AP TAX ACCOUNT */
         run update_vod_det
            (input tx2d_det.tx2d_cur_recov_amt).
         {&APVOMTI-P-TAG28}
      end.
   ...
   then do:
      ...
      {&APVOMTI-P-TAG37}
      run update_vod_det
         (input tx2d_det.tx2d_cur_abs_ret_amt).
      {&APVOMTI-P-TAG38}
...
for each vod_det where vod_domain = global_domain and
                       vod_ref = vo_ref and
                       vod_tax > "" and
                       vod_amt = 0
exclusive-lock:
   ...
   then do:
      ...
      then do:
         for each tx2d_det
            ...
            and   tx2d_ref    = vod_ref
            and   tx2d_line   = vod_ln
         exclusive-lock:

            for first tx2ddet1
               fields (tx2ddet1.tx2d_domain tx2ddet1.tx2d_ref
                       tx2ddet1.tx2d_nbr    tx2ddet1.tx2d_line
                       tx2ddet1.tx2d_trl    tx2ddet1.tx2d_tr_type
                       tx2ddet1.tx2d_tax_code)
               where tx2ddet1.tx2d_domain   = global_domain
               and   tx2ddet1.tx2d_ref      = tx2d_det.tx2d_ref
               and   tx2ddet1.tx2d_nbr      = tx2d_det.tx2d_nbr
               and   tx2ddet1.tx2d_line     = l_vod_ln
               and   tx2ddet1.tx2d_trl      = tx2d_det.tx2d_trl
               and   tx2ddet1.tx2d_tr_type  = tx2d_det.tx2d_tr_type
               and   tx2ddet1.tx2d_tax_code = tx2d_det.tx2d_tax_code
            no-lock:
            end. /* FOR FIRST tx2ddet1 */

            if not available tx2ddet1
            then
               tx2d_det.tx2d_line = l_vod_ln.
...
for each vod_det
   fields(vod_domain vod_ref vod_tax vod_amt vod_base_amt)
    where vod_domain = global_domain
     and  vod_ref = vo_ref
     and vod_tax > ""
no-lock:
...
for each vod_det
   fields(vod_domain vod_ref vod_tax vod_acct vod_sub vod_cc vod_project
...
no-lock:
   ...
   assign
      vod_amt:format in frame tax_dist = vod_amt_fmt.
...
PROCEDURE update_rcpt_det_for_partial_qty:
   ...
   if rcpt_det.tx2d_line_site_ent <> "" then
      find en_mstr where en_domain = global_domain and
                         en_entity = rcpt_det.tx2d_line_site_ent
      no-lock no-error.
   else
   /* FIND en_mstr USING TRANSACTION ENTITY            */
      find en_mstr where en_domain = global_domain and
                         en_entity = rcpt_det.tx2d_trans_ent
      no-lock no-error.
...
PROCEDURE update_rcpt_det_for_final_qty:
   ...
   rndamt = (tx2d_det.tx2d_tottax * ((l_transaction_qty - vouchered_qty)
             / l_transaction_qty)).
   ...
   rndamt = (tx2d_det.tx2d_totamt * ((l_transaction_qty - vouchered_qty) / l_transaction_qty)).
   ...
   rndamt = (tx2d_det.tx2d_cur_tax_amt * ((l_transaction_qty - vouchered_qty)
                                       / l_transaction_qty)).
   {gprunp.i "mcpl" "p" "mc-curr-rnd"
      "(input-output rndamt,
        input rndmthd,
   ...
   rndamt = (tx2d_det.tx2d_tax_amt * ((l_transaction_qty - vouchered_qty) / l_transaction_qty)).
   ...
   rcpt_det.tx2d_tax_amt = rcpt_det.tx2d_tax_amt - rndamt.

   rndamt = (tx2d_det.tx2d_ent_tax_amt * ((l_transaction_qty - vouchered_qty)
                                       / l_transaction_qty)).
   {gprunp.i "mcpl" "p" "mc-curr-rnd"
      "(input-output rndamt,
        input ent_rndmthd,
   ...
   rndamt = (tx2d_det.tx2d_cur_nontax_amt * ((l_transaction_qty - vouchered_qty)
                                          / l_transaction_qty)).
   {gprunp.i "mcpl" "p" "mc-curr-rnd"
      "(input-output rndamt,
        input rndmthd,
   ...
   rndamt = (tx2d_det.tx2d_nontax_amt * ((l_transaction_qty - vouchered_qty)
                                      / l_transaction_qty)).
   {gprunp.i "mcpl" "p" "mc-curr-rnd"
      "(input-output rndamt,
        input gl_rnd_mthd,
   ...
   rndamt = (tx2d_det.tx2d_ent_nontax_amt * ((l_transaction_qty - vouchered_qty)
                                          / l_transaction_qty)).
   {gprunp.i "mcpl" "p" "mc-curr-rnd"
      "(input-output rndamt,
        input ent_rndmthd,
   ...
   rndamt = (tx2d_det.tx2d_taxable_amt * ((l_transaction_qty - vouchered_qty)
                                       / l_transaction_qty)).
   {gprunp.i "mcpl" "p" "mc-curr-rnd"
      "(input-output rndamt,
        input gl_rnd_mthd,
   ...
   rndamt = (tx2d_det.tx2d_cur_recov_amt * ((l_transaction_qty - vouchered_qty)
                                         / l_transaction_qty)).
   {gprunp.i "mcpl" "p" "mc-curr-rnd"
      "(input-output rndamt,
        input rndmthd,
   ...
   rndamt = (tx2d_det.tx2d_recov_amt * ((l_transaction_qty - vouchered_qty) / l_transaction_qty)).
   ...
   rndamt = (tx2d_det.tx2d_ent_recov_amt * ((l_transaction_qty - vouchered_qty)
                                         / l_transaction_qty)).
   {gprunp.i "mcpl" "p" "mc-curr-rnd"
      "(input-output rndamt,
        input ent_rndmthd,
   ...
   rndamt = (tx2d_det.tx2d_cur_abs_ret_amt * ((l_transaction_qty - vouchered_qty)
                                           / l_transaction_qty)).
   {gprunp.i "mcpl" "p" "mc-curr-rnd"
      "(input-output rndamt,
        input rndmthd,
   ...
   rndamt = (tx2d_det.tx2d_abs_ret_amt * ((l_transaction_qty - vouchered_qty)
                                       / l_transaction_qty)).
   ...
   rndamt = (tx2d_det.tx2d_ent_abs_ret_amt * ((l_transaction_qty - vouchered_qty)
                                           / l_transaction_qty)).
   {gprunp.i "mcpl" "p" "mc-curr-rnd"
      "(input-output rndamt,
        input ent_rndmthd,
...
PROCEDURE update_vod_det:
   ...
   if tax_amt <> 0 then do:
      ...
      for first tt-vod_det
         fields (tt-vod_desc)
         where tt-vod_det.tt-vod_type     = l_vod_type
         and   tt-vod_det.tt-vod_entity   = tax_entity
         and   tt-vod_det.tt-vod_acct     = tax_acct
         and   tt-vod_det.tt-vod_sub      = tax_sub
         and   tt-vod_det.tt-vod_cc       = tax_cc
         and   tt-vod_det.tt-vod_project  = tax_project
         and   tt-vod_det.tt-vod_exp_acct = l_expacct
         and   tt-vod_det.tt-vod_exp_sub  = l_expsub
         and   tt-vod_det.tt-vod_exp_cc   = l_expcc
         no-lock:
      end. /* FOR FIRST tt-vod_det */

      if available tt-vod_det
      then
         l_vod_desc = tt-vod_desc.
      else do:
         for first ac_mstr
      ...
      for first tt-vod_tax
         fields(tt-recid)
         where tt-vod_tax.tt-vod_entity  = tax_entity
         and   tt-vod_tax.tt-vod_acct    = tax_acct
         and   tt-vod_tax.tt-vod_sub     = tax_sub
         and   tt-vod_tax.tt-vod_cc      = tax_cc
         and   tt-vod_tax.tt-vod_project = tax_project
         and   tt-vod_tax.tt-vod_tax     = "t"
         no-lock :
      end. /*FOR FIRST tt-vod_tax*/

      if available tt-vod_tax then
         for first vod_det
            where recid(vod_det) = tt-vod_tax.tt-recid
            exclusive-lock:
      ...
      if not available vod_det then do:
         ...
         create tt-vod_tax.
         assign
            tt-vod_tax.tt-recid       = recid(vod_det)
            tt-vod_tax.tt-vod_ln      = vod_ln
            tt-vod_tax.tt-vod_entity  = vod_entity
            tt-vod_tax.tt-vod_acct    = vod_acct
            tt-vod_tax.tt-vod_sub     = vod_sub
            tt-vod_tax.tt-vod_cc      = vod_cc
            tt-vod_tax.tt-vod_project = vod_project
            tt-vod_tax.tt-vod_tax     = vod_tax.
      ...
      do:
         ...
         if tax_acct <> ""
            and
           (tax_acct = prm_rate_var_acct or
            tax_acct = prm_usage_var_acct or
            tax_acct = prm_ppv_acct)
         then
            for first pod_det
               fields(pod_domain pod_pjs_line)
                where pod_det.pod_domain = global_domain
                  and pod_det.pod_nbr = pvo_mstr.pvo_order
...
PROCEDURE p_tax_code:
   ...
   for last b_tx2_mstr
         fields(tx2_domain tx2_ap_acct    tx2_ap_cc    tx2_effdate
                tx2_tax_code   tx2_tax_type tx2_tax_usage
                tx2_apr_acct   tx2_apr_cc   tx2_apr_use
                tx2_exp_date   tx2_pt_taxc)
          where b_tx2_mstr.tx2_domain    = global_domain and
                b_tx2_mstr.tx2_tax_type  = tax_type   and
                b_tx2_mstr.tx2_pt_taxc   = tax_class  and
                b_tx2_mstr.tx2_tax_usage = tax_usage  and
                b_tx2_mstr.tx2_effdate   <= tax_date  and
               (b_tx2_mstr.tx2_exp_date  = ?          or
                b_tx2_mstr.tx2_exp_date  >= tax_date)
   no-lock: end.

   if not available(b_tx2_mstr) then
   for last b_tx2_mstr
         fields(tx2_domain tx2_ap_acct    tx2_ap_cc    tx2_effdate
                tx2_tax_code   tx2_tax_type tx2_tax_usage
                tx2_apr_acct   tx2_apr_cc   tx2_apr_use
...
/* --------------------------------------------------------------- */
   ...
   for first gl_ctrl where gl_domain = global_domain no-lock:
      ...
      for first pvo_mstr
         fields( pvo_domain pvo_order pvo_line)
         where recid (pvo_mstr) = in_pvo_recid
      no-lock:

         for first pod_det
            fields(pod_domain pod_part pod_type)
            where pod_domain = global_domain and
                  pod_nbr = pvo_order     and
                  pod_line = pvo_line
         no-lock:
...
PROCEDURE determine_usage_tax_point:
   ...
   for first tx2_mstr
      fields(tx2_domain tx2_usage_tax_point)
       where tx2_domain = global_domain and
             tx2_tax_code = ip_tax_code
   no-lock:
...
PROCEDURE get_pvod_exchange_rate:
define input parameter ip_pvo_id like vph_pvo_id no-undo.
define input parameter ip_pvod_id_line like vph_pvod_id_line no-undo.
define output parameter op_ex_rate as decimal no-undo.
define output parameter op_ex_rate2 as decimal no-undo.
...
assign
   op_ex_rate = pvod_ex_rate
   op_ex_rate2 = pvod_ex_rate2.
