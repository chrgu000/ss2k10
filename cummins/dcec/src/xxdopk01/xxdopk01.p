...
/* Revision: 1.29     BY: Paul Donnelly (SB)    DATE: 06/26/03 ECO: *Q00B*    */
/* Revision: 1.30     BY: Geeta Kotian          DATE: 10/29/03 ECO: *P17M*    */
/* Revision: 1.32     BY: Sunil Fegade          DATE: 05/10/05 ECO: *Q0J4*    */
/* Revision: 1.32.1.1    BY: Ruma Bibra       DATE: 04/11/07 ECO: *P5NG*    */
/* $Revision: 1.32.1.2 $    BY: Rohan Golatkar   DATE: 12/11/08 ECO: *Q20Q*    */
/*-Revision end---------------------------------------------------------------*/
...
/*324**************************************************************************
 *1. 在做12.17.19中DRP移库时，增加一个“是否考虑最小包装量(SNP)”的选项，用户操作时，可选择移库时，是否考虑SNP;
 *2. 最小包装量取1.4.17中 地点为SV-0710的 ptp_ord_mult的字段；
 *3. 当考虑SNP选项为“Yes”时，只按SNP的整数倍移库；
 *******************************************************************************/
...
define variable l_totalqty like ds_qty_all no-undo.

/*324*/ define variable vmult like mfc_logical initial yes no-undo.
...
/*1*/   define variable q_mult like pt_ord_mult.
   ...
   l_totalqty = 0.
   for each dss_mstr
       where dss_mstr.dss_domain = global_domain and (  (dss_nbr >= nbr)
        and (dss_nbr <= nbr1)
...
/*324*/            {gprun.i ""xxdspkall.p""
      ...
      for each ds_det no-lock
          where ds_det.ds_domain = global_domain and  ds_nbr = dss_nbr
           ...
           and can-find(first lad_det  where lad_det.lad_domain = global_domain
         ...
         then do:
            ...
            find tm_mstr  where tm_mstr.tm_domain = global_domain and  tm_code
            = dss_shipvia
               no-lock no-error.
         ...
         for each lad_det no-lock
            ...
            then do:
               find laddet
                  where recid(laddet) = recid(lad_det)
         ...
         then do:
            find dsdet
               where recid(dsdet) = recid(ds_det)
            exclusive-lock no-error.
            if available dsdet
            then
               assign
