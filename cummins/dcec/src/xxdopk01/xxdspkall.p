...
/* $Revision: 1.5.1.6.1.2 $  BY: Rohan Golatkar         DATE: 12/11/08   ECO: *Q20Q* */
/*-Revision end---------------------------------------------------------------*/
...
/*324*/ define input parameter p_mult like mfc_logical no-undo.
define output parameter p_totalqty like ds_qty_all no-undo.
...
/*324*/  define variable q_mult like pt_ord_mult initial 1.
/*324*/  define variable q_qty  like ld_qty_oh.
define stream b.
...
/*324*/ {xxdspkall.i &sort1 = "(if icc_pk_ord = 1 then ld_loc else ld_lot)" }
      end.
      else do:
/*324*/ {xxdspkall.i &sort1 = "(if icc_pk_ord = 3 then ld_date else ld_expire)" }
      end.
   end.
else do:
   if icc_pk_ord <= 2 then do:
