...
&Scoped-define INTERNAL-TABLES xxtmppi

/* Definitions for BROWSE brList                                        */
&Scoped-define FIELDS-IN-QUERY-brList xxpi_list xxpi_cs xxpi_part xxpi_curr xxpi_um xxpi_start xxpi_expir xxpi_amt xxpi_chk   
&Scoped-define ENABLED-FIELDS-IN-QUERY-brList   
&Scoped-define SELF-NAME brList
&Scoped-define QUERY-STRING-brList FOR EACH xxtmppi
&Scoped-define OPEN-QUERY-brList OPEN QUERY {&SELF-NAME} FOR EACH xxtmppi.
&Scoped-define TABLES-IN-QUERY-brList xxtmppi
&Scoped-define FIRST-TABLE-IN-QUERY-brList xxtmppi
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brList wWin _FREEFORM
  QUERY brList DISPLAY
      xxpi_list    column-label "价格单号"
      xxpi_cs      COLUMN-LABEL "客户"
      xxpi_part    column-label "零件编码"
      xxpi_curr    column-label "货币"
      xxpi_um      column-label "计量单位"
      xxpi_start   column-label "开始日期"
      xxpi_expir   column-label "结束日期"
      xxpi_amt     column-label "单价"
      xxpi_chk     column-label "结果"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brList
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH xxtmppi.
     _END_FREEFORM
     _Query            is OPENED
*/  /* BROWSE brList */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* 销售价格表装入 */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-RESIZED OF wWin /* 销售价格表装入 */
DO:
...
DO:
  ...
  find first xxtmppi no-lock where xxpi_chk <> "" no-error.
  if available xxtmppi then do:
  ...
  SYSTEM-DIALOG GET-FILE vfile
    ...
    if search(fifile) <> ? then do:
       if selet then do:
          {gprun.i ""xxpild1.p""}
          OPEN QUERY brList FOR EACH xxtmppi.
          if can-find(first xxtmppi) then brlist:REFRESH() NO-ERROR.
       end.
    end.
    ASSIGN fiFile:SCREEN-VALUE = vfile.
    SESSION:SET-WAIT-STAT("").
...
DO:
  ...
  for each xxtmppi exclusive-lock: delete xxtmppi. end.
     {gprun.i ""xxpild0.p"" "(input fiFile)" }
     {gprun.i ""xxpild1.p""}
      DO TRANSACTION:
