...
/* $Revision: 1.26.2.1 $   BY: R.McCarthy/T.Giustozzi DATE: 03/10/04  ECO: *P15V* */
...
for first wo_mstr
fields (wo_domain wo_acct wo_sub wo_cc
...
where wo_domain = global_domain and wo_lot = cumwo_lot no-lock:
...
where si_domain = global_domain and si_site = wo_site no-lock:
...
{&REBKFLIS-P-TAG10}
where pk_domain = global_domain and pk_user = mfguser no-lock:
   ...
   if pk_qty <> 0 then do:
      ...
      if available pt_mstr then do:
         ...
         if available pl_mstr then do:
            ...
            where pld_domain = global_domain and pld_prodline = pl_prod_line
      ...
      repeat:
         ...
         do transaction:
            ...
            find wo_mstr where wo_domain = global_domain and wo_lot = cumwo_lot exclusive-lock.

            find wod_det where wod_domain = global_domain and wod_lot = wo_lot
            ...
            find wr_route where wr_domain = global_domain and wr_lot = wo_lot and wr_op = op
            exclusive-lock no-error .
            ...
            if wo_costing_mthd = "AVG" then do:
               assign
                  ...
                  if available wr_route then                   /*tfq*/
               ...
               if var_amt <> 0 then do:
                  ...
                  create trgl_det. trgl_domain = global_domain.
                  assign
                     trgl_trnbr = tr_trnbr
                     trgl_type = "RATE VAR"
               ...
               assign
                  ...
                  wr_mtl_ll_act  = wr_mtl_ll_act + gl_amt - var_amt . /*tfq*/
