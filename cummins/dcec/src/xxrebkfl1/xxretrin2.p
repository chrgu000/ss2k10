/* GUI CONVERTED from retrin2.p (converter v1.78) Fri Oct 29 14:37:53 2004 */
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   set part op line with frame a editing:
     ...
     {mfnp05.i pt_mstr pt_part  " pt_mstr.pt_domain = global_domain and yes "
...
.         find ln_mstr where ln_domain = global_domain and ln_site = site and ln_line = line
.     no-lock no-error.
.
.         find first lnd_det
.         where ln_domain = global_domain and lnd_line = line
.         and lnd_site = site
.         and lnd_part = part
...
.            find first lnd_det where ln_domain = global_domain and lnd_site = site and lnd_part = part
.            no-lock no-error.
.
 ...
 *
 *   if line <> "" then do:
 *      find ln_mstr where ln_domain = global_domain and ln_site = site and ln_line = line no-lock no-error.
 *
 *      if not available ln_mstr then do:
 *   {mfmsg.i 8526 3}
 *   next-prompt line.
 ...
 *
 *      find first lnd_det
 *      where lnd_domain = global_domain and lnd_line = line
 *      and lnd_site = site
 *      and lnd_part = part
 *      and (lnd_start <= today or lnd_start = ?)
 *      no-lock no-error.
 ...
 *   end.
 *   else do:
 *      find first lnd_det where lnd_domain = global_domain and lnd_site = site and lnd_part = part
 *      no-lock no-error.
 *
 *      if available lnd_det then do:
 *   {mfmsg.i 5107 3}
