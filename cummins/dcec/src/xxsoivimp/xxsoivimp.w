...
&Scoped-define FIELDS-IN-QUERY-brList tso_nbr "订单号" tso_cust "销往" tso_bill "发票地址" tso_ship "货物发往" tso_ord_date "订货日期" tso_rmks "备注" tso_site "地点" "TEMP" "库位" tso_channel "通道" tso_tax_usage "税用途" tsod_line "项次" tsod_part "料号" tsod_site "地点" tsod_qty_ord "数量" "M" "类别" tsod_acct "账户" tsod_sub "分账户" tsod_cc "分账户" tsod_project "分账户" tsod_chk "状态"   
...
tsod_cc         column-label    "分账户"
tsod_project    column-label    "分账户"
...
DEFINE FRAME fMain
     fiFile AT ROW 1.56 COL 5.13 COLON-ALIGNED
     btnOpen AT ROW 1.56 COL 48
     btnExp AT ROW 1.56 COL 60
     btnGenCimFile AT ROW 1.56 COL 70
     tnLoad AT ROW 1.56 COL 82
     brList AT ROW 3.19 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "待开发票装入(xxsoivimp.p)"
         HEIGHT             = 31
         WIDTH              = 101.13
         MAX-HEIGHT         = 42.88
         MAX-WIDTH          = 170.75
         VIRTUAL-HEIGHT     = 42.88
...
ON END-ERROR OF wWin /* 待开发票装入(xxsoivimp.p) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* 待开发票装入(xxsoivimp.p) */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-RESIZED OF wWin /* 待开发票装入(xxsoivimp.p) */
DO:
...
DO:
  ...
  ASSIGN GFILE = fifile.
    SYSTEM-DIALOG GET-FILE gfile
        ...
        UPDATE gfret.
        if gfret then do:
           {gprun.i ""xxsoivimp1.p"" "(input gfile)"}
        end.
...
{src/adm2/windowmn.i}
  ...
  FIND FIRST usrw_wkfl NO-LOCK WHERE usrw_domain = GLOBAL_domain AND usrw_key1 = execname AND usrw_key2 = global_userid NO-ERROR.
     IF AVAILABLE usrw_wkfl THEN DO:
         DISPLAY usrw_key3 @ fiFile WITH FRAME fmain.
         ASSIGN fifile.
     END.
