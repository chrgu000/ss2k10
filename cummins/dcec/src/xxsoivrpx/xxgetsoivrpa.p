/* GUI CONVERTED from soivrpa.p (converter v1.78) Mon Aug 25 03:59:37 2008 */
...
/* Revision: 1.16.1.18.1.3 BY: Praveen Sequeira    DATE: 09/19/06 ECO: *P56M* */
/* Revision: 1.16.1.18.1.4 BY: Rafiq S.            DATE: 11/15/06 ECO: *P4YY* */
/* Revision: 1.16.1.18.1.5  BY: Anuradha K.        DATE: 04/10/07 ECO: *P5SG* */
/* $Revision: 1.16.1.18.1.6 $ BY: Mallika Poojary               DATE: 08/14/08 ECO: *P6Z8* */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
...
define            variable l_printonly    as integer initial 1 no-undo.
...
define            variable l_conso_done    like mfc_logical no-undo initial no.
define            variable l_acct          like glt_acct           no-undo.
define            variable l_sub           like glt_sub            no-undo.
define            variable l_cc            like glt_cc             no-undo.
define            variable l_diff          like gltw_amt           no-undo.
...
define new shared variable m_inv_nbr       like so_inv_nbr         no-undo.
define     shared variable m_cons_cnt      as   integer init 0     no-undo.
...
define variable l_mixed_rpt         like mfc_logical     no-undo.
...
with STREAM-IO /*GUI*/  frame prepd width 80
   side-labels.
...
/*36  /* Invoice Header */                                                   */
/*36  if page-size - line-counter <= 11 then page.                           */
/*36  do with frame h1:                                                      */
/*36     /* SET EXTERNAL LABELS */                                           */
/*36     setFrameLabels(frame h1:handle).                                    */
/*36     display                                                             */
/*36        so_inv_nbr                                                       */
/*36        so_bill                                                          */
/*36        bill_name                                                        */
/*36        so_cust                                                          */
/*36        name                                                             */
/*36        so_slspsn[1] label "Salespsn"                                    */
/*36        so_ar_acct                                                       */
/*36        so_ar_sub                                                        */
/*36        so_ar_cc                                                         */
/*36     with frame h1 width 132 STREAM-IO /*GUI*/ .                         */
/*36  end. /* do with frame h1 */                                            */

/*36*/   v36_so_invnbr      = so_inv_nbr.
/*36*/   v36_so_bill        = so_bill.
/*36*/   v36_so_bill_name   = bill_name.
/*36*/   v36_so_cust        = so_cust.
/*36*/   v36_so_cust_name   = name.
/*36*/   v36_so_slspsn      = so_slspsn[1].
/*36*/   v36_so_ar_acct     = so_ar_acct.
/*36*/   v36_so_ar_sub      = so_ar_sub.
/*36*/   v36_so_ar_cc       = so_ar_cc.
/*36*/   v36_so_tax_env     = so_tax_env.
/*36*/   v36_so_tax_usage   = so_tax_usage.

/*36   do with frame env:                                                    */
/*36      /* SET EXTERNAL LABELS */                                          */
/*36      setFrameLabels(frame env:handle).                                  */
/*36      display                                                            */
/*36         so_tax_env at 60                                                */
/*36         so_tax_usage                                                    */
/*36      with frame env width 132 side-labels no-box STREAM-IO /*GUI*/ .    */
/*36   end. /* if */                                                         */
...
/*36  /*GUI*/ {mfguichk.i  &loop="soloop"} /*Replace mfrpchk*/   */
      ...
      then do:
         ...
         /*TEMPORARY CONSOLIDATED TAX RECORD IS CREATED IN TX2D_DET */
         /*THIS IS ONLY FOR DISPLAY PURPOSE, ON EXIT OF PENDING INVOICE REGISTER*/
         /*THIS RECORD WILL BE DELETED IN SOIVRP.P*/
         ...
         for each tx2d_det
            where tx2d_domain = global_domain
              and tx2d_ref    = m_inv_nbr
         exclusive-lock:
            assign tx2d_nbr = 'REGISTER'.
         end.
        ...
        {gprun.i ""soivtrl2.p""
         ...
         /*FIND THE CONSOLDIATED TAX AMT AND ADD THE */
         /*DIFFRENCE TO THE ORDER AMT AND UPDATE THE */
         /*TAX FIELD ACCORDINGLY */

         assign
            l_diff         = 0
            l_diff         = tax_amt - invtot_tax_amt
            invtot_ord_amt = invtot_ord_amt + l_diff
            invtot_tax_amt = tax_amt
            tot_base_amt   = tot_base_amt + l_diff.
        ...
        /*MODIFY GL ENTRIES FOR CONSOLIDATED RECORDS*/
         for first tx2d_det
            where tx2d_domain = global_domain
              and tx2d_ref    = m_inv_nbr
              and tx2d_nbr    = 'REGISTER' no-lock:
         end.
         ...
         then do:
            ...
            where gltw_wkfl.gltw_domain = global_domain
               and   gltw_acct    = l_acct
               and   gltw_sub     = l_sub
               and   gltw_cc      = l_cc
               and   gltw_project = ""
               and   gltw_ref     = ""
               and   gltw_userid  = mfguser
               and   gltw_entity  = tx2d_trans_ent
            exclusive-lock:
               gltw_amt = gltw_amt - l_diff.
         ...
         for first gltw_wkfl
         where gltw_wkfl.gltw_domain = global_domain
         and   gltw_acct    = so_ar_acct
         and   gltw_sub     = so_ar_sub
         and   gltw_cc      = so_ar_cc
         and   gltw_project = ""
         and   gltw_ref     = mfguser
         and   gltw_userid  = mfguser
         and   gltw_entity  = post_entity
         exclusive-lock:
            gltw_amt = gltw_amt + l_diff.
         end. /*for first gltw_wkfl*/
      end. /*l_conso_done = true*/
...
/*36         tot_prepaid_nett                                                */
/*36         amt_due_af_prep                                                 */
/*36      with frame prepd STREAM-IO /*GUI*/ .                               */
      else do:
         {gprunp.i "mcpl" "p" "mc-curr-conv"
               "(input  so_curr,
                 input  et_sec_curr,
...
/*36               tot_prepaid_nett                                          */
/*36               amt_due_af_prep                                           */
/*36               et_tot_prepaid_amt                                        */
/*36               et_tot_ptax_amt                                           */
/*36               et_tot_prepaid_nett                                       */
/*36               et_amt_due_af_prep                                        */
/*36         with frame prepdeuro STREAM-IO /*GUI*/ .                        */
      end.
   end.
...
PROCEDURE get_inv_nbr:
   m_cons_cnt = m_cons_cnt + 1.
   m_inv_nbr = 'IV' + substring(mfguser,4,4) + STRING(m_cons_cnt).
END. /*PROCEDURE get_inv_nbr*/
