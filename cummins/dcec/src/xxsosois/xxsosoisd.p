...
/* Revision: 1.100.1.6  BY: Mochesh Chandran      DATE: 08/08/06  ECO: *Q0X9* */
/* Revision: 1.100.1.7  BY: Mochesh Chandran      DATE: 10/11/06  ECO: *P54H* */
/* Revision: 1.100.1.10 BY: Rijoy Ravindran       DATE: 10/10/08  ECO: *P70J* */
/* Revision: 1.100.1.18 BY: Xiaolei Meng          DATE: 03/02/09  ECO: *Q2HS* */
/* Revision: 1.100.1.19 BY: Hemant Dhavse         DATE: 03/17/09  ECO: *Q2KX* */
/* Revision: 1.100.1.24 BY: Xiaolei Meng          DATE: 04/15/09  ECO: *Q2M9* */
/* Revision: 1.100.1.27 BY: Rijoy Ravindran       DATE: 07/14/09  ECO: *Q32T* */
/* Revision: 1.100.1.28 BY: Rajalaxmi Ganji       DATE: 10/22/09  ECO: *Q3HY* */
/* Revision: 1.100.1.29 BY: Niranjan Ranka        DATE: 11/09/09  ECO: *Q3LN* */
/* Revision: 1.100.1.31 BY: Niranjan Ranka        DATE: 11/09/09  ECO: *Q3MX* */
/* $Revision: 1.100.1.32 $       BY: Trupti Khairnar      DATE: 11/25/10 ECO: *Q4HP* */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
...
define variable l_rma_eu           like rma_enduser no-undo.
define variable l_rmd_part         like rmd_part    no-undo.
define variable l_rmd_ref          like rmd_ref     no-undo.
define variable l_rma_avail        like mfc_logical no-undo.
...
(if sorec = fsseoship or sorec = fsktship then 4 else 5) down width 80.
{&SOSOISD-P-TAG14}
...
repeat transaction:
   ...
   repeat:
      ...
      do on error undo, retry:
         ...
         if sorec <> fssodoship
            and not can-find (first pl_mstr
        ...
        and sod_fsm_type = "RMA-ISS"
         ...
         /* SINCE mfnp01.i COULD NOT BE CHANGED,SO THAT IT WON'T SHOW */
         ...
         if sorec = fssodoship then do:
            ...
            then do:
               ...
               {pxmsg.i &MSGNUM=8861 &ERRORLEVEL=3}
               if batchrun then
                  undo loop0, leave loop0.
               else
                  undo, retry.
            ...
            then do:
               ...
               {pxmsg.i &MSGNUM=8852 &ERRORLEVEL=3}
               if batchrun then
                  undo loop0, leave loop0.
               else
                  undo, retry.
            ...
            then do:
               ...
               then do:
                  /* Open MO's exist */
                  {pxmsg.i &MSGNUM=783 &ERRORLEVEL=3}
                  if batchrun then
                     undo loop0, leave loop0.
                  else
                     undo, retry.
               end.
            end. /* if can-find(first itm_det */
            else do:
               ...
               then do:
                  ...
                  {pxmsg.i &MSGNUM=10971 &ERRORLEVEL=3}
                  if batchrun then
                     undo loop0, leave loop0.
                  else
                     undo, retry.
...
/*Q011*/ if lotserial_qty >= 0 then do:
/*Q011*/   {mfmsg.i 8441 3}
/*Q011*/    undo setlot,retry.
/*Q011*/ end.
         ...
         then do:
            ...
            then
               ...
               if     available rmd_det
                  and available rma_mstr
               then
                  assign
                     l_rma_eu    = rma_enduser
                     l_rmd_part  = rmd_part
                     l_rmd_ref   = rmd_ref
                     l_rma_avail = yes.
         end. /* IF multi_entry and ..*/
         ...
         if multi_entry then do:
            ...
            ship_line = sod_line.
            /* Validate location */
            {gprun.i ""sosoisu2.p"" "(input p-site-to,
                                      input p-loc-to)"}
         ...
         no-lock:
            ...
            then do:
               for first tt_ssm_sod_det
                  ...
                  and   tt_ssm_sod_det.tt_ssm_sod_line = sod_line
               no-lock:
               end. /* FOR FIRST tt_ssm_sod_det */
               if not available tt_ssm_sod_det
               then do:
                  create tt_ssm_sod_det.
                  assign
                     tt_ssm_sod_det.tt_ssm_sod_nbr  = sod_nbr
