...
/* Revision: 1.34.1.81.1.9  BY: Hemant Dhavse      DATE: 03/17/08 ECO: *Q2KX* */
/* Revision: 1.34.1.81.1.10 BY: Xiaolei Meng       DATE: 04/15/09 ECO: *Q2M9* */
/* Revision: 1.34.1.81.1.11 BY: Jean Miller        DATE: 09/02/09 ECO: *Q3C7* */
/* $Revision: 1.34.1.81.1.12 $             BY: Niranjan Ranka     DATE: 11/30/09 ECO: *Q3MX* */
/*-Revision end---------------------------------------------------------------*/
   ...
   do on error undo mainloop, leave mainloop:
      ...
      do transaction:
               ...
               if frame-field = "so_nbr" then do:
                  ...
                  if sorec = fsrmarec or sorec = fsrmaship then do:
                     ...
                     {mfnp05.i
                        ...
                        (so_fsm_type  = "" "" or so_fsm_type = 'FSM-RO')"
                        so_nbr
                        "input so_nbr"}
                  end.
                  if recno <> ? then do:
         ...
         if so_stat <> "" then do:
            ...
            {pxmsg.i &MSGNUM=623 &ERRORLEVEL=2 &MSGARG1=so_stat}
         end.
...
PROCEDURE p_err_msg:
   ...
   if fsmtype = "SEO" then do:
      ...
      {pxrun.i &PROC  = 'validateDepotCall' &PROGRAM = 'fssmxr.p'
               &HANDLE = ph_fssmxr
      ...
      /* OF DECEVING, BUT THAT IS THE TRUTH */
         ...
         if so_mstr.so_conf_date = ? then do:
            ...
            {pxmsg.i &MSGNUM=8839 &ERRORLEVEL=3}
         end.
         else if so_mstr.so_stat <> "" then do:
            err_chk = yes.
            /* ORDER STATUS IS NOT BLANK */
            {pxmsg.i &MSGNUM=7314 &ERRORLEVEL=3}
         end.
         else
            sorec = fssodoship.
      end.
...
PROCEDURE p-recalc-so-tax-det:
   ...
   if so_mstr.so_fsm_type = "" or so_mstr.so_fsm_type = "FSM-RO" and
      l_recalc
   then do:
