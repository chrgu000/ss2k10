/**
 ...
 @History: 
**/
/* $Revision:eb21sp12  $ BY: Jordan Lin            DATE: 08/15/12  ECO: *SS-20120815.1*   */
...
REPEAT ON ERROR UNDO, LEAVE :
    ...
    i = 2 .
    FOR EACH ar_mstr NO-LOCK WHERE  /* *SS-20120815.1*   */ ar_mstr.ar_domain = global_domain and ar_bill >= cust AND ar_bill <= cust1
        ...
        FIND ad_mstr NO-LOCK WHERE  /* *SS-20120815.1*   */ ad_mstr.ad_domain = global_domain and ad_addr = ar_bill NO-ERROR .
        ...
        FIND FIRST glt_det NO-LOCK WHERE /* *SS-20120815.1*   */ glt_det.glt_domain = global_domain and glt_batch = ar_batch
            AND glt_doc = ar_nbr
            AND glt_doc_type = ar_type
            USE-INDEX glt_batch NO-ERROR .
        IF AVAILABLE glt_det THEN excelsheetmain:cells(i,5) = glt_ref .
        ELSE DO :
            FIND FIRST gltr_hist NO-LOCK WHERE /* *SS-20120815.1* */ gltr_hist.gltr_domain = global_domain and gltr_batch = ar_batch
                AND gltr_doc = ar_nbr
        ...
        mtax = 0 .
        FOR EACH ard_det NO-LOCK WHERE /* *SS-20120815.1* */ ard_det.ard_domain = global_domain and ard_nbr = ar_nbr AND ard_acc = "2171005" :
        ...
        IF ar_type = "I" THEN DO :
            FIND FIRST idh_hist NO-LOCK WHERE /* *SS-20120815.1* */ idh_hist.idh_domain = global_domain and idh_inv_nbr = ar_nbr NO-ERROR .
            IF AVAILABLE idh_hist THEN excelsheetmain:cells(i,8) = idh_nbr .

            IF s = NO THEN DO :
                FOR EACH idh_hist NO-LOCK WHERE /* *SS-20120815.1* */ idh_hist.idh_domain = global_domain and idh_inv_nbr = ar_nbr :
                    FIND pt_mstr NO-LOCK WHERE /* *SS-20120815.1* */ pt_mstr.pt_domain = global_domain and pt_part = idh_part NO-ERROR .
