...
{mfdtitle.i "130407.1"}
define variable flushdate like tr_effdate . /*format "99/99/9999".*/
define variable flushdate1 like tr_effdate. /* format "99/99/9999".*/
define variable effdate  like tr_effdate.  /* format "99/99/9999".*/
define variable effdate1 like tr_effdate.  /*format "99/99/9999".*/
define variable site like tr_site .
define variable model LIKE wo_part.
define variable model1 LIKE wo_part.
define variable bktotal as integer .
define variable bkall as integer.
define variable plall AS INTEGER.
define variable yn AS LOGICAL INIT YES LABEL "明细Y/汇总N".
define TEMP-TABLE xxrps
    FIELD xxrps_part LIKE pt_part
    FIELD xxrps_qty LIKE tr_qty_loc
    FIELD xxrps_site LIKE pt_site
    index xxrps_part is primary xxrps_part.
...
REPEAT:
  ...
  if flushdate1 = hi_date then flushdate1 = ?.
    ...
    {mfselprt.i "printer" 132}
          plall = 0.
          FOR EACH rps_mstr WHERE rps_domain = global_domain and rps_part >= model AND rps_part <= model1
   ...
   IF yn = NO THEN DO:
            ...
            PUT "-------------------- ------------------------------- --------    -------- ------   ----" SKIP.
            FOR EACH tr_hist fields(tr_domain tr_type tr_effdate
                tr_loc tr_site tr_loc tr_date tr_part tr_userid tr_qty_loc)
                use-index tr_eff_trnbr
                where tr_domain = global_domain
                and (tr_effdate >= effdate and tr_effdate <= effdate1)
                and tr_type ="rct-wo" and tr_loc ="FG"
                and (tr_site = site or site = "")
                and (tr_date >= flushdate and tr_date <= flushdate1)
                AND tr_part >= model  AND tr_part <= model1
                AND (tr_userid ="MRP" OR tr_userid ="Admin" or tr_userid = "mfg" or tr_userid = "qaduser") NO-LOCK break by tr_part.
   ...
   IF yn = YES THEN DO:
            ...
            FOR EACH tr_hist fields(tr_domain tr_type tr_effdate
                ...
                and (tr_site = site or site = "")
                and (tr_date >= flushdate and tr_date <= flushdate1)
                AND tr_part >= model AND tr_part <= model1  AND
