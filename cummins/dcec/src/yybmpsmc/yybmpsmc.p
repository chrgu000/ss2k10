...
repeat:
 ...
 part_yn = yes.                         /*kevin*/
   find pt_mstr no-lock where pt_domain = global_domain and pt_part = part no-error.
   ...
   if not can-find (first ps_mstr where ps_domain = global_domain and ps_comp = part) then do:
      {pxmsg.i &MSGNUM=100 &ERRORLEVEL=3}
      undo, retry.
   end.

/*1205*/   if  action = "R" then do:
/*1205*/       find first vp_mstr no-lock where vp_domain = global_domain
/*1205*/              and vp_part = part no-error.
/*1205*/       if available vp_mstr then do:
/*1205*/          {pxmsg.i &MSGTEXT=""不允许替换指定供应商的零件""
/*1205*/                   &ERRORLEVEL=3}
/*1205*/          undo,retry.
/*1205*/       end.
/*1205*/   end.
   ...
   if action = "A" or action = "R" then do on error undo, retry:
      ...
      find pt_mstr no-lock where pt_domain = global_domain and pt_part = part1 no-error.
      if available pt_mstr then do with frame b:

         ptstatus = pt_status.
         substring(ptstatus,9,1) = "#".
         if can-find(isd_det where isd_domain = global_domain
                 and isd_status = ptstatus and isd_tr_type = "ADD-PS")
         then do:
...
/****************end added by kevin**************/
   ...
   do transaction:
      ...
      if can-find (first ps_mstr where ps_domain = global_domain and ps_comp = part)
      then do:
      ...
      for each ps_mstr where ps_domain = global_domain and ps_comp = part
            and (effdate = ?
         ...
         if action = "A" or action = "R" then do:
            ...
            if not available psmstr then do:
               ...
               for each psmstr no-lock where psmstr.ps_domain = global_domain
                    and psmstr.ps_par = ps_mstr.ps_par
                    and psmstr.ps_comp = part1
                    and psmstr.ps_ref = ps_mstr.ps_ref
                    and psmstr.ps_ps_code <> "J"
               ...
               if action = "R" then do:
/*1205*/          find first xxptmp_mstr where xxptmp_domain = global_domain
/*1205*/                and xxptmp_par = ps_mstr.ps_par
/*1205*/                and xxptmp_comp = ps_mstr.ps_comp no-error.
/*1205*/          if not available xxptmp_mstr then do:
...
/*1205*/          end.
               ...
               for each in_mstr exclusive-lock where in_domain = global_domain
                  and in_part = ps_mstr.ps_comp
                  and not can-find (ptp_det where ptp_domain = global_domain
                                and ptp_part = in_part and ptp_site = in_site):
               ...
               for each ptp_det where ptp_domain = global_domain
                  ...
                  find in_mstr where in_domain = global_domain
                   and in_part = ptp_part and in_site =  ptp_site
