...
find first bom_mstr where bom_domain = global_domain and bom_par = old_part no-lock no-error.
if not avail bom_mstr then do:
	find first pt_mstr where pt_domain = global_domain and pt_part = old_part no-lock no-error.
...
find first bom_mstr where bom_domain = global_domain and bom_par = new_part no-lock no-error.
if not avail bom_mstr then do:
	find first pt_mstr where pt_domain = global_domain and pt_part = new_part no-lock no-error.
	if not avail pt_mstr then do:
...
do transaction on error undo,retry:
		for each ps_mstr where ps_domain = global_domain and ps_par 		= old_part 
											 and (ps_start	<= effdate or ps_start = ?)
											 and (ps_end		>= effdate or ps_end = ?):
				find first psmstr where psmstr.ps_domain = global_domain and psmstr.ps_par = ps_mstr.ps_comp
				...
				find first psmstr where psmstr.ps_domain = global_domain and psmstr.ps_par = new_part
		...
		for each ps_mstr where ps_domain = global_domain and ps_comp 		= old_part 
											 and (ps_start	<= effdate or ps_start = ?)
											 and (ps_end		>= effdate or ps_end = ?):
				find first psmstr where psmstr.ps_domain = global_domain and psmstr.ps_par = ps_mstr.ps_par 
														and psmstr.ps_comp = new_part 
														and (psmstr.ps_start <= effdate or psmstr.ps_start = ?)
														and (psmstr.ps_end		>= effdate or psmstr.ps_end = ?)
														no-lock no-error.
		...
		for each copyps:
         ...
         for each in_mstr exclusive-lock where in_domain = global_domain and in_part = ps_mstr.ps_comp
            and not can-find (ptp_det where ptp_domain = global_domain and ptp_part = in_part
                                        and ptp_site = in_site):
            if available in_mstr then
               in_level = 99999.
         end.

         for each ptp_det where ptp_domain = global_domain and ptp_part = ps_mstr.ps_comp:
            find in_mstr where in_domain = global_domain and in_part = ptp_part and
                               in_site =  ptp_site
		...
		for each ps_mstr where ps_mstr.ps_domain = global_domain and ps_mstr.ps_comp 		= old_part 
		...
		for each ps_mstr where ps_mstr.ps_domain = global_domain and ps_mstr.ps_par = old_part 
											 and (ps_mstr.ps_start	> effdate ):
				find first psmstr where psmstr.ps_domain = global_domain and psmstr.ps_par = ps_mstr.ps_comp
														and psmstr.ps_comp = new_part 
														and (psmstr.ps_start <= effdate or psmstr.ps_start = ?)
