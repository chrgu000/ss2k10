...
{mfdeclre.i}
if action = "D" then do:
for each xxptmp_mstr where xxptmp_domain = global_domain and xxptmp_comp = part:
delete xxptmp_mstr .
end.
end.
...
create xxptmp-mstr. xxptmp-mstr.xxptmp_domain = global_domain.
...
do :
repeat :
find next xxptmp_mstr where xxptmp_domain = global_domain and xxptmp_comp = part no-error .
if not available xxptmp_mstr then leave .
else do:
create xxptmp-mstr. xxptmp-mstr.xxptmp_domain = global_domain.
assign xxptmp-mstr.xxptmp_par = xxptmp_mstr.xxptmp_par 
