...
FORM /*GUI*/ 
 ...
 SKIP(.1)  /*GUI*/
  /* *SS-20120824.1*   */ site           colon 25       si_desc no-label at 47 skip(1) 
ps_par   colon 25
   bomdesc  colon 25
 SKIP(.4)  /*GUI*/
...
FUNCTION chkcomp RETURNS LOGICAL (INPUT comp AS CHAR, INPUT site AS CHAR) :
    ...
    FIND pt_mstr NO-LOCK WHERE pt_mstr.pt_domain = global_domain and  pt_part = comp NO-ERROR .
    FIND bom_mstr NO-LOCK WHERE bom_mstr.bom_domain = global_domain and bom_parent = comp NO-ERROR .
    ...
    FIND FIRST ps_mstr NO-LOCK WHERE  ps_mstr.ps_domain = global_domain and  ps_par = comp AND ps__chr01 <> site NO-ERROR .
    ...
    IF AVAILABLE pt_mstr THEN DO :
        find first ptp_det where ptp_det.ptp_domain = global_domain and  ptp_site = site and ptp_part = comp no-lock no-error.
...
/*GUI*/ if global-beam-me-up then undo, leave.
  /* *SS-20120824.1* -b  */
   find first xxbomc_ctrl where  /* *SS-20120824.1*   */ xxbomc_ctrl.xxbomc_domain = global_domain  no-lock no-error.                
...
/*added by kevin, 10/16/2003 for site control*/
            fc_type = no.
            update site with no-validate frame a
	    editing :
                {mfnp.i si_mstr site si_site site si_site si_site}
                if recno <> ? then
                    disp si_site @ site si_desc with no-validate frame a.
            end.
                 find si_mstr no-lock where  /* *SS-20120824.1*   */ si_mstr.si_domain = global_domain and  si_site = site no-error.
...
/*added by kevin, 10/20/2003 for site control*/
          if available pt_mstr then do:
                find ptp_det where ptp_det.ptp_domain = global_domain and  ptp_site = site and ptp_part =  
                cPar no-lock no-error.
                if not available ptp_det then do:
                  message "该零件的地点计划数据不存在,请重新输入!" view-as alert-box error.
                    undo,retry.
                end.
...
/*cj*/ FIND bom_mstr WHERE bom_domain = global_domain and
 ...
 /* *SS-20120824.1*  -b */
   ...
   if not available pt_mstr and bom__chr01 <> site 
        ...
        find first ps_mstr where /* *SS-20120824.1*   */ ps_mstr.ps_domain = global_domain and  ps_par = cPar and ps__chr01 <> site no-lock no-error. 
            if available ps_mstr and not fc_type then do:
                 message "该父零件存在 '" + ps__chr01 + "'地点的产品结构" view-as alert-box error.
                  undo,retry.
            end.
            find first ps_mstr where /* *SS-20120824.1*   */ ps_mstr.ps_domain = global_domain and ps_comp = cPar and ps__chr01 <> 
...
/*GUI*/ if global-beam-me-up then undo, leave.
      ...
      if c-application-mode <> "API" then do:
         prompt-for
            ps_comp  /* *SS-20120824.1*   */ VALIDATE(chkcomp(ps_comp,site),"不合法，请重新输入！")
            ps_ref
            ps_start
         ...
         editing:
            ...
            then do:
               ...
               then do:
                  ...
                  for first pt_mstr
                     fields( pt_domain pt_desc1 pt_desc2 pt_formula pt_joint_type
                             pt_part pt_rev pt_status pt_um  /* *SS-20120824.1*   */ pt_group)
                  ...
                  for first bom_mstr
                     fields( bom_domain bom_batch bom_batch_um bom_desc bom_formula
                              bom_fsm_type bom_mod_date bom_parent bom_userid  /* *SS-20120824.1*   */ bom__chr01)
                     no-lock  where bom_mstr.bom_domain = global_domain and
                     bom_parent = ps_comp:
                  end. /* for first bom_mstr */
...
/*cj*/   find first ps_mstr exclusive-lock
	  ...
	  ps_par = cPar and ps_comp = cComp and cRef = ps_ref and ps_start = dtStart no-error.
