...
repeat:
   ...
   find bom_mstr where  /* *SS-20120816.1*   */ bom_mstr.bom_domain = global_domain and bom_parent = parent no-lock no-error.
   ...
   find first ps_mstr use-index ps_parcomp where /* *SS-20120816.1*   */ ps_mstr.ps_domain = global_domain and ps_par = parent
   no-lock no-error.
   ...
   repeat: /*for expand the ps*/
		  ...
		  repeat:
		     ...
		     no-lock no-error.
		     comp = ps_par.
		     find next ps_mstr use-index ps_parcomp where  /* *SS-20120816.1*   */ ps_mstr.ps_domain = global_domain and ps_par = comp
		     no-lock no-error.
	       ...
	       and (ps_end = ? or effdate <= ps_end)) then do:
                ...
                find ptp_det where /* *SS-20120816.1*   */ ptp_det.ptp_domain = global_domain and ptp_site = ps__chr01 and ptp_part = ps_comp no-lock no-error.
                ...
                ((available ptp_det and not ptp_phantom)
                  ...
                  (not available ptp_det and not pt_phantom)) then do:
                      ...
                      find first opm_mstr where /* *SS-20120816.1*   */ opm_mstr.opm_domain = global_domain and opm_std_op = string(ps_op) no-lock no-error.
                      if available opm_mstr then do:
                            assign xxwk.wkctr = opm_wkctr.
                            
                            find wc_mstr where /* *SS-20120816.1*   */ wc_mstr.wc_domain = global_domain and wc_wkctr = opm_wkctr and wc_mch = opm_mch no-lock no-error.
		  ...
		  then do:
		     comp = ps_comp.
		     level = level + 1.
		     find first ps_mstr use-index ps_parcomp where /* *SS-20120816.1*   */ ps_mstr.ps_domain = global_domain and ps_par = comp
		     no-lock no-error.
		  end.
		  else do:
		     find next ps_mstr use-index ps_parcomp where /* *SS-20120816.1*   */ ps_mstr.ps_domain = global_domain and ps_par = comp
   ...
   for each xxwk no-lock 
       ...
       partcost = 0.
       for each pc_mstr where /* *SS-20120816.1*   */ pc_mstr.pc_domain = global_domain and pc_part = xxwk.comp 
                              and pc_start <= endate 
                        no-lock:
