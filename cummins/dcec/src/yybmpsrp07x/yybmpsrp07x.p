...
repeat:
   ...
   repeat: /*for expand the ps*/
		  ...
		  repeat:
		     ...
		     find ps_mstr where ps_domain = global_domain and recid(ps_mstr) = record[level]
		     no-lock no-error.
		     comp = ps_par.
	       ...
	       and (ps_end = ? or effdate <= ps_end)) then do:
                ...
                find ptp_det where ptp_domain = global_domain and ptp_site = ps__chr01 and ptp_part = ps_comp no-lock no-error.
		  ...
		  and ((available ptp_det  ) or (not available ptp_det and available pt_mstr  )
   ...
   FOR EACH xxwk WHERE (string(xxwk.op) >= op and string(xxwk.op) <= op1)
     ...
     IF AVAIL ptp_det THEN DO:
       ...
       FOR EACH qad_wkfl WHERE qad_key1 = "poa_det" AND qad_charfld[2] = xxwk.comp AND qad_datefld[1] <= effdate BREAK BY qad_decfld[1].
            /*BY qad_decfld[1].*/
