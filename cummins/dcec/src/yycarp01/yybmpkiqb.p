/* $Revision:eb21sp12  $ BY: Jordan Lin            DATE: 09/19/12  ECO: *SS-20121022.1*   */
    ...
    repeat:                 
        comp = part.
/*G234*/    /* Added section*/
        find ptp_det no-lock where ptp_domain = global_domain and ptp_part = part
...
/*G1JF*/          by pkop :
           ...
           find pt_mstr where pt_domain = global_domain and 
           		  pt_part = pkpart no-lock no-error.
...
/*H100*/    and ((op = pkop) or (op = 0))) :
        find first ptp_det where ptp_domain = global_domain and 
        				   ptp_part = pkpart and ptp_site = site no-lock no-error .
        if available ptp_det then
        do:
                if ptp_pm_code <> "P" then
                do:
                find first ps_mstr where ps_domain = global_domain and
                					 ps_par = pkpart no-lock no-error .
