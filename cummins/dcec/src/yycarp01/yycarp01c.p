...
DEFINE VARIABLE iLoop1 AS INTEGER.
	...
	find first usrw_wkfl no-lock where usrw_domain = global_domain 
				 and usrw_key1 = nbr and usrw_key2 = "ORDER-TEST-MSTR"
...
*/	
	...
	for each xxwk:
		...
		find first in_mstr no-lock where in_domain = global_domain 
					 and in_part = xxpart and in_site = site no-error.
	...
	for each usrw_wkfl no-lock where usrw_domain = global_domain and
