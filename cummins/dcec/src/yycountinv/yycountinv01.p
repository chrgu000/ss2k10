...
&SCOPED-DEFINE PP_GUI_CONVERT_MODE REPORT
   ...
   {mfdtitle.i "13625.1"}
   def var tag             like tag_mstr.tag_nbr init ?.
   def var tag1            like tag_mstr.tag_nbr label {t001.i} init ?.
...
/*bn083*/       def var keeper1      like pt_article label "保管员".
   ...
   def var lotserial       like tag_mstr.tag_serial.
   def var lotref          like tag_mstr.tag_ref.   /*D887*/
   ...
   def buffer tagmstr for tag_mstr.
   DEF VAR page1 AS INT INIT 0.
   def var tagnumber       like tag_mstr.tag_nbr extent 21.
     tagline[01] = "一|          | ".
     tagline[02] = "一|          | ".
...
/*GUI*/   mainloop: do on error undo, return error on endkey undo, return error:
 ...
 /*D765*/
      ...
      for each tag_mstr exclusive where tag_mstr.tag_domain = global_domain
           and tag_mstr.tag_nbr >= tag and tag_mstr.tag_nbr <= tag1 
           AND tag_mstr.tag_serial = ""
      and (reprint_tags or tag_mstr.tag_prt_dt = ?): /*F460*/
          find first tagmstr use-index tag_pslsn no-lock where
                     tagmstr.tag_domain = global_domain and
                     tagmstr.tag_part = tag_mstr.tag_part and
                     tagmstr.tag_site = tag_mstr.tag_site and
                     tagmstr.tag_serial <> "" no-error.
          if available tagmstr then do:
             next.
          end.

         tag_mstr.tag_prt_dt = today.                /*F460*/
         ...
         if tag_mstr.tag_type = "I" then do:

/*GUI*/ {mfguichk.i } /*Replace mfrpchk*/
                     /*G348*/
      site = tag_mstr.tag_site.
      loc = tag_mstr.tag_loc.
      part = tag_mstr.tag_part.
      find pt_mstr where pt_domain = global_domain and pt_part = tag_mstr.tag_part no-lock no-error.
...
/*F003*/          find in_mstr where in_domain = global_domain and in_part = tag_mstr.tag_part and in_site = tag_mstr.tag_site
/*F003*/          no-lock no-error.
/*F003*/          if available in_mstr then abc = in_abc.

      lotserial = tag_mstr.tag_serial.
      lotref = tag_mstr.tag_ref.
          keeper = IN__qadc01. /*add fm268*/
         end.

           page1 = page1 + 1.
           substr(tagline[03],32,8)  = substr(string(tag_mstr.tag_nbr) + fill(" ",8),1,8).
...
/*FL74*/          else
         ...
         tagnumber[forms_built] = tag_mstr.tag_nbr.
