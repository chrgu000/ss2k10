/**
 @File: yyexpctrl.p
 @Description: 制造费用控制文件
 @Version: 1.0
 @Author: Cai Jing
 @Created: 2005-8-26
 @BusinessLogic: 制造费用控制文件
 @Todo: 
 @History: 
**/
...
REPEAT:
    ...
    UPDATE xxexp_expense VALIDATE(CAN-FIND(gra_mstr WHERE /* *SS-20120813.1*   */ gra_mstr.gra_domain = global_domain and gra_an_code = xxexp_expense AND gra_gl_type = "1"),"请输入有效的分析代码！")
        xxexp_sales VALIDATE(CAN-FIND(gra_mstr WHERE  /* *SS-20120813.1*   */ gra_mstr.gra_domain = global_domain and gra_an_code = xxexp_sales AND gra_gl_type = "1"),"请输入有效的分析代码！")
        xxexp_discount VALIDATE(CAN-FIND(gra_mstr WHERE  /* *SS-20120813.1*   */ gra_mstr.gra_domain = global_domain and gra_an_code = xxexp_discount AND gra_gl_type = "1"),"请输入有效的分析代码！")
