...
REPEAT:
    ...
    ent = "dcec".
    ent1 = "dcec".
    UPDATE expana VALIDATE(CAN-FIND(gra_mstr WHERE  /* *SS-20120813.1*   */ gra_mstr.gra_domain = global_domain and gra_an_code = expana AND gra_gl_type = "1") OR expana = "", "请输入有效的科目分析代码！")
        ccana VALIDATE(CAN-FIND(gra_mstr WHERE  /* *SS-20120813.1*   */ gra_mstr.gra_domain = global_domain and  gra_an_code = ccana AND gra_gl_type = "2") OR ccana = "", "请输入有效的成本中心分析代码！")
        ent ent1
        glyear
