...
(input location as character):
   ...
   **/
   ...
   where /* *SS-20121009.1*   */ wcIsWCLocation.wc_domain = global_domain and wc_wkctr = location
   no-error .
...
(input routingCode as character,
 input operation as integer):
   ...
   **/
   ...
   where /* *SS-20121009.1*   */ roGetWCLocation.ro_domain = global_domain and ro_routing = routingCode
   and ro_op = operation
   no-error .
...
(input site as character,
 input part as character):
   ...
   **/
   ...
   where /* *SS-20121009.1*   */ ptp_det.ptp_domain = global_domain and ptp_site = site
   and ptp_part = part
   no-error .
   ...
   if available(ptp_det) then do:
      ...
      if BOMCode = "" then do:
         ...
         where /* *SS-20121009.1*   */ pt_mstr.pt_domain = global_domain and pt_part = part
         no-error .
...
(input site as character,
 ...
 input produceDate as date):
   ...
   **/
   ...
   where  /* *SS-20121009.1*   */ shop_cal.shop_domain = global_domain and shop_site = site
   ...
   if not available(shop_cal) then
   for first shop_cal no-lock
   where /* *SS-20121009.1*   */ shop_cal.shop_domain = global_domain and  shop_site = ""
   and shop_wkctr = "":  end .
   ...
   if not available(shop_cal) then
      ...
      where /* *SS-20121009.1*   */ shft_det.shft_domain = global_domain and  shft_site = shop_site
