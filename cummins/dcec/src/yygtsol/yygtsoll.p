  ...
  */
    ...
    if gref = "无" or gref = "" then do:  /*lb01 2004-09-02 08:54*/
      ...
      next.
        ...
        find first so_mstr
           where so_mstr.so_domain = global_domain and so_nbr = substring(gref,4,8)
           exclusive-lock /*lb01 2004-09-02 09:21*/
           no-error.
        if available so_mstr then do:
                type    = "SO".

                if can-find(ar_mstr where ar_mstr.ar_domain = global_domain and ar_nbr = ginv)
                or can-find(first ih_hist where ih_hist.ih_domain = global_domain and  ih_inv_nbr = ginv)
                then do:
            put stream rpt
              "发票 " at 1
              ginv at 10
            ...
            ELSE DO:
              ...
              if can-find(ar_mstr where ar_mstr.ar_domain = global_domain and ar_nbr = ginvx)
              or can-find(first ih_hist where ih_hist.ih_domain = global_domain and ih_inv_nbr = ginvx)
              then do:
                put stream rpt
                     "发票 " at 1
                     ginvx      at 10
      ...
      and (not can-find(first sod_det where sod_det.sod_domain = global_domain and sod_nbr = so_nbr and sod_part = xpart)) no-error.
      ...
      for each sod_det exclusive-lock where sod_det.sod_domain = global_domain and sod_nbr = so_nbr:
/*        find first xinvd where xnbr = sod_nbr and xpart = sod_part no-error.  lb02*/
      ...
      if (ABSOLUTE(gtotamt - gmfgtotamt) <= v_drange) or (v_drange = 0) then do:
        /*在容差范围内，调整*/
          ...
          FIND FIRST trl_mstr WHERE trl_mstr.trl_domain = global_domain and  trl_code = so_trl3_cd NO-LOCK NO-ERROR.
...
/*313*/         so_trl3_amt = gtaxamt - gmfgtaxamt.
      ...
      if gpglef_result > 0 then do:
        ...
        find first so_mstr
           where so_mstr.so_domain = global_domain and  so_inv_nbr = ginvx
           and   so_nbr     = gnbr
           and   so_to_inv  = no
           and   so_invoiced = yes
