...
/* Revision: 1.40.1.1    BY: Jean Miller         DATE: 08/22/03  ECO: *P10J*  */
/* Revision: 1.40.1.2    BY: Dorota Hohol        DATE: 10/02/03  ECO: *P0ZL*  */
/* $Revision: 1.40.1.3 $   BY: Russ Witt           DATE: 01/28/04  ECO: *P1CZ*  */
/* ss - 121010.1 by: Steven */ /*add project code ; domain */
...
/******************************************************************************/
/*  Note: A parallel copy of this program is used for kanban transactions.    */
/*        Any changes made here may also have to be applied to kbintr.p.      */
/******************************************************************************/
...
define variable l_proj           like pl_pur_cc   no-undo. /* ss - 121010.1 */ 
{&ICINTR-P-TAG8}
{&ICINTR-P-TAG6}
...
define variable account-length as integer .
/* DISPLAY TITLE */
...
if transtype = "ISS-RV" then do:
   if can-find(first po_mstr where po_domain = global_domain and po_nbr >= "") and
      can-find(first ap_mstr where ap_domain = global_domain and ap_type >= "" and ap_ref >= "")
   then do:
      /* Use PO RTS Issue */
...
/*********added by tfq*****************************/
 ...
 and code_fldname = "account-length" no-lock no-error .
 if not available code_mstr 
 then do:
  MESSAGE "通用代码中没有维护账户长度"
   VIEW-AS ALERT-BOX ERRor BUTTONS OK
   TITLE "错误" .
 undo,leave .
 end.
 else account-length = integer(code_value) .
/****************tfq added end*******************/ 
   do transaction:
   ...
   find pt_mstr using pt_part where pt_domain = global_domain no-lock no-error.
   ...
   and pl_prod_line = pt_prod_line no-lock.
   ...
   repeat on endkey undo mainloop, retry mainloop:
      ...
      for each sr_wkfl no-lock where sr_domain = global_domain 
      and sr_userid = mfguser and sr_lineid = cline:
         i = i + 1.
         if i > 1 then leave.
      end.
      ...
      if i = 1 then do:
         ...
         and sr_userid = mfguser and sr_lineid = cline no-lock.
         site = sr_site.
         location = sr_loc.
         lotserial = sr_lotser.
         lotref = sr_ref.
      ...
      /* tfq added begin in 2005/06/20 ****/
		  find first code_mstr where code_domain = global_domain 
		         and code_fldname = "so_job"
		         and code_value = so_job no-lock no-error.
		  /* ss - 121010.1 -b*/ 
		  /*dr_acct = substring(code_cmmt,1,8).
		  cr_acct = substring(code_cmmt,1,8).
		  dr_sub = substring(dr_acct,account-length + 1 ).
		  cr_sub = substring(cr_acct,account-length + 1 ).
		  dr_acct = substring(dr_acct,1,account-length) .
		  cr_acct = substring(cr_acct,1,account-length) .*/
		  if available code_mstr then do:
		  	IF transtype = "ISS-UNP" THEN DO:     
		  			dr_acct = IF entry(1,code_cmmt,'-') <> "Y" THEN entry(1,code_cmmt,'-') ELSE "".   
		  			dr_sub =  IF entry(2,code_cmmt,'-') <> "Y" THEN entry(2,code_cmmt,'-') ELSE "".       
		  			dr_cc =  IF entry(3,code_cmmt,'-') <> "Y" THEN entry(3,code_cmmt,'-') ELSE "".       
		  			dr_proj =  IF entry(4,code_cmmt,'-') <> "Y" THEN entry(4,code_cmmt,'-') ELSE "".       
		  	END.
		  	ELSE IF transtype = "RCT-UNP" THEN DO:
		  		  cr_acct = IF entry(1,code_cmmt,'-') <> "Y" THEN entry(1,code_cmmt,'-') ELSE "".   
		  			cr_sub =  IF entry(2,code_cmmt,'-') <> "Y" THEN entry(2,code_cmmt,'-') ELSE "".       
		  			cr_cc =  IF entry(3,code_cmmt,'-') <> "Y" THEN entry(3,code_cmmt,'-') ELSE "".       
         ...
         if not available in_mstr then do:
            ...
            and in_part = pt_part and in_site = gl-site
            no-lock: end.
         ...
         i = 0.
         for each sr_wkfl where sr_domain = global_domain and sr_userid = mfguser
         and sr_lineid = cline
         no-lock:
            i = i + 1.
            if i > 1 then do:
         ...
         if multi_entry then do:
            ...
            find first sr_wkfl where sr_domain = global_domain and 
      ...
      if issrct = "RCT" then do:
         ...
         assign
            ...
            dr_proj = ""
            dr_desc = "".
/*****tfq deleted begin from 2005/06/20**************
         ...
         if transtype = "RCT-RS" then do:
            ...
            find first ptp_det where ptp_part = pt_part
...
*****tfq deleted end from 2005/06/20**************/
         find ac_mstr no-lock where ac_domain = global_domain 
...
/*message "333--" cr_sub . */
         ...
         find si_mstr where si_domain = global_domain and si_site = site no-lock.
         {gpglef1.i
            &module = ""IC""
            &entity = si_entity
            &date = eff_date
   ...
   {gprun.i ""icintra.p"" "(shipnbr, ship_date, inv_mov, 0, true)" }
    find last tr_hist no-lock where tr_domain = global_domain no-error.
