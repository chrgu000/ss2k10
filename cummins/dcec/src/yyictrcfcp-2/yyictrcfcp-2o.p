    ...
    for
       /* each tmp_site no-lock, */
        each in_mstr fields(in_domain in_site in_gl_set in_part in_qty_oh in__qadc01 in_qty_nonet in_user1 in_gl_cost_site in_cur_set) USE-INDEX in_site
                      where in_domain = global_domain and in_site >= site and in_site <= site1 and
       ...
       oth = 0.
        ...
        for each tr_hist FIELDS (tr_domain tr_part tr_effdate tr_type tr_qty_loc tr_site tr_ship_type) USE-INDEX tr_eff_trnbr no-lock
