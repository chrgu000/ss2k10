...
FOR EACH pt_mstr NO-LOCK WHERE  /* *SS-20120814.1*   */ pt_mstr.pt_domain = global_domain and pt_part >= pt AND pt_part <= pt1
    AND pt_prod_line >= pl AND pt_prod_line <= pl1 ,
    EACH IN_mstr NO-LOCK WHERE  /* *SS-20120814.1*   */ IN_mstr.in_domain = global_domain and  IN_part = pt_part:
...
FOR EACH xx WHERE xx_qty > 0 :
    ...
    FOR EACH tr_hist NO-LOCK WHERE /* *SS-20120814.1*   */ tr_hist.tr_domain = global_domain and (tr_type = "rct-po" OR tr_type = "rct-wo") 
        AND tr_part = xx_part
        USE-INDEX tr_part_eff BY tr_date DESCENDING :
        IF tr_type = "rct-po" THEN DO :
            FIND pod_det NO-LOCK WHERE  /* *SS-20120814.1*   */ pod_det.pod_domain = global_domain and pod_nbr = tr_nbr AND pod_line = tr_line NO-ERROR .
...
find in_mstr where /* *SS-20120814.1*   */ IN_mstr.in_domain = global_domain and in_site =  "dcec-c"  and in_part = xx_part no-lock no-error.
if avail in_mstr then do:
 ...
 /*judy 05/10/12*/ 
    FIND si_mstr NO-LOCK WHERE /* *SS-20120814.1*   */ si_mstr.si_domain = global_domain and si_site = "dcec-c" NO-ERROR .
    FIND FIRST sct_det NO-LOCK WHERE /* *SS-20120814.1*   */ sct_det.sct_domain = global_domain and sct_part = xx_part AND sct_sim = l_gl_set
