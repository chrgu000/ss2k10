...
define variable sodate AS character .
...
repeat:
    DO TRANSACTION ON ERROR UNDO, LEAVE.
      ...
      UPDATE  src_file xxraw validate(input xxraw > 0 ,"行号小于等于零是不允许的")  with frame a.
       ...
       FOR EACH yyinvpro_det WHERE yyinvpro_domain = global_domain and yyinvpro_edn = TODAY:
           DELETE yyinvpro_det.
       END.
     ...
     repeat:    /*asn input repeat*/
           assign   sonbr = excelsheetmain:cells(i,1):TEXT
                    sodate = excelsheetmain:cells(2,m):TEXT
                    soqty = excelsheetmain:cells(i,m):TEXT

                    m = m + 1 .
            
          IF trim(sonbr) <> "" AND trim(sodate) <> "" AND soqty <> 0 THEN DO:
              create  yyinvpro_det .
                   yyinvpro_domain = global_domain.
                   yyinvpro_part = TRIM(sonbr). 
                   yyinvpro_qty = soqty.
                   yyinvpro_date = DATE(sodate).
                   yyinvpro_edn = TODAY.
              END.           
