...
for each tr_hist no-lock where tr_domain = "dcec" AND tr_type = "iss-wo" and tr_effdate >= date2 and tr_effdate <= date1 and tr_site >= site and tr_site <= site1 
    /*cj*/ AND tr_part >= part AND tr_part <= part1 AND tr_prod_line >= LINE2 AND tr_prod_line <= line1 USE-INDEX tr_eff_trnbr
...
find pt_mstr where pt_part = ws_part no-error.
...
find pt_mstr where pt_part = ws_so no-error.
     if available pt_mstr then do:
