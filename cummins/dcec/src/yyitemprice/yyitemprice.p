...
start_date = date(01,01,1990).
bac_list1 = "".
for each pt_mstr where  /* *SS-20120816.1*   */ pt_mstr.pt_domain = global_domain and (pt_prod_line <= prod_line_to)
                 and (pt_prod_line >= prod_line_from )
                 and (pt_part <= part_to)
                 and (pt_part >= part_from)
   use-index pt_part no-lock :                              
   for each pc_mstr where  /* *SS-20120816.1*   */ pc_mstr.pc_domain = global_domain and pc_part = pt_part
