...
for each tr_hist no-lock where  /* *SS-20120814.1*   */ tr_hist.tr_domain = global_domain and tr_type = "rct-wo"  and tr_effdate >= date and tr_effdate <= date1 and tr_prod_line >= line and tr_prod_line <= line1
...
for each tr_hist no-lock where /* *SS-20120814.1*   */ tr_hist.tr_domain = global_domain and (tr_type = "iss-wo") and tr_effdate >= date and tr_effdate <= date1 and tr_prod_line >= line and tr_prod_line <= line1
    /*cj*/ AND tr_part >= part AND tr_part <= part1 USE-INDEX tr_eff_trnbr :
find first wo_sum where ws_part = tr_part no-error.
if not available wo_sum then do:
create wo_sum.
...
ws_qty_comp3 = ws_qty_comp3 + tr_qty_loc.
    ...
    for each wo_sum 
    , each pt_mstr where /* *SS-20120814.1*   */  pt_mstr.pt_domain = global_domain and pt_part = ws_part
    , each pl_mstr where /* *SS-20120814.1*   */  pl_mstr.pl_domain = global_domain and pl_prod_line = pt_prod_line BREAK by pl_prod_line:
        if pl_prod_line >= line and pl_prod_line <= line1 and ws_part >= part and ws_part <= part1 then do:
        find spt_det where  /* *SS-20120814.1*   */ spt_det.spt_domain = global_domain and spt_site = site and spt_sim = cost_set and spt_element = "直接人工" and spt_part = ws_part no-lock no-error.
        ...
        find spt_det where /* *SS-20120814.1*   */ spt_det.spt_domain = global_domain and spt_site = site and spt_sim = cost_set and spt_element = "费用" and spt_part = ws_part no-lock no-error.
        if available spt_det then do:
