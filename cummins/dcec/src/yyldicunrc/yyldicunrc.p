...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brList wWin _FREEFORM
  QUERY brList DISPLAY
      xim_part    COLUMN-LABEL '料号'
        ...
        xim_effdate COLUMN-LABEL '生效日'
        xim_lot     COLUMN-LABEL '批序号'
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "DCEC-计划外入库批量导入(yyldicunrc.p)"
         HEIGHT             = 33.68
         WIDTH              = 117.3
         MAX-HEIGHT         = 39.95
         MAX-WIDTH          = 146.2
...
ON END-ERROR OF wWin /* DCEC-计划外入库批量导入(yyldicunrc.p) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* DCEC-计划外入库批量导入(yyldicunrc.p) */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-RESIZED OF wWin /* DCEC-计划外入库批量导入(yyldicunrc.p) */
DO:
...
DO:
    ...
    FOR EACH xim EXCLUSIVE-LOCK WHERE xim_chk = "":
        ...
        FIND FIRST tr_hist NO-LOCK WHERE tr_domain = GLOBAL_domain AND tr_trnbr >= integer(trrecid)
               ...
               AND tr_loc = xim_loc AND tr_serial = xim_lot AND tr_type = "RCT-UNP"
               AND tr_qty_loc = xim_qty NO-ERROR.
        IF AVAILABLE tr_hist THEN DO:
            ASSIGN xim_chk = "RCT-UNP:[" + trim(string(tr_trnbr,">>>>>>>>>>>>>9")) + "]".
             os-delete value(vfile + ".bpi") no-error.
             os-delete value(vfile + ".bpo") no-error.
        END.
        ELSE DO:
             ASSIGN xim_chk = "cim_load失败 请查看log " + vfile.
        END.
    END.
    SESSION:SET-WAIT-STATE ("").
