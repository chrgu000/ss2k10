...
FORM /*GUI*/ 
 ...
 SKIP(.1)  /*GUI*/
   part           colon 15
 /* *SS-20120917.1*    /*part1          label {t001.i} colon 49*/ skip */ 
    /* *SS-20120917.1*   */ part1 label {t001.i} colon 49 skip
   line           colon 15
...
do:
   ...
   for each pc_mstr
      fields (pc_amt   pc_amt_type pc_curr pc_expire
              ...
              pc_start pc_um /* *SS-20120917.1* */ pc_domain)
/* *SS-20120917.1*  -b */
/*
 * /*judy*/      
 ...
 * /*judy*/
 */
        where pc_mstr.pc_domain = global_domain and 
	      (pc_part >= part and pc_part <= part1) 
/* *SS-20120917.1*  -e */
        and (pc_amt_type >= type and pc_amt_type <= type1)      
        and  (pc_list  >=  pclist1 and pc_list <=   pclist2 and NOT (pc_list begins "ETS"))
...
/********************tfq************************
         ...
         then do:
            ...
            find pl_mstr
               where  /* *SS-20120917.1*   */ pl_mstr.pl_domain = global_domain and pl_prod_line = pt_prod_line
               no-lock no-error.
...
***************tfq**************************/
         ...
         find last pcmstr
            where  /* *SS-20120917.1*   */ pcmstr.pc_domain = global_domain  
	      and pcmstr.pc_list    =  pc_mstr.pc_list
...
********************tfq*******************************/
            find first ad_mstr where  /* *SS-20120917.1*   */ ad_mstr.ad_domain = global_domain and  (ad_addr = substring(pcmstr.pc_list,1,LENGTH(pcmstr.pc_list) - 1) OR ad_addr =pcmstr.pc_list) no-lock no-error .
            ...
            do i = 1 to 15:
               ...
               then do:
                  ...
                  find in_mstr
                     where  /* *SS-20120917.1*   */ in_mstr.in_domain = global_domain
		       and  in_part = pt_part
                       and in_site = pt_site
                     no-lock no-error.
