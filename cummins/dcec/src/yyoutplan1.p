...
repeat:
  ...
  FOR each tr_hist NO-LOCK WHERE tr_domain = "DCEC" AND tr_type = "ISS-UNP" and tr_effdate >= effdate and tr_effdate <= effdate1,
    each trgl_det NO-LOCK where  trgl_domain = "DCEC" AND trgl_trnbr =tr_trnbr break by tr_trnbr:
    find first pt_mstr where tr_domain = "DCEC" AND pt_part = tr_part no-lock no-error.
    ...
    disp tr_trnbr label "事务号" tr_nbr label "订单" tr_part label "零件号"
      ...
      tr_type tr_so_job label "销售/定制品" tr_qty_loc label "数量" 
      trgl_cr_acct trgl_dr_acct trgl_gl_amt label "价格"  tr_rmks  LABEL "备注"
      tr_effdate  with width 200 STREAM-IO.
 end.            
    {mfreset.i}
