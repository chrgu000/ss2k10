     ...
     if src_file = "" then src_file = "D:\N-DFL入库计划-101008.XLS".
      ...
      UPDATE site_file src_file xxraw validate(input xxraw > 0 ,"行号小于等于零是不允许的")  with frame a.
       ...
       IF TRIM(site_file) = "dcec-b" THEN do:
        ...
        FOR EACH yyusrw_wkfl WHERE yyusrw_domain = global_domain and yyusrw_key1 = "POB" :
               DELETE yyusrw_wkfl.
           END.
       ...
       IF TRIM(site_file) = "dcec-c" THEN DO:
           SITE = "POC".
          FOR EACH yyusrw_wkfl WHERE yyusrw_domain = global_domain and yyusrw_key1 = "POC" :
               DELETE yyusrw_wkfl.
           END.
            create  yyusrw_wkfl.
                    yyusrw_domain = global_domain.
...
/*--------------------------------------比较---------------------------------*/
      ...
      create "Excel.Application" excelapp.
       ...
       repeat:    /*asn input repeat*/
         ...
         IF TRIM(SONBR) <> "" THEN DO:
             FIND FIRST pt_mstr WHERE pt_domain = global_domain and pt_part = trim(sonbr) NO-LOCK NO-ERROR .
     ...
     FOR EACH zzwk NO-LOCK where zzwk_domain = global_domain and
            ...
            if (not last-of(zzwkduedate)) then next.

            FIND pt_mstr NO-LOCK WHERE pt_domain = global_domain and pt_part = zzwknbr.
             IF AVAIL pt_mstr THEN sodesc = pt_desc2.
                ELSE sodesc = "".

           IF n <> 0 THEN DO:
               create yyusrw_wkfl.
                    yyusrw_domain = global_domain.
                    yyusrw_key1 = TRIM(zzwksite).
                    yyusrw_key2 = trim(zzwknbr).
                    yyusrw_key3  = "ORDER-TEST-DET".
                    yyusrw_datefld[1] = zzwkduedate.
                    yyusrw_decfld[1] =  n.
                    yyusrw_key4 = sodesc.
               END.
        END.
