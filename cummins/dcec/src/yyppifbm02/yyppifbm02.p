/** yyppifbm01.p 13.1.2 bmpsmt.p BOM代码生成 CIM_LOAD out of ERP  **/
...
define variable item_pm_code as char.
define variable cummins_par  as char.
define variable cummins_child as char.
define variable local_par  as char.
define variable local_child as char.
define variable exec_par as char.
define variable exec_child as char.
define variable date_str as char format "x(8)".
define variable recno_ps_mstr as recid.
define variable bom-b as log.
define variable bom-c as log.
...
/*J007*     end.  */
      ...
      find pt_mstr where pt_domain = global_domain and
           pt_part = exec_child no-lock no-error.
...
**J003*/
    ...
    if lookup(xxppif_tr_code,"SOSA,SOSC") > 0 then do:
      find code_mstr where code_domain = global_domain and
           code_fldname = "SO SITE" and code_value = substr(exec_par,1,3) no-lock no-error.
      ...
      find ptp_det where ptp_domain = global_domain and
           ptp_site = code_cmmt and ptp_part = exec_par no-lock no-error.
      ...
      find ptp_det where ptp_domain = global_domain and
           ptp_site = SITE-C and ptp_part = exec_par no-lock no-error.
      if not avail ptp_det then do:
        xxppif_err = 2.
...
/*J005*/  item_type = 0.
/*J005*/  find pt_mstr where pt_domain = global_domain and
               pt_part = exec_par no-lock no-error.
...
/*J005*/  if (lookup(pt_group,"PH") > 0 and item_pm_code = "M" and item_phantom = yes) then item_type = 6.   /*6.虚件/随机件*/
    ...
    find ptp_det where ptp_domain = global_domain and
         ptp_site = SITE-C and ptp_part = exec_child no-lock no-error.
    if not avail ptp_det then do:
...
/*J005*/      end.
          ...
          if not avail ps_mstr then do:
                    ...
                    if can-find(isd_det where isd_domain = global_domain and
                                isd_status = ptstatus1 and
                                isd_tr_type = "ADD-PS") then do:
                        xxppif_err = 2.
                        xxppif_msg = "2023-" + exec_par.
                        next.
                    end.
                    find first pt_mstr no-lock where pt_domain = global_domain
                           and pt_part = exec_child no-error.
                    if available pt_mstr then
                       ptstatus1 = pt_status.
                    else ptstatus1 = "".
                    substring(ptstatus1,9,1) = "#".
                    if can-find(isd_det where isd_domain = global_domain
                            and isd_status = ptstatus1
...
/*J010*/  predef_site = "".
    ...
    if lookup(xxppif_tr_code,"SOSA,SOSC") > 0 then do:
      find code_mstr where code_domain = global_domain and
           ...
           code_value = substr(exec_par,1,3) no-lock no-error.
      if not avail code_mstr then do:
            xxppif_err = 2.
            xxppif_msg = "2010-" + exec_par.
            next.
...
/*J004*/     new_bom = yes.
/*J001*/    find last ps_mstr where ps_domain = global_domain and
                      ps_par = exec_par and ps_comp = exec_child and
...
/*J008*/        find last ps_mstr where ps_domain = global_domain and
                          ps_par = exec_par and ps_comp = exec_child and
                         (ps_end = ? or ps_end >= xxppif__dte01) no-lock no-error.
...
/*W001**                          find last ps_mstr where ps_domain = global_domain and ps_par = cummins_par and ps_comp = cummins_child and (ps_end = ? or ps_end >= xxppif__dte01) no-lock no-error.*/

/*W001*//*W011           find last ps_mstr where ps_domain = global_domain and ps_par = cummins_par
                  and ps_comp = cummins_child
                  and (ps_end = ? or ps_end >= xxppif__dte01)  no-error. */
/*W001*//*W011 */          find last ps_mstr where ps_domain = global_domain
                  and ps_par = cummins_par
                  and ps_comp = cummins_child
...
/* y0220 */                  put stream batchdata unformatted skip.
 ...
 /*W011            find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01)  no-error. */
 /*W011*/       find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01 OR trim(substr(xxppif_content,4,2)) = "CO" ) NO-ERROR .
...
/*W001*/    find code_mstr where code_domain = global_domain
             and code_fldname = "SO SITE"
             and code_value = substr(exec_par,1,3) no-lock no-error.
...
/*W001*/ /*W002 move down           find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01)  no-error.
...
/*W001*/                          if avail ps_mstr then do: /*判断结构是否存在,若存在,结束日期=数据中生效日期(xxppif__dte01) - 1 **/
          W002*/
/*W007*/ /* find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01) AND (xxppif__dte01 > ps_start OR ps_start = ?) no-error.
*/
 /*W010*/  find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01) AND (xxppif__dte01 < ps_start OR ps_start = ?) AND ps_start < TODAY NO-ERROR .
...
/*W005 begin*//*add the logic of PDSCI and PDSCO*/
        if lookup(xxppif_tr_code,"PDSC") > 0 and trim(substr(xxppif_content,5,1)) = "O" then do:
          find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01)  no-error.
...
/*W002*/            find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01)  no-error.
                          ...
                          IF AVAIL ptp_det THEN DO:
                              IF  ptp_phantom THEN DO:
                                  FIND CODE_mstr WHERE code_domain = global_domain
                                   and code_fldname = "PHANTOM OP"
                                   AND code_value = "DCEC C" NO-LOCK NO-ERROR .
               ...
               or (item_type = 1 AND AVAIL code_mstr and code_cmmt = SITE-B) then do:  /* site-b总成*/
                     exec_par = exec_par + "ZZ".
                       ...
                       /**if item_type = 1 then do:**/  /*只要子件是虚件的 都用BOM CODE Item + "ZZ"*/
                           find ptp_det where ptp_domain = global_domain and
                                ptp_site = SITE-C and ptp_part = exec_child no-lock no-error.
/*J005*/                   if not avail ptp_det then for first ptp_det where ptp_domain = global_domain and ptp_site = SITE-B and ptp_part = exec_child no-lock: end.
...
/*J009*/        find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01) no-lock no-error.
...
/*J004*/    if new_bom = yes then do:   /* For SITE-B */
                  /* CHECK FOR CYCLIC PRODUCT STRUCTURES */
                     find first ps_mstr no-lock where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child
                            and ps_ref = "" and ps_start = xxppif__dte01 no-error.
                     newBom = no.
                     if not avail ps_mstr then do:
                            find first pt_mstr no-lock where pt_domain = global_domain and pt_part = exec_par no-error.
                            if available pt_mstr then
                                ptstatus1 = pt_status.
                            else ptstatus1 = "".
                            substring(ptstatus1,9,1) = "#".
                            if can-find(isd_det where isd_domain = global_domain and isd_status = ptstatus1
                            and isd_tr_type = "ADD-PS") then do:
                                xxppif_err = 2.
                                xxppif_msg = "2023-" + exec_par.
                                next.
                            end.
                            find first pt_mstr no-lock where pt_domain = global_domain and pt_part = exec_child no-error.
                            if available pt_mstr then
                               ptstatus1 = pt_status.
                            else ptstatus1 = "".
                            substring(ptstatus1,9,1) = "#".
...
/*J005*/          for first ptp_det where ptp_domain = global_domain and ptp_site = SITE-B and ptp_part = exec_child no-lock: end.
/*J005*/            if not avail ptp_det then do:
                        xxppif_err = 2.
...
**********/
 /*W011                    find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01)  no-error.
*/
 /*W011*/            find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01
...
/*W001*/         find code_mstr where code_domain = global_domain and code_fldname = "SO SITE" and code_value = substr(exec_par,1,3) no-lock no-error.
...
/*W001*//*W002            find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01)  no-error.
/*W001*/            if avail ps_mstr then last_bom_qty = ps_qty_per.
/*W001*/            if avail ps_mstr then do: /*判断结构是否存在,若存在,结束日期=数据中生效日期(xxppif__dte01) - 1 **/
        W002*/
/*W007*//*W010  find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01) AND (xxppif__dte01 > ps_start OR ps_start = ?) no-error.
*/
/*W010*/find last ps_mstr where ps_domain = global_domain and ps_par = exec_par and ps_comp = exec_child and (ps_end = ? or ps_end >= xxppif__dte01) AND (xxppif__dte01 < ps_start OR ps_start = ?) AND ps_start < TODAY no-error.
...
quit.
 ...
 procedure fix_date_format:
    ...
    date_format = "DDMMYY".
    new_date = date-io.
    find code_mstr where code_domain = global_domain and code_fldname = "DATE FORMAT" AND code_value = "6" no-lock no-error.
    if avail code_mstr then do:
