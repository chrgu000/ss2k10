...
/* $Revision: se2011 $   BY: Jordan Lin        DATE: 10/31/12  ECO: *SS-20121031.1*  */
...
in_abc             colon 20
   ...
   in_rctpo_active    colon 37   in_user1        colon 58 
   in_rctwo_status    colon 20
   in_rctwo_active    colon 37
  /* *SS-20121031.1* */ in__qadc01  colon 58 label "保管员"
 SKIP(.4)  /*GUI*/
            ...
            if recno <> ? then do:
               ...
               if available pt_mstr then do:
                  ...
                  display
                     ...
                     in_rctwo_status
                     in_rctwo_active
  /* *SS-20121031.1* */ in__qadc01   in_user1
                  with frame c.
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         if c-application-mode <> "API" then do:
            set
               ...
               in_cyc_int
               in_loc
               in_rctpo_status in_rctpo_active in_user1 in__qadc01 
	    ...
	    /* *SS-20121031.1* -b */ 
            ...
            /*added by tfq, 2005/06/23 to add a field to record the warehouse clerk*/
                  find loc_mstr where loc_domain = global_domain and loc_site = in_site and loc_loc = input in_loc no-lock no-error.
               ...
               &ERRORLEVEL=3
                       ...
                       next-prompt in_loc with frame c.
                       undo setc, retry setc.
                  end.
                  if pt_site = in_site and pt_loc <> input in_loc then do:
