...
*   */
     ...
     assign site = pt_site.
/*316*/ find first code_mstr no-lock where code_domain = "dcec" and
/*316*/            code_fldname = "so site" and part begins code_value no-error.
/*316*/ if available code_mstr then do:
/*316*/    assign site = code_cmmt.
/*316*/ end.
 ...
 /*tfq added end for bom validation****/
     ...
     if not available xxptmp_mstr then do:
        ...
        assign xxptmp_site = site
               xxptmp_par = part
               xxptmp_comp = input xxptmp_comp
               xxptmp_qty_per = pkqty when available pkdet.
