...
for first absc_det
   fields(absc_domain absc_abs_id absc_carrier absc_seq)
   where absc_domain = global_domain and absc_abs_id = abs_id
   no-lock:
end. /* FOR FIRST absc_det */

for first df_mstr
   fields(df_domain df_format df_form_code df_inv df_type)
   where df_domain = global_domain and df_type   = "1"
...
assign
   ...
   v_multi       = available absc_det
                   and (not can-find (absc_det where absc_domain = global_domain and absc_abs_id = abs_id))
   v_format      = abs_format
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   set
      ...
      v_carrier when (can-find (absc_det where absc_domain = global_domain and absc_abs_id = abs_id) or
                      not can-find (first absc_det where absc_domain = global_domain and absc_abs_id = abs_id))
      v_multi
      abs_shipvia
      v_fob
      v_trans_mode  /*james*/ VALIDATE(CAN-FIND(FIRST CODE_mstr WHERE code_domain = glbal_domain and CODE_fldname = "xx-mot" AND CODE_value = v_trans_mode), "请输入有效的运输方式")
      v_carr_ref
      v_veh_ref
      v_format
      v_cons_ship
   ...
   editing:
      ...
      then do:
         ...
         {mfnp05.i
            ad_mstr
            ad_addr
            "(ad_domain = global_domain and ad_type = 'carrier' or
              can-find (ls_mstr where ls_domain = global_domain and ls_type = 'carrier'
                                and   ls_addr = ad_addr))"
            ad_addr
            "input v_carrier"}
      ...
      /* DOCUMENT FORMAT FIELD */
         ...
         then do:
            ...
            {mfnp05.i
               df_mstr
               df_format
               "(df_domain = global_domain and df_type = '1')"
               df_format
               "input v_format" }
         ...
         /* CONSOLIDATION FLAG FIELD */
            ...
            then do:
               ...
               {mfnp05.i
                  lngd_det
                  lngd_trans
                  "(lngd_domain = global_domain and lngd_dataset = 'sgad_det' and
   ...
   then do:
      ...
      for first b_abs_mstr
         fields (b_abs_mstr.abs_domain b_abs_mstr.abs_shipfrom b_abs_mstr.abs_par_id)
...
/*GUI*/ if global-beam-me-up then undo, leave.
      ...
      /* DELETE COMMENTS */
      for each cmt_det
         where cmt_domain = global_domain and cmt_indx = abs_cmtindx
         exclusive-lock:
         delete cmt_det.
      ...
      for each cmt_det
         where cmt_domain = global_domain and cmt_indx = abs_trl_cmtindx
         exclusive-lock:
         delete cmt_det.
      end. /* FOR EACH cmt_det */
...
/*GUI*/ if global-beam-me-up then undo, leave.
  /* IF LASTKEY */
   ...
   /* VALIDATE CARRIER */
   if (can-find (absc_det where absc_domain = global_domain and absc_abs_id = abs_id) or
       not can-find (first absc_det where absc_domain = global_domain and absc_abs_id = abs_id))
   and v_carrier <> ""
   then do:

      if not can-find (ad_mstr where ad_domain = global_domain and ad_addr = v_carrier)
      then do:
      ...
      if not can-find(ls_mstr where ls_domain = global_domain and ls_addr = v_carrier
                              and   ls_type = "carrier" )
      then do:
   ...
   then do:
      ...
      {&ICSHHDR-P-TAG1}
         ...
         and (can-find (im_mstr where im_domain = global_domain and im_inv_mov  = abs_inv_mov
   ...
   then
      for first lngd_det
         fields (lgnd_domain lngd_dataset lngd_field lngd_key1 lngd_key2 lngd_key3
                 lngd_key4 lngd_lang lngd_translation)
         where lgnd_domain = global_domain and lngd_dataset = "sgad_det"
   ...
   for first ad_mstr
      fields(ad_domain ad_addr ad_lang ad_type)
      where ad_domain = global_domain and ad_addr = abs_shipto
      no-lock:
   ...
   then do:
      ...
      if not can-find (lng_mstr where lng_domain = global_domain and lng_lang = abs_lang)
      then do:
         /* LANGUAGE MUST EXIST */
