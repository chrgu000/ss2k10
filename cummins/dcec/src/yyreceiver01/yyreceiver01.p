...
{mfdtitle.i "121002.1"}
define variable nbr        as char.
define variable receiver   as char.
define variable pageno     as integer. /***页号***/.
define variable duplicate  as char.    /***副本***/.
define variable vendor     as char extent 6.
define variable pdate      as date initial today.    /***打印日期***/.
define variable revision   as char.    /***采购单版本***/.
define variable vend_phone as char format "x(20)". /***供应商电话***/
define variable rmks       as char format "x(80)". /***备注***/
define variable i          as integer.
define variable j          as integer.
define variable k          as integer.
define variable site1      like ptp_site label "地点" initial "DCEC-B".
define variable site2      like ptp_site label "至" initial "DCEC-B".
define variable userid1    like tr_userid LABEL "用户".
define variable userid2    like tr_userid LABEL "至".
define variable psnbr      like prh_ps_nbr.
define variable psnbr1     like prh_ps_nbr.
define variable nbr_from   as char label "采购单".
define variable nbr_to     as char label "至".
define variable rcv_from   as char label "收货单".
define variable rcv_to     as char label "至".
define variable sup_from   as char label "供应商".
define variable sup_to     as char label "至".
define variable date_from  as date label "收货日期".
define variable date_to    as date label "至".
define variable flag1      as logical label "只打印未打印过的收货单".
define variable prhqty like prh_rcvd.
...
FORM
   ...
   SKIP(.1)
   nbr_from  colon 20
   nbr_to    colon 46 skip
   rcv_from  colon 20
   rcv_to    colon 46 skip
   sup_from  colon 20
   sup_to    colon 46 skip
   site1     colon 20
   site2     colon 46 skip
   psnbr     colon 20
   psnbr1    colon 46 skip
   date_from colon 20
   date_to colon 46 skip
/*judy 05/07/28*/    keeper COLON 20
/*judy 05/07/28*/   keeper1 COLON 46 LABEL {t001.i} SKIP
    /*judy 05/07/28*/    userid1 COLON 20
/*judy 05/07/28*/   userid2 COLON 46 LABEL {t001.i} SKIP
...
form "DCP-03-17-00-02 F002" skip(1)
   ...
   for each prh_hist no-lock  where prh_domain = global_domain
                                        ...
                                        and (prh_site >= site1)
                                        and (prh_site <= site2)
                                        and prh_ps_nbr >= psnbr
                                        and prh_ps_nbr <= psnbr1
...
IF NOT (keeper =  " " AND  keeper1 =  hi_char) THEN DO:
  ...
  END.
     ...
     find first tr_hist where tr_domain = global_domain and tr_part=prh_part
          and tr_nbr=prh_nbr and tr_line=prh_line and tr_type="rct-po" and tr_lot=prh_receiver AND tr_userid>= userid1 AND tr_userid<=userid2 no-lock no-error.
     if available tr_hist then do:
        ...
        find first ptp_det where ptp_domain  = global_domain and ptp_part = prh_part /* and ptp_ins_rqd = no*/ and ptp_site >=site1 and ptp_site <= site2 no-lock no-error.
...
/*judy 05/07/28*/ /*  in__qadc01 @ pt_article*/
             ...
             if available po_mstr and po_cmtindx <> 0 then DO:
                   find first cmt_det where cmt_domain = global_domain and (cmt_indx = po_cmtindx ) no-lock no-error.
                   repeat:
                    ...
                    if available cmt_det and lookup("rc",cmt_print) > 0 then
                     ...
                     find next cmt_det use-index cmt_ref where cmt_domain = global_domain no-lock no-error.
                    end.
           ...
           i = i + 1.
          FIND FIRST  in_mstr where in_domain = global_domain and in_site = prh_site and in_part = prh_part NO-LOCK NO-ERROR.
...
/*judy 05/07/28*/    AND  in__qadc01 >= keeper AND in__qadc01 <= keeper1 NO-LOCK  */
      ...
      IF NOT (keeper =  " " AND  keeper1 =  hi_char) THEN DO:
               /*   MESSAGE "AA" xxkeeper xxkeeper1.
                  PAUSE.*/
                   FIND FIRST  in_mstr where in_domain = global_domain and in_site = prh_site and in_part = prh_part
                    and in__qadc01 >= keeper and in__qadc01 <= keeper1 NO-LOCK NO-ERROR.
                   IF NOT AVAIL IN_mstr  THEN NEXT.
        END.

     find first tr_hist where tr_domain = global_domain and tr_part=prh_part and tr_nbr=prh_nbr and tr_line=prh_line and tr_type="rct-po" and tr_lot=prh_receiver AND tr_userid>= userid1 AND tr_userid<=userid2 no-lock no-error.
     if available tr_hist then do:
           find first po_mstr where po_domain  = global_domain and po_nbr=prh_nbr no-lock no-error.
           find first ad_mstr where ad_domain  = global_domain and ad_addr = prh_vend no-lock no-error.
           find first pod_det where pod_domain = global_domain and pod_nbr = prh_nbr and pod_line = prh_line and pod_type="S" no-lock no-error.
           find first pt_mstr where pt_domain  = global_domain and pt_part = prh_part no-lock no-error.
...
IF NOT (keeper =  " " AND  keeper1 =  hi_char) THEN DO:
  ...
  END.
     ...
     if available tr_hist then do:
           find first po_mstr where po_domain  = global_domain and po_nbr=prh_nbr no-lock no-error.
           find first ad_mstr where ad_domain  = global_domain and ad_addr = prh_vend no-lock no-error.
           find first pod_det where pod_domain = global_domain and pod_nbr = prh_nbr and pod_line = prh_line and (pod_type="M" or pod_type="m") no-lock no-error.
           find first pt_mstr where pt_domain  = global_domain and pt_part = prh_part no-lock no-error.
           if (available pt_mstr)  and (available pod_det) then do:
              if  j = 1  then do:
                  if prh__log01 = yes then
...
IF NOT (keeper =  " " AND  keeper1 =  hi_char) THEN DO:
         /*   MESSAGE "AA" xxkeeper xxkeeper1.
            ...
            and in__qadc01 >= keeper and in__qadc01 <= keeper1 NO-LOCK NO-ERROR.
             IF NOT AVAIL IN_mstr  THEN NEXT.
  END.
     find first tr_hist where tr_domain = global_domain and tr_part = prh_part and tr_nbr = prh_nbr
          and tr_line = prh_line and tr_type = "rct-po" and tr_lot = prh_receiver AND tr_userid >= userid1 AND tr_userid <= userid2 no-lock no-error.
     if available tr_hist then do:
           find first po_mstr where po_domain  = global_domain and po_nbr=prh_nbr no-lock no-error.
           find first ad_mstr where ad_domain  = global_domain and ad_addr = prh_vend no-lock no-error.
           find first pod_det where pod_domain = global_domain and pod_nbr = prh_nbr and pod_line = prh_line no-lock no-error.
  ...
  /*judy 05/07/28*/ /*  find first in_mstr where in_part = prh_part no-lock no-error.*/
           ...
           find first ptp_det where ptp_domain = global_domain and ptp_part = prh_part and Ptp_site >= site1 and ptp_site <= site2 no-lock no-error.
...
/*judy 05/07/28*/  {mfguirpb.i &flds="nbr_from nbr_to rcv_from rcv_to sup_from sup_to site1 site2 psnbr psnbr1 date_from date_to keeper keeper1 userid1 userid2 flag1 "}
