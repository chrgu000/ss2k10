...
REPEAT:
 ...
 FOR EACH tr_hist WHERE tr_domain = global_domain and tr_type = "RCT-PO" AND tr_effdate >= v_date1 AND tr_effdate <= v_date2 BREAK BY tr_part BY tr_nbr.
    IF FIRST-OF(tr_part) OR FIRST-OF(tr_nbr) THEN sume = 0.
    sume = sume + tr_qty_loc.
    IF LAST-OF(tr_part) OR LAST-OF(tr_nbr) THEN DO:
       FIND LAST pod_det WHERE pod_domain = global_domain and pod_part = tr_part and pod_site = tr_site and pod_nbr = tr_nbr no-lock no-error.
       IF AVAIL pod_det THEN minpack = pod_ord_mult.
       else minpack = 0.
       
       FIND LAST qad_wkfl WHERE qad_domain = global_domain and qad_key1 = "poa_det" AND qad_datefld[1]<= v_date2 AND qad_charfld[1] = tr_nbr and qad_charfld[2] = tr_part NO-ERROR.
