...
/* $Revision:eb21sp12  $ BY: Jordan Lin            DATE: 08/29/12  ECO: *SS-20120830.1*   */
...
define            variable opmcmmts    like mfc_logical
 ...
 /* *SS-20120830.1*   */ def var msg-nbr as inte.                                 /*kevin*/
...
FORM /*GUI*/ 
 ...
 SKIP(.1)  /*GUI*/
opm_std_op     colon 30
 /* *SS-20120830.1*   */   opm__chr01     colon 30 label "µØµã"                    /*kevin*/
   opm_desc       colon 30
   opm_wkctr      colon 30
   opm_mch        colon 30
   skip(1)
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   editing:
      ...
      if recno <> ? then do:
         display
            opm_std_op
 /* *SS-20120830.1*   */            opm__chr01                               /*kevin*/
            opm_desc
            opm_wkctr
            opm_mch
            opm_setup
...
/*GB32*/       opm_inv_val
	       opm_sub_cost opm_sub_lead opm_tran_qty opmcmmts.
             ...
             find si_mstr no-lock where si_mstr.si_domain = global_domain and  si_site = opm__chr01 no-error.
...
/**************added by kevin, 10/16/2003 for site control*/
                 find si_mstr no-lock where  si_mstr.si_domain = global_domain and si_site = input opm__chr01 no-error.
                 if not available si_mstr or (si_db <> global_db) then do:
                     if not available si_mstr then msg-nbr = 708.
                     else msg-nbr = 5421.
                     /*TFQ{mfmsg.i msg-nbr 3} */
