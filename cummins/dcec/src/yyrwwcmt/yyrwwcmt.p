...
wc_wkctr       colon 30 wc_mch
  /* *SS-20120830.1*   */  wc__chr01 colon 30 label "µØµã"                     /*kevin*/
   wc_desc        colon 30
   skip(1)
...
/*GUI*/ RECT-FRAME-LABEL:SCREEN-VALUE in frame a = F-a-title.
      ...
      /*added by kevin, 12/08/2003 for site control*/
                 find si_mstr no-lock where si_mstr.si_domain = global_domain and  si_site = input wc__chr01 no-error.
                 if not available si_mstr or (si_db <> global_db) then do:
                     if not available si_mstr then msg-nbr = 708.
                     else msg-nbr = 5421.
        /*tfq */          {pxmsg.i &MSGNUM=msg-nbr &ERRORLEVEL=3}
