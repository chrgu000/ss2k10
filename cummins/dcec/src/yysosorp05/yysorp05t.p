...
{mfdeclre.i}
	...
	where code_domain = global_domain and 
			  code_fldname = "zz-soprn-trail" break by code_value:
	...
	find first ad_mstr where ad_domain = global_domain and ad_addr = comp_addr no-lock no-error.
	find first bk_mstr where bk_domain = global_domain and bk_code = ad_user1 no-lock no-error.
	bkline = 1.
	...
	find first bk_mstr where bk_domain = global_domain and bk_code = ad_user2 no-lock no-error.
	bkline = 2.
	if available ad_mstr and available bk_mstr then do:
		display
			bkline
