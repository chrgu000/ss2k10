...
/*J034*/          end.
     ...
     for each tr where tr.qty_pick <> 0 break by tr.site by tr.nbr by tr.part by tr.loc_fr:
            find pt_mstr where pt_domain = global_domain and pt_part = tr.part no-lock no-error.
...
/*F089*/   end.
   ...
   find loc_mstr where loc_domain = global_domain and
        loc_site = tr.site and loc_loc = tr.loc_fr no-lock no-error.
   ...
   find isd_det no-lock
        where isd_domain = global_domain and isd_tr_type = "iss-tr"
          and isd_status =
   ...
   if available isd_det then do:
       assign tr.error = "移出库位 " + tr.site + "," + tr.loc_fr + " 的状态为受限制状态".
         ...
         next.
            ...
            if not can-find(whl_mstr where whl_domain = global_domain
                        and whl_site = tr.site
   ...
   find loc_mstr where loc_domain = global_domain and
        loc_site = tr.site and loc_loc = tr.loc_to no-lock no-error.
   if not available loc_mstr then do:
   ...
   /*verify the status of loc_from*/
   find isd_det no-lock
        where isd_domain = global_domain and isd_tr_type = "rct-tr"
          and isd_status =
          (if available loc_mstr and loc_status <> "" then loc_status
          else si_status) no-error.
   if available isd_det then do:
         ...
         next.
            ...
            if not can-find(whl_mstr where whl_domain = global_domain and
                            whl_site = tr.site
   ...
   find ld_det where ld_domain = global_domain and
        ld_site = tr.site and ld_part = tr.part
                    ...
                    if last-of(tr.loc_fr) then do:
                        if qty_tot <> 0 then do:
                              find ld_det where ld_domain = global_domain
                               and ld_site = tr.site and ld_loc = tr.loc_fr
                               and ld_part = tr.part and ld_lot = ""
                               and ld_ref = ""
                              no-lock no-error.
...
/*F701*/    find pt_mstr where pt_domain = global_domain and
                 pt_part = global_part no-lock no-error.
...
/*F701*/    display pt_part pt_desc1 pt_um lotserial_qty.
        ...
        prompt-for pt_part with frame a editing:
           ...
           {mfnp.i pt_mstr pt_part " pt_domain = global_domain and pt_part " pt_part pt_part pt_part}
...
/*added by kevin*/
      find pt_mstr where pt_domain = global_domain and
           pt_part = tr.part no-lock no-error.
/*end added by kevin*/
...
/*J034*/          find si_mstr where si_domain = global_domain and
 ...
 * /*F190*/       find ld_det where ld_domain = global_domain and
                       ld_part = pt_part and ld_site = site_from
 * /*F190*/       and ld_loc = loc_from and ld_lot = lotser_from
 * /*F190*/       and ld_ref = lotref_from no-lock no-error.
 *G1D2* * * END COMMENT OUT */

/*G1D2*/         find ld_det where ld_det.ld_domain = global_domain
...
/*F0D2*/          find si_mstr where si_domain = global_domain and
                       si_site = site_from no-lock no-error.
/*F0D2*/          find loc_mstr where loc_domain = global_domain and
                       loc_site = site_from and
/*F0D2*/               loc_loc = loc_from no-lock no-error.
...
/*F0D2*/             end.
/*F0D2*/             else do:
/*F0D2*/                find is_mstr where is_domain = global_domain and
                             is_status = si_status
/*F0D2*/                no-lock no-error.
/*F0D2*/                if available is_mstr and is_overissue then do:
/*F0D2*/                   create loc_mstr. loc_domain = global_domain.
/*F0D2*/                   assign
...
/*F0D2*/          end.
/*F0D2*/
/*G0SQ /*F0D2*/   find is_mstr where is_domain = global_domain and is_status = si_status      */
/*G0SQ*/          find is_mstr where is_domain = global_domain and is_status = loc_status
/*F0D2*/          no-lock no-error.
...
/*F0D2*/          then do:
/*F0D2*/             create ld_det. ld_domain = global_domain.
...
/*J034*/          find si_mstr where si_domain = global_domain
                   and si_site = site_to no-lock no-error.
/*J034*/          if not available si_mstr then do:
/*J034*/             {mfmsg.i 708 3}  /* SITE DOES NOT EXIST */
...
/*F190*/          if not can-find(first loc_mstr where loc_domain = global_domain
...
/*F190*/          find lddet where lddet.ld_domain = global_domain
...
/*F190*/                create lddet. lddet.ld_domain = global_domain.
/*F190*/                assign
...
/*F190*/             find loc_mstr where loc_domain = global_domain
                      and loc_site = site_to
/*F190*/              and loc_loc = loc_to no-error.
...
/*GH52/*F701*/             find loc_mstr where loc_domain = global_domain
                            and loc_site = site_to  */
/*GH52/*F701*/              and loc_loc = loc_to no-lock no-error.  */
/*F701*/             create lddet. lddet.ld_domain = global_domain.
/*F701*/             assign
...
/*GH52*/             find loc_mstr where loc_domain = global_domain and
...
/*GH52*/             end.
/*GH52*/             else do:
/*GK75/*GH52*/        find si_mstr where si_domain = global_domain
                       and si_site = loc_site no-lock no-error. */
/*GK75*/              find si_mstr where si_domain = global_domain and
...
/*J1W2*           BEGIN ADDED SECTION */
             ...
             /* LDDET EXACTLY MATCHES THE USER'S 'TO' CRITERIA */
               ...
               &MSGARG1=mesg_desc
                        ...
                        find first lddet1 where lddet1.ld_domain = global_domain
                             and lddet1.ld_part = pt_part
...
/*kevin
/*F895*/          find is_mstr where is_domain = global_domain and
...
/*G1FP*/             can-find(si_mstr where si_domain = global_domain and si_site = site_to) and
/*G1FP*/             not can-find(loc_mstr where loc_domain = global_domain and
                        loc_site = site_to and
...
/*GE98*/             where loc_domain = global_domain and loc_site = ld_det.ld_site
...
/*GE98*/                            where tag_domain = global_domain
                                      and tag_site   = ld_det.ld_site
