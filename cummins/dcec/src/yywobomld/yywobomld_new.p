...
/*eb2+sp7 retrofit: tao fengqin, 06/22/05              */
         /* DISPLAY TITLE */
/*Last modified by: QAD Leo Zhou, Mar/05/2013              */
...
define variable nbr like wod_nbr  no-undo.
define variable lot like wod_lot  no-undo.
define variable woqtyord like wo_qty_ord.
/*define variable area like mfc_logical
   label "East/West" format "East/West" initial yes.  *CLZ*/
define variable area like si_site no-undo.     /*CLZ*/
  ...
  repeat with frame a:
        ...
        prompt-for wo_nbr wo_lot editing:
           ...
           if frame-field = "wo_nbr" then do:
              ...
              {mfnp.i wo_mstr wo_nbr " wo_domain = global_domain and wo_nbr "
              ...
              /* FIND NEXT/PREVIOUS RECORD */
              if nbr = "" then do:
                 {mfnp.i wo_mstr wo_lot " wo_domain = global_domain and wo_lot "
                         wo_lot wo_lot wo_lot}
              end.
              else do:
                 {mfnp01.i wo_mstr wo_lot " wo_domain = global_domain and wo_lot "
                            nbr wo_nbr wo_nbr}
              end.
           end.
           else do:
...
/*GUI*/ RECT-FRAME-LABEL:SCREEN-VALUE in frame a = F-a-title.
    ...
    if index(wo_bom_code,"zz") > 0 then do:
       /*assign area = no.      *CLZ*/
       assign area = "DCEC-B". /*CLZ*/
    end.
    else do:
       assign area = "DCEC-C" /*yes *CLZ*/ .
    end.
        end.
        else do:
       ...
       assign area = "DCEC-C". /*CLZ*/
        ...
        if nbr <> "" and lot <> "" then
          find wo_mstr where wo_domain = global_domain and wo_nbr = nbr
           and wo_lot = lot no-lock no-error.
        if not available wo_mstr then if nbr = "" and lot <> "" then
           find wo_mstr where wo_domain = global_domain
            and wo_lot = lot no-lock no-error.
        if not available wo_mstr then if nbr <> "" and lot = "" then
           find first wo_mstr where wo_domain = global_domain
                  and wo_nbr = nbr no-lock no-error.
        if nbr <> "" or lot <> "" then
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   update area.
       ...
       find first code_mstr where code_domain = global_domain
              and code_fldname = "Wo-iss-Loc-Area"
        and code_value = area no-lock no-error.
       if not avail code_mstr then do:
    /*User not authorized for this site*/
    {pxmsg.i &MSGNUM=2328 &ERRORLEVEL=3}
          next.
       end.
       /*CLZ* End*/
    ...
    if part <> chExcelWorkbook:Worksheets(1):Cells(2,6):FormulaR1C1 then do:
      ...
      if not available wod_det then do:
       ...
       output stream wobomld to "c:\wobomldfile".
         /*put stream wobomld unformat '"' wo_nbr '" -' skip.  *CLZ*/
   put stream wobomld unformat '- "' wo_lot '"' skip.  /*CLZ*/
      ...
      repeat:
        ...
        where wod_domain = global_domain and wod_nbr = wo_nbr and wod_lot = wo_lot
          and wod_part = wodpart and wod_op = 0 no-error.
        ...
        find pt_mstr where pt_domain = global_domain and pt_part = wodpart no-lock no-error.
        ...
        assign vloc = "".
        if wo_nbr begins "RMB" then do:
            assign vloc = "RB0001".
        end.
        ELSE IF wo_nbr BEGINS "RMC" THEN do:
            assign vloc = "RC001".
        end.
        else do:
          find first in_mstr where in_domain = global_domain
                 and in_part = wodpart and in_site = area no-lock no-error.
          ...
          put stream wobomld unformat
        /*trim(string(woqtyord * chExcelWorkbook:Worksheets(1):Cells(iRow,7):FormulaR1C1)) *CLZ*/
        trim(string(chExcelWorkbook:Worksheets(1):Cells(iRow,7):FormulaR1C1)) /*CLZ*/
              " - - ".

        put stream wobomld unformat "NO - - " .
        /*trim(string(chExcelWorkbook:Worksheets(1):Cells(iRow,7):FormulaR1C1)) *CLZ*/
        " - - ".
          /*******CLZ*******
    ...
    ********CLZ*******/
          put stream wobomld unformat '"' area '" '.             /*CLZ*/
          PUT stream wobomld unformat '"' vloc '" '.
        ...
        * Added section
        ...
        find in_mstr where in_domain = global_domain and in_site = wod_site
        ...
        if available in_mstr then do:
          ...
          wod_bom_amt = glxcst.
           ...
           find ptp_det where ptp_domain = global_domain and ptp_part = wod_part and
         ...
         if available ptp_det then iss_pol = ptp_iss_pol.
             ...
             find in_mstr where in_domain = global_domain and in_part = wodpart and in_site = wod_site
