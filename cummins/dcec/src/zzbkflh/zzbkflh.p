...
define variable cim_yn as logic.
/*judy*/
...
put stream chklog unformat "Start Progress " + list.filename + ":" + string(time,"HH:MM:SS") skip.
      if ok_yn = no then do:
       ...
       if not available emp_mstr then do:
              put stream chklog unformat "错误: 雇员号不存在" at 5 skip.
              ok_yn = no.
       end.
...
/*    put stream chklog unformat "CIM_LOAD 完成,检查CIM结果:" string(time,"HH:MM:SS") skip. */
    ...
    if not available tr_hist then do:
       put unformatted "错误:" + srcdir + list.filename + "CIM_LOAD失败。" at 5.
       put unformat xxwk.par " " xxwk.parloc  " iss-wo " xxwk.qty_comp at 5.
       assign cim_yn = no.
    end.
    for each xxwk where xxwk.comp <> "" and xxwk.qty_iss <> 0 no-lock:
        ...
        if not availabl tr_hist then do:
            put unformatted "错误:" + srcdir + list.filename + "CIM_LOAD失败。" at 5.
            put unformat xxwk.comp " " xxwk.comploc " " xxwk.complot " iss-wo " xxwk.qty_iss at 5.
            assign  cim_yn = no.
        end.
    end.
    ...
    if ok_yn = no or cim_yn = no then do:
         ...
         if cim_yn = yes then do:
            Dos silent value("move " + "~"" + srcdir  + list.filename + "~"" + " " + okdir).
         end.
         else do:
            Dos silent value("move " + "~"" + srcdir + list.filename + "~"" + " " + errdir + "CIM_" + list.filename).
         end.
    end.
end. /*for each list*/
