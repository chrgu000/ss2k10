/* $Revision:eb21sp12  $ BY: Jordan Lin            DATE: 09/06/12  ECO: *SS-20120906.1*   */
    ...
    repeat:                 
        comp = part.
/*G234*/    /* Added section*/
        find ptp_det no-lock where  /* *SS-20120906.1*   */ ptp_det.ptp_domain = global_domain and  ptp_part = part
...
/*G1JF*/          by pkop :
           ...
           find pt_mstr where  /* *SS-20120906.1*   */ pt_mstr.pt_domain = global_domain and pt_part = pkpart no-lock no-error.
...
/*H100*/    and (pkend = ? or eff_date <= pkend)
/*H100*/    and ((op = pkop) or (op = 0))) :
        find first ptp_det where  /* *SS-20120906.1*   */ ptp_det.ptp_domain = global_domain and  ptp_part = pkpart and ptp_site = site no-lock no-error .
        if available ptp_det then
        do:
                if ptp_phantom then
                do:
                find first ps_mstr where /* *SS-20120906.1*   */ ps_mstr.ps_domain = global_domain and ps_par = pkpart no-lock no-error .
        ...
        find first pt_mstr where /* *SS-20120906.1*   */ pt_mstr.pt_domain = global_domain and pt_part = pkpart no-lock no-error .
        if available pt_mstr then
        do:
        if pt_phantom then
                do:
                find ps_mstr where /* *SS-20120906.1*   */ ps_mstr.ps_domain = global_domain and ps_par = pkpart no-lock no-error .
                if available ps_mstr then
                do:
                 create xxtmp2_wkfl .
                assign xxtmp2_par = part
