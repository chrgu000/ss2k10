/* GUI CONVERTED from chglutrp.p (converter v1.71) Sun Oct 21 21:39:26 2007 */
/* chglutrp.p - GENERAL LEDGER UNPOSTED TRANSACTION REGISTER - CAS*/
...
/* $Revision: 9.2 $      BY: Leo Zhou           DATE: 10/15/07  ECO: *CLZ1*   */
/******************************************************************************/
...
/*CF* Add Begin */
         ...
         ***CLZ1**/
         ...
         /**CLZ1 Add Begin**/
         find first xcf_mstr where xcf_domain = global_domain
	       and  xcf_ac_code = glt_det.glt_acct and xcf_active = yes 
               and (xcf_sub = "*" or glt_det.glt_sub = xcf_sub)  
               and (xcf_cc  = "*" or glt_det.glt_cc = xcf_cc)
               and (xcf_pro = "*" or glt_det.glt_project = xcf_pro)                            
               and  glt_det.glt_ref begins "JL" no-lock no-error.

         find first xcf1_mstr where xcf1_domain = global_domain
	       and  xcf1_mfgc_ac_code = glt_det.glt_acct   
               and (glt_det.glt_ref begins "AP" or glt_det.glt_ref begins "AR")
               and (xcf1_mfgc_sub = "*" or glt_det.glt_sub = xcf1_mfgc_sub)
               and (xcf1_mfgc_cc  = "*" or glt_det.glt_cc = xcf1_mfgc_cc)
               and (xcf1_mfgc_pro = "*" or glt_det.glt_project = xcf1_mfgc_pro)   
               and  xcf1_cf_acc = yes and xcf1_active = yes no-lock no-error.         
	 /**CLZ1 Add End**/

         /*if available xcf_mstr then do:  *CLZ1*/
         if available xcf_mstr or available xcf1_mstr then do:  /*CLZ1*/
