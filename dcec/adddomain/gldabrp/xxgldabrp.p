/* GUI CONVERTED from gldabrp.p (converter v1.76) Fri Jul 11 06:30:33 2003 */
...
/*fm268*/
DEFINE NEW SHARED VAR i AS INT INIT 0.
DEFINE NEW SHARED VAR j AS INT INIT 2.
DEFINE NEW SHARED VAR outdir AS CHAR.
DEFINE NEW SHARED VAR outfile AS CHAR.
DEFINE NEW SHARED VAR excelappout AS COM-HANDLE.
DEFINE NEW SHARED VAR excelworkbookout AS COM-HANDLE.
DEFINE NEW SHARED VAR excelsheetmainout AS COM-HANDLE. 

DEFINE NEW SHARED TEMP-TABLE wkacctdt
    FIELD sosum AS INT
...
{wbrp01.i}
   /*fm268*/
    ...
    ASSIGN outfile = "GLdetail.xls".
    FOR EACH wkacctdt .
       DELETE wkacctdt.
    END.
    IF SEARCH("D:\GLreport\template\GLdetail.xlt") = ? THEN DO: 
       MESSAGE "报表模板不存在!"  VIEW-AS ALERT-BOX ERROR.
       UNDO,RETRY.
    END.

    CREATE "Excel.Application" excelappout.
    excelworkbookout = excelappout:workbooks:ADD("D:\GLreport\template\GLdetail.xlt").
...
/*GUI*/ RECT-FRAME:HEIGHT-PIXELS in frame a = FRAME a:HEIGHT-PIXELS - RECT-FRAME:Y in frame a - 2.
   ...
   for each asc_mstr
      ...
      */
      ...
      no-lock where ac_domain = global_domain and ac_code = asc_acc:
      end.
      if rpt_curr = base_curr or ac_curr = rpt_curr then do:
         asc_recno = recid(asc_mstr).
  /*judy 07/06/05*/  /* {gprun.i ""gldabrpa.p""}  */
  /*judy 07/06/05*/     /* {gprun.i ""yygldabrpa.p""} */ 
/*fm268 07/06/05*/      {gprun.i ""xxgldabrpa.p""}
...
/*GUI*/ {mfguichk.i } /*Replace mfrpchk*/
  ...
  /* put {gplblfmt.i
          ...
          &CONCAT = "':'"
        } et_report_curr to 60 SKIP  /*fm268 币种*/

      {gplblfmt.i
         &FUNC=getTermLabel(""TOTAL_BEGINNING_BALANCE"",35)
         &CONCAT = "':'"
      } string(et_beg_tot, prtfmt) format "x(20)" to 63 SKIP  /*fm268 期初*/

      {gplblfmt.i
         &FUNC=getTermLabel(""TOTAL_ACTIVITY_TO_DATE"",35)
         &CONCAT = "':'"
      } string(et_per_tot, prtfmt) format "x(20)" to 63 SKIP /*fm268 活动*/

      {gplblfmt.i
         &FUNC=getTermLabel(""TOTAL_ENDING_BALANCE"",35)
         &CONCAT = "':'"
      } string(et_end_tot, prtfmt) format "x(20)" to 63 skip. fm268 期末*/
   {&GLDABRP-P-TAG27}
   {&GLDABRP-P-TAG20}
   FOR EACH wkacctdt NO-LOCK  BY sosum.
       j = j + 1.
       excelworkbookout:worksheets(1):cells(j,1):VALUE = wkacctdt.account.
       excelworkbookout:worksheets(1):cells(j,2):VALUE = wkacctdt.detail.
       IF wkacctdt.frsum <> 0 THEN excelworkbookout:worksheets(1):cells(j,3):VALUE = wkacctdt.frsum.
       excelworkbookout:worksheets(1):cells(j,4):VALUE = wkacctdt.perriod.
       excelworkbookout:worksheets(1):cells(j,5):VALUE = wkacctdt.dodate.
       excelworkbookout:worksheets(1):cells(j,6):VALUE = wkacctdt.pzhsum.
       excelworkbookout:worksheets(1):cells(j,7):VALUE = wkacctdt.des1.
       excelworkbookout:worksheets(1):cells(j,8):VALUE = wkacctdt.des2.
       excelworkbookout:worksheets(1):cells(j,9):VALUE = wkacctdt.des3.
       excelworkbookout:worksheets(1):cells(j,10):VALUE = wkacctdt.fdr1.
       excelworkbookout:worksheets(1):cells(j,11):VALUE = wkacctdt.fcr1.
   END.
       j = j + 1.
       excelworkbookout:worksheets(1):cells(j,6):VALUE = "帐户总合计".
       excelworkbookout:worksheets(1):cells(j,7):VALUE = "期初余额合计:".
       excelworkbookout:worksheets(1):cells(j,8):VALUE = "总累计活动金额:".
       excelworkbookout:worksheets(1):cells(j,9):VALUE = "期末余额合计:".
       excelworkbookout:worksheets(1):cells(j,10):VALUE = et_beg_tot.
       excelworkbookout:worksheets(1):cells(j,11):VALUE = et_per_tot.
       excelworkbookout:worksheets(1):cells(j,12):VALUE = et_end_tot.
    FOR EACH wkacctdt .
       DELETE wkacctdt.
       RELEASE wkacctdt.
    END.
  ...
  excelworkbookout:saveas(outdir + outfile,,,,,,1).
  excelworkbookout:CLOSE.
  RELEASE OBJECT excelworkbookout.
  excelappout:QUIT.
  RELEASE OBJECT excelappout.
  MESSAGE "报表完毕,文件名称为GLdetail.xls，在D:\GLreport目录下!"  VIEW-AS ALERT-BOX.
  UNDO,RETRY.
end.
...
PROCEDURE validate-input:
   ...
   for first glc_cal
      ...
      where glc_domain = global_domain and glc_year = yr and glc_per = 1
   no-lock:
   end.
   ...
   per = glc_per.
   find last glc_cal where glc_domain = global_domain and glc_year = yr no-lock.
   assign
      yr_end = glc_end
      begdt0 = begdt
      enddt0 = enddt.
