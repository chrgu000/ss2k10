...
/*GUI*/ RECT-FRAME:HEIGHT-PIXELS in frame a = FRAME a:HEIGHT-PIXELS - RECT-FRAME:Y in frame a - 2.
	...
	put "开始计算:" yyinvi_part string(time, "HH:MM:SS") format "x(100)" skip.
	assign 
	    pcqty = 0
	    stdmtlcost = 0
	    stdmfgcost = 0
	...
	*/
	...
	if not avail yywobmspt_mstr then do:
		...
		if not avail yywobmspt_mstr then do:
			find last yywobmspt_mstr where yywobmspt_part = yyactcs_part no-lock no-error.
	...
	for each ttvardet :
		...
		find first bptmstr where bptmstr.pt_part = ttvardet_comp 
