...
/* ss - 121010.1 by: Steven */ /* add filter by user id */
...
FORM /*GUI*/ 
 ...
 SKIP(.1)  /*GUI*/
    ...
    efdt COLON 20 efdt1 COLON 48 LABEL {t001.i} SKIP
    /* ss - 121010.1 - B */ 
    user_id   colon 20    
    user_id1 colon 48 label {t001.i} skip(1)
    /* ss - 121010.1 - E */ 
...
repeat:
    ...
    /* ss - 121010.1 - E */ 
    UPDATE bh bh1 dt dt1 vd vd1 efdt efdt1 user_id user_id1
    ...
    FOR EACH vd_mstr NO-LOCK WHERE /* *SS-20120815.1*   */ 
    vd_mstr.vd_domain = global_domain and vd_addr >= vd AND vd_addr <= vd1 :

        FOR EACH ap_mstr NO-LOCK WHERE /* *SS-20120815.1*   */ 
        ap_mstr.ap_domain = global_domain and ap_batch >= bh AND ap_batch <= bh1
            ...
            if avail gltr_hist and gltr_user >= user_id and gltr_user <= user_id1 then do:
            /* ss - 121010.1 - e */ 
               /*GUI*/ 
               ...
               IF FIRST-OF(ap_vend) THEN DO :
                  {gprun.i ""yyapvn1.p"" "(input ap_vend,
                   ...
                   FIND ad_mstr NO-LOCK WHERE /* *SS-20120815.1*  */ ad_mstr.ad_domain = global_domain and ad_addr = ap_vend NO-ERROR .
               ...
               IF ap_type = "CK" THEN DO :
                   FIND FIRST ckd_det NO-LOCK WHERE /* *SS-20120815.1*  */ 
                   ckd_det.ckd_domain = global_domain and ckd_ref = ap_ref 
                   AND (ckd_acct = "6666666" OR ckd_acct = "9999999") NO-ERROR .
            ...
            end. /*if avail gltr_hist and gltr_user >= user_id and gltr_user <= user_id1*/  /* ss - 121010.1 */
