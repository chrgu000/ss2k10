/* GUI CONVERTED from arcsrp03.p (converter v1.78) Mon Aug 31 07:14:43 2009 */
...
/* $Revision: 1.11.1.13.3.1 $     BY: Rijoy Ravindran     DATE: 08/28/09 ECO: *Q3BJ* */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
...
FORM /*GUI*/ 
   ...
   contested  no-label
   ar_curr
with STREAM-IO /*GUI*/  frame c width 80 down.
...
find ad_mstr  where ad_mstr.ad_domain = global_domain and  ad_addr
   ...
   for each cm_mstr  where cm_mstr.cm_domain = global_domain and (
      ...
      for each ar_mstr
         ...
         or       ar_date             >= start_date)
         and     (bal_out             = no
         or       ar_amt - ar_applied <> 0)
         and      ar_type             <> "A"
         and      ar_curr             <> ""
         and (not ar_type             = "D"
         or       ar_draft            = true))
      ...
      break by ar_bill
         ...
         if available ct_mstr and ct_dating = yes then do:
            ...
            /*DISPLAY THE LINE*/
            display ar_date with frame c STREAM-IO /*GUI*/ .
         ...
         if last-of(ar_curr) then do:
            ...
            if contest_tot <> 0 then
               display
                  skip(2)
                  l_msgdesc no-label at 3 skip WITH STREAM-IO /*GUI*/ .
...
/*GUI*/ {mfguirpb.i &flds=" cust cust1 start_date bal_out stmt_cyc comp_addr age_days [ 1 for 3 ] msg "} /*Drive the Report*/
