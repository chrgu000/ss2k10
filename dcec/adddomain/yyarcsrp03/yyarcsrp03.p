...
repeat:
    ...
    IF efdt = ? THEN efdt = 01/01/04 .
    IF efdt1 = ? THEN efdt1 = TODAY .
   ...
   no-lock:
      ...
      for each ar_mstr where ar_domain = global_domain and
         ar_bill = cm_addr and
      ...
      by ar_effdate DESCENDING :
         ...
         if first-of(ar_bill) then do:
            find ad_mstr where ad_domain = global_domain and ad_addr = ar_bill no-lock no-error.
            {gprun.i ""yyarbal0.p"" "(input ar_bill,
                INPUT (efdt - 1),
                OUTPUT startamt)"
             }
            {gprun.i ""yyarbal0.p"" "(input ar_bill,
            ...
            PUT "生效日期" AT 2 "发票/支票" AT 12 "类型" AT 22 "货币金额" TO 50 "货币" AT 52 "人民币金额" TO 75 "余额" TO 95 "总帐参考号" AT 99 SKIP .
         ...
         if ar_type = "P" and
            can-find(first ard_det where ard_domain = global_domain and ard_nbr = ar_nbr and
            ard_ref = "")
         then do:
         ...
         IF base_amt <> 0 THEN DO :
             FIND FIRST gltr_hist NO-LOCK WHERE gltr_domain = global_domain and
             					  gltr_batch = ar_batch AND gltr_doc = ar_nbr USE-INDEX gltr_batch NO-ERROR .
             IF AVAILABLE gltr_hist THEN glref = gltr_ref .
             ELSE DO :
                 FIND FIRST glt_det NO-LOCK WHERE glt_domain = global_domain and
                 					  glt_batch = ar_batch AND glt_doc = ar_nbr USE-INDEX glt_batch NO-ERROR .
                 IF AVAILABLE glt_det THEN glref = glt_ref .
                 ELSE glref = "" .
             END.
             PUT ar_effdate AT 2 nbr0 AT 12 TYPE AT 22 base_amt TO 50 ar_curr AT 52 amtrmb TO 75 amt0 TO 95 glref AT 99 SKIP .             
         END.


         amt0 = amt0 - amtrmb .

         IF LAST-OF(ar_bill) THEN PUT (efdt - 1) AT 2 "期初" AT 12 startamt TO 95 SKIP(2) .
