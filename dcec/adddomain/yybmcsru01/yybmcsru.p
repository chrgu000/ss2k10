/* GUI CONVERTED from bmcsru.p (converter v1.75) Thu Jun 28 21:18:54 2001 */
...
FORM /*GUI*/ 
 ...
 SKIP(.1)  /*GUI*/
   ...
   method                  no-label
   part           colon 18 /*james**part1  label {t001.i} colon 49
...
repeat:
   ...
   if not batchrun then do:
      ...
      do transaction on error undo, retry:
         ...
         editing:
            ...
            if recno <> ? then do:
               csset = cs_set.
               display csset with frame a.
               find cs_mstr where cs_domain = global_domain and cs_set = csset no-lock no-error.
               method = "".
         ...
         if transtype = "SC" then do:
            if not can-find(first cs_mstr where cs_domain = global_domain and cs_set = csset
      ...
      status input.
      update
         part VALIDATE(CAN-FIND(FIRST pt_mstr WHERE pt_domain = global_domain and pt_part = part), "«Î ‰»Î¡„º˛∫≈...")
          /*james***part1
         line line1
         type type1
         grp grp1 */
      ...
      repeat transaction on error undo, retry:
         ...
         do:
            find qad_wkfl exclusive-lock
               where qad_wkfl.qad_domain = global_domain and qad_key1 = "BMCSRUB"
         ...
         find qad_wkfl share-lock where qad_wkfl.qad_domain = global_domain and qad_key1 = "BMCSRUB"
            and qad_key2 = site + "::" + csset
            no-error.
         ...
         then do:
            ...
            do on endkey undo mainloop, retry mainloop:
               ...
               {pxmsg.i &MSGNUM=3336 &ERRORLEVEL=1
                  ...
                  if yn = no then do:
                     find qad_wkfl exclusive-lock
                        where qad_domain = global_domain and qad_key1 = "BMCSRUB"
                        and   qad_key2 = site + "::" + csset
                        no-error no-wait.
                     if available qad_wkfl then delete qad_wkfl.
                     else yn = yes.
...
/*GUI*/ RECT-FRAME:HEIGHT-PIXELS in frame a = FRAME a:HEIGHT-PIXELS - RECT-FRAME:Y in frame a - 2.
   ...
   if mtl_flag or lbr_flag or bdn_flag or ovh_flag or sub_flag
      ...
      {gprun.i ""bmcsrub.p""}
      /*james*/ {gprun.i ""yywobmfmbdx.p"" "(input part, input site, input csset, input eff_date)"}
   end.
   ...
   do transaction:
      find qad_wkfl
         where qad_domain = global_domain and
         qad_key1       = "BMCSRUB" and
