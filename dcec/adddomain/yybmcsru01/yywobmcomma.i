PROCEDURE xxpro-get-cost-det:
    ...
    FIND FIRST sct_det 
        WHERE sct_domain = global_domain and sct_part = p_part
        AND sct_site = p_site AND sct_sim  = v_costset NO-LOCK NO-ERROR.
...
PROCEDURE xxpro-get-cost-tot:
    ...
    IF p_asofflag = YES THEN DO:
        for first tr_hist
            ...
            where tr_domain = global_domain and tr_part    =  p_part
