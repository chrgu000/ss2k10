...
find last xxwobmfm_mstr where xxwobmfm_domain = global_domain and xxwobmfm_site = iSite
	and xxwobmfm_part = iPart
	use-index xxwobmfm_idx1 no-lock no-error.
if available xxwobmfm_mstr then do:
        /* get cost set */
	find first in_mstr where in_domain = global_domain and in_part = iPart and in_site = iSite no-lock no-error.
	v_costset = (if available in_mstr and in_gl_set <> "" then in_gl_set else "STANDARD").

	/* explode part by standard picklist logic */
	{gprun.i ""woworla2.p""}

	for each spt_det no-lock where spt_domain = global_domain and spt_site = iSite
		and spt_sim = v_costset
		and spt_part = iPart
		by spt_element:
		/* 生成对应版本镜像BOM成本元素明细 */
		find first yywobmspt_mstr where yywobmspt_domain = global_domain and yywobmspt_site = iSite
	...
	for each yywobmsptd_det where yywobmsptd_domain = global_domain and yywobmsptd_part = iPart
	...
	for each pkdet
		...
		for each spt_det no-lock where spt_domain = global_domain and spt_site = iSite
			...
			and spt_part = pkpart
			by spt_element:
			find first yywobmsptd_det where yywobmsptd_domain = global_domain and yywobmsptd_part = iPart 
				and yywobmsptd_site    = iSite
				and yywobmsptd_version = xxwobmfm_version
