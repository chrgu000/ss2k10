...
repeat:
  ...
  set parent1
	...
	with frame a editing:
	 ...
	 if	frame-field	= "parent1"	then do:
	   ...
	   if recno	<> ? then do:
		  ...
		  find pt_mstr where  /* *SS-20120816.1*   */ pt_mstr.pt_domain = global_domain and pt_part = bom_parent no-lock no-error.
	...
	find first bom_mstr where /* *SS-20120816.1*   */ bom_mstr.bom_domain = global_domain and  bom_parent = parent1 no-lock no-error.
	...
	find first bom_mstr where /* *SS-20120816.1*   */ bom_mstr.bom_domain = global_domain and bom_parent = parent2 no-lock no-error.
	if not available bom_mstr then do:
		message "产品结构不存在，请重新输入! " view-as alert-box.
	...
	find pt_mstr where /* *SS-20120816.1*   */ pt_mstr.pt_domain = global_domain and pt_part = parent1 no-lock no-error.
	if available pt_mstr then
		desc11 = pt_desc2. 				
	find pt_mstr where /* *SS-20120816.1*   */ pt_mstr.pt_domain = global_domain and  pt_part = parent2 no-lock no-error.
	if available pt_mstr then
		desc12 = pt_desc2. 				
