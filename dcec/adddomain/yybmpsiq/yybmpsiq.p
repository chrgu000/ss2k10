...
repeat:
   ...
   then do:
      ...
      for first bom_mstr
         ...
         where bom_domain = global_domain and bom_parent = parent
         no-lock:
      ...
      then do:
         ...
         if bom_desc <> ""  /*OR NOT SUBSTRING(PARENT, LENGTH(PARENT) - 1 ,2) = "zz" */
         then
            desc1 = bom_desc.
         else
         if available pt_mstr
...
/*GUI*/ RECT-FRAME:HEIGHT-PIXELS in frame a = FRAME a:HEIGHT-PIXELS - RECT-FRAME:Y in frame a - 2.
   ...
   then do:
      ...
      for each ecm_mstr
         fields(ecm_domain ecm_eff_date ecm_nbr)
         no-lock
         where ecm_domain = global_domain and ecm_eff_date <> ?,
            each ecd_det
            fields(ecd_domain ecd_nbr ecd_new_rev ecd_part)
            no-lock
            where ecd_domain = global_domain and ecd_nbr = ecm_nbr
            and ecd_new_rev = rev
...
PROCEDURE process_report:
   ...
   for first bom_mstr
      fields(bom_domain bom_batch_um bom_desc bom_fsm_type bom_parent)
      where bom_domain = global_domain and bom_parent = comp
      no-lock:
   end. /* FOR FIRST bom_mstr */
   ...
   open query q_ps_mstr
      ...
      where ps_domain = global_domain and ps_par = comp no-lock.
...
/*GUI*/ {mfguichk.i } /*Replace mfrpchk*/
      ...
      then do:
         ...
         for first pt_mstr
            ...
            where pt_domain = global_domain and pt_part = ps_comp
            no-lock:
         end. /* FOR FIRST pt_mstr */
         ...
         then do:
            ...
            for first bom_mstr
               fields(bom_domain bom_batch_um bom_desc bom_fsm_type bom_parent)
               where bom_domain = global_domain and bom_parent = ps_comp
               no-lock:
            end. /* FOR FIRST bom_mstr */
            if available bom_mstr
            then
