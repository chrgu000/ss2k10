...
for first mfc_ctrl
   ...
   where mfc_domain = global_domain and mfc_field = "num_ckrecs" no-lock:
end. /* for first mfc_ctrl */

if available mfc_ctrl then ckrecsz = mfc_integer.

for first ps_mstr
   fields (ps_domain ps_comp ps_par)
...
then do:
 ...
 /*tfq  {mfmsg.i 5004 1}    */
   ...
   if can-find(first ps_mstr where ps_domain = global_domain and ps_par = part1 and ps_comp = part)
   then do:
      ps_recno = 0.
      leave.
   end. /* then do: */
   ...
   for first ps_mstr
      fields (ps_domain ps_comp ps_par)
      use-index ps_comp where ps_domain = global_domain and ps_comp = part no-lock:
   end. /* for first ps_mstr */
   ...
   repeat:
      ...
      then do:
         for each ptp_det
            fields (ptp_domain ptp_bom_code ptp_part ptp_site)
            no-lock where ptp_domain = global_domain and ptp_bom_code = par:
            if ptp_part = part1
            then do:
               ps_recno = 0.
               leave.
         ...
         for each pt_mstr
            fields (pt_domain pt_bom_code pt_part)
            no-lock where pt_domain = global_domain and pt_bom_code = par:
            if pt_part = part1
            then do:
               ps_recno = 0.
               leave.
         ...
         repeat:
            ...
            do while available ps_mstr:
               ...
               find next ps_mstr no-lock use-index ps_comp
                  where ps_domain = global_domain and ps_comp = part no-error.
      ...
      then do:
         ...
         for first ps_mstr
               fields (ps_domain ps_comp ps_par)
               no-lock use-index ps_comp where ps_domain = global_domain and ps_comp = par:
         end. /* for first ps_mstr */
      end. /* then do: */
      else do:
         find next ps_mstr no-lock use-index ps_comp
            where ps_domain = global_domain and ps_comp = par no-error.
      end. /* else do: */
   end. /* repeat: */
