...
REPEAT:
 ...
 DO TRANSACTION ON ERROR UNDO, LEAVE:   
     ...
     dtn_file = src_file.
      ...
      UPDATE src_file sheetname startline VALIDATE(INPUT startline > 0 ,"行号小于等于零是不允许的")  with frame a.
       ...
       worksheet:Columns("O"):ColumnWidth = 24.
       i = startline.
       ...
       END.
       ELSE DO:
           FOR EACH sod_det WHERE sod_domain = global_domain and 
           			    sod_nbr = sonbr NO-LOCK:
      ...
      REPEAT:    /*so input repeat*/
           ASSIGN    
                   ...
                   amount = INT(worksheet:cells(i,13):TEXT).
           ...
           FIND FIRST ad_mstr WHERE ad_domain = global_domain and 
           					  ad_type = "customer" AND ad_addr = cust NO-LOCK NO-ERROR.
           ...
           IF AVAIL pt_mstr THEN DO:
                worksheet:Range("L" + STRING(i)):VALUE = pt_desc1 + pt_desc2 .
           END.
           ELSE DO:
           ...
           FIND FIRST in_mstr WHERE in_domain = global_domain and 
           					  in_part = part AND in_site = "dcec-b" NO-LOCK NO-ERROR.
           IF AVAIL in_mstr THEN DO:
               worksheet:Range("P" + STRING(i)):VALUE = in_qty_oh. 
           END.
           ELSE DO:
               worksheet:Range("P" + STRING(i)):VALUE = "库存信息不存在！". 
           END.

           FIND FIRST in_mstr WHERE in_domain = global_domain and 
           					  in_part = part AND in_site = "dcec-c" NO-LOCK NO-ERROR.
           IF AVAIL in_mstr THEN DO:
               worksheet:Range("Q" + STRING(i)):VALUE = in_qty_oh. 
           END.
           ELSE DO:
               worksheet:Range("Q" + STRING(i)):VALUE = "库存信息不存在！". 
           END.

           FIND FIRST in_mstr WHERE in_domain = global_domain and 
                      in_part = part AND in_site = "dcec-sv" NO-LOCK NO-ERROR.
           IF AVAIL in_mstr THEN DO:
           ...
           FIND FIRST in_mstr WHERE in_domain = global_domain and 
           					  in_part = part AND in_site = "CEBJ" NO-LOCK NO-ERROR.
           IF AVAIL in_mstr THEN DO:
               worksheet:Range("U" + STRING(i)):VALUE = in_qty_oh. 
           END.
           ELSE DO:
           ...
           FIND FIRST in_mstr  WHERE in_domain = global_domain and 
           				 in_part = part and
                   in_site = "dcec-sv" NO-LOCK NO-ERROR.
           IF AVAIL in_mstr THEN DO:      
              worksheet:Range("S" + STRING(i)):VALUE = in_qty_ord. 
              worksheet:Range("T" + STRING(i)):VALUE = in_qty_ord + in_qty_oh - in_qty_req.
           ...
           IF AVAIL in_mstr THEN DO:      
              worksheet:Range("V" + STRING(i)):VALUE = in_qty_ord.
              worksheet:Range("W" + STRING(i)):VALUE = in_qty_ord + in_qty_oh - in_qty_req. 
           ...
           FOR EACH pi_mstr NO-LOCK WHERE pi_domain = global_domain   
           		 and  pi_list = pricelist 
               AND ((TODAY >= pi_start AND TODAY <= pi_expire) OR (pi_start = ? AND pi_expire = ?))
               AND pi_um = "ea"
               AND pi_curr = "RMB":
               IF pi_part_code MATCHES "qadall*" AND pi_cs_code MATCHES "qadall*" THEN
                   mprlist = YES.
               IF pi_part_code MATCHES "qadall*" AND pi_cs_code = cust THEN
                   mprlist = YES.
               IF pi_part_code = part AND pi_cs_code MATCHES "qadall*" THEN
                   mprlist = YES.
           ...
           IF mprlist = NO THEN DO:
               worksheet:Range("X" + STRING(i)):VALUE = "价格单未维护，价格为零！"   .
           END.
           ...
           IF AVAIL ptp_det THEN DO:
               ...
               IF omin = 0 OR omin = 44444 THEN  DO:             
                   IF omin = 0 THEN DO:
                       worksheet:Range("O" + STRING(i)):VALUE = amount.
                   END.
                   IF omin = 44444 THEN DO:
                       worksheet:Range("O" + STRING(i)):VALUE = "".
                       FIND FIRST pts_det WHERE pts_domain = global_domain and 
                       		  pts_par = "" AND pts_part = part NO-LOCK NO-ERROR.
                       IF AVAIL pts_det THEN DO:
                           worksheet:Range("N" + STRING(i)):VALUE = pts_sub_part.
                       END.
                       ELSE DO:
                           worksheet:Range("N" + STRING(i)):VALUE = "没有替代零件".
                       END.
               ...
               ELSE DO:
                   ...
                   REPEAT:
                      ...
                      k = k + 1.
                   END.
                   worksheet:Range("O" + STRING(i)):VALUE = k * omin.
               END.
               IF omin <> 44444 THEN
                    worksheet:Range("N" + STRING(i)):VALUE = omin.
           END.
           ELSE DO:
               FIND FIRST pt_mstr WHERE pt_domain = global_domain and 
               					  pt_part = part NO-LOCK NO-ERROR.
               ...
               END.
               ELSE DO:               
                   worksheet:Range("N" + STRING(i)):VALUE = "零件信息不存在！".
               END.
