/* GUI CONVERTED from mgbdpro.p (converter v1.78) Mon Sep 12 06:50:29 2011 */
/* mgbdpro.p - Process CIM batch data that has been loaded into the database  */
...
/* Revision: 1.31     BY: Paul Donnelly (SB) DATE: 06/28/03  ECO: *Q00G*     */
/* Revision: 1.32     BY: Ed van de Gevel    DATE: 07/08/03  ECO: *Q003*     */
/* Revision: 1.33     BY: Max Iles           DATE: 12/14/04  ECO: *P2ZP*     */
/* Revision: 1.33.1.1 BY: Ashim Mishra       DATE: 10/22/07  ECO: *P6B9*     */
/* Revision: 1.33.1.2 BY: Alex Joy           DATE: 10/29/07  ECO: *P6BN*     */
/* Revision: 1.33.1.3 BY: Alex Joy           DATE: 01/29/07  ECO: *P6KT*     */
/* Revision: 1.33.1.4 BY: Mukesh Singh       DATE: 03/04/10  ECO: *Q3VB*     */
/* $Revision: 1.33.1.8 $      BY: Keny Fernandes     DATE: 09/02/11  ECO: *Q50S*     */
...
/*YY*/ {zycimload.i}
/**dss01 begin added**/
/*yy define input parameter fid as int.
define input parameter lid as int. */
/*dss01-01 define input parameter xslogname as char format "x(150)". */
/* YY define input parameter transid as char .*/
/* YY define output parameter odberr as int.
define output parameter opgmerr as int.*/
define var   msgtxt as char.
define var   grppro as char.
define var   dberr  as char.
define var   pgmwrn as char.
define var   pgmerr as char.
/**dss01 end   added**/
...
define variable access_err as character no-undo.
 ...
 * 
 * /* USER NOT AUTHORISED TO THIS MENU */
 * {pxmsg.i &MSGNUM=5218 &ERRORLEVEL=4 &MSGBUFFER=access_err}
 */
/* *SS-20121009.1* -e */
assign
   ...
   procedure_err = substring(procedure_err,1,length(procedure_err) - 1)
                 + " ->"
   access_err    = substring(access_err,1,length(access_err) - 1)
                 + " ->"
 ...
 */
          ...
          msgtxt =  "ÅúÊý¾Ý: " + string(today) + " " + string(time,"HH:MM:SS").
         /*dss01-01 {xslogger.i xslogname ""CIM-WARN"" msgtxt} */
...
/*yy          odberr = progress_errors.
          opgmerr = function_errors.  */
...
PROCEDURE process_by_screen:
   ...
   new_pgm_loop:
   repeat:
      run checkMenuRestrictions (input execname,
                                 output l_err).
      if l_err
      then do:
 ...
 */
            ...
            /*dss01-01 {xslogger.i xslogname ""CIM-WARN"" msgtxt}  */
/*yy            {xslogger.i transid ""CIM"" ""IMP"" ""WARN"" MSGTXT}.*/
            CREATE xxerrtb.
            xxerr = "01".
            xxmsg = msgtxt.
...
/*
               put stream errorlog unformatted
                  ...
                  space.
 */
                msgtxt = getTermLabel("INPUT_FILE_NAME",20) + " " +
                        file_name.
/* *SS-20121009.1* -e */
            do i = 1 to 40:
...
/* *SS-20121009.1* -e */
            ...
            /* index of last word */
            i = i - 1.
  /* *SS-20121009.1*          put stream errorlog skip(1). */ 
         end.
...
PROCEDURE setup_input:
 ...
 /*     {pxmsg.i &MSGNUM=391 &ERRORLEVEL=3 &MSGARG1=file_name}  */
       ...
       {pxmsg.i &MSGNUM=391 &ERRORLEVEL=4 &MSGARG1=file_name &MSGBUFFER=MSGTXT}
          CREATE xxerrtb.
          xxerr = "09".
          xxmsg = msgtxt.
...
/* *SS-20121009.1*    {mfrtrail.i "stream errorlog"} */ 

END PROCEDURE.

procedure checkMenuRestrictions:
   define input  parameter p_function_to_run as character no-undo.
   define output parameter p_err             as logical no-undo.

   define variable l_can_do_menu as logical no-undo.

   define buffer mnd_det for mnd_det.

   p_err = no.
   for each mnd_det
      where mnd_exec = p_function_to_run
   no-lock:
      {gprun1.i ""mfsec.p"" "(input mnd_nbr,
                              input mnd_select,
                              input false,
                              output l_can_do_menu)"}
      if not l_can_do_menu
      then do:
         p_err = yes.
         leave.
      end.
