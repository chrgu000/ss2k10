/* yyfsbudrp01.p  - Finance budget report - Difference of budget and actual occurs                              */
...
/*  比较预算和实际发生金额 */
/* $Revision:eb21sp12  $ BY: Jordan Lin            DATE: 08/14/12  ECO: *SS-20120814.1*   */
...
define variable bgdcode like bgd_code initial "2005aop".
...
repeat:
      ...
      update entity_from entity_to account_from account_to date_from date_to center_from center_to
      bgdcode validate(bgdcode<>"","请选择预算代码") detail ddetail
      with frame a.
      ...
      for each cc_mstr where  /* *SS-20120814.1*   */ cc_mstr.cc_domain = global_domain and cc_ctr >= center_from and cc_ctr <= center_to no-lock:
         create sel.
...
for each bgd_det no-lock where /* *SS-20120814.1*   */ bgd_det.bgd_domain = global_domain 
                           and bgd_entity >= entity_from and bgd_entity <= entity_to 
                           and bgd_cc>= center_from and bgd_cc<=center_to                 
                           and (bgd_acc >= account_from and bgd_acc <=account_to)  
                           and (substring(bgd_acc,1,4)="5501" or substring(bgd_acc,1,4)="5502" or substring(bgd_acc,1,4)="4105")
                           and bgd_code = bgdcode,
/*kevin*/ each sel where sel.ctr = bgd_cc and sel.id = "*" no-lock                          
                    by bgd_cc by bgd_year by bgd_per by bgd_acc :

                    
                   /*MESSAGE bgd_acc bgd_year bgd_per .
                   PAUSE.  */
    /*日期period  date(month,day,year)*/
    if date(bgd_per,01,bgd_year) >= date_from and date(bgd_per,01,bgd_year)<= date_to  then do:
            gltsumamt=0.
            kk=0.
            create comtwo.   
            assign entity = bgd_entity                 
                   ...
                   different = bgdamt - gltamt
                    logn = 1.
            for each gltr_hist no-lock where /* *SS-20120814.1*   */ gltr_hist.gltr_domain = global_domain and gltr_entity=bgd_entity and gltr_acc = bgd_acc 
            ...
            for each glt_det no-lock where /* *SS-20120814.1*   */ glt_det.glt_domain = global_domain and glt_entity=bgd_entity and glt_cc = bgd_cc 
...
for each gltr_hist no-lock where /* *SS-20120814.1*   */ gltr_hist.gltr_domain = global_domain and gltr_entity >= entity_from and gltr_entity <= entity_to
                   and gltr_ctr>= center_from and gltr_ctr<=center_to
                   and gltr_eff_dt>= date_from and gltr_eff_dt<=date_to
                   and (gltr_acc >= account_from and  gltr_acc <=account_to) /*and (gltr_sub ='0' or gltr_sub ='1' and gltr_sub ='2')*/
                   and (substring(gltr_acc,1,4)="5502" or substring(gltr_acc,1,4)="5501" or substring(gltr_acc,1,4)="4105"),
...
for each glt_det no-lock where /* *SS-20120814.1*   */ glt_det.glt_domain = global_domain and glt_entity >= entity_from and glt_entity <= entity_to
                   and glt_cc>= center_from and glt_cc<=center_to
                   and glt_effdate>= date_from and glt_effdate<=date_to
                   and (glt_acct >= account_from and glt_acct <=account_to) /*and (glt_sub ='0' or glt_sub ='1' or glt_sub ='2')*/
                   and (substring(glt_acct,1,4)="5502" or substring(glt_acct,1,4)="5501" or substring(glt_acct,1,4)="4205"),
...
/*显示明细记录*/
if detail then
display "预算代码:" at 1 bgdcode at 12 with frame ff2 width 240 no-label .
...
page.
hide frame ff2.
display "预算代码:" at 1 bgdcode at 12 with frame ff3 width 240 no-label .
