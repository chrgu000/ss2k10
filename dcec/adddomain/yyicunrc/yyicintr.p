...
if transtype = "ISS-RV" then do:
   if can-find(first po_mstr where po_nbr >= "") and
      can-find(first ap_mstr where ap_type >= "" and ap_ref >= "")
   then do:
      /* Use PO RTS Issue */
...
repeat:
/*********added by tfq*****************************/
 find first code_mstr where code_fldname = "account-length" no-lock no-error .
...
/****************tfq added end*******************/ 
  ...
  /* tfq added begin in 2005/06/20 ****/
		  find first code_mstr where code_fldname = "so_job"
		...
		/* message "111--" cr_sub . */
         ...
         if not available in_mstr then do:
            ...
            /* CALLING gpingl.p.                                */
            for first si_mstr where si_site = site no-lock: end.
...
*****tfq deleted end from 2005/06/20**************/
         find ac_mstr no-lock where ac_code = cr_acct no-error.
...
*************tfq deleted begin from 2005/06/20 */
         find ac_mstr no-lock where ac_code = dr_acct no-error.
         if available ac_mstr then
            dr_desc = ac_desc.
         else
            dr_desc = "".
...
PROCEDURE checkConsignmentInventory:
   ...
   if not op_error then do:
      ...
      for first ld_det
          fields(ld_qty_oh ld_cust_consign_qty)
          where ld_part = ip_part     and
                ld_site = ip_site     and
