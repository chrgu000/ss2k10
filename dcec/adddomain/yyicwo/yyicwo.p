...
/*GUI*/ RECT-FRAME:HEIGHT-PIXELS in frame a = FRAME a:HEIGHT-PIXELS - RECT-FRAME:Y in frame a - 2.
   ...
   if v_update then do:
           /*Clear*/
	   for each yycsvar_mstr where /*ss2012-8-15 b*/ yycsvar_mstr.yycsvar_domain = global_domain and /*ss2012-8-15 e*/ yycsvar_year = v_year and yycsvar_per = v_period
	   ...
	   for each yycsvard_det where /*ss2012-8-15 b*/ yycsvard_det.yycsvard_domain = global_domain and /*ss2012-8-15 e*/ yycsvard_year = v_year and yycsvard_per = v_period
	   ...
	   for each yyinvi_mstr where /*ss2012-8-15 b*/ yyinvi_mstr.yyinvi_domain = global_domain and /*ss2012-8-15 e*/ yyinvi_year = v_year and yyinvi_per = v_period
				and yyinvi_part >= part and yyinvi_part <= part1:
	       assign yyinvi_mfg_qty = 0.
	   end. /*for each yyinvi_mstr*/

	   for each yyinvid_det where /*ss2012-8-15 b*/ yyinvid_det.yyinvid_domain = global_domain and /*ss2012-8-15 e*/ yyinvid_year = v_year and yyinvid_per = v_period
   ...
   for each tr_hist no-lock where /*ss2012-8-15 b*/ tr_hist.tr_domain = global_domain and /*ss2012-8-15 e*/ tr_type = "RCT-WO"
       ...
       if last-of(tr_lot) then do:
          find first wo_mstr where /*ss2012-8-15 b*/ wo_mstr.wo_domain = global_domain and /*ss2012-8-15 e*/ wo_nbr = tr_nbr and wo_lot = tr_lot
					no-lock no-error.
          create two.
	  assign two_nbr  = tr_nbr 
		 ...
		 two_part = tr_part
		 two_pl   = tr_prod_line.
		 two_qty  = (if wo_type = "E" and can-find(first wod_det where /*ss2012-8-15 b*/ wod_det.wod_domain = global_domain and /*ss2012-8-15 e*/
		                        wod_lot = wo_lot and wod_part = wo_part					
					) then 0 else v_qty).     /*Receipt Qty*/
       end.
   end.  /*for each tr_hist*/
   /* Add the work orders without receipt */
   for each wo_mstr no-lock where /*ss2012-8-15 b*/ wo_mstr.wo_domain = global_domain and /*ss2012-8-15 e*/ (wo_type = "C" or wo_type = "E")
	and wo_rel_date >= v_start and wo_rel_date <= v_end
	and wo_due_date >= v_start and wo_due_date <= v_end
	and wo_part >= part and wo_part <= part1:
	
	find first two where two_nbr = wo_nbr
		and two_lot = wo_lot
		and two_type = wo_type
		and two_part = wo_part no-lock no-error.
	if not avail two then do:
          create two.
	  assign two_nbr  = wo_nbr 
	         two_lot  = wo_lot
		 two_type = wo_type
		 two_part = wo_part.
	  for first pt_mstr where /*ss2012-8-15 b*/ pt_mstr.pt_domain = global_domain and /*ss2012-8-15 e*/ pt_part = wo_part no-lock:
		assign two_pl = pt_prod_line.
	  end.
	end.
...
procedure crt-rct-mstr:
	...
	if not avail yyinvi_mstr then do:
	   ...
	   */
	   find last tr_hist use-index tr_part_eff where /*ss2012-8-15 b*/ tr_hist.tr_domain = global_domain and /*ss2012-8-15 e*/ tr_part = two_part
			and tr_effdate >= v_start
			and tr_effdate <= v_end
			and tr_type = "RCT-WO" no-lock no-error.
	   if avail tr_hist then assign v_site = tr_site.
	   else assign v_site = "dcec-c".	
	   
	   find last yywobmspt_mstr where /*ss2012-8-15 b*/ yywobmspt_mstr.yywobmspt_domain = global_domain and /*ss2012-8-15 e*/ yywobmspt_site = v_site
		and yywobmspt_part = two_part 
		and ( year(yywobmspt_mod_date) < v_year
			or (year(yywobmspt_mod_date) = v_year 
	   ...
	   if not avail yywobmspt_mstr then do:
		   ...
		   yywobmspt_site = v_site and yywobmspt_part = two_part		   
		   no-lock no-error.
		   if not avail yywobmspt_mstr then do:
			find last yywobmspt_mstr where /*ss2012-8-15 b*/ yywobmspt_mstr.yywobmspt_domain = global_domain and /*ss2012-8-15 e*/ yywobmspt_part = two_part
				 no-lock no-error.
		   end.
	   end.
	   create yyinvi_mstr.
	...
	for each wod_det no-lock where /*ss2012-8-15 b*/ wod_det.wod_domain = global_domain and /*ss2012-8-15 e*/ wod_lot = two_lot:

            find first yyinvid_det where /*ss2012-8-15 b*/ yyinvid_det.yyinvid_domain = global_domain and /*ss2012-8-15 e*/ yyinvid_year = v_year
...
procedure crt-var-det:
   ...
   for each two :
           ...
           for each xxwobmvd_det no-lock where /*ss2012-8-15 b*/ xxwobmvd_det.xxwobmvd_domain = global_domain and /*ss2012-8-15 e*/ 
										 xxwobmvd_det.xxwobmvd_woid = two_lot:
               
	       find first pt_mstr where /*ss2012-8-15 b*/ pt_mstr.pt_domain = global_domain and /*ss2012-8-15 e*/ pt_part = xxwobmvd_det.xxwobmvd_comp 
								no-lock no-error.
	       if not avail pt_mstr then next.
	       find first ptp_det where /*ss2012-8-15 b*/ ptp_det.ptp_domain = global_domain and /*ss2012-8-15 e*/ ptp_part = xxwobmvd_det.xxwobmvd_comp 
								and ptp_site begins "dcec-" no-lock no-error.
	       if avail ptp_det then assign v_pmcode = ptp_pm_code.
	       else assign v_pmcode = pt_pm_code.

               find first yycsvard_det where /*ss2012-8-15 b*/ yycsvard_det.yycsvard_domain = global_domain and /*ss2012-8-15 e*/ yycsvard_year = v_year 
	       ...
	       find first wo_mstr where /*ss2012-8-15 b*/ wo_mstr.wo_domain = global_domain and /*ss2012-8-15 e*/ wo_lot = two_lot
								 no-lock no-error.
		  ...
		  /* 方法差异其他 */
		  if xxwobmvd_det.xxwobmvd_var_det[5] <> 0 then 
		     assign yycsvard_variance[8] = yycsvard_variance[8] + xxwobmvd_det.xxwobmvd_var_det[5].	  
	       end.  /*else do*/
   ...
   for each ttt no-lock:
       ...
       if not avail pt_mstr then next.
       find first ptp_det where /*ss2012-8-15 b*/ ptp_det.ptp_domain = global_domain and /*ss2012-8-15 e*/  ptp_part = ttt.ttt_comp and ptp_site begins "dcec-"
							no-lock no-error.
       if avail ptp_det then assign v_pmcode = ptp_pm_code.
       else assign v_pmcode = pt_pm_code.
       find first xxwobmvd_det  where /*ss2012-8-15 b*/ xxwobmvd_det.xxwobmvd_domain = global_domain and /*ss2012-8-15 b*/ xxwobmvd_det.xxwobmvd_woid = ttt.ttt_wolot 
...
procedure crt-var-mstr:
   ...
   for each two no-lock group by two_part:
        ...
        find first wo_mstr where /*ss2012-8-15 b*/ wo_mstr.wo_domain = global_domain and /*ss2012-8-15 e*/ wo_lot = two_lot no-lock no-error.
			find first xxwobmvm_mstr where /*ss2012-8-15 b*/ xxwobmvm_mstr.xxwobmvm_domain = global_domain and /*ss2012-8-15 e*/ xxwobmvm_woid = two_lot 
									no-error.
				if avail xxwobmvm_mstr then do:
					find first yycsvar_mstr where /*ss2012-8-15 b*/ yycsvar_mstr.yycsvar_domain = global_domain and /*ss2012-8-15 e*/ yycsvar_year = v_year
											and yycsvar_per = v_period and yycsvar_part = two_part no-error.
						if not avail yycsvar_mstr then do:
							  find first pt_mstr where /*ss2012-8-15 b*/ pt_mstr.pt_domain = global_domain and /*ss2012-8-15 e*/ pt_part = two_part 
												no-lock no-error.
		...
		if avail wo_mstr and wo_type = "E" then
			assign yycsvar_variance[2] = yycsvar_variance[2] + xxwobmvm_var_tot.
		else
			assign yycsvar_variance[8] = yycsvar_variance[8] + xxwobmvm_var_det[5].
	end.
	
       if last-of(two_part) then do:
	       for each yycsvard_det no-lock where /*ss2012-8-15 b*/ yycsvard_det.yycsvard_domain = global_domain and /*ss2012-8-15 e*/ yycsvard_year = v_year 
		    and yycsvard_per = v_period 
		    and yycsvard_part = two_part:
		   find first yycsvar_mstr where /*ss2012-8-15 b*/ yycsvar_mstr.yycsvar_domain = global_domain and /*ss2012-8-15 e*/ yycsvar_year = v_year
		   ...
		   if not avail yycsvar_mstr then do:
		      ...
		      if avail ptp_det then assign yycsvar_pm_code = ptp_pm_code.
	           ...
	           innerloop:		  
		   do i = 1 to 12:
		      if i = 8 OR i = 2 then next innerloop.
