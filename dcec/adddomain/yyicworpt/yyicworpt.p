...
/*	    tttt_bomchgvar = yycsvar_variance[1] */
	...
	release tttt.	
	/* 差异明细 */
	for each yycsvard_det no-lock where yycsvard_domain = global_domain 
	  and yycsvard_year = yycsvar_year
		and yycsvard_per = yycsvar_per
		and yycsvard_part = yycsvar_part
		by yycsvard_comp:
		find first pt_mstr where pt_domain = global_domain and pt_part = yycsvard_comp no-lock no-error.
