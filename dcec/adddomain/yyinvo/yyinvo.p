...
index ttinvo_part IS PRIMARY ttinvo_year ttinvo_per ttinvo_out_type ttinvo_part ttinvo_reason ttinvo_sojob.
...
form
  ...
  ttinvo_reason column-label "原因代码"
...
repeat:
	...
	for each tr_hist no-lock where 
		/*ss2012-8-16 b*/ tr_hist.tr_domain = global_domain and /*ss2012-8-16 e*/
		...
		if tr_type = "ISS-WO" then do:
			find first wo_mstr where /*ss2012-8-16 b*/ wo_mstr.wo_domain = global_domain and /*ss2012-8-16 e*/ wo_lot = tr_lot
								no-lock no-error.
			if avail wo_mstr then do:
				if wo_type = "E" then do:
					if tr_part = wo_part then next.
					v_type = "改制".
				end.
	...
	if lupdate then do:
		for each yyinvo_mstr where /*ss2012-8-16 b*/ yyinvo_mstr.yyinvo_domain = global_domain and /*ss2012-8-16 e*/ yyinvo_year = theyear
			and yyinvo_per = theper:
			delete yyinvo_mstr.
		end.
		for each ttinvo_mstr no-lock:	
			find first yyinvo_mstr where /*ss2012-8-16 b*/ yyinvo_mstr.yyinvo_domain = global_domain and /*ss2012-8-16 e*/ yyinvo_year = ttinvo_year
				and yyinvo_per = ttinvo_per
				and yyinvo_out_type = ttinvo_out_type 
				and yyinvo_part = ttinvo_part 
				and yyinvo_reason = ttinvo_reason
				and yyinvo_sojob = ttinvo_sojob no-error.
