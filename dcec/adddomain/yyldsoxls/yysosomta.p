/* GUI CONVERTED from sosomta.p (converter v1.78) Mon Mar 28 21:58:01 2011 */
...
/* Revision: 1.105.1.7    BY: Tejasvi Kulkarni    DATE: 07/14/06  ECO: *P4MJ* */
/* Revision: 1.105.1.8    BY: Anuradha K.         DATE: 04/02/07  ECO: *P5S6* */
/* Revision: 1.105.1.9    BY: Abhijit Gupta       DATE: 07/23/07  ECO: *P62G* */
/* Revision: 1.105.1.11   BY: Sanat Paul          DATE: 08/06/07  ECO: *P63B* */
/* Revision: 1.105.1.12   BY: Prajakta Patil      DATE: 11/12/07  ECO: *P6DD* */
/* Revision: 1.105.1.13   BY: Dilip Manawat       DATE: 11/20/07  ECO: *Q185* */
/* Revision: 1.105.1.23   BY: Sandeep Rohila      DATE: 11/30/09  ECO: *Q3M0* */
/* Revision: 1.105.1.24   BY: Rajat Kulshreshtha  DATE: 03/05/10  ECO: *Q3WM* */
/* Revision: 1.105.1.26   BY: Laxmikant Bondre    DATE: 03/24/10  ECO: *Q3W5* */
/* Revision: 1.105.1.27   BY: Trupti Khairnar     DATE: 07/16/10  ECO: *Q482* */
...
/* *SS-20120927.1*    {fsmnp01.i ""sosomt.p"" 20 solinerun}  */
/* *SS-20120927.1*  */    /*tfq*/  {fsmnp01.i ""yysosomt.p"" 20 solinerun} 
end.
    ...
    /* editing */
      ...
      if not available sod_det then do:
         ...
         display
            ...
            desc1 /* *SS-20120927.1*   sod_std_cost base_curr */
            sod_pricing_dt
...
/* *SS-20120927.1* -b */
/*cj*add*beg*/
            FIND FIRST pts_det NO-LOCK WHERE pts_det.pts_domain = global_domain and pts_part = INPUT sod_det.sod_part NO-ERROR .
            IF AVAILABLE pts_det THEN ptssub = pts_sub .
            FIND pt_mstr NO-LOCK WHERE pt_mstr.pt_domain = global_domain and pt_part = INPUT sod_det.sod_part NO-ERROR .
...
/*GUI*/ if global-beam-me-up then undo, leave.
            ...
            /* TAKES CARE OF DELAY OF COUPLE OF SECONDS IN .NET UI FOR MEMO ITEMS */
            if {gpiswrap.i} then pause 0.
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         if available pt_mstr then do:
            ...
            /* FOR SO'S, DEFAULT THE in_loc IF in_mstr RECORD */
            /* IS AVAILABLE ELSE DEFAULT pt_mstr LOCATION.    */
...
/*GUI*/ if global-beam-me-up then undo, leave.
      ...
      /* DELETE INFO IN REMOTE DB'S IF LINE IS CONFIRMED */
      if sod_det.sod_confirm
      then do:

         /* LINE SITE CHANGED */
         if     not new sod_det
            and del-yn    = no
            and sod_det.sod_site <> prev_site
         then do:
