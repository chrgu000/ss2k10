/* GUI CONVERTED from sosomtf8.p (converter v1.78) Wed Aug  8 02:44:42 2007 */
...
/* Revision: 1.41       BY: Paul Donnelly (SB)   DATE: 06/28/03  ECO: *Q00P*  */
/* Revision: 1.43       BY: Jean Miller          DATE: 02/23/04  ECO: *Q063*  */
/* Revision: 1.44       BY: Preeti Sattur        DATE: 03/18/04  ECO: *P1T4*  */
/* Revision: 1.45       BY: Alka Duseja          DATE: 06/02/04  ECO: *P24L*  */
/* Revision: 1.46       BY: Tejasvi Kulkarni     DATE: 02/01/05  ECO: *P366*  */
/* Revision: 1.46.1.1   BY: Suyash Keny          DATE: 01/29/07  ECO: *P5MW* */
/* $Revision: 1.46.1.3 $  BY: Ambrose Almeida      DATE: 07/16/07  ECO: *P628* */
...
/* *SS-20120927.1*  {solinfrm.i}  /* DEFINE SHARED FORMS FOR LINE DETAIL */   */
...
fields( pic_domain pic_so_fact pic_so_rfact)
 where pic_ctrl.pic_domain = global_domain no-lock: end.
...
for first so_mstr
fields( so_domain so_curr so_cust so_ex_rate so_ex_rate2 so_nbr)
...
for first pt_mstr
fields(pt_domain pt_abc pt_avg_int   pt_bom_code  pt_cfg_type pt_cyc_int
...
for first ptp_det
fields(ptp_domain ptp_bom_code   ptp_cfg_type ptp_ins_lead ptp_ins_rqd
    ...
    where ptp_det.ptp_domain = global_domain and  ptp_part = sod_part
...
then do:
   ...
   for first si_mstr
      ...
      si_git_sub
       ...
       where si_mstr.si_domain = global_domain and  si_site = prev_site
      no-lock:
...
/*GUI*/ if global-beam-me-up then undo, leave.
 /* FOR EACH sob_det */
      ...
      *******************************************/
      ...
      for first sob_det
      fields( sob_domain sob_feature sob_line   sob_nbr  sob_parent sob_part
      ...
      then do:
        ...
        with frame bom1.
         ...
         then do:
            ...
            absloop:
            for each abs_mstr
                where abs_mstr.abs_domain = global_domain and  abs_shipfrom =
                sod_site
...
/*GUI*/ if global-beam-me-up then undo, leave.
               ...
               if par_absid <> ? then do:
                  ...
                  for first abs_tmp
                  fields( abs_domain abs_canceled abs_dataset abs_id
                  abs_line
                         abs_order    abs_par_id  abs_shipfrom abs_status)
                      where abs_tmp.abs_domain = global_domain and
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   then do:
      for first sob_det
         ...
         and   sob_nbr     = sod_nbr
         and   sob_line    = sod_line
         and   sob_feature = ps_ref
      no-lock:
      end. /* FOR FIRST sob_det */
      if not available sob_det
      then do:
         /* No selections made for Mandatory Feature */
         {pxmsg.i &MSGNUM=635 &ERRORLEVEL=2 &MSGARG1=ps_ref}

         if not batchrun
         then
            pause.

      end. /* IF NOT AVAILABLE sob_det */
   end.
/*GUI*/ if global-beam-me-up then undo, leave.
 /* IF FIRST-OF(ps_par) */
