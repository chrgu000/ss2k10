...
for each acd_det no-lock where  /* *SS-20120814.1*   */ acd_det.acd_domain = global_domain and  acd_entity <= entity1 and acd_entity>=entity and acd_acc>=acc and acd_acc<=acc1 and acd_project>=project and acd_project<=project1 
...
for each acd_det where /* *SS-20120814.1*   */ acd_det.acd_domain = global_domain and acd_entity <= entity1 and acd_entity>=entity and acd_acc>=acc and acd_acc<=acc1 and acd_project>=project and acd_project<=project1 
and acd_per >= month(effdate) and acd_per <= month(effdate1) break by acd_acc by acd_project :
 if last-of(acd_project) then do:
   ...
   if not available accpj_sum then do:
      ...
      accpj_amt = 0.
      accpj_curramt = 0.
      find first pj_mstr where /* *SS-20120814.1*   */ pj_mstr.pj_domain = global_domain and  trim(pj_project) = trim(accpj_project) no-lock no-error.
      if available pj_mstr then
        accpj_pjdesc = pj_desc.
      else
        accpj_pjdesc="".
      find ac_mstr where /* *SS-20120814.1*   */ ac_mstr.ac_domain = global_domain and  ac_code = accpj_acc no-lock no-error.
...
for each accpj_sum break by accpj_acc:
   ...
   find first gltr_hist where /* *SS-20120814.1*   */ gltr_hist.gltr_domain = global_domain and gltr_acc = accpj_acc and gltr_project = accpj_project  and gltr_eff_dt >= effdate and gltr_eff_dt <= effdate1 /*cj*/ USE-INDEX gltr_project no-lock no-error.
   if available(gltr_hist) then do:
      find pj_mstr where  /* *SS-20120814.1*   */ pj_mstr.pj_domain = global_domain and  pj_project = accpj_project no-lock no-error.
      if available pj_mstr then
         pjdesc = pj_desc.
      else
         pjdesc="".
...
/*kangjian v4      disp acccode accname pjcode pjdesc startbal startcurrbal with stream-io frame phead1 side-labels width 132. kangjian v4*/
      ...
      for each gltr_hist no-lock where /* *SS-20120814.1*   */ gltr_hist.gltr_domain = global_domain and gltr_acc = accpj_acc and gltr_project = accpj_project and gltr_eff_dt >= effdate and gltr_eff_dt <= effdate1 /*cj*/ USE-INDEX gltr_project by gltr_acc by gltr_project:
