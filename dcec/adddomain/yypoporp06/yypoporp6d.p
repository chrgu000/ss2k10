...
for each prh_hist where prh_domain = global_domain
      ...
      and (can-find (first pod_det where pod_domain = global_domain and
                pod_nbr      = prh_nbr            and
      ...
      and (uninv_only = no  /* Non-Vouchered Receipts Only = NO */
           ...
           (uninv_only   and  /* Non-Vouchered Receipts Only = YES */
              not can-find (first pvo_mstr where pvo_domain = global_domain and
                  pvo_lc_charge         = ""                 and
   ...
   if first-of(prh_nbr) then do:
      ...
      for first po_mstr
         ...
         where po_domain = global_domain and po_nbr = prh_nbr
         no-lock:
   ...
   for first pvo_mstr
      ...
      if uninv_only = yes then
         ...
         for each vph_hist where vph_domain = global_domain and
   ...
   for each pvo_mstr
      fields (pvo_domain pvo_ex_rate  pvo_ex_rate2   pvo_internal_ref_type
      ...
      where pvo_domain = global_domain and pvo_internal_ref = prh_receiver
      and   pvo_line         = prh_line
   no-lock:
   ...
   for first pj_mstr
      fields (pj_domain pj_project pj_desc) no-lock
      where pj_domain = global_domain and pj_project = po_project:
   end.
