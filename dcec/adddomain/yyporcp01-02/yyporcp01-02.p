...
*/
	...
	for each zzwk:
		...
		and prh_rcp_date >= zzfrom and prh_rcp_date <= zzto
...
/*
	...
	/*计划量*/
	for each zzwk,
	each scx_ref where scx_domain = global_domain and
		   scx_part >= part and scx_part <= part1 and scx_type = 2 
	no-lock,
	each po_mstr no-lock where po_domain = global_domain and po_nbr = scx_po
   and po_vend >= vend and po_vend <= vend1
	:

		find last sch_mstr where sch_domain = global_domain and sch_type = 4
        ...
        where schd_domain = global_domain and schd_type = sch_type
	...
	FOR EACH zzwk  BREAK BY zzto.
      ...
      FOR EACH xxwk WHERE xxmonth = string(year(zzto)) + "." + string(month(zzto)).
		  ...
		  find pt_mstr no-lock where pt_domain = global_domain and pt_part = xxpart no-error.
		  ...
		  find first vd_mstr no-lock where vd_domain = global_domain and vd_addr = xxvend no-error.
