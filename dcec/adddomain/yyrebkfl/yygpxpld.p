...
/*************tfq added begin********************/
for each xxpk_det where pk_domain = global_domain and xxpk_user = mfguser:
delete xxpk_det .
end.
/*************tfq added end***************/
...
/*tfq*/                   pk_reference = string(ps_op) 
                   ...
                   if ps_end = ? then pk_end = effend.
                    ...
                    /*************tfq added end*********************/
                   create pkwkfl. /* pkwkfl.pk_domain = global_domain. */
 ...
 *J31Z** END DELETE SECTION **/
          ...
          where ps_domain = global_domain and ps_par = xpldcomp no-lock no-error.
           ...
           find next ps_mstr use-index ps_parcomp where ps_domain = global_domain and ps_par = xpldcomp
           no-lock no-error.
        end.
     end. /*repeat*/
/********tfq added begin********************
