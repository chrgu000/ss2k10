...
/*************tfq added end***************/
/*N0CD**   find first ps_mstr use-index ps_parcomp where ps_par = xpldcomp */
...
/*N0CD*/    where /*ss2012-8-21 b*/ ps_domain = global_domain and /*ss2012-8-21 e*/ ps_par = xpldcomp
...
/*N0CD*/     assign
          ...
          where /*ss2012-8-21 b*/ ps_domain = global_domain and /*ss2012-8-21 e*/ ps_par = xpldcomp no-lock no-error.
...
/*G1DF*              and ps_qty_per <> 0                               */
          ...
          then do:
             find first pk_det exclusive-lock
                where /*ss2012-8-21 b*/ pk_domain = global_domain and /*ss2012-8-21 e*/ pk_user      = mfguser
                and   pk_part      = ps_comp
 ...
 *J31Z** END DELETE SECTION **/
          ...
          where /*ss2012-8-21 b*/ ps_domain = global_domain and /*ss2012-8-21 e*/ ps_par = xpldcomp  no-lock no-error.
           ...
           find next ps_mstr use-index ps_parcomp where /*ss2012-8-21 b*/ ps_domain = global_domain and /*ss2012-8-21 e*/ ps_par = xpldcomp
           no-lock no-error.
        end.
     end. /*repeat*/
/********tfq added begin********************
for each pk_det where pk_user = mfguser :
find first xxpk_det where xxpk_user = mfguser and xxpk_part = pk_part and string(xxpk_op) <> pk_reference no-error .
