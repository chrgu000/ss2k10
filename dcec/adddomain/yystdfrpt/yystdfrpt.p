...
for each ld_det,each pt_mstr where ld_qty_frz <> 0 and pt_part = ld_part
...
find in_mstr where in_part = ld_part and in_site = pt_site no-lock no-error.
...
for each tag_mstr no-lock where tag_part = ld_part break by tag_part:
