...
IF v_sum = NO THEN DO:
 ...
 FOR EACH b_req_mstr NO-LOCK WHERE b_req_domain = global_domain and b_req_due_date >= v_eff_date1 AND b_req_due_date <= v_eff_date2 AND b_req_site >= v_site1 AND b_req_site <= v_site2.
    FIND FIRST ptp_det WHERE ptp_domain = global_domain and ptp_part = b_req_part AND ptp_site = b_req_site NO-LOCK NO-ERROR.
