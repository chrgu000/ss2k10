...
/*G1YQ*/      bom_code = ""
            ...
            FIND FIRST pt_mstr WHERE /* *SS-20120816.1*   */ pt_mstr.pt_domain = global_domain and pt_part = inp-part NO-LOCK NO-ERROR.
...
/*N027*/    /* COMBINED FOLLOWING ASSIGNMENTS */
            find ptp_det no-lock where /* *SS-20120816.1*   */ ptp_det.ptp_domain = global_domain and ptp_part = parent AND ptp_site = site no-error.
...
find pt_mstr no-lock where /* *SS-20120816.1*   */ pt_mstr.pt_domain = global_domain and pt_part = comp no-error.
...
open query q_ps_mstr for each ps_mstr use-index ps_parcomp
     where /* *SS-20120816.1*   */ ps_mstr.ps_domain = global_domain and ps_par = comp and ps_ps_code <> "J" no-lock.
...
repeat while avail ps_mstr:
  ...
  if eff_date = ? or (eff_date <> ? and
   (ps_start = ? or ps_start <= eff_date)
    ...
    find ptp_det no-lock where /* *SS-20120816.1*   */ ptp_det.ptp_domain = global_domain and ptp_part = ps_comp
