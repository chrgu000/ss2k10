...
REPEAT:
    ...
    UPDATE 
        v_site VALIDATE(CAN-FIND(FIRST si_mstr WHERE  /* *SS-20120816.1*   */ si_mstr.si_domain = global_domain and si_site = v_site), "Please input site data...")
        v_part1 v_part2
        v_line1 v_line2
        v_type1 v_type2
        v_group1 v_group2
...
PROCEDURE xxpro-bud-ttx9:
    ...
    FOR EACH pt_mstr NO-LOCK 
        WHERE  /* *SS-20120816.1*   */ pt_mstr.pt_domain = global_domain 
	and pt_part >= v_part1 AND pt_part <= v_part2
        ...
        v_pm_code = "".
        /*check in_mstr*/
        FIND FIRST IN_mstr WHERE /* *SS-20120816.1*  */ IN_mstr.in_domain = global_domain and IN_part = pt_part AND IN_site = v_site NO-LOCK NO-ERROR.
        IF NOT AVAILABLE IN_mstr THEN NEXT.
        FIND FIRST ptp_det WHERE /* *SS-20120816.1*  */ ptp_det.ptp_domain = global_domain and ptp_part = pt_part AND ptp_site = v_site NO-LOCK NO-ERROR.
...
PROCEDURE xxpro-report:
    ...
    FOR EACH ttx9 WHERE (ttx9_chk_bom = YES OR ttx9_chk_cwo = YES):
        FIND FIRST pt_mstr WHERE  /* *SS-20120816.1*   */ pt_mstr.pt_domain = global_domain and pt_part = ttx9_part NO-LOCK NO-ERROR.
