	   ...
	   for each workorder where woor_qty>=0  :
		  ...
		  display wo_nbr wo_lot wo_qty_ord wo_ord_date wo_part      
		      ...
		      wo_status wo_rmks with frame c side-labels  .
                display "        零件号            零件名称                        需求量      库位      保管员         实际领料量 " with  WIDTH 200  frame bt1.
  	         display "        -----------------------------------------------------------------------------------------------------" with width 200 frame bt3.  	     
                line = line + 8.
	       end.
          /* MESSAGE site woor_part "BBB".
	       ...
	       if page-size - line-counter < 2 then do:
                 display "        -----------------------------------------------------------------------------------------------------" AT 1  with width 200 frame ww.  
                 display "        保管员：           库房经理：           发运人：                 接收人：" AT 1 with width 200  frame ww.  	     
	          page.
	          pageno = pageno + 1.
	          line = 1.
	       end.       
           ...
           display "        ------------------------------------------------------------------------------------------------------" AT 1 with width 200 no-labels frame ww1.  
           display "        保管员：           库房经理：           发运人：                 接收人：" AT 1 with width 200 no-labels frame ww1.  	     
/* 显示回收物料清单 */
           line = 1 .
	    page.
	    pageno = pageno + 1.
		  ...
		  desc1 = "".
		   ...
		   display wo_nbr wo_lot wo_qty_ord wo_ord_date wo_part wo_qty_comp wo_rel_date  wo_qty_rjct wo_due_date
  	         ...
  	         display "        -----------------------------------------------------------------------------------------------------"  AT 1 with width 200 no-labels frame bt4.  	     
  	         line = line + 8.
  	         nextp=nextp + 1.
	       end.
	       ...
	       down 1.
  	       line = line + 1.
	       if page-size - line-counter < 2  then do:
  	          display "        -----------------------------------------------------------------------------------------------------" AT 1 with width 200 no-labels frame ww2.  
         	   display "        保管员：           库房经理：           发运人：                 接收人：" AT 1 with width 200 no-labels frame ww2.  	     	     
	          page.
	          pageno = pageno + 1.
	          line =  1.
	       end.	       
	    ...
	    if nextp>=1 then do:
  	    display "        --------------------------------------------------------------------------------"  AT 1 with width 200 no-labels frame ww3.  
           display "        保管员：              库房经理：                         发运人：                           接收人："  AT 1  with width 200 no-labels frame ww3.  
           end.	     
