...
REPEAT:
    ...
    FOR EACH tr_hist 
        FIELDS (tr_domain tr_trnbr tr_part tr_site tr_effdate tr_type tr_lot tr_nbr tr_date) 
        NO-LOCK
        WHERE tr_domain = global_domain 
        AND  (tr_date >= v_date1 AND tr_date <= v_date2)
        ...
        if (tr_hist.tr_type = "ISS-WO" or tr_hist.tr_type = "RCT-WO") and (tr_hist.tr_lot <> "") and (tr_hist.tr_nbr = "") then do:
            find first xxtr_hist where xxtr_domain = global_domain and xxtr_trnbr = tr_hist.tr_trnbr use-index xxtr_idx0 no-lock no-error.
            if not available xxtr_hist then do:
                i = i + 1.
                create xxtr_hist. xxtr_domain = global_domain.
