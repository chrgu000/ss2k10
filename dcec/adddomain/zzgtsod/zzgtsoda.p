...
for each so_mstr
	...
	/*check bill to*/
	find first ad_mstr where /*ss2012-8-16 b*/ ad_mstr.ad_domain = global_domain and /*ss2012-8-16 e*/ ad_addr = so_bill no-lock no-error.
	...
	/*check gold-tax info.*/
	find first ad_mstr where /*ss2012-8-16 b*/ ad_mstr.ad_domain = global_domain and /*ss2012-8-16 e*/ ad_type = "c/s_bank" and ad__chr01 = so_bill
						no-lock no-error.
	if not available ad_mstr then do:
		wkgtm_status = "X".
	...
	/*check sod_det*/
	find first sod_det where sod_nbr = so_nbr and sod_qty_inv <> 0 no-lock no-error.
	...
	/*check trl*/
	  if so_trl2_amt <> 0 then do:
	    find first trl_mstr 
	         where /*ss2012-8-16 b*/ trl_mstr.tr_domain = global_domain and /*ss2012-8-16 e*/ 
		 trl_code = so_trl2_cd
		 no-lock no-error.
	...
	for each sod_det where /*ss2012-8-16 b*/ sod_det.sod_domain = global_domain and /*ss2012-8-16 e*/
	         sod_nbr = so_nbr 
		and sod_qty_inv <> 0 
...
*/ 
	...
	for each sodet where not sodet.sod__log01 break by sodet.sod_line:
	    ...
	    for each sodet2 where not sodet2.sod__log01 no-lock break by sodet2.sod_line:
            ...
            FIND cp_mstr WHERE /*ss2012-8-16 b*/ cp_mstr.cp_domain = global_domain and /*ss2012-8-16 e*/ cp_part =pt_part AND cp_cust = v_cust3
						NO-LOCK NO-ERROR.
			...
			*/		
			if sodet2.sod_taxable then do:
				find first vt_mstr
				   where /*ss2012-8-16 b*/ vt_mstr.vt_domain = global_domain and /*ss2012-8-16 e*/
				   vt_class = sodet2.sod_taxc
				   no-lock no-error.
			...
			*/		
			...
			where /*ss2012-8-16 b*/ usrw_wkfl.usrw_domain = global_domain and /*ss2012-8-16 e*/
			usrw_key1 = "GOLD-TAX-SO"
			and usrw_key2 = wkgtm_ref 
			no-error.
