...
/*J034*/          end.               
    ...
    for each pt_mstr where (pt_part >= part and pt_part <= part1) and
                           (pt_prod_line >= line and pt_prod_line <= line1) and
                           (pt_part_type >= type and pt_part_type <= type1) and
                           (pt_loc >= loc and pt_loc <= loc1) and
                       (pt_article >= keeper and pt_article <= keeper1) and
       ...
       bgqty = edqty - inqty + outqty.
        ...
        if (yn_zero and (edqty <> 0 or bgqty <> 0 or rctpo <> 0 or rcttr <> 0 or rctunp <> 0 or rctwo <> 0
          ...
          or isswo <> 0 or invadj <> 0 or oth <> 0)) 
           ...
           disp pt_part pt_desc2 pt_prod_line pt_abc pt_article label "保管员"
              pt_loc label "缺省库位" bgqty label "期初库存"
