...
/* $Revision: 1.37.1.1 $   BY: Shivanand H             DATE: 02/24/04  ECO: *P1QK* */
...
age_by = getTermLabel("DUE_DATE", 3).
/* Must be DUE, EFF, or INV */
{pxmsg.i &MSGNUM=3719 &ERRORLEVEL=3 &MSGBUFFER=l_msg}
...
/* judy 05/06/29*/              "生效日期"
/* judy 05/06/29*/              "支付方式"
/* judy 05/06/29*/              age_range[1]       space (4)
/* judy 05/06/29*/             age_range[2]       space (1)
/* judy 05/06/29*/             age_range[3]       space (1)
/* judy 05/06/29*/              age_range[4]    skip
/* judy 05/06/29*/             "发票        "
/* judy 05/06/29*/              "到期日期"
/* judy 05/06/29*/             "兑换率  "
/* judy 05/06/29*/             "货币         "
...
/* judy 05/06/29*/ /* mc-curr-label et_report_curr skip
/* judy 05/06/29*/     mc-exch-label mc-exch-line1 skip
/* judy 05/06/29*/      mc-exch-line2 at 23 skip(1)
/* judy 05/06/29*/     (getTermLabel("VOUCHER",12))       format "x(12)"
/* judy 05/06/29*/     (getTermLabel("INVOICE_DATE",8))   format "x(8)"
/* judy 05/06/29*/     (getTermLabel("EFFECTIVE_DATE",8)) format "x(8)"
/* judy 05/06/29*/     (getTermLabel("CREDIT_TERMS",8))   format "x(8)"
/* judy 05/06/29*/     age_range[1]       space (4)
/* judy 05/06/29*/     age_range[2]       space (1)
/* judy 05/06/29*/      age_range[3]       space (1)
/* judy 05/06/29*/     age_range[4]    skip(0)
/* judy 05/06/29*/     (getTermLabel("INVOICE",12))          format "x(12)"
/* judy 05/06/29*/      (getTermLabel("DUE_DATE",8))          format "x(8)"
/* judy 05/06/29*/      (getTermLabel("EXCHANGE_RATE",8))     format "x(8)"
/* judy 05/06/29*/      (getTermLabel("CURRENCY",8))          format "x(8)"
/* judy 05/06/29*/      (getTermLabelCentered("DAYS_OLD",17)) format "x(17)"
/* judy 05/06/29*/      (getTermLabelCentered("DAYS_OLD",17)) format "x(17)"
/* judy 05/06/29*/     (getTermLabelCentered("DAYS_OLD",17)) format "x(17)"
/* judy 05/06/29*/      (getTermLabelCentered("DAYS_OLD",17)) format "x(17)"
/* judy 05/06/29*/     (getTermLabelRt("TOTAL_AMOUNT",17))   format "x(17)"
/* judy 05/06/29*/    skip
...
/* judy 05/06/29*/      "-----------------"
/* judy 05/06/29*/      "-----------------" skip
/* judy 05/06/29*/   with frame phead1 width 132 page-top.*/
 ...
 form
      ...
      at 1
/* judy 05/06/29*/     /* space(5)*/   
/* judy 05/06/29*/       invdate AT 16 FORMAT "99/99/99"
/* judy 05/06/29*/       effdate AT 25   FORMAT "99/99/99"
/* judy 05/06/29*/       vo_mstr.vo_cr_terms  AT 34 FORMAT "x(8)"
/* judy 05/06/29*/       et_age_amt[1]  AT 49  format "->>>>>,>>>,>>9.99"
      et_age_amt[2]       format "->>>>>,>>>,>>9.99"
      et_age_amt[3]       format "->>>>>,>>>,>>9.99"
      et_age_amt[4]       format "->>>>>,>>>,>>9.99"
      et_base_amt         format "->>>>>,>>>,>>9.99"   
   with frame c width 140 no-labels no-attr-space no-box down.
   {&APVORP04-P-TAG45}
   {&APVORP04-P-TAG7}
...
/* judy 05/06/29*/       et_age_amt[1] AT 49  format "->>>>>,>>>,>>9.99"
      ...
      et_age_amt[4]       format "->>>>>,>>>,>>9.99"
      et_base_amt         format "->>>>>,>>>,>>9.99"
   with frame e width 140  no-labels no-attr-space no-box down. 

   {&APVORP04-P-TAG47}
   for each vd_mstr fields (vd_addr vd_sort vd_type)
         where (vd_addr >= vend and vd_addr <= vend1)
      ...
      for first ad_mstr
         fields (ad_addr ad_attn ad_ext ad_name ad_phone)
...
/*judy 05/08/16*/   and (ap_acct >= acct and ap_acct <= acct1)              
         ...
         for each vo_mstr
               fields (vo_applied  vo_confirmed vo_cr_terms vo_due_date
               vo_hold_amt vo_invoice   vo_ref      vo_type)
               where vo_ref = ap_ref and vo_confirmed = yes
            ...
            for each ckd_det
                  fields (ckd_amt ckd_cur_amt ckd_cur_disc ckd_disc
                  ckd_ref ckd_voucher)
                  where ckd_voucher = ap_mstr.ap_ref no-lock,
                  each ck_mstr
                  fields (ck_curr ck_ref ck_voideff)
                  where ck_ref      = ckd_ref and
                  (ck_voideff = ? or
                  ck_voideff  > vodate1)
               no-lock:

               for first apmaster
                  fields (ap_amt    ap_curr   ap_date    ap_effdate
                  ap_entity           ap_ex_rate ap_ref
                  ap_base_amt ap_exru_seq ap_ex_rate2
                  ap_ex_ratetype ap_type   ap_vend)
                  where ap_ref = ckd_ref
                  and ap_type = "CK"
               no-lock:
               end.
            ...
            for first ct_mstr
               fields (ct_code ct_dating ct_due_date ct_due_days
               ct_from_inv)
               where ct_code = vo_cr_terms
            no-lock:
            end.
         ...
         do:
               ...
               for each ctd_det
                     fields (ctd_code ctd_date_cd ctd_pct_due)
                     where ctd_code = vo_cr_terms no-lock
                     break by ctd_code:
            ...
            run ip-disp-voucher.
            {&APVORP04-P-TAG52}
/*judy 07/05/05*/            /*{mfrpchk.i}  */
/*judy 07/05/05*/            {mfguichk.i} 
...
/*judy 05/08/16*/  if acct1 = hi_char then acct1 = "".
   {&APVORP04-P-TAG61}
   do i = 1 to 4:
...
PROCEDURE ip-param-quoter:
   ...
   {mfquoter.i votype         }
   {mfquoter.i votype1        }
/*judy 05/08/16*/  {mfquoter.i acct       }
/*judy 05/08/16*/  {mfquoter.i acct1      }
   {mfquoter.i vodate1        }
   {&APVORP04-P-TAG28}
   {mfquoter.i age_by         }
   {&APVORP04-P-TAG29}
...
/*judy 05/08/16*/  if acct1 = "" THEN acct1 = hi_char.
   {&APVORP04-P-TAG64}
   ...
   if net <> 0 then do:
      ...
      display
         ...
         et_base_amt 
          hold   
      with frame c.
/* judy 05/06/29*/      /*down 1 with frame c . */  
