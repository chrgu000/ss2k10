...
REPEAT:
   ...
   IF yn = NO THEN DO:
            ...
            FOR EACH tr_hist where (tr_date >= flushdate and tr_date <= flushdate1) 
                ...
                AND tr_part >= model   AND tr_part <= model1 AND tr_prod_line >= "7000" AND tr_prod_line <= "8ZZZ"
                and tr_type ="rct-wo" and tr_site = site AND (tr_userid ="MRP" OR tr_userid ="Admin") NO-LOCK break by tr_part.
   ...
   IF yn = YES THEN DO:
            ...
            FOR EACH tr_hist  where (tr_date >= flushdate and tr_date <= flushdate1) 
                and (tr_effdate >= effdate and tr_effdate <= effdate1) 
                AND tr_part >= model   AND tr_part <= model1 AND tr_prod_line >= "7000" AND tr_prod_line <= "8ZZZ"
                and tr_type ="rct-wo"  and tr_site = site AND (tr_userid ="MRP" OR tr_userid ="Admin") NO-LOCK BY tr_part.
                bkall = bkall + tr_qty_loc.
                FIND FIRST xxrps WHERE xxrps_part = tr_part NO-LOCK NO-ERROR.
                FIND FIRST pt_mstr WHERE pt_part = tr_part NO-LOCK NO-ERROR.
                IF AVAIL xxrps THEN DO:
