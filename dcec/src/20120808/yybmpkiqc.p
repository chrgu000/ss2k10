       ...
       field pkbombatch like bom_batch
        ...
        for each ps_mstr where ps_par = part and (ps__chr01 = site1 or ps__chr01 = "") 
        and (ps_start <= eff_date1 or ps_start = ?) and (ps_end >= eff_date1 or ps_end = ?) no-lock  :
        find first pkdet where pkpart = ps_comp no-error .
