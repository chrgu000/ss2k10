...
function get_acct returns char (input param1 as char, input param2 as char,input param3 as char,input param4 as char):
	...
	if param1 = "wo_account" or param1 = "lbrmfg_account" then do:
		find first code_mstr where code_fldname	= param1 and param2 = code_value no-lock no-error.
		...
		vflag = false.
		for each code_mstr no-lock where code_fldname = "so_export" :
		...
		if vflag then do:
			...
			find first code_mstr where code_fldname	= param1 and ("D" + param2) = code_value no-lock no-error.
			if not avail code_mstr then do:
		...
		/* 6B or 7B */
		find first code_mstr where code_fldname	= "so_job1" and code_value = param2 no-lock no-error.
		if avail code_mstr then do:
			if num-entries(code_cmmt,"-") > 2 then 
			...
			find first code_mstr where code_fldname	= "so_job1" and code_value = "other" no-lock no-error.
