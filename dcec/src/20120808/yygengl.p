...
define temp-table tt
   ...
   fields tt_rate	like yyvarated_mtl_rate
...
form
   ...
   tt_qty	column-label "出库数量"
   tt_rate	column-label "分摊率"
   tt_amt	column-label "结转金额" 
   " "
...
form
   ...
   tt2_amt	column-label "贷方金额" 
   " "
   tt2_cr_acct1	column-label "借方"
   tt2_cr_amt1	column-label "借方金额"
   " "
   tt2_cr_acct2	column-label "借方"
   tt2_cr_amt2	column-label "借方金额"
...
form
  ...
  ttglt_ctrl_amt column-label "金额"
with frame f width 320.

function gen_lbrmfg returns logical (input param1 as logical,input param2 as logical) FORWARD.

function gen_gl_fun returns logical (input param1 as char,input param2 as logical) FORWARD.
...
/*GUI*/ RECT-FRAME:HEIGHT-PIXELS in frame a = FRAME a:HEIGHT-PIXELS - RECT-FRAME:Y in frame a - 2.
    ...
    /* 创建要结转的记录 */
    gen_gl_fun("装机",v_wo).
    gen_lbrmfg(v_lbrmfg,true).
    gen_lbrmfg(v_lbrmfg,false).
    gen_gl_fun("改制",v_wo2).
    gen_gl_fun("销售",v_so).
    gen_gl_fun("计划外",v_other).
...
gen_lbrmfg()                                      人工、费用结转
****************************************************************/
function gen_lbrmfg returns logical (input param1 as logical,input param2 as logical):
	def var v_desc as char.
	def var v_flag as logical.
	if param2 then v_desc=  "人工差异". 
	else v_desc= "制造费用差异".
	for each yylbrmfg_det no-lock where yylbrmfg_year = iyear
		...
		assign 
		   ...
		   tt2_dr_acct = get_acct("lbrmfg_account",yylbrmfg_cc,"","").
		tt2_cr_acct1 = get_acct("lbrmfg_account",yylbrmfg_prod_line,"","").
		tt2_cr_amt1 = (if param2 then yylbrmfg_lbr_std else yylbrmfg_mfg_std).
		tt2_cr_acct2 = get_acct("wo_account",yylbrmfg_prod_line,"","").
		tt2_cr_amt2 = (if param2 then yylbrmfg_lbr_tl else yylbrmfg_mfg_std).
		/* 合并 */
	...
	for each tt where tt2_dr_acct = "Account Error." or tt2_cr_acct1 = "Account Error." 
...
****************************************************************/
function gen_gl_fun returns logical (input param1 as char,input param2 as logical):
    def var v_flag as logical.
    for each yyinvo_mstr no-lock where yyinvo_year = iyear
	and yyinvo_per = iper 
	and yyinvo_out_type begins param1 :
	
	find first yyvarated_det where yyvarated_year = iyear
		and yyvarated_per = iper
		and yyvarated_part = yyinvo_part no-lock no-error.
	if not avail yyvarated_det then do:
		if can-find(yycsvar_mstr where yycsvar_year = iyear and yycsvar_per = iper 
		...
		assign 
		   ...
		   tt_sojob = yyinvo_reason
		   tt_qty = yyinvo_qty
		   tt_rate = yyvarated_mtl_rate
		   tt_amt = yyinvo_qty * yyvarated_mtl_rate.
	end.
	tt_dr_acct = get_acct("wo_account",yyinvo_part_pl,"",""). 

	if param1 = "装机" or param1 = "改制" then do:
		find first pt_mstr where pt_part = yyinvo_reason no-lock no-error.
		if not avail pt_mstr then do:
			assign tt_status = "装机产品不存在".
			next.
		end.
		tt_cr_acct = get_acct("wo_account",pt_prod_line,"","").
	end.
	...
	if tt_cr_acct <> "Account Error." and tt_dr_acct <> "Account Error." 
		...
		run create_gldet(input ttglt_id,input tt_dr_acct,input param1,input - tt_amt).
		run create_gldet(input ttglt_id,input tt_cr_acct,input param1,input tt_amt).
    ...
    for each ttglt_mstr no-lock:
	put "贷方            借方                金额" skip.
	put ttglt_dr_acct "      " ttglt_cr_acct	ttglt_ctrl_amt skip.
...
function get_acct returns char (input param1 as char, input param2 as char,input param3 as char,input param4 as char):
	...
	if param1 = "wo_account" or param1 = "lbrmfg_account" then do:
		find first code_mstr where code_fldname	= param1 and param2 begins code_value no-lock no-error.
		...
		if lookup(param3,"DCEC-B,DCEC-C") = 0 then do:
			find first code_mstr where code_fldname	= param1 and ("D" + param2) begins code_value no-lock no-error.
		end.
		else do:
			find first code_mstr where code_fldname	= param1 and param2 begins code_value no-lock no-error.
		end.
		...
		/* 6B or 7B */
		find first code_mstr where code_fldname	= "so_job" and code_value = param2 no-lock no-error.
		if avail code_mstr then do:
			if num-entries(code_cmmt,"-") > 2 then 
