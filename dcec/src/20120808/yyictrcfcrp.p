...
/*cj*/ DEF VAR loc LIKE tr_loc .
/*cj*/ DEF VAR loc1 LIKE tr_loc .
...
def var edwip like tr_qty_loc.
def var bgwip like tr_qty_loc.
def var inwip like tr_qty_loc.
def var outwip like tr_qty_loc.
...
/*cj*/    if loc1 = hi_char then loc1 = "".
    ...
    update site site1 effdate effdate1 line line1 type type1 group1 group2 part part1 /*site site1*/
           /*cj*/ loc loc1 keeper keeper1 yn_zero with frame a.
    ...
    for each in_mstr WHERE (in_site >= site and in_site <= site1) and 
       ...
       oth = 0.
        ...
        FOR EACH ld_det WHERE ld_part = pt_part,
            FIRST loc_mstr WHERE loc_site = ld_site AND loc_loc = ld_loc AND loc_status = "wip":
            edwip = edwip + ld_qty_oh.
        END.
        ...
        for each tr_hist no-lock where tr_part = pt_part
                 ...
                 and (tr_qty_loc <> 0 or tr_type = "cst-adj")
                 /*cj*/ and (tr_loc >= loc and tr_loc <= loc1),
            FIRST loc_mstr WHERE loc_site = tr_site AND loc_loc = tr_loc:
            
            if tr_effdate <= effdate1 then do:
                if tr_type begins "Iss" then  
                    outwip = outwip - tr_qty_loc. 
                if tr_type begins "rct" then
                    inwip = inwip + tr_qty_loc.
            end. /* if tr_effdate */           
            else if tr_qty_loc <> 0 then
                edwip = edwip - tr_qty_loc.
        end. /* for each tr_hist */
        ...
        if (yn_zero and (edqty <> 0 or bgqty <> 0 or rctpo <> 0 or rcttr <> 0 or rctunp <> 0 or rctwo <> 0
          ...
          or isswo <> 0 or invadj <> 0 or oth <> 0)) 
           ...
           disp pt_part pt_desc2 pt_prod_line pt_abc in__qadc01 label "保管员"
              in_user1 label "缺省库位"  bgwip LABEL "期初WIP" bgqty - bgwip LABEL "期初材料"
               bgqty label "期初库存"
              ...
              edwip LABEL "期末WIP"  edqty - edwip LABEL "期末材料" edqty label "期末库存" 
              glxcst edqty_amt label "期末库存金额" with width 320 stream-io.
