   ...
   per_end = glc_end.
	...
	repeat:
		...
		if not avail td then do:
			...
			/* 取标准成本 */
			find last yywobmspt_mstr where yywobmspt_part = td_part
			...
			else assign v_site = yywobmspt_site
				    v_version = yywobmspt_version.

			find first yywobmspt_mstr where yywobmspt_site = v_site
			...
			if avail yywobmspt_mstr then assign 
				td_lbr_cost = td_qty * yywobmspt_elem_cost.
			...
			if avail yywobmspt_mstr then assign 
				td_mfg_cost = td_qty * yywobmspt_elem_cost.
			assign td_reason = "成功".
		end. /* if not avail td  */
	...
	if v_update then do:
		for each td no-lock:
			find first yylbrmfg_det where yylbrmfg_year = v_year
	...
	if p_line = "6" then do:
		...
		for each yylbrmfg_det where yylbrmfg_year = v_year
			...
			if last-of( yylbrmfg_part) then do:
				find first  yyvarated_det where yyvarated_part = yylbrmfg_part
				...
				if avail yyinvi_mstr then 
					assign v_qty = yyinvi_upl_qty + yyinvi_buy_qty + yyinvi_mfg_qty.
				else
					assign v_qty = 0.
				find first yyinvpbd_det where yyinvpbd_year = v_year
					and yyinvpbd_per = v_period
					and yyinvpbd_part = yylbrmfg_part.
				if avail yyinvpbd_det then 
					assign v_qty = v_qty + yyinvpbd_qty.

				if v_qty <> 0 then assign 
					yyvarated_lbr_rate = tot_lbr_amt / v_qty
					yyvarated_mfg_rate = tot_mfg_amt / v_qty.
				else assign 
					yyvarated_lbr_rate = tot_lbr_amt
					yyvarated_mfg_rate = tot_mfg_amt.
		...
		for each wo_mstr no-lock where wo_status = "C"
			...
			for each wod_det no-lock where wod_lot = wo_lot
				...
				find first ptp_det where ptp_part = wod_part and ptp_site = wod_site no-lock no-error.

				if avail ptp_det and ptp_pm_code <> "M" then next.
				else if not avail ptp_det and pt_pm_code <> "M" then next.

				find first yyvarated_det where yyvarated_year = v_year
				...
				yycsvar_variance[16] = yycsvar_variance[16] + wod_qty_iss * yyvarated_lbr_rate.
				yycsvar_variance[18] = yycsvar_variance[18] + wod_qty_iss * yyvarated_mfg_rate.
		...
		for each yylbrmfg_det where yylbrmfg_year = v_year
			...
			if v_qty <> 0 then assign 
				yyvarated_lbr_rate = (yylbrmfg_lbr_tl + yylbrmfg_lbr_ll) / v_qty
				yyvarated_mfg_rate = (yylbrmfg_mfg_tl + yylbrmfg_mfg_ll) / v_qty.
			else assign 
				yyvarated_lbr_rate = (yylbrmfg_lbr_tl + yylbrmfg_lbr_ll)
				yyvarated_mfg_rate = (yylbrmfg_mfg_tl + yylbrmfg_mfg_ll).
			find first yycsvar_mstr where yycsvar_year = v_year
				and yycsvar_per = v_period
				and yycsvar_part = yylbrmfg_part no-error.
			if not avail yycsvar_mstr then do:
