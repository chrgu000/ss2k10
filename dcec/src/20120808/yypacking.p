...
REPEAT:
  ...
  IF v_log = YES THEN DO:
   FOR EACH dsd_det WHERE dsd_part >=part1 AND dsd_part <= part2 AND dsd_per_date >= date1 AND dsd_per_date <= date2 
    ...
    FOR EACH IN_mstr WHERE IN_part = dsd_par AND IN_site = dsd_shipsite AND (in_qty_oh <> 0 OR in_qty_nonet <> 0) NO-LOCK.
    ...
    FIND FIRST ptp_det WHERE ptp_part = dsd_part AND ptp_site = dsd_site NO-LOCK NO-ERROR.
