/* GUI CONVERTED from poporcm.p (converter v1.77) Wed Mar 24 04:01:29 2004 */
...
/* DISPLAY TITLE */
{mfdeclre.i}
/*judy 05/08/12*/ /*{cxcustom.i "POPORCM.P"}  */
/*judy 05/08/12*/ {cxcustom.i "yyPOPORCM.P"} 
...
/*GUI*/ if global-beam-me-up then undo, leave.
 /* IF fill-all */
      ...
      assign
         ...
         proceed   = no.
/*judy 05/08/12*/     /* {gprun.i ""poporca.p""}  */
/*judy 05/08/12*/      {gprun.i ""yypoporca-1.p""}   
...
/*judy 05/08/12*/ /*{pxrun.i &PROC='commitReceipt' &PROGRAM='porcxr.p'	*/
/*judy 05/08/12*/ {pxrun.i &PROC='commitReceipt' &PROGRAM='yyporcxr.p'	 
                &PARAM="(input shipnbr,
                        input ship_date,
                        input inv_mov,
                        input ip_usage,
...
/*LB01*/		/*Call work order receive/backflush */	
					...
					message "   外包零件 “" + wo_part + "” 入库回冲.   " view-as alert-box.
                    			{gprun.i ""yywowoisrc.p"" "(input wosublot, input wosubqty)"}
					hide all.
