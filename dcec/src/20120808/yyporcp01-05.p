...
/*LAST MODIFIED BY *LB01*             LONG BO   2004-7-12  
  LAST MODIFIED BY *frk*              Frank Sun 2008-4-7                          
		...
		break by prh_part by prh_vend:
			...
			if last-of(prh_part) or last-of(prh_vend) then do:
				...
				assign
					...
					xxpo		= prh_nbr
					xxqtyrcv  	= tmpqty
					xxmonth		= string(year(zzto)) + "." + string(month(zzto),"99").
			end. 
		end.
	end.	
...
/*	chExcelWorkbook:Worksheets(1):Range(Cells(iRow - 1, iCol), Cells(iRow - 1, iCol + 1)):MergeCells = -1.*/
	...
	for each xxwk where can-find(first tmpwk where 
		tmpwk.xxpart = xxwk.xxpart and tmpwk.xxpo = xxwk.xxpo
		and	(tmpwk.xxqtyplan <> 0 or tmpwk.xxqtyrcv <> 0))
	break by xxpart by xxpo by xxmonth:
		if first-of(xxpart) or first-of(xxpo) then do:
			iRow = iRow + 1.
		...
		if last-of(xxpart) or last-of(xxpo) then do:
			iCol = iCol + 1.
