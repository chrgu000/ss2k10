...
define temp-table tttt RCODE-INFORMATION
	...
	fields tttt_invvar	like yycsvar_variance[14] label "发票价格差异"
	fields tttt_qty		like yyinvpbd_qty label "采购数量"
...
REPEAT:
	...
	for each yycsvar_mstr no-lock where yycsvar_part >= part and yycsvar_part <= part1
		and yycsvar_pm_code = "P"
		and yycsvar_year >= theyear and yycsvar_year <= theyear1
		and yycsvar_per >= theper and yycsvar_per <= theper1:
		
		find first pt_mstr where pt_part = yycsvar_mstr.yycsvar_part no-lock no-error.
		...
		assign 
		    ...
		    tttt_invvar = yycsvar_variance[14].

		/* 取本期入库量 */
		find first yyinvi_mstr where yyinvi_part = yycsvar_part
			and yyinvi_year = yycsvar_year
			and yyinvi_per = yycsvar_per no-lock no-error.
		if avail yyinvi_mstr then assign tttt_qty = yyinvi_buy_qty.
		else assign tttt_qty = 0.

		if tttt_qty = 0 then 
			assign ppvrate = (yycsvar_variance[13] + yycsvar_variance[14]).
		else 
			assign ppvrate = (yycsvar_variance[13] + yycsvar_variance[14]) / tttt_qty.
		tttt_rate = ppvrate.
		release tttt.
