...
REPEAT:
   ...
   FOR EACH tr_hist WHERE tr_part =itpart AND tr_date >= bedate AND tr_date <= eddate AND tr_type = "iss-wo" AND (tr_userid ="Admin" OR tr_userid ="MRP") BREAK BY tr_lot .
   ...
   FOR EACH xxwkpt.
    FOR EACH tr_hist WHERE tr_lot = xxwkpt.lot AND tr_type ="rct-wo" AND  tr_date >= bedate AND tr_date <=eddate AND (tr_userid ="Admin" OR tr_userid ="MRP") BREAK BY tr_lot. 
