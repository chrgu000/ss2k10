...
/* Revision: 1.62.5.1    BY: Salil Pradhan        DATE: 11/12/03  ECO: *P197* */
/* Revision: 1.62.5.2    BY: Ed van de Gevel      DATE: 12/03/03  ECO: *P1D6* */
/* Revision: 1.62.5.4    BY: Pankaj Goswami       DATE: 12/29/03  ECO: *P1FV* */
/* $Revision: 1.62.5.6 $         BY: Gaurav Kerkar        DATE: 04/22/04  ECO: *P1Y5* */
/******************************************************************************/
...
/* REVISION: eb2+sp7    LAST MODIFIED:2005/06/20           BY: *tfq* Tao fengqin  cj    */
{mfdeclre.i}
{cxcustom.i "YYRSPOMTB.P"}
...
form
   ...
   pod_tax_in        colon 25
with frame set_tax row 8 overlay centered side-labels attr-space.
...
repeat:
   ...
   do with frame pod:
      ...
      do:
         ...
         do:
            for first pod_det where pod_nbr = po_nbr and
...
/*LB01*/    {yychkbuyer.i "input frame pod "}			
     ...
     if c-application-mode <> "API" then
         ...
         fields(scx_line scx_order scx_part scx_po scx_shipfrom scx_shipto
           ...
           where scx_type = 2
           and scx_shipfrom = po_vend
           and scx_shipto = ttScheduleCrossRef.shipto
           and scx_part = ttScheduleCrossRef.part
           and scx_po = po_nbr no-lock:
         end.
     ...
     /*cj*/ pod__chr01 		= "".      /*GD20*/
         ...
         for first pod_det where pod_nbr = scx_order
...
find in_mstr where in_site = pod_site and in_part = pod_part no-lock no-error.
if avail in_mstr then do:
 ...
 find sct_det where sct_sim = l_gl_set and sct_site = gl_site and sct_part = pod_part no-lock no-error.
 if (l_gl_set="") or (not available sct_det) or (sct_cst_tot <= 0) then do:
     message "零件成本不存在，请先维护成本！" VIEW-AS ALERT-BOX ERROR BUTTONS OK.		
      undo , retry.
 end. 
 /*judy 05/10/12*/ 
 /*********
...
/************tfq added end********************************/
      ...
      else if input pod_um <> l_old_um
      then do:
         pod_um_conv = 1.
      end. /* IF INPUT pod_um <> l_old_um */
...
PROCEDURE checkPart:
   ...
   for first pt_mstr where pt_part = ttScheduleCrossRef.part no-lock:
   end.
   if not available pt_mstr
   then do:

      run replaceSupplierItem (input  ttScheduleCrossRef.part,
                               input  po_vend,                                                                 output l_part,
                               output l_vend_avail).
      run readpt_mstr (input l_part,
                       buffer pt_mstr).
   ...
   /* INITIALIZE SUPPLIER ITEM IF AVAILABLE */
      ...
      if can-find(first vp_mstr
         where vp_part = ttScheduleCrossRef.part
         and   ( vp_vend = po_vend
                 or (vp_vend = ""
                 and not can-find(first vp_mstr
                 where vp_part = ttScheduleCrossRef.part
...
PROCEDURE getScheduleLine:
   ...
   do:
      ...
      if not can-find (scx_ref where scx_type = 2
                   and scx_order = po_nbr
...
PROCEDURE replaceSupplierItem:
   ...
   define output       parameter pVendAvail      as logical   no-undo.
            ...
            for first vp_mstr
               fields(vp_part vp_vend_part vp_vend)
               where vp_vend_part = pItemId
...
PROCEDURE getVendorItem:
      ...
      define output parameter p_supp_part like pt_part no-undo.
         ...
         for each vp_mstr
            fields(vp_part vp_q_date vp_vend vp_vend_part)
            where vp_vend_part = p_vend_part
            and ( vp_vend = p_vend
                  or (vp_vend = ""
                      and not can-find(first vp_mstr
