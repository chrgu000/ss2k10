...
/* $Revision: 1.54.1.1 $  BY: Vinay Soman             DATE: 02/16/04  ECO: *P1P7* */
/******************************************************************************/
...
{mfdeclre.i}
/*tfq     {cxcustom.i "SOSOMTP.P"} */
/*tfq*/ {cxcustom.i "yySOSOMTP.P"}
...
/*tfq  {sosomt02.i}*/  /* Form definitions for shared frames a & b */
/*tfq*/  {yysosomt02.i}
/*V8-*/
...
do on error undo, retry with frame b:
 ...
 /*tfq*/		/*REQ DATE AND DUE DATE*/
 /*tfq*/		   assign sod_req_date = so_mstr.so_req_date
						           sod_due_date = so_mstr.so_due_date.	
         end. /* FOR EACH sod_det */
   ...
   /*lb01---*/
		if new so_mstr then do:
			...
			find first code_mstr no-lock where code_fldname = "zz-soslspsn-ctrl"
			and code_value = global_userid no-error.
			if available code_mstr then
				so_slspsn[1] = code_cmmt.
		end.
		/*---lb01 8/12/2004 11:34AM*/
   /********tfq added end******************************************/
