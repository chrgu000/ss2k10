...
/*GUI*/ RECT-FRAME:HEIGHT-PIXELS in frame a = FRAME a:HEIGHT-PIXELS - RECT-FRAME:Y in frame a - 2.
    ...
    for first yycsvar_mstr where yycsvar_year = theyear
	and yycsvar_per = theper
	and yycsvar_part = part
	and (yycsvar_pm_code <> "P" or 
	     can-find(first code_mstr where code_fldname = "sub_line" and code_value = yycsvar_part_pl no-lock)) :

	for each yycsvard_det where yycsvard_year = theyear
	...
	if can-find(first code_mstr where code_fldname = "sub_line" and code_value = yycsvar_part_pl no-lock) then
		assign 
		  yycsvar_variance[13] = yycsvar_variance[21]
		  yycsvar_variance[14] = yycsvar_variance[22].
    end. /* for each yycsvar_mstr */
    /*********************************
    for first yyvarated_det where yyvarated_year = theyear
    ...
    ********************************************/
    for first yylbrmfg_det where yylbrmfg_year = theyear
...
PROCEDURE yypro-cal-var:    
	...
	*/
		...
		if avail yyvarated_det then do:
			...
			/* 自制半成品差异 */
				...
				assign yycsvard_variance[14] = yycsvard_variance[14] + wod_qty_iss * yyvarated_mtl_rate
				       ...
				       yycsvard_variance[16] = yycsvard_variance[16] + wod_qty_iss * yyvarated_lbr_rate
				       yycsvard_variance[18] = yycsvard_variance[18] + wod_qty_iss * yyvarated_mfg_rate. 
			find first yylbrmfg_det where yylbrmfg_year = theyear
					and yylbrmfg_per = theper
	...
	/* 期初库存，料差 */
	find first yypbd_det where yypbd_year = theyear
	...
	/* 当期入库 */
	find first yyinvi_mstr where yyinvi_year = theyear
	...
	/* 当期材料差异 */
	for first yycsvar_mstr no-lock where yycsvar_year = theyear
		and yycsvar_per = theper
	...
	/* 当期人工、费用*/
	for first yylbrmfg_det no-lock where yylbrmfg_year = theyear
		and yylbrmfg_per = theper
		and yylbrmfg_part = part:
