...
def var start like ps_start label "需求日期" INITIAL TODAY.
DEF VAR enddate LIKE ps_start label "结束日期" INITIAL TODAY.
def var outpath as char format "x(40)" label "输出目录".
def var testdir as char.
def var i as inte.
DEFINE VAR drp_qty LIKE ds_qty_conf INITIAL 0. /*added by LXG*/
DEF WORKFILE xxdrp
    FIELD dsite LIKE ds_site
    FIELD dssite LIKE ds_shipsite
    FIELD dpart LIKE ds_part
    FIELD dsdate LIKE ds_shipdate
    FIELD drpqty LIKE ds_qty_conf
    FIELD dsg LIKE ds_qty_conf. /*added by LXG*/
...
def workfile xxwk
   ...
   FIELD sv LIKE ds_qty_conf /*added by LXG*/
   FIELD cebj LIKE ds_qty_conf /*added by LXG*/
...
form
   ...
   SKIP(1)  /*GUI*/
   start colon 22       enddate  colon 45 label {t001.i}skip(1)
...
/*	outpath = "E:\Project\DCEC\Customization(6702C.08)". lb01*/
    ...
    /*lb01--*/
    update start enddate with frame a.
    if start = ? then do:
...
/* lb01   if search("E:\Project\DCEC\Customization(6702C.08)\supplier schedule template.xlt") = ? then do:*/
  if search("\\qadtemp\appeb2\template\purchase\supplier schedule templateV1.xlt") = ? then do: 
    ...
    for each pod_det where (pod_site >= site and pod_site <= site1)
        ...
        each schd_det where schd_type = 4
                     and schd_rlse_id = pod_curr_rlse_id[1] 
                       ...
                       and schd_date >= START AND schd_date <= enddate no-lock:              /*kevin*/
    ...
    /*added by Liu xianggang*/
    FOR EACH xxdrp.
        DELETE xxdrp.
    END.
    FOR EACH ds_det WHERE ds_shipdate >= mstart AND ds_shipdate <= enddate  BREAK BY ds_site BY ds_shipsite BY ds_part .
    IF FIRST-OF(ds_site) OR FIRST-OF(ds_shipsite) OR FIRST-OF(ds_part)  THEN drp_qty = 0.
    drp_qty = drp_qty +(ds_qty_conf -  ds_qty_ship).
    IF LAST-OF (ds_site) OR LAST-OF (ds_shipsite) OR LAST-OF (ds_part) THEN 
       CREATE xxdrp.
       ASSIGN xxdrp.dsite = ds_site
              xxdrp.dssite = ds_shipsite
              xxdrp.dpart = ds_part
              xxdrp.dsdate = ds_shipdate
              xxdrp.drpqty = drp_qty
              xxdrp.dsg = 0.
    END.
    FOR EACH xxwk.
        FIND FIRST xxdrp WHERE xxdrp.dssite = xxwk.site AND xxdrp.dpart = xxwk.part NO-ERROR.
        IF AVAILABLE xxdrp AND xxdrp.dsite = "DCEC-SV" AND xxdrp.dsg <> 1 THEN DO:
           xxwk.sv = xxdrp.drpqty.
           xxdrp.dsg = 1.
        END.
        IF AVAILABLE xxdrp AND xxdrp.dsite = "CEBJ" AND xxdrp.dsg <>1 THEN DO: 
           xxwk.cebj = xxdrp.drpqty.
           xxdrp.dsg = 1.
        END.
    END. /*added by LXG*/
...
/*         chExcelWorkbook = chExcelApplication:Workbooks:ADD("E:\Project\DCEC\Customization(6702C.08)\supplier schedule template.xlt").*/
         chExcelWorkbook = chExcelApplication:Workbooks:ADD("\\qadtemp\appeb2\template\purchase\supplier schedule templateV1.xlt").
        ...
        */
        ...
        chExcelWorkbook:worksheets(1):cells(itotalline,"W"):value = xxwk.qty_ord - xxwk.sv - xxwk.cebj.
        chExcelWorkbook:worksheets(1):cells(itotalline,"X"):value = xxwk.sv.
        chExcelWorkbook:worksheets(1):cells(itotalline,"Y"):value = xxwk.cebj.

        chExcelWorkbook:worksheets(1):cells(itotalline,"Z"):value = xxwk.stime.
        chExcelWorkbook:worksheets(1):cells(itotalline,"AB"):value = xxwk.loc.
        chExcelWorkbook:worksheets(1):cells(itotalline,"AD"):value = xxwk.qty_pack.
        chExcelWorkbook:worksheets(1):cells(itotalline,"AF"):value = xxwk.keeper.
/*        chExcelWorkbook:worksheets(1):cells(itotalline,"AG"):value = xxwk.site.*/              /*kevin,12/2*/
        chExcelWorkbook:worksheets(1):cells(itotalline,"AH"):value = xxwk.ponbr.
        chExcelWorkbook:worksheets(1):cells(itotalline,"AJ"):value = xxwk.poline.
        
        /*lb01--*/
        chExcelWorkbook:worksheets(1):cells(itotalline,"AK"):value = xxwk.sftstock.
        chExcelWorkbook:worksheets(1):cells(itotalline,"AL"):value = xxwk.mqty.
        chExcelWorkbook:worksheets(1):cells(itotalline,"AM"):value = xxwk.supperc.
        chExcelWorkbook:worksheets(1):cells(itotalline,"AN"):value = xxwk.mqty * xxwk.supperc / 100.
        chExcelWorkbook:worksheets(1):cells(itotalline,"AO"):value = xxwk.cumrctqty.
        /*--lb01*/
        FIND FIRST pt_mstr WHERE pt_part = xxwk.part NO-LOCK NO-ERROR.
        IF AVAILABLE pt_mstr THEN chExcelWorkbook:worksheets(1):cells(itotalline,"AP"):value = pt_status.
