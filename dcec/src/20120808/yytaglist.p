...
DEFINE VAR costg LIKE sct_sim INITIAL "STD2010".
...
FORM 
    ...
    lotyes COLON 25 LABEL "Display Barcode(Yes/No)"
...
REPEAT:
 ...
 FOR EACH tag_mstr WHERE tag_site >= site1 AND tag_site <= site2 AND tag_loc >= loc1 AND tag_loc <= loc2  USE-INDEX tag_slpsn NO-LOCK BY tag_nbr.
     ...
     ELSE DO:
         v_desc = "". v_prod_line = "".
     END.
     FIND FIRST sct_det WHERE sct_site = tag_site AND (tag_site = "DCEC-B" OR  tag_site = "DCEC-C") AND sct_sim = costg AND sct_part = tag_part NO-LOCK NO-ERROR.
     IF AVAILABLE sct_det THEN part_cost = sct_cst_tot. ELSE part_cost = 0.

     FIND FIRST sct_det WHERE sct_site = "DCEC-C" AND (tag_site <> "DCEC-B" AND tag_site <> "DCEC-C") AND sct_sim = costg AND sct_part = tag_part NO-LOCK NO-ERROR.
 ...
 FOR EACH xxwkqad BY xxwkqad.tag.
     FOR EACH b_co_mstr WHERE b_co_part = xxwkqad.part AND b_co_lot = xxwkqad.serial AND b_co_site = xxwkqad.site AND b_co_loc = xxwkqad.loc AND b_co_status ="RCT" 
         ...
         IF FIRST-OF (b_co_lot) THEN DO:
             CREATE xxwkall.
             ASSIGN
                 xxwkall.tag = xxwkqad.tag
                 xxwkall.site = xxwkqad.site
                 xxwkall.loc = xxwkqad.loc
                 xxwkall.part = xxwkqad.part
                 xxwkall.desc1 = xxwkqad.desc1
                 xxwkall.prod_line = xxwkqad.prod_line
                 xxwkall.qty_frz = xxwkqad.qty_frz
                 xxwkall.abc =  xxwkqad.abc
                 xxwkall.keeper =  xxwkqad.keeper
                 xxwkall.cost = xxwkqad.cost
                 xxwkall.serial =  xxwkqad.serial
                 xxwkall.cnt_qty =  xxwkqad.cnt_qty
                 xxwkall.rcnt_qty =  xxwkqad.rcnt_qty
                 xxwkall.bar_code = b_co_code
                 xxwkall.bar_qty = b_co_qty_cur
                 xxwkall.sign = b_co_cntst.
         END.
         ELSE DO:
             CREATE xxwkall.
             ASSIGN
