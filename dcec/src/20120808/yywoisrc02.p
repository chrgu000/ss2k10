/* GUI CONVERTED from woisrc02.p (converter v1.77) Tue Oct 21 01:39:27 2003 */
...
{cxcustom.i "yyWOISRC02.P"} /* judy 05/08/12*/
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   assign
      ...
      reject_qty   = 0
      um           = ""
/*judy 05/08/12*/ /*LB01*/ lotserial_qty = wosubqty.
...
/*GUI*/ if global-beam-me-up then undo, leave.
      ...
      assign
         ...
         location = ""
         lotref = ""
/*judy 05/08/12*/         /*lotserial_qty = total_lotserial_qty
/*judy 05/08/12*/         total_lotserial_qty = 0.  */
/*judy 05/08/12*//*LB01*/   lotserial_qty = wosubqty.
/*judy 05/08/12*//*LB01*/   total_lotserial_qty = open_ref - wosubqty.
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         /* IF THE WORK ORDER IS BEING RECEIVED INTO A WAREHOUSE */
         /* LOCATION, SET THE REFERENCE FIELD TO THE WAREHOUSE'S */
         /* DEFAULT INVENTORY STATUS UNLESS THE USER HAS         */
         /* EXPLICITLY ENTERED A REFERENCE VALUE. THIS IS        */
         /* NECESSARY IN ORDER TO PERMIT A SINGLE WAREHOUSE TO   */
         /* STORE INVENTORY WITH MULTIPLE STATUSES (E.G. GOOD,   */
         /* SCRAP, INSPECT) FOR THE SAME ITEM NUMBER.  THE       */
         /* PRIMARY KEY STRUCTURE OF ld_det WILL FORCE THE       */
         /* INVENTORY TO HAVE THE SAME STATUS UNLESS THE         */
         /* QUANTITIES CAN BE DISTINGUISHED THROUGH DIFFERENT    */
         /* LOT/SERIAL OR REF NUMBERS.                           */
         if lotref = ""
            and can-find(whl_mstr where whl_site = site
