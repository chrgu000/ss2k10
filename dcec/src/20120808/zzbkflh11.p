...
/*judy begin added*/	      
	...
	/*G1ZV*/   if can-find(first isd_det where
	/*G1ZV*/              isd_status = string(pt_status,"x(8)") + "#"
	/*G1ZV*/              and (isd_tr_type = "ISS-WO")) then do:
...
/*judy begin added*/
             FIND FIRST IS_mstr WHERE IS_status = (if available loc_mstr and loc_status <> "" then loc_status
                                      else si_status) NO-LOCK NO-ERROR.
             IF IS_overissue = NO and avail loc_mstr then DO:
	       	for each ld_det  WHERE ld_site = xxwk.site AND ld_part = xxwk.comp
		 ...
		 if avail xxld_wkfl then do:
			 ...
			 /*fm268 if xxld_qty_oh < 0 then  do:
			     PUT unformatted "错误: 子零件 " + xxwk.comp + " 所对应的库位 " + xxwk.site + ","
