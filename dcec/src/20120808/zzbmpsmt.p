...
*/
/* REVISION: 8.5       LAST EDIT: 10/05/03      BY: *DCEC101* Kevin, for bom 
trace*/
...
SKIP(.1)  /*GUI*/
site           colon 25       si_desc no-label at 47 skip(1)             
/*kevin*/
ps_par         colon 25
...
/*GE64*/                                    ps_scrp_pct colon 59 format 
">9.99%"
                                              ps_lt_off colon 59
                                                  ps_op colon 59
                                             ps_item_no colon 59
...
/*F0WG*/          find bom_mstr where recid(bom_mstr) = recno no-lock 
no-error.
...
alert-box error.
                ...
                if
                /*ptp_pm_code = "F" or*/                              
/*control "F" type item as engine*/
                ptp_pm_code = "C" then assign fc_type = yes.

                /*added by kevin, not to control the item of pt_group is 
"M"*/
...
view-as alert-box error.
            ...
            find first ps_mstr where ps_par = input ps_par and ps__chr01 <> 
...
/*GC10*/          if frame-field = "ps_comp" then do:
                     /* FIND NEXT/PREVIOUS RECORD */
                     {mfnp01.i ps_mstr ps_comp ps_comp parent ps_par 
ps_parcomp}
                     ...
                     if recno <> ? then do:
                        ...
                        find pt_mstr where pt_part = ps_comp no-lock 
no-error.
...
"根据BOM控制文件,本地点只能使用零件号作为'子零件'!" view-as alert-box error.
                  ...
                  if bom__chr01 <> site and not available pt_mstr then do:
                        find first ps_mstr no-lock using ps_par and ps_comp 
and ps_ref and ps_start no-error.
...
/*added by kevin, 10/20/2003 for site control*/
                  find first ptp_det where ptp_site = site and ptp_part = 
...
then do:
            find first ps_mstr where ps_par = input ps_comp and ps__chr01 <> 
site no-lock no-error.
...
view-as alert-box error.
            ...
            find first ps_mstr where ps_comp = input ps_comp and ps__chr01 
<> site no-lock no-error.
            if available ps_mstr then do:
                 message "该子零件存在 '" + ps__chr01 + " '地点的产品结构" 
view-as alert-box error.
...
/*J005*/             find first ps_mstr1 no-lock where ps_mstr1.ps_par = 
comp
/*J07Z*  /*J005*/    and ps_mstr1.ps_ps_code = "J" . */
/*J07Z*/             and ps_mstr1.ps_ps_code = "J" no-error.
/*J005*/             if available ps_mstr1 then do:
/*J005*/       /*A JOINT PRODUCT MAY NOT BE A COMPONENT IN A PHANTOM 
STRUCTURE*/
/*J005*/                {mfmsg.i 6534 3}
/*J005*/                undo, retry.
/*J005*/             end.
...
/*F0WG*/             /*! LOCATE PHANTOM STRUCTURE AND 'BOM CODE' PARENTS OF 
THIS
...
/*J07Z*/                ps_qty_type = ps_qty_type.
                     ...
                     if ps_qty_type = "B" and batch_size <> 0 then
                        assign
                           ps_batch_pct = ps_qty_per_b * conv / batch_size * 
100
                           ps_qty_per_b = ps_qty_per * batch_size.
                     else
                     if ps_qty_type = "P" then
                        assign
                           ps_qty_per = ps_batch_pct * .01 * batch_size / 
conv
                          ps_qty_per_b = ps_batch_pct * .01 * batch_size / 
conv.
...
**F0WG****  DELETED SECTION */
                     ...
                     if ps_item_no <> 0 then do:
                        item_no = ps_item_no.
                        for each ps_mstr1 no-lock where ps_mstr1.ps_par = 
parent
...
/*F0WG*/          /*! UPDATE THE MRP REQUIRED AND LOW LEVEL CODE
                   *  RECALCULATION FLAGS FOR APPROPRIATE ITEMS 
(MANUFACTURED
                   *  ITEMS WHICH SOMEHOW REFERENCE THIS PARENT CODE AS 
THEIR
                   *  BOM CODE). */
...
./*GG22*/             and not can-find (ptp_det where ptp_part = pt_part  
and
