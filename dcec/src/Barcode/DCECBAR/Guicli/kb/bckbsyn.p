...
assign
    ...
    bc_po_line = string(xkprhline)
    bc_po_part = xkprhpart
    bc_po_qty = xkprhqty
    bc_site = xkprhdsite
    bc_loc = xkprhdloc.
FIND FIRST pod_det WHERE pod_nbr = bc_po_nbr AND string(pod_line) = bc_po_line  NO-LOCK NO-ERROR.
bc_po_qty = bc_po_qty * pod_um_conv.

     lntyp = ''.  
  
    DISP bc_po_nbr bc_po_line bc_po_part bc_po_qty bc_site bc_loc WITH FRAME bc.
     DISABLE bc_po_nbr bc_po_line bc_po_part bc_po_qty WITH FRAME bc. 
    IF lntyp = 's' THEN do:
        bc_sub = YES.
      ...
      ELSE DO:
            ...
            FIND FIRST b_co_mstr WHERE b_co_code = bc_id EXCLUSIVE-LOCK NO-ERROR.
              IF AVAILABLE b_co_mstr THEN DO:
                 ...
                 bc_ref = b_co_ref.
                  bc_qty = b_co_qty_cur.
                  IF bc_part = bc_po_part  AND bc_qty =  bc_po_qty THEN DO:
                  ...
                  END.
                  ELSE DO:
                    MESSAGE '请扫描'  bc_po_part '!'             VIEW-AS ALERT-BOX.
                  END.
      END.
      END.
      END.
...
PROCEDURE main:
    ...
    bki_nbr = bc_po_nbr.
      ...
      IF NOT AVAILABLE bki_tmp THEN DO: succ = NO.
