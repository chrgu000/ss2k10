...
DEFINE BUTTON bc_button LABEL "清零" SIZE 8 BY 1.20.
...
DEF FRAME bc
  ...
  bc_button AT ROW 10.5 COL 10
    
    WITH SIZE 30 BY 12 TITLE "未复盘清零"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
  ...
  /*  id = SUBSTR(string(YEAR(TODAY),'9999'),3,2) + STRING(MONTH(TODAY),'99') + STRING(DAY(TODAY),'99') + string(NEXT-VALUE(b_cnt_sq01),'999999999999').
    */
    ...
    IF bc_loc1 = '' THEN bc_loc1 = hi_char.
        ...
        MESSAGE '确定要将未复盘条码作清零处理？' VIEW-AS ALERT-BOX
    ...
    FOR EACH b_co_mstr USE-INDEX b_co_sort4 WHERE b_co_site >= bc_site AND b_co_site <= bc_site1 AND b_co_loc >= bc_loc AND b_co_loc <= bc_loc1 AND b_co_part >= bc_part AND b_co_part <= bc_part1 AND (b_co_status = 'rct' OR b_co_status = 'all') AND b_co_cntst = 'i' EXCLUSIVE-LOCK:
      ASSIGN b_co_cntst = 'r0'.
    END.
