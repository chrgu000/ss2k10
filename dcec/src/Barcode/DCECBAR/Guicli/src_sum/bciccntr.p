...
PROCEDURE main:
   /*  IF id = ''THEN DO:
    ...
    FIND FIRST b_co_mstr WHERE b_co_code = bc_id  AND b_co_cntst = 'i'  EXCLUSIVE-LOCK NO-ERROR.
    IF AVAILABLE b_co_mstr THEN DO:
    FIND FIRST b_cnt_wkfl WHERE    b_cnt_part = b_co_part AND b_cnt_site = b_co_site AND b_cnt_loc = b_co_loc AND b_cnt_lot = b_co_lot AND b_cnt_status = 'i'  EXCLUSIVE-LOCK NO-ERROR.
       ...
       IF AVAILABLE b_cnt_wkfl THEN DO:
           ...
           ASSIGN b_cnt_qty_rcnt = IF b_cnt_lot = b_co_code THEN b_co_qty_cur
                                    ELSE (b_cnt_qty_rcnt + b_co_qty_cur)
                                         b_cnt_qty_oh = IF AVAILABLE b_ld_det AND b_cnt_qty_oh <> b_ld_qty_oh THEN b_ld_qty_oh ELSE b_cnt_qty_oh
                  b_co_cntst = 'r'.
       END.
         ELSE MESSAGE b_co_part  '' b_co_lot '该批料件存在未盘点条码!' VIEW-AS ALERT-BOX .

           END.   
           ELSE MESSAGE '该条码未作初盘,或已作重盘!' VIEW-AS ALERT-BOX.
