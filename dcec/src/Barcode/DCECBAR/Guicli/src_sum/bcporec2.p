...
DO:
       ...
       ASSIGN bc_poshp.
       FIND FIRST b_co_mstr USE-INDEX b_co_sort5 NO-LOCK WHERE  b_co_ref <> '' AND b_co_cntst = '' AND (b_co_status = 'ac' OR b_co_status = 'iss') AND b_co_ref > bc_poshp  NO-ERROR.
...
DO:
        ...
        ASSIGN bc_poshp.
       FIND LAST b_co_mstr USE-INDEX b_co_sort5 NO-LOCK WHERE b_co_ref <> '' AND b_co_cntst = '' AND (b_co_status = 'ac' OR b_co_status = 'iss') AND b_co_ref < bc_poshp NO-ERROR.
       IF AVAILABLE b_co_mstr THEN DO:
           ASSIGN bc_poshp = b_co_ref.
           DISPLAY bc_poshp WITH FRAME bc.
       END.
...
DO:
   ...
   FIND FIRST b_co_mstr use-index b_co_sort5 WHERE  b_co_ref <> '' AND b_co_cntst = '' AND (b_co_status = 'ac' OR b_co_status = 'iss') AND b_co_ref = bc_poshp    EXCLUSIVE-LOCK NO-ERROR.
   IF NOT AVAILABLE b_co_mstr THEN DO:
   
   MESSAGE '货运单不存在,或已入库,或所属条码处于盘点状态!' VIEW-AS ALERT-BOX error.
        UNDO,RETRY.  
        END.
        ELSE DO:
            FOR EACH b_co_mstr USE-INDEX b_co_ref WHERE b_co_ref = bc_poshp AND b_co_cntst = '' AND (b_co_status = 'ac' OR b_co_status = 'iss') NO-LOCK:
                CREATE reclist_tmp.
                ASSIGN
                    rec_sess = g_sess
                    rec_code = b_co_code   
...
IF NOT success THEN LEAVE.
        ...
        DELETE reclist_tmp.  
        END.
       ELSE MESSAGE rec_code ' 已入库，或处于盘点状态!' VIEW-AS ALERT-BOX.
    END.
