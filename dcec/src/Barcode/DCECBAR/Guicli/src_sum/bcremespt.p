...
DEF FRAME bc
   ...
   bc_button AT ROW 10.8 COL 10
    ...
    WITH SIZE 30 BY 12.5 TITLE "MES发料单打印"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
...
chWorkbook = chExcelApplication:Workbooks:Add().
     mcnt = 1.
 FOR EACH b_mes_wkfl WHERE b_mes_due_date >= bc_date AND b_mes_due_date <= bc_date1 AND b_mes_site >= bc_site AND b_mes_site <= bc_site1 AND b_mes_ln_loc >= bc_loc AND b_mes_ln_loc <= bc_loc1 AND b_mes_part >= bc_part AND b_mes_part <= bc_part1 NO-LOCK BREAK BY b_mes_staff:
IF FIRST-OF(b_mes_staff) THEN DO:
rowstart = 1.
     
     chWorkSheet = chExcelApplication:Sheets:Item(mcnt).      
           chExcelApplication:Sheets:Item(mcnt):SELECT().
       chExcelApplication:Sheets:Item(mcnt):NAME = "ME发料单-" + b_mes_staff.
 ...
 chWorkSheet:Range("b1:b1"):VALUE = "地点".
       chWorkSheet:Range("c1:c1"):VALUE = "线边库位".
       chWorkSheet:Range("d1:d1"):VALUE = "零件号".
    
      chWorkSheet:Range("e1:e1"):VALUE = "零件描述".
     chWorkSheet:Range("f1:f1"):VALUE = "需求数".
     chWorkSheet:Range("g1:g1"):VALUE = "实发数".
    chWorkSheet:Range("h1:h1"):VALUE = "欠交数".
      chWorkSheet:Range("i1:i1"):VALUE = "供应商".
    mcnt = mcnt + 1.
END.
...
chWorkSheet:Range(crange):VALUE = b_mes_vend.
