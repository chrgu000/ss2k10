...
PROCEDURE main:
 ...
 chExcelApplication:selection:Font:bold = TRUE.
   ...
   chWorkSheet:Range("g1:g1"):VALUE = "批/序号".
    ...
    rowstart = 1.
 FOR EACH b_tr_hist  WHERE b_tr_date >= bc_date  AND b_tr_date <= bc_date1 AND (IF bc_btrtyp <> '' THEN b_tr_type = bc_btrtyp ELSE YES) AND b_tr_site >= bc_site AND b_tr_site <= bc_site1
