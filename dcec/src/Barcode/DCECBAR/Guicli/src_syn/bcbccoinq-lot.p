...
DEF VAR bc_status AS CHAR FORMAT "x(3)" LABEL "状态".
DEF VAR  bc_btype AS CHAR FORMAT "x(1)" LABEL "类型".
DEF FRAME bc
    bc_id AT ROW 1.2 COL 4
    bc_part AT ROW 2.4 COL 2.5
   
    bc_part_desc  AT ROW 3.6 COL 1
    bc_part_desc1  NO-LABEL AT ROW 4.6 COL 8.5

   bc_lot AT ROW 5.8 COL 1.6
    bc_pack AT ROW 7 COL 2.5
    bc_qty AT ROW 8.2 COL 4
    bc_status AT ROW 9.4 COL 4
    bc_btype AT ROW  10.6 COL 4
   bc_site AT ROW 11.8 COL 4
     bc_loc AT ROW 11.8 COL 20 NO-LABEL
  ...
  /*  bc_qty_std AT ROW 9.5 COL 1*/
    ...
    WITH SIZE 30 BY 14 TITLE "条码查询"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
...
DO:
     ...
     {bcrun.i ""bcmgcheck.p"" "(input ""bd"",
       ...
       IF AVAILABLE pt_mstr THEN
           ASSIGN
            bc_part_desc = pt_desc1
        bc_part_desc1 = pt_desc2.
        DISP bc_part bc_lot bc_qty bc_pack bc_part_desc bc_part_desc1 bc_status bc_btype bc_site bc_loc WITH FRAME bc.
                  END.
