   ...
   /* FIND FIRST b_cnt_wkfl WHERE    b_cnt_qty_oh <> b_cnt_qty_cnt 
    ...
    IF NOT diff THEN DO:
        ...
        IF AVAILABLE  b_ld_det AND b_ld_qty_oh <> b_cnt_qty_oh THEN DO:
          MESSAGE b_ld_site '' b_ld_loc '' b_ld_part '' b_ld_lot '该库存与盘点库存不一致，该批次不能更新！' VIEW-AS ALERT-BOX.
          FOR EACH b_co_mstr USE-INDEX b_co_lot WHERE b_co_part = b_cnt_part AND b_co_lot = b_cnt_lot AND b_co_site = b_cnt_site AND b_co_loc = b_cnt_loc EXCLUSIVE-LOCK:
                  b_co_cntst = ''.
              END.
              b_cnt_status = ''.
     ...
     mqty = if b_cnt_status = 'i'  OR b_cnt_status = 'f' then b_cnt_qty_cnt else b_cnt_qty_rcnt.
            OUTPUT TO VALUE(g_sess). 

           put  "@@BATCHLOAD icccaj.P" skip.
            PUT '"R"' SKIP .
            PUT UNFORMAT '"' b_cnt_part '"' SKIP.
            PUT UNFORMAT '"' b_cnt_site '" "' b_cnt_loc '" "' b_cnt_lot '"'  SKIP.
