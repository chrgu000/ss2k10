...
END.*/
 ...
 /*   END.*/
    IF oktocmt  /*OR NOT AVAILABLE b_cnt_wkfl*/ THEN DO:

  FOR EACH b_co_mstr USE-INDEX b_co_st_loc WHERE b_co_site >= bc_site AND b_co_site <= bc_site1 AND b_co_loc >= bc_loc AND b_co_loc <= bc_loc1 AND b_co_status <> 'ia' AND b_co_cntst <> '' EXCLUSIVE-LOCK:
  b_co_cntst = ''.
  END.
    ...
    END.
    FOR EACH b_cnt_wkfl WHERE b_cnt_site >= bc_site AND b_cnt_site <= bc_site1 AND b_cnt_loc >= bc_loc AND b_cnt_loc <= bc_loc1 AND b_cnt_status = 'f' EXCLUSIVE-LOCK:
        DELETE b_cnt_wkfl.
    END.
