...
DEF FRAME bc
  ...
  /* bc_pkqty AT ROW 10 COL 4*/
   ...
   bc_button AT ROW 8 COL 10
    ...
    WITH SIZE 30 BY 10 TITLE "盘点差异报表(R)" SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
...
PROCEDURE main:
 ...
 chExcelApplication:Sheets:Item(1):SELECT().
       chExcelApplication:Sheets:Item(1):NAME = "盘点差异报表(R)".
    ...
    chWorkSheet:Range("d1:d1"):VALUE = "零件描述".
     ...
     chWorkSheet:Range("f1:f1"):VALUE = "批/序号".
      ...
      chWorkSheet:Range("h1:h1"):VALUE = "库存数".
       chWorkSheet:Range("i1:i1"):VALUE = "复盘数".
    ...
    FOR EACH b_cnt_wkfl WHERE 
        ...
        isst = YES. 
         FIND FIRST b_ld_det WHERE b_ld_site = b_cnt_site AND b_ld_loc = b_cnt_loc AND b_ld_part = b_cnt_part AND b_ld_lot = b_cnt_lot NO-LOCK NO-ERROR.
           ...
           FOR EACH b_co_mstr USE-INDEX b_co_lot WHERE b_co_part = b_cnt_part AND b_co_lot = b_cnt_lot AND b_co_site = b_cnt_site AND b_co_loc = b_cnt_loc AND (b_co_cntst = 'i' OR b_co_cntst = 'i0' OR b_co_status = 'rct'OR b_co_status = 'all') NO-LOCK :
              IF b_co_cntst = '' OR substr(b_co_cntst,1,1) = 'i' THEN do:
                  isst = NO.
 ...
 FOR EACH b_cnt_wkfl WHERE 
     ...
     crange = "e" + STRING(rowstart).  
      ...
      crange = "i" + STRING(rowstart).  
       chWorkSheet:Range(crange):numberformat = '0'.
      chWorkSheet:Range(crange):VALUE = b_cnt_qty_rcnt.
      crange = "j" + STRING(rowstart). 
      chWorkSheet:Range(crange):numberformat = '0'.
      chWorkSheet:Range(crange):VALUE = IF b_cnt_status = 'r' THEN b_cnt_qty_rcnt - b_cnt_qty_oh ELSE 0.
            /* DISP 
              b_cnt_site 
        b_cnt_loc 
        b_cnt_part 
