...
DEFINE BUTTON bc_button1 LABEL "浏览" SIZE 8 BY 1.50.
...
DEF FRAME bc
   ...
   bc_button1 AT ROW 11 COL 10
    /*bc_button2 AT ROW 11 COL 16*/
    
    WITH SIZE 30 BY 14 TITLE "库存初始化"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.

ENABLE bc_site  bc_site1 bc_loc bc_loc1 bc_part bc_part1 bc_button1 /*bc_button2*/ WITH FRAME bc IN WINDOW c-win.
...
PROCEDURE main:
   ...
   create "Excel.Application" chExcelApplication.
     ...
     chWorkSheet = chExcelApplication:Sheets:Item(1).      
       ...
       chExcelApplication:Sheets:Item(1):NAME = "存货报表" .
 ...
 chExcelApplication:selection:Font:bold = TRUE.
    ...
    chWorkSheet:Range("f1:f1"):VALUE = "库存数".
    rowstart = 1.
 FOR EACH b_ld_det WHERE b_ld_site >= bc_site AND b_ld_site <= bc_site1 AND b_ld_loc >= bc_loc AND b_ld_loc <= bc_loc1 
     AND b_ld_part >= bc_part AND b_ld_part <= bc_part1 NO-LOCK:
     FIND FIRST pt_mstr WHERE pt_part = b_ld_part NO-LOCK NO-ERROR.
     rowstart = rowstart + 1.
chWorkSheet:Rows(rowstart):RowHeight = 15. 
   ...
   chWorkSheet:Range(crange):VALUE = b_ld_site.
   crange = "b" + STRING(rowstart).  
   chWorkSheet:Range(crange):VALUE = b_ld_loc.
   crange = "c" + STRING(rowstart).  
   chWorkSheet:Range(crange):VALUE = b_ld_part.
   crange = "d" + STRING(rowstart).  
   chWorkSheet:Range(crange):VALUE = pt_desc1 + pt_desc2.
   crange = "e" + STRING(rowstart).  
   chWorkSheet:Range(crange):VALUE = b_ld_lot.
   crange = "f" + STRING(rowstart).  
  chWorkSheet:Range(crange):VALUE = b_ld_qty_oh .
