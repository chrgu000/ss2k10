...
DEF VAR bc_site AS CHAR FORMAT "x(8)" LABEL "出地点".
DEF VAR bc_loc AS CHAR FORMAT "x(8)" LABEL "出库位".
...
DEF VAR bc_dr AS CHAR FORMAT "x(8)" LABEL "定制码".
...
DEF FRAME bc
  ...
  bc_so_job AT ROW 8.7 COL 2.5
   ...
   /*bc_dr AT ROW 10.8 COL 4*/
    ...
    WITH SIZE 30 BY 12 TITLE "计划外出库"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
...
DO:
   ...
   IF NOT AVAILABLE code_mstr THEN DO:
    MESSAGE '无效预算号!' VIEW-AS ALERT-BOX.
    ASSIGN
        bc_so_job = ''.
...
DO:
   ...
   IF NOT AVAILABLE code_mstr THEN DO:
    MESSAGE '无效定制码!' VIEW-AS ALERT-BOX.
    ASSIGN
        bc_dr = ''.
...
FIND FIRST b_co_mstr WHERE b_co_code = bc_id EXCLUSIVE-LOCK NO-ERROR.
 ...
 OUTPUT TO value(g_sess).
            ...
            PUT UNFORMAT STRING(b_co_qty_cur) " - - " '"' + b_co_site + '" "' + b_co_loc + '"' ' "     "'  SKIP.
            PUT "- - - - - " TODAY SKIP.
            PUT      " " skip
                     "." SKIP
                     "@@END" SKIP.
