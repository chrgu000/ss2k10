...
DO:
   ...
   IF NOT AVAILABLE ABS_mstr THEN DO:
       MESSAGE '该供应商没有货运单!' VIEW-AS ALERT-BOX.
       UNDO,RETRY.
   END.
   ELSE DO:
...
DO:
   ...
   MESSAGE '货运单不存在,或条码未生成或，已入库,或所属条码处于盘点状态!' VIEW-AS ALERT-BOX error.
        UNDO,RETRY.  
        END.
        ELSE DO:
...
END.
      ...
      ELSE DO:
         success = NO.
          MESSAGE rec_code ' 已入库，或处于盘点状态!' VIEW-AS ALERT-BOX.
          LEAVE.
      END.
  ...
  PUT "@@batchload rsporc.p" SKIP.
       ...
       {bcrun.i ""bcmgbdpro.p"" "(INPUT g_sess,INPUT ""out.txt"")"}
           ...
           FOR EACH prh_hist USE-INDEX prh_ps_nbr WHERE prh_vend = bc_vend AND prh_ps_nbr = bc_poshp NO-LOCK:
               isavail = YES.
               FIND LAST tr_hist USE-INDEX tr_date_trn WHERE tr_date = TODAY AND tr_type = 'rct-po' AND tr_lot = prh_receiver AND tr_nbr = prh_nbr AND tr_line = prh_line AND tr_serial = '' NO-LOCK NO-ERROR.
               IF AVAILABLE tr_hist THEN DO:
                   FOR EACH b_co_mstr WHERE b_co_ref = bc_poshp EXCLUSIVE-LOCK:
                       ASSIGN
                           b_co_status = 'rct'.
                         {bctrcr.i
         &ord=b_co_ord
         ...
         &mdate1=TODAY
          ...
          &mtime=TIME
           ...
           END.
           IF NOT isavail THEN MESSAGE '更新QAD失败!' VIEW-AS ALERT-BOX.
