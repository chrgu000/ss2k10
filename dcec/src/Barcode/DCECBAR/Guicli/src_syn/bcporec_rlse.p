...
DEF FRAME bc
    bc_date AT ROW 1.2 COL 4
     bc_po_nbr  AT ROW 2.4 COL 2.5
  bc_id  AT ROW 3.6 COL 4
    bc_po_line AT ROW 4.8 COL 4
      bc_part AT ROW 6 COL 2.5
 /* bc_po_part AT ROW 3.6 COL 2.5*/
    bc_po_qty AT ROW 7.2 COL 2.5 
    'EA' AT ROW 7.2 COL 22 
   ...
   /* bc_ref AT ROW 9.6 COL 2.5*/
    bc_qty AT ROW 9.6 COL 4 'EA' AT ROW 9.6 COL 22 
    ...
    WITH SIZE 30 BY 14 TITLE "PO收货-国产"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
bc_date = TODAY.
...
DO:
       ...
       APPLY 'entry':u to bc_id.
         ...
         {bcrun.i ""bcmgcheck.p"" "(input ""po_rlse"" ,
        input """",
        input """", 
        input """", 
        input """", 
  ...
  END.
        ...
        FIND FIRST pt_mstr WHERE pt_part = b_co_part NO-LOCK NO-ERROR.
         ...
         PUT UNFORMAT '"' bc_po_nbr '"' SKIP.
          ...
          PUT UNFORMAT string(b_co_qty_cur)  ' - - ' pt_um ' - - "' bc_site '" "' bc_loc '"' ' "     "'  SKIP.
        ...
        FIND FIRST po_mstr WHERE po_nbr = bc_po_nbr NO-LOCK NO-ERROR.
          ...
          FIND FIRST b_po_wkfl WHERE b_po_due_date  =  TODAY AND b_po_nbr = bc_po_nbr AND b_po_line = bc_po_line  AND b_po_part = b_co_part    EXCLUSIVE-LOCK NO-ERROR.
          IF AVAILABLE b_po_wkfl THEN
                  ASSIGN
                      b_po_qty_rec = b_po_qty_rec + b_co_qty_cur.

         FIND FIRST b_po_rec WHERE b_po_recdate =  TODAY  AND b_po_ponbr = bc_po_nbr AND b_po_poln = bc_po_line AND b_po_popart = bc_part EXCLUSIVE-LOCK NO-ERROR.
