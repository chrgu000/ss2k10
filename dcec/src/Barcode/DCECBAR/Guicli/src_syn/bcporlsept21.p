...
PROCEDURE main:
    ...
    mcnt = 1.
          ...
          FOR EACH b_po_wkfl WHERE b_po_vend >= bc_po_vend AND b_po_vend <= bc_po_vend1 AND b_po_nbr >= bc_po_nbr AND b_po_nbr <= bc_po_nbr1 
              AND b_po_part >= bc_part AND b_po_part <= bc_part1 AND b_po_due_date >= bc_date AND b_po_due_date <= bc_date1 
               AND  CAN-FIND(FIRST code_mstr WHERE code_fldname = 'part_prod' AND code_value = b_po_prod NO-LOCK) NO-LOCK BREAK BY b_po_staff:
            
              IF FIRST-OF(b_po_staff) THEN DO:
           rowstart = 1.
            chWorkSheet = chExcelApplication:Sheets:Item(mcnt).
                 chExcelApplication:Sheets:Item(mcnt):SELECT().
       chExcelApplication:Sheets:Item(mcnt):NAME = "调拨单-" + b_po_staff.
...
chWorkSheet:Rows(rowstart):RowHeight = 15. 
     ...
     chWorkSheet:Range(crange):VALUE = b_po_vend.
