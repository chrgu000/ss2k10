...
DEF FRAME bc
   ...
   /*bc_pkqty AT ROW 10 COL 4*/
    ...
    WITH SIZE 30 BY 12 TITLE "MES生产领料"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
...
DO:
      ...
      ELSE DO:
          ...
          FIND FIRST b_mes_wkfl WHERE b_mes_part = b_co_part AND b_mes_vend = b_co_vend NO-LOCK NO-ERROR.
          ...
          ELSE DO:
              ...
              IF AVAILABLE b_co_mstr THEN DO:
                 ...
                 bc_ref = b_co_ref.
                  ...
                  FIND FIRST b_ld_det WHERE b_ld_part = b_co_part AND b_ld_site = b_co_site AND b_ld_loc = b_co_loc AND b_ld_lot < b_co_lot NO-LOCK NO-ERROR.
                  IF AVAILABLE b_ld_det THEN MESSAGE '该料件存在更小的批次!' VIEW-AS ALERT-BOX WARNING.
...
DO:
        ...
        output success)"}
           ...
           END.
           ELSE DO:
         FIND FIRST b_mes_wkfl WHERE b_mes_part = bc_part AND b_mes_site = bc_site1 AND b_mes_ln_loc = bc_loc1 AND b_mes_vend = bc_vend AND b_mes_qty_req > b_mes_qty_iss NO-LOCK NO-ERROR.
         IF NOT AVAILABLE b_mes_wkfl THEN DO:
...
FIND FIRST b_mes_wkfl WHERE b_mes_due_date = TODAY AND b_mes_part = bc_part AND b_mes_site = bc_site1 AND b_mes_ln_loc = bc_loc1 AND b_mes_vend = bc_vend AND b_mes_qty_req > b_mes_qty_iss EXCLUSIVE-LOCK NO-ERROR.
    IF AVAILABLE b_mes_wkfl THEN DO:
 
          ASSIGN
            b_mes_qty_iss = b_mes_qty_iss + bc_qty.
          isok = YES.
    END.
      ELSE DO:
      
          FIND FIRST b_mes_wkfl WHERE b_mes_part = bc_part AND b_mes_site = bc_site1 AND b_mes_ln_loc = bc_loc1  AND b_mes_vend = bc_vend AND b_mes_qty_req > b_mes_qty_iss EXCLUSIVE-LOCK NO-ERROR.
         IF AVAILABLE b_mes_wkfl THEN DO:
             ASSIGN
                 b_mes_qty_iss = b_mes_qty_iss + bc_qty.
             isok = YES.
