...
DO:
     ...
     IF NOT success THEN 
      ...
      ELSE DO:
        ...
        /*  FIND FIRST b_mes_wkfl WHERE b_mes_part = b_co_part  NO-LOCK NO-ERROR.
...
DO:
        ...
        output success)"}
           ...
           END.
           ELSE DO:
        /* FIND FIRST b_mes_wkfl WHERE b_mes_part = bc_part AND b_mes_site = bc_site1 AND b_mes_ln_loc = bc_loc1 AND b_mes_qty_req > b_mes_qty_iss NO-LOCK NO-ERROR.
         IF NOT AVAILABLE b_mes_wkfl THEN DO:
...
IF NOT success THEN LEAVE.
     ...
     {bcrun.i ""bcmgcheck.p"" "(input ""ictr_typ"" ,
       ...
       output success)"}      
IF NOT success THEN LEAVE.
    FIND FIRST b_mes_wkfl WHERE b_mes_due_date = TODAY AND b_mes_part = bc_part AND b_mes_site = bc_site AND b_mes_ln_loc = bc_loc AND b_mes_vend = bc_vend EXCLUSIVE-LOCK NO-ERROR.
    ...
    END.
      ELSE DO:
          ...
          FIND FIRST b_mes_wkfl WHERE b_mes_part = bc_part AND b_mes_site = bc_site AND b_mes_ln_loc = bc_loc AND b_mes_vend = bc_vend  EXCLUSIVE-LOCK NO-ERROR.
         IF AVAILABLE b_mes_wkfl THEN DO:
             ASSIGN
                 b_mes_qty_iss = b_mes_qty_iss - bc_qty.
    ...
    {bctrcr.i
         ...
         &b_lot=""""
         &id=?
         &b_qty_req="if available b_mes_wkfl then b_mes_qty_req  else 0"
         &b_qty_loc="b_co_qty_cur * -1"
         &b_qty_chg="b_co_qty_cur * -1"
         &b_qty_short="if available b_mes_wkfl then b_mes_qty_req - b_mes_qty_iss else 0"
         &b_um=b_co_um
         &mdate1=TODAY
          &mdate2=TODAY
          &mdate_eff=TODAY
