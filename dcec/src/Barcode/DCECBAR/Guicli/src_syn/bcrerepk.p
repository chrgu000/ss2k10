...
DEF FRAME bc
   ...
   /*bc_pkqty AT ROW 10 COL 4*/
    ...
    WITH SIZE 30 BY 12 TITLE "自制件生产领料"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
...
FIND FIRST b_rep_wkfl WHERE b_rep_due_date = TODAY AND b_rep_part = bc_part AND b_rep_site = bc_site1 AND b_rep_ln_loc = bc_loc1 AND b_rep_qty_req > b_rep_qty_iss EXCLUSIVE-LOCK NO-ERROR.
    ...
    END.
      ELSE DO:
         ...
         END.
         ELSE DO:
             FIND FIRST IN_mstr WHERE IN_site = bc_site AND IN_part = bc_part NO-LOCK NO-ERROR.  
             CREATE b_rep_wkfl.
             ASSIGN
