...
IF NOT success THEN LEAVE.
     ...
     {bcrun.i ""bcmgcheck.p"" "(input ""ictr_typ"" ,
       ...
       output success)"}      
IF NOT success THEN LEAVE.
    FIND FIRST b_mes_wkfl WHERE b_mes_due_date = TODAY AND b_mes_part = bc_part AND b_mes_site = bc_site AND b_mes_ln_loc = bc_loc EXCLUSIVE-LOCK NO-ERROR.
    ...
    END.
      ELSE DO:
          ...
          FIND FIRST b_mes_wkfl WHERE b_mes_part = bc_part AND b_mes_site = bc_site AND b_mes_ln_loc = bc_loc  EXCLUSIVE-LOCK NO-ERROR.
         IF AVAILABLE b_mes_wkfl THEN DO:
             ASSIGN
                 b_mes_qty_iss = b_mes_qty_iss - bc_qty.
