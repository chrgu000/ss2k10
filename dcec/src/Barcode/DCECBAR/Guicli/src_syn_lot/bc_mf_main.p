...
DEFINE VARIABLE exec_pro AS CHARACTER VIEW-AS EDITOR SIZE 12 BY 1 LABEL "执行程序".
DEFINE VARIABLE i_tab AS INTEGER.
DEF VAR tit AS CHAR FORMAT "x(15)".
    tit = "系统模块功能".
/*
DEFINE FRAME a
    SKIP(1)
...
DEFINE FRAME a
    tit AT ROW 1.5 COL 10.5 NO-LABEL
     mdu_list AT ROW 2.5 COL 2.5 NO-LABEL 
     exec_pro AT ROW 15.5 COL 3 
...
DISP tit WITH FRAME a.
DISABLE tit WITH FRAME a.
/*internal procedure*/
...
DO:
    ...
    sel = SUBSTRING(mdu_list:SCREEN-VALUE,1,1) NO-ERROR.
 IF ERROR-STATUS:ERROR THEN LEAVE.
    ...
    CASE sel:
        ...
        otherwise DO:
            ...
            FIND FIRST b_mnd_det NO-LOCK WHERE b_mnd_nbr = mnu_nbr AND b_mnd_select  = sel  NO-ERROR.
           IF AVAILABLE b_mnd_det THEN DO:
           
            IF  b_mnd_exec <> "" THEN DO:
                FIND FIRST b_usrd_det NO-LOCK WHERE b_usrd_usrid = barusr 
                ...
                IF AVAILABLE b_usrd_det THEN DO:
                     ...
                     exec_pro:SENSITIVE = FALSE.
                     {bcrun.i " b_mnd_exec "}
                       exec_pro:SENSITIVE = TRUE.
                    
                END.
                ELSE DO:
                    MESSAGE "用户" + barusr + "无此权限！" VIEW-AS ALERT-BOX ERROR.
                    RETURN.
                END.
...
DO:
    IF g_user <> "mfg" OR barusr <> "mfg" THEN DO:
        MESSAGE "只有mfg能够操作！" VIEW-AS ALERT-BOX ERROR.
        RETURN.
    END.
    ASSIGN exec_pro.
