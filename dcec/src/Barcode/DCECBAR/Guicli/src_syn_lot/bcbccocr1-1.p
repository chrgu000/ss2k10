...
DEF VAR bc_part AS CHAR FORMAT "x(22)" LABEL "零件号".
...
DEF FRAME bc
  ...
  /* bc_pkqty AT ROW 10 COL 4*/
   ...
   bc_qty_label AT ROW 9.4 COL 4
   
    WITH SIZE 30 BY 12 TITLE "条码生成-国产"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
ismodi = NO.
...
DO:
   ...
   /* IF LASTKEY = KEYCODE("cursor-up") THEN DO:
    ...
    IF INDEX(mstr,'^') <> 0 THEN DO:
        bc_part = substr(mstr,1,INDEX(mstr,'^') - 1).
        bc_vend = SUBSTR(mstr,INDEX(mstr,'^') + 1).
        DISP bc_part bc_vend WITH FRAME bc.
    END.
...
DO:
    bc_qty = DECIMAL(bc_qty:SCREEN-VALUE).
      ...
      IF bc_qty = ? OR bc_qty <= 0 THEN DO:
        MESSAGE '数量不能为空，或零，或为负！' VIEW-AS ALERT-BOX error.
        UNDO,RETRY.    
        END.
