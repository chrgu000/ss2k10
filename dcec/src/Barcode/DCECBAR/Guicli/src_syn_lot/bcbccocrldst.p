...
DEF VAR bc_staff AS CHAR LABEL "库管员".
DEF VAR bc_staff1 AS CHAR LABEL "至".
  ...
  bc_site AT ROW 3.6 COL 4
   ...
   bc_loc AT ROW 6 COL 4
    ...
    bc_part1 AT ROW 9.6 COL 5.5
     bc_mult AT ROW 10.8 COL 1
   bc_button1 AT ROW 12 COL 10
    /*bc_button2 AT ROW 11 COL 16*/
    
    WITH SIZE 30 BY 14 TITLE "零件库存标签生成"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
bc_mult = NO.
ENABLE bc_staff  bc_staff1 bc_site bc_site1 bc_loc bc_loc1 bc_part bc_part1 bc_mult bc_button1 /*bc_button2*/ WITH FRAME bc IN WINDOW c-win.
...
DO:
       ...
       ASSIGN bc_staff.
       FIND FIRST in_mstr NO-LOCK WHERE IN__qadc01 > bc_staff NO-ERROR.
       IF AVAILABLE in_mstr THEN DO:
           ASSIGN bc_staff = IN__qadc01.
           DISPLAY bc_staff WITH FRAME bc.
       END.
...
DO:
       ...
       ASSIGN bc_staff.
        FIND LAST in_mstr NO-LOCK WHERE IN__qadc01 < bc_staff NO-ERROR.
       IF AVAILABLE in_mstr THEN DO:
           ASSIGN bc_staff = IN__qadc01.
           DISPLAY bc_staff WITH FRAME bc.
       END.
...
DO:
    bc_staff = bc_staff:SCREEN-VALUE.
END.
...
DO:
    ...
    /*{bcrun.i ""bcmgcheck.p"" "(input ""site"",
        ...
        IF NOT success THEN
            UNDO,RETRY.
        ELSE DO:*/
   /*  DISABLE bc_staff WITH FRAME bc.*/
...
DO:
       ...
       ASSIGN bc_staff1.
       FIND FIRST si_mstr NO-LOCK WHERE si_site > bc_staff1 NO-ERROR.
       IF AVAILABLE si_mstr THEN DO:
           ASSIGN bc_staff1 = si_site.
           DISPLAY bc_staff1 WITH FRAME bc.
       END.
...
DO:
       ...
       ASSIGN bc_staff1.
       FIND LAST si_mstr NO-LOCK WHERE si_site < bc_staff1 NO-ERROR.
       IF AVAILABLE si_mstr THEN DO:
           ASSIGN bc_staff1 = si_site.
           DISPLAY bc_staff1 WITH FRAME bc.
       END.
...
DO:
    bc_staff1 = bc_staff1:SCREEN-VALUE.
END.
...
DO:
   ...
   /* {bcrun.i ""bcmgcheck.p"" "(input ""site"",
        ...
        IF NOT success THEN
            UNDO,RETRY.
        ELSE DO:*/
 /*    DISABLE bc_staff1 WITH FRAME bc.*/
...
DO:
       ...
       ASSIGN bc_loc.
       FIND FIRST loc_mstr NO-LOCK WHERE /*loc_site = bc_staff AND*/ loc_loc > bc_loc NO-ERROR.
...
DO:
       ...
       ASSIGN bc_loc.
       FIND LAST loc_mstr NO-LOCK WHERE /*loc_site = bc_staff AND*/ loc_loc < bc_loc NO-ERROR.
       IF AVAILABLE loc_mstr THEN DO:
           ASSIGN bc_loc = loc_loc.
           DISPLAY bc_loc WITH FRAME bc.
       END.
...
DO:
       ...
       ASSIGN bc_loc1.
       FIND FIRST loc_mstr NO-LOCK WHERE /*loc_site = bc_staff1 AND*/ loc_loc > bc_loc1 NO-ERROR.
       IF AVAILABLE loc_mstr THEN DO:
...
DO:
       ...
       ASSIGN bc_loc1.
       FIND LAST loc_mstr NO-LOCK WHERE /*loc_site = bc_staff1 AND*/ loc_loc < bc_loc1 NO-ERROR.
       IF AVAILABLE loc_mstr THEN DO:
           ASSIGN bc_loc1 = loc_loc.
           DISPLAY bc_loc1 WITH FRAME bc.
       END.
 ...
 j = 1.
    bcprefix = SUBSTR(string(YEAR(TODAY),'9999'),3,2) + STRING(MONTH(TODAY),'99') + STRING(DAY(TODAY),'99') + STRING(TIME,'999999') + SUBstr(STRING(etime),LENGTH(STRING(etime)) - 1,2).
     ...
     NOT
         ...
         FIRST IN_mstr WHERE in__qadc01 >= bc_staff AND in__qadc01 <= bc_staff1 AND IN_site = ld_site AND IN_part = ld_part NO-LOCK BY IN__qadc01:
...
/* FIND FIRST b_usr_mstr WHERE b_usr_usrid = g_user NO-LOCK NO-ERROR.*/
 ...
 /*OUTPUT TO VALUE(b_usr_printer).*/
              ...
              RELEASE b_co_mstr.
               ...
               RELEASE b_po_wkfl.
               ENABLE bc_staff WITH FRAME bc.
               APPLY 'entry':u TO bc_staff.
