...
DEF TEMP-TABLE bcomstr_merge LIKE b_co_mstr
    FIELD bcomstr_merge_sess LIKE g_sess
    INDEX bcomstr_merge_sess IS PRIMARY bcomstr_merge_sess ASC.
...
PROCEDURE main:
    ...
    FIND FIRST bd_merge_list WHERE bd_merge_sess = g_sess NO-LOCK NO-ERROR.
     ...
     CREATE bcomstr_merge.
     BUFFER-COPY b_co_mstr TO bcomstr_merge
         ASSIGN
            bcomstr_merge_sess = g_sess.
   FOR EACH bd_merge_list  WHERE bd_merge_sess = g_sess NO-LOCK:
    ...
    END.
    FIND FIRST bcomstr_merge WHERE bcomstr_merge.b_co_code = bc_merge_destid AND bcomstr_merge_sess = g_sess EXCLUSIVE-LOCK NO-ERROR.

    CREATE b_co_mstr.
                   BUFFER-COPY
                       bcomstr_merge 
                       EXCEPT bcomstr_merge.b_co_code bcomstr_merge.b_co_qty_cur  bcomstr_merge.b_co_btype bcomstr_merge.b_co_userid bcomstr_merge_sess
                       TO
                        b_co_mstr
  ...
  OPEN QUERY bc_qry FOR EACH bd_merge_list  WHERE bd_merge_sess = g_sess NO-LOCK.
      mdesc = b_co_mstr.b_co_desc2 + b_co_mstr.b_co_desc1.
      {bclabel.i ""zpl"" ""part"" "b_co_mstr.b_co_code" "b_co_mstr.b_co_part" 
     "b_co_mstr.b_co_lot" "b_co_mstr.b_co_ref" "b_co_mstr.b_co_qty_cur" "b_co_mstr.b_co_vend" "mdesc" }
