...
PROCEDURE main:
 ...
 FOR EACH loc_mstr WHERE loc_site >= bc_site AND loc_site <= bc_site1 AND loc_loc >= bc_loc AND loc_loc <= bc_loc1 NO-LOCK:

    
     blocid = trim(loc_site) + '.' + TRIM(loc_loc).
    
  
 {bclabel.i ""zpl"" ""loc"" "blocid" "loc_site" 
     "loc_loc" """" """" """" """"  }
