...
{bcrun.i ""bcmgcheck.p"" "(input ""ictr_typ"" ,
    ...
    INPUT """",
        output success)"}      
IF NOT success THEN LEAVE.
    /*FIND LAST tr_hist USE-INDEX tr_date_trn WHERE tr_date >= TODAY - 2 AND  (tr_type = 'cyc-err' OR tr_type = 'cyc-cnt') AND tr_program = 'icccaj.p' AND tr_site = bc_site AND tr_loc = bc_loc AND /*AND tr_nbr = bc_po_nbr AND string(tr_line) = bc_po_line*/ tr_part = b_co_part AND tr_serial = b_co_lot   NO-LOCK NO-ERROR.
       mqty = (IF AVAILABLE tr_hist THEN tr_qty_req ELSE 0) + (IF b_co_cntst = '' THEN b_co_qty_cur ELSE 0).
 */


    mqty = 0.
    FOR EACH b_co_mstr USE-INDEX b_co_lot WHERE b_co_part = bc_part AND b_co_lot = bc_lot AND b_co_site = bc_site AND b_co_loc = bc_loc AND b_co_cntst = 'i' AND b_co_status <> 'ia' NO-LOCK:
     mqty = mqty + b_co_qty_cur.
    END.
 FIND FIRST b_co_mstr WHERE b_co_code = bc_id EXCLUSIVE-LOCK NO-ERROR.
 IF b_co_cntst = '' OR b_co_cntst = 'r' OR (b_co_cntst = 'i' AND (b_co_status = 'ac' OR b_co_status = 'iss' OR b_co_status = 'issln') AND (b_co_site <> bc_site OR b_co_loc <> bc_loc))  THEN mqty = mqty + b_co_qty_cur.
             ...
             LEAVE.
              ...
              IF b_co_status = 'ac' OR b_co_status = 'iss' OR b_co_status = 'issln' THEN
                     ASSIGN
                        b_co_site = bc_site
                        b_co_loc = bc_loc.
      IF tr_type = 'cyc-cnt' THEN 
          FOR EACH b_co_mstr USE-INDEX b_co_lot WHERE b_co_part = bc_part AND b_co_lot = bc_lot AND b_co_site = bc_site AND b_co_loc = bc_loc AND b_co_cntst = 'i' AND (b_co_status = 'ac' OR b_co_status = 'iss' OR b_co_status = 'issln') EXCLUSIVE-LOCK:
