...
DEF FRAME bc
  ...
  bc_button AT ROW 8 COL 10
    ...
    WITH SIZE 30 BY 12 TITLE "盘点单打印"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
...
END.*/
  ...
  /*  id = SUBSTR(string(YEAR(TODAY),'9999'),3,2) + STRING(MONTH(TODAY),'99') + STRING(DAY(TODAY),'99') + string(NEXT-VALUE(b_cnt_sq01),'999999999999').
    */
    ...
    FIND FIRST b_cnt_wkfl NO-LOCK NO-ERROR.
    IF AVAILABLE b_cnt_wkfl THEN DO:
        MESSAGE '要盘点清单初始化打印，还是重打？' VIEW-AS ALERT-BOX
    ...
    FOR EACH b_ld_det WHERE  b_ld_site >= bc_site AND b_ld_site <= bc_site1 AND 
        b_ld_loc >= bc_loc AND b_ld_loc <= bc_loc1 NO-LOCK:
        FIND FIRST b_cnt_wkfl WHERE  b_cnt_part = b_ld_part AND b_cnt_site = b_ld_site AND b_cnt_loc = b_ld_loc AND b_cnt_lot = b_ld_lot EXCLUSIVE-LOCK NO-ERROR.
          IF AVAILABLE b_cnt_wkfl  THEN DO:
