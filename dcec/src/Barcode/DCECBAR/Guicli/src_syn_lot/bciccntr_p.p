...
DEF FRAME bc
  ...
  /* bc_pkqty AT ROW 10 COL 4*/
    ...
    WITH SIZE 30 BY 12 TITLE "静态复盘扫描"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
VIEW c-win.
...
DO:
   ASSIGN bc_tag.
   FIND FIRST tag_mstr WHERE string(tag_nbr) = bc_tag AND tag_cnt_dt <> ? AND NOT tag_post AND (IF tag_site <> '' OR tag_loc <> '' THEN tag_site = bc_site AND tag_loc = bc_loc ELSE YES)EXCLUSIVE-LOCK NO-ERROR.
   IF NOT AVAILABLE tag_mstr THEN DO:
       MESSAGE '无效标签，或未作静态初盘，或标签库位不匹配，或已更新！' VIEW-AS ALERT-BOX ERROR.
       UNDO,RETRY.
   END.
   ELSE DO:
       mqty = tag_rcnt_qty.
...
DO:
      ...
      ELSE DO:
         IF ((mpart <> ''  OR mlot <> '') 
                AND (mpart <> b_co_part  OR mlot <> b_co_lot)) OR substr(b_co_cntst,3) <> bc_tag  THEN DO:
                    MESSAGE '该盘点标签与条码信息不匹配，或与条码静止初盘标签不一致！' VIEW-AS ALERT-BOX ERROR.
                    ASSIGN bc_id = ''.
              DISP bc_id WITH FRAME bc.
...
PROCEDURE main:
 ...
 OUTPUT TO VALUE(g_sess). 
           ...
           put  "@@BATCHLOAD pitcmt2.P" skip.
            PUT UNFORMAT '"' bc_tag '"' SKIP .
          
            IF mtype = 'B' THEN
            PUT UNFORMAT '"' bc_site '" "'  bc_loc '" "' b_co_part '" "' b_co_lot '"'  SKIP.
            PUT UNFORMAT STRING(IF msite = bc_site AND mloc = bc_loc AND mpart = b_co_part AND mlot = b_co_lot THEN (IF substr(b_co_cntst,1,2) = 'ip' THEN (mqty + b_co_qty_cur) ELSE mqty) ELSE b_co_qty_cur)   SKIP.
             ...
             {bcrun.i ""bcmgbdpro.p"" "(INPUT g_sess,INPUT ""out.txt"")"}
              ...
              OS-DELETE VALUE('out.txt').
                  ...
                  IF msite = tag_site AND mloc = tag_loc AND mpart = tag_part AND mlot = tag_serial  THEN DO:
                      IF tag_rcnt_qty <> mqty + (IF substr(b_co_cntst,1,2) = 'ip' THEN b_co_qty_cur ELSE 0) THEN   DO:  
                          MESSAGE  bc_site ' ' bc_loc ' ' b_co_part  b_co_lot ' 更新QAD失败！'  VIEW-AS ALERT-BOX ERROR.
                        LEAVE.
                      END.
                  END.
                  ELSE 
                      IF tag_rcnt_qty <> b_co_qty_cur THEN  DO:  
                           MESSAGE  bc_site ' ' bc_loc ' ' b_co_part  b_co_lot ' 更新QAD失败！'  VIEW-AS ALERT-BOX ERROR.
