...
DEF VAR bc_site AS CHAR FORMAT "x(8)" LABEL "移出地点".
DEF VAR bc_loc AS CHAR FORMAT "x(8)" LABEL "移出库位".
DEF VAR bc_lot AS CHAR FORMAT "x(18)" LABEL "批/序号".
DEF VAR bc_site1 AS CHAR FORMAT "x(18)" LABEL "移入地点".
DEF VAR bc_loc1 AS CHAR FORMAT "x(8)" LABEL "移入库位". 
...
DEF VAR bc_lot1 AS CHAR  FORMAT "x(18)" LABEL "批/序号".
DEF FRAME bc
  ...
  /* bc_pkqty AT ROW 10 COL 4*/
    bc_site AT ROW 6 COL 1

    bc_loc AT ROW 7.2 COL 1
    bc_lot1 AT ROW 8.4 COL 1.6
    bc_site1 AT ROW 9.6 COL 1
   bc_loc1 AT ROW 10.8 COL 1
 bc_so_job1 AT ROW 12 COL 2.5
    bc_nbr AT ROW 13.2 COL 2.5
    WITH SIZE 30 BY 15 TITLE "移库-带批/序号变更"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
      ...
      ELSE DO:
              ...
              IF AVAILABLE b_co_mstr THEN DO:
                 ...
                 bc_ref = b_co_ref.
                  ...
                  bc_loc = b_co_loc.
                  bc_lot1 = bc_lot.
                  DISP bc_part bc_lot bc_site bc_loc bc_lot1 /*bc_ref*/ bc_qty WITH FRAME bc.
                   {bccntlock.i "bc_site" "bc_loc"}
                  ENABLE bc_lot1 WITH FRAME bc.
                  END.
...
DO:
   ...
   IF bc_site = bc_site1 AND bc_loc = bc_loc1  AND bc_lot = bc_lot1 THEN DO:
...
IF NOT success THEN LEAVE.
    ...
    FIND FIRST b_co_mstr WHERE b_co_code = bc_id EXCLUSIVE-LOCK NO-ERROR.
     OUTPUT TO value(g_sess).
            ...
            PUT UNFORMAT STRING(b_co_qty_cur) ' ' string(eff_date) ' "' bc_nbr '" "' bc_so_job1 '"'  skip.
            PUT 'Y T Y - ' SKIP.
            PUT UNFORMAT '"' + b_co_site + '" "' + b_co_loc + '"' + ' "' b_co_lot '"'  SKIP.
            PUT UNFORMAT '"' bc_site1 '" "' bc_loc1 '"' +  ' "' bc_lot1 '"' SKIP.
          PUT      SKIP(2)
                     "." SKIP
                     "@@END" SKIP.
          OUTPUT CLOSE.
   ...
   /* MESSAGE '业务操作成功！' VIEW-AS ALERT-BOX INFORMATION. */
    {bcrelease.i}
      ...
      bc_qty = 0
        ...
        bc_loc = ''
        bc_loc1 = ''*/.
  DISP bc_id bc_part bc_lot bc_qty bc_lot1 bc_site bc_site1 bc_loc bc_loc1 WITH FRAME bc.
