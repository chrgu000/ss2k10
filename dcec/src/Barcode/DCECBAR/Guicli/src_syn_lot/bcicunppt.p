...
DEF VAR bc_nbr AS CHAR FORMAT 'x(18)' LABEL '定制码'.
DEF VAR bc_nbr1 AS CHAR FORMAT 'x(18)' LABEL '至'.
DEF VAR bc_job AS CHAR FORMAT 'x(8)' LABEL '预算号'.
DEF VAR bc_job1 AS CHAR FORMAT 'x(8)' LABEL '至'.
...
DEF VAR bc_type AS CHAR FORMAT 'x(8)' LABEL '类型'.
...
DEF FRAME bc
   ...
   bc_button AT ROW 16.8 COL 10
    ...
    WITH SIZE 30 BY 18.5 TITLE "计划外出库备料单打印"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
...
rowstart = 1.
     ...
     chWorkSheet = chExcelApplication:Sheets:Item(mcnt).      
       ...
       chExcelApplication:Sheets:Item(mcnt):NAME = "计划外出入库备料单-" + SUBSTR(b_cnt_site,2,9).
 ...
 chExcelApplication:selection:Font:bold = TRUE.
  chWorkSheet:Range("a1:a1"):VALUE = "定制码".   
  chWorkSheet:Range("b1:b1"):VALUE = "预算号".      
 chWorkSheet:Range("c1:c1"):VALUE = "地点".
       chWorkSheet:Range("d1:d1"):VALUE = "库位".
    ...
    chWorkSheet:Range("i1:i1"):VALUE = "欠交数".
          ...
          chWorkSheet:Range("j1:j1"):VALUE = "条码".
          chWorkSheet:Range("k1:k1"):VALUE = "批号".
          chWorkSheet:Range("l1:l1"):VALUE = "条码数量".
    mcnt = mcnt + 1.
END.
