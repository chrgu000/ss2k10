...
DEF TEMP-TABLE po1ia_tmp
    FIELD po1ia_nbr LIKE pod_nbr
    FIELD po1ia_line LIKE pod_line
    FIELD po1ia_part LIKE pod_part
    FIELD po1ia_site LIKE pod_site
    FIELD po1ia_loc LIKE pod_loc
    FIELD po1ia_qty LIKE pod_qty_ord
    FIELD po1ia_lot LIKE b_co_lot
   
    FIELD po1ia_sess LIKE g_sess
    INDEX po1ia_sess IS PRIMARY po1ia_sess ASC.
DEF TEMP-TABLE bid1ia_tmp
    FIELD bid1ia_id LIKE b_co_code
    FIELD bid1ia_lot LIKE b_co_lot
    FIELD bid1ia_par_id LIKE b_co_code
    FIELD bid1ia_line LIKE pod_line
    FIELD bid1ia_sess LIKE g_sess
    INDEX bid1ia_sess IS PRIMARY bid1ia_sess bid1ia_line bid1ia_par_id ASC.
...
DEF FRAME bc
 ...
 /* bc_po_part AT ROW 3.6 COL 2.5*/
   ...
   /* bc_ref AT ROW 9.6 COL 2.5*/
    ...
    po1ia_qty AT ROW 10.8 COL 18 NO-LABEL
...
DO:
    FIND FIRST po1ia_tmp WHERE po1ia_sess = g_sess AND po1ia_nbr <> bc_po_nbr NO-LOCK NO-ERROR.
   IF AVAILABLE po1ia_tmp THEN do:
...
DO:
     ...
     bc_loc = ''.
       ...
       FIND FIRST bid1ia_tmp WHERE bid1ia_sess = g_sess AND bid1ia_par_id = bc_id NO-LOCK  NO-ERROR.
       IF AVAILABLE bid1ia_tmp THEN DO:
       ...
       FIND FIRST b_co_mstr WHERE b_co_code = bc_id  /*AND b_co_ref = ''*/ EXCLUSIVE-LOCK NO-ERROR.
           IF AVAILABLE b_co_mstr AND b_co_ref = '' AND b_co_status = 'ia' THEN DO:
               bc_part = b_co_part.
               bc_lot = b_co_lot.
                bc_qty = b_co_qty_cur.
               mqty = 0.
         ...
         ELSE DO:
             bc_po_line = lntyp.
              ...
              /*bc_part = b_co_part.
                  bc_qty = b_co_qty_cur.
                  bc_lot = b_co_lot.*/
               FOR EACH bid1ia_tmp WHERE bid1ia_sess = g_sess AND bid1ia_par_id = bc_id:
                   bid1ia_line = INTEGER(bc_po_line).
                   END.
          FIND FIRST po1ia_tmp WHERE po1ia_sess = g_sess AND po1ia_nbr = bc_po_nbr AND po1ia_line = INTEGER(bc_po_line) AND po1ia_part = bc_part AND po1ia_lot <> bc_lot NO-LOCK NO-ERROR.
      IF AVAILABLE po1ia_tmp THEN DO:
    ...
    ELSE DO:
        ...
        input bc_po_line,
         input """", 
        input STRING((IF AVAILABLE po1ia_tmp THEN po1ia_qty ELSE 0) + bc_qty),
         input """",
        output success)"}
            IF NOT success THEN  DO:
                bc_id = ''.
...
DO:  
     ...
     ON MOUSE-SELECT-CLICK	 OF bc_button
DO:
...
PROCEDURE barscan:
    ...
    FIND FIRST po1ia_tmp WHERE po1ia_sess = g_sess AND po1ia_nbr = bc_po_nbr AND po1ia_line = INTEGER(bc_po_line) AND po1ia_part = bc_part AND po1ia_lot = bc_lot EXCLUSIVE-LOCK NO-ERROR.
     
           
    IF AVAILABLE po1ia_tmp THEN po1ia_qty = po1ia_qty + bc_qty.
    ELSE DO:
   
    CREATE po1ia_tmp.
    ASSIGN po1ia_sess = g_sess
          po1ia_nbr = bc_po_nbr
          po1ia_line = INTEGER(bc_po_line)
        po1ia_site = bc_site
        po1ia_loc = bc_loc
         po1ia_part = bc_part
        po1ia_lot = bc_lot
    ...
    bc_id = ''.
    DISP bc_id po1ia_qty WITH FRAME bc.
...
PROCEDURE main:
     ...
     FOR EACH po1ia_tmp WHERE po1ia_sess = g_sess NO-LOCK:
       
      
          
     FIND FIRST pod_det WHERE pod_nbr = po1ia_nbr AND pod_line = po1ia_line NO-LOCK NO-ERROR.
     IF pod_site <> po1ia_site THEN DO:
...
END.
        /*   FIND FIRST b_co_mstr WHERE b_co_code = bc_id EXCLUSIVE-LOCK NO-ERROR.  
               */
               FIND FIRST pt_mstr WHERE pt_part = po1ia_part NO-LOCK NO-ERROR.
          ...
          PUT UNFORMAT string(po1ia_line) SKIP.
          PUT UNFORMAT string(po1ia_qty)  ' - - ' pt_um ' - - "' po1ia_site '" "' po1ia_loc '"' ' "' po1ia_lot '"'  SKIP.
          ...
          mdate = TODAY.
           ...
           FIND LAST tr_hist USE-INDEX tr_date_trn WHERE tr_date >= mdate AND tr_date <= TODAY AND tr_type = 'rct-po' AND tr_program = 'poporc.p' AND tr_nbr = po1ia_nbr AND tr_line = po1ia_line AND tr_serial = po1ia_lot AND tr_site =  po1ia_site AND tr_loc =  po1ia_loc  AND tr_qty_loc = po1ia_qty AND tr_userid = g_user AND tr_time >= mtime AND tr_time <= TIME  NO-LOCK NO-ERROR.
        ...
        FIND FIRST po_mstr WHERE po_nbr = po1ia_nbr NO-LOCK NO-ERROR.
        FIND FIRST pod_det WHERE pod_nbr = po_nbr NO-LOCK NO-ERROR.
      

          FIND FIRST b_po_wkfl WHERE b_po_due_date  =  (IF issch THEN TODAY ELSE pod_due_date) AND b_po_nbr = po1ia_nbr AND b_po_line = string(po1ia_line)  AND b_po_part = po1ia_part   EXCLUSIVE-LOCK NO-ERROR.
          IF AVAILABLE b_po_wkfl THEN
                  ASSIGN
                      b_po_qty_rec = b_po_qty_rec + po1ia_qty.
          FIND FIRST b_po_rec WHERE b_po_recdate = (IF issch THEN TODAY ELSE pod_due_date) AND b_po_ponbr = po1ia_nbr AND b_po_poln = string(po1ia_line) AND b_po_popart = po1ia_part EXCLUSIVE-LOCK NO-ERROR.
                   IF AVAILABLE b_po_rec THEN
                        ASSIGN
                           b_po_recqty = b_po_recqty + po1ia_qty.
                     ELSE DO:
                         CREATE b_po_rec.
                         ASSIGN
                             b_po_recdate = IF issch THEN TODAY ELSE pod_due_date
                             b_po_ponbr = po1ia_nbr
                              b_po_poln = STRING(po1ia_line)
                              b_po_popart = po1ia_part
                              b_po_recqty =  po1ia_qty  + (IF issch THEN 0 ELSE (IF AVAILABLE pod_det THEN (pod_qty_rcvd * IF pod_um_conv <> 0 THEN pod_um_conv ELSE 1  /*- pod_qty_rtn*/) ELSE 0)) .
                     END.
        ...
        FOR EACH bid1ia_tmp WHERE bid1ia_sess = g_sess AND bid1ia_line = po1ia_line AND bid1ia_lot = po1ia_lot:
       FIND FIRST b_co_mstr WHERE b_co_code = bid1ia_id EXCLUSIVE-LOCK NO-ERROR.
     ...
     FOR EACH bid1ia_tmp WHERE bid1ia_sess = g_sess:
         DELETE bid1ia_tmp.
     END.
     FOR EACH po1ia_tmp WHERE po1ia_sess = g_sess:
         DELETE po1ia_tmp.
     END.
     RELEASE b_po_wkfl.
             {bcrelease.i}
