...
DEF TEMP-TABLE po3_tmp
    FIELD po3_nbr LIKE pod_nbr
    FIELD po3_line LIKE pod_line
    FIELD po3_part LIKE pod_part
    FIELD po3_site LIKE pod_site
    FIELD po3_loc LIKE pod_loc
    FIELD po3_qty LIKE pod_qty_ord
    FIELD po3_lot LIKE b_co_lot
   
    FIELD po3_sess LIKE g_sess
    INDEX po3_sess IS PRIMARY po3_sess ASC.
DEF TEMP-TABLE bid3_tmp
    FIELD bid3_id LIKE b_co_code
    FIELD bid3_lot LIKE b_co_lot
    FIELD bid3_par_id LIKE b_co_code
    FIELD bid3_line LIKE pod_line
    FIELD bid3_sess LIKE g_sess
    INDEX bid3_sess IS PRIMARY bid3_sess bid3_line bid3_par_id bid3_id ASC.
...
DEF FRAME bc
 ...
 /* bc_po_part AT ROW 3.6 COL 2.5*/
   ...
   /* bc_ref AT ROW 9.6 COL 2.5*/
    ...
    po3_qty AT ROW 10.8 COL 18 NO-LABEL
...
DO:
    FIND FIRST po3_tmp WHERE po3_sess = g_sess AND po3_nbr <> bc_po_nbr NO-LOCK NO-ERROR.
   IF AVAILABLE po3_tmp THEN do:
...
DO:
     ...
     bc_loc = ''.
       ...
       FIND FIRST b_co_mstr WHERE b_co_code = bc_id AND NOT can-find(FIRST bid3_tmp WHERE bid3_sess = g_sess AND IF bid3_par_id <> '' THEN bid3_par_id = substr(b_co_btype,2) ELSE NO NO-LOCK) /*AND b_co_ref = ''*/ EXCLUSIVE-LOCK NO-ERROR.
      IF AVAILABLE b_co_mstr AND b_co_ref = ''  THEN DO:
          mtype = IF b_co_status = 'po' THEN b_co_status ELSE ''.
       FIND FIRST bid3_tmp WHERE bid3_sess = g_sess AND (IF mtype = 'po' THEN bid3_par_id = bc_id ELSE bid3_id = bc_id) NO-LOCK  NO-ERROR.
       IF AVAILABLE bid3_tmp THEN DO:
         ...
         ELSE DO:
             bc_po_line = lntyp.
              ...
              /*bc_part = b_co_part.
                  ...
                  bc_lot = b_co_lot.*/
              IF mtype = 'po' THEN
              FOR EACH bid3_tmp WHERE bid3_sess = g_sess AND   bid3_par_id = bc_id:
                   bid3_line = INTEGER(bc_po_line).
                   END.
          FIND FIRST po3_tmp WHERE po3_sess = g_sess AND po3_nbr = bc_po_nbr AND po3_line = INTEGER(bc_po_line) AND po3_part = bc_part AND po3_lot <> bc_lot NO-LOCK NO-ERROR.
      IF AVAILABLE po3_tmp THEN DO:
    ...
    ELSE DO:
        ...
        input bc_po_line,
         input """", 
        input STRING((IF AVAILABLE po3_tmp THEN po3_qty ELSE 0) + bc_qty),
         input """",
        output success)"}
            IF NOT success THEN  DO:
                bc_id = ''.
...
PROCEDURE barscan:
 IF mtype <> 'po' THEN DO:
    ...
    ASSIGN bid3_sess = g_sess
        bid3_id = bc_id
           ...
           bid3_line = INTEGER(bc_po_line).
             ...
             END.
    FIND FIRST po3_tmp WHERE po3_sess = g_sess AND po3_nbr = bc_po_nbr AND po3_line = INTEGER(bc_po_line) AND po3_part = bc_part AND po3_lot = bc_lot EXCLUSIVE-LOCK NO-ERROR.
     
           
    IF AVAILABLE po3_tmp THEN po3_qty = po3_qty + bc_qty.
    ELSE DO:
   
    CREATE po3_tmp.
    ASSIGN po3_sess = g_sess
          po3_nbr = bc_po_nbr
          po3_line = INTEGER(bc_po_line)
        po3_site = bc_site
        po3_loc = bc_loc
         po3_part = bc_part
        po3_lot = bc_lot
    ...
    bc_id = ''.
    DISP bc_id po3_qty WITH FRAME bc.
...
PROCEDURE main:
     ...
     FOR EACH po3_tmp WHERE po3_sess = g_sess NO-LOCK:
       
      
          
     FIND FIRST pod_det WHERE pod_nbr = po3_nbr AND pod_line = po3_line NO-LOCK NO-ERROR.
     IF pod_site <> po3_site THEN DO:
...
END.
        /*   FIND FIRST b_co_mstr WHERE b_co_code = bc_id EXCLUSIVE-LOCK NO-ERROR.  
               */
               FIND FIRST pt_mstr WHERE pt_part = po3_part NO-LOCK NO-ERROR.
          ...
          PUT UNFORMAT string(po3_line) SKIP.
          PUT UNFORMAT string(po3_qty)  ' - - ' pt_um ' - - "' po3_site '" "' po3_loc '"' ' "' po3_lot '"'  SKIP.
          ...
          mdate = TODAY.
           ...
           FIND LAST tr_hist USE-INDEX tr_date_trn WHERE tr_date >= mdate AND tr_date <= TODAY AND tr_type = 'rct-po' AND tr_program = 'poporc.p' AND tr_nbr = po3_nbr AND tr_line = po3_line AND tr_serial = po3_lot AND tr_site =  po3_site AND tr_loc =  po3_loc  AND tr_qty_loc = po3_qty AND tr_userid = g_user AND tr_time >= mtime AND tr_time <= TIME  NO-LOCK NO-ERROR.
        ...
        FIND FIRST po_mstr WHERE po_nbr = po3_nbr NO-LOCK NO-ERROR.
        FIND FIRST pod_det WHERE pod_nbr = po_nbr NO-LOCK NO-ERROR.
      

          FIND FIRST b_po_wkfl WHERE b_po_due_date  =  (IF issch THEN TODAY ELSE pod_due_date) AND b_po_nbr = po3_nbr AND b_po_line = string(po3_line)  AND b_po_part = po3_part   EXCLUSIVE-LOCK NO-ERROR.
          IF AVAILABLE b_po_wkfl THEN
                  ASSIGN
                      b_po_qty_rec = b_po_qty_rec + po3_qty.
          FIND FIRST b_po_rec WHERE b_po_recdate = (IF issch THEN TODAY ELSE pod_due_date) AND b_po_ponbr = po3_nbr AND b_po_poln = string(po3_line) AND b_po_popart = po3_part EXCLUSIVE-LOCK NO-ERROR.
                   IF AVAILABLE b_po_rec THEN
                        ASSIGN
                           b_po_recqty = b_po_recqty + po3_qty.
                     ELSE DO:
                         CREATE b_po_rec.
                         ASSIGN
                             b_po_recdate = IF issch THEN TODAY ELSE pod_due_date
                             b_po_ponbr = po3_nbr
                              b_po_poln = STRING(po3_line)
                              b_po_popart = po3_part
                              b_po_recqty =  po3_qty  + (IF issch THEN 0 ELSE (IF AVAILABLE pod_det THEN (pod_qty_rcvd * IF pod_um_conv <> 0 THEN pod_um_conv ELSE 1  /*- pod_qty_rtn*/) ELSE 0)) .
                     END.
            ...
            /* ELSE DO:
             ...
             END.*/
                  ...
                  ismodi = NO.
        FOR EACH bid3_tmp WHERE bid3_sess = g_sess AND bid3_line = po3_line AND bid3_lot = po3_lot:
       FIND FIRST b_co_mstr WHERE b_co_code = bid3_id EXCLUSIVE-LOCK NO-ERROR.
     ...
     IF AVAILABLE b_co_mstr THEN
       assign
            ...
            /* b_co_vend =  po_vend */
            b_co_ord = po_nbr
            b_co_line = string(po3_line)
            b_co_due_date = IF issch THEN TODAY ELSE pod_due_date
       /*b_co_qty_req = bc_po_qty*/.
             IF mtype = 'po'  /*NOT ismodi*/ THEN DO: 
               FIND FIRST b_co_mstr WHERE b_co_code = bid3_par_id EXCLUSIVE-LOCK NO-ERROR.
     ...
     FOR EACH bid3_tmp WHERE bid3_sess = g_sess:
         DELETE bid3_tmp.
     END.
     FOR EACH po3_tmp WHERE po3_sess = g_sess:
         DELETE po3_tmp.
     END.
     RELEASE b_po_wkfl.
             {bcrelease.i}
