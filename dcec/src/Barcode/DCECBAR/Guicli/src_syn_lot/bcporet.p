...
{bcrun.i ""bcmgcheck.p"" "(input ""ictr_typ"" ,
    ...
    INPUT """",
        output success)"}      
IF NOT success THEN LEAVE.*/
          MESSAGE '地点不一致，不能退货！' VIEW-AS ALERT-BOX ERROR.
         LEAVE.
     END.
  ELSE DO:
  {bcrun.i ""bcmgcheck.p"" "(input ""si_au"" ,
...
IF NOT success THEN LEAVE.
  END.
       ...
       PUT  UNFORMAT '- "' b_co_site '" - ' string(eff_date) ' N N N Y' SKIP.
             IF NOT po_fix_rate AND po_cur <> gl_base_curr THEN  PUT '- -' SKIP.
         PUT UNFORMAT string(bc_po_line) SKIP.
           PUT UNFORMAT string(b_co_qty_cur)  ' - ' pt_um ' - - "' b_co_site '" "' b_co_loc '" "' b_co_lot '" - - - N' SKIP.
           PUT '.' SKIP.
           PUT SKIP(3)
              "." SKIP
           "@@END" SKIP.
           ...
           FIND LAST tr_hist USE-INDEX tr_date_trn WHERE tr_date >= mdate AND tr_date <= TODAY AND tr_type = 'iss-prv' AND tr_program = 'porvis.p' AND tr_nbr = bc_po_nbr AND string(tr_line) = bc_po_line AND tr_serial = b_co_lot AND tr_site = (IF pod_site <> bc_site THEN pod_site ELSE bc_site) AND tr_loc = (IF pod_site <> bc_site THEN pod_loc ELSE bc_loc)  AND tr_qty_loc = (b_co_qty_cur * -1) AND tr_userid = g_user AND tr_time >= mtime AND tr_time <= TIME  NO-LOCK NO-ERROR.
             ...
             END.
               ELSE DO:
                ...
                MESSAGE '该料件不存在，或订单不存在！' VIEW-AS ALERT-BOX ERROR.
         ...
         assign
          ...
          FIND FIRST b_po_wkfl WHERE  b_po_due_date  =  b_co_due_date AND b_po_nbr = bc_po_nbr AND b_po_line = bc_po_line  AND b_po_part = b_co_part  EXCLUSIVE-LOCK NO-ERROR.
