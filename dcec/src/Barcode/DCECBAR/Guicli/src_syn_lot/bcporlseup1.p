...
DEF TEMP-TABLE b_po_chk
    ...
    INDEX b_po_g_sess IS PRIMARY b_po_g_sess ASC.
DEF FRAME bc
    bc_date AT ROW 1.2 COL 4
    bc_po_vend AT ROW 2.4 COL 2.5
    bc_po_vend1  AT ROW 3.6 COL 5.6
    bc_po_nbr AT ROW 4.8 COL 2.5
   
    bc_po_nbr1  AT ROW 6 COL 5.6
   
   bc_part AT ROW 7.2 COL 2.5
   /* bc_po_nbr AT ROW 6.5 COL 2.5*/
    bc_part1 AT ROW 8.4 COL 5.6
...
ENABLE bc_date bc_po_vend bc_po_vend1 bc_po_nbr bc_po_nbr1 bc_part bc_part1   bc_button WITH FRAME bc IN WINDOW c-win.
...
PROCEDURE main:
   ...
   where sch_type = 4   
   and sch_nbr = pod_nbr and sch_line = pod_line AND sch_rlse_id = pod_curr_rlse_id[1]:
    ...
    and schd_rlse_id = sch_rlse_id AND schd_date = bc_date:
             IF schd_discr_qty <> 0 THEN DO:
             
             FIND FIRST IN_mstr WHERE IN_part = pod_part AND IN_site = scx_shipto   NO-LOCK NO-ERROR.
     ...
     END.
      ...
      RELEASE b_po_wkfl.
      MESSAGE '送货单更新成功！' VIEW-AS ALERT-BOX.
