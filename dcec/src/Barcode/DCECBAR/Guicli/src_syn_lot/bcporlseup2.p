...
DEF TEMP-TABLE b_po_tmp
    ...
    INDEX b_sess IS PRIMARY b_sess ASC.
       ...
       DO WHILE chworksheet:range('a' + string(rowstart)):VALUE <> ?:
           ...
           crange = "a" + STRING(rowstart).  
               ...
               IF ERROR-STATUS:ERROR OR b_due_date = ? THEN DO:
                   MESSAGE chWorkSheet:Range(crange):VALUE ' 无效日期格式！' VIEW-AS ALERT-BOX ERROR.
                  success = NO.
                   LEAVE.
                   END.
             ...
             FIND FIRST pt_mstr WHERE pt_part = mpart  NO-LOCK NO-ERROR.
               IF NOT AVAILABLE pt_mstr THEN DO:
                   MESSAGE chWorkSheet:Range(crange):VALUE ' 无效零件！' VIEW-AS ALERT-BOX ERROR.
                   success = NO.
                   LEAVE.
                   END.
                b_part = pt_part.
             crange = "c" + STRING(rowstart).  
               ...
               IF ERROR-STATUS:ERROR  THEN DO:
                   MESSAGE chWorkSheet:Range(crange):VALUE ' 无效数字！' VIEW-AS ALERT-BOX ERROR.
                   success = NO.
                   LEAVE.
                   END.
                ...
                /*  mvend = SUBSTR(mvend,1,INDEX(mvend,'.') - 1).*/
                  ...
                  IF NOT AVAILABLE vd_mstr THEN DO:
                      MESSAGE chWorkSheet:Range(crange):VALUE ' 无效供应商！' VIEW-AS ALERT-BOX ERROR.
                      success = NO.
                      LEAVE.
                      END.
                      b_vend = vd_addr.
    ...
    END.
     ...
     FOR EACH b_po_tmp WHERE b_sess = g_sess NO-LOCK:
          ...
          FOR EACH b_co_mstr USE-INDEX b_co_sort6 WHERE b_co_ref <> '' AND b_co_shpmch = '' AND b_co_status = 'rct'  AND b_co_part = b_part AND b_co_vend = b_vend AND
              CAN-FIND(FIRST ABS_mstr USE-INDEX ABS_par_id WHERE abs_shipfrom = b_co_vend AND substr(ABS_par_id,2,50) = b_co_ref AND  abs_order = b_co_ord AND abs_line = b_co_line AND ABS_item = b_co_part /*AND ABS_lot = b_co_lot*/ AND ABS_site = b_co_site AND ABS_loc = b_co_loc NO-LOCK)    EXCLUSIVE-LOCK :
