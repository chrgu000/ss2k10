...
DEF VAR mid AS recID.
 DEF TEMP-TABLE rectr1_tmp 
     ...
     INDEX rectr1_sess IS PRIMARY rectr1_sess ASC.
...
DEF FRAME bc
   ...
   /* bc_poshp AT ROW 9.6 COL 2.5*/
    ...
    bc_site AT ROW 9.2 COL 4

    bc_loc AT ROW 10.4 COL 4 
  
   
    bc_button AT ROW 11.8 COL 10
...
DO:
      ...
      DISP bc_site bc_loc WITH FRAME bc.
        ...
        ELSE DO:
        MESSAGE '存在数量不一致，或地点库位空或相同！' VIEW-AS ALERT-BOX ERROR.
         ASSIGN bc_site = ''
                     bc_loc = ''.
               DISP bc_site bc_loc WITH FRAME bc.
               ENABLE bc_site WITH FRAME bc.
...
PROCEDURE main:
 ...
 FIND FIRST b_ct_ctrl NO-LOCK NO-ERROR.
    ...
    FOR EACH rectr1_tmp WHERE rectr1_sess = g_sess EXCLUSIVE-LOCK:
         {bccntlock.i "rectr1_site" "rectr1_loc"}
...
IF NOT success THEN LEAVE.
         ...
         FIND FIRST po_mstr WHERE po_nbr = rectr1_po_nbr NO-LOCK NO-ERROR.  
           ...
           */
            ...
            PUT UNFORMAT '"' rectr1_site1 '" "' rectr1_loc1 '"' +  ' "' b_co_lot '"' SKIP.
          PUT      SKIP(2)
                     "." SKIP
                     "@@END" SKIP.
          OUTPUT CLOSE.
      ...
      /*   {bctrcr.i
        ...
        DELETE rectr1_tmp. 
     END.
     ELSE  MESSAGE rectr1_code ' 未入外库，或已移库！' VIEW-AS ALERT-BOX ERROR.
    END.
       OPEN QUERY bc_qry FOR EACH rectr1_tmp WHERE rectr1_sess = g_sess NO-LOCK.
