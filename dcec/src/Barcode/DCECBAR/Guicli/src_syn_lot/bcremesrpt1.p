...
DEF VAR bc_time_h AS INT FORMAT '->99' LABEL '时间'.
DEF VAR bc_time_m AS INT FORMAT '->99'.
DEF VAR bc_time_s AS INT FORMAT '->99'.
DEF VAR bc_time1_h AS INT FORMAT '->99' LABEL '至'.
DEF VAR bc_time1_m AS INT FORMAT '->99'.
DEF VAR bc_time1_s AS INT FORMAT '->99'.
...
DEF FRAME bc
   bc_staff AT ROW 1.2 COL 2.5
    ...
    bc_date1 AT ROW 7.2 COL 5.5
        bc_time_h AT ROW 8.4 COL 4 ':' AT ROW 8.4 COL 12 bc_time_m AT ROW 8.4 COL 12.8 NO-LABEL  ':' AT ROW 8.4 COL 16.2 bc_time_s AT ROW 8.4 COL 17 NO-LABEL   
       bc_time1_h AT ROW 9.6 COL 5.5 ':' AT ROW 9.6 COL 12 bc_time1_m AT ROW 9.6 COL 12.8 NO-LABEL  ':' AT ROW 9.6 COL 16.2 bc_time1_s AT ROW 9.6 COL 17 NO-LABEL   
    bc_site AT ROW 10.8 COL 4
   bc_site1 AT ROW 12 COL 5.5 
   /* bc_part_desc  AT ROW 5 COL 1
    bc_part_desc1  NO-LABEL AT ROW 6 COL 8.5*/

   bc_loc AT ROW 13.2 COL 4
    bc_loc1 AT ROW 14.4 COL 5.5
  
    bc_part AT ROW 15.6 COL 2.5
    bc_part1 AT ROW 16.8 COL 5.5
    bc_vend AT ROW 18 COL 2.5
    bc_vend1 AT ROW 19.2 COL 5.5
   bc_button AT ROW 20.5 COL 10
    /*bc_button2 AT ROW 11 COL 16*/
    
    WITH SIZE 30 BY 22 TITLE "MES实发汇总报表"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.

ENABLE bc_staff bc_staff1 bc_nbr bc_nbr1 bc_date bc_date1 bc_time_h bc_time_m bc_time_s bc_time1_h bc_time1_m bc_time1_s  bc_site  bc_site1 bc_loc bc_loc1 bc_part bc_part1 bc_vend bc_vend1 bc_button /*bc_button2*/ WITH FRAME bc IN WINDOW c-win.
/*DISABLE bc_part_desc  bc_part_desc1 WITH FRAME bc .*/
 bc_date = TODAY.
 bc_date1 = TODAY.
 bc_time_h = integer(SUBstr(STRING(TIME,"hh:mm:ss"),1,2)).
  bc_time_m = integer(SUBstr(STRING(TIME,"hh:mm:ss"),4,2)).
   bc_time_s = integer(SUBstr(STRING(TIME,"hh:mm:ss"),7,2)).
 bc_time1_h = integer(SUBstr(STRING(TIME,"hh:mm:ss"),1,2)).
  bc_time1_m = integer(SUBstr(STRING(TIME,"hh:mm:ss"),4,2)).
   bc_time1_s = integer(SUBstr(STRING(TIME,"hh:mm:ss"),7,2)).
 
 DISP bc_date bc_date1 bc_time_h bc_time_m bc_time_s bc_time1_h bc_time1_m bc_time1_s WITH FRAME bc.
VIEW c-win.
...
ON VALUE-CHANGED OF bc_time_h
DO:
    ASSIGN bc_time_h.
    IF bc_time_h < 0 OR bc_time_h > 23 THEN DO:
        bc_time_h = 0.
       DISP bc_time_h WITH FRAME bc.
    END.
    
END.
ON VALUE-CHANGED OF bc_time_m
DO:
    ASSIGN bc_time_m.
    IF bc_time_m < 0 OR bc_time_m > 59 THEN DO:
        bc_time_m = 0.
        DISP bc_time_m WITH FRAME bc.
    END.
    
END.
ON VALUE-CHANGED OF bc_time_s
DO:
    ASSIGN bc_time_s.
    IF bc_time_s < 0 OR bc_time_s > 59 THEN DO:
        bc_time_s = 0.
        DISP bc_time_s WITH FRAME bc.
    END.
    
END.

ON VALUE-CHANGED OF bc_time1_h
DO:
    ASSIGN bc_time1_h.
    IF bc_time1_h < 0 OR bc_time1_h > 23 THEN DO:
        bc_time1_h = 0.
        DISP bc_time1_h WITH FRAME bc.
    END.
    
END.
ON VALUE-CHANGED OF bc_time1_m
DO:
    ASSIGN bc_time1_m.
    IF bc_time1_m < 0 OR bc_time1_m > 59 THEN DO:
        bc_time1_m = 0.
        DISP bc_time1_m WITH FRAME bc.
    END.
    
END.
ON VALUE-CHANGED OF bc_time1_s
DO:
    ASSIGN bc_time1_s.
    IF bc_time1_s < 0 OR bc_time1_s > 59 THEN DO:
        bc_time1_s = 0.
        DISP bc_time1_s WITH FRAME bc.
    END.
    
END.
ON enter OF bc_time_h
DO:
    APPLY 'entry':u TO bc_time1_h.
END.
ON enter OF bc_time_m
DO:
    APPLY 'entry':u TO bc_time1_h.
END.
ON enter OF bc_time_s
DO:
    APPLY 'entry':u TO bc_time1_h.
END.

ON enter OF bc_time1_h
DO:
    APPLY 'entry':u TO bc_site.
END.
ON enter OF bc_time1_m
DO:
    APPLY 'entry':u TO bc_site.
END.
ON enter OF bc_time_s
...
chWorkSheet = chExcelApplication:Sheets:Item(mcnt).      
       ...
       chExcelApplication:Sheets:Item(mcnt):NAME = "MES实发汇总单" .
        chWorkSheet:Rows(1):RowHeight = 25.
       chWorkSheet:Columns("A:i"):ColumnWidth = 15.
       chWorkSheet:Columns("h:h"):ColumnWidth = 35.
...
chWorkSheet:Range("a1:a1"):VALUE = '库管员'.
   chWorkSheet:Range("b1:b1"):VALUE = "移仓单".  
  chWorkSheet:Range("c1:c1"):VALUE = "日期范围". 
  chWorkSheet:Range("d1:d1"):VALUE = "时间范围". 
 chWorkSheet:Range("e1:e1"):VALUE = "地点".
       chWorkSheet:Range("f1:f1"):VALUE = "线边库位".
       chWorkSheet:Range("g1:g1"):VALUE = "零件号".
    
      chWorkSheet:Range("h1:h1"):VALUE = "零件描述".
     chWorkSheet:Range("i1:i1"):VALUE = "实发数".
    /* chWorkSheet:Range("i1:i1"):VALUE = "".
    chWorkSheet:Range("j1:j1"):VALUE = "欠交数".
    chWorkSheet:Range("k1:k1"):VALUE = "供应商".*/
     mcnt = 1.
         rowstart = 1.
 FOR EACH b_mes_wkfl WHERE b_mes_staff >= bc_staff AND b_mes_staff <= bc_staff1 AND b_mes_id >= bc_nbr AND b_mes_id <= bc_nbr1 /*AND b_mes_due_date >= bc_date AND b_mes_due_date <= bc_date1 */ AND b_mes_site >= bc_site AND b_mes_site <= bc_site1 AND b_mes_ln_loc >= bc_loc AND b_mes_ln_loc <= bc_loc1 AND b_mes_part >= bc_part AND b_mes_part <= bc_part1
   ...
   IF LAST-OF(b_mes_part) THEN DO:
           mqty = 0.
            rowstart = rowstart + 1.
        FOR EACH tr_hist USE-INDEX tr_date_trn WHERE tr_date >= bc_date AND tr_date <= bc_date1 AND tr_type = 'rct-tr' AND  tr_part = b_mes_part AND tr_site = b_mes_site AND tr_loc = b_mes_ln_loc AND tr_serial = '' 
            AND (string(year(tr_date),'9999') + '/' + string(MONTH(tr_date),'99') + '/' + string(DAY(tr_date),'99') + STRING(tr_time,'hh:mm:ss')) >= (string(year(bc_date),'9999') + '/' + string(MONTH(bc_date),'99') + '/' + string(DAY(bc_date),'99') + string(bc_time_h,'99') + ':' + STRING(bc_time_m,'99') + ':' + STRING(bc_time_s,'99'))
                AND (string(year(tr_date),'9999') + '/' + string(MONTH(tr_date),'99') + '/' + string(DAY(tr_date),'99') + STRING(tr_time,'hh:mm:ss')) <= (string(year(bc_date1),'9999') + '/' + string(MONTH(bc_date1),'99') + '/' + string(DAY(bc_date1),'99') + string(bc_time1_h,'99') + ':' + STRING(bc_time1_m,'99') + ':' + STRING(bc_time1_s,'99')) NO-LOCK:
            mqty = mqty + tr_qty_loc.
        END.
...
chWorkSheet:Rows(rowstart):RowHeight = 15. 
                   crange = "a" + STRING(rowstart) + ":"  + "i" + STRING(rowstart).
   ...
   chWorkSheet:Range(crange):VALUE = /*b_mes_id*/ ''.
     ...
     chWorkSheet:Range(crange):VALUE = string(bc_time_h,'99') + ':' + STRING(bc_time_m,'99') + ':' + STRING(bc_time_s,'99') + '-' + string(bc_time1_h,'99') + ':' + STRING(bc_time1_m,'99') + ':' + STRING(bc_time1_s,'99').
