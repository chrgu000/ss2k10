...
DO:
    ASSIGN bc_prod_line.
      FIND FIRST ln_mstr WHERE ln_site = bc_site AND  ln_line > bc_prod_line NO-LOCK NO-ERROR.
    IF AVAILABLE ln_mstr THEN DO:
        bc_prod_line = ln_line.
...
DO:
     ASSIGN bc_prod_line.
    FIND LAST ln_mstr WHERE ln_site = bc_site AND  ln_line < bc_prod_line NO-LOCK NO-ERROR.
    IF AVAILABLE ln_mstr THEN DO:
        bc_prod_line = ln_line.
        DISP bc_prod_line WITH FRAME bc.
    END.
...
DO:
    ...
    IF AVAILABLE ln_mstr THEN DO:
        ...
        FIND FIRST lnd_det WHERE lnd_line = ln_line AND lnd_part = bc_part NO-LOCK NO-ERROR.
    IF NOT AVAILABLE lnd_det THEN DO:
       MESSAGE '无效生产线，或不属于该地点该零件！' VIEW-AS ALERT-BOX ERROR.
       ASSIGN bc_prod_line = ''.
       DISP bc_prod_line WITH FRAME bc.
       UNDO,RETRY.
    END.
...
FIND FIRST b_co_mstr WHERE b_co_code = bc_id NO-LOCK NO-ERROR.
   ...
   FIND FIRST ptp_det WHERE ptp_part = b_co_part AND ptp_site = bc_site   NO-LOCK NO-ERROR.
     IF AVAILABLE ptp_det AND ptp_routing <> '' THEN rout = ptp_routing.
         ELSE
         DO:
             FIND FIRST pt_mstr WHERE pt_part = b_co_part NO-LOCK NO-ERROR.
             IF AVAILABLE pt_mstr THEN rout = IF pt_routing <> '' THEN pt_routing ELSE pt_part.
         END.
         mcnt = 0.
         FOR EACH ro_det WHERE ro_routing = rout NO-LOCK:
            mcnt = mcnt + 1.
        END.
            FIND LAST ro_det WHERE ro_routing = rout AND ro_milestone NO-LOCK NO-ERROR.
            ...
            */    
            ...
            PUT UNFORMAT '"' bc_emp '"' SKIP.
            PUT UNFORMAT string(eff_date) ' - "' bc_site '"' SKIP.
            PUT UNFORMAT '"' b_co_part '" ' lstop ' "' bc_prod_line  '"' SKIP.
            PUT '- - -' SKIP.
            PUT '- -' SKIP.
        /*  IF ro_op <> lstop  then
          ...
          ELSE*/
            IF mcnt <= 1 THEN  PUT UNFORMAT '- ' STRING(b_co_qty_cur) ' - - - - - - - - Y Y - -' SKIP.

                ELSE
                      PUT UNFORMAT '- ' STRING(b_co_qty_cur) ' - - - - - - - - - Y Y - -' SKIP.
        ...
        /* END.*/
         ...
         mtime = TIME.
          ...
          FIND LAST tr_hist USE-INDEX tr_date_trn WHERE tr_date >= today - 1 AND tr_type = 'rct-wo' AND tr_program = 'rebkfl.p' AND tr_part = b_co_part  AND tr_serial = b_co_lot AND tr_site =  bc_site AND tr_loc =  bc_loc  AND tr_qty_loc = b_co_qty_cur AND tr_userid = g_user AND tr_time >= mtime AND tr_time <= TIME  NO-LOCK NO-ERROR.
            IF  NOT AVAILABLE tr_hist THEN do:
             ...
             LEAVE.
              ...
              {bcrun.i ""bcrererctst.p"" "(input bc_id, input bc_site, input bc_loc)"}
