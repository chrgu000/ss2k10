...
DEF TEMP-TABLE b_rep_chk
    ...
    INDEX b_rep_g_sess IS PRIMARY b_rep_g_sess ASC.
DEF TEMP-TABLE b_rep_tmp 
    ...
    INDEX b_reptmp_sess IS PRIMARY b_reptmp_sess ASC.
DEF FRAME bc
...
PROCEDURE main:
   ...
   where rps_due_date = bc_date AND rps_site >= bc_site AND rps_site <= bc_site1  AND rps_part >= bc_part AND rps_part <= bc_part1:
        ...
        RUN bomexp(rps_part,rps_due_date,rps_site,rps_qty_req,rps_line,rps_part).

     END.
     FOR EACH b_rep_tmp WHERE b_reptmp_sess = g_sess EXCLUSIVE-LOCK:
     ...
     FOR EACH b_rep_wkfl  WHERE b_rep_due_date = bc_date AND b_rep_site >= bc_site AND b_rep_site <= bc_site1 AND substr(b_rep_line,10,18) >= bc_part AND substr(b_rep_line,10,18) <= bc_part1 
      ...
      END.
      RELEASE b_rep_wkfl.
      FOR EACH b_rep_chk WHERE b_rep_g_sess = g_sess EXCLUSIVE-LOCK:
          DELETE b_rep_chk.
          END.
      MESSAGE '自制件领料清单计算成功！' VIEW-AS ALERT-BOX.
               END.
...
PROCEDURE bomexp:
    ...
    DEF INPUT PARAMETER mdate AS DATE.
     ...
     DEF INPUT PARAMETER par_part LIKE rps_part.
     ...
     mult = 0.
     bomcode = mpart.
    FIND FIRST ptp_det WHERE ptp_part = mpart AND ptp_site = msite   /*AND ptp_bom_code <> ''*/ NO-LOCK NO-ERROR.
    
    IF AVAILABLE ptp_det THEN assign
        bomcode = IF ptp_bom_code <> ''  THEN ptp_bom_code ELSE mpart
        routing = IF ptp_routing <> '' THEN ptp_routing ELSE mpart.
   /* ELSE DO:*/
       FIND FIRST pt_mstr WHERE pt_part = mpart NO-LOCK NO-ERROR.
       IF AVAILABLE pt_mstr THEN
          assign
           bomcode = IF pt_bom_code <> '' AND bomcode = mpart THEN pt_bom_code ELSE pt_part
           routing = IF pt_routing <> ''  AND routing = mpart THEN pt_routing ELSE pt_part.
   /* END.*/
    ...
    FIND FIRST ptp_det WHERE ptp_part = ps_comp AND ptp_site = msite   NO-LOCK NO-ERROR.
         IF AVAILABLE ptp_det THEN  assign
             phantom = ptp_phantom
             mult = ptp_ord_mult.
    ...
    END.
        IF ps_ps_code = 'x' THEN
           ...
           RUN bomexp(ps_comp,mdate,msite,mqty * ps_qty_per,mline,par_part).
        ELSE DO:
       ...
       IF phantom  THEN  DO:
          ...
          IF NOT noexp THEN*/   RUN bomexp(ps_comp,mdate,msite,mqty * ps_qty_per,mline,par_part).
             END.
            ELSE DO:
              ...
              FIND FIRST ro_det WHERE ro_routing = routing AND  ro_op = ps_op AND (IF ro_start <> ? THEN ro_start <= mdate ELSE YES) AND (IF ro_end <> ? THEN ro_end >= mdate ELSE YES) EXCLUSIVE-LOCK NO-ERROR.
               FIND FIRST IN_mstr WHERE IN_part = ps_comp AND IN_site = msite   NO-LOCK NO-ERROR.             
                IF mult <> 0 THEN DO:
                           ...
                           ASSIGN
                               ...
                               b_reptmp_staff = IF AVAILABLE IN_mstr THEN IN__qadc01 ELSE ''
                                   b_reptmp_line = mline
                                   SUBSTR(b_reptmp_line,10,18) = par_part
                                   b_reptmp_sess = g_sess.
