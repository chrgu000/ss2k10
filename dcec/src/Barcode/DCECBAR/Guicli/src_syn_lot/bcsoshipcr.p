...
DEF VAR bc_ship AS CHAR  FORMAT "x(18)" LABEL "发运单".
 DEF TEMP-TABLE shiplist_tmp 
    ...
    FIELD ship_site LIKE bc_site
     ...
     FIELD ship_part_desc LIKE pt_desc1
     INDEX shiplist_sess IS PRIMARY shiplist_sess ship_loc ASC.
   ...
   /*  DEF TEMP-TABLE shipcode_tmp
    ...
    DISPLAY
        ...
        ship_loc COLUMN-LABEL "库位"
        
        ship_part_desc COLUMN-LABEL '零件描述'
          ...
          /* ASSIGN bc_so_nbr = sod_nbr
           ...
           IF AVAILABLE b_ex_sod THEN qty_iss = b_ex_issqty /*+ b_ex_all*/.
            ...
            IF sod_qty_ord * (IF sod_um_conv <> 0 THEN sod_um_conv ELSE 1) > qty_iss THEN DO:
                FIND FIRST ld_det WHERE ld_site = 'dcec-sv' AND ld_part = sod_part AND ld_qty_oh > 0 NO-LOCK NO-ERROR.
               ...
               ASSIGN 
                   ...
                   ship_so_qty_open = sod_qty_ord * (IF sod_um_conv <> 0 THEN sod_um_conv ELSE 1) - (IF AVAILABLE b_ex_sod THEN qty_iss ELSE sod_qty_ship * (IF sod_um_conv <> 0 THEN sod_um_conv ELSE 1))
                   ship_so_qty_req = sod_qty_ord * (IF sod_um_conv <> 0 THEN sod_um_conv ELSE 1)
                   ship_so_due_date = sod_due_date
                   ship_part_desc = IF AVAILABLE pt_mstr THEN pt_desc2 + pt_desc1 ELSE ''
                   ship_site = 'dcec-sv'
                  ship_loc = IF AVAILABLE ld_det THEN ld_loc ELSE ''
...
ENABLE bc_brw shipcode_brw  WITH FRAME bc.
FIND FIRST shiplist_tmp WHERE shiplist_sess = g_sess AND ship_so_qty_open <> 0 NO-LOCK NO-ERROR.
IF AVAILABLE shiplist_tmp THEN ENABLE bc_id WITH FRAME bc.
...
DO:
        ...
        input string(shiplist_tmp.ship_so_line),
         input """", 
        input """",
         input """",
        output success)"}
        ...
        UNDO,RETRY.
          END.
      ELSE DO: 
          IF b_co_part <> shiplist_tmp.ship_so_part OR b_co_qty_cur > (shiplist_tmp.ship_so_qty_open - shiplist_tmp.ship_so_qty_all)  THEN DO:
              MESSAGE  b_co_code ' 零件不匹配，或超过待发量！' VIEW-AS ALERT-BOX ERROR.
              ASSIGN bc_id = ''.
              DISP bc_id WITH FRAME bc.
              UNDO,RETRY.
          END.
         ...
         CREATE b_shp_wkfl.
           ...
           ELSE DO:
               ...
               ASSIGN
                   ...
                   b_ex_issqty = IF AVAILABLE sod_det THEN sod_qty_ship * (IF sod_um_conv <> 0 THEN sod_um_conv ELSE 1) ELSE 0.
           END.
