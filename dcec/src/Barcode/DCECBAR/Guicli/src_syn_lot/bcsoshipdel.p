...
DEF TEMP-TABLE b_shp_tmp
    ...
    INDEX shptmp_sess IS PRIMARY shptmp_sess ASC.
DEFINE QUERY bc_qry FOR b_shp_tmp.
       
    DEFINE BROWSE bc_brw  QUERY bc_qry
    DISPLAY
          b_shp_tmpid COLUMN-LABEL "销售订单" FORMAT "x(20)"
...
DEF FRAME bc
  ...
  /* bc_pkqty AT ROW 10 COL 4*/
   /* bc_qty_std AT ROW 8 COL 1*/
    ...
    WITH SIZE 30 BY 12 TITLE "销售订单备料删除"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
ismodi = NO.
...
DO:
   ...
   /* DISABLE bc_ship WITH FRAME bc.*/
    FOR EACH b_shp_tmp WHERE shptmp_sess = g_sess:
        DELETE b_shp_tmp.
    END.
