...
DEF VAR bc_offset AS INT.
 ...
 DEF TEMP-TABLE shipcode_tmp
     ...
     FIELD shipcode_id LIKE b_co_code
       ...
       INDEX shipcode_sess IS PRIMARY shipcode_sess ASC.
  ...
  {bcrun.i ""bcmgcheck.p"" "(input ""so"",
   ...
   bc_ship = bc_so_nbr.
    ...
    IF   AVAILABLE b_shp_wkfl THEN DO:
       FOR EACH shipcode_tmp WHERE shipcode_sess = g_sess:
           DELETE shipcode_tmp.
       END.
       ...
       OPEN QUERY bc_qry FOR EACH shipcode_tmp WHERE shipcode_sess = g_sess NO-LOCK.
           FOR EACH shipcode_tmp WHERE shipcode_sess = g_sess NO-LOCK:
               ...
               IF NOT AVAILABLE b_co_mstr THEN DO:
                   MESSAGE b_co_code ' 已出货，或盘亏，请将其从备料单删除！' VIEW-AS ALERT-BOX ERROR.
                   success =  NO.
                   LEAVE.
               END.
           END.
...
{bcrun.i ""bcmgcheck.p"" "(input ""ictr_typ"" ,
 ...
 {bcrun.i ""bcmgcheck.p"" "(input ""so_iss"" ,
      ...
      IF NOT success THEN LEAVE.
           END.
      ELSE DO: MESSAGE '存在条码已发货，或盘亏！' view-as ALERT-BOX ERROR.
      success = NO.
      END.
    END.
      IF  NOT success THEN LEAVE.
          ...
          {bcsodshplock.i "b_shp_so" "b_shp_line"}
           {bcrun.i ""bcmgcheck.p"" "(input ""so_iss"" ,
        input """",
        input """", 
        input """", 
     ...
     FIND FIRST b_co_mstr WHERE b_co_code = b_shp_code AND /* b_co_cntst = '' AND*/ (b_co_status = 'rct' OR b_co_status = 'all') EXCLUSIVE-LOCK NO-ERROR.  
          ...
          PUT UNFORMAT '"' b_shp_so '" ' string(eff_date) ' - - - -' SKIP.
         PUT UNFORMAT string(b_shp_line) ' -' SKIP.
         PUT UNFORMAT STRING(b_co_qty_cur / umconv) ' "' b_co_site '" "' b_co_loc '"' ' "' b_co_lot '"' SKIP.
         PUT '.' SKIP.
         PUT  SKIP(2).
        PUT '-' SKIP.
     PUT '-' SKIP.
     ...
     PUT '@@END' SKIP.
       ...
       b_shp_status = 'c'.
           END.
           ELSE MESSAGE b_shp_code ' 已发运，或盘亏！' VIEW-AS ALERT-BOX ERROR.
           {bcsodrelease.i}
    END.
