...
DEF FRAME bc
 ...
 /* bc_dss_part AT ROW 3.6 COL 2.5*/
   ...
   /* bc_ref AT ROW 9.6 COL 2.5*/
    ...
    WITH SIZE 30 BY 12 TITLE "备件转中间库"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
...
bc_drp_site = 'dcec-sv'.
DISP bc_drp_site WITH FRAME bc.
/*
bc_sub = NO.
...
DO:
   ...
   ELSE DO: 
       ...
       ENABLE bc_drp_site WITH FRAME bc.
 
   END.
  
    END.

ON enter OF bc_drp_site
DO:
    ASSIGN bc_drp_site.
    FIND FIRST si_mstr WHERE si_site = bc_drp_site NO-LOCK NO-ERROR.
    ...
    END.
    ELSE DO:
        DISABLE bc_drp_site WITH FRAME bc.
...
DO:
       ...
       APPLY 'entry':u TO bc_id.
           FIND FIRST b_co_mstr WHERE b_co_code = bc_id AND b_co_status = 'rct' AND substr(b_co_desc1,80,1) = ''  EXCLUSIVE-LOCK NO-ERROR.
           ...
           DO:
              MESSAGE '无效条码，或已发生该业务！' VIEW-AS ALERT-BOX ERROR.
               ...
               DO:
                   ...
                   FIND FIRST dsd_det WHERE dsd_nbr = bc_dss_nbr AND dsd_part = b_co_part AND dsd_site = bc_drp_site NO-LOCK NO-ERROR.
              IF NOT AVAILABLE dsd_det THEN
              DO:
                  MESSAGE '该订单无该零件，或发货地点不一致，或已关闭，或已取消！' VIEW-AS ALERT-BOX ERROR.
                  bc_id = ''.
                  DISP bc_id WITH FRAME bc.
                  UNDO,RETRY.
              END.
...
IF NOT success THEN LEAVE.
         ...
         FIND FIRST dss_mstr WHERE dss_nbr = bc_dss_nbr NO-LOCK NO-ERROR.
              ...
              PUT "@@batchload dsdois.p" SKIP.
          PUT UNFORMAT '"' bc_dss_nbr '" "' b_co_site '" "' bc_drp_site '" ' string(eff_date) ' no no' SKIP.
         PUT UNFORMAT '"' b_co_part '"' SKIP.
         PUT UNFORMAT STRING(b_co_qty_cur) ' no "' b_co_site '" "' b_co_loc '"' ' "' b_co_lot '"' SKIP.
         PUT '.' SKIP.
         PUT  SKIP(1).
     ...
     PUT '@@END' SKIP.
           ...
           IF  NOT AVAILABLE tr_hist THEN do:
              ...
              LEAVE.
               ...
               ASSIGN
                  ...
                  b_co_site = bc_drp_site
                   b_co_loc = mloc
                   SUBSTR(b_co_desc1,80,1) = 'd'
                   SUBSTR(b_co_desc1,82,8) = bc_site.
               {bcrun.i ""bcmgcheck.p"" "(input ""si_au"" ,
        input b_co_site,
        input """", 
        input """", 
        ...
        input """",
         input """",
        output success)"}   
     IF NOT success THEN undo,LEAVE.
...
IF NOT success THEN undo,LEAVE.   
          OUTPUT TO value(g_sess).
            ...
            PUT 'Y T Y - ' SKIP.
            PUT UNFORMAT '"' + b_co_site + '" "' + b_co_loc + '" "'  bc_lot '"' SKIP.
          ...
          mdate = TODAY.
             {bcrun.i ""bcmgbdpro.p"" "(INPUT g_sess,INPUT ""out.txt"")"}
                 OS-DELETE VALUE(g_sess).
              OS-DELETE VALUE('out.txt').
                 FIND LAST tr_hist USE-INDEX tr_date_trn WHERE tr_date >= mdate AND tr_date <= TODAY AND tr_type = 'iss-tr' AND tr_program = 'iclotr04.p' AND tr_site = b_co_site AND tr_loc = b_co_loc AND /*tr_nbr = bc_nbr AND tr_so_job = bc_so_job1 AND*/ tr_part = bc_part AND /*tr_serial = b_co_lot  AND*/ tr_qty_loc = (b_co_qty_cur * -1) AND tr_userid = g_user AND tr_time >= mtime AND tr_time <= TIME  NO-LOCK NO-ERROR.

             IF  NOT AVAILABLE tr_hist THEN do:
              MESSAGE 'QAD更新失败！' VIEW-AS ALERT-BOX ERROR.
              LEAVE.
               END.
