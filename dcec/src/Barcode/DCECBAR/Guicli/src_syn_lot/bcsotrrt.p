 ...
 /* bc_dss_part AT ROW 3.6 COL 2.5*/
   ...
   /* bc_ref AT ROW 9.6 COL 2.5*/
    ...
    bc_loc AT ROW 9.6 COL 4
     bc_site1 AT ROW 10.8 COL 4
    bc_loc1 AT ROW 12 COL 4
   
    WITH SIZE 30 BY 14 TITLE "备件转中间库"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
...
IF NOT success THEN LEAVE.
         ...
         FIND FIRST dss_mstr WHERE dss_nbr = bc_dss_nbr NO-LOCK NO-ERROR.
          ...
          PUT UNFORMAT '"' bc_dss_nbr '" "' bc_site1 '" "' bc_drp_site '" ' string(eff_date) ' no no' SKIP.
         PUT UNFORMAT '"' b_co_part '"' SKIP.
         PUT UNFORMAT STRING(b_co_qty_cur * -1) ' no "' bc_site1 '" "' bc_loc1 '"' ' "' b_co_lot '"' SKIP.
         PUT '.' SKIP.
         PUT  SKIP(1).
