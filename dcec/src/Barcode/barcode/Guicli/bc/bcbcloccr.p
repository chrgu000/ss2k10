...
DEF FRAME bc
   ...
   bc_loc AT ROW 5 COL 4
    ...
    WITH SIZE 30 BY 10 TITLE "库位条码生成"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
   ...
   {bcrun.i ""bcmgcheck.p"" "(input ""loc"",
       ...
       INPUT """",
        ...
        IF NOT success THEN
            UNDO,RETRY.
        ELSE DO: 
            FIND FIRST b_loc_mstr WHERE b_loc_site = bc_site AND b_loc_loc = bc_loc NO-LOCK NO-ERROR.
            IF AVAILABLE b_loc_mstr THEN
            DO:
                MESSAGE '该库位条码已生成!' VIEW-AS ALERT-BOX ERROR.
                UNDO,RETRY.
            END.
                ELSE DO:
...
PROCEDURE main:
      DEF VAR b_id LIKE b_loc_code.
    
             
                   FIND LAST b_loc_mstr NO-LOCK NO-ERROR.
                   IF AVAILABLE b_loc_mstr  THEN
                       IF b_loc_code <> '999999999999999' THEN
                       b_id = string(integer(b_loc_code) + 1 , "999999999999999").
                       ELSE DO:
