...
CASE bd_case :
    WHEN 'cust' THEN DO:
    succ = YES.
        FIND cm_mstr WHERE cm_addr = bd_data NO-LOCK NO-ERROR.
...
END.
   ...
   END.
    ...
    IF AVAILABLE glc_cal THEN DO:
      FIND FIRST glcd_det WHERE glcd_year = year(today) AND glcd_per = glc_per NO-LOCK NO-ERROR.
...
END.
    ...
    IF AVAILABLE pod_det THEN DO:
       FIND b_co_mstr WHERE b_co_code = bd_code NO-LOCK NO-ERROR.
...
isfirst = YES.
 succ = YES.
     FOR EACH bd_merge_list NO-LOCK :
         FIND FIRST b_ld_det WHERE b_ld_code = bd_code  AND b_ld_qty_oh <> 0 NO-LOCK NO-ERROR.
