...
PUT SKIP.
   ...
   {bcrun.i ""bcmgbdpro.p"" "(INPUT ""cim.txt"",INPUT ""out.txt"")"}
    FIND LAST tr_hist USE-INDEX tr_trnbr SHARE-LOCK WHERE tr_type = 'rct-unp' AND tr_program = 'icunrc.p' AND tr_nbr = b_bf_nbr AND tr_line = b_bf_line AND tr_part = b_bf_part AND tr_site = b_bf_site AND tr_loc = b_bf_loc AND tr_userid = global_userid AND tr_date = b_bf_date AND tr_qty_loc = b_bf_qty_loc   no-error.
