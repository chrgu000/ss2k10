...
DEF FRAME bc
  ...
  bc_po_part AT ROW 3.6 COL 2.5
   ...
   bc_lot AT ROW 12 COL 1.6
    ...
    WITH SIZE 30 BY 18.5 TITLE "采购退货"  SIDE-LABELS  NO-UNDERLINE THREE-D AT COLUMN 1 ROW 1.
        ...
        output success)"}
          IF NOT success THEN do:
             ...
             END.
                      END.
                       ELSE do:
                          MESSAGE '不能负退货！' VIEW-AS ALERT-BOX.
                      ENABLE bc_po_nbr WITH FRAME bc.
                      UNDO,RETRY.
                      END.
...
DO:
         ...
         IF AVAILABLE b_co_mstr AND (bc_qty >  b_co_qty_cur) THEN DO:
             MESSAGE "退货数量超过当前量" VIEW-AS ALERT-BOX ERROR.
         END.
...
DO:
         ...
         run bcmgcheck.p(input 'site',input bc_site,input '', input '', input '', input '', input '', input '', input '', input '', input '', input '', input '',output success).
    IF NOT success THEN UNDO,RETRY.
    ELSE DO:
...
DO:
        ...
        run bcmgcheck.p(input 'loc',input bc_site,input bc_loc, input '', input '', input '', input '', input '', input '', input '', input '', input '', input '',output success).
   IF NOT success THEN UNDO,RETRY.
    ELSE DO:
