...
PROMPT-FOR  vend vend1 po_nbr nbr1 WITH FRAME a.
IF INPUT vend1 = "" THEN vend1 = hi_char.
...
hcurr = hroot.
FOR EACH po_mstr WHERE /*po_vend = '5017000' OR po_vend = '002'*/ po_vend >= INPUT vend AND po_vend <=  vend1 AND po_nbr >= INPUT po_nbr AND po_nbr <=  nbr1 EXCLUSIVE-LOCK:
...
CREATE X-NODEREF htemp.
hdoc:CREATE-NODE(htemp,"total_cost","element").
mamt = 0.
FOR EACH pod_det WHERE pod_nbr = po_nbr:
