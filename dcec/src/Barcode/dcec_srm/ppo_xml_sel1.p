...
PROMPT-FOR   scx_po  WITH FRAME a.
...
hcurr = hroot.
FOR EACH scx_ref WHERE scx_type = 2 AND scx_po = INPUT scx_po /*po_vend = '5017000' OR po_vend = '002'*/  NO-LOCK:
...
hcurr = htemp.
    ...
    FOR EACH sch_mstr WHERE sch_type = 4 AND sch_nbr = scx_po AND sch_line = scx_line  EXCLUSIVE-LOCK:
...
htemp:SET-ATTRIBUTE("value",sch_rlse_id).
hcurr:APPEND-CHILD(htemp).
/*
CREATE X-NODEREF htemp.
...
CREATE X-NODEREF htemp.
hdoc:CREATE-NODE(htemp,"requriement_time","element").
