...
pre = ''.
   isend = NO.
    FOR EACH schd_det  USE-INDEX schd_dtr WHERE   schd_type = 4  AND /*substr(schd_rlse_id,1,8) = string(year(TODAY),'9999') + STRING(MONTH(TODAY),'99') + STRING(DAY(TODAY),'99')*/ schd_date = 04/13/07 AND /*AND schd_nbr = '0000401c'*/  /*SUBSTR(schd__chr10,8,1) = ''  */ NO-LOCK :
