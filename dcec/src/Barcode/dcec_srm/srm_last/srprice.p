...
DEF VAR mstr AS CHAR FORMAT "x(30)" EXTENT 15 INITIAL "0 0 ".
DEF VAR pre AS CHAR  .
...
IF path = '' THEN DO: MESSAGE "The path is empty!" VIEW-AS ALERT-BOX.
    UNDO,RETRY.
END.
FIND CODE_mstr WHERE CODE_fldname = 'srmpath_log' NO-LOCK NO-ERROR.
path1 = CODE_value.
...
END.
 ...
 DO m = 1 TO 15:
   ...
   IF AVAILABLE pc_mstr THEN  DO:
       ...
       END.
        ...
        pre = price_part.t_part_po_code + price_part.t_part_code + price_part.t_part_prod_line + price_part.t_part_um + price_part.t_part_amttyp + price_part.t_part_po_effectivedate + price_part.t_part_po_inactivedate.
 ...
 END.
 /*DO m= 1 TO 15:
        MESSAGE mstr[m] VIEW-AS ALERT-BOX.
...
PROCEDURE fieldcheck:
           ...
           IF price_part.t_part_po_code = ''  OR length(price_part.t_part_po_code) > 8 THEN do:
               pass = NO.
              CREATE t_error.
         t_er_pricecode = price_part.t_part_po_code + ' ' + price_part.t_part_code.
   ...
   RUN strcheck('part_po_effectivedate','pc_start',price_part.t_part_po_effectivedate,YES).
    ...
    pass = NO.
      CREATE t_error.
        ...
        END.
     ELSE DO:  
          IF (SUBSTR(estr,i,1) < '0' OR SUBSTR(estr,i,1) > '9') AND SUBSTR(estr,i,1) <> '.'  THEN  DO:
    pass = NO.
      CREATE t_error.
