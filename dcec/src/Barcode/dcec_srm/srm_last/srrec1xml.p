...
PROMPT-FOR 
   ...
   nbr1 
    ...
    rdate1 
    WITH FRAME a.
IF INPUT vendor1 = '' THEN vendor1 = hi_char.
IF INPUT site1 = '' THEN site1 = hi_char.
IF INPUT nbr1 = '' THEN nbr1 = hi_char.
IF INPUT rdate = ? THEN rdate = low_date.
IF INPUT rdate1 = ? THEN rdate = hi_date.*/
...
FOR EACH prh_hist   USE-INDEX prh_receiver    where prh_rcp_date >= rdate and /*prh_rcp_date <= rdate1*/
     /*and (prh_vend >= INPUT vendor and prh_vend <= vendor1)
      and (prh_site >= INPUT site and prh_site <= site1)
    AND (prh_nbr >= INPUT po_nbr AND prh_nbr <= nbr1) AND*/ prh__chr10 <> 'c':
...
END.
  ...
  PUT UNFORMAT '<cust_part_no value="' (IF AVAILABLE pod_det AND pod__chr01 <> '' AND pod__chr01 <> 'LS' THEN upper(prh_vend) + '#' ELSE '') upper(PRH_PART) '"/>' SKIP.
