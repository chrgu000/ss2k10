...
totcnt = 0.
mcnt = 0.
    OUTPUT TO VALUE(CODE_CMMT + '\XML\PPOC.XML') CONVERT TARGET 'utf-8' SOURCE 'gb2312'.
...
pre = ''.
   ...
   and sch_nbr = pod_nbr and sch_line = pod_line:
    ...
    where schd_type = sch_type AND schd_date = TODAY 
    ...
    and schd_rlse_id = sch_rlse_id:
     ...
     PUT UNFORMAT '<flex name="' po_vend '" value="' ad_name '"/>' SKIP.
        ELSE PUT UNFORMAT '<flex name="' po_vend '" value=""/>' SKIP.
            yn = NO.
            isend = YES.
         END.
