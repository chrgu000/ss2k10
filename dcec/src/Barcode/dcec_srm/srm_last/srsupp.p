...
FOR EACH supp WHERE t_sup_code = '':
    DELETE supp .
END.
FIND FIRST CODE_mstr WHERE CODE_fldname = 'srmpath_log' NO-LOCK NO-ERROR.
path1 = CODE_value.
iscim = NO.
...
PROCEDURE fieldcheck:
    IF length(supp.t_sup_code) > 8  THEN DO:

                 pass = NO.
                 CREATE t_error.
         t_er_supcode = supp.t_sup_code.
          t_er_xmlfield = 'sup_code'.
              t_er_qadfield = 'vd_addr'.
          t_er_mess = 'Please check sup code!'.
    END.
