...
DEFINE BUFFER fm_mstr01 FOR fm_mstr.
    ...
    DEFINE BUFFER fm_mstrtotal FOR fm_mstr.
...
ASSIGN /*sys_date = TODAY*/
 ...
 entity = gl_entity.
 FIND frist glc_cal WHERE glc_start <= TODAY AND glc_end >= TODAY NO-LOCK NO-ERROR.
 mperiod = glc_per.
...
Function fpbalance RETURN DECIMAL (INPUT fp AS int , INPUT period AS CHAR,input isc as log):
 ...
 FIND FIRST fm_mstr01 WHERE fm_sums_into = fp  NO-LOCK NO-ERROR.
...
ELSE DO:
       ...
       FOR EACH fm_mstr WHERE fm_sums_into = a1 AND (fm_type = "B") AND (fm_dr_cr = direction) NO-LOCK:
        ...
        IF totalfm = 11000 OR totalfm = 12000 or totalfm = 10000 or totalfm = 21000 or totalfm = 22000 OR totalfm = 15000 or totalfm =31000 then do:
             AC_SUM=0.
           balance_begin = fpbalance(totalfm,"begin",NO).
            
             AC_SUM=0.
            balance_end = fpbalance(totalfm,"end",NO).
             IF totalfm = 15000 OR totalfm = 31000 THEN DO:
