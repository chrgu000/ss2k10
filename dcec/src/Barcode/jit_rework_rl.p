...
DEFINE BUTTON b-done1 LABEL "Release" SIZE 12 BY 1.2.
  
     DEFINE BUTTON b-done2 LABEL "Print" SIZE 12 BY 1.2.
...
DO:
  ...
  RETURN NO-APPLY.
END.
ENABLE wo_id wo_part wo_type wo_qty brw b-done1 b-done2 WITH frame wo_header IN WINDOW c-win.
...
DO:
   ...
   MESSAGE "ID can't be blank!" VIEW-AS ALERT-BOX ERROR BUTTONS OK.
      iscontinue = NO. 
       END.
         FIND FIRST j_wo_mstr WHERE j_wo_id = wo_id:SCREEN-VALUE NO-LOCK  NO-ERROR.
           IF NOT AVAILABLE j_wo_mstr THEN DO:
               MESSAGE "IThis work order doesn't exist!" VIEW-AS ALERT-BOX ERROR BUTTONS OK.
               iscontinue = NO.
           END.
       FIND pt_mstr WHERE pt_part = wo_part:SCREEN-VALUE NO-LOCK NO-ERROR. 
      ...
      iscontinue = NO. 
       ...
       IF  iscontinue THEN DO:
         ...
         DEF VAR i AS INT.
          m_qty = j_wo_qty.
          DELETE j_wo_mstr.
          REPEAT i = 1 TO m_qty BY 1:
          CREATE j_wo_mstr.
             IF LENGTH(STRING(I)) = 1 THEN j_wo_id = wo_id:SCREEN-VALUE + "-00" + STRING(i). 
             IF LENGTH(STRING(I)) = 2 THEN j_wo_id = wo_id:SCREEN-VALUE + "-0" + STRING(i).  
             IF LENGTH(STRING(I)) = 3 THEN j_wo_id = wo_id:SCREEN-VALUE + "-" + STRING(i). 
             ASSIGN  j_wo_part = wo_part:SCREEN-VALUE
               j_wo_qty =  1
