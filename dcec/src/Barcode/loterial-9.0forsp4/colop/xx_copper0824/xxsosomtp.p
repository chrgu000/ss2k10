...
&SCOPED-DEFINE sosomtp_p_1 "Confirmed"
/* MaxLen: Comment: */

&SCOPED-DEFINE sosomtp_p_3 "Promise Date"
/* MaxLen: Comment: */

&SCOPED-DEFINE sosomtp_p_4 "Comments"
/* MaxLen: Comment: */

&SCOPED-DEFINE sosomtp_p_6 "Calculate Freight"
/* MaxLen: Comment: */

&SCOPED-DEFINE sosomtp_p_7 "Display Weights"
/* MaxLen: Comment: */
...
define shared variable promise_date  as   date
   label {&sosomtp_p_3}.
define     shared variable perform_date  as   date
   label "Perform Date".
{&SOSOMTP-P-TAG1}
...
/*xx*/  define            variable l_old_rate    like so__chr01   no-undo.
...
{xxsosomt02.i}  /* Form definitions for shared frames a & b */
/*V8+*/
...
for first soc_ctrl
      fields(soc_apm soc_use_btb) no-lock:
end. /* FOR FIRST SOC_CTRL */

for first iec_ctrl
      fields(iec_impexp) no-lock:
...
/**xx*/    l_old_rate  = so__chr01.
/*xx*/      DISPLAY l_old_rate @ so_mstr.so__chr01. 
   ...
   editing:
      ...
      if frame-field = "so_cr_terms" then do:
         ...
         then do:
            ...
            for first ct_mstr
                  ...
                  where ct_code = input so_cr_terms no-lock:
            end. /* FOR FIRST CT_MSTR */

            if available ct_mstr then do:
               display
                  string(ct_terms_int,"->>>9.99")
                  @ socrt_int.
      ...
      if frame-field = "so_po" then do:
         if (lastkey = keycode("RETURN") or
            ...
            lastkey = keycode("F1"))
         then do:
            if input so_po = "" and soc_use_btb and
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   /**xx***/
   if not new so_mstr and so__chr01 <> l_old_rate
   then do:
   ...
   if new_order and not line_pricing then do:
      ...
      for first mfc_ctrl
            ...
            where mfc_field = "soc_pt_req" no-lock:
...
/*GUI*/ if global-beam-me-up then undo, leave.
      ...
      if merror then undo setb, retry.
         ...
         if cfexists then do:
            /* Work through the cf_sod_rm file removing any files*/
            for each cf_sod_rm:
   ...
   if so_po = "" and soc_use_btb and not this-is-rma then do:
      {pxmsg.i &MSGNUM=2897 &ERRORLEVEL=2}
      pause.
   end.

   /**xx***/
   /* IDENTIFDY COPPER  RATE CALCULATED ONLY */
   FIND FIRST code_mstr WHERE code_fldname = "cm_class" AND code_value = so_mstr.so__chr01 NO-LOCK NO-ERROR .
   IF NOT AVAILABLE CODE_mstr THEN DO:   
       MESSAGE "Invalid Copper Fixed Rate Definition in the Customer Class!  "  VIEW-AS ALERT-BOX ERROR BUTTONS OK. 
       next-prompt so_mstr.so__chr01.
       undo setb, retry setb.
   END.
   ...
   if so_mstr.so_po <> "" then do for somstr:
         ...
         if can-find(first somstr
            where somstr.so_po   =  so_mstr.so_po
            and   somstr.so_cust =  so_mstr.so_cust
            and   somstr.so_nbr  <> so_mstr.so_nbr) or
            can-find(first ih_hist
            where ih_po   =  so_mstr.so_po
            and   ih_cust =  so_mstr.so_cust
            and   ih_nbr  <> so_mstr.so_nbr) then
         do:
      ...
      /* FOLLOWING CODE TO SUPPORT CHECK ON CUSTOMERS FOR WHOM A PO IS REQUIRED */
         if so_fsm_type = " " and (not so_sched) then do:
            ...
            for first cm_mstr
                  fields(cm_addr cm_po_reqd cm_promo)
