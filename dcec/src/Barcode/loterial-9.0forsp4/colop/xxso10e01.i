/* xxso10e01.i - DEFINE FRAME FOR INVOICE LINE ITEM INFORMATION                 */
...
then
   assign
      ext_price:label in frame d     = getTermLabel("Final Amount",16)
      ext_price:label in frame deuro = getTermLabel("Final Price",16).
else
   assign
      ext_price:label in frame d     = getTermLabel("NET_Amount",16)
...
/*xx      qty_bo           column-label "Backorder" when (sod_fsm_type <> "FSM-RO") **/
      sod_taxable
...
/***       sod_price   when (sod_fsm_type <> "FSM-RO"
       ...
       sod_fixed_price  when (sod_fsm_type =  "FSM-RO"
                        ...
                        ****/
/*xx*/  surchange   COLUMN-LABEL "Total Cu-Surcharge" 
