...
/* Revision: 1.95.1.1      BY: Kirti Desai        DATE: 10/14/03  ECO: *P165* */
/* $Revision: 1.95.1.2 $       BY: Veena Lad       DATE: 10/14/03  ECO: *P160*   */
/******************************************************************************/
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         {xxsoivmt02.i}  /* Definitions for shared frames a & b & b1 */
...
/*xx*/  FIND FIRST so_mstr WHERE so_nbr = input so_nbr NO-LOCK NO-ERROR.
           IF NOT AVAILABLE so_mstr THEN DO :
               BELL.
               MESSAGE "The Sales Oredr has not exists ! "  VIEW-AS ALERT-BOX ERROR BUTTONS OK.
               UNDO,RETRY .
           END.
...
/*GUI*/ RECT-FRAME-LABEL:SCREEN-VALUE in frame b = F-b-title.
            ...
            if not lgData and so_app_owner > "" then do:
               ...
               for first lgs_mstr where lgs_app_id = so_app_owner
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         if use-log-acctg and
            so_fr_terms <> "" and (can-find(first sod_det
            where sod_nbr = so_nbr and sod_fr_list <> ""))
         then do transaction on error undo, retry:
            ...
            then do:
               ...
               lv_nrm_seqid = "".
               for first lac_ctrl
                 fields (lac_soship_nrm_id)
                 no-lock:
               end.
/*GUI*/ if global-beam-me-up then undo, leave.

               if available lac_ctrl then
         ...
         {gprun.i ""xxsosoprc.p"" "(input so_recno,
                                  input reprice,
                                  input new_order,
                                  input line_pricing)" }
/*GUI*/ if global-beam-me-up then undo, leave.
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         if can-find (iec_ctrl where iec_use_instat = yes) then do:
            for each sod_det
               fields (sod_nbr sod_line sod_btb_type sod_btb_po sod_site
...
END PROCEDURE.
   ...
   PROCEDURE assign_new_so:
      ...
      then do:
         ...
         do:
            for first ad_mstr
               fields(ad_addr ad_city ad_country ad_inv_mthd ad_lang
               ad_line1 ad_line2 ad_name ad_pst_id ad_state
...
/*GUI*/ if global-beam-me-up then undo, leave.
      ...
      then do:
         ...
         for first ad_mstr
            fields(ad_addr ad_taxable ad_tax_usage ad_taxc)
      ...
      do counter = 1 to 4:
         ...
         if cm_mstr.cm_slspsn[counter] <> "" then do:
            ...
            find spd_det
               where spd_addr = cm_mstr.cm_slspsn[counter] and
                     spd_prod_ln = "" and
                     spd_part = ""    and
   ...
   PROCEDURE new-proc-j2dd:
      ...
      do transaction on error undo, retry:
         for first mfc_ctrl
            ...
            where mfc_field = "l_vqc_sold_usage"
            no-lock.
...
END PROCEDURE.  /* p-disp-frameb */
   ...
   PROCEDURE p-shipto-change:
      ...
      l_edittax = no.
      if not batchrun and l_old_shipto <> "" and l_old_shipto <> so_ship
      then do:
