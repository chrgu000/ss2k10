...
/**xxx**restore  the original value to the  sod_det, including the fields sod_price,sod_list_pr sod__chr01  */
      so_mstr-loop-xx:
      for each work_so WHERE wso_user = mfguser , 
       EACH so_mstr NO-LOCK   where 
           (so_cust >= cust
         and    so_cust <= cust1)
         and   (so_bill >= bill
         and    so_bill <= bill1)
         AND so_nbr = wso_nbr: 
          ...
          for each sod_det  EXCLUSIVE-LOCK             
             where sod_nbr = so_nbr              
               and sod_line > 0
               AND sod__chr01 <> "" 
          break by sod_line with frame xx  width 132:
