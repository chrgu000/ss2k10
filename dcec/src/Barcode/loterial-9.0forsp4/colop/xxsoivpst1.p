/* xxsoivpst1.p - POST INVOICES TO AR AND GL REPORT                             */
...
/* Revision: 1.96.1.8   BY: Kirti Desai         DATE: 11/12/03  ECO: *P195*  */
/* $Revision: 1.96.1.10 $ BY: Ashish Maheshwari   DATE: 11/13/03  ECO: *P15L*  */
...
/*xx*/  define shared temp-table work_so
...
FORM /*GUI*/ 
   ...
   so_prepaid     colon 40
   so_fob         colon 15
   so__dec01 COLON 48   LABEL "Surcharge Amt"  FORMAT "->>>,>>>,>>9.9<<<<<"
...
/*xx**/   sod__dec02 format "->>>,>>9.9<<<<<<" COLUMN-LABEL "Invoiced Copper" 
with STREAM-IO /*GUI*/  frame e width 196 down.
...
FORM /*GUI*/ 
   ...
   skip(1)
with STREAM-IO /*GUI*/  frame rpttot no-labels width 132.
...
with STREAM-IO /*GUI*/  frame gltwtot width 132 down no-labels.
...
do on error undo, leave:
   ...
   FORM /*GUI*/  header
      ...
      batch
   with STREAM-IO /*GUI*/  frame jrnl width 80 page-top.
         ...
         do transaction on error undo , leave:
            ...
            break by so_inv_nbr:
                ...
                with frame xx width 132:
                     ...
                     ASSIGN sod__chr01 = string(sod_price) .
                     ...
                     AND ( so_inv_date >= pi_start  OR pi_start = ? ) AND ( so_inv_date <= pi_expire OR pi_expire = ? ) 
                     NO-LOCK NO-ERROR .
            ...
            end.  /**for each so_mstr and  *SUBSTRING(so__chr01,1,1) = "V" **xxx****/
            ...
            break by so_inv_nbr:
               ...
               if first-of(so_inv_nbr) then do:
                  ...
                  do on endkey undo, leave:
                     ...
                     FORM /*GUI*/  with STREAM-IO /*GUI*/  frame h1 width 80.
                     /* SET EXTERNAL LABELS */
                     ...
                     with frame h1 width 132 STREAM-IO /*GUI*/ .
               ...
               do on endkey undo, leave:
                  ...
                  do with frame h2:
                     ...
                     display
                        ...
                        so_ord_date
                        so_po       label "P/O"
                     with frame h2 side-labels width 132 no-box STREAM-IO /*GUI*/ .
...
/**xx**     DISPLAY invtot_taxable_amt invtot_tax_amt  WITH FRAME xxx.   ***/
...
/*xx**         invtot_tax_amt = 0 .  ***/
                  ...
                  if so_fsm_type = "PRM" and so_prepaid <> 0 then do:
                     ...
                     with STREAM-IO /*GUI*/  frame prep-frame side-labels width 80.
                     ...
                     do on endkey undo, leave:
                        display
                           ...
                           so_prep_tax
                           amt-due
                        with frame prep-frame STREAM-IO /*GUI*/ .
                     end. /* DO ON ENDKEY UNDO, LEAVE */
               ...
               if undo_all then do:
                  ...
                  display
                     ...
                     error_msg
                     skip(3)
                  with frame unpost no-labels width 80 STREAM-IO /*GUI*/ .
               ...
               if cardProcessed then do:
                  ...
                  if success = false then do:
                     ...
                     display
                        ...
                        error_msg skip
                        skip(3)
                     with frame unpost2 no-labels width 80 STREAM-IO /*GUI*/ .
   ...
   with frame gltwtot:
      ...
      if not gl_sum then do:
         do on endkey undo, leave:
            display
               ...
               gltw_date
               gltw_desc
            with frame gltwtot STREAM-IO /*GUI*/ .
         end. /* DO ON ENDKEY UNDO, LEAVE */

         do on endkey undo, leave:
            if dr_amt <> 0 then display dr_amt WITH STREAM-IO /*GUI*/ .
            if cr_amt <> 0 then display cr_amt WITH STREAM-IO /*GUI*/ .
      ...
      if last-of(gltw_cc) then do:
         ...
         if (accum total by gltw_cc dr_amt) <> 0 then do:
            ...
            do on endkey undo, leave:
               display gltwdr @ dr_amt with frame gltwtot STREAM-IO /*GUI*/ .
            end. /* DO ON ENDKEY UNDO, LEAVE */
         end.
         ...
         if (accum total by gltw_cc cr_amt) <> 0 then do:
            ...
            do on endkey undo, leave:
               display gltwcr @ cr_amt with frame gltwtot STREAM-IO /*GUI*/ .
            end. /* DO ON ENDKEY UNDO, LEAVE */
         end.
      ...
      if last-of(gltw_entity) then do:
         ...
         do on endkey undo, leave:
            display
               ...
               accum total (cr_amt) @ cr_amt with frame gltwtot STREAM-IO /*GUI*/ .
            down 1 with frame gltwtot.
         end. /* DO ON ENDKEY UNDO, LEAVE */
      end.
   ...
   if auto_balance_amount <> 0 then
      do with frame autobalance:
         ...
         display
            ...
            batch
         with frame autobalance width 132 no-label STREAM-IO /*GUI*/ .
      end.
   ...
   if available invoice_err then do:
      ...
      with frame c width 80 STREAM-IO /*GUI*/ .
