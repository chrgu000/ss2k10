/* xxsoivpstb.p - SALES ORDER LINE INVOICE POST                                */
...
/* Revision: 1.29.3.2   BY: Hareesh V.            DATE: 10/06/03 ECO: *N2F8* */
/* $Revision: 1.29.3.3 $  BY: Shoma Salgaonkar      DATE: 11/04/03 ECO: *P18C* */
/******************************************************************************/
...
define variable gen_desc          like  glt_desc      no-undo.
define variable gltline           like  glt_line      no-undo.
define variable l_gltline2        like  glt_line      no-undo.
define variable l_glt_tot         like  glt_amt       no-undo.
define variable l_diff            like  glt_amt       no-undo.
...
/*GUI*/ if global-beam-me-up then undo, leave.
 /* LAST-OF(pih_disc_proj) */
      ...
      /* IF THERE ARE ROUNDING DIFFERENCES B/W GL AND AR, POST THE DIFFERENCE */
      /* INTO cu_ex_rnd_acct TO AVOID MISMATCH B/W AR AND GL.                */
      if base_curr <> so_curr
      and so_fsm_type = "PRM"
      then do:

         /* GET THE TOTAL GL AMOUNT FROM SUM OF GL AMTS FOR EACH LINE */
         l_glt_tot = 0.
         for each glt_det
            where glt_ref = ref
            no-lock:
            l_glt_tot = l_glt_tot + glt_amt.
         end. /* FOR EACH glt_det */

         if ard_amt <> ( - l_glt_tot)
         then do:

            /* POST THE DIFF B/W AR AND GL INTO THE CURR ROUNDING ACCT */
            l_diff = 0.
            for first cu_mstr
               fields(cu_curr cu_ex_rnd_acct cu_ex_rnd_sub cu_ex_rnd_cc)
               where cu_curr = so_curr
               no-lock:

               l_diff = ( - ard_cur_amt) - l_glt_tot.
               if post
               then do:

                  {mfgltd.i &acct=cu_ex_rnd_acct
                            &sub=cu_ex_rnd_sub
                            &cc=cu_ex_rnd_cc
                            &amt=l_diff
                            &date=eff_date
