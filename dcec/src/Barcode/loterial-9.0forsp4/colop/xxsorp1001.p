/* xxsorp1001.p - SALES ORDER INVOICE PRINT FOR ENGLISH PRINT CODE "1"          */
/* Copyright 1986-2003 QAD Inc., Carpinteria, CA, USA.                        */
/* All rights reserved worldwide.  This is an unpublished work.               */
/* $Revision: 1.29.1.23.1.3 $                                                                 */
...
define variable trl_length   as integer initial 5.
...
/*xx*/  DEFINE NEW SHARED VARIABLE baseprice LIKE pi_list_price .
/*xx*/  DEFINE  NEW  SHARED  VARIABLE  line_price  like sod_price format "->>>>,>>>,>>9.99".
/*xx*/  DEFINE VARIABLE spname1 LIKE ad_name  FORMAT "x(12)".
/*xx*/  DEFINE VARIABLE spname2 LIKE ad_name   FORMAT "x(12)" .
/*xx*/  DEFINE VARIABLE sptel1 LIKE ad_phone   FORMAT "x(12)".
/*xx*/  DEFINE VARIABLE sptel2 LIKE ad_phone   FORMAT "x(12)".
/*xx*/  DEFINE VARIABLE spemail1 LIKE ad_attn LABEL "E-Mail".
/*xx*/  DEFINE VARIABLE spemail2 LIKE ad_attn .
            DEFINE VARIABLE netweight LIKE pt_net_wt FORMAT "->>>>,>>9.99<<<<<<" .
            DEFINE VARIABLE custtax LIKE  ad_gst_id .
            DEFINE SHARED VARIABLE mytax   LIKE  ad_gst_id.
            DEFINE VARIABLE surchange LIKE line_price   LABEL "Surcharge Amt" .
/*xx*/   DEFINE VARIABLE tot_var     LIKE ar_amt .
/*xx*/   DEFINE VARIABLE cuum       LIKE pt_um  .
/*xx*/   DEFINE VARIABLE addprice  LIKE sod_price.
/*xx*/   DEFINE VARIABLE addtax     LIKE pi_list_price.

/*xx*/  define new shared temp-table work_so
               field wso_nbr  like sod_nbr
               field wso_user like mfguser .
...
FORM /*GUI*/ 
   so_nbr        colon 15
   so_ship_date  colon 65
   so_ord_date   colon 15
   hdr_po        to 88    no-label
   so_slspsn[1]  colon 15 label "Salesperson(1)"   spname1  NO-LABEL sptel1 NO-LABEL spemail1  NO-LABEL 
   so_slspsn[2]   colon 15 label "Salesperson(2)"  spname2 NO-LABEL sptel2 NO-LABEL spemail2  NO-LABEL 
    /**xx
...
/*xx**/   
   baseprice  COLON 15  LABEL "Copper base"
    ...
    mytax   COLON 69   LABEL "Company Tax"

   hdr_call      to 74 no-label
   so_rmks       colon 15
/*   skip(1)  */
with STREAM-IO /*GUI*/  frame phead2 side-labels width 118.
...
{fsconst.i}     /* FIELD SERVICE CONSTANTS */
    ...
    FORM /*GUI*/ 
      so_curr                       COLON 60 
        line_total                     colon 80         NO-ATTR-SPACE  
        user_desc[1]                to 73   no-label
        so_trl1_cd format "x(2)"    to 78   no-label
        ":"                         at 80
        so_trl1_amt                         no-label
        user_desc[2]                to 73   no-label
        so_trl2_cd format "x(2)"    to 78   no-label
        ":"                         at 80
        so_trl2_amt                         no-label
        user_desc[3]                to 73   no-label
        so_trl3_cd format "x(2)"    to 78   no-label
        ":"                         at 80
       ...
       disc_amt                    colon 100         no-attr-space
       ...
       ***/
       tax_amt                     colon 80 skip
       /***
       tax_edit_lbl format "x(28)" to 30   no-label no-attr-space
       tax_edit                    at 32   no-label
       ***/
       ord_amt                     colon 80 SKIP       
    with STREAM-IO frame xxivtot side-labels width 132 NO-ATTR-SPACE  .
...
/*xx*/  FOR EACH work_so WHERE wso_user = mfguser:
                 DELETE work_so .        
            END.
...
/****xx*************/
        FOR  EACH  so_mstr where (so_nbr >= nbr and so_nbr <= nbr1)
         ...
         AND SUBSTRING(so__chr01,1,1) = "V"  
            exclusive-lock
            use-index so_invoice
            break by so_inv_nbr:
                
                FIND FIRST work_so WHERE wso_user = mfguser 
                AND wso_nbr = so_nbr EXCLUSIVE-LOCK NO-ERROR.
                IF NOT AVAILABLE work_so THEN DO:
                    CREATE work_so.
                    ASSIGN wso_user = mfguser
                                  wso_nbr = so_nbr .
                END.
                ...
                FOR EACH  sod_det  EXCLUSIVE-LOCK             
                   where sod_nbr = so_nbr                 
                     and sod_line > 0
                     and sod_qty_inv <> 0 
                with frame x1 width 132:
                         
                    /** 
                    FIND FIRST pt_mstr WHERE pt_part = sod_part  NO-LOCK NO-ERROR.
                     IF AVAILABLE pt_mstr  AND SUBSTRING(so__chr01,1,1) = "F"  THEN sod__dec01 = pt_price .
                     ***/

                     IF sod__dec01 <> 0  AND sod__dec02 = 0 THEN sod__dec02 = sod__dec01 .
                     IF sod__dec02 <> 0  AND sod__dec01 = 0 THEN sod__dec01 = sod__dec02 .
                     IF sod__dec02 =  0  THEN NEXT .
                    
                     sod__chr01 = STRING(sod_price)  .

                     FIND FIRST pt_mstr WHERE pt_part = sod_part NO-LOCK NO-ERROR .
                    
                     FIND LAST pi_mstr WHERE pi_list = "base"  AND pi_part_code = icc_user1 
                     AND ( TODAY >= pi_start  OR pi_start = ? ) AND ( TODAY <= pi_expire OR pi_expire = ? ) 
                     NO-LOCK NO-ERROR .
                     ...
                     IF baseprice <> 0 THEN ASSIGN 
                        ...
                        sod_price = sod_price  +  round(addprice,5)   /**( sod__dec02 - baseprice ) * ( pt__dec01 + pt__dec02 )  * sod_um_conv **/
                        sod_list_pr = sod_price .
                     ELSE   ASSIGN addprice = 0 . 
                     
                     /*****Update the tx2d_det records for tax type = 13  and 16****/
                     IF so_taxable THEN DO:
                         
                         FIND LAST tx2_mstr WHERE tx2_pt_taxc = so_taxc AND tx2_tax_usage = so_tax_usage NO-LOCK NO-ERROR.
                         IF AVAILABLE tx2_mstr THEN ASSIGN 
                             addtax  =  addprice * sod_qty_inv  * tx2_tax_pct / 100  .     

                         FIND LAST  tx2d_det where tx2d_ref = so_nbr and
                         tx2d_nbr = ""  and tx2d_line = sod_line and
                         tx2d_tr_type = "13" EXCLUSIVE-LOCK NO-ERROR.
                         IF AVAILABLE tx2d_det  THEN DO:
                            
                         /**tx2d_cur_tax_amt = tx2d_cur_tax_amt + addtax .**/
                            /*tx2d_tax_amt = tx2d_tax_amt + addtax .             */
                            tx2d__dec01 = tx2d_cur_tax_amt .
                            tx2d__dec02 = tx2d_tax_amt .
                            tx2d_cur_tax_amt = sod_qty_inv * sod_price * tx2_tax_pct / 100 .
                            tx2d_tax_amt = sod_qty_inv * sod_price * tx2_tax_pct / 100 .
                         
                        END. /*available tx2d_det***/
                        
                        FIND LAST  tx2d_det where tx2d_ref = so_inv_nbr 
                     ...
                     /**                 DISPLAY sod_part sod_price "   "    sod__chr01 baseprice  so_inv_date  .  **/
...
then do:
   for first so_mstr where (so_nbr >= nbr and so_nbr <= nbr1)
         ...
         and (so_bill >= bill and so_bill <= bill1)
         and (so_lang >= lang and so_lang <= lang1)
/*xx*   AND SUBSTRING(so__chr01,1,1) = "V"   ***/
   no-lock: end.
...
****/         
         {gprun.i ""xxsoivtrl3.p"" "(input so_inv_nbr,
                                input so_nbr,
                                input col-80 /* REPORT WIDTH */,
                                input '16',
                                input tot_cont_charge,
      ...
      if available ad_mstr then do:
         ...
         /* (if it fits)                                  */
         if addr[6] = "" and so_po <> "" then
            assign
               billto[6] = po-lbl + so_po
               hdr_po    = "".
         else
            assign
               billto[6] = addr[6]
      ...
      FORM /*GUI*/  header
         skip(8)
         getTermLabelRt("BANNER_INVOICE",25) to 58 format "x(25)" SKIP(1)
         company[1]    at 4
/*xx*         getTermLabelRt("BANNER_INVOICE",25) to 80 format "x(25)"   **/
         company[2]    at 4
         company[3]    at 4
         getTermLabelRtColon("INVOICE",14)   to 66 format "x(14)"
         so_inv_nbr    at 68
         getTermLabelRtColon("REVISION",10)  to 86 format "x(10)"
         so_rev        to 90
         company[4]    at 4
         getTermLabelRtColon("INVOICE_DATE",14) to 66 format "x(14)"
         inv_date      at 68
         getTermLabel("PAGE_OF_REPORT",5) + ": " +
            string(page-number - pages,">>9")
            format "x(9)" to 90
         company[5]    at 4
         getTermLabelRtColon("PRINT_DATE",14) to 66 format "x(14)" today at 68
/**xx     company[6]    at 4   **/
         skip(1)
         getTermLabel("BILL_TO",15) + ": " + so_bill at 8  format "x(25)"
         getTermLabel("SOLD_TO",15) + ": " + so_cust at 59 format "x(25)"
/**xx         skip(1)   ***/
         billto[1]      at 8
         soldto[1]      at 60
         billto[2]      at 8
...
***********/         
         ...
         vatreglbl   to 65
         vatreg
      with STREAM-IO /*GUI*/  frame phead1 page-top width 118.

      view frame phead1.

      {&SORP1001-P-TAG20}
/**xx      if old_sod_nbr <> ? then page.   **/
...
/*xxx**line_price */
      {gprun.i ""xxsorp1a01.p"" "(output tot_cont_charge,
                                output tot_line_charge)"}
/**
PUT "xxxxxxxxxxx" AT 1 .        
      PUT "xxxxxxxxxxx1" AT 1 .        
      PUT "xxxxxxxxxxx2" AT 1 .        
      PUT "xxxxxxxxxxx3" AT 1 .        
      PUT "xxxxxxxxxxx4" AT 1 .        
      PUT "xxxxxxxxxxx5" AT 1 .             
      PUT STRING(page-size)  AT 1 .      
PUT STRING(line-counter)  AT 1 .      
*****/
...
***/
      ...
      /*************Hide the display the tax detail data*****/
      {&SORP1001-P-TAG32}
...
************xx********/
      ...
      ELSE  cuum = "".
      FOR EACH sod_det WHERE sod_nbr = so_nbr AND ( sod__dec01 <> 0  OR sod__dec02 <> 0 )  NO-LOCK :
            FIND FIRST pt_mstr WHERE pt_part = sod_part NO-LOCK NO-ERROR .
            ...
            ELSE DO:
                baseprice = 0 .
            END.
            tot_var = tot_var + ( sod__dec02 - baseprice ) * sod_qty_inv * sod_um_conv *  ( pt__dec01 + pt__dec02 ) .
      END.
      
/**  DISPLAY invtot_line_total tot_var . ***/

/**       ASSIGN invtot_linecharge_amt  = tot_var.
                      invtot_line_total =  invtot_line_total + tot_var . **/

       IF so_taxable THEN DO:
            FIND LAST tx2_mstr WHERE tx2_pt_taxc = so_taxc AND tx2_tax_usage = so_tax_usage NO-LOCK NO-ERROR.
            ASSIGN /** invtot_tax_amt  = invtot_tax_amt   +  tot_var * tx2_tax_pct / 100   
                                invtot_ord_amt   = invtot_ord_amt  + tot_var * ( 1 + tx2_tax_pct / 100 )      .   ***/
                         invtot_tax_amt =  invtot_line_total * tx2_tax_pct / 100       
                         invtot_ord_amt   = invtot_line_total  +  invtot_line_total * tx2_tax_pct / 100  .
       END.
       ELSE 
          ASSIGN  invtot_ord_amt   = invtot_ord_amt  + tot_var  .
           
/**xx**/   invtot_ord_amt = line_price + invtot_tax_amt  +  invtot_trl1_amt + invtot_trl2_amt + invtot_trl3_amt  .
       
       /*xx****
       {xxsoivtot8.i}
         *******/ 
       DISPLAY   so_curr 
/*xx**     invtot_line_total          @ line_total    **/
/*xx*/     invtot_line_total         @ line_total 
               user_desc
       ...
       WITH FRAME xxivtot SIDE-LABELS WIDTH 132.   
...
/**xxx**resume the original value for sod_price ****/
      upso-loop:
      FOR  EACH  work_so WHERE wso_user = mfguser , 
      EACH so_mstr NO-LOCK   WHERE  so_nbr = wso_nbr : 

          for each sod_det  EXCLUSIVE-LOCK             
             where sod_nbr = so_nbr              
               and sod_line > 0
               AND sod__chr01 <> "" 
          break by sod_line with frame x2  width 132:

               ASSIGN 
                   sod_price = decimal(sod__chr01)  
                   sod_list_pr = sod_price 
                   sod__chr01 = "" .
          END.
      END.  /******FOR EACH WORK_SO */
/************************xx****/
