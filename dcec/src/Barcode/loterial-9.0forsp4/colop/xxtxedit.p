...
/* REVISION:          BY: Larry Leeth       DATE: 10/06/00  ECO: *N0RP*     */
...
/* Revision: 1.39.3.1      BY: Rajaneesh S.       DATE: 12/19/03  ECO: *P1GK*    */
/* $Revision: 1.39.3.2 $      BY: Prashant Parab       DATE: 01/22/04  ECO: *P1KR*    */
/***************************************************************************/
...
if c-application-mode = "API" then do on error undo, return:
   ...
   with frame a no-labels width 80
      title color normal (getFrameTitle("TOTAL_TAX_AMOUNTS",25))
      attr-space.
   form
      tax_line colon 10
   ...
   with frame b title color normal
      (getFrameTitle("TAX_DETAIL_RECORD",25)) width 80
...
then
   ...
   for first code_mstr
       fields(code_desc code_fldname code_value)
       where code_fldname = "txe_tax_env" and
   ...
   typeloop2:
   for each tx2d_det
       fields(tx2d_abs_ret_amt tx2d_by_line tx2d_cur_abs_ret_amt
    ...
    where tx2d_ref = ref
      ...
      for first tx2_mstr
          fields(tx2_apr_use tx2_ara_use tx2_by_line tx2_desc tx2_pt_taxc
   ...
   repeat on endkey undo, leave:
      ...
      editing:
         ...
         /* FROM  "true"                                  */
            ...
            {mfnp05.i tx2d_det
                    tx2d_ref_nbr
                   "tx2d_ref = ref and tr_type = tx2d_tr_type"
                    tx2d_line
                   "input tax_line"}
         end.
         if recno <> ? then do:
         ...
         if tx2_update_tax then do:
            ...
            then do:
               ...
               then do:
                  ...
                  for first so_mstr
                     fields(so_curr so_exru_seq so_ex_rate so_ex_rate2
                            so_ex_ratetype so_fix_rate so_nbr so_ship_date
                            so_tax_date)
                     where so_nbr = tx2d_ref
                     no-lock:
