...
/*M0L8*/ define variable frametitle as character no-undo.
       DEFINE VAR mtotal_qty AS INT.
         DEFINE VAR mshortage_qty AS INT. 
         DEFINE VAR mavailable_qty AS INT.
         DEFINE SHARED VAR totshort_qty LIKE sr_qty.
         DEFINE VAR msurplus_qty AS INT.
...
/*G656*/                      and lad_line = string(wod_op)
                              and lad_part = wod_part EXCLUSIVE-LOCK:
                                  FIND pt_mstr WHERE lad_part = pt_part  NO-LOCK NO-ERROR.
                                  IF pt_lot_ser <> '' AND lad_lot = '' THEN lad_qty_chg = 0.
                                 ELSE DO: lad_qty_chg = lad_qty_all.
                                 
                                  mtotal_qty = mtotal_qty + lad_qty_all.
                            ...
                            END.
                           IF mtotal_qty > lotserial_qty THEN DO:
                               msurplus_qty = mtotal_qty - lotserial_qty.
                              
                               FOR EACH lad_det where lad_dataset = "wod_det"
/*G656*                       and lad_nbr = wod_nbr and lad_line = wod_lot */
/*G656*/                      and lad_nbr = wod_lot
/*G656*/                      and lad_line = string(wod_op)
                              and lad_part = wod_part AND lad_qty_chg <> 0  EXCLUSIVE-LOCK BY lad_lot DESC  :
                                    FIND ld_det WHERE ld_part = part AND ld_site = site AND ld_loc = location AND lad_lot = ld_lot  NO-ERROR.
                                                               
                                   IF lad_qty_chg >= msurplus_qty THEN DO:
                                   
                                lad_qty_chg = lad_qty_chg - msurplus_qty.
                                 ld_qty_all = ld_qty_all - msurplus_qty.
                              IF lad_qty_chg = 0 THEN DELETE lad_det.
                              msurplus_qty = 0.
                              LEAVE.
                                END.
                               ELSE DO:
                               
                                    ld_qty_all = ld_qty_all - lad_qty_chg.
                                    msurplus_qty = msurplus_qty - lad_qty_chg.
                                    DELETE lad_det.
                            ...
                            mshortage_qty = lotserial_qty - mtotal_qty.
                             mavailable_qty = 0.
                             
                             
                             FOR EACH ld_det WHERE ld_part = part AND ld_site = site AND ld_loc = location BY ld_lot:
                            
                                 mavailable_qty = mavailable_qty + (ld_qty_oh - ld_qty_all - ld_qty_frz).
                                 
                                 
                                 
                                 END.
                         /*   DISPLAY mtotal_qty mshortage_qty mavailable_qty lotserial_qty.*/
                                 IF mtotal_qty < lotserial_qty THEN DO:
                             
                                   IF  mavailable_qty >= mshortage_qty THEN DO:
                                  
                                
                                FOR EACH ld_det WHERE ld_part = part AND ld_site = site AND ld_loc = location AND (ld_qty_oh - ld_qty_all - ld_qty_frz)  > 0 BY ld_date BY ld_lot :
                                 IF ld_qty_oh - ld_qty_all - ld_qty_frz >=  mshortage_qty 
                                      THEN DO:
                                /* DISPLAY ld_lot.*/
...
/*G656*/                      and lad_line = string(wod_op)
                              and lad_part = wod_part  AND lad_lot = ld_lot AND lad_ref = ld_ref AND lad_site = ld_site AND lad_loc = ld_loc  EXCLUSIVE-LOCK NO-ERROR.
                                 ...
                                 lad_qty_all =   mshortage_qty.
                                 lad_qty_chg =  mshortage_qty.

                                      END.
                                    ELSE DO:
                                    
                                         lad_qty_all = lad_qty_all +  mshortage_qty.
                                 lad_qty_chg = lad_qty_chg + mshortage_qty.
                                 ...
                                 ld_qty_all = ld_qty_all + mshortage_qty.
                                 mshortage_qty = 0.
                                 LEAVE.
                                      END.  
                                      ELSE DO:
     ...
     /*G656*/                      and lad_line = string(wod_op)
                                 ...
                                 lad_lot = ld_lot.
                                  ...
                                  lad_site = ld_site.
                                 lad_loc = ld_loc. 
                                 lad_qty_all =  ld_qty_oh - ld_qty_all - ld_qty_frz.
                                 lad_qty_chg = ld_qty_oh - ld_qty_all - ld_qty_frz.

                                 END.
                                 ELSE DO:                                 
                                     lad_qty_all = lad_qty_all + ld_qty_oh - ld_qty_all - ld_qty_frz.
                                lad_qty_chg = lad_qty_chg + ld_qty_oh - ld_qty_all - ld_qty_frz.

                                     END.
                                 ld_qty_all = ld_qty_all + lad_qty_all.     
                                 mshortage_qty =  mshortage_qty - lad_qty_all.
