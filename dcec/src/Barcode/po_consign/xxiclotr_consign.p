...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         IF NOT AVAILABLE po_mstr THEN DO:
            MESSAGE '无效PO！' VIEW-AS ALERT-BOX ERROR.
            UNDO,RETRY.
        END.
          ELSE ASSIGN mvend = po_vend
                      mcur = po_curr.
        FIND FIRST pod_det WHERE pod_nbr = nbr AND pod_part = INPUT part NO-LOCK NO-ERROR.
        IF NOT AVAILABLE pod_det THEN DO:
            MESSAGE '该po没有该零件!' VIEW-AS ALERT-BOX ERROR.
            UNDO,RETRY.
        END.
        ELSE assign
            mline = pod_line
            ...
            mdisc = pod_disc_pct.
        mcount = 0.
        FOR EACH pod_det WHERE pod_nbr = nbr AND pod_part = INPUT part NO-LOCK:
          mcount = mcount + 1.
            END.
            IF mcount > 1 THEN DO:
               
                FIND FIRST pod_det WHERE pod_nbr = nbr AND pod_part = INPUT part AND STRING(pod_line) = so_job NO-LOCK NO-ERROR.
                  IF NOT AVAILABLE pod_det THEN DO:
                      MESSAGE '该零件有多个po行，请输入行号！' VIEW-AS ALERT-BOX ERROR.
                      UNDO,RETRY.
                  END.
                  ELSE mline = pod_line.
            END.
