/* GUI CONVERTED from iclotr.p (converter v1.78) Fri Oct 29 14:33:29 2004 */
...
/* Revision: 1.35.4.2    BY: A.R. Jayaram        DATE: 09/11/03  ECO: *P12D*  */
/* Revision: 1.35.4.3    BY: Swati Sharma        DATE: 03/26/04  ECO: *P1VZ*  */
/* $Revision: 1.35.4.4 $          BY: Nishit V            DATE: 06/01/04  ECO: *P24C*  */
/*****************************************************************************/
...
DEF VAR mline AS INT.
DEF VAR mprice AS INT.
DEF VAR mvend AS CHAR.
DEF VAR mdisc AS INT.
DEF VAR mloc AS CHAR.
DEF VAR mcur AS CHAR.
DEF VAR isok AS LOGICAL.
DEF VAR AVAIL_qty AS INT.
DEF VAR isadd AS LOGICAL.
DEF TEMP-TABLE match_lst
 FIELD  match_sess like mfguser
 FIELD  match_price LIKE xxpo_price
  FIELD  match_disc LIKE xxpo_disc
   FIELD  match_nbr LIKE xxpo_nbr
    FIELD match_loc LIKE xxpo_loc
     FIELD match_line like  xxpo_line
       FIELD match_cur LIKE xxpo_cur
       FIELD match_qty LIKE xxpo_qty
    FIELD match_id LIKE xxpo_conid
    INDEX match_sess IS PRIMARY match_sess match_id ASC.
...
/*GUI*/ if global-beam-me-up then undo, leave.
            ...
            FIND FIRST loc_mstr WHERE loc_site = site_from AND loc_loc = loc_from 
            AND CAN-FIND(FIRST vd_mstr WHERE vd_addr = loc_desc  NO-LOCK) NO-LOCK NO-ERROR.
               IF AVAILABLE loc_mstr THEN mvend =  loc_desc.
                  ELSE DO:
                      MESSAGE '寄售库位缺少供应商！' VIEW-AS ALERT-BOX ERROR.
                      UNDO,RETRY.
                  END.
                 isok = YES.
                 AVAIL_qty = 0.
                 FOR EACH xxpo_consign USE-INDEX xxpo_condttyp WHERE xxpo_vend = mvend and  xxpo_part = INPUT part AND xxpo_lot = lotser_from : 
            isadd = NO.
         FIND FIRST po_mstr WHERE po_nbr = xxpo_nbr NO-LOCK NO-ERROR.
           IF AVAILABLE po_mstr AND NOT po_sched THEN DO:
            FIND FIRST pod_det WHERE pod_nbr = xxpo_nbr AND pod_line = xxpo_line AND pod_qty_ord >= lotserial_qty NO-LOCK NO-ERROR.
             IF  AVAILABLE pod_det THEN isadd = YES.
             
            END.
            ELSE isadd = YES.
               
          IF isadd AND (lotserial_qty - AVAIL_qty > 0) AND (xxpo_qty - xxpo_cumconsume_qty > 0) THEN do:
              CREATE match_lst.
              ASSIGN match_sess = mfguser
                    match_id = xxpo_conid
                     match_price = xxpo_price
                match_disc = xxpo_disc
              match_nbr = xxpo_nbr
             match_line = xxpo_line
             match_loc = xxpo_loc
              match_cur = xxpo_cur
              match_qty = MIN((xxpo_qty - xxpo_cumconsume_qty),(lotserial_qty - AVAIL_qty)).
                  
          
           AVAIL_qty = AVAIL_qty + MIN((xxpo_qty - xxpo_cumconsume_qty),(lotserial_qty - AVAIL_qty)).
          
             IF lotserial_qty - AVAIL_qty <= 0 THEN LEAVE.
          END.
      END.
            
      
      IF AVAIL_qty = 0 THEN DO:   
                MESSAGE '无可消耗的记录，或超过可消耗数量,或对应PO的数量<现消耗数量！' VIEW-AS ALERT-BOX ERROR.
           UNDO,RETRY.
            END.
             
           IF AVAIL_qty < lotserial_qty THEN MESSAGE '可消耗数量小于欲消耗数量！' VIEW-AS ALERT-BOX QUESTION BUTTON YES-NO UPDATE isok.

              IF isok THEN do:
                  lotserial_qty = AVAIL_qty.
                  DISP lotserial_qty WITH FRAME a.
              END.
                  ELSE UNDO,RETRY.
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   then do:
      ...
      for first cnsix_mstr
         ...
         where cnsix_lotser = lotser_from
   ...
   end. /* IF using l_supplier-consignment */
