    ...
    do with frame a:
         ...
         /* find first absmstr where absmstr.abs_par_id = abs_mstr.abs_id no-lock no-error.*/
          /* IF AVAILABLE abs_mstr /*and available absmstr*/  THEN  DO:*/
           ...
           FOR EACH so_mstr WHERE so_cust = cm_addr  AND so_nbr >= ord AND so_nbr <= ord1  no-lock:
...
/* FIND ad_mstr WHERE ad_addr = SO_MSTR.SO_CUST AND (ad_type = 'ship-to' OR ad_type = 'customer') NO-LOCK NO-ERROR.*/
    for LAST sct_det where (sct_site = '1000' OR sct_site = '9000') AND sct_part = sod_part and sct_sim = 'standard' by sct_cst_date :
   
   
 IF sotot_qty <> 0 THEN DO:  
      soship_qty = 0. 
       FOR EACH tr_hist WHERE  tr_loc = '8888' AND (tr_type = 'iss-so' OR tr_type = 'iss-tr') AND (IF tr_type = 'iss-so' THEN tr_nbr = sod_nbr AND tr_line = sod_line ELSE YES )     AND tr_part = sod_part  AND (IF tr_type = 'iss-tr' THEN tr_qty_loc < 0  ELSE YES) NO-LOCK:
