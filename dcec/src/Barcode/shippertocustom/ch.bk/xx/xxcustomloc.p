  ...
  sod_line  column-label "行号"
   ...
   PT_DESC1 COLUMN-LABEL "零件描述"
    ...
    sotot_qty  column-LABEL "发运数量"
     sod_qty_ship   column-LABEL "已开票数量"  SPACE(1.5)
...
/* FIND ad_mstr WHERE ad_addr = SO_MSTR.SO_CUST AND (ad_type = 'ship-to' OR ad_type = 'customer') NO-LOCK NO-ERROR.*/
 ...
 IF sotot_qty <> 0 THEN DO:  
    mprice = 0.
     ...
     mcost = (sotot_qty - sod_qty_ship) * sct_cst_tot .
    
        open_qty = sotot_qty - sod_qty_ship.
        cuopen_qty = cuopen_qty + open_qty.
          cucost = cucost + mcost.
           cuship_qty = cuship_qty + sod_qty_ship.
           if page-size - line-counter < 2 then page.
                     if sod_price <> 0 or sod_list_pr <> 0 THEN mprice = if sod_price <> 0 then sod_price else sod_list_pr.
                                     else DO:
               FIND scx_ref WHERE scx_ord = so_nbr NO-LOCK NO-ERROR.
              ...
              find last pc_mstr where pc_list = sod_pr_list  and pc_part = sod_part and sod_um = pc_um and if pc_start = ? then yes else pc_start <= today and IF pc_expire = ? THEN YES ELSE pc_expire >= today and so_curr = pc_curr    no-lock no-error.
             ...
             /*  DISP pc_amt[1]. */
               mprice = pc_amt[1].
                 ...
                 ELSE DO:
                     ...
                     FIND LAST pi_mstr WHERE pi_list = sod_pr_list AND pi_amt_type = '1' AND pi_cs_code = so_cust AND pi_part_code = sod_part AND sod_um = pi_um AND so_curr = pi_curr AND pi_start <= TODAY AND IF pi_expire = ? THEN YES ELSE pi_expire >= TODAY NO-LOCK NO-ERROR.
               ...
               DISPLAY  sod_nbr sod_line sod_part pt_desc1  mprice sotot_qty sod_qty_ship  OPEN_qty  mcost  WITH FRAME b.
            if pt_desc2 <> '' then do:
                     down 1 .
                    put space(46).
                    put pt_desc2.
