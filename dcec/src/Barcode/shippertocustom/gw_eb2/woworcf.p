/* GUI CONVERTED from woworcf.p (converter v1.78) Fri Oct 29 14:34:27 2004 */
...
/* Revision: 1.16.1.1    BY: Mercy Chittilapilly   DATE: 11/27/03 ECO: *P1CM* */
/* $Revision: 1.16.1.2 $ BY: Vandna Rohira         DATE: 01/28/04 ECO: *P1LK* */
/******************************************************************************/
...
DEF SHARED VAR path AS CHAR FORMAT "x(20)" LABEL "½Ó¿Ú".
{mfworc.i }
{gpglefv.i}
undo_all = yes.
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   if nbr          <> "" and
      input wo_lot <> ""
   then
      find wo_mstr no-lock where wo_nbr = nbr using wo_lot no-error.

   if nbr           = "" and
      input wo_lot <> ""
   then
      find wo_mstr no-lock using wo_lot no-error.

   if nbr         <> "" and
      input wo_lot = ""
   then
      find first wo_mstr no-lock where wo_nbr = nbr no-error.
