...
/*xx**/  define variable up_abs_id like abs_mstr.abs_id .
   DEF BUFFER TRHIST FOR TR_HIST.
       DEF VAR M_QTY AS INT.
       DEF VAR RET_QTY AS INT.
...
/*GUI*/ if global-beam-me-up then undo, leave.
  /* DO TRANSACTION */
            ...
            if transfer_mode then DO:
                ...
                FIND FIRST loc_mstr WHERE loc_loc = '8888' AND loc_site = si_site  NO-LOCK.
      ...
      update
               ...
               with frame a.
               END.
            /*MESSAGE loc_to VIEW-AS ALERT-BOX.*/
               run p_vtmstr_chk .
...
/*GUI*/ if global-beam-me-up then undo, leave.
            ...
            */
            ...
            do transaction:
               ...
               clear frame b all.
               for each work_abs_mstr where work_abs_mstr.abs_item <> "" with frame b down THREE-D  :
                   mabsqty = work_abs_mstr.ABS_qty.
                   display work_abs_mstr.abs_item format "x(16)" column-label "Item" 
                     mabsqty COLUMN-LABEL "Qty to ship" work_abs_mstr.abs_site work_abs_mstr.abs_loc 
                     work_abs_mstr.abs_lot work_abs_mstr.abs_order format "X(8)" .
                  down 1 .    
               end.
              ...
              yn = yes.
               ...
               IF LOC_TO = '8888'  THEN DO:
                   ...
                   IF abs_mstr.ABS__chr01 = "" THEN DO:
                     
               for each work_abs_mstr no-lock where work_abs_mstr.abs_item <> "" , 
               each pt_mstr where pt_part = work_abs_mstr.abs_item no-lock with frame b down :

                  lotserial_qty = work_abs_mstr.abs_qty .
                global_part = pt_part .
                
                  assign
                     site_from   = work_abs_mstr.abs_site
                     loc_from    = work_abs_mstr.abs_loc
                     lotser_from = work_abs_mstr.abs_lot
                     lotref_from = work_abs_mstr.abs_ref.               

                  assign
                     site_to   = work_abs_mstr.abs_site
                     lotser_to = work_abs_mstr.abs_lot
                     lotref_to = work_abs_mstr.abs_ref.
...
/*FT37*/          if undo-input then undo, retry.
                  ...
                  find first loc_mstr where loc_site = work_abs_mstr.abs_site and loc_loc = loc_to  no-lock no-error.
...
/*K04X*/        assign  v_shipnbr  = substr(ABS_par_id,2,50)
/*K04X*/                v_shipdate = ?
/*K04X*/                v_invmov   = "".
...
/*GUI*/ if global-beam-me-up then undo, leave.
             ...
             END.
             ELSE MESSAGE "该货运已移入8888库位!" VIEW-AS ALERT-BOX.
            END.
             ELSE DO:
               /*  MESSAGE 'ok' VIEW-AS ALERT-BOX.*/
                 FOR EACH TR_HIST WHERE TR_HIST.TR_SHIP_ID = INPUT ABS_ID   AND TR_HIST.TR_LOC = '8888' AND TR_HIST.TR_TYPE = 'RCT-TR' NO-LOCK:
                     RET_QTY = 0.
                     FIND FIRST WORK_ABS_MSTR WHERE work_abs_mstr.ABS_ITEM <> '' AND work_abs_mstr.ABS_ITEM = tr_hist.TR_PART AND work_abs_mstr.ABS_LOT = TR_HIST.TR_SERIAL  NO-LOCK NO-ERROR.
     IF NOT AVAILABLE WORK_ABS_MSTR THEN DO:
     
    M_QTY = 0.
    FOR EACH TRHIST WHERE TRHIST.TR_SHIP_ID = TR_HIST.TR_SHIP_ID AND TRHIST.TR_PART = TR_HIST.TR_PART AND TRHIST.TR_LOC = '8888' AND TRHIST.TR_SERIAL = TR_HIST.TR_SERIAL  AND TRHIST.TR_TYPE BEGINS 'ISS' NO-LOCK:
   M_QTY = M_QTY + TRHIST.TR_QTY_LOC .
    END.
   RET_QTY = TR_HIST.TR_QTY_LOC + M_QTY.
     END.
     ELSE DO:
         IF work_abs_mstr.ABS_QTY < TR_HIST.TR_QTY_LOC THEN DO:
    M_QTY = 0.
    FOR EACH TRHIST WHERE  TRHIST.TR_SHIP_ID = TR_HIST.TR_SHIP_ID AND TRHIST.TR_PART = TR_HIST.TR_PART AND TRHIST.TR_LOC = '8888' AND TRHIST.TR_SERIAL = TR_HIST.TR_SERIAL  AND TRHIST.TR_TYPE BEGINS 'ISS' NO-LOCK:
   M_QTY = M_QTY + TRHIST.TR_QTY_LOC .
    END.
    RET_QTY = TR_HIST.TR_QTY_LOC + M_QTY - work_abs_mstr.abs_QTY.
         END.
     END.
      FIND FIRST PT_MSTR WHERE PT_PART = TR_HIST.TR_PART NO-LOCK NO-ERROR.
    /*  MESSAGE ret_qty VIEW-AS ALERT-BOX.*/
      IF RET_QTY <> 0 THEN DO:

 lotserial_qty = RET_QTY .
                global_part = pt_part .
                
                  assign
                     site_from   = TR_HIST.TR_site
                     loc_from    = TR_HIST.TR_loc
                     lotser_from = TR_HIST.TR_SERIAL
                     lotref_from = TR_HIST.TR_REF.               

                  assign
                     site_to   = TR_HIST.TR_site
                     lotser_to = TR_HIST.TR_SERIAL
                     lotref_to = TR_HIST.TR_REF.
                  /* MESSAGE site_from VIEW-AS ALERT-BOX.
                   MESSAGE loc_from VIEW-AS ALERT-BOX.
                   MESSAGE site_to VIEW-AS ALERT-BOX.
                   MESSAGE loc_to VIEW-AS ALERT-BOX.*/
                  from-loop1:
                  do on error undo:
/*GUI*/ if global-beam-me-up then undo, leave.
...
/*FT37*/          if undo-input then undo, retry.
                  ...
                  find first loc_mstr where loc_site = site_to and loc_loc = loc_to  no-lock no-error.
...
/*K04X*/        assign  v_shipnbr  = tr_hist.tr_ship_id
/*K04X*/                v_shipdate = ?
/*K04X*/                v_invmov   = "".
/*F003*/       lotserial = lotser_from.
                  {gprun.i ""xxicxfer.p"" "("""",
...
END.
    ...
    END.
             ...
             /**Update the transfer flag from company to customer location**/
              ...
              IF LOC_TO = '8888' AND ISRCT THEN DO:
               find first abs_mstr where recid(abs_mstr) = abs_recid exclusive-lock no-error.
               /*IF ismodi THEN */ ASSIGN  abs_mstr.abs__chr01 = "1"
                     up_abs_id = abs_mstr.abs_id .
               for each abs_mstr where abs_mstr.abs_par_id = up_abs_id
               exclusive-lock :
                  /*IF ismodi THEN */
                    
                   
                   assign abs_mstr.abs_loc = loc_to
                                             abs_mstr.ABS__qad08 = STRING(year(eff_date),"9999") + STRING(MONTH(eff_date), "99") + STRING(DAY(eff_date), "99").       .
