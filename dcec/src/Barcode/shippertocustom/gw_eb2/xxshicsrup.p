...
DEF SHARED VAR path AS CHAR  FORMAT "x(20)".
DEF VAR mavailable_qty AS DECIMAL.
DEF VAR mremain_qty AS DECIMAL.
DEF SHARED VAR ISCREATE AS LOGICAL.
DEF VAR xx_sr_tot_qty LIKE sr_qty.
DEF VAR xx_diff_qty LIKE sr_qty.
DEF VAR mstr AS CHAR FORMAT "x(150)".
{&ICSRUP-P-TAG3}
...
/*FOR EACH sr_wkfl where sr_userid = mfguser
                                      ...
                                      DELETE sr_wkfl.
                                       ...
                                       mavailable_qty = 0.
                                        ...
                                        xx_sr_tot_qty = 0.
                                          FOR EACH sr_wkfl where sr_userid = mfguser
                                            and sr_lineid = cline
                                            and sr_site = base_site
                                            and sr_loc = location NO-LOCK:
                                        xx_sr_tot_qty = xx_sr_tot_qty + sr_qty.
                                      
                                           END. 
                                  
                                           IF xx_sr_tot_qty >  lotserial_qty THEN     DO:
                                              xx_diff_qty = xx_sr_tot_qty - lotserial_qty.
                                              
                                                    FOR EACH sr_wkfl where sr_userid = mfguser
                                            and sr_lineid = cline
                                            and sr_site = base_site
                                            and sr_loc = location EXCLUSIVE-LOCK BY sr_lotser DESC:
                                              
                                              IF sr_qty <= xx_diff_qty THEN DO:
                                               xx_diff_qty = xx_diff_qty - sr_qty.
                                                DELETE sr_wkfl. 
                                                 END.
                                             ELSE do:
                                                   sr_qty = sr_qty - xx_diff_qty.
                                                  xx_diff_qty = 0.
                                                       
                                                       END.
                                                   
                                                     IF xx_diff_qty = 0 THEN LEAVE.
                                       ...
                                       END.
                                            mremain_qty = lotserial_qty - xx_sr_tot_qty.
                                   ...
                                   IF pt_lot_ser = 's' THEN DO:
                                       ...
                                       FIND FIRST sr_wkfl where sr_userid = mfguser
                                            and sr_lineid = cline
                                            and sr_site = base_site
                                            and sr_loc = location AND sr_lotser = ld_lot AND sr_ref = ld_ref NO-LOCK NO-ERROR.
                                     ...
                                     END.
                                      ELSE do:
                                       ...
                                       FIND FIRST sr_wkfl where sr_userid = mfguser
                                            and sr_lineid = cline
                                            and sr_site = base_site
                                            and sr_loc = location AND sr_lotser = ld_lot AND sr_ref = ld_ref EXCLUSIVE-LOCK NO-ERROR.
                                       IF NOT AVAILABLE sr_wkfl THEN DO:
   ...
   /* FOR EACH sr_wkfl where sr_userid = mfguser
                                  ...
                                  IF path <> '' THEN DO:
                                   ...
                                   sr_qty = DECIMAL(SUBSTR(mstr,61,8)).
                                         END.
                                         ELSE     MESSAGE  SUBSTR(mstr,35,18) + " ,欲倒入的序号不在库存中！" view-AS ALERT-BOX  error BUTTONS OK.                                  
