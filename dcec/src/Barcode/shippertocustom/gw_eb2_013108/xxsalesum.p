     ...
     hide message no-pause.
       ...
       chWorkSheet:Columns("A:ab"):ColumnWidth = 15.
        ...
        chWorkSheet:Columns("n:n"):ColumnWidth = 25.
         ...
         chWorkSheet:Range("a1:aa1"):Select().
     ...
     /*  pre_curr = idh_curr.*/
        ...
        ISFIRST = YES.
             ...
             IF so_slspsn[4] <> '' THEN DO: 
               ...
               XXID = RECID(ABS_MSTR).
                END.*/
          FOR EACH ABS_mstr WHERE ABS_loc = '8888' AND ABS__qad08 >= cmdate AND ABS__qad08 <= cmdate1 AND ABS_order = so_mstr.so_nbr   NO-LOCK BREAK BY (trim(ABS__qad08) + trim(abs_order) + trim(ABS_line)) :
        ...
        pre_taxable = NO.
          ...
          END.
                    ...
                    END.
                     ...
                     FOR EACH ih_hist WHERE ih_cust = cm_addr AND ((ih_slspsn[1] >= slspsn AND ih_slspsn[1] <= slspsn1) 
                                       OR (ih_slspsn[2] >= slspsn AND ih_slspsn[2] <= slspsn1) OR (ih_slspsn[3] >= slspsn AND ih_slspsn[3] <= slspsn1)
                                       OR (ih_slspsn[4] >= slspsn AND ih_slspsn[4] <= slspsn1)) AND ih_nbr >= nbr AND ih_nbr <= nbr1 AND NOT CAN-FIND(FIRST so WHERE ord = ih_nbr NO-LOCK )
                          NO-LOCK:
...
FIND FIRST sp_mstr WHERE sp_addr = salerep1 NO-LOCK NO-ERROR.
             ...
             /*         
               ...
               XXID = RECID(ABS_MSTR).
                END.*/
          FOR EACH ABS_mstr WHERE ABS_loc = '8888' AND ABS__qad08 >= cmdate AND ABS__qad08 <= cmdate1 AND ABS_order = ih_hist.ih_nbr   NO-LOCK BREAK BY (trim(ABS__qad08) + trim(abs_order) + trim(ABS_line)):
