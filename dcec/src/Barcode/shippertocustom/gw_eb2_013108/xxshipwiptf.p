   ...
   {mfphead.i}
         ...
         mopamt = 0.
               ...
               /* FOR EACH ABS_mstr WHERE ABS_loc = '8888' /*AND SUBSTR(ABS__qad08,1,4) = string(m_year) AND SUBSTR(ABS__qaD08,5,2) = STRING(m_month,"99")*/  AND ABS_item >= part AND ABS_item <= part1   NO-LOCK  BY ABS_item  BY ABS_order BY ABS_line:
                    FIND FIRST tr_hist WHERE tr_type = 'iss-so' AND tr_loc = '8888' AND tr_nbr = ABS_order AND string(tr_line) = ABS_line AND tr_ship_id = SUBSTR(ABS_par_id,2,50) AND  NO-LOCK NO-ERROR.
                    ...
                    FIND FIRST pt_mstr WHERE pt_part = ABS_item AND pt_pm_code = 'c' NO-LOCK NO-ERROR.
                     ...
                     XXID = RECID(ABS_MSTR).
                      END.
                     END.*/
               /*  FOR LAST ABS_mstr WHERE ABS_loc = '8888' /*AND SUBSTR(ABS__qad08,1,4) = string(m_year) AND SUBSTR(ABS__qaD08,5,2) = STRING(m_month,"99")*/  AND ABS_item >= part AND ABS_item <= part1    AND CAN-FIND(FIRST pt_mstr WHERE pt_part = ABS_item AND pt_pm_code = 'c' NO-LOCK) 
                          NO-LOCK   BY ABS_order BY ABS_line:
                     ...
                     END.*/
                     FOR EACH ABS_mstr WHERE ABS_loc = '8888' /*AND SUBSTR(ABS__qad08,1,4) = string(m_year) AND SUBSTR(ABS__qaD08,5,2) = STRING(m_month,"99")*/  AND ABS_item >= part AND ABS_item <= part1     AND CAN-FIND(FIRST pt_mstr WHERE pt_part = ABS_item AND pt_pm_code = 'c' NO-LOCK) 
                               NO-LOCK    BREAK BY (trim(ABS_order) + trim(ABS_line)):

                    /*
                    IF NOT isFIRST AND (mcheckstr <>  ABS_order + ABS_line) THEN do:
                     m_tr_qty = 0.
                    FOR EACH tr_hist WHERE tr_type = 'iss-so' AND tr_loc = '8888' AND tr_nbr = pre_order AND string(tr_line) = pre_line AND year(tr_effdate) = m_year AND MONTH(tr_effdate) = m_month  NO-LOCK:
                    m_tr_qty = m_tr_qty + tr_qty_loc.
                        END.
                        mopamt = mcst * m_tr_qty.
