...
iscim = NO.
OUTPUT TO 'c:\supp_error_log.txt' .   
FOR EACH supp:
    pass = YES.
        ...
        PUT '"'.
             ...
             PUT UNFORMAT t_sup_fax2.
              PUT '"'.
               PUT space(1).
               PUT '"'.
             PUT UNFORMAT string(date(INTEGER(SUBSTR(t_sup_regdate,6,2)),INTEGER(SUBSTR(t_sup_regdate,9,2)),INTEGER(SUBSTR(t_sup_regdate,1,4)))) .
              PUT '"'.
               PUT space(1).
               PUT SKIP.
               PUT '-'.
...
END.
     /* END.*/*/
    OUTPUT CLOSE.
    OUTPUT TO 'c:\supp_check_log.txt'.
    FOR EACH t_error:
        PUT SKIP.
        PUT t_er_supcode.
        PUT SPACE(1).
       ...
       IF NOT AVAILABLE ac_mstr THEN do:
         ...
         t_er_supcode = supp.t_sup_code.
          ...
          t_er_mess = 'Please check sup account code!'.
               END.
              END.
     RUN strcheck('sup_regdate','ad_date',supp.t_sup_regdate,YES). 
...
PROCEDURE strcheck:
    ...
    DEF VAR i AS INT.
    IF isdate AND LENGTH(estr) <> 10 THEN DO:
    CREATE t_error.
...
DEF  VAR i AS INT.
 ...
 IF ihnode:NUM-CHILDREN = 0  THEN DO:
     ...
     CASE ihnode:NAME:
   when "contact_name" THEN DO:
        
         t_sup_cname =  ihnode:GET-ATTRIBUTE('value').
         MESSAGE  t_sup_cname VIEW-AS ALERT-BOX.  
         END.
 ...
 when "sup_opflag" THEN DO:
  ...
  IF ihnode:NAME = 'po_header'THEN DO:
  isread = YES.
