...
DEFINE FRAME a
    ...
    SPACE(48)  btn_prt1 SPACE(1)  btn_prt2 SPACE(1)  btn_prt3 SPACE(1)
    WITH WIDTH 80  THREE-D TITLE "条码查询"  SIDE-LABELS  NO-UNDERLINE.

ON 'choose':U OF btn_prt1 
DO:
    {bcgetprt.i}
        {gprun.i ""bccopr.p"" "(input  t_co_mstr.t_co_code, input ""SIN"", input pname, input no)"}
    RETURN.
END.
...
DO:
    {bcgetprt.i}
        {gprun.i ""bccopr.p"" "(input  t_co_mstr.t_co_code, input ""DOU"", input pname, input no)"}
    RETURN.
END.
...
DO:
    {bcgetprt.i}
        {gprun.i ""bccopr.p"" "(input  t_co_mstr.t_co_code, input ""FUL"", input pname, input no)"}
    RETURN.
END.
...
REPEAT :
     ...
     bcsta = "FINI-REL".
     DISPLAY bcsta WITH FRAME a.
    UPDATE part part1 lot lot1  WITH FRAME a.
    ...
    REPEAT:
        UPDATE b_co btn_prt1 btn_prt2 btn_prt3 WITH FRAME a.
    END.
END.
