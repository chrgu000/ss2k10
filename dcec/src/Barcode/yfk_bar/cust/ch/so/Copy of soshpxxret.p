...
DEF FRAME a
    ...
    space(23) btn_view 
WITH  WIDTH 30 TITLE "YFK退货"  SIDE-LABELS  NO-UNDERLINE THREE-D.
...
DO:
    ...
    IF bcstatus NE "FINI-SHIPPED" THEN DO:
        STATUS INPUT "该条码状态为:" + bcstatus + "不能退货,退出".
        RETURN.
    END.
...
*/
    ...
    FIND FIRST xshloc_mstr NO-LOCK WHERE xshloc_part = bcpart
        ...
        AND xshloc_cust_loc = bcloc NO-ERROR.
    IF AVAILABLE xshloc_mstr THEN DO:
        tloc = xshloc_cust_loc.
        bcloc = xshloc_yfk_loc.
        ...
        IF AVAILABLE loc_mstr THEN DO:
            ...
            STATUS INPUT "退货类型为" + shptype .
        END.
        ELSE DO:
            STATUS INPUT "无法在loc_mstr找到退货库位".
            RETURN.
        END.
    END.
    ELSE DO:
    ...
    IF AVAILABLE b_shp_wkfl THEN DO:
        STATUS INPUT "本次退货存不同退货库位,退出".
        RETURN.
    END.


    FIND FIRST ld_det NO-LOCK WHERE ld_part  = bcpart AND ld_loc =tloc /*AND ld_lot = bclot*/  NO-ERROR.
    IF NOT AVAILABLE ld_det  THEN DO:
         STATUS INPUT "在库位" + tloc + "无法发现该零件库存,退出".
         RETURN.
    END.
    ELSE DO:
        IF ld_qty_oh < qty_ship THEN DO:
            STATUS INPUT "库位" + tloc + "批号" + bclot + "不足，退出".
            RETURN.
        END.
    END.
