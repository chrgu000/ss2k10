...
DEFINE BUTTON btn_exec LABEL "调拨".
...
DEFINE FRAME a
    ...
    b_shp
    SKIP(.3)
    space(16)  SPACE(2)  btn_exec /*SPACE(2) btn_quit*/
...
PROCEDURE exec:
 ...
 DO ON ERROR UNDO, LEAVE:
         ...
         FOR FIRST b_shp_wkfl WHERE b_shp_batch = batchid:
         END.
         FOR EACH t_shp:
             FIND FIRST sod_det  NO-LOCK WHERE sod_nbr = b_shp_sonbr AND sod_part = t_part NO-ERROR.
             IF AVAILABLE sod_det THEN DO:
                 IF sod_qty_ord NE t_qty THEN DO:
                     MESSAGE "零件" + t_part + "与定单数量不匹配".
                     LEAVE mainloop.
                 END.
             END.
         END.
     ...
     FOR EACH b_shp_wkfl WHERE b_shp_batch = batchid AND b_shp_type = "sotr" AND b_shp_status = "TOCIM":
...
PROCEDURE sotr:
    ...
    FOR EACH b_shp_wkfl WHERE b_shp_batch = batchid AND b_shp_type = "sotr" AND b_shp_status = "TOCIM"
        ...
        IF LAST-OF(b_shp_lot) THEN DO:
            {gprun.i ""mgwrbf.p"" "(INPUT b_shp_sonbr,
                                                 INPUT b_shp_soline,
                                                 INPUT """",
                                                 INPUT b_shp_part,
                                                 INPUT b_shp_lot,
...
PROCEDURE sotrprt:
    STATUS INPUT "开始打印调拨类型出门证".
    DEFINE VARIABLE cust LIKE ad_addr.
    FIND FIRST b_shp_wkfl NO-LOCK  WHERE b_shp_batch = batchid AND 
        b_shp_type = "SOTR" NO-ERROR. 
    IF AVAILABLE b_shp_wkfl THEN DO:
       /* RUN GetShipperNbr(OUTPUT shippernbr).*/

        FOR EACH b_shp_wkfl WHERE b_shp_batch = batchid AND b_shp_type = "SOTR" BREAK BY b_shp_part:
            ACCUMULATE b_shp_qty (TOTAL BY b_shp_part).
