...
DEFINE BUTTON btn_rec LABEL "入库".
...
DEF FRAME a
    ...
    SKIP(3)
     space(22)  btn_rec 
WITH  WIDTH 30 TITLE "返工入库"  SIDE-LABELS  NO-UNDERLINE THREE-D.
...
DO:
    ...
    FIND FIRST b_rwk_mstr EXCLUSIVE-LOCK WHERE b_rwk_code = bccode NO-ERROR.
    IF AVAILABLE b_rwk_mstr THEN DO:
         ASSIGN b_rwk_status = "FINISH"
                     b_rwk_end_date = TODAY.
          {bcco001.i bccode bcpart bcqty tosite toloc bcloc """"}
          ...
          END.
          ELSE DO:
              STATUS INPUT "返工入库出现问题".
              LEAVE.
          END.
          {bcco002.i ""FINI-GOOD""}
    END.
    ELSE DO:
        MESSAGE "条码：" + bccode + "没有处于返工状态，退出".
        RETURN.
    END.
