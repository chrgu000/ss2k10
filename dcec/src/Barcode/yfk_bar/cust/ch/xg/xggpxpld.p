/* GUI CONVERTED from gpxpld.p (converter v1.75) Wed Jan 24 02:16:39 2001 */
...
/*G656*/ define variable assy_op as char format "x(20)" no-undo.
...
/*N0CD*/    where bom_parent = xpldcomp no-lock:
...
/*N0CD*/            ps_ps_code ps_qty_per ps_qty_per_b ps_qty_type ps_scrp_pct
/*N0CD*/            ps_start)
/*N0CD*/     where recid(ps_mstr) = record[level] no-lock:
/*N0CD*/  end. /* FOR FIRST PS_MSTR */
...
/*N0CD*/     assign
          ...
          where ps_par = xpldcomp no-lock no-error.
...
/*G656*/   if level = 1 then assy_op = string(ps_op).
/*zx*/     else assy_op = assy_op + "." + string(ps_op).
/*N0CD*/   assign
              phantom = no
              iss_pol = yes.
...
/*N0CD*/      where pt_part = ps_comp no-lock:
...
/*N0CD*/                ptp_site)
/*N0CD*/         where ptp_part = ps_comp and
 ...
 *J31Z** END DELETE SECTION **/
          ...
          where ps_par = xpldcomp no-lock no-error.
           ...
           find next ps_mstr use-index ps_parcomp where ps_par = xpldcomp
           no-lock no-error.
        end.
     end. /*repeat*/
