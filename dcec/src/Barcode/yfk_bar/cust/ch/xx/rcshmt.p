...
/* Revision: 1.20.1.3 BY: Sunil Fegade          DATE: 09/15/03   ECO: *P134*  */
/* Revision: 1.20.1.4 BY: Dan Herman            DATE: 09/10/04   ECO: *M1LL*  */
/* $Revision: 1.20.1.5 $ BY: Manish Dani           DATE: 02/21/05   ECO: *P392*  */
/* Revision: 1.20.1.5 BY: Manish Dani           DATE: 02/21/05   ECO: *P392*  */
/* Revision: 1.20.1.6 BY: Prashant Parab        DATE: 10/04/06   ECO: *P58V*  */
/* Revision: 1.20.1.7 BY: Antony LejoS          DATE: 12/07/06   ECO: *P5HM*  */
/* Revision: 1.20.1.8 BY: Amandeep Saini        DATE: 07/31/07   ECO: *P636*  */
/* $Revision: 1.20.1.9 $ BY: Amandeep Saini    DATE: 08/24/07  ECO: *P637* */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
...
{icshxt01.i} /* API shipper temp tables */
...
on entry of abs_shipfrom in frame a do:
   /*V8! run p-handle-entry in global-drop-down-utilities.*/
   run enterShipFrom.
...
repeat:
   ...
   then do:
      ...
      if not
         (can-find (ad_mstr where ad_addr = i_src_addr) and
          can-find (ad_mstr where ad_addr = i_dest_addr))
      then
         leave main_blk.
      ...
      for first sg_mstr
         fields (sg_desc sg_grp)
         where sg_grp = v_shipgrp
         no-lock:
      ...
      for first sgid_det
         fields (sgid_default sgid_grp sgid_inv_mov sgid_ship_nr_id)
         where sgid_grp = sg_grp and
         can-find (first im_mstr where
   ...
   for first abs_mstr
      fields (abs_canceled abs_cons_ship abs_eff_date abs_format abs_id
   ...
   repeat with frame a on endkey undo main_blk, leave main_blk:
      ...
      prompt-for
         ...
         then do:
            ...
            {mfnp05.i
               abs_mstr
               abs_id
               " abs_id  begins 's' or (execname <> 'rcshmt.p'
      ...
      if c-application-mode <> "API" then do:
         ...
         if v_upd_addrs then do:
            assign
               {mfaiset.i absShipfrom ttShipper.shipfrom}
               {mfaiset.i absShipto   ttShipper.shipto}
               {mfaiset.i absId       ttShipper.id}
               {mfaiset.i sgInvMov    ttShipper.invMov}.
         end.
         else do:
            assign
               {mfaiset.i absId    ttShipper.id}
      ...
      for first abs_mstr
         fields (abs_canceled abs_cons_ship abs_eff_date abs_format abs_id
                 abs_inv_mov abs_lang abs_nr_id abs_qty abs_shipfrom abs_shipto
                 abs_shp_date abs_status abs_type abs__qad01)
         where abs_shipfrom =       absShipFrom
         and   abs_id       = "s" + absId
         no-lock:
      end. /* FOR FIRST abs_mstr */
      ...
      then do:
         /* VALIDATE SHIPFROM */
         if not can-find (si_mstr where si_site = absShipFrom)
         then do:
            /* SITE DOES NOT EXIST */
         ...
         /* VALIDATE SHIPTO */
         if not can-find(ad_mstr where ad_addr = absShipto)
         then do:
         ...
         if i_tr_type = "ISS-WO" and
            {&ICSHMT-P-TAG12}
            not can-find (vd_mstr where vd_addr = absShipto)
         then do:
         ...
         then do:
            ...
            do while v_addr <> "":
               for first cm_mstr
                  ...
                  where cm_addr = v_addr
                  no-lock:
               end . /* FOR FIRST cm_mstr */
               for ad_mstr
                  fields (ad_addr ad_line1 ad_name ad_ref)
                  where ad_addr = v_addr
                  no-lock:
      ...
      /* VAlIDATE NON-BLANK INVENTORY MOVEMENT CODE */
         ...
         if not can-find
            (im_mstr where im_inv_mov =  sgInvMov)
         then do:
      ...
      do transaction:
         ...
         then do:
            ...
            then do:
               if c-application-mode <> "API" then
                  display substring(abs_id,2) @ abs_id.
               assign absID = substring(abs_id,2).
               run ip_refresh_id.
               leave create_blk.
            end.  /* IF v_cont */
         end.  /* IF AVAILABLE abs_mstr */
         ...
         assign
            ...
            abs_id       = "s" + absId
            abs_type     = if i_tr_type = "ISS-WO" and sgInvMov = ""
                           then "u"
                           else "s"
            abs_shipto   = absShipto
            abs_inv_mov  = sgInvMov
            abs_nr_id    = if sgInvMov <> ""
                           then sgid_ship_nr_id
                           else shc_ship_nr_id
            abs_shp_date = today
            abs_qty      = 1.
...
PROCEDURE ip_refresh_shipfrom:
   for first si_mstr
      fields (si_desc si_site)
      where si_site = absShipfrom
      no-lock:
   end. /* FOR FIRST si_mstr */
   for first ad_mstr
      fields (ad_addr ad_line1 ad_name ad_ref)
      where ad_addr = absShipfrom
      no-lock:
   end. /* FOR FIRST ad_mstr */
   if c-application-mode <> "API" then
   display
...
PROCEDURE ip_refresh_grp:
   ...
   with frame a.
   if available sg_mstr then sgGrp = sg_grp.
   else sgGrp = "".
   if c-application-mode <> "API"
   then do:
      if absShipfrom <> input frame a abs_mstr.abs_shipfrom
      then
         absShipfrom = input frame a abs_mstr.abs_shipfrom.
      if absId <> input frame a abs_mstr.abs_id
      then
         absId = input frame a abs_mstr.abs_id.
       if absShipto <> input frame a abs_mstr.abs_shipto
      then
         absShipto = input frame a abs_mstr.abs_shipto.
   end. /* IF c-application-mode <> "API" */
   {&ICSHMT-P-TAG21}
   for first b_abs_mstr
      fields (abs_canceled abs_cons_ship abs_eff_date abs_format abs_id
              abs_inv_mov abs_lang abs_nr_id abs_qty abs_shipfrom abs_shipto
              abs_shp_date abs_status abs_type abs__qad01)
      where b_abs_mstr.abs_shipfrom = absShipfrom
...
PROCEDURE ip_refresh_invmov:
   for first im_mstr
      fields (im_desc im_inv_mov im_tr_type)
      where im_inv_mov = sgInvMov
      no-lock:
...
PROCEDURE enterShipFrom:
   if c-application-mode <> "API" then
      v_save_shipfrom = input frame a abs_shipfrom.
   else
      v_save_shipfrom = absShipFrom.
END PROCEDURE.
PROCEDURE enterAbsID:
   if c-application-mode <> "API" then
      v_save_id = input frame a abs_id.
   else
      v_save_id = absID.
END.
PROCEDURE enterShipTo:
   if c-application-mode <> "API" then
      v_save_shipto = input frame a abs_shipto.
   else
      v_save_shipto = absShipTo.
END.
PROCEDURE enterInvMov:
   if c-application-mode <> "API" then
      v_save_invmov = input frame a v_invmov.
   else
