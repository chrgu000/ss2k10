...
procedure createpartlist:
   ...
   /*Jeff*/for each xpal_ctrl where xpal_line = p_line 
                AND xpal_cust_def <> "m111a" no-lock:   /*exclude semi-product*/
...
/*xwh0610      and xwo_lot = tmplot and xwo_pdnbr = "" no-lock no-error.*/
      and xwo_lot = tmplot and xwo_cust = xpal_cust AND xwo_pdnbr = "" no-lock no-error.
...
/*est*/   {bcrun.i ""bckbcom01.p"" "(INPUT part, OUTPUT sucess)"}
...
/*xwh0610      and xwo_lot = tmplot and xwo_pdnbr = ttfg_nbr no-lock no-error.*/
       and xwo_lot = tmplot and xwo_cust = ttfg_cust AND xwo_pdnbr = ttfg_nbr no-lock no-error.
      ...
      disp part pt_desc1 defcust cname defdloc ldesc fulqty 
           subqty v_cnt v_qty_short v_rmks v_close
      with frame c.
      
      update subqty v_close btnY btnN with frame c.
...
put "1118--" time "==" pkkpart skip.
               ...
               if avail xwosd_det then do:
put "1119--" time "==" pkkpart skip.
                  find bfxwod where rowid(bfxwod) = rowid(xwosd_det) 
                  exclusive-lock no-error.
                  if avail bfxwod then do:
put "1120--" time "==" pkkpart skip.
                     assign	
