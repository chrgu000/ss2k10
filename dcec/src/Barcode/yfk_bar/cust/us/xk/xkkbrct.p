...
/*est*/  FORM 
             ...
             SKIP(0.3)
             effDate     colon 16 label "有效日"
             pnbr        colon 16 label "订单号"
             xPrtRn	 colon 16 
             xPrtKb	 colon 16 
             SKIP(0.3)
         with frame a SIDE-labelS width 80 THREE-D .
...
/*est*/    CURRENT-WINDOW:WIDTH = 40.
/*est*/    CURRENT-WINDOW:HEIGHT = 20.
...
/*est*/  IF po THEN DO:
                     MESSAGE "外部要货单" VIEW-AS ALERT-BOX.
/*est*/          {bcrun.i ""bckbrct01.p"" "(input ""EXTERNAL"", output sucess)"}
                     IF NOT sucess THEN
                       UNDO mainloop, RETRY mainloop.
             END.
             ELSE DO:
                      MESSAGE "内部要货单" VIEW-AS ALERT-BOX.
/*est*/           {bcrun.i ""bckbrct01.p"" "(input ""INTERNAL"", output sucess)"}     
                      IF NOT sucess THEN
                        UNDO mainloop, RETRY mainloop.
/*est*/  END.
             {xkbcls.i}
