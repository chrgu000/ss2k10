/* GUI CONVERTED from icintr.p (converter v1.78) Wed Apr 11 23:46:19 2012 */
...
/* Revision: 1.47.1.1    BY: Dilip Manawat       DATE: 08/11/06  ECO: *P4ZK*  */
/* Revision: 1.47.1.2    BY: Munira Savai        DATE: 11/14/06  ECO: *P5F9*  */
/* Revision: 1.47.1.3    BY: Prashant S          DATE: 02/05/08  ECO: *P6JV*  */
/* Revision: 1.47.1.4    BY: Avishek Chakraborty DATE: 11/18/08  ECO: *Q20F*  */
/* Revision: 1.47.1.5    BY: Xiaolei Meng        DATE: 04/15/09  ECO: *Q2M9*  */
/* Revision: 1.47.1.6    BY: Sandeep Rohila      DATE: 08/10/09  ECO: *Q35K*  */
/* Revision: 1.47.1.7    BY: Rajat Kulshreshtha  DATE: 04/15/10  ECO: *Q40C*  */
/* Revision: 1.47.1.9    BY: Tushar Shetty       DATE: 09/22/11  ECO: *Q51N*  */
/* $Revision: 1.47.1.10 $  BY: Tushar Shetty  DATE: 04/12/12  ECO: *Q5B5* */
/*-Revision end---------------------------------------------------------------*/
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   if valid-handle(menu menu-bar-ptr:handle)
      and current-window:menubar = ?
   then do:
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   repeat on endkey undo mainloop, retry mainloop:
      ...
      then
         ...
         end. /* IF i = 0 AND AVAILABLE pt_mstr */
         else
            if i = 1
            then do:
               for first sr_wkfl
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         then do:
            ...
            /* WHEN DOING SSM RESTOCKING, DON'T PROMPT FOR ORDER AND LINE */
            ...
            if issrct = "ISS" then
               assign
                  ...
                  {mfaiset.i dr_sub  ttInventoryTrans.drSub}
                  {mfaiset.i dr_cc   ttInventoryTrans.drCc}.
            else
               assign
                  {mfaiset.i cr_acct ttInventoryTrans.crAcct}
                  {mfaiset.i cr_sub  ttInventoryTrans.crSub}
...
PROCEDURE defaultDepotOrderRestock:
   ...
   {pxrun.i &PROC  = 'getOidDosrDet' &PROGRAM = 'fssmxr.p'
            &HANDLE=ph_fssmxr
