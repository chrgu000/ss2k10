/* GUI CONVERTED from popomta.p (converter v1.78) Fri Mar 16 04:34:20 2012 */
...
/* Revision: 1.83.1.8   BY: Ruma Bibra            DATE: 09/18/06  ECO: *P565* */
/* Revision: 1.83.1.9   BY: Mochesh Chandran      DATE: 16/11/06  ECO: *P4W8* */
/* Revision: 1.83.1.10  BY: Munira Savai          DATE: 04/18/07  ECO: *P5TK* */
/* Revision: 1.83.1.11  BY: Sundeep Kalla         DATE: 08/14/07  ECO: *P60Q* */
/* Revision: 1.83.1.12  BY: Archana kirtane       DATE: 08/17/07  ECO: *P64J* */
/* Revision: 1.83.1.14  BY: Sundeep Kalla         DATE: 10/22/07  ECO: *P6BF* */
/* Revision: 1.83.1.15  BY: Vivek Kamath          DATE: 12/05/07  ECO: *P6G7* */
/* Revision: 1.83.1.17  BY: Ashim Mishra          DATE: 02/27/08  ECO: *P6M9* */
/* Revision: 1.83.1.18  BY: Gerard Menezes        DATE: 05/28/08  ECO: *P6V3* */
/* Revision: 1.83.1.19  BY: Namita Patil          DATE: 06/23/08  ECO: *P6W1* */
/* Revision: 1.83.1.20  BY: Alex Joy              DATE: 07/10/08  ECO: *P6WM* */
/* Revision: 1.83.1.23  BY: Gerard Menezes        DATE: 09/24/08  ECO: *P70S* */
/* Revision: 1.83.1.24  BY: Amit Kumar            DATE: 06/12/09  ECO: *Q30J* */
/* Revision: 1.83.1.25  BY: Chandrakant Ingale    DATE: 02/02/10  ECO: *Q3T3* */
/* Revision: 1.83.1.26  BY: Chandrakant Ingale    DATE: 02/12/10  ECO: *Q3V2* */
/* Revision: 1.83.1.27  BY: Avishek Chakraborty   DATE: 03/10/10  ECO: *Q2P7* */
/* Revision: 1.83.1.28  BY: Ruchita Shinde        DATE: 09/28/10  ECO: *Q4DB* */
/* Revision: 1.83.1.30  BY: Rajesh Srivastava     DATE: 11/25/11  ECO: *Q54P* */
/* Revision:  $  BY: Nisha Thasale         DATE: 03/13/12  ECO: *Q58L* */
/*-Revision end---------------------------------------------------------------*/
/*                                                                            */
/******************************************************************************/
...
define variable l_min_disc       as decimal initial -999.99 no-undo.
...
define variable l_tol_cost_chk   like mfc_logical                   no-undo.
define variable l_req_nbr        as   character                     no-undo.
...
/*GUI*/ if global-beam-me-up then undo, leave.
            ...
            if old_pod_site <> pod_site
            then do:
               {inmrp.i &part=pod_part &site=old_pod_site}
            end. /* IF old_pod_site <> pod_site */
...
/*GUI*/ if global-beam-me-up then undo, leave.
            ...
            then
               pause 0.
            old_site = old_pod_site.
            {gprun.i ""popomth.p""
               "(input l_delay,
                 input-output l_pod_qty_ord,
                 input-output l_pod_pur_cost,
                 input l_pod_site)" }
/*GUI*/ if global-beam-me-up then undo, leave.
...
/*GUI*/ if global-beam-me-up then undo, leave.
                  ...
                  then do:
                     assign
                        ...
                        then
                           assign
                              l_display_disc = pod_disc_pct
                              pod_disc_pct   = l_max_disc.
                        else if pod_disc_pct < l_min_disc
                        then
                           assign
                              l_display_disc = pod_disc_pct
                              pod_disc_pct   = l_min_disc.
               ...
               then do:
                  if can-find(rqd_det
                                 ...
                                 and    rqd_aprv_stat <> "2")
                  then do:
                     /*CANNOT CHNAGE UNIT COST- REQUISITION IS UNAPPROVED*/
                     {pxmsg.i
                        &MSGNUM=7752
                        &ERRORLEVEL=3}
                     undo, retry.
               ...
               if l_display_disc <> pod_disc_pct
                  and  l_display_disc <> 0
                  and (pod_disc_pct   = l_max_disc
                  or   pod_disc_pct   = l_min_disc)
               then
                  pod_disc_pct = l_display_disc.
               ...
               if po_pr_list2 <> ""
               then do:
                  {pxrun.i &PROC='setNetPriceDecimalAndWholeNumber'
                           &PROGRAM='popoxr1.p'
                           &PARAM="(buffer pod_det,
                                    input old_pur_cost,
                                    input old_disc_pct,
              ...
              if (old_qty_ord * old_pur_cost) <> (pod_qty_ord * pod_pur_cost)
          then
             l_tol_cost_chk = yes.
          else
             l_tol_cost_chk = no.
...
/*GUI*/ if global-beam-me-up then undo, leave.
  /* SETC-1 */
         ...
         if using_grs
            ...
            and pod_req_nbr > ""
         then do:
            if lookup(pod_req_nbr, l_req_nbr) = 0
            then do:
               assign
                  l_req_nbr = l_req_nbr + "," + pod_req_nbr
                  new_po    = yes.
            end. /* IF LOOKUP(pod_req_nbr, l_req_nbr1) = 0 */
            if new_po
            then do:
...
/*GUI*/ if global-beam-me-up then undo, leave.
 /* Do transaction */
      ...
      /* AFTER THE CREATION OF THE PO LINE WITH THE REQ */
      /* WE CAN DELETE THE qad_wkfl LINK WITH THE PO */
