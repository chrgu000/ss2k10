/* GUI CONVERTED from popomtd.p (converter v1.78) Sun Mar 13 21:45:04 2011 */
...
/* Revision: 1.14.4.30.1.12  BY: Ruchita Shinde    DATE: 05/19/10 ECO: *Q42X* */
/* Revision: 1.14.4.30.1.13  BY: Ambrose A         DATE: 07/23/10 ECO: *Q48W* */
/* $Revision: 1.14.4.30.1.14 $ BY: Ravi Swami      DATE: 03/08/11 ECO: *Q4P6* */
/*-Revision end---------------------------------------------------------------*/
...
/*GUI*/ if global-beam-me-up then undo, leave.
  /* IF can-find (FIRST vp_mstr where ... */
   ...
   /* FOR BLANKET PO GET THE TYPE MEMO or INVENTORY */
   if l_pod_type = "B"
   then do:
   ...
   then do:
      ...
      {pxrun.i &PROC='validateProjectCode' &PROGRAM='glacxr.p'
               &PARAM="(input pod_project)"
               &NOAPPERROR=true
               &CATCHERROR=true}

   end. /* ELSE IF l_pod_type = " " */
