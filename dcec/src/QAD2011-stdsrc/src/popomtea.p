/* GUI CONVERTED from popomtea.p (converter v1.78) Wed Jul 11 03:03:37 2007 */
...
/* Revision: 1.26.2.2      BY: Naseem Torgal        DATE:07/12/06 ECO: *P4WV* */
/* Revision: 1.26.2.3      BY: Dhananjay Govekar    DATE:03/07/07 ECO: *Q12T* */
/* Revision: 1.26.2.4      BY: Anuradha K.          DATE:05/15/07 ECO: *Q15C* */
/* $Revision: 1.26.2.5 $            BY: Antony LejoS         DATE:07/09/07 ECO: *Q17N* */
/*-Revision end---------------------------------------------------------------*/
...
define variable l_errflag                as integer no-undo.
...
/*GUI*/ if global-beam-me-up then undo, leave.
  /*if c-application-mode = "API" */
   ...
   assign l_db        = global_db
          new_site    = si_site
          global_part = pod_part.
   if execname = "popomt.p"
   then do:

     /* LOGIC TO SWITCH DOMAIN TO THAT OF THE PO LINE SITE  */

     if si_db <> global_db then
       {gprun.i ""gpmdas.p"" "(input si_db,output l_errflag)" }
/*GUI*/ if global-beam-me-up then undo, leave.


     /* LOGIC TO CHECK AVAILABILITY OF ITEM IN in_mstr */
     {gprun.i ""gpptsi02.p""}
/*GUI*/ if global-beam-me-up then undo, leave.
