/* GUI CONVERTED from popomtr2.p (converter v1.78) Wed Mar  9 20:22:53 2005 */
...
/* Revision: 1.28     BY: Paul Donnelly (SB)      DATE: 06/28/03  ECO: *Q00J* */
/* Revision: 1.29     BY: Kirti Desai             DATE: 10/31/03  ECO: *P188* */
/* $Revision: 1.30 $       BY: Annapurna V             DATE: 03/08/05  ECO: *P3BS* */
/*-Revision end---------------------------------------------------------------*/
...
FORM /*GUI*/ 
   p_pod_req_nbr
with frame a overlay row p_frame_row column 14 THREE-D /*GUI*/.
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   then do:
      ...
      editing:
         ...
         then do:
            ...
            {mfnp01.i
               ...
               req_nbr
               p_pod_site
                " req_det.req_domain = global_domain and req_site "
               req_nbr}
         end.
         else do:
            /* FIND NEXT/PREVIOUS RECORD */
            {mfnp.i
               req_det
               pod_req_nbr
                " req_det.req_domain = global_domain and req_nbr "
               pod_req_nbr
               req_nbr
               req_nbr}
         end.
      ...
      if available si_mstr
      then do:
         for first req_det  where req_det.req_domain = global_domain and
         req_nbr = ttPurchaseOrderDet.reqNbr
