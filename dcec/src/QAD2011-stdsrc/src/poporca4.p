/* GUI CONVERTED from poporca4.p (converter v1.78) Mon May  3 14:28:19 2010 */
...
/* Revision: 1.20.1.1   BY: Rajat Kulshreshtha   DATE: 03/26/10  ECO: *Q3YX* */
/* $Revision: 1.20.1.2 $  BY: Zheng Huang   DATE: 04/30/10  ECO: *Q41G* */
/*-Revision end---------------------------------------------------------------*/
...
do on error undo, retry:
   ...
   for first poc_ctrl
      fields( poc_domain poc_tol_cst poc_tol_pct)  where poc_ctrl.poc_domain =
   ...
   for first po_mstr
      fields( po_domain po_curr po_vend)
...
/* this is the end tag that will be only in API mode (not an extraction tag) */
      ...
      then do:
         ...
         if pod_blanket <> "" then do for poddet:
            ...
            for first poddet
                where poddet.pod_domain = global_domain and  poddet.pod_nbr  =
                pod_det.pod_blanket
            ...
            if available poddet and
               ...
               if c-application-mode = "API" then
                  assign {mfaiset.i updt_blnkt ttPurchaseOrderTransDet.updateBlanketOrder}.
