/* GUI CONVERTED from poporp04.p (converter v1.78) Mon Jun 25 23:12:02 2007 */
...
/* Revision: 1.9.1.13  BY: Paul Donnelly (SB)    DATE: 06/28/03  ECO: *Q00J*  */
/* Revision: 1.9.1.13  BY: Vandna Rohira         DATE: 10/30/03  ECO: *P17Y*  */
/* Revision: 1.9.1.14  BY: Abhishek Jha          DATE: 10/21/04  ECO: *P2QN*  */
/* Revision: 1.9.1.15  BY: Alok Gupta            DATE: 03/23/05  ECO: *P3DC*  */
/* Revision: 1.9.1.17  BY: Robin McCarthy        DATE: 08/11/05  ECO: *P2PJ*  */
/* $Revision: 1.9.1.18 $  BY: Shivakumar Patil      DATE: 06/22/07  ECO: *P605*  */
...
find first gl_ctrl  where gl_ctrl.gl_domain = global_domain no-lock no-error.
   ...
   for each tr_hist
       where tr_hist.tr_domain = global_domain and (  (tr_part >= part and
       tr_part <= part1)
      ...
      for first pod_det
         fields( pod_domain pod_line pod_nbr pod_op pod_wo_lot)
          where pod_det.pod_domain = global_domain and  pod_nbr  = tr_nbr
      ...
      then do:
         for first wo_mstr
            fields(wo_domain wo_lot wo_nbr)
         ...
         then do:
            for first wr_route
               fields( wr_domain wr_due wr_op wr_lot wr_nbr wr_start wr_sub_cost)
                where  wr_route.wr_domain = global_domain
                and    wr_nbr     = wo_nbr
                and    wr_op      = pod_op
                and    wr_lot     = wo_lot
                and   (wr_start  <= tr_effdate
                or     wr_start   = ?)
                and   (wr_due    >= tr_effdate
                or     wr_due     = ?)
                no-lock:
      ...
      then
         std_var = (tr_price - (tr_mtl_std + tr_lbr_std +
                                tr_bdn_std + tr_sub_std)) * tr_qty_loc.
      ...
      then
         ...
         then
            std_var = (tr_price - wr_sub_cost) * tr_qty_loc.
         else
            std_var = tr_price * tr_qty_loc.
      ...
      then do:
         name = "".
         find ad_mstr
             where ad_mstr.ad_domain = global_domain and  ad_addr = tr_addr
            no-lock no-wait no-error.
         ...
         then do:
            ...
            display
               ...
               accum total  (std_var) @ std_var WITH STREAM-IO /*GUI*/ .
         end.
      end.  /* NOT summary */
