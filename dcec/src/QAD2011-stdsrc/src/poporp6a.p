/* GUI CONVERTED from poporp6a.p (converter v1.78) Fri Sep 30 02:57:19 2011 */
...
/* Revision: 1.33.1.9.1.5 BY: Abhishek Jha        DATE: 11/29/06  ECO: *P5FC* */
/* Revision: 1.33.1.9.1.6 BY: Abhijit Gupta       DATE: 01/29/07  ECO: *P5MQ* */
/* Revision: 1.33.1.9.1.8 BY: Anuradha K.         DATE: 02/04/07  ECO: *P5SC* */
/* Revision: 1.33.1.9.1.9 BY: Alex Joy            DATE: 08/31/07  ECO: *Q1B9* */
/* Revision: 1.33.1.9.1.12 BY: Prashant S.        DATE: 02/25/08  ECO: *P6LG* */
/* Revision: 1.33.1.9.1.13 BY: Rajalaxmi Ganji    DATE: 04/11/08  ECO: *P6QL* */
/* Revision: 1.33.1.9.1.16 BY: Alex Joy           DATE: 07/02/08  ECO: *P6WT* */
/* Revision: 1.33.1.9.1.17 BY: Gerard Menezes     DATE: 08/04/08  ECO: *Q1Q2* */
/* Revision: 1.33.1.9.1.18 BY: Gerard Menezes     DATE: 09/24/08  ECO: *P70S* */
/* Revision: 1.33.1.9.1.19  BY: Ashim Mishras     DATE: 12/19/08  ECO: *Q24F* */
/* Revision: 1.33.1.9.1.20  BY: Ravi Swami      DATE: 05/25/08  ECO: *Q3ZN* */
/* Revision: 1.33.1.9.1.21 BY: Chandrakant Ingale DATE: 10/22/10  ECO: *Q4FG* */
/* $Revision: 1.33.1.9.1.22 $  BY: Dipali Parkar DATE: 09/29/11  ECO: *Q521* */
/*-Revision end---------------------------------------------------------------*/
...
define input parameter ip_inc_log      as logical   no-undo.
   ...
   do with frame bhead:
      ...
      for each tt_pvoddet no-lock:
         ...
         find sct_det
            ...
            and   sct_det.sct_part   = prh_part
            and   sct_det.sct_site   = prh_site
         no-lock no-error.

         if available sct_det
         then do:
            if prh_type = "" then
               std_cost = pvoddet_pur_std - sct_ovh_tl.
         ...
         no-lock:
         end. /* FOR FIRST lac_ctrl */
         if prh_type <> "S"
         then do:
            if available lac_ctrl
               and lac_use_logistics = yes
            then do:
               if use_tot
                  and (use_tot
                  or  ip_inc_log
         ...
         then do:
            ...
            display
               prh_nbr
               po_del_to when available po_mstr format "x(10)"
               prh_vend
               descname format "x(28)"  no-label
            with frame bhead no-box width 132 STREAM-IO /*GUI*/ .

            if available pj_mstr then
               display
                  pj_project
                  pj_desc no-label
               with frame bhead STREAM-IO /*GUI*/ .
         ...
         if qty_open <> 0 or prh_ps_qty <> 0 then do:
             ...
             if page-size - line-counter < 0 then do:
               ...
               if available pj_mstr then
                  display
                     prh_nbr
                     po_del_to when available po_mstr
                     prh_vend
                     descname
                     pj_project
                     pj_desc      no-label
                  ...
                  display
                     ...
                     "" @ pj_project
                     "" @ pj_desc no-label
                     "  " + dynamic-function('getTermLabelFillCentered' in
                     h-label,
                     input "CONTINUE", input 12, input '*') format "x(14)"
                  with frame bhead STREAM-IO /*GUI*/ .
            end.   /* if page-size - line-counter < 0 */

            if prh_tax_in = yes then do:
               base_cost = base_cost - tax_amt.
               pur_ext = pur_ext - tax_amt.
            end.  /* prh_tax_in = yes */
            ...
            put
              (if base_rpt = "" then
               ...
               accum total by prh_nbr tax_amt
                  format "->>>>>>>>>>9.99<<<<" to 117
               skip(1).
         end. /*IF (ACCUM TOTAL BY prh_nbr std_ext) <> 0 or*/
