...
/* Revision: 1.23         BY: Vivek Gogte    DATE: 02/27/03   ECO: *P0N8*     */
/* Revision: 1.25         BY: Paul Donnelly (SB) DATE: 06/28/03 ECO: *Q00J*   */
/* Revision: 1.26         BY: Ed van de Gevel    DATE: 07/08/03 ECO: *Q003*   */
/* Revision: 1.28         BY: Pankaj Goswami     DATE: 01/16/04 ECO: *P1FV*   */
/* Revision: 1.29         BY: Deepak Rao         DATE: 05/26/04 ECO: *P23M*   */
/* Revision: 1.30         BY: Shivganesh Hegde   DATE: 12/21/04 ECO: *P312*   */
/* Revision: 1.31         BY: Nishit V           DATE: 06/07/05 ECO: *P3NR*   */
/* Revision: 1.31.1.2     BY: B. Gates           DATE: 08/03/05  ECO: *P3WP*  */
/* Revision: 1.31.1.3     BY: Dhananjay Govekar  DATE:03/07/07 ECO: *Q12T*    */
/* Revision: 1.31.1.4     BY: Dhananjay Govekar  DATE: 04/27/07 ECO: *Q14P*   */
/* Revision: 1.31.1.4     BY: Anuradha K.        DATE: 05/02/07 ECO: *Q14R*   */
/* Revision: 1.31.1.6     BY: Anuradha K.        DATE: 05/17/07 ECO: *Q15C*   */
/* Revision: 1.31.1.7     BY: Dhananjay Govekar  DATE: 05/23/07 ECO: *P5WS*   */
/* Revision: 1.31.1.8     BY: Archana Kirtane    DATE: 06/25/08  ECO: *P6WB*  */
/* Revision: 1.31.1.9     BY: Ashim Mishra       DATE: 09/11/08  ECO: *P70K*  */
/* Revision: 1.31.1.10    BY: Amit Kumar         DATE: 06/12/09  ECO: *Q30J*  */
/* Revision: 1.31.1.11    BY: Ravi Swami         DATE: 02/02/10  ECO: *Q3SY*  */
/* $Revision: 1.31.1.12 $    BY: Avishek Chakraborty   DATE: 03/10/10  ECO: *Q2P7* */
/*-Revision end---------------------------------------------------------------*/
/*                                                                            */
...
------------------------------------------------------------------------------*/
   ...
   if can-find(pt_mstr  where pt_mstr.pt_domain = global_domain and  pt_part =
   pItemId) and
      can-find(first vp_mstr  where vp_mstr.vp_domain = global_domain and
      vp_part = pRequisitionItemId
...
------------------------------------------------------------------------------*/
   ...
   if can-find(pod_det  where pod_det.pod_domain = global_domain and
               pod_nbr =  pPOId     and
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if can-find(first ie_mstr  where ie_mstr.ie_domain = global_domain and
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if return-value = {&SUCCESS-RESULT} then do :
         ...
         if return-value <> {&SUCCESS-RESULT} then do:
            create pod_det. pod_det.pod_domain = global_domain.
            {pxgetph.i pxgblmgr}
            assign
               pod_nbr       = pPOOrderId
               pod_line      = pPOLineId
...
/*------------------------------------------------------------------------------
Purpose:       Delete a Purchase Order Line in Central and Remote domain
Exceptions:    NONE
Conditions:
        Pre:
        Post:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
     ...
     if siteDB <> global_db then do:
       ...
       /* Delete the line in the remote domain */
       {gprun.i ""gpalias3.p"" "(siteDB, output err-flag)" }

       /* Popoxf2.p is a facade for popomta2.p */
       {gprun.i ""popoxf2.p"" "(pod_nbr, pod_line, pIsBlanket, false)"}

       {gprun.i ""gpalias3.p"" "(old_db, output err-flag)" }

       /* Adjust Blanket PO in Header/Central domain */
       ...
       /* Delete the local domain line data */
       ...
       /* DELETE PURCHASE ORDER IN CENTRAL DOMAIN */
       /* Popoxf2.p is a facade for popomta2.p */
       {gprun.i ""popoxf2.p"" "(pod_nbr, pod_line, pIsBlanket, true)"}
     end. /* ELSE DO */
   end.
   return {&SUCCESS-RESULT}.
...
---------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      find first pod_det
          where pod_det.pod_domain = global_domain and  pod_nbr = pPOId
         use-index pod_nbrln
         no-lock no-error.
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for each vp_mstr
         fields( vp_domain vp_part vp_vend vp_q_date vp_q_qty vp_um
                 vp_curr vp_q_price vp_vend_part)
          where vp_mstr.vp_domain = global_domain and  vp_part = pItemId
...
------------------------------------------------------------------------------*/
   ...
   define variable errorCode                    as integer   no-undo.
   define buffer pt_mstr for pt_mstr.
   define variable err_stat1 as integer no-undo.


   do on error undo, return error {&GENERAL-APP-EXCEPT}:

     poDB = {pxfunct.i &FUNCTION='getCharacterValue'
                       &PROGRAM='pxgblmgr.p'
                       &HANDLE=ph_pxgblmgr
                       &PARAM="input 'global_db'"}.
      ...
      {pxrun.i &PROC='getSiteDataBase' &PROGRAM='icsixr.p'
                                       &HANDLE=ph_icsixr
                                       &PARAM="(input pSiteID, output siteDB)"}

      if siteDB <> poDB then do:
        {gprun.i ""gpmdas.p"" "(siteDB, output errorCode)"}
      end.

      if pPOLineType = ""
      then do:
        {gprun.i ""gpptsi03.p"" "(input-output err_stat1)"}
      end.

      if poDB <> siteDB then do:
        {gprun.i ""gpmdas.p"" "(poDB, output errorCode)"}
      end.

      if (err_stat1 <> 0 and pPolineType = "") or
         (pItemType = "M" and pPOLineType = "")
      then do:
         assign
        ...
        /* MESSAGE #25 - TYPE SET TO (M)EMO */
         ...
         for first pt_mstr no-lock
             where pt_mstr.pt_domain = global_domain and  pt_part = pItemId:
...
/*---------------------------------------------------------------------------
  Purpose:     Get the PO domain name and determine if the PO domain
               is the same as the default domain.
  Exceptions:  NONE
  Dependencies:
       Input-      pod_det(r)
       Output-     pod_det(r)
...
---------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first pod_det fields( pod_domain pod_nbr pod_po_db) no-lock
          where pod_det.pod_domain = global_domain and  pod_nbr = pPONbr:
         {pxgetph.i pxgblmgr}
         if pod_po_db <>
            {pxfunct.i &FUNCTION='getCharacterValue' &PROGRAM='pxgblmgr.p'
                        &HANDLE=ph_pxgblmgr
...
------------------------------------------------------------------------------*/
   ...
   define input  parameter pEffectiveDate as date no-undo.
   define input  parameter pSupplierItem  as character no-undo.
...
Conditions:
       ...
       Post:   None
Notes:         This procedure switches the domain in mutli-Dom environment to
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      then do:
         ...
         /* GET THE BASE CURRENCY OF THE REMOTE DOMAIN */
      ...
      /* THE BASE CURRENCY OF THE ORDER DOMAIN       */
      assign
         exch_rate  = 1
         exch_rate2 = 1.
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first vp_mstr
         fields(vp_domain vp_part vp_vend vp_vend_part vp_curr vp_q_date
                vp_q_price vp_q_qty vp_um)
      ...
      then do:
         ...
         for each vp_mstr
            fields(vp_domain vp_vend vp_vend_part vp_q_date vp_part vp_curr
                   vp_q_price vp_q_qty vp_um)
...
------------------------------------------------------------------------------*/
   ...
   define variable minDisc as decimal initial -999.99 no-undo.
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for each po_mstr
         fields( po_domain po_vend po_nbr po_blanket) no-lock
          where po_mstr.po_domain = global_domain and  po_vend = pSupplierId and
               po_nbr <> pPOId :

         if po_blanket <> pPOId then next.

         for first pod_det
            fields( pod_domain pod_nbr pod_part pod_line pod_blanket
            pod_blnkt_ln) no-lock
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
         if can-find(first prh_hist  where prh_hist.prh_domain = global_domain
         and
                     prh_nbr  = pPOId and
                     prh_line = pLineId)
         then do:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if can-find(first sch_mstr  where sch_mstr.sch_domain = global_domain and
                  sch_type = 4 and
                  sch_nbr = pPOId and
                  sch_line = pLineId)
      then do :
...
/*------------------------------------------------------------------------------
Purpose:       Validates items that exist on a Remote domain
Exceptions:    APP-ERROR-RESULT
Conditions:
        Pre:   pod_det(r)
       Post:   pt_mstr(r)
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if err-flag <> 0 then do:
         /* MESSAGE #6137 - DOMAIN # NOT AVAILABLE */
         {pxmsg.i
            &MSGNUM=6137
            &ERRORLEVEL={&APP-ERROR-NO-REENTER-RESULT}
            &MSGARG1="siteDB"}
         ...
         if not can-find (first pt_mstr  where pt_mstr.pt_domain =
         global_domain and  pt_part = pItemId) then do:
      ...
      /* If any errors then reset domain aliases and Item Type */
      if err-flag <> 0 then do:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first pod_det
         fields( pod_domain pod_qty_ord pod_rel_qty) no-lock
          where pod_det.pod_domain = global_domain and  pod_nbr  =
          pBlanketOrderNbr
...
/*---------------------------------------------------------------------------
  Purpose:     Determine if the PO DOMAIN is the same as the default domain
  Exceptions:  APP-ERROR-RESULT
  Dependencies:
       Input-      pod_det(r)
       Output-     pod_det(r)
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if not(can-find(po_mstr  where po_mstr.po_domain = global_domain and
      po_nbr = pValue)) then do:
         {pxmsg.i
            &MSGNUM=3770
            &ERRORLEVEL={&APP-ERROR-RESULT}
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      /* MESSAGE # 6171 - INVALID DOMAIN */
      if not(can-find(dom_mstr where dom_domain = pValue) or pValue = "")
      then do:
         {pxmsg.i
            &MSGNUM=6171
            &ERRORLEVEL={&APP-ERROR-RESULT}
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      if pPOLineRequisitionNbr <> "" and
         can-find(last req_det  where req_det.req_domain = global_domain and
         req_nbr = pPOLineRequisitionNbr) and
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first wr_route
         fields( wr_domain wr_lot wr_op wr_status wr_sub_cost)
          where wr_route.wr_domain = global_domain and  wr_lot = pWorkOrderId
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      find last req_det  where req_det.req_domain = global_domain and (
      ...
      if pOrderStatus = "c" or
         ...
         if pSiteId <> "*" and pPOLineId <> 0 then do:
            for each purchaseOrderLine no-lock
                where purchaseOrderLine.pod_domain = global_domain and
                purchaseOrderLine.pod_nbr  = pOrderNumber
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      if (pQtyOrdered = 0 or
         not can-find(pt_mstr  where pt_mstr.pt_domain = global_domain and
         pt_part = pItemId)) and
         pPOType = "S"
      then do:
...
------------------------------------------------------------------------------*/
   ...
   if return-value = {&SUCCESS-RESULT} then do:
      ...
      if available vd_mstr then
         ...
         for first gl_ctrl
            fields( gl_domain gl_pur_acct gl_pur_sub gl_pur_cc)
