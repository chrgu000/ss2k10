/* GUI CONVERTED from pppcrp01.p (converter v1.78) Fri Mar 14 04:58:54 2008 */
...
/* $Revision: 1.19.6.1 $  BY: Prajakta Patil           DATE: 03/12/08 ECO: *P6NG* */
/*-Revision end---------------------------------------------------------------*/
...
line           colon 15
   ...
   pm             colon 15
   pm1            label {t001.i} colon 49 skip(1)
   pclist         colon 30
   curr           colon 30
   pcdate         colon 30
 SKIP(.4)  /*GUI*/
...
define buffer pcmstr for pc_mstr.
   ...
   for each pc_mstr
      fields( pc_domain pc_amt   pc_amt_type pc_curr pc_expire
              pc_list  pc_min_qty  pc_part pc_prod_line
              pc_start pc_um)
       where pc_mstr.pc_domain = global_domain and (  ((pc_part >= part and
              pc_part <= part1) or
              pc_part = "")
        and  (pc_list  =  pclist    or pclist = "")
      ...
      for each pt_mstr
         ...
         find last pcmstr
             where pcmstr.pc_domain = global_domain and (  pcmstr.pc_list    =
             pc_mstr.pc_list
         ...
         then do:
            ...
            then do:
               ...
               then do with frame c:
                  ...
                  display
                     ...
                     pt_prod_line
                     pl_desc
                  with frame c side-labels no-box STREAM-IO /*GUI*/ .
            ...
            display
               ...
               pt_um
               pt_desc1
               tmp-price @ pt_price WITH STREAM-IO /*GUI*/ .

            if pt_desc2 <> ""
            then do:
               down 1.
               display
                  pt_desc2 @ pt_desc1 WITH STREAM-IO /*GUI*/ .
            ...
            do i = 1 to 15:
               ...
               then do:
                  display
                     pcmstr.pc_amt_type
                     pcmstr.pc_min_qty[i]
                     @ pcmstr.pc_min_qty[1] label {&pppcrp01_p_3}
                     pcmstr.pc_amt[i]
                     @ pcmstr.pc_amt[1] label {&pppcrp01_p_1}
                     format "->>>,>>>,>>9.9<<<<"
                     round(net_price,2) @ net_price WITH STREAM-IO /*GUI*/ .
                  down 1.
               end.
            end. /* for i = 1 to 15 */
...
/*GUI*/ {mfguirpb.i &flds=" line line1 part part1 type type1 ptgroup group1 added added1 pm pm1 pclist curr pcdate "} /*Drive the Report*/
