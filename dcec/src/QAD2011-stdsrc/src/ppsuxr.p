...
/* Revision: 1.3  BY: Paul Donnelly (SB) DATE: 06/28/03 ECO: *Q00K* */
/* $Revision: 1.5 $  BY: Pankaj Goswami  DATE: 01/16/04   ECO: *P1FV* */
/*-Revision end---------------------------------------------------------------*/
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
     if pSupplierItemId = "" then do :
         ...
         for first vp_mstr
            fields( vp_domain vp_part vp_mfgr vp_vend vp_mfgr_part vp_um)
            where vp_mstr.vp_domain = global_domain and
            vp_part = pItemId and
            ( vp_vend = pSupplierId
              or (vp_vend = ""
         ...
         /* FOR AN INVENTORY ITEM                                      */
         for first vp_mstr
           fields( vp_domain vp_part vp_vend_part vp_mfgr vp_vend vp_mfgr_part
           vp_um)  where vp_mstr.vp_domain = global_domain and
           vp_part      = pItemId and
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      /* FOR AN INVENTORY ITEM                                      */
      for each vp_mstr
         fields( vp_domain vp_part vp_vend_part vp_vend vp_q_date
         ...
         and   ( vp_vend = pSupplierId
                 or (vp_vend = ""
                  ...
                  and   vp_part = pItemId
                  and   vp_vend = pSupplierId)) )
         and   (pSupplierItemId = "" or vp_vend_part = pSupplierItemId)
         ) no-lock
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      then do:
         ...
         /* FOR AN INVENTORY ITEM                                      */
         for each vp_mstr
            fields( vp_domain vp_part vp_vend vp_vend_part vp_q_date vp_um
