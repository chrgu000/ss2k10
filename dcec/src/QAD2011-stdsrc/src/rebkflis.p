/* GUI CONVERTED from rebkflis.p (converter v1.78) Fri Feb 24 01:34:54 2012 */
...
/* Revision: 1.28      BY: Paul Donnelly (SB)     DATE: 06/28/03  ECO: *Q00K* */
/* Revision: 1.29      BY: R.McCarthy/T.Giustozzi DATE: 04/19/04  ECO: *P15V* */
/* Revision: 1.30      BY: Shoma Salgaonkar       DATE: 08/25/04  ECO: *Q0CJ* */
/* Revision: 1.30.1.3  BY: Vaibhav Desai          DATE: 11/17/06  ECO: *P52L* */
/* Revision: 1.30.1.7  BY: Ravi Swami             DATE: 10/09/09  ECO: *Q3GS* */
/* Revision: 1.30.1.8  BY: Priti Jha              DATE: 09/14/11  ECO: *Q50W*  */
/* Revision: 1.30.1.9  BY: Ravi Swami        DATE: 12/30/11  ECO: *Q55S*  */
/* $Revision: 1.30.1.10 $  BY: Keny Fernandes      DATE: 2/23/12    ECO: *Q580* */
/*-Revision end---------------------------------------------------------------*/
...
define input parameter ip-kbtr-trans-nbr like kbtr_trans_nbr no-undo.
...
for first wo_mstr
   fields (wo_domain wo_acct wo_sub wo_cc wo_line wo_lot wo_mtl_var wo_mvrr_acct
           wo_mvrr_sub wo_mvrr_cc wo_nbr wo_part wo_project wo_site wo_so_job
           wo_vend wo_wip_tot)
...
no-lock:
   ...
   for first wo_mstr
      fields (wo_domain wo_acct wo_sub wo_cc wo_line wo_lot wo_mtl_var
              wo_mvrr_acct wo_mvrr_sub wo_mvrr_cc wo_nbr wo_part
   ...
   if pk_qty <> 0 then do:
      ...
      {gprun.i ""csavg01.p""
               "(input pk_part,
                 input wo_site,
                 output component_cost_set,
                 output component_costing_mthd,
                 output component_cur_set,
                 output component_cur_mthd)"}
/*GUI*/ if global-beam-me-up then undo, leave.
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         {gpsct01.i &set=component_cost_set
            &site=wo_site
            &part=pk_part
            &ExclLockMode=false}
...
/*GUI*/ if global-beam-me-up then undo, leave.
      ...
      for first pt_mstr
         fields (pt_domain pt_abc pt_avg_int pt_cyc_int pt_loc pt_part
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         do transaction:
            ...
            {ictrans.i
               ...
               &exruseq=0
               &glamt=gl_amt
               &kbtrans=ip-kbtr-trans-nbr
               &lbrstd=0
               &line=0
               &location="(if srsite <> wo_site then l_loc
                           else srloc)"
               &lotnumber=wo_lot
               &lotserial=srlotser
               &lotref=srref
  ...
  {gprunmo.i &program  = ""pocnsix4.p"" &module = "ACN"
                          &param    =  """(input wod_part,
                                           ...
                                           input srlotser,
                                           input srref,
                                           input (if srqty < 0 then true
                                                  else false),
                                           output consign_flag)"""}

                /*IF CONSIGNED INVENTORY EXISTS, DETERMINE WHETHER TO */
                /*USE IT PRIOR TO UNCONSIGNED INVENTORY.              */
               /* IF THE SITES ARE DIFFERENT THEN WE KNOW THAT ICXFER.P WAS   */
               /* CALLED ABOVE, WHICH CALLS ICXFERCN.P WHICH CALLS ICTRANCN.P */
               /* THEREFORE WE DO NOT NEED TO CALL ICTRANCN.P HERE AGAIN      */
