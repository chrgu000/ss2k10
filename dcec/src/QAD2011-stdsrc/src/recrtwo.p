/* GUI CONVERTED from recrtwo.p (converter v1.78) Fri Dec 30 07:26:13 2011 */
...
/* Revision: 1.22     BY: Paul Donnelly (SB)  DATE: 06/28/03  ECO: *Q00K*     */
/* Revision: 1.22.2.1 BY: Amit Singh          DATE: 12/20/05  ECO: *P4CM*     */
/* Revision: 1.22.2.2 BY: Denzel Paul         DATE: 06/28/09  ECO: *Q2R2*    */
/* $Revision: 1.22.2.3 $ BY: Ravi Swami      DATE: 12/30/11  ECO: *Q55S*    */
/*-Revision end---------------------------------------------------------------*/
...
for first pt_mstr
   fields( pt_domain pt_loc pt_ord_qty pt_part pt_prod_line pt_status)
    where pt_mstr.pt_domain = global_domain and  pt_part = part no-lock:
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   do transaction on error undo create-qad, retry create-qad:
      ...
      find first qadlock
          where qadlock.qad_domain = global_domain and  qadlock.qad_key1 =
          l_qad_key1
...
/*GUI*/ if global-beam-me-up then undo, leave.
 /* FOR FIRST MFC_CTRL */
      ...
      then do:
         ...
         for first glc_cal
            fields( glc_domain glc_end glc_start)
             where glc_cal.glc_domain = global_domain and  glc_start <= eff_date
      ...
      for each ro_det
         fields( ro_domain ro_auto_lbr ro_desc ro_end ro_mch ro_mch_op
         ro_men_mch
         ...
         for first wc_mstr
            fields( wc_domain wc_bdn_pct wc_bdn_rate wc_lbr_rate wc_mch
             ...
             where wc_mstr.wc_domain = global_domain and  wc_wkctr = ro_wkctr
              and wc_mch = ro_mch no-lock:
         end. /* FOR FIRST WC_MSTR */

         create wr_route. wr_route.wr_domain = global_domain.
         ...
         create iro_det. iro_det.iro_domain = global_domain.
...
/*GUI*/ if global-beam-me-up then undo, leave.
            ...
            for first pt_mstr
               fields( pt_domain pt_loc pt_ord_qty pt_part pt_prod_line
               pt_status)
...
/*GUI*/ if global-beam-me-up then undo, leave.
 /* FOR FIRST PT_MSTR */
            ...
            then
                do: create wod_det. wod_det.wod_domain = global_domain. end.
            ...
            then
                do: create qad_wkfl. qad_wkfl.qad_domain = global_domain. end.
