/* GUI CONVERTED from sopk01.p (converter v1.78) Tue Feb  8 22:09:21 2005 */
...
/* $Revision: 1.33 $ BY: Reena Ambavi DATE: 02/04/05 ECO: *P36Q* */
/*-Revision end---------------------------------------------------------------*/
...
so_mstr_loop:
for each so_mstr
    where so_mstr.so_domain = global_domain and  (so_nbr >= nbr and so_nbr <=
    nbr1)
   ...
   then do:
      for each sod_det
         ...
         for first qad_wkfl
             ...
             string(so_nbr,"x(8)") + string(sod_line,"999")
   ...
   /*    ELSE, THIS ISN'T A SCHEDULED ORDER, DO */
      ...
      for first sod_det
         fields( sod_domain sod_btb_type sod_confirm sod_curr_rlse_id
         sod_due_date
      ...
      for each sod_det
         ...
         then do:
            ...
            if so_fsm_type = "RMA" then do:
               for first rmd_det
                  fields( rmd_domain rmd_line rmd_link rmd_nbr rmd_prefix
                  rmd_qty_acp)
                   where rmd_det.rmd_domain = global_domain and
                   rmd_det.rmd_nbr    = sod_nbr
               ...
               if available rmd_det and rmd_det.rmd_link <> 0 then
                  for first rma_mstr
                     fields( rma_domain  rma_ctype rma_nbr rma_prefix)
                      where rma_mstr.rma_domain = global_domain and  rma_nbr
                      = sod_nbr
                       and rma_prefix = "C"
                  no-lock:
               ...
               if available rma_mstr then
                  for first sv_mstr
                     fields( sv_domain sv_code sv_shp_b4rtn sv_type)
                      where sv_mstr.sv_domain = global_domain and  sv_code =
                      rma_ctype
                       and sv_type = ""
                  no-lock:
               ...
               if available sv_mstr and not sv_shp_b4rtn then
                  for first rmdreceipt
                     fields( rmd_domain rmd_line rmd_link rmd_nbr
                      ...
                      where rmdreceipt.rmd_domain = global_domain and
                      rmdreceipt.rmd_nbr    = sod_nbr
      ...
      if so_fsm_type = "SEO" and so_eng_code = so_ship then do:
         find eng_mstr  where eng_mstr.eng_domain = global_domain and  eng_code
         = so_ship no-lock no-error.
         if available eng_mstr then
            find ad_mstr  where ad_mstr.ad_domain = global_domain and  ad_addr
            = eng_address no-lock no-error.
      end.
      else
         find ad_mstr  where ad_mstr.ad_domain = global_domain and  ad_addr =
         so_ship no-lock no-error.
