/* GUI CONVERTED from sorp0501.p (converter v1.78) Fri Jul 25 02:05:11 2008 */
...
/* Revision: 1.37         BY: Paul Donnelly (SB)  DATE: 06/28/03  ECO: *Q00L* */
/* Revision: 1.37.2.2     BY: Prashant Menezes    DATE: 10/18/06  ECO: *P4ZB* */
/* $Revision: 1.37.2.3    BY: Prashant Menezes    DATE: 05/22/07  ECO: *P5X9* */
/* $Revision: 1.37.2.6 $  BY: Deepak Keni         DATE: 11/28/07  ECO: *Q1DS* */
/* $Revision: 1.37.2.6 $  BY: Deepak Keni         DATE: 07/25/08  ECO: *P6Y9* */
/*-Revision end---------------------------------------------------------------*/
...
for each so_mstr  where so_mstr.so_domain = global_domain and (  (so_nbr >= nbr)
   and (so_nbr <= nbr1)
   and (so_cust >= cust)
   and (so_cust <= cust1)
   and (so_print_so = yes)
   ...
   do transaction on error undo , leave on endkey undo , leave :
      ...
      no-lock by sod_nbr by sod_line:
         ...
         if using_line_charges and print_line_charges then do:
            ...
            for each sodlc_det  where sodlc_det.sodlc_domain = global_domain and
         ...
         *******************************************/
         ...
         if available pt_mstr then do:
            ...
            sod_part and ptp_site = sod_site
            no-lock no-error.
            if available ptp_det then
               pm_code = ptp_pm_code.
            if pm_code = "C" and
               not can-find(first sob_det  where sob_det.sob_domain =
               global_domain and  sob_nbr  = sod_nbr and
                                                sob_line = sod_line)
            then do:
               {sopiprn1.i &doctype="1" &nbr=sod_nbr &line=sod_line
                  &part=sod_part
