/* GUI CONVERTED from sosotrle.p (converter v1.78) Wed Nov 30 02:01:38 2011 */
...
/* Revision: 1.46.2.6   BY: Xiaolei Meng        DATE: 04/15/09  ECO: *Q2M9*   */
/* Revision: 1.46.2.8   BY: Nafees Khan         DATE: 07/28/09  ECO: *Q36K*   */
/* Revision: 1.46.2.9   BY: Prabu M             DATE: 09/02/09  ECO: *Q3C2*   */
/* Revision: 1.46.2.10  BY: Abhijit Gupta       DATE: 08/22/09  ECO: *Q39Z*   */
...
define variable l_tmp_amt         as   decimal.
   ...
   if so_stat = ""
      ...
      then do:
         ...
         then do:
            ...
            /* Depot Order placed on credit hold */
            {pxmsg.i &MSGNUM=690 &ERRORLEVEL=1
                     &MSGARG1=getTermLabel(""DEPOT_ORDER"",20)}
         end.
...
/*GUI*/ if global-beam-me-up then undo, leave.
 /*IF USING_LINE_CHARGES*/
      ...
      /* FOR A SOD_DET.                                            */
      if sod_fsm_type = fsmro_c and
         can-find(first ca_mstr
                  where ca_domain = global_domain
                  and   ca_nbr    = sod_nbr
                  and   ca_category = "0"
                  and   ca_so_nbr   = sod_nbr
                  and   can-find(first fwk_mstr
                           where fwk_domain = global_domain
                           and   fwk_ca_int_type = ca_int_type
                           and   fwk_repair_ctr  = yes))
...
/*GUI*/ if global-beam-me-up then undo, leave.
 /* FOR EACH SFB_DET */
   ...
   if mc-error-number <> 0 then do:
    ...
    l_tmp_amt = taxable_amt * so_disc_pct / 100.
    /* ROUND PER DOCUMENT CURRENCY ROUND METHOD */
    {gprunp.i "mcpl" "p" "mc-curr-rnd"
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   then do:
      ...
      if   (taxable_amt + nontaxable_amt
         - (so_trl1_amt + so_trl2_amt + so_trl3_amt)) / (100 - so_disc_pct)
         -  truncate((taxable_amt + nontaxable_amt
         - (so_trl1_amt + so_trl2_amt + so_trl3_amt)) / (100 - so_disc_pct),0)
         <> 0
      then
