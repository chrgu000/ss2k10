...
/* Revision: 1.73.1.7 BY: Ed van de Gevel          DATE: 12/01/03 ECO: *N2G5* */
/* Revision: 1.73.1.8 BY: Ed van de Gevel          DATE: 01/26/04 ECO: *P1L2* */
/* Revision: 1.73.1.9 BY: Swati Sharma             DATE: 05/07/04 ECO: *P1ZG* */
/* $Revision: 1.73.1.10 $ BY: Ed van de Gevel DATE: 05/04/04 ECO: *P207* */
/******************************************************************************/
      ...
      do for apc_ctrl transaction:
         ...
         ****************************************************************************/
         ...
         do while true:
            ...
            for each vod_det
               fields (vod_acct vod_amt vod_base_amt vod_cc vod_desc
         ...
         then do:
            ...
            then do:
               ...
               then do:
                  {gprunp.i "nrm" "p" "nr_can_void"
                           "(input ap_dy_code, output allow-gaps)"}
                  if not allow-gaps
                  then  do:
                     /* SEQUENCE DOES NOT ALLOW GAPS */
                     {pxmsg.i &MSGNUM=1349 &ERRORLEVEL=2}
                  end. /* IF NOT allow-gaps */
            ...
            then do:
               ...
               then do:
                  for first glt_det
                     fields (glt_dy_code glt_dy_num glt_ref)
                     where glt_ref = ap_ref
                     and glt_dy_num > "" no-lock:
...
PROCEDURE  p_create_vod_det:
   ...
   then
      ...
      assign
         ...
         vod_desc     = l_desc
         vod_acct     = l_ex_rnd_acct
         vod_sub      = l_ex_rnd_sub
         vod_cc       = l_ex_rnd_cc
         vod_tax_at   = "n"
