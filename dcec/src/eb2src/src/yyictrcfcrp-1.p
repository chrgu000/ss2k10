...
/*J034*/          end.               
    ...
    for /*each in_mstr where /*(in_site >= site and in_site <= site1) and*/
        ...
        each pt_mstr where pt_part >= part and  pt_part <= part1 AND
       ...
       invadj = 0.
       oth = 0.
         FIND FIRST IN_mstr WHERE IN_site >= site  AND in_site <= site1  AND  IN_part = pt_part NO-LOCK NO-ERROR.
       ...
       bgqty = edqty - inqty + outqty.
        ...
        if (yn_zero and (edqty <> 0 or bgqty <> 0 or rctpo <> 0 or rcttr <> 0 or rctunp <> 0 or rctwo <> 0
          ...
          FIND FIRST IN_mstr WHERE IN_site = tr_site AND IN_part = tr_part NO-LOCK NO-ERROR.
           disp pt_part pt_desc2 pt_prod_line pt_abc in__qadc01 WHEN AVAIL IN_mstr  label "保管员"
              in_user1 WHEN AVAIL in_mstr label "缺省库位" bgqty label "期初库存"
