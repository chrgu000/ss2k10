/* xxinrctrp03.p  - Inventroy Activity Report - Materials Receiving Financial Report                              */
...
/* RHB 材料入库数量报表,由xxinrctrp02.p材料入库数量金额报表屏蔽金额显示而来
*/
...
def var sum_rct_po as decimal format "->>>,>>>,>>9.99" initial 0 label "采购入库金额合计".
def var prod_rct_po as decimal format "->>>,>>>,>>9.99" initial 0 label "产品类采购入库金额合计".
def var sum_iss_prv as decimal format "->>>,>>>,>>9.99" initial 0 label "采购退货金额合计".
def var prod_iss_prv as decimal format "->>>,>>>,>>9.99" initial 0 label "产品类采购退货金额合计".
def var sum_rct_unp as decimal format "->>>,>>>,>>9.99" initial 0 label "计划外入库金额合计".
def var prod_rct_unp as decimal format "->>>,>>>,>>9.99" initial 0 label "产品类计划外入库金额合计".
def var sum_rct_wo as decimal format "->>>,>>>,>>9.99" initial 0 label "完工入库金额合计".
def var prod_rct_wo as decimal format "->>>,>>>,>>9.99" initial 0 label "产品类完工入库金额合计".
...
if pr_detail = no then do:
    ...
    display date date1 with frame b.
    display pt_prod_line COLUMN-LABEL "产品类" pldesc no-label with width 132 frame c side-labels STREAM-IO.
    output close.
    end.
    end.
...
/* 	amt_rct_unp199 */ 
 ...
 disp pt_part COLUMN-LABEL "零件" pt_desc2 COLUMN-LABEL "描述" pt_um 
 qty_rct_po column-label "本期采购" 
 /* amt_rct_po column-label "金额"  */
 qty_iss_prv column-label "本期采购退货" 
 /* amt_iss_prv column-label "金额" */
 qty_rct_wo column-label "本期完工入库" 
 /* amt_rct_wo column-label "金额" */
 qty_rct_unp column-label "计划外入库合计" 
 /* amt_rct_unp column-label "金额" */
 qty_receive column-label "本期入库合计" 
 /* amt_receive column-label "入库金额" */
 with width 256  STREAM-IO.
 end.
if last-of(pt_prod_line) then do:
/*       display prod_rct_po prod_iss_prv prod_rct_unp prod_rct_wo with width 132 frame z.
*/       prod_rct_po=0.
       prod_iss_prv=0.
       prod_rct_unp = 0.
       prod_rct_wo =0.
...
/* disp sum_rct_po column-label "采购入库合计" sum_iss_prv column-label "采购退货合计" sum_rct_unp column-label "计划外入库合计" sum_rct_wo column-label "完工入库合计" with frame y.
*/ sum_rct_po =0.
sum_iss_prv = 0.
sum_rct_unp = 0.
sum_rct_wo = 0.
