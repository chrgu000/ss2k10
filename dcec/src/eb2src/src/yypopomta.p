...
/* Revision: 1.72.1.1   BY: Reena Ambavi    DATE: 11/21/03     ECO: *P18S*    */
/* $Revision: 1.72.1.2 $  BY: Pankaj Goswami   DATE: 12/24/03     ECO: *P1FV* */
/******************************************************************************/
...
/* REVISION: eb2+sp7     LAST MODIFIED: 06/20/05    BY: *tfq* taofengqin        */
{mfdeclre.i}
{cxcustom.i "YYPOPOMTA.P"}
{gplabel.i}
{pxmaint.i}
...
repeat on endkey undo, leave:
   ...
   /*tfq         {gprun.i ""popomti.p""}  */
    /*tfq*/         {gprun.i ""yypopomti.p""}     /*include original popomti.p and popomte.p*/
            if c-application-mode = "API" then
            do:
           ...
           /*tfq {gprun.i ""popomtr.p"" "(input yes,
                  input frame_row,
                  input po_curr,
                  input po_vend,
                  input pod_site,
            ...
            /*tfq*/ {gprun.i ""yypopomtr.p"" "(input yes,
                  input frame_row,
                  input po_curr,
                  input po_vend,
                  input pod_site,
           ...
           then
            ...
            assign
               ...
               continue = no.
          /*tfq   {gprun.i ""popomtea.p""} */
           /*tfq*/   {gprun.i ""yypopomtea.p""} 
      ...
      /*tfq      {gprun.i ""popomth.p""} */
/*tfq*/  {gprun.i ""yypopomth.p""}
            if c-application-mode <> "API" and
...
/*LB01*/			 end.
/*LB01*/			 find sct_det where sct_sim = l_gl_set and sct_site = "dcec-c" and pod_part = sct_part no-lock no-error.
...
/*LB01                  {mfmsg.i 7364 3}*/
/*LB01*/			    message "单位成本必须不能小于0，请重新输入！" VIEW-AS ALERT-BOX ERROR BUTTONS OK.
/*LB01*/                undo setc-2, retry.
/*LB01*/		     end.
...
/*****tfq added begin ********************************************/
   ...
   /*tfq {gprunmo.i &module="GRS" &program="popomtra.p"
                     &param="""(input yes,
                        input pod_site,
                        input pod_req_nbr,
                        input pod_req_line,
                  ...
                  {gprunmo.i &module="GRS" &program="yypopomtra.p"
                     &param="""(input yes,
                        input pod_site,
                        input pod_req_nbr,
                        input pod_req_line,
            ...
            if sngl_ln = yes then do:
               assign continue = no
                  pod_recno = recid(pod_det).
  /*tfq             {gprun.i ""popomtd.p""}   */
    /*tfq */            {gprun.i ""yypopomtd.p""}   
               if c-application-mode <> "API" then
               do:
               ...
               assign continue = no
                  pod_recno = recid(pod_det).
     /*tfq          {gprun.i ""popomtd2.p""}  */
     /*tfq*/          {gprun.i ""yypopomtd2.p""}  
               if c-application-mode <> "API" then
                  if continue = no then undo mainloop, next mainloop.
               else  /* c-application-mode = "API" */
                  if continue = no then
