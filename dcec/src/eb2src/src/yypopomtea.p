...
/* Revision: 1.19.3.1 BY:Ashish Maheshwari          DATE:11/27/03 ECO: *P1CJ* */
/* $Revision: 1.19.3.2 $  BY: Pankaj Goswami  DATE: 12/24/03   ECO: *P1FV* */
/******************************************************************************/
...
do on endkey undo, leave on error undo, retry:
   ...
   if pod_pur_cost = 0 then do:
      ...
      /* NEW PARAMETER pod_vpart PASSED TO GET EXACT SUPPLIER ITEM COST */
      {pxrun.i &PROC='getPurchaseCost' &PROGRAM='yypopoxr1.p'
         &PARAM="(input pod_part,
                        input po_vend,
                        input si_site,
                        input po_curr,
   ...
   if available pt_mstr then do:
      ...
      {pxrun.i &PROC='getManufacturerItemDataOfLastQuote' &PROGRAM='yyppsuxr.p'
         &PARAM="(input pod_part,
                        input po_vend,
                        input-output pod_vpart,
                        output mfgr,
   ...
   if not available pt_mstr
      and pod_req_nbr <> ""
   then do:
      {pxrun.i &PROC='getTaxableData' &PROGRAM='yypopoxr1.p'
               &PARAM="(input po_taxc,
                        input po_taxable,
                        input po_taxable,
                        output pod_taxc,
         ...
         if available pt_mstr  then do:
            ...
            then do:
               ...
               {pxrun.i &PROC='getAccountFieldsForLine' &PROGRAM='yypopoxr1.p'
                        &PARAM="(input po_ord_date,
                                 input pod_part,
                                 input si_site,
                                 input poc_insp_loc,
   ...
   /*SET TYPE*/
   {pxrun.i &PROC='getPOLineTypeAndTaxFlag' &PROGRAM='yypopoxr1.p'
      &PARAM="(input pod_part,
                     input l_pod_type,
                     input po_taxable,
...
PROCEDURE vpMstrAvailable:
   ...
   if pur_cost = ? then do:
      ...
      if return-value = {&SUCCESS-RESULT} then do:
         ...
         /* Added supplier type as fourth input parameter */
         {pxrun.i &PROC='getRemoteItemData' &PROGRAM='yypopoxr1.p'
                  &PARAM="(input po_ord_date,
                           input pt_part,
                           input si_site,
                           input """",
