...
for each wo_mstr
   fields (wo_acct wo_sub wo_bom_code wo_cc wo_due_date wo_flr_acct wo_flr_sub
...
for first pt_mstr
   fields (pt_loc pt_ord_qty pt_part pt_prod_line pt_status)
...
repeat:
   ...
   do transaction on error undo create-qad, retry create-qad:
      ...
      find first qadlock
         where qadlock.qad_key1 = l_qad_key1
   ...
   do transaction:
      ...
      ***********************************************************/
      ...
      for first mfc_ctrl
         fields (mfc_char mfc_field mfc_integer mfc_logical)
         where mfc_field = "rpc_eff_dflt" no-lock:
      ...
      for first mfc_ctrl
         fields (mfc_char mfc_field mfc_integer mfc_logical)
         where mfc_field = "rpc_inc_yld" no-lock:
      end. /* FOR FIRST MFC_CTRL */
      ...
      then do:
         ...
         for first glc_cal
            fields (glc_end glc_start)
            where glc_start <= eff_date
      ...
      then do:
         ...
         for first mfc_ctrl
            fields (mfc_char mfc_field mfc_integer mfc_logical)
            where mfc_field = "rpc_eff_days" no-lock:
         end. /* FOR FIRST MFC_CTRL */
      ...
      for first ptp_det
         fields (ptp_ord_qty ptp_part ptp_site)
     ...
     /*tfq {gpxpldps.i
      ...
      for each ro_det
         fields (ro_auto_lbr ro_desc ro_end ro_mch ro_mch_op ro_men_mch
       ...
       /**********tfq added begin***********************************/
         find first wr_route where wr_nbr = wo_nbr
          ...
          if not available wr_route then
            do:
            find first wod_det where wod_part = pk_part and wod_lot = wo_loc and
