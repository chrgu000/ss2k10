...
{mfphead.i}
lineno=0.
for each tr_hist no-lock where tr_nbr>=nbr and tr_nbr <= nbr1  and tr_type="ISS-wo" /*cj*/ USE-INDEX tr_nbr_eff break by tr_nbr by tr_part:
...
disp tr_part pt_desc2 pt_article column-label "保管员" pt_um tr_loc absolute(tr_qty_chg) column-label "已发数量  " tr_effdate  with width 144  STREAM-IO.
end.
