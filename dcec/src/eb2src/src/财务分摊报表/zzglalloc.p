...
/*GL93*/ width 132 attr-space.
		...
		where glc_start <= today and glc_end >= today 
		no-error.
...
/*GUI*/ RECT-FRAME:HEIGHT-PIXELS in frame a = FRAME a:HEIGHT-PIXELS - RECT-FRAME:Y in frame a - 2.
		...
		/*计算6B销售收入*/
		{zzglasccal.i ""5101000"" ""1"" ""?"" f1}
		
		/*计算非6B销售收入*/
		{zzglasccal.i ""5101000"" ""2"" ""?"" f2}
		
		/*折扣*/
		{zzglasccal.i ""5101999"" ""1"" ""?"" d1}
		{zzglasccal.i ""5101999"" ""2"" ""?"" d2}
...
/*		if ( F1 + F2 - D1 - D2) <> 0 then
		...
		/*分摊*/
		output stream batchdata to value("c:\zzglalloc.tmp") no-echo.
		/*header*/
		...
		/*detail*/
		for each ac_mstr no-lock where ac_code >= acct and ac_code <= acct1,
		each cc_mstr no-lock  
		:
			{zzglasccal.i ac_code ""0"" cc_ctr v0}
			...
			assign
				...
				xxv1	= v1
				xxv2	= v2.
		  /*MESSAGE xxv0 xxv1 xxv2 v0 v1 v2.
          PAUSE.*/
			put stream batchdata	"-" at 1.
			...
			tmpv = ABSOLUTE(v0). */
			put stream batchdata	"- " AT 1 " - "  AT 1 "  -  " AT 1.
             put stream batchdata  0 -  xxv0 FORMAT "->>>>,>>>,>>9.99"  at 1.
			...
			/*if v0 > 0 then
				...
				put stream batchdata	"- - - no" at 1.
			tmpv = ABSOLUTE(v1).  judy
			put stream batchdata  tmpv at 1.*/
             put stream batchdata	"- " AT 1 " - "  AT 1 "  -  " AT 1.
               put stream batchdata xxv1   FORMAT "->>>>,>>>,>>9.99" at 1.
...
/*			put stream batchdata	ABSOLUTE(v2) at 1.*/
			put stream batchdata	"- " AT 1 " - "  AT 1 "  -  " AT 1.
             put stream batchdata  xxv2  FORMAT "->>>>,>>>,>>9.99" at 1.
		end.		
		put stream batchdata "."  at 1.
		put stream batchdata "."  at 1.
		...
		up_yn = no.
		p2 = 1 - p1.
		disp f1 f2 d1 d2 p1 p2 "准备分摊数据" @ ref with frame pref.
...
/*H0RW*/           end. /* IF CONTINUOUS */
		...
		/* CIM LOAD */
		batchrun = yes.
		input from value("c:\zzglalloc.tmp") no-echo.
		output to value("c:\zzglalloc.out") keep-messages.
		...
		find first glt_det no-lock where glt_ref > ref
		...
		if not available glt_det then do:
	 		find last gltr_hist no-lock where gltr_ref >= ref
