...
/*bn083*/       def var keeper1    	 like pt_article label "保管员".
	 ...
	 def var tagline         as char extent 16.
	 def var printline       as char extent 16.
	 def var forms_built     as int init 0.
	 def var i               as int.
	 def var j               as int.
	 def var tagnumber       like tag_nbr extent 20.
	 ...
	 tagline[03] = "        地点:                                     ".
	 tagline[04] = "        库位:                                     ".
	 tagline[05] = "      零件号:                   单位:     ABC:    ".
	 tagline[06] = "        描述:                                     ".
	 tagline[07] = "      保管员:                                     ".
	 /*D887 added ref to following*/
	 tagline[08] = "     批/序号:                     参考:           ".
	 tagline[09] = "                                                  ".
	 tagline[10] = "      盘点量: ________      重新盘点量: ________  ".
	 tagline[11] = "    盘点单位: ________    重新盘点单位: ________  ".
	 tagline[12] = "盘点换算因子: ________    重盘换算因子: ________  ".
	 tagline[13] = "      盘点人: ________      重新盘点人: ________  ".
	 tagline[14] = "    盘点日期: ________    重新盘点日期: ________  ".
	 tagline[15] = "                                                  ".
	 tagline[16] = "        备注: __________________________________  ".
...
/*GUI*/   mainloop: do on error undo, return error on endkey undo, return error:
 ...
 /*D765*/
	    ...
	    for each tag_mstr exclusive where tag_nbr >= tag and tag_nbr <= tag1
...
/*F003*/          if available in_mstr then abc = in_abc.
		  ...
		  lotref = tag_ref.
	       ...
	       substr(tagline[01],15,8)  = substr(string(tag_nbr)
					 + fill(" ",8),1,8).
	       substr(tagline[03],15,8)  = substr(site + fill(" ",8),1,8).
	       substr(tagline[04],15,8)  = substr(loc + fill(" ",8),1,8).
	       substr(tagline[05],15,18) = substr(part + fill(" ",18),1,18).
	       substr(tagline[05],39,2)  = substr(um + fill(" ",2),1,2).
	       substr(tagline[05],48,1)  = substr(abc + " ",1,1).
/*bn083	       substr(tagline[06],15,24) = substr(desc1 + fill(" ",24),1,24). */
/*bn083*/       substr(tagline[06],15,24) = substr(desc2 + fill(" ",24),1,24).
/*bn083	       substr(tagline[07],15,24) = substr(desc2 + fill(" ",24),1,24). */
/*bn083*/       substr(tagline[07],15,24) = substr(keeper + fill(" ",24),1,24). 
	       substr(tagline[08],15,18) = substr(lotserial
					 + fill(" ",18),1,18).
	       substr(tagline[08],41,8)  = substr(lotref
					 + fill(" ",8),1,8). /*D887*/
