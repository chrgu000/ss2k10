...
for each pt_mstr no-lock where pt_prod_line >= line and pt_prod_line <= line1 
    ...
    use-index pt_prod_part break by pt_prod_line with width 255 no-box:
...
find last tr_hist where tr_part=pt_part and tr_site=pt_site and tr_price<>0  and tr_effdate<=date1 use-index tr_part_eff no-error.
...
disp pt_part pt_site pt_desc2 pt_um in_abc 
     pt_loc label "默认库位"
