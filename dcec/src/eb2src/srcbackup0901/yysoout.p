...
def var qty_cri like tr_qty_chg label "应发数量".
...
form "出库单"      at 33  
    ...
    /* "客户地址:"*/      ad_line1 label "客户地址:"   at  1
     ...
     "运输方式:" at 1       "运费承担单位：" at 35 skip(1)
     "零件号           零件名称                    日期     实发数量 流水号       序号 合同数量       累计 保管员"
     skip "---------------------------------------------------------------------------------------------------------"
...
form "出库单"      at 33  
     ...
     pdate         label "打印日期: "   at 70 
    /* "客户地址:"*/     ad_line1 label "客户地址:" at  1
     ih_rmks     label "备    注: "   at 70 skip(.1)
     "运输方式:" at 1       "运费承担单位：" at 35 skip(1)
     
     "零件号           零件名称                    日期     实发数量 流水号       序号 合同数量       累计 单位"
     skip "---------------------------------------------------------------------------------------------------------"
    ...
    if available tr_hist then do:
      ...
      /*   find first in_mstr where in_part = tr_part and in_site = site.*/
         ...
         if SOAV = yes and soddet="Y" THEN             
             disp tr_part  pt_desc2 tr_effdate qty "  " tr_serial format "x(8)"  tr_line format ">>>"  sod_qty_ord format "->>>>>>>" sod_qty_ship  format "->>>>>>>" space(3) /*pt_article*/ in__qadc01  with no-box no-labels width 132 frame c down.
         else
             if available(idh_hist) then
             disp tr_part  pt_desc2 tr_effdate qty "  " tr_serial format "x(8)"  tr_line format ">>>"  IDH_qty_ord format "->>>>>>>" IDH_qty_ship  format "->>>>>>>" space(3)  /*pt_article*/ in__qadc01  with no-box no-labels width 132 frame cIH down.
             else
