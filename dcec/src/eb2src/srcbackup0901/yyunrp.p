...
/*judy 05/08/05*/           AND  in__qadc01 >= keeper AND in__qadc01 <= keeper1 USE-INDEX IN_part  break by tr_nbr  by tr_effdate by tr_part: 
  ...
  /*judy 05/08/0 5*/   in__qadc01               
             format "x(8)" tr_serial  tr_site no-label tr_rmks with no-box no-labels width 250 frame c down.
        disp "----------------------------------------------------------------------------------------------------------------"
...
for each  tr_hist where  (tr_nbr >= nbr) and (tr_nbr <= nbr1)
 ...
 /*judy 05/08/05*/
      ...
      IF NOT AVAIL IN_mstr THEN UNDO, NEXT.
/*judy 05/08/05*/
