...
define variable xxpart       like lad_part.
define variable qopen      like lad_qty_all.
define variable yn         like mfc_logical.
define variable l_recno    as   recid        no-undo.

/*tfq*/ def var cbo as logical . /*getTermLabel("cancel_backorders",8).label "取消欠交量".*/
/*tfq*/ cbo = no.
/*tfq*/ def var temp_qty like lad_qty_chg.
/*tfq  cbo = getTermLabel("cancel_backorders",8).*/
issue_or_receipt = getTermLabel("ISSUE",18).
...
RECT-FRAME       AT ROW 1 COLUMN 1.25
 ...
 SKIP(.1)  /*GUI*/
   ...
   xxpart           colon 13       site           colon 53 location label "Loc"
...
/*tfq {gprun.i ""xxrepkisb.p"" "(nbr, prod_site, alloc, picked)"} */
   /*tfq*/ {gprun.i ""repkisb.p"" "(nbr, prod_site, alloc, picked)"} 
  ...
  do while true:
      ...
      hide message no-pause .
         if global-beam-me-up then undo mainloop, leave mainloop.
           message "按ESC键退出，其他任意键继续" /*VIEW-AS ALERT-BOX*/ .
         readkey .
        if lastkey = 27 then  undo mainloop, retry mainloop.
         /***tfq added end*****/
         /********tfq added begin***********/
         yn = yes.
         /* DISPLAY WO LINES BEING SHIPPED? */
       
        /*tfq*/ {pxmsg.i &MSGNUM=636 &ERRORLEVEL=1 &CONFIRM=yn }   
              if global-beam-me-up then undo mainloop, leave mainloop.  
     ...
     hide message no-pause .
         ...
         then do:
            ...
            /* AND xxpart FOR ALL COMPONENTS IN ISSUE SCREEN.       */
...
/*tfq*/   "" @ cbo   
      ...
      do on error undo, retry on endkey undo, leave:
       ...
       /*tfq*/ temp_qty = 0.   
   if cbo then do:       /*tfq*/                 
/*tfq*/ if cbo and (lad_qty_all + lad_qty_pick > sr_qty) then do:
 /*tfq*/ temp_qty = lad_qty_all + lad_qty_pick - sr_qty.
/*tfq/tfq*/ if lad_qty_pick >= sr_qty then do:
             lad_qty_pick = sr_qty.
             lad_qty_all = 0.
/*tfq*/ end.
/*tfq*/ else do:
/*tfq*/                  lad_qty_all = sr_qty - lad_qty_pick .  
/*tfq             lad_qty_all = lad_qty_all + lad_qty_pick - sr_qty.*/
/*tfq*/ end.
end .   /*tfq*/
/****************tfq added begin ***************************/
