...
/*judy*/
	 ...
	 define variable xxkeeper1 as char.
/*judy*/
...
nbr            colon 15
        ...
        xxkeeper COLON 15
       xxkeeper1 COLON 49 LABEL {t001.i}  skip (1)
...
/*GUI*/ procedure p-enable-ui:
        ...
        IF xxkeeper1 = hi_char THEN xxkeeper1 = "".
...
/*GUI*/ procedure p-report-quote:
        ...
        IF xxkeeper1 = ""  THEN  xxkeeper1 = hi_char.
...
procedure p-report: 
 ...
 FORM  
		  wo_nbr         colon 15        wo_lot  colon 35
		  wo_rmks        colon 61
	   ...
	   for each workorder where woor_qty>=0  :
		  ...
		  display wo_nbr wo_lot wo_qty_ord wo_ord_date wo_part      
		      ...
		      wo_status wo_rmks with frame c side-labels  .
                display "        零件号            零件名称                        需求量      保管员         实际领料量 " with  width 132 frame bt1.
  	         display "        -------------------------------------------------------------------------------------------" with width 132 frame bt3.  	     
                line = line + 8.
	       end.
          /* MESSAGE site woor_part "BBB".
           PAUSE.*/
             ...
             display space(8) woor_part woor_desc woor_qty  space(5) 
                         in__qadc01  WHEN AVAIL in_mstr WITH STREAM-IO no-labels .
	       down 1.
	       line = line + 1.
	       if page-size - line-counter < 2 then do:
                 display "        -------------------------------------------------------------------------------------------" AT 1  with width 132 frame ww.  
                 display "        保管员：           库房经理：           发运人：                 接收人：" AT 1 with width 132 frame ww.  	     
	          page.
	          pageno = pageno + 1.
	          line = 1.
	       end.       
           ...
           display "        -------------------------------------------------------------------------------------------" AT 1 with width 132 no-labels frame ww1.  
           display "        保管员：           库房经理：           发运人：                 接收人：" AT 1 with width 132 no-labels frame ww1.  	     
/* 显示回收物料清单 */
           line = 1 .
	    page.
	    pageno = pageno + 1.
	       ...
	       if line = 1 then do :
		  desc1 = "".
                display  "  页号:" pageno format ">>" space(24) "DCEC加工单领料单物料回收清单" with width 132 no-labels frame bt02.
		   display wo_nbr wo_lot wo_qty_ord wo_ord_date wo_part wo_qty_comp wo_rel_date  wo_qty_rjct wo_due_date
                 wo_status wo_rmks with frame c side-labels   .
  	         display "        零件号            零件名称                        回收量      保管员          实际回收量" AT 1 with width 132 no-labels frame bt2.
  	         display "        -------------------------------------------------------------------------------------------"  AT 1 with width 132 no-labels frame bt4.  	     
  	         line = line + 8.
  	         nextp=nextp + 1.
	       end.
	       ...
	       desc1 = "".
              ...
              display space(8) woor_part woor_desc woor_qty  space(5)  in__qadc01  WHEN AVAIL in_mstr WITH STREAM-IO no-labels . /* Modify by Zhang weihua 2004-09-28 */
	       down 1.
  	       line = line + 1.
	       if page-size - line-counter < 2  then do:
  	          display "        -------------------------------------------------------------------------------------------" AT 1 with width 132 no-labels frame ww2.  
         	   display "        保管员：           库房经理：           发运人：                 接收人：" AT 1 with width 132 no-labels frame ww2.  	     	     
	          page.
	          pageno = pageno + 1.
	          line =  1.
	       end.	       
	    ...
	    if nextp>=1 then do:
  	    display "        ----------------------------------------------------------------------"  AT 1 with width 132 no-labels frame ww3.  
           display "        保管员：           库房经理：           发运人：                 接收人："  AT 1  with width 132 no-labels frame ww3.  
           end.	     
...
/*GUI*/ {mfguirpb.i &flds=" nbr nbr1 part part1 xxkeeper xxkeeper1 site lot "} /*Drive the Report*/
