/* GUI CONVERTED from wowoisrc.p (converter v1.77) Mon Dec  1 04:07:00 2003 */
...
/* Revision: 1.33.1.3      BY: Hareesh V.       DATE: 09/17/03  ECO: *P132*   */
/* $Revision: 1.33.1.7 $ BY: Mike Dempsey         DATE: 11/27/03  ECO: *N2GM* */
...
{gldydef.i new}
{gldynrm.i new}
/*judy 05/08/12*/ define input parameter wosublot like wod_lot.
/*judy 05/08/12*/ define input parameter wosubqty like pod_qty_chg.
...
define variable w_whl_src_dest_id like whl_mstr.whl_src_dest_id.
...
wo_nbr      colon 12 wo_lot      eff_date    colon 68
 ...
 SKIP(.4)  /*GUI*/
with frame b /* frame a judy 05/08/12*/ side-labels width 80 attr-space NO-BOX THREE-D /*GUI*/.

 DEFINE VARIABLE F-a-title AS CHARACTER INITIAL "".
 RECT-FRAME-LABEL:SCREEN-VALUE in frame b /* frame a judy 05/08/12*/ = F-a-title.
 RECT-FRAME-LABEL:HIDDEN in frame b /* frame a judy 05/08/12*/ = yes.
 RECT-FRAME:HEIGHT-PIXELS in frame b /* frame a judy 05/08/12*/ =
  frame b /* frame a judy 05/08/12*/:HEIGHT-PIXELS - RECT-FRAME:Y in frame b /* frame a judy 05/08/12*/ - 2.
 RECT-FRAME:WIDTH-CHARS IN frame b /* frame a judy 05/08/12*/ = frame b /* frame a judy 05/08/12*/:WIDTH-CHARS - .5.  /*GUI*/

/*GUI preprocessor frame b /* frame a judy 05/08/12*/ undefine */
...
/* SET EXTERNAL LABELS */
setFrameLabels(frame b :handle). /* frame a judy 05/08/12*/
...
/*LB01*/ display wo_nbr wo_lot eff_date wo_part wo_status desc1 issue receive with frame b /*tfq frame a */.

/**************judy 05/08/12 added end**************************************/
   {&WOWOISRC-P-TAG9}
   display
      eff_date
      issue
      receive
      {&WOWOISRC-P-TAG10}
   with frame b /* frame a judy 05/08/12*/.

   prompt-for
/*judy 05/08/12*/    /*  wo_nbr
/*judy 05/08/12*/      wo_lot */
      eff_date
      receive
      issue
      {&WOWOISRC-P-TAG11}
   ...
   editing:
      ...
      then do:
         ...
         desc1 = "".
         for first pt_mstr
            fields(pt_loc pt_desc1 pt_part pt_prod_line)
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   then do:
      ...
      for last wlrm_mstr
         fields(wlrm_end wlrm_routing wlrm_start wlrm_trc_parent)
         where wlrm_routing = routing_code
      ...
      then do:
         ...
         for first wlc_ctrl
            fields(wlc_trc_parents wlc_enable_wlt)
            no-lock:
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   then do:
      ...
      or base
      then do:
         hide frame b /* frame a judy 05/08/12*/ no-pause.
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   and receive)
   then do:
      view frame b /* frame a judy 05/08/12*/.
 /*judy 05/08/12*/     /*{gprun.i ""woisrc02.p"" "(output trans_ok)"}*/
 /*judy 05/08/12*/     {gprun.i ""yywoisrc02.p"" "(input wosubqty, output trans_ok)"}
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   then do:
      ...
      for first wo_mstr
         fields(wo_acct wo_base_id wo_cc wo_due_date wo_fsm_type wo_joint_type
      ...
      then do:
         ...
         for first bom_mstr
            fields(bom_parent bom_mthd)
            where bom_parent = base_item
            no-lock:
...
/*GUI*/ if global-beam-me-up then undo, leave.
         ...
         then do:
            ...
            then do:
               ...
               for each sr_wkfl
                  fields(sr_loc sr_lotser sr_qty sr_ref sr_site sr_userid)
                  where sr_userid = mfguser
                  no-lock:

                  for first wo_mstr
                     fields(wo_acct wo_base_id wo_cc wo_due_date wo_fsm_type
                            wo_joint_type wo_lot wo_nbr wo_part wo_qty_chg
                            wo_qty_ord wo_rel_date wo_rjct_chg wo_routing
                            wo_site wo_status wo_stat_close_date
                            wo_stat_close_userid wo_sub wo_type)
 ...
 /* IF yn */
   ...
   then
      display
         ...
         wo_status
      with frame b /* frame a judy 05/08/12*/.
/*judy 05/08/12*/  /*LB01*/ hide frame b   no-pause.
				leave.	
