     ...
     if src_file = "" then src_file = "D:\N-DFL入库计划-101008.XLS".
      ...
      UPDATE site_file src_file xxraw validate(input xxraw > 0 ,"行号小于等于零是不允许的")  with frame a.
       ...
       IF TRIM(site_file) = "dcec-b" THEN do:
        ...
        FOR EACH yyusrw_wkfl WHERE yyusrw_key1 = "POB" : 
               DELETE yyusrw_wkfl.
           END.
       ...
       IF TRIM(site_file) = "dcec-c" THEN DO:
           SITE = "POC".
          FOR EACH yyusrw_wkfl WHERE yyusrw_key1 = "POC" : 
               DELETE yyusrw_wkfl.
           END.
            create  yyusrw_wkfl.
                    yyusrw_key1 = "POC". 
...
/*--------------------------------------比较---------------------------------*/
      ...
      create "Excel.Application" excelapp.
       ...
       repeat:    /*asn input repeat*/
         ...
         IF TRIM(SONBR) <> "" THEN DO: 
             FIND FIRST pt_mstr WHERE pt_part = trim(sonbr) NO-LOCK NO-ERROR .
     ...
     FOR EACH zzwk NO-LOCK where zzwksite = SITE and zzwkduedate >= today GROUP BY zzwknbr BY zzwkduedate:
            ...
            FIND pt_mstr NO-LOCK WHERE pt_part = zzwknbr.
