...
/*GUI*/ if global-beam-me-up then undo, leave.
  ...
  prompt-for usrw_key2 editing:
    ...
    {mfnp01.i usrw_wkfl usrw_key2 usrw_key2 'GOLDTAX-CTRL' usrw_key1 usrw_index1 }
  
    if recno <> ? then do:
      {zzgt003.i}
      find first ad_mstr 
           where ad_addr = usrw_key4
           no-lock no-error.
...
/*GUI*/ if global-beam-me-up then undo, leave.
    ...
    find first ad_mstr where ad_addr = usrw_key4 no-lock no-error.
    ...
    /* lb01
    find first ad_mstr where ad_addr = v_itemgtax no-lock no-error.
    ...
    if usrw_charfld[4] <> "" then do:
      ...
      do i = 1 to j:
        if entry(i,usrw_charfld[4], ",") <> "" then do:
          find first si_mstr where si_site = entry(i,usrw_charfld[4], ",") no-lock no-error.
          ...
          /*lb01
          if can-find(first bbusrwwkfl 
             where bbusrwwkfl.usrw_key1 = "GOLDTAX-CTRL"
