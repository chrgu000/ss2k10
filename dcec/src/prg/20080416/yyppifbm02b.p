...
/*J006**		if lookup(xxppif_tr_code,"PDSA,PDSC,SOSA,SOSC") > 0  then do:**/
						if lookup(xxppif_tr_code,"PDSA,PDSC,SOSA,SOSC") > 0  and trim(substr(xxppif_content,5,1)) <> "O" then do:
/*J004*/			 new_bom = yes.
...
/*J004*/		if avail ptp_det and new_bom = yes then do:		/* For SITE-C */		/*J005* Add an new condition to judge if it exist in SITE-C*/
...
/*J005*/           if (lookup(xxppif_tr_code,"PDSA,PDSC") = 0 and ((not avail code_mstr) or code_cmmt = site-c) or lookup(xxppif_tr_code,"PDSA,PDSC") > 0)
/*J005*/					   then do:  
...
/*J005*/						 put stream batchdata unformatted (if item_pm_code = "P" or item_phantom = yes then "X " else "- ") date_str.
