...
def var msg_desc as char format "x(80)".
...
FORM WITH FRAME b	down width 180 STREAM-IO .
...
repeat:
    ...
    if part1 = "" then part1 = hi_char.
       ...
       for each xxppif_log no-lock where xxppif_act_date >= actdate
                                     ...
                                     /*and xxppif_err <> 99 /*20050520*/*/
                                     and (errcode = 4 or xxppif_err = errcode) 
                                     and xxppif_tr_code <> ""
                                     break by xxppif_tr_id:
					 msg_desc = xxppif_msg.
					 
           if lookup(xxppif_tr_code,"XSCD") = 0 then 
           find FIRST code_mstr where code_fldname = "PPIF_MSG" and code_value = substr(msg_desc,1,4) no-lock no-error.    /*PPIF TO QAD*/
           if avail code_mstr then do:
              msg_desc = code_cmmt + "-" + substr(msg_desc,6).
           end.
       		 disp xxppif_tr_id 
       		      ...
       		      xxppif_act_time 
      				  ...
      				  xxppif_bld_date 
      				  msg_desc
    			      xxppif_content FORMAT "x(120)" colon 8
			     skip(1)
				with frame b.
				down with frame b.
		end.
