...
it's phantom when maintain
   the type 'x'*/
/* REVISION: 8.5       LAST EDIT: 06/22/08      BY: Wilber Yu, ECO: W001 detlet the control 
of phurchasing item has bom */   
...
/*********************************************************/
/* Last Modify by Jean 04/02/2006  If change production structure then transfer the operation of old structure to new structure *J001*/
/* Last Modify by Jean 04/02/2006  If change production structure then transfer the Structure Type of old structure to new structure *J002*/
/* DISPLAY TITLE */
{mfdtitle.i "b+ "}
...
/*cj*/ FIND bom_mstr WHERE bom_parent = INPUT ps_par EXCLUSIVE-LOCK .
 ...
 /*W001       find first ps_mstr where ps_par = input ps_par and ps__chr01 <> site no-lock no-error. 
...
/*cj*/   assign ps__chr01 = site.                     /*kevin*/
      ...
      do on error undo, retry:
         ...
         assign
            ...
            parent = ps_par.
/***Jean 02/21/2006***/           
				   if lookup(ps_rmks,"PDSA,PDSC,SOSA,SOSC") > 0 then do:

/*J001*	       run overlap_chk(parent,comp,recid(ps_mstr),ps_start,input-output ps_rmks,input-output ps_qty_per). */
				       run overlap_chk(parent,comp,recid(ps_mstr),ps_start,input-output ps_rmks,input-output ps_qty_per,input-output ps_op,input-output ps_ps_code).
				       disp ps_qty_per ps_rmks ps_op ps_ps_code.

           end.
           /**防止产生0的记录*/
/***Jean 02/22/2006***/            
           if ps_qty_per = 0 then do:
              message "需求数量不能为0".
...
status input.
/***Jean add 02/21/2006***/
procedure overlap_chk:
def input parameter input_par like ps_par.
def input parameter input_comp like ps_comp.
def input parameter input_recno as recid.
def input parameter input_start like ps_start.
def input-output parameter input_output_rmks like ps_rmks.
def input-output parameter input_output_qty like ps_qty_per.
/*J001*/ def input-output parameter input_output_operation like ps_op.
/*J002*/ def input-output parameter input_output_code like ps_ps_code.
def buffer ps_buff for ps_mstr.
find last ps_buff where recid(ps_buff) <> input_recno 
                     and ps_buff.ps_par = input_par
                     and ps_buff.ps_comp = input_comp
                     and ps_buff.ps_start <> input_start
                     and (ps_buff.ps_end = ? or ps_buff.ps_end >= input_start)
                     no-error.
if avail ps_buff then do:
/*J001*/	 input_output_operation 	= ps_buff.ps_op.
/*J002*/	 input_output_code			= ps_buff.ps_ps_code.
   if lookup(input_output_rmks,"PDSC,SOSC") > 0 then do:
      input_output_qty = ps_buff.ps_qty_per.
      {pxrun.i &PROC = 'processDelete' &PROGRAM = 'bmpscxr.p'
                    &PARAM = "(buffer ps_buff)"
               &CATCHERROR = true }
   end.
   else do:
      if ps_buff.ps_start < input_start then
      ps_buff.ps_end = input_start - 1.
   end.
end.                     
input_output_rmks = "".
end procedure.
