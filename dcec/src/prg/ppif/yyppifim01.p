...
/*W002 begin*/
        ...
        put stream batchdata unformatted "~"yyppptmt04-2.p~"" at 1.   /*1.4.3*/
        		for each xxppif_log where lookup(xxppif_tr_code,"PDIC,SOIC,PDIA,SOIA") > 0        		                      
        		                      and xxppif_err <> 2
...
/*W003   */
                   find first code_mstr no-lock where code_fldname = "ID_CODE" 
                        AND code_value = substring(xxppif_content,24,2) AND code_cmmt = "y" use-index code_fldval no-error.
                   if available code_mstr then do:
                       strTmp = "M".  /*强行改为M*/
...
/*W001*/ 
                ...
                put stream batchdata unformatted "~"" at 1 exec_part format "x(16)" "~"" skip.
                strTmp = "EA".
