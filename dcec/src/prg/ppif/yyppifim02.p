...
/*W003	 if i_exec = 1  then exec_part = exec_part.  */
        if i_exec = 1  then exec_part = trim(substring(xxppif_content,12,12)).
         ...
         /*    /** Check exist Item error**/
        	 find pt_mstr where pt_part = exec_part no-lock no-error.
...
/*W003*/       find first code_mstr no-lock where code_fldname = "ID_CODE" 
/*W003*/               AND code_value = substring(xxppif_content,24,2) AND code_cmmt = "y" use-index code_fldval no-error.
...
/*W003*/	         end.
                if lookup(item_type,"1,2,3,4,5") > 0 then do:
                   ...
                   if lookup(item_type,"3") > 0 then do:
                      strTmp = exec_part.
                      find first ptp_det  where ptp_part = exec_part and ptp_site = SITE-B no-error.
                  ...
                  if lookup(item_type,"2") > 0 then do:
                       strTmp = exec_part.
/*W001                 find first ps_mstr no-lock where ps_par = strTmp and ps_comp <> ""  no-error.
