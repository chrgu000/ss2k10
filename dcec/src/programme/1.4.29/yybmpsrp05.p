...
/*frk**/ AND xxwk.pmcode = "p"
       break by xxwk.wkctr by xxwk.comp:

       /*FIND FIRST ps_mstr NO-LOCK WHERE xxwk.par = ps_par NO-ERROR NO-WAIT.*/
       FIND FIRST ptp_det NO-LOCK WHERE xxwk.par = ptp_part NO-ERROR NO-WAIT.
       IF AVAIL ptp_det AND ptp_pm_code = "p" THEN NEXT.
