      ...
      then do:
         ...
         then do:
            ...
            then do:
               ...
               for first pt_mstr
                  fields(pt_bom_code pt_desc1 pt_desc2 pt_iss_pol
...
/*judy*/  
      ...
      then do:
         for first ecm_mstr
            fields(ecm_eff_date ecm_nbr)
            where ecm_nbr = string(ecmnbr,"x(8)") +
                            string(ecmid,"x(8)") +
                            string(dbase,"x(8)")
            no-lock:
         end. /* FOR FIRST ecm_mstr */
...
/*GUI*/ RECT-FRAME:HEIGHT-PIXELS in frame a = FRAME a:HEIGHT-PIXELS - RECT-FRAME:Y in frame a - 2.
   ...
   then do:
      ...
      for each ecm_mstr
         ...
         where ecm_eff_date <> ?,
            each ecd_det
            fields(ecd_nbr ecd_new_rev ecd_part)
            no-lock
            where ecd_nbr = ecm_nbr
            and ecd_new_rev = rev
...
/*judy*/      
      IF SUBSTRING(ps_comp, LENGTH(ps_comp) - 1 ,2) = "zz" THEN  DO: 
             xxpart = SUBSTRING(ps_comp, 1,LENGTH(ps_comp) - 2) .  /*judy*/
                    FIND FIRST pt_mstr WHERE pt_part = xxpart
                         NO-LOCK NO-ERROR.
                    IF AVAIL pt_mstr THEN do:
