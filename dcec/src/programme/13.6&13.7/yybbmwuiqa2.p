...
/*FL07*/ define variable eff_date as date column-label "As of Dt" .
...
define temp-table item-bom
   ...
   index  item-bom-par is unique item bom par.
...
/*hj01*/ empty temp-table item-bom.
...
/*G265*/                 desc1 = bom_desc.
/*hj01--*/
           IF SUBSTRING(ps_par , LENGTH(ps_par) - 1 , 2) = "zz" THEN DO:
            FIND FIRST pt_mstr NO-LOCK 
                WHERE pt_part = substring(ps_par, 1 , LENGTH(ps_par) - 2)
                NO-ERROR .
            IF AVAIL pt_mstr THEN xxstatus = pt_status .
           END.
/*--hj01*/
/*G265*/          end.
/*judy*/
...
/*hj01*/   
              IF yn = NO THEN display lvl ps_par desc1 ps_qty_per um  
                  l_phantom @ phantom
               ps_ps_code
               l_iss_pol @ iss_pol
...
/*J3J4*/        if available ps_mstr then
/*J3J4*/        do:
/*J3J4*/           run p_find_ptp_det(input false,
                                  input false).
...
procedure p_find_ptp_det:
   ...
   for each ptp_det
      ...
      then do:
         ...
         find first ps_mstr
            use-index ps_comp
            where ps_comp = ptp_part
            no-lock no-error.

         if not avail ps_mstr
         then
            next.
...
/* CHECK IF THE GIVEN COMBINATION OF PART, BOM CODE AND PARENT  */
/* IS ALREADY EXISTING. IF NOT EXISTING THEN WE CREATE THE TEMP */
/* TABLE OTHERWISE WE CLEAR THE ps_mstr BUFFER                  */
...
/* CREATE TEMP-TABLE WITH A PART,BOM CODE AND PARENT COMBINATION */
