...
DEF VAR runseq AS CHAR LABEL "过期过量".
/*Phi for label*/
...
/*J034*/          end.               
    ...
    for each in_mstr fields(in_site in_gl_set in_part in_qty_oh in__qadc01 in_qty_nonet in_user1 in_gl_cost_site in_cur_set) USE-INDEX in_site where /*(in_site >= site and in_site <= site1) and*/
                       in_site = site and 
                           (in_part >= part and in_part <= part1) and 
                           (in__qadc01 >= keeper and in__qadc01 <= keeper1) no-lock,
        each pt_mstr fields(pt_part pt_prod_line pt_abc pt_part_type pt_group pt_desc1 pt_desc2) USE-INDEX pt_part where pt_part = in_part and
       ...
       oth = 0.
        ...
        for each tr_hist FIELDS (tr_part tr_effdate tr_type tr_qty_loc tr_site tr_ship_type) USE-INDEX tr_part_eff no-lock where tr_part = pt_part
...
/*Phi --for label  */         
              ...
              disp pt_part pt_desc2 pt_prod_line pt_abc in__qadc01 label "保管员" 
                 in_user1 label "缺省库位" 
/*Phi -- add pt_buyer and pt_vend*/ buyer LABEL "计划员" vend LABEL "供应商"   runseq LABEL "过期过量" 
               bgqty label "期初库存"
