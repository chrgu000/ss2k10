...
repeat:
    ...
    update 
        site VALIDATE(CAN-FIND(si_mstr NO-LOCK WHERE si_site = site),
                         "地点错误,请重新输入!" )
    ...
    FIND FIRST tr_hist NO-LOCK WHERE tr_effdate >= v_firstday NO-ERROR .
    ...
    FOR EACH in_mstr fields(in_part in_site in_qty_oh)
        NO-LOCK WHERE in_site = site
        AND in_part >= part AND in_part <= part1:
        FIND FIRST ptp_det NO-LOCK WHERE ptp_part = in_part 
        ...
        IF v_buyer >= buyer AND v_buyer <= buyer1 
            ...
            FOR EACH mrp_det 
                ...
                WHERE mrp_dataset = "wod_det" AND mrp_part = in_part
                ...
                IF LAST-OF(mrp_part) THEN do:
                    ...
                    FOR EACH tr_hist FIELDS(tr_site tr_part tr_nbr tr_type
                        ...
                        NO-LOCK WHERE tr_trnbr >= v_trnbr
