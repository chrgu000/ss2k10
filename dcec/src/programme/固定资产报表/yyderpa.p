...
empty temp-table tt_fabddetail.
 find first fabk_mstr where fabk_post = yes no-lock no-error.
...
/* GET EACH ASSET ID WITHIN USER SELECTED RANGES */
     for each fa_mstr where fa_id >= fromAsset and fa_id <= toAsset 
   ...
   then do:
      ...
      then
         ...
         for first fabddet
            ...
            where fabddet.fabd_fa_id   = fa_id  /* fabd_det.fabd_fa_id  */
   ...
   then do:
      ...
      ovramt = 0.
     find first facls_mstr where facls_mstr.facls_id = fa_mstr.fa_facls_id no-lock no-error.
       if avail facls_mstr then cls_desc = facls_mstr.facls_desc.
    
     find last fab_det where fab_fa_id = fa_id and fab_fabk_id = bk_id
             no-lock no-error.
      ...
      find last fabd1 where fabd1.fabd_fa_id =  fa_id  and fabd1.fabd_fabk_id =  bk_id
          no-lock no-error.
      ...
      if ovramt > 0 then do:
        ...
        for each glc_cal where glc_start > ovrdt and 
       ...
       if ovramt = 0 then do:
             used_yrper = 0.
        for each glc_cal where glc_start > service_date and 
       ( glc_year < integer(substring(last_yrper,1,4))
                or  (glc_year = integer(substring(last_yrper,1,4)) and  glc_per <= integer(substring(last_yrper,5,6))) )
            no-lock:
              used_yrper = used_yrper + 1.
     ...
     find first  faloc_mstr where  faloc_mstr.faloc_id =  fa_mstr.fa_faloc_id no-lock no-error.
        if avail faloc_mstr then loc_desc = trim(faloc_mstr.faloc_desc).
        
     find first ad_mstr where ad_addr = string(trim(fa_mstr.fa_faloc_id)) no-lock no-error.
     ...
     find first code_mstr where code_fldname = "fa_chr04" and trim(string(code_value))= 
