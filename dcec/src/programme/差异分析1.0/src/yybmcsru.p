...
repeat:
   ...
   if not batchrun then do:
      ...
      do transaction on error undo, retry:
         ...
         editing:
            ...
            if transtype = "SC" then do:
               {mfnp01.i cs_mstr csset cs_set ""SIM"" cs_type cs_type}
            end.
            else do:
         ...
         if transtype = "SC" then do:
            if not can-find(first cs_mstr where cs_set = csset
      ...
      status input.
      update
         part VALIDATE(CAN-FIND(FIRST pt_mstr WHERE pt_part = part), "«Î ‰»Î¡„º˛∫≈...")
          /*james***part1
         line line1
         type type1
         grp grp1 */
