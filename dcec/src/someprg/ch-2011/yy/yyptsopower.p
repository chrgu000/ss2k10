...
DEFINE VAR v_power LIKE ps_par.
...
REPEAT:
   ...
   FOR EACH tr_hist WHERE  tr_type = "iss-wo" AND tr_effdate >= bedate AND tr_effdate <= eddate AND tr_part =itpart USE-INDEX tr_type BREAK BY tr_lot .
   ...
   FOR EACH xxwkpt.
    FOR EACH tr_hist WHERE tr_type ="rct-wo" AND  tr_effdate >= bedate AND tr_effdate <=eddate AND tr_lot = xxwkpt.lot USE-INDEX tr_type BREAK BY tr_lot. 
        IF FIRST-OF (tr_lot) THEN sosum =0.
        sosum= sosum + tr_qty_loc.
        IF LAST-OF(tr_lot) THEN do:
           FIND FIRST ps_mstr WHERE ps_comp = tr_part OR SUBSTRING(ps_comp , 1 , LENGTH(ps_comp) - 2) = tr_part NO-LOCK NO-ERROR.
  ...
  FOR EACH xxwkso.
      DISP xxwkso WITH WIDTH 180 STREAM-IO.
  END.
