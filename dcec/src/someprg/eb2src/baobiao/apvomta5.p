...
/* Revision: 1.79.1.3   BY: Deepali Kotavadekar   DATE: 09/30/03  ECO: *P13S* */
/* Revision: 1.79.1.6   BY: Vandna Rohira         DATE: 11/03/03  ECO: *P14V* */
/* Revision: 1.79.1.10  BY: Mike Dempsey          DATE: 11/27/03  ECO: *N2GM* */
/* Revision: 1.79.1.12  BY: Ed van de Gevel       DATE: 12/01/03  ECO: *P0ZG* */
/* Revision: 1.79.1.13  BY: Bhagyashri Shinde     DATE: 02/25/04  ECO: *P1QH* */
/* Revision: 1.79.1.14  BY: Shivanand H           DATE: 03/12/04  ECO: *P1SL* */
/* Revision: 1.79.1.15  BY: Manisha Sawant        DATE: 04/29/04  ECO: *P1ZK* */
/* $Revision: 1.79.1.16 $         BY: Preeti Sattur         DATE: 05/19/04  ECO: *P217* */
/******************************************************************************/
...
repeat:
   ...
   do on error undo, retry:
      ...
      then do:
         ...
         {gpcontxt.i
            &STACKFRAG = 'txtrnpop,apvomta5,apvomta,apvomt'
  ...
  /*huangjie       set
         ...
         go-on (F4 PF4 CTRL-E ESC)
         with frame f. */
/*J001*/ set vph_inv_qty = rcvd_open 
/*J001*/     vph_curr_amt = l_prh_pur_cost.
/*J001*/ Disp
/*J001*/            vph_inv_qty
/*J001*/            vph_curr_amt
/*J001*/         with frame f.
      ...
      then
          ...
          * ADDED PARAMETER AS CLOSE LINE WHILE       *
          * PARTIAL INVOICING A PO RECEIPT.           */

         /* ADDED INPUT-OUTPUT PARAMETER l_ctr TO DELETE */
         /* PO INTRASTAT RECORDS ON FULL VOUCHERING.     */
...
PROCEDURE backOutVoucherQuantity:
   ...
   if ip_vouchered_qty < 0
   then
      l_sign = -1.

   work_vouchered_qty = absolute(ip_vouchered_qty).
   ...
   for each pvomstr
      ...
      break by pvo_external_ref descending:
         ...
         pvo_vouchered_qty = pvo_vouchered_qty * l_sign.
...
PROCEDURE setLastVoucher:
   ...
   for each pvomstr
      where pvo_internal_ref = ip_receiver
        and pvo_line = ip_line
        and (pvo_vouchered_qty <> 0 or ip_new_vo = "")
