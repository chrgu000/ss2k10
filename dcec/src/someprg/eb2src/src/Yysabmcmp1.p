...
delete mybomcmp .
  ...
  set parent1
	...
	***********tfq deleted end*************/
	...
	find first ps_mstr where ps_par = parent1 and ps__chr01 <> "" no-lock no-error .
	if available ps_mstr then site = ps__chr01 .
	 {gprun.i ""yybmpkiqc.p"" "(input parent1,
                               INPUT site,
                               INPUT effdate1)"}
         for each pkdet  :
	...
	chExcelWorkbook:Worksheets(1):Cells(iRow,6) = "'" + string(effdate2).
