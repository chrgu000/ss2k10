...
/* Revision: 1.82         BY: Shoma Salgaonkar    DATE: 05/06/03  ECO: *P0RC* */
/* Revision: 1.82.1.1     BY: K Paneesh           DATE: 06/30/03  ECO: *N2HS* */
/* Revision: 1.82.1.2     BY: Jyoti Thatte        DATE: 08/29/03  ECO: *P106* */
/* Revision: 1.82.1.4     BY: Veena Lad           DATE: 11/21/03  ECO: *P1BN* */
/* $Revision: 1.82.1.5 $       BY: Manish Dani   DATE: 02/13/04 ECO: *P1P0* */
/******************************************************************************/
...
/******************************************************************************/
/* $Revision: eb2+sp7       BY:tao fengqin   DATE: 06/21/05 ECO: *tfq* */
/*cj* add for substitute part and part status*/
{mfdeclre.i}
/*tfq {cxcustom.i "SOSOMTA.P"}*/
{cxcustom.i "yySOSOMTA.P"}
{gplabel.i} /* EXTERNAL LABEL INCLUDE */

/* CHANGES MADE TO THIS PROGRAM MAY ALSO NEED TO BE MADE */
/* TO PROGRAM fseomta.p.                                 */
/*judy 051226*/  SESSION:DATA-ENTRY-RETURN = TRUE.
/*judy 051226*/  SESSION:AUTO-RETURN = TRUE.
...
define new shared variable cf_pm_code     like pt_pm_code.
define new shared variable cf_pt_part_o   like pt_part.
define new shared variable cf_pt_part_n   like pt_part     no-undo.
define new shared variable cf_line        like sod_line    no-undo.
define new shared variable cf_rp_part     like mfc_logical no-undo.
define new shared variable cf_endcfg      like mfc_logical no-undo.
define new shared variable original_model as   character   format "x(40)".
...
define            variable cf_sod__qadc01 like sod__qadc01.
...
/*cj*/ DEF VARIABLE ptssub LIKE pts_sub .
/*cj*/ DEF VARIABLE ptsta LIKE pt_status .
...
for first so_mstr
   fields(so_conf_date   so_curr     so_cust       so_div
...
if this-is-rma then do:
   ...
   for first rmc_ctrl
      fields(rmc_edit_isb rmc_lcmmts)
   no-lock: end.
   ...
   for first rma_mstr
      fields(rma_contract rma_ctype rma_enduser rma_loc_iss
   ...
   rma-line-type = if rma-issue-line
                   then
                      "RMA-ISS"
                   else
                      "RMA-RCT".
...
if this-is-rma then do:
 ...
 /*tfq  {fsmnp01.i ""sosomt.p"" 20 solinerun} */
  /*tfq*/  {fsmnp01.i ""yysosomt.p"" 20 solinerun} 
end.
   ...
   repeat with frame c down on endkey undo, next linefmt:
      ...
      {&SOSOMTA-P-TAG12}
       ...
       then do:
          ...
          run getLastSodLine (input so_nbr,
                              output l_last_line).
      ...
      if line < 999 then line = line + 1.
         ...
         /* LINE CANNOT BE > 999 */
         run p-mfmsg (input 7418, input 2).
      end.

      for first sod_det where sod_nbr  = so_nbr and
                              sod_line = line
      no-lock: end.
      ...
      then do:
         ...
         for first pt_mstr
         fields(pt_desc1 pt_fr_class pt_loc pt_part pt_pm_code
     ...
     /*tfq          base_curr sod_std_cost*/ desc1
               sod_pricing_dt
      ...
      if soc_use_btb and not so_secondary and first-repeat then do:
         ...
         for each sod_buff2 where sod_buff2.sod_nbr = so_nbr
      ...
      if not del-yn then do:
         ...
         if not cf_rp_part then do:
            {&SOSOMTA-P-TAG13}

            lineEntryLoop:
            repeat on endkey undo, next linefmt:
/*judy */ /*GUI*/ if global-beam-me-up then undo, leave.
...
/*judy*/
                  ...
                  if recno <> ? then do:
                     ...
                     for first pt_mstr
                     fields(pt_desc1 pt_fr_class pt_loc pt_part
                     ...
                     display
                        ...
                        sod_qty_ship sod_qty_inv
                        sod_loc sod_site sod_serial
                    /*tfq    sod_std_cost*/  desc1 /*tfq base_curr*/
                        sod_pricing_dt
                        sod_req_date sod_per_date sod_due_date
                        sod_promise_date
                        sod_fr_list
               ...
               */
               if so_app_owner <> "" and
                  not can-find(sod_det where sod_nbr  = so_nbr
            ...
            end. /*if not cf_rp_part*/

         if cf_rp_part then do:
            line = cf_line.
         ...
         exclusive-lock no-error.
         if available sod_det  and not this-is-rma then do:
            ...
            for each tblcd_det no-lock where
               tblcd_table = "sod_det":
      ...
      if not available sod_det then do:
         if line = 0 then do:
            /* INVALID LINE NUMBER */
            run p-mfmsg (input 642, input 4).
            undo mainloop, retry.
         end.

         if so_secondary then do:
            /* NEW LINE IS NOT ALLOWED AT THE SBU */
            run p-mfmsg (input 2939, input 3).
            undo mainloop, retry.
         end.
         ...
         display
            ...
            desc1 /*tfq sod_std_cost base_curr*/
            sod_pricing_dt
            sod_req_date sod_per_date sod_due_date
            sod_promise_date
            sod_fr_list
...
/*cj*add*beg*/
            FIND FIRST pts_det NO-LOCK WHERE pts_part = INPUT sod_det.sod_part NO-ERROR .
            IF AVAILABLE pts_det THEN ptssub = pts_sub .
            FIND pt_mstr NO-LOCK WHERE pt_part = INPUT sod_det.sod_part NO-ERROR .
            IF AVAILABLE pt_mstr THEN ptsta = pt_status .
            MESSAGE "Ìæ´úÁã¼þ£º" ptssub "        Áã¼þ×´Ì¬£º" ptsta .
/*cj*add*end*/

         end. /*if not cf_rp_part*/

         if cf_rp_part then do:
            sod_part = cf_pt_part_n.
            display sod_part with frame c.
         end.

         if not cf_rp_part then
            assign sod_part.
         ...
         if this-is-rma then do:
            ...
            if rma-issue-line then do:
               ...
               if sod_fsm_type = "RMA-RCT" then .
                  ...
                  run p-mfmsg (input 7207, input 3).
                  undo mainloop, retry.
               end.
            end.    /* else do */
            {&SOSOMTA-P-TAG34}
            ...
            for first rmd_det
            fields(rmd_edit_isb rmd_exp_date rmd_line rmd_link
                   rmd_nbr rmd_prefix rmd_rma_line rmd_rma_nbr
                   rmd_rma_rtrn rmd_sa_nbr rmd_sv_code rmd_type)
            where rmd_nbr    = sod_nbr and
                  rmd_prefix = "C"     and
                  rmd_line   = sod_line no-lock:
   ...
   /*tfq*/   {gprun.i ""yysosomtla.p""
      ...
      if del-yn = yes then do:
         ...
         for first wo_mstr
         fields(wo_nbr wo_part)
         where wo_nbr  = (sod_nbr + "." + string(sod_line)) and
               wo_part = sod_part
         no-lock:
            /* Final assy work order exists for this line */
            run p-mfmsg (input 515, input 2).
            pause.
         ...
         if sod_sched and
            (can-find (first sch_mstr where
             ...
             can-find (first sch_mstr where
                             ...
                             sch_line = sod_line))
         then do:
            run p-mfmsg (input 6022, input 3).
         ...
         if so_secondary then do:
            ...
            run p-mfmsg (input 2938, input 3).
            undo mainloop, retry mainloop .
         end.
        ...
        {gprun.i ""gppiqty2.p""
         ...
         run p-sobdel.  /*tfq /*LB01*/          {gprun.i ""zzsosomtk.p""}*/
         ...
         if cfexists then cf_sod__qadc01 = sod__qadc01.
         ...
         if cfexists and cf_sod__qadc01 <> "" then
            /* Delete any associated configuration files*/
            os-delete value(cf_cfg_path + cf_chr  + cf_sod__qadc01).
         /* Line Item deleted */
         run p-mfmsg (input 6, input 1).
         next mainloop.
      end.
      run pl-comment.
 ...
 * ------------------------------------------------------------------*/
   ...
   no-lock:
      ...
      if pih_promo2 = "B" then do on endkey undo, leave:
         ...
         for first pi_mstr
            fields(pi_cmtindx pi_list_id)
            where pi_list_id = pih_list_id
         no-lock:
 ...
 * ------------------------------------------------------------------*/
   ...
   for each sob_det where
            sob_nbr  = sod_det.sod_nbr
        and sob_line = sod_det.sod_line
   exclusive-lock:

      for each cmt_det where cmt_indx = sob_det.sob_cmtindx
      exclusive-lock:
         delete cmt_det.
   ...
   for each sct_det where
            sct_part = sod_det.sod_part
        and sct_sim  = string(sod_det.sod_nbr) + "." + string(sod_det.sod_line)
   exclusive-lock:
      delete sct_det.
