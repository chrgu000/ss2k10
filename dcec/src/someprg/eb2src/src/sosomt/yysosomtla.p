...
/* Revision: 1.138.1.3   BY: Ashish Maheshwari   DATE: 11/28/03  ECO: *P1CN* 
  */
/* Revision: 1.138.1.6   BY: Ed van de Gevel     DATE: 12/03/03  ECO: *P1D6* 
  */
/* Revision: 1.138.1.7   BY: Subramanian Iyer    DATE: 01/12/03  ECO: *P1J7* 
  */
/* Revision: 1.138.1.9   BY: Kirti Desai         DATE: 01/30/04  ECO: *P1JH* 
  */
/* Revision: 1.138.1.10  BY: Shoma Salgaonkar    DATE: 02/23/04  ECO: *P1PY* 
  */
/* Revision: 1.138.1.11  BY: Pankaj Goswami      DATE: 03/16/04  ECO: *P1SQ* 
  */
/* Revision: 1.138.1.14  BY: Deepak Rao          DATE: 04/19/04  ECO: *P1XL* 
  */
/* $Revision: 1.138.1.15 $          BY: Manjusha Inglay     DATE: 04/29/04  
ECO: *P1ZV*  */
/******************************************************************************/
/* All patch markers and commented out code have been removed from the 
source */
/* code below. For all future modifications to this file, any code which is  
  */
/* no longer required should be deleted and no in-line patch markers should  
  */
/* be added.  The ECO marker should only be included in the Revision 
History. */
/******************************************************************************/
...
*/
/* REVISION: eb2+sp7     retrofit: 06/21/05   BY: *tfq* Taofengqin         
*/
         ...
         /*!
            ...
            price) for Sales Orders and RMA's.  It is called by SOSOMTA.P, 
and
         ...
         /*!
            Applies v8.5 pricing to RMA issue lines.  The use of 
rma-issue-line
            will appear wherever a v8.5 pricing function will occur.  This
            supports both Sales Orders and RMA issue lines.  Where a 
coverage
            discount applies, it will be treated as a "manual" discount for 
the
...
do on error undo, leave:
   ...
   find first so_mstr where recid(so_mstr) = so_recno exclusive-lock 
no-error.

   find first sod_det where recid(sod_det) = sod_recno exclusive-lock 
no-error.
...
*/
         ...
         if not del-yn then do:
            ...
            set1:
            do with frame c on error undo, retry on endkey undo, leave 
loopc:
               ...
               if available pt_mstr and pt_lot_ser = "s" and  pt_um <> 
sod_um
               then do:
               ...
               then do:
                  ...
                  if this-is-rma then do:
                     if not rma-issue-line then do for rtsbuff:
                        ...
                        no-lock:
                           /* THIS RMA LINE IS LINKED TO ONE OR MORE RTS 
LINES */
                  ...
                  if so_app_owner > "" then do:
                     ...
                     {pxmsg.i &MSGNUM=2948 &ERRORLEVEL=4 
&MSGARG1=so_app_owner}
   ...
   then do:
      ...
      then do:
         ...
         for each ieh_hist
            where ieh_authority = iec_authority
...
/*cj1009*/      MESSAGE "订单倍数：" pt_ord_mult .
     ...
     do:
            ...
            undo_all2 = true.
     /*tfq       {gprun.i ""sosomtf8.p""} */
     /*tfq*/       {gprun.i ""yysosomtf8.p""}
            hide frame bom.
...
/*         if new_order or reprice_dtl then
            update
               sod_list_pr
      /*tfq         discount  */ /*limit modify discount*/
         ...
         if rma-issue-line then do:
            ...
            if minerr or maxerr then do:
               ...
               assign
                  ...
                  parent_list_price = sod_list_pr.  /*gppiwk02.p NEEDS 
THIS*/
            ...
            /* CONSIDER COVERAGE DISCOUNT WHEN LIST PRICE IS ZERO */
                        ...
                        then
                           assign
                              ...
                              l_discount = yes.
                     end. /* IF rma-issue-line AND coverage-discount > 0 */
...
------------------------------------------------------------------*/
   ...
   then do:
      ...
      save_disc_pct = if sod_det.sod_list_pr <> 0
                      then
                         (1 - (sod_price / sod_list_pr)) * 100
                      else
                          0.
...
------------------------------------------------------------------*/
   ...
   if sod_type = "" then do:
      ...
      if sod_qty_ord < 0 and l_frc_returns = "z"
      then
         sod_fr_wt = 0.
      else
