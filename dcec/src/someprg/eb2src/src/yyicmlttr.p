...
/*review by tfq cj frm 2006/06/20 for eb2+sp7 retrofit********************************/
         /* DISPLAY TITLE */
...
def var tr_recno as recid format "->>>>>>9".
...
def temp-table tr
     field id as char format "x(2)" label "选择"
...
form
 ...
 SKIP(.1)  /*GUI*/
   eff_date colon 22          so_job colon 44
   nbr colon 22               rmks colon 44
...
form
 SKIP(.1)  /*GUI*/
   ...
   tr.qty_tr label "数量"
   skip(.1) 
   /*with /*down*/ frame c width 90 /*overlay*/ title color normal frametitle THREE-D stream-io /*GUI*/.         */
   with frame c center column 5 row 12 overlay 
   width 90 title "Transfer Detail Maintain" THREE-D stream-io /*GUI*/.   
...
repeat:
   ...
   update eff_date so_job nbr rmks site_from loc_from site_to loc_to with frame a.   
   
   /*verify the from site and from location*/
...
/*J034*/          end.
   ...
   /*verify the to site and to location*/
...
/*J034*/          end.
   ...
   if site_to = site_to and loc_from = loc_to then do:
      message "'从'地点、库位与'至'地点、库位不能一致,请重新输入!" view-as alert-box error.
      next-prompt loc_to with frame a.
      undo,retry.
   end.
  

   /*verify the status of loc_to*/
   find isd_det no-lock
...
/*F089*/   end.
   ...
   find first tr no-lock no-error.
   if not available tr then do:
      message "无需要处理的数据,请重新输入!" view-as alert-box error.
      undo,retry.
   end.
        ...
        repeat:
         ...
         /* INCLUDE SCROLLING WINDOW TO ALLOW THE USER TO SCROLL      */
               ...
               {swselect.i
                  ...
                  &display3     = tr.part
                  &display4     = tr.um
                  &display5     = tr.lot_from
                  &display6     = tr.ref_from
                  &display7     = tr.qty
                  &display8     = """"
                  &exitlabel    = sw_block
           ...
           if keyfunction(lastkey) = "end-error"
            ...
            or lastkey = keycode("CTRL-E") then do:
             ...
             for each tr where tr.id = "*" no-lock:
               ...
               if not ok_yn then undo mainloop,retry mainloop.
...
/*FR46*/        /*tfq  {mfmsg.i 7179 3}  */ /*Item does not exist */
            {pxmsg.i
               &MSGNUM=7179
               &ERRORLEVEL=3
                           }
...
/*/*GH52*/    repeat:*/                         /*marked by kevin*/
            do /*tfqtransaction*/ on error undo , leave:     /*added by kevin*/
/*GUI*/ if global-beam-me-up then undo, leave.
