...
&SCOPED-DEFINE repkupd_p_14 "Target loc"
/* tfq: Comment: */

&SCOPED-DEFINE repkupd_p_15 "Comment"
/* tfq: Comment: */
...
define temp-table shortages no-undo
   ...
   /*sunman**************/
	  field short-site like wo_site
	  field short-wkctr like wr_wkctr
	  field short-wcdesc like wc_desc
	  field short-pknbr as char format "x(10)"
          field short_deliv like wod_deliv
/************sunman*/
 index shortages short-part short-date short-assy.
/*************tfq*************************
define temp-table short-pklst no-undo
   field short-part  like ps_comp
   field short-assy  like ps_par
   field short-qty   like wod_qty_chg column-label {&repkupd_p_11}
   field short-date  like wr_start
...
***************tfq************************/
/*******tfq added begin******************/
define variable xxtoloc as character .
define variable xxcomment as character format "x(20)" .
define variable xxremqty like tr_qty_loc .
define variable xxallqty like tr_qty_loc .
/*********tfq added end******************/  
...
by wr_start by wr_part by wr_op
   with frame c width 132 no-attr-space down no-box stream-io :
   ...
   if first-of(wr_wkctr) then do:
      ...
      form
         ...
         nbr
         skip(1)
      with STREAM-IO frame b  width 132 side-labels page-top.
      ...
      display
         ...
         skip(1)
      with frame b width 132 side-labels page-top STREAM-IO .
   end.
...
/*roger*/		     {gprun.i ""yypkall.p""
		     "(nbr,wod_part,wo_site,wr_wkctr,wod_deliver
		     ,comp_max, wod_lot
		     ,input-output qtyneed)" }
 /*tfq        {gprun.i ""repkall.p"" "(nbr,
                                  wod_part,
                                  wo_site,
                                  wr_wkctr,
                                  wod_deliver ,
         ...
         if detail_display then do
            with frame dd width 255 no-attr-space down no-box:
            ...
            xxremqty = accum total by wod_deliver
               (max(wod_qty_req - wod_qty_all -
               wod_qty_pick - wod_qty_iss,0)) .   /*tfq*/ 
            display
            ...
            by lad_part by lad_site with frame dd width 255:
               xxallqty = lad_qty_all + xxallqty .  /*tfq*/
               if xxallqty <= xxremqty then  /*tfq*/
               do:    /*tfq*/ 
               display
                  lad_loc
                  lad_lot
                  lad_qty_all column-label {&repkupd_p_12}
                  xxtoloc   column-label {&repkupd_p_14}
                  lad_user1 @ wod_deliver 
                  "" @ xxcomment column-label {&repkupd_p_15} /*tfq*/
                  WITH STREAM-IO /*GUI*/ .
                end .  /*tfq*/
                /*******tfq added begin*********/
                else do:
                    if last-of(lad_site) then
                    do:
                    xxremqty = xxallqty - xxremqty .
                    xxcomment = "批量多余量--" + string(xxremqty) .
                  display
                  lad_loc
                  lad_lot
                  lad_qty_all column-label {&repkupd_p_12}
                  xxtoloc column-label {&repkupd_p_14}
                  "" @ wod_deliver column-label {&repkupd_p_15}
                   xxcomment  /*tfq*/
                  WITH STREAM-IO /*GUI*/ .
                  end.
                end.
            ...
            if qtyneed > 0 then do with frame dd width 255:
               if can-find
         ...
         else do with frame d width 255 no-attr-space down no-box stream-io:

            /* SET EXTERNAL LABELS */
            setFrameLabels(frame d:handle).
            display
               wod_part @ ps_comp desc1.
        /*tfq*/    xxremqty = accum total by wod_deliver
                  (max(wod_qty_req - wod_qty_all -
                   wod_qty_pick - wod_qty_iss,0)) .   
            display
               accum total by wod_deliver
            ...
            where lad_dataset = "rps_det"
              ...
              and lad_user1 = wod_deliver
            break by lad_dataset by lad_nbr by lad_line
            by lad_part by lad_site with frame d width 255:
                xxallqty = xxallqty + lad_qty_all .   /*tfq*/ 
                if xxallqty < xxremqty then /*tfq*/
                do:
               display
                  lad_loc
                  lad_lot
                  lad_qty_all column-label {&repkupd_p_12}  
                  xxtoloc    column-label {&repkupd_p_14}      /*tfq*/
                  lad_user1 @ wod_deliver 
                  "" xxcomment column-label {&repkupd_p_15} 
                  WITH STREAM-IO /*GUI*/ .
                 end .
                 /**********tfq added begin****************/
                 else do:
            ...
            if qtyneed > 0 then do with frame d width 255:
   ...
   if last-of (wr_wkctr) then do:
      ...
      if available shortages then do:
         ...
         with frame shortage WITH STREAM-IO /*GUI*/ .
         ...
         with frame short width 132 no-attr-space:
            ...
            display
               space(19)
            with frame short WITH STREAM-IO /*GUI*/ .
      ...
      end. /*if available shortages*/
      *********************  SUNMAN COMMENTS IF AVAILALE SHORTAGE */
...
/*SUNMAN ADDED */
		...
		if first-of (short-wkctr) then do:
			...
			create short-pklst.
			assign
			short-pklst.short-site = shortages.short-site
			short-pklst.short-wkctr = shortages.short-wkctr
			short-pklst.short-wcdesc = shortages.short-wcdesc
			short-pklst.short-part = shortages.short-part
			short-pklst.short-assy = shortages.short-assy
			short-pklst.short-qty = shortages.short-qty
			short-pklst.short-pknbr = substring(shortages.short-pknbr,9).
			short-pklst.short-deliv = short_deliv.
			delete shortages.
		     end.
		     hide frame shortage.
 ...
 /*sunman *****/
 end. /*if last-of(wr_wkctr)*/

end. /*for each wr_router*/
...
/*tfq*/ find wo_mstr where wo_nbr = wod_nbr and wo_lot = wod_lot no-lock .
/*tfq*/ find first wc_mstr where wc_wkctr = wr_wkctr no-lock .
   ...
   do:
      ...
      do:
         ...
         assign
            ...
            short-date  = wr_start
            short-assy  = wr_part
            short_deliv = wod_deliver
            /*sunman ***********/
            short-site = wo_site
	    short-wkctr = wr_wkctr
	    short-wcdesc = if available wc_mstr then wc_desc else ""
	    short-pknbr = if not delete_pklst then nbr else nbr_replace.
