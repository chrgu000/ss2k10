...
/* Revision: 1.13.2.23.1.2  BY: Dipesh Bector     DATE: 12/26/03 ECO: *P1H9*  */
/* Revision: 1.13.2.23.1.3  BY: Salil Pradhan     DATE: 01/06/04 ECO: *P1HT*  */
/* Revision: 1.13.2.23.1.4  BY: Dipesh Bector     DATE: 01/12/04 ECO: *P1JG*  */
/* Revision: 1.13.2.23.1.5  BY: Bhagyashri Shinde DATE: 02/27/04 ECO: *P1R1* */
/* $Revision: 1.13.2.23.1.6 $  BY: Subramanian Iyer DATE: 03/12/04 ECO: *N2QC* */
/******************************************************************************/
...
/* REVISION: eb2+sp7    LAST MODIFIED: 06/20/05    BY: *tfq* tao fengqin   cj      */
/* DISPLAY TITLE */
{mfdtitle.i "2+ "}
/* Clear anything displayed by mftitle if api mode.*/
{mfaititl.i}
/*tfq  {cxcustom.i "POMT.P"}*/
/*tfq*/  {cxcustom.i "YYPOMT.P"}
{pxmaint.i}
...
/*tfq added begin******cj***************************************/
         IF GLOBAL_userid <> po_user_id THEN do:
                MESSAGE "您不是该订单的创建用户，您不能修改该订单" VIEW-AS ALERT-BOX ERROR.
...
/****tfq added end*******cj**************************************/
      ...
      then do:
         tax_in = no.
         for first ad_mstr
        fields (ad_addr ad_tax_in)
            where ad_addr = po_vend
         no-lock:
            tax_in = ad_tax_in.
         end.
            release ad_mstr.
 ...
 /*tfq     {gprun.i ""popomtb.p"" "(input using_supplier_consignment)"}  */
 /*tfq*/ {gprun.i ""yypopomtb.p"" "(input using_supplier_consignment)"}
      /*V8!
      if c-application-mode <> "API"
         and keyfunction(lastkey) = ""
         and not del-yn
   ...
   */
   ...
   /* LINE ITEMS */
 /*tfq  {gprun.i ""popomta.p""} */

/*tfq*/  {gprun.i ""yypopomta.p""} 
   /* TRAILER */
   hide all.
   if c-application-mode <> "API" then
   do:
 ...
 /*tfq  {gprun.i ""popomtf.p""} */

 /*tfq */  {gprun.i ""yypopomtf.p""} 
   {&POMT-P-TAG1}
