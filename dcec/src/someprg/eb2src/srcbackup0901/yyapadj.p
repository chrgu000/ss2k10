...
FOR EACH ap_mstr NO-LOCK WHERE ap_mstr.ap_vend = vend AND ap_mstr.ap_type = "vo" :

    FIND vo_mstr NO-LOCK WHERE vo_ref = ap_ref AND vo_confirmed NO-ERROR .
    IF NOT AVAILABLE vo_mstr THEN NEXT .
  
    amt1 = 0 .
    FOR EACH ckd_det NO-LOCK WHERE ckd_voucher = vo_ref USE-INDEX ckd_voucher :
        FIND ck_mstr NO-LOCK WHERE ck_ref = ckd_ref AND ck_status <> "void" NO-ERROR .
...
IF yn THEN DO TRANSACTION ON ERROR UNDO, LEAVE :
    FOR EACH CODE_mstr WHERE CODE_fldname = "ap_adjust" + vend :
        DELETE CODE_mstr .
    END.
    FOR EACH xx :
