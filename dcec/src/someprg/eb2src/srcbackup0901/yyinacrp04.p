...
for each pt_mstr no-lock where pt_prod_line >= line and pt_prod_line <= line1
    ...
    and pt_site <= site1 
/*judy 06/30/05*/  AND pt_buyer >= planer AND pt_buyer <= planer1 
    use-index pt_prod_part break by pt_prod_line:
    if first-of (pt_prod_line) then do:
       ...
       PUT "零件号" AT 1 "零件名称" AT 19 "单位" AT 45 "ABC" AT 49 "默认库位" AT 59 "保管员" AT 69 
           ...
           "销售出库" TO 229 "本期出库合计" TO 244 "期末库存" TO 259 "转移数量" TO 274 SKIP .
    end.
...
/*judy 06/30/05*/  for each ld_det no-lock where ld_part = pt_part USE-INDEX ld_part_loc:
 qty = qty + ld_qty_oh.
end.

for each tr_hist no-lock where tr_part=pt_part and tr_site=pt_site and substring(tr_type,1,3)="ISS" and tr_effdate>=date and tr_effdate<=date1:
...
for each tr_hist no-lock where tr_part=pt_part and tr_site=pt_site and substring(tr_type,1,3)="ISS" and tr_effdate>date1:
if tr_type="ISS-SO" then
...
for each tr_hist no-lock where tr_part=pt_part and tr_site=pt_site and tr_type="TAG-CNT" and tr_effdate>date1:
qty_tag_cnt2=qty_tag_cnt2 + tr_qty_chg.
end.

for each tr_hist no-lock where tr_part=pt_part and tr_site=pt_site and tr_type="TAG-CNT" and tr_effdate>=date and tr_effdate<=date1:
qty_tag_cnt=qty_tag_cnt + tr_qty_chg.
end.

for each tr_hist no-lock where tr_part=pt_part and tr_site=pt_site and tr_type="RCT-PO" and tr_effdate>=date and tr_effdate<=date1:
qty_rct_po = qty_rct_po + tr_qty_chg.
end.

for each tr_hist no-lock where tr_part=pt_part and tr_site=pt_site and tr_type="ISS-PRV" and tr_effdate>=date and tr_effdate<=date1:
qty_iss_prv = qty_iss_prv + tr_qty_chg.
end.

for each tr_hist no-lock where tr_part=pt_part and tr_site=pt_site and tr_type ="RCT-UNP" and tr_effdate>=date and tr_effdate<=date1:
qty_rct_unp = qty_rct_unp + tr_qty_chg.
end.

/*Kang Jian*/
for each tr_hist no-lock where tr_part=pt_part and tr_site=pt_site and tr_type ="RCT-WO" and tr_effdate>=date and tr_effdate<=date1:
qty_rct_wo = qty_rct_wo + tr_qty_chg.
end.

qty_receive=qty_rct_po + qty_rct_unp + qty_rct_wo + qty_iss_prv.

for each tr_hist no-lock where tr_part=pt_part and tr_site=pt_site and (TR_TYPE="ISS-PRV" OR tr_type="RCT-PO" or tr_type="RCT-UNP" or tr_type="RCT-WO") and tr_effdate>date1:
...
PUT pt_part AT 1 pt_desc2 AT 19 pt_um AT 45 in_abc AT 49 pt_loc AT 59 pt_article format "x(6)" AT 69
    ...
    qty_end format "->>>>>>>>>>>9" TO 259
    absolute(qty_iss_tr) format "->>>>>>>>>>>9" TO 274 /*WITH no-box no-labels FRAME F6 width 340*/ SKIP .
