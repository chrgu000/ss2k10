...
/* $Revision: 1.8.2.5.3.1 $   BY: Gnanasekar         DATE: 07/22/03  ECO: *P0XW*  */
/******************************************************************************/
...
/******************************************************************************/
/* Revision: eb2+sp7    BY: tao fengqin   DATE: 06/20/2005  ECO: *tfq*  cj  */
{mfdeclre.i}
{gplabel.i}
...
if (pod_qty_ord = 0 or
   not can-find(pt_mstr where pt_part = pod_part))
   and pod_type = "S"
then do:
...
if pod_req_nbr <> ""
   and can-find(last req_det where req_nbr = pod_req_nbr)
   and can-find(pt_mstr where pt_part      = pod_part)
   and blanket                             = false
...
/*cj /*tfq   {gprun.i ""popomtd1.p""}*/
 /*tfq*/   {gprun.i ""yypopomtd1.p""} */

/*cj*add*beg*/
   wolotloop:
   DO ON ERROR UNDO, LEAVE :
      {gprun.i ""popomtd1.p""}
      FIND wo_mstr NO-LOCK WHERE wo_lot = worklot NO-ERROR .
      IF NOT AVAILABLE wo_mstr THEN DO :
          MESSAGE "没有输入有效加工单！继续输入加工单吗？" VIEW-AS ALERT-BOX BUTTONS YES-NO UPDATE xxyn .
          IF xxyn THEN UNDO wolotloop, RETRY wolotloop .
      END.
      ELSE DO :
          FIND wr_route NO-LOCK WHERE wr_lot = worklot AND wr_op = routeop NO-ERROR .
          IF NOT AVAILABLE wr_route THEN DO :
              MESSAGE "没有输入有效工序！继续输入工单工序吗？" VIEW-AS ALERT-BOX BUTTONS YES-NO UPDATE xxyn .
              IF xxyn THEN UNDO wolotloop, RETRY wolotloop .
          END.
          ELSE xxyn = YES .
      END.
