...
define output parameter undotran like mfc_logical no-undo.
 /*judy  05/08/12 */  DEFINE VARIABLE xxallover AS LOGICAL INIT NO.
{pxmaint.i}
...
do on error undo, retry:
 ...
 /*judy  05/08/12 */  IF pod_pkg_code = "x"  THEN xxallover = NO.
            ELSE xxallover = YES.
 /*judy  05/08/12 */     /* {pxrun.i &PROC='checkScheduleQtyTolerance' &PROGRAM='porcxr1.p'*/
 /*judy  05/08/12 */      {pxrun.i &PROC='checkScheduleQtyTolerance' &PROGRAM='yyporcxr1-1.p'
	       &PARAM="(input divisor,
                        input qty_open,
                        input total_lotserial_qty,
 ...
 /*judy  05/08/12 */    /*if return-value <> {&SUCCESS-RESULT} then do:
 /*judy  05/08/12 */          if not batchrun then pause.
 /*judy  05/08/12 */      end. */
/*judy  05/08/12 */     IF xxallover = NO AND return-value <> {&SUCCESS-RESULT} then do:
 /*judy  05/08/12 */          undo, retry.
 /*judy  05/08/12 */      end. 
 /*judy  05/08/12 */   ELSE IF xxallover = YES AND return-value <> {&SUCCESS-RESULT} THEN DO:
 /*judy  05/08/12 */    if not batchrun then pause.
/*judy  05/08/12 */    END.

 /*judy  05/08/12 */     /*{pxrun.i &PROC='checkScheduleCostTolerance' &PROGRAM='porcxr1.p' */
 /*judy  05/08/12 */     {pxrun.i &PROC='checkScheduleCostTolerance' &PROGRAM='yyporcxr1-1.p'
