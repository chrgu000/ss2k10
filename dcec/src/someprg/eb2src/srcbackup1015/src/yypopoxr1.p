...
/* Revision: 1.22         BY: Vivek Gogte    DATE: 02/27/03   ECO: *P0N8*     */
/* Revision: 1.22.1.1     BY: Pankaj Goswami DATE: 12/24/03   ECO: *P1FV*     */
/* Revision: 1.22.1.2     BY: Pankaj Goswami DATE: 01/16/04   ECO: *P1K0*     */
/* $Revision: 1.22.1.3 $    BY: Deepak Rao     DATE: 05/26/04   ECO: *P23M*     */
/*                                                                            */
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      /* READ SUPPLIER ITEM */
      {pxrun.i &PROC='processRead' &PROGRAM='yyppsuxr.p'
                &HANDLE=ph_ppsuxr
               &PARAM="(input pItemId,
                        input pSupplierId,
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      {pxrun.i &PROC='getPriceListRequired' &PROGRAM='yypopoxr.p'
                &HANDLE=ph_popoxr
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      {pxrun.i &PROC='processRead' &PROGRAM='yypopoxr.p'
                &HANDLE=ph_popoxr
               &PARAM="(input pod_nbr,
                        buffer po_mstr,
                        input {&NO_LOCK_FLAG},
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      {pxrun.i &PROC='readPOControl' &PROGRAM='yypopoxr.p'
                &HANDLE=ph_popoxr
               &PARAM="(buffer poc_ctrl)"}
...
------------------------------------------------------------------------------*/
   ...
   /* NEW PARAMETER pSupplierItem PASSED TO GET EXACT SUPPLIER ITEM COST */
      {pxrun.i &PROC='getSupplierQuotePrice' &PROGRAM='yyppsuxr.p'
                &HANDLE=ph_ppsuxr
               &PARAM="(input pItem,
                        input pSupplier,
                        input pCurr,
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first vp_mstr
         fields(vp_part vp_vend_part vp_vend vp_curr vp_q_date vp_q_price
                vp_q_qty vp_um )
      ...
      then do:
         ...
         if avail pt_mstr and not can-find( first vp_mstr
         ...
         for each vp_mstr
            fields(vp_vend vp_vend_part vp_part vp_curr vp_q_date vp_q_price
                   vp_q_qty vp_um )
