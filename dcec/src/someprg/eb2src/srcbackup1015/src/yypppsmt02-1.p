...
/* REVISION:eb2+sp7 retrofit by taofengqin date: 06/22/05  *cj*         */
...
display site /*V8! skip(.1) */ with frame a.
...
DEFINE VARIABLE F-c1-title AS CHARACTER INITIAL "".
F-c1-title = (getFrameTitle("ITEM_PLANNING_DATA",26)).
 RECT-FRAME-LABEL:SCREEN-VALUE in frame c1 = F-c1-title.
 RECT-FRAME-LABEL:HIDDEN in frame c1 = yes.
 RECT-FRAME:HEIGHT-PIXELS in frame c1 =
  FRAME c1:HEIGHT-PIXELS - RECT-FRAME:Y in frame c1 - 2.
 RECT-FRAME:WIDTH-CHARS IN FRAME c1 = FRAME c1:WIDTH-CHARS - .5. 
/* SET EXTERNAL LABELS */
...
/*added by kevin, 2003/12/09*/
      ...
      if not available xxbomc_ctrl then do:
          message "错误: BOM控制文件不存在!" view-as alert-box error.
          pause .
          leave.
      end.
/*end added by kevin*/
...
*cj*del*end*/            
            ...
            {gplnga2n.i &file  = ""emt""
               ...
               &label = btb-type-desc}
            btb-type = "".
            /*****added by tfq, 06/22/2005*********/
                  if input site = xxbomc_code_site and input ptp_phantom then do:
                       message "根据控制文件,地点" + input site + "不允许维护虚件!" view-as alert-box error.
                       next-prompt ptp_phantom with frame c1.
                       undo,retry.
                  end.
                  ...
                  if input ptp_det.ptp_phantom = yes then do:
                      ...
                      if available ptpbuf then do:
                         message "该虚件已经存在地点 '" + ptpbuf.ptp_site + "' 的计划数据" view-as alert-box error.
...
/*cj*/       FIND pt_mstr NO-LOCK WHERE pt_part = INPUT pt_part NO-ERROR .
/*cj*/       IF AVAILABLE pt_mstr THEN DO :
/*cj*/           {gprun.i ""yyphantom.p"" "(input pt_part, input pt_phantom)"}
/*cj*/       END.

/******end added by tfq, 06/22/2006*/
         end.
      end.
