...
repeat:
   ...
   do with frame po:
      ...
      if c-application-mode <> "API" then
         ...
         for first po_mstr where po_nbr = ttPurchaseOrder.nbr no-lock:
         end.
      ...
      then do:
         ...
         if c-application-mode <> "API" then
            for first ad_mstr where ad_addr = po_vend
               no-lock:
            end.
         else
         do:
            ...
            for first ad_mstr where ad_addr = vend no-lock:
            end.
         end.
         if c-application-mode <> "API" then
            display
      ...
      if c-application-mode <> "API" then
         for first vd_mstr where vd_addr = input po_vend no-lock:
         end.
      else
      do:
         if available po_mstr then
            ...
            assign vend = "".
         assign {mfaiset.i vend ttPurchaseOrder.vend}.
         for first vd_mstr where vd_addr = vend no-lock:
         end.
      ...
      if c-application-mode <> "API" then
         for first pod_det where pod_nbr = input po_nbr
         no-lock:
         end.
      else
         for first pod_det where pod_nbr = ttPurchaseOrder.nbr
         no-lock:
         end.
      if available pod_det and pod_po_db <> global_db
      then do:

         {pxmsg.i &MSGNUM=2514 &ERRORLEVEL=3
   ...
   do transaction:
      ...
      do with frame po1 on error undo, retry:
         ...
         if not can-find(ad_mstr where ad_addr = po_bill)
         then do:
   ...
   /*tfq*/ {gprun.i ""yyrspomtb.p""
      "(input po_recid,
        input using_supplier_consignment)"}

/*tfq   {gprun.i ""rspomtb.p""
      "(input po_recid,
        input using_supplier_consignment)"} */
