...
/*Revision: Eb2 + sp7       Last modified: 08/01/2005             By: Judy Liu   */
...
/*judy 05/08/01*/  DEF VAR site1 LIKE xxtl_site1.
...
DEF VAR msg_fileerr AS CHAR FORMAT "x(60)".
def var qty_tot like xxtl_qty_pick.
/*judy 05/08/01*/ DEF VAR effdate AS DATE INIT TODAY.

def temp-table tr
     field nbr like xxtl_nbr
     field site like xxtl_site
/*judy 05/08/01*/ FIELD site1 LIKE xxtl_site1     
...
/*judy 05/08/01*/ {gpglefv.i}
/*judy 05/08/01*/ {gprunpdf.i "gpglvpl" "p"}
...
FORM /*GUI*/ 
 ...
 SKIP(.1)  /*GUI*/
/*judy 05/08/01*/     /* site colon 22
/*judy 05/08/01*/      nbr colon 22*/
    nbr COLON 22 
/*judy 05/08/01*/   effdate COLON 50
    site COLON 22
...
mainloop:
repeat:
/*judy 05/08/01*/     FOR EACH tr:
/*judy 05/08/01*/         DELETE tr.
/*judy 05/08/01*/     END.
    if msg_file = "" then msg_file = "c:\TL-transfer.txt". 
/*judy 05/08/01*/    /* update site nbr msg_file with frame a editing:*/
/*judy 05/08/01*/     UPDATE  nbr with frame a editing: 
/*judy 05/08/01*/  /*   if frame-field = "site" then do:
/*judy 05/08/01*/                 {mfnp.i si_mstr site si_site site si_site si_site}
/*judy 05/08/01*/                 if recno <> ? then do:
/*judy 05/08/01*/                     disp si_site @ site with frame a.    */
/*judy 05/08/01*/    if frame-field = "nbr" then do:
/*judy 05/08/01*/               {mfnp.i xxtl_det nbr xxtl_nbr  nbr xxtl_nbr xxtl_index}
/*judy 05/08/01*/                     if recno <> ? then do:
 /*judy 05/08/01*/                        disp xxtl_nbr @ nbr xxtl_site @ site  xxtl_effdate @ effdate
 /*judy 05/08/01*/                            xxtl_site1 @ site1 with frame a.    
 /*judy 05/08/01*/                  end.
 /*judy 05/08/01*/       end.
            else do:
                readkey.
 ...
 /*judy 05/08/01*/   site = xxtl_site.
 /*judy 05/08/01*/   site1 = xxtl_site1.
 /*judy 05/08/01*/   effdate = xxtl_effdate.
 /*judy 05/08/01*/    DISP site site1 effdate WITH FRAME a.
  /*judy 05/08/01*/ /*    if xxtl_site <> site then do:
 /*judy 05/08/01*/           message "移仓单地点与输入地点不一致,请重新输入!" view-as alert-box error.
 /*judy 05/08/01*/           next-prompt nbr with frame a.
 /*judy 05/08/01*/           undo,retry.           
 /*judy 05/08/01*/      end.*/

 /*judy 05/08/01*/   UPDATE  effdate  msg_file WITH FRAME a.
                         IF msg_file = "" THEN DO:
                                MESSAGE "错误:日志文件不能为空,请重新输入!" view-as alert-box error.
                                 NEXT-PROMPT msg_file WITH FRAME a.
                                 UNDO, RETRY.
                           END. 
                           msg_fileerr = msg_file + ".err".  /*judy*/
                      /*verify site*/ 
 ...
 /*judy 05/08/01*/    /*effective check relating to gl calendar*/
 ...
 /*judy 05/08/01*/    /*effective check relating to gl calendar*/
      ...
      find first xxtl_det where xxtl_site = site and
                                xxtl_nbr = nbr and
 /*judy 05/08/01*/   xxtl_qty_tr > 0
 /*judy 05/08/01 xxtl_status = ""*/   no-lock no-error.
 /*judy 05/08/01*/     if available xxtl_det then do: 
 /*judy 05/08/01   IF NOT available xxtl_det then do: */   
      ...
      for each xxtl_det where xxtl_nbr = nbr 
/*judy 05/08/01*/         AND xxtl_status = ""  no-lock:
...
/*F089*/   end.
 ...
 /*judy 05/08/01*/  /*find loc_mstr where loc_site = tr.site and loc_loc = tr.loc_to no-lock no-error.*/
 /*judy 05/08/01*/  find loc_mstr where loc_site = tr.site1 and loc_loc = tr.loc_to no-lock no-error. 
   if not available loc_mstr then do:
         assign tr.error = "车间库位 " + tr.site1 + "," + tr.loc_to + " 不存在".
         ok_yn = no.
         next.
   end.
   ...
   if available isd_det then do:
         assign tr.error = "车间库位 " + tr.site1 + "," + tr.loc_to + " 的状态为受限制状态".
         ok_yn = no.
         next.
   end.
            ...
            if not can-find(whl_mstr where whl_site = tr.site1
        ...
        assign tr.error = "零件 " + tr.part + " 在 " + tr.site + "," + tr.loc_fr + " 的库存不足".
         ...
         for each tr no-lock:
                disp tr.nbr label "移仓单"
                     tr.site label "移出地点"
/*judy 05/08/01*/ tr.site1 LABEL "移入地点"
                     tr.part label "零件号"
...
/*judy*/
      ...
      if ok_yn then do:
         ...
         /*message "移仓单数据校验失败,请察看日志文件!" view-as alert-box error.*/   /*judy*/
         undo,retry.
      end.
...
/*judy 05/08/01*/  define variable iss_trnbr like tr_trnbr no-undo.
/*judy 05/08/01*/  define variable rct_trnbr like tr_trnbr no-undo.
...
/*added by kevin*/
 /*judy 05/08/01*/    assign site_to = site1 
                      loc_to = tr.loc_to
                      lotser_to = ""
                      lotref_to = "".
/*end added by kevin*/
...
/*judy 05/08/01*/    """",
 /*judy 05/08/01*/   """",
  /*judy 05/08/01*/   0,
                       output glcost,
/*judy 05/08/01*/   output iss_trnbr,
/*judy 05/08/01*/   output rct_trnbr,
                       input-output assay,
                       input-output grade,
                       input-output expire)"
           }
...
/*GUI*/ if global-beam-me-up then undo, leave.
        ...
        FIND FIRST  xxtl_det where xxtl_nbr = tr.nbr and xxtl_part = tr.part 
                       and xxtl_loc_fr = tr.loc_fr and xxtl_loc_to = tr.loc_to no-error.
        if available xxtl_det then do:
  /*judy 05/08/01*/              assign xxtl_qty_tr = xxtl_qty_tr + tr.qty_pick
 /*judy 05/08/01*/                          xxtl_qty_pick = xxtl_qty_req - xxtl_qty_tr.
 /*judy 05/08/01*/               IF xxtl_qty_pick < 0 THEN xxtl_qty_pick = 0.
 /*judy 05/08/01*/               IF  xxtl_qty_tr   >=  xxtl_qty_req THEN xxtl_status =  "C".
        end.
