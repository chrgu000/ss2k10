   ...
   no-lock:
      ...
      by ar_effdate DESCENDING :
         ...
         IF base_amt <> 0 THEN DO :
             FIND FIRST gltr_hist NO-LOCK WHERE gltr_batch = ar_batch AND gltr_doc = ar_nbr USE-INDEX gltr_batch NO-ERROR .
             IF AVAILABLE gltr_hist THEN glref = gltr_ref .
             ELSE DO :
                 FIND FIRST glt_det NO-LOCK WHERE glt_batch = ar_batch AND glt_doc = ar_nbr USE-INDEX glt_batch NO-ERROR .
