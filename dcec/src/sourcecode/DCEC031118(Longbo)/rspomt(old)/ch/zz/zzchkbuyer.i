...
/*LB01*/    find first ptp_det where ptp_part = {1}scx_part 
				and ptp_site = {1}scx_shipto no-lock no-error.               
/*LB01*/    if available ptp_det then
/*LB01*/  		find first code_mstr where code_fldname = "ptp_buyer" 
									   and code_value = ptp_buyer no-lock no-error.    
/*LB01*/	else do:  /*if there is no record in itme-site(1.4.17), then check 1.4.7 */
/*LB01*/    	find first pt_mstr where pt_part = {1}scx_part use-index pt_part no-lock no-error.
/*LB01*/    	if available pt_mstr then
/*LB01*/    		find first code_mstr where code_fldname = "pt_buyer"
...
/*LB01*/	    message "通用代码ptp_buyer或pt_buyer未设置当前用户，您不能维护该零件！" VIEW-AS ALERT-BOX ERROR BUTTONS OK.
/*LB01*/        undo, retry.
			end.