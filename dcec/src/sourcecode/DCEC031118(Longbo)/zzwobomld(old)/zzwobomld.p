...
/*Last modified by: Long Bo, 11/24/2003              */
...
/*GUI*/ if global-beam-me-up then undo, leave.
	   ...
	   iZero = 0.
		...
		output to value(foutputfile).
		/* out put file header */		
		put "发动机拆装机型的加工单物料清单导入  " + string(today) + "  "+ STRING(TIME,"HH:MM:SS") at 1 format "x(60)".
		put " " at 1.
		put "加工单:" + nbr + "  ID:" + lot at 1 format "x(60)".
		put "零  件：" + part at 1 format "x(60)".
		put "导入文件:" + finputfile at 1 format "x(60)".
		put "用  户：" + global_userid at 1 format "x(60)".
		put " " at 1.
		...
		if part <> chExcelWorkbook:Worksheets(1):Cells(2,6):value then do:
			put "错误：导入文件中拆装机零件号于加工单零件不一致，导入失败！" at 1 format "x(60)".
			iError = iError + 1.
			iRow = iRow + 1.
		end.
		else do:
			...
			if not available wod_det then do:
				put "警告：被拆装机在该加工单物料单中找不到！" at 1 format "x(60)".
				iWarning = iWarning + 1.
			end.
			...
			repeat:
				...
				if available wod_det then do:
					...
					put "警告：" + wodpart + "已经存在，忽略导入。" at 1 format "x(50)".
					iwarning = iwarning + 1.
					next.
				end.
				...
				if not available pt_mstr then do:
					put "错误：" + wodpart + "不存在，忽略导入。" at 1 format "x(50)".
					iError = iError + 1.
					next.
				end.
		...
		put " " at 1.
		put "执  行：" + string(iRow - 3) at 1 format "x(60)".
		put "无差异：" + string(iZero) at 1 format "x(60)".
		put "错  误：" + string(ierror) at 1 format "x(60)".
		put "警  告：" + string(iwarning) at 1 format "x(60)".
		put "执行结束!" at 1.
