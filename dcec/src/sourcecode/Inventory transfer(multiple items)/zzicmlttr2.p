/*zzicmlttr2.p for inventory transfer by multiple lines(部品)，writed by Kevin,2003/12*/
...
def temp-table tr
     ...
     field lot_from like ld_lot format "x(8)" column-label "批/序号"          /*kevin for lot/ref*/
...
form
 ...
 SKIP(.1)  /*GUI*/
   ...
   nbr colon 18               rmks colon 44 skip(1)   
    site_from colon 18 label "调出地点"
    site_to   colon 44 label "调入地点"
   locfrom colon 18 label "调出缺省库位"
   locto colon 44 label "调入缺省库位"
    SKIP(.4)  /*GUI*/
...
/*J034*/          end.
   ...
   if locfrom <> "" then do:
       ...
       if not available loc_mstr then do:
           message "库位不存在,请重新输入!" view-as alert-box error.
...
/*GA32*/             set tr.qty_tr tr.loc_from tr.lot_from tr.ref_from tr.loc_to with frame c editing:
                     ...
                     if not available loc_mstr then do:
                              message "错误: 库位" + tr.loc_from + "在地点" + site_from + "不存在,请重新输入!".
                              undo,retry.
                     end.                     
   ...
   if available isd_det then do:
       ...
       undo, retry.   
                     ...
                     if not available loc_mstr then do:
                              message "错误: 库位" + tr.loc_to + "在地点" + site_to + "不存在,请重新输入!".
                              undo,retry.
                     end.
