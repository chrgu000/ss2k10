/*zzsssumrp.p for 零件需求预测汇总报表打印（也即采购计划单打印），writed by Kevin,2003/11*/
...
/*J034*/       end.
  ...
  if search("\\qadtemp\qadguifs\template\PO planning form template.xlt") = ? then do: 
    ...
    for each pod_det where (pod_site >= site and pod_site <= site1)
     ...
     for each xxwk no-lock break by xxwk.buyer by xxwk.vend by xxwk.nbr by xxwk.part by xxwk.site:
       ...
       if first-of(xxwk.nbr) then do:
        ...
        chExcelWorkbook = chExcelApplication:Workbooks:ADD("\\qadtemp\qadguifs\template\PO planning form template.xlt"). 
        ...
        */
        ...
        if last-of(xxwk.nbr) then do:
           ...
           /* save the new chExcel data workbook file */ 
             chExcelWorkbook:SaveAs(outpath + "\" + xxwk.buyer + "-" + xxwk.vend + "-" + xxwk.nbr + "-" + xxwk.rlse_id + ".xls",,,,,,1).
