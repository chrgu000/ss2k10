...
/* REVISION: 8.5     LAST MODIFIED: 10/17/03    BY: Kevin   */
...
/*GUI*/ if global-beam-me-up then undo, leave.
		  ...
		  assign
	       ...
	       assign
		      ...
		      cr_cc[2]   = pl_ovh_cc*/                    /*kevin*/
		      cr_proj[2] = pod_proj
		      entity[2]  = pod_entity
		      gl_amt[2]  = trqty * sct_ovh_tl.

/*added by kevin, 10/17/2003 for change overhead account and cost center to PPV's*/
            find ptp_det where ptp_site = pod_site and ptp_part = pt_part no-lock no-error.
            if available ptp_det then do:
                if ptp_pm_code = "m" then do:
                    assign cr_acct[2] = pl_ppv_acct
                           cr_cc[2] = pl_ppv_cc.
                end.
                else do:
                    assign cr_acct[2] = pl_ovh_acct
                           cr_cc[2] = pl_ovh_cc.
                end.            
            end.
            else do:
