...
def var cpart as char format "x(2)" initial "B".
def var cvend as char format "x(2)" initial "L".
def var cloc as char format "x(2)" initial "R".
def var cponbr as char format "x(2)" initial "X".
def var cpoline as char format "x(2)" initial "Z".
def var cqty as char format "x(2)" initial "AA".
...
define variable exch_rate   like exd_rate.
...
def workfile xxwk
    ...
    field ent_exch like exd_ent_rate
...
/*F089*/   end.
       ...
       if available isd_det then do:
        ...
        if base_curr <> po_curr then do:
	         ...
	         find last exd_det where ((exd_from_curr = base_curr
			                   and  exd_curr = po_curr)
			                   or ( exd_from_curr = po_curr 
			                   and  exd_curr = base_curr ))
			                   and exd_eff_date <= effdate
			                   and exd_end_date >= effdate
			                   no-lock no-error.
	         if available exd_det then do:
		            if exd_curr = po_curr then 
		                exch_rate = exd_rate. 
		            else
		                exch_rate = 1 / exd_rate.
