/*zzrwroauto.p for generate the routing based on bom automatically*/
/*Last modified by: Kevin, 11/09/2003                */
/*Last modified by: Kevin, 01/03/2004, can create new op in existing routing*/
...
/*J034*/          end.
    ...
    Do transaction:        
    for each bom_mstr where bom__chr01 = site 
       ...
       if not available pt_mstr then do:
          find pt_mstr where pt_part = bom__chr02 no-lock no-error.
          if not available pt_mstr then next.
      end.    
       
       if not pt_prod_line begins "7" then next.   
        /*end added by kevin,11/09/2003*/
        
        find first ps_mstr where ps__chr01 = bom__chr01 and ps_par = bom_parent no-lock no-error.
        if not available ps_mstr then next.

        /*verify the routing record, if existing then next*/
        /*marked by kevin, for create new op in existing routing
        find first ro_det where ro_routing = bom_parent no-lock no-error.
	 ...
	 repeat:
          ...
          find first ptp_det where ptp_site = site and ptp_part = ps_comp no-lock no-error.
          if available ptp_det and ptp_phantom = no and ps_op <> 0 then do:
                    
              find first ro_det where ro_routing = bom_parent and ro_op = ps_op no-error.
              if not available ro_det then do:
                    ...
                    find opm_mstr where opm_std_op = string(ps_op) no-lock no-error.
