...
repeat:
     ...
     update site effdate effdate1 nbr nbr1 part part1 locfr locfr1 locto locto1 
...
/*J034*/          end.   
    ...
    {mfphead.i}
     ...
     for each xxtl_det where (xxtl_site = site) and
                             ...
                             (open_only = no or (open_only and xxtl_qty_req > xxtl_qty_tr)) no-lock:
            
            find first b1 where b1.xxtl_nbr = xxtl_det.xxtl_nbr and
                                b1.xxtl_qty_tr <> 0 no-lock no-error.
            
            if (untr_only = no or (untr_only and not available b1)) then do:                   
                    qty_open = max(xxtl_det.xxtl_qty_req - xxtl_det.xxtl_qty_tr,0).
                    
                    disp xxtl_det.xxtl_nbr xxtl_det.xxtl_site xxtl_det.xxtl_effdate xxtl_det.xxtl_part
                     xxtl_det.xxtl_loc_fr xxtl_det.xxtl_loc_to
                     xxtl_det.xxtl_qty_req xxtl_det.xxtl_qty_pick xxtl_det.xxtl_qty_tr 
                        qty_open with width 132 stream-io.
                    
                  if available b1 then disp "已移仓" @ rmks label "说明".
                  else disp "未移仓" @ rmks label "说明".
            end.
