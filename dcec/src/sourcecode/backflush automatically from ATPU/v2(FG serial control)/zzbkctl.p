...
repeat :
	...
	usrw_charfld[10] validate(usrw_charfld[10] <> "", "Blank not allowed!")
       ...
       /*verify the source file directory*/
        if usrw_charfld[10] matches "*\" or usrw_charfld[10] matches "*/" then do:
            message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt usrw_charfld[10] with frame a.
              undo,retry.
        end.              
       testdir = usrw_charfld[10] + "\testdir".
       ...
       /*verify the correct file directory*/
        if usrw_charfld[11] matches "*\" or usrw_charfld[11] matches "*/" then do:
            message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt usrw_charfld[11] with frame a.
              undo,retry.
        end.        
        testdir = usrw_charfld[11] + "\testdir".
        OS-CREATE-DIR value(testdir).
        if OS-ERROR <> 0 then do:
              message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt usrw_charfld[11] with frame a.
       ...
       /*verify the error file directory*/ 
        if usrw_charfld[12] matches "*\" or usrw_charfld[12] matches "*/" then do:
            message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt usrw_charfld[12] with frame a.
              undo,retry.
        end.
        testdir = usrw_charfld[12] + "\testdir".
        OS-CREATE-DIR value(testdir).
        if OS-ERROR <> 0 then do:
              message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt usrw_charfld[12] with frame a.
       ...
       if usrw_charfld[14] matches "*\" or usrw_charfld[14] matches "*/" then do:
            message "目录不存在,请重新输入!" view-as alert-box error.
