...
def var srcdir as char format "x(40)".           /*source file directory*/
def var okdir as char format "x(40)".            /*verified file directory*/
def var errdir as char format "x(40)".    /*incorrect file directory*/
def var tempdir as char format "x(40)".   /*temp file directory*/
...
FORM /*GUI*/ 
 ...
 SKIP(.1)  /*GUI*/
 "提示: 本功能将执行自动入库回冲功能!" at 5 skip(1)
   srcdir colon 15  label "源文件目录"   format "x(60)" skip
    okdir colon 15  label "正确文件目录"   format "x(60)" skip
    errdir colon 15  label "出错文件目录"   format "x(60)" skip
    logfile colon 15  label "LOG 目录\文件"   format "x(60)" skip 
    tempdir colon 15  label "临时工作目录"   format "x(60)" skip  
          SKIP(.4)  /*GUI*/
...
repeat:
    ...
    tempdir = usrw_charfld[14].

    update srcdir validate(srcdir <> "", "不允许为空!")
           okdir validate(okdir <> "", "不允许为空!")
           errdir validate(errdir <> "", "不允许为空!")
           logfile validate(logfile <> "", "不允许为空!")
           tempdir validate(tempdir <> "", "不允许为空!")
           with frame a.

       /*verify the source file directory*/
        if srcdir matches "*\" or srcdir matches "*/" then do:
            message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt srcdir with frame a.
              undo,retry.
        end.              
       testdir = srcdir + "\testdir".
       ...
       /*verify the correct file directory*/
        if okdir matches "*\" or okdir matches "*/" then do:
            message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt okdir with frame a.
              undo,retry.
        end.        
        testdir = okdir + "\testdir".
        OS-CREATE-DIR value(testdir).
        if OS-ERROR <> 0 then do:
              message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt okdir with frame a.
       ...
       /*verify the error file directory*/ 
        if errdir matches "*\" or errdir matches "*/" then do:
            message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt errdir with frame a.
              undo,retry.
        end.
        testdir = errdir + "\testdir".
        OS-CREATE-DIR value(testdir).
        if OS-ERROR <> 0 then do:
              message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt errdir with frame a.
       ...
       if tempdir matches "*\" or tempdir matches "*/" then do:
            message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt tempdir with frame a.
              undo,retry.
        end.       
        testdir = tempdir + "\testdir".
        OS-CREATE-DIR value(testdir).
        if OS-ERROR <> 0 then do:
              message "目录不存在,请重新输入!" view-as alert-box error.
              next-prompt tempdir with frame a.
    ...
    exec_yn = yes.
    {mfmsg01.i 12 1 exec_yn}
    if exec_yn = no then undo,retry.
    
            
    bkflh_file = tempdir + "\bkflh_file.in".
...
for each list where list.filename <> "" no-lock:
      ...
      ok_yn = yes.
      put skip(1).
      put unformatted "Now process file: " + list.filename at 1.             /*for log file*/
      ...
      if ok_yn = no then do:
       ...
       /*verify every field before cimload*/
        find emp_mstr where emp_addr = xxwk.emp no-lock no-error.
       if not available emp_mstr then do:
              ...
              ok_yn = no.
               ...
               /*verify the site*/
               find si_mstr where si_site = xxwk.site no-lock no-error.
...
/*J04T*/       end.
        ...
        find pt_mstr where pt_part = xxwk.par no-lock no-error.
...
/*G1ZV*/              and (isd_tr_type = "ADD-RE" or isd_tr_type = "RCT-WO")) then do:
...
/*G1Z7*/       end.
         ...
         if xxwk.routing = "" then 
            find first ro_det where ro_routing = xxwk.par 
                                and (ro_start = ? or ro_start  <= xxwk.effdate)
                                and (ro_end = ? or ro_end    >= xxwk.effdate)no-lock no-error.
         else find first ro_det where ro_routing = xxwk.routing 
                                and (ro_start = ? or ro_start  <= xxwk.effdate)
                                and (ro_end = ? or ro_end    >= xxwk.effdate)no-lock no-error.
     ...
     if not available ro_det then do:
      ...
      /*verify the routing code and bom code*/
         find ptp_det no-lock where ptp_part = xxwk.par
    ...
    if cumwo_lot <> ? then do:
      ...
      if cumwo_lot <> ? then do:
        find first wr_route where wr_lot = cumwo_lot and wr_op = xxwk.lastop no-lock no-error.
        if not available wr_route then do:
            put unformatted "错误: 输入的最后一道工序在加工单工艺流程中不存在" at 5.
            ok_yn = no.
        end.
        
        find last wr_route where wr_lot = cumwo_lot no-lock no-error.
        if available wr_route then do:
         ...
         if xxwk.routing = "" then
             find last ro_det where ro_routing = xxwk.par no-lock no-error.
         else find last ro_det where ro_routing = xxwk.routing no-lock no-error.
         ...
         find wc_mstr where wc_wkctr = xxwk.wkctr and wc_mch = xxwk.mch no-lock no-error.
       ...
       /*          　　
         ...
         find dpt_mstr where dpt_dept = xxwk.dept no-lock no-error.
      ...
      */   
      ...
      find first xxwk no-lock no-error.
       repeat:
           if available xxwk then do:
              ...
              find pt_mstr where pt_part = xxwk.comp no-lock no-error.
              if not available pt_mstr then do:
              ...
              if cumwo_lot <> ? then do:
                  find first wr_route where wr_lot = cumwo_lot and wr_op = xxwk.compop no-lock no-error.
                  if not available wr_route then do:
                  ...
                  if xxwk.routing > "" then
                        find ro_det where ro_routing = xxwk.routing and
                                          ro_op      = xxwk.compop and
                                         (ro_start = ? or ro_start  <= xxwk.effdate) and
                                         (ro_end = ? or ro_end    >= xxwk.effdate)
                                         no-lock no-error.
                  else
                        find ro_det where ro_routing = xxwk.par and
                                          ro_op      = xxwk.compop and
                                          (ro_start = ? or ro_start  <= xxwk.effdate) and
                                          (ro_end = ? or ro_end    >= xxwk.effdate)
                                          no-lock no-error.
              ...
              find loc_mstr where loc_site = xxwk.site and loc_loc = xxwk.comploc no-lock no-error.
              ...
              find isd_det no-lock where isd_tr_type = "iss-wo"
                                      and isd_status =
      ...
      repeat:
           if available xxwk then do:                  
                  ...
                  find pt_mstr where pt_part = xxwk.par2 no-lock no-error.
                  if not available pt_mstr then do:
                  ...
                  more than zero*/
                  find first ld_det where ld_site = xxwk.site and
                  ...
                  find loc_mstr where loc_site = xxwk.site and loc_loc = xxwk.parloc no-lock no-error.
                  if not available loc_mstr then do:
                         put unformatted "发动机所对应的库位 " + xxwk.site + "," + xxwk.parloc + " 不存在" at 5.
                             ok_yn = no.
                     end.
              
                     find isd_det no-lock where isd_tr_type = "rct-wo"
                                             and isd_status =
                                            (if available loc_mstr and loc_status <> "" then loc_status
                                            else si_status) no-error.
                     if available isd_det then do:              
...
*/
     ...
     /*create the detail components issue format*/ 
      /*put stream bkflh "-" at 1.*/                             /*kevin,2004/01/06*/
      ...
      /*create the final goods receipts format*/
       ...
       put stream bkflh "Y" at 1.
       put stream bkflh "Y" at 1.
       if count <> 1 then                /*kevin,2004/01/07*/
          put stream bkflh "Y" at 1.
       put stream bkflh "." at 1.
