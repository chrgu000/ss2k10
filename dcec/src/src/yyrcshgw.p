...
DO TRANSACTION :
   ...
   repeat:
      ...
      if ship_parent <> "" then do:
         ...
         find ship_record
            where ship_record.abs_shipfrom = ship_from
      ...
      then do transaction:
         ...
         if ship_type = "S" then do:
            find si_mstr where si_site = ship_from no-lock no-error.
            ...
            if not can-find(ad_mstr where ad_addr = ship_from)
            then do:
            ...
            find first sg_mstr no-lock where sg_grp = v_shipgrp
               no-error.
         ...
         if ship_type = "R" then do:
            ...
            find ad_mstr where ad_addr = ship_from no-lock no-error.
         ...
         if ship_id = "" then do:
            ...
            if ship_type = "S" then do:
               find ad_mstr
                  where ad_addr = ship_to no-lock no-error.
            ...
            if ship_type = "R" then do:
               find si_mstr where si_site = ship_to
                  no-lock no-error.
