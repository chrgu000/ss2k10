...
do:
   ...
   if ship_site <> "" then do:
      ...
      if v_inv_mov <> "" and ship_from <> ship_site then do:
         ...
         if can-find(sg_mstr where sg_grp = v_shipgrp and sg_auto_tr = false)
         then do:
   ...
   if ship_loc <> "" then do:
      ...
      find loc_mstr where loc_site = ship_site
...
/*james*/ ASSIGN ABS_mstr.ABS_user1 = ship_via.
      ...
      repeat while available ship_record:
         ...
         if ship_record.abs_par_id <> "" then do:
            ...
            find ship_record
               where ship_record.abs_shipfrom = abs_mstr.abs_shipfrom
