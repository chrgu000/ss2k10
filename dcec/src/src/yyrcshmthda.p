...
/*GUI*/ if global-beam-me-up then undo, leave.
 /* FOR FIRST abs_mstr */
      ...
      then do:
         ...
         /* VALIDATE SHIPFROM */
         if not can-find (si_mstr where si_site = input abs_shipfrom)
         then do:
            /* SITE DOES NOT EXIST */
         ...
         /* VALIDATE SHIPTO */
         if not can-find(ad_mstr where ad_addr = input abs_shipto)
         then do:
         ...
         if i_tr_type = "ISS-WO" and
            {&ICSHMT-P-TAG12}
            not can-find (vd_mstr where vd_addr = input abs_shipto)
         then do:
         ...
         then do:
            ...
            for first ls_mstr
               ...
               where ls_addr = input abs_shipto
...
/*GUI*/ if global-beam-me-up then undo, leave.
      ...
      /* VAlIDATE NON-BLANK INVENTORY MOVEMENT CODE */
         ...
         if not can-find
            (im_mstr where im_inv_mov = input v_invmov)
         then do:
...
PROCEDURE ip_refresh_id:
   ...
   for first b_abs_mstr
      fields (abs_canceled abs_cons_ship abs_eff_date abs_format abs_id
              abs_inv_mov abs_lang abs_nr_id abs_qty abs_shipfrom abs_shipto
              abs_shp_date abs_status abs_type abs__qad01)
      where abs_shipfrom = input frame a abs_mstr.abs_shipfrom
...
PROCEDURE ip_refresh_shipfrom:
   ...
   for first si_mstr
      ...
      where si_site = input frame a abs_mstr.abs_shipfrom
      no-lock:
   end. /* FOR FIRST si_mstr */

   for first ad_mstr
      fields (ad_addr ad_line1 ad_name ad_ref)
      where ad_addr = input frame a abs_mstr.abs_shipfrom
      no-lock:
   end. /* FOR FIRST ad_mstr */
...
/*GUI*/ if global-beam-me-up then undo, leave.
   ...
   for first b_abs_mstr
      fields (abs_canceled abs_cons_ship abs_eff_date abs_format abs_id
              abs_inv_mov abs_lang abs_nr_id abs_qty abs_shipfrom abs_shipto
              abs_shp_date abs_status abs_type abs__qad01)
      where b_abs_mstr.abs_shipfrom = input frame a abs_mstr.abs_shipfrom
   ...
   then do:
      for first sgid_det
         where sgid_grp     = input frame a sg_grp
...
PROCEDURE ip_refresh_shipto:
   ...
   for first ad_mstr
      fields (ad_addr ad_line1 ad_name ad_ref)
      where ad_addr = input frame a abs_mstr.abs_shipto
      no-lock:
   end. /* FOR FIRST ad_mstr */
...
PROCEDURE ip_refresh_invmov:
   ...
   for first im_mstr
      ...
      where im_inv_mov = input frame a v_invmov
      no-lock:
