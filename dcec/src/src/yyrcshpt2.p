...
PROCEDURE xxpro-input:
    ...
    WITH FRAME a EDITING :
        if frame-field = "v_shipper" then do:
            {mfnp05.i ABS_mstr abs_id "abs_id begins 'S'and abs_type = 's'" abs_id "'s' + input v_shipper"}
        END.
        ELSE DO:
            status input.
            readkey.
...
PROCEDURE xxpro-build:
    ...
    find first ABS_mstr WHERE ABS_shipfrom = v_ship_fr 
    ...
    ASSIGN
        ...
        .
    FIND FIRST CODE_mstr WHERE code_fldname = "xx-mot" AND CODE_value = xxwk1_mot NO-LOCK NO-ERROR.
    IF AVAILABLE CODE_mstr THEN xxwk1_mot = CODE_cmmt.
    /*mot convert*/

    /*one level parts*/
    FOR EACH ABS_mstr WHERE ABS_mstr.abs_shipfrom = v_ship_fr 
    ...
    FOR EACH bbabs_mstr 
        WHERE bbABS_mstr.abs_shipfrom = v_ship_fr 
...
PROCEDURE xxpro-post-process:
    ...
    FOR EACH Xxwk1:
        FIND FIRST ad_mstr WHERE ad_addr = xxwk1_buyer[1] NO-LOCK NO-ERROR.
        ...
        END.
        FIND FIRST ad_mstr WHERE ad_addr = xxwk1_billto[1] NO-LOCK NO-ERROR.
        IF AVAILABLE ad_mstr THEN DO:
            ASSIGN
        ...
        END.
        FIND FIRST ad_mstr WHERE ad_addr = xxwk1_seller[1] NO-LOCK NO-ERROR.
        IF AVAILABLE ad_mstr THEN DO:
            ASSIGN
        ...
        IF AVAILABLE xxwk2 THEN DO:
            FIND FIRST so_mstr WHERE so_nbr = xxwk2_order NO-LOCK NO-ERROR.
        ...
        FOR EACH xxwk2 WHERE xxwk2_inv_nbr = xxwk1_inv_nbr:
            ...
            END.
            FIND FIRST sod_det WHERE sod_nbr = xxwk2_order AND sod_line = xxwk2_mfg_line NO-LOCK NO-ERROR.
...
PROCEDURE xxpro-excel:
    ...
    FOR EACH xxwk1 BREAK BY xxwk1_inv_nbr:
        ...
        /*pic*/
        v_pic_name = "".
        FIND FIRST CODE_mstr WHERE CODE_fldname = "xx-image-list"
