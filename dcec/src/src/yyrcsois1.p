/* GUI CONVERTED from rcsois1.p (converter v1.77) Mon Sep  8 02:27:16 2003 */
...
FORM /*GUI*/ 
 ...
 RECT-FRAME-LABEL AT ROW 1 COLUMN 3 NO-LABEL VIEW-AS TEXT SIZE-PIXELS 1 BY 1
 SKIP(.1)  /*GUI*/
abs_veh_ref colon 35 label "Vehicle ID"
 ...
 /* do with frame a */
      ...
      for first abs_mstr
         fields (abs_canceled abs_eff_date abs_format abs_id
                 ...
                 abs_shipfrom abs_shipto abs_shp_date abs_status
                 abs_type abs__qad01
                 /*james*/ ABS_master_id ABS__chr10
                 )
...
/*james*/ 
IF /*ABS_master_id = ""*/ SUBSTRING(ABS_status,11,1) <> "Y" THEN DO:
    MESSAGE "未通过计划核准,不能确认发货." VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
    UNDO, RETRY.
END.
...
/*GUI*/ if global-beam-me-up then undo, leave.
      ...
      on endkey undo CK-CC-HOLD, leave CK-CC-HOLD:
         ...
         then do:
            ...
            /* ADDED so_quote IN THE FIELDLIST */
            for first so_mstr
...
/*GUI*/ if global-beam-me-up then undo, leave.
      ...
      /*james*/ IF (USE_shipper <> YES) OR (auto_post <> YES) THEN DO:
                    MESSAGE "请使用发货单号作为发票号并同时过帐...".
