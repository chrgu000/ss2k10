...
PROCEDURE xxpro-chk-price:
    ...
    ELSE DO:
        ...
        FIND FIRST pc_mstr WHERE pc_list = pod_pr_list 
...
PROCEDURE xxpro-create-release:
    ...
    FIND FIRST pod_det WHERE pod_nbr = p_ponbr AND pod_line = p_poln AND pod_sched = YES NO-LOCK NO-ERROR.
    IF NOT AVAILABLE pod_det THEN LEAVE.

    IF p_rls_id = pod_curr_rlse_id[1]  THEN LEAVE.
    
    FIND first scx_ref where  scx_type  = 2 and scx_order = pod_nbr and scx_line  = pod_line NO-LOCK NO-ERROR.
    ...
    FIND FIRST pod_det WHERE pod_nbr = p_ponbr AND pod_line = p_poln AND pod_sched = YES NO-ERROR.
...
PROCEDURE xxpro-check:
    ...
    END.
    FOR EACH xxwk3:
        FIND FIRST pod_det WHERE pod_nbr = xxwk3_ponbr AND pod_part = xxwk3_part
            AND pod_sched = YES 
            NO-LOCK NO-ERROR.
        ...
        find first scx_ref no-lock where scx_type = 2 AND scx_po = xxwk3_ponbr and scx_line = xxwk3_poln no-error.
