...
PROCEDURE xxpro-input:
    ...
    WITH FRAME a EDITING :
        ...
        if frame-field = "v_ship_fr" then do:
            {mfnp05.i vd_mstr vd_addr "yes = yes" vd_addr "INPUT v_ship_fr"}
            if recno <> ? then DO:
                DISPLAY vd_addr @ v_ship_fr WITH FRAME a.
                FIND FIRST ad_mstr WHERE ad_addr = vd_addr NO-LOCK NO-ERROR.
                IF AVAILABLE ad_mstr THEN DISPLAY ad_name @ v_description[1] WITH FRAME a.
            END.
        END.
        ELSE if frame-field = "v_ship_to" then do:
            {mfnp05.i si_mstr si_site "yes = yes" si_site "INPUT v_ship_to"}
            if recno <> ? then DO:
               DISPLAY si_site  @ v_ship_to WITH FRAME a.
               FIND FIRST ad_mstr WHERE ad_addr = si_site NO-LOCK NO-ERROR.
...
PROCEDURE xxpro-build:
    ...
    FOR EACH po_mstr 
        WHERE (po_vend = v_ship_fr OR v_ship_fr = "")
        AND (po_sched = YES)
