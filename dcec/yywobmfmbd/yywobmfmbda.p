...
PROCEDURE xxpro-bud-bom:
    ...
    /* 镜像明细 Added by James Duan */
        ...
        /* get cost set */
	find first in_mstr where in_part = p_part and in_site = p_site no-lock no-error.
	v_costset = (if available in_mstr and in_gl_set <> "" then in_gl_set else "STANDARD").

	for each spt_det no-lock where spt_site = p_site
		and spt_sim = v_costset
		and spt_part = p_part
		by spt_element:
		/* 生成对应版本镜像BOM成本元素明细 */
		find first yywobmspt_mstr where yywobmspt_site = p_site
	...
	for each yywobmsptd_det where yywobmsptd_site    = p_site
	...
	/* 加入镜像明细 Added by James Duan */
		for each spt_det no-lock where spt_site = p_site
			...
			and spt_part = pkpart
			by spt_element:
			find first yywobmsptd_det where yywobmsptd_site    = p_site 
				and yywobmsptd_part = p_part 
