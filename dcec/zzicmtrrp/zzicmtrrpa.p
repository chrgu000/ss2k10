...
/*GL93*/ width 132 attr-space.
		...
		for each lad_det where lad_domain = global_domain
		     and lad_dataset = "itm_det" and lad_nbr = tmpnbr 
		     and lad_qty_all <> 0 break by integer(lad_line) : /*8/9/2004 1:39PM*/
			find pt_mstr no-lock where pt_domain = global_domain and pt_part = lad_part no-error.
			find first in_mstr no-lock where in_domain = global_domain and in_site = site_from and in_part = lad_part no-error.
