...
/*M0P3*/ batchdelete = "".
         ...
         prompt-for pc_list pc_curr pc_prod_line pc_part
         pc_um pc_start
...
/*M0P3*/ batchdelete no-label when (batchrun)
        editing:
           ...
           if recno <> ? then do:
               ...
               display pc_list pc_curr pc_prod_line pc_part
                       pc_um pc_start pc_expire pc_amt_type
                       pc_user1 with frame a.
...
/*L020*/          {mfmsg.i mc-error-number 3}  /* INVALID CURRENCY CODE */
        ...
        do on error undo, retry with frame a no-validate:
           ...
           update pc_expire pc_amt_type pc_user1 go-on ("F5" "CTRL-D").
...
/*Y715*/    if pc_user1 <> "" then do:
/*Y715*/       find first si_mstr no-lock where si_site = pc_user1 no-error.
/*Y715*/       if not available si_mstr then do:
/*Y715*/          {mfmsg.i 708 3}   /*  Site does not exist  */
/*Y715*/          next-prompt pc_user1 with frame a.
/*Y715*/          undo, retry.
/*Y715*/       end.
/*Y715*/    end.
