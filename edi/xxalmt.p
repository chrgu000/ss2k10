...
{mfdtitle.i "110627.1"}
 ...
 define variable code           like al_code  no-undo.
 define variable code1          like al_code  no-undo.
 define variable domain         like ad_domain  extent 6 no-undo.
 define variable i as integer.

 define buffer b_al_mstr for al_mstr.
...
i = 1.
   ...
   do on error undo, retry with frame a:
    ...
    for each al_mstr where al_domain = global_domain 
                       and al_code >= code
		       and al_code <= code1
		       no-lock:
       i = 1.
       repeat i = 1 to 6:
          if domain[i] <> "" then do:
             find first dom_mstr  no-lock where dom_domain = domain[i] no-error.      
                ...
                buffer-copy al_mstr                                                 
	      	except al_domain oid_al_mstr 
	      	to b_al_mstr                                                    
	      	assign                           
	               b_al_mstr.al_domain     = domain[i]                      
		       .
