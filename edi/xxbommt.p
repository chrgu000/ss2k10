...
{mfdtitle.i "110627.1"}
 ...
 define variable part1          like ps_par  no-undo.
 define variable domain         like ad_domain  extent 6 no-undo.
 define variable i as integer.

 define buffer b_ps_mstr for ps_mstr.
 define buffer b_bom_mstr for bom_mstr.

/* DISPLAY SELECTION FORM */
form
	parent			colon 15
	part1           	colon 40 label {t001.i} skip(1)
	 ...
	 domain[6]
      ...
      editing:
         ...
         then do:
            ...
            {mfnp.i ps_mstr parent  " ps_mstr.ps_domain = global_domain and
            ps_par "  parent ps_par ps_par}
         end.
         else do:
            status input.
            readkey.
       ...
       repeat i = 1 to 6:
          if domain[i] <> "" then do:
             find first dom_mstr  no-lock where dom_domain = domain[i] no-error.      
                ...
                buffer-copy ps_mstr                                                 
	      	except ps_domain oid_ps_mstr 
	      	to b_ps_mstr                                                    
	      	assign                           
	               b_ps_mstr.ps_domain     = domain[i]                      
		       .

		 find first bom_mstr where bom_domain = global_domain and bom_parent = ps_par no-lock no-error.
		 if available bom_mstr then do:
		     buffer-copy bom_mstr                                       
		     except bom_domain oid_bom_mstr                              
		     to b_bom_mstr                                              
		     assign                                                    
		            b_bom_mstr.bom_domain     = domain[i]      
			    .
		 end.
