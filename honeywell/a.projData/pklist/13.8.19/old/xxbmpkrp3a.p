...
/*K10Y*/    end. /* if c-application-mode */
	...
	/* *SS-20130925.1 */	put UNFORMATTED  "PICKING LIST: " v_number skip.
...
{gprun.i ""xxpklnew.p"" "( input v_number, 
     ...
     if v_output <> "" then do:
        message "保存领料单" v_number " " ii "失败! " v_output.
        message v_output.
     end.
	...
	/* *SS-20130925.1  -e */
				...
				if not newpage then
					if ( page-size - line-counter < 2 or last-of ( pp_loc ))
						and not last ( pp_loc ) then do:
						page.
/* *SS-20130925.1  -b */


 ii = 0.
 put UNFORMATTED  "PICKING LIST: " v_number skip. 
 end. /* and not last ( pp_loc ) then do: */
/* *SS-20130925.1  -e */
					else 
						do:
