...
/*GN61*/ /* *SS-20130925.1  {mfdtitle.i "f+ "}  */
/* *SS-20130925.1 */ {mfdtitle.i "131114.1"}

/* *SS-20130925.1 -b */ 

 define variable v_number as character format "x(20)" label "PICKING LIST".
 define variable errorst as logical.
 define variable errornum as integer.
 define variable xx as character initial "XXPKLNBR".


 {gprun.i ""gpnrmgv.p"" "('XXPKLNBR',input-output v_number, output errorst ,output errornum)" }

if errorst = yes then do:
   message  "序号XXPKLNBR设置错误!"  VIEW-AS ALERT-BOX  .
   LEAVE .
end.
 define variable ii as int format ">99" initial 0 .
 define variable 	v_output as char .	
/* *SS-20130925.1 -e */
...
/*GN61*/       if available si_mstr then display si_desc with frame a.
	Case type:
		...
		for each ckseq_mstr where ckseq_due_date >= duedate and
										  ...
										  ckseq_part >= part1 and ckseq_part <= part2 and
										  ckseq_line >= line and ckseq_line <= line1 and
/*mich*/								  ( if index("Ss", type) <> 0
										    then ckseq_site = "SMT" 
											 else ckseq_site <> "SMT" ) no-lock: 
		...
		for 
			 ...
			 each tblpp where pp_part = tmpld_part and pp_qty <> 0  
				...
				((( pp_promo = "stk" or pp_promo = "vmi" ) and type <> "p" ) or
				 ( pp_promo = "pou" and type = "p" ) or type = "u") and pp_iss_pol
				use-index pp_part 
			 	break by pp_line by tmpld_locator by pp_part with frame b: 
			if pp_qty = 0 or 
				( pp_promo <> "stk" and pp_promo <> "vmi" and 
					index("PU",type) = 0 ) or
				( pp_promo <> "pou" and type = "P" ) or 
				not pp_iss_pol then next.
			if first-of ( pp_line) and type <> "P"  then 
			do:
      		put skip(1). 
				put "Line: " + pp_line  format "x(40)" skip. 
				/*
            	...
            	*/
				...
				do:
      			...
      			do:
					...
					put fill("-", 129) format "x(129)" skip.
				{mfrpexit.i "false"}
			end. /*for each tblpp*/
		...
		for each tblld no-lock, 
			...
			first sct_det where sct_part = pp_part and sct_site = site and
				sct_sim = "standard" no-lock
			 	break by pp_line by pp_parent by tmpld_locator by pp_part 
				with frame c:
			if pp_qty = 0 or 
...
/* *SS-20130925.1  -b */	put " PICKING LIST: " + v_number   format "x(40)" skip.
			end.
...
{gprun.i ""xxpklnew.p"" "( input v_number  ,      
			   input 2,
			   input today,
			   input pp_vsm,   
			   input pp_line,
