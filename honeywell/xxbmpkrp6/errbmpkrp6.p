...
/*GN*/ define variable xx as character initial "XXPKLNBR".
       define variable itype as integer initial 20.
       define variable newpk like mfc_logical initial no no-undo.
       define variable totqty like pk_qty no-undo.
...
/*H100*/    field pkop as integer format ">>>>>9"
...
/*12*/  find first xxpklm_mstr no-lock where xxpklm_type = itype and xxpklm_date = eff_date
...
/*12*/  end.
  ...
  if m_count > m_error then do:
    ...
    for each tblln no-lock use-index pln_line,
      ...
      do:
          put skip(1).
        put "Line: " + pp_line  format "x(40)" "PkLISTNUMBER:" at 56 v_Number skip.
        /*
      ...
      do:
      assign totqty = ( accumu total by pln_part pp_qty ).
      if newpk then do:
           assign ii = ii + 1.
           {gprun.i ""xxpklnew.p"" "(
                    input v_number,
        ...
        for each tblln1 where tblln1.pln_part = xxpkld_part
            and tblln1.pln_line = xxpkld_loc_from
            and tblln.pln_loc <> tblln1.pln_loc
            and tblln.pln_vsm = tblln1.pln_vsm no-lock,
          each tblld1 where tmpld_part = xxpkld_part
