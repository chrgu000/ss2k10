/* xxpkltr2.p - from assembline Return to Loc                                */
...
repeat with frame a:
  ...
  if  trim(xxpklm_status) <> "I" then do:
      message "领料单状态 " trim(xxpklm_status) ", 不能退仓!" .
      undo,retry  mainloop.
  end.
   clear frame c.
   hide frame c.
   ...
   do on error undo,retry:
    ...
    for each xxpkld_det where xxpkld_nbr = pklnbr and xxpkld__chr01 = "I" no-lock by xxpkld_line:
       ...
       for each ld_det no-lock use-index ld_part_lot where
            ...
            if vqty >= min(vqty,ld_qty_oh,xxpkld_qty_iss - xxpkld_qty_ret)
                   then assign vqty = vqty - min(vqty,ld_qty_oh,xxpkld_qty_iss - xxpkld_qty_ret).
                   else assign vqty = 0.
       end. /* for each ld_det */
