...
for each tt1 exclusive-lock where tt1_sel = "*" and tt1_qty1 > 0:
    ...
    put stream bf unformat trim(string(tt1_qty1)) ' - - "' tt1_site '" "'.
        put stream bf unformat tt1_loc_from '" "' tt1_lot '" "' tt1_ref '"' skip.
    put stream bf unformat '"' tt1_pkl '" - "' rknbr '" - - - "'.
        put stream bf unformat acct '" "' cc '"'  skip.
    ...
    FIND FIRST tr_hist NO-LOCK WHERE tr_trnbr > integer(trrecid)
     ...
     IF AVAILABLE tr_hist THEN DO:
         ASSIGN tt1_chk = "ok".
     /* tt1_chk = "ISS-TR:[" + trim(string(tr_trnbr,">>>>>>>>>>>>>9")) + "]" */
         assign yn = yes.
