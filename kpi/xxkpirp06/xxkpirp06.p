...
define variable summary  like mfc_logical format "1)Vend/2)Summary".
...
define temp-table tmpkpi06
       ...
       index k06_addr is primary k06_addr.
...
repeat:
   ...
   if nbr1 = hi_char then nbr1 = "".
    ...
    do on error undo,retry:
      ...
      update vend vend1 nbr nbr1 per per1 summary with frame a.
/*    if per = "" then do:                                                   */
/*       {mfmsg.i 495 3}                                                     */
...
/*    end.                                                                   */
   ...
   {wbrp06.i &command = update
             &fields = "vend vend1 nbr nbr1 per per1 summary" &frm = "a"}
...
display vend vend1 nbr nbr1 per per1 summary with frame a.
do i = 1 to 24:
...
do i = 1 to maxi:
   ...
   for each tr_hist no-lock use-index tr_type where tr_domain = global_domain
        and tr_type = "RCT-PO" and tr_effdate >= datef
       ...
       if last-of(tr_line) then do:
          ...
          find first pod_det no-lock where pod_domain = global_domain and
                     pod_nbr = tr_nbr and pod_line = tr_line no-error.
          if available pod_det then do:
             if tr_effdate <= pod_due_date then do:
       ...
       if last-of(tr_addr) then do:
          find first tmpkpi06 exclusive-lock where k06_addr = tr_addr no-error.
          if not available tmpkpi06 then do:
             create tmpkpi06.
             assign k06_addr = tr_addr
   ...
   for each pod_det no-lock where pod_domain = global_domain
        and pod_due_date >= datef and pod_due_date <= v_datet[i]
        and pod_qty_rcvd = 0:
        assign v_conf[i] = v_conf[i] + 1.
   end.
end.
...
if summary then do:
   put unformat getTermLabel("SUPPLIER",16) format "x(16)" skip.
