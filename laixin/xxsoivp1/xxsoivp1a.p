...
/* $Revision: 1.106.2.1 $    BY: Dayanand Jethwa       DATE: 02/24/05 ECO: *P27M* */
/* $Revision: 1.98.1.7 $          BY: Bill Jiang      DATE: 05/24/06  ECO: *SS - 20060524.1* */
/* $Revision: 1.98.1.7 $          BY: Bill Jiang      DATE: 04/07/06  ECO: *SS - 20060524.2* */
/*-Revision end---------------------------------------------------------------*/
...
/* SS - 20060524.1 - B */
DEFINE VARIABLE DEL_so AS LOGICAL.
/* SS - 20060524.1 - E */

/*V8:ConvertMode=Maintenance                                                  */

{mfdeclre.i}
{cxcustom.i "SOIVPSTA.P"}
{&SOIVPSTA-P-TAG1}
...
define shared variable tot_inv_comm   as decimal format "->>,>>9.99"
                                         extent 4 no-undo.
define shared variable tot_line_comm  as decimal format "->>,>>>,>>9.999999"
                                         extent 4.
...
define new shared temp-table so_shipper_info no-undo
   ...
   index tt_ship_id tt_nbr tt_line tt_inv_nbr.
...
do transaction on error undo, leave:
   ...
   if not available soc_ctrl then
       do: create soc_ctrl. soc_ctrl.soc_domain = global_domain. end.
   ...
   */
   ...
   if so_fsm_type = "RMA" and deleterma then do:
      {gprun.i ""fsivrhd.p""
         "(input so_recno,
           input ""RMA"",
           output restock-pct)"}
   ...
   if so_app_owner > "" then do:
      ...
      ExtInvoicing = can-find (first lgs_mstr
          where lgs_mstr.lgs_domain = global_domain and  lgs_app_id   =
          so_app_owner
   ...
   for each sod_det
         fields( sod_domain sod_cmtindx sod_fa_nbr sod_fsm_type sod_line
                 sod_lot sod_nbr sod_part sod_price sod_qty_all sod_qty_inv
                 sod_qty_ord sod_qty_pick sod_qty_ship sod_sched sod_site
                 sod_status sod_taxable sod_taxc sod_tax_in sod_type
                 sod_qty_ivcd sod_cum_qty sod_btb_type sod_consignment sod_um
                 sod_um_conv)
          where sod_det.sod_domain = global_domain and (  sod_nbr = so_nbr
         and (sod_qty_inv <> 0
      ...
      if sod_qty_inv <> 0
         and can-find (iec_ctrl  where iec_ctrl.iec_domain = global_domain and
         iec_use_instat = yes)
      then do:
      ...
      for each ecx_ref no-lock
          where ecx_ref.ecx_domain = global_domain and  ecx_nbr = sod_nbr
         and ecx_line = string(sod_line)
         and ecx_order_type = "so":
         create ecxref.
         assign orderType = ecx_order_type
                orderNbr  = ecx_nbr
 ...
 /* SS - 20060524.1 - B */
   ...
   if (can-find(first lgs_mstr
        where lgs_mstr.lgs_domain = global_domain and  lgs_app_id =
        so_app_owner)) and
      (can-find(first esp_mstr
        where esp_mstr.esp_domain = global_domain and  esp_app_id = so_app_owner
       and esp_doc_typ = "LOAD_PLINVOICE"
       and esp_publ_flg))
   then
      {gprun.i ""lgplivex.p"" "(input so_inv_nbr,
                                table ecxref,
                                input prepaid_amt)"}
   ...
   for each sor_mstr
       where sor_mstr.sor_domain = global_domain and  sor_nbr = so_nbr
   no-lock:
   ...
   for last sod_det
      ...
      sod_nbr
              sod_part sod_price sod_qty_all sod_qty_inv sod_qty_ord
   ...
   if not available sod_det or so_fsm_type = "RMA" and deleterma
   then do:
      ...
      for each sod_det
         ...
         for each sob_det
             where sob_det.sob_domain = global_domain and  sob_nbr  = so_nbr
              and sob_line = sod_line
         exclusive-lock:
            run del-cmt-det
               (input sob_cmtindx).
      ...
      if so_fsm_type <> "RMA"  or deleterma then do:
         ...
         for each ie_mstr exclusive-lock  where ie_mstr.ie_domain =
         global_domain and
               ie_type = {&IE_TYPE_SO} and ie_nbr = so_nbr:
            delete ie_mstr.
         end.
...
PROCEDURE create-ar-distribution:
   ...
   for first gl_ctrl
    ...
    where gl_ctrl.gl_domain = global_domain no-lock: end.
   ...
   if so_trl1_amt <> 0 then do:
      ...
      for first trl_mstr
         ...
         trl_taxc)
          where trl_mstr.trl_domain = global_domain and  trl_code = so_trl1_cd
      no-lock: end.

      find first ard_det
          where ard_det.ard_domain = global_domain and  ard_nbr  = so_inv_nbr
   ...
   {gprun.i ""txmkard.p"" "(input so_inv_nbr,
                            input so_nbr,
                            input '16'  /* TRXN TYPE */,
                            input ''    /* ACCOUNT */,
                            input ''    /* SUB ACCOUNT */,
   ...
   so_inv_nbr exclusive-lock no-error.
   {&SOIVPSTA-P-TAG5}
   if not available ar_mstr then do:
      ...
      {gprunp.i "mcpl" "p" "mc-copy-ex-rate-usage"
         "(input exch_exru_seq, output ar_exru_seq)" }
      ...
      for first ct_mstr
         fields( ct_domain ct_code ct_dating)
          where ct_mstr.ct_domain = global_domain and  ct_code = so_cr_terms
      no-lock: end.
   ...
   find cm_mstr  where cm_mstr.cm_domain = global_domain and  cm_addr = ar_bill
   exclusive-lock.
...
PROCEDURE create-invoice-history:
   ...
   create ih_hist. ih_hist.ih_domain = global_domain.
   assign
   ...
   if ih_inv_date = ? then ih_inv_date = eff_date.
   if ih_tax_date = ?
   then do:
      if can-find(first cncu_mstr
                  where cncu_domain  = global_domain
                  and   cncu_invoice = ih_inv_nbr)
      then do:
         for first cncu_mstr
            fields(cncu_domain cncu_invoice cncu_cust_usage_date)
            where cncu_domain  = global_domain
            and   cncu_invoice = ih_inv_nbr no-lock:
            ih_tax_date  = cncu_cust_usage_date.
         end. /* FOR FIRST cncu_mstr */
      end. /* IF can-find... */
      else
         ih_tax_date = so_ship_date.
   end. /* IF ih_tax_date = ? */

   /* Copy exchange rate usage records */
   {gprunp.i "mcpl" "p" "mc-copy-ex-rate-usage"
      "(input exch_exru_seq, output ih_exru_seq)" }
...
PROCEDURE mark-as-posted:
   ...
   if can-find(mfc_ctrl
       where mfc_ctrl.mfc_domain = global_domain and  mfc_module = "SO"
        and mfc_seq    = 170)
   then do:

      for each qad_wkfl
          where qad_wkfl.qad_domain = global_domain and  qad_key1 = inpar_nbr4
           and r-index(qad_key2,"utsoship") > 0
           and qad_key3 = inpar_inv_nbr
      exclusive-lock:
         assign qad_charfld[4] = "posted".
...
PROCEDURE del-cmt-det:
   ...
   for each cmt_det
          where cmt_det.cmt_domain = global_domain and  cmt_indx = inpar_indx
          exclusive-lock:
      delete cmt_det.
   end.
...
PROCEDURE del-sct-det:
   ...
   for each sct_det exclusive-lock
          where sct_det.sct_domain = global_domain and  sct_part = inpar_part
         and sct_sim  = var_sim:
      delete sct_det.
   end.
...
PROCEDURE del-ied-det:
   ...
   for each ied_det exclusive-lock
...
PROCEDURE del-pih-hist:
   ...
   for each pih_hist
          where pih_hist.pih_domain = global_domain and  pih_doc_type = 1
   ...
   define buffer   b_abs_mstr  for  abs_mstr.
      ...
      for each abs_mstr
            fields( abs_domain abs_id abs_order abs_shipfrom )
             where abs_mstr.abs_domain = global_domain and  abs_order =
             inpar_nbr3
            no-lock:
...
PROCEDURE create-inv-rel-history:
   ...
   create ihr_hist. ihr_hist.ihr_domain = global_domain.
   assign
...
PROCEDURE del-sodr-det:
   ...
   for each sodr_det exclusive-lock
          where sodr_det.sodr_domain = global_domain and  sodr_nbr  =
          sod_det.sod_nbr
...
PROCEDURE del-sor-mstr:
   ...
   for each sor_mstr exclusive-lock
          where sor_mstr.sor_domain = global_domain and  sor_nbr =
          so_mstr.so_nbr:
      delete sor_mstr.
   end.
...
PROCEDURE del-btb-det:
   ...
   for each btb_det
       where btb_det.btb_domain = global_domain and  btb_so        =
       sod_det.sod_nbr
      ...
      save_sbinfo = can-find(first mfc_ctrl
                        ...
                        mfc_field   = "enable_self_bill"
                         ...
                         and can-find(first cm_mstr
                                 where cm_mstr.cm_domain = global_domain and
                                 cm_addr   = p-so-bill
                                  and cm__qad06 = yes)
...
PROCEDURE del_tx2d_13:
   ...
   for each tx2d_det
       where tx2d_det.tx2d_domain = global_domain and  tx2d_ref = sonbr
        and tx2d_nbr = ""
        and tx2d_line = soline
        and tx2d_tr_type = "13"
   exclusive-lock:
      delete tx2d_det.
   end.
...
PROCEDURE del_tx2d_line13:
   ...
   if can-find (first tx2d_det
       where tx2d_det.tx2d_domain = global_domain and  tx2d_ref     = soinvnbr
      and   tx2d_nbr     <> ""
      and   tx2d_line    <> 0
      ...
      then do:
         {gprun.i ""txdelete.p""
            "( input '13',
               input sodnbr,
               input ''    )" }
      end. /* IF CAN-FIND (FIRST tx2d_det WHERE tx2d_ref ... */
 ...
 * -------------------------------------------------------------*/
   ...
   exclusive-lock:
      ...
      */
      ...
      for first so_mstr
          where so_mstr.so_domain = global_domain and  so_nbr = orderNumber
      exclusive-lock:
