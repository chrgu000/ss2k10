...
/*roger*/ DEF VAR vap-amt LIKE tx2d_totamt.
{&SORP1001-P-TAG1}
...
FORM /*GUI*/
   ...
   tax_edit_lbl format "x(28)"    no-label no-attr-space to 30
...
then do:
   for first so_mstr where so_domain = global_domain and (so_nbr >= nbr and so_nbr <= nbr1)
/*roger         and (so_to_inv = yes) */
   ...
   for first so_mstr where so_domain = global_domain and so_nbr = l_so_nbr
   no-lock: end .
end.
...
repeat while available so_mstr:
   ...
   do:
      ...
      {&SORP1001-P-TAG2}
/*roger*/ FIND FIRST sod_det WHERE sod_domain = global_domain and sod_nbr = so_nbr AND (sod_qty_inv + sod__dec01) <> 0
    AND ((np_fp = YES AND sod_user1 = "NP") OR (np_fp = NO AND sod_user1 = "FP")) NO-LOCK NO-ERROR.
/*roger*/   IF NOT AVAIL sod_det THEN LEAVE blocka.
/*roger      if not so_to_inv then leave blocka.*/
      {&SORP1001-P-TAG3}
      ...
      then do:
         ...
         if can-find (lgs_mstr where lgs_domain = global_domain and lgs_app_id = so_app_owner
            and lgs_invc_imp = yes no-lock) then do:
            /* Finish it up without a print */
            so_recno = recid(so_mstr).
/*roger*/        {gprun.i ""xxsosoina.p""}
            leave blocka.
         end.
      end.
      ...
      if (oldcurr <> so_curr) or (oldcurr = "") then do:
         ...
         find rnd_mstr where rnd_domain = global_domain and rnd_rnd_mthd = rndmthd no-lock no-error.
      ...
      if can-find(qad_wkfl where qad_domain = global_domain and qad_key1 = "sosois.p" + so_nbr
                             and qad_key2 = "BATCH" )
      then
         leave blocka.
      {&SORP1001-P-TAG5}
...
/*roger*/      {gprun.i ""xxsosoina.p""}
      ...
      find ct_mstr where ct_domain = global_domain and ct_code = so_cr_terms no-lock no-error.
      if available ct_mstr then
         termsdesc = ct_desc.
      else
         termsdesc = "".

      find ad_mstr where ad_domain = global_domain and ad_addr = so_bill no-lock no-wait no-error.
      update billto = "".
      ...
      find ad_mstr where ad_domain = global_domain and ad_addr = so_cust no-lock no-wait no-error.
      update soldto = "".
      if available ad_mstr then do:
         assign
            addr[1] = ad_name
      ...
      find cm_mstr where cm_domain = global_domain and cm_addr = so_cust no-lock no-error.
      ...
      hdr_call = "".
      if rmaso then do.
         find first rma_mstr where rma_domain = global_domain and rma_nbr = so_nbr
                               and rma_prefix = "C"
         no-lock no-error.
...
/*roger*/ {xxtxusrw.i}
      ...
      if rmaso then do:
         if available rma_mstr then do:
            ...
            find first ca_mstr where ca_domain = global_domain and ca_nbr = rma_ca_nbr
                                and ca_category = "0"
            no-lock no-error.
...
/*roger*/   {gprun.i ""xxsorp1a01.p"" "(output tot_cont_charge,
                                output tot_line_charge)"}

      find si_mstr where si_domain = global_domain and si_site = so_site no-lock  no-error.
      ...
      undo_trl2 = true.
/*roger*/      {gprun.i ""xxsoivtrl2.p"" "(input so_inv_nbr,
                                input so_nbr,
                                input col-80 /* REPORT WIDTH */,
                                input '16',
                                input tot_cont_charge,
      ...
      if conso then do:
         ...
         then
         for each somstr2
            where so_domain = global_domain and (so_nbr >= so_mstr.so_nbr and so_nbr <= nbr1)
/*roger              and (so_to_inv = yes) */
              and (so_ship_date >= shipdate and so_ship_date <= shipdate1)
              and (so_lang >= lang and so_lang <= lang1)
...
/*roger*/               {gprun.i ""xxsoivtrl2.p""
            ...
            for first somstr2 where so_domain = global_domain and so_nbr = l_so_nbr2
            no-lock: end.
      ...
      undo_txdetrp = true.
/*roger*/      {gprun.i ""xxtxdetrp.p"" "(input '16',
                               input so_inv_nbr,
                               input '*',
                               input col-80,
...
/*roger*/      {soivtot8.i}
   ...
   if (execname <> "rcsois.p"  and
       ...
       not lgData)
   then
      find next so_mstr where so_domain = global_domain and
...
/* THIS PROCEDURE PROCESSES CONSOLIDATIONS OF SALES ORDERS */
   ...
   for first somstr3 where so_domain = global_domain and so_nbr = l_so_nbr3
   no-lock: end.
...
/*roger*/      {gprun.i ""xxsorp1a01.p"" "(output tot_cont_charge,
      ...
      /* TOTAL ORDER, BUT DON'T PRINT THE DETAIL REPORT YET */
      undo_trl2 = true.
/*roger*/      {gprun.i ""xxsoivtrl2.p"" "(input comb_inv_nbr,
                                input so_nbr,
                                input col-80,
                                input '16',
                                input tot_cont_charge,
