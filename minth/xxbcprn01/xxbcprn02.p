...
*/
/****140127.1*
 * H3方位信息由于要显示上下FL有一个标签改为2个标签，
 * 为了旧程序可以使用新增YYYYYYYY,ZZZZZZZZZ的转换
 *****/
...
procedure myprnlabel:
   define input parameter myprint as char . /*打印机名*/
   define input parameter mypar1 as char .  /* 公司名 */
   define input parameter mypar2 as char .  /* 说明1 */
   define input parameter mypar3 as char .  /* 说明2 */
   ...
   Find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="Temp"no-lock no-error.
        ...
        TmpPath = TmpPath + "/".
  IF NOT (SEARCH(LabelsPath + "xxbcprn02.lbl") <> ?) THEN DO:
     /* 文件不存在 */
  ...
  INPUT FROM VALUE(LabelsPath + "xxbcprn02.lbl").
  REPEAT:
     CREATE tt.
...
/*140127.1*/  if index(tt1_c1,"YYYYYYYY") > 0  then do:
/*140127.1*/      tt1_c1 = replace(tt1_c1,"YYYYYYYY",  substring(trim(mypar9),1,1)).
/*140127.1*/    next.
/*140127.1*/  end.
/*140127.1*/  if index(tt1_c1,"ZZZZZZZZ") > 0  then do:
/*140127.1*/      tt1_c1 = replace(tt1_c1,"ZZZZZZZZ",  substring(trim(mypar9),2,1)).
/*140127.1*/    next.
/*140127.1*/  end.
