...
/* Revision: 1.7.1.10     BY: Hareesh V          DATE: 09/12/02  ECO: *M209* */
/* Revision: 1.7.1.11     BY: W.Palczynski       DATE: 05/13/03  ECO: *P0R8* */
/* Revision: 1.7.1.12     BY: Orawan S.          DATE: 05/20/03  ECO: *P0RX* */
/* Revision: 1.7.1.14     BY: Paul Donnelly (SB) DATE: 06/26/03  ECO: *Q00B* */
/* Revision: 1.7.1.15     BY: Reena Ambavi       DATE: 10/30/03  ECO: *P17X* */
/* $Revision: 1.7.1.16 $           BY: Geeta Kotian       DATE: 01/16/04  ECO: *P1K1* */
/* $Revision: 1.7.1.16 $           BY: Bill Jiang       DATE: 08/17/07  ECO: *SS - 20070817.1* */
/*-Revision end---------------------------------------------------------------*/
...
do transaction with frame b:
   ...
   editing:
      ...
      {mfnp01.i ar_mstr ar_check ar_check batch  " ar_mstr.ar_domain =
      ...
      then do:
         ...
         find first bk_mstr  where bk_mstr.bk_domain = global_domain and
         bk_code = ar_bank no-lock no-error.
   ...
   then
      ...
      find first ar_mstr
          where ar_mstr.ar_domain = global_domain and  ar_check = arnbr and
          ar_batch = batch
      no-lock no-error.
      ...
      then do:
         ...
         then
            ...
            display
               " " @ desc1.
         for first dy_mstr fields( dy_domain dy_dy_code dy_desc)
...
if not cash_book or (cash_book and new_line) then
   ...
   do on error undo, retry:
      ...
      find cm_mstr  where cm_mstr.cm_domain = global_domain and  cm_addr =
      input ar_bill no-lock no-error.
      if available cm_mstr
      then
         display
            cm_sort.
   ...
   if batch <> ""
   then do:
      find first ba_mstr  where ba_mstr.ba_domain = global_domain and  ba_batch
      = batch and
...
/* CODE MOVED HERE FROM ABOVE TO PARTIALLY AVOID SKIPPING OF CHECK NUMBERS */
if (input ar_check = ""
   and not cash_book)
   {&ARPAMTG-P-TAG4}
then
