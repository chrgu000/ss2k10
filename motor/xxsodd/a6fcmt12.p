...
/*J1TK*/ if available a6fcs_sum then
         do i = 1 to 52:
            ...
            */
            /*V101201.01 added */
            fcs_fcst[i] = a6fcs_fcst_qty[i] - a6fcs_sold_qty [i].
          
            
            if i <= 13 then totals[1] = totals[1] + a6fcs_fcst_qty[i]- a6fcs_sold_qty [i] .
            if i >= 14 and i <= 26 then totals[2] = totals[2] + a6fcs_fcst_qty[i] - a6fcs_sold_qty [i] .
            if i >= 27 and i <= 39 then totals[3] = totals[3] + a6fcs_fcst_qty[i] - a6fcs_sold_qty [i] .
            if i >= 40 then totals[4] = totals[4] + a6fcs_fcst_qty[i] - a6fcs_sold_qty [i] .
        end.
