...
/*SS - 110926.1 -E*/
            ...
            /*{gprun.i ""popomtr.p"" "(input yes,*/	/*---Remark by davild 20080624.1*/
	    /*---Add by davild 20080624.1*/
            {gprun.i ""xxpopom1r.p"" "(input yes,
                  input frame_row,
                  input po_curr,
                  input po_vend,
                  input pod_site,
           ...
           /* CHECK IF REQUISITION IS NON-GRS */
   ...
   */
   ...
   if avail pt_mstr then do:
	...
	if pt_net_wt <= 0 then do:
		/*---Add Begin by davild 20080722.1*/
		find first code_mstr where code_fldname = "D.pt_net_wt" no-error.
		if not avail code_mstr then do:
			create code_Mstr .
			assign code_fldname = "D.pt_net_wt" 
			       code_value = "j"
				   code_cmmt = "J字头物料重量可以<=0" .
			create code_Mstr .
			assign code_fldname = "D.pt_net_wt" 
			       code_value = "z"
				   code_cmmt = "Z字头物料重量可以<=0" .
			create code_Mstr .
			assign code_fldname = "D.pt_net_wt" 
			       code_value = "2"
				   code_cmmt = "2字头物料重量可以<=0" .
			create code_Mstr .
			assign code_fldname = "D.pt_net_wt" 
			       code_value = "y"
				   code_cmmt = "Y字头物料重量可以<=0" .
		end.
		find first code_mstr where code_fldname = "D.pt_net_wt" and code_value = substring(pt_part,1,1) no-lock no-error.
		if not avail code_Mstr then do:
		/*---Add End   by davild 20080722.1*/
		message "零件 " + pt_part + " 净重必须大于0才能建立PO." view-as alert-box .
		undo mainloop, next mainloop.
		end.	/*---Add by davild 20080722.1*/

	end.
	find first rqf_ctrl where rqf__log01 = yes no-lock no-error .
	if avail rqf_ctrl then do:
		if pt__Log02 = yes then do:	/*需要CER*/
			find first xxcer_mstr where 
				xxcer_avail = yes	/*---Add by davild 20080707.1*/
			     and  xxcer_part = pod_part
			     and  xxcer_vend = po_vend
			     /*01.在维护料号时不做日期判断，再维护到期日期xxpopom1d.p再作判断*/
			     /*and  xxcer_effdate <= today and (xxcer_expire >= today or xxcer_expire = ?)*/
			     and  xxcer_dec_result <> "不合格(NG)"
			     no-lock no-error.
			if avail xxcer_mstr then do:
				/*if xxcer_dec_result = "限购(QTY)" and (xxcer_pur_qty < rqpo_net_qty + xxcer_ord_qty) then do:
					message "申请号" + rqpo_nbr + "-" + string(rqpo_Line) 
						+"中零件 " + pt_part + " 供应商" + po_vend + "的申请数量不能大于CER的限购数量." 
						skip 
				...
				message "零件 " + pt_part + " 供应商" + po_vend + " 无合格物料测试表." 
					view-as alert-box .
				undo mainloop, next mainloop.
			end.
		end.
   ...
   /*g)--采购订单明细中的税用途(pod_tax_usage)和纳税类型(pod_taxc)默认取值采购订单单头对应值(po_taxc/po_tax_usage*/
   if new_pod then do:
	assign pod_taxc = po_taxc
        pod_taxable = po_taxable   /* SS - 110223.1 */
   ...
   /*---Add End   by davild 20080624.1*/
	    ...
	    /* ss-081201.1 -e */
            ...
            if lineffdate = ? then
             ...
             if use-log-acctg and po_tot_terms_code <> "" then do:
               ...
               pPurCost = ?.
               for each vp_mstr
                  fields(vp_part vp_vend vp_q_date vp_um
                         vp_q_qty vp_curr vp_q_price)
                  where vp_part = pod_part
                  and   vp_vend = po_vend
                  no-lock break by vp_q_date descending:

                  if first(vp_q_date) then do:

                     if po_curr = vp_curr and vp_um <> pod_um then do:
                        ...
                        pPurCost = pPurCost * conversion_factor.
	...
	old_pur_cost "d"
	old_disc_pct
	view-as alert-box .	---Remark by davild 20080709.1*/
               ...
               then do:
                  if not new_pod
                  then do:
                     if l_podcost[2] <> l_podcost[1]
                     then
                         display
                            l_podcost[1] @ pod_pur_cost
                         with frame c.

                     if l_discpct[2] <> l_discpct[1]
                     then
                        display
...
/*SS - 110926.1 -B*/
                  {gprunmo.i &module="GRS" &program="xxpopomtra1.p"
                     &param="""(input yes,
                        input pod_site,
                        input pod_req_nbr,
                        input pod_req_line,
            ...
            if sngl_ln = yes then do:
               assign continue = no
                  pod_recno = recid(pod_det).
               {gprun.i ""xxpopom1d.p""}	/*---Add by davild 20080624.1*/
               if c-application-mode <> "API" then
               do:
