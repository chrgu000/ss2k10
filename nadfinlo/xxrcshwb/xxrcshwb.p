...
/* $Revision: 1.19.1.19.3.1 $  BY: K Paneesh       DATE: 07/15/03 ECO: *P0X0*   */
/******************************************************************************/
...
/* REVISION: 9.1      LAST MODIFIED: 05/03/04   BY: *ss 090618.1* mage chen        */
/* ss - 120611.1 */ /* modify print xxsofmsv01.p */
/*ss 090618.1 b*
{mfdtitle.i "2+ "}
*ss 090618.1 e*/
/*ss 090618.1 b*/
...
define variable doc_type like mfc_logical format {&rcshwb_p_1}
   no-undo.
...
define variable multi_entry    like mfc_logical label {&rcshwb_p_5} no-undo.
...
form
   ...
   doc_type             colon 25 label {&rcshwb_p_1}
   abs_id               colon 25 label {&rcshwb_p_3}
   skip(1)
   abs_shipto           colon 25 label {&rcshwb_p_9}
...
on leave of abs_shipfrom, abs_shipto in frame a do:
   ...
   {gprun.i
      ...
      "(input input frame a abs_shipfrom,
        ...
        output shipgrp)"}
   find sg_mstr no-lock where sg_grp = shipgrp no-error.
   {&RCSHWB-P-TAG1}
...
repeat with frame a:
   ...
   prompt-for abs_shipfrom doc_type abs_id
      abs_shipto inv_mov
   editing:
      ...
      do:
         if frame-field = "abs_shipfrom" then do:
            ...
            if available abs_mstr and abs_shipfrom <> ""
               then global_site = abs_shipfrom.
         ...
         if frame-field = "abs_id" then do:
           ...
           if input doc_type then do:
            ...
            if recno <> ? then do:
               ...
               find im_mstr where im_inv_mov = abs_inv_mov no-lock
                  no-error.
   ...
   do transaction:
      ...
      display shipgrp (if not available sg_mstr then ""
      ...
      if not available abs_mstr then do:
         ...
         /* REMOVED THE SITE ADDRESS VALIDATION */

         run validate_shipto (input frame a abs_shipto,
            output errorst).
         if errorst then do:
         ...
         if not can-find (first sgid_det where
               sgid_grp = shipgrp and
               sgid_inv_mov = input frame a inv_mov) then
               msgnum = 5985.
            /* INVENTORY MOVEMENT CODE IS NOT VALID FOR SHIPPING GROUP # */
            else
            {&RCSHWB-P-TAG7}
            if not can-find (first im_mstr where im_inv_mov =
               input frame a inv_mov and
               im_tr_type = "ISS-SO") then
            {&RCSHWB-P-TAG8}
            msgnum = 5980.
         ...
         /* CODE MOVED TO PROCEDURE P-SGID DUE TO THE
         ...
         find ad_mstr where ad_addr = input frame a abs_shipto
            no-lock no-error.
 ...
 /*ss090614.1 b*/
  ...
  /*ss090614.1 e*/
         ...
         if can-find
            ...
            df_type   = "1" and df_inv) and
            length(abs_id) > 9 then
            abs_format = "".
      ...
      find im_mstr where im_inv_mov = input frame a inv_mov
         no-lock no-error.
      display (if not available im_mstr then ""
      ...
      if abs_shipto <> input frame a abs_shipto and
         input frame a abs_shipto <> "" then do:
      ...
      /* RELEASED ONCE THE PRE-SHIPPER/SHIPPER NUMBER IS GENERATED  */
   end. /* DO TRANSACTION */
   ...
   do transaction:
      ...
      carrier = "".
/*ss 090618.1 b*
      display carrier multi_entry abs_shipvia
...
/*ss 090618.1 b*/
      display carrier multi_entry abs_shipvia
         abs_fob abs_trans_mode  abs_carr_ref
         abs_veh_ref abs_format cons_ship
         abs_lang abs_arr_date merge_docs cmmts
      with frame c.
/*ss 090618.1 e*/
      assign old_abs_format = abs_format
      ...
      repeat:
         ...
         find first absc_det where
            absc_abs_id = abs_id no-lock no-error.
...
/*ss 090618.1 e*/
            ...
            then do:
               ...
               if go-pending and can-find(first absd_det where
                  absd_abs_id = abs_id
                  and absd_shipfrom = abs_shipfrom
         ...
         find first df_mstr where
            df_format = abs_format and
            df_type = "1" no-lock no-error.
         ...
         if available df_mstr and df_inv = true and v_so_ship then do:
            ...
            run chk_abs_inv_cons (input abs_id, input abs_shipfrom,
                                  "","","", output cons_ok).
         ...
         if old_abs_format <> abs_format then do:
            find df_mstr where df_format = old_abs_format
               and df_type = "1"
               no-lock no-error.
            old_form_code = if available df_mstr then
                               df_form_code
                            else "".
            find df_mstr where df_format = abs_format
               and df_type = "1" no-lock no-error.
         ...
         if abs_lang <> ad_lang then
            do:
            if not (can-find (lng_mstr where
                        lng_lang = abs_lang)) then do:
         ...
         if merge_docs and
            not (v_confirmed or abs_canceled) and v_so_ship then do:
            /* MERGE PICKLISTS */
            {gprun.i ""rcshwbe.p"" "(input recid(abs_mstr),
   ...
   if not can-find(abs_mstr where
   ...
   /* Print shipper */
/*nad001   {gprun.i ""icshprt.p"" "(abs_recid)" }*/
/*nad001*/   {gprun.i ""xxicshprt.p"" "(abs_recid)" }
...
do on error undo, leave:
   ...
   find im_mstr no-lock where im_inv_mov = abs_buff.abs_inv_mov
   no-error.
   transtype =
   if available im_mstr then im_tr_type else "ISS-SO".
   {&RCSHWB-P-TAG11}
...
PROCEDURE p-sgid:
   find sgid_det
      where sgid_grp = shipgrp and
      sgid_inv_mov = input frame a inv_mov no-lock no-error.
   if available sgid_det then
      nrseq = if doc_type then
                 sgid_preship_nr_id
              else sgid_ship_nr_id.
end. /* PROCEDURE P-SGID */
...
PROCEDURE maintainLogAcctDetail:
   ...
   for first b1_abs_mstr where b1_abs_mstr.abs_shipfrom = ipShipFrom and
   ...
   if (not available b1_abs_mstr) or
             (l_FirstOrder = "" or l_FrTermsOnFirstOrder = "") then
   do:
...
PROCEDURE chk_shipto_prefix:
   ...
   find first b_abs_mstr
      where abs_shipfrom = i_abs_shipfrom
      and   abs_id       = i_abs_id
   exclusive-lock.
