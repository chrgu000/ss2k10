...
if can-find (mfc_ctrl
   where mfc_field = "enable_sequence_schedules"
     and mfc_logical)
     and file_found
then
...
/*         ,each tmpso where tso1_nbr = sod_nbr and tso1_line = sod_line no-lock */
   ...
   */
   if not can-find(first qad_wkfl
                   where qad_key1 begins string(so_nbr,"x(8)")
...
:
      ...
      if sod_qty_inv <> 0
         and can-find (iec_ctrl where iec_use_instat = yes)
      then do:
 ...
 /* SS - 20060524.1 - B */
   ...
   if (can-find(first lgs_mstr
       where lgs_app_id = so_app_owner)) and
      (can-find(first esp_mstr
       where esp_app_id = so_app_owner
   ...
   for last sod_det
      fields (sod_cmtindx sod_fa_nbr sod_fsm_type sod_line sod_lot sod_nbr
...
PROCEDURE create-ar-distribution:
   ...
   for first gl_ctrl
      fields (gl_ar_acct gl_ar_sub gl_ar_cc gl_base_curr
   ...
   if disc_amt <> 0 then do:
      ...
      find first ard_det
         where ard_nbr    = so_mstr.so_inv_nbr
   ...
   if so_trl1_amt <> 0 then do:
      ...
      for first trl_mstr
         fields (trl_acct trl_sub trl_cc trl_code trl_taxable trl_taxc)
         where trl_code = so_trl1_cd
      no-lock: end.
   ...
   find ar_mstr where ar_nbr = so_inv_nbr exclusive-lock no-error.
   {&SOIVPSTA-P-TAG5}
   ...
   find cm_mstr where cm_addr = ar_bill exclusive-lock.
...
PROCEDURE create-invoice-history:
   ...
   if ih_tax_date = ? then ih_tax_date = so_ship_date.
...
PROCEDURE del-cmt-det:
   ...
   for each cmt_det
         where cmt_indx = inpar_indx exclusive-lock:
      delete cmt_det.
   end.
...
PROCEDURE del-tx2d-det:
   ...
   define buffer   b_abs_mstr  for  abs_mstr.
      ...
      for each abs_mstr
            fields (abs_id abs_order abs_shipfrom )
            where abs_order = inpar_nbr3
            no-lock:
...
PROCEDURE del-sodr-det:
   ...
   for each sodr_det exclusive-lock
         where sodr_nbr  = sod_det.sod_nbr
...
PROCEDURE del_tx2d_line13:
   ...
   if can-find (first tx2d_det
      where tx2d_ref     = soinvnbr
      and   tx2d_nbr     = sodnbr
      and   tx2d_line    = sodline
 ...
 * -------------------------------------------------------------*/
   ...
   for first qad_wkfl
      where qad_key1 = string(orderNumber, "x(8)") +
