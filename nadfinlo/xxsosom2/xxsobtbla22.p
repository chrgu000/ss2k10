...
/*        if sod_site = "NSZA" then do:*/
        if substring(trim(sod_site),length(trim(sod_site)),1) = "A" then do:
	   if substring(trim(sod_part),1,2) = "NT" then do:
	      if substring(trim(sod_part),length(trim(sod_part)) - 2,3) = "C03" then do:
	      end.
	      else do:
	       message "该地点以NT开头的零件号必须是C03结尾.请重新输入". pause.
