...
/*
/* SS - 111018.1 -B */
        if so_site = "NSZA" then do:
	   if substring(trim(sod_part),1,2) = "NT" and substring(trim(sod_part),length(trim(sod_part)) - 3,3) = "C03" then do:
	   end.
	   else do:
	       message "NSZA地点只能维护NT开头且C03结尾的零件号.请重新输入".
               next-prompt sod_part with frame c.
               undo, retry.
	   end.
	end.
/* SS - 111018.1 -E */
...
/* SS - 20111019.1*         {gprun.i ""sosomtla.p""*/
/* SS - 20111019.1*/         {gprun.i ""xxsosomtla2.p""
            "(input this-is-rma,
              input rma-recno,
