...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv02wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[批号库存查询]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[批号库存查询]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[批号库存查询]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1500.
            ...
            THEN DO:
                  IF recid(LD_DET) = ? THEN find first LD_DET where 
                              LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_SITE = V1002 AND LD_DOMAIN = V1001 AND  
                              LD_LOT >=  INPUT V1500
                               no-lock no-error.
                  else do: 
                       if LD_LOT =  INPUT V1500
                       then find next LD_DET
                       WHERE LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_SITE = V1002 AND LD_DOMAIN = V1001
                        no-lock no-error.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[批号库存查询]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1510 width 150 no-box.
                ...
                /* LABEL 4 - START */ 
                find first ld_det where ld_part = V1300 and ld_lot = V1500 and ld_qty_oh <> 0 and ld_site = V1002 and ld_DOMAIN = V1001 no-lock no-error.
If AVAILABLE ( ld_det ) then
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1510.
            ...
            THEN DO:
                  IF recid(LD_DET) = ? THEN find first LD_DET where 
                              LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_LOT = V1500 AND LD_SITE = V1002 AND LD_DOMAIN = V1001 AND  
                              LD_REF >=  INPUT V1510
                               no-lock no-error.
                  else do: 
                       if LD_REF =  INPUT V1510
                       then find next LD_DET
                       WHERE LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_LOT = V1500 AND LD_SITE = V1002 AND LD_DOMAIN = V1001
                        no-lock no-error.
                        else find first LD_DET where 
                              LD_PART = V1300 AND LD_QTY_OH <> 0 AND LD_LOT = V1500 AND LD_SITE = V1002 AND LD_DOMAIN = V1001 AND  
                              LD_REF >=  INPUT V1510
