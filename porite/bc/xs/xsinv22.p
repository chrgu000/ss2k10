...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv22wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1100.
            ...
            THEN DO:
                  IF recid(CODE_MSTR) = ? THEN find first CODE_MSTR where 
                              CODE_DOMAIN = V1001 and CODE_FLDNAME = "barcode_RCT_UNP" AND  
                              CODE_VALUE >=  INPUT V1100
                               no-lock no-error.
                  else do: 
                       if CODE_VALUE =  INPUT V1100
                       then find next CODE_MSTR
                       WHERE CODE_DOMAIN = V1001 and CODE_FLDNAME = "barcode_RCT_UNP"
                        no-lock no-error.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first CODE_MSTR where CODE_DOMAIN = V1001 and CODE_FLDNAME = "barcode_RCT_UNP" AND index ( V1100,CODE_VALUE ) <> 0  no-lock no-error.
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1290 width 150 no-box.
...
If 1=1 and t_v100[1]<>'' then
        leave V1300L.
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 width 150 no-box.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LOC_MSTR where LOC_DOMAIN = V1001 and LOC_SITE = V1002 AND LOC_LOC = V1400  no-lock no-error.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 width 150 no-box.
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1500.
            ...
            THEN DO:
                  IF recid(LD_DET) = ? THEN find first LD_DET where 
                              LD_DOMAIN = V1001 AND LD_SITE = V1002 AND LD_PART = V1300 AND LD_LOC = V1400 AND LD_QTY_OH <> 0 AND LD_REF = "" AND  
                              LD_LOT >=  INPUT V1500
                               no-lock no-error.
                  else do: 
                       if LD_LOT =  INPUT V1500
                       then find next LD_DET
                       WHERE LD_DOMAIN = V1001 AND LD_SITE = V1002 AND LD_PART = V1300 AND LD_LOC = V1400 AND LD_QTY_OH <> 0 AND LD_REF = ""
                        no-lock no-error.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 width 150 no-box.
                ...
                /* LABEL 4 - START */ 
                find first ld_det where ld_domain = V1001 and ld_site = V1002 and ld_part = V1300 and ld_loc = V1400 and ld_lot = V1500 and ld_site = V1002 and ld_qty_oh <> 0 and ld_ref = "" use-index ld_part_lot no-lock no-error.
If AVAILABLE ( ld_det ) then
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where ( decimal(V1600) > 0 ) OR ( decimal(V1600) < 0 AND (ld_domain = V1001 and ld_part  = V1300 AND ld_loc = V1400 and ld_site = V1002 AND ld_ref = "" AND  ld_lot = V1500 and
...
If AVAILABLE ( code_mstr ) then
        ...
        leave V1630L.
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1630 width 150 no-box.
...
If AVAILABLE ( code_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1640 width 150 no-box.
...
If not avail sb_mstr then do:
                display skip "分帐户不存在,请查" @ WMESSAGE NO-LABEL with fram F1640.
                pause 0 before-hide.
                Undo, retry.
End.
...
If AVAILABLE ( code_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1650 width 150 no-box.
...
End.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 width 150 no-box.
...
End.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first ld_det where decimal(V1600) > 0 OR ( decimal(V1600) < 0 AND (ld_domain = V1001 and ld_part  = V1300 AND ld_site = V1002 AND ld_ref = "" AND ld_loc = V1400 and ld_lot = V1500 and ld_site = V1002 AND ld_ref = "" and  ld_QTY_oh + DECIMAL ( V1600 ) >= 0 ) )  NO-ERROR NO-WAIT.
        IF NOT AVAILABLE ld_det then do:
...
If trnbr<>0 then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 width 150 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外入库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 width 150 no-box.
     ...
     INPUT FROM VALUE(LabelsPath + "inv22").
