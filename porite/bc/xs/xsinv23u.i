...
usection = TRIM ( string(year(TODAY)) + string(MONTH(TODAY)) + string(DAY(TODAY)))  + trim(STRING(TIME)) + trim(string(RANDOM(1,100))) + "inv23" .
output to value( trim(usection) + ".i") .
        display 
"""" + trim ( V1300) + """" format "x(50)" skip
trim ( V1600 ) + " " + V1530 + " " + """" + trim ( V1100 ) + """"  + " - " + """" + trim ( V1305 ) + """"   format "x(50)" skip
"Y T Y" format "x(50)" skip
trim(V1002) + " " +  trim( V1490 ) format "X(50)"  if trim( V1500 ) = "" then " - " else """" + trim(V1500) + """"  format "X(50)" skip
trim(V1513) + " "  + trim( V1520 ) format "x(50)" skip
"Y" skip
"." skip
"."
