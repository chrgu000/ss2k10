...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv25wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1100.
            ...
            THEN DO:
                  IF recid(CODE_MSTR) = ? THEN find first CODE_MSTR where 
                              CODE_DOMAIN = V1001 AND CODE_FLDNAME = "ISS-UNPREASON" AND  
                              CODE_VALUE >=  INPUT V1100
                               no-lock no-error.
                  else do: 
                       if CODE_VALUE =  INPUT V1100
                       then find next CODE_MSTR
                       WHERE CODE_DOMAIN = V1001 AND CODE_FLDNAME = "ISS-UNPREASON"
                        no-lock no-error.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first CODE_MSTR where CODE_DOMAIN = V1001 AND CODE_FLDNAME = "ISS-UNPREASON" AND index ( V1100,CODE_VALUE ) <> 0  no-lock no-error.
...
If AVAILABLE ( code_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 width 150 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 width 150 no-box.
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 width 150 no-box.
                ...
                /* LABEL 2 - START */ 
                find first ld_det where ld_domain = V1001 and ld_part = V1300 and ld_loc = V1400 and ld_site = V1002 and ld_ref = "" and ld_qty_oh <> 0 use-index ld_part_lot no-lock no-error.
...
If AVAILABLE ( ld_det ) then
     ...
     REPEAT:
        ...
        leave V1503L.
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1503 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
     ...
     REPEAT:
        ...
        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V1504           as char format "x(150)".
        define variable PV1504          as char format "x(150)".
        ...
        leave V1504L.
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1504 width 150 no-box.

                /* LABEL 1 - START */ 
                L15041 = "**物料未过期**" .
                display L15041          format "x(40)" skip with fram F1504 width 150 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                L15042 = "批  号:" + trim ( V1500 ) .
                display L15042          format "x(40)" skip with fram F1504 width 150 no-box.
                /* LABEL 2 - END */ 
...
If AVAILABLE ( pt_mstr ) then
                ...
                display L15043          format "x(40)" skip with fram F1504 width 150 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                L15044 = "到期日:"  + if expireinv <> ? Then STRING ( expireinv ) else " " .
                display L15044          format "x(40)" skip with fram F1504 width 150 no-box.
                /* LABEL 4 - END */ 
                display "输入或按E退出"       format "x(40)" skip
        skip with fram F1504 width 150 no-box.
        Update V1504
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 width 150 no-box.
...
If AVAILABLE ( ld_det ) then
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where ( decimal(V1600) < 0 and ld_part = "BARCODEXZYIU" ) OR ( ld_part  = V1300 AND ld_loc = V1400 and ld_domain = V1001 and  ld_site = V1002 and ld_ref = "" and ld_lot = V1500 and  ld_QTY_oh >= DECIMAL ( V1600 ) )  no-lock no-error.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 width 150 no-box.
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.
...
If NOT AVAILABLE ( tr_hist ) then
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 width 150 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外出过期料]"     + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 width 150 no-box.
     ...
     INPUT FROM VALUE(LabelsPath + "inv25").
