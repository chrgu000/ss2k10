...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv44wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 width 150 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1205 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1205.
            ...
            THEN DO:
                  IF recid(TAG_MSTR) = ? THEN find first TAG_MSTR where 
                              NOT TAG_VOID AND NOT TAG_POSTED AND TAG_LOC = V1200  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = "" AND TAG_DOMAIN = V1001 AND  
                              TAG_NBR >=  INTEGER ( INPUT V1205)
                               no-lock no-error.
                  else do: 
                       if TAG_NBR =  INTEGER ( INPUT V1205)
                       then find next TAG_MSTR
                       WHERE NOT TAG_VOID AND NOT TAG_POSTED AND TAG_LOC = V1200  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = "" AND TAG_DOMAIN = V1001
                        no-lock no-error.
                        else find first TAG_MSTR where 
                              NOT TAG_VOID AND NOT TAG_POSTED AND TAG_LOC = V1200  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = "" AND TAG_DOMAIN = V1001 AND  
                              TAG_NBR >=  INTEGER ( INPUT V1205)
                               no-lock no-error.
                  end.
            ...
            IF LASTKEY = keycode("F9") or keyfunction(lastkey) = "CURSOR-UP"
            THEN DO:
                  IF recid(TAG_MSTR) = ? THEN find last TAG_MSTR where 
                              NOT TAG_VOID AND NOT TAG_POSTED AND TAG_LOC = V1200  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = "" AND TAG_DOMAIN = V1001 AND  
                              TAG_NBR <=  INTEGER ( INPUT V1205)
                               no-lock no-error.
                  else do: 
                       if TAG_NBR =  INTEGER ( INPUT V1205)
                       then find prev TAG_MSTR
                       where NOT TAG_VOID AND NOT TAG_POSTED AND TAG_LOC = V1200  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = "" AND TAG_DOMAIN = V1001
                        no-lock no-error.
                        else find first TAG_MSTR where 
                              NOT TAG_VOID AND NOT TAG_POSTED AND TAG_LOC = V1200  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = "" AND TAG_DOMAIN = V1001 AND  
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first TAG_MSTR where NOT TAG_VOID AND NOT TAG_POSTED AND TAG_NBR = integer ( V1205 ) AND TAG_LOC = V1200  AND TAG_DOMAIN = V1001 AND  TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = ""  NO-ERROR NO-WAIT.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
                ...
                /* LABEL 3 - START */ 
                find first tag_mstr where tag_domain = V1001 and tag_nbr = integer(V1205)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
                L13003 = "图号:" + trim( tag_part ) .
                else L13003 = "" . 
...
If AVAILABLE ( tag_mstr ) then
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first TAG_MSTR where TAG_DOMAIN = V1001 AND TAG_NBR = integer ( V1205 ) AND ( ( V1300 = "" ) OR ( TAG_PART = ENTRY(1, V1300, "@") ) AND TAG_SERIAL = ( if ENTRY(2, V1300, "@") = "" then "" else ENTRY(2, V1300, "@") ) )  no-lock no-error.
        IF NOT AVAILABLE TAG_MSTR then do:
...
If AVAILABLE ( tag_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 width 150 no-box.
     ...
     REPEAT:
        ...
        leave V1600L.
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 width 150 no-box.
...
If AVAILABLE ( tag_mstr ) then
                ...
                /* LABEL 4 - START */ 
                L16004 = "标签/库位:" + trim(V1205) + "/" + trim ( V1200 ) .
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 width 150 no-box.

                /* LABEL 1 - START */ 
                find first tag_mstr where tag_domain = V1001 and tag_nbr = integer(V1205)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
                L17001 = "图号:" + trim(tag_part) .
                else L17001 = "" . 
...
If AVAILABLE ( tag_mstr ) then
                ...
                /* LABEL 4 - START */ 
                L17004 = "库位/数量:" + trim ( V1200 ) + "/" + trim( V1600 ) .
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.

                /* LABEL 1 - START */ 
                find first tag_mstr where tag_domain = V1001 and tag_nbr = integer(V1205) and tag_rcnt_qty = decimal (V1600)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
                L90101 = "交易已提交" .
                else L90101 = "" . 
...
If AVAILABLE ( tag_mstr ) then
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 width 150 no-box.
...
If AVAILABLE ( tag_mstr ) then
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 width 150 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 width 150 no-box.
     ...
     INPUT FROM VALUE(LabelsPath + "inv44").
     ...
     Do While True:
       ...
       find first pt_mstr where pt_domain = V1001 and pt_part = V1410  no-lock no-error.
