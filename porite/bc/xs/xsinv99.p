...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv99wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[交易查询]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[交易查询]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 width 150 no-box.

                /* LABEL 1 - START */ 
                find last tr_hist where tr_domain = V1001  no-lock no-error.
If AVAILABLE ( tr_hist ) then
                L14001 = trim(tr_type) + "/L*" + string(tr_trnbr) .
                else L14001 = "" . 
...
If AVAILABLE ( tr_hist ) then
   ...
   V1600LMAINLOOP:
   REPEAT:
     /* START  LINE :1600  交易明细[Next Transaction NBR]  */
     V1600L:
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find prev tr_hist where tr_domain = V1001  no-lock no-error.
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[交易查询]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 width 150 no-box.
     ...
     END.
     PV1600 = V1600.
     /* END    LINE :1600  交易明细[Next Transaction NBR]  */


     /* START  LINE :1800  交易明细[Next]  */
     V1800L:
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[交易查询]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1800 width 150 no-box.

                /* LABEL 1 - START */ 
                find first tr_hist where tr_domain = V1001 and tr_trnbr = integer(V1600) no-lock no-error.
If AVAILABLE ( tr_hist ) then
                L18001 = trim(tr_type) + "/N*" + string(tr_trnbr) .
                else L18001 = "" . 
                ...
                find first tr_hist where tr_domain = V1001 and tr_trnbr = integer(V1600) no-lock no-error.
If AVAILABLE ( tr_hist ) then
                L18002 = trim(tr_nbr) + "/" + trim(tr_lot) .
                else L18002 = "" . 
                ...
                find first tr_hist where tr_domain = V1001 and tr_trnbr = integer(V1600) no-lock no-error.
If AVAILABLE ( tr_hist ) then
                L18003 = trim(tr_part) + "/" + trim( string( tr_qty_chg )) .
                else L18003 = "" . 
