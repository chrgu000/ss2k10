...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xslap06wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1100.
            ...
            THEN DO:
                  IF recid(WO_MSTR) = ? THEN find first WO_MSTR where 
                              wo_domain = V1001 and WO_SITE = V1002 AND  
                              WO_NBR >=  INPUT V1100
                               no-lock no-error.
                  else do: 
                       if WO_NBR =  INPUT V1100
                       then find next WO_MSTR
                       WHERE wo_domain = V1001 and WO_SITE = V1002
                        no-lock no-error.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1103 then
        leave V1103L.
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1103 width 150 no-box.
...
If AVAILABLE ( wo_mstr ) then
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1103.
            ...
            THEN DO:
                  IF recid(wo_mstr) = ? THEN find first wo_mstr where 
                              wo_domain = V1001 and wo_nbr = V1100 AND wo_site = V1002 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" ) AND  
                              wo_lot >=  INPUT V1103
                               no-lock no-error.
                  else do: 
                       if wo_lot =  INPUT V1103
                       then find next wo_mstr
                       WHERE wo_domain = V1001 and wo_nbr = V1100 AND wo_site = V1002 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" )
                        no-lock no-error.
                        else find first wo_mstr where 
                              wo_domain = V1001 and wo_nbr = V1100 AND wo_site = V1002 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" ) AND  
                              wo_lot >=  INPUT V1103
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first wo_mstr where wo_domain = V1001 and wo_lot = V1103 AND wo_site = V1002 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" )  no-lock no-error.
        IF NOT AVAILABLE wo_mstr then do:
...
If AVAILABLE ( wo_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1104 width 150 no-box.
...
If AVAILABLE ( wo_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1105 width 150 no-box.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first SOD_DET where SOD_DOMAIN = V1001 AND SOD_NBR = V1105 and SOD_PART = V1104  no-lock no-error.
     ...
     REPEAT:
        ...
        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V1106           as char format "x(150)".
        define variable PV1106          as char format "x(150)".
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find first so_mstr where sp_domain = V1001 and so_nbr  = V1105 no-lock no-error.
If AVAILABLE ( so_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1106 width 150 no-box.

                /* LABEL 1 - START */ 
                  L11061 = "" . 
                display L11061          format "x(40)" skip with fram F1106 width 150 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                  L11062 = "" . 
                display L11062          format "x(40)" skip with fram F1106 width 150 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                  L11063 = "" . 
                display L11063          format "x(40)" skip with fram F1106 width 150 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                  L11064 = "" . 
                display L11064          format "x(40)" skip with fram F1106 width 150 no-box.
                /* LABEL 4 - END */ 
                display "输入或按E退出"       format "x(40)" skip
        skip with fram F1106 width 150 no-box.
        Update V1106
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1310 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1320 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1320.
            ...
            THEN DO:
                  IF recid(CODE_MSTR) = ? THEN find first CODE_MSTR where 
                              CODE_DOMAIN = V1001 AND CODE_FLDNAME = "BARCODEFGLINE" AND  
                              CODE_VALUE >=  INPUT V1320
                               no-lock no-error.
                  else do: 
                       if CODE_VALUE =  INPUT V1320
                       then find next CODE_MSTR
                       WHERE CODE_DOMAIN = V1001 AND CODE_FLDNAME = "BARCODEFGLINE"
                        no-lock no-error.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first CODE_MSTR where CODE_DOMAIN = V1001 AND CODE_FLDNAME = "BARCODEFGLINE" and CODE_VALUE = V1320  no-lock no-error.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1330 width 150 no-box.
        ...
        IF not length( trim ( V1330 ) ) = 1 THEN DO:
                display skip "长度不能支能1位" @ WMESSAGE NO-LABEL with fram F1330.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1335 width 150 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9020 width 150 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[销售工单标签]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9030 width 150 no-box.
...
If AVAILABLE ( pt_mstr )  then
       ...
       END.
        av9030 = trim ( WeekResult )  + trim ( V1320).
...
If AVAILABLE ( pt_mstr )  then
       ...
       + SUBSTRING( ts9030 , index(ts9030 ,"$D") + length("$D"), LENGTH(ts9030) - ( index(ts9030 ,"$D" ) + length("$D") - 1 ) ).
       END.
       find first ad_mstr where ad_domain = V1001 and ad_addr = V1106  no-lock no-error.
If AVAILABLE ( ad_mstr )  then
       ...
       END.
       find first cp_mstr where cp_domain = V1001 and cp_cust = V1106 and cp_cust_part <> "" and cp_part = V1104  no-lock no-error.
...
If AVAILABLE ( pt_mstr )  then
       ...
       END.
       find first so_mstr where so_domain = V1001 and so_nbr = V1105  no-lock no-error.
If AVAILABLE ( so_mstr )  then
        av9030 = so_po.
