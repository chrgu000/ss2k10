...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xslap08wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[自制标签补打]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[自制标签补打]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[自制标签补打]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[自制标签补打]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 width 150 no-box.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where LD_DOMAIN = V1001 AND LD_PART = V1300 AND LD_SITE = V1002  no-lock no-error.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[自制标签补打]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[自制标签补打]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9020 width 150 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[自制标签补打]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9030 width 150 no-box.
     ...
     INPUT FROM VALUE(LabelsPath + "lap08").
...
If AVAILABLE ( pt_mstr )  then
       ...
       END.
       find first pt_mstr where pt_domain = V1001 and pt_part = trim( V1300 )  no-lock no-error.
If AVAILABLE ( pt_mstr )  then
