...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xslap09wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 width 150 no-box.
     ...
     REPEAT:
        ...
        leave V1300L.
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
     ...
     PV1300 = V1300.
     V1300 = ENTRY(1,V1300,"@").
     /* END    LINE :1300  图号[ITEM]-INV  */


     /* START  LINE :1310  图号[ITEM]-WO  */
     V1310L:
     REPEAT:
        ...
        leave V1310L.
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1310 width 150 no-box.
...
and tr_site = V1002 AND  
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 width 150 no-box.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where LD_DOMAIN = V1001 AND LD_PART = V1300 AND LD_LOT = V1500 AND LD_SITE = V1002  no-lock no-error.
        IF NOT AVAILABLE LD_DET then do:
                display skip "没有库存,不可打印" @ WMESSAGE NO-LABEL with fram F1500.
                pause 0 before-hide.
                undo, retry.
        end.
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        if INDEX(PV1310,"@") <> 0 then
        V1510 = ENTRY(2, PV1310, "@").
        V1510 = ENTRY(1,V1510,"@").
        /* --FIRST TIME DEFAULT  VALUE -- END  */
        ...
        leave V1510L.
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1510 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1510.
            ...
            THEN DO:
                  IF recid(TR_HIST) = ? THEN find first TR_HIST where 
                              TR_DOMAIN = V1001 AND TR_NBR = V1200 and TR_TYPE = "ISS-WO" and TR_PART = V1310 AND TR_SITE = V1002 AND  
                              TR_SERIAL >=  INPUT V1510
                               no-lock no-error.
                  else do: 
                       if TR_SERIAL =  INPUT V1510
                       then find next TR_HIST
                       WHERE TR_DOMAIN = V1001 AND TR_NBR = V1200 and TR_TYPE = "ISS-WO" and TR_PART = V1310 AND TR_SITE = V1002
                        no-lock no-error.
                        else find first TR_HIST where 
                              TR_DOMAIN = V1001 AND TR_NBR = V1200 and TR_TYPE = "ISS-WO" and TR_PART = V1310 AND TR_SITE = V1002 AND  
                              TR_SERIAL >=  INPUT V1510
                  ...
                  IF AVAILABLE TR_HIST then display skip 
            TR_SERIAL @ V1510 trim(TR_SERIAL) + "/" + trim(string(TR_QTY_CHG)) @ WMESSAGE NO-LABEL with fram F1510.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first TR_HIST where TR_DOMAIN = V1001 AND TR_NBR = V1200 and TR_TYPE = "ISS-WO" and TR_PART = V1310 AND TR_SITE = V1002  no-lock no-error.
        IF NOT AVAILABLE TR_HIST then do:
                display skip "该物料未曾发料" @ WMESSAGE NO-LABEL with fram F1510.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
     ...
     PV1510 = V1510.
     /* END    LINE :1510  批号[LOT]-WO  */


   /* Internal Cycle Input :1600    */
   V1600LMAINLOOP:
   REPEAT:
     /* START  LINE :1600  总数数量[QTY ON LABEL]  */
     V1600L:
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 width 150 no-box.
     ...
     END.
     PV1600 = V1600.
     /* END    LINE :1600  总数数量[QTY ON LABEL]  */


     /* START  LINE :9010  订单倍数,条码上的数量[QTY ON LABEL]  */
     V9010L:
     REPEAT:
        ...
        V9010 = If avail pt_mstr then string( pt_ord_mult ) else "0".
        V9010 = ENTRY(1,V9010,"@").
        /* --FIRST TIME DEFAULT  VALUE -- END  */
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.
        ...
        /* PRESS e EXIST CYCLE */
        IF V9010 = "e" THEN  LEAVE V1600LMAINLOOP.
     ...
     END.
     PV9010 = V9010.
     /* END    LINE :9010  订单倍数,条码上的数量[QTY ON LABEL]  */


     /* START  LINE :9020  箱数[NO OF LABEL]  */
     V9020L:
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        V9020 = if integer(dec(V1600) / dec(V9010)) > (dec(V1600) / dec(V9010)) then string(integer(dec(V1600) / dec(V9010)) - 1) else string(integer(dec(V1600) / dec(V9010))).
        V9020 = ENTRY(1,V9020,"@").
        /* --FIRST TIME DEFAULT  VALUE -- END  */
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9020 width 150 no-box.
        ...
        /* PRESS e EXIST CYCLE */
        IF V9020 = "e" THEN  LEAVE V1600LMAINLOOP.
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        IF not (dec(v1600) >= dec(v9010) * dec(v9020)) THEN DO:
                display skip "总数超过!" @ WMESSAGE NO-LABEL with fram F9020.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
     ...
     END.
     PV9020 = V9020.
     /* END    LINE :9020  箱数[NO OF LABEL]  */


   wtm_num = V9020.
     /* START  LINE :9021  尾数数量[QTY ON LABEL]  */
     V9021L:
     REPEAT:

        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V9021           as char format "x(150)".
        define variable PV9021          as char format "x(150)".
        define variable L90211          as char format "x(40)".
        define variable L90212          as char format "x(40)".
        define variable L90213          as char format "x(40)".
        define variable L90214          as char format "x(40)".
        define variable L90215          as char format "x(40)".
        define variable L90216          as char format "x(40)".
        /* --DEFINE VARIABLE -- END */


        /* --FIRST TIME DEFAULT  VALUE -- START  */
        V9021 = string(dec(V1600) - ( dec(V9010) * dec(V9020) ) ).
        V9021 = ENTRY(1,V9021,"@").
        /* --FIRST TIME DEFAULT  VALUE -- END  */
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9021 width 150 no-box.

                /* LABEL 1 - START */ 
                L90211 = "尾数数量?" .
                display L90211          format "x(40)" skip with fram F9021 width 150 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                  L90212 = "" . 
                display L90212          format "x(40)" skip with fram F9021 width 150 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                  L90213 = "" . 
                display L90213          format "x(40)" skip with fram F9021 width 150 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                  L90214 = "" . 
                display L90214          format "x(40)" skip with fram F9021 width 150 no-box.
                /* LABEL 4 - END */ 
                display "输入或按E退出"       format "x(40)" skip
        skip with fram F9021 width 150 no-box.
        Update V9021
        WITH  fram F9021 NO-LABEL
        EDITING:
          readkey pause wtimeout.
          if lastkey = -1 Then quit.
        if LASTKEY = 404 Then Do: /* DISABLE F4 */
        ...
        /* PRESS e EXIST CYCLE */
        IF V9021 = "e" THEN  LEAVE V1600LMAINLOOP.
        display  skip WMESSAGE NO-LABEL with fram F9021.

         /*  ---- Valid Check ---- START */

        display "...PROCESSING...  " @ WMESSAGE NO-LABEL with fram F9021.
        pause 0.
        /* CHECK FOR NUMBER VARIABLE START  */
        If V9021 = "" OR V9021 = "-" OR V9021 = "." OR V9021 = ".-" OR V9021 = "-." then do:
                display skip "CAN NOT EMPTY  " @ WMESSAGE NO-LABEL with fram F9021.
                pause 0 before-hide.
                undo, retry.
        end.
        DO i = 1 to length(V9021).
                If index("0987654321.-", substring(V9021,i,1)) = 0 then do:
                display skip "Format  Error  " @ WMESSAGE NO-LABEL with fram F9021.
                pause 0 before-hide.
                undo, retry.
                end.
        end.
        ...
        display  "" @ WMESSAGE NO-LABEL with fram F9021.
        pause 0.
        leave V9021L.
     END.
     PV9021 = V9021.
     /* END    LINE :9021  尾数数量[QTY ON LABEL]  */
...
If AVAILABLE ( upd_det ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9030 width 150 no-box.
        ...
        /* PRESS e EXIST CYCLE */
        IF V9030 = "e" THEN  LEAVE V1600LMAINLOOP.
        display  skip WMESSAGE NO-LABEL with fram F9030.
     ...
     INPUT FROM VALUE(LabelsPath + "lap09").
     ...
     Do While True:
       ...
       find first pt_mstr where pt_domain = V1001 and pt_part = trim( IF V1200 <> "" THEN V1310 ELSE V1300 )  no-lock no-error.
If AVAILABLE ( pt_mstr )  then
       ...
       END.
        av9030 = IF V1200 <> "" THEN V1310 ELSE V1300.
...
If AVAILABLE ( pt_mstr )  then
       ...
       END.
        av9030 = IF V1200 <> "" THEN V1510 ELSE V1500.
       ...
       END.
        av9030 = trim(IF V1200 <> "" THEN V1310 ELSE V1300) + "@" + trim(IF V1200 <> "" THEN V1510 ELSE V1500).
...
If 1 = 1 then
        leave V9040L.
        /* LOGICAL SKIP END */
                display "[库存标签分箱]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9040 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F9040.
            ...
            THEN DO:
                  ...
                  IF AVAILABLE PRD_DET then display skip 
            PRD_DEV @ V9040 PRD_DESC @ WMESSAGE NO-LABEL with fram F9040.
                  else   display skip "" @ WMESSAGE with fram F9040.
            END.
            IF LASTKEY = keycode("F9") or keyfunction(lastkey) = "CURSOR-UP"
            THEN DO:
                  IF recid(PRD_DET) = ? THEN find last PRD_DET where 
        ...
        /* PRESS e EXIST CYCLE */
        IF V9040 = "e" THEN  LEAVE V1600LMAINLOOP.
        display  skip WMESSAGE NO-LABEL with fram F9040.
     ...
     END.
     PV9040 = V9040.
     /* END    LINE :9040  打印机[PRINTER]  */


     Define variable ts9040 AS CHARACTER FORMAT "x(100)".
     Define variable av9040 AS CHARACTER FORMAT "x(100)".
     PROCEDURE lap099040l.
        /* Define Labels Path  Start */
        Define variable LabelsPath as character format "x(100)" init "/app/bc/labels/".
        Find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="LabelsPath"no-lock no-error.
        If AVAILABLE(code_mstr) Then LabelsPath = trim ( code_cmmt ).
     ...
     output to value( trim(wsection) + ".l") .
     Do While True:
              IMPORT UNFORMATTED ts9040.
       find first pt_mstr where pt_domain = V1001 and pt_part = trim( IF V1200 <> "" THEN V1310 ELSE V1300 )  no-lock no-error.
If AVAILABLE ( pt_mstr )  then
        av9040 = trim(pt_desc1).
       IF INDEX(ts9040,"$F") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "$F") - 1) + av9040 
       + SUBSTRING( ts9040 , index(ts9040 ,"$F") + length("$F"), LENGTH(ts9040) - ( index(ts9040 ,"$F" ) + length("$F") - 1 ) ).
       END.
        av9040 = IF V1200 <> "" THEN V1310 ELSE V1300.
       IF INDEX(ts9040,"$P") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "$P") - 1) + av9040 
       + SUBSTRING( ts9040 , index(ts9040 ,"$P") + length("$P"), LENGTH(ts9040) - ( index(ts9040 ,"$P" ) + length("$P") - 1 ) ).
       END.
       find first pt_mstr where pt_domain = V1001 and pt_part = trim( IF V1200 <> "" THEN V1310 ELSE V1300 )  no-lock no-error.
If AVAILABLE ( pt_mstr )  then
        av9040 = pt_um.
       IF INDEX(ts9040,"$U") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "$U") - 1) + av9040 
       + SUBSTRING( ts9040 , index(ts9040 ,"$U") + length("$U"), LENGTH(ts9040) - ( index(ts9040 ,"$U" ) + length("$U") - 1 ) ).
       END.
        av9040 = IF V1200 <> "" THEN V1510 ELSE V1500.
       IF INDEX(ts9040,"$L") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "$L") - 1) + av9040 
       + SUBSTRING( ts9040 , index(ts9040 ,"$L") + length("$L"), LENGTH(ts9040) - ( index(ts9040 ,"$L" ) + length("$L") - 1 ) ).
       END.
        av9040 = trim(IF V1200 <> "" THEN V1310 ELSE V1300) + "@" + trim(IF V1200 <> "" THEN V1510 ELSE V1500).
       IF INDEX(ts9040,"&B") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "&B") - 1) + av9040 
       + SUBSTRING( ts9040 , index(ts9040 ,"&B") + length("&B"), LENGTH(ts9040) - ( index(ts9040 ,"&B" ) + length("&B") - 1 ) ).
       END.
        av9040 = " ".
       IF INDEX(ts9040,"&R") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "&R") - 1) + av9040 
       + SUBSTRING( ts9040 , index(ts9040 ,"&R") + length("&R"), LENGTH(ts9040) - ( index(ts9040 ,"&R" ) + length("&R") - 1 ) ).
       END.
        av9040 = V9010.
       IF INDEX(ts9040,"$Q") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "$Q") - 1) + av9040 
       + SUBSTRING( ts9040 , index(ts9040 ,"$Q") + length("$Q"), LENGTH(ts9040) - ( index(ts9040 ,"$Q" ) + length("$Q") - 1 ) ).
       END.
       find first pt_mstr where pt_domain = V1001 and pt_part = V1300  no-lock no-error.
If AVAILABLE ( pt_mstr )  then
        av9040 = if pt_avg_int <> 0 and pt_avg_int <> 90 then "保质期:" + trim ( string ( pt_avg_int ) ) + "月" else "".
       IF INDEX(ts9040,"&D") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "&D") - 1) + av9040 
       + SUBSTRING( ts9040 , index(ts9040 ,"&D") + length("&D"), LENGTH(ts9040) - ( index(ts9040 ,"&D" ) + length("&D") - 1 ) ).
       END.
        av9040 = string(today).
       IF INDEX(ts9040,"$D") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "$D") - 1) + av9040 
       ...
       find first pt_mstr where pt_domain = V1001 and pt_part = trim( IF V1200 <> "" THEN V1310 ELSE V1300 )  no-lock no-error.
If AVAILABLE ( pt_mstr )  then
        av9040 = trim(pt_desc2).
       IF INDEX(ts9040,"$E") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "$E") - 1) + av9040 
       + SUBSTRING( ts9040 , index(ts9040 ,"$E") + length("$E"), LENGTH(ts9040) - ( index(ts9040 ,"$E" ) + length("$E") - 1 ) ).
       END.
        av9040 = " ".
       IF INDEX(ts9040,"$G") <> 0  THEN DO:
       TS9040 = substring(TS9040, 1, Index(TS9040 , "$G") - 1) + av9040 
       + SUBSTRING( ts9040 , index(ts9040 ,"$G") + length("$G"), LENGTH(ts9040) - ( index(ts9040 ,"$G" ) + length("$G") - 1 ) ).
       END.
       put unformatted ts9040 skip.
     End.
     INPUT CLOSE.
     OUTPUT CLOSE.
     unix silent value ("chmod 777  " + trim(wsection) + ".l").
     ...
     run lap099040l.
     do i = 1 to integer(wtm_num):
       find first PRD_DET where PRD_DEV = V9040 no-lock no-error.
       IF AVAILABLE PRD_DET then do:
         unix silent value (trim(prd_path) + " " + trim(wsection) + ".l").
         unix silent value ( "clear").
       end.
