...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsrep02wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 width 150 no-box.
...
If AVAILABLE ( lad_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1105 width 150 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 width 150 no-box.
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 width 150 no-box.
...
ld_ref  = ""     and
ld_loc <> V1105  and 
ld_loc = "P-4RPS" and ld_qty_oh <> 0 use-index ld_part_lot no-lock no-error.
If AVAILABLE ( ld_det ) then
...
LD_LOC <> V1105 AND  
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1510 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1510.
            ...
            THEN DO:
                  IF recid(LD_DET) = ? THEN find first LD_DET where 
                              LD_DOMAIN = V1001 AND LD_PART = V1300 AND LD_QTY_OH <> 0  AND
LD_SITE = V1002 AND LD_REF = ""     AND 
LD_LOT = V1500  AND LD_LOC <> V1105 AND ld_loc = "P-4RPS" AND  
                              LD_LOC >=  INPUT V1510
                               no-lock no-error.
                  else do: 
                       ...
                       WHERE LD_DOMAIN = V1001 AND LD_PART = V1300 AND LD_QTY_OH <> 0  AND
LD_SITE = V1002 AND LD_REF = ""     AND 
LD_LOT = V1500  AND LD_LOC <> V1105 AND ld_loc = "P-4RPS"
                        no-lock no-error.
                        else find first LD_DET where 
                              LD_DOMAIN = V1001 AND LD_PART = V1300 AND LD_QTY_OH <> 0  AND
LD_SITE = V1002 AND LD_REF = ""     AND 
...
LD_LOT = V1500  AND LD_LOC <> V1105 AND ld_loc = "P-4RPS" AND  
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LOC_MSTR where LOC_DOMAIN = V1001 AND LOC_LOC = V1510 AND LOC_SITE = V1002 AND loc_loc = "P-4RPS"  no-lock no-error.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1520 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 width 150 no-box.
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where ( ld_part  = V1300 AND ld_lot = V1500 AND ld_domain = V1001 AND  ld_site = V1002 AND ld_ref = "") AND ( ( decimal(V1600) < 0  AND ( ld_loc = V1520 AND  LD_SITE = V1002 and ld_ref = "" and ld_QTY_oh >= - DECIMAL ( V1600 ) ) ) OR ( decimal(V1600) > 0 AND  ld_loc = V1510 AND  LD_SITE = V1002 and ld_ref = "" and  ld_QTY_oh >= DECIMAL ( V1600 ) ) )  no-lock no-error.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 width 150 no-box.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first ld_det where ld_part  = V1300 AND ld_domain = V1001 AND
ld_site = V1002 AND 
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.
...
If NOT AVAILABLE ( tr_hist ) then
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 width 150 no-box.
   ...
   wtm_num = V9120.
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find first upd_det where upd_nbr = "REP02" and upd_select = 99 no-lock no-error.
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[重复生产备料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 width 150 no-box.
     ...
     INPUT FROM VALUE(LabelsPath + "rep02").
