...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xssoi10wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 width 150 no-box.
...
If AVAILABLE ( so_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1105 width 150 no-box.
...
If AVAILABLE (sod_det) and string ( sod_line ) = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
...
If AVAILABLE ( so_mstr ) then
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first SOD_DET where SOD_DOMAIN = V1001 AND SOD_NBR = V1100 AND SOD_LINE = integer ( V1110 ) AND SOD_PART = ENTRY(1, V1300, "@")  no-lock no-error.
        IF NOT AVAILABLE SOD_DET then do:
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 width 150 no-box.
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 width 150 no-box.
...
If AVAILABLE ( ld_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1550 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 width 150 no-box.
                ...
                /* LABEL 4 - START */ 
                find first ld_det where ld_domain = V1001 and ld_part = V1300 and ld_site = V1002 and 
ld_ref = ""     and 
ld_loc = V1400  and 
...
If AVAILABLE ( ld_det ) then
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where ld_part  = V1300 AND ld_loc = V1400 and ld_domain = V1001 and 
ld_site = V1002 and ld_ref = "" and  ld_lot = V1500 and  ld_QTY_oh >= DECIMAL ( V1600 )  no-lock no-error.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 width 150 no-box.
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 width 150 no-box.
...
If 1 = 1 then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9005 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.
...
If NOT AVAILABLE ( tr_hist ) then
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 width 150 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[销售订单发货]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 width 150 no-box.
     ...
     INPUT FROM VALUE(LabelsPath + "soi10").
...
If AVAILABLE ( pt_mstr )  then
       ...
       END.
       find first cp_mstr where cp_domain = V1001 and cp_cust = V1105 and cp_cust_part <> "" and cp_part = V1300 no-lock no-error.
