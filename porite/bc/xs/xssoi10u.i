...
usection = TRIM ( string(year(TODAY)) + string(MONTH(TODAY)) + string(DAY(TODAY)))  + trim(STRING(TIME)) + trim(string(RANDOM(1,100))) + "soi10" .
output to value( trim(usection) + ".i") .
        display 
trim ( V1100 ) format "x(50)" " - N N " format "x(50)" skip
trim (V1110) format "x(50)"  skip
