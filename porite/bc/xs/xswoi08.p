...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xswoi08wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 width 150 no-box.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 width 150 no-box.
...
If AVAILABLE ( wo_mstr ) then
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 width 150 no-box.
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        V1500 = ( if ENTRY(2, PV1300, "@") = "" then "00000000" else ENTRY(2, PV1300, "@") ).
        V1500 = ENTRY(1,V1500,"@").
        /* --FIRST TIME DEFAULT  VALUE -- END  */
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1500.
            ...
            THEN DO:
                  IF recid(LD_DET) = ? THEN find first LD_DET where 
                              LD_DOMAIN = V1001 AND LD_PART = V1300 AND LD_LOC = V1400 AND 
LD_QTY_OH <> 0 AND LD_SITE = V1002 AND LD_REF = "" AND  
                              LD_LOT >=  INPUT V1500
                               no-lock no-error.
                  else do: 
                       if LD_LOT =  INPUT V1500
                       then find next LD_DET
                       WHERE LD_DOMAIN = V1001 AND LD_PART = V1300 AND LD_LOC = V1400 AND 
LD_QTY_OH <> 0 AND LD_SITE = V1002 AND LD_REF = ""
                        no-lock no-error.
...
LD_QTY_OH <> 0 AND LD_SITE = V1002 AND LD_REF = "" AND  
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 width 150 no-box.
...
If AVAILABLE ( ld_det ) then
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where decimal(V1600) > 0 OR ( decimal(V1600) < 0 AND (ld_part  = V1300 AND ld_domain = V1001 AND ld_site = V1002 AND ld_ref = "" AND  ld_loc = V1400 and ld_lot = V1500 and  ld_QTY_oh + DECIMAL ( V1600 ) >= 0 ) )  no-lock no-error.
        IF NOT AVAILABLE LD_DET then do:
                display skip "该操作导致库存小於 0 " @ WMESSAGE NO-LABEL with fram F1600.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 width 150 no-box.
...
If AVAILABLE ( wod_det ) then
                L17004 = "工单已发数量:" + string ( wod_qty_iss ) .
                else L17004 = "" . 
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 width 150 no-box.
...
IF 1 = 2 THEN
        leave V9010L.
        /* LOGICAL SKIP END */
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.
...
If NOT AVAILABLE ( tr_hist ) then
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 width 150 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 width 150 no-box.
     ...
     INPUT FROM VALUE(LabelsPath + "woi08").
