...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xswoi09wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 width 150 no-box.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first WO_MSTR where WO_DOMAIN = V1001 AND WO_NBR = V1100 AND WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0  NO-ERROR NO-WAIT.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 width 150 no-box.
                ...
                /* LABEL 2 - START */ 
                find first wo_mstr where wo_domain = V1001 and wo_nbr  = V1100 and wo_site = V1002 and      no-lock no-error.
If AVAILABLE ( wo_mstr ) then
                L11102 = wo_lot .
                else L11102 = "" . 
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
...
If AVAILABLE (wod_det) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 width 150 no-box.
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1500.
            ...
            THEN DO:
                  IF recid(LD_DET) = ? THEN find first LD_DET where 
                              LD_DOMAIN = V1001 AND LD_PART = V1300 AND
LD_SITE = V1002 AND  LD_LOC = V1400 AND LD_QTY_OH <> 0 AND LD_REF = "" AND  
                              LD_LOT >=  INPUT V1500
                               no-lock no-error.
                  else do: 
                       if LD_LOT =  INPUT V1500
                       then find next LD_DET
                       WHERE LD_DOMAIN = V1001 AND LD_PART = V1300 AND
LD_SITE = V1002 AND  LD_LOC = V1400 AND LD_QTY_OH <> 0 AND LD_REF = ""
                        no-lock no-error.
...
ld_site = V1002 and ld_loc = V1400  and
ld_lot = V1500  and ld_qty_oh <> 0  and ld_ref = ""  no-lock no-error.
        IF NOT AVAILABLE LD_DET then do:
                display skip "批号有误" @ WMESSAGE NO-LABEL with fram F1500.
                pause 0 before-hide.
                undo, retry.
        end.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 width 150 no-box.
...
If AVAILABLE ( ld_det ) then
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where ld_domain = V1001 and ld_part  = V1300 AND 
ld_site = V1002 AND ld_ref = "" AND  ld_loc = V1400 and ld_lot = V1500 and  ld_QTY_oh >= DECIMAL ( V1600 )  no-lock no-error.
        IF NOT AVAILABLE LD_DET then do:
                display skip "在库数 <: " + string( V1600 ) + "不可超量" @ WMESSAGE NO-LABEL with fram F1600.
                pause 0 before-hide.
                undo, retry.
        end.
        IF not CAN-FIND(FIRST wod_det where wod_domain = V1001 and wod_lot = V1110 and wod_part = V1300 and wod_qty_req - wod_qty_iss >= DECIMAL ( V1600 ) ) THEN DO:
                display skip "在库数 <: " + string( V1600 ) + "不可超量" @ WMESSAGE NO-LABEL with fram F1600.
                pause 0 before-hide.
                undo, retry.
        end.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 width 150 no-box.
                ...
                find first wod_det where  wod_domain = V1001 and wod_lot = V1110 and wod_part = V1300  no-lock no-error.
If AVAILABLE ( wod_det ) then
                L17004 = "工单短缺数量:" + ( if wod_qty_req <= wod_qty_iss then "0" else ( string ( wod_qty_req - wod_qty_iss ) ) ) .
                else L17004 = "" . 
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.
...
tr_date = today     and 
tr_trnbr > integer ( V9000 ) and 
tr_nbr  = V1100     and  tr_type = "ISS-WO"  and  tr_site = V1002     and tr_lot  = V1110     and tr_part = V1300     and tr_serial = V1500   and 
tr_time  + 15 >= TIME 
use-index tr_date_trn no-lock no-error.
If AVAILABLE ( tr_hist ) then
...
If NOT AVAILABLE ( tr_hist ) then
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 width 150 no-box.
   ...
   wtm_num = V9120.
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find first upd_det where upd_nbr = "WOI09" and upd_select = 99 no-lock no-error.
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单限量发料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 width 150 no-box.
     ...
     INPUT FROM VALUE(LabelsPath + "woi09").
