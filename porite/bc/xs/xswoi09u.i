...
usection = TRIM ( string(year(TODAY)) + string(MONTH(TODAY)) + string(DAY(TODAY)))  + trim(STRING(TIME)) + trim(string(RANDOM(1,100))) + "woi09" .
output to value( trim(usection) + ".i") .
        display 
"- " + trim ( V1110 ) format "x(50)" " - - N N " format "x(50)" skip
trim ( V1300) format "x(50)" skip
trim ( V1600 ) + " - - - " +  trim( V1400 ) format "X(50)"  if trim( V1500 ) = "" then " - " else trim(V1500)  format "X(50)" skip
"." skip
"Y" skip
"Y" skip
"."
