...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xswor01wtimeout"
 and can-find ( first dom_mstr where  dom_type = "SYSTEM " and dom_domain = code_domain and dom_active = yes ) 
 no-lock no-error. /*  Timeout - All Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 width 150 no-box.
     ...
     /* START  LINE :1005  入仓单号[ORDER]  */
     V1005L:
     REPEAT:

        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V1005           as char format "x(150)".
        define variable PV1005          as char format "x(150)".
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1005 width 150 no-box.

                /* LABEL 1 - START */ 
                L10051 = "入仓单号?" .
                display L10051          format "x(40)" skip with fram F1005 width 150 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                  L10052 = "" . 
                display L10052          format "x(40)" skip with fram F1005 width 150 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                  L10053 = "" . 
                display L10053          format "x(40)" skip with fram F1005 width 150 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                  L10054 = "" . 
                display L10054          format "x(40)" skip with fram F1005 width 150 no-box.
                /* LABEL 4 - END */ 
                display "输入或按E退出"       format "x(40)" skip
        skip with fram F1005 width 150 no-box.
        Update V1005
        ...
        IF not ( length(V1005) <= 10 and length(V1005) > 0 ) THEN DO:
                display skip "入仓单号格式有误!" @ WMESSAGE NO-LABEL with fram F1005.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 width 150 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1100.
            ...
            THEN DO:
                  IF recid(WO_MSTR) = ? THEN find first WO_MSTR where 
                              WO_DOMAIN = V1001 AND WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0 AND  ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" ) AND  
                              WO_NBR >=  INPUT V1100
                               no-lock no-error.
                  else do: 
                       if WO_NBR =  INPUT V1100
                       then find next WO_MSTR
                       WHERE WO_DOMAIN = V1001 AND WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0 AND  ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" )
                        no-lock no-error.
            ...
            WO_NBR @ V1100 trim( WO_PART ) + "/" + trim (wo_lot) @ WMESSAGE NO-LABEL with fram F1100.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first WO_MSTR where WO_DOMAIN = V1001 AND WO_NBR = V1100 AND WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" )  NO-ERROR NO-WAIT.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 width 150 no-box.
...
If AVAILABLE ( wo_mstr ) then
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1110.
            ...
            THEN DO:
                  IF recid(wo_mstr) = ? THEN find first wo_mstr where 
                              wo_domain = V1001 and wo_site = V1002 AND wo_nbr = V1100 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" ) AND  
                              wo_lot >=  INPUT V1110
                               no-lock no-error.
                  else do: 
                       if wo_lot =  INPUT V1110
                       then find next wo_mstr
                       WHERE wo_domain = V1001 and wo_site = V1002 AND wo_nbr = V1100 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" )
                        no-lock no-error.
                        else find first wo_mstr where 
                              wo_domain = V1001 and wo_site = V1002 AND wo_nbr = V1100 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" ) AND  
                              wo_lot >=  INPUT V1110
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first wo_mstr where wo_domain = V1001 and wo_site = V1002 AND wo_lot = V1110 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" )  NO-ERROR NO-WAIT.
        IF NOT AVAILABLE wo_mstr then do:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first WO_MSTR where WO_DOMAIN = V1001 AND WO_LOT = V1110 AND WO_PART = ENTRY(1, V1300, "@")  no-lock no-error.
        IF NOT AVAILABLE WO_MSTR then do:
                display skip "图号不匹配!" @ WMESSAGE NO-LABEL with fram F1300.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 width 150 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 width 150 no-box.
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 width 150 no-box.
...
If AVAILABLE ( wo_mstr ) then
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where ( decimal(V1600) > 0 ) OR ( decimal(V1600) < 0 AND (ld_domain = V1001 and ld_site = V1002 and ld_ref = "" AND ld_part  = V1300 AND ld_loc = V1400 and ld_lot = V1500 and  ld_QTY_oh + DECIMAL ( V1600 ) >= 0 ) )  no-lock no-error.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 width 150 no-box.
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 width 150 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入库]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 width 150 no-box.

                /* LABEL 1 - START */ 
                find last tr_hist where tr_domain = V1001 and 
tr_date = today     and 
tr_trnbr > integer ( V9000 ) and 
tr_nbr  = V1100     and  tr_type = "RCT-WO"  and  tr_lot  = V1110     and 
