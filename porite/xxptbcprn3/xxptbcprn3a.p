...
procedure mycreatelabel:
  ...
  IF NOT (SEARCH(LabelsPath + "poriteProd1") <> ?) THEN DO:
     /* 文件不存在 */
  ...
  INPUT FROM VALUE(LabelsPath + "poriteProd").
  REPEAT:
     CREATE tt.
  ...
  for each mypack:
    ...
    do while curpage < mypages :
      ...
      for each tt:
        create tt1 .
	tt1_c1 = tt_c1.
        if index(tt1_c1,"2011-5-29") > 0  then do:
  			tt1_c1 = replace(tt1_c1,"2011-5-29", string(year(mydate)) + "-" + string(month(mydate)) + "-" + string(day(mydate))  ).
			next.
        end.
        if index(tt1_c1,"AAAAAAAA") > 0  then do:
  			tt1_c1 = replace(tt1_c1,"AAAAAAAA", mycustpart  ).
			next.
        end.
	if index(tt1_c1,"BBBBBBBBBBBBBBBBBB") > 0  then do:
  			tt1_c1 = replace(tt1_c1,"BBBBBBBBBBBBBBBBBB", mypart   ).
			next.
        end.

	if index(tt1_c1,"99999") > 0  then do:
	  if curpage * 3 + 1 <= mypa_lots then 
  			tt1_c1 = replace(tt1_c1,"99999", string(mypa_num[ curpage * 3 + 1] )  ).
	  else tt1_c1 = replace(tt1_c1,"99999", ""  ).
	  next.
	end.
        if index(tt1_c1,"88888") > 0  then do:
	  if curpage * 3 + 2 <= mypa_lots then 
  			tt1_c1 = replace(tt1_c1,"88888", string(mypa_num[ curpage * 3 + 2] )  ).
	  else tt1_c1 = replace(tt1_c1,"88888", ""  ).
	  next.
	end.
        if index(tt1_c1,"77777") > 0  then do:
	  if curpage * 3 + 3 <= mypa_lots then 
  			tt1_c1 = replace(tt1_c1,"77777", string(mypa_num[ curpage * 3 + 3] )  ).
	  else tt1_c1 = replace(tt1_c1,"77777", ""  ).
	  next.
	end.
        if index(tt1_c1,"333333333333333") > 0  then do:
	  if curpage * 3 + 1 <= mypa_lots then 
  			tt1_c1 = replace(tt1_c1,"333333333333333", mypa_lot[ curpage * 3 + 1] + "@ " +  string(mypa_num[ curpage * 3 + 1] )  ).
	  else delete tt1. next.
	end.
        if index(tt1_c1,"222222222222222") > 0  then do:
	  if curpage * 3 + 2 <= mypa_lots then 
  			tt1_c1 = replace(tt1_c1,"222222222222222", mypa_lot[ curpage * 3 + 2] + "@ " +  string(mypa_num[ curpage * 3 + 2] )  ).
	  else delete tt1. next.
        end.
	if index(tt1_c1,"111111111111111") > 0  then do:
	  if curpage * 3 + 3 <= mypa_lots then 
  			tt1_c1 = replace(tt1_c1,"111111111111111", mypa_lot[ curpage * 3 + 3] + "@ " +  string(mypa_num[ curpage * 3 + 3] )  ).
	  else delete tt1. next.
	end.
	
	if index(tt1_c1,"00000") > 0  then do:
	  if ( curpage + 1 ) = mypages then    /* 本箱最后一张标签 */
  			tt1_c1 = replace(tt1_c1,"00000", string( mypa_qty )  ).
          else tt1_c1 = replace(tt1_c1,"00000", "").
	  next.
	end.
	if index(tt1_c1,"66666") > 0  then do:
	  tt1_c1 = replace(tt1_c1,"66666", string( myweight ) + "g" ).          
	  next.
	end.
      end.
      curpage = curpage + 1.
      /* 打印标签 */
       usection =  "LABOR_Prn_Prod"  + mfguser + "_" + TRIM ( string(year(TODAY)) + string(MONTH(TODAY)) + string(DAY(TODAY)))  + trim(STRING(TIME)) + trim(string(RANDOM(1,10)))  .
       ...
       output close.
       os-command silent value("lp -dbc248 " + usection  + "  > /dev/null") .
       /*
