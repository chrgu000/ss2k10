   ...
   if cn_date1 = hi_date then cn_date1 = ? .
    ...
    /* 
     if UP_qty_oh_fir  = yes and day(cn_date1) < 20 then 
       do : message "结转时至日期 >= 20 " .
       undo , retry .
       end.
  
     if mm > 12 or yy < 2011 then 
     do :
       message "当期年份<2011 或 当期月份>12 " .
       undo , retry .
     end.
     */
...
for each vendp where vendp_qty_con <> 0 and thism = yes and (cost# = 1 or cost# = 3)
  ...
  if first-of(vendp_vend) then assign s_qty_con = 0 s_qty_potr = 0  s_nub = 0 ii = 0 .       
   ...
   do :
      display "小计:" @ xxcn_part  II @ pt_desc1 
...
*/
  ...
  do: 
   xxxcode1 = "" .
   message "注册码" update xxxcode1 .
   qad_key3 = xxxcode1 .
   if xxxcode1 <> "A123890C" then do : message "不许可!" .
                                  pause(3) .
                                  return .
                                end.
  end.
  */
  ...
  /*
  if thism = yes and yn = yes then do: message "不能只选择耗用<>0的数据,当更新数据时" .
                         pause(3) .
                         return .
                      end.  
  */       
