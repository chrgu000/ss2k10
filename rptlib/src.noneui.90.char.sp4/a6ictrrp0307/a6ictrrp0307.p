...
/* REVISION: 9.0      LAST MODIFIED: 08/25/05   BY: *SS - 20050825* Bill Jiang       */
/* REVISION: 9.0      LAST MODIFIED: 08/30/05   BY: *SS - 20050830* Bill Jiang       */
     ...
     */
     ...
     /* SS - 20050830 - B */
     DEFINE VARIABLE p LIKE pt_part.
     /* SS - 20050830 - E */
    /* SS - 20050713 - E */
...
/* ********** End Translatable Strings Definitions ********* */
     ...
     */
     define INPUT PARAMETER efdate like tr_effdate.
     define INPUT PARAMETER efdate1 like tr_date.
     define INPUT PARAMETER trtype like tr_type.
     ...
     DEFINE INPUT PARAMETER acc1 LIKE gltr_acc.
     DEFINE INPUT PARAMETER sub LIKE gltr_sub.
     DEFINE INPUT PARAMETER sub1 LIKE gltr_sub.
     DEFINE INPUT PARAMETER ctr LIKE gltr_ctr.
     DEFINE INPUT PARAMETER ctr1 LIKE gltr_ctr.
     /* SS - 20050809 - E */
     /* SS - 20050713 - E */
...
/* SS - 20050825 - B */
/*GF20*/    if entity1  = hi_char then entity1  = "".
/*GF20*/    if acc1  = hi_char then acc1  = "".
/*GF20*/    if sub1  = hi_char then sub1  = "".
/*GF20*/    if ctr1    = hi_char then ctr1    = "".
/* SS - 20050825 - E */
...
/*F268*/    {mfquoter.i gl_yn          }
            /* SS - 20050825 - B */
/*F268*/    {mfquoter.i entity             }
/*F268*/    {mfquoter.i entity1            }
/*F268*/    {mfquoter.i acc           }
/*F268*/    {mfquoter.i acc1          }
/*F268*/    {mfquoter.i sub             }
/*F268*/    {mfquoter.i sub1            }
/*F268*/    {mfquoter.i ctr             }
/*F268*/    {mfquoter.i ctr1            }
            /* SS - 20050825 - E */
...
/* SS - 20050825 - B */
/*GF20*/    if entity1  = "" then entity1  = hi_char.
/*GF20*/    if acc1  = "" then acc1  = hi_char.
/*GF20*/    if sub1  = "" then sub1  = hi_char.
/*GF20*/    if ctr1    = "" then ctr1    = hi_char.
/* SS - 20050825 - E */
...
/*F268*/      and (tr_type = trtype or trtype = "")
    */
/*F268*/    where (tr_type = trtype or trtype = "")
...
/*F268*/    where trgl_trnbr = tr_trnbr
    /* SS - 20050830 - B */
/*F268*/      and (
/*F268*/           (trgl_dr_acct >= acc and trgl_dr_acct <= acc1)
/*F268*/            or (trgl_cr_acct >= acc and trgl_cr_acct <= acc1)
/*F268              or (acct1 = "")         */
/*F268*/          )
/*F268*/      and (
/*F268*/           (trgl_dr_cc >= ctr and trgl_dr_cc <= ctr1)
/*F268*/            or (trgl_cr_cc >= ctr and trgl_cr_cc <= ctr1)
/*F268              or (cc1 = "")         */
/*F268*/           )
    /* SS - 20050830 - E */
    /* SS - 20050713 - B */
...
/*F268*/      and (trgl_gl_amt <> 0)
    ...
    ,EACH gltr_hist
    NO-LOCK
    WHERE gltr_ref = trgl_gl_ref
    AND gltr_rflag = FALSE
    /* SS - 20050809 - B */
    AND (gltr_line = trgl_dr_line OR gltr_line = trgl_cr_line)
    /* SS - 20050809 - E */
    ...
    */
    ...
    /* SS - 20050809 - E */
    /*
/*F268*/    break by tr_effdate by tr_trnbr 
    */
    /* SS - 20050713 - E */
...
/*F268*/       end. /*if tr_yn*/
    ...
    /*
    FIND FIRST tttrgl 
        WHERE tttrgl_inv_nbr = tr_rmks
        AND tttrgl_nbr = tr_nbr
        AND tttrgl_line = tr_line
        AND tttrgl_acct = trgl_dr_acct 
        NO-LOCK 
        NO-ERROR
        .
    IF AVAILABLE tttrgl THEN DO:
        ASSIGN
            tttrgl_base_price = tttrgl_base_price + trgl_gl_amt
            .
    END.
    ELSE DO:
        CREATE tttrgl.
        ASSIGN
            tttrgl_inv_nbr = tr_rmks
            tttrgl_nbr = tr_nbr
            tttrgl_line = tr_line
            tttrgl_acct = trgl_dr_acct
            tttrgl_cc = trgl_dr_cc
            tttrgl_base_price = trgl_gl_amt
            .
    END.
    */
    /* SS - 20050830 - B */
    ...
    FOR FIRST tta6ictrrp0307
        WHERE tta6ictrrp0307_site = tr_site
        AND tta6ictrrp0307_part = p
        AND tta6ictrrp0307_nbr = tr_nbr
        AND tta6ictrrp0307_lot = tr_lot
        NO-LOCK:
    END.
    IF NOT AVAILABLE tta6ictrrp0307 THEN DO:
        CREATE tta6ictrrp0307.
        ASSIGN
            tta6ictrrp0307_site = tr_site
            tta6ictrrp0307_part = p
            tta6ictrrp0307_nbr = tr_nbr
            tta6ictrrp0307_lot = tr_lot
            .
    END.      
    IF  (trgl_dr_acct >= acc and trgl_dr_acct <= acc1) THEN DO:
        ASSIGN
            tta6ictrrp0307_amt_dr = tta6ictrrp0307_amt_dr + trgl_gl_amt
            .
    END.
    ELSE DO:
        ASSIGN
            tta6ictrrp0307_amt_cr = tta6ictrrp0307_amt_cr - trgl_gl_amt
            .
    END.
    /*
    IF gltr_amt > 0 THEN DO:
        ASSIGN
            tta6ictrrp0307_amt_dr = tta6ictrrp0307_amt_dr + gltr_amt
            .
    END.
    ELSE DO:
        ASSIGN
            tta6ictrrp0307_amt_cr = tta6ictrrp0307_amt_cr + gltr_amt
            .
    END.
    */
    /* SS - 20050830 - E */
