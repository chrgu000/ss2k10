...
/* $Revision: 1.23 $      BY: Bill Jiang  DATE: 10/17/05 ECO: *SS - 20051017*            */
...
/*V8!
     ...
     define variable next-handle as widget-handle no-undo.
*/
   ...
   repeat:
      ...
      &IF defined(BATCHID) &THEN
         ...
         do:
            for first qad_wkfl
                  ...
                  where (lookup(qad_key1, execname, "~/") <> 0
                  or     lookup(qad_key1, execname, "~\") <> 0)
      ...
      */
      ...
      repeat while local-handle <> ?:
         if local-handle:type = "literal" or local-handle:type = "text"
         then next-handle = local-handle:next-sibling.
         if not valid-handle(next-handle)
         then next-handle = local-handle.

         if local-handle:type = "fill-in"
            or ((local-handle:type = "literal")
            and (next-handle:label = ? or next-handle:label = ""))
            or ((local-handle:type = "text")
            and (next-handle:label = ? or next-handle:label = ""))
         then do:
            &IF DEFINED(BATCHID) &THEN
