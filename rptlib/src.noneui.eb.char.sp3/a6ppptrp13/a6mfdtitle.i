...
/* EXTERNALIZED LABEL INCLUDE. */ /*N08T*/
{gplabel.i &ClearReg = yes}       /*N08T*/
...
if substring(dtitle,1,1) >= "0" and substring(dtitle,1,1) <= "9" then
   ...
   dtitle = substr(dtitle,1,index(dtitle," ")) + "{1}" +
      substr(dtitle,index(dtitle," ") + length("{1}","raw":U) + 1,78).

/* SS - 20051017 - B */
/*
if c-application-mode <> 'WEB-ChUI':U then
...
/*N096*/ /* NOW THAT ALL THE MENUS ARE BUILT. WALK THE    */
/*N096*/ /* MENU TREE AND SET EXTERNALIZED LABELS BEFORE  */
/*N096*/ /* MENU IS SET TO CURRENT WINDOW AND DISPLAYED.  */
/*N096*/ if valid-handle(menu menu-bar-ptr:handle) then do:
/*N096*/    setMenuLabels(menu menu-bar-ptr:handle).
/*N096*/    /* ASSIGN MENU BAR TO CURRENT WINDOW */
/*N096*/    assign current-window:menubar = menu menu-bar-ptr:handle.
/*N096*/ end.
