...
part1 curr curr1 eff" &frm = "a"}
   */
/*K0MD*/ if c-application-mode <> 'web' then
update plist plist1 prod prod1 part part1 curr curr1 with frame a.

/*K0MD*/ {wbrp06.i &command = update &fields = "  plist plist1 prod prod1 part
part1 curr curr1" &frm = "a"}
   /* SS - 20060411 - E */
...
/*H0GH*/ temp_max_price @ pc_amt[07].
   ...
   {mfrtrail.i}
      */
      ...
      FOR EACH tta6pppcrp03:
         DELETE tta6pppcrp03.
      END.
      
      {gprun.i ""a6pppcrp03.p"" "(
         INPUT plist,
         INPUT plist1,
         INPUT prod,
         INPUT prod1,
