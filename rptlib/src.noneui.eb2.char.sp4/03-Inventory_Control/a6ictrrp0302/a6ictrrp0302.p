...
/* $Revision: 1.20.1.2 $  BY: Bill Jiang    DATE: 09/21/05  ECO: *SS - 20050921*  */
/* $Revision: 1.20.1.2 $  BY: Bill Jiang    DATE: 10/12/05  ECO: *SS - 20051012*  */
/* $Revision: 1.20.1.2 $  BY: Bill Jiang    DATE: 10/13/05  ECO: *SS - 20051013*  */
/* $Revision: 1.20.1.2 $  BY: Bill Jiang    DATE: 01/25/06  ECO: *SS - 20060125*  */
...
repeat:
   ...
   for each tr_hist no-lock
      ...
      end. /* if si_entity >= entity and si_entity <= entity1 then do: */
      */

       /* SS - 20051012 - B */
       /*
       CREATE tta6ictrrp0302.
       ASSIGN
           tta6ictrrp0302_inv_nbr = tr_rmks
           tta6ictrrp0302_nbr = tr_nbr
           tta6ictrrp0302_line = tr_line
           tta6ictrrp0302_site = tr_site
           tta6ictrrp0302_pl = tr_prod_line
           tta6ictrrp0302_part = tr_part
           tta6ictrrp0302_wo_part = tr_part
           tta6ictrrp0302_trnbr = tr_trnbr
           tta6ictrrp0302_lot = tr_lot
           tta6ictrrp0302_effdate = tr_effdate
           tta6ictrrp0302_date = tr_date
           tta6ictrrp0302_type = tr_type
           tta6ictrrp0302_loc = tr_loc
           .
       IF  (trgl_dr_acct >= acct and trgl_dr_acct <= acct1) THEN DO:
           ASSIGN
               tta6ictrrp0302_acct = trgl_dr_acct
               tta6ictrrp0302_sub = trgl_dr_sub
               tta6ictrrp0302_cc = trgl_dr_cc
               tta6ictrrp0302_qty_dr = tr_qty_loc
               tta6ictrrp0302_amt_dr = trgl_gl_amt
               .
       END.
       ELSE DO:
           ASSIGN
               tta6ictrrp0302_acct = trgl_cr_acct
               tta6ictrrp0302_sub = trgl_cr_sub
               tta6ictrrp0302_cc = trgl_cr_cc
               tta6ictrrp0302_qty_cr = tr_qty_loc
               tta6ictrrp0302_amt_cr = - trgl_gl_amt
               .
       END.
       FIND FIRST wo_mstr WHERE wo_nbr = tr_nbr AND wo_lot = tr_lot NO-LOCK NO-ERROR.
       IF AVAILABLE wo_mstr THEN DO:
           ASSIGN
               tta6ictrrp0302_wo_part = wo_part
               .
       END.
       */
       /* SS - 20051013 - B */
       /* SS - 20060125 - B */
       /*
       IF trgl_dr_acct <> trgl_cr_acct OR trgl_dr_sub <> trgl_cr_sub OR trgl_dr_cc <> trgl_cr_cc OR trgl_dr_proj <> trgl_cr_proj THEN DO:
           */
           /* SS - 20060125 - E */
           /* SS - 20051013 - E */
           CREATE tta6ictrrp0302.
           ...
           IF  (trgl_dr_acct >= acct and trgl_dr_acct <= acct1) THEN DO:
               ASSIGN
                   ...
                   tta6ictrrp0302_cc = trgl_dr_cc
                   /* SS - 20051013 - B */
                   tta6ictrrp0302_proj = trgl_dr_proj
                   /* SS - 20051013 - E */
                   tta6ictrrp0302_qty_dr = tr_qty_loc
                   tta6ictrrp0302_amt_dr = trgl_gl_amt
                   .
           ...
           ELSE DO:
               ASSIGN
                   ...
                   tta6ictrrp0302_cc = trgl_cr_cc
                   /* SS - 20051013 - B */
                   tta6ictrrp0302_proj = trgl_cr_proj
                   /* SS - 20051013 - E */
                   tta6ictrrp0302_qty_cr = tr_qty_loc
                   tta6ictrrp0302_amt_cr = - trgl_gl_amt
                   .
