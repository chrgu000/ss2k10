...
/* $Revision: 1.20.2.9.3.1 $    BY: Bill Jiang  DATE: 05/11/07 ECO: *SS - 20070511.1* */
/******************************************************************************/
...
for each sod_det where (sod_nbr = so_nbr)
   ...
   */
   CREATE tta6soivrp0001.
   ASSIGN
      tta6soivrp0001_so_inv_nbr = so_inv_nbr
      tta6soivrp0001_so_bill = so_bill
      tta6soivrp0001_so_cust = so_cust
      tta6soivrp0001_so_slspsn[1] = so_slspsn[1]
      tta6soivrp0001_so_ar_acct = so_ar_acct
      tta6soivrp0001_so_ar_sub = so_ar_sub
      tta6soivrp0001_so_ar_cc = so_ar_cc

      tta6soivrp0001_so_tax_env = so_tax_env
      tta6soivrp0001_so_tax_usage = so_tax_usage

      tta6soivrp0001_so_nbr = so_nbr
      tta6soivrp0001_so_ship = so_ship
      tta6soivrp0001_ship_name = ship_name
      tta6soivrp0001_so_ord_date = so_ord_date
      tta6soivrp0001_so_po = so_po

      tta6soivrp0001_sod_line = sod_line
      tta6soivrp0001_sod_part = sod_part
      tta6soivrp0001_sod_um = sod_um
      tta6soivrp0001_sod_acct = sod_acct
      tta6soivrp0001_sod_sub = sod_sub
      tta6soivrp0001_sod_cc = sod_cc
      tta6soivrp0001_sod_qty_inv = sod_qty_inv
      tta6soivrp0001_sod_taxable = sod_taxable
      tta6soivrp0001_sod_taxc = sod_taxc
      tta6soivrp0001_net_price = net_price
      tta6soivrp0001_ext_price = ext_price
      tta6soivrp0001_ext_gr_margin = ext_gr_margin
      tta6soivrp0001_desc1 = desc1
      tta6soivrp0001_qty_bo = qty_bo
      tta6soivrp0001_sod_tax_usage = sod_tax_usage
      .
   find ad_mstr where ad_addr = so_bill no-lock no-wait no-error.
   if available ad_mstr then DO:
      ASSIGN
         tta6soivrp0001_bill_name = ad_name
         .
   END.
   else DO:
      ASSIGN
         tta6soivrp0001_bill_name = ""
         .
   END.
   find ad_mstr where ad_addr = so_cust no-lock no-wait no-error.
   if available ad_mstr then DO:
      ASSIGN
         tta6soivrp0001_name = ad_name
         .
   END.
   else DO:
      ASSIGN
