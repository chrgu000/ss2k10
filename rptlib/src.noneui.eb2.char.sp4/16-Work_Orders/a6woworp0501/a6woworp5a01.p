...
/* $Revision: 1.20 $ BY: Bill Jiang            DATE: 10/05/05 ECO: *SS - 20051005*    */
/* $Revision: 1.20 $ BY: Bill Jiang            DATE: 10/25/05 ECO: *SS - 20051025*    */
   ...
   */
   ...
   for each wod_det where wod_lot = wo_lot no-lock break by wod_lot
      ...
      */
      ...
      IF wod_qty_iss <> 0 OR wod_tot_std <> 0 THEN DO:
          find pt_mstr where pt_part = wod_part no-lock no-error.
          if available pt_mstr then do:
              find ptp_det where ptp_part = pt_part and ptp_site = wo_site no-lock no-error.
              if (available ptp_det and ptp_iss_pol = no) or (not available ptp_det and pt_iss_pol = no) then do:
                  CREATE tta6woworp0501.
                  ASSIGN
                      tta6woworp0501_wo_part = wo_part
                      tta6woworp0501_wo_nbr = wo_nbr
                      tta6woworp0501_wo_lot = wo_lot
                      tta6woworp0501_wo_qty_comp = wo_qty_comp
                      tta6woworp0501_wod_part = wod_part
                      tta6woworp0501_wod_qty_iss = wod_qty_iss
                      tta6woworp0501_wod_tot_std = wod_tot_std
                      .
              END.
          END.
      END.
      ...
      */
      ...
      if glx_mthd = "AVG" and wo_joint_type = "" then do:
         if display_det then do:
            ...
            */
             ASSIGN
                 tta6woworp0501_wipamt = wo_wip_tot
                 .
            /* SS - 20051005 - E */
         end.
         else do:
      ...
      /* REGULAR PRODUCTS (STD) AND JOINT PRODUCTS (AVG & STD) */
         ...
         */
         /* SS - 20051005 - E */
            ...
            */
             ASSIGN
                 tta6woworp0501_wipamt = (wo_mtl_tot + wo_lbr_tot + wo_bdn_tot + wo_sub_tot - wo_rjct_tot - wo_wip_tot - wo_mthd_var)
                 .
            /* SS - 20051005 - E */
         end.
