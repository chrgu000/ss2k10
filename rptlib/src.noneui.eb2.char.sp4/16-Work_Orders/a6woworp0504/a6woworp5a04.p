...
for EACH pt_mstr
   ...
   */
   ...
   with frame b side-labels.
           */
      CREATE tta6woworp0504.
      ASSIGN
          tta6woworp0504_part = wo_part
          tta6woworp0504_nbr = wo_nbr
          tta6woworp0504_lot = wo_lot
          tta6woworp0504_qty_comp = wo_qty_comp
          .
      /* SS - 20060118 - E */
      ...
      for each wr_route where wr_lot = wo_lot no-lock
         ...
         */
         ...
         if last(wr_lot) then do with frame c:
            ...
            */
            IF i = 1 THEN
               ASSIGN  tta6woworp0504_lbrtot = (accum total by wr_lot acccst).
            ELSE IF i = 2 THEN
               ASSIGN  tta6woworp0504_bdntot = (accum total by wr_lot acccst).
            ELSE IF i = 3 THEN
               ASSIGN  tta6woworp0504_subtot = (accum total by wr_lot acccst).
      ...
      */
      ...
      if glx_mthd = "AVG" and wo_joint_type = "" then do:
         if display_det then do:
            ...
            */
             ASSIGN
                 tta6woworp0504_wipamt = wo_wip_tot
                 .
            /* SS - 20060118 - E */
         end.
         else do:
      ...
      /* REGULAR PRODUCTS (STD) AND JOINT PRODUCTS (AVG & STD) */
         ...
         */
         /* SS - 20060118 - E */
            ...
            */
             ASSIGN
                 tta6woworp0504_wipamt = (wo_mtl_tot + wo_lbr_tot + wo_bdn_tot + wo_sub_tot - wo_rjct_tot - wo_wip_tot - wo_mthd_var)
                 .
            /* SS - 20060118 - E */
         end.
