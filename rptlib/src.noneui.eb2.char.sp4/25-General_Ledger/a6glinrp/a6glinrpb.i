...
/* $Revision: 1.9 $    BY: Bill Jiang           DATE: 09/22/05  ECO: *SS - 20050922*  */
/******************************************************************************/
...
mainloop:
for each asc_mstr
fields (asc_acc asc_cc asc_fpos asc_sub)
...
on endkey undo, leave mainloop:
    ...
    /*
   for first ac_mstr
   fields (ac_code ac_active ac_type ac_desc ac_curr)
   no-lock where ac_code=asc_acc: end.
   */
    for first ac_mstr
    fields (ac_fpos ac_code ac_active ac_type ac_desc ac_curr)
    no-lock where ac_code=asc_acc: end.
   /* SS - 20050922 - E */
