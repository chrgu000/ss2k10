...
/* $Revision: 1.9.1.25 $    BY: Bill Jiang     DATE: 02/10/06  ECO: *SS - 20060210*   */
/* $Revision: 1.9.1.25 $    BY: Bill Jiang     DATE: 03/01/06  ECO: *SS - 20060301*   */
/******************************************************************************/
...
/* SS - 20060301 - B */
/*
AND STRING(YEAR(fa_startdt)) + SUBSTRING(STRING(100 + MONTH(fa_startdt)),2,2) >= fromYrPer
AND STRING(YEAR(fa_startdt)) + SUBSTRING(STRING(100 + MONTH(fa_startdt)),2,2) <= toYrPer
*/
/* SS - 20060301 - E */
...
for each fa_mstr
   fields(fa_id fa_faloc_id fa_facls_id fa_entity
          fa_disp_dt fa_dep fa_desc1 fa_puramt)
   ...
   /* CHECK FOR NON AND RETIRED DEPRECIATION ASSETS */
      ...
      for each fab_det
         ...
         then do:
            ...
            then do:
               ...
               /* IF ASSET IS ALREADY RETIRED, REPORT SHOULD NOT DISPLAY */
               /* ASSET WHEN DISPOSED DATE IS PRIOR OR EQUAL TO GIVEN    */
               /* 'TO YEAR/PERIOD'.REPORT SHOULD DISPLAY ASSETS WHEN 'TO */
               /* YEAR/PERIOD' IS PRIOR TO  DISPOSED DATE.               */
               if disDate <= toYrPer
                  and not inclRet
               then
                  next.
                  ...
                  /* IS PRIOR OR EQUAL TO 'AS OF DATE' AND        */
                  /* 'INCLUDE RETIRED ASSET' IS SET TO YES IN     */
                  /* INPUT CRITERIA, SET l_zeronbv TO YES TO      */
           ...
           */
           ...
           then do:
              ...
              then do:
                 ...
                 CREATE tta6faaorp.
                 ASSIGN
                    tta6faaorp_fa_faloc_id = fa_faloc_id
                    tta6faaorp_fa_id = fa_mstr.fa_id
                    tta6faaorp_fa_desc1 = fa_mstr.fa_desc1
                    tta6faaorp_fa_facls_id = fa_mstr.fa_facls_id
                    tta6faaorp_fabk_id = fab_det.fab_fabk_id
                    tta6faaorp_costAmt = costAmt
                    tta6faaorp_accDepr = accDepr
                    tta6faaorp_netBook = netBook
                    tta6faaorp_annDepr = annDepr
                    tta6faaorp_fa_puramt = fa_mstr.fa_puramt
                    .
                    /* SS - 20060210 - E */
              end. /* IF printTot */
                 ...
                 then do:
                    ...
                    then do:
                       ...
                       ASSIGN
                          ...
                          tta6faaorp_fa_desc1 = fa_mstr.fa_desc1
                          tta6faaorp_fa_facls_id = fa_mstr.fa_facls_id
                          tta6faaorp_fabk_id = fauop_det.fauop_fabk_id
                          tta6faaorp_costAmt = costAmt
                          tta6faaorp_accDepr = accDepr
                          tta6faaorp_netBook = netBook
                          tta6faaorp_annDepr = annDepr
         ...
         then do:
            ...
            if line-counter = 6
               ...
               */
               ...
               ASSIGN
                  tta6faaorp_fa_faloc_id = fa_faloc_id
                  tta6faaorp_fa_id = fa_id
                  tta6faaorp_fa_desc1 = fa_desc1
                  tta6faaorp_fa_facls_id = fa_facls_id
                  tta6faaorp_fabk_id = l_fab_fabk_id
                  tta6faaorp_costAmt = costAmt
                  tta6faaorp_accDepr = accDepr
                  tta6faaorp_netBook = netBook
                  tta6faaorp_annDepr = annDepr
