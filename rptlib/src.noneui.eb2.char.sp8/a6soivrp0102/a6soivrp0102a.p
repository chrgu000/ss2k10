...
/* Revision: 1.16.1.14.3.1 By: Gnanasekar        DATE: 08/18/03   ECO: *P0ZW**/
/* $Revision: 1.16.1.14.3.2 $   By: Prashant Parab        DATE: 04/05/04   ECO: *P1WT**/
/* $Revision: 1.16.1.14.3.2 $   By: Bill Jiang        DATE: 06/02/06   ECO: *SS - 20060602.1**/
...
2. 执行列表:
   a6soivrp0102.p
   a6soivrp0102a.p
   a6soivrp0102b.p,a6soivtot80102.i,a6soivtrl20102.p
   a6soivtrlc0102.p
   a6soivtrl20102.i
   a6sototfrm0102.i
*/
...
/* SS - 20060602.1 - B */
DEFINE NEW SHARED VARIABLE rec1 AS RECID.
...
for each so_mstr where (so_nbr >= nbr) and (so_nbr <= nbr1)
   ...
   */
   ...
   ASSIGN
      tta6soivrp0102_so_inv_nbr = so_inv_nbr
      tta6soivrp0102_so_bill = so_bill
      tta6soivrp0102_bill_name = bill_name
      tta6soivrp0102_so_cust = so_cust
      tta6soivrp0102_name = name
      tta6soivrp0102_so_slspsn[1] = so_slspsn[1]
      tta6soivrp0102_so_ar_acct = so_ar_acct
      tta6soivrp0102_so_ar_sub = so_ar_sub
      tta6soivrp0102_so_ar_cc = so_ar_cc
      tta6soivrp0102_so_tax_env = so_tax_env
      tta6soivrp0102_so_tax_usage = so_tax_usage
      .
   rec1 = RECID(tta6soivrp0102).
   /* SS - 20060602.1 - E */
   ...
   assign
      undo_trl2 = true
      consolidate = true.
   find first tx2d_det where tx2d_ref = so_inv_nbr
      and tx2d_nbr = so_nbr
   ...
   if available tx2d_det then do:
      ...
      {gprun.i ""soivtrl2.p"" "(input so_inv_nbr,
                     ...
                     input l_consolidate)"}
         */
      {gprun.i ""a6soivtrl20102.p"" "(input so_inv_nbr,
                     input so_nbr,
                     input col-80 /* REPORT WIDTH */,
                     input '16'   /* TRANSACTION TYPE */,
                     input cont_charges,
      ...
      {gprun.i ""txdetrp1.p"" "(input '16',
                      ...
                      input l_accumonly)"}
                      */
         {gprun.i ""a6txdetrp10102.p"" "(input '16',
                         input so_inv_nbr,
                         input so_nbr,
                         input col-80,
                         input 0,
      ...
      then do:
         ...
         {gprun.i ""soivtrl2.p"" "(input so_nbr,
                         ...
                         input l_consolidate)"}
            */
         {gprun.i ""a6soivtrl20102.p"" "(input so_nbr,
                         ' ',
                         input col-80 /* REPORT WIDTH */,
                         input '13'   /* TRANSACTION TYPE */,
                         input cont_charges,
         ...
         {gprun.i ""txdetrp1.p"" "(input '13',
                         ...
                         input l_accumonly)"}
            */
            {gprun.i ""a6txdetrp10102.p"" "(input '13',
                            input so_nbr,
                            input '',
                            input col-80,
                            input 0,
   ...
   */
   ...
   if conso then do:
      ...
      for each somstr2
         ...
         find si_mstr where si_site = somstr2.so_site no-lock no-error.
   ...
   {gprun.i ""txdetrp1.p"" "(input '',
                      ...
                      input l_printonly)"}
                      */
   {gprun.i ""a6txdetrp10102.p"" "(input '',
                      input '',
                      input '',
                      input col-80,
                      input 0,
...
PROCEDURE container_line_charge_calc:
   ...
   sod_qty_inv <> 0:
      ...
      if using_line_charges then do:
         absl_exists = no.
         for each absl_det no-lock where
        ...
        if not absl_exists then do:
           for each sodlc_det no-lock where
