...
/* Revision: 1.9.1.25.1.1  BY: Dayanand Jethwa  DATE: 01/13/04  ECO: *P1JN*   */
/* Revision: 1.9.1.25.1.2  BY: Preeti Sattur    DATE: 01/21/04  ECO: *P1KG*   */
/* Revision: 1.9.1.25.1.3  BY: Jignesh Rachh    DATE: 10/12/04  ECO: *P2NZ*   */
/* Revision: 1.9.1.25.1.4  BY: Milind Shahane   DATE: 12/29/04  ECO: *P31Z*   */
/* $Revision: 1.9.1.25.1.6 $  BY: Milind Shahane   DATE: 12/31/04  ECO: *P321*   */
/* $Revision: 1.9.1.25.1.6 $  BY: Bill Jiang   DATE: 05/05/06  ECO: *SS - 20060505.1*   */
/* $Revision: 1.9.1.25.1.6 $  BY: Bill Jiang   DATE: 05/11/06  ECO: *SS - 20060511.1*   */
/* $Revision: 1.9.1.25.1.6 $  BY: Bill Jiang   DATE: 05/11/06  ECO: *SS - 20060511.2*   */
/******************************************************************************/
...
loop1:
for each fa_mstr
   fields(fa_id fa_faloc_id fa_facls_id fa_entity
   ...
   /* IF ASSET IS RETIRED AND INCLUDE RETIRED IS NO OR            */
   /* ASSET IS NON-DEPRECIATING AND INCLUDE NO-DEPRECIATING IS NO */
   ...
   /* CHECK FOR NON-DEPRECIATED AND RETIRED ASSETS */
   ...
   /* ASSET SHOULD BE DISPLAYED ONLY IF FROM/TO YRPERIOD IS */
   /* WITHIN THE LIFE OF THE ASSET                          */
   if  (   (    servDate <> ""
            and servDate <= fromYrPer)
         or fromYrPer = "")
   and (   disDate = ""
        or (   disDate >= toYrPer
            or toYrPer = hi_char))
   then do:
      ...
      for each fab_det
         ...
         /* CHECK FOR FULLY DEPRECIATED ASSET       */
         if (not can-find(first fabd_det
                             where fabd_fa_id       = fab_fa_id
         ...
         then do:
            /* FOR THE RETIRED ASSET WHEN DISPOSITION DATE  */
            /* IS PRIOR OR EQUAL TO 'AS OF DATE'            */
            /* SET l_zeronbv TO YES TO                      */
    ...
    then do:
           ...
           for last fabd_det
              fields(fabd_fa_id fabd_fabk_id fabd_yrper fabd_accup
                     fabd_accamt)
              where fabd_fa_id   = fa_id
              and   fabd_fabk_id = fab_fabk_id
           ...
           then do:
              ...
              for each fauop_det
                 fields(fauop_fa_id fauop_fabk_id
                        fauop_yrper fauop_upper)
                 where fauop_fa_id   = fa_id
           ...
           */
           ...
           then do:
              ...
              then do:
                 ...
                 CREATE tta6faaorp01.
                 ASSIGN
                    tta6faaorp01_fa_faloc_id = fa_faloc_id
                    tta6faaorp01_fa_id = fa_mstr.fa_id
                    tta6faaorp01_fa_desc1 = fa_mstr.fa_desc1
                    tta6faaorp01_fa_facls_id = fa_mstr.fa_facls_id
                    tta6faaorp01_fabk_id = fab_det.fab_fabk_id
                    tta6faaorp01_costAmt = costAmt
                    tta6faaorp01_accDepr = accDepr
                    tta6faaorp01_netBook = netBook
                    tta6faaorp01_annDepr = annDepr
                    /* SS - 20060511.1 - B */
                    tta6faaorp01_fa_puramt = fa_mstr.fa_puramt
                    /* SS - 20060511.1 - E */
                    .
                    /* SS - 20060505.1 - E */
              end. /* IF printTot */
           ...
           */
           ...
           then do:
              ...
              if can-find (first fauop_det
                              where fauop_det.fauop_fa_id = fa_id
              ...
              then do:
                 ...
                 for last fauop_det
                    fields(fauop_fa_id fauop_fabk_id fauop_accup
                           fauop_yrper)
                    where fauop_fa_id   = fa_id
                    and   fauop_fabk_id = fab_fabk_id
                    and   fauop_yrper  <= yrPeriod
                 ...
                 then do:
                    ...
                    then do:
                       ...
                       ASSIGN
                          ...
                          tta6faaorp01_fa_desc1 = fa_mstr.fa_desc1
                          tta6faaorp01_fa_facls_id = fa_mstr.fa_facls_id
                          tta6faaorp01_fabk_id = fauop_det.fauop_fabk_id
                          tta6faaorp01_costAmt = costAmt
                          tta6faaorp01_accDepr = accDepr
                          tta6faaorp01_netBook = netBook
                          tta6faaorp01_annDepr = annDepr
         ...
         then do:
            ...
            if line-counter = 6
               ...
               */
               ...
               ASSIGN
                  tta6faaorp01_fa_faloc_id = fa_faloc_id
                  tta6faaorp01_fa_id = fa_id
                  tta6faaorp01_fa_desc1 = fa_desc1
                  tta6faaorp01_fa_facls_id = fa_facls_id
                  tta6faaorp01_fabk_id = l_fab_fabk_id
                  tta6faaorp01_costAmt = costAmt
                  tta6faaorp01_accDepr = accDepr
                  tta6faaorp01_netBook = netBook
                  tta6faaorp01_annDepr = annDepr
                  /* SS - 20060511.1 - B */
                  tta6faaorp01_fa_puramt = fa_puramt
                  /* SS - 20060511.1 - E */
                  .
               /* SS - 20060505.1 - E */
