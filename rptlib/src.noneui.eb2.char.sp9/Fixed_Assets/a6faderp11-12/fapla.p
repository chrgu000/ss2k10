...
/* Revision: 1.14.1.1     BY: Reena Ambavi        DATE: 03/17/04  ECO: *P1TP* */
/* $Revision: 1.14.1.3 $           BY: Nishit V            DATE: 07/15/04  ECO: *P2B5* */
/* $Revision: 1.14.1.3 $           BY: Bill Jiang            DATE: 05/11/06  ECO: *SS - 20060511.3* */
/******************************************************************************/
...
PROCEDURE fa-get-per:
   ...
   CASE i_cal:
      ...
      then do:
         for first glc_cal
            fields (glc_start glc_end glc_year glc_per)
            no-lock
            where glc_start <= i_date
            and   glc_end   >= i_date:
      ...
      otherwise do:
         for first facld_det
            fields (facld_facl_id facld_start facld_end facld_year facld_per)
            no-lock
            where facld_facl_id = i_cal
   ...
   CASE i_cal:
      ...
      then do:
         for each glc_cal
            fields (glc_year)
            no-lock
            where glc_year = integer(substring(i_per,1,4)):
         o_perinyr = o_perinyr + 1.
         end. /* FOR EACH glc_cal */
      end. /* WHEN " " */
      otherwise do:
         for each facld_det
            fields (facld_facl_id facld_year)
            no-lock
            where facld_facl_id = i_cal
...
PROCEDURE fa-get-halfyear:
   ...
   then
      ...
      then do:
         for first glc_cal
            fields (glc_year glc_start glc_end)
            no-lock
            where glc_start <= i_date
            and   glc_end   >= i_date:
         end. /* FOR FIRST glc_cal */
         ...
         for first facld_det
            fields (facld_facl_id facld_year facld_start facld_end)
            no-lock
            where facld_facl_id = i_cal
            and   facld_start  <= i_date
            and   facld_end    >= i_date:
...
PROCEDURE fa-get-cost:
   ...
   for each fab_det
      fields(fab_fa_id fab_fabk_id fab_amt)
      no-lock
      where fab_fa_id = i_fa_id
...
PROCEDURE fa-get-basis:
   ...
   /* SUM ALL RESERVES TO DETERMINE TOTAL COST */
   for each fab_det
      fields (fab_fa_id fab_fabk_id fab_amt fab_salvamt fab_famt_id)
      no-lock
      where fab_fa_id = i_fa_id
      and fab_fabk_id = i_fabk_id:
      assign
   ...
   for last fab_det
      ...
      for first famt_mstr
         fields (famt_id famt_salv)
         no-lock
         where famt_id = fab_det.fab_famt_id:
...
PROCEDURE fa-get-salvage:
   ...
   for each fab_det fields (fab_fa_id fab_fabk_id fab_salvamt)
      no-lock
      where fab_fa_id = i_fa_id
      and fab_fabk_id = i_fabk_id:
...
PROCEDURE fa-get-accdep:
   ...
   /* WILL BE EXECUTED WHEN CALCULATING PERIOD DEPRECIATION */
   if execname = "faderp.p"
      or execname = "faaorp.p"
   then do:
      for each fabd_det
         fields (fabd_fa_id     fabd_fabk_id fabd_yrper   fabd_accamt
                 fabd_suspend   fabd_peramt)
         where fabd_fa_id   = i_fa_id
         and   fabd_fabk_id = i_fabk_id
...
PROCEDURE fa-get-accdepa:
   ...
   /* FOR THE BELOW MENTIONED REPORTS THE SUSPENDED PERIODS */
   /* WILL BE EXECUTED WHEN CALCULATING PERIOD DEPRECIATION */
   if execname BEGINS "a6faderp"
      or execname BEGINS "a6faaorp"
   then do:
      for each fabd_det
         fields (fabd_fa_id     fabd_fabk_id fabd_yrper   fabd_accamt
                 fabd_suspend   fabd_peramt)
...
PROCEDURE fa-get-perdep:
   ...
   for each fabd_det
      fields (fabd_fa_id fabd_fabk_id fabd_yrper fabd_peramt fabd_suspend)
      no-lock
      where fabd_fa_id   = i_fa_id
      and   fabd_fabk_id = i_fabk_id
      and   fabd_yrper   = i_per:
...
PROCEDURE fa-get-cur-perdep:
   ...
   for each fabd_det
      fields(fabd_fa_id fabd_fabk_id fabd_yrper fabd_peramt fabd_adj_yrper)
      no-lock
      where fabd_fa_id   = i_fa_id
      and fabd_fabk_id   = i_fabk_id
      and fabd_yrper     = i_per
...
PROCEDURE fa-get-forecast-perdep:
   ...
   for first fauop_det
      fields (fauop_fa_id fauop_fabk_id fauop_yrper fauop_upper)
      no-lock
      where fauop_fa_id = i_fa_id
      and fauop_fabk_id = i_fabk_id
      and fauop_yrper   = i_per:

      for first fab_det
         fields (fab_fa_id fab_fabk_id fab_upcost)
         no-lock
         where fab_fa_id = i_fa_id
         and fab_fabk_Id = i_fabk_id:
...
PROCEDURE fa-get-forecast-anndep:
   ...
   /* FIND BEGINNING ANNUAL DEPRECIATION */
   for first fauop_det
      fields (fauop_fa_id fauop_fabk_id fauop_yrper fauop_accup)
      no-lock
      where fauop_fa_id = i_fa_id
      and fauop_fabk_id = i_fabk_id
   ...
   then do:
      case i_cal:
         ...
         otherwise do:
            ...
            for first facld_det
                fields(facld_facl_id facld_year facld_start)
                where facld_facl_id = i_cal
                and   facld_year    = integer(substring(i_per,1,4))
                no-lock:
             ...
             for last facld_det
                fields(facld_facl_id facld_year facld_end)
                where facld_facl_id = i_cal
                and   facld_year    = integer(substring(i_per,1,4))
                no-lock:
...
PROCEDURE fa-get-daysinyr:
   ...
   case i_cal:
      ...
      otherwise do:
         for each facld_det
            fields (facld_facl_id facld_year facld_end facld_start)
            where facld_facl_id = i_cal
            and   facld_year    = integer(substring(i_per,1,4))
            no-lock:
...
PROCEDURE fa-conv-per-to-date:
   ...
   then do:
      for first facld_det
         fields (facld_facl_id facld_start facld_year facld_per)
         where facld_facl_id = i_calendar
           and facld_year    = integer(substring(i_per,1,4))
      ...
      for first glc_cal
         fields (glc_start glc_year glc_per)
         where glc_year = integer(substring(i_per,1,4))
...
PROCEDURE fa-get-cost-asof-date:
   ...
   for first fab_det
      fields(fab_amt fab_fabk_id fab_fa_id fab_resrv)
      where fab_fa_id       = i_faid
