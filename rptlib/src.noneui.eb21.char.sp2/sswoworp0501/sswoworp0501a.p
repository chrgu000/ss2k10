...
/* $Revision: 1.22 $ BY: Bill Jiang DATE: 05/06/08 ECO: *SS - 20080506.1* */
/*-Revision end---------------------------------------------------------------*/
      ...
      */
      ...
      IF wod_qty_iss <> 0 OR wod_tot_std <> 0 THEN DO:
          ...
          if available pt_mstr then do:
              find ptp_det where ptp_domain = global_domain AND ptp_part = pt_part and ptp_site = wo_site no-lock no-error.
              if (available ptp_det and ptp_iss_pol = no) or (not available ptp_det and pt_iss_pol = no) then do:
                  CREATE ttsswoworp0501.
                  ASSIGN
                      ttsswoworp0501_wo_part = wo_part
                      ttsswoworp0501_wo_nbr = wo_nbr
                      ttsswoworp0501_wo_lot = wo_lot
                      ttsswoworp0501_wo_qty_comp = wo_qty_comp
                      ttsswoworp0501_wod_part = wod_part
                      ttsswoworp0501_wod_qty_iss = wod_qty_iss
                      ttsswoworp0501_wod_tot_std = wod_tot_std
                      .
              END.
