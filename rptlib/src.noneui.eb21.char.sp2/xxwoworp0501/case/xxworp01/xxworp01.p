...
/* SS - 090318.1 - B */
define variable ord_date   like wo_ord_date.
define variable ord_date1  like wo_ord_date.
define variable stat_CLOSE_date   like wo_stat_close_date.
define variable stat_close_date1  like wo_stat_close_date.
...
DEFINE VARIABLE prod_line_pt LIKE pt_prod_line.
DEFINE VARIABLE desc1_pt LIKE pt_desc1.
DEFINE VARIABLE bom_qty_wod LIKE wod_bom_qty.
/* SS - 090318.1 - E */
...
form
   ...
   stat_CLOSE_date         colon 15
   stat_close_date1        label {t001.i} colon 49 skip (1)
...
repeat on error undo, retry:
   ...
   update
      ...
      /* SS - 090318.1 - B */
      ord_date ord_date1
      stat_CLOSE_date stat_CLOSE_date1
      acct_close
   ...
   if (c-application-mode <> "WEB") or
      ...
      {mfquoter.i stat_CLOSE_date     }
      {mfquoter.i stat_close_date1    }
   ...
   */
   ...
   EMPTY TEMP-TABLE ttxxwoworp0501.

    {gprun.i ""xxwoworp0501.p"" "(
       INPUT nbr,
       INPUT nbr1,
       INPUT lot,
       INPUT lot1,
       ...
       */
       ...
       INPUT ord_date,
       INPUT ord_date1,
       INPUT stat_CLOSE_date,
       INPUT stat_CLOSE_date1,
    ...
    /*
    EXPORT DELIMITER ";" "加工单" "ID" "批处理" "已订购量" "订货日期" "总账成本计算方法" "父物料号" "父物料说明" "完成数量" "发放日期" "premsg" "客户单/工" "次品量" "截止日期" "供应商" "加工单状态" "注释" "子物料号" "累计量" "预期成本(仅作参考)" "应计差异(仅作参考)" "累计成本" "过账的费率差异" "过账的使用差异" "收到的成品平均成本" "余额".
    EXPORT DELIMITER ";" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出".
    EXPORT DELIMITER ";" "wo_nbr" "wo_lot" "wo_batch" "wo_qty_ord" "wo_ord_date" "glx_mthd" "wo_part" "desc1" "wo_qty_comp" "wo_rel_date" "premsg" "wo_so_job" "wo_qty_rjct" "wo_due_date" "wo_vend" "wo_status" "wo_rmks" "wod_part" "accqty" "expcst" "acrvar" "acccst" "rtevar" "usevar" "wowipx" "wipamt".
    */
    EXPORT DELIMITER ";" "产品线" "成品" "成品描述" "工单ID" "原料" "描述" "标准数量" "标准成本" "实际使用" "实际成本" "差异" "地点" "关闭日期" "更新日期" "更新时间" "更新人".
    FOR EACH ttxxwoworp0501
       ,EACH wo_mstr NO-LOCK
       WHERE wo_domain = GLOBAL_domain
       AND wo_lot = ttxxwoworp0501_wo_lot
       :
       FIND FIRST pt_mstr WHERE pt_domain = global_domain AND pt_part = ttxxwoworp0501_wo_part NO-LOCK NO-ERROR.
       IF AVAILABLE pt_mstr THEN DO:
          prod_line_pt = pt_prod_line.
       END.
       ELSE DO:
          prod_line_pt = "".
       END.

       FIND FIRST pt_mstr WHERE pt_domain = global_domain AND pt_part = ttxxwoworp0501_wod_part NO-LOCK NO-ERROR.
       IF AVAILABLE pt_mstr THEN DO:
          desc1_pt = pt_desc1 + " " + pt_desc2.
       END.
       ELSE DO:
          desc1_pt = "".
       END.
       ...
       FOR EACH wod_det NO-LOCK
          ...
          AND wod_lot = ttxxwoworp0501_wo_lot
          AND wod_part = ttxxwoworp0501_wod_part
          :
          bom_qty_wod = bom_qty_wod + wod_bom_qty * ttxxwoworp0501_wo_qty_comp.
       END.

       EXPORT DELIMITER ";" 
          prod_line_pt
          ttxxwoworp0501_wo_part
          ttxxwoworp0501_desc1
          ttxxwoworp0501_wo_lot
          ttxxwoworp0501_wod_part
          desc1_pt
          bom_qty_wod
          ttxxwoworp0501_expcst
          ttxxwoworp0501_accqty
          ttxxwoworp0501_acccst
          ttxxwoworp0501_acrvar
          wo_site
          wo_stat_close_date
          TODAY
