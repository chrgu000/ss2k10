...
顺序输出了以下字段:
  ...
  - 标准输出: ID[wo_lot]
  - 标准输出: 批处理[wo_batch]
  - 标准输出: 已订购量[wo_qty_ord]
  - 标准输出: 订货日期[wo_ord_date]
  - 标准输出: 总账成本计算方法[glx_mthd]
  - 标准输出: 父物料号[wo_part]
  - 标准输出: 父物料说明[desc1]
  - 标准输出: 完成数量[wo_qty_comp]
  - 标准输出: 发放日期[wo_rel_date]
  - 标准输出: premsg[premsg]
  - 标准输出: 客户单/工[wo_so_job]
  - 标准输出: 次品量[wo_qty_rjct]
  - 标准输出: 截止日期[wo_due_date]
  - 标准输出: 供应商[wo_vend]
  - 标准输出: 加工单状态[wo_status]
  - 标准输出: 注释[wo_rmks]
  - 标准输出: 子物料号[wod_part]
  - 标准输出: 累计量[accqty]
  - 标准输出: 预期成本(仅作参考)[expcst]
  - 标准输出: 应计差异(仅作参考)[acrvar]
  - 标准输出: 累计成本[acccst]
  - 标准输出: 过账的费率差异[rtevar]
  - 标准输出: 过账的使用差异[usevar]
  - 标准输出: 收到的成品平均成本[wowipx]
  - 标准输出: 余额[wipamt]
   ...
   */
   ...
   EMPTY TEMP-TABLE ttxxwoworp0501.
    ...
    FOR EACH ttxxwoworp0501:
        EXPORT DELIMITER ";" ttxxwoworp0501.
    END.
