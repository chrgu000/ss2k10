...
/* $Revision: 1.10.1.13 $ BY: Mercy Chittilapilly  DATE: 11/17/03 ECO: *P19J* */
/* $Revision: 1.10.1.13 $ BY: Bill Jiang  DATE: 03/01/07 ECO: *SS - 20070301.1* */
/*-Revision end---------------------------------------------------------------*/
...
repeat:
   ...
   {wbrp06.i &command = update &fields = "  vend vend1 
    apdate apdate1  open_only base_rpt et_report_curr" &frm = "a"}
   /* SS - 20070301.1 - E */
   ...
   vdloop:
   for each vd_mstr  where vd_mstr.vd_domain = global_domain and
      ...
      do pass = 1 to 2:
         ...
         (ap_vend = vd_addr)
            ...
            for first ck_mstr
            fields( ck_domain ck_bank ck_clr_date ck_cmtindx ck_curr
                    ck_exru_seq ck_ex_rate ck_ex_rate2
                    ck_ex_ratetype ck_nbr ck_ref ck_status
                    ck_type ck_voiddate ck_voideff)
            no-lock
             where ck_mstr.ck_domain = global_domain and  ck_ref = ap_ref: end.
            ...
            for each ckd_det  where ckd_det.ckd_domain = global_domain and
            ckd_ref = ap_ref no-lock:
                ...
                apmstr.ap_type = "VO" and apmstr.ap_ref
   ...
   */
   ...
   EXPORT DELIMITER ";" "ap_vend" "ap_effdate" "ap_date" "ap_ref" "ap_curr" "ap_amt" "ap_base_amt" "ap_acct" "ap_sub" "ap_cc" "ap_type" "vo_invoice" "vo_due_date" "disp_curr" "base_amt" "amt_open" "notes" "ap_batch" "ap_rmk".
