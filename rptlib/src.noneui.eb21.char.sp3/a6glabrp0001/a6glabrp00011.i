...
/* $Revision: 1.19 $    BY: Bill Jiang          DATE: 11/17/06  ECO: *SS - 20061117*  */
/* $Revision: 1.19 $    BY: Bill Jiang          DATE: 12/17/07  ECO: *SS - 20071217.1*  */
/*-Revision end---------------------------------------------------------------*/
...
if print_acct = yes then do:
   ...
   if first_acct then do:
      ...
      */
      /* SS - 20071217.1 - B */
      /*
      FIND FIRST tta6glabrp0001 WHERE tta6glabrp0001_code = d_acct NO-LOCK NO-ERROR.
      IF NOT AVAILABLE tta6glabrp0001 THEN DO:
          CREATE tta6glabrp0001.
          ASSIGN
              tta6glabrp0001_code = d_acct
              tta6glabrp0001_desc = d_desc
              .
      END.
      */
      /* SS - 20071217.1 - E */
   ...
   then do:
      ...
      for first sb_mstr fields( sb_domain sb_sub sb_desc)
          where sb_mstr.sb_domain = global_domain and  sb_sub = asc_sub no-lock:
      end.
   ...
   /*
   FIND FIRST tta6glabrp0001 WHERE tta6glabrp0001_code = d_acct NO-LOCK NO-ERROR.
   IF AVAILABLE tta6glabrp0001 THEN DO:
       ASSIGN tta6glabrp0001_et_beg_bal = tta6glabrp0001_et_beg_bal + et_beg_bal.
   END.
   */
   IF et_beg_bal <> 0 THEN DO:
      FIND FIRST tta6glabrp0001 
         WHERE tta6glabrp0001_acc = ASC_acc
         AND tta6glabrp0001_sub = ASC_sub
         AND tta6glabrp0001_cc = ASC_cc
         NO-LOCK 
         NO-ERROR
         .
      IF NOT AVAILABLE tta6glabrp0001 THEN DO:
         CREATE tta6glabrp0001.
         ASSIGN
            tta6glabrp0001_acc = ASC_acc
            tta6glabrp0001_sub = ASC_sub
            tta6glabrp0001_cc = ASC_cc
            .
      END.
      ASSIGN tta6glabrp0001_et_beg_bal = tta6glabrp0001_et_beg_bal + et_beg_bal.
   END.
...
if last(cal.glc_per) then do:
   ...
   FIND FIRST tta6glabrp0001 WHERE tta6glabrp0001_code = d_acct NO-LOCK NO-ERROR.
   IF AVAILABLE tta6glabrp0001 THEN DO:
       ASSIGN tta6glabrp0001_et_end_bal = tta6glabrp0001_et_end_bal + et_end_bal.
   END.
   */
   IF et_end_bal <> 0 THEN DO:
      FIND FIRST tta6glabrp0001 
         WHERE tta6glabrp0001_acc = ASC_acc
         AND tta6glabrp0001_sub = ASC_sub
         AND tta6glabrp0001_cc = ASC_cc
