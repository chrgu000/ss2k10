...
/* $Revision: 1.10.3.1 $ BY: Bill Jiang  DATE: 04/19/06 ECO: *SS - 20070410.1*        */
  ...
  for each ac_mstr no-lock where ac_code >= acc and ac_code <= acc1:
      ...
      */
      ...
      if not available gltr_hist then DO:
         CREATE tta6glacstrp01.
         ASSIGN
            tta6glacstrp01_acc = ac_code
            tta6glacstrp01_beg_amt = beg_amt
            .
         next.
      END.
      /* SS - 20070410.1 - E */
  ...
  */
  ...
  if not summary then do:
     ...
     */
     CREATE tta6glacstrp01.
     ASSIGN
        tta6glacstrp01_acc = gltr_hist.gltr_acc
        tta6glacstrp01_ref = gltr_hist.gltr_ref
        tta6glacstrp01_doc = doc
        tta6glacstrp01_bank = bank
        tta6glacstrp01_ckfrm = ckfrm
        tta6glacstrp01_addr = gltr_hist.gltr_addr
        tta6glacstrp01_eff_dt = gltr_hist.gltr_eff_dt
        tta6glacstrp01_beg_amt = beg_amt
        tta6glacstrp01_dr_amt = ip_dr_amt
        tta6glacstrp01_cr_amt = ip_cr_amt
        tta6glacstrp01_batch = gltr_hist.gltr_batch
        .
    IF ip_sub THEN DO:
       ASSIGN
          tta6glacstrp01_sub = gltr_hist.gltr_sub
          .
    END.
    IF ip_cc = FALSE THEN DO:
       ASSIGN
          tta6glacstrp01_ctr = gltr_hist.gltr_ctr
          .
    END.
    if trn_desc and gldesc <> "" then
       ASSIGN tta6glacstrp01_gldesc = gldesc.
    if disp_curr   and
       gltr_hist.gltr_curr <> base_curr and
       gltr_hist.gltr_curramt <> 0      then do:
       ASSIGN
          tta6glacstrp01_curr = gltr_hist.gltr_curr
          tta6glacstrp01_curramt = gltr_hist.gltr_curramt
          .
    end.
     /* SS - 20070410.1 - E */
