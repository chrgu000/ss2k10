...
/* $Revision: 1.47.1.2 $ BY: Bill Jiang DATE: 07/26/07 ECO: *SS - 20070726.1*        */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
...
/* SS - 20070726.1 - E */
for each ih_hist  
   where ih_hist.ih_domain = global_domain and (  
      /* SS - 20070726.1 - B */
      /*
      (ih_inv_nbr >= inv) and (ih_inv_nbr <= inv1)
      */
      (ih_inv_nbr = ar_nbr)
      /* SS - 20070726.1 - E */
   ...
   if can-find (first idh_hist  where idh_hist.idh_domain = global_domain and
      idh_inv_nbr = ih_inv_nbr and
      idh_nbr     = ih_nbr)    and
      not can-find (first idh_hist  where idh_hist.idh_domain = global_domain
      and
              idh_nbr = ih_nbr
   ...
   */
   ...
   idh_inv_nbr = ih_inv_nbr
      ...
      */
      ...
      /* SS - 20070726.1 - B */
      CREATE tta6soivrp0901.

      ASSIGN
         tta6soivrp0901_ih_inv_nbr = ih_inv_nbr
         tta6soivrp0901_ih_rev = ih_rev
         tta6soivrp0901_ih_cust = ih_cust
         tta6soivrp0901_name = NAME
         tta6soivrp0901_ih_po = ih_po
         tta6soivrp0901_ih_ship =ih_ship
         tta6soivrp0901_ih_ship_date = ih_ship_date
         tta6soivrp0901_inv_date = ih_inv_date
         tta6soivrp0901_ih_slspsn[1] = ih_slspsn[1]
         tta6soivrp0901_ih_slspsn[2] = ih_slspsn[2]
         tta6soivrp0901_ih_slspsn[3] = ih_slspsn[3]
         tta6soivrp0901_ih_slspsn[4] = ih_slspsn[4]
         tta6soivrp0901_ih_curr = ih_curr
         tta6soivrp0901_base_curr = base_curr
         tta6soivrp0901_ih_ex_rate = ih_ex_rate
         tta6soivrp0901_ih_ex_rate2 = ih_ex_rate2
         tta6soivrp0901_ih_exru_seq = ih_exru_seq
         tta6soivrp0901_v_disp_line1 = v_disp_line1
         tta6soivrp0901_v_disp_line2 = v_disp_line2
         tta6soivrp0901_ih_rmks = ih_rmks
         .

      ASSIGN
         tta6soivrp0901_idh_nbr = idh_nbr
         tta6soivrp0901_idh_line = idh_line
         tta6soivrp0901_idh_part = idh_part
         tta6soivrp0901_idh_um = idh_um
         .

      IF ih_fsm_type = "FSM-RO" THEN DO:
         ASSIGN
            tta6soivrp0901_idh_qty_ord = l_qty_call
            tta6soivrp0901_idh_qty_inv = l_qty_call
            .
      END.
      ELSE DO:
         ASSIGN
            tta6soivrp0901_idh_qty_ord = idh_qty_ord
            tta6soivrp0901_idh_qty_inv = idh_qty_inv
            .
      END.
      ...
      then do:
         IF ih_fsm_type <> "FSM-RO" THEN DO:
            IF idh_qty_ord >= 0 THEN DO:
               ASSIGN
                  tta6soivrp0901_idh_bo_chg = max(idh_qty_ord - idh_qty_ship, 0)
                  .
            END.
            ELSE DO:
               ASSIGN
                  tta6soivrp0901_idh_bo_chg = MIN(idh_qty_ord - idh_qty_ship, 0)
                  .
            END.
         END.
         ELSE DO:
            ASSIGN
               tta6soivrp0901_idh_bo_chg = tta6soivrp0901_idh_bo_chg
               .
         END.
      END.
      ELSE DO:
         ASSIGN
            tta6soivrp0901_idh_bo_chg = l_idh_bo_chg
            .
      END.

      ASSIGN
         tta6soivrp0901_disp_curr = DISP_curr
         tta6soivrp0901_base_net_price = base_net_price
         tta6soivrp0901_base_price = base_price
         tta6soivrp0901_base_margin = base_margin
         tta6soivrp0901_idh_due_date = idh_due_date
         tta6soivrp0901_idh_type = idh_type
         .

      ASSIGN
         tta6soivrp0901_ih_channel = ih_channel
         tta6soivrp0901_ih_project = ih_project

         tta6soivrp0901_idh_site = idh_site
         tta6soivrp0901_idh_prodline = idh_prodline
         tta6soivrp0901_idh_taxc = idh_taxc
         tta6soivrp0901_idh_taxable = idh_taxable
         tta6soivrp0901_idh_tax_in = idh_tax_in
         tta6soivrp0901_idh_acct = idh_acct
         tta6soivrp0901_idh_sub = idh_sub
         tta6soivrp0901_idh_cc = idh_cc
         tta6soivrp0901_idh_project = idh_project
         
         tta6soivrp0901_ar_effdate = ar_effdate

         tta6soivrp0901_ext_price = ext_price
         tta6soivrp0901_ext_margin = ext_gr_margin
         tta6soivrp0901_ext_tax = tax_amt + l_tax_in
         tta6soivrp0901_base_tax = tax_amt + l_tax_in
         .
      /* SS - 20070726.1 - E */
