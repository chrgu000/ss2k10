...
/* $Revision: 1.8.1.10 $  BY: Paul Donnelly  DATE: 01/04/02  ECO: *N16J*  */
/* $Revision: 1.8.1.10 $  BY: Bill Jiang  DATE: 04/17/07  ECO: *SS - 20070417.1*  */
...
/* $Revision: 1.8.1.10 $ BY: Paul Donnelly (SB) DATE: 06/26/03 ECO: *Q00B* */
/*-Revision end---------------------------------------------------------------*/
...
report_loop:
{&APCKRPA-P-TAG3}
for each ap_mstr  where ap_mstr.ap_domain = global_domain and (  ap_type = "CK"
   ) no-lock,
      each ck_mstr no-lock  where ck_mstr.ck_domain = global_domain and (
      ck_ref = ap_ref
   ...
   */
   {ssapckrp0001i.i &sort1="ap_mstr.ap_batch"}
   /* SS - 20070417.1 - E */
