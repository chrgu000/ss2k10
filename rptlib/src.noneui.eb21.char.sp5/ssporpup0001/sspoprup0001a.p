...
/* $Revision: 1.12.1.18 $ BY: Bill Jiang  DATE: 02/24/08 ECO: *SS - 20080224.1* */
/*-Revision end---------------------------------------------------------------*/
...
/* SS - 20080224.1 - B */
/*
for each po_mstr no-lock  where po_mstr.po_domain = global_domain and
      po_stat <> "c" and po_stat <> "x"
      and po_nbr      >= ponbr     and po_nbr      <= ponbr1
*/
FOR FIRST prh_hist NO-LOCK
   WHERE RECID(prh_hist) = l_prh_recid 
   ,each po_mstr no-lock  where po_mstr.po_domain = global_domain and
         po_stat <> "c" and po_stat <> "x"
   AND po_nbr = prh_nbr
/* SS - 20080224.1 - E */
   ...
   for each pod_det
      ...
      */
      ...
      /* SS - 20080224.1 - B */
      l_pvod_curr_amt = ROUND(pod_pur_cost * (100 - pod_disc_pct) / 100,5).
      UNDO,NEXT.
      /* SS - 20080224.1 - E */
