...
/* Revision: 1.10.1.19    BY: Paul Donnelly (SB) DATE: 06/26/03 ECO: *Q00C* */
/* $Revision: 1.10.1.20 $   BY: Bhavik Rathod     DATE: 04/06/06 ECO: *P4NK* */
/*-Revision end---------------------------------------------------------------*/
...
define variable l_disDate as character format "x(6)" no-undo.
...
for each fabd_det
   fields( fabd_domain fabd_fa_id fabd_fabk_id fabd_facls_id fabd_entity
   fabd_yrper)
   ...
   first fa_mstr
      /* SS - 090423.1 - B
      fields( fa_domain fa_id fa_desc1 fa_dep fa_puramt fa_disp_dt)
      SS - 090423.1 - E */
      ...
      /* SS - 090423.2 - B */
      /* 仅限于此前已经增加的资产 */
      AND STRING(YEAR(fa_startdt)) + SUBSTRING(STRING(100 + MONTH(fa_startdt)),2,2) <= l-yrper1
      /* SS - 090423.2 - E */
      no-lock
   ...
   IF fa_disp_dt <> ? THEN DO:
      IF STRING(YEAR(fa_disp_dt)) + SUBSTRING(STRING(100 + MONTH(fa_disp_dt)), 2) < l-yrper THEN DO:
         NEXT.
      END.
   END.
   /* SS - 090423.1 - E */
   ...
   then do:
      ...
      then
         ...
         for first fabddet
            fields( fabd_domain fabd_fa_id fabd_fabk_id fabd_yrper)
             where fabddet.fabd_domain = global_domain and  fabddet.fabd_fa_id
              = fabd_det.fabd_fa_id
      ...
      SS - 090423.1 - E */
      /* SS - 090423.1 - B */
      {gprunp.i "fapla" "p" "fa-get-accdepa"
         "(input  fa_id,
           input  fabd_fabk_id,
           input  l_begyrper,
           output accDepr)"}
      ...
      /* OR EQUAL TO l-yrper1, SET l_zeronbv TO YES TO        */
      /* DISPLAY ZERO NET BOOK VALUE.                         */
      if l-yrper1 >= l_disDate
         and l_disDate <> ""
      then
         l_zeronbv = yes.
      ...
      if can-find(first fabddet
                   ...
                   fabddet.fabd_fa_id    =  fabd_det.fabd_fa_id
                  and   fabddet.fabd_fabk_id  =  fabd_det.fabd_fabk_id
   ...
   then do:
      ...
      then do with frame f-2:
         ...
         /* SS - 090423.1 - B */
         CREATE ttxxfaderp0001.
         ASSIGN
            ttxxfaderp0001_fabd_entity = fabd_entity
            ttxxfaderp0001_fabd_fabk_id = fabd_fabk_id
            ttxxfaderp0001_fabd_facls_id = fabd_facls_id

            ttxxfaderp0001_fabd_fa_id = fabd_det.fabd_fa_id
            ttxxfaderp0001_fa_desc1 = fa_mstr.fa_desc1
            ttxxfaderp0001_costAmt = costAmt
            ttxxfaderp0001_accDepr = accDepr
            ttxxfaderp0001_expAmt = expAmt
            ttxxfaderp0001_netBook = netBook
            ttxxfaderp0001_annDepr = annDepr

            ttxxfaderp0001_fa_puramt = fa_mstr.fa_puramt
            .
         /* SS - 090423.1 - E */
      end. /* IF printTot = no */
