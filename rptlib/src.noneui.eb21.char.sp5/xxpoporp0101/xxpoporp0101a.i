...
/* Revision: 1.19  BY: Shilpa Athalye     DATE: 08/26/03   ECO: *N2JF*        */
/* $Revision: 1.20 $    BY: Rajaneesh S.       DATE: 09/12/03   ECO: *N2KY*        */
/*-Revision end---------------------------------------------------------------*/
...
for each po_mstr  where po_mstr.po_domain = global_domain and (
...
) no-lock break by {&sort1} by {&sort2} by {&sort3}
with frame {&frame1} width 132 no-attr-space no-box:
   ...
   if first-of(po_nbr) then do for ship-to:
      ...
      /*  SS - 090227.1 - B  */
      {gprun.i ""xxpoporp0101b.p"" "(input po_nbr)"}.
      /*  SS - 090227.1 - E */
   ...
   then
   display
      pod_line pod_req_nbr pod_part pod_um pod_qty_ord
      qty_open base_cost  pod_disc_pct ext_cost pod_due_date
      pod_wo_lot pod_so_job pod_status.
   else
   display
      pod_line pod_req_nbr pod_part pod_um pod_qty_ord
      qty_open pod_pur_cost @ base_cost pod_disc_pct
      ext_cost pod_due_date
      pod_wo_lot pod_so_job pod_status.
   ...
   if can-find(first poc_ctrl  where poc_ctrl.poc_domain = global_domain and
   poc_ers_proc = yes) then do:
      put
   ...
   /* SS - 090227.1 - B */
      ...
      /* 写入临时表 */
      /*
      CREATE ttxxpoporp0101.
      */
   FIND FIRST ttxxpoporp0101a NO-LOCK NO-ERROR.
   IF AVAILABLE ttxxpoporp0101a THEN DO:
       CREATE ttxxpoporp0101.
       BUFFER-COPY ttxxpoporp0101a TO ttxxpoporp0101.
       ASSIGN
     ttxxpoporp0101.ttxxpoporp0101_pod_line = pod_line
     ttxxpoporp0101.ttxxpoporp0101_pod_req_nbr = pod_req_nbr
     ttxxpoporp0101.ttxxpoporp0101_pod_part = pod_part
     ttxxpoporp0101.ttxxpoporp0101_desc1 = desc1
     ttxxpoporp0101.ttxxpoporp0101_desc2 = desc2
     ttxxpoporp0101.ttxxpoporp0101_pod_um = pod_um
     ttxxpoporp0101.ttxxpoporp0101_pod_rev = pod_rev
     ttxxpoporp0101.ttxxpoporp0101_pod_ers_opt = pod_ers_opt
     ttxxpoporp0101.ttxxpoporp0101_pod_pr_lst_tp = pod_pr_lst_tp
     ttxxpoporp0101.ttxxpoporp0101_pod_vpart = pod_vpart
     ttxxpoporp0101.ttxxpoporp0101_pod_qty_ord = pod_qty_ord
     ttxxpoporp0101.ttxxpoporp0101_qty_open = qty_open
     ttxxpoporp0101.ttxxpoporp0101_base_cost = base_cost
     ttxxpoporp0101.ttxxpoporp0101_pod_disc_pct = pod_disc_pct
     ttxxpoporp0101.ttxxpoporp0101_ext_cost = ext_cost
     ttxxpoporp0101.ttxxpoporp0101_pod_due_date = pod_due_date
     ttxxpoporp0101.ttxxpoporp0101_pod_wo_lot = pod_wo_lot
     ttxxpoporp0101.ttxxpoporp0101_pod_so_job = pod_so_job
     ttxxpoporp0101.ttxxpoporp0101_pod_status = pod_status
     .
   END.
      /* SS - 090227.1 - E */
   ...
   ASSIGN 
        ttxxpoporp0101.ttxxpoporp0101_base_cost = pod_pur_cost.
