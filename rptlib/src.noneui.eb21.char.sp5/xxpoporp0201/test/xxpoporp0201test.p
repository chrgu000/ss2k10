...
repeat:
   ...
   /* 清空临时表 */
   EMPTY TEMP-TABLE ttxxpoporp0201.

   /* 调用报表类公用程序 */
   {gprun.i ""xxpoporp0201.p"" "(
        input part,
        input part1,
        input nbr,
        input nbr1,
   ...
   EXPORT DELIMITER ";" "pod_part" "desc1" "pod_rev" "po_vend" "name" "pod_nbr" "pod_line" "pod_qty_ord" "qty_open" "pod_um" "base_cost" "ext_cost" "pod_due_date" "pod_per_date" "po_buyer" "pod_type".

   /* 输出用分号分隔的临时表 */
   FOR EACH ttxxpoporp0201:
      EXPORT DELIMITER ";" ttxxpoporp0201.
   END.
