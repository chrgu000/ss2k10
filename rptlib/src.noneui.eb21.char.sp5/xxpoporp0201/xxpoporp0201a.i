...
with frame {&file1}  width 132 no-attr-space no-box:
   ...
   if last-of(pod_rev) then do:
      ...
      display
         ...
         um @ pod_um
         accum total by pod_rev (ext_cost) @ ext_cost.
   ...
   if last({&sort1}) then do:
      ...
      display
         ...
         um @ pod_um
         accum total (ext_cost) @ ext_cost.
       SS - 090223.1 - E */
             /* SS - 090223.1 - B */


      /* 写入临时表 */
      CREATE ttxxpoporp0201.
      ASSIGN
	    ttxxpoporp0201_pod_part = pod_part
	    ttxxpoporp0201_desc1 = desc1
	    ttxxpoporp0201_pod_rev = pod_rev
	    ttxxpoporp0201_po_vend = po_vend
	    ttxxpoporp0201_name = name
	    ttxxpoporp0201_pod_nbr = pod_nbr
	    ttxxpoporp0201_pod_line = pod_line
	    ttxxpoporp0201_pod_qty_ord = pod_qty_ord
	    ttxxpoporp0201_qty_open = qty_open
	    ttxxpoporp0201_pod_um = pod_um
	    ttxxpoporp0201_base_cost = base_cost
	    ttxxpoporp0201_ext_cost = ext_cost
	    ttxxpoporp0201_pod_due_date = pod_due_date
	    ttxxpoporp0201_pod_per_date = pod_per_date
	    ttxxpoporp0201_po_buyer = po_buyer
	    ttxxpoporp0201_pod_type = pod_type
    .
      /* SS - 090223.1 - E */
