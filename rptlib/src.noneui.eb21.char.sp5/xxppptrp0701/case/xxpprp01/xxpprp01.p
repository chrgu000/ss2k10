...
DEFINE VARIABLE DESC_pl LIKE pl_desc.
/* SS - 090322.1 - E */
...
repeat:
   ...
   */
   ...
   /*
   EXPORT DELIMITER ";" "地点" "库位" "物料号" "批/序号" "说明" "ABC" "库存量" "UM" "总账成本" "总账成本合计" "非寄售" "客户寄售" "供应商寄售".
   EXPORT DELIMITER ";" "标准输出" "标准输出" "标准输出" "扩展输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "标准输出" "扩展输出" "扩展输出" "扩展输出".
   EXPORT DELIMITER ";" "site" "loc" "part" "lot" "desc" "abc" "qty" "um" "sct" "ext" "qty_non_consign" "qty_cust_consign" "qty_supp_consign".
   */
   EXPORT DELIMITER ";" "运行标志" "提交人" "生效日期" "产品线" "描述" "料号" "地点" "库位" "库存数量" "库存金额" "更新日期" "更新时间" "更新人".
   FOR EACH ttxxppptrp0701
      :
      FIND FIRST pt_mstr WHERE pt_domain = GLOBAL_domain AND pt_part = ttxxppptrp0701_part NO-LOCK NO-ERROR.
      ...
      EXPORT DELIMITER ";" 
         STRING(YEAR(TODAY)) + SUBSTRING(STRING(100 + MONTH(TODAY)), 2) + SUBSTRING(STRING(100 + DAY(TODAY)), 2) + REPLACE(STRING(TIME, "hh:mm:ss"),":","")
         GLOBAL_userid
         AS_of_date
         prod_line_pt
         DESC_pl
