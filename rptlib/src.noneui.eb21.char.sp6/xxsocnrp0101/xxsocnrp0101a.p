/* socnrp1a.p - Consignment Cross Reference Sub-pgm - Sort ShipFrom/Cust      */
...
define variable site_description like si_desc no-undo.
...
define variable curr_error1      as character format "x(80)" no-undo.
define variable curr_error2      as character format "x(80)" no-undo.
...
PROCEDURE get_ship_name:
   ...
   for first si_mstr no-lock
        where si_mstr.si_domain = global_domain and  si_site = ip_shipto:
       op_shipname = si_desc.
   end.
   if not available si_mstr then
      for first ad_mstr no-lock
           where ad_mstr.ad_domain = global_domain and  ad_addr = ip_shipto:
          op_shipname = ad_name.
      end.
...
PROCEDURE get_site_name:
   ...
   for first si_mstr no-lock
        where si_mstr.si_domain = global_domain and  si_site = ip_shipfrom:
       op_sitename = si_desc.
   end.
...
PROCEDURE get_cust_name:
   ...
   for first cm_mstr no-lock
        where cm_mstr.cm_domain = global_domain and  cm_addr = ip_cust:
...
PROCEDURE get_part_desc:
   ...
   for first pt_mstr no-lock
        where pt_mstr.pt_domain = global_domain and  pt_part = ip_part:
...
for each cncix_mstr no-lock  where cncix_mstr.cncix_domain = global_domain and
 ...
 break by cncix_site
   ...
   if first-of(cncix_sod_line) then do:
      ...
      for first bf_sod_det no-lock  where bf_sod_det.sod_domain = global_domain
      and
   ...
   /* SS - 090325.1 - B */
   CREATE ttxxsocnrp0101.
   ASSIGN
      ttxxsocnrp0101_cncix_site = cncix_site
      ttxxsocnrp0101_site_description = site_description
      ttxxsocnrp0101_cncix_so_nbr = cncix_so_nbr
      ttxxsocnrp0101_cncix_sod_line = cncix_sod_line
      ttxxsocnrp0101_cncix_cust = cncix_cust
      ttxxsocnrp0101_cust_description = cust_description
      ttxxsocnrp0101_cncix_shipto = cncix_shipto
      ttxxsocnrp0101_ship_description = ship_description
      ttxxsocnrp0101_cncix_part = cncix_part
      ttxxsocnrp0101_part_description = part_description
      ttxxsocnrp0101_cncix_po = cncix_po
      ttxxsocnrp0101_max_aging_days = max_aging_days

      ttxxsocnrp0101_cncix_current_loc = cncix_current_loc
      ttxxsocnrp0101_cncix_lotser = cncix_lotser
      ttxxsocnrp0101_cncix_auth = cncix_auth
      ttxxsocnrp0101_cncix_qty_stock = cncix_qty_stock
      ttxxsocnrp0101_cncix_stock_um = cncix_stock_um
      ttxxsocnrp0101_cncix_qty_ship = cncix_qty_ship
      ttxxsocnrp0101_cncix_asn_shipper = cncix_asn_shipper
      ttxxsocnrp0101_cncix_ship_date = cncix_ship_date

      ttxxsocnrp0101_cncix_ref = cncix_ref
      ttxxsocnrp0101_cncix_intransit = cncix_intransit
      ttxxsocnrp0101_cncix_aged_date = cncix_aged_date
      .

   assign
     sales_order = cncix_so_nbr
     qoh_value = cncix_ship_value.
   ...
   ASSIGN
      ttxxsocnrp0101_cncix_ship_value = cncix_ship_value
      ttxxsocnrp0101_cncix_curr = cncix_curr
      ttxxsocnrp0101_qoh_base_curr = qoh_base_curr
      ttxxsocnrp0101_gl_base_curr = gl_base_curr
      ttxxsocnrp0101_curr_error1 = curr_error1
      ttxxsocnrp0101_curr_error2 = curr_error2
      .
   /* SS - 090325.1 - E */
