...
/* Revision: 1.39     BY: Paul Donnelly (SB)    DATE: 06/28/03   ECO: *Q00L*  */
/* Revision: 1.41     BY: Swati Sharma          DATE: 02/27/04   ECO: *P1R4*  */
/* Revision: 1.42     BY: Ajay Nair             DATE: 03/31/04   ECO: *P1W7*  */
/* Revision: 1.43     BY: Sachin Deshmukh       DATE: 07/07/04   ECO: *P268*  */
/* Revision: 1.44     BY: Sushant Pradhan       DATE: 08/09/04   ECO: *P2DW*  */
/* Revision: 1.45     BY: Dan Herman            DATE: 10/21/04   ECO: *P2QS*  */
/* Revision: 1.46     BY: Shivganesh Hegde      DATE: 01/04/05   ECO: *Q0G5*  */
/* Revision: 1.47     BY: Bhagyashri Shinde     DATE: 02/01/05   ECO: *P365*  */
/* Revision: 1.47.1.1 BY: Priyank Khandare      DATE: 03/21/05   ECO: *P339*  */
/* Revision: 1.47.1.2 BY: Karel Groos           DATE: 04/01/05   ECO: *P2BV*  */
/* Revision: 1.47.1.3 BY: Alok Gupta            DATE: 08/02/05   ECO: *P3WH*  */
/* Revision: 1.47.1.4 BY: Alok Gupta            DATE: 10/03/05   ECO: *P43Q*  */
/* Revision: 1.47.1.5 BY: Nishit V              DATE: 02/02/06   ECO: *P4GY*  */
/* Revision: 1.47.1.6 BY: Tejasvi Kulkarni      DATE: 10/24/06   ECO: *P552*  */
/* $Revision: 1.47.1.7 $ BY: Ruma Bibra            DATE: 06/22/07   ECO : *Q170* */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
...
define shared variable disp_curr as character format "x(2)" label "C".
...
define variable l_idh_bo_chg like idh_bo_chg  initial 0.
...
FOR EACH ar_mstr NO-LOCK
   ...
   AND ar_effdate >= eff_dt
   AND ar_effdate <= eff_dt1
   AND ar_nbr >= inv
   AND ar_nbr <= inv1
   ...
   DO:
/* SS - 100819.1 - E */
for each ih_hist  where ih_hist.ih_domain = global_domain and (  
      /* SS - 100819.1 - B
   (ih_inv_nbr >=
inv) and (ih_inv_nbr <= inv1)
      SS - 100819.1 - E */
      /* SS - 100819.1 - B */
      (ih_inv_nbr = ar_nbr)
      /* SS - 100819.1 - E */
   ...
   if can-find (first idh_hist
      where idh_hist.idh_domain = global_domain
      and   idh_inv_nbr         = ih_inv_nbr
      and   idh_nbr             = ih_nbr)
      and not can-find (first idh_hist
              where idh_hist.idh_domain = global_domain
              and   idh_nbr             = ih_nbr
              and   idh_inv_nbr         = ih_inv_nbr
              and (idh_fsm_type = "fsm-ro"  or
                   idh_fsm_type = "rma-iss" or
                   idh_fsm_type = "rma-rct" or
                   ((idh_contr_id >= po) and (idh_contr_id <= po1))))
   then
      next.
   ...
   name = "".
   find ad_mstr  where ad_mstr.ad_domain = global_domain and  ad_addr = ih_cust
   ...
   detail_lines = no.
   for each idh_hist  where idh_hist.idh_domain = global_domain and (
   idh_inv_nbr = ih_inv_nbr
      ...
      ext_price = ext_price * idh_qty_inv.

      if  idh_fsm_type <> "SC"
      and idh_fsm_type <> "SC-ADD"
      then do:
         /* ROUND PER DOCUMENT CURRENCY */
         {gprunp.i "mcpl" "p" "mc-curr-rnd"
             "(input-output ext_price,
      ...
      or idh_fsm_type = "SC-ADD"
      then do:
         /* ROUND OFF THE GROSS MARGIN BEFORE CALCULATING THE        */
      ...
      ext_gr_margin = idh_qty_inv * ext_gr_margin.
        ...
        /* CHECK AUTHORIZATION RECORDS FOR DIFFERENT EXTENDED PRICE */
         ...
         then do:
            ...
            assign
              ...
              ext_gr_margin = auth_price - (curr_cost * idh_qty_inv) .
         end. /*IF idh_sched */
      ...
      idh_part no-lock no-wait no-error.
      ...
      then do:
         ...
         for first itm_det
               fields( itm_domain itm_nbr itm_line itm_part itm_qty_call)
                where itm_det.itm_domain = global_domain and  itm_nbr = idh_nbr
         ...
         then
            ...
            for first itmh_hist
                  fields( itmh_domain itmh_nbr itmh_line itmh_part
                  itmh_qty_call)
                   where itmh_hist.itmh_domain = global_domain and  itmh_nbr =
                   idh_nbr
      ...
      then do:
         ...
         then do:
            if idh_qty_ord >= 0
            then /* show bo regardless of sign */
      ...
      end. /* IF CAN-FIND sod_det */

      else
         display l_idh_bo_chg @ idh_bo_chg with frame c.

      display
         disp_curr
         base_net_price format "->>>>>,>>9.99<<<"
         base_price
         base_margin
      ...
      /* SS - 100819.1 - B */
      CREATE ttxxsoivrp0901.

      ASSIGN
         ttxxsoivrp0901_ih_inv_nbr = ih_inv_nbr
         ttxxsoivrp0901_ih_rev = ih_rev

         ttxxsoivrp0901_ih_cust = ih_cust
         ttxxsoivrp0901_name = NAME
         ttxxsoivrp0901_ih_po = ih_po

         ttxxsoivrp0901_ih_ship =ih_ship
         ttxxsoivrp0901_ih_ship_date = ih_ship_date
         ttxxsoivrp0901_inv_date = ih_inv_date
         ttxxsoivrp0901_ih_slspsn[1] = ih_slspsn[1]
         ttxxsoivrp0901_ih_slspsn[2] = ih_slspsn[2]
         ttxxsoivrp0901_ih_slspsn[3] = ih_slspsn[3]
         ttxxsoivrp0901_ih_slspsn[4] = ih_slspsn[4]
         ttxxsoivrp0901_ih_curr = ih_curr
         ttxxsoivrp0901_base_curr = base_curr
         ttxxsoivrp0901_ih_ex_rate = ih_ex_rate
         ttxxsoivrp0901_ih_ex_rate2 = ih_ex_rate2
         ttxxsoivrp0901_ih_exru_seq = ih_exru_seq
         ttxxsoivrp0901_v_disp_line1 = v_disp_line1
         ttxxsoivrp0901_v_disp_line2 = v_disp_line2
         ttxxsoivrp0901_ih_rmks = ih_rmks
         .

      ASSIGN
         ttxxsoivrp0901_idh_nbr = idh_nbr
         ttxxsoivrp0901_idh_line = idh_line
         ttxxsoivrp0901_idh_part = idh_part
         ttxxsoivrp0901_idh_um = idh_um
         .

      IF ih_fsm_type = "FSM-RO" THEN DO:
         ASSIGN
            ttxxsoivrp0901_idh_qty_ord = l_qty_call
            ttxxsoivrp0901_idh_qty_inv = l_qty_call
            .
      END.
      ELSE DO:
         ASSIGN
            ttxxsoivrp0901_idh_qty_ord = idh_qty_ord
            ttxxsoivrp0901_idh_qty_inv = idh_qty_inv
            .
      END.
      ...
      then do:
         IF ih_fsm_type <> "FSM-RO" THEN DO:
            IF idh_qty_ord >= 0 THEN DO:
               ASSIGN
                  ttxxsoivrp0901_idh_bo_chg = max(idh_qty_ord - idh_qty_ship, 0)
                  .
            END.
            ELSE DO:
               ASSIGN
                  ttxxsoivrp0901_idh_bo_chg = MIN(idh_qty_ord - idh_qty_ship, 0)
                  .
            END.
         END.
         ELSE DO:
            ASSIGN
               ttxxsoivrp0901_idh_bo_chg = ttxxsoivrp0901_idh_bo_chg
               .
         END.
      END.
      ELSE DO:
         ASSIGN
            ttxxsoivrp0901_idh_bo_chg = l_idh_bo_chg
            .
      END.

      ASSIGN
         ttxxsoivrp0901_disp_curr = DISP_curr
         ttxxsoivrp0901_base_net_price = base_net_price
         ttxxsoivrp0901_base_price = base_price
         ttxxsoivrp0901_base_margin = base_margin
         ttxxsoivrp0901_idh_due_date = idh_due_date
         ttxxsoivrp0901_idh_type = idh_type
         .

      ASSIGN
         ttxxsoivrp0901_ih_channel = ih_channel
         ttxxsoivrp0901_ih_project = ih_project

         ttxxsoivrp0901_idh_site = idh_site
         ttxxsoivrp0901_idh_prodline = idh_prodline
         ttxxsoivrp0901_idh_taxc = idh_taxc
         ttxxsoivrp0901_idh_taxable = idh_taxable
         ttxxsoivrp0901_idh_tax_in = idh_tax_in
         ttxxsoivrp0901_idh_acct = idh_acct
         ttxxsoivrp0901_idh_sub = idh_sub
         ttxxsoivrp0901_idh_cc = idh_cc
         ttxxsoivrp0901_idh_project = idh_project
         
         ttxxsoivrp0901_ar_effdate = ar_effdate

         ttxxsoivrp0901_ext_price = ext_price
         ttxxsoivrp0901_ext_margin = ext_gr_margin
         .
      ...
      {gprun.i ""txtotal1.p"" "(input '16',
                                ...
                                output l_tax_in)"}
      ASSIGN
         ttxxsoivrp0901_ext_tax = tax_amt + l_tax_in
         ttxxsoivrp0901_base_tax = tax_amt + l_tax_in
         .
      /* SS - 100819.1 - E */
      ...
      if using_line_charges then do:
         ...
         if can-find(first absl_det
            ...
            and   absl_order        = idh_nbr
            and   absl_ord_line     = idh_line)
         then do:
            for each absl_det
               fields (absl_confirmed absl_inv_nbr absl_inv_post absl_lc_amt
                      absl_order absl_ord_line absl_trl_code absl_domain
                      absl_ext_price)
               where  absl_domain   = global_domain
               and    absl_order    = idh_nbr
               and    absl_ord_line = idh_line
               and    (absl_inv_nbr = "" or absl_inv_nbr = idh_inv_nbr)
         ...
         else if not available absl_det then
         do:
            for each idhlc_hist
               fields (idhlc_inv_nbr idhlc_line idhlc_nbr idhlc_part idhlc_price
                       idhlc_trl idhlc_domain)
               where idhlc_domain  = global_domain
               and   idhlc_nbr     = idh_nbr
               and   idhlc_inv_nbr = idh_inv_nbr
               and   idhlc_line    = idh_line
            no-lock:
       ...
       then do:
          for each abscc_det
             ...
             and   abscc_order     = idh_nbr
             and   abscc_ord_line  = idh_line
             and   (abscc_inv_nbr  = "" or abscc_inv_nbr = idh_inv_nbr)
             and   abscc_confirm   = yes
             and   abscc_inv_post  = yes
          no-lock:
                if abscc_taxable = yes
                then
                   l_tax_amt    = l_tax_amt    + abscc_cont_price.
                else
                   l_nontax_amt = l_nontax_amt + abscc_cont_price.
   ...
   then do:
      l_conso = yes.
      {gprun.i ""soihtrl2.p"" "(input l_conso)"}
