...
/** ss - 20120525 - KEY *0525
	借用pt_rev栏位存周期，现在pt_rev的存储内容为非数字出现转换错误。
...
/*0525   FIELD empty1_t LIKE pt_ord_per     LABEL "订货周期"   */
/*0525*/ FIELD empty1_t LIKE pt_rev     LABEL "订货周期"
...
FOR EACH npod_det where npod_due_date = 12/12/12,
    ...
    EACH vd_mstr WHERE vd_addr = vend BREAK BY mrp_part BY mrp_due_date:
      ...
      if not available empty1 then do:
         ...
         empty1_qty = mrp_qty
/*0525   empty1_t = int(pt_rev)         /*送货周期*/                   */
/*0525*/ empty1_t = pt_rev         /*送货周期*/
...
/*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& 修改时间: 07/12/13&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
	 ...
	 do:
/*0525	    if abs(empty1_duedate - empty2_duedate) < empty1_t then */
/*0525*/ if abs(empty1_duedate - empty2_duedate) < 14 then
	       empty2_qty = empty2_qty + empty1_qty. 
/*0525         if abs(empty1_duedate - empty2_duedate) >= empty1_t then  */
				 if abs(empty1_duedate - empty2_duedate) >= 14 then
	       do:
	          XX:
		   repeat:
		    empty1_duedate = empty1_duedate - 1.
...
/*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& 修改时间: 07/12/13&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/                                                                                        
         ...
         IF empty1_yn = 1 THEN
            DO:
/*0525     if abs(empty1_duedate - empty2_duedate) < empty1_t then   /*同是可送货日但日期间隔小于送货周期的,将数量合并.*/  */
/*0525*/ if abs(empty1_duedate - empty2_duedate) < 14 then   /*同是可送货日但日期间隔小于送货周期的,将数量合并.*/
	          empty2_qty = empty2_qty + empty1_qty. 
