...
/* REVISION: 8.6     LAST MODIFIED: 03/09/98    BY: *K1L2* Beena Mol          */
/* REVISION: 8.6     LAST MODIFIED: 05/20/98    BY: *K1Q4* Alfred Tan         */
/* REVISION: 8.6     LAST MODIFIED: 06/03/98    BY: *K1RS* Shobha Appukuttan  */
...
/* $Revision: 1.13.1.4.4.1 $  BY: Robin McCarthy      DATE: 07/08/05  ECO: *P2PJ* */
/* $Revision: 1.13.1.4.4.1 $  BY: Bill Jiang      DATE: 11/28/06  ECO: *SS - 20061128.1* */

/* SS - 20061128.1 - B */
/*
1. 解决了输出到"terminal"时每屏只能显示一行的问题
*/
/* SS - 20061128.1 - E */
...
repeat:
   ...
   if part <> "" then
      repeat for prh_hist with frame b width 80 
      /* SS - 20061128.1 - B */
      DOWN
      /* SS - 20061128.1 - E */
   ...
   for each prh_hist where (prh_nbr = nbr) no-lock,
       ...
       each pvod_det no-lock where
            pvod_id = pvo_id
      with frame c width 80 on endkey undo, leave loopc:
      /* SET EXTERNAL LABELS */
   ...
   if vend <> "" then
      repeat for prh_hist with frame d width 80 on endkey undo, leave:
      ...
      if available prh_hist then do:
         ...
         for each pvo_mstr no-lock where
            ...
            if available pod_det then display pod_so_job.
   ...
   if receiver <> "" then
      repeat for prh_hist with frame e width 80 on endkey undo, leave:
   ...
   if rcp_date <> ? then
      repeat for prh_hist with frame f width 80 on endkey undo, leave:
   ...
   for each prh_hist where prh_nbr >= "" and prh_receiver >= "" and
      ...
      with frame h width 80 on endkey undo, leave:
      /* SET EXTERNAL LABELS */
