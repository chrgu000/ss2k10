...
/* Revision: 1.1.1.2   BY: Vijaykumar Patil     DATE: 03/27/06  ECO: *P4MH* */
/* Revision: 1.1.1.3  BY: Vijaykumar Patil     DATE: 03/30/06  ECO: *P4MH* */
/* $Revision: 1.1.1.4 $  BY: Mochesh Chandran     DATE: 06/12/06  ECO: *P4TF* */
/* $Revision: 1.1.1.1 $      BY: Bill Jiang       DATE: 06/12/06  ECO: *SS - 20060612.1* */
/* $Revision: 1.1.1.1 $      BY: Bill Jiang       DATE: 06/16/06  ECO: *SS - 20060616.1* */
/* $Revision: 1.1.1.1 $      BY: Bill Jiang       DATE: 02/16/08  ECO: *SS - 20080216.1* */
...
/* SS - 20060612.1 - B */
/*
1. 取消了如下条件:and   cncix_qty_stock   > 0
*/
/* SS - 20060612.1 - E */
...
define input        parameter invoice_db        as   character          no-undo.
...
then do:
      ...
      break by cncix_ship_date:
         ...
         then do:
           if first(cncix_ship_date) then
            ...
            then do:
               for first loc_mstr no-lock where loc_loc = cncix_current_loc:
               end.
               if available loc_mstr
               then do:
                  for first is_mstr no-lock where is_status = loc_status :
                  end.
   ...
   break by cncix_ship_date:
      ...
      then do:
         ...
         end.  /* IF (ac_tot_qty_consumed) > l_consigned_qty */
 ...
 * -------------------------------------------------------------------------- */
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first so_mstr
         fields (so_curr so_ex_ratetype so_fix_rate so_nbr)
         where so_nbr = ip_order
      no-lock:
 ...
 * -------------------------------------------------------------------------- */
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first sod_det
         fields (sod_sched sod_curr_rlse_id sod_site sod_cum_qty
                 sod_auto_replenish)
         where sod_nbr    = ip_order
 ...
 * -------------------------------------------------------------------------- */
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      /* GET SCHEDULE MASTER */
      for first sch_mstr
         fields (sch_cumulative sch_pcr_qty)
         where   sch_nbr     = ip_order
 ...
 * -------------------------------------------------------------------------- */
   ...
   for each b_ConsignedShipment
      where b_ConsignedShipment.cncix_part        = p_cpart
