...
/* $Revision: 1.62.1.5 $ BY: Bill Jiang DATE: 10/08/06 ECO: *SS - P200* */
/* $Revision: 1.62.1.5 $ BY: Bill Jiang DATE: 10/08/06 ECO: *SS - 20061008.1* */
...
ECO Date: 05/27/04 
ECO Release: 9.2W 
ECO First Released in: SP7 
ECO Class: A 
ECO Modified File Count: 1 
ECO Compile List File Count: 1 
...
In multi-user environment,Transaction Post (25.13.7 - gltrpst.p) now verifies the availability of records before posting the transactions and skips the transactions if they are locked, thereby avoiding the possibility of posting unbalanced transactions due to simultaneous updates. Previously, in the above scenario, transactions simultaneously getting updated were posted without considering the resultant balanced/unbalanced status of the transactions.  
...
define new shared temp-table ttglt_det no-undo like glt_det
   index glt_ref glt_ref glt_rflag glt_line.
...
repeat:
   ...
   repeat:
      ...
      find first ttglt_det where glt_ref > ref no-lock no-error.
      ...
      */
      ...
      loopa:
      {&GLTRPST-P-TAG27}
      for each ttglt_det where glt_ref > ref
      ...
      {&GLTRPST-P-TAG28}
         ...
         */
         ...
         if gltcurr_rndmthd <> oldgltcurr_rndmthd then do:
            ...
            find rnd_mstr where rnd_rnd_mthd = gltcurr_rndmthd
            no-lock no-error.
         ...
         if last-of(ttglt_det.glt_ref) then do:
            ...
            for each w1 where w1.glt_ref = ref with frame b:
            ...
            */
            ...
            if unb = yes then do:
               ...
               for each det where det.glt_ref = ref:
                  det.glt_unb = yes.
            ...
            */
            ...
            if entities_balanced and ent_flag then do:
               ...
               no-lock break by det.glt_entity:
                  ...
                  find ttac_mstr where ac_code = det.glt_acc
                  no-lock no-error.
            ...
            */
            ...
            exclusive-lock:
               ...
               if can-find(ttal_mstr where al_code = det.glt_acc)
               then
                  assign
                     det.glt_amt      = 0
                     det.glt_curr_amt = 0
...
PROCEDURE ip-create-hist:
   ...
   {&GLTRPST-P-TAG38}
   find ttac_mstr where ac_code = gltr_acc
   no-lock no-error.
   ...
   do transaction:
      ...
      if co_cont_page and post_yn then do:
         if co_enty_bal and entity = entity1 then do:
            find en_mstr where en_entity = entity.
...
PROCEDURE ip-create-retearn:
   /* CREATE RETAINED EARNINGS OFFSET FOR ACCTS CREATED BY EXPLODING
      ALLOCATION CODE */
   find ttac_mstr where ac_code = det.glt_acc no-lock no-error.
...
PROCEDURE ip-explode-verif:
   ...
   find ttal_mstr where al_code = ttglt_det.glt_acc no-lock no-error.
   ...
   /* VERIFY ACCOUNT INFORMATION */
      ...
      if unb then
         ...
         find ttac_mstr where ac_code = ttglt_det.glt_acc
         no-lock no-error.
...
PROCEDURE ip-upd-error:
   ...
   define input parameter p-error as char no-undo.
...
PROCEDURE ip-gltrpstb:
   ...
   find ttac_mstr where ac_code = ttglt_det.glt_acct no-lock no-error.
   ...
   then do:
      ...
      find sb_mstr where sb_sub = ttglt_det.glt_sub no-lock no-error.
   ...
   then do:
      ...
      find cc_mstr where cc_ctr = ttglt_det.glt_cc no-lock no-error.
   ...
   if ttglt_det.glt_project <> "" and
      p-error-msg    = ""
   then do:
      find pj_mstr where pj_project = ttglt_det.glt_project
      no-lock no-error.
   ...
   if daybooks-in-use and glt_dy_code <> "" and p-error-msg = ""
   then do:

      find dy_mstr where dy_dy_code = ttglt_det.glt_dy_code
      no-lock no-error.
