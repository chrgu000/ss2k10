...
&SCOPED-DEFINE apmcmt_p_1 "Ctrl"
/* MaxLen: Comment: */

&SCOPED-DEFINE apmcmt_p_2 "Due Date"
/* MaxLen: Comment: */

&SCOPED-DEFINE apmcmt_p_3 "Draft"
/* MaxLen: Comment: */

&SCOPED-DEFINE apmcmt_p_4 "Disc Bal"
/* MaxLen: Comment: */

&SCOPED-DEFINE apmcmt_p_5 "Batch"
/* MaxLen: Comment: */

&SCOPED-DEFINE apmcmt_p_6 "Amt to Disc"
/* MaxLen: Comment: */

&SCOPED-DEFINE apmcmt_p_7 "Check Ctrl"
/* MaxLen: Comment: */

&SCOPED-DEFINE apmcmt_p_8 "Balance"
/* MaxLen: Comment: */

&SCOPED-DEFINE apmcmt_p_9 "Amt to Apply"
/* MaxLen: Comment: */
...
define new shared variable batch        like ap_batch
   label {&apmcmt_p_5}.
define new shared variable amt_open     like ap_amt
   label {&apmcmt_p_8}.
define new shared variable amt_disc     like ap_amt
   label {&apmcmt_p_4}.
{&APMCMT-P-TAG1}
define new shared variable aptotal      like ap_amt
   ...
   format ">>>>>,>>>,>>9.99".
{&APMCMT-P-TAG2}
define new shared variable apply_amt    like ap_amt
   label {&apmcmt_p_9}.
define new shared variable apply_disc   like ap_amt
   label {&apmcmt_p_6}.
define new shared variable gen_desc     like glt_desc.
define new shared variable ckdamt       like ckd_amt.
define new shared variable ap_due_date  like ap_date
   label {&apmcmt_p_2}.
...
do for apc_ctrl:
   ...
   /* MOVED LOGIC FOR GENERATING JOURNAL REFERENCE TO LOOPB BELOW */
   /* SO THAT NEXT JOURNAL IS NOT INCREMENTED WHEN THE USER EXITS */
   /* THE FUNCTION FROM BATCH, CTRL, BANK OR CHECK FIELD          */
   ...
   do transaction:
      ...
      if batch <> "" then do:
         ...
         if available ap_mstr then do:
            find last ck_mstr where ck_ref = ap_ref no-lock no-error.
            if available ck_mstr then
               bank = string(ck_bank, "X(2)").
         end.
         if available ap_mstr and ap_type <> "CK" then do:
   ...
   repeat with frame b:
      ...
      do transaction:
         ...
         do on error undo, retry:
            ...
            find bk_mstr where bk_code = input ck_bank
            exclusive-lock no-error.
            if not available bk_mstr then do:
               {pxmsg.i &MSGNUM=1200 &ERRORLEVEL=3} /*INVALID BANK*/
               {&APMCMT-P-TAG11}
               undo, retry.
            end.
            ...
            prompt-for ck_nbr
               ...
               if frame-field = "ck_nbr" and batch <> "" then do:
                  ...
                  {mfnp06.i ap_mstr ap_batch "batch = ap_batch" ap_ref
                     ref ap_batch batch}
                  ...
                  if recno <> ? then do:
                     ...
                     assign ap_amt:format in frame b = ap_amt_fmt
                     ...
                     find vd_mstr where vd_addr = ap_vend no-lock
                        no-error.
            ...
            {gprun.i ""apmcmtb.p"" "(input   bk-action,
                 input   bk_code,
            ...
            /* THIS PROGRAM                                      */
            for first ck_mstr
               fields (ck_nbr ck_ref ck_type ck_status
               ck_bank ck_curr ck_exru_seq
               ck_ex_rate ck_ex_rate2 ck_ex_ratetype)
               where ck_ref = string(input ck_bank, "x(2)" ) +
               {&APMCMT-P-TAG44}
               string(input ck_nbr)
               {&APMCMT-P-TAG45}
            no-lock:
            ...
            for first ck_mstr
               fields (ck_bank ck_nbr ck_ref ck_curr
               ck_exru_seq ck_ex_rate
               ck_ex_rate2 ck_ex_ratetype
               ck_status ck_type)
               where ck_ref = ap_ref no-lock:
            end. /* FOR FIRST CK_MSTR.... */
            if available ck_mstr and ck_nbr <> input ck_nbr or
               not available ck_mstr then do:
               /* ADDING NEW RECORD */
            ...
            if available ck_mstr then
            find first ap_mstr where ap_ref = ck_ref
               and ap_type = "CK"
            no-lock no-error.
      ...
      do transaction on error undo, leave:
         ...
         do while true:
            ...
            if (available ck_mstr  and (ck_nbr <> input ck_nbr     or
               ...
               find bk_mstr where bk_code = string(ck_bank, "X(2)")
               no-lock no-error.
            ...
            if del-yn then do
               on error undo mainloop, leave:
                  /* DELETE PAYMENT */
                  for each ckd_det exclusive-lock where ckd_ref = ap_ref:
                     assign
                     ...
                     if base_curr <> ap_mstr.ap_curr then do:
                        ...
                        if ckd_voucher <> "" then do:
                           find vo_mstr where vo_mstr.vo_ref = ckd_voucher
                           no-lock no-error.
                     ...
                     if ckd_voucher <> "" then do:
                        find vo_mstr where vo_mstr.vo_ref = ckd_voucher
                        exclusive-lock no-error.
                        find apmstr where apmstr.ap_ref = vo_mstr.vo_ref
                        ...
                        find vd_mstr where vd_addr = ap_vend
                        exclusive-lock no-error.
               ...
               find vd_mstr where vd_addr = input ap_vend
               exclusive-lock
                  no-error.
               display vd_sort.
               {&APMCMT-P-TAG35}
               ...
               find first ckd_det where ckd_ref = ap_mstr.ap_ref
               no-lock no-error.
               ...
               repeat:   /* TRAP UNDO */
                  if available ap_mstr then do:
                     ...
                     if aptotal <> apamt and aptotal <> 0 then do:
                        bell.
                        {mfmsg03.i 1163 1 aptotal apamt ctrldiff}
