...
&SCOPED-DEFINE apvoco01_p_1 "Mixed Currencies"
/* MaxLen: Comment: */

&SCOPED-DEFINE apvoco01_p_3 "Assigned-To"
/* MaxLen: Comment: */

&SCOPED-DEFINE apvoco01_p_4 "Effective (Blank to not change)"
/* MaxLen: Comment: */

&SCOPED-DEFINE apvoco01_p_6 "Confirm"
/* MaxLen: Comment: */

&SCOPED-DEFINE apvoco01_p_12 "Voucher Date"
/* MaxLen: Comment: */

&SCOPED-DEFINE apvoco01_p_13 "Summary/Detail"
/* MaxLen: Comment: */
...
define variable apdate like ap_date label {&apvoco01_p_12}.
...
define variable confirm like mfc_logical
   label {&apvoco01_p_6} initial yes.
...
define variable summary like mfc_logical format {&apvoco01_p_13}
   label {&apvoco01_p_13}.
...
form
   skip (1)
   batch          colon 15
   batch1         colon 45 label {t001.i}
   vend           colon 15
   vend1          colon 45 label {t001.i}
   voucher        colon 15
   voucher1       colon 45 label {t001.i}
   votype         colon 15
   votype1        colon 45 label {t001.i} skip
   apdate         colon 15
   apdate1        colon 45 label {t001.i}
   entity         colon 15
   entity1        colon 45 label {t001.i}
   authorized     colon 15 label {&apvoco01_p_3}
   authorized1    colon 45 label {t001.i} skip(1)
   effdate        colon 32
   confirm        colon 32
   summary        colon 32
   base_rpt       colon 32
...
form
   vo__qad01 column-label {&apvoco01_p_3}
   vo_ref
   ap_vend
   name
   apeffdate
...
form
   ...
   ap_curr
   base_amt
with frame e no-labels width 132 down.
...
repeat:
   ...
   voucherloop:
   for each ap_mstr where ap_type = "VO"
         ...
         no-lock,
         each vo_mstr where vo_ref = ap_ref and vo_confirmed = no
      ...
      if global_db <> "" then do:
         for each vph_hist
            ...
            if first-of(si_db)
               ...
               put {gplblfmt.i
                  &FUNC=getTermLabel(""DATABASE_NOT_AVAILABLE"",29)
                  &CONCAT="': '"
                  } si_db
                  skip(1).
      ...
      do transaction on error undo voucherloop, leave:
         ...
         if confirm = yes then do:
            ...
            if effdate <> ? then
            do for apmstr:
               find apmstr where recid(apmstr) = ap_recno
                  exclusive-lock.
               ap_effdate = effdate.
               release apmstr.
            end.
         ...
         for each vod_det where vod_ref = vo_ref no-lock:
            ...
            if summary = no then do:
               ...
               display detl_amt @ base_amt vod_desc with frame d.
               down with frame d.
