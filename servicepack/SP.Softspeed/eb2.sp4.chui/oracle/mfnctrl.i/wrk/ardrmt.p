...
/* $Revision: 1.20.1.24 $  BY: Manjusha Inglay  DATE: 07/29/02 ECO: *N1P4*   */
/******************************************************************************/
...
repeat with frame a:
   ...
   repeat with frame b:
      ...
      do transaction on error undo, retry:
         ...
         find ba_mstr where ba_batch = batch
         ...
         do on error undo, retry:
            ...
            if del-yn then do:
               ...
               if ar_applied <> 0 then do:
                  ardnbr = "".
                  find first ard_det where ard_ref = ar_nbr no-lock
                     no-error.
               ...
               if old_amt <> 0 then do:
                  ...
                  find cm_mstr where cm_addr = old_cust
                     exclusive-lock.
            ...
            find cm_mstr where cm_addr = input ar_bill no-lock.
            display cm_sort.
            if new ar_mstr then do:
               assign
                  old_cust     = ar_bill
               ...
               do counter = 1 to 4:
                  if ar_slspsn[counter] <> "" then do:
                     find sp_mstr where sp_addr = ar_slspsn[counter]
                        no-lock no-error.
         ...
         if ar_disc_date = ? or ar_due_date = ? then do:
            ...
            if not available ct_mstr or ct_dating = no then do:
               ...
               find last ctd_det where ctd_code = ar_cr_terms
                  no-lock no-error.
...
PROCEDURE p_round:
   ...
   for last glt_det
      fields(glt_batch glt_tr_type glt_ref)
      where glt_batch   = batch and
      glt_tr_type = "AR"
      no-lock:
         l_ref = glt_ref.
