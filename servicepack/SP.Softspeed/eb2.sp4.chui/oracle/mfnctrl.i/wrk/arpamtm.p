...
define new shared variable ref          as character format "X(14)" no-undo.
...
repeat with frame a:
         ...
         do while true:
            ...
            if not available ar_mstr then do: /* abc */
               ...
               if can-find(first cu_mstr where cu_curr >= "")
                  ...
                  find first bk_mstr where bk_curr = ar_curr
                  no-lock no-error.
               else
               if cash_book then
                  find bk_mstr where bk_code = bank
                  no-lock no-error.
               {&ARPAMTM-P-TAG14}
               ...
               {&ARPAMTM-P-TAG15}
               /* VARIANCE ACCOUNT AND CC FROM CU_MSTR */
               /* GAIN ACCT AND CC ASSIGNED TEMPORARILY */
               /* ACTUAL ACCT AND CC ASSIGNED IN ARARGL2.P */
               for first cu_mstr
                  fields (cu_rgain_acct cu_rgain_sub cu_rgain_cc)
                  where cu_curr = ar_curr
                  no-lock:
               end.
               if available cu_mstr then
                  assign
                     ar_var_acct = cu_rgain_acct  /* R. GAIN ACT. */
                     ar_var_sub  = cu_rgain_sub   /* R. GAIN SUB-ACCT */
                     ar_var_cc   = cu_rgain_cc.   /* R. GAIN CC   */
               display
                  ar_amt
                  ar_type
                  ar_batch.
               ...
               if bank = "" and ar_curr <> base_curr then
                  find first bk_mstr where bk_acct = ar_acct
                  and bk_sub = ar_sub
                  and bk_cc = ar_cc no-lock no-error.
               else
                  find first bk_mstr where bk_code = ar_bank
                  no-lock no-error.
               {&ARPAMTM-P-TAG17}
               ...
               if auto_apply then auto_apply = false.
               ...
               if not cash_book then do:
                  find ba_mstr where ba_batch = ar_batch
                     and ba_module = "CM" no-lock no-error.
                  if available ba_mstr then do:
                     find cb_mstr where cb_batch = ba_batch
            ...
            old_amt = ar_amt.
            for each ard_det where ard_nbr = ar_nbr and ard_type <> "N":
               old_amt = old_amt + ard_disc.
            end.
            find first ard_det where ard_nbr = ar_nbr
               and ard_ref = ""
            ...
            do on error undo, retry:
               ...
               do on error undo, retry:
                  ...
                  if base_curr <> ar_curr and new ar_mstr then do:
                     ...
                     then
                        assign
                           ar_ex_rate = bank_ex_rate
                           ar_ex_rate2 = bank_ex_rate2
                  ...
                  if new ar_mstr then do:
                     ...
                     then do:
                        {&ARPAMTM-P-TAG22}
                        find first bk_mstr where bk_code = bank
                           no-lock no-error.
                        ...
                        if available bk_mstr and
                           ...
                           /* VARIANCE ACCOUNTS RETRIEVED FROM CU_MSTR */
                           ...
                           if available cu_mstr then
                              assign
                                 ar_var_acct = cu_rgain_acct
                                 ar_var_sub  = cu_rgain_sub
                                 ar_var_cc   = cu_rgain_cc.
                           if is_transparent then  /*BANK EURO TRANSPARENT*/
                           do:
                     ...
                     then do:
                        ...
                        /*ABOVE VARIANCE ACCOUNTS RETRIEVED FROM CU_MSTR */
                        /* JUST A TEMPORARY ASSIGNMENT,*/
                        /* GAIN OR LOSS ACCT ASSIGNED LATER IN ARARGL2.P */
                        for first cu_mstr
                           fields (cu_rgain_acct cu_rgain_sub cu_rgain_cc)
                     ...
                     do on error undo, retry:
                        ...
                        do:
                           if not can-find (en_mstr
                              where en_entity = ar_entity) then
                           do:
                        ...
                        if daybooks-in-use and not cash_book and
                           ...
                           if can-find(dy_mstr where dy_dy_code = ar_dy_code)
                           then do:
                           ...
                           if ar_curr <> base_curr then do:
                              find ac_mstr where ac_code = ar_acct
                                 no-lock no-error.
            ...
            find cm_mstr where cm_addr = ar_cust no-lock
               no-error.
            ...
            if cash_book and not new_line then do:
            ...
            find cm_mstr where cm_addr = ar_mstr.ar_bill exclusive-lock
               no-error.

            find first ard_det where ard_nbr = ar_mstr.ar_nbr no-lock
               no-error.
            ...
            then do:
               ...
               then do for armstr_temp:
                  ...
                  for each armstr_temp
                     fields (ar_bill ar_type ar_date)
                     where ar_bill = cm_addr and
                           ar_type = "P"
                     no-lock:
   ...
   /* CALL THE PROCEDURE nr_dispense IN nrm.p TO GENERATE SEQ NOS. */
   if ref <> ""
      and daybooks-in-use
   then do:
      ...
      /* UPDATE ard_dy_num WITH nrm-seq-num */
      for each ard_det
         where ard_nbr = ar_nbr
         exclusive-lock:
         assign
            ard_dy_num  = nrm-seq-num
            ard_dy_code = ar_dy_code.
      end. /* FOR EACH ard_det */

      /* UPDATE gld_dy_num WITH nrm-seq-num */
      for each glt_det
         where glt_ref = ref
         exclusive-lock:
