...
define shared variable using_grs as log no-undo.
...
repeat:
   do transaction:
      ...
      editing:
         ...
         {mfnp.i req_det req_nbr req_nbr req_nbr req_nbr req_nbr}
         ...
         if recno <> ? then do:
            ...
            can-find(first cmt_det where cmt_indx = req_cmtindx).
   ...
   do transaction:
      ...
      find req_det where req_nbr = reqnbr
      ...
      do with frame a on error undo, retry:
         ...
         if req_po_site <> "" then do:
            if not can-find(si_mstr where si_site = req_po_site) then do:
