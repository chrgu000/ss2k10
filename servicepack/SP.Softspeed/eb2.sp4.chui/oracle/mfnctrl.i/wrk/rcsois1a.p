...
/* Revision: 1.100.1.2       BY: Subramanian Iyer  DATE: 09/10/03 ECO: *P12N* */
/* Revision: 1.100.1.3       BY: Orawan S.         DATE: 09/29/03 ECO: *P14K* */
/* Revision: 1.100.1.4       BY: Veena Lad         DATE: 10/28/03 ECO: *P17K* */
/* Revision: 1.100.1.5       BY: Kirti Desai       DATE: 11/12/03 ECO: *P195* */
/* Revision: 1.100.1.7       BY: Ed van de Gevel   DATE: 12/02/03 ECO: *P1BX* */
/* Revision: 1.100.1.8       BY: Vinay Soman       DATE: 12/30/03 ECO: *N2NZ* */
/* Revision: 1.100.1.9       BY: Karan Motwani     DATE: 01/05/04 ECO: *P1HP* */
/* Revision: 1.100.1.10      BY: Vinay Soman       DATE: 01/15/04 ECO: *P1JP* */
/* Revision: 1.100.1.12      BY: Vinay Soman       DATE: 01/29/04 ECO: *P1LR* */
/* Revision: 1.100.1.15      BY: Salil Pradhan     DATE: 03/09/04 ECO: *P1GM* */
/* Revision: 1.100.1.16      BY: Vinay Soman       DATE: 03/18/04 ECO: *P1TW* */
/* Revision: 1.100.1.18      BY: Robin McCarthy    DATE: 03/20/04 ECO: *P15V* */
/* Revision: 1.100.1.20      BY: Vinay Soman       DATE: 04/01/04 ECO: *P1WK* */
/* Revision: 1.100.1.23      BY: Sunil Fegade      DATE: 06/24/04 ECO: *P27K* */
/* Revision: 1.100.1.25      BY: Shivganesh Hegde  DATE: 08/03/04 ECO: *P26L* */
/* Revision: 1.100.1.26      BY: Sunil Fegade      DATE: 09/29/04 ECO: *P2MD* */
/* Revision: 1.100.1.27      BY: Manish Dani       DATE: 10/28/04 ECO: *P2RT* */
/* Revision: 1.100.1.29      BY: Reena Ambavi      DATE: 11/05/04 ECO: *P2T1* */
/* Revision: 1.100.1.30      BY: Jignesh Rachh     DATE: 12/01/04 ECO: *P2XJ* */
/* Revision: 1.100.1.31      BY: Binoy John        DATE: 01/27/05 ECO: *P35M* */
/* Revision: 1.100.1.32      BY: Preeti Sattur     DATE: 03/15/05 ECO: *P3CL* */
/* Revision: 1.100.1.33      BY: Preeti Sattur     DATE: 03/31/05 ECO: *P3DD* */
/* Revision: 1.100.1.34      BY: Jignesh Rachh     DATE: 03/31/05 ECO: *P3DG* */
/* Revision: 1.100.1.37      BY: Bhagyashri Shinde DATE: 04/01/05 ECO: *P2CR* */
/* Revision: 1.100.1.39      BY: Shivagensh Hegde  DATE: 06/27/05 ECO: *P3LK* */
/* Revision: 1.100.1.40      BY: Priya Idnani      DATE: 07/05/05 ECO: *P3N3* */
/* Revision: 1.100.1.41      BY: Jignesh Rachh     DATE: 08/01/05 ECO: *P3V0* */
/* $Revision: 1.100.1.42 $              BY: Jignesh Rachh     DATE: 09/28/05 ECO: *P3Z2* */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
   ...
   do transaction:
      ...
      break by work_abs_mstr.abs_order
         ...
         then do:
            ...
            for first abs_comp
               where abs_comp.abs_shipfrom = abs_work.abs_shipfrom
         ...
         if (available abs_comp
             or (not available abs_comp
                 and can-find(first sod_det
                              where sod_nbr          = abs_work.abs_order
         ...
         then do:
            ...
            for first so_mstr
               fields (so_app_owner so_bill so_bol so_ca_nbr
                       ...
                       so_trl2_amt so_trl3_amt so__qadc01 so_sched)
            ...
            else if sod_cum_date[2] < eff_date - 1
            then
               assign
                  sod_cum_date[2] = eff_date - 1
   ...
   then do transaction:
      ...
      do i = 1 to inv_ct:
         ...
         no-lock
            ...
            /* sod_contr_id ADDED TO FIELDS LIST */
            for each sod_det
               fields (sod_bo_chg sod_cum_date sod_cum_qty
                       ...
                       sod_um sod_um_conv sod__qadl01 sod_cfg_type
   ...
   no-lock:
      ...
      for first sod_det
         fields (sod_bo_chg sod_cum_date sod_cum_qty sod_fsm_type
                 ...
                 sod__qadl01
                 sod_cfg_type sod_contr_id sod_disc_pct sod_promise_date)
 ...
 *-------------------------------------------------------------------------*/
   ...
   no-lock:
      ...
      if (confirm_mode = yes and work_abs_buff.abs_qty > 0 ) or
...
no-lock :
   if    ((can-find (first absl_det
                   where absl_abs_id           = work_abs_mstr.abs_id
      ...
      or
         (can-find(first absl_det
                   where absl_order            = work_abs_mstr.abs_order
                   and   string(absl_ord_line) = work_abs_mstr.abs_line
                   and   absl_abs_shipfrom     = work_abs_mstr.abs_shipfrom
                   and   absl_confirmed        = yes
      ...
      or ((can-find (first abscc_det
                    where abscc_abs_id           = work_abs_mstr.abs_id
         ...
         (can-find(first abscc_det
                    where abscc_order            = work_abs_mstr.abs_order
                    and   string(abscc_ord_line) = work_abs_mstr.abs_line
                    and   abscc_abs_shipfrom     = work_abs_mstr.abs_shipfrom
                    and   abscc_confirmed        = yes
