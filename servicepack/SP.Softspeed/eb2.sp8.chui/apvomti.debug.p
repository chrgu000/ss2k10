...
/* Revision: 1.26.2.47.1.2 BY: Gnanasekar        DATE: 09/18/03 ECO: *P139*  */
/* Revision: 1.26.2.47.1.3 BY: Manjusha Inglay   DATE: 10/27/03 ECO: *P175*  */
/* Revision: 1.26.2.47.1.5 BY: Ed van de Gevel   DATE: 12/01/03 ECO: *N2N7* */
/* Revision: 1.26.2.47.1.6 BY: Vandna Rohira     DATE: 01/30/04 ECO: *P1LT* */
/* Revision: 1.26.2.47.1.7 BY: Nishit V          DATE: 02/24/04 ECO: *P1Q9* */
/* Revision: 1.26.2.47.1.8 BY: Vandna Rohira     DATE: 09/03/04 ECO: *P2J0* */
/* $Revision: 1.26.2.47.1.9 $ BY: Vinay Nayak-Sujir     DATE: 10/07/04 ECO: *P2NL* */
/* $Revision: 1.26.2.47.1.9 $ BY: Bill Jiang     DATE: 06/26/06 ECO: *SS - 20060626.1* */
/* $Revision: 1.26.2.47.1.9 $ BY: Bill Jiang     DATE: 06/27/06 ECO: *SS - 20060627.1* */
/* $Revision: 1.26.2.47.1.9 $ BY: Bill Jiang     DATE: 08/02/06 ECO: *SS - 20060802.1* */
   ...
   for each tx2d_det where tx2d_det.tx2d_ref = pvo_internal_ref and
      ...
      then do:
         ...
         then do:
            ...
            MESSAGE "1:" + STRING(tax_acct) + STRING(rcv_tax) VIEW-AS ALERT-BOX.
        run update_vod_det
               (rcv_tax).
        ...
        /* THE TAX AMOUNT SHOULD BE ADDED TO THE TOTAL AMOUNT */
         ...
         if rcpt_det.tx2d_cur_recov_amt <> 0 then do:
            ...
            then do:
               ...
               MESSAGE "2:" + STRING(tax_acct) + STRING(- recov_amt) VIEW-AS ALERT-BOX.
               run update_vod_det
                  (- recov_amt).
            ...
            {&APVOMTI-P-TAG19}
               MESSAGE "3:" + STRING(tax_acct) + STRING(recov_amt) VIEW-AS ALERT-BOX.
            run update_vod_det
               (recov_amt).
      ...
      then do:
         ...
         MESSAGE "4:" + STRING(tax_acct) + STRING(rcv_tax) VIEW-AS ALERT-BOX.
         run update_vod_det
            (rcv_tax).
      ...
      then do:
         ...
         then do:
            MESSAGE "5:" + STRING(tax_acct) + STRING(recov_amt + nonrecov_amt) VIEW-AS ALERT-BOX.
            run update_vod_det
         ...
         then do:
            ...
            /* CREATE/MODIFY VOD_DET FOR PO RECEIPTS ACCOUNT */
            MESSAGE "6:" + STRING(tax_acct) + STRING(recov_adj + nonrecov_adj) VIEW-AS ALERT-BOX.
            run update_vod_det
...
for each tx2d_det where tx2d_det.tx2d_ref = vo_ref and
      ...
      */
      ...
      /* FIRST DO PO RECEIPTS TAXES */
         ...
         /* SINCE HERE por_acct HAS THE VALUE OF vph_acct    */
            ...
            MESSAGE "7:" + STRING(tax_acct) + STRING(rcv_tax - rcv_rec - rcv_rec_discrep) VIEW-AS ALERT-BOX.
      run update_vod_det
      ...
      MESSAGE "8:" + STRING(tax_acct) + STRING(rcv_rec_discrep) VIEW-AS ALERT-BOX.
      run update_vod_det
         (rcv_rec_discrep).
      ...
      {&APVOMTI-P-TAG21}
      /* CREATE/MODIFY VOD_DET FOR AP TAX ACCOUNT */
         MESSAGE "9:" + STRING(tax_acct) + STRING(rcv_rec) VIEW-AS ALERT-BOX.
      ...
      IF prh_tax_at = "" 
         ...
         assign
            ...
            tax_sub     = ppv_sub
            tax_cc      = ppv_cc.
                             MESSAGE "10:" + STRING(tax_acct) + STRING(- rcv_rec) VIEW-AS ALERT-BOX.
                             run update_vod_det (- rcv_rec).
      END.
      /* SS - 20060626.1 - E */
      ...
      if ppv_tax <> 0 then do:
         ...
         /* CREATE/MODIFY VOD_DET FOR RATE VARIANCE ACCOUNT */
                             ...
                             MESSAGE "11:" + STRING(tax_acct) + STRING(ppv_tax) VIEW-AS ALERT-BOX.
         if not tx2d_det.tx2d_tax_in then
         ...
         then do:
            ...
            MESSAGE "12:" + STRING(tax_acct) + STRING(- ppv_rec - ppv_rec_discrep) VIEW-AS ALERT-BOX.
            run update_vod_det
            ...
            if ppv_rec_discrep <> 0 then do:
               ...
               MESSAGE "13:" + STRING(tax_acct) + STRING(ppv_rec_discrep) VIEW-AS ALERT-BOX.
            ...
            {&APVOMTI-P-TAG23}
            /* CREATE/MODIFY VOD_DET FOR AP TAX ACCOUNT */
               MESSAGE "14:" + STRING(tax_acct) + STRING(ppv_rec) VIEW-AS ALERT-BOX.
      ...
      if usg_tax <> 0 then do:
         ...
         if not tx2d_det.tx2d_tax_in then
         MESSAGE "14:" + STRING(tax_acct) + STRING(usg_tax) VIEW-AS ALERT-BOX.
         if not tx2d_det.tx2d_tax_in then
         ...
         if usg_rec <> 0 then do:
            ...
            MESSAGE "15:" + STRING(tax_acct) + STRING(- usg_rec) VIEW-AS ALERT-BOX.
            ...
            {&APVOMTI-P-TAG25}
            /* CREATE/MODIFY VOD_DET FOR AP TAX ACCOUNT */
               MESSAGE "16:" + STRING(tax_acct) + STRING(usg_rec) VIEW-AS ALERT-BOX.
            run update_vod_det (usg_rec).
            {&APVOMTI-P-TAG26}
   ...
   /* OR RECEIVER LINES TAXED BY TOTAL */
      ...
      then do:
         ...
         {&APVOMTI-P-TAG27}
         /* CREATE/MODIFY VOD_DET FOR AP TAX ACCOUNT */
            MESSAGE "17:" + STRING(tax_acct) + STRING(tx2d_det.tx2d_cur_recov_amt) VIEW-AS ALERT-BOX.
         run update_vod_det
            (tx2d_det.tx2d_cur_recov_amt).
         {&APVOMTI-P-TAG28}
      end.
         ...
         if not tx2d_det.tx2d_tax_in then
            MESSAGE "18:" + STRING(tax_acct) + STRING(tx2d_det.tx2d_cur_tax_amt) VIEW-AS ALERT-BOX.
            run update_vod_det
         ...
         if tx2d_det.tx2d_cur_recov_amt <> 0 then do:
            ...
            MESSAGE "19:" + STRING(tax_acct) + STRING(- tx2d_det.tx2d_cur_recov_amt) VIEW-AS ALERT-BOX.
            run update_vod_det
               (- tx2d_det.tx2d_cur_recov_amt).
            ...
            {&APVOMTI-P-TAG29}
            /* CREATE/MODIFY VOD_DET FOR AP TAX ACCOUNT */
               MESSAGE "20:" + STRING(tax_acct) + STRING(tx2d_det.tx2d_cur_recov_amt) VIEW-AS ALERT-BOX.
            run update_vod_det
               (tx2d_det.tx2d_cur_recov_amt).
            {&APVOMTI-P-TAG30}
   ...
   then do:
      ...
      {&APVOMTI-P-TAG37}
         MESSAGE "21:" + STRING(tax_acct) + STRING(tx2d_det.tx2d_cur_abs_ret_amt) VIEW-AS ALERT-BOX.
      run update_vod_det
...
PROCEDURE update-memo:
   ...
   if tax_amt <> 0 then do:
      ...
      then do:
         ...
         MESSAGE "22:" + STRING(tax_acct) + STRING(- tax_amt) VIEW-AS ALERT-BOX.
         ...
         MESSAGE "23:" + STRING(tax_acct) + STRING(tax_amt) VIEW-AS ALERT-BOX.
         run update_vod_det ( tax_amt).
...
PROCEDURE calculate_gain_loss :
   ...
   then
      ...
      MESSAGE "24:" + STRING(tax_acct) + STRING(- l_gl) VIEW-AS ALERT-BOX.
   ...
   MESSAGE "25:" + STRING(tax_acct) + STRING(l_gl) VIEW-AS ALERT-BOX.   
   run update_vod_det (l_gl).
