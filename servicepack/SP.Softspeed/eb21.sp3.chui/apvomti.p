...
/* $Revision: 1.26.2.60 $ BY: Bill Jiang DATE: 06/26/06 ECO: *SS - 20060626.1* */
/* $Revision: 1.26.2.60 $ BY: Bill Jiang DATE: 06/27/06 ECO: *SS - 20060627.1* */
/*-Revision end---------------------------------------------------------------*/
...
for each tx2d_det where tx2d_det.tx2d_domain = global_domain and
   ...
   then do:
      ...
      for first vph_hist exclusive-lock
      ...
      */
      ...
      run update_vod_det
         (input rcv_tax - rcv_rec - rcv_rec_discrep).
...
PROCEDURE update_vod_det:
   ...
   if tax_amt <> 0 then do:
      ...
      for first vod_det
          fields(vod_domain vod_ref vod_type vod_entity
                 vod_acct vod_sub vod_cc vod_project
                 vod_exp_acct vod_exp_sub vod_exp_cc vod_desc)
           where vod_domain  = global_domain
             and vod_ref     = vo_mstr.vo_ref
