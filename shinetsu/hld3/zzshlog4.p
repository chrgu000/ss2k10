/*zzshlog4.p   Ship Weight Preparation With PackSize*/

def input parameter v_ship_wt  as deci.
def input parameter v_ship_qty as int.
def input parameter v_comp_qty as int.
def input parameter v_pknbr    as int.   /*Pack Size*/
...
if v_debug then do:
    ...
    message "zzshllog4.p  Target:" v_ship_wt v_ship_qty
            "  PackSize:" v_pknbr " Comp:" v_comp_qty.
    pause.
end.
...
for each tb by tb_lotno:
	...
	then do:
	   if v_debug then do:
	      hide message no-pause.
	      message "TotQty:" v_comp_qty + i + 1 "  PKnbr:" v_pknbr 
	              "TotWT:" v_curr_wt + tb_calwt "  TargetWT:" v_ship_wt.
	      pause.
	   end.
	   if (v_comp_qty + i + 1) mod v_pknbr = 0 and v_ship_qty = 0 or
	      v_ship_qty <> 0 then do:
	      v_init = no.
