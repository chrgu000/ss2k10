/*zzshpcrt2.p   Ship Weight Preparation For "Less Than" */
/*Last Modify by Leo Zhou   01/05/2012 */
...
def var v_over_flag1 as char.  /*  有规格限制的通常品(规格内)*/
def var v_over_flag2 as char.  /*  有规格限制的通常品(规格外)*/
def var v_over_flag3 as char.  /*  有规格限制的Defect品*/
...
v_over_flag1 = usrw_charfld[4].
v_over_flag2 = "2".
v_over_flag3 = "2".

/*Defect Rate*/
find first mpd_det no-lock where mpd_domain = global_domain      
     and mpd_nbr = "X7" + v_part and mpd_type = "00122" no-error.
if avail mpd_det then do:                                        
   if mpd_tol <> "" then v_def_rate = deci(mpd_tol) / 100.       
   else v_def_rate = 0.                                          
end.                                                             
else do:                                                         
...
if v_debug then do:
    ...
    message "Part:" v_part " Target:" v_ship_wt v_ship_qty 
            "  DefClass:" v_def_class "  Defect rate:" v_def_rate 
	    " v_defect_wt=" v_defect_wt.
    pause.
end.
...
if v_len_rate = 0 then do:
   if v_def_class = 1 then do:
	...
	for each tf:
        ...
        if v_act_qty = v_ship_qty and v_ship_qty <> 0 then do:
           v_finish = yes.
	   return.
	end.
	...
	for each tf:
        ...
        if v_debug then do:
           hide message no-pause.
           message "After No_Limit_Normal.  3. v_act_wt=" v_act_wt " i=" i.
           pause.
        end.
