...
/*find first xxsod_det where trim(xxsod_cust) = trim(V1003) and trim(xxsod_due_date1)=trim(V1004) and trim(xxsod_due_time1)=trim(V1005) and trim(xxsod_part)=tmpparta /*ENTRY(1, V1300, "@")*/ and  xxsod__dec02=0 NO-LOCK NO-ERROR.
...
tr_site = V1002     and  tr_part = V1302     and tr_serial = V1500   and 
tr_time  + 15 >= TIME 
...
tr_date = today     and 
tr_trnbr > integer ( V9000 ) and 
tr_nbr  = V1551     and  tr_type = "ISS-SO"  and  tr_site = V1002     and tr_part = V1302     and tr_serial = V1500   and 
tr_time  + 15 >= TIME 
use-index tr_date_trn no-lock no-error.
...
tmpint="1".
for each xxsod_det where trim(xxsod_cust) = trim(V1003) and trim(xxsod_due_date1)=trim(V1004) and trim(xxsod_due_time1)=trim(V1005) and 
    /* SS - 20071225.1 - B */ 
    /*trim(xxsod_part)= ENTRY(1, V1300, "@")*/ TRIM(xxsod_part) = custpart 
...
/*SS - 080905.1 B*/
    ...
    FIND FIRST xxsod_det WHERE trim(xxsod_due_date1)=trim(V1004) and trim(xxsod_due_time1)=trim(V1005) AND xxsod_qty_ord <> xxsod__dec01 NO-LOCK NO-ERROR.
    IF NOT AVAIL xxsod_det THEN DO:
     ...
     IF AVAILABLE PRD_DET then do:
       ...
       */  
           ...
           EMPTY TEMP-TABLE ttsod_det.
            ...
            /*
            PUT UNFORMATTED "#def REPORTPATH=$/计划科/仓管组/放行条/" + SUBSTRING(execname, 1, LENGTH(execname) - 2) SKIP.
            PUT UNFORMATTED "#def :end" SKIP.
            FOR EACH ttsod_det NO-LOCK:
                    EXPORT  DELIMITER ";" ttsod_det.
            END.
            */
            
          INPUT FROM VALUE("/app/bc/labels/labelfxt") .
          REPEAT:
          ...
          DO vv_i = 1 TO vv_sum_ttsod:
            ...
            vv_filename = "labelfxt" + STRING(TIME) + string(vv_i) + ".l".

            OUTPUT TO VALUE(vv_filename) .
            FOR EACH ttlbfxt :
              ...
              IF INDEX(ttlbfxt_cmmt,"%PER10D%") <> 0 THEN DO:
                  FIND FIRST ttsod_det NO-LOCK NO-ERROR.
                  IF AVAIL ttsod_det THEN
                     ttlbfxt_cmmt = SUBSTRING(ttlbfxt_cmmt,1,LENGTH(ttlbfxt_cmmt) - 8) + ttsod_time.
              END.
