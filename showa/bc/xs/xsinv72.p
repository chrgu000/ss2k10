...
/* ASSY=>YB-1-1 */
/* Generate date / time  2010-3-9 10:47:26 */
...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv72wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
define variable wxxin_pallet like xxin_pallet.
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[临时=>货架]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[临时=>货架]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
	...
	End.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[临时=>货架]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
...
IF 1 = 1 THEN
	...
	End.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        IF not V1400 <> "ASSY" THEN DO:
                display skip "请输入有效货架" @ WMESSAGE NO-LABEL with fram F1400.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[临时=>货架]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[临时=>货架]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1500.
            ...
            THEN DO:
                  IF recid(XXIN_DET) = ? THEN find first XXIN_DET where 
                              XXIN_PART = V1300 AND XXIN_LOC  = "ASSY" AND XXIN_FB   = no AND  
                              XXIN_LOT >=  INPUT V1500
                               no-lock no-error.
                  else do: 
                       if XXIN_LOT =  INPUT V1500
                       then find next XXIN_DET
                       WHERE XXIN_PART = V1300 AND XXIN_LOC  = "ASSY" AND XXIN_FB   = no
                        no-lock no-error.
            ...
            XXIN_LOT @ V1500 XXIN_LOT @ WMESSAGE NO-LABEL with fram F1500.
...
XXIN_LOT = V1500 and XXIN_LOC = "ASSY"  no-lock no-error.
        IF  ( NOT AVAILABLE XXIN_DET ) OR length ( trim(V1500) ) = 0 then do:
                display skip V1500 + "LOT有误/不能为空" @ WMESSAGE NO-LABEL with fram F1500.
                pause 0 before-hide.
                undo, retry.
        end.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[临时=>货架]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
        ...
        display  skip WMESSAGE NO-LABEL with fram F1700.
         ...
         /*  ---- Valid Check ---- START */
                 ...
                 find first CODE_MSTR where CODE_Fldname = "partLoc" + trim ( V1300 ) and code_value = trim ( V1400 )  no-lock no-error.
        IF  NOT AVAILABLE CODE_MSTR   then do:
                display skip "该货架无效." @ WMESSAGE NO-LABEL with fram F1700.
                pause 0 before-hide.
                undo, retry.
        end.
        {xsckbypartloc.i}
	if xsckbypartloc = no  then do:
                display skip "该货架已满." @ WMESSAGE NO-LABEL with fram F1700.
                pause 0 before-hide.
                Undo, retry.
...
XXIN_LOT = V1500 and XXIN_LOC = "ASSY"  no-lock no-error.
        IF   NOT AVAILABLE XXIN_DET  then do:
                display skip V1500 + "LOT有误/不能为空" @ WMESSAGE NO-LABEL with fram F1700.
                pause 0 before-hide.
                undo, retry.
        end.
...
IF 1 <> 1 then
        leave V9010L.
        /* LOGICAL SKIP END */
                display "[临时=>货架]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
