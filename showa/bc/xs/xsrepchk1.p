...
/* BY: Micho Yang         DATE: 10/10/08  ECO: *SS - 20081010.1*  */
...
FOR EACH tta6bmpkiq :
    ...
    */
    IF v_qty > 0 AND NOT (tta6bmpkiq_part BEGINS "N") THEN DO:
        ...
        CREATE ttdqpt_mstr.
        ASSIGN ttdqpt_cmmt = ErrorMsg.
        /*SS - 080908.1 E*/
        FIND FIRST pt_mstr WHERE pt_part = tta6bmpkiq_part NO-LOCK NO-ERROR.
        IF AVAIL pt_mstr THEN DO:
            FIND FIRST lnd_det WHERE lnd_part = pt_part AND lnd_site = pt_site NO-LOCK NO-ERROR.
            IF AVAIL lnd_det THEN DO:
           
              ASSIGN ttdqpt_cmmt = lnd_line + ":" + ErrorMsg + "  " + pt_desc1 .
            END.
            ELSE DO:

                ASSIGN ttdqpt_cmmt =  ErrorMsg + "  " + pt_desc1 .
            END.
        END.
    END.
    /******************** SS - 20061101.1 - E ********************/
