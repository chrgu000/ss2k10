...
/* ss - 090916.1 -b */
define var v_log01 as logical initial yes .  
define var v_log02 as logical initial yes. 
/* ss - 090916.1 -e */
...
/* ss - 090916.1 -b */
form
   "采购税用途" colon 10                     "销售税用途"  colon 40
   v_log01      colon 15 label "是否输入"    v_log02       colon 40 label "是否输入"
   pt__chr02    colon 15                     pt__chr06     colon 40
   pt__chr03    colon 15                     pt__chr07     colon 40
   pt__chr04    colon 15                     pt__chr08     colon 40
   pt__log01    colon 15                     pt__qad22     colon 40
   pt__log02    colon 15                     pt__qad26     colon 40
   pt__chr05    colon 15
   with frame set_tax row 13 overlay
   centered side-labels attr-space.
...
repeat:
   ...
   do transaction with frame a on endkey undo, leave mainloop:
     ...
     do on error undo , retry:
      view frame set_tax .
      /* ss - 090916.1 -b
      display pt__chr02 pt__chr03 pt__chr04 pt__log01 pt__log02 pt__chr05 with frame set_tax .
      set pt__chr02 pt__chr03 pt__chr04 pt__log01 pt__log02 pt__chr05 with frame set_tax .
        ss - 090916.1 -e */
	/* ss - 090916.1 -b */
	v_log01 = yes .
	v_log02 = yes .
	display  v_log01 pt__chr02 pt__chr03 pt__chr04 pt__log01 pt__log02 pt__chr05 v_log02 pt__chr06 pt__chr07 pt__chr08 pt__qad22 pt__qad26 with frame set_tax .
      set v_log01 pt__chr02 pt__chr03 pt__chr04 pt__log01 pt__log02 pt__chr05 v_log02 pt__chr06 pt__chr07 pt__chr08 pt__qad22 pt__qad26 with frame set_tax .

	/* ss - 090916.1 -e */
	/* ss - 090916.1 -b */
	if v_log01 = yes then do:
	/* ss - 090916.1 -e */
       {pxrun.i &PROC='validateTaxUsage'
                  &PROGRAM='txenvxr.p'
                  &PARAM="(input pt__chr02)"
                  &NOAPPERROR=true
                  &CATCHERROR=true}
         ...
         then do:
            ...
            then do:
               next-prompt pt__chr02 with frame set_tax no-validate.
         ...
         then do:
            if c-application-mode <> "API"
            then do:
               next-prompt pt__chr03 with frame set_tax no-validate.
               undo taxloop, retry taxloop.
            end.  /* If c-application-mode <> "API" */
            else /* c-application-mode = "API" */
               undo taxloop, return error.
	 ...
	 {pxrun.i &PROC='validateTaxClass' &PROGRAM='txenvxr.p'
	   ...
	   then do:
	      if c-application-mode <> "API"
	      then do:
		 next-prompt pt__chr04 with frame set_tax no-validate.
		 undo taxloop, retry taxloop.
	      end.
	      else
		 undo taxloop, return error.
	    ...
	    find first cu_mstr where cu_curr = pt__chr05 no-lock no-error .
	 if not available cu_mstr then do:
	     message " 币别不存在" view-as alert-box .
	     undo taxloop, retry taxloop.
	 end .
          /* ss - 090916.1 -b */
	  end .
	  /* ss - 090916.1 -e */
	   ...
	   /* ss - 090916.1 -b */
	   if v_log02 = yes then do:
	     {pxrun.i &PROC='validateTaxUsage'
                  &PROGRAM='txenvxr.p'
                  &PARAM="(input pt__chr06)"
                  &NOAPPERROR=true
                  &CATCHERROR=true}
         ...
         then do:
            if c-application-mode <> "API"
            then do:
               next-prompt pt__chr06 with frame set_tax no-validate.
               undo taxloop, retry taxloop.
            end.  /* If c-application-mode <> "API" */
            else /* c-application-mode = "API" */
               undo taxloop, return error.
         ...
         then do:
            if c-application-mode <> "API"
            then do:
               next-prompt pt__chr07 with frame set_tax no-validate.
               undo taxloop, retry taxloop.
            end.  /* If c-application-mode <> "API" */
            else /* c-application-mode = "API" */
               undo taxloop, return error.
	 ...
	 {pxrun.i &PROC='validateTaxClass' &PROGRAM='txenvxr.p'
	   ...
	   then do:
	      if c-application-mode <> "API"
	      then do:
		 next-prompt pt__chr08 with frame set_tax no-validate.
		 undo taxloop, retry taxloop.
	      end.
	      else
		 undo taxloop, return error.
