...
with frame list_price side-labels width 80.
 ...
 /* ss - 090908.1 -b */
 form
   pi_list_price  colon 27  pi__chr01   colon 60
   skip(1)
   pi_min_price   colon 27
   pi_max_price   colon 27
with frame list_price side-labels width 80.
...
if pi_amt_type = "1" then do with frame list_price:
   ...
   do on error undo, retry:
	 ...
	 prompt-for pi__chr01 with frame list_price editing :
	    {mfnp01.i code_mstr pi__chr01  code_value   code_fldname ""pi__chr01"" code_fldval}

              if recno <> ? then do:
                    display code_value @ pi__chr01 with frame list_price .
	       end .
	 end .
	 assign pi__chr01 .
	...
	set
	 ...
	 /* ss - 090908.1 -b */
	  find first code_mstr where code_fldname = "pi__chr01" and code_value = pi__chr01 no-lock no-error .
	    if not available code_mstr then do:
	     {pxmsg.i &MSGNUM=5604 &ERRORLEVEL=3}
	     next-prompt pi__chr01 with frame list_price.
	     undo, retry.
	    end .
	 /* ss - 090908.1 -e */
