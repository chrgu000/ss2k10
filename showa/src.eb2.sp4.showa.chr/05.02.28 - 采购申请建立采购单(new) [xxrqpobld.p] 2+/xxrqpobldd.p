/* GUI CONVERTED from rqpobldd.p (converter v1.76) Wed Mar 19 20:44:21 2003 */
...
define buffer rqd_buff             for rqd_det.
...
FORM /*GUI*/ 
   po_buyer
with /* STREAM-IO /*GUI*/ */  frame buyer side-labels width 132.
...
FORM /*GUI*/ 
   rqd_vend
with /* STREAM-IO /*GUI*/  */ frame vend side-labels width 132.
...
with /* STREAM-IO /*GUI*/ */  frame curr side-labels width 132.
...
with /* STREAM-IO /*GUI*/  */ frame um side-labels width 132.
...
FORM /*GUI*/ 
   ...
   pod_um        label "PO Unit of Measure"
with /* STREAM-IO /*GUI*/  */ frame um_change side-labels width 132.
...
with /* STREAM-IO /*GUI*/  */ frame base_cost side-labels width 132.
...
FORM /*GUI*/ 
   pod_rev
with /* STREAM-IO /*GUI*/ */  frame rev side-labels width 132.
...
with /* STREAM-IO /*GUI*/ */   frame price side-labels width 132.
...
FORM /*GUI*/ 
   ...
   pod_so_job
with /* STREAM-IO /*GUI*/ */  frame po_line_rpt width 132 down.
...
do with frame po_header_rpt:
   ...
   display
      ...
      po_vend
      po_ship
   with frame po_header_rpt side-labels width 132 /* STREAM-IO */ /*GUI*/ .
end. /* do with */
...
if not {gpcode.v po_buyer} then do:
   ...
   display
      po_buyer
   with frame buyer /* STREAM-IO /*GUI*/ */  . 
   run put_message (input 716, input 2).
end.

/*ZH002      find first rqpo_wrk no-lock.*/
/*ZH002*/    find first rqpo_wrk no-lock where rqpo_supplier = vendor.
...
/*ZH002      rqpo_copy_to_po no-lock*/
/*ZH002*/    rqpo_copy_to_po and rqpo_supplier = vendor no-lock
   ...
   if not available pod_det then do:
      ...
      display
         ...
         ext_cost
         pod_so_job
      with frame po_line_rpt width 132 down /* STREAM-IO /*GUI*/ */ .
      ...
      if pod_um <> wkrqd_um then do:
         display
            ...
            wkrqd_um
         with frame um_change /* STREAM-IO /*GUI*/ */.
...
/*ZH002 ADD************************************************/
    ...
    find pt_mstr where pt_part = pod_part 
      ...
      if avail pt_mstr then do:
         for each rqpo4_wrk where rqpo4_supplier = po_vend and rqpo4_site = pod_site
                             and rqpo4_item = pod_part break by rqpo4_nbr:

              find first rqd_det
                   where rqd_nbr = rqpo4_nbr and rqd_line = rqpo4_line exclusive-lock no-error.
		      ...
		      {gprun.i ""rqporef.p""
                ...
                if first-of(rqpo4_nbr) then do:
                              
                   logical = no.
                   for each rqd_buff where rqd_buff.rqd_nbr = rqpo4_nbr no-lock:
                       if rqd_buff.rqd_status = "" and rqd_open = yes then do:
                          logical = yes.
                          leave.
                       end.
                   end.
                   
                   if not logical then do:
                      find rqm_mstr where rqm_nbr = rqpo4_nbr exclusive-lock no-error.
...
/*ZH002 END************************************************/
      ...
      then do:
         ...
         display
            base_cost
         with frame base_cost /* STREAM-IO /*GUI*/ */ .
         run put_message (input 716, input 2).
      end.
      ...
      if var_req_cost <> 0 then do:
         if dummy_cost <> 0 and
            ...
            display
               var_req_cost
               dummy_cost
            with frame price /* STREAM-IO /*GUI*/ */ .
...
/*ZH002 END*************************************************************/
      ...
      if using_consignment and pod_consignment then
         display c-consigment-contract no-label /* WITH STREAM-IO /*GUI*/ */ .
...
/*CHAR*/ {mfrpchk.i } /*Replace mfrpchk*/
