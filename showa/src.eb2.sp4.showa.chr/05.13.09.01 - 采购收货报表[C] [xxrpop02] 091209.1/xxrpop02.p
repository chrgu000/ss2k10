/* ss - 090908.1 by: jack */
/* ss - 091013.1 by: jack */
/* ss - 091019.1 by: jack */  /* 修改当检验选择全部时输出未检验部分*/
/* ss - 091102.1 by: jack */
/* ss - 091110.1 by: jack */
/* ss - 091209.1 by: jack */  /* 增加图纸图号提示*/
...
/* ss  - 091209.1 -b
export delimiter ";" "供应商" "供应商名称" "结算供应商" "收货日期" "收货单" "订单" "项次" "零件" "数量" "订单单价" "币别" "汇率" "原币金额" /* "关税(原币)" "手续费(原币)" "增值税(原币)" */ "发票价格(原币)" "发票数量"  "库存账户" "库存分账户" "成本中心" "税用途" "装箱单"  "检验状态" "类型" . /* 发票价格为0*/
ss - 091209.1 -e */

/* ss - 091216.1 -b
/* ss - 091209.1 -b */
export delimiter ";" "供应商" "供应商名称" "结算供应商" "收货日期" "收货单" "订单" "项次" "零件" "零件说明"  "图纸图号"  "数量" "订单单价" "币别" "汇率" "原币金额" /* "关税(原币)" "手续费(原币)" "增值税(原币)" */ "发票价格(原币)" "发票数量"  "库存账户" "库存分账户" "成本中心" "税用途" "装箱单"  "检验状态" "类型" . /* 发票价格为0*/
/* ss - 091209.1 -e */
ss - 091216.1 -e */

/* ss - 091216.1 -b */
export delimiter ";" "供应商" "供应商名称" "结算供应商" "收货日期" "收货单" "订单" "项次" "零件" "零件说明"  "图纸图号"  "数量" "订单单价" "币别" "汇率" "原币金额" /* "关税(原币)" "手续费(原币)" "增值税(原币)" */ "发票价格(原币)" "发票数量"  "库存账户" "库存分账户" "成本中心" "税用途" "装箱单"  "检验状态" "类型"  "采购员"  . /* 发票价格为0*/
/* ss - 091216.1 -e */
   ...
   /*  if v_logical = yes then do: */
		...
		and ( uninv_only = no 
	     ...
	     find first pod_det where pod_nbr = tr_nbr and pod_line = tr_line   no-lock no-error .
          ...
          ELSE DO:
            /* ss - 091110.1 -b
               FIND FIRST xxmqc_det WHERE xxmqc_rcp_date = tr_effdate AND xxmqc_vend = tr_addr AND xxmqc_part = tr_part AND xxmqc_serial = tr_serial AND ( ( v_check <> "" AND xxmqc_status = v_check) OR v_check = "" )  NO-LOCK NO-ERROR .
               IF AVAILABLE xxmqc_det  THEN DO:
                ss - 091110.1 -e */
              /* ss - 091110.1 -b */
              IF v_check <> ""  THEN DO:
              
              FIND FIRST xxmqc_det WHERE xxmqc_rcp_date = tr_effdate AND xxmqc_vend = tr_addr AND xxmqc_part = tr_part AND xxmqc_serial = tr_serial  AND xxmqc_status = v_check  NO-LOCK NO-ERROR .
              IF AVAILABLE xxmqc_det   THEN do:
