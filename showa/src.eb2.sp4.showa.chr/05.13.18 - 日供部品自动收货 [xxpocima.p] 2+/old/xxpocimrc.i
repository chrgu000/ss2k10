        ...
        */
        ...
        for each po_mstr where po_vend = xx_inv_vend 
            ...
            each pod_det where pod_nbr = po_nbr 
			    ...
			    IF (xx_ship_qty - xx_ship_rcvd_qty) >= tmp_order_qty then do:          
                    ...
                    MESSAGE "1." + po_nbr +  " " + string(tmp_qty) VIEW-AS ALERT-BOX.
			        /*累加未结量小于未收量，直接收货B*/
			        {xxpocimrcyes.i}  
                end. 
			    else do: 
                    ...
                    MESSAGE "2." + pod_nbr + " " + STRING(pod_line) + " " + string(tmp_qty) VIEW-AS ALERT-BOX.
                      */
                    MESSAGE "2." + pod_nbr + " " + string((xx_ship_qty - xx_ship_rcvd_qty) - (tmp_order_qty - tmp_qty)) VIEW-AS ALERT-BOX.
                    /*累加未结量大于未收量，直接收货B*/
                    {xxpocimrcyes1.i}  
        ...
        /*PO未结量不足，增加订单，再收货B*/
        IF tmp_flagt = "1" AND (xx_ship_qty - xx_ship_rcvd_qty) > tmp_order_qty then do:
            j = j + 1. 
            ...
            MESSAGE "3." + tmp_ponbr + " " + string(xx_ship_qty - xx_ship_rcvd_qty - tmp_order_qty) VIEW-AS ALERT-BOX.
        ...
        IF tmp_flagt = "" then do:
            ...
            MESSAGE "4." + tmp_ponbr + " " + string(xx_ship_qty - xx_ship_rcvd_qty) VIEW-AS ALERT-BOX.
            /*PO未结量不足，余量增加定单B*/
		    {xxpocimrcaddpo1.i}
