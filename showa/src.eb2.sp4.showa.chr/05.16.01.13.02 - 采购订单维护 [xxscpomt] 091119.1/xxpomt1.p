...
define temp-table ttpod no-undo
	field ttpod_line   like pod_line
	field ttpod_status like pod_status
	field ttpod_qty_ord like pod_qty_ord
	field ttpod_due_date like pod_due_date
	.
/* SS 090907.1 - E */
...
/* SS 090907.1 - B */
			/*记录pod值*/
			empty temp-table ttpod.
			if po_user1 = "10" or po_user1 = "11" or po_user1 = "12" then do:
				for each pod_det where pod_nbr = po_nbr no-lock:
					create 	ttpod.
					assign 	ttpod_line = pod_line
					        ttpod_status = pod_status
									ttpod_qty_ord = pod_qty_ord
									ttpod_due_date = pod_due_date
									.
				end.
			end.
/* SS 090907.1 - E */
          {gprun.i ""xxpopomta1.p""}
...
/* SS 090907.1 - B */
			...
			if po_user1 = "10" or po_user1 = "11" or po_user1 = "12" then do:
				...
				for each pod_det where pod_nbr = po_nbr :
				  find first ttpod where ttpod_line = pod_line no-lock no-error.
				  if not avail ttpod or ttpod_status <> pod_status
							or	ttpod_qty_ord <> pod_qty_ord
							or	ttpod_due_date <> pod_due_date then do:
						if avail ttpod and ttpod_status <> pod_status then 
							assign pod_user1 = "12".
						else assign pod_user1 = "11".
						rspnbr = rspnbr + 1.
					end.
				end.
				...
				if rspnbr <>  0 then do:
					/* 导入scm系统 */
					{gprun.i ""xxpomt1xa1.p"" "(input po_nbr,
		                       				    output ifpass)"}
				end.
			end.
/* SS 090907.1 - E */
   ...
   /* SS - 090911.1 - B */
   	define var scmtbcont as logical.
   	scmtbcont = no.
/* SS 091116.1 - B */
/*
   	for first pod_det where pod_nbr = po_nbr and pod_user1 < "10" :
   		{gprun.i ""xxscpomtpouser1Popup.p"" "(
    	  INPUT-OUTPUT scmtbcont
   		)"}
   	end.
   	if scmtbcont then do:
   		for each pod_det where pod_nbr = po_nbr :
   			if pod_status <> "" then pod_user1 = "12".
   			else if pod_user1 = "03" then pod_user1 = "11".
   			else pod_user1 = "10".
   		end.
   	end.
*/
...
------------------------------------------------------------------------------*/
   ...
   then do:
/* SS 090902.1 - B */
			{gprun.i ""xxscvendcontrl.p"" "(input po_vend,input po_stat,input po_user1,
																	output rspnbr)"}
			if rspnbr <> 0 then do:
				/*禁止修改已结(C)或取消(X)的记录*/
				/*	{pxmsg.i &msgnum = 51601 &errorlevel = 3}*/
				message "不允许修改".
				undo,retry.
			end.
/* SS 090902.1 - E */
