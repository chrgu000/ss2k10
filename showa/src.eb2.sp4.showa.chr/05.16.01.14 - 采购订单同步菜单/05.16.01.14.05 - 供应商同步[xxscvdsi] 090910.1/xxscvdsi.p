...
repeat:
   ...
   {mfphead.i}
			...
			/*调用bill程序*/
		{gprun.i ""xxsccomCreateSendCommandData_VD.p"" "(input vend,
		                               input vend1,
		                               input 'xxscvdsi.p',
...
/* SS 090921.1 - B */	  	
	  		for each vd_mstr where vd_addr >= vend and vd_addr <= vend1 no-lock,
	  	 		each ad_mstr where ad_addr = vd_addr no-lock
	  			with frame c width 80 down :
	  		
	  			setframelabels(frame c:handle).
	  		
	  			disp vd_addr ad_name with frame c.
	  			down with frame c.
	  		end.
/* SS 090921.1 - E */	  	
			end.
