...
repeat on error undo, retry:
   ...
   /* 前缀 */
   FIND FIRST mfc_ctrl WHERE /*  mfc_domain = GLOBAL_domain AND */ mfc_field = "SoftspeedIR_rqm_nbr" NO-LOCK NO-ERROR.
   ...
   if available rqm_mstr then do:
      ...
      if rqm_open = YES 
         ...
         FIND FIRST rqd_det
            ...
            AND rqd_det.rqd_nbr = INPUT nbr_rqm
   ...
   if not available rqm_mstr then do:	       
      if INPUT nbr_rqm = "" then do :
         FIND FIRST mfc_ctrl 
            ...
            AND */ mfc_field = "SoftspeedIR_rqm_nbr" 
   ...
   do on error undo, leave:
      ...
      IF auto_select = YES THEN DO:
         ...
         /* 创建临时表 - 更新 */  /* 创建已经存在的申请单资料到tmep-table*/
         {gprun.i ""xxssirm2m.p"" "(
            INPUT nbr_rqm
            )"}
         FOR EACH tt1:
         ...
         FOR EACH tt1:
            IF tt1_status = "*" THEN DO:
               sel_total = sel_total + tt1_sod_qty_inv * tt1_sod_price.
            END.
         END.
         DISPLAY
            sel_total
         ...
         do on endkey undo, leave:
            ...
            {swselect.i
               ...
               &include1     = "
                  ...
                  sel_total = sel_total - tt1_qty_open * tt1_sod_price.
         ...
         FOR EACH tt1 NO-LOCK 
            WHERE tt1_status <> "*" 
            :
            {gprun.i ""xxssirm2dd.p"" "(
               INPUT nbr_rqm,
               INPUT STRING(tt1_SoftspeedIR_VAT),
               INPUT tt1_tr_trnbr
               )"}
            DELETE tt1.
         END.
      ...
      REPEAT WITH FRAME match_maintenance:
         ...
         IF ABSOLUTE(INPUT sod_qty_inv_tt1) > ABSOLUTE(tt1_qty_open) THEN DO:
            /* Quantity exceeds available remaining */
            {pxmsg.i &MSGNUM=125 &ERRORLEVEL=3}
            NEXT-PROMPT sod_qty_inv_tt1.
            UNDO,RETRY.
         END.

         ASSIGN
            tt1_sod_qty_inv = INPUT sod_qty_inv_tt1
            .
