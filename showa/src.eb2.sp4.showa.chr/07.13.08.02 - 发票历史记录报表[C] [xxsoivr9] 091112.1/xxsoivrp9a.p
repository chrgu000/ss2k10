...
/******************************************************************************/
/* ss - 091112.1 by: jack */  /* using_container_charges = no  using_line_charges = no   cclc.i 中设置 */
/* ss - 091112.1 by: 取消按订单，项次排序 */
...
DEFINE TEMP-TABLE tt
   FIELD  tt_nbr LIKE idh_nbr
   FIELD tt_line LIKE idh_line
  FIELD tt_part LIKE  idh_part
  FIELD tt_um LIKE  idh_um
  FIELD tt_qty_ord LIKE   idh_qty_ord 
  FIELD tt_qty_inv LIKE   idh_qty_inv
  FIELD tt_bo_chg LIKE  idh_bo_chg
  FIELD tt_curr LIKE   disp_curr
  FIELD tt_net_price LIKE idh_price
  FIELD tt_price LIKE ext_price
  FIELD tt_margin LIKE ext_gr_margin
  FIELD tt_due_date LIKE idh_due_date
  FIELD tt_type LIKE idh_type
  FIELD tt_desc LIKE pt_desc1 FORMAT "x(49)".
/* ss - 091112.1 -e */
...
for each ih_hist where (ih_inv_nbr >= inv) and (ih_inv_nbr <= inv1)
       ...
       /* ss - 091112.1 -b  */
          CREATE tt .
                 ASSIGN
                      ...
                      tt_um  =  idh_um 
                      tt_curr = DISP_curr 
                      tt_net_price = base_net_price
                      tt_price = base_price
                      tt_margin = base_margin
                      tt_due_date = idh_due_date
                      tt_type = idh_type 
                      tt_desc = desc1 .

                 IF ih_fsm_type <> "FSM-RO" THEN DO:
                     tt_qty_ord = idh_qty_ord .
                     tt_qty_inv = idh_qty_inv .
                     END.
                 ELSE DO:
                     tt_qty_ord =  l_qty_call .
                     tt_qty_inv = idh_qty_inv .
                 END.
             
                  if ih_fsm_type <> "FSM-RO"   then  DO :
                     if idh_qty_ord >= 0 then /* show bo regardless of sign */
                       tt_bo_chg =  max(idh_qty_ord - idh_qty_ship, 0)  .
                     else
                        tt_bo_chg =  min(idh_qty_ord - idh_qty_ship, 0)  .
                  end. /* IF ih_fsm_type <> "FSM-RO" */
                  ELSE
                     tt_bo_chg = 0 .
   ...
   then do:
       ...
       FOR EACH tt   BREAK BY tt_part BY ROUND( tt_net_price , 2 )    :
           
           ACCUMULATE tt_qty_inv (TOTAL BY round(tt_net_price , 2 ) )  .
       ...
       /*
           ...
           */
           DISPLAY
                    ...
                    tt_part     LABEL "零件"
                    tt_um        LABEL "UM"
                    tt_qty_ord    LABEL "已定购量"
                    tt_qty_inv    LABEL "发票数量"
                    tt_bo_chg   LABEL "欠交量"
                    tt_curr   LABEL "币别"
                    tt_net_price       LABEL "价格"
                    tt_price  LABEL "价格"
                    tt_margin    LABEL "毛利合计"
                    tt_due_date     LABEL "截止日期"
                    tt_type "类型" WITH WIDTH 150 STREAM-IO  .

              
              IF LAST-OF( ROUND (tt_net_price , 2 ) )  THEN  DO:

                 PUT  "小计："  AT 1  tt_desc at  7 .
             
                 PUT (  ACCUMULATE TOTAL BY  ROUND(tt_net_price , 2 ) tt_qty_inv )  AT 65  tt_net_price AT 80 SKIP  .
              END.
