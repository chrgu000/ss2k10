...
/******************************************************************************/
/* ss - 091116.1 by: jack */ /* 同一发票按零件单价汇总输出 */

/* 以下为版本历史 */
/* SS - 090505.1 By: Bill Jiang */
/* ss - 090824.1 by: jack */
...
repeat:
   ...
   */
   ...
   /* ss - 091116.1 -b */
   FOR EACH ttxxsoivrp0901 NO-LOCK BREAK BY ttxxsoivrp0901_ih_inv_nbr  BY ttxxsoivrp0901_idh_part BY ttxxsoivrp0901_base_net_price :

    ACCUMULATE ttxxsoivrp0901_base_price ( TOTAL BY ttxxsoivrp0901_base_net_price ) .
    FIND FIRST  pt_mstr NO-LOCK WHERE pt_part = ttxxsoivrp0901_idh_part NO-ERROR .
   ...
   /* ss - 091116.1 -e */
      ...
      EXPORT DELIMITER ";" 
         ...
         .
    /* ss - 091116.1 -b */
    IF LAST-OF(ttxxsoivrp0901_base_net_price)  THEN DO:
      EXPORT DELIMITER ";" 
         "小计:"
         ""
         ""
        ...
        ""
         ""
         ttxxsoivrp0901_base_net_price
          ACCUMULATE  TOTAL BY  ttxxsoivrp0901_base_net_price   ttxxsoivrp0901_base_price .
         .
    END.
   /* ss - 091116.1 -e */
   END.
