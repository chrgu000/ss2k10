/* GUI CONVERTED from reisslst.p (converter v1.76) Fri May  9 04:30:36 2003 */
...
/*zh003*/ define variable reason  like rsn_code.
{&REISSLST-P-TAG1}
...
openqty = pk_qty.
 aaa:
for each ld_det where ld_site = wosite and ld_loc = wkctr and ld_part = pk_part and ld_qty_oh > 0 :
	 ...
	 find sr_wkfl where 
					 ...
					 end. /*  if  openqty > ld_qty_oh then do:*/
					 else do:
 find sr_wkfl where 
		  sr_userid = mfguser
                     and   sr_lineid = l_sr_lineid
	...
	if openqty > 0 then  message pk_part "is less "  openqty  "at workcentre location "
	 view-as alert-box.
               end. /* FOR EACH pk_det */
zh003 add  end *******************************/
seta:
...
/*GUI*/ if global-beam-me-up then undo, leave.
 ...
 /*zh003      add begin     ******************************************/ 
               find rsn_ref no-lock where rsn_type = "SCRAP"
                  and rsn_code = reason no-error.
               if  not available rsn_ref then do with frame d:
                  ...
                  for each sr_wkfl
                     ...
                     with frame e:
                      display
                        ...
                        column-label "Ref"
                        sr_qty
/*zh003 */				sr_user2 column-label "Reason".
                     down.
                  end. /* FOR EACH sr_wkfl */
               end. /* FOR EACH pk_det */
               view frame e.
...
/*GUI*/ if global-beam-me-up then undo, leave.
               ...
               for each sr_wkfl fields(sr_userid sr_lineid sr_site
                                       sr_loc    sr_lotser sr_ref sr_qty sr_user2)
