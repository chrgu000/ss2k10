/* MRP 订货量预示报表                                   */
/* 输入订单号，程序根据到货日期规划计算后续的订货量     */
/* 订单的零件以及该供应商的所有AC和SP状态零件均进行计算 */
...
DEF TEMP-TABLE empty2
    field empty2_ord_on like pod_nbr  label "采购单号" 
    field empty2_addr like vd_addr label "供应商"
...
FOR EACH npod_det where npod_due_date = 12/12/12,
    ...
    if not available vd_mstr then remark = "0111110".
    else
    if vd_rmks = "" then remark = "0111110".                                    /*如果供应商备注没维护默认每星期一到五都可送货 */
    
    c = ok_date(mrp_due_date, remark).
    n = n + 1.
...
for each empty2 no-lock,
    each pt_mstr where pt_part = empty2_part no-lock :
    disp 
        empty2_part
	pt_draw label "图纸号"
	pt_desc1  label "名称"
