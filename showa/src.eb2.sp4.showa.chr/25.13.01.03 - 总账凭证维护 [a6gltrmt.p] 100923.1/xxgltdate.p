...
/* 不能小于前一笔时间 */
FIND LAST glt_det WHERE SUBSTRING(glt_ref , 1, 9) = v_ref AND (  INT (SUBSTRING(glt_ref,10,5)) ) < i  NO-LOCK NO-ERROR .
...
FIND FIRST glt_det WHERE SUBSTRING(glt_ref , 1, 9) = v_ref AND (  INT (SUBSTRING(glt_ref,10,5)) ) > i  NO-LOCK NO-ERROR .
IF AVAILABLE glt_det  THEN DO:
      IF v_date1 > glt_effdate  THEN DO:
          v_error = YES .
          v_date2 = glt_effdate .
