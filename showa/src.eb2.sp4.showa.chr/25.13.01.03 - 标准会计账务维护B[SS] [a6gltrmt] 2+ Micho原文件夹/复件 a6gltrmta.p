         ...
         do on error undo, retry:
            ...
            FOR EACH usrw_wkfl NO-LOCK WHERE usrw_key1 = 'glsum' 
                ...
                FOR EACH usrw NO-LOCK WHERE usrw.usrw_key1 = 'glsum' 
                    ...
                    FIND FIRST cr_mstr WHERE cr_code = usrw.usrw_key2 
                        AND cr_type = 'glsum_acct' NO-LOCK NO-ERROR.
                    IF (AVAIL cr_mstr AND (glt_acct >= cr_code_beg AND glt_acct <= cr_code_end /*AND cr_code_beg <> "" AND cr_code_end <> ""*/ )) THEN DO:
