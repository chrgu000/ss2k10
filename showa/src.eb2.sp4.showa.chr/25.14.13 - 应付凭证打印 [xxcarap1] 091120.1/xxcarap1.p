...
/* $Revision: 1.11.2.13 $ BY: Bill Jiang DATE: 08/26/07 ECO: *SS - 091120.1* */
...
repeat:
   ...
   */
   ...
   FOR EACH ttssapvorp0002
      ...
      WHERE ac_code = ttssapvorp0002_vod_acc
      AND INDEX("ALIE",ac_type) > 0
      ,EACH sb_mstr NO-LOCK
      WHERE sb_sub = ttssapvorp0002_vod_sub
      ,EACH cc_mstr NO-LOCK
      WHERE cc_ctr = ttssapvorp0002_vod_cc
      ,EACH pj_mstr NO-LOCK
      WHERE pj_proj = ttssapvorp0002_vod_project
      ...
      IF LAST-OF(ttssapvorp0002_vo_ref) THEN DO:
         ...
         {gprun.i ""xxcaren.p"" "(
            INPUT ttssapvorp0002_ap_entity,
         ...
         FIND FIRST vo_mstr WHERE vo_ref = ttssapvorp0002_vo_ref NO-LOCK NO-ERROR.
         IF AVAILABLE vo_mstr THEN DO:
            FIND FIRST emp_mstr WHERE emp_addr = vo__qad01 NO-LOCK NO-ERROR.
