...
/* REVISION: 9.1      LAST MODIFIED: 02/10/06   BY: *SS - Micho - 20060524* Micho Yang     */
...
/* ********** End Translatable Strings Definitions ********* */
          ...
          define new shared variable acc like ac_code NO-UNDO INIT "1001" .
          define new shared variable acc1 like ac_code NO-UNDO INIT '1009zzzz'.
          ...
          define variable ccflag       like mfc_logical INIT NO label "汇总成本中心" NO-UNDO .
          DEF VAR acct1 LIKE glta_acct .
          DEF VAR acct2 LIKE glta_acct .
          DEF VAR v_occur AS logical INIT YES LABEL "只输出有发生额的科目"  .
          DEF VAR inclpost AS LOGICAL INIT NO .
          /****************************** Add by SS - Micho - 20060714 B ******************************/
...
/*J241*/  for first co_ctrl fields (co_ret co_use_sub) no-lock: end.
          ...
          repeat:
            ...
            if acct2 = hi_char then assign acct2 = "".

            if c-application-mode <> 'web' then
               update entity entity1 cname   acc acc1
                      sub when (use_sub)
                      sub1 when (use_sub)
                      acct1 
                      acct2
            ...
            {wbrp06.i &command = update &fields = "  entity entity1 cname
               acc acc1 sub when (use_sub)  sub1 when (use_sub)
               acct1 acct2 ctr ctr1 
             effdate effdate1 ccflag v_occur inclpost
             " &frm = "a"}
               /* SS - 20060210 - E */
...
/*K1S9*/       run quote-vars.
               ...
               if acct2 = "" then assign acct2 = hi_char.
