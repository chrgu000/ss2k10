...
/* $Revision: 1.10.3.1 $ BY: Micho Yang  DATE: 07/14/06 ECO: *SS - Micho - 20060714*        */
...
define variable beg_amt       as decimal   format  "->>,>>>,>>>,>>9.99" no-undo.
/*
{mfdtitle.i "2+ "}
*/
{a6mfdtitle.i "2+ "}
  ...
  for each ac_mstr no-lock where ac_code >= acc and ac_code <= acc1:
      ...
      */
      ...
      if not available gltr_hist then DO:
         CREATE tta6glacstrp02.
         ASSIGN
            tta6glacstrp02_acc = ac_code
            tta6glacstrp02_beg_amt = beg_amt
            .
         next.
      END.
      /* SS - 20060419.1 - E */
  ...
  */
  ...
  if not summary then do:
     ...
     */
     CREATE tta6glacstrp02.
     ASSIGN
        tta6glacstrp02_acc = gltr_hist.gltr_acc
        tta6glacstrp02_ref = gltr_hist.gltr_ref
        /****************************** Add by SS - Micho - 20060714 B ******************************/
         tta6glacstrp02_line = gltr_hist.gltr_line
        /****************************** Add by SS - Micho - 20060714 B ******************************/
        tta6glacstrp02_doc = doc
        tta6glacstrp02_bank = bank
        tta6glacstrp02_ckfrm = ckfrm
        tta6glacstrp02_addr = gltr_hist.gltr_addr
        tta6glacstrp02_eff_dt = gltr_hist.gltr_eff_dt
        tta6glacstrp02_beg_amt = beg_amt
        tta6glacstrp02_dr_amt = ip_dr_amt
        tta6glacstrp02_cr_amt = ip_cr_amt
        tta6glacstrp02_batch = gltr_hist.gltr_batch
        .
    IF ip_sub THEN DO:
       ASSIGN
          tta6glacstrp02_sub = gltr_hist.gltr_sub
          .
    END.
    IF ip_cc = TRUE THEN DO:
       ASSIGN
          tta6glacstrp02_ctr = gltr_hist.gltr_ctr
          .
    END.
    if trn_desc and gldesc <> "" then
       ASSIGN tta6glacstrp02_gldesc = gldesc.
    if disp_curr   and
       gltr_hist.gltr_curr <> base_curr and
       gltr_hist.gltr_curramt <> 0      then do:
       ASSIGN
          tta6glacstrp02_curr = gltr_hist.gltr_curr
          tta6glacstrp02_curramt = gltr_hist.gltr_curramt
          .
    end.
     /* SS - 20060419.1 - E */
  ...
  if ip_level = 2 then do:
     ...
     */
     ...
     ASSIGN
        ...
        tta6glacstrp02_cr_amt = ip_cr_amt
        .
     IF subflag = FALSE THEN DO:
        ASSIGN
           tta6glacstrp02_sub = gltr_hist.gltr_sub
           .
     END.
     IF ccflag = FALSE THEN DO:
        ASSIGN
           tta6glacstrp02_ctr = gltr_hist.gltr_ctr
           .
     END.
