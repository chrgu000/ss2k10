...
/*
修改为这种调用方式
{gprun.i ""xxpo-newmicho.p"" "( input ponbr ,采购单号
                                input part, 零件
                                input today, 日期
                                output qty1, 下月预示量
                                output qty2)  下下月预示量 
...
DEFINE INPUT PARAMETER iptponbr LIKE po_nbr.
define input parameter iptpart like pt_part.
define input parameter iptdate as date.
define output parameter optqty1  like ld_qty_oh.
define output parameter optqty2  like ld_qty_oh.
...
FOR EACH pod_det WHERE pod_nbr = ponbr AND pod_part = iptpart NO-LOCK,
    EACH po_mstr WHERE po_nbr = pod_nbr NO-LOCK,
    EACH pt_mstr WHERE pt_part = pod_part and pt_buyer = buyer NO-LOCK:
/* SS - 091214.1 E*/
...
For each pt_mstr where ( pt_status = "AC" or pt_status = "SP") and pt_buyer = buyer and pt_vend = vend AND pt_part = iptpart no-lock:
/* SS - 091214.1 E*/
...
*/
FOR EACH npod_det where npod_due_date = 12/12/12 AND npod_part = iptpart,
    EACH pt_mstr where pt_part = npod_part,
    EACH mrp_det WHERE mrp_part = npod_part and mrp_due_date <= (next2date + 31)  AND mrp_detail = "计划单"  USE-INDEX mrp_part,                       /*FOR 1*/
    EACH vd_mstr WHERE vd_addr = vend BREAK BY mrp_part BY mrp_due_date:
/* SS - 091214.1 E*/          
...
FOR EACH empty1 break by empty1_part:                   /*FOR 2*/
    ...
    /*&*/	   find po_mstr where po_nbr = n1  NO-LOCK NO-ERROR.                                                                                                   /*&*/
...
if ch = "Y" THEN do:
      ...
      FOR EACH npod_det :
      */      
      FOR EACH npod_det WHERE npod_part = iptpart:
      /* SS - 091214.1 E*/   
...
CASE substring(vend_str,1,1):
    WHEN "a" or WHEN "A" THEN
      do:
         ...
         */
         date1 = date(MONTH(one_date),INTEGER(substring(vend_str,2,2)),year(one_date)).
         /* SS - 091214.1 E*/
    ...
    */
    date2 = date(month(one_date),INTEGER(substring(vend_str,4,2)),year(one_date)).
    /* SS - 091214.1 E*/
    ...
    */
    date3 = date(month(one_date),INTEGER(substring(vend_str,6,2)),year(one_date)).
    /* SS -091214.1 E*/
