     ...
     DO i = 1 TO 9 :
        if avail vd_mstr then do:
            assign tmp_ponbr = substring(xx_ship_type,1,1) + substring(string(year(today)),3) + tmpmonth + substring(vd_sort,1,2) + STRING(i) .
        end.
        else do:
            assign tmp_ponbr = substring(xx_ship_type,1,1) + substring(string(year(today)),3) + tmpmonth + "ER" + STRING(i) .
        end.

        FIND FIRST po_mstr WHERE po_mstr.po_nbr = tmp_ponbr NO-LOCK NO-ERROR.
     ...
     output to value( trim(usection) + ".i") .
	   ...
	   put UNFORMATTED string(tmp_qty)  skip.
        /* 汇率问题要等财务AP模块上线后才确定 */
       /*
