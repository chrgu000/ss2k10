     ...
     output to value( trim(usection) + ".i") .
      ...
      PUT  UNFORMATTED trim ( string((xx_ship_qty - xx_ship_rcvd_qty) - tmp_order_qty ) ) + " - N - - - " + trim (xx_inv_site) + " " +  trim( tmp_loc ) + " " + """" + trim(tmp_lot) + """" + " - - N N N "   format "X(50)"   skip.
	...
	find last tr_hist where tr_date = today  
                        ...
                        and tr_time  + 15 >= TIME  
                        and tr_qty_chg = (xx_ship_qty - xx_ship_rcvd_qty)  - tmp_order_qty
	...
	If AVAILABLE ( tr_hist ) then do:
        ...
        MESSAGE "tr_hist 1. " + STRING(tmp_qty) VIEW-AS ALERT-BOX.
          */
        assign 	
            xx_ship_rcvd_qty = xx_ship_rcvd_qty + (xx_ship_qty - xx_ship_rcvd_qty)  - tmp_order_qty
