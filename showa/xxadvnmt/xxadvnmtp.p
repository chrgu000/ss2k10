...
do:
   ...
   do on endkey undo, leave frameloop:
      ...
      /* ss - 090907.1 -e */
         ...
         .
        /* ss  - 090907.1 -b */
  assign vd__chr02 vd__chr03.

  if vd__chr02 <> "" and
     not can-find( first vd_mstr where vd_addr = INPUT vd__chr02 ) then do:
    {pxmsg.i &MSGNUM= 8601 &ERRORLEVEL=3}
     next-prompt vd__chr02 with frame b.
     undo , retry .
  end .
  if not can-find(first code_mstr no-lock where code_fldname = "vd__chr03"
                    and code_value = input vd__chr03)
     and input vd__chr03 <> "" then do:
    {pxmsg.i &MSGNUM= 716 &ERRORLEVEL=3}
     next-prompt vd__chr03 with frame b.
     undo ,retry.
  end.
  /* ss - 090907.1 -e */
      ad_sort = vd_sort.
