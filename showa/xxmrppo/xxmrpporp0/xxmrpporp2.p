...
define variable vkey_podate like usrw_key1 no-undo initial "xxmrpporp.rec_date".
...
define temp-table tmp_tmd
   ...
   fields tm_duedate like mrp_due_date
...
repeat:
  ...
  assign tmpDate = date(month(today),1,year(today)).
  run iniPOdate(tmpdate,tmpDate + 130).
...
/* end.                                                                          */
   ...
   FOR EACH pt_mstr no-lock where pt_part >= part and pt_part <= part1
       ...
       EACH vd_mstr no-lock where vd_addr = pt_vend and vd__chr04 <> "":
           create tmp_tmd.
...
/*   end.                                                                    */
   ...
   for each tmp_tmd exclusive-lock:
       assign tm_role = getRole(tm_vend,tm_part).
   end.
   
   /* 计算到货日  */
   for each tmp_tmd exclusive-lock:
       assign tm_edate = getRelDate(tm_role,tm_duedate).
   end.
   for each tmp_tmd exclusive-lock where tm_edate > due:
       delete tmp_tmd.
   end.
