...
form
   xxlr_sn      colon 20
   xxlr_Start   colon 20
   xxlr_minutes colon 20
with frame bb side-labels width 80
title color normal (getFrameTitle("RESET_TIME",14)).
...
repeat:
   ...
   then do transaction :
      ...
      for each xxlr_det where xxlr_line = ln_line
                          and xxlr_site = ln_site
      exclusive-lock:
   ...
   repeat with frame bb:
      ...
      editing:
         ...
         if frame-field = "xxlr_sn" then do:
            {mfnp06.i xxlr_det xxlr_sn
               "xxlr_line = input ln_line and xxlr_site = input ln_site"
               xxlr_sn "input xxlr_sn" """" """"}
      ...
      find xxlr_det
         where xxlr_line = ln_line
           and xxlr_site = ln_site
           and xxlr_sn  = input xxlr_sn
      exclusive-lock no-error.

      if not available xxlr_det then do:
         create xxlr_det.
         assign
                xxlr_line = ln_line
                xxlr_site = ln_site
      ...
      do on error undo, retry:
         ...
         set xxlr_Start xxlr_minutes
         go-on (F5 CTRL-D) with frame bb.
         assign xxlr_stime = s2t(xxlr_Start).
         display
              xxlr_sn
              xxlr_Start
              xxlr_minutes
