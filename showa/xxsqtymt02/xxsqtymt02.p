...
/* REVISION: 090805.1      Create Date: 20090805  BY: Softspeed roger xiao    */
/*转GUI界面，改成固定地点，加宽数量格式*/
/*计算基准日期,由 xsqty_date 改为当前系统日期today */
/* SS - 090820.1 - RNB
...
/* SS - 100906.1 - E */
                ...
                v_i = 0.
                do v_i = month(today) to 12 : /*本年*/
                    /*run lastday (input date(v_i,01,year(today)), output v_lastday) .*/
                    v_lastday = date(v_i,01,year(today)) .

                    v_qty = if v_i = month(today) then 0 else xsqty_sqty[v_i] - pt_sfty_stk .
                ...
                v_i = 0.
                if month(today) > 1 then do v_i = 1 to month(today) - 1 : /*次年*/
                    /*run lastday (input date(v_i,01,year(today) + 1 ), output v_lastday) .*/
                    v_lastday =  date(v_i,01,year(today) + 1 ) .
