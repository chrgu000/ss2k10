...
/****************************************************************************
   ...
   Parameters:
     ...
     include3  -    include file if special processing is desired on
                    ...
                    document, etc.
*af* selectd   -    if select not display sel
****************************************************************************/

/*af*  define variable sym              as character.                       */
/*af*  define variable sw_new           as character.                       */
/*af*  define variable sw_i             as integer.                         */
/*af*  define variable sw_first_display as logical.                         */
/*af*  define variable sw_found_recs    as logical.                         */
/*af*  define variable sw_recid         as recid.                           */
/*af*  define variable sw_temp_recid    as recid.                           */
/*af*  define variable sw_frame_recid   as recid extent 25.                 */
/*af*  define variable sw_temp_new      like sw_new.                        */
...
/*af*/ end.
      ...
      /*scroll down                                                    */
/*af*/ do on error undo,retry:
      if keyfunction(lastkey) = "return" or
         keyfunction(lastkey) = " "
      then do:
/*af*/   if {&selectd} then do:
/*af*/      {&include1}
/*af*/      if dec({&display6}) > dec({&display4}) - dec({&display5}) then do:
/*af*/          {pxmsg.i &msgnum = 9108 &ERRORLEVEL = 3}
/*af*/          undo,retry.
/*af*/      end.
/*af*/      if not can-find(first si_mstr no-lock where
/*af*/             si_domain = global_domain and si_site = {&display7})
/*af*/      then do:
/*af*/          {pxmsg.i &msgnum = 708 &ERRORLEVEL = 3}
/*af*/          next-prompt {&display7} with frame {&framename}.
/*af*/          undo,retry.
/*af*/      end.
/*af*/      if not can-find(first loc_mstr no-lock where
/*af*/             loc_domain = global_domain and loc_site = {&display7} and
/*af*/             loc_loc = {&display8}) then do:
/*af*/          {pxmsg.i &msgnum = 709 &ERRORLEVEL = 3}
/*af*/          next-prompt {&display8} with frame {&framename}.
/*af*/          undo,retry.
/*af*/      end.
/*af*/   end.
/*af*/   else do:
