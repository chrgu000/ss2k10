...
form
  /* usrw_key1 colon 25 format "x(20)" skip(1) */
   ...
   usrw_key3 colon 25 format "x(60)" view-as fill-in size 30 by 1 skip(1)
   usrw_key4 colon 25 format "x(60)" view-as fill-in size 30 by 1
   usrw_key5 colon 25 format "x(60)" view-as fill-in size 30 by 1
   usrw_key6 colon 25 format "x(120)" view-as fill-in size 30 by 1
...
/*   display vlgvdkey @ usrw_key1. */
   ...
   do on error undo, retry:
      ...
      {mfnp05.i usrw_wkfl usrw_index1 " {xxusrwdom.i} {xxand.i}
                usrw_key1 = vlgvdkey " usrw_key2 "input usrw_key2"}
      ...
      if length(input usrw_key2) = 0 then do:
          assign v_number = "".
          {gprun.i ""gpnrmgv.p"" "(""SHIP_VD"",input-output v_number
   ...
   find usrw_wkfl where {xxusrwdom.i} {xxand.i} usrw_key1 = vlgvdkey and
                        usrw_key2 = input usrw_key2 no-error.

   if not available usrw_wkfl then do:
      {pxmsg.i &MSGNUM=1 &ERRORLEVEL=1}
      create usrw_wkfl. {xxusrwdom.i}.
      assign usrw_key1 = vlgvdkey
             usrw_key2.
