...
/* 100715.1  $  BY: mage chen  DATE: 07/14/10                    ECO: *P45S*  */
...
/*    {mfphead.i}                                             */
   ...
   if pick_ship then
      ...
      assign
         ...
         range  = "s" + ship
         range1 = "s" + ship1.
   for each xsh_det exclusive-lock:
       delete xsh_det.
   end.
...
PROCEDURE p_abs_item :
   define buffer bf-absmstr for abs_mstr .
      ...
      where bf-absmstr.abs_domain    = global_domain
        and bf-absmstr.abs_shipfrom = xxsh_site
        and bf-absmstr.abs_id = xxsh_abs_id
        and   bf-absmstr.abs_type      = "s"
      no-lock use-index abs_item
      break by abs_id:
      ...
      then do:
         ...
         do while l_shpno = "" :
            for first abs_mstr
               fields (abs_canceled abs_id     abs_inv_mov  abs_item
                       abs_order    abs_par_id abs_shipfrom abs_shipto
                       abs_status   abs_type   abs_domain abs__chr01 )
...
/*ss - 100715.1 - b* /
        if index( abs_mstr.abs__chr01,  substring(global_userid, 1, 2)) <> 1 then
  / *ss - 100715.1 - e*/
                  if  l_shpno               >= range
                  and l_shpno               <= range1
...
PROCEDURE p_abs_shipto :
   ...
   for each xxsh_mst no-lock use-index xxsh_abs_nbr where
        xxsh_domain = global_domain and
       (xxsh_site >= site and xxsh_site <= site1) and
       (xxsh_abs_id >= "S" + absid and xxsh_abs_id <= "S" + absid1) and
       (xxsh_nbr >= shnbr and xxsh_nbr <= shnbr1) and
       (stat = "*" or xxsh_stat = stat),
     each bf-absmstr
      fields (abs_canceled abs_id     abs_inv_mov  abs_item
...
/*         and   bf-absmstr.abs_id       <= range1         */
      ...
      break by abs_id:
/*ss - 100715.1 - b* /
        if index(bf-absmstr.abs__chr01,  substring(global_userid, 1, 2)) <> 1 then
       next .
 / *ss - 100715.1 - e*/
      for each abs_mstr
         fields (abs_canceled abs_id     abs_inv_mov  abs_item
