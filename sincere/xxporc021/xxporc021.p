...
/* ss - 101209.1 by: zhangyun */ /*修改表尾签名 cp from xxporc02.p*/
...
DEFINE TEMP-TABLE tmp_det NO-UNDO
    ...
    FIELD tmp_part     AS   CHARACTER FORMAT "x(14)"
...
REPEAT :
    ...
    UPDATE
        nbr nbr1 rcvr rcvr1 vend vend1 rcv_date rcv_date1
        loc NEW_only1 v_logical1 includereturn
        /*
        printcmts
        */
        WITH FRAME a.
    {wbrp06.i &COMMAND = UPDATE &FIELDS =" nbr nbr1 rcvr rcvr1 vend vend1
              rcv_date rcv_date1 loc NEW_only1 v_logical1 /* printcmts */ "
              &frm = "a"}
     bcdparm = "".
    {mfquoter.i nbr}
    {mfquoter.i nbr1}
...
/*                   PUT  "供应商签名" AT 5 "仓库收货确认:"   AT 30             "品管验货确认:"  AT 62  "页码:" TO 100  page-start   FORMAT ">>9"  " end "     SKIP.         */
                     run putpagetail.
          ...
          PUT UNFORMATTED  v_line AT 1 tmp_receiver AT 6  tmp_part AT 16  tmp_desc1 + ","  +  tmp_desc2 AT 30   tmp_um AT 84  tmp_rcvd AT 92   SKIP .
           ...
           IF LAST-OF(tmp_rcp_date) THEN DO:
              put skip(page-size - line-counter - 10).
/*                    PUT "------------------------------------------------------------------------------------------------------------------------" AT 1 .                    */
/*                    PUT "白色联(财务) " AT 5        " 粉红色联(仓库)" AT 20      " 蓝色联(品管)" AT 35            " 绿色联(估价)"  AT 52          " 黄色联(供应商)" AT 67   .*/
/*                    PUT "供应商签名" AT 5  "仓库收货确认:"   AT 30             "品管验货确认:"  AT 62  "页码:" TO 100  page-start   FORMAT ">>9"  " end "     SKIP.          */
              run putpagetail.
                  ...
                  FORM  HEADER
                   ...
                   /*    DISPLAY tmp_rcp_date tmp_name v_date  WITH FRAME phead11 .   */
                      VIEW FRAME phead11 .
                       ...
                       PUT "序号 退货单号  物料编码     物料描述                                           单位    数量        备注"  AT 1 .
                   PUT unformat fill("-",120) AT 1 .          /* tmp_rcp_date  */    /* first-of(tmp_rcp_date) */
                  ss - 100809.1 -e */
                  /* ss - 100809.1 -b */
                  v_name = tmp_name .
                  v_rcp_date = tmp_rcp_date .
...
procedure putPageTail:
  PUT unformat fill("-",120) AT 1.
  PUT unformat "白色联(财务)" AT 5 "粉红色联(仓库)" AT 26 "蓝色联(采购)" AT 50.
  PUT unformat "绿色联(估价)" AT 74 " 黄色联(供应商)" AT 90.
  PUT unformat "供应商签名:" AT 5 "仓库收货确认:" AT 50 "页码:" TO 90.
  PUT unformat page-start FORMAT ">>9"  " end".
end procedure.