...
form
   "工    单："  at 1   wo_nbr   colon 10      "ID:"   at   30   wo_lot
   "发放日期："  at 86
   wod_iss_date   at 98
   "零 件 号：" at 1
   wo_part  colon 10
...
/*      display
    ...
    for each tmp
  where  /* ( tmp_log = yes or tmp_log1 = yes ) and */   tmp_qtyx  > 0
  ...
  setFrameLabels(frame ttmp:handle).
    ...
    if available wr_route then do:
         find first wc_mstr where wc_domain = global_domain
                and wc_wkctr = wr_wkctr no-lock no-error.
...
/*ss - 101023.1    display skip(1) with frame floorstk1. */
   end.
end.  /*for each tmp ***************************/
...
PROCEDURE PAK:
   ...
   put "部门主管:" at 32.
   put "发料员:" at 60 skip.
   put "白色联:财务 ,  粉红色联:仓库 ,   蓝色联:车间"
   /*space(30) "打印时间:" today space(1) string(Time,"hh:mm:ss")skip(0)*/.
END PROCEDURE.
...
/* put "------------工单领料单 尾页-------------------" at  30  skip.*/
   ...
   put "部门主管:" at 32.
   put "发料员:" at 60 skip.
   put "白色联:财务 ,  粉红色联:仓库 ,   蓝色联:车间" space(30)
    /*"打印时间:" today space(1) string(Time,"hh:mm:ss")*/ skip(0).
   page .
END PROCEDURE.
/*ss  - 100727.1 e*/
