...
/*      display
  ...
  if available wr_route then do:
      find first wc_mstr where wc_domain = global_domain and
                 wc_wkctr = wr_wkctr no-lock no-error.
        if not available wc_mstr then wcdesc  = "". else wcdesc = wc_desc.
  end.
  else wcdesc  = "".
  ...
  if first-of(tmp_loc) and not first-of(tmp_lot)  then do:
     ...
     display wo_nbr wo_lot wo_part wod_iss_date wo_due_date
             wo_des wo_qty wo_um tmp_loc wcdesc with frame picklist.
  end.

   if first-of(tmp_lot) then do:
      find first wr_route where wr_domain = global_domain
             and wr_lot = wo_lot no-lock no-error.
      display wo_nbr wo_lot wo_part wod_iss_date wo_due_date
              wo_des wo_qty wo_um tmp_loc wcdesc with frame picklist.
   end.
...
/*ss - 101023.1 - b*     display skip(1) with frame floorstk1. */
