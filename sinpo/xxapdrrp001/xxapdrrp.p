...
/* Revision: 1.8.1.14  BY: Paul Donnelly (SB) DATE: 06/26/03 ECO: *Q00B* */
/* $Revision: 1.8.1.15 $ BY: Reena Ambavi DATE: 11/30/04 ECO: *P2XC* */
/*-Revision end---------------------------------------------------------------*/
...
/***********************************************************************/
 ...
 {xxmfdtitle.i "1+ "}
  {xxapdrrp.i}
    DEFINE INPUT PARAMETER i_bank        AS CHARACTER.
    DEFINE INPUT PARAMETER i_bank1       AS CHARACTER.
    ...
    DEFINE VARIABLE v_bk_dftap_acct AS CHARACTER.
    DEFINE VARIABLE v_bk_dftap_sub  AS CHARACTER.
    DEFINE VARIABLE v_bk_dftap_cc   AS CHARACTER.
    DEFINE VARIABLE v_bk_desc       AS CHARACTER.
...
{&APDRRP-P-TAG1}
define variable nbr             like ck_nbr label "Draft".
...
define variable due_date        like ap_date label "Due Date".
...
*/
   ...
   check_loop:
   for each ck_mstr  where ck_mstr.ck_domain = global_domain and (  ck_bank >=
   bank and ck_bank <= bank1
      ...
      then do:
         ...
         if ap__qad01 > "" then do:
            ...
            then do:
               ...
               then do:
                  ...
                  for each ckd_det
                     fields( ckd_domain ckd_amt ckd_cur_amt ckd_ref ckd_voucher)
                      where ckd_det.ckd_domain = global_domain and  ckd_ref =
                      ck_ref
                  no-lock:
               ...
               ad_addr = ap_vend no-lock no-wait
               no-error.
               if available ad_mstr then name = ad_name.
               {&APDRRP-P-TAG5}
/*            display                                                       */
/*               ck_nbr       column-label "Draft" format "999999"          */
/*               ck_type                                                    */
/*               ap_vend                                                    */
/*               name                                                       */
/*               format "x(23)"                                             */
/*                                                                          */
/*               space(1)                                                   */
/*               (- base_amt) column-label "Draft Amount"                   */
/*               format "->>>>>,>>>,>>9.99"                                 */
/*                                                                          */
/*               space(1)                                                   */
/*               ap_acct                                                    */
...
/*               ap_sub                                                     */
/*               space(1)                                                   */
/*               ap_date      column-label "Date"                           */
/*               space(1)                                                   */
/*               ap_effdate   column-label "Eff Date"                       */
/*               space(1)                                                   */
/*               ap_due_date  column-label "Due Date"                       */
/*               space(1)                                                   */
