...
/*By: Neil Gao 08/07/09 ECO: *SS 20080709* */
...
/*
                  message desc1 desc2.
*/
									find first cd_det where cd_domain = global_domain and cd_ref = sod_part and cd_type = "sc" 
...
/*SS 20080807 - B*/
										for each xxcffw_mstr where xxcffw_key1 = "check" and xxcffw_key_nbr = sod_det.sod_nbr
											and xxcffw_key_line = sod_det.sod_line :
											delete xxcffw_mstr.
										end.
										for each xxcff_mstr where xxcff_key1 = "check" and xxcff_key_nbr = sod_det.sod_nbr 
	     								and xxcff_key_line = sod_det.sod_line :
	     								delete xxcff_mstr.
	     							end.
	     							for each xxsob_det where xxsob_domain = global_domain and xxsob_nbr = sod_det.sod_nbr 
	     								and xxsob_line = sod_det.sod_line :
	     								delete xxsob_det.
	     							end.
/*SS 20080807 - E*/
...
if sod_list_pr = 0 then do:
    ...
    for each xxpc_mstr
        ...
        where xxpc_domain = global_domain
            and xxpc_part = sod_part
            and xxpc_um = sod_um
            and xxpc_curr = so_curr
    ...
    find first xrate_ctrl no-lock no-error.
    if available(xrate_ctrl) then decPrice = decPrice * (1 + xrate_rate / 100).
    sod_list_pr = decPrice.
end.
/* SS - 100620.1 - E */
            update
               sod_list_pr
...
/* ss 20071228 -b */
						if cmtindx = 0 then do:
							...
							find first cd_det where cd_domain = global_domain and cd_ref = sod_part and cd_type = "so" 
								and cd_lang = "CH" no-lock no-error.
							if not avail cd_det then do:
								find last cmt_det where cmt_domain = global_domain and cmt_ref = sod_part and cmt_type = "SO"
									and cmt_lang = "ch" no-lock no-error.
								if avail cmt_det then do:
									cmtindx = cmt_indx.
									for each cmt_det where cmt_domain = global_domain and cmt_ref = sod_part and cmt_type = "SO"
										and cmt_lang = "ch" and cmt_indx = cmtindx no-lock:
										find first cd_det where cd_domain = global_domain and cd_ref = cmt_ref and cd_type = cmt_type
  										and cd_lang = cmt_lang and cd_seq = cmt_seq no-lock no-error.
										if not avail cd_det then do:
  										create 	cd_det.
  										assign 	cd_domain = cmt_domain
         										cd_ref = cmt_ref
         										cd_type = cmt_type
         										cd_lang = cmt_lang
         										cd_seq  = cmt_seq .
   										repeat xxi = 1 to 15.
         								cd_cmmt[xxi] = cmt_cmmt[xxi].
   										end.
  									end. /* if not avail cd_det */
									end. /* for each cmt_det */
									cmtindx = 0.
								end. /* if avail cmt_det then do */
							end. /* if not avail cd_det then do */
						end. /* if cmtindx = 0 */
/* ss 20071228 -e  */
...
------------------------------------------------------------------*/
   ...
   if yn and p-errnum = 4639
          then do:
             ...
             if available po_mstr and po_stat = "c"
             then do:
                find first pod_det
                 where pod_domain = global_domain and
                       pod_nbr    = po_nbr and
                       pod_line = sod_det.sod_btb_pod_line and
                       pod_stat = "c" no-error.
                  assign yn = no.
...
----------------------------------------------------------------------------*/
   ...
   no-lock:
      ...
      for first si_mstr
         ...
         where si_domain = global_domain and si_site = p-site
      no-lock:
