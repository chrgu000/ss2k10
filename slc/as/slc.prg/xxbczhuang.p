...
define variable usection as char format "x(16)". 
	...
	FORM
		sodnbr		 colon 8 label "订 单"
		sodline		 colon 8 label "项"	format ">>9"
		huoyundan_no colon 8 label "发运单"
		pack_no      colon 8 label "装箱单"
		vin			 colon 8 label "VIN码"
		/*"箱号总出库数:" at 3 
		chr_tot_qty no-label*/
		skip(2)
		msg at 1 no-label  
...
REPEAT : 	
	...
	update 
		...
		sodline 
	with frame a.
	find first sod_det where sod_domain = global_domain and sod_nbr = sodnbr 
	and sod_line = sodline	/*---Add by davild 20080909.1*/
	no-lock no-error.
	...
	/*---Add End   by davild 20081011.1*/
 	repeat:
		...
		if avail xxsovd_det then do:
			if not(xxsovd_nbr = sodnbr and xxsovd_line = sodline) then do:
 				msg = "错误:VIN码对应的订单项是 " + xxsovd_nbr + "-" + string(xxsovd_line) .
				display msg with frame a .
				next .
			end.
			wolot = xxsovd_wolot .
