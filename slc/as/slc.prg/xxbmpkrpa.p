...
*/
					define variable op  as int format ">>>>>>" no-undo.
					define variable op1 as int format ">>>>>>" no-undo.
/* SS 091020.1 - E */
...
/* SS 091020.1 - B */
define var xxcmmt as char format "x(210)" label "零件描述".
define var xxamt  like ar_amt.
/* SS 091020.1 - E */
...
/*H100*/    op				colon 15 op1   label {t001.i} colon 45
            line			colon 15 line1 label {t001.i} colon 45
            skip(1)
            qty            colon 15
            eff_date       colon 15
...
/*GN61*/ find si_mstr where si_domain = global_domain and si_site = site  no-lock no-error.
...
/*K10Y*/    {wbrp06.i &command = update &fields = " part part1 fromset
               site op op1 line line1 qty eff_date    " &frm = "a"}
 ...
 * /*G234*/ /* Added section */
 *          find si_mstr where si_domain = global_domain and si_site = site no-lock no-error.
...
/*GN61*/          if not can-find(si_mstr where si_domain = global_domain and si_site = site) then do:
 ...
 * /*G265*/ where bom_domain = global_domain and bom_parent >= part and bom_parent <= part1
 * /*G265*/ with frame b width 80 no-attr-space down:
 *
 * /*G265*/    find pt_mstr where pt_domain = global_domain and pt_part = bom_parent no-lock no-error.
...
/*G1H5*/    find first pt_mstr where pt_domain = global_domain and pt_part >= part and
/*G1H5*/                             pt_part <= part1 and
/*G1H5*/                             pt_prod_line >= line and
/*G1H5*/                             pt_prod_line <= line1
/*G1H5*/    no-lock no-error.

/*G1H5*/    find first bom_mstr where bom_domain = global_domain and bom_parent >= part and
...
*/
						repeat with frame b width 400 no-attr-space down:
/* SS 091020.1 - E */
               /* SET EXTERNAL LABELS */
...
/*G1H5*/             find bom_mstr where bom_domain = global_domain and bom_parent = pt_bom_code
/*G1H5*/             no-lock no-error.
/*G1H5*/          else
/*G1H5*/             find bom_mstr where bom_domain = global_domain and bom_parent = pt_part
/*G1H5*/             no-lock no-error.
/*G1H5*/       end.
...
/*G1JF*/             if last-of(pkop) then do:
                        find ptmstr where ptmstr.pt_domain = global_domain and ptmstr.pt_part = pkpart
                        no-lock no-error.

                        if page-size - line-counter < 2 then page.

/*SS 20080504 - B*/
									find first spt_det where spt_domain = global_domain and spt_det.spt_site = site and spt_sim = fromset and spt_part = pkpart
										and spt_element = "物料" no-lock no-error.
/*SS 20080504 - E*/
...
/*SS 20080504 - E*/                                                     
/*SS 20080623 - B*/
													(accum total by pkop pkqty) * (spt_cst_tl + spt_cst_ll) when avail spt_det @ ar_amt
/*
/*H100*/                   pkop label {&bmpkrpa_p_2}
                           ptmstr.pt_loc when (available ptmstr)
*/
/*SS 20080623 - E*/
                        with frame b.
...
/*G234*/                for each ps_mstr no-lock where ps_domain = global_domain and ps_par = parent_bom
                            and ps_comp = pkpart
...
/* SS 091020.1 - B */
												find first cd_det where cd_domain = global_domain and cd_ref = pkpart and cd_type = "sc" and cd_lang = "ch" no-lock no-error.
												if avail cd_det then do:
													disp cd_cmmt[1] + cd_cmmt[2] + cd_cmmt[3] + cd_cmmt[4] @ xxcmmt with frame b.
												end.
												if avail spt_det then xxamt = xxamt + (accum total by pkop pkqty) * (spt_cst_tl + spt_cst_ll).
/* SS 091020.1 - E */
...
/* SS 091020.1 - B */
									disp "----------------" @ ar_amt with frame b.
									down 1 with frame b.
									disp parent_bom + " 合计:" @ pt_mstr.pt_part xxamt @ ar_amt with frame b.
									down 1 with frame b.
									xxamt = 0.
/* SS 091020.1 - E */
...
/*G1H5*/             find first bom_mstr where bom_domain = global_domain and bom_parent = bom-code
/*G1H5*/             no-lock no-error.
/*G1H5*/          else
/*G1H5*/             find first bom_mstr where bom_domain = global_domain and bom_parent > item-code
/*G1H5*/                                   and bom_parent <= part1
/*G1H5*/             no-lock no-error.
/*G1H5*/          find first pt_mstr where pt_domain = global_domain and pt_part > item-code
...
/*G1H5*/       end.
/*G1H5*/       else do:
/*G1H5*/          find first bom_mstr where bom_domain = global_domain and bom_parent > item-code and
/*G1H5*/                                    bom_parent <= part1
/*G1H5*/          no-lock no-error.
/*G1H5*/       end.
