...
define variable oldname like vp_vend_part.
define variable totalnum as integer init 0.
...
form
   lvl
   ps_comp
   oldname    column-label "老件号"
   desc1
   ps_qty_per
   ps_op
   pt_ord_pol format "x(4)"
   pt_ord_per 
   pt_iss_pol format "x(4)"
   pt_buyer
   pt_pm_code 
   pt_insp_rqd 
   pt_insp_lead 
   ...
   pt_ord_min
   pt_ord_mult
   xxtemp     column-label "描述"
with frame heading  width 350 no-attr-space.
...
repeat:
   ...
   then
	 ...
	 find first cd_det where cd_domain = global_domain and cd_ref = parent and cd_type = "SC" and cd_lang = "ch" no-lock no-error. 
	 do j = 1 to 3 : 
...
PROCEDURE process_report:
   ...
   define query q_ps_mstr for ps_mstr
      fields( ps_domain ps_comp ps_end ps_par ps_ps_code ps_qty_per ps_start ps_op).
   ...
   repeat while available ps_mstr with frame heading  down:
      ...
      then do:
         ...
         find first vp_mstr where vp_domain = global_domain and vp_part = ps_comp and vp_vend = "" no-lock no-error.
         if avail vp_mstr then
         	do:
         		oldname = vp_vend_part.
         	end.

        find first cd_det where cd_domain = global_domain and cd_ref = ps_comp and cd_type = "SC" and cd_lang = "ch" no-lock no-error. 		
				do j = 1 to 3 : 
				        	if (avail cd_det and cd_cmmt[j] <> "") then do:
				        	   xxtemp = xxtemp + cd_cmmt[j].
		              end.
				end.
         display
