...
repeat with frame a:
   ...
   for first pt_mstr
      fields( pt_domain pt_desc1 pt_desc2 pt_formula pt_joint_type pt_drwg_size
...
/*SS 20081222 - B*/
			if lastkey = keycode("CTRL-A") then do:
				{gprun.i ""xxbmpsmt01.p"" "(input (input ps_comp),output comp)"}
				disp comp @ ps_comp with frame b.
				next.
			end.
/*SS 20081222 - B*/
...
/*SS 20081117 - B*/
					if avail pt_mstr and pt_phantom then do:
						ps_ps_code = "X".
					end.
/*SS 20081117 - B*/
...
/*SS 20081203 - B*/
				...
				define var xxrst as logical.
				{pxrun.i &PROC = 'validateItemStatus' &PROGRAM = 'xxptcxr.p'
              &PARAM = "(input ps_par, input 'mt-ps',output xxrst)"
      	}
      	if xxrst then do:
         if batchrun = yes then
            undo b-loop, leave.
