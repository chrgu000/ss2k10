/* Modify by axiang 2009-07-01                    */
/* SS - 100223.1 By: Randy Li */
...
/* ss 20080227 - e */
	...
	define variable xxold as character format "x(22)".
	define variable oldname like pt_desc1 format "x(18)".
	...
	define variable xxqty34 like xxqty1.        /*无采购订单退货*/
	...
	define variable xxqty33 like xxqty1.        /*品技换件*/
...
form
	...
	/* SS - 100223.1 - B */
	pt_article column-label "库管员" format "x(4)"
	/* SS - 100223.1 - E */
	ld_loc  COLUMN-LABEL "供应商" 
	vd_sort column-label "简称" format "x(8)"
	xxqty1  column-label "期初"
	xxqty11 column-label "送货"
	xxqty23 column-label "退货"
	xxqty24 column-label "送货小计"
	xxqty25 column-label "生产"
	xxqty26 column-label "品技"
	xxqty27 column-label "采购"
	xxqty28 column-label "售后领用"
	xxqty33 column-label "品技换件"
	xxqty29 column-label "小领料小计"
	xxqty32 column-label "散件领用"
	xxqty16 column-label "生产领用"
	xxqty19 column-label "生产换件"
	xxqty30 column-label "委外"
	xxqty31 column-label "领用小计"
...
with frame c down width 320.
   ...
   if loc1 = hi_char then loc1 = "".
	 ...
	 date2 = today.
      ...
      vend vend1
		...
		{wbrp06.i &command = UPDATE
			&fields = "date1 date2 part part2 loc loc1 vend vend1"
			&frm = "a"}
    ...
    {mfselprt.i "printer" 132}
		...
		for each xxvd where xxvd_part = pt_part 
		                and xxvd_vendno >= vend and xxvd_vendno <= vend1 no-lock:			                    
...
/*      if (tr_type = "RCT-TR" and tr_program = "xxlotr04.p") then xxqty14 = xxqty14 + tr_qty_loc.  */
        ...
        if (tr_type = "RCT-PO" and tr_program = "xxpoporc.p" and tr_qty_loc < 0) then xxqty34 = xxqty34 - tr_qty_loc. 
        ...
        if (tr_type = "ISS-UNP" and tr_program = "xxicunis.p" and tr_rmks begins "换件") then xxqty33 = xxqty33 - tr_qty_loc. 
        if (tr_type = "ISS-SO" and tr_program = "sosois.p" and tr_nbr begins "W") then xxqty27 = xxqty27 - tr_qty_loc.

        if ((tr_type = "ISS-TR" or tr_type = "RCT-TR") and tr_program = "xxlotr04.p" and tr_rmks begins "售后") then xxqty28 = xxqty28 - tr_qty_loc.
        if (tr_type = "ISS-TR" and tr_program = "xxlotr04.p" and tr_rmks begins "委外") then xxqty30 = xxqty30 - tr_qty_loc. 
        if (tr_type = "ISS-TR" and (tr_program = "xxlotr04.p" or tr_program = "iclotr03.p") and tr_rmks begins "散件") then xxqty32 = xxqty32 - tr_qty_loc.   
/************************end***************************************************/				
			end.
...
*/
				...
				/*送货数小计*/
				xxqty24 = xxqty11 - xxqty23 - xxqty34.
				/*小领料小计*/
				xxqty29 = xxqty25 + xxqty26 + xxqty27 + xxqty28 + xxqty33.
				/*领用小计*/
				xxqty31 = xxqty29 + xxqty16 + xxqty19 + xxqty30 + xxqty32.
				
				xxqty1 = xxqty5 + xxqty31 - xxqty24.
        ...
        and substring(ld_lot,7) = xxvd_vendno no-lock 
				break by ld_loc by substring(ld_lot,7):
					...
					if last-of(substring(ld_lot,7)) then do:	
						...
						{gprun.i ""xxaddoldname.p"" "(input pt_part,output oldname)"}
						disp 
							pt_part @ tr_part
							pt_desc1
							oldname
