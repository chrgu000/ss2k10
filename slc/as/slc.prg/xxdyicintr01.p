/* icintr01.p - COMMON PROGRAM FOR MISC INVENTORY TRANSACTIONS                */
...
/* Revision: 1.31    BY: Paul Donnelly (SB) DATE: 06/26/03  ECO: *Q00G*       */
/* Revision: 1.32    BY: Rajinder Kamra     DATE: 06/23/03  ECO: *Q003*       */
/* Revision: 1.34    BY: Anitha Gopal       DATE: 09/29/03  ECO: *P14L*       */
/* Revision: 1.35    BY: Inna Fox           DATE: 06/21/04  ECO: *P1CZ*       */
/* Revision: 1.36    BY: Shivanand H        DATE: 08/30/04  ECO: *P2HL*       */
/* $Revision: 1.36.2.1 $ BY: Jean Miller        DATE: 01/10/06  ECO: *Q0PD*       */
...
define input parameter iptloc like ld_loc.
define input parameter iptlot like ld_lot.
define input parameter iptref like ld_ref.
/*SS 20080928 - E*/
...
define variable trans_ok   as   logical     no-undo.
...
/*SS 20080928 - B*/
	input through echo value(iptpart) no-echo.
/*SS 20080928 - E*/
...
repeat:
   ...
   editing:
      ...
      {mfnp.i pt_mstr pt_part  " pt_mstr.pt_domain = global_domain and pt_part
...
find pt_mstr using  pt_part where pt_mstr.pt_domain = global_domain  no-lock
no-error.
   ...
   if pt_joint_type = "1"
   then do:
      find ptmstr2  where ptmstr2.pt_domain = global_domain and
   ...
   repeat on endkey undo mainloop, retry mainloop:
      ...
      if not can-find (first sr_wkfl  where sr_wkfl.sr_domain = global_domain
      and  sr_userid = mfguser
                                       and sr_lineid = cline)
      then do:
      ...
      if can-find (sr_wkfl  where sr_wkfl.sr_domain = global_domain and
      sr_userid = mfguser
                             and sr_lineid = cline)
      then do:
...
/*SS 20080928 - B*/
				disp iptsite @ wip_site 
						iptqty @ lotserial_qty
						iptloc @ location
						iptlot @ lotserial
						iptref @ lotref with frame a.
				back_mod = yes.
/*SS 20080928 - E*/
         ...
         if not can-find (sr_wkfl  where sr_wkfl.sr_domain = global_domain and
         sr_userid = mfguser
            and sr_lineid = cline)     /* (failed or ambiguous) */
            and can-find (first sr_wkfl  where sr_wkfl.sr_domain =
         ...
         find ptp_det  where ptp_det.ptp_domain = global_domain and  ptp_site =
         wip_site and
         ...
         for each sr_wkfl
             fields(sr_domain sr_lotser sr_loc sr_site
                    sr_lineid sr_userid sr_qty sr_ref)
      ...
      sr_userid = mfguser
         and sr_lineid = cline)     /* (failed or ambiguous) */
         and can-find (first sr_wkfl  where sr_wkfl.sr_domain = global_domain
         and  sr_userid = mfguser
         and sr_lineid = cline)     /* (found so must be multiples) */
      then
      repeat:
   ...
   if back_mod = no and back_qty <> 0 then do:
      ...
      for each sr_wkfl
         ...
         find first pk_det  where pk_det.pk_domain = global_domain and  pk_user
         = sr_userid and
