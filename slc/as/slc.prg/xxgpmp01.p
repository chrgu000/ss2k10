...
/* $Revision: 1.9.1.6 $  BY: Jean Miller        DATE: 12/12/01  ECO: *P03N*  */
/* Revision: 1.9.1.4  BY: Tony Brooks DATE: 05/02/02 ECO: *P05X* */
/* $Revision: 1.9.1.6 $ BY: Paul Donnelly (SB) DATE: 06/26/03 ECO: *Q00F* */
...
for first drp_ctrl
   fields( drp_domain drp_mrp)
 where drp_ctrl.drp_domain = global_domain no-lock: end.

if available drp_ctrl then
   drp = drp_mrp.

for first mrpc_ctrl
   fields( mrpc_domain mrpc_drp)
