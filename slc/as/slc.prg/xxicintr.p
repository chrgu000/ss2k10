...
/* By: Neil Gao Date: 07/04/13 Date: * ss 20070413.1 * */
...
/* ss 20070413.1 - b */
define new shared variable lcsn as char format "x(11)".
define var snresult as logical.
/* ss 20070413.1 - e */

/* SS - 100704.1 - B */
define variable bIsRightRmks as logical no-undo.
/* SS - 100704.1 - E */
...
/* ss 20070413.1 - b */
		...
		editing:
			{mfnp.i usrw_wkfl lcsn
							...
							( ( transtype = 'ISS-UNP' and usrw_key1 = 'lcsnjc'  ) or 
							  ( transtype = 'RCT-UNP' and usrw_key1 = 'lcsnjr'  ) )
							and usrw_key2"
...
/*   	if lcsn = "" then do:
*   		find last usrw_wkfl where usrw_key1 begins "lcsn" no-lock no-error.
*   		if avail usrw_wkfl then do:
*   			lcsn = substring( string(year(today)) , 3 , 2 ) + string ( month(today),"99" )
*   						+ string(day(today),"99") .
*   			if usrw_key2 > lcsn then lcsn = lcsn + string( ( int ( substring(usrw_key2,7,3) )  + 1  ),"999" ).
*   			else lcsn = lcsn + "001".
*   		end.
*   		else do:
*   			lcsn = substring( string(year(today) )  , 3 , 2  ) + string ( month(today),"99" )
...
*  	end. /* if lcsn = "" */
*  	else do:
*  		find first usrw_wkfl where usrw_key1 = "lcsn" + transtype no-lock no-error.
...
*/
   do transaction :
   	{gprun.i ""xxicintra.p"" "(shipnbr, ship_date, inv_mov, 0, true)" }
   end.
   
   	hide frame a no-pause.
   	if transtype = "ISS-UNP" then do:
			{gprun.i ""xxunisrp02.p"" "( input lcsn )"}
		end.
		else if transtype = "RCT-UNP" then do:
			{gprun.i ""xxunrcrp02.p"" "(input lcsn)"}
		end.
