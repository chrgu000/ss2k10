...
then do:    /* FIND NEXT RECORD */
    find next {1} where {3} and {4} > {5} use-index {2} no-lock no-error.

    if not available {1} then do:
        {pxmsg.i &MSGNUM=20 &ERRORLEVEL=2}  /* End of file */
        if recno = ? then
            find first {1} where {3} use-index {2}
                no-lock no-error.
        else if recno <> ? then
...
then do:    /* FIND PREVIOUS RECORD  */
    find prev {1} where {3} and {4} < {5} use-index {2} no-lock no-error.

    if not available {1} then do:
        {pxmsg.i &MSGNUM=21 &ERRORLEVEL=2}  /* Beginning of file */
        if recno = ? then
            find last {1} where {3} use-index {2} no-lock no-error.
        else if recno <> ? then
            find {1} where recno = recid({1}) no-lock no-error.
        input clear.
    end.
