...
define temp-table xxtt
  field xxtt_f1 like pt_part
	field xxtt_f2 like pt_desc1
	field xxtt_f3 like pt_desc2
	index xxtt_f1
	      xxtt_f1 .
...
repeat with frame a:
	...
	for each pt_mstr where pt_domain = global_domain and pt_part begins part
		and length(pt_part) = 7 no-lock:
		
		create xxtt.
		assign xxtt_f1 = pt_part
						xxtt_f2 = pt_desc1
						xxtt_f3 = pt_desc2
						.
	...
	repeat:
		...
		{xuview.i
    	     ...
    	     &exit-flag = true
    	     &record-id = tt_recid
    	     &cursordown = " 	if avail xxtt then run dispcmmt(xxtt_f1 ).
                         "
    	     &cursorup   = "  if avail xxtt then run dispcmmt(xxtt_f1 ).
                         "
    	     }
    ...
    if keyfunction(lastkey) = "return" then do:
    	level    = 1.
    	run process_crtb (input xxtt_f1,input level).
