...
/* BY: Neil Gao Date: 08/01/13 ECO: * ss 20080113 */
/* By: Neil Gao Date: 08/01/26 ECo: * ss 20080126 */
         ...
         then do with frame b:
            ...
            then do with frame b:
               ...
               then do:
                  ...
                  for first pt_mstr
                     fields( pt_domain pt_part pt_status pt_rev)
...
/* ss 20071226 - b */
               	...
               	if index(wo_mstr.wo_so_job,"&") > 0 then do:
              		for each womstr where womstr.wo_domain = global_domain and womstr.wo_nbr = entry(1,wo_mstr.wo_so_job,"&")
              			and womstr.wo_lot = entry(2,wo_mstr.wo_so_job,"&") no-lock:
              			sodnbr = womstr.wo_so_job .
              			sodline = womstr.wo__dec02.
              			wolotext[dwn - mindwn + 1] = womstr.wo_lot.
              		end.
              		if sodnbr = "" then do:
              			for each sod_det where sod_domain = global_domain and sod_nbr = entry(1,wo_mstr.wo_so_job,"&")
              				and string(sod_line) = entry(2,wo_mstr.wo_so_job,"&") no-lock:
              				sodnbr = sod_nbr.
              				sodline = sod_line.
...
/* ss 20071226 - e */
               ...
               for each xxsob_det where xxsob_domain = global_domain and xxsob_nbr = sodnbr 
               			and xxsob_line = sodline and xxsob_part = wo_mstr.wo_part no-lock:
               
               		vendext[dwn - mindwn + 1]  = xxsob_user1. 
               		expertext[dwn - mindwn + 1] = (xxsob_user2 <> "").

/* ss 20080113 - b */
									if xxsob_user1 <> "" then do:
										find first vp_mstr where vp_domain = global_domain and vp_part = xxsob_part 
											and vp_vend = xxsob_user1 no-lock no-error.
										if not avail vp_mstr then vendext[dwn - mindwn + 1] = "".
									end.
/* ss 20080113 - e */
...
*/
									find first vp_mstr where vp_domain = global_domain and vp_part = wo_part and vp_tp_pct = 100 and vp_vend <> "" no-lock no-error.
									if avail vp_mstr then vend = vp_vend.
/*SS 20080902 - E*/
               end.
...
/*SS 20080417 - B*/
/*
													 tmpdate1 = min(wo_rel_date,today).
													 tmpdate1 = max(tmpdate1,today).
													 tmpdate2 = min(tmpdate1 + ( wo_due_date - wo_rel_date ),wo_due_date).
													 tmpdate2 = max(tmpdate2,today).
*/
...
/* ss 20080126 - e */
                           ...
                           find first req_det where req_domain = global_domain and req_site = wo_site 
