...
/*SS 20080713 - E*/
   ...
   assign
      seg_type          = "5"
      seg_exe_generate  = "seg_generate_xdate"
      seg_exe_validate  = "seg_validate_xdate"
      seg_exe_edit      = "seg_edit_xdate"
      seg_exe_display   = "seg_display_xdate"
      seg_format        = "Y"
      seg_rank          = "1"
      seg_valuemask     = "0909".

END PROCEDURE. /* seg_make_xdate */

PROCEDURE seg_set_xdate:
...
/*SS 20080713 - E*/
   ...
   run seg_format_xdate ( p-seg-recid, output date-min ).

   Trans_effdate = Expdate.
   run seg_format_xdate ( p-seg-recid, output date-max ).
   ...
   run seg_generate_xdate (p-seg-recid, output dummy).

END PROCEDURE. /* seg_set_xdate */

PROCEDURE seg_generate_xdate:
   ...
   run seg_format_xdate ( p-seg-recid, output seg-fmt-val ).
...
END PROCEDURE. /* seg_generate_xdate */

PROCEDURE seg_validate_xdate:
...
END PROCEDURE. /* seg_validate_xdate */

PROCEDURE seg_format_xdate:
...
/*SS 20080713 - B*/
		define var xxnryear 	as char format "x(80)".
		define var xxnrmonth 	as char format "x(30)".
		define var xxnrday 		as char format "x(80)".
		xxnryear = "1,2,3,4,5,6,7,8,9,A,B,C,D,E,F,G,H,J,K,L,M,N,P,R,S,T,V,W,X,Y". 
		xxnrmonth = "1,2,3,4,5,6,7,8,9,A,B,C". 
		xxnrday = "1,2,3,4,5,6,7,8,9,A,B,C,D,E,F,G,H,J,K,L,M,N,P,R,S,T,U,V,W,X,Y".
/*SS 20080713 - E*/
   ...
   do i = 1 to length(seg_format):
      ...
      CASE substring(seg_format,i,1):
         ...
         then do:
/*SS 20080713 - B*/
						if substring(segment.seg_format,i,2) = "Y4" then
							 assign
                  seg-fmt-val = seg-fmt-val +
                                string(year(Trans_effdate),"9999")
                  i           = i + 1.
            else if substring(segment.seg_format,i,2) = "YY" then
            	assign
            			seg-fmt-val = seg-fmt-val +
                           substring((string(year(Trans_effdate),"9999")),3,2)
                  i 					= i + 1.
            else do:
            	seg-fmt-val = seg-fmt-val + entry(( year(Trans_effdate) mod 100 mod 30 ),xxnryear).
            end.
/*
...
*/
            seg-fmt-val = seg-fmt-val + entry(month(Trans_effdate),xxnrmonth).
/*SS 20080713 - E*/

         when "D"
         then
/*SS 20080713 - B*/
            seg-fmt-val = seg-fmt-val + entry(day(Trans_effdate),xxnrday).
/*SS 20080713 - E*/
...
END PROCEDURE. /* seg_format_xdate */

PROCEDURE seg_edit_xdate:
   ...
   do on endkey undo, leave:
      ...
      run seg_set_xdate (seg-recno, date-format, control-seg).
      if Error_state
      then do:
...
END PROCEDURE. /* seg_edit_xdate */

PROCEDURE seg_display_xdate:
   define input  parameter seg-recno    as recid.
...
END PROCEDURE. /* seg_display_xdate */
