...
/* By: Neil Gao Date: 07/11/15 ECO: * ss 20071115 * */
...
/* ss 20071115 - b */
/*SS 20080418 - B*/
define input parameter inptype as char no-undo.
/*SS 20080418 - E*/
define input parameter inpvend like po_vend no-undo.
define new shared variable xxreqnbr like req_nbr.
...
/*SS 20090218 - B*/
define shared var site like pt_site.
/*SS 20090218 - E*/
...
/* ss 20071115 - b */ 
						if new_po then
							disp inpvend @ po_vend with frame a.
/* ss 20071115 - e */
...
/* ss 20071115 - b */
			if inpvend <> input po_vend then do:
				message "供应商不相同".
				next-prompt po_vend with frame a.
        undo, retry.
      end.
...
/* ss 20071211 - b */
/*
      {gprun.i ""popomtb.p"" "(input using_supplier_consignment)"}
*/
...
*/
	 xxreqnbr = "".
	 for each xxreq_det use-index xxreq_vend where xxreq_sel <> "" and xxreq_vend = inpvend and xxreq_type = inptype no-lock:
	 	xxreqnbr = xxreq_nbr.
	 	{gprun.i ""xxpopomt02a.p""}
	 end.
/* ss 20071219 - b 
	 {gprun.i ""popomta.p""}
*/
...
/*SS 20090218 - B*/
/*
					if inptype = "so" then do:
*/
					if site = "12000" then do:
/*SS 20090218 - E*/
						/*
						find last po_mstr where po_domain = global_domain and po_nbr begins "pj" and length(po_nbr) > 6 no-lock no-error.
						if not avail po_mstr then ponbr = "PJ00001".
						else ponbr = "PJ" + string(int(substring(po_nbr,3)) + 1,"99999").
						*/
						ponbr = "PJ" + string( next-value(xxpod_sq01) ).
					end.
/*SS 20080418 - E*/
