...
repeat:
   ...
   {mfselprt.i "printer" 132}          
    ...
    for each po_mstr where po_domain = global_domain and po_nbr >= nbr and po_nbr <= nbr1 
        ...
        /*and (buyer = "" or po_buyer = buyer) */
    		...
    		each pod_det where pod_domain = global_domain and pod_nbr = po_nbr and pod_part >= part and pod_part <= part1 
        	...
        	find first cd_det where cd_domain = global_domain and cd_ref = pod_part and cd_type = "SC" 
        		and cd_lang = "ch" no-lock no-error.
        	
        	find first oa_det where oa_domain = global_domain and oa_nbr = pod_nbr and oa_line = string(pod_line)
