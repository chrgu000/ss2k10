...
define variable v_rt            as character   format "x(5)" label "·µ»Ø" column-label "·µ»Ø".
define variable v_desc          as character  format "x(48)".
define variable xxentry         as char.
define var trt as char.
define var xxres as char format "x(20)".
...
/*SS 20080313 - E*/
define var thnbr as char .

form
   tt-dwn format ">>>"  tt-vin
   skip
   cmmt1 no-label
   cmmt2 no-label
   cmmt3 no-label
...
/*SS 20080313 - B*/
				{xuview.i	&buffer = tt-rqm-mstr
         					...
         					&exit-flag = true
         					&record-id = tt_recid
         					&cursorup =  "  {xxscrp0401.i}
         												"
         					&cursordown = " {xxscrp0401.i}
         							 "
       	}
