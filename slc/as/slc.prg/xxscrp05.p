...
repeat:
   ...
   for each xxcff_mstr where xxcff_key1 = "check" and xxcff_key_nbr >= sonbr and xxcff_key_nbr <= sonbr1 
		     	and xxcff_nbr = "30" no-lock, 
   		each xxcffw_mstr where  xxcffw_key_nbr = xxcff_key_nbr and xxcffw_key_line = xxcff_key_line
		     	/*and xxcffw_check = no*/ and xxcffw_nbr = "30" no-lock,
         ...
         and sod_part >= part and sod_part <= part2 
          ...
          find first pt_mstr where pt_part = sod_part no-lock no-error.
	       	...
	       	assign	tt-dwn = v_dwn
                  ...
                  tt-part   = sod_part
                  tt-status   = "S:(" + so_curr + ")" + string(tmpprice1)
                  tt-yn1 = release_all
      ...
      repeat:
      	...
      	{xuview.i	&buffer = xxtt1
       	...
       	if keyfunction(lastkey) = "return" and avail xxtt1 then do:
       		...
       		{gprun.i ""xxscrp05a.p"" "(input xxtt1_f1)"}
          ...
          if yn then do on error undo, retry:	
          	for each xxtt1 no-lock where xxtt1_f6 or xxtt1_f7 <> "" :
							{gprun.i ""xxscrpmd01.p"" "(input '30',input xxtt1_f1,input xxtt1_f6,input xxtt1_f7)"}
