...
define var crlimit like cm_cr_limit.
define var aramt like ar_amt.
define var soamt like ar_amt.
/*SS 20080313 - E*/
...
form
   "订单:" tt-nbr no-label 
   tt-line    no-label  
   "机型:" pt_desc1 no-label
   skip
   crlimit label "信贷限额"
   aramt label "应收账款" format ">>>>>>"
   soamt label "订单金额"
   cmmt1 no-label
   cmmt2 no-label
   cmmt3 no-label
...
/*SS 20080313 - B*/
				{xuview.i	&buffer = tt-rqm-mstr
         					...
         					&exit-flag = true
         					&record-id = tt_recid
         					&cursorup =  "  {xxscrp0601.i}
         												"
         					&cursordown = " {xxscrp0601.i}
         							 "
       	}
