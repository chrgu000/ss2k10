...
/*By: Neil Gao 08/03/11 ECO: *SS 20080311* */
...
define variable v_yn            as integer format "->>" label "周期" column-label "周期"  no-undo .
define variable v_rt            as character format "x(5)" label "返回" column-label "返回".
...
/* GET NEXT GRS REQUISITION NBR IF RELEASE_ALL = YES */
      ...
      /*V8-*/
         ...
         &exit-flag = true
         &record-id = tt_recid
         &cursorup =  "  {xxscrp0701.i}
         							"
         &cursordown = " {xxscrp0701.i}
         							 "
       	}
...
/*SS 20080311 - B*/
       		...
       		hide frame bb no-pause.
					{gprun.i ""xxmgpcmmt.p"" "(input tt-nbr , input tt-line)"}
					{gprun.i ""xxmbomrp1.p"" "(input tt-nbr , input tt-line)"}
/*SS 20080311 - E*/
					view frame bb.
       		{xxscrp0701.i}
       		update tt-yn with frame d.
       		tt-xc = today + tt-yn - tt-req-date.
       		disp tt-xc with frame d.
       		if tt-req-date < today + tt-yn then do:
				      v_mess = "销售订单" + tt-nbr + "," + string(tt-line) + "时间差" + string((today + tt-yn) - tt-req-date) + "天".
				      message(v_mess).
				 	end.
       	end. 
