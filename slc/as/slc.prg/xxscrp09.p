...
/*By: Neil Gao 08/05/14 ECO: *SS 20080514* */
         ...
         and sod_part >= part and sod_part <= part2 
		    ...
		    break by so_nbr:
	  		...
	  		assign 	dwn = dwn + 1
                ...
                .
        find first pt_mstr where pt_domain = global_domain and pt_part = substring(sod_part,1,4) no-lock no-error.
	      ...
	      assign 	tt-dwn = v_dwn
               	...
               	tt-part   = sod_part
               	tt-status   = ( if avail pt_mstr then pt_desc1 else "")
		  					tt-req-date = today + sod__dec01 + sod__dec02
		  					tt-due-date = sod_due_date
		  					tt-pur-lt   = sod__dec02
		  					tt-yn  			= sod__qadi02
								tt-wo-lt    = integer(sod__dec01)
		  					tt-qty = sod_qty_ord
                tt-yn1 = release_all
		  					tt-desc = ( if avail pt_mstr then pt_desc2 else "")
 		  					tt-cust = so_cust.
      ...
      repeat:
      	...
      	{xuview.i	&buffer = xxtt1
       	...
       	if keyfunction(lastkey) = "return" and avail xxtt1 then do:
       		...
       		{gprun.i ""xxscrp09a.p"" "(input xxtt1_f1)"}
       		update xxtt1_f5 xxtt1_f6 with frame xxtt1.
          ...
          if yn then do on error undo, retry:	
          	for each xxtt1 no-lock where xxtt1_f6 or xxtt1_f7 <> "" :
							{gprun.i ""xxscrpmd01.p"" "(input '80',input xxtt1_f1,input xxtt1_f6,input xxtt1_f7)"}
							{gprun.i ""xxscrpmd02.p"" "(input xxtt1_f1,input xxtt1_f8)"}
							if xxtt1_f5 > today then do:
								for each sod_det where sod_domain = global_domain and sod_nbr = xxtt1_f1:
									sod_due_date = xxtt1_f5.
								end.
							end.
						end.
					end.
