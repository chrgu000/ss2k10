...
/* $Revision: ss-ken 20071205.1   BY: ken        DATE: 12/05/07  ECO: *ss* */
...
find first so_mstr where so_nbr = input so_nbr   no-lock no-error.
/* ss 20071225 - b */
/*
if avail so_mstr then
do:
*/
if avail so_mstr and global_userid <> "mfg" and global_userid <> "lgs01" and global_userid <> "lgi01" then do:
/* ss 20071225 - e */
    if so_userid <> global_userid then
    do:
        message("此订单不是你维护的").
				undo,retry.
    end.
...
*/
   	define variable errorst        as logical no-undo.
		...
		v_number = "".
		find first udd_det where udd_domain = global_domain and udd_userid = global_userid
			and lookup("LGI",udd_groups) > 0  no-lock no-error.
		if avail udd_det then do:
			...
			find first udd_det where udd_domain = global_domain and udd_userid = global_userid 
			and lookup("LGS",udd_groups) > 0  no-lock no-error.
			if avail udd_det then do:
				{gprun.i  ""gpnrmgv.p""
            		"('sonbr2',
              	input-output v_number,
				...
				find first udd_det where udd_domain = global_domain and udd_userid = global_userid 
				and lookup("LGT",udd_groups) > 0  no-lock no-error.
				if avail udd_det then do:
					{gprun.i  ""gpnrmgv.p""
            		"('sonbr3',
              	input-output v_number,
