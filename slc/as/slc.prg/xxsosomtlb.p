...
/*By: Neil Gao 08/03/07 ECO: *SS 20080307* */
/*By: Neil Gao 08/03/26 ECO: *SS 20080326* */
/*By: Neil Gao 08/04/03 ECO: *SS 20080403* */
...
/*ss-ken 2007-12-31 base begin*/
define variable v_price like sod_list_pr.
define variable v_lt like pt_ord_per.
define variable v_mess  as character.
define variable v_price_type as character.
/*ss-ken end */

/* ss 20071228 - b */
define variable bgvin as char format "x(18)".
/* ss 20071228 - e */
...
do on error undo, retry:
   ...
   if sngl_ln then do:
      ...
      assign
         prev_qty_all      = if new sod_det then 0
                             else sod_det.sod_qty_all
...
/* ss 20071223 - b */
			if new_line then do:
				disp no @ sod_det.sod_confirm 0 @ sod_det.sod_qty_all with frame d .
...
/* ss 20071223 - b */
			find first pt_mstr where pt_part = sod_det.sod_part no-lock no-error.
			if pt_pm_code = "M" then do:
      	
      	setpack:
      	do on error undo, retry on endkey undo, leave seta:
      	   	form
      	      bgvin                          colon 10 label "客户机型"
      	      sod_det.sod__chr01             colon 45 label "VIN规则" format "x(18)"
      	      sod_det.sod__chr04             colon 10 label "指定表"
							sod_det.sod_order_category     colon 45 label "打刻式样" format "x(16)"
							sod_det.sod__chr05						 colon 10 label "测试要求" view-as editor no-box size 51 by 1
      	   	with frame set_pack overlay side-labels centered row 16 width 66.
      	   
      	   	/* SET EXTERNAL LABELS */
      	   	setFrameLabels(frame set_pack:handle).

/*SS 20080612 - B*/
						/*
						find first xxbcbc_item where xxbcbc_part = sod_det.sod_part no-lock no-error.
						if avail xxbcbc_item then do:
							if xxbcbc_char01 = "" then 
								find first xxbcbc_item where xxbcbc_part = substring(sod_det.sod_part,1,4) no-lock no-error.
							if avail xxbcbc_item then bgvin = xxbcbc_char01.
						end.
						if bgvin = "" then bgvin = substring(sod_det.sod_part,1,4).
						*/
/*SS 20080612 - E*/
/*SS 20080702 - B*/
						if new_line then do:
							
							{gprun.i ""xxsosomtlbx01.p"" "(	input sod_det.sod_domain,
																							input so_cust,
																							input sod_det.sod_nbr,
																							input sod_det.sod_part,
																							output bgvin,
																							output sod_det.sod__chr01,
																							output sod_det.sod__chr04,
																							output sod_det.sod_order_category,
																							output sod_det.sod__chr05
																					 )"
							}
							if bgvin = "" then do:
								find last xxbcbc_item where xxbcbc_lng = "us" and xxbcbc_part = sod_det.sod_part and xxbcbc_char01 <> "" no-lock no-error.
								if not avail xxbcbc_item then
								find last xxbcbc_item where xxbcbc_lng = "ch" and xxbcbc_part = sod_det.sod_part and xxbcbc_char01 <> ""  no-lock no-error.
								if not avail xxbcbc_item then
								find last xxbcbc_item where xxbcbc_lng = "us" and xxbcbc_part = substring(sod_det.sod_part,1,4) and xxbcbc_char01 <> ""  no-lock no-error.
								if not avail xxbcbc_item then
								find last xxbcbc_item where xxbcbc_lng = "ch" and xxbcbc_part = substring(sod_det.sod_part,1,4) and xxbcbc_char01 <> ""  no-lock no-error.
								if avail xxbcbc_item then do:
									bgvin = xxbcbc_char01.
									sod_det.sod__chr01 = xxbcbc_char37.
									sod_det.sod_order_category = xxbcbc_char38.
									sod_det.sod__chr05 = xxbcbc_char39.
								end.
							end.
						end.
						else do:
							find first xxsov_mstr where xxsov_domain = global_domain and xxsov_nbr = sod_det.sod_nbr
								and xxsov_line = sod_det.sod_line no-lock no-error.
							if avail xxsov_mstr then bgvin = 	xxsov_fix1.
						end.
/*SS 20080702 - E*/
      	   	...
      	   	with frame set_pack.
	 					...
	 					find first xxvbom_mstr where xxvbom_cust = so_cust and  sod_det.sod_part = pt_group no-lock no-error.
	 					if avail xxvbom_mstr then disp xxvbom_nbr @ sod_det.sod__chr04 with frame set_pack.
	 					
	 					set	bgvin when (new_line)
	 							sod_det.sod__chr01 when ( new_line )
								sod_det.sod__chr04
								sod_det.sod_order_category
								sod_det.sod__chr05
	 					with frame set_pack.
...
/*SS 20080616 - B*/
/*
						if sod_det.sod_order_category = "" then do:
							message "打刻式样不能为空".
							undo,retry.
						end.
*/
...
/*SS 20080613 - B*/
						if new_line then do:
							find first nr_mstr where nr_domain = global_domain and nr_seqid = sod_det.sod__chr01 no-lock no-error.
      	   	 	if not avail nr_mstr then do:
      	      	message sod_det.sod__chr01 "Vin规则不存在".
      	      	undo ,retry.
							end.
						
							find first xxsov_mstr where xxsov_domain = global_domain and xxsov_nbr = sod_det.sod_nbr
								and xxsov_line = sod_det.sod_line no-error.
							if not avail xxsov_mstr then do:
								create 	xxsov_mstr.
								assign 	xxsov_domain = global_domain 
											xxsov_nbr = sod_det.sod_nbr
											xxsov_line = sod_det.sod_line
											xxsov_part = sod_det.sod_part.
							end.
							xxsov_seqid = sod_det.sod__chr01.
							xxsov_seg_format = nr_seg_format.
							xxsov_fix1  = bgvin.
						end.
/*SS 20080613 - E*/						
	 					find first xxvbom_mstr where xxvbom_cust = so_cust and  xxvbom_parent = substring(sod_det.sod_part,1,4)
	 						and xxvbom_nbr = sod_det.sod__chr04 no-lock no-error.
	 					if not avail xxvbom_mstr  and sod_det.sod__chr04 <> "" then	do:
	    		 		message "指定件表不存在".
	    	 			undo ,retry.
	 					end.
...
/*
	    	    {gprun.i ""xxscbom.p"" "(INPUT sod_det.sod_part, input sod_det.sod_site, 
	    	    												 ...
	    	    												 input so_curr,input so_cust)"}
*/
	    	    {gprun.i ""xxscbom1.p"" "(INPUT so_cust, input sod_det.sod_part, 
	    	    												 input sod_det.sod__chr04 ,input sod_det.sod_nbr,
	    	    												 input sod_det.sod_line)"}
/*SS 20090225 - E*/
	    	 		
	    	 		find first xxsct_det where  xxsct_part = substring(sod_det.sod_part,1,4) and xxsct_sim = so_cust no-lock no-error.
	    	 		if avail xxsct_det then
	    	      assign sod_det.sod__dec01 = xxsct_ovh_tl.
	    	 		else do:
	    	  	 	message "利润没有维护".
	    	     	undo ,retry.
	    	 		end.
...
/*	    	 		
	    	 		/*2类型计算平均价*/
	    	 		v_price_type  = "2" .
	    	 		{gprun.i ""xxscprice.p"" "(INPUT sod_det.sod_nbr,input sod_det.sod_line,input v_price_type ,OUTPUT v_price,output v_mess)"} 
	    	 		if v_price  <> -1 then assign sod_det.sod__dec02 =  (v_price * ( 1 + xxsct_ovh_ll / 100 ) + xxsct_lbr_tl ) * (1 + xxsct_ovh_tl / 100).
	    	 		else do: message(v_mess).	undo,retry.	end.      		 				
	    	 		/*1类型计算最高价*/
      	    v_mess = "" .	v_price_type  = "1" .
	    	 		{gprun.i ""xxscprice.p"" 	"(INPUT sod_det.sod_nbr,input sod_det.sod_line,input v_price_type ,OUTPUT v_price,output v_mess)"}
	    	 		if v_price  <> -1 then assign sod_det.sod__qadd03 =  (v_price * ( 1 + xxsct_ovh_ll / 100 ) + xxsct_lbr_tl ) * (1 + xxsct_ovh_tl / 100).
	    	 		else do: 	message(v_mess).	undo,retry. 		end.
*/
/*SS 20080525 - E*/
	          /*技术采购最长LT*/
         		{gprun.i ""xxpurlt.p"" "(input sod_det.sod_site,input sod_det.sod_nbr ,input sod_det.sod_line ,OUTPUT v_lt)"}
						assign 	sod_det.sod__qadi02 = v_lt	v_lt = 0.
	     			/*计算生产LT*/
          	{gprun.i ""xxwolt.p"" "(input sod_det.sod_site,input sod_det.sod_part ,input sod_det.sod_qty_ord ,OUTPUT v_lt)"}
						assign sod_det.sod__chr03 = string(v_lt).
         		
         		find first mfc_ctrl where mfc_field = "sc_type" no-error.
	 					if not avail mfc_ctrl then do:
            	message("控制档没有维护").
	     				undo,retry.
          	end.
	     			find first xxcff_mstr where xxcff_key1 = mfc_char and xxcff_key_nbr = sod_det.sod_nbr 
	     			and xxcff_key_line = sod_det.sod_line no-lock no-error.
	     			if not avail xxcff_mstr then do:
	     					{gprun.i ""xxscfw.p"" "(INPUT mfc_char, input sod_det.sod_nbr ,input sod_det.sod_line)"} 
	         			if pt_status = "AC" then do:   
                   /*状态审核默认通过*/
		     					for each xxcffw_mstr where xxcffw_key1 = mfc_char and xxcffw_key_nbr = sod_det.sod_nbr 
		     					and xxcffw_key_line = sod_det.sod_line and xxcffw_nbr = "1" exclusive-lock:
		        				assign 	xxcffw_check = yes 
			       							xxcffw_ck_id = "system"
		        				...
		        				create 	xxcffwh_hist.
			  						assign 	xxcffwh_key1 = xxcffw_key1
			       							xxcffwh_key_nbr = xxcffw_key_nbr
				 									...
				 									xxcffwh_ck_id = "system"
				 									xxcffwh_ck_date = today
				 									xxcffwh_ck_time = time
				 									xxcffwh_date = today.
		     					end.
									find first xxcff_mstr where xxcff_key1 = mfc_char and xxcff_key_nbr = sod_det.sod_nbr 
									and xxcff_key_line = sod_det.sod_line no-error.
									if avail xxcff_mstr then assign xxcff_nbr = "2".
		 						end. /* pt_status = "ac" */
	     			end. /* not avail xxcff_mstr */
     		end. /* setpack: */
	     	...
	     	if not avail xxcff_mstr then do:
					...
					assign 	xxcff_key1 = "check"
									xxcff_nbr  = "8"
									xxcff_key_nbr = sod_det.sod_nbr
									xxcff_key_line = sod_det.sod_line.
					...
					create 	xxcffw_mstr.
    			assign 	xxcffw_key1 = "check"
	   				xxcffw_key_nbr = sod_det.sod_nbr
	   				xxcffw_key_line = sod_det.sod_line
	   				xxcffw_nbr = "8"
	   				xxcffw_cf_desc = "质量部门散件审批"
	   				xxcffw_cf_nbr = "8"
	   			...
	   			create 	xxcffw_mstr.
					assign 	xxcffw_key1 = "check"
	   				...
	   				xxcffw_key_line = sod_det.sod_line
	   				xxcffw_nbr = "8.5"
	   				xxcffw_cf_desc = "采购部散件审批"
	   				xxcffw_cf_nbr = "10"
	   				xxcffw_user = global_userid
	   				xxcffw_date = today
	   				xxcffw_time = time.	
	     	end. /* if not avail xxcff_mstr */
			end. /* else do */
/* ss 20071223 - e */
   ...
   then do:
      ...
      assign
         sod_det.sod_promise_date = exp-del when (sod_det.sod_promise_date = ?)
...
/*SS 20080401*/ hide frame set_pack no-pause.
