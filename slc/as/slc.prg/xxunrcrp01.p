...
for each usrw_wkfl where usrw_domain = global_domain and usrw_key1 = sntype and
	...
	if not first(usrw_key2) then page.
	
/*	find first cp_mstr where cp_domain = global_domain and cp_cust = "c0001" and cp_part = tr_part no-lock no-error.    */
	find first ad_mstr where ad_domain = global_domain and ad_addr = substring(tr_hist.tr_serial,10) no-lock no-error.
	...
	find first vp_mstr where vp_domain = global_domain and vp_part = tr_part and  vp_vend = ad_addr no-lock no-error.
  find first tpt_mstr where tpt_mstr.pt_domain = global_domain and tpt_mstr.pt_part = substring(cd_cmmt[1],1,4) no-lock no-error.
	
	for each ld_det where ld_domain = global_domain and ld_part = tr_part and ld_loc = tr_loc no-lock:
								after_qty = after_qty + ld_qty_oh.          
	end.
	begin_qty = after_qty - tr_qty_loc. 
	xxtemp = tr_qty_loc.  

          put "隆鑫工业有限公司四轮车本部" at 40 skip(1).
					put "计划外入库单" at 45 skip(1).
					put "供应商代码:" at 1.
					if avail ad_mstr then put ad_addr.
					put "制单日期:" at 40 today string(time,"hh:mm am").
					put "单据号:" at 75 usrw_key2 format "x(11)" skip.
					put "供应商名称:".
					...
					if avail vp_mstr then 
						put vp_vend_part format "x(16)".
					put "物料名称:" at 52 pt_mstr.pt_desc1 format "x(24)".
					put "单位:" at 86 pt_mstr.pt_um skip.
					put "批序号:" tr_serial.
					put "参考号:" at 30 tr_ref.
					put "旧机型:" at 52.
					if avail tpt_mstr then 
						put tpt_mstr.pt_desc1. 
					put skip. 
					put "描述:".
					do j = 1 to 15 : 
          ...
          put "期初数量:" string(begin_qty).
          put "本次入库数:" at 30 string(xxtemp).
          put "期末数量:" at 52 string(after_qty).
          put "库位:" at 70 tr_loc skip.
					put "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" skip.
					put "制单人:" usr_name format "x(10)".
					put "库管员:" at 30.
					put "配料员" at 50 skip.
					put "[白联：换件库库管 红联：财务 蓝联：合格库库管 黄联：生产回执]" .
					after_qty = 0.
					begin_qty = 0.
