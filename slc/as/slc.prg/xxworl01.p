...
/* By: Neil Gao Date: 07/11/26 ECO: * ss 20071116 * */
...
/*SS 20081117 - B*/
			...
			if avail usr_mstr then do:
				deliv = substring(usr_remark,1,8).
			end.
/*SS 20081117 - E*/
...
/*      
      {gprun.i ""woworl1.p""}
*/
		  for each wod_det where wod_domain = global_domain and wod_lot = wrlot and wod_loc <> deliv ,
		  	each pt_mstr where pt_domain = global_domain and pt_part = wod_part and pt_loc = deliv no-lock:
		  	assign wod_loc = pt_loc.
			end.
		  {gprun.i ""xxwoworl101.p""}
/* ss 20071126 - e */
      {mfreset.i}
