...
/* By: Neil Gao Date: 07/04/18 ECO: * ss 20070418 * */
...
/* ss 20070418.1 - b */
define new shared variable lcsn as char format "x(11)" no-undo.
define var snresult as logical.
/* ss 20070418.1 - e */
/* ss 20070412.1 - b */
DEFINE NEW SHARED VARIABLE strLoc LIKE loc_loc.
/* ss 20070412.1 - e */

/* ss 20070606.1 - b */
define new shared temp-table tqc
	...
	field tqc_log like xxqc_autoissue label "连带"
...
/* ss 20070418.1 - b */
		...
		*/
		set lcsn with frame a
		editing:
			{mfnp.i usrw_wkfl lcsn
							"usrw_domain = global_domain and usrw_key1 = 'lcsnlc'  and usrw_key2"
...
/*
*   	if lcsn = "" then do:
*   		find last usrw_wkfl where usrw_domain = global_domain and usrw_key1 = "lcsnlc" no-lock no-error.
*   		if avail usrw_wkfl then do:
*   			lcsn = "LC" +  substring( string(year(today)) , 3 , 2 ) + string ( month(today),"99" )
*   						+ string(day(today),"99") .
...
*  	end. /* if lcsn = "" */
*  	else do:
*  		find first usrw_wkfl where usrw_domain = global_domain and usrw_key1 = "lcsnlc"  and usrw_key2 = lcsn no-lock no-error.
...
*/
         {mfnp.i wo_mstr wo_nbr  " wo_mstr.wo_domain = global_domain and wo_status = 'R' and wo_nbr
         "  wo_nbr wo_nbr wo_nbr}
/* ss 20071211 - e */
...
/* ss 20070412.1 - b */
		...
		if avail usr_mstr then do:
			strloc = substring(usr_remark,1,8).
		end.
		UPDATE strLoc WITH FRAME a.
/* ss 20070412.1 - e */
...
*/
   repeat transaction on error undo,retry:
   		view frame a.
   		{gprun.i ""xxwowoisbx01"" "(wo_recno,wo-op, fill_all, fill_pick, output undo-input)"}
   		if undo-input then do:
   		
   			message "是否修改备料信息?" update undo-input.

				if undo-input then do:
					hide frame a no-pause.
					/*{gprun.i ""xxwowoisx01"" "(wo_nbr,wo_lot,strloc)"}*/
					{gprun.i ""xxdywowamt.p"" "(wo_nbr,wo_lot,strloc)"}
				end.
				else next mainloop.
   			next.
  		end.
