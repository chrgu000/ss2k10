...
/* Revision: 1.30     BY: Niranjan Ranka           DATE: 06/25/02 ECO: *P09L* */
/* Revision: 1.31     BY: Dorota Hohol             DATE: 03/06/03 ECO: *P0N6* */
/* Revision: 1.32     BY: Narathip Weerakitpanich  DATE: 04/29/03 ECO: *P0QN* */
/* Revision: 1.33     BY: Narathip Weerakitpanich  DATE: 05/09/03 ECO: *P0RN* */
/* Revision: 1.34     BY: Ashish Maheshwari        DATE: 05/15/03 ECO: *N278* */
/* Revision: 1.36     BY: Paul Donnelly (SB)       DATE: 06/28/03 ECO: *Q00N* */
/* Revision: 1.37     BY: R.McCarthy/T.Giustozzi   DATE: 04/19/04 ECO: *P15V* */
/* Revision: 1.38     BY: Bhagyashri Shinde        DATE: 07/14/04 ECO: *P29X* */
/* $Revision: 1.39 $    BY: Shoma Salgaonkar         DATE: 08/25/04 ECO: *Q0CJ* */
/* By: Neil Gao Date: 07/05/21 ECO: * ss 20070521 * */
...
/* ss 20070418.1 - b */
define shared variable lcsn as char format "x(11)" no-undo.
...
for first wo_mstr
   fields (wo_domain wo_site wo_nbr wo_lot wo_type)
...
for each wo_mstr exclusive-lock
   ...
   {gprun.i ""gpincr.p""
            "(input no,
              ...
              input if available pt_mst then pt_rctpo_active
                    else no,
   ...
   no-lock:
      ...
      {ictrans.i
         ...
         &crproj="if available pt_mstr then wo_project else """""
         &curr=""""
         &dracct=dr_acct
         &drsub=dr_sub
         &drcc=dr_cc
         &drproj="if available pt_mstr then wo_project else """""
         &effdate=eff_date
         &exrate=0
         &exrate2=0
         &exratetype=""""
         ...
         &lbrstd=0
         &line=0
         &location="(if site <> wo_site then pt_loc else location)"
         &lotnumber=l_wo_lot
         &lotref=lotref
         &lotserial=lotser
         &mtlstd=0
...
/* ss 20070418.1 - b */
/*			find first usrw_wkfl where usrw_domain = global_domain and usrw_key1 = "lcsnlc"  and usrw_key2 = lcsn no-error.
			if avail usrw_wkfl then do:
				assign tr__chr01 = lcsn.
				repeat xxi = 1 to 15 :
					if usrw_charfld[xxi] = "" then do:
					usrw_charfld[xxi] = string(tr_trnbr).					
					leave.
				end.
				if xxi = 15 then usrw_charfld[xxi] = usrw_charfld[xxi] + "," + string(tr_trnbr) .
				end. /* repeat */
			end.
*/
...
/* ss 20070521.1 - e */
      ...
      if wo_fsm_type <> "RMA" then do:
         if is_wiplottrace_enabled()
            and is_operation_queue_lot_controlled(wod_lot, wod_op, OUTPUT_QUEUE)
            ...
            no-lock:
               ...
               run add_trace_record in h_wiplottrace_procs
                  (input OPERATION_HISTORY,
                   ...
                   input wod_part,
                   input lotser,
                   input if is_wocomp_reference_traced(wod_lot, wod_part) then
                            lotref
                         else '',
                   input trqty).
      ...
      if sr_qty <> 0 then do:
         ...
         if available lad_det then do:
            ...
            if available ld_det then
               ld_qty_all = ld_qty_all - open_ref.
