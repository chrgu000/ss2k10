...
/* By: Neil Gao Date: 08/01/02 ECo: * ss 20080102 */
...
/* ss 20080102*/	and wod_loc = deliv
	no-lock:
	xxqty1 = 0.
	for each ld_det where ld_domain = global_domain and ld_part = wod_part and ld_site = wod_site
		and ld_status begins "Y"	and ld_qty_oh - ld_qty_all > 0 and substring(ld_lot,7) <> ""  no-lock 
...
wo_lot 
/* ss 20080102*/	and wod_loc = deliv no-lock:
