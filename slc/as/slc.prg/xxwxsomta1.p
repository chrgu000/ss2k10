...
/* $Revision: 1.71.1.5 $          BY: Preeti Sattur        DATE: 03/21/06  ECO: *Q0SL* */
/* By: Neil Gao Date: 07/12/26 ECO: * ss 20071226 * */ 
...
if xxglobal_site = "slc" then do:	
	define var orderrules as char init "W".
/*SS 20081210 - B*/
/*
	{gprun.i ""xxsosomta1x03.p"" "(output orderrules)"}
...
*/
/*SS 20081210 - E*/
  disp orderrules @ so_nbr with frame a.
end.
/*SS 20080611 - E*/
...
/*订单权限控制*/
if xxglobal_site = "slc" then do:
	if not ( input so_nbr ) begins orderrules then do:
		message "不是选择此类订单" view-as alert-box.
		undo,retry.
	end.
	if substring(input so_nbr,2) > "" then do:
		define var presult as char.
		{gprun.i ""xxsosomta1x01.p"" "(input (input so_nbr), output presult)"}
		if presult > "0" then undo,retry.
	end.
end.
...
/*SS 20080611 - B*/
		if xxglobal_site = "rato" or xxglobal_site = "SLC" then do:
			if substring(input so_nbr,2) = "" then 
				disp "" @ so_nbr with frame a.
		end.
/*SS 20080611 - E*/
...
/*SS 20080609 - B*/
		/* 自定义产生订单号规则 */
		if xxglobal_site = "rato" or xxglobal_site = "SLC" then do:
				{gprun.i ""xxsosomta1x02.p"" "(input '4', output sonbr)"}
		end.
		else do:
      if this-is-rma then do:
