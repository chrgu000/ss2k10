...
/*SS 20080807 - B*/
repeat:
	find first tt_tb no-error.
	if not avail tt_tb then do:
		leave.
	end.
	else if tt_f1 = "@@begin" then do:
		leave.
	end.
	delete tt_tb.
end.
find first tt_tb where tt_sel = 2 no-error.
if not avail tt_tb then do:
...
/*SS 20080805 - B*/
input through echo value(entry(1,tt_f1)) no-echo.
/*SS 20080805 - E*/
...
*/
      find first tt_tb where tt_sel = 3 no-error.
			if avail tt_tb then do:
				{gprun.i ""xxdrsosomtp.p""
               "(input this-is-rma,
                 input using_cust_consignment)"}
      end.
      else do:
