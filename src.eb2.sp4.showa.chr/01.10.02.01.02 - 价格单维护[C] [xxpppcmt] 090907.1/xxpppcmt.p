...
with frame c overlay no-labels row 11 width 80 attr-space.
 ...
 form
   ...
   glxcst                      to 46 base_curr2
   skip(1)
   ptable_label   to 29 pc_amt[1]   v_label to 60 pc_user1  
   min_label      to 29 pc_min_price
   max_label      to 29 temp_max_price
   /*V8+*/
   /*V8!
	...
	/* ss - 090904.1 -b
	  ...
	  /* ss - 090904.1 -e */
	   ...
	   /* ss - 090904.1 -b
	    ...
	    prompt-for pc_user1  with frame c editing:
	     /*{mfnp.i code_mstr pc_user1 "code_fldname = 'pc_user1' and code_value" pc_user1 code_value code_fldval}
             */
	     {mfnp01.i code_mstr pc_user1  code_value   code_fldname ""pc_user1"" code_fldval}

              if recno <> ? then do:
                    display code_value @ pc_user1 with frame c .
	       end .
	    ...
	    set pc_min_price temp_max_price go-on ("F5" "CTRL-D")
            with frame c width 80.
	    ...
	    /* ss - 090904.1 -b */
	    find first code_mstr where code_fldname = "pc_user1" and code_value = pc_user1 no-lock no-error .
	    if not available code_mstr then do:
	     {pxmsg.i &MSGNUM=5604 &ERRORLEVEL=3}
	     next-prompt pc_user1 with frame c.
	     undo, retry.
	    end .
	    /* ss - 090904.1 -e */
