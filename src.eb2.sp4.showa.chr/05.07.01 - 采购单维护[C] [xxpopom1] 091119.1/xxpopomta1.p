...
/* SS 090902.1 - B */
						/* 控制SCM */
						{gprun.i ""xxscvendcontrl.p"" "(input po_vend,input pod_stat,input pod_user1,
								output rspnbr)"}
						if rspnbr <> 0 then do:
...
/* SS 090902.1 - E */
	    ...
	    /* ss - 090803.1 -b */
              ...
              find last pc_mstr where pc_list = po_pr_list2 and pc_curr = po_curr and pc_part = pod_part
	    and pc_um = pod_um and ( ( pc_start <= po_ord_date or pc_start = ? ) and (pc_expire >= po_ord_date or pc_expire = ?)) no-lock no-error .
	    if not available pc_mstr then do:
	       {pxmsg.i &MSGNUM=7253 &ERRORLEVEL=3}  /* 价格表须存在*/
               undo ,retry . 
	    end .
	    /* ss - 090803.1 -e */
	     ...
	     if available pt_mstr then do:
	       /* ss - 090909.1 -b */  /* 税用途不存在 */
		 if pt__chr02 = "" or pt__chr03 = "" or pt__chr04 = "" then do:
		/* message "税用途等信息不存在" view-as alert-box .
		 */
		 {pxmsg.i &MSGNUM=874 &ERRORLEVEL=3}
