...
/* SS 090902.1 - B */
					define var ifscmvend as logical.
					{gprun.i ""xxwpmtkz01.p"" "(input po_vend,output ifscmvend)"}
					if ifscmvend then do:
						for each pod_det where pod_nbr = po_nbr and pod_status <> "" no-lock:
							/*禁止删除已结(C)或取消(X)的记录*/
							{pxmsg.i &msgnum = 51602 &errorlevel = 3}
							undo,retry.
						end.
					end.
/* SS 090902.1 - E */
