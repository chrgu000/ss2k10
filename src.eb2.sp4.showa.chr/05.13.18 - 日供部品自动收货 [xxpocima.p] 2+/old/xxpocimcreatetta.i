        ...
        IF FIRST-OF(pott_order_type) THEN DO:
            ...
            DO i = 1 TO 99 :
                if avail vd_mstr then do:
                    assign tmp_ponbr = substring(pott_order_type,1,1) + substring(string(year(today)),3) + tmpmonth + substring(vd_sort,1,2) + STRING(i,"99") .
                end.
                else do:
                    assign tmp_ponbr = substring(pott_order_type,1,1) + substring(string(year(today)),3) + tmpmonth + "ER" + STRING(i,"99") .
                end.
        ...
        for each po_mstr where po_vend = pott_vend 
			...
			each pod_det where pod_nbr  = po_nbr 
                           ...
                           AND (( SUBSTRING(pod_nbr,1,1) = "P" AND substring(xx_ship_type,1,1) = "Z") OR
                                ( SUBSTRING(pod_nbr,1,1) = "N" AND substring(xx_ship_type,1,1) = "R") )
