...
do on error undo, leave:
   ...
   if del-yn then
     ...
     do:
      ...
      /* ss - 091008.1 -b
      /* ss - 090824.1 -b */
       find first pi_mstr where pi_list = so_cust and pi_cs_type = "9"
       and pi_cs_code = so_cust and pi_part_type = "6" and pi_part_code = sod_part
       and pi_curr = so_curr and pi_um = sod_um and ((pi_start <= so_ord_date ) and (pi_expire >= so_ord_date or pi_expire = ?)) no-lock no-error .
       if not available pi_mstr then do:
        /* "价格单不存在" 2022 */
	message "价格单不存在" view-as alert-box .
