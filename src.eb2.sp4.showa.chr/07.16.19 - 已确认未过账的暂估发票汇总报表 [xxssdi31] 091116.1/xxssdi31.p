...
/******************************************************************************/
/* ss - 091116.1 by: jack */  /* 增加增值税发票价格，增值税发票金额，按增值税发票价格汇总,*/
/* 以下为版本历史 */
...
DEFINE TEMP-TABLE tt
  FIELD tt_ih_cust  LIKE so_cust
  FIELD tt_name  LIKE ad_name
  FIELD tt_desc1 AS CHAR FORMAT "x(30)"  
  FIELD tt_po LIKE   so_po
  FIELD tt__chr01 LIKE          rqd__chr01
  FIELD tt__chr02 LIKE       rqd__chr02
 FIELD tt__chr03 LIKE            rqd__chr03
  FIELD tt__chr04 LIKE          rqd__chr04
 FIELD tt_effdate LIKE ar_effdate
 FIELD tt_inv_date LIKE ar_effdate
 FIELD tt_inv_nbr LIKE  ih_inv_nbr
 FIELD tt_idh_nbr LIKE  idh_nbr
 FIELD tt_line LIKE idh_line
 FIELD tt_part LIKE idh_part
 FIELD tt_desc2 AS CHAR FORMAT "x(48)"
 FIELD tt_um LIKE idh_um
 FIELD tt_qty LIKE rqd_req_qty          
 FIELD tt_price LIKE net_price          
 FIELD tt_amt LIKE ih_trl1_amt         
 FIELD tt_curr LIKE ih_curr
 FIELD tt_list_price LIKE net_price
 FIELD tt_inv_amt LIKE  ih_trl1_amt
    .
   ...
   */
   ...
   FOR EACH ttxxsoivrp0901
      ...
      AND rqd_cmtindx = tr_trnbr
       /* ss - 091116.1 -b */
       AND (CAN-FIND ( FIRST so_mstr WHERE ( so_nbr = rqd__chr01 OR so_nbr = rqd__chr03 ) AND so__chr01 <> "yes" ) )
       /* ss - 091116.1 -e */
      BREAK 
      BY rqd_nbr
      BY rqd_part
      ...
      FIND FIRST so_mstr 
         ...
         AND */ (so_nbr = rqd__chr01 OR so_nbr = rqd__chr03)
         NO-LOCK NO-ERROR.
      ...
      FIND FIRST so_mstr 
         WHERE /* so_domain = GLOBAL_domain
         AND */ (so_nbr = rqd__chr01 OR so_nbr = rqd__chr03) AND so__chr01 <> "yes"
         NO-LOCK NO-ERROR.
      /* ss - 091116.1 -e */

      ACCUMULATE rqd_req_qty (TOTAL BY rqd_nbr BY rqd_part BY idh_inv_nbr BY idh_nbr BY idh_line).
      IF LAST-OF(idh_line) THEN DO:
         ...
         EXPORT DELIMITER ";" 
            ...
            (ACCUMULATE TOTAL BY idh_line rqd_req_qty)
            ttxxsoivrp0901_base_net_price
            ((ACCUMULATE TOTAL BY idh_line rqd_req_qty) * ttxxsoivrp0901_base_net_price)
            .
            ss - 091116.1 -e */
          /* ss -091116.1 -b */
          FIND LAST xxpi_mstr WHERE xxpi_list =  ttxxsoivrp0901_ih_cust AND xxpi_part = ttxxsoivrp0901_idh_part AND xxpi_um =  ttxxsoivrp0901_idh_um AND xxpi_curr = ih_curr NO-LOCK NO-ERROR .
          IF AVAILABLE xxpi_mstr  THEN
              v_list_price = xxpi_list_price .
          ELSE
              v_list_price = 0 .
          CREATE tt .  
          ASSIGN
          tt_ih_cust  =  ttxxsoivrp0901_ih_cust
          tt_name  =  ttxxsoivrp0901_name
           tt_desc1 =   (rqm_nbr + "." + rqd_part)
          tt_po = so_po 
         tt__chr01 =   rqd__chr01
         tt__chr02  =     rqd__chr02
         tt__chr03 =  rqd__chr03
         tt__chr04 =    rqd__chr04
        tt_effdate =ttxxsoivrp0901_ar_effdate
        tt_inv_date = ttxxsoivrp0901_inv_date
       tt_inv_nbr = ttxxsoivrp0901_ih_inv_nbr
       tt_idh_nbr =   ttxxsoivrp0901_idh_nbr
        tt_line  =  ttxxsoivrp0901_idh_line
        tt_part = ttxxsoivrp0901_idh_part
       tt_desc2 =  (pt_desc1 + pt_desc2)
       tt_um = ttxxsoivrp0901_idh_um
        tt_qty =     (ACCUMULATE TOTAL BY idh_line rqd_req_qty)    
       tt_price =         ttxxsoivrp0901_base_net_price  
        tt_amt =      ((ACCUMULATE TOTAL BY idh_line rqd_req_qty) * ttxxsoivrp0901_base_net_price)   
        tt_curr = ih_curr
        tt_list_price =  v_list_price 
        tt_inv_amt =   (ACCUMULATE TOTAL BY idh_line rqd_req_qty) * v_list_price
           .
          /* ss - 091116.1 -e */
   ...
   /* ss - 091116.1 -b */
   FOR EACH tt NO-LOCK BREAK BY  tt__chr04 BY tt_part BY tt_list_price :
       ACCUMULATE tt_inv_amt (TOTAL BY  tt_list_price  ) .
       ACCUMULATE tt_amt (TOTAL BY  tt_list_price  ).
       ACCUMULATE tt_qty (TOTAL BY  tt_list_price  ).
       ...
       IF LAST-OF( tt_list_price  ) THEN DO:
            EXPORT DELIMITER ";"
           "小计:"
          ""
           "" 
      ...
      ""
       ...
       ""
      ""
        ACCUMULATE TOTAL BY  tt_list_price   tt_qty 
        ""
        ACCUMULATE TOTAL BY  tt_list_price  tt_amt
        ""
        ACCUMULATE TOTAL BY  tt_list_price  tt_inv_amt 
           .
