...
控制字段如下:
  - SoftspeedDI_rqm_nbr: 申请前缀及下一个申请
  - SoftspeedDI_Max: 发票限额
  - SoftspeedDI_Tol: 发票限额容差,当发票金额与限额之差小或等于此容差时需另开发票
  - SoftspeedDI_VAT: 下一个发票,可填写在发标"备注"栏,以备参考
  - SoftspeedDI_so_pre1: 红字订单前缀及下一个红字订单
  - SoftspeedDI_so_pre2: 正式订单前缀及下一个正式订单
  - SoftspeedDI_sod_type1: 红字订单类型
  - SoftspeedDI_sod_type2: 正式订单类型
  - SoftspeedDI_inv_pre1: 红字发票前缀及下一个红字发票
  - SoftspeedDI_inv_pre2: 正式发票前缀及下一个正式发票
  - SoftspeedDI_inv: 下一个增值税专用发票
