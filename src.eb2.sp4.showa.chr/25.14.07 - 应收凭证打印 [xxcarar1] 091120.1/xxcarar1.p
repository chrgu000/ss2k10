...
/* $Revision: 1.11.1.26 $ BY: Bill Jiang        DATE: 07/17/08 ECO: *SS - 091120.1* */
...
repeat:
   ...
   for each ar_mstr  where (  (ar_batch
   >= batch) and
         (ar_batch <= batch1) and
      ...
      /* GET AR DETAIL  */
      for each ard_det  where ard_nbr =
      ar_nbr and
            ard_ref = "" no-lock
            by ard_acct
            by ard_sub
   ...
   */
   ...
   FOR EACH ttssardrrp0001
      ...
      WHERE ac_code = ttssardrrp0001_ard_acct
      AND INDEX("ALIE",ac_type) > 0
      ,EACH sb_mstr NO-LOCK
      WHERE sb_sub = ttssardrrp0001_ard_sub
      ,EACH cc_mstr NO-LOCK
      WHERE cc_ctr = ttssardrrp0001_ard_cc
      ,EACH pj_mstr NO-LOCK
      WHERE pj_proj = ttssardrrp0001_ard_project
      ...
      IF LAST-OF(ttssardrrp0001_ar_nbr) THEN DO:
         ...
         ASSIGN 
            ...
            /* 8 - 截止日期 */
            tt2_user2 = "截止日期: " + string(ttssardrrp0001_ar_due_date)
            ...
            /* 6 - 客户 */
            tt2_ex_rate = "客户: " + ttssardrrp0001_ar_cust + " " + ttssardrrp0001_name
            /* 7 - 批处理 */
            tt2_char1 = "批处理: " + ttssardrrp0001_ar_batch
            .

         {gprun.i ""xxcaren.p"" "(
            INPUT ttssardrrp0001_ar_entity,
