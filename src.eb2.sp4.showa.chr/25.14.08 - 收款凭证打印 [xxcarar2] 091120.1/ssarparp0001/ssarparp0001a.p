...
{&ARPARPA-P-TAG21}
   ...
   for each ar_mstr where (ar_batch >= batch) and (ar_batch <= batch1)
      ...
      for first ard_det where ard_nbr = ar_nbr
      ...
      if ard_type = "U" then do for appl:
         assign b_appl_amt = 0
                c_appl_amt = 0.
         for each appl
            fields(ar_check ar_xslspsn1 ar_type ar_base_amt ar_amt)
            where appl.ar_check     = ar_mstr.ar_check
         ...
         if (ptype = "U" and ard_amt = c_appl_amt) then next.
      end.
         ...
         if not summary then do with frame e:
            ...
            */
            ...
            if ard_type = "U" then do:
               ...
               if ar_mstr.ar_curr <> gl_base_curr and base_rpt <> "" then
            ...
            /* SS - 091120.1 - B
            else display 0 @ unamt.
            SS - 091120.1 - E */
            /* SS - 091120.1 - B */
            ELSE ASSIGN ttssarparp0001_unamt = 0.
            /* SS - 091120.1 - E */
