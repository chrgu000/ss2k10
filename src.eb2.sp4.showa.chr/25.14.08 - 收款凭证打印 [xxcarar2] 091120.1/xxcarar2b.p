...
FOR FIRST ttssarparp0001 NO-LOCK WHERE 
   ...
   WHERE ac_code = ttssarparp0001_ar_disc_acct
   AND INDEX("ALIE",ac_type) > 0
   ,EACH sb_mstr NO-LOCK
   WHERE sb_sub = ttssarparp0001_ar_disc_sub
   ,EACH cc_mstr NO-LOCK
   WHERE cc_ctr = ttssarparp0001_ar_disc_cc
   :
   ...
   assign 
      tt1_dr_amt = ttssarparp0001_ard_base_disc
      .
         
   IF ac_curr <> base_curr THEN DO:
      ASSIGN
         tt1_curramt = ABS(ttssarparp0001_ard_disc)
