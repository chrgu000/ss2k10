...
/* SS - 20080512.1 - B */
/*
1. code_fldname = "glta_acct1"
2. code_user1
*/
/* SS - 20080512.1 - E */

DEF BUFFER codemstr FOR CODE_mstr.  
DEF VAR vdesc LIKE CODE_cmmt.
...
*/
define variable fieldname like code_fldname INITIAL "glta_acct1" no-undo.
/* SS - 20051206 - E */
...
form
    ...
    */
    /* SS - 20051206 - E */
   code_value     colon 25 LABEL "资金帐" format "x(30)" skip(1)
   code_cmmt      colon 25
   /* SS - 20051206 - B */
   CODE_user1 COLON 25 LABEL "格式位置"
   vdesc COLON 25 LABEL "说明" FORMAT "x(40)"
   /* SS - 20051206 - E */
...
repeat with frame a:
   ...
   do on error undo, retry:
      ...
      editing:
         ...
         */
          ...
          if recno <> ? then DO:
              display code_value code_cmmt CODE_user1.
              FOR FIRST codemstr WHERE codemstr.CODE_fldname = 'glta_acct1_fmpos' 
                  AND codemstr.CODE_value = CODE_mstr.CODE_user1 NO-LOCK:
              END.
              IF AVAIL codemstr THEN DO:
                  DISPLAY codemstr.CODE_cmmt @ vdesc WITH FRAME a.
              END.
          END.
      ...
      if length(input CODE_mstr.code_value) > fieldlen then do:
         ...
         if batchrun then do:
            ...
            next-prompt CODE_mstr.code_value with frame a.
         undo, retry.
   ...
   */
   find code_mstr where CODE_mstr.code_fldname = fieldname
                    and CODE_mstr.code_value = input CODE_mstr.code_value
   no-error.
   ...
   DO ON ERROR UNDO,RETRY :
       update
          CODE_mstr.code_cmmt
          /* SS - 20051206 - B */
          CODE_mstr.CODE_user1
          /* SS - 20051206 - E */
       go-on(F5 CTRL-D).

       /* SS - 20080512.1 - B */
       FIND FIRST CODEmstr WHERE codemstr.CODE_fldname = 'glta_acct1_fmpos' 
           AND codemstr.CODE_value = code_mstr.CODE_user1 NO-LOCK NO-ERROR.
       IF NOT AVAIL CODEmstr THEN DO:
           MESSAGE "错误: 该格式位置不存在,请重新输入" .
           NEXT-PROMPT code_mstr.CODE_user1 WITH FRAME a.
           UNDO,RETRY.
       END.
       ELSE DO: 
           DISP codemstr.CODE_cmmt @ vdesc WITH FRAME a.
       END.
       /* SS - 20080512.1 - E */
