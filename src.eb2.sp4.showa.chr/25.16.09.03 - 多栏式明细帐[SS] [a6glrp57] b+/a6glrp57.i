...
IF inclpost = YES THEN DO:
    ...
    v_xxac_name = "" .
    v_xxac_name_desc = "" .
    FOR EACH glt_det NO-LOCK WHERE glt_sub >= sub AND glt_sub <= sub1
                                 AND glt_cc >= ctr AND glt_cc <= ctr1 
                                 AND glt_entity >= entity AND glt_entity <= entity1 
                                 AND glt_effdate >= effdate AND glt_effdate <= effdate1
                                 AND glt_tr_type = "JL" :
    
        FIND FIRST xxac_det WHERE xxac_code = acc
                              AND (glt_acc >= xxac_acctfrom OR xxac_acctfrom = "" )
                              AND (glt_acc <= xxac_acctto OR xxac_acctto = "")
                              AND (glt_sub >= xxac_subfrom OR xxac_subfrom = "")
                              AND (glt_sub <= xxac_subto OR xxac_subto = "") NO-LOCK NO-ERROR.
        IF AVAIL xxac_det THEN DO:
            v_xxac_drcr = xxac_drcr .
            v_xxac_name = xxac_name .
            v_xxac_name_desc = xxac_name_desc .
            ...
            ASSIGN
                ...
                tt_name_desc = v_xxac_name_desc
                tt_ref  = glt_ref
                tt_line = string(int(glt_line))
                tt_ref_desc = glt_desc
                tt_amt = glt_amt
                tt_eff_dt = glt_effdate 
                tt_gltr__qadc01 = glt__qadc01
                .
            v_xxac_drcr = "" .
            v_xxac_name = "" .
            v_xxac_name_desc = "" .
    ...
    FOR EACH glt_det NO-LOCK WHERE glt_sub >= sub AND glt_sub <= sub1
                                 AND glt_cc >= ctr AND glt_cc <= ctr1 
                                 AND glt_entity >= entity AND glt_entity <= entity1 
                                 AND glt_effdate >= date(1,1,year(effdate)) AND glt_effdate <= effdate1
                                 AND glt_tr_type = "JL" :
        ...
        IF AVAIL xxac_det THEN DO:
            ...
            ASSIGN
                ...
                tt1_name_desc = v_xxac_name_desc
                tt1_ref  = glt_ref
                tt1_line = string(int(glt_line))
                tt1_ref_desc = glt_desc
                tt1_amt = glt_amt
                tt1_eff_dt = glt_effdate 
                tt1_gltr__qadc01 = glt__qadc01
                .
            v_xxac_drcr = "" .
            v_xxac_name = "" .
            v_xxac_name_desc = "" .
...
END.
IF inclpost = YES THEN DO:
    FOR EACH glt_det NO-LOCK WHERE glt_effdate < effdate AND glt_tr_type = "JL" :
        FIND FIRST xxac_det WHERE xxac_code = acc
                              AND (glt_acc >= xxac_acctfrom OR xxac_acctfrom = "" )
                              AND (glt_acc <= xxac_acctto OR xxac_acctto = "")
                              AND (glt_sub >= xxac_subfrom OR xxac_subfrom = "")
...
/* 输出到BI */
PUT UNFORMATTED "#def REPORTPATH=$/财务/BI报表/a6glrp57" SKIP.
PUT UNFORMATTED "#def :end" SKIP.

v_disptitle = v_code_desc + "(多栏式明细帐)" .
...
/* 行标题1 */
PUT UNFORMATTED v_disptitle ";" v_dispdate ";" "日期" ";" "总帐参考号" ";" "摘要" ";" .
maxii = 1 .
...
END.
PUT UNFORMATTED "余额" ";" .
DO i = 1 TO 65 - maxii - 1 :
    PUT UNFORMATTED ";" .
END.
PUT UNFORMATTED SKIP.

PUT UNFORMATTED v_disptitle ";" v_dispdate ";" ";" ";" ";" .
FOR EACH tt4 NO-LOCK BREAK BY tt4_drcr DESCENDING BY tt4_name :
    IF FIRST-OF(tt4_drcr) THEN DO:
       PUT UNFORMATTED "合计" ";" .
    END.
...
PUT UNFORMATTED v_disptitle ";" v_dispdate ";" ";" ";" "期  初  余  额 " ";"  .
FOR EACH tt4 NO-LOCK BREAK BY tt4_drcr DESCENDING BY tt4_name :
    IF FIRST-OF(tt4_drcr) THEN DO:
       PUT UNFORMATTED ";" .
    END.
...
ELSE IF v_amt > 0 THEN DO:
   PUT UNFORMATTED  "借" ";" string(abs(v_amt)) .
END.
ELSE DO:
   PUT UNFORMATTED  "贷" ";" string(abs(v_amt)) .
END.
DO i = 1 TO 65 - maxii - 1 :
    PUT UNFORMATTED ";" .
END.
...
FOR EACH ttc NO-LOCK BREAK BY YEAR(ttc.tt_eff_dt) BY MONTH(ttc.tt_eff_dt) 
    ...
    IF LAST-OF(ttc.tt_line) THEN DO:
       PUT UNFORMATTED v_disptitle ";" v_dispdate ";" string(year(ttc.tt_eff_dt)) + "." + STRING(MONTH(ttc.tt_eff_dt),"99") + "." + STRING(DAY(ttc.tt_eff_dt),"99") ";" ttc.tt_ref ";" ttc.tt_ref_desc ";"  .
       v_tot_amt1 = 0.         
       v_tot_amt1a = 0.
       FOR EACH tt3 :
           DELETE tt3 .
       ...
       IF v_tot_amt2 > 0 THEN PUT UNFORMATTED "借 " ";"      STRING(abs(v_tot_amt2)) .
       ELSE IF v_tot_amt2 = 0 THEN PUT UNFORMATTED "平 " ";" .
                              ELSE PUT UNFORMATTED "贷 " ";" STRING(abs(v_tot_amt2)) .
    ...
    IF LAST-OF(MONTH(ttc.tt_eff_dt)) THEN DO:
            /* 列和总合计 */
            PUT UNFORMATTED v_disptitle ";" v_dispdate ";" ";" ";" "本 月 合 计" ";" .
            v_tot_amt1 = 0 .
            v_tot_amt1a = 0 .
            FOR EACH tt3: 
                DELETE tt3 .
    ...
    /************************************************ 本年累计 ********************************************/
    IF LAST-OF(MONTH(ttc.tt_eff_dt)) THEN DO:
            PUT UNFORMATTED v_disptitle ";" v_dispdate ";" ";" ";" "本 年 累 计" ";".
            v_tot_amt1 = 0 .
            v_tot_amt1a = 0 .
            FOR EACH tt3: 
                DELETE tt3 .
