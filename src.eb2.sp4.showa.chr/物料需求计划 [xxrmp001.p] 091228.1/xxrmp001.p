...
/* ********** End Translatable Strings Definitions ********* */
    ...
    /* ss - 091223.1 -b */
    DEFINE VAR due2 LIKE po_due_date .
    DEFINE VAR due3 LIKE po_due_date .
    DEFINE VAR due4 LIKE po_due_date .
    DEFINE VAR due5 LIKE po_due_date .
     /* ss - 091223.1 -e */
    ...
    DEFINE TEMP-TABLE xxld_det 
        ...
        .
    DEFINE BUFFER xxlddet FOR xxld_det .
    ...
    DEFINE TEMP-TABLE t4 /* 行列转换*/
        ...
        FIELD t4_sfty_stk LIKE pt_sfty_stk 
...
/*K103*/ end.
          ...
          {mfselbpr.i "printer" 132}
            ...
            /* 处理逻辑 -b */
              ...
              due4 = ? .  /* 未来开始*/
              due5 = ? .  /* 未来结束*/
              IF (TODAY >= due AND TODAY <= due1) THEN DO:
                  due2 = due .
                  due3 = TODAY .
                  due4 = TODAY + 1 .
                  due5 = due1 .
              END.
              ELSE IF( due > TODAY) THEN DO:
                  due4 = due .
                  due5 = due1 .
                  
              END.
              ELSE IF ( due1 <= TODAY) THEN DO:
                  due2 = due .
                  due3 = due1 .
              END.
              ...
              FOR EACH pt_mstr  FIELD(pt_part pt_buyer pt_vend) NO-LOCK WHERE (pt_part >= part AND pt_part <= part1 ) AND (pt_buyer >= buyer AND pt_buyer <= buyer1)
               ...
               EACH  IN_mstr  FIELD( IN_site IN_part IN_qty_oh )  NO-LOCK WHERE (IN_site >= site AND IN_site <= site1) AND IN_part = pt_part :
                 ...
                 /* ss - 091223.1 -b */
                   IF due4 = ? OR due2 <> ? THEN DO:
                  
                 /* ss - 091223.1 -e */
                  v_start = due3 .
                  DO WHILE v_start >= due2 :
                  ...
                  IF due4 <> ? THEN DO:  /* due4 <> ? */
                            v_start = due5 .
                  DO WHILE v_start >= due4 :
                        FIND xxld_det WHERE xxld_site = IN_site AND xxld_part = IN_part AND xxld_due_date = v_start NO-LOCK NO-ERROR .
                     IF NOT AVAILABLE xxld_det THEN DO:
             ...
             DO WHILE   v_start >= due2 :
                 ...
                 /* ss - 091227.1 -b
                      FOR EACH wo_mstr    FIELD (wo_site wo_due_date )  NO-LOCK WHERE  wo_due_date = v_start AND   ( wo_site >= site AND wo_site <= site1)  AND wo_status <> "p"
                         ,  EACH wod_det  FIELD (wod_nbr wod_part wod_qty_req wod_qty_iss)   NO-LOCK WHERE wod_lot = wo_lot  AND (wod_part >= part AND wod_part <= part1) 
                     ...
                     END.
                      ...
                      ss - 091227.1 -e */
                         ...
                         FOR EACH xxld_det WHERE xxld_due_date = v_start :
                             /* ss - 091228.1 -b */
                             /* 计算实际发料*/
                               FOR EACH tr_hist  fields( tr_site tr_part tr_qty_loc tr_effdate)  WHERE tr_site = xxld_site AND  tr_effdate = xxld_due_date AND   tr_part = xxld_part  AND tr_type = "iss-wo" NO-LOCK:
                                    xxld_qty_plan = xxld_qty_plan + ( - tr_qty_loc) .
                               END.
                               /* ss - 091228.1 -e */
                               /* 计算so 需求*/
                               FOR EACH sod_det FIELDS(sod_site sod_part sod_due_date sod_qty_ord) WHERE  sod_site = xxld_site AND sod_part = xxld_part AND sod_due_date = xxld_due_date NO-LOCK :
                                   xxld_qty_plan = xxld_qty_plan + sod_qty_ord .
                               END.
            ...
            /* ss - 091223.1 -b */
             ...
             IF ( due4 <> ?) THEN DO:
                 ...
                 v_start = due4 .
                 DO WHILE v_start <= due5:
                     
                     FOR EACH  xxld_det WHERE xxld_due_date = v_start  :

                      
                             FIND FIRST IN_mstr WHERE IN_site = xxld_site AND IN_part = xxld_part NO-LOCK NO-ERROR .
                             IF AVAILABLE IN_mstr THEN
                                 v_qty_oh = IN_qty_oh .
                             ELSE 
                                 v_qty_oh = 0 .
                      
                         FOR EACH mrp_det WHERE mrp_due_date = xxld_due_date AND mrp_part = xxld_part AND mrp_site = xxld_site NO-LOCK :
                              /* 计划需求*/ 
                              IF mrp_dataset = "wod_det" OR mrp_dataset = "sod_det" THEN DO:
                                  xxld_qty_plan = xxld_qty_plan + mrp_qty .
                              END.
                              ELSE IF (mrp_dataset = "pod_det" ) THEN  DO:
                                  xxld_qty_ord = xxld_qty_ord + mrp_qty .
                              END.  /* 订单计划*/
                             
                          END. /* mrp_det */

                          /* 计算未来理论库存*/
                          IF xxld_due_date = due4  THEN DO:
                              xxld_qty_oh = xxld_qty_oh + ( v_qty_oh + xxld_qty_ord - xxld_qty_plan) .
                          END.
                          ELSE DO:
                              FIND FIRST xxlddet WHERE xxlddet.xxld_part = xxld_det.xxld_part AND xxlddet.xxld_due_date = xxld_det.xxld_due_date - 1 NO-ERROR .
                              IF AVAILAB xxlddet THEN DO:
                                  xxld_det.xxld_qty_oh = xxld_det.xxld_qty_oh + ( xxlddet.xxld_qty_oh -  xxld_det.xxld_qty_plan  + xxld_det.xxld_qty_ord  ) . 
                              END.
                          END.
                            /* 计算未来理论库存*/
...
PUT UNFORMATTED "#def :end" SKIP.
	    ...
	    */
     FOR EACH xxld_det  NO-LOCK USE-INDEX xxld_part  WHERE  BREAK  BY xxld_part  BY xxld_due_date :
                      ...
                      i = 1 .
                       ...
                       END.
                         PUT SKIP .
                           ...
                           /*  输出差异 */
                     PUT  t4_part  ";"  t4_desc ";"  "差异" + "(" +  STRING(t4_sfty_stk) + ")" FORMAT "x(20)" .
                      i = 1 .
                       DO WHILE i <= t4_number :
                           PUT ";"  t4_comp2[i]  .
                             i = i + 1 .
