...
/*J053*/ define shared frame c.
         ...
         define variable mfgr like vp_mfgr format "x(72)".
         ...
         define variable part-lbl as character format "x(20)".
...
/*N09M* -- END INITIAL VALUES --------------------- */
         {po03b01.i}
/*J053*/ {po03d01.i} /* DEFINE FOR FRAME C */
				 assign manuf-lbl = trim(getTermLabel("MANUFACTURER_CODE",20)) + ": ".
				 				part-lbl = trim(getTermLabel("MANUFACTURER_ITEM",20)) + ": ".
...
/*F0HF*/       else desc2 = pt_desc2.
            ...
            if pod_vpart <> "" then do:
               ...
               if available vp_mstr then do:
                  mfgr = vp_mfgr.
/*120910.1*/      find first ad_mstr no-lock where ad_addr = vp_mfgr
/*120910.1*/            and ad_type = "supplier" no-error.
/*120910.1*/      if avail ad_mstr then do:
/*120910.1*/        assign mfgr = vp_mfgr + " "
/*120910.1*/                    + getTermLabel("MANUFACTURER_NAME",12) + ":" + ad_name.
/*120910.1*/      end.
...
/*N09M*/          typedesc = getTermLabel("SUBCONTRACT",14).
            ...
            if mfgr <> "" or mfgr_part <> "" then
               put unformat manuf-lbl at 5 mfgr skip.
               put unformat part-lbl at 5 mfgr_part skip.
