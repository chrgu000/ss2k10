...
/* $Revision: 1.6.1.12 $       BY: Micho Yang         DATE: 05/26/08  ECO: *SS - 20080526.1* */
/*-Revision end---------------------------------------------------------------*/
...
define new shared  temp-table  tmp_det
   ...
   then
      ...
      for first pt_mstr
         fields( pt_domain pt_bom_code pt_buyer   pt_desc1     pt_desc2
                pt_part  pt_vend   pt_pm_code pt_prod_line pt_um pt_phantom )
      ...
      repeat with frame b:
         ...
         then do:
            ...
            for first ptp_det
               fields( ptp_domain ptp_vend ptp_bom_code ptp_buyer ptp_part ptp_pm_code
               ptp_site ptp_phantom)
            ...
            then
               for each pk_det
                  ...
                  then do:
                     ...
                     for first ptp_det
                        fields( ptp_domain ptp_vend ptp_bom_code ptp_buyer ptp_part
                               ptp_pm_code  ptp_site ptp_site ptp_phantom )
                         where ptp_det.ptp_domain = global_domain and ptp_det.ptp_part
                          = ptmstr.pt_part
                          and ptp_site  = site
                          and ptp_buyer >= buyer
                          and ptp_buyer <= buyer1
                     ...
                     */
                     ...
                     if unittot <> 0 AND NOT (available ptp_det AND ptp_phantom    OR
                            not available ptp_det AND  ptmstr.pt_phantom )
                         then do:
                        display 
                             v_vend 
                             ptmstr.pt_part  @ pk_part 
