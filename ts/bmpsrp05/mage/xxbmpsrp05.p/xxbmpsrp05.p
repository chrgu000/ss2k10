...
/*ts*/ define buffer ptmstr for pt_mstr.
/*ts*/ define variable desc1       as   character   format "x(27)" no-undo.
/*ts*/ define variable desc2       as   character   format "x(27)" no-undo.
/*ts*/ define variable rmks        as   character   format "x(8)" initial "*****"  no-undo.
/*ts*/ define variable deltot like sct_cst_tot format "->>>>>>>>9.99<<<<<<<"  no-undo.

/*ts*/  define new shared  temp-table  tmp_det
...
define buffer ptmstr for pt_mstr.
      ...
      for first pt_mstr
         fields( pt_domain pt_bom_code pt_buyer   pt_desc1     pt_desc2
                pt_part     pt_pm_code pt_prod_line pt_um)
...
/*ts add*/        if unittot <> 0  then do:
                     display ptmstr.pt_part  @ pk_part 
	                         ptmstr.pt_desc1 @ desc1
                             ptmstr.pt_desc2 @ desc2
			                 ptmstr.pt_um @ um
                             ptp_pm_code when (available ptp_det) @ pmcode
                             ptmstr.pt_pm_code when (not available ptp_det) @ pmcode
                             sum_usage
...
/*ts add*/        end.  /*if mtl <> 0 or sub <> 0 then do: */
