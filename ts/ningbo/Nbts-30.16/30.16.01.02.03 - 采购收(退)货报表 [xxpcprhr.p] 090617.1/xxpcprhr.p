...
repeat:
   ...
   FOR EACH en_mstr NO-LOCK
      ...
      AND xxpcprh_year = yr
      AND xxpcprh_per = per
      AND xxpcprh_type >= TYPE
      AND xxpcprh_type <= TYPE1
      ...
      BY xxpcprh_type
      :
      ACCUMULATE xxpcprh_qty (TOTAL BY xxpcprh_site BY xxpcprh_year BY xxpcprh_per BY xxpcprh_part BY xxpcprh_vend BY xxpcprh_type).
      ACCUMULATE xxpcprh_mtl_tot (TOTAL BY xxpcprh_site BY xxpcprh_year BY xxpcprh_per BY xxpcprh_part BY xxpcprh_vend BY xxpcprh_type).
      IF LAST-OF(xxpcprh_type) THEN DO:
         EXPORT DELIMITER ";"
            xxpcprh_year
            xxpcprh_per
            si_site
            ...
            ad_name
            xxpcprh_type
            (ACCUMULATE TOTAL BY xxpcprh_type xxpcprh_qty)
            (ACCUMULATE TOTAL BY xxpcprh_type xxpcprh_mtl_tot)
            ((ACCUMULATE TOTAL BY xxpcprh_type xxpcprh_mtl_tot) / (ACCUMULATE TOTAL BY xxpcprh_type xxpcprh_qty))
            .
      END. /* IF LAST-OF(xxpcprh_vend) THEN DO: */
   END.
