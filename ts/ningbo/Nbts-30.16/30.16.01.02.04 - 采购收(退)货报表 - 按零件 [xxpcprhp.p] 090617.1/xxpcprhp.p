...
repeat:
   ...
   FOR EACH en_mstr NO-LOCK
      ...
      BY xxpcprh_part
      :
      ACCUMULATE xxpcprh_qty (TOTAL BY xxpcprh_site BY xxpcprh_year BY xxpcprh_per BY xxpcprh_part).
      ACCUMULATE xxpcprh_mtl_tot (TOTAL BY xxpcprh_site BY xxpcprh_year BY xxpcprh_per BY xxpcprh_part).
      IF LAST-OF(xxpcprh_part) THEN DO:
         EXPORT DELIMITER ";"
            xxpcprh_year
            xxpcprh_per
            si_site
            ...
            pt_part
            (pt_desc1 + " " + pt_desc2)
            (ACCUMULATE TOTAL BY xxpcprh_part xxpcprh_qty)
            (ACCUMULATE TOTAL BY xxpcprh_part xxpcprh_mtl_tot)
            ((ACCUMULATE TOTAL BY xxpcprh_part xxpcprh_mtl_tot) / (ACCUMULATE TOTAL BY xxpcprh_part xxpcprh_qty))
            .
      END. /* IF LAST-OF(xxpcprh_part) THEN DO: */
   END.
