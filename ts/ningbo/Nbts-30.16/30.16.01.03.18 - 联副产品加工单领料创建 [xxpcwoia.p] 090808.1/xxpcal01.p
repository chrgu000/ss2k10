/* wocsal01.p - -- JOINT PRODUCTS AVG COST: ALLOCATION METHOD 01              */
...
/* Revision: 1.4  BY: Jean Miller DATE: 03/26/01 ECO: *P008* */
...
/************************************************************************/
/* Allocation Method based on Physical Measure                          */
/*                                                                      */
...
/*                                                                      */
/* Need to write a custom program for a special allocation method?      */
/* You may find the utility utjpavg.p useful for de-bugging.            */
/*                                                                      */
/************************************************************************/
...
find wo_mstr where recid(wo_mstr) = base_recid
no-lock no-error.
...
no-lock:
   ...
   if available pt_mstr then do:
      ...
      /* SS - 090713.1 - B
      alloc_numer = max(0,(wo_qty_comp + wo_qty_rjct) * conv).
      SS - 090713.1 - E */
      /* SS - 090713.1 - B */
      alloc_numer = max(0,(wo_qty_ord) * conv).
      /* SS - 090713.1 - E */
      if alloc_numer = ? then
         alloc_numer = 0.
...
if alloctot <= 0 or alloctot >= 1 then
   return.
