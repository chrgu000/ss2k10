...
repeat:
   ...
   FOR EACH en_mstr NO-LOCK
      ...
      AND xxpcwor_year = yr
      AND xxpcwor_per = per
      AND ((xxpcwor_type = "RCT-WO" AND execname = "xxpcworp.p") OR (xxpcwor_type = "RJCT-WO" AND execname = "xxpcworq.p"))
      ,EACH pt_mstr NO-LOCK
      WHERE pt_mstr.pt_domain = GLOBAL_domain
      AND pt_mstr.pt_part = xxpcwor_part
      ...
      IF LAST-OF(xxpcwor_part) THEN DO:
         EXPORT DELIMITER ";"
            xxpcwor_year
            xxpcwor_per
            si_site
            ...
            (ACCUMULATE TOTAL BY xxpcwor_part xxpcwor_qty)
            .
      END.
   END.
