...
DEFINE VARIABLE qty_xxpctr LIKE xxpctr_qty.
...
repeat:
   ...
   DO TRANSACTION ON STOP UNDO:
      ...
      FOR EACH si_mstr NO-LOCK
         ...
         BY tr_type
         :
         FIND FIRST CODE_mstr WHERE CODE_domain = GLOBAL_domain AND CODE_fldname = "SoftspeedPC_Porite_loc" AND CODE_value = tr_loc NO-LOCK NO-ERROR.
         IF NOT AVAILABLE CODE_mstr THEN DO:
            qty_xxpctr = qty_xxpctr + tr_qty_loc.
         END.

         IF LAST-OF(tr_type) THEN DO:
            IF qty_xxpctr = 0 THEN DO:
               NEXT.
            END.
