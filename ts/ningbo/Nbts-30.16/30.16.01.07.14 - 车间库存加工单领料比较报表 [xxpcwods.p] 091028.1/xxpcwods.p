...
DEFINE TEMP-TABLE tt1
   ...
   FIELD tt1_tr LIKE xxpctr_qty
   FIELD tt1_wod LIKE xxpcwod_qty_tot
   FIELD tt1_woi LIKE xxpcwoi_qty
   INDEX tt1_site_part tt1_site tt1_part
   .
...
repeat:
   ...
   FOR EACH en_mstr NO-LOCK
      ...
      AND pt_mstr.pt_part = xxpcwod_comp
      ...
      BY pt_part
      :
      ACCUMULATE xxpcwod_qty_tot (TOTAL
                                  BY si_site
                                  BY pt_part
                                  ).
      ...
      IF LAST-OF(pt_part) THEN DO:
         FIND FIRST tt1
            WHERE tt1_site = si_site
            AND tt1_part = pt_part
         ...
         ASSIGN
            tt1_wod = (ACCUMULATE TOTAL BY pt_part xxpcwod_qty_tot)
            .
      END.
   END.
   ...
   FOR EACH en_mstr NO-LOCK
      ...
      AND xxpctr_year = yr
      AND xxpctr_per = per
      AND (xxpctr_type = "ISS-UNP" OR xxpctr_type = "ISS-WO")
      AND xxpctr_charfld[1] = "FS"
      ,EACH pt_mstr NO-LOCK
      WHERE pt_mstr.pt_domain = GLOBAL_domain
      AND pt_mstr.pt_part = xxpctr_part
      AND pt_mstr.pt_part >= part
      AND pt_mstr.pt_part <= part1
      BREAK
      BY si_site
      ...
      IF LAST-OF(pt_part) THEN DO:
         ...
         ASSIGN
            tt1_tr = - (ACCUMULATE TOTAL BY pt_part xxpctr_qty)
            .
      END.
   END.
   ...
   FOR EACH en_mstr NO-LOCK
      ...
      ,EACH pt_mstr NO-LOCK
      WHERE pt_mstr.pt_domain = GLOBAL_domain
      AND pt_mstr.pt_part = xxpcwoi_comp
      AND pt_mstr.pt_part >= part
      AND pt_mstr.pt_part <= part1
      BREAK
      BY si_site
      ...
      IF LAST-OF(pt_part) THEN DO:
         ...
         ASSIGN
            tt1_woi = - (ACCUMULATE TOTAL BY pt_part xxpcwoi_qty)
            .
      END.
   END.
