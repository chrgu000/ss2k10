...
包含以下字段:
  - SoftspeedPC_glr_code_ce: 自定义报表.成本要素
...
define variable SoftspeedPC_glr_code_ce like mfc_char FORMAT "x(8)".

/* DISPLAY SELECTION FORM */
{&APPMFRM-I-TAG1}
form
   SoftspeedPC_glr_code_ce          colon 38
...
repeat with frame appm-a:
   ...
   /* ADD MFC_CTRL FIELD SoftspeedPC_glr_code_ce */
   for first mfc_ctrl where mfc_domain = global_domain and mfc_field = "SoftspeedPC_glr_code_ce"
   exclusive-lock: end.
   ...
   if not available mfc_ctrl then do:
      ...
      assign
         ...
         mfc_field   = "SoftspeedPC_glr_code_ce"
         mfc_type    = "C"
         mfc_label   = c-label
         mfc_module  = execname
         mfc_seq     = 10
   ...
   assign
      SoftspeedPC_glr_code_ce = mfc_char.

   display
      SoftspeedPC_glr_code_ce
   with frame appm-a.
   ...
   do on error undo, retry:
      ...
      if SoftspeedPC_glr_code_ce = "" then do:
         /* Blank not allowed */
         {pxmsg.i &MSGNUM=40 &ERRORLEVEL=3}
         next-prompt SoftspeedPC_glr_code_ce.
         undo seta, retry.
      end.
   end.

   find first mfc_ctrl where mfc_domain = global_domain and mfc_field = "SoftspeedPC_glr_code_ce" no-error.
   if available mfc_ctrl then mfc_char = SoftspeedPC_glr_code_ce.
end.
