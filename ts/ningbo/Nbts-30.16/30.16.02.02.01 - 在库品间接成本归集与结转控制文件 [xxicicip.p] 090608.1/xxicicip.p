...
包含以下字段:
  - SoftspeedIC_glr_code_in: 自定义报表.在库品间接成本
...
define variable SoftspeedIC_glr_code_in like mfc_char FORMAT "x(8)".

/* DISPLAY SELECTION FORM */
{&APPMFRM-I-TAG1}
form
   SoftspeedIC_glr_code_in          colon 38
...
repeat with frame appm-a:
   ...
   /* ADD MFC_CTRL FIELD SoftspeedIC_glr_code_in */
   for first mfc_ctrl where mfc_domain = global_domain and mfc_field = "SoftspeedIC_glr_code_in"
   exclusive-lock: end.
   ...
   if not available mfc_ctrl then do:
      ...
      assign
         ...
         mfc_field   = "SoftspeedIC_glr_code_in"
         mfc_type    = "C"
         mfc_label   = c-label
         mfc_module  = execname
         mfc_seq     = 10
   ...
   assign
      SoftspeedIC_glr_code_in = mfc_char.

   display
      SoftspeedIC_glr_code_in
   with frame appm-a.
   ...
   do on error undo, retry:
      ...
      if SoftspeedIC_glr_code_in = "" then do:
         /* Blank not allowed */
         {pxmsg.i &MSGNUM=40 &ERRORLEVEL=3}
         next-prompt SoftspeedIC_glr_code_in.
         undo seta, retry.
      end.
   end.

   find first mfc_ctrl where mfc_domain = global_domain and mfc_field = "SoftspeedIC_glr_code_in" no-error.
   if available mfc_ctrl then mfc_char = SoftspeedIC_glr_code_in.
end.
