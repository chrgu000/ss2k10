   ...
   DO TRANSACTION ON STOP UNDO:
      ...
      FOR EACH si_mstr NO-LOCK
         ...
         FOR EACH glrd_det NO-LOCK
            ...
            AND glrd_fpos = 0
            AND ((glrd_acct <= xxici_ac OR glrd_acct = "") AND (glrd_acct1 >= xxici_ac OR glrd_acct1 = ""))
            AND ((glrd_sub <= xxici_sb OR glrd_sub = "") AND (glrd_sub1 >= xxici_sb OR glrd_sub1 = ""))
            AND ((glrd_cc <= xxici_cc OR glrd_cc = "") AND (glrd_cc1 >= xxici_cc OR glrd_cc1 = ""))
            USE-INDEX glrd_code
            BY glrd_sub DESC
            BY glrd_acct DESC
            BY glrd_cc DESC
         ...
         arg1 = STRING(xxici_fpos).
         arg2 = (xxici_ac + "-" + xxici_sb + "-" + xxici_cc).
