...
注意,本程序因程序名而异:
  - xxpcfmce.p: 成本要素格式位置维护
  - xxpcfmin.p: 在库品间接成本格式位置维护
  - xxpcfmwo.p: 在制品间接成本格式位置维护
  - xxpcfmpt.p: 零件分配范围格式位置维护
  - xxpcfmpl.p: 产品线分配范围格式位置维护
  - xxpcfmcc.p: 适用于标准加工单的在制品成本中心分配范围格式位置维护
  - xxpcfmln.p: 适用于累计加工单的生产线分配范围格式位置维护

增加了以下验证:
  - 定制代码[glr_code]: 必须在控制文件中已经定义
  
以下程序的格式位置必须与<间接费用格式位置维护 [xxicfmie.p]>保持一致:
  - xxpcfmpt.p: 零件分配范围格式位置维护
  - xxpcfmpl.p: 产品线分配范围格式位置维护
  - xxpcfmcc.p: 适用于标准加工单的在制品成本中心分配范围格式位置维护
  - xxpcfmln.p: 适用于累计加工单的生产线分配范围格式位置维护
...
IF execname = "xxpcfmpt.p" THEN DO:
   FIELD_mfc = "SoftspeedIC_to_pt".
END.
ELSE IF execname = "xxpcfmpl.p" THEN DO:
   FIELD_mfc = "SoftspeedIC_to_pl".
END.
ELSE IF execname = "xxpcfmcc.p" THEN DO:
   FIELD_mfc = "SoftspeedIC_to_cc".
END.
ELSE IF execname = "xxpcfmln.p" THEN DO:
   FIELD_mfc = "SoftspeedIC_to_ln".
END.
ELSE IF execname = "xxpcfmin.p" THEN DO:
   FIELD_mfc = "SoftspeedIC_glr_code_in".
END.
ELSE IF execname = "xxpcfmwo.p" THEN DO:
   FIELD_mfc = "SoftspeedIC_glr_code_wo".
END.
ELSE IF execname = "xxpcfmce.p" THEN DO:
...
no-error.
   ...
   do on error undo, retry:
      ...
      repeat with frame b:
         ...
         /* SS - 090616.1 - B */
         IF execname = "xxpcfmce.p" THEN DO:
            FIND FIRST CODE_mstr
               WHERE CODE_fldname = "SoftspeedPC_element"
               AND CODE_value = STRING(INPUT glrd_fpos)
               NO-LOCK NO-ERROR.
         ...
         IF CAN-DO("xxpcfmpt.p,xxpcfmpl.p,xxpcfmcc.p,xxpcfmln.p",execname) THEN DO:
            {gprun.i ""xxpcfmcev.p"" "(
               INPUT INPUT glrd_fpos
               )"}
