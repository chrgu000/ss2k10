...
FOR EACH tt1
   ...
   /* 验证 */
   REPEAT:
      IF NOT CAN-FIND(FIRST en_mstr WHERE en_domain = GLOBAL_domain AND en_entity = tt1_c1[1]) THEN DO:
         /* Invalid entity */
      ...
      FOR EACH si_mstr NO-LOCK
         ...
         AND si_entity = tt1_c1[1]
         ,EACH xxpcc_det NO-LOCK
         WHERE xxpcc_domain = GLOBAL_domain
         AND xxpcc_site = si_site
         AND xxpcc_year = INTEGER(tt1_c1[2])
         AND xxpcc_per = INTEGER(tt1_c1[3])
      ...
      IF tt1_c1[5] <> "" THEN DO:
         IF NOT CAN-FIND(FIRST wo_mstr WHERE wo_domain = GLOBAL_domain AND wo_lot = tt1_c1[5]) THEN DO:
            ...
            {pxmsg.i &MSGNUM=503 &ERRORLEVEL=3 &MSGARG1=tt1_c1[5]}
            LEAVE.
         END.
      END.

      IF NOT CAN-FIND(FIRST code_mstr WHERE code_domain = GLOBAL_domain AND code_fldname = "SoftspeedIC_AR" AND code_value = tt1_c1[6]) THEN DO:
         /* Rate Code is required */
         {pxmsg.i &MSGNUM=5333 &ERRORLEVEL=3}
         LEAVE.
      END.
   ...
   /* 写入 */
   FIND FIRST xxicar_det 
      WHERE xxicar_domain = GLOBAL_domain
      AND xxicar_entity = tt1_c1[1]
      AND xxicar_year = INTEGER(tt1_c1[2])
      AND xxicar_per = INTEGER(tt1_c1[3])
      AND xxicar_part = tt1_c1[4]
      AND xxicar_lot = tt1_c1[5]
      AND xxicar_ar = tt1_c1[6]
      NO-ERROR.
   IF NOT AVAILABLE xxicar_det THEN DO:
      CREATE xxicar_det.
      ASSIGN
         xxicar_domain = GLOBAL_domain
         xxicar_entity = tt1_c1[1]
         xxicar_year = INTEGER(tt1_c1[2])
         xxicar_per = INTEGER(tt1_c1[3])
         xxicar_part = tt1_c1[4]
         xxicar_lot = tt1_c1[5]
         xxicar_ar = tt1_c1[6]
         .
   END.
   IF tt1_c1[7] <> "-" THEN DO:
      ASSIGN
         xxicar_qty = DECIMAL(tt1_c1[7])
         .
   END.
   IF tt1_c1[8] <> "-" THEN DO:
      ASSIGN
         xxicar_bp = DECIMAL(tt1_c1[8])
         .
   END.
   IF tt1_c1[9] <> "-" THEN DO:
      ASSIGN
         xxicar_mp1 = DECIMAL(tt1_c1[9])
         .
   END.
   IF tt1_c1[10] <> "-" THEN DO:
      ASSIGN
         xxicar_mp2 = DECIMAL(tt1_c1[10])
         .
   END.
   IF tt1_c1[11] <> "-" THEN DO:
      ASSIGN
         xxicar_mp3 = DECIMAL(tt1_c1[11])
         .
   END.
   IF tt1_c1[12] <> "-" THEN DO:
      ASSIGN
         xxicar_dp1 = DECIMAL(tt1_c1[12])
         .
   END.
   IF tt1_c1[13] <> "-" THEN DO:
      ASSIGN
         xxicar_dp2 = DECIMAL(tt1_c1[13])
         .
   END.
   IF tt1_c1[14] <> "-" THEN DO:
      ASSIGN
         xxicar_dp3 = DECIMAL(tt1_c1[14])
         .
   END.
