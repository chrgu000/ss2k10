...
FOR EACH tt1
   ...
   FIND FIRST xxpcinb_mstr 
      ...
      AND xxpcinb_site = tt1_c1[1]
      AND xxpcinb_year = INTEGER(tt1_c1[2])
      AND xxpcinb_per = INTEGER(tt1_c1[3])
      AND xxpcinb_part = tt1_c1[4]
      NO-ERROR.
   IF NOT AVAILABLE xxpcinb_mstr THEN DO:
      CREATE xxpcinb_mstr.
      ASSIGN
         xxpcinb_domain = GLOBAL_domain
         xxpcinb_site = tt1_c1[1]
         xxpcinb_year = INTEGER(tt1_c1[2])
         xxpcinb_per = INTEGER(tt1_c1[3])
         xxpcinb_part = tt1_c1[4]
         .
   END.
   IF tt1_c1[5] <> "-" THEN DO:
      ASSIGN
         xxpcinb_qty = DECIMAL(tt1_c1[5])
         .
   END.
