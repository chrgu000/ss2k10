/* SS - 20090114 by Jeff modify */
/* SS - 091204.1 BY Ken Chen */
/************************
香港库位可以在通用代码里控制:
field = hk_loc
*****************************/
/** 130827.1  修正bug库存状态为Y-Y-N时负数库存被发货了             130827.1  */
...
/*ss-min001 add*************************************************************/
      ...
      no-lock break by tr1_nbr by tr1_trnbr  :
        ...
        with width 80:
                  ...
                  IF CAN-FIND(FIRST ld_det WHERE ld_domain = GLOBAL_domain
                              AND ld_site = sr_site
                              AND ld_loc = sr_loc
                              AND ld_part = tr1_part
                              AND ld_lot = sr_lotser
                              AND ld_ref = sr_ref
                              AND ld_qty_oh < sr_qty ) THEN DO:
                      {pxmsg.i &MSGNUM=8435 &ERRORLEVEL=3}
                      UNDO,RETRY.
                  END.
                  /* SS - 091204.1 E */
         ...
         then do:
            ...
            then do:
               ...
               IF CAN-FIND(FIRST ld_det WHERE ld_domain = GLOBAL_domain
                           ...
                           AND ld_ref = lotref
                           AND ld_qty_oh < lotserial_qty ) THEN DO:
                   {pxmsg.i &MSGNUM=8435 &ERRORLEVEL=3}
                   UNDO,RETRY.
               END.
               /* SS - 091204.1 E */
