...
repeat on error undo, retry:
   ...
   /* 前缀 */
   FIND FIRST mfc_ctrl WHERE mfc_domain = GLOBAL_domain AND mfc_field = "SoftspeedDI_rqm_nbr" NO-LOCK NO-ERROR.
   ...
   if not available rqm_mstr then do:
      if INPUT nbr_rqm = "" then do :
         FIND FIRST mfc_ctrl
            ...
            AND mfc_field = "SoftspeedDI_rqm_nbr"
   ...
   do on error undo, leave:
      ...
      IF auto_select = YES THEN DO:
         ...
         /* 同步已经选择的记录 */
         {gprun.i ""xxssdim2sdb.p"" "(INPUT nbr_rqm)"}
