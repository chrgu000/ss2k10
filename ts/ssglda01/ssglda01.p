...
/* $Revision: 1.33 $ BY: Micho Yang         DATE: 05/26/08  ECO: *SS - 080526.1* */
/*-Revision end---------------------------------------------------------------*/
...
/*
修改如下:
  1) 在"显示未选择的事务"下面增加一个可编辑的选择条件字段"包括未过账".注意:输出不变
  2) 将"说明"的字节长度由21增加到42
  3) 报表可以输出到EXCEL,具体格式请参考此程序的SPEC
*/                                                                          
/* SS - 080526.1 - E */
...
*/
define variable cname          like glname FORMAT "x(42)" no-undo.
/* SS - 080526.1 - E */
...
/* SS - 080526.1 - B */
DEFINE VARIABLE include_unpost   as logical init no .
DEFINE VARIABLE begdt1           as date .
DEFINE VARIABLE enddt1           as date .
DEFINE VARIABLE v_dr_amt         as decimal .
DEFINE VARIABLE v_cr_amt         as decimal .
DEFINE VARIABLE v_dr_curr_amt    as decimal .
DEFINE VARIABLE v_cr_curr_amt    as decimal .
DEFINE VARIABLE v_dr_totamt         as decimal .
DEFINE VARIABLE v_cr_totamt         as decimal .
DEFINE VARIABLE v_dr_curr_totamt    as decimal .
DEFINE VARIABLE v_cr_curr_totamt    as decimal .
DEFINE VARIABLE v_ac_desc        as CHARACTER .
DEFINE VARIABLE v_sb_desc        as CHARACTER .
DEFINE VARIABLE v_cc_desc        as CHARACTER .
DEFINE VARIABLE v_pj_desc        as CHARACTER .
/* SS - 080526.1 - E */
...
repeat:
   ...
   FOR EACH gltr_hist no-lock
      ...
      DO:
         ...
         for first cc_mstr fields( cc_domain cc_desc cc_ctr)
             ...
             and  cc_ctr = gltr_ctr no-lock:
         end.          	
         IF available cc_mstr THEN
         DO:
            v_cc_desc = cc_desc.         	
         END.
      END. /* IF gltr_ctr <> "" THEN */

      IF gltr_project <> "" THEN
      DO:
         v_pj_desc = "".
         for first pj_mstr fields( pj_domain pj_desc pj_project)
             where pj_mstr.pj_domain = global_domain
             and  pj_project = gltr_project no-lock:
         end.          	
         IF available pj_mstr THEN
         DO:
            v_pj_desc = pj_desc.         	
         END.      	
      END. /* IF gltr_project <> "" THEN */

      /* 输出报表内容 :
         账户代码	账户说明	分账户代码	分账户说明	成本中心代码	成本中心说明	项目代码	项目说明	会计单位	说明	
         总账参考号 摘要 生效日期	输入日期	用户	批处理	行号	借方金额	贷方金额	借方货币金额	贷方货币金额	货币	
         地址	类型	参考号
      */
      put unformatted 
         acc + " - " + acc1 ";" 
         sub + " - " + sub1 ";"
         ctr + " - " + ctr1 ";"
         string(year(begdt),"9999") + "/" + string(month(begdt),"99") + "/" + string(day(begdt),"99") + " - " + 
         string(year(enddt),"9999") + "/" + string(month(enddt),"99") + "/" + string(day(enddt),"99") ";" 
         gltr_acc ";"
         v_ac_desc ";"
         gltr_sub ";"
         v_sb_desc ";"
      ...
      put unformatted 
         ...
         v_dr_totamt = v_dr_totamt + v_dr_amt .
         v_cr_totamt = v_cr_totamt + v_cr_amt.
         v_dr_curr_totamt = v_dr_curr_totamt + v_dr_curr_amt.
         v_cr_curr_totamt = v_cr_curr_totamt + v_cr_curr_amt .
   ...
   DO:
      FOR EACH glt_det no-lock
         ...
         DO:
            ...
            for first pj_mstr fields( pj_domain pj_desc pj_project)
                where pj_mstr.pj_domain = global_domain
                and  pj_project = glt_project no-lock:
            end.          	
            IF available pj_mstr THEN
            DO:
               v_pj_desc = pj_desc.         	
            END.      	
         END. /* IF glt_project <> "" THEN */

         /* 输出报表内容 :
            账户代码	账户说明	分账户代码	分账户说明	成本中心代码	成本中心说明	项目代码	项目说明	会计单位	说明	
            总账参考号	生效日期	输入日期	用户	批处理	行号	借方金额	贷方金额	借方货币金额	贷方货币金额	货币	
            地址	类型	参考号
         */
         put unformatted 
            acc + " - " + acc1 ";" 
   ...
   END. /* IF include_unpost = yes THEN */
