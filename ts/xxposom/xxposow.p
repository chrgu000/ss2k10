...
form
   ...
   firstplan            colon 35  label  "订单的首周月计划"
   skip     
...
/*修改global_domain*******/
 ...
 update nbr rcpdate rcpdate1
   ...
   with frame a.
    ...
    outfile = "/home/temp/" + string(global_domain)  + string(year(today))  + string(month(today))  + string(day(today))  + string(time) + "w.txt".
    errfile = "/home/temp/" + string(global_domain)  + string(year(today))  + string(month(today))  + string(day(today))  + string(time) + "we.prn".
    process1 = no.
    poderr   = no.
    soderr   = no.
    process1 = no.
  ...
  /*输入头栏**/
   if first-of(schd_line) then do:
     ...
     put   "." skip .
        ...
        put UNFORMATTED  schd_date   space.
	if schd_time <> ""  then put UNFORMATTED   space quote schd_time   quote " -  -  " skip .
	else put UNFORMATTED   " -  -  -  " skip .
	put UNFORMATTED quote   schd_discr_qty quote space  quote  "F" quote space  " - - " skip .
