...
/* $Revision: 1.50.1.1 $ BY: Mage  Chen       DATE: 07/07/06 ECO: *dak*     */
/*-Revision end---------------------------------------------------------------*/
...
/*dak*/ define buffer ptmstr1             for pt_mstr.
/*dak*/ define buffer lnddet              for lnd_det.
...
/*dak*/ define new shared variable line1 like ln_line .
/*dak*/ define new shared variable tot-qty like rps_qty_req.
...
for each  ptmstr1 no-lock where ptmstr1.pt_domain = global_domain and (ptmstr1.pt_run_seq2 <> "" or ptmstr1.pt_run_seq2 = "" and ptmstr1.pt_part = part )
    and ptmstr1.pt_run_seq2 = pt_mstr.pt_run_seq2 
    and ptmstr1.pt_pm_code = "l" ,
    each lnddet no-lock where lnddet.lnd_domain = global_domain 
    and lnddet.lnd_line = line and lnddet.lnd_part = ptmstr1.pt_part  : 
...
/*dak***        {gprun.i ""rebkfli1.p"" */
/*dak***/       {gprun.i ""xxrebkfli1.p""
            "(input cumwo_lot,
              input op,
              output undo_stat)"}
               ...
               then do:
                 ...
                 {gprun.i ""rercvlst.p""
                  ...
                  do transaction:
                     ...
                     then
                       sr_loc = pt_loc. 
/*dak /*dak*/          if pt_part > "1" and pt_part < "1x"  then
	      sr_loc = line1.
	      else  sr_loc = pt_loc . */
      ...
      {gprun.i ""reintchk.p""
         "(input cumwo_lot)"}
/*dak add*/  end.  /*for each ptmstr1 no-lock where..   ****/ 
   end. /* MAINLOOP: */
end. /* MAIN */
