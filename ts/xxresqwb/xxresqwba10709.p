...
define temp-table ts_work no-undo
...
used_cap = 0.
   ...
   if seq_qty_req >= 0
	 ...
	 find last lnd_det where lnd_site = site and
	 lnd_line = prline and lnd_part = seq_part no-error.
	 if available lnd_det then  do: 
	  used_cap = used_cap + seq_qty_req / lnd_rate.
...
used_cap = max(used_cap + ts_cap - ts_used , 0).
 ...
 next_priority = 0.
 find last seq_mstr where seq_site = site and
 seq_line = prline and seq_due_date = ts_date no-error.
 if available seq_mstr then do:
 if seq_part = tsm_part then seq_qty_req = seq_qty_req + tsm_qty.
 else do:
...
delete tsm_move.
 ...
 move_cap = free_cap.
 find first seq_mstr where seq_site = site 
   ...
   if available seq_mstr then do:
	...
	if tsm_qty_req >= seq_qty_req then delete seq_mstr.
	else seq_qty_req = seq_qty_req - tsm_qty_req.
	end.
...
/*以下程序有时出错  */
  ...
  do while free_cap > 0:
  find next  seq_mstr where seq_site = site 
    and seq_line = prline and seq_due_date = ts_date no-error.
   if available seq_mstr then do:
    create tsm_move.
    assign
...
for each  seq_mstr where seq_site = site 
 ...
 find pt_mstr where pt_part = seq_part no-lock.
  if pt_run_seq1 <> "000" and seq_qty_req > 0  or seq_qty_req <= 0 then  
 delete seq_mstr.
end.
