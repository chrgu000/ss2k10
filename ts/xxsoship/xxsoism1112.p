/* xxsoism1112.p -cp from sosoim.p SALES ORDER SHIPMENT WITH SERIAL NUMBERS  */
...
define variable l_ship_oneline  like mfc_logical no-undo.
   ...
   do on error undo mainloop, leave mainloop:
      ...
      do transaction:
         ...
         if ship_site = "" then do:
            ...
            or fill_pick and
               ((not batchrun) or
                (batchrun and execname <> "sosois.p") or
                (batchrun and execname = "sosois.p" and not l_ship_oneline))
            then do:
...
/* Ship One Line During CIM IS SET TO YES IN WAREHOUSE CONTROL FILE    */
   ...
   if not batchrun
      or ((batchrun and
           execname = "sosois.p" and
           available sr_wkfl and
           l_ship_oneline and
           total_lotserial_qty <> 0)
      or (batchrun and not l_ship_oneline)
      or (batchrun and execname <> "sosois.p"))
   then do:
