...
/*----rev history-------------------------------------------------------------------------------------*/
/*A0304**/ /*delete the warning message: NO TAX ENVIRONMENT FOUND; USING DEFAULT*/
/*-Revision end---------------------------------------------------------------*/
...
*******************A0304*/
/*A0304****************/
/*delete the warning message: NO TAX ENVIRONMENT FOUND; USING DEFAULT*/
{gprun.i ""txtxget1.p""
            "(input  zoneTo,
            input  zoneFrom,
            input  pTaxClass,
            output pTaxEnvironment)" }
if pTaxEnvironment = "" then do:
  for first txc_ctrl
     fields(txc_tax_env) no-lock:
  end. 
  if available txc_ctrl then
     pTaxEnvironment = txc_tax_env.
end.
/********************A0304*/
