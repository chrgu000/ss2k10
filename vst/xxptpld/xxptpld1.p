...
for each xxtmppt no-lock where xxpt_chk = "".
    ...
    put unformat xxpt_phantom ' ' xxpt_ord_min ' - '  xxpt_ord_mult  ' - ' xxpt_yld_pct ' - - - - - '.
    if xxpt_routing = "-" then do:
       put unformat "- ".
    end.
    else do:
      put unformat '"' xxpt_routing '" '.
    end.
    if xxpt_bom_code = "-" then do:
       put unformat "- ".
    end.
    else do:
...
if cloadfile then do:
   ...
   for each xxtmppt exclusive-lock where xxpt_chk = "":
       ...
       if available ptp_det and
                    ...
                    (xxpt_routing = ptp_routing or xxpt_routing = "-") and
                    (xxpt_bom_code = ptp_bom_code or xxpt_bom_code = "-")
          then do:
          assign xxpt_chk = "OK".
       end.
       else do:
