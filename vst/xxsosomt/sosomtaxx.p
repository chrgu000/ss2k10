/* sosomtaxx.p - SALES ORDER MAINTENANCE SINGLE LINE ITEMS                     */
/*     copied from sosomta.p - SALES ORDER MAINTENANCE SINGLE LINE ITEMS     */
/* Copyright 1986-2001 QAD Inc., Carpinteria, CA, USA.                       */
/* All rights reserved worldwide.  This is an unpublished work.              */
/*F0PN*/ /*V8:ConvertMode=Maintenance                                        */
/*N014*/ /*V8:RunMode=Character,Windows                                      */
/* REVISION: 6.0      LAST MODIFIED: 03/19/90   BY: ftb *D007*               */
/* REVISION: 6.0      LAST MODIFIED: 04/06/90   BY: ftb *D002*added site     */
/* REVISION: 6.0      LAST MODIFIED: 04/10/90   BY: MLB *D021*               */
/* REVISION: 6.0      LAST MODIFIED: 07/06/90   BY: EMB *D040*               */
/* REVISION: 6.0      LAST MODIFIED: 08/09/90   BY: MLB *D055*               */
/* REVISION: 6.0      LAST MODIFIED: 12/06/90   BY: MLB *D238*               */
/* REVISION: 6.0      LAST MODIFIED: 12/14/90   BY: AFS *D260*               */
/* REVISION: 6.0      LAST MODIFIED: 04/10/91   BY: WUG *D512*               */
/* REVISION: 6.0      LAST MODIFIED: 09/17/91   BY: MLV *F015*               */
/* REVISION: 6.0      LAST MODIFIED: 09/19/91   BY: afs *F040*               */
/* REVISION: 7.0      LAST MODIFIED: 10/30/91   BY: pma *F003*               */
/* REVISION: 7.0      LAST MODIFIED: 11/13/91   BY: WUG *D887*               */
/* REVISION: 6.0      LAST MODIFIED: 11/18/91   BY: afs *D934*               */
/* REVISION: 7.0      LAST MODIFIED: 03/23/92   BY: dld *F297*               */
/* REVISION: 7.0      LAST MODIFIED: 03/27/92   BY: pma *F315*               */
/* REVISION: 7.0      LAST MODIFIED: 04/06/92   BY: afs *F356*               */
/* REVISION: 7.0      LAST MODIFIED: 04/16/92   BY: dld *F382*               */
/* REVISION: 7.0      LAST MODIFIED: 05/06/92   BY: tjs *F470*               */
/* REVISION: 7.0      LAST MODIFIED: 06/10/92   BY: tjs *F504*               */
/* REVISION: 7.3      LAST MODIFIED: 09/15/92   BY: tjs *G035*               */
/* REVISION: 7.3      LAST MODIFIED: 11/17/92   BY: tjs *G191*               */
/* REVISION: 7.3      LAST MODIFIED: 01/15/93   BY: afs *G501*               */
/* REVISION: 7.3      LAST MODIFIED: 01/21/93   BY: tjs *G579*               */
/* REVISION: 7.3      LAST MODIFIED: 02/04/93   BY: bcm *G415*               */
/* REVISION: 7.3      LAST MODIFIED: 02/24/93   BY: sas *G457*               */
/* REVISION: 7.3      LAST MODIFIED: 04/05/93   BY: bcm *G889*               */
/* REVISION: 7.3      LAST MODIFIED: 04/15/93   BY: tjs *G948*               */
/* REVISION: 7.4      LAST MODIFIED: 06/21/93   BY: pcd *H008*               */
/* REVISION: 7.4      LAST MODIFIED: 10/10/93   BY: cdt *H086*               */
/* REVISION: 7.4      LAST MODIFIED: 10/2/93    BY: cdt *H184*               */
/* REVISION: 7.4      LAST MODIFIED: 11/03/93   BY: bcm *H206*               */
/* REVISION: 7.4      LAST MODIFIED: 11/22/93   BY: afs *H238*               */
/* REVISION: 7.4      LAST MODIFIED: 01/13/94   BY: dpm *GI46*               */
/* REVISION: 7.4      LAST MODIFIED: 02/18/94   BY: afs *FL81*               */
/* REVISION: 7.4      LAST MODIFIED: 03/16/94   BY: afs *H295*               */
/* REVISION: 7.4      LAST MODIFIED: 03/28/94   BY: WUG *GJ21*               */
/* REVISION: 7.4      LAST MODIFIED: 04/08/94   BY: afs *H330*               */
/* REVISION: 7.3      LAST MODIFIED: 04/25/94   BY: cdt *GJ56*               */
/* REVISION: 7.4      LAST MODIFIED: 07/11/94   BY: bcm *H438*               */
/* REVISION: 7.4      LAST MODIFIED: 09/13/94   BY: bcm *H494*               */
/* REVISION: 7.4      LAST MODIFIED: 09/22/94   BY: jpm *GM78*               */
/* REVISION: 7.4      LAST MODIFIED: 10/16/94   BY: dpm *FR95*               */
/* REVISION: 7.3      LAST MODIFIED: 11/01/94   BY: ame *GN90*               */
/* REVISION: 7.3      LAST MODIFIED: 11/16/94   BY: qzl *FT43*               */
/* REVISION: 7.4      LAST MODIFIED: 12/14/94   BY: rxm *F09F*               */
/* REVISION: 7.4      LAST MODIFIED: 01/28/95   BY: ljm *G0D7*               */
/* REVISION: 7.4      LAST MODIFIED: 01/31/95   BY: bcm *F0G8*               */
/* REVISION: 8.5      LAST MODIFIED: 02/17/95   BY: dpm *J044*               */
/* REVISION: 7.4      LAST MODIFIED: 02/27/95   BY: jzw *H0BM*               */
/* REVISION: 7.4      LAST MODIFIED: 03/02/95   BY: aep *F0K6*               */
/* REVISION: 7.4      LAST MODIFIED: 03/29/95   BY: rxm *F0PJ*               */
/* REVISION: 8.5      LAST MODIFIED: 03/05/95   BY: DAH *J042*               */
/* REVISION: 7.4      LAST MODIFIED: 06/15/95   BY: rxm *G0Q5*               */
/* REVISION: 7.3      LAST MODIFIED: 10/12/95   BY: ais *G0Z8*               */
/* REVISION: 8.5      LAST MODIFIED: 11/07/95   BY: *J053* T. Farnsworth     */
/* REVISION: 8.5      LAST MODIFIED: 12/05/95   BY: *J04C* Tom Vogten        */
/* REVISION: 8.5      LAST MODIFIED: 04/11/96   BY: *J04C* Sue Poland        */
/* REVISION: 8.5      LAST MODIFIED: 04/29/96   BY: *J0KJ* Dennis Henson     */
/* REVISION: 8.5      LAST MODIFIED: 05/22/96   BY: *J0N2* Dennis Henson     */
/* REVISION: 8.5      LAST MODIFIED: 07/04/96   BY: *J0XR* Dennis Henson     */
/* REVISION: 8.5      LAST MODIFIED: 07/15/96   BY: ajw *J0Z6*               */
/* REVISION: 8.5      LAST MODIFIED: 07/23/96   BY: ajw *J0R1*               */
/* REVISION: 8.5      LAST MODIFIED: 08/01/96   BY: *J13C* Sue Poland        */
/* REVISION: 8.5      LAST MODIFIED: 09/12/96   BY: *J152* Suresh Nayak      */
/* REVISION: 8.5      LAST MODIFIED: 09/18/96   BY: *G2F7* Sanjay Patil      */
/* REVISION: 8.6      LAST MODIFIED: 09/23/96   BY: *K003* Forrest Mori      */
/* REVISION: 8.6      LAST MODIFIED: 09/27/96   BY: svs *K007*               */
/* REVISION: 8.6      LAST MODIFIED: 10/22/96   BY: *K004* Elke Van Maele    */
/* REVISION: 8.6      LAST MODIFIED: 11/05/96   BY: *K01T* Stephane Collard  */
/* REVISION: 8.6      LAST MODIFIED: 11/08/96   BY: *J15C* Markus Barone     */
/* REVISION: 8.6      LAST MODIFIED: 01/01/97   BY: *K03Y* Jean Miller       */
/* REVISION: 8.5      LAST MODIFIED: 01/08/97   BY: *J1DS* Sue Poland        */
/* REVISION: 8.6      LAST MODIFIED: 02/15/97   BY: *J1L2* Suresh Nayak      */
/* REVISION: 8.6      LAST MODIFIED: 02/21/97   BY: *H0SM* Suresh Nayak      */
/* REVISION: 8.6      LAST MODIFIED: 05/19/97   BY: *K0DF* Taek-Soo Chang    */
/* REVISION: 8.6      LAST MODIFIED: 06/19/97   BY: *J1TL* Molly Balan       */
/* REVISION: 8.6      LAST MODIFIED: 07/15/97   BY: *K0G6* Arul Victoria     */
/* REVISION: 8.6      LAST MODIFIED: 07/01/97   BY: *K0DH* Jim Williams      */
/* REVISION: 8.6      LAST MODIFIED: 07/21/97   BY: *K0GS* Kieu Nguyen       */
/* REVISION: 8.6      LAST MODIFIED: 09/21/97   BY: *K0J4* Kieu Nguyen       */
/* REVISION: 8.6      LAST MODIFIED: 09/27/97   BY: *K0HB* Kieu Nguyen       */
/* REVISION: 8.6      LAST MODIFIED: 10/09/97   BY: *J22D* Molly Balan       */
/* REVISION: 8.6      LAST MODIFIED: 12/29/97   BY: *J28V* Surekha Joshi     */
/* REVISION: 8.6      LAST MODIFIED: 01/16/98   BY: *K1BN* Bryan Merich      */
/* REVISION: 8.6      LAST MODIFIED: 01/31/98   BY: *J2D6* Seema Varma       */
/* REVISION: 8.6      LAST MODIFIED: 02/12/98   BY: *K1GQ* Jim Williams      */
/* REVISION: 8.6E     LAST MODIFIED: 02/23/98   BY: *L007* A. Rahane         */
/* REVISION: 8.6E     LAST MODIFIED: 03/24/98   BY: *J2FG* Niranjan R.       */
/* REVISION: 8.6E     LAST MODIFIED: 05/20/98   BY: *K1Q4* Alfred Tan        */
/* REVISION: 8.6E     LAST MODIFIED: 07/03/98   BY: *L024* Sami Kureishy     */
/* REVISION: 9.0      LAST MODIFIED: 11/24/98   BY: *M017* David Morris      */
/* REVISION: 9.0      LAST MODIFIED: 01/12/99   BY: *J38F* Reetu Kapoor      */
/* REVISION: 9.0      LAST MODIFIED: 03/05/99   BY: *M09Y* Katie Hilbert     */
/* REVISION: 9.0      LAST MODIFIED: 03/13/99   BY: *M0BD* Alfred Tan        */
/* REVISION: 9.0      LAST MODIFIED: 05/11/99   BY: *J3FL* Santosh Rao       */
/* REVISION: 9.1      LAST MODIFIED: 10/01/99   BY: *N014* Patti Gaultney    */
/* REVISION: 9.1      LAST MODIFIED: 07/12/99   BY: *N011* Anup Pereira      */
/* REVISION: 9.1      LAST MODIFIED: 12/15/99   BY: *N05D* Steve Nugent      */
/* REVISION: 9.1      LAST MODIFIED: 03/24/00   BY: *N08T* Annasaheb Rahane  */
/* REVISION: 9.1      LAST MODIFIED: 05/30/00   BY: *N0BC* Arul Victoria     */
/* REVISION: 9.1      LAST MODIFIED: 06/19/00   BY: *N0CG* Santosh Rao       */
/* REVISION: 9.1      LAST MODIFIED: 06/29/00   BY: *N0DX* Mudit Mehta       */
/* REVISION: 9.1      LAST MODIFIED: 08/12/00   BY: *N0KN* myb               */
/* REVISION: 9.1      LAST MODIFIED: 09/25/00   BY: *L121* Gurudev C         */
/* REVISION: 9.1      LAST MODIFIED: 10/20/00   BY: *M0TV* Santosh Rao       */
/* REVISION: 9.1      LAST MODIFIED: 11/03/00   BY: *L15F* Kaustubh K        */
/* REVISION: 9.1      LAST MODIFIED: 10/20/00   BY: *N0WP* Mudit Mehta       */
/* REVISION: 9.0      LAST MODIFIED: 06/05/01   BY: *M11Z* Jean Miller       */
/* REVISION: 9.1      LAST MODIFIED: 07/23/01   BY: *M136* Rajesh Thomas     */
/* REVISION: 9.0      LAST MODIFIED: 08/14/01   BY: *M1HP* Jean Miller       */
/* REVISION: 9.1      LAST MODIFIED: 12/06/01   BY: *N16Z* Ed van de Gevel   */
/* ADM1    11/17/03 Brian Lo -  add audit trail logic      */

          {mfdeclre.i}
/*N0WP*/  {cxcustom.i "SOSOMTA.P"}
{gplabel.i &ClearReg=yes} /* EXTERNAL LABEL INCLUDE */

/*N05D*/ /* CHANGES MADE TO THIS PROGRAM MAY ALSO NEED TO BE MADE */
/*N05D*/ /* TO PROGRAM fseomta.p.                                 */

/*N0CG*/ /* CREATED INTERNAL PROCEDURES TO REMOVE ACTION SEGMENT ERROR */

         /* ********** Begin Translatable Strings Definitions ********* */

/*N0BC*  &SCOPED-DEFINE sosomta_p_1 "Return Qty"         */
         /* MaxLen: Comment: */

         &SCOPED-DEFINE sosomta_p_2 "Reprice"
         /* MaxLen: Comment: */

/*N0BC*  &SCOPED-DEFINE sosomta_p_3 "Qty Returned"       */
         /* MaxLen: Comment: */

         &SCOPED-DEFINE sosomta_p_4 "Promise Date"
         /* MaxLen: Comment: */

         &SCOPED-DEFINE sosomta_p_5 "Comments"
         /* MaxLen: Comment: */

/*N0BC*  &SCOPED-DEFINE sosomta_p_7 "Expected Date"      */
         /* MaxLen: Comment: */

         &SCOPED-DEFINE sosomta_p_8 "Discount"
         /* MaxLen: Comment: */

/*N0BC*  &SCOPED-DEFINE sosomta_p_9 "Due Date"           */
         /* MaxLen: Comment: */

/*N0DX*
 *        &SCOPED-DEFINE sosomta_p_6 "Item not in inventory"
 *        /* MaxLen: Comment: */
 *
 *        &SCOPED-DEFINE sosomta_p_10 "   RMA Issue Line   "
 *        /* MaxLen: Comment: */
 *
 *        &SCOPED-DEFINE sosomta_p_11 "  RMA Receipt Line  "
 *        /* MaxLen: Comment: */
 *
 *        &SCOPED-DEFINE sosomta_p_12 "  Sales Order Line  "
 *        /* MaxLen: Comment: */
 *N0DX*/

         /* ********** End Translatable Strings Definitions ********* */


         define input parameter this-is-rma      as  logical.
         define input parameter rma-recno        as  recid.
         define input parameter rma-issue-line   as  logical.

         define new shared workfile sobfile no-undo
            field sobpart like sob_part
            field sobsite like sob_site
            field sobissdate like sob_iss_date
            field sobqtyreq like sob_qty_req
            field sobconsume like sob_qty_req
            field sobabnormal like sob_qty_req.

         define     shared stream bi.
         define     shared frame  bi.

         /* DEFINE RNDMTHD FOR CALL TO SOSOMTLA.P */
         define shared variable rndmthd like rnd_rnd_mthd.
         define new shared variable pl               like pt_prod_line.
         define new shared frame    c.
         define new shared frame    d.
         define new shared variable undo_all         like mfc_logical
                                                     initial no.
         define new shared variable sod_recno        as recid.
         define new shared variable sodcmmts         like soc_lcmmts
                                                     label {&sosomta_p_5}.
         define new shared variable prev_consume     like sod_consume.
         define new shared variable clines           as integer.
         define new shared variable sod-detail-all   like soc_det_all.
         define new shared variable sodreldate       like sod_due_date.
         define new shared variable prev_type        like sod_type.
         define new shared variable prev_site        like sod_site.
         define new shared variable new_line         like mfc_logical.
         define new shared variable solinerun        as character.
         define new shared variable delete_line      like mfc_logical.
         define new shared variable sonbr            like sod_nbr.
         define new shared variable soline           like sod_line.
         define new shared variable new_site         like sod_site.
         define new shared variable err_stat         as integer.

         define new shared variable temp_sod_qty_ord  like sod_qty_ord.
         define new shared variable temp_sod_qty_ship like sod_qty_ship.
         define new shared variable temp_sod_qty_all  like sod_qty_all.
         define new shared variable temp_sod_qty_pick like sod_qty_pick.

         define     shared variable all_days         as integer.
         define     shared variable base_amt         like ar_amt.
         define     shared variable calc_fr          like mfc_logical.
         define     shared variable consume          like sod_consume.
         define     shared variable del-yn           like mfc_logical.
         define     shared variable inv_db           like dc_name.
         define     shared variable line             like sod_line.
         define     shared variable mult_slspsn      like mfc_logical
                                                        no-undo.
         define     shared variable part             as character
                                                        format "x(18)".
         define     shared variable prev_due         like sod_due_date.
         define     shared variable prev_qty_ord     like sod_qty_ord.
         define     shared variable promise_date     as date
                                                        label {&sosomta_p_4}.
         define     shared variable qty              like sod_qty_ord.
         define     shared variable sngl_ln          like soc_ln_fmt.
/*H0SM*/ define            variable l_sngl_ln        like soc_ln_fmt no-undo.
         define     shared variable so_db            like dc_name.
         define     shared variable so_recno         as recid.
         define     shared variable so-detail-all    like soc_det_all.
         define     shared variable soc_pc_line      like mfc_logical.
         define     shared variable socrt_int        like sod_crt_int.
         define     shared variable tax_in           like cm_tax_in.

         define variable cchar           as character.
         define variable desc1           like pt_desc1.
         define variable err-flag        as integer.
         define variable first_time      like mfc_logical initial yes.
         define variable i               as integer.
         define variable j               as integer.
         define variable open_qty        like mrp_qty.
         define variable prev_abnormal   like sod_abnormal.
         define variable ptstatus        like pt_status.
         define buffer   sod_buff        for sod_det.
         define variable vtclass         as character extent 3.
         define variable imp-okay like mfc_logical no-undo.
         define variable frametitle      as character format "x(20)".
         define variable frametitle-so   as character format "x(20)"
/*N0DX*         initial {&sosomta_p_12} */ .
         define variable frametitle-rma-iss  as  character format "x(20)"
/*N0DX*         initial {&sosomta_p_10} */ .
         define variable frametitle-rma-rec  as  character format "x(20)"
/*N0DX*         initial {&sosomta_p_11} */ .
         define variable rmd-recno           as  recid.
         define variable rma-line-type       like sod_fsm_type initial " ".

         /* THESE 'HANDLE' VARIABLES ARE USED TO GIVE DIFFERENT LABELS */
         /* TO FIELDS WHEN MAINTAINING RMA RECEIPT LINES               */
         define variable hdl-ord-qty-field   as  handle.
         define variable hdl-ship-qty-field  as  handle.
         define variable hdl-order-nbr-field as  handle.
         define variable hdl-due-date-field  as  handle.

         define new shared variable discount as decimal label {&sosomta_p_8}
                                                        format "->>>9.9<<<".
         define new shared variable reprice_dtl      like mfc_logical
                                                     label {&sosomta_p_2}.
         define     shared variable reprice          like mfc_logical.
         define     shared variable line_pricing     like mfc_logical.
         define     shared variable new_order        like mfc_logical.
         define new shared variable save_parent_list like sod_list_pr.

         /* BTB VARIABLES */
/*K004*/ define            variable first-repeat     as   logical no-undo.
/*K004*/ define            variable err_flag         as   integer.

/*K004*/ {sobtbvar.i}    /* BACK TO BACK SHARED WORKFILES */
/*M1HP* /*M11Z*/ define variable use-cmf          as logical no-undo. */

         define            variable disc_min_max     like mfc_logical.
         define            variable disc_pct_err     as decimal.
         define new shared variable save_qty_ord     like sod_qty_ord.
/*K007*/ define new shared variable cmtindx          like cmt_indx.
/*K0G6*/ define new shared variable po-ack-wait      as logical no-undo.

/*K004*/ define buffer sod_buff2 for sod_det.
         define buffer rmdbuff   for rmd_det.

         define variable l_tmp_string1 as character no-undo.
/*K0DH*/ define variable p-edi-rollback as logical no-undo initial no.

/*K1BN*/ define new shared variable cf_pm_code     like pt_pm_code.
/*K1BN*/ define new shared variable cf_pt_part_o   like pt_part.
/*K1BN*/ define new shared variable cf_pt_part_n   like pt_part     no-undo.
/*K1BN*/ define new shared variable cf_line        like sod_line    no-undo.
/*K1BN*/ define new shared variable cf_rp_part     like mfc_logical no-undo.
/*K1BN*/ define new shared variable cf_endcfg      like mfc_logical no-undo.
/*K1BN*/ define new shared variable original_model as   character format "x(40)".
/*K1BN*/ define shared variable cfexists           like mfc_logical.
/*K1BN*/ define shared variable cf_cfg_path        like mfc_char.
/*K1BN*/ define shared variable cf_chr             as   character.
/*K1BN*/ define variable cf_sod__qadc01            like sod__qadc01.

/*J2FG*/ define variable l_part                    like pt_part  no-undo.
/*L024*/ define variable mc-error-number           like msg_nbr  no-undo.
/*M017*/ define variable apm-ex-sub  as character format "x(24)" no-undo.
/*M017*/ define variable error_flag     like mfc_logical         no-undo.
/*M017*/ define variable err_mess_no    like msg_nbr             no-undo.
/*M136*/ define variable l_sonbr        like sod_nbr             no-undo.

/*N0DX* SET INITIAL VALUES------- */
/*N0DX*/ assign
         frametitle-so      = getFrameTitle("SALES_ORDER_LINE",20)
         frametitle-rma-iss = getFrameTitle("RMA_ISSUE_LINE",20)
         frametitle-rma-rec = getFrameTitle("RMA_RECEIPT_LINE",20).

/*N0WP*/ {&SOSOMTA-P-TAG1}
         form
            space(1)
            so_nbr
            so_cust
            sngl_ln
         with frame a side-labels width 80.
/*N0WP*/ {&SOSOMTA-P-TAG2}

         /* SET EXTERNAL LABELS */
         setFrameLabels(frame a:handle).

         /* READ SO_MSTR EARLY SO WE'RE ABLE TO SET THE MULTIPLE SALESPERSON
            FLAG FOR ITS DISPLAY.  PREVENT A TRANSACTION BEING SCOPED TO THIS
            ENTIRE PROCEDURE BLOCK BY NOT LOCKING RECORDS HERE. */
/*N0CG** find so_mstr where recid(so_mstr) = so_recno no-lock.  */
/*N0CG** find first soc_ctrl no-lock.                           */
/*N0CG** find first gl_ctrl no-lock.                            */
/*N0CG** find first pic_ctrl no-lock.                           */

/*N0CG*/ for first so_mstr
/*N0CG*/    fields(so_conf_date so_curr so_cust so_div so_due_date
/*N0CG*/           so_ex_rate so_ex_rate2 so_fix_pr so_fr_list so_nbr
/*N0CG*/           so_pricing_dt so_primary so_project so_pr_list so_pst
/*N0CG*/           so_req_date so_secondary so_ship so_site so_slspsn
/*N0CG*/           so_taxable so_taxc so_tax_env so_tax_usage)
/*N0CG*/    where recid(so_mstr) = so_recno no-lock:
/*N0CG*/ end. /* FOR FIRST SO_MSTR */
/*N0WP*/ {&SOSOMTA-P-TAG3}

/*N0CG*/ for first soc_ctrl
/*N0CG*/    fields(soc_apm soc_lcmmts soc_use_btb) no-lock:
/*N0CG*/ end. /* FOR FIRST SOC_CTRL */

/*N0CG*/ for first gl_ctrl
/*N0CG*/    fields(gl_can gl_vat) no-lock:
/*N0CG*/ end. /* FOR FIRST GL_CTRL */

/*N0CG*/ for first pic_ctrl
/*N0CG*/    fields(pic_so_fact pic_so_rfact) no-lock:
/*N0CG*/ end. /* FOR FIRST PIC_CTRL */

         /* IF WE'RE IN RMA MAINTENANCE, READ RMA/SERVICE CONTROL */
         /* FILES AND RELATED RECORDS.                            */
         if this-is-rma then do:
/*N0CG** BEGIN DELETE **
 *          find first rmc_ctrl no-lock no-error.
 *          find first svc_ctrl no-lock no-error.
 *          find rma_mstr where recid(rma_mstr) = rma-recno
 *              no-lock no-error.
 *          find eu_mstr where eu_addr = rma_enduser
 *              no-lock no-error.
 *N0CG** END DELETE **/

/*N0CG*/    for first rmc_ctrl
/*N0CG*/       fields(rmc_edit_isb rmc_lcmmts) no-lock:
/*N0CG*/    end. /* FOR FIRST RMC_CTRL */

/*N0CG*/    for first svc_ctrl
/*N0CG*/       fields(svc_pt_prod) no-lock:
/*N0CG*/    end. /* FOR FIRST SVC_CTRL */

/*N0CG*/    for first rma_mstr
/*N0CG*/       fields(rma_contract rma_ctype rma_enduser rma_loc_iss
/*N0CG*/              rma_loc_rec rma_site_iss rma_site_rec)
/*N0CG*/       where recid(rma_mstr) = rma-recno no-lock:
/*N0CG*/    end. /* FOR FIRST RMA_MSTR */

/*N0CG*/    for first eu_mstr
/*N0CG*/       fields(eu_addr eu_type)
/*N0CG*/       where eu_addr = rma_enduser no-lock:
/*N0CG*/    end. /* FOR FIRST EU_MSTR */

            if rma-issue-line then rma-line-type = "RMA-ISS".
            else rma-line-type = "RMA-RCT".
         end.
         else rma-recno = ?.

         /* SET DISPLAY OF THE MULTIPLE SALESPERSON FLAG          */
         /* MULPITLE COMMISSION UPDATES WILL ONLY HAPPEN IF TRUE. */
         if so_slspsn[2] <> "" or so_slspsn[3] <> "" or so_slspsn[4] <> ""
            then mult_slspsn = true.
         else mult_slspsn = false.

         /* PREPARE FOR POSSIBLE USER EXIT PROGRAMS */
         if this-is-rma then do:
            if rma-issue-line then do:
                {fsmnp01.i    ""fsrmamt.p"" 20 solinerun}      /****issues****/
            end.    /* if rma-issue-line */
            else do:
                {fsmnp01.i    ""fsrmamt.p"" 30 solinerun}      /****returns***/
            end.
         end.   /* if this-is-rma */
         else do:
            {fsmnp01.i   ""sosomt.p"" 20 solinerun}
         end.

         if sngl_ln then clines = 1.
/*N0WP*/ {&SOSOMTA-P-TAG4}
         {solinfrm.i}  /* Define shared frames */
         /*V8:HiddenDownFrame=c */

         /* FOR RMA'S, IT'S BETTER IF THE SO_NBR LABEL DOESN'T READ  */
         /* "SALES ORDER", AND WE HAVE DIFFERENT LABELS FOR DUE DATE */
         /* AND SOME CHANGES FOR RMA RECEIPT LINES REGARDING QTYS    */

/*N0BC* --------------------- BEGIN DELETE ------------------------- *
 *       if this-is-rma then do:
 *          assign
 *             hdl-order-nbr-field       = so_nbr:handle
 *             hdl-order-nbr-field:label = "RMA"
 *             hdl-due-date-field        = sod_due_date:handle.
 *          if rma-issue-line then
 *             hdl-due-date-field:label  = {&sosomta_p_9}.
 *          else
 *             assign
 *                hdl-ord-qty-field        = sod_qty_ord:handle
 *                hdl-ord-qty-field:label  = {&sosomta_p_1}
 *                hdl-ship-qty-field       = sod_qty_ship:handle
 *                hdl-ship-qty-field:label = {&sosomta_p_3}
 *                hdl-due-date-field:label = {&sosomta_p_7}.
 *       end.   /* if this-is-rma */
 *N0BC* --------------------- END   DELETE ------------------------- */

/*N0BC* --------------------- BEGIN ADD    ------------------------- */
         if this-is-rma then do:
            assign
               hdl-order-nbr-field       = so_nbr:handle
               hdl-order-nbr-field:label = getTermLabel("RMA",6)
               hdl-due-date-field        = sod_due_date:handle.
            if rma-issue-line then
               hdl-due-date-field:label  = getTermLabel("DUE_DATE",8).
            else
               assign
                  hdl-ord-qty-field        = sod_qty_ord:handle
                  hdl-ord-qty-field:label  =
                          getTermLabel("RETURN_QUANTITY",11)
                  hdl-ship-qty-field       = sod_qty_ship:handle
                  hdl-ship-qty-field:label =
                          getTermLabel("QUANTITY_RETURNED",14)
                  hdl-due-date-field:label = getTermLabel("EXPECTED_DATE",13).
         end.   /* if this-is-rma */
/*N0BC* --------------------- END ADD      ------------------------- */
/*N0WP*/ {&SOSOMTA-P-TAG5}

         linefmt: repeat on endkey undo, leave:

/*N0WP*/ {&SOSOMTA-P-TAG6}
/*H0SM*/    assign l_sngl_ln = sngl_ln.
/*N0WP*/ {&SOSOMTA-P-TAG7}
            if not first_time then update sngl_ln with frame a.

/*H0SM*/    if l_sngl_ln <> sngl_ln  then
/*H0SM*/       clear frame c all no-pause.

            clines = ?.
            if sngl_ln then clines = 1.
/*N0WP*/    {&SOSOMTA-P-TAG8}
            first_time = no.

            if this-is-rma then
               if rma-issue-line then
                  frametitle = frametitle-rma-iss.
               else
                  frametitle = frametitle-rma-rec.
            else
/*N0WP*/    {&SOSOMTA-P-TAG9}
               frametitle = frametitle-so.

/*K004*/    first-repeat = yes.

            mainloop:
            repeat with frame c down on endkey undo, next linefmt:

/*L15F*/       find first so_mstr
/*L15F*/          where recid(so_mstr) = so_recno exclusive-lock no-error.
/*N0WP*/       {&SOSOMTA-P-TAG10}

               reprice_dtl = reprice.

               display so_nbr so_cust sngl_ln with frame a.
/*N0WP*/       {&SOSOMTA-P-TAG11}

               /* need this hide/view scheme to re-realize frames when they change.*/
               hide frame d.
               hide frame c.
               view frame c. /*to re-realize if single/multi line changes */
               if sngl_ln then view frame d.

/*N011*/ /*V8! hide message. */

               {mfdel.i sobfile}

               if this-is-rma then sodcmmts = rmc_lcmmts.
               else sodcmmts = soc_lcmmts.
               sod-detail-all = so-detail-all.

               /* RMA RECEIPT LINES DO NOT GET ALLOCATIONS */
               if this-is-rma and not rma-issue-line
                    then sod-detail-all = no.

/*N0WP*/       {&SOSOMTA-P-TAG12}
               /* SCREENS & REPORTS DON'T SUPPORT 4 DIGIT LINE NOS. */
               if line < 999 then line = line + 1.
               else if line = 999 then do:
/*N0CG**          {mfmsg.i 7418 2}  /* LINE CANNOT BE > 999 */  */
/*N0CG*/          /* LINE CANNOT BE > 999 */
/*N0CG*/          run p-mfmsg (input 7418, input 2).
               end.

/*N0CG**       find sod_det where sod_nbr = so_nbr and sod_line = line  */
/*N0CG**          no-lock no-error.                                     */

/*N0CG*/       for first sod_det
/*N0CG*/          where sod_nbr  = so_nbr and
/*N0CG*/                sod_line = line no-lock:
/*N0CG*/       end. /* FOR FIRST SOD_DET */

               /* IF WE'RE MAINTAINING RMA'S, AN INPUT PARAMETER TOLD US */
               /* WHETHER WE'RE CURRENTLY DOING THE ISSUE OR RECEIPT     */
               /* LINES, AND RMA-LINE-TYPE NOW CONTAINS THE LITERAL      */
               /* WHICH SOD_DET'S USE TO IDENTIFY THIS TYPE OF LINE...   */
               /* SO, LOOK FOR THEM...                                   */

               if this-is-rma then do:
                  do while available sod_det and
                     sod_fsm_type <> rma-line-type:
                     find next sod_det where sod_nbr = so_nbr
                        no-lock no-error.
                  end.
                  if available sod_det then
                     if sod_line < 999 then
                        assign line = sod_line + 1.
                     else  do:
                        line = 999.
/*N0CG**                {mfmsg.i 7418 2}  /* LINE CANNOT BE > 999 */  */
/*N0CG*/                /* LINE CANNOT BE > 999 */
/*N0CG*/                run p-mfmsg (input 7418, input 2).
                     end.
               end.     /* if this-is-rma */

               if not available sod_det or
                  (this-is-rma and sod_fsm_type <> rma-line-type) then do:
                  discount = if not pic_so_fact then 0 else 1.
                  display
                     line
                      "" @ sod_part
                      0  @ sod_qty_ord
                      "" @ sod_um
                      0  @ sod_list_pr
                      discount
                      0  @ sod_price
                  with frame c.
               end.
               else do:
/*N0DX*            desc1 = {&sosomta_p_6}. */
/*N0DX*/           desc1 = getTermLabel("ITEM_NOT_IN_INVENTORY",24).
/*N0CG**          find pt_mstr where pt_part = sod_part  */
/*N0CG**             no-lock no-error no-wait.           */
/*N0CG*/          for first pt_mstr
/*N0CG*/             fields(pt_desc1 pt_fr_class pt_loc pt_part pt_pm_code
/*N0CG*/                    pt_price pt_prod_line pt_ship_wt pt_ship_wt_um
/*N0CG*/                    pt_site pt_status pt_taxable pt_taxc pt_um)
/*N0CG*/             where pt_part = sod_part no-lock:
/*N0CG*/          end. /* FOR FIRST PT_MSTR */

                  if available pt_mstr then desc1 = pt_desc1.
                  else if sod_desc <> "" then desc1 = sod_desc.

                  /* DETERMINE DISCOUNT DISPLAY FORMAT AND SET VARIABLE discount
                     ACCORDINGLY */
                  {gppidisc.i pic_so_fact sod_disc_pct pic_so_rfact}
                  display
                     line sod_part sod_qty_ord sod_um
                     sod_list_pr discount sod_price
                  with frame c.

                  if sngl_ln then
                     display
                        sod_qty_all sod_qty_pick
                        sod_qty_ship sod_qty_inv
                        sod_loc sod_site sod_serial
/*K03Y*/ /* /*K007*/    sod_bonus */
                        base_curr sod_std_cost desc1
                        sod_pricing_dt
                        sod_req_date sod_per_date sod_due_date
                        sod_fr_list
                        sod_acct
/*N014*/                sod_sub
                        sod_cc
/*N014*/                sod_project
                        sod_dsc_acct
/*N014*/                sod_dsc_sub
                        sod_dsc_cc
/*N014*                 sod_project                                    */
/*N014*/                sod_dsc_project
                        sod_confirm
                        sod_type sod_um_conv sod_consume sod-detail-all
                        sod_slspsn[1] mult_slspsn sod_comm_pct[1]
                        sod_taxable
                        sod_taxc /* when (not {txnew.i}) */
                        (sod_cmtindx <> 0) @
                        sodcmmts
                        sod_crt_int
                        sod_fix_pr
                     with frame d.

                  /* RMA RECEIPT LINES ARE FOR RETURNS FROM CUSTOMERS -  */
                  /* OR, SHIPPING NEGATIVE QUANTITIES TO CUSTOMERS.      */
                  /* IN RMA MAINTENANCE, HOWEVER, THESE NEGATIVE QTYS    */
                  /* NEED TO DISPLAY AS POSITIVE VALUES (AND, IN THE     */
                  /* RMD_DET FILE, THEY'RE STORED AS POSITIVE VALUES).   */
                  if this-is-rma and not rma-issue-line then do:
                     if sngl_ln then
                        display (sod_qty_ship * -1) @ sod_qty_ship
                                (sod_qty_inv  * -1) @ sod_qty_inv
                        with frame d.
                     display (sod_qty_ord * -1) @ sod_qty_ord
                     with frame c.
                  end.
               end.  /* else, available sod_det, do */

/*K004*/       /* added next section */
               /* BTB ROLL-BACK OF ADDED LINES                  */
               /*               - DELETING THEM IF ROLLING BACK */

               s-rollback = no.

               /* ONLY WHEN ENTERING THE LINE FRAME THE FIRST TIME */
/*K0DH*        if soc_use_btb and so_primary and first-repeat then do:     */
/*K0DH*/       if soc_use_btb and not so_secondary and first-repeat then do:

                  /* FIRST LOOP THROUGH ALL SO LINES TO FIND ALL EXISTING    */
                  /* CMF RECORDS FOR ADDED LINES                             */
                  for each sod_buff2 where sod_buff2.sod_nbr = so_nbr no-lock
                     break by sod_buff2.sod_line:
                     if last(sod_buff2.sod_line) then first-repeat = no.

/*K0DH*/             /* ADDED input parameter p-edi-rollback */
                     {gprun.i ""sobtbrb.p""
                              "(input recid(so_mstr),
                                input sod_buff2.sod_line,
                                input ""pod_det"",
                                input ""pod_line"",
                                input p-edi-rollback,
                                output return-msg)" }
                     /* DISPLAY ERROR MESSAGE RETURN FROM SOBTBRB.P */
                     if return-msg <> 0 then do:
/*N0CG**                {mfmsg.i return-msg 4}  */
/*N0CG*/                run p-mfmsg (input return-msg, input 4).
                        return-msg = 0.
                        if not batchrun then pause.
                        undo mainloop, retry.
                     end.
/*N05D**
 *                   if sod_buff2.sod_btb_po + ","
 *                       + string(sod_buff2.sod_btb_pod_line, ">>9")
 *                                    = s-cmdkeyval
 *N05D**/

/*N05D*/             if sod_buff2.sod_btb_po + ","
/*N05D*/             + trim(string(sod_buff2.sod_btb_pod_line, ">>9"))
/*N05D*/             = s-cmdkeyval
                     then do:
/*M017*/                /* ADDED ASSIGN STATEMENT */
/*M017*/                assign
                           line =  sod_buff2.sod_line
                           s-rollback = yes
                           del-yn = yes.
                        leave.
                        /* IF CMF IS FOUND THEN LEAVE LOOP TO DELETE LINE */
                     end.
                  end.  /* for each sod_buff2 */
               end.  /*  roll-back of added lines */

               /* SKIP FURTHER UPDATES IF THIS LINE IS A BTB SO LINE THAT */
               /* HAS TO BE DELETED CAUSED BY A ROLL-BACK                 */
               /* (cmd_det exists for field "pod_line"                    */
               if not del-yn then do:
                  s-rb-init = no.
/*K004*/          /* end added section */

/*K1BN*/          /* if a replacement part is sent back by the configurator, */
/*K1BN*/          /* the program should redisplay the line number, the       */
/*K1BN*/          /* replacement part and prompt the user to enter site      */
/*K1BN*/          if not cf_rp_part then do:
/*N0WP*/          {&SOSOMTA-P-TAG13}
                     update line with frame c editing:

/*M136*/             /* TO SHOW SO LINES FOR SELECTED SO IN CHAR AND GUI  */
/*M136*/             /* USING LOOK-UP OR DRILL DOWN BROWSE ON LINE FIELD. */

/*M136*/                {gpbrparm.i &browse=gpbr241.p
/*M136*/                            &parm=c-brparm1 &val="so_nbr"}
/*M136*/                {gpbrparm.i &browse=gplu241.p
/*M136*/                            &parm=c-brparm1 &val="so_nbr"}

/*M136*/                l_sonbr = so_nbr.

/*M136*/                on leave of line in frame c do:
/*M136*/                   /*V8!
/*M136*/                   do on error undo, leave:
/*M136*/                      {gpbrparm.i &browse=gpbr241.p
/*M136*/                                  &parm=c-brparm1 &val="l_sonbr"}
/*M136*/                      {gpbrparm.i &browse=gplu241.p
/*M136*/                                  &parm=c-brparm1 &val="l_sonbr"}

/*M136*/                      run q-leave in global-drop-down-utilities.
/*M136*/                   end. /* DO ON ERROR ... */

/*M136*/                   run q-set-window-recid in global-drop-down-utilities.
/*M136*/                   if return-value = "error"
/*M136*/                      then return no-apply. */

/*M136*/                end. /* ON LEAVE ... */


                        /* NEXT/PREVIOUS PROCESSING MUST BE SENSITIVE TO THE   */
                        /* TYPE OF ORDER BEING MAINTAINED (SALES ORDER OR RMA) */
                        /* AND, IF RMA'S, THE TYPE OF LINE ITEM CURRENTLY      */
                        /* BEING MAINTAINED (ISSUE OR RECEIPT)                 */
                        if this-is-rma then do:
                           {mfnp05.i
                            sod_det
                            sod_nbrln
                            "sod_nbr = so_nbr and sod_fsm_type = rma-line-type"
                            sod_line
                            "line"}
                        end.
                        else do:
                           {mfnp01.i sod_det line sod_line so_nbr sod_nbr sod_nbrln}
                        end.

                        if recno <> ? then do:
/*N0CG**                   find pt_mstr where pt_part = sod_part  */
/*N0CG**                      no-lock no-error no-wait.           */
/*N0CG*/                   for first pt_mstr
/*N0CG*/                      fields(pt_desc1 pt_fr_class pt_loc pt_part
/*N0CG*/                             pt_pm_code pt_price pt_prod_line
/*N0CG*/                             pt_ship_wt pt_ship_wt_um pt_site
/*N0CG*/                             pt_status pt_taxable pt_taxc pt_um)
/*N0CG*/                      where pt_part = sod_part no-lock:
/*N0CG*/                   end. /* FOR FIRST PT_MSTR */

                           if available pt_mstr then
                              desc1 = pt_desc1.
                           else
                              if sod_desc <> "" then
                                 desc1 = sod_desc.
                              else
/*N0DX*                          desc1 = {&sosomta_p_6}. */
/*N0DX*/                         desc1 = getTermLabel("ITEM_NOT_IN_INVENTORY",24).
                           line = sod_line.
                           /* DETERMINE DISCOUNT DISPLAY FORMAT AND SET VARIABLE
                              discount ACCORDINGLY */
                           {gppidisc.i pic_so_fact sod_disc_pct pic_so_rfact}
                           display
                              line sod_part sod_qty_ord sod_um
                              sod_list_pr discount sod_price
                           with frame c.

                           if sngl_ln then
                              display
                                 sod_qty_all sod_qty_pick
                                 sod_qty_ship sod_qty_inv
                                 sod_loc sod_site sod_serial
/*K03Y*/ /* /*K007*/             sod_bonus */
                                 sod_std_cost desc1 base_curr
                                 sod_pricing_dt
                                 sod_req_date sod_per_date sod_due_date
                                 sod_fr_list
                                 sod_acct
/*N014*/                         sod_sub
                                 sod_cc
/*N014*/                         sod_project
                                 sod_dsc_acct
/*N014*/                         sod_dsc_sub
                                 sod_dsc_cc
/*N014*/                         sod_dsc_project
/*N014*                          sod_project                                  */
                                 sod_confirm
                                 sod_type sod_um_conv sod_consume sod-detail-all
                                 sod_slspsn[1] mult_slspsn sod_comm_pct[1]
                                 sod_taxable
                                 sod_taxc /* when (not {txnew.i}) */
                                 (sod_cmtindx <> 0) @
                                 sodcmmts
                                 sod_crt_int
                                 sod_fix_pr
                              with frame d.

                           /* AS ABOVE, RMA RECEIPT LINES ARE RETURNS, AND  */
                           /* HAVE NEGATIVE SOD_QTY'S, HOWEVER, THESE QTYS  */
                           /* MUST DISPLAY POSITIVE TO THE USER.            */

                           if this-is-rma and not rma-issue-line then do:
                              if sngl_ln then
                                  display (sod_qty_ship * -1) @ sod_qty_ship
                                          (sod_qty_inv  * -1) @ sod_qty_inv
                                  with frame d.
                              display (sod_qty_ord * -1) @ sod_qty_ord
                              with frame c.
                           end.
                        end.    /* if recno <> ? */
                     end.    /* editing */
/*K1BN*/          end. /*if not cf_rp_part*/
/*K1BN*/          if cf_rp_part then do:
/*K1BN*/             line = cf_line.
/*K1BN*/             display line with frame c.
/*K1BN*/          end.

                  /* ADD/MOD/DELETE  */
                  find sod_det where sod_nbr = so_nbr and sod_line = input line
/*N0CG**             no-error.             */
/*N0CG*/             exclusive-lock no-error.
/*K004*/       end.  /* if not del-yn */
/*K004*/       else  /* IF DEL-YN THEN FIND LINE TO DELETE */
/*K004*/          find sod_det where sod_nbr = so_nbr and sod_line = line
/*K004*/             exclusive-lock no-error.
/*N0WP*/       {&SOSOMTA-P-TAG14}
               if not available sod_det then do:
                  if line = 0 then do:
/*N0CG**             {mfmsg.i 642 4}  /* Invalid line number */  */
/*N0CG*/             /* INVALID LINE NUMBER */
/*N0CG*/             run p-mfmsg (input 642, input 4).
                     undo mainloop, retry.
                  end.

/*K0J4*/          if so_secondary then do:
/*N0CG** /*K0J4*/    {mfmsg.i  2939 3} */
                     /* NEW LINE IS NOT ALLOWED AT THE SBU */
/*N0CG*/             run p-mfmsg (input 2939, input 3).
/*K0J4*/             undo mainloop, retry.
/*K0J4*/          end.

                  if not new_order then
                     reprice_dtl = yes.  /*This will cause line to be priced*/

                  new_line = yes.
                  create sod_det.

/*N0WP*/          {&SOSOMTA-P-TAG15}
                  assign sod_nbr = so_nbr
                         sod_line       = input line
                         sod_due_date   = so_due_date
                         sod_pricing_dt = so_pricing_dt
                         sod_pr_list    = so_pr_list
                         sod_consume    = consume
                         sod_site       = so_site
                         sod_slspsn[1]  = so_slspsn[1]
                         sod_slspsn[2]  = so_slspsn[2]
                         sod_slspsn[3]  = so_slspsn[3]
                         sod_slspsn[4]  = so_slspsn[4]
                         sodcmmts       = soc_lcmmts
                         sod_fr_list    = so_fr_list
                         sod_fix_pr     = so_fix_pr
                         sod_crt_int    = socrt_int
                         /* END USER DEFAULTS FROM CUSTOMER SHIP-TO, BUT MAY */
                         /* BE OVERRIDDEN IN THE ISB DEFAULTS POPUP FOR EACH */
                         /* LINE ITEM.                                       */
                         sod_enduser     = so_ship
                         sod_project     = so_project
/*N014*/                 sod_dsc_project = so_project
/*K004*/                 sod_dir_all     = no.
/*N0WP*/          {&SOSOMTA-P-TAG16}

                  if this-is-rma then do:
                     sodcmmts = rmc_lcmmts.
                     create rmd_det.

/*N0WP*/             {&SOSOMTA-P-TAG17}
                     assign rmd_nbr   = so_nbr
                        rmd_prefix    = "C"
                        rmd_line      = input line
                        rmd_edit_isb  = rmc_edit_isb
/*J22D*                 rmd_rma_rtrn  = no */
                        rmd_sa_nbr    = rma_contract
                        sod_enduser   = rma_enduser
                        sod_fsm_type  = rma-line-type
/*J15C*/                /* RMD_SV_CODE WILL NOW HOLD THE SVC TYPE THAT */
/*J15C*/                /* PROVIDES COVERAGE                           */
/*J15C*/                rmd_sv_code   = rma_ctype
/*J15C*/                sodcmmts      = rmc_lcmmts.
/*N0WP*/             {&SOSOMTA-P-TAG18}

                     {fssvsel.i rma_ctype """" eu_type}

/*J28V*/             /* FOR RMA ISSUE LINE, IF THE HEADER PROMISE DATE IS DIFFERENT  */
/*J28V*/             /* FROM THE HEADER DUE DATE, DEFAULT THE LINE ITEM PROMISE DATE */
/*J28V*/             /* TO THE HEADER PROMISE DATE. ELSE DEFAULT IT TO ?.            */
/*J28V*/             /* ALSO DEFAULT THE LINE ITEM REQUIRED DATE TO ?                */

/*N0WP*/             {&SOSOMTA-P-TAG19}
                     if rma-issue-line then do:

                        assign
                           sod_rma_type  = "O"
                           rmd_type      = "O"
                           rmd_exp_date  = so_req_date
                           sod_due_date  = so_req_date
/*J28V*                    sod_per_date  = so_req_date */
/*J28V*/                   sod_per_date  = promise_date
/*J28V*/                                    when (promise_date <> so_req_date)
/*J28V*                    sod_req_date  = so_req_date */
                           sod_site      = rma_site_iss
                           sod_loc       = rma_loc_iss.

                     end.   /* if rma-issue-line */

/*J28V*/             /* FOR RMA RECEIPT LINE, DEFAULT THE LINE ITEM REQUIRED DATE AND */
/*J28V*/             /* PROMISE DATE TO ?                                             */

                     else   /* this must be a receipt-line */
                        assign
                            sod_rma_type  = "I"
                            rmd_type      = "I"
                            rmd_exp_date  = so_due_date
/*J22D*/                    rmd_rma_rtrn  = yes
                            sod_consume   = no
                            sod_due_date  = so_due_date
/*J28V**                    sod_per_date  = so_due_date  */
/*J28V**                    sod_req_date  = so_due_date  */
                            sod_site      = rma_site_rec
                            sod_loc       = rma_loc_rec.
/*N0WP*/                {&SOSOMTA-P-TAG20}

                     rmd-recno = recid(rmd_det).

                  end.  /* if this-is-rma */

                  /* INITIALIZE TAX MANAGEMENT FIELDS */
/*M09Y*                  if {txnew.i} then do:         *
 *                    sod_taxable   = so_taxable.
 *                    sod_taxc      = so_taxc.
 *                    sod_tax_usage = so_tax_usage.
 *                    sod_tax_env   = (if this-is-rma then "" else so_tax_env).  */
/*M09Y*/          if {txnew.i} then assign
/*M09Y*/             sod_taxable   = so_taxable
/*M09Y*/             sod_taxc      = so_taxc
/*M09Y*/             sod_tax_usage = so_tax_usage
                     /* FOR RMA'S, DON'T DEFAULT THE ENVIRONMENT BECAUSE IT MUST */
                     /* BE RECALCULATED FOR RCPTS/ISS (DIFFERENT FROM/TO)        */
/*M09Y*/             sod_tax_env   = (if this-is-rma then "" else so_tax_env)
                     sod_tax_in    = tax_in.
/*M09Y*                  end.                                                    */

                  sod_confirm = so_conf_date <> ?.
                  if not this-is-rma then do:
                     if so_req_date <> so_due_date then
                        sod_req_date = so_req_date.
                     if promise_date <> so_due_date then
                        sod_per_date = promise_date.
                  end. /* if not this-is-rma */
                  if gl_can or gl_vat then sod_tax_in = tax_in.
                               /*cm_tax_in*/
                  if gl_can then sod_pst = so_pst.    /*cm_pst*/
                  if not {txnew.i} then
                     assign
                        sod_taxable = so_taxable
                        sod_taxc    = so_taxc.
/*N0WP*/          {&SOSOMTA-P-TAG21}
                  desc1 = "".
                  assign line.

                  /* DETERMINE DISCOUNT DISPLAY FORMAT AND SET VARIABLE discount
                     ACCORDINGLY */
                  {gppidisc.i pic_so_fact sod_disc_pct pic_so_rfact}
                  display
                     line sod_part sod_qty_ord sod_um
                     sod_list_pr discount sod_price
                  with frame c.

/*N0WP*/          {&SOSOMTA-P-TAG22}
                  if sngl_ln then
                     display
                        sod_qty_all sod_qty_pick sod_qty_ship
                        sod_qty_inv sod_loc sod_site sod_serial
/*K03Y*/ /* /*K007*/    sod_bonus  */
                        desc1 sod_std_cost base_curr
                        sod_pricing_dt
                        sod_req_date sod_per_date sod_due_date
                        sod_fr_list
                        sod_acct
/*N014*/                sod_sub
                        sod_cc
/*N014*/                sod_project
                        sod_dsc_acct
/*N014*/                sod_dsc_sub
                        sod_dsc_cc
/*N014*/                sod_dsc_project
/*N014*                 sod_project                                          */
                        sod_confirm
                        sod_type sod_um_conv sod_consume sod-detail-all
                        sod_slspsn[1] mult_slspsn sod_comm_pct[1]
                        sod_taxable
                        sod_taxc
                        sodcmmts
                        sod_crt_int
                        sod_fix_pr
                     with frame d.

/*N0WP*/          {&SOSOMTA-P-TAG23}
/*K1BN*/          if not cf_rp_part then do:
                     prompt-for sod_det.sod_part with frame c editing:
                        /* FIND NEXT/PREVIOUS  RECORD */
                        {mfnp.i pt_mstr sod_part pt_part sod_part pt_part pt_part}

                        if recno <> ? then do:
/*M09Y*                           desc1 = pt_desc1.                        */
/*M09Y*/                   assign desc1    = pt_desc1
                                  sod_part = pt_part.

/*L024*                    sod_price = pt_price * so_ex_rate.              */
/*L024*                    sod_price = if rma-line-type <> "RMA-RCT" then  */
/*L024*                                   pt_price * so_ex_rate            */
/*L024*                                else                                */
/*L024*                                   sod_price.                       */

/*L024*/                   if rma-line-type <> "RMA-RCT" then do:
/*L024*/                      {gprunp.i "mcpl" "p" "mc-curr-conv"
                                 "(input  base_curr,
                                   input  so_curr,
                                   input  so_ex_rate2,
                                   input  so_ex_rate,
                                   input  pt_price,
                                   input  false,
                                   output sod_price,
                                   output mc-error-number)"}.
/*L024*/                      if mc-error-number <> 0 then do:
/*N0CG** /*L024*/                {mfmsg.i mc-error-number 2}  */
/*N0CG*/                         run p-mfmsg (input mc-error-number, input 2).
/*L024*/                      end.
/*L024*/                   end.

/*M09Y*                           sod_list_pr =                         */
/*M09Y*/                   assign sod_list_pr =
                                     if rma-line-type <> "RMA-RCT"
                                     then sod_price
/*M09Y*                                 else sod_list_pr.               */
/*M09Y*/                             else sod_list_pr
                                  sod_um = pt_um.
                           /* FOR RMA'S, PRICE HAS NEVER DISPLAYED WHEN */
                           /* THE USER NEXT/PREV'ED ON PART NUMBER.  TO */
                           /* PREVENT CONFUSION WHEN RESTOCK AMOUNTS    */
                           /* APPLY, DON'T DISPLAY THEM NOW EITHER.     */
                           display
                              sod_part sod_um
                              sod_list_pr when (not this-is-rma)
                              sod_price   when (not this-is-rma)
                           with frame c.
/*N0WP*/                   {&SOSOMTA-P-TAG24}
                           if sngl_ln then display desc1 with frame d.
/*N0WP*/                   {&SOSOMTA-P-TAG25}
                        end.

/*K0DF*/             end. /* end of editing */

/*J1TL*/             hide message no-pause.
/*K1BN*/          end. /*if not cf_rp_part*/
/*K1BN*/          if cf_rp_part then do:
/*K1BN*/             sod_part = cf_pt_part_n.
/*K1BN*/             display sod_part with frame c.
/*K1BN*/          end.

/*K1BN*/          if not cf_rp_part then
                     assign sod_part.

/*K0DF*           end. /* end of editing */    */

/*J2FG*/          l_part = sod_part.

/*J2FG*/          /* SEARCH CUSTOMER ITEM/ ITEM MASTER */
/*J2FG*/          {gprun.i ""sopart.p"" "(input so_cust,
                                          input so_ship,
                                          input-output sod_part,
                                          output sod_custpart)"}

/*J2FG*/          for first pt_mstr
/*J2FG*/              fields ( pt_desc1 pt_fr_class pt_loc pt_part pt_pm_code
/*J2FG*/                       pt_price pt_prod_line pt_ship_wt pt_ship_wt_um
/*J2FG*/                       pt_site pt_status pt_taxable pt_taxc pt_um )
/*J2FG*/              where pt_part = sod_part no-lock :
/*J2FG*/          end. /* FOR FIRST PT_MSTR */

/*J2FG*/          if l_part <> sod_part then do:
/*J2FG*/             display sod_part with frame c.
/*J2FG*/             /* CUSTOMER ITEM # REPLACED BY # */
/*J2FG*/             {mfmsg03.i 56 1 l_part sod_part """"}
/*J2FG*/          end. /* IF L_PART <> SOD_PART */

/*J2FG*/          if available pt_mstr and pt_pm_code = "F" then do with frame c:
/*J2FG*/             part = sod_part.

/*J2FG*/             /* SCROLLING WINDOW FOR FAMILY ITEMS */
/*J2FG*/             {gprun.i ""swbom.p""}.

/*J2FG*/             if part > "" then
/*J2FG*/                sod_part = part.
/*J2FG*/             display part @ sod_part.
/*J2FG*/             for first pt_mstr
/*J2FG*/                 fields ( pt_desc1 pt_fr_class pt_loc pt_part pt_pm_code
/*J2FG*/                          pt_price pt_prod_line pt_ship_wt pt_ship_wt_um
/*J2FG*/                          pt_site pt_status pt_taxable pt_taxc pt_um )
/*J2FG*/                 where pt_part = sod_part no-lock :
/*J2FG*/             end. /* FOR FIRST PT_MSTR */

/*J2FG*/          end. /* IF AVAILABLE PT_MSTR AND PT_PM_CODE = "F" */

/*J2FG*/          else if not available pt_mstr then do:
/*J2FG*/               /* SHIP TYPE SET TO (M)EMO */
/*N0CG** /*J2FG*/      {mfmsg.i 25 2}  */
/*N0CG*/               run p-mfmsg (input 25, input 2).
/*J2FG*/               sod_type = "M".

/*N011*/ /*V8!         if not batchrun then
/*N011*/               do:
/*N011*/                  pause.
/*N011*/                  hide message no-pause.
/*N011*/               end. /* IF NOT BATCHRUN */
/*N011*/    */

/*J2FG*/          end. /* IF NOT AVAILABLE PT_MSTR */

/*M017*/          for first cm_mstr fields(cm_addr cm_promo) no-lock
/*M017*/             where cm_addr = so_cust: end.
/*M017*/          if available cm_mstr then do:
/*M017*/             if soc_apm
/*M017*/                and cm_promo <> "" then do:
/*M017*/                /* CHECK IF PART BELONGS TO AN APM DIVISION */
/*M017*/                if available pt_mstr then do:
/*M017*/                   apm-ex-sub = "if/":U.
/*M017*/                   {gprunex.i
                              &module  = 'APM':U
                              &subdir  = apm-ex-sub
                              &program = 'ifapm001.p':U
                              &params  = "(input sod_part,
                                           input so_div,
                                           output error_flag,
                                           output err_mess_no)"}
/*M017*/                   if error_flag then do:
/*M017*/                      {mfmsg03.i err_mess_no 3 sod_part so_div """"}
/*M017*/                      /* ITEM # LINE# DOES NOT BELONG TO */
/*M017*/                      /* APM DIVISION # */
/*M017*/                      undo, next.
/*M017*/                   end.
/*M017*/                   /* CONFIGURED PART - CHECK COMPONENTS */
/*M017*/                   if pt_pm_code = "C" then do:
/*M017*/                      for each ps_mstr no-lock
/*M017*/                         where ps_par = sod_part:
/*M017*/                         apm-ex-sub = "if/":U.
/*M017*/                         {gprunex.i
                                    &module  = 'APM':U
                                    &subdir  = apm-ex-sub
                                    &program = 'ifapm001.p':U
                                    &params  = "(input ps_comp,
                                                 input so_div,
                                                 output error_flag,
                                                 output err_mess_no)"}
/*M017*/                         if error_flag then do:
/*M017*/                            {mfmsg03.i 4543 3 ps_comp so_div """"}
/*M017*/                            /* COMPONENT # DOES NOT BELONG TO */
/*M017*/                            /* APM DIVISION # */
/*M017*/                            undo, leave.
/*M017*/                         end.
/*M017*/                      end.
/*M017*/                      if error_flag then undo, next.
/*M017*/                   end.

/*M017*/                end. /* IF AVAILABLE PT_MSTR */
/*M017*/                sod_div = so_div.
/*M017*/             end.  /* IF SOC_APM AND CM_PROMO <> "" */
/*M017*/          end.  /* IF AVAILABLE CM_MSTR */

/*J2FG*/          /* MOVED THE LOGIC FOR PT_MSTR/CP_MSTR SEARCH TO SOPART.P */

                  /* SET COMM PCT FROM SPD_DET IF AVAILABLE */
                  pl = "".
/*N0CG**          find pt_mstr where pt_part = sod_part no-lock no-error.  */
/*N0CG*/          for first pt_mstr
/*N0CG*/             fields(pt_desc1 pt_fr_class pt_loc pt_part pt_pm_code
/*N0CG*/                    pt_price pt_prod_line pt_ship_wt pt_ship_wt_um
/*N0CG*/                    pt_site pt_status pt_taxable pt_taxc pt_um)
/*N0CG*/             where pt_part = sod_part no-lock:
/*N0CG*/          end. /* FOR FIRST PT_MSTR */

               if this-is-rma and rma-issue-line and available pt_mstr then do:
                     /* CHECK FOR ADD-RMA RESTRICTED TRANSACTION */
/*M09Y*                     ptstatus = pt_status.                */
/*M09Y*/             assign ptstatus = pt_status
                            substring(ptstatus,9,1) = "#".
                     if can-find(isd_det where isd_status = ptstatus and
                        isd_tr_type = "ADD-RMA") then do:
                        {mfmsg02.i 358 3 pt_status}
                        undo, retry.
                     end.
                  end.  /* if this-is-rma... */

                  /* FOR SO'S, PRODUCT LINE COMES FROM PT_MSTR.*/
                  /* FOR RMA'S, IF "USE ITEM PRODUCT LINE",    */
                  /* USE P/L FROM ITEM, ELSE USE P/L FROM THE  */
                  /* SERVICE TYPE */
/*N0WP*/          {&SOSOMTA-P-TAG26}
                  if not this-is-rma or svc_pt_prod then do:
                     if available pt_mstr then pl = pt_prod_line.
                  end.
/*N0WP*/          {&SOSOMTA-P-TAG27}
                  if this-is-rma and not svc_pt_prod then do:
                     {fssvsel.i rma_ctype """" eu_type}
                     if available sv_mstr then pl = sv_prod_line.
                     else if available pt_mstr then pl = pt_prod_line.
                  end.
                  sod_recno = recid(sod_det).
                  {gprun.i ""sosocom.p""}

/*N0WP*/          {&SOSOMTA-P-TAG28}
                  if available pt_mstr then do:
                     /* INITIALIZE FREIGHT VALUES*/
/*M017*/             assign
/*M017*/                sod_fr_class = pt_fr_class
/*M017*/                sod_fr_wt_um = pt_ship_wt_um.
/*M017*              sod_fr_class = pt_fr_class. */
                     if calc_fr then sod_fr_wt = pt_ship_wt.
/*M017*              sod_fr_wt_um = pt_ship_wt_um. */

/*M09Y*                     ptstatus = pt_status.                */
/*M09Y*/             assign ptstatus = pt_status
                            substring(ptstatus,9,1) = "#".
/*N0WP*/             {&SOSOMTA-P-TAG29}
                     if this-is-rma then .
                     else do:
/*N0WP*/             {&SOSOMTA-P-TAG30}
                        if can-find(isd_det where isd_status = ptstatus and
                           isd_tr_type = "ADD-SO") then do:
                           {mfmsg02.i 358 3 pt_status}
                           undo, retry.
                        end.
                     end.    /* else do... */

                     /* SET TAX DEFAULTS */
                     /* INITIALIZE TAX MANAGEMENT FIELDS */
/*M09Y*                     if {txnew.i} then do:        */
/*M09Y*                        sod_taxable = (so_taxable and pt_taxable).      */
/*M09Y*/             if {txnew.i} then assign
/*M09Y*/                sod_taxable = (so_taxable and pt_taxable)
                        sod_taxc = pt_taxc.
/*M09Y*                     end.                                               */
/*N16Z*/             {&SOSOMTA-P-TAG43}
                     else if gl_vat or gl_can then do:
/*M09Y*                        if pt_taxable = yes and so_taxable = yes then do:    */
/*M09Y*/                if pt_taxable = yes and so_taxable = yes then
/*M017*/                /* ADDED ASSIGN STATEMENT */
/*M017*/                assign
                           sod_taxable = yes
                           sod_taxc = pt_taxc.
/*M09Y*                        end.                                            */
                        else do:
/*M017*/                /* ADDED ASSIGN STATEMENT */
/*M017*/                assign
                           sod_taxable = no
                           sod_taxc = pt_taxc.
                           /*IF PART VAT CODE <> 0% THEN USE THE SO_TAXC*/
                           if pt_taxable then do:
/*N0CG** BEGIN DELETE **
 *                            find last vt_mstr where
 *                               vt_class   = pt_taxc      and
 *                               vt_start  <= sod_due_date and
 *                               vt_end    >= sod_due_date and
 *                               vt_tax_pct = 0 no-lock no-error.
 *N0CG** END DELETE **/

/*N0CG*/                      for last vt_mstr
/*N0CG*/                         fields(vt_class vt_end vt_start vt_tax_pct)
/*N0CG*/                         where vt_class   = pt_taxc      and
/*N0CG*/                               vt_start  <= sod_due_date and
/*N0CG*/                               vt_end    >= sod_due_date and
/*N0CG*/                               vt_tax_pct = 0 no-lock:
/*N0CG*/                      end. /* FOR LAST VT_MSTR */

                              if not available vt_mstr then sod_taxc = so_taxc.
                           end.
                        end.  /* else do */
                        /* VALIDATE FEWER THAN 4 TAX CLASSES */
                        if not sngl_ln then do:
                           {gpvatchk.i &counter = j &buffer = sod_buff
                              &ref = so_nbr
                              &buffref = sod_nbr     &file = sod_det
                              &taxc = sod_taxc
                              &frame = "NO-FRAME"       &undo_yn = true
                              &undo_label = mainloop}
                        end.
                     end.     /* if gl_vat or gl_can.. */
                     else /*not gl_vat or gl_can or {txnew.i} */
/*M09Y*                        if sod_taxable = yes and pt_taxable = no then do:    */
/*M09Y*/                if sod_taxable = yes and pt_taxable = no then
/*M017*/                /* ADDED ASSIGN STATEMENT */
/*M017*/                assign
                           sod_taxable = no
                           sod_taxc    = pt_taxc.
/*M09Y*                        end.                              */
/*N16Z*/             {&SOSOMTA-P-TAG44}
                     /* FOR SO'S, DEFAULT THE PT_MSTR LOCATION.  */
                     /* FOR RMA'S, LOCATION CAME FROM THE HEADER */
                     /* VALUES EARLIER.                          */
                     assign
                        sod_prodline = pt_prod_line
                        sod_um  = pt_um.
                     if not this-is-rma then sod_loc = pt_loc.

                     /* FOR RMA'S, WE MAY NEED PRODUCT LINE FROM */
                     /* THE SERVICE TYPE, AND WE WANT ISSUE OR   */
                     /* RECEIVE LOCATION FROM THE CONTROL FILE   */
/*N0WP*/             {&SOSOMTA-P-TAG31}
                     if this-is-rma then do:
                        if not svc_pt_prod then
                           if available sv_mstr then sod_prodline = sv_prod_line.
                     end.    /* if this-is-rma */
/*N0WP*/             {&SOSOMTA-P-TAG32}

/*M09Y*                     if so_curr = base_curr then do:       */
/*M09Y*/             if so_curr = base_curr then assign
                        sod_price =
                           if rma-line-type <> "RMA-RCT"
                              then pt_price
/*M09Y*                              else sod_price.              */
/*M09Y*/                      else sod_price
                        sod_list_pr =
                           if rma-line-type <> "RMA-RCT"
                              then pt_price
                              else sod_list_pr.
/*M09Y*                     end.                                  */

                  end.  /* if available pt_mstr */
                  else do:
                     if not sngl_ln and so_site = "" then do:
/*N0CG**                {mfmsg.i 941 3} /* BLANK SITE NOT ALLOWED */  */
/*N0CG*/                /* BLANK SITE NOT ALLOWED */
/*N0CG*/                run p-mfmsg (input 941, input 3).
                        undo mainloop, retry.
                     end.
                     /* FOR RMA'S, WE MAY NEED TO GET PRODUCT LINE */
                     /* FROM THE SERVICE TYPE...                   */
                     if this-is-rma and not svc_pt_prod and available sv_mstr then
                             sod_prodline = sv_prod_line.
                  end.   /* else, not available pt_mstr, do */

                  /*  Set default line site to pt_mstr site if header site is */
                  /*  not valid for this item.                                */
                  global_part = sod_part.
                  if sod_type = "" then do:
                     new_site = sod_site.
                     {gprun.i ""gpptsi01.p""}
                     if err_stat <> 0 then do:
                        new_site = pt_site.
                        {gprun.i ""gpptsi01.p""}
                        if err_stat = 0 then do:
                           sod_site = pt_site.
                           {mfmsg02.i 6201 1 sod_site}
/*J3FL**                   pause.                                           */
/*J3FL*/                   if not batchrun then pause.
                           /* Default site invalid; changed to Item default */
                        end.
                        /* If multi-line, we need to reject the line NOW. */
                        else if not sngl_ln then do:
/*N0CG**                   {mfmsg.i 715 3} /* Item does not exist at site */ */
/*N0CG*/                   /* ITEM DOES NOT EXIST AT SITE */
/*N0CG*/                   run p-mfmsg (input 715, input 3).
                           if not batchrun then pause.
                           undo mainloop, retry.
                        end.    /* if not sngl_ln */
                     end.  /* if err_stat <> 0  */
                  end.  /* if sod_type = "" */

               end. /* if new line */
               else do:  /* MODIFYING EXISTING LINE */

                  /* IF WE'RE MAINTAINING RMA ISSUE LINES, AND THE    */
                  /* USER HAS PICKED A RECEIPT LINE (OR VICE VERSA),  */
                  /* DON'T LET HIM CONTINUE WITH IT...                */
                  if this-is-rma then do:
/*N0WP*/             {&SOSOMTA-P-TAG33}
                     if rma-issue-line then do:
                        if sod_fsm_type = "RMA-ISS" then .
                        else do:
/*N0CG**                   {mfmsg.i 824 3}  */
                           /* NOT A VALID RMA ISSUE LINE */
/*N0CG*/                   run p-mfmsg (input 824, input 3).
                           undo mainloop, retry.
                        end.
                     end.    /* if rma-issue-line */
                     else do:
                        if sod_fsm_type = "RMA-RCT" then .
                        else do:
/*N0CG**                   {mfmsg.i 7207 3}  */
                           /* NOT A VALID RMA RECEIPT LINE */
/*N0CG*/                   run p-mfmsg (input 7207, input 3).
                           undo mainloop, retry.
                        end.
                     end.    /* else do */
/*N0WP*/             {&SOSOMTA-P-TAG34}
/*N0CG**             find rmd_det where rmd_nbr = sod_nbr             */
/*N0CG**                 and rmd_line = sod_line and rmd_prefix = "C" */
/*N0CG**                 no-lock no-error.                            */
/*N0CG*/             for first rmd_det
/*N0CG*/                fields(rmd_edit_isb rmd_exp_date rmd_line rmd_link
/*N0CG*/                       rmd_nbr rmd_prefix rmd_rma_line rmd_rma_nbr
/*N0CG*/                       rmd_rma_rtrn rmd_sa_nbr rmd_sv_code rmd_type)
/*N0CG*/                where rmd_nbr    = sod_nbr and
/*N0CG*/                      rmd_prefix = "C"     and
/*N0CG*/                      rmd_line   = sod_line no-lock:
/*N0CG*/             end. /* FOR FIRST RMD_DET */

                     assign rmd-recno = recid(rmd_det).
                  end.    /* if this-is-rma */

/*M017*/          /* ADDED ASSIGN STATEMENT */
/*M017*/          assign
                     global_loc = sod_loc
                     new_line = no.

                  /* Reverse old history */
                  /* CREATE SCREEN BUFFER WITH OLD LINE INFO */
                  do with frame bi on error undo, retry:
                     /* This overlap frame bi sets the display format of those */
                     /* fields to what have been re-formatted in solinfrm.i    */

/*J2D6*/             /* REPLACED BELOW BLOCK BY SOBIFRM.I */
/*J2D6**             BEGIN DELETE **
 *                   form
 *                      sod_qty_ord          format "->>>>,>>9.9<<<<"
 *                      sod_list_pr          format ">>>,>>>,>>9.99<<<"
 *                      sod_disc_pct label "Disc%"     format "->>>>9.99"
 *                   with frame bi width 80.
 *J2D6**             END DELETE   */

/*J2D6*/             /* FORM DEFINITION FOR HIDDEN FRAME BI */
/*J2D6*/             {sobifrm.i}

                     form sod_det with frame bi width 80.

                     {mfoutnul.i &stream_name = "bi"}
                     display stream bi sod_det with frame bi.
                     output stream bi close.
                  end.
                  if sod_confirm then do:
                     {mfdel.i sobfile}
                     if sod_type = "" then
                     for each sob_det where sob_nbr  = sod_nbr
                              and sob_line = sod_line
                             no-lock:
                        create sobfile.
                        assign sobpart    = sob_part
                           sobsite    = sob_site
                           sobissdate = sob_iss_date
                           sobqtyreq  = sob_qty_req .
                        if sod_consume = yes then
                            sobconsume  = - sob_qty_req.
                        else sobabnormal = - sob_qty_req.
                     end.  /* for each sob_det  */
                  end.   /* if sod_confirm */
               end.   /* else, modify existing line, do */

/*M017*/       /* ADDED ASSIGN STATEMENT */
/*M017*/       assign
                  so_recno = recid(so_mstr)
                  sod_recno = recid(sod_det).

/*K0G6*/       /*  INITIAL ORDER WAITING FOR PO ACK */
/*K0G6*/       if can-find(trq_mstr where trq_doc_type = "SO":U
/*K0G6*/          and trq_doc_ref  = so_nbr
/*K0G6*/          and (trq_msg_type = "ORDRSP-I":U))
/*K0G6*/       then po-ack-wait = yes.
/*K0G6*/       else if so_primary and sod_btb_po <> "" then do:
/*N0CG** /*K0G6*/  find first po_mstr where po_nbr = sod_btb_po no-lock */
/*N0CG**           no-error.                                            */
/*N0CG*/          for first po_mstr
/*N0CG*/             fields(po_nbr po_xmit)
/*N0CG*/             where po_nbr = sod_btb_po no-lock:
/*N0CG*/          end. /* FOR FIRST PO_MSTR */

/*K0G6*/          if available po_mstr and (po_xmit = "2" or po_xmit = "5")
/*K0G6*/             then po-ack-wait = yes.
/*K0G6*/       end.

/*J1TL*/       /* THIS WILL CAUSE ANY WARNINGS THAT ARE DISPLAYED FOR THE */
/*J1TL*/       /* ITEM TO PAUSE.                                          */
/*J1TL*/       message.
/*J1TL*/       message.

/*K004*/       /* SKIP FURTHER UPDATES IF THIS LINE IS A BTB SO LINE THAT */
/*K004*/       /* HAS TO BE DELETED CAUSED BY A ROLL-BACK */
/*K004*/       if not del-yn then do:

                  /* THE LINE MAINTENANCE SUBROUTINES ARE TOLD   */
                  /* WHETHER THEY'RE DOING SALES ORDERS OR RMA'S */
                  /* AND, IF RMA'S, ARE PASSED ADDITIONAL INFO.  */
                  /* ABOUT THE CURRENT RMA LINE.                 */
/*N0WP*/          {&SOSOMTA-P-TAG35}
/*ADM1                  {gprun.i ""sosomtla.p""*/
/*ADM1*/          {gprun.i ""sosomtlaxx.p""
                           "(input this-is-rma,
                             input rma-recno,
                             input rma-issue-line,
                             input rmd-recno)"}
/*K004*/       end.  /* if not del-yn */

               if undo_all then undo mainloop, next mainloop.
/*N0WP*/       {&SOSOMTA-P-TAG36}

               if del-yn = yes then do:

/*N0CG**          find first wo_mstr where wo_nbr =         */
/*N0CG**             (sod_nbr + "." + string(sod_line)) and */
/*N0CG**             wo_part = sod_part no-lock no-error.   */
/*N0CG*/          for first wo_mstr
/*N0CG*/             fields(wo_nbr wo_part)
/*N0CG*/             where wo_nbr  = (sod_nbr + "." + string(sod_line)) and
/*N0CG*/                   wo_part = sod_part no-lock:
/*N0CG*/          end. /* FOR FIRST WO_MSTR */

                  if available wo_mstr then do:
/*N0CG**             {mfmsg.i 515 2}  */
                     /* Final assy work order exists for this line */
/*N0CG*/             run p-mfmsg (input 515, input 2).
                     pause.
                  end.

                  if sod_sched and
                     (can-find (first sch_mstr where
                        sch_type = 1       and
                        sch_nbr  = sod_nbr and
                        sch_line = sod_line) or
                      can-find (first sch_mstr where
                         sch_type = 2       and
                         sch_nbr  = sod_nbr and
                         sch_line = sod_line) or
                      can-find (first sch_mstr where
                         sch_type = 3       and
                         sch_nbr  = sod_nbr and
                         sch_line = sod_line)) then do:
/*N0CG**             {mfmsg.i 6022 3}  */
/*N0CG*/             run p-mfmsg (input 6022, input 3).
                     undo mainloop, retry mainloop.
                  end.

/*K004*/          /* added next section */
                  /* DELETE BTB RELATED DATA */

                  /* PRIMARY SO */
/*K0DH*           if soc_use_btb and so_primary then do:             */
/*K0DH*/          if soc_use_btb and not so_secondary then do:

/*M1HP*              s-create-pod-line = no. */

                     /* TRANSMIT DELETE OF PRIM. SO TO PO AND SEC. SO */
/*K0HB*              ADD input no before output return-msg   */
/*M11Z*/ /*****************************************************
 *                    {gprun.i ""sosobtb1.p""
 *                             "(input recid(sod_det),
 *                               input yes,
 *                               input ""?"",
 *                               input ""?"",
 *                               input 0,
 *                               input ?,
 *                               input ?,
 *                               input ""?"",
 *                               input no,
 *                               output return-msg )" }
 ***********************/

/*M1HP*/             /* Moved processing to internal proc to reduce size */
/*M1HP*/             run p-process-emt
                        (output return-msg).

/*M1HP*/ /***********************************************
 * /*M11Z*/             find first wkf-btb2 where w2-sodline = sod_line
 *                    no-lock no-error.
 * /*M11Z*/             if available wkf-btb2 and
 *                       (w2-po-xmit = "2" or w2-po-xmit = "4"
 *                       or (w2-po-xmit = "3" and w2-cmf-status = "x"))
 *                    then
 *                       use-cmf = yes.
 *                    else
 *                       use-cmf = no.
 *
 * /*M11Z*/             {gprun.i ""socram01.p""
 *                       "(input sod_nbr,
 *                         input sod_line,
 *                         input-output sod_btb_po,
 *                         input-output sod_btb_pod_line,
 *                         input no,
 *                         input yes,
 *                         input sod_site,
 *                         input so_ship,
 *                         input sod_qty_ord,
 *                         input no,
 *                         input sod_due_date,
 *                         input sod_due_date,
 *                         input use-cmf,
 *                         output return-msg)"}
 *                    s-rollback = no.  /* RESET VARIABLE WHEN ROLLING BACK */
 *************************/ /*M1HP*/

                     /* DISPLAY ERROR MESSAGE RETURN FROM SOSOBTB1.P */
/*M11Z*              if return-msg <> 0 then do: */
/*M11Z*/             if return-msg <> 0 and return-msg <> 343 then do:
/*N0CG**                {mfmsg.i return-msg 3}  */
/*N0CG*/                run p-mfmsg (input return-msg, input 3).
                        return-msg = 0.
                        undo mainloop, retry mainloop .
                     end.

/*M11Z*              find first wkf-btb2 where w2-sodline = sod_line */
/*M11Z*              no-lock no-error.                               */

/*M1HP*/ /***** Moved to internal procedure ********************
 *                    /* DELETE DIRECT ALLOCATION AT THE SBU SITE */
 *                    if available wkf-btb2 and
 *                       (w2-po-xmit = "1" or w2-po-xmit = "2") then do:
 *
 *                       /* SWITCH TO DATABASE OF SECONDARY BUSINESS UNIT */
 *                       {gprun.i ""gpalias2.p""
 *                                "(sod_btb_vend, output err_flag)"}
 *
 *                       if err_flag = 0 then do:
 *                          /* SBU DATABASE CONNECTION = OK */
 *
 *                          /* DELETE DIRECT SO ALLOCATIONS BTB ON SBU */
 *                          {gprun.i ""sosoall.p""
 *                                   "(sod_nbr, sod_line, sod_part,
 *                                     prev_qty_ord, sod_um_conv,
 *                                     sod_btb_vend, yes)"}
 *                       end.      /* if err_flag = 0 */
 *
 *                       /* CHANGE DATABASE BACK TO PRIMARY BUSINESS UNIT */
 *                       {gprun.i ""gpalias3.p"" "(so_db, output err_flag)"}
 *
 *                    end. /* delete direct allocation */
 **********************************/ /*M1HP*/

                  end.  /* if soc_use_btb and so_primary */

                  /* SECUNDARY SO */
/*K0DH*           if not so_primary then do:  */
/*K0DH*/          if so_secondary then do:
/*K0GS*              DO NOT ALLOW DELETE EMT SO FROM THE SBU          */
/*N0CG** /*K0GS*/    {mfmsg.i 2938 3} */
/*N0CG*/             run p-mfmsg (input 2938, input 3).
/*K0GS*/             undo mainloop, retry mainloop .

/*K0GS*              find btb_det where btb_so = sod_nbr              */
/*K0GS*                             and btb_sod_line = sod_line       */
/*K0GS*              exclusive-lock no-error.                         */
/*K0GS*              if available btb_det then delete btb_det.        */
                  end.

/*K004*/          /* end added section */

/*K1GQ*/          /* WE ONLY WANT TO DELETE INFO IN REMOTE DB'S IF LINE IS CONFIRMED */
/*K1GQ*/          if sod_confirm then do:

                     /* Delete line information that might exist in other databases */
/*N0CG**             find si_mstr where si_site = sod_site no-lock. */
/*N0CG*/             for first si_mstr
/*N0CG*/                fields(si_db si_site)
/*N0CG*/                where si_site = sod_site no-lock:
/*N0CG*/             end. /* FOR FIRST SI_MSTR */

                     if si_db <> so_db then do:
                        {gprun.i ""gpalias3.p"" "(si_db, output err-flag)" }
                        if err-flag = 0 or err-flag = 9 then do:
                           assign
                              sonbr  = so_nbr
                              soline = sod_line .
/*L121**                   {gprun.i ""solndel.p""} */

/*L121*/                   /* ADDED INPUT PARAMETER no TO NOT EXECUTE */
/*L121*/                   /* MFSOFC01.I AND MFSOFC02.I WHEN CALLED   */
/*L121*/                   /* FROM DETAIL LINES                       */
/*L121*/                   {gprun.i ""solndel.p""
                                    "(input no)"}
                        end.
                        /* Reset the db alias to the sales order database */
                        {gprun.i ""gpalias3.p"" "(so_db, output err-flag)" }
                        sod_recno = recid(sod_det).
                     end.

/*K1GQ*/          end. /* if sod_confirm */

                  /* DELETE ALLOCATION DETAIL*/
/*M09Y*/          run p-laddel in THIS-PROCEDURE.
/*M09Y* Moved the following to internal procedure to reduce action segment size *
 *                 for each lad_det exclusive-lock where
 *                   lad_dataset = "sod_det" and
 *                   lad_nbr     = sod_nbr   and
 *                   lad_line = string(sod_line):
 *                   find ld_det where ld_site = lad_site
 *                      and ld_loc  = lad_loc
 *                      and ld_lot  = lad_lot
 *                      and ld_ref  = lad_ref
 *                      and ld_part = lad_part
 *                      exclusive-lock no-error.
 *                   if available ld_det then
          *                      ld_qty_all = ld_qty_all -
 *                      (lad_qty_all + lad_qty_pick).
 *                   delete lad_det.
 *M09Y*                end.                              */

/*M0TV** /*K003*/ /* DELETE KIT COMPONENT DETAIL ALLOCATIONS */       */
/*M0TV** /*K003*/ {gprun.i ""soktdel1.p"" "(input recid(sod_det))"}   */

                  /* UPDATE ACCUM QTY WORKFILE WITH REVERSAL.  save_parent_list
                     DETERMINED IN sosomtla.p */
                  if rma-line-type <> "RMA-RCT" and
                     (line_pricing or not new_order) then do:
                     {gprun.i ""gppiqty2.p"" "(
                                               sod_line,
                                               sod_part,
                                               - save_qty_ord,
                                               - (save_qty_ord *
                                                  save_parent_list),
                                               sod_um,
                                               yes,
                                               yes,
                                               yes
                                              )"}
                  end.

                  /* DELETE SALES ORDER LINE RELATED PRICE LIST HISTORY */
                  if rma-line-type <> "RMA-RCT" then do:
                     {gprun.i ""gppihdel.p"" "(
                                               1,
                                               sod_nbr,
                                               sod_line
                                              )"}
                  end.

                  /* DELETE SOB_DET */
/*J38F**          delete_line = yes.       */
/*J38F**           {gprun.i ""sosomtk.p""} */

/*J38F*/         /** TO DELETE SOB_DET RECORDS FOR CENTRAL DB WHEN A  **/
/*J38F*/         /** SALES ORDER LINE WITH REMOTE DB IS DELETED       **/
/*J38F*/         run p-sobdel.

/*N0WP*/          {&SOSOMTA-P-TAG37}
                  if sod_sched then do:
                     find scx_ref
                     where scx_type = 1 and scx_order = sod_nbr and
                     scx_line = sod_line
                     exclusive-lock.

                     delete scx_ref.
                  end.

                  /* DELETE IMPORT EXPORT RECORDS */
                  find first ied_det exclusive-lock where
                     ied_type = "1" and ied_nbr = so_nbr and ied_line = sod_line
                     no-error.
                  if available ied_det then delete ied_det.
/*N0WP*/          {&SOSOMTA-P-TAG38}

                  assign
                     sod_qty_ord  = temp_sod_qty_ord
                     sod_qty_ship = temp_sod_qty_ship
                     sod_qty_all  = temp_sod_qty_all
                     sod_qty_pick = temp_sod_qty_pick.

/*K1BN*/          if cfexists then cf_sod__qadc01 = sod__qadc01.

/*M017*/          /* DELETE SALES ORDER DETAIL RELATIONSHIPS */
/*M017*/          {gprun.i ""sosoapm4.p"" "(input sod_nbr, input sod_line)"}
                  delete sod_det.

                  /* FOR RMA'S, WE HAVE SOME ADDITIONAL DELETE CLEANUP TO DO:

                      - IF THE RMA LINE IS LINKED TO ANOTHER RMA LINE,
                         ERASE THAT LINK.  NOTE: AN RMA LINE NUMBER IS STORED
                         IN RMD_LINE. THE RMA LINE IT'S LINKED TO WOULD BE
                         NOTED IN RMD_LINK.

                     - IF THE RMA RECEIPT LINE IS LINKED TO AN RTS, ERASE
                         THAT LINK. NOTE: THIS LINKAGE BETWEEN RMA'S AND
                         RTS'S IS ACCOMPLISHED VIA FIELDS RMD_RMA_LINE AND
                         RMD_RMA_NBR.  ON AN RTS RETURN LINE, THESE FIELDS
                         WOULD POINT TO THE RMA RECEIPT LINE, AND ON THE RMA
                         RECEIPT LINE, THEY'D POINT TO THE RTS RETURN LINE.  */

                  if this-is-rma then do:
                     /* CLEAN UP LINKAGE WITHIN THE CURRENT RMA */
                     find rmd_det where recid(rmd_det) = rmd-recno
                        exclusive-lock no-error.
                     if rmd_det.rmd_link <> 0 then do:
                        find rmdbuff
                           where rmdbuff.rmd_nbr    = rmd_det.rmd_nbr
                           and   rmdbuff.rmd_prefix = "C"  /* C = Customer */
                           and   rmdbuff.rmd_line   = rmd_det.rmd_link
                           exclusive-lock no-error.
                        if available rmdbuff then
                           rmdbuff.rmd_link = 0.
                     end.    /* if rmd_det.rmd_link <> 0 then */

                     /* FOR RECEIPT LINES, SEE IF THERE'S AN RTS TO CLEAR ALSO */
                     if not rma-issue-line and rmd_det.rmd_rma_nbr <> " "then do:
                        find rmdbuff
                           where rmdbuff.rmd_nbr    = rmd_det.rmd_rma_nbr
                           and   rmdbuff.rmd_prefix = "V"  /* V = Vendor */
                           and   rmdbuff.rmd_line   = rmd_det.rmd_rma_line
                           exclusive-lock no-error.
                        if available rmdbuff then
                           assign rmdbuff.rmd_rma_nbr = " "
                                  rmdbuff.rmd_rma_line = 0.
                     end.  /* if not rma-issue-line and... */
                     delete rmd_det.
                  end.    /* if this-is-rma */

/*M017*/          /* ADDED ASSIGN STATEMENT */
/*M017*/          assign
                     line = line - 1
                     del-yn = no.
                  clear frame c.
                  if sngl_ln then clear frame d.

/*M09Y* /*K1BN*/          if cfexists and cf_sod__qadc01 <> "" then do:    */
/*M09Y*/          if cfexists and cf_sod__qadc01 <> "" then
/*K1BN*/             /* delete any associated configuration files*/
/*K1BN*/             os-delete value(cf_cfg_path + cf_chr  + cf_sod__qadc01).
/*M09Y* /*K1BN*/          end.                                             */

/*N0CG**          {mfmsg.i 6 1}  */
/*N0CG*/          run p-mfmsg (input 6, input 1).
                  next mainloop.
               end.
/*M017*/       /* CODE MOVED TO PROCEDURE PL-COMMENT DUE TO ACTION SEGMENT */
/*M017*/       /* ALSO MODIFIED FOR APM API 86F */
/*M017*/       run pl-comment in THIS-PROCEDURE.

/*M017* **** BEGIN DELETED SECTION ****
 * /*K007*/       if not sod_bonus and soc_apm then
 * /*K007*/       for each pih_hist where pih_doc_type = 1
 * /*K007*/                           and pih_nbr      = sod_nbr
 * /*K007*/                           and pih_line     = sod_line no-lock :
 * /*K007*/          if pih_promo2 = "B" then do on endkey undo, leave :
 * /*K007*/             /* display the comment associated to the pi_list */
 * /*K007*/             find pi_mstr where pi_list_id = pih_list_id
 * /*K007*/                no-lock no-error.
 * /*K007*/
 * /*K007*/             if available pi_mstr then do :
 * /*K007*/                cmtindx = pi_mstr.pi_cmtindx.
 * /*K007*/                {gprun.i ""gpcmmt04.p"" "(input ""pi_mstr"")"}
 * /*K007*/             end.
 * /*K007*/          end. /* contain bonus stock price list */
 * /*K007*/       end. /*  for each pih_hist  */
 *M017* **** END DELETED SECTION ****/

               if not sngl_ln then down 1 with frame c.

               /* IF IMPORT EXPORT MASTER RECORD EXIST  THEN CALL THE IMPORT     */
               /* EXPORT DETAIL LINE CREATION PROGRAM TO CREATE ied_det          */

               imp-okay = no.
               if can-find
                  (first ie_mstr where ie_type = "1" and ie_nbr =  sod_nbr)
               then do:
                  {gprun.i ""iedetcr.p"" "(input ""1"",
                                           input sod_nbr,
                                           input sod_line,
                                           input recid(sod_det),
                                           input-output imp-okay)"}
                  if imp-okay = no then undo mainloop, retry.
               end.
            end. /* mainloop: */

         end. /* linefmt */
/*N0WP*/ {&SOSOMTA-P-TAG39}

         if sngl_ln then hide frame d.
         hide frame c.
         hide frame a.
/*N0WP*/ {&SOSOMTA-P-TAG40}

/*M017* **** BEGIN NEW CODE ****/

         PROCEDURE pl-comment:
            /* -----------------------------------------------------------
            Purpose:     Display the comment for the associated price list.
                         Has been moved to an internal procedure due to
                         exceeding avtion segment size.
            Parameters:  <None>
            Notes:
            -------------------------------------------------------------*/

            for first cm_mstr fields(cm_addr cm_promo) no-lock
            where cm_addr = so_mstr.so_cust: end.
            if not sod_det.sod_bonus
               and soc_ctrl.soc_apm
               and available cm_mstr
               and cm_promo <> ""
               and not this-is-rma then
               for each pih_hist where pih_doc_type = 1
                                   and pih_nbr      = sod_nbr
                                   and pih_line     = sod_line no-lock :
                  if pih_promo2 = "B" then do on endkey undo, leave :
                     /* DISPLAY THE COMMENT ASSOCIATED TO THE PI_LIST */
/*N0CG**             find pi_mstr where pi_list_id = pih_list_id  */
/*N0CG**                no-lock no-error.                         */
/*N0CG*/             for first pi_mstr
/*N0CG*/                fields(pi_cmtindx pi_list_id)
/*N0CG*/                where pi_list_id = pih_list_id no-lock:
/*N0CG*/             end. /* FOR FIRST PI_MSTR */

                     if available pi_mstr then do :
                        cmtindx = pi_mstr.pi_cmtindx.
                        {gprun.i ""gpcmmt04.p"" "(input ""pi_mstr"")"}
                     end.
                  end. /* CONTAIN BONUS STOCK PRICE LIST */
               end. /*  FOR EACH PIH_HIST  */
         END PROCEDURE.

/*M017* **** END NEW CODE ****/

/*J38F*/  procedure p-sobdel:

/*J38F*/  define variable x3          as   character format "x(3)"  no-undo.
/*J38F*/  define variable qadkey2     like qad_key2                 no-undo.

/*J38F*/     for each sob_det exclusive-lock
/*J38F*/         where sob_nbr  = sod_det.sod_nbr
/*J38F*/         and   sob_line = sod_det.sod_line:

/*J38F*/        for each cmt_det exclusive-lock
/*J38F*/            where cmt_indx = sob_det.sob_cmtindx:
/*J38F*/            delete cmt_det.
/*J38F*/        end. /* FOR EACH CMT_DET */

/*J38F*/        assign x3      = string(sob_line)
/*J38F*/               qadkey2 = right-trim(string(sob_nbr,"x(8)") +
/*J38F*/                         string(x3,"x(3)") +
/*J38F*/                         string(sob_feature,"x(43)") + sob_part).
/*J38F*/        for first qad_wkfl
/*J38F*/            fields(qad_key1 qad_key2)
/*J38F*/            where qad_key1 = "sob_det"
/*J38F*/              and qad_key2 = qadkey2:
/*J38F*/        end. /* FOR FIRST QAD_WKFL */
/*J38F*/        if available qad_wkfl then delete qad_wkfl.

/*J38F*/        if (line_pricing or not new_order) and
/*J38F*/            so_db = global_db then do:
/*J38F*/           {gprun.i ""gppiqty2.p"" "(sod_det.sod_line,
                         sob_part,
                         - (sob_qty_req
                        / sod_det.sod_um_conv),
                         - (sob_tot_std * sob_qty_req
                                     / sod_det.sod_um_conv),
                         sod_det.sod_um,
                         yes,
                         yes,
                         yes)"}
/*J38F*/        end.
/*J38F*/        delete sob_det.
/*J38F*/     end. /* FOR EACH SOB_DET */

/*J38F*/     /* DELETE COST SIMULATION DETAIL */
/*J38F*/     for each sct_det exclusive-lock
/*J38F*/         where sct_part = sod_det.sod_part
/*J38F*/         and   sct_sim  = string(sod_det.sod_nbr)
                  + "." + string(sod_det.sod_line):
/*J38F*/         delete sct_det.
/*J38F*/     end. /* FOR EACH SCT_DET */
/*N0WP*/     {&SOSOMTA-P-TAG41}
/*J38F*/  end. /* PORCEDURE P-SOBDEL */

/*M09Y* Added internal procedure to reduce action segment size */
          procedure p-laddel:
                  for each lad_det exclusive-lock where
                     lad_dataset = "sod_det" and
                     lad_nbr     = sod_det.sod_nbr   and
                     lad_line = string(sod_det.sod_line):
                     find ld_det where ld_site = lad_site
                        and ld_loc  = lad_loc
                        and ld_lot  = lad_lot
                        and ld_ref  = lad_ref
                        and ld_part = lad_part
                        exclusive-lock no-error.
                     if available ld_det then
                        ld_qty_all = ld_qty_all -
                        (lad_qty_all + lad_qty_pick).
                     delete lad_det.
                  end.
          end procedure.
/*M09Y* End of new internal procedure */
/*N0WP*/ {&SOSOMTA-P-TAG42}

/*N0CG*/ procedure p-mfmsg:

/*N0CG*/    define input parameter l_num  as integer no-undo.
/*N0CG*/    define input parameter l_stat as integer no-undo.
/*N0CG*/    {mfmsg.i l_num l_stat}

/*N0CG*/ end.  /* PROCEDURE P-MFMSG */

PROCEDURE p-process-emt:
/* ------------------------------------------------------------------
   Purpose:     Queue doc for transmission to SBU if line item deleted
   Parameters:
                p-return-msg Error Message Number
   Notes:       Added with M1HP to reduce compile size and add
                creation of record for document queueing
   ------------------------------------------------------------------*/
   define output parameter p-return-msg like msg_mstr.msg_nbr no-undo.

   define variable use-cmf    as logical no-undo.
   define variable queue-doc  as logical no-undo.

   s-create-pod-line = no.

   find first wkf-btb2 where w2-sodline = sod_det.sod_line
   no-lock no-error.

   if available wkf-btb2 and
      (w2-po-xmit = "2" or w2-po-xmit = "4"
   or (w2-po-xmit = "3" and w2-cmf-status = "x"))
   then
      assign
         queue-doc = yes
         use-cmf = yes.
   else
      assign
         queue-doc = no
         use-cmf = no.

   {gprun.i ""socram01.p""
      "(input sod_det.sod_nbr,
        input sod_det.sod_line,
        input-output sod_det.sod_btb_po,
        input-output sod_det.sod_btb_pod_line,
        input no,
        input yes,
        input sod_det.sod_site,
        input so_mstr.so_ship,
        input sod_det.sod_qty_ord,
        input no,
        input sod_det.sod_due_date,
        input sod_det.sod_due_date,
        input use-cmf,
        output p-return-msg)"}

   s-rollback = no.  /* RESET VARIABLE WHEN ROLLING BACK */

   /* Only continue processing if no error has occurred */
   if p-return-msg = 0 or p-return-msg = 343 then do:

      if w2-po-xmit = "1" and sod_det.sod_btb_po <> "" then
         queue-doc = yes.

      if queue-doc = yes then do:

         find first wkf-btb where
                w-so-nbr   = sod_det.sod_nbr
            and w-po-nbr   = sod_det.sod_btb_po
            and w-pod-line = sod_det.sod_btb_pod_line
         no-lock no-error.

         if not available wkf-btb
            or w-sod-line <> sod_det.sod_line
         then do:
            /* CREATE WORKFILE TO QUEUE DOCUMENT AGAIN */
            create wkf-btb.
            assign
               w-so-nbr   = sod_det.sod_nbr
               w-sod-line = sod_det.sod_line
               w-btb-vend = sod_det.sod_btb_vend
               w-btb-type = sod_det.sod_btb_type
               w-po-nbr   = sod_det.sod_btb_po
               w-pod-line = 0.
         end.  /* not available wkf-btb */

         w-msg-type = "ORDCHG".

      end. /* if queue-doc */

      /* DELETE DIRECT ALLOCATION AT THE SBU SITE */
      if available wkf-btb2 and
         (w2-po-xmit = "1" or w2-po-xmit = "2")
      then do:

         /* SWITCH TO DATABASE OF SECONDARY BUSINESS UNIT */
         {gprun.i ""gpalias2.p""
            "(sod_det.sod_btb_vend, output err_flag)"}

         if err_flag = 0 then do:
            /* SBU DATABASE CONNECTION = OK */
            /* DELETE DIRECT SO ALLOCATIONS BTB ON SBU */
            {gprun.i ""sosoall.p""
               "(input sod_det.sod_nbr,
                 input sod_det.sod_line,
                 input sod_det.sod_part,
                 input prev_qty_ord,
                 input sod_det.sod_um_conv,
                 input sod_det.sod_btb_vend,
                 input yes)"}
         end.      /* if err_flag = 0 */

         /* CHANGE DATABASE BACK TO PRIMARY BUSINESS UNIT */
         {gprun.i ""gpalias3.p""
            "(input so_db,
              output err_flag)"}

      end. /* delete direct allocation */

   end. /* if return-msg */

END PROCEDURE.
