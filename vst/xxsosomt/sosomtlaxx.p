/* sosomtlaxx.p - SALES ORDER MAINTENANCE LINE PRICE/QTY SUBROUTINE            */
/*     copied from sosomtla.p - SALES ORDER MAINTENANCE LINE PRICE/QTY SUBROUTINE*/
...
/* Revision: 1.104.2.1    BY: Sandeep P.       DATE: 05/07/01   ECO: *M159*  */
/* Revision: 1.104.2.3    BY: Jean Miller      DATE: 06/07/01   ECO: *M11Z*  */
/* Revision: 1.104.2.4    BY: Chris Green      DATE: 07/27/01   ECO: *N101*  */
/* Revision: 1.104.2.7    BY: Inna Fox         DATE: 07/31/01   ECO: *M12Q*  */
/* Revision: 1.104.2.8    BY: Inna Fox         DATE: 08/03/01   ECO: *N110*  */
/* Revision: 1.104.2.9    BY: Nikita Joshi     DATE: 08/08/01   ECO: *L16Z*  */
/* Revision: 1.104.2.10   BY: Chris Russell    DATE: 08/17/01   ECO: *M1HN*  */
/* Revision: 1.104.2.14   BY: Inna Fox         DATE: 11/28/01   ECO: *M1LR*  */
/* Revision: 1.104.2.15   BY: Julie Milligan   DATE: 11/28/01   ECO: *N162*  */
/* Revision: 1.104.2.16   BY: Ed van de Gevel  DATE: 12/06/01   ECO: *N16Z*  */
/* Revision: 1.104.2.17   BY: Vivek Dsilva     DATE: 12/10/01   ECO: *M1RY*  */
/* Revision: 1.104.2.18   BY: Rajiv Ramaiah    DATE: 01/04/02   ECO: *M1SX*  */
/* $Revision: 1.104.2.19 $ BY: Ashish M.       DATE: 02/15/02   ECO: *N193*  */
/*                                                                           */
...
/*N0CG*/ /* CREATED INTERNAL PROCEDURES TO REMOVE ACTION SEGMENT ERROR */
         ...
         {mfdeclre.i}
/*L18P*/ {cxcustom.i "SOSOMTLA.P"}

/*L18P*  {gplabel.i &ClearReg=yes} /* EXTERNAL LABEL INCLUDE */ */
/*L18P*/ {gplabel.i}

/*M12Q*/ {gprunpdf.i "mcpl" "p"}
         ...
         &SCOPED-DEFINE sosomtla_p_4 " Enterprise Material Transfer Data "
         /* MaxLen: Comment: */
...
/*N0DX*
 *        &SCOPED-DEFINE sosomtla_p_7 "Item Not In Inventory"
 *        /* MaxLen: Comment: */
 *N0DX*/
...
/*L18P*  define            variable return-to-remove-isb as logical no-undo. */
...
/*L18P*  define            variable minprice        like pc_min_price. */
/*L18P*  define            variable maxprice        like pc_min_price. */
...
/*L18P*  define            variable pc_recno        as recid. */
...
/*N162*/ define variable apoAtpDelAvail           as logical no-undo
   initial yes.
/*N162*/ define variable apoAtpdelAvailMsg        as integer no-undo.
/*M12Q*/ define variable apoDemandId              as character no-undo.
/*M12Q*/ define variable apoEarliestDate          like sod_due_date no-undo.
/*M1LR*/ define variable apoErrorLevel            as character no-undo.
...
/*M12Q*/ define variable useApoAtpForLine         as logical no-undo.
/*L16Z*/ define variable l_sod_list_pr            like sod_list_pr no-undo.
/*L16Z*/ define variable l_sod_price              like sod_price   no-undo.
/*L16Z*/ define stream listprice.
/*M12Q*/ define new shared stream apoAtpStream.
...
/*M12Q*/ pause 0.
/*M12Q*/ run setUpForApoAtp (output useApoAtp,
                             output moduleGroup).
...
/*L18P*/ {&SOSOMTLA-P-TAG2}
   if sngl_ln then view frame d.

/*N0CG**   find so_mstr where recid(so_mstr) = so_recno. */
/*N0CG*/   find first so_mstr
/*N0CG*/      where recid(so_mstr) = so_recno exclusive-lock no-error.

/*N0CG**   find sod_det where recid(sod_det) = sod_recno. */
/*N0CG*/   find first sod_det
/*N0CG*/      where recid(sod_det) = sod_recno exclusive-lock no-error.
...
/*M12Q*  Moved code to internal procedure set-rma-restock-pct          *
 *            find rma_mstr where recid(rma_mstr) = rma-recno
 *             exclusive-lock no-error.
 *
 *             restock-pct = rma_rstk_pct.
 * /*N0CG**    find eu_mstr where eu_addr = rma_enduser no-lock no-error. */
 ...
 *M12Q*             restock-amt = rmd_restock.  */

/*N0CG*/ /* COMMENTED THE BELOW FIND FIRST ON RMC_CTRL SINCE IT IS NOT */
/*N0CG*/ /* BEING USED                                                 */
/*N0CG**    find first rmc_ctrl no-lock no-error.                      */

/*M12Q*/    find rma_mstr where recid(rma_mstr) = rma-recno
            exclusive-lock no-error.

/*M12Q*/    run set-rma-restock-pct.

/*M12Q*/    find rmd_det where recid(rmd_det) = rmd-recno
               exclusive-lock no-error.
         end.

/*N0CG**    find first soc_ctrl no-lock. */
/*N0CG*/    for first soc_ctrl
/*N0CG*/       fields(soc_all_avl soc_apm soc_edit_isb soc_lcmmts
/*N0CG*/       soc_returns_isb soc_use_btb) no-lock:
...
/*M11Z*/ run getPriceTableRequiredFlag.
/*M11Z*/ /*********
 *        /* Read price table required flag from mfc_ctrl */
 * /*N0CG** find first mfc_ctrl where mfc_field = "soc_pt_req" no-lock no-error. */
 * /*N0CG*/ for first mfc_ctrl
 * /*N0CG*/    fields(mfc_char mfc_field mfc_logical)
 * /*N0CG*/    where mfc_field = "soc_pt_req" no-lock:
...
/*M11Z*/ run setOrderLineAndDesc.
/*M11Z*/ /*********************
 * /*J2FH*/ assign
 * /*J2FH*/     line = sod_line
 * /*N0DX* /*J2FH*/     desc1 = {&sosomtla_p_7} */
 ...
 * /*N0CG*/    fields(pt_desc1 pt_desc2 pt_fsc_code pt_isb pt_lot_ser
 * /*N0CG*/           pt_ord_max pt_ord_min pt_ord_mult pt_part pt_pm_code
 * /*N0CG*/           pt_price pt_promo pt_ship_wt pt_ship_wt_um pt_um)
 ...
 *          if v_picked then do:
 * /*N0CG**   {mfmsg.i 2023 3}  /* INVENTORY PICKED AT SUPPLIER */  */
 * /*N0CG*/   /* INVENTORY PICKED AT SUPPLIER */
 * /*N0CG*/   run p-mfmsg (input 2023, input 3).
 *
 ...
 * /*N0CG**    {mfmsg.i 2859 3} /* CANNOT CHANGE BTB SALES ORDER LINE */ */
 * /*N0CG*/    /* CANNOT CHANGE BTB SALES ORDER LINE */
 * /*N0CG*/    run p-mfmsg (input 2859, input 3).
 *
 *             if not batchrun then pause.
...
/*M11Z*/ run saveCurrentQtyPricingInfo.
 ...
 *                                       no-lock no-error.
 * *N0CG** END DELETE **/
 * /*N0CG*/       for first pih_hist
 * /*N0CG*/          fields(pih_amt pih_amt_type pih_bonus_line pih_doc_type
 * /*N0CG*/                 pih_feature pih_line pih_nbr pih_option pih_parent
 * /*N0CG*/                 pih_promo1 pih_source)
 * /*N0CG*/          where pih_doc_type = 1        and
 * /*N0CG*/                pih_nbr      = sod_nbr  and
 * /*N0CG*/                pih_line     = sod_line and
 * /*N0CG*/                pih_parent   = ""       and
 * /*N0CG*/                pih_feature  = ""       and
 * /*N0CG*/                pih_option   = ""       and
 * /*N0CG*/                pih_amt_type = "1" no-lock:
 * /*N0CG*/        end. /* FOR FIRST PIH_HIST */
 *
...
/*N0CG*  find pt_mstr no-lock where pt_part = sod_part no-error. */
/*N0CG*/ for first pt_mstr
/*N0CG*/    fields(pt_desc1 pt_desc2 pt_fsc_code pt_isb pt_lot_ser
/*N0CG*/           pt_ord_max pt_ord_min pt_ord_mult pt_part pt_pm_code
/*N0CG*/           pt_price pt_promo pt_ship_wt pt_ship_wt_um pt_um)
...
/*M12Q* /*J2D8*/   run find-ptp-j2d8 (input pt_part, input sod_site). */
/*M12Q*/     run find-ptp-det (input pt_part, input sod_site).
          end.
...
/*M017*/          {gprun.i ""sosoapm2.p"" "(input cm_addr,
                 input sod_nbr,
                 input sod_line,
                             output error_flag)"}
...
/*M017*/    end.  /* IF AVAILABLE PT_MSTR AND SOC_APM */
            ...
            {gpaud.i &uniq_num1 = 01  &uniq_num2 = 02
               &db_file = sod_det &db_field = sod_nbr}
            /*ADM1 end*/
...
/*M11Z*/       run rollBackValues.

/*M11Z *** BEGIN DELETE  - Moved to Internal Procedure section
...
/*N05D**          undo set1, leave. */
               ...
               display
                  sod_qty_ord
               with frame c.
/*1312*/        assign sod__chr10 = "HD" when sod__chr10 = "".
               if this-is-rma and not rma-issue-line then
                  display (sod_qty_ord * -1) @ sod_qty_ord
                  with frame c.
...
/*N0CG**          {mfmsg.i 685 1} /* Unit of measure change not allowed */ */
/*N0CG*/          /* UNIT OF MEASURE CHANGE NOT ALLOWED */
/*N0CG*/          run p-mfmsg (input 685, input 1).
               ...
               then do:
                  ...
                  /* ITEM */
/*N0CG**          {mfmsg.i 367 3}  */
/*N0CG*/          run p-mfmsg (input 367, input 3).
                  next-prompt sod_um with frame c.
                  undo set1,retry.
...
/*N0CG**          {mfmsg.i 2854 3} /* Returns not allowed for BTB SO */ */
/*N0CG*/          /* RETURNS NOT ALLOWED FOR BTB SO */
/*N0CG*/          run p-mfmsg (input 2854, input 3).
...
/*N0CG**          {mfmsg.i 2825 3} /* Change not allowed for BTB SO */ */
/*N0CG*/          /* CHANGE NOT ALLOWED FOR BTB SO */
/*N0CG*/          run p-mfmsg (input 2825, input 3).
                  undo set1, retry set1.
               end.
 ...
 * /*N0CG*                       find ptp_det where ptp_part = sod_part    */
 * /*N0CG*                       and ptp_site = sod_site no-lock no-error. */
 * /*N0CG*/                      for first ptp_det
 * /*N0CG*/                      fields(ptp_ord_max ptp_ord_min ptp_ord_mult
 * /*N0CG*/                             ptp_part ptp_pm_code ptp_site)
 * /*N0CG*/                      where ptp_part = sod_part
 * /*N0CG*/                        and ptp_site = sod_site no-lock:
 * /*N0CG*/                      end. /* FOR FIRST PTP_DET */
 *
 ...
 * /*N0CG*/                         for first pt_mstr
 * /*N0CG*/                         fields(pt_desc1 pt_desc2 pt_fsc_code pt_isb
 * /*N0CG*/                          pt_lot_ser pt_ord_max pt_ord_min
 * /*N0CG*/                          pt_ord_mult pt_part pt_pm_code pt_price
 * /*N0CG*/                          pt_promo pt_ship_wt pt_ship_wt_um pt_um)
 * /*N0CG*/                        where pt_part = sod_part no-lock:
 * /*N0CG*/                        end. /* FOR FIRST PT_MSTR */
 *
 ...
 **************************/ /*L18P*/
                        ...
                        /* CHECK IF ORDER QTY CORRESPONDS TO ORDER MODIFIERS */
/*L18P*/                warning = no.
/*L18P*/                if soc_use_btb and sod_btb_type <> "01" and
/*L18P*/                           sod_qty_ord > 0 then do:
/*L18P*/                   run check-order-modifiers
/*L18P*/                      (output warning).

                           if warning = yes then do:
/*N0CG**                      {mfmsg.i 2811 2} */
/*N0CG*/                      run p-mfmsg (input 2811, input 2).
                              /* Qty is not according to the Order Modifiers */
                              if not batchrun then pause.
                           end.
/*L18P*/                end.

/*L18P*                 end. /* if soc_use_btb */ */
               ...
               if this-is-rma then do:
                    if sod_qty_ord < 0 then do:
/*N0CG**               {mfmsg.i 234 3}  /* ORDER QTY CANNOT BE NEGATIVE */ */
/*N0CG*/               /* ORDER QTY CANNOT BE NEGATIVE */
/*N0CG*/               run p-mfmsg (input 234, input 3).
...
/*N0CG*/               run p-mfmsg (input 7201, input 2).
                    end.
              end.  /* if this-is-rma */

/*L18P*/      {&SOSOMTLA-P-TAG7}
              /* CONFIRM DELETE */
...
/*N0CG**                   find rtsbuff                         */
/*N0CG*/                   for first rtsbuff
/*N0CG*/                      fields(rmd_chg_type rmd_edit_isb rmd_line rmd_nbr
/*N0CG*/                             rmd_prefix rmd_restock rmd_rma_line
/*N0CG*/                             rmd_rma_nbr rmd_sv_code)
                              where rtsbuff.rmd_nbr    = rmd_det.rmd_rma_nbr
                              and   rtsbuff.rmd_prefix = "V"
                              and   rtsbuff.rmd_line   = rmd_det.rmd_rma_line
/*N0CG**                      no-lock no-error.      */
/*N0CG*/                      no-lock:
/*N0CG*/                   end. /* FOR FIRST RTSBUFF */

                          if available rtsbuff then do:
/*N0CG**                     {mfmsg.i 1120 2}  */
                         /* THIS RMA LINE IS LINKED TO ONE OR MORE RTS LINES */
/*N0CG*/                     run p-mfmsg (input 1120, input 2).
                           end.
                        end.    /* else, rma-receipt-line, do */
                 end.   /* if this-is-rma */
                 del-yn = yes.
/*M12Q*          {mfmsg01.i 11 1 del-yn} */
/*N110*  Modified p-mfmsg01 call to have del-yn be an input-output parameter */
/*M12Q*/         run p-mfmsg01 (input 11,
                                input 1,
                                input-output del-yn).
                 if sod_qty_inv <> 0 and del-yn then do:
/*N0CG**            {mfmsg.i 604 3} /* Outstanding qty to inv, */  */
/*N0CG*/            run p-mfmsg (input 604, input 3).
                    undo.           /* delete not allowed.     */
                 end.

/*N049*/         /* Do not allow user delete of Sales Orders */
/*N049*/         /* Which are owned by an external system. */
/*N049*/         if so_app_owner > "" then do:
/*N049*/            /* Cannot Process.  Document owned by application # */
/*N0CG** /*N049*/   {mfmsg03.i 2948 4 so_app_owner """" """"}  */
/*N0CG*/            run p-mfmsg03 (input 2948,
                                   input 4,
                                   input so_app_owner,
                                   input "",
                                   input "").
 ...
 *                  end.
 *N0CG** END DELETE **/
/*N0CG*/            run p-delete-cmt-det.
                 end.
              end.
/*L18P*/      {&SOSOMTLA-P-TAG8}
            ...
            pause 0.
/*N0CG**    {mfmsg.i 6014 1} /* Schedule exists for this order line */  */
/*N0CG*/    /* SCHEDULE EXISTS FOR THIS ORDER LINE */
/*N0CG*/    run p-mfmsg (input 6014, input 1).
         end.
...
/*M11Z*/    run checkWhetherToChangeDB.
...
/*M11Z *** BEGIN DELETE  - Moved to checkWhetherToChangeDB Internal Procedure *
 *           if chg-db then do:
...
/*M12Q*  Moved to internal procedure p-get-ex-rate *
 ...
 *                  output exch-rate2,
 *                  output exch-rate,
 *M12Q*             output mc-error-number)" }  */
/*M12Q*/    run p-get-ex-rate.

/*M0WD*/    if mc-error-number <> 0 then
/*M0WD*/    do:
/*M0WD*/       run p-mfmsg (input mc-error-number, input 2).
/*M0WD*/    end. /* IF mc-error-number <> 0 */

/*N101*/    {&SOSOMTLA-P-TAG23}
            if new sod_det or (not new sod_det and pt_pm_code <> "C")
/*N101*/    {&SOSOMTLA-P-TAG24}
        then
...
/*M12Q* /*J2D8*/ run find-ptp-j2d8 (input sod_part, input sod_site). */
/*M12Q*/    run find-ptp-det (input sod_part, input sod_site).
...
/*M12Q* /*M11Z*/    {mfmsg.i 4638 4} */
/*M12Q*/   run p-mfmsg (input 4638, input 4).
...
/*M12Q* /*M0X4*/    {mfmsg.i 420 2} */
/*M12Q*/            run p-mfmsg (input 420, input 2).
/*M0X4*/            pause.
...
/*M11Z*/       run reviseManualDiscountAdjustment.
...
/*K234*/          else if sod_list_pr = 0 and sod_price <> 0 then
/*K234*/             sod_covered_amt = sod_price * (coverage-discount / 100).
/*J2VF**          else do: */
/*J2VF*/          else
/*J2VF*/             assign
                        sod_covered_amt = 0
...
/*M11Z*/          run adjustManualDiscountPercent.
...
/*N0CG*/          run p-mfmsg (input 1186, input 1).
               end.
            end.    /* else, rma-receipt-line, do */
         end.   /* if this-is-rma */

/*M1LR* Moved to Internal Procedure determine-discount *
*
*        assign
*           old_price = sod_price
*           old_list_pr = sod_list_pr
...
/*J2VF*/                 validate({gppswd1.v
            ...
            {gpaud2xx.i &uniq_num1 = 01 &uniq_num2 = 02
                      &db_file = sod_det &db_field = sod_nbr &db_field1 = sod_line}
            /*ADM1 end*/
...
/*J2VZ*/                display discount sod_price with frame c.
         ...
         end.  /* do on error undo, retry (for update of sod_list_pr) */
...
/*L18P*/    {&SOSOMTLA-P-TAG16}
/*M1RY**       set sod_price with frame c. */
/*M1RY*/       update
/*M1RY*/          sod_price
/*M1RY*/       with frame c.

/*L18P*/    {&SOSOMTLA-P-TAG17}

            /* Disallow price change if uninvoiced shipment exists */
            if sod_price <> last_sod_price and sod_qty_inv <> 0 then do:
/*N0CG**       {mfmsg.i 546 3} /*"Invoice qty pending, use inv maint"*/ */
/*N0CG*/       /*"INVOICE QTY PENDING, USE INV MAINT"*/
/*N0CG*/       run p-mfmsg (input 546, input 3).
               undo, retry.
            end.

/*L18P*/    {&SOSOMTLA-P-TAG18}
...
/*M12Q* /*M11Z*/         {mfmsg.i return-msg 3} */
/*M12Q*/                 run p-mfmsg (input return-msg, input 3).
/*M11Z*/                 return-msg = 0.
/*M11Z*/                 if not batchrun then pause.
/*M11Z*/                undo, retry.
/*M11Z*/             end.
 ...
 *J37J** END DELETE **/
            ...
            if  rma-issue-line   and
               (sod_list_pr <> 0 or
                sod_bonus)
            then do:
/*L16Z*/       if  (sod_list_pr <> 0
/*L16Z*/       and  sod_list_pr <> l_sod_price)
/*L16Z*/       or   sod_bonus
/*L16Z*/       then
/*N0CG*/       assign
...
/*N193** /*M0TZ*/    and sod_fr_list =  "" */
/*N193*/    and sod__qadl02 = no
/*M0TZ*/ then
...
/*M1HN*     run updateWithApoAtpData. */
/*M1HN*/    if apoEarliestDate <> ? then sod_det.sod_due_date = apoEarliestDate.

/*M12Q End Added Code */

/*M1LR* Moved code lower *
 *       /* SOSOMTLB.P WILL, FOR RMA'S, CREATE/MODIFY THE RMD_DET */
 * /*L18P*/ {&SOSOMTLA-P-TAG20}
 *       {gprun.i ""sosomtlb.p""
 *                 "(input this-is-rma,
 *                   input rma-recno,
 *                   input rma-issue-line,
...
/*M12Q* Added Code*/
         ...
         /* The user can choose to modify the Apo site id in giaposo.p       */
            ...
            if (new_line and sod_confirm = yes and useApoAtp) or
               (prev_confirm = no and sod_confirm = yes and useApoAtp) then do:

               loopvalidate:
               do on error undo, leave:

                  /* Determine what module is being processed */
/*N162*/ /* Add fields apoAtpDelAvail and apoAtpDelAvailMsg */
/*M1LR Added output for apoErrorLevel, Removed apoCompErrorList */
                     {pxrun.i &proc = 'validateApoAtp' &program = 'sosoxr1.p'
                        &handle=ph_sosoxr1
                        &param = "(input moduleGroup,
                                   ...
                                   output useApoAtpForLine,
                                   output apoConnected,
                                   output apoDemandId,
                                   output apoErrorLevel)"
                        &catcherror = TRUE
                        &noapperror = TRUE
                        }

/*M1LR* Added Code */
                  /* If an error/warning occured and the user      */
                  /* pressed F4 then the quote needs to be removed */
                  /* from the APO ATP model.                       */
                  if (keyfunction(lastkey) = "END-ERROR")
                  and apoErrorLevel = {&APP-ERROR-RESULT}
                  then do:
                     {pxrun.i &proc = 'deleteDemandRecord'
                        &program = 'giapoxr.p'
                        &handle = ph_giapoxr
                        &param = "(input sod_site,
                                   input sod_part,
                                   input apoDemandId,
...
/*M1LR* End Added Code */
                  ...
                  /* If there was a connection problem to the APO ATP */
                  /* model user can attempt to connect again.         */
                  if not batchrun then do:
                     if not apoConnected and useApoAtpForLine then do:
                        /* Continue with procedure? */
                        confirm_yn = yes.
/*N110*  Modified p-mfmsg01 call to have del-yn be an input-output parameter */
                        run p-mfmsg01 (input 3140,
                                       input 1,
                                       input-output confirm_yn).

/*M1HN*                 if not confirm_yn then */
/*M1HN*/                if (not confirm_yn) or
/*M1HN*/                   (keyfunction(lastkey) = "END-ERROR") then
                           undo loopdet, retry.
                     end.
                  end.

/*M1LR* Removed section and moved to sosoxr1.p *
 *                if apoCompErrorList <> "" then do:
 *                   /* These components did not exist in the APO ATP Model */
 *                   run p-mfmsg03 (input 4623,
 *                                  input 2,
 *                                  input substring(apoCompErrorList,2),
 *                                  input "",
 *                                  input "").
 *
 *                   if not batchrun then pause.
 ...
 *M1LR */
                  ...
                  /* Demand insufficient */
                  if return-value <> {&SUCCESS-RESULT} and apoConnected then do:
                     hide frame c.
                     hide frame d.
                     ...
                     if keyfunction(lastkey) = "end-error":U then do:
                        ...
                        if return-value <> {&SUCCESS-RESULT} then do:
                           run p-mfmsg (input apoMessageNumber, input 2).
                        end.

/*M1LR* Added Code */
            assign apoEarliestDate = sod_due_date
                   apoSiteId = sod_site.
/*M1LR* End Added Code */
                     ...
                     {pxrun.i &proc='processResultsFromApoInsufficient'
                        &program = 'sosoxr1.p'
                        &handle=ph_sosoxr1
...
/*M1SX*/    if parent_list_price = 0
/*M1SX*/    then
/*M1SX*/       parent_list_price = best_list_price.

/*N014*     Inserted sod_dsc_sub in parameter list below                    */
/*N014*     Changed sod_project to sod_dsc_project in parameter list below  */
            {gprun.i ""gppiwk02.p"" "(
                                      1,
                                      sod_nbr,
                                      sod_line,
...
/*M11Z*/    {gprunp.i "soemttrg" "p" "process-order-detail"
                 "(input new_line,
                   ...
                   output return-msg)"}
/*M11Z*/    if return-msg <> 0 then do:
/*M12Q* /*M11Z*/       {mfmsg.i return-msg 4} */
/*M12Q*/       run p-mfmsg (input return-msg, input 4).
/*M11Z*/       return-msg = 0.
/*M11Z*/       if not batchrun then pause.
...
/*M12Q* Moved to internal procedure get-btb-mneumonic *
 *         {gplngn2a.i &file = ""emt""
 *                   &field = ""btb-type""
 *                   &code = sod_btb_type
 *                   &mnemonic = btb-type
 *M12Q*              &label = btb-type-desc}          */
/*M12Q*/    run get-btb-mneumonic.

/*N0CG**    find pod_det where pod_nbr = sod_btb_po              */
/*N0CG**    and pod_line = sod_btb_pod_line no-lock no-error.    */
/*N0CG*/    for first pod_det
/*N0CG*/       fields(pod_cmtindx pod_due_date pod_line pod_nbr
/*N0CG*/              pod_need pod_so_status)
/*N0CG*/       where pod_nbr = sod_btb_po
/*N0CG*/        and pod_line = sod_btb_pod_line no-lock:
/*N0CG*/    end. /* FOR FIRST POD_DET */
 ...
 * /*K20S** BEGIN DELETE **
 * *             find po_mstr where po_so_nbr = sod_nbr and po_is_btb
 * *             no-lock no-error.
 * *             if avail po_mstr then do:
 * * /*K1YM**     find pod_det where pod_nbr = po_nbr and pod_line = sod_line */
 * * /*K1YM*/     find pod_det where pod_nbr  = po_nbr
 * *K20S** END DELETE **/
 *
 * /*K20S*/          find pod_det
 * /*K20S*/             where pod_nbr  = sod_btb_po
...
/*M12Q*     {gprun.i  ""sosomtu1.p""} */
/*M12Q*/    run confirmOrder (input apoDemandId).
         end.
...
/*M1LR*  Begin Add Section */

         PROCEDURE determine-discount:

            assign
               old_price = sod_det.sod_price
               old_list_pr = sod_list_pr
               old_disc = sod_disc_pct.
            ...
            /* THE VIOLATION.                                                */
               ...
               run p-disc-disp (input yes).
               if disc_min_max then do:     /* found a discount range violation */
            /* THE FOLLOWING ASSIGNMENTS WERE MOVED TO A SINGLE ASSIGN STATEMENT */
            /* DUE TO ACTION SEGMENT VIOLATION.                                  */

                  assign save_disc_pct = disc_pct_err
                  new_disc_pct  = if pic_ctrl.pic_so_fact then
                                     (1 - discount) * 100
                                  else
                                           discount
                  sod_disc_pct = new_disc_pct.
                  find first wkpi_wkfl where wkpi_wkfl.wkpi_parent   = sobparent  and
                                             wkpi_feature  = sobfeature and
                                             wkpi_option   = sobpart    and
                                             wkpi_amt_type = "2"        and
                                             wkpi_source   = "1"
         ...
         END PROCEDURE. /* determine-discount*/

/*M1LR*  End Add Section */

/*M12Q*  Begin Add Section */

         PROCEDURE set-rma-restock-pct:
         ...
         END PROCEDURE. /* set-rma-restock-pct */

/*M12Q*  End Add Section */

/*M11Z*/ /* BEGIN ADD SECTION  */
...
/*N0CG**    find sod_det where sod_nbr  = sales_nbr and sod_line = line_nbr */
/*N0CG**    no-lock.                                                        */

/*N0CG*/    for first sod_det
/*N0CG*/       where sod_nbr = sales_nbr
/*N0CG*/         and sod_line = line_nbr no-lock:
/*N0CG*/    end. /* FOR FIRST SOD_DEET */
...
/*N0CG*/          for first pih_hist
/*N0CG*/             fields(pih_amt pih_amt_type pih_bonus_line
/*N0CG*/                    pih_doc_type pih_feature pih_line pih_nbr
/*N0CG*/                    pih_option pih_parent pih_promo1 pih_source)
/*N0CG*/             where pih_doc_type = 1         and
/*N0CG*/                   pih_nbr      = sod_nbr   and
/*N0CG*/                   pih_line     = sod_line  and
/*N0CG*/                   pih_source   = "1"       and
/*N0CG*/                   pih_amt_type = "2" no-lock:
/*N0CG*/          end. /* FOR FIRST PIH_HIST */
...
/*N0CG**       {mfmsg03.i 6932 3 disc_pct_err """" """"}  */
               /* DISCOUNT # VIOLATES THE MIN OR MAX ALLOWABLE */
/*N0CG*/       run p-mfmsg03 (input 6932,
                              input 3,
                              input disc_pct_err,
                              input "",
                              input "").
...
/*N0CG** /*J39H*/    {mfmsg03.i 6231 4 sod_det.sod_part sod_det.sod_um """"} */
/*N0CG*/             run p-mfmsg03 (input 6231,
                                    input 4,
...
/*J39H*/             then do:
/*J39H*/                /* REQUIRED PRICE TABLE FOR ITEM # IN UM # NOT FOUND */
/*N0CG** /*J39H*/   {mfmsg03.i 6231 2 sod_det.sod_part sod_det.sod_um """"} */
/*N0CG*/                run p-mfmsg03 (input 6231,
                                       input 2,
                                       input sod_det.sod_part,
                                       input sod_det.sod_um,
...
/*N0CG*/    /* COMMENTED THE BELOW FIND ON SO_MSTR, SOD_DET AND QUALIFIED  */
/*N0CG*/    /* THE FIELDS SO_PRIMARY, SOD_NBR AND SOD_LINE WITH THEIR      */
/*N0CG*/    /* RESPECTIVE TABLE                                            */
/*N0CG**    find so_mstr where recid(so_mstr) = so_recno no-lock no-error. */
/*N0CG**    find sod_det where recid(sod_det) = sod_recno no-lock no-error. */
...
/*N0CG**       find pod_det where pod_nbr  = sod_btb_po      */
/*N0CG**                    and pod_line = sod_btb_pod_line  */
/*N0CG**                    no-lock no-error.                */
/*N0CG*/       for first pod_det
/*N0CG*/          fields(pod_cmtindx pod_due_date pod_line pod_nbr pod_need
/*M11Z*/                 pod_status
/*N0CG*/                 pod_so_status)
/*N0CG*/          where pod_nbr  = sod_det.sod_btb_po
/*N0CG*/            and pod_line = sod_det.sod_btb_pod_line no-lock:
/*N0CG*/       end. /* FOR FIRST POD_DET */

/*M11Z*/       find po_mstr where po_nbr = sod_det.sod_btb_po no-lock no-error.
...
/*M12Q* /*M11Z*/        {mfmsg.i p-errnum l-errsev} */
/*M12Q*/        run p-mfmsg (input p-errnum, input l-errsev).
/*M11Z*/        if not batchrun then pause.
/*M11Z*/        p-error = yes.
/*M11Z*/     end.
...
/*M11Z*/        hide message no-pause.
/*M12Q* /*M11Z*/        {mfmsg01.i p-errnum l-errsev yn} */
/*N110*  Modified p-mfmsg01 call to have del-yn be an input-output parameter */
/*M12Q*/        run p-mfmsg01 (input p-errnum,
                               input l-errsev,
                               input-output yn).
/*M11Z*/        if yn then
/*M11Z*/           p-error = no.
...
/*N0CG** /*H1HQ*/    {mfmsg.i 669 2} */
/*N0CG*/             run p-mfmsg (input 669, input 2).
...
/*N0CG** /*H1HQ*/ find mfc_ctrl where mfc_field = "frc_returns" no-lock */
/*N0CG**          no-error.                                             */
/*N0CG*/       for first mfc_ctrl
/*N0CG*/          fields(mfc_char mfc_field mfc_logical)
/*N0CG*/          where mfc_field = "frc_returns" no-lock:
/*N0CG*/       end. /* FOR FIRST MFC_CTRL */
...
/*M12Q*  PROCEDURE find-ptp-j2d8: */
/*M12Q*/ PROCEDURE find-ptp-det:
...
/*N0CG*/    for first ptp_det
/*N0CG*/       fields(ptp_ord_max ptp_ord_min ptp_ord_mult ptp_part
/*N0CG*/              ptp_pm_code ptp_site)
/*N0CG*/       where ptp_part = inpar_part
/*N0CG*/         and ptp_site = inpar_site no-lock:
/*N0CG*/    end. /* FOR FIRST PTP_DET */
...
/*M12Q* /*J2VF*/    {mfmsg10.i 4072 4 l_err_msg """" """" """" } */
/*M12Q*/    {pxmsg.i &MSGNUM     = 4072
                     &MSGBUFFER  = l_err_msg
...
/*N0CG** /*J37J*/     find pi_mstr where pi_list_id = wkpi_list_id no-lock. */
/*N0CG*/             for first pi_mstr
/*N0CG*/                fields(pi_cost_set pi_list_id)
/*N0CG*/                where pi_list_id = wkpi_list_id no-lock:
...
/*M12Q* /*L124*/           then {mfmsg.i mc-error-number 2}. */
/*M12Q*/                   then do:
                              {pxmsg.i
...
/*L16Y*/    end. /* FOR EACH wkpi_wkfl */

/*L16Z*/    /* DISPLAYING sod_list_pr AND RE-ASSIGNING THE SCREEN VALUE */
/*L16Z*/    /* TO AVOID ROUNDING ERRORS                                 */
/*L16Z*/    l_sod_list_pr = sod_det.sod_list_pr.

/*L16Z*/    output stream listprice to value(mfguser + "l.out").
/*L16Z*/    put stream listprice sod_det.sod_list_pr skip.
/*L16Z*/    output stream listprice close.

/*L16Z*/    input stream listprice from value(mfguser + "l.out").
/*L16Z*/    import stream listprice sod_det.sod_list_pr.
/*L16Z*/    input stream listprice close.

/*L16Z*/    if sod_det.sod_list_pr <> sod_det.sod_price
/*L16Z*/    then
/*L16Z*/       sod_det.sod_list_pr = l_sod_list_pr.

/*L16Z** /*J37J*/       if sod_det.sod_list_pr <> 0 then */
/*L16Z*/    if  sod_det.sod_list_pr <> 0
/*L16Z*/    and sod_det.sod_list_pr <> l_sod_price
/*L16Z*/    then
...
/*N0CG** /*J3GV*/      {mfmsg03.i 6231 4 sob_part sod_um """"}  */
/*N0CG*/                run p-mfmsg03 (input 6231,
                                       input 4,
                                       input sob_part,
                                       input sod_um,
...
/*N0CG** /*J3K7*/       {mfmsg03.i 1118 3 shipper_found save_abs """"}  */
/*N0CG*/                run p-mfmsg03 (input 1118,
                                       input 3,
                                       input shipper_found,
                                       input save_abs,
                                       input "").
...
/*N0CG** /*J3K7*/       {mfmsg03.i 3314 2 l_conf_ship l_conf_shid """"}  */
/*N0CG*/       run p-mfmsg03 (input 3314,
                              input 2,
                              input l_conf_ship,
                              input l_conf_shid,
                              input "").
...
/*M12Q* /*N0CG*/    {mfmsg.i l_num l_stat} */
/*M12Q*/    {pxmsg.i
                &MSGNUM = l_num
                &ERRORLEVEL = l_stat}
...
/*N110*  Modified parameter to be an input-output */
            define input-output parameter l_yn   as logical no-undo.

            {pxmsg.i &MSGNUM     = l_num &ERRORLEVEL = l_stat
                    &CONFIRM  = l_yn &CONFIRM-TYPE = 'LOGICAL'}

         END PROCEDURE.  /* PROCEDURE P-MFMSG01 */

/*M12Q*  End Add Code */

/*N0CG*/ procedure p-mfmsg03:

/*N0CG*/    define input parameter l_num     as integer   no-undo.
/*N0CG*/    define input parameter l_stat    as integer   no-undo.
/*N0CG*/    define input parameter l_msgarg1 as character no-undo.
/*N0CG*/    define input parameter l_msgarg2 as character no-undo.
/*N0CG*/    define input parameter l_msgarg3 as character no-undo.
/*M12Q* /*N0CG*/    {mfmsg03.i l_num l_stat l_msgarg1 l_msgarg2 l_msgarg3} */
/*M12Q*/    {pxmsg.i
                &MSGNUM = l_num
                &ERRORLEVEL = l_stat
                &MSGARG1 = l_msgarg1
...
/*N0CG*/ end.  /* PROCEDURE P-MFMSG03 */

/*N0CG*/ procedure p-gppiwkad:

/*N0CG*/    {gprun.i ""gppiwkad.p"" "(sod_det.sod_um,
                                      sobparent,
                                      sobfeature,
                                      sobpart,
                                      ""1"",
...
/*N0CG*/ end. /* PROCEDURE P-GPPIWKAD */

/*N0CG*/ procedure p-delete-cmt-det:
/*N0CG*/    /*DELETE COMMENTS*/
/*N0CG*/    for each cmt_det
/*N0CG*/       where cmt_indx = sod_det.sod_cmtindx
/*N0CG*/       exclusive-lock:
/*N0CG*/       delete cmt_det.
/*N0CG*/    end. /* FOR EACH CMT_DET */
/*N0CG*/ end. /* PROCEDURE P-DELETE-CMT-DET */

/*N0CG*/ procedure p-gpmnmx01:
/*N0CG*/    define input parameter l_warning like mfc_logical no-undo.

/*N0CG*/    {gprun.i ""gpmnmx01.p"" "(
                                      l_warning,
                                      yes,
                                      min_price,
                                      max_price,
...
/*N0CG*/ end. /* PROCEDURE P-GPMNMX01 */
   ...
   define input parameter pApoDemandId as character no-undo.

   define variable apoMessageNumber as integer no-undo.

   {gprun.i  ""sosomtu1.p""}

   if useApoAtpForLine and apoConnected then do:
      ...
      if return-value <> {&SUCCESS-RESULT} then do:
        {pxmsg.i
            &MSGNUM = apoMessageNumber
            &MSGARG1 = pApoDemandId
            &ERRORLEVEL = {&WARNING-RESULT}}
      end.
   end.
...
/* ---------------------------------------------------------------------------
Purpose:       This procedure determines is APO ATP is in use.  if it is in use
               then open then io stream used for APO ATP.
Exceptions:    None
Conditions:
Pre:
Post:
...
----------------------------------------------------------------------------*/
   ...
   define output parameter useApoAtp as logical.
   define output parameter moduleGroup as character.

/*N162*/   define variable apoAtpDelAvailField as character no-undo.
   ...
   {pxrun.i &proc = 'setUseApoAtpFlag' &program = 'sosoxr1.p'
         ...
         &param = "(input moduleGroup,
                    input this-is-rma,
                    input-output useApoAtp)"
   }

   /* A Shared Stream cannot be used with app server logic */
   if useApoAtp then do:

/*N162*/ /* BEGIN ADD */
      /* Validate that the demand listener program can be found */
...
/*M1HN* PROCEDURE get-btb-mnemonic: */
/*M1HN*/ PROCEDURE get-btb-mneumonic:
...
END PROCEDURE. /* get-btb-mneumonic */

PROCEDURE p-get-ex-rate:
