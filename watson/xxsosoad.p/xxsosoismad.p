      ...
      do transaction:
         ...
         */
         ...
         FOR EACH sod_det WHERE sod_domain = GLOBAL_domain
            ...
            /* 对类型为退货行的记录，默认备料量为（订单量 - 发运量).
               因为在显示to ship的数量时。引用了fill_all的逻辑 */
            IF (sod_order_category MATCHES "*" + v_mfc_char + "*" ) THEN DO:
