...
with field {3} value {2} using index {4}.
*/
...
then do:
   if recno = ? then
      find first {1} WHERE tt9_type = "CA" AND tt9_msg <> "" AND  {3} > input {2}
      use-index {4} no-lock no-error.
   else
      find next {1} WHERE tt9_type = "CA" AND tt9_msg <> "" use-index {4} no-lock no-error.
   if not available {1} then do:
      ...
      if recno <> ? then
         ...
         find last {1} WHERE tt9_type = "CA" AND tt9_msg <> "" use-index {4}
         no-lock no-error.
      input clear.
   end.
   recno = recid({1}).
...
then do:
   if recno = ? then
      find last {1} where tt9_type = "CA" AND tt9_msg <> "" AND {3} < input {2}
      
      use-index {4} no-lock no-error.
   else
      find prev {1} WHERE tt9_type = "CA" AND tt9_msg <> "" 
      use-index {4} no-lock no-error.
   if not available {1} then do:
      ...
      if recno <> ? then
         ...
         find first {1} WHERE tt9_type = "CA" AND tt9_msg <> "" use-index {4}
         no-lock no-error.
      input clear.
   end.
   recno = recid({1}).
