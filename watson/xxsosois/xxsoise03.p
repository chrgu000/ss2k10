...
no-lock:
   ...
   if not is_mstr.is_overissue
      ...
      and (not available ld_det
           or  ("ISS-FAS" begins "I"
                and ld_det.ld_qty_oh < (sr_qty)))
   then do:

      do on endkey undo, leave:

         /* MAKE SURE SUFFICIENT QTY */
         v_msg-arg = if available ld_det
                   then
                      string(ld_qty_oh)
                   else
                      "0".

         {pxmsg.i &MSGNUM=208 &ERRORLEVEL=2 &MSGARG1=v_msg-arg}
         PAUSE .
