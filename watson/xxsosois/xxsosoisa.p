...
by sod_line:
   ...
   do for soddet transaction:
      ...
      then do:
         ...
         /* SS - 20081114.1 - B */
         {gprun.i ""xxsoise03.p"" "(input sod_nbr, input sod_line)"}
         {gprun.i ""xxsoise04.p"" "(input sod_nbr, input sod_line)"}
         /* SS - 20081114.1 - E */
      ...
      /*
      MESSAGE "1: " + STRING(sod_det.sod_qty_ship) + " " + string(sod_det.sod_qty_chg) + " " + 
          string(sod_det.sod_bo_chg) + " " + STRING(sod_det.sod_qty_ord) VIEW-AS ALERT-BOX.
        */
      {mfsotr.i "QTYCHG"}
      ...
      /* SS - 20080812.1 - B */
      /* sod__dec02用于存放发运数量的累计数 */
      IF v_flag_item = NO THEN sod__dec02 = sod__dec02 + sod__qadd01.

      /* 更新so__log01和 sod__qadd02 为初始值 */
      ASSIGN
         sod__log01 = no 
         sod__qadd01 = 0 
         sod__chr10 = "" .

      IF v_flag_item = NO THEN sod_qty_ord    = qty_ord .
      /* SS - 20080812.1 - E */
