                  ...
                  IF sod__log01 = NO THEN DO:
                     ...
                     IF FILL_all THEN DO:
                        ...
                        display
                           ...
                           IF (sod_qty_ord - sod__dec02) > 0 
                           THEN (sod_qty_ord - sod__dec02)
                           ELSE 0 @ v_sod_qty_chg  format "->>>>>9" label "QtyChg"
                           sod_order_category
                           sod__chr10 @ location
                        with frame b.
                        */
                        FOR FIRST tt9 WHERE tt9_line = sod_line
                            AND tt9_part = sod_part NO-LOCK :
                        END.
                        IF AVAIL tt9 THEN DO:
                            display
                               ...
                               sod__dec02  FORMAT "->>>>>9" LABEL "Shipped"
                               (sod_qty_ord - sod__dec02) @ v_backord    format "->>>>>9" label "Qty B/0"
                               tt9_qty_chg @ v_sod_qty_chg  format "->>>>>9" label "QtyChg"
                               sod_order_category
                               sod__chr10 @ location
                            with frame b.
                        END.
                  ...
                  /* 1. 如果sod__log01 = yes ,表示已经在明细项中输入了资料，
                     ...
                     3. sod__chr10临时保存LOC的资料   */
                        ...
                        display
                           ...
                           sod__dec02     FORMAT "->>>>>9" LABEL "Shipped"
                           (sod_qty_ord - sod__dec02) @ v_backord format "->>>>>9" label "Qty B/O"
                           sod_qty_chg @ v_sod_qty_chg    format "->>>>>9" label "QtyChg"
                           sod_order_category
                           sod__chr10 @ location
                        with frame b.
                  END.
            ...
            editing:
               ...
               if frame-field = "line" then do:
                  ...
                  {mfnp01.i sod_det line sod_line sod_nbr
                    ...
                    */
                  {xxmfnp01.i sod_det line sod_line sod_nbr
                          " sod_domain = global_domain and so_nbr " sod_nbrln}
                  /* SS - 20080901.1 - E */
      ...
      do on error undo, retry:
         ...
         IF FILL_all = YES THEN DO:
            IF sod__log01 = NO THEN do:
               ...
               lotserial_qty = (sod_qty_ord - sod__dec02 ) .
               ELSE lotserial_qty = 0.
               */
               FOR FIRST tt9 WHERE sod_line = tt9_line 
                   AND sod_part = tt9_part :
               END.
               IF AVAIL tt9 THEN DO:
                   lotserial_qty = tt9_qty_chg .
               END.
               ELSE lotserial_qty = 0 .
               /* SS - 20090606.1 - E */
            END.
         ...
         IF pt_pm_code = "C" THEN DO:
            IF NOT CAN-FIND( loc_mstr 
               ...
               AND loc_site = site 
               AND loc_loc = location) THEN DO :
               {pxmsg.i &MSGNUM=709 &ERRORLEVEL=3}  /* SITE DOES NOT EXIST */
         ...
         */
         ...
         IF AVAIL pt_mstr THEN DO:
            FOR EACH ps_mstr NO-LOCK WHERE ps_domain = GLOBAL_domain
               AND ps_par = sod_part BREAK BY ps_comp :
               FOR FIRST ld_det NO-LOCK 
                  ...
                  AND ld_ref = lotref :
               END.
               IF (NOT AVAIL ld_det) OR (AVAIL ld_det AND ld_qty_oh < (lotserial_qty * ps_qty_per) ) THEN DO:
                  ...
                  v_flag_pm = YES .                             
               END.
               IF LAST(ps_comp) AND v_flag_pm = YES THEN DO:
                  NEXT-PROMPT lotserial_qty WITH FRAME c .
                  UNDO setlot,RETRY.
               END.   
            END.
         ...
         /* 冲减的数量超过了原有发货量 */
         IF lotserial_qty < 0 AND ( - lotserial_qty > sod__dec02) THEN DO:
      ...
      then do:
         repeat:
            ...
            */
            ...
            /* SS - 100813.1 - B
            IF sod_line = 801 THEN DO:
               MESSAGE 
                  "1 sod_qty_chg: " + STRING(sod_qty_chg)
                  + "; lotserial_qty: " + STRING(lotserial_qty)
                  + "; total_lotserial_qty: " + STRING(total_lotserial_qty)
                  VIEW-AS ALERT-BOX.
            END.
            SS - 100813.1 - E */
            {gprun.i ""xxsoise01.p""
            ...
            IF v2_flag = NO THEN DO:
                 for each sod_det no-lock
                    ...
                    */
                    ...
                    IF sod_line = 801 THEN DO:
                       MESSAGE 
                          "2 sod_qty_chg: " + STRING(sod_qty_chg)
                          + "; lotserial_qty: " + STRING(lotserial_qty)
                          + "; total_lotserial_qty: " + STRING(total_lotserial_qty)
                          VIEW-AS ALERT-BOX.
                    END.
