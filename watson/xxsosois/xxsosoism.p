...
/* $Revision: 1.34.1.81.1.6 $  BY: Micho Yang         DATE: 08/23/08  ECO: *SS - 20080823.1* */
/*-Revision end---------------------------------------------------------------*/
...
/* SS - 20081226.1 - B */
DEFINE NEW SHARED VAR v_flag_item AS LOGICAL .
/* SS - 20090304.1 - B */
DEFINE VAR vv_site LIKE so_site NO-UNDO.
/* SS - 20090304.1 - E */
/* SS - 20081226.1 - E */

/* SS - 20090305.1 - B */
DEFINE VAR vv_nbr LIKE so_nbr NO-UNDO .
/* SS - 20090305.1 - E */

/* SS - 100813.1 - B */
DEFINE VARIABLE oPart AS CHARACTER.
/* SS - 100813.1 - E */
...
/* SS - 20080823.1 - B */
/* 用于存放loc,dn,bill-to,ship-to */
DEF NEW SHARED VAR v_loc LIKE tr_loc .
DEF NEW SHARED VAR v_dn LIKE tr_loc .
DEF NEW SHARED VAR v_bill LIKE so_bill.
DEF NEW SHARED VAR v_ship LIKE so_ship.

/* 默认的 shipall */
fill_all = NO .

/* 取得用户指定的退货行 */
DEF NEW SHARED VAR v_mfc_char AS CHAR.
find first mfc_ctrl where mfc_domain = GLOBAL_domain
and mfc_field = 'cops_return' no-lock no-error.
if avail mfc_ctrl then v_mfc_char = mfc_char .
/* SS - 20080823.1 - E */
...
DEFINE VARIABLE v_tt9_line AS INTEGER FORMAT ">>9" NO-UNDO .
DEFINE VARIABLE v_flag1 AS LOGICAL.
DEFINE VARIABLE v2_um_conv LIKE sod_um_conv .
DEFINE VARIABLE v1_flag AS LOGICAL .
DEFINE VARIABLE v2_flag AS LOGICAL .
DEFINE VARIABLE v1_part LIKE pt_part .

DEFINE NEW SHARED TEMP-TABLE tt9
...
DEFINE BUFFER tt9b FOR tt9.
DEFINE BUFFER lddet11 FOR ld_det .    

form
   tt9_line        LABEL "Ln"          FORMAT ">>9"
   tt9_part        LABEL "Item Number" FORMAT "x(18)"
   tt9_um          LABEL "UM"          FORMAT "x(2)"
   tt9_qty_bo      label "Qty B/0"     format "->>>>>9"
   tt9_qty_avail   label "QtyAvail"    format "->>>>>9"
   tt9_qty_chg     label "Qtychg"     format "->>>>>9"
   tt9_tr_um       LABEL "UM"          FORMAT "x(2)"
   tt9_tr_uom      LABEL  "UOM"        FORMAT "x(2)"
   tt9_conv        label "Conv"        format ">>>9.9999<"
   tt9_msg         LABEL "Message#"    FORMAT "x(9)"
with frame v_disp title color normal (getFrameTitle("Invalid_line",28))
7 down width 80.

/* SET EXTERNAL LABELS */
setFrameLabels(frame v_disp:handle).    

form
   v_tt9_line       COLON 13 LABEL "Ln"             FORMAT ">>9"
   tt9_part       COLON 40 LABEL "Item Number"    FORMAT "x(18)"
   tt9_um         COLON 65 LABEL "UM"             FORMAT "x(2)"
   tt9_qty_chg    COLON 13 label "Qtychg"         format "->>>>>9"
   tt9_qty_avail  COLON 40 label "QtyAvail"       format "->>>>>9"
   tt9_qty_bo     COLON 65 label "Qty B/0"        format "->>>>>9"
   tt9_tr_um      COLON 13 LABEL "UM"             FORMAT "x(2)"
   tt9_tr_uom     COLON 25 LABEL  "UOM"           FORMAT "x(2)"            
   tt9_conv       COLON 40 label "Conv"           format ">>>9.9999<"
   tt9_msg        COLON 65 LABEL "Message#"       FORMAT "x(9)"           
with frame v_update side-labels attr-space width 80.

/* SET EXTERNAL LABELS */
setFrameLabels(frame v_update:handle).  
   ...
   do on error undo mainloop, leave mainloop:
      ...
      do transaction:
         ...
         */
            ...
            */
            ...
            editing:
               ...
               if frame-field = "so_nbr" then do:
                  IF INPUT so_nbr <> vv_nbr THEN DO:
                     FOR FIRST so_mstr WHERE so_domain = GLOBAL_domain
                        AND so_nbr = INPUT so_nbr NO-LOCK:
                     END.
         ...
         if INPUT v_dn = "" then do:
            /* Blank not allowed */
            {pxmsg.i &MSGNUM=40 &ERRORLEVEL=3}
            next-prompt v_dn WITH FRAME a.
            UNDO, retry.
         end.

         /* SS - 20081202.1 - B */
         FOR FIRST usrw_wkfl WHERE usrw_domain = GLOBAL_domain
            AND usrw_key1 = "DNID-RTN" 
            AND usrw_key2 = v_dn NO-LOCK :
         END.
         ...
         FOR FIRST usrw_wkfl WHERE usrw_domain = GLOBAL_domain
            AND usrw_key1 = "DNID-RTN-LAST" 
            AND usrw_key2 = v_dn EXCLUSIVE-LOCK :
         END.
         IF NOT AVAIL usrw_wkfl THEN DO:
            CREATE usrw_wkfl .
            ASSIGN
               usrw_domain = GLOBAL_domain
               usrw_key1 = "DNID-RTN-LAST"
               usrw_key2 = v_dn
               .
         END.
         /* SS - 20081202.1 - E */
         ...
         FOR FIRST mfc_ctrl WHERE mfc_domain = GLOBAL_domain
            AND mfc_field = "xxsoipm_post_date" NO-LOCK :
         END.
         IF AVAIL mfc_ctrl THEN DO:
            IF eff_date <= mfc_date THEN DO:
               /* 日期不在有效的周期内 */
         ...
         FOR FIRST cm_mstr WHERE cm_domain = GLOBAL_domain
            AND cm_addr = v_bill NO-LOCK :
         END.
         IF AVAIL cm_mstr THEN DO:
            /* 8156 */
            IF cm_type = "x" THEN DO:
               {pxmsg.i &MSGNUM=8156 &ERRORLEVEL=3}
               next-prompt v_bill WITH FRAME a.
               UNDO, retry.
            END.
         END.
         /* SS - 20090223.1 - B */
         ...
         FOR FIRST loc_mstr WHERE loc_domain = GLOBAL_domain 
            AND loc_site = ship_site 
            AND loc_loc = v_loc NO-LOCK :
         END.
         IF NOT AVAIL loc_mstr AND v_loc <> "" THEN DO:
         ...
         /* SS - 20081127.1 - B */
	      /* Check LOC Security */
	      {gprun.i ""xxgploca.p""
		            "(ship_site,v_loc ,recid(si_mstr), output return_int)"}
        ...
        IF FILL_all THEN DO: 
            ...
            FOR EACH sod_det NO-LOCK 
                ...
                ASSIGN 
                    ...
                    tt9_tr_um = sod_um
                    tt9_tr_uom = sod_um
                    tt9_qty_bo = (sod_qty_ord - sod__dec02) /* 短缺量 */
                    tt9_conv = sod_um_conv
                    tt9_site = ship_site
                    tt9_loc = v_loc
                    .
                ...
                FOR EACH pt_mstr NO-LOCK 
                    ...
                    IF pt_pm_code = "C" THEN DO:
                        ASSIGN 
                            tt9_type = "CA"
                            tt9_qty_chg = (sod_qty_ord - sod__dec02) .
                        ...
                        FOR EACH ps_mstr NO-LOCK 
                            ...
                            ASSIGN
                                ...
                                tt9_tr_um = sod_um
                                tt9_tr_uom = sod_um
                                tt9_qty_bo = (sod_qty_ord - sod__dec02) * ps_qty_per /* 短缺量 */
                                tt9_conv = sod_um_conv
                                .
                            ...
                            FOR FIRST ld_det NO-LOCK 
                              ...
                              AND ld_site = ship_site
                              AND ld_loc = v_loc
                              AND ld_part = ps_comp
                              AND ld_lot = ""
                              AND ld_ref = ""
                              AND (ld_qty_oh - (sod_qty_ord - sod__dec02) * ps_qty_per) >= 0 :
                            END.
                            IF (NOT AVAIL ld_det) THEN DO:
                                FOR FIRST lddet11 NO-LOCK 
                                  WHERE lddet11.ld_domain = GLOBAL_domain
                                  AND lddet11.ld_site = ship_site
                                  AND lddet11.ld_loc = v_loc
                                  AND lddet11.ld_part = ps_comp
                                  AND lddet11.ld_lot = ""
                                  AND lddet11.ld_ref = "" :
                                END.
                                IF NOT AVAIL lddet11 THEN DO:
                                    ASSIGN
                                        tt9_msg = "208" 
                                        tt9_qty_avail = 0
                                        tt9_qty_chg = 0
                                ...
                                ELSE DO:
                                    ASSIGN
                                        tt9_msg = "208" 
                                        tt9_qty_avail = lddet11.ld_qty_oh
                                        tt9_qty_chg = 0
                                        .
                                END.     
                                v1_flag = YES.
                            ...
                            ELSE DO:
                                ASSIGN
                                    tt9_qty_chg = tt9_qty_bo 
                                    tt9_qty_avail = ld_det.ld_qty_oh 
                                    tt9_site = ship_site
                                    tt9_loc = v_loc
                                    tt9_lotser = ld_det.ld_lot
                                    tt9_ref = ld_det.ld_ref
                                    .
                            END.

                            IF v1_flag = YES THEN DO:
                                FOR FIRST tt9b WHERE tt9b.tt9_type = "CA"
                                    AND tt9b.tt9_line = sod_line NO-LOCK :
                                END.
                                IF AVAIL tt9b THEN DO:
                                    ASSIGN
                                        tt9b.tt9_qty_chg = 0.
                                END.
                            END. /* IF v1_flag = YES THEN DO: */
                    ...
                    ELSE DO:
                        ...
                        END.
                    END.
                END. /* FOR EACH pt_mstr NO-LOCK */

                /* 当配置产品的任何子件不够时，整个配置产品都不能发运 */
                FOR EACH tt9 WHERE tt9_type = "CA"
                    AND tt9_qty_chg = 0 ,
                    EACH tt9b WHERE tt9b.tt9_type = "CB"
                    AND tt9b.tt9_line = tt9.tt9_line :
                    ASSIGN
                        tt9b.tt9_qty_chg = 0.
                END.
                 ...
                 IF AVAIL pt_mstr THEN DO:
                    IF sod_um = pt_um AND sod_um_conv <> 1 THEN DO:
                        FIND FIRST tt9 WHERE tt9_line = sod_line
                            AND tt9_part = sod_part NO-ERROR.
                        IF AVAIL tt9 THEN DO:
                            IF tt9_msg = "" THEN ASSIGN tt9_msg = "4982".
                            ELSE ASSIGN tt9_msg = tt9_msg + "/" + "4982" .
                        END.
                    END.
                    ...
                    IF sod_um <> pt_um AND v2_um_conv <> round(sod_um_conv,8) THEN DO:
                        FIND FIRST tt9 WHERE tt9_line = sod_line
                            AND tt9_part = sod_part NO-ERROR.
                        IF AVAIL tt9 THEN DO:
                            IF tt9_msg = "" THEN ASSIGN tt9_msg = "4982".
            ...
            END. /* FOR EACH sod_det NO-LOCK  */

            v_flag1 = NO.                                     
            FIND FIRST tt9 WHERE tt9_msg <> "" NO-LOCK NO-ERROR.
            IF AVAIL tt9 THEN v_flag1 = YES .

            /* 只显示数量不足的零件 */
            IF v_flag1 = YES THEN DO:
                clear frame v_disp all no-pause.
                clear frame v_update all no-pause.
            
                VIEW FRAME v_disp.
                VIEW FRAME v_update.

                /* 如果配置产品的子件存在不足的，则将整个配置产品的都显示出来 */
                FOR EACH tt9 WHERE tt9_type = "CB" AND tt9_msg <> "",
                    EACH tt9b WHERE tt9b.tt9_line = tt9.tt9_line AND tt9b.tt9_msg = "" :
                    ASSIGN
                        tt9b.tt9_msg = "C"
                        .
                END.
    
                FOR EACH tt9 WHERE tt9_msg <> "" BY tt9_line BY tt9_type  :
                    DISP 
                         tt9_line
                         tt9_part
                         tt9_um  
                      ...
                      if frame-line(v_disp) = frame-down(v_disp) then leave.
                      down 1 with frame v_disp.
                END.
                ...
                REPEAT:
                    ...
                    DO ON ERROR UNDO,RETRY :
                        UPDATE v_tt9_line
                            WITH FRAME v_update EDITING:
        
                            IF FRAME-FIELD = "v_tt9_line" THEN DO:
                                {xxmfnp01sosois.i tt9 v_tt9_line tt9_line tt9line }
                                 ...
                                 IF recno <> ? THEN DO:
                                     ...
                                     IF tt9_type BEGINS "C" THEN v1_part = tt9_part .
                                 END.
                                 ELSE DO:
                                     status input.
                                     readkey.
                                     apply lastkey.
                                 END.
                            END. /* IF FRAME-FIELD = "line" THEN DO: */
                        END. /* UPDATE tt9_line */
        
                        STATUS INPUT .     
        
                        IF v_tt9_line = 0 THEN LEAVE .
        
                        FIND FIRST tt9 WHERE tt9_msg <> "" AND tt9_line = v_tt9_line AND (tt9_part = v1_part OR v1_part = "") NO-ERROR.
                        IF AVAIL tt9 THEN DO:
                             DISP 
                                 tt9_line @ v_tt9_line
                                 tt9_part
                        ...
                        ELSE DO:
                            /*  4671 订单项不存在 */
                            {pxmsg.i &MSGNUM=4671 &ERRORLEVEL=3}
                            UNDO , retry.
                        END.
                        ...
                        DO ON ERROR UNDO ,RETRY :
                            ...
                            /* 当type = "CA" 且 msg = "C"时表示，配置产品的子件不足 */
                            IF tt9_msg = "C" THEN DO:
                                /* 当父件的输入数量大于短缺量时 */
                                IF tt9_qty_chg > tt9_qty_bo THEN DO:
                                    /* 10028: 无效数量 */
                                    {pxmsg.i &MSGNUM=10028 &ERRORLEVEL=3}
                                    UNDO setv2, retry.
                                END.
    
                                v2_flag = NO .
                                FOR EACH tt9b WHERE tt9b.tt9_type = "CB" AND tt9b.tt9_line = tt9.tt9_line:
                                    IF (tt9b.tt9_qty_bo / tt9.tt9_qty_bo * tt9.tt9_qty_chg) > tt9b.tt9_qty_avail THEN DO: 
                                        v2_flag = YES .
                                        LEAVE .
                                    END.
                                END.
    
                                IF v2_flag = YES THEN DO:
                                    /* 10028: 无效数量 */
                                    {pxmsg.i &MSGNUM=10028 &ERRORLEVEL=3}
                                    UNDO setv2, retry.
                                END.
                                ...
                                FOR EACH tt9b WHERE tt9b.tt9_type = "CB" AND tt9b.tt9_line = tt9.tt9_line:
                                    ASSIGN
                                        tt9b.tt9_qty_chg = tt9b.tt9_qty_bo / tt9.tt9_qty_bo * tt9.tt9_qty_chg
                                        .
                                END.
                            END.
                            ELSE DO:
                                /* 当输入的数量大于现有数量 */
                                IF tt9_qty_chg > tt9_qty_avail THEN DO:
                                    /* 10028: 无效数量 */
                                    {pxmsg.i &MSGNUM=10028 &ERRORLEVEL=3}
                                    UNDO setv2, retry.
                                END.
                        ...
                        FOR EACH tt9 WHERE tt9_msg <> "" AND tt9_line >= v_tt9_line NO-LOCK 
                            BY tt9_line BY tt9_type :
                             DISP 
                                 tt9_line
                                 tt9_part
                                 tt9_um  
                ...
                END.  /* REPEAT: */
            END. /* IF v_flag1 = YES THEN DO: */
        END. /* IF FILL_all THEN DO: */
        /* SS - 20090606.1 - E */
         ...
         /*
         FOR FIRST cm_mstr WHERE cm_domain = GLOBAL_domain
            AND cm_addr = INPUT v_bill NO-LOCK :
         END.
         ...
         FOR FIRST ad_mstr WHERE ad_domain = GLOBAL_domain
            AND ad_addr = INPUT v_ship NO-LOCK :
         END.
         IF NOT AVAIL ad_mstr THEN DO:
            /* Not a valid customer */
            {pxmsg.i &MSGNUM=3 &ERRORLEVEL=3}
            NEXT-PROMPT v_ship WITH FRAME a. 
            undo, retry.
         END.
           */
         ...
         FOR EACH sod_det WHERE sod_domain = GLOBAL_domain
            ...
            /* 对类型为非退货行的记录，默认备料量为（订单量 - 发运量).
               因为在显示to ship的数量时。引用了fill_all的逻辑 */
            IF NOT (sod_order_category MATCHES "*" + v_mfc_char + "*" ) THEN DO:
               /* 用于每次进入前清空明细项中sod__qadd01,sod__log01.这2个字段主要是用于控制发运数量 */
               sod__qadd01 = 0.
               sod__log01 = NO .
   
               /* 用于临时保存loc的值 */
               IF FILL_all = YES THEN sod__chr10 = v_loc .
               ELSE sod__chr10 = "".

               IF (sod_qty_ord - sod__dec02) > 0 then
                  sod_qty_all = sod_qty_ord - sod__dec02.
               ELSE sod_qty_all = 0.
            END.
            ELSE sod_qty_all = 0.
         END.
      ...
      IF FILL_ALL THEN DO:
          ...
          for each tt9 WHERE tt9_qty_chg <> 0 
              AND (tt9_type = "" OR tt9_type = "CA") :
              create sr_wkfl. sr_wkfl.sr_domain = global_domain.
              ASSIGN
                  sr_userid = mfguser
                  sr_lineid = STRING(tt9_line)
                  sr_site = tt9_site
                  sr_loc = tt9_loc
                  sr_lotser = tt9_lotser
                  sr_ref = tt9_ref
                  sr_qty = tt9_qty_chg .

              FOR FIRST sod_det WHERE sod_domain = GLOBAL_domain 
                  AND sod_nbr = so_nbr
                  AND sod_line = tt9_line :
              END.
              IF AVAIL sod_det THEN DO:
                  ASSIGN
                      sod_bo_chg = 0
                      sod_qty_chg = tt9_qty_chg .
              END.
          ...
          for each tt9 WHERE tt9_qty_chg <> 0 
              ...
              /*
              MESSAGE "1 " + string(tt9_line) + " " + tt9_part + " " + tt9_loc VIEW-AS ALERT-BOX.
                */

              create sr_wkfl. sr_wkfl.sr_domain = global_domain.
              ASSIGN
                  sr_userid = mfguser
                  sr_lineid = STRING(tt9_line) + "ISS" + tt9_part
                  sr_site = tt9_site
                  sr_loc = tt9_loc
                  sr_lotser = tt9_lotser
                  sr_ref = tt9_ref
      ...
      /* SS - 100813.1 - B */
      oPart = "".
      {gprun.i ""xxsosoisma.p"" "(
         OUTPUT oPart
         )"}
      IF oPart <> "" THEN DO:
         /* Insufficient quantity available to be consumed */
         {pxmsg.i &MSGNUM=6754 &MSGARG1=oPart &ERRORLEVEL=3}
