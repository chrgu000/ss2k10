...
DEF VAR v1_lotserial_qty LIKE ld_qty_oh .
DEF VAR v_total_lotserial_qty LIKE ld_qty_oh.
/* SS - 20080905.1 - E */
...
repeat:
   ...
   /* DO NOT ACCESS UI WHEN PROCESSING API REQUEST */
      ...
      /* SS - 20080905.1 - B */
      v_total_lotserial_qty = - TOTAL_lotserial_qty .

      /* Total lot/serial quantity entered: # */
      {pxmsg.i &MSGNUM=300 &ERRORLEVEL=1
               &MSGARG1= v_total_lotserial_qty}
      /* SS - 20080905.1 - E */
      ...
      do on error undo, retry:
         ...
         if c-application-mode <> "API" then do:
            display
               ...
               lotserial_qty
               */
               (- lotserial_qty) @ v1_lotserial_qty
               /* SS - 20080905.1 - E */
               .
        ...
        /* GET NEXT RECORD IN API TEMP-TABLE AND SET INPUT VALUES */
         ...
         /* SS - 20080905.1 - B */
         v1_lotserial_qty = - lotserial_qty .
         ...
         DO ON ERROR UNDO,RETRY:
            ...
            /*
            IF v_flag = YES AND v_qty = 0 THEN DO:
               {pxmsg.i &MSGNUM=208 &ERRORLEVEL=2 &MSGARG1="0"}
            END. */
            IF (v1_lotserial_qty + v__dec01) > v_ord AND v_ord <> 0 THEN DO: 
               {pxmsg.i &MSGNUM=90002 &ERRORLEVEL=2}
                PAUSE .
            END.
         END.
         /* SS - 20080905.1 - E */
   ...
   /* Total lot serial quantity entered */
   {pxmsg.i &MSGNUM=300 &ERRORLEVEL=1 &MSGARG1= v_total_lotserial_qty}
   /* SS - 20080905.1 - E */
