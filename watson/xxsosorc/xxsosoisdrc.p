...
form
   ...
   */
   ...
   lotserial      colon 53
   */
   sod__dec02     COLON 36 LABEL "Rtned" FORMAT "->>>>>9"
   lotserial      
   /* SS - 20080901.1 - E */
   sod_part       colon 13
   sod_um         COLON 36
...
repeat transaction:
   ...
   repeat:
      ...
      then do:
         ...
         */
         ...
         for each sod_det no-lock
            ...
            and  (sod_site = ship_site
            or    ship_site = "") )
            AND (sod_order_category MATCHES "*" + v_mfc_char + "*" )
         by sod_line:
         /* SS - 20080823.1 - E */
                  ...
                  IF sod__log01 = NO THEN DO:
                     ...
                     IF FILL_all THEN DO:
                        display
                           ...
                           sod__dec02  FORMAT "->>>>>9" LABEL "Rtned"
                           (sod_qty_ord - sod__dec02) @ v_backord    format "->>>>>9" label "Qty B/0"
                           IF (sod_qty_ord - sod__dec02) > 0 THEN (sod_qty_ord - sod__dec02)
      ...
      do on error undo, retry:
            ...
            editing:
               ...
               if frame-field = "line" then do:
                  ...
                  {mfnp01.i sod_det line sod_line sod_nbr
                    ...
                    */
                  {xxmfnp01rc.i sod_det line sod_line sod_nbr
                          " sod_domain = global_domain and so_nbr " sod_nbrln}
                  /* SS - 20080901.1 - E */
         ...
         */
         /* 如果输入了退货行的数据，则提示错误 */
         if (not available sod_det) OR (AVAILABLE sod_det AND NOT ( sod_order_category MATCHES "*" + v_mfc_char + "*" )) then do:
            /* Line item does not exist */
            {pxmsg.i &MSGNUM=45 &ERRORLEVEL=3}
            if batchrun then
               undo loop0, leave loop0.
      ...
      do on error undo, retry:
         ...
         IF FILL_all = YES THEN DO:
            IF sod__log01 = NO THEN do:
               IF (sod_qty_ord - sod__dec02) > 0 THEN lotserial_qty = (sod_qty_ord - sod__dec02 ) .
         ...
         IF sod__log01 = NO THEN DO:
            lotserial = sod_serial .
         END.
         lotref = sod_order_category.
         DISP lotref WITH FRAME c .
         /* SS - 20080823.1 - E */
         ...
         /* 冲减的数量超过了原有发货量 */
         IF ( lotserial_qty + sod__dec02) > sod_qty_ord AND sod_qty_ord <> 0 THEN DO:
            {pxmsg.i &MSGNUM=90002 &ERRORLEVEL=2 &MSGARG1=sod_qty_ord}
            PAUSE.
         END.
