...
/* Revision: 1.55.1.3   BY: Suyash Keny         DATE: 09/18/06  ECO: *P56J*   */
/* $Revision: 1.55.1.5 $         BY: Dilip Manawat       DATE: 03/13/07  ECO: *P5R6*  */
/*----rev history-------------------------------------------------------------------------------------*/
...
define variable l_close_pgm  like mfc_logical initial no  no-undo.
...
form
     ...
     v_person2      colon 15
     v_mail2        colon 15
     v_tradeterm    colon 15
     v_tradedesc    colon 15 no-label 
...
repeat on error undo mainloop, leave mainloop:
   ...
   repeat:
      ...
      do transaction:  /* Oracle lock problem */
         ...
         if l_close_pgm then undo mainloop, leave mainloop.
        ...
        v_cmmt       = if can-find(first cmt_det  where cmt_det.cmt_domain = global_domain and  cmt_indx = cmtindx) 
                       then yes else no .
        v_tradeterm  = ad__chr01 .
        find first code_mstr where code_domain = global_domain and code_fldname = "v_tradeterm" and code_value = v_tradeterm no-lock no-error.
        v_tradedesc  = if avail code_mstr then code_cmmt else "" .
        ...
        do on error undo, leave:
            ...
            find first code_mstr 
                ...
                and code_fldname = "v_tradeterm" 
            no-lock no-error.
            if avail code_mstr then do:
                find first code_mstr 
                    where code_domain = global_domain 
                    and code_fldname = "v_tradeterm" 
                    and code_value = v_tradeterm 
                no-lock no-error.
                if not avail code_mstr then do:
                    {pxmsg.i &MSGNUM=716 &ERRORLEVEL=3}
                    next-prompt v_tradeterm with frame f-mails.
                    undo , retry.
                end.
            end.
...
PROCEDURE closepgm :
/*-----------------------------------------------------------------------------
 Purpose:     Logic to close program in GUI when clicked on close button.
 Exceptions:  NONE
 Notes:
 History:
------------------------------------------------------------------------------*/
