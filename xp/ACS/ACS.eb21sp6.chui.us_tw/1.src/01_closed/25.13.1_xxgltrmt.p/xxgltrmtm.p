...
/* Revision: 1.62     BY: Paul Donnelly (SB)  DATE: 06/26/03 ECO: *Q00P*     */
/* Revision: 1.63     BY: K Paneesh           DATE: 08/08/03 ECO: *N2JT*     */
/* Revision: 1.64     BY: Dorota Hohol        DATE: 10/27/03 ECO: *P11J*     */
/* Revision: 1.65     BY: Ed van de Gevel     DATE: 05/17/04 ECO: *Q07S*     */
/* Revision: 1.66     BY: Sukhad Kulkarni     DATE: 05/25/04 ECO: *P236*     */
/* Revision: 1.67     BY: Rob DiMeola         DATE: 08/17/04 ECO: *P2F2*     */
/* Revision: 1.68     BY: Milind Shahane      DATE: 08/23/04 ECO: *P2FG*     */
/* Revision: 1.70     BY: Pankaj Arora        DATE: 11/03/04 ECO: *P2SF*     */
/* Revision: 1.71  BY: SurenderSingh Nihalani DATE: 11/10/05 ECO: *P47W*     */
/* Revision: 1.71.1.1 BY: Ed van de Gevel     DATE: 05/12/06 ECO: *Q0V1*     */
/* $Revision: 1.71.1.2 $  BY: Sarita Gonsalves  DATE: 07/27/06 ECO: *P4YR*   */
...
define new shared variable l_doc_type        like glt_doc_type no-undo.
...
do transaction:
   ...
   then do:
      ...
      for first bk_mstr
         fields( bk_domain bk_acct bk_cc bk_code bk_entity bk_sub)
...
repeat:
   ...
   for first glt_det
      fields( glt_domain glt_acct    glt_addr       glt_amt        glt_batch
             ...
             {&GLTRMTM-P-TAG40})
       where glt_det.glt_domain = global_domain and  glt_ref = l_glt_ref
      no-lock:
   ...
   do transaction:
      ...
      then do:
         ...
         then do:
            ...
            for last glt_det
               fields( glt_domain glt_acct    glt_addr       glt_amt
               glt_batch
                      glt_cc      glt_correction glt_curr       glt_curr_amt
                      glt_date    glt_desc       glt_doc        glt_doc_type
                      glt_dy_code glt_dy_num     glt_ecur_amt   glt_effdate
            ...
            repeat:
               ...
               and not can-find(first gltr_hist
                                   where gltr_hist.gltr_domain = global_domain
                                   and  gltr_ref = ref)
               then do:
      ...
      {&GLTRMTM-P-TAG32}
         ...
         if (can-find(first glt_det
                       where glt_det.glt_domain = global_domain and  glt_ref =
                       l_glt_ref )) = false
         then do:
   ...
   do transaction:
      ...
      then
         assign
            ctrl_amt:format in frame ya = ctrl_amt_base_fmt
            tot_amt:format in frame ya  = tot_amt_base_fmt.
      else
         assign
            ctrl_amt:format in frame a = ctrl_amt_base_fmt
            tot_amt:format in frame a  = tot_amt_base_fmt.
      ...
      then do:
         ...
         if can-find(first gltr_hist
                      where gltr_hist.gltr_domain = global_domain and  gltr_ref
                      = l_glt_ref )
         then do:
      ...
      /* AVAILABLE glt_det */
         ...
         then do:
            ...
            then do:
               ...
               run p-disp-msg(input 6277, input 3).
               undo, next mainloop.
            ...
            then do:
               ...
               run p-disp-msg(input 5763, input 3).
               undo, next mainloop.
            end. /* IF type_parm <> "YA" ... */
            ...
            then do:
               ...
               for first cb_mstr
                  fields( cb_domain cb_ref cb_type)
                   where cb_mstr.cb_domain = global_domain and  cb_ref  = ref
                  and   cb_type = "G"
                  no-lock:
            ...
            for each glt_det
               ...
               for first ac_mstr
                  fields( ac_domain ac_active ac_code ac_curr ac_type)
                   where ac_mstr.ac_domain = global_domain and  ac_code =
                   glt_acc
                  no-lock:
   ...
   do transaction:
      ...
      do on error undo, retry:
         ...
         then do:
            ...
            then do:
               ...
               if not can-find (nr_mstr
                where nr_mstr.nr_domain = global_domain and  nr_seqid =
                dft-daybook)
               and l_daybook_flag
               then
                  allow-gaps = yes.
         ...
         then do:
            ...
            break by glt_entity:
               ...
               if first-of (glt_entity)
               then do:

                   /* CHECK ENTITY SECURITY */
                   {glenchk.i &entity=glt_entity &entity_ok=entity_ok}
                   if not entity_ok
                   then
                      undo seta.
               end. /* IF FIRST-OF (glt_entity)*/
            end. /* FOR EACH glt_det */
         end. /* IF del-yn */
         ...
         if frame a corr-flag entered
            or frame ya corr-flag entered
         then do:
         ...
         end. /* IF FRAME a corr-flag ENTERED */
      ...
      repeat:
         ...
         /* ENTER LINE ITEMS */
         {gprun.i ""xxgltrmta.p"" "(input type_parm, input l_doc_type)"} /* SS - 101221.1 */
      ...
      repeat:
         ...
         then do:
            ...
            run p-disp-msg(input 3011, input 2).
            ...
            then do:
               ...
               run p-disp-msg(input 3030, input 2).
            ...
            then do:
               ...
               then do:
                  ...
                  then do:
                     ...
                     run p-disp-msg(input mc-error-number, input 2).
                     if not batchrun
                     then
                        pause.
                  end. /* IF mc-error-number <> 0 */
      ...
      then do:
         ...
         for first glt_det
            fields( glt_domain glt_ref glt_dy_code glt_dy_num)
             where glt_det.glt_domain = global_domain and  glt_ref = ref
            no-lock:
   ...
   then
      ...
      do transaction:
         ...
         find glt_det
             where glt_det.glt_domain = global_domain and  glt_tr_type = "**"
            and   glt_ref     = ref
            exclusive-lock
            no-error.
...
then do:
   ...
   do transaction:
      ...
      for each gltdet
          where gltdet.glt_domain = global_domain and  gltdet.glt_ref     =  ref
...
PROCEDURE get_glt_ref:
   ...
   then do:
      ...
      editing:
         ...
         {mfnp.i glt_det glt_ref  " glt_det.glt_domain = global_domain and
         glt_ref "  glt_ref glt_ref
            glt_ref }
      ...
      editing:
         ...
         /* FIND NEXT/PREVIOUS RECORD */
         {mfnp05.i glt_det glt_tr_type " glt_det.glt_domain = global_domain and
         glt_tr_type  = ""YA"""
...
PROCEDURE ip-invalid-acc:
   ...
   for each glt_det exclusive-lock
      ...
      for first ac_mstr
         fields( ac_domain ac_active ac_code ac_curr ac_type)
          where ac_mstr.ac_domain = global_domain and  ac_code = glt_det.glt_acc
         no-lock:
      end. /* FOR FIRST ac_mstr */
      ...
      then do:
         for first al_mstr
            fields( al_domain al_code)
             where al_mstr.al_domain = global_domain and  al_code =
             glt_det.glt_acc
            no-lock:
...
PROCEDURE build_glt_det:
   /* THE PROJECT CODE SHOULD BE BLANK FOR THE CASH ACCOUNT LINE, */
   /* GENERATED THROUGH CASH BOOK MAINTENANCE.                    */
   assign
