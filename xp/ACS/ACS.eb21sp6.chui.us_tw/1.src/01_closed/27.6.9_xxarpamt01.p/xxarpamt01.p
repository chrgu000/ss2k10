...
/* Revision: 1.44       BY: Narathip W.        DATE: 05/19/03 ECO: *P0SH*     */
/* Revision: 1.46       BY: Paul Donnelly (SB) DATE: 06/26/03 ECO: *Q00B*     */
/* Revision: 1.49       BY: Orawan S.          DATE: 05/17/04 ECO: *P21R*     */
/* Revision: 1.50       BY: Bhagyashri Shinde  DATE: 03/15/05 ECO: *P3CQ*     */
...
define variable l_addr           like global_addr no-undo.
...
repeat with frame a:
   do transaction:
      ...
      then do:
         find first ar_mstr
             where ar_mstr.ar_domain = global_domain and  ar_batch = batch
            no-lock no-error.
   ...
   repeat with frame b:
        ...
        /* SS - 110114.1 - B */
            {xxgpdescm3.i 
                ...
                &desc=ar_po 
                &gltdoc=ar_nbr
                &gltacct="ar_acct or glt_acct = ar_disc_acct"
            }
            {xxgpdescm3.i 
                &table=ard_det
                &desc=ard_desc
      ...
      do transaction:
         ...
         editing:
            ...
            {mfnp01.i ar_mstr ar_nbr ar_nbr batch  " ar_mstr.ar_domain =
            global_domain and ar_batch "  ar_batch}
            if recno <> ?
            then do:
         ...
         then do:
            ...
            "arc_ctrl.arc_domain" "ar_mstr.ar_domain = global_domain" arc_ctrl
            arc_memo ar_mstr ar_nbr arnbr}
            display
               arnbr @ ar_nbr.
         end.
         else do:
            arnbr = input ar_nbr.
            find first ar_mstr
                where ar_mstr.ar_domain = global_domain and  ar_nbr   = arnbr
               and   ar_batch = batch
               no-lock no-error.
         ...
         find ar_mstr
             where ar_mstr.ar_domain = global_domain and  ar_nbr = string(arnbr)
            no-error.
      ...
      do transaction:
         ...
         then do:
            ...
            editing:
               ...
               cm_addr "  ar_bill cm_addr cm_addr}
               if recno <> ?
               then
                  display
                     cm_addr @ ar_bill
            ...
            find cm_mstr
                where cm_mstr.cm_domain = global_domain and  cm_addr = input
                ar_bill
               no-lock no-error.
            if available cm_mstr
               then
            ...
            do for armstr:
               ...
               {&ARPAMT01-P-TAG14}
               find first armstr
                   where armstr.ar_domain = global_domain and  armstr.ar_check
                   = ar_mstr.ar_check
                  and   armstr.ar_bill  = ar_mstr.ar_bill
                  and   armstr.ar_type  = "P"
                  no-lock no-error.
         ...
         do on error undo, retry:
            ...
            then do:
               ...
               do for armstr:
                  ...
                  prompt-for
                     ...
                     then do:
                        {mfnp07.i armstr ar_mstr.ar_check
                                         armstr.ar_check
                                         armstr.ar_bill
                                          " armstr.ar_domain = global_domain
                                          and ar_mstr.ar_bill "
                                         artype
                                         armstr.ar_type
                                         yes
                     ...
                     then do:
                        ...
                        /* global_addr IS USED TO PASS PARAMETER IN  */
                        /* LOOKUP arlu007.p                          */
                        assign
                           check_nbr   = input frame b ar_mstr.ar_check
                           global_addr = string(ar_mstr.ar_bill, "x(8)")
                                        + check_nbr.

                        {mfnp05.i ard_det
                                  ard_nbr
                                  " ard_det.ard_domain = global_domain and
                  ...
                  find first armstr
                      where armstr.ar_domain = global_domain and
                      armstr.ar_check = input ar_mstr.ar_check
                     and armstr.ar_bill  = ar_mstr.ar_bill
                     and armstr.ar_type  = "P"
                  ...
                  then do:
                     ...
                     then do:
                        ...
                        if not can-find(dy_mstr
                                         where dy_mstr.dy_domain =
                                         global_domain and  dy_dy_code = input
                                         ar_dy_code)
                        then do:
         ...
         /*ADDED 1ST parameter TEMP TABLE t_store_glt_ref TO STORE NEWLY*/
         /*CREATED  GL REFERENCES AND ARD NUMBERS                       */
         {gprun.i ""xxarpamt1a.p""
            "(input-output table t_store_glt_ref)"}  /* SS - 110114.1 */
         ...
         break by glt_ref:
            ...
            then do:
               ...
               for each ard_det
                  ...
                  and   ard_nbr     = t_ard_nbr
                  and   ard_dy_code = t_ar_dy_code
               use-index ard_nbr
               exclusive-lock:
...
PROCEDURE p-set-batch:
   ...
   then do:
      if can-find(first ar_mstr
                   where ar_mstr.ar_domain = global_domain and  ar_batch =
                   ba_batch)
      then do:
         if ba_ctrl   <> ba_total
         then do:
...
PROCEDURE find_tot_amt_open:
   ...
   for each armstr1
       where armstr1.ar_domain = global_domain and  armstr1.ar_check  = p_check
