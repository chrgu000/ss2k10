...
/* $Revision: 1.92.1.13 $   BY: Devna Sahai         DATE: 03/20/07  ECO: *P5RG* */
/*----rev history-------------------------------------------------------------------------------------*/
      ...
      do transaction:
         ...
         do on error undo, retry:
            ...
            if pod_type = "S" then do:
               ...
               {gprun.i ""xxpoporca2x1.p""}  /* SS - 110112.1 */
               if undo_all then do:
               ...
               if multi_entry = no then do:
                   ...
                   if not avail loc_mstr then do:
                        ...
                        next-prompt location with frame d.
                        undo,retry.
                   end.
               end.
                /* SS - 110112.1 - E */
            ...
            if multi_entry then do:
               ...
               */
               ...
               if lotserial_qty = 0 then do:
                  if available sr_wkfl 
                       and (not can-find(first sr_wkfl where                
...
/* SS - 110112.1 - B */
if pod_type = "s" then do:
    if pod_qty_ord >= 0 and total_received < 0 then do:
    ...
    if total_lotserial_qty < 0 then do:
        ...
        if wo_qty_comp + total_lotserial_qty * trans_conv < 0 then do:
                {pxmsg.i &MSGNUM=556 &MSGARG1=""subwo:"" &MSGARG2=wo_lot  &MSGARG3=wo_qty_comp &ERRORLEVEL=3}
                undo lineloop, retry lineloop.
        end.
    end.
end.
         ...
         if yn then do:
            ...
            for each sr_wkfl
               ...
               if return-value <> {&SUCCESS-RESULT} then
                ...
                /* SS - 110112.1 - B */
                if porec and is-return = no then do:
                    find first pod_Det 
