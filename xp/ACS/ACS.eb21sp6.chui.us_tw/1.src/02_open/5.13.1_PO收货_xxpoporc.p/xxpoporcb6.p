...
/* Revision: 1.87     BY: Sachin Deshmukh          DATE: 03/01/05 ECO: *P2TJ* */
/* Revision: 1.87.1.1 BY: Ed van de Gevel          DATE: 03/24/05 ECO: *P2NT* */
/* Revision: 1.87.1.2 BY: Anitha Gopal             DATE: 07/07/05 ECO: *P3RL* */
/* Revision: 1.87.1.3 BY: Steve Nugent             DATE: 08/11/05 ECO: *P2PJ* */
/* Revision: 1.87.1.4 BY: Tejasvi Kulkarni         DATE: 09/16/05 ECO: *P3TD* */
/* Revision: 1.87.1.5 BY: Sachin Deshmukh          DATE: 11/03/05 ECO: *P44Y* */
/* Revision: 1.87.1.6 BY: Mochesh Chandran         DATE: 08/25/06 ECO: *P53F* */
/* $Revision: 1.87.1.7 $   BY: Sarita Gonsalves     DATE: 05/18/07 ECO: *P5WZ* */
...
for first pod_det
      fields( pod_domain pod_acct pod_sub
...
for first po_mstr
   fields( po_domain po_curr po_is_btb po_nbr po_so_nbr po_tax_pct po_vend
   po_tot_terms_code)
    where po_mstr.po_domain = global_domain and  po_nbr    = pod_nbr no-lock:
end. /* FOR FIRST PO_MSTR */

for first pt_mstr
      fields( pt_domain pt_part pt_pm_code pt_prod_line pt_routing)
...
if using_supplier_consignment and ip_is_usage then
   ...
   then do:
      ...
      for first pl_mstr
            fields( pl_domain pl_cop_acct pl_cop_sub pl_cop_cc pl_inv_acct
                    pl_inv_sub pl_inv_cc pl_ovh_acct pl_ovh_sub pl_ovh_cc pl_ppv_acct
      ...
      run p-costconv(input l_sct_cst_tot,
                     input sct_ovh_tl,
                     output l_total_cost,
                     input-output gl_amt[1]).
      ...
      then do:
         ...
         then do:
            ...
            {&POPORCB6-P-TAG26}
         end. /* if not(using_supplier_consignment ... */
         else
            gl_amt[2] = 0.
      end.
      ...
      then do:
         ...
         then do:
            for first ld_det
              fields( ld_domain ld_assay ld_expire ld_grade ld_loc ld_lot
              ld_part
   ...
   /*SUBCONTRACT RECEIPTS*/
      ...
      if can-find(first wr_route
          where wr_route.wr_domain = global_domain and  wr_lot = wolot and
          wr_op = woop)
      then do:

         for first wo_mstr
               fields( wo_domain wo_lot wo_nbr wo_part wo_routing wo_status
               wo_type)
         ...
         find wr_route
             where wr_route.wr_domain = global_domain and  wr_lot = wolot
         ...
         if index ("FPC",wo_status) = 0 then do:
            if wo_type = "c" and wo_nbr = ""
               ...
               for first ro_det
                     fields( ro_domain ro_mv_nxt_op ro_op ro_routing
                     ro_sub_cost)
               ...
               /* PORCSUB.P AS THE SUBCONTRACT OPERATION HISTORY IS TRACKED   */
         ...
         for first ptp_det
               fields( ptp_domain  ptp_part ptp_routing ptp_site )
         ...
         for first ro_det
               fields( ro_domain ro_mv_nxt_op ro_op ro_routing ro_sub_cost)
                where ro_det.ro_domain = global_domain and  ro_routing =
                l_ro_routing
               and   ro_op = woop no-lock:
         end. /* FOR FIRST RO_DET */
...
/*SS-20100904*   {gprun.i ""poporcc.p""*/
/*SS-20100904*/   {gprun.i ""xxpoporcc.p""
      "(input shipnbr,
        input ship_date,
        input inv_mov,
        input ip_consign_flag,
...
PROCEDURE p-costconv:
   ...
   if glx_mthd = "AVG"
   then
      p_gl_amt = trqty * p_total_cost.

END PROCEDURE. /* PROCEDURE p-costconv */
PROCEDURE getGLDefaults:
   for first vd_mstr
      fields( vd_domain vd_addr vd_type)
       where vd_mstr.vd_domain = global_domain and  vd_addr = po_mstr.po_vend
       no-lock: end.
...
for each lacd_det
    ...
    lacd_det.lacd_internal_ref_type = {&TYPE_PO}
   and lacd_det.lacd_internal_ref = po_mstr.po_nbr
   no-lock:

   for first in_mstr
       where in_mstr.in_domain = global_domain and  in_part = pod_det.pod_part
   ...
   for first spt_det
       where spt_det.spt_domain = global_domain and  spt_site = in_gl_cost_site
      and spt_sim = sc_sim
...
/* AND IF THE KANBAN SUPERMARKET IS A WIP INVENTORY TYPE.   */
   ...
   if kbtransnbr <> 0 then do:
      for first kbtr_hist
        ...
        kbtr_trans_nbr)
         where kbtr_domain = global_domain and
            kbtr_trans_nbr = kbtransnbr
      no-lock,
      first knbsm_mstr
         fields(knbsm_supermarket_id knbsm_site knbsm_inv_loc_type)
         where knbsm_domain = global_domain and
               knbsm_site = kbtr_supermarket_site and
...
PROCEDURE getProductLineDetailInfo:
   ...
   for first pld_det
         fields( pld_domain pld_inv_acct pld_inv_sub
         ...
         and   pld_loc      = l_location1 no-lock:
...
PROCEDURE deleteWorkfile:
   ...
   for each sr_wkfl
      ...
      and p_is_usage         = no)
      or  (sr_userid          = mfguser + "cons"
      and ip_is_usage))
      and sr_lineid           = string(p_podline))
   no-lock:
      find b_srwkfl
         where recid(b_srwkfl) = recid(sr_wkfl)
...
assign
   ...
   p_line_tax = 0.
for each tx2d_det
    fields(tx2d_domain tx2d_cur_recov_amt
           tx2d_cur_tax_amt tx2d_line
           tx2d_nbr tx2d_rcpt_tax_point
           tx2d_ref tx2d_tax_in
