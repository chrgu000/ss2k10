...
/*SS-20100904 add***************************/
   ...
   then do:
      /* Blank not allowed */
      {pxmsg.i &MSGNUM=80101 &ERRORLEVEL=3}
      next-prompt recgp with frame {&frame}.
      undo seta1, retry.
   end. /* IF recgp = "" */
   
   find first nr_mstr where nr_domain = global_domain 
			and nr_seqid = recgp no-lock no-error.
   if not available nr_mstr 
   then do:
      /* Group Code does not exist */
      {pxmsg.i &MSGNUM=80102 &ERRORLEVEL=3}
      next-prompt recgp with frame {&frame}.
      undo seta1, retry.
...
/*SS-20100904 add***************************/
      for first poc_ctrl
           ...
           rcv_type2 = poc_rcv_type.
      end. /* FOR FIRST POC_CTRL */

   if rcv_type2 = 2 then do:
      /* Set Receiver Type to 1 on */
      {pxmsg.i &MSGNUM=80103 &ERRORLEVEL=3}
      next-prompt receivernbr with frame {&frame}.
      undo seta1, retry.
   end. /* if rcv_type2 <> 2  */
/*SS-20100904 end***************************/
