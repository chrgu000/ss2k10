...
/* Revision: 1.28  BY: Rajinder Kamra  DATE: 06/23/03  ECO *Q003*  */
/* Revision: 1.29  BY: Sandy Brown (OID) DATE: 12/06/03 ECO: *Q04L* */
/* $Revision: 1.29.1.1 $    BY: Reena Ambavi      DATE: 05/09/05 ECO: *P3KH* */
/*----rev history-------------------------------------------------------------------------------------*/
/* SS - 110112.1  By: Roger Xiao */  /*modify procedure: validateSubcontractWorkOrder */
/*-Revision end---------------------------------------------------------------*/
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      assign
         ...
         pod_rum          = pod_um
         pod_rum_conv     = 1
         pCancelBackOrder = can-find(first poc_ctrl  where poc_ctrl.poc_domain
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if not pPODetailScheduled then
         for each pvo_mstr
             ...
             pPurchaseOrderId and
            pvo_line = pPurchaseOrderLineId and
            pvo_last_voucher <> "" no-lock:

         for first ap_mstr fields( ap_domain ap_open ap_ref ap_type)
                where ap_mstr.ap_domain = global_domain and  ap_type = "VO" and
...
/* SS - 110112.1 - B 
   ...
   SS - 110112.1 - E */
/* SS - 110112.1 - B */
         if pWo_mstr.wo_status <> "R" then do:
            {pxmsg.i
               &MSGNUM=525
               &MSGARG1=pWo_mstr.wo_status
               &ERRORLEVEL={&WARNING-RESULT}}
            assign vRetVal = {&WARNING-RESULT}.
         end.
/* SS - 110112.1 - E */
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      for first pt_mstr fields( pt_domain pt_part pt_shelflife)
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      is_status = pInventoryStatus) then do:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      for each pod_det
            fields( pod_domain pod_nbr pod_op pod_qty_chg pod_type pod_um_conv
            pod_wo_lot)
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if use-log-acctg then do:
         ...
         for first po_mstr
            fields( po_domain po_nbr po_tot_terms_code)
      ...
      then do:
         ...
         for first po_mstr
            fields( po_domain po_nbr po_ex_rate po_ex_rate2)
             where po_mstr.po_domain = global_domain and  po_nbr = pod_nbr
         no-lock:
         end.
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first poc_ctrl
            fields( poc_domain poc_tol_cst poc_tol_pct)
...
------------------------------------------------------------------------------*/
   ...
   for each sr_wkfl
         ...
         sr_ref sr_site sr_userid sr_vend_lot)
          where sr_wkfl.sr_domain = global_domain and  sr_userid = vMfgUser
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      create prh_hist.  /* TEMPORARY, DELETED LATER */ prh_hist.prh_domain =
      global_domain.
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for each prh_hist exclusive-lock  where prh_hist.prh_domain =
      global_domain and (
         ...
         for first pvo_mstr
            ...
            {gpextdel.i &OWNER = 'T2:T3'
                  &TABLENAME = 'prh_hist'
                  &REFERENCE = 'returnqty'
                  &KEY1      = prh_hist.prh_domain
                  &KEY2      = prh_hist.prh_nbr
                  &KEY3      = prh_hist.prh_receiver
                  &KEY4      = string(prh_hist.prh_line)
                  &KEY5      = prh_hist.prh_element}
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first pt_mstr
            fields( pt_domain pt_part pt_um)
             where pt_mstr.pt_domain = global_domain and  pt_part = pPartId
             no-lock:
      end. /* FOR FIRST PT_MSTR */
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for each  tx2d_det exclusive-lock
         ...
         if tx2d_line = 0 or
            can-find(pod_det  where pod_det.pod_domain = global_domain and
            pod_nbr = pPurchaseOrderId
