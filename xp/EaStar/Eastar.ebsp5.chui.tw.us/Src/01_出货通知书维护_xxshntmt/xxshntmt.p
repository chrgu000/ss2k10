...
/* SS - 100222.1  By: Roger Xiao */
/* SS - 100407.1  By: Roger Xiao */
...
/* SS - 091230.1 - B */
define new shared variable soline    like sod_line no-undo.
/* SS - 091230.1 - E */
...
/* SS - 091230.1 - B */
/* SS - 100913.1 - B 
define new shared variable qtyper        as integer format ">,>>>,>>9.9<".   
   SS - 100913.1 - E */
/* SS - 100913.1 - B */
define new shared variable qtyper        as integer format ">,>>>,>>9.9<<<".   
/* SS - 100913.1 - E */
         ...
         go-on(F5 CTRL-D) with frame a editing:
            ...
            if lastkey = keycode("F5") or lastkey = keycode("CTRL-D") then do:
               ...
               if del-yn then do:
                 ...
                 for each snp_totals use-index snp_sn_nbr  where snp_sn_nbr = snnbr exclusive-lock:
                      snp_status = "D".
                      snp_last_date = today.
                      snp_last_user = global_userid.
                 end.
                 for each snp_totals use-index snp_sn_nbr  where snp_sn_nbr = snnbr no-lock:
                 end.
   ...
   repeat on endkey undo mainloop, retry:
      ...
      with frame b . 
        ...
        if part <> "" and order <> "" then do:
            find first sod_det where sod_nbr = order and sod_part = part /* SS - 091230.1 - B */ no-lock no-error.
...
/* SS - 091230.1 - B */
                  xxsq_qty_open = 0.
                  for each sod_det where sod_nbr = snh_so_order and sod_line = snh_so_line and sod_part = snh_part no-lock:
                      xxsq_qty_open = xxsq_qty_open + sod_qty_ord - sod_qty_ship.
                  end.
/* SS - 091230.1 - E */
...
/*               if snd_plt_nbr = 0 then total_grosswt = total_grosswt + snd_ext_grosswt.*/
        ...
        for each snp_totals use-index snp_sn_nbr where snp_sn_nbr = snnbr and snp_status <> "D" no-lock:
