...
/*easnew*/  DEFINE VARIABLE mm AS CHARACTER.
	    ...
	    if sanbr = "" and snnbr <> "" then do transaction:
		   ...
		   find first doc_nbr_ctl where doc_type = "SA" and doc_year = string(year(today))   
		        ...
		        assign
		          ...
		          /*easnew*  sanbr = "Y" +  substr(doc_year,3,2) + m.  */
				  /*easnew*  pinbr = "1" +  substr(doc_year,3,2) + m.  */
				  /*easnew*/  strYearChr = strYear2Let(doc_year).
				  /*easnew*/  strYrChr = strYr2Lt(STRING(YEAR(TODAY))).
				  /*easnew*/  mm = fill("0",6 - length(string(doc_sn_last))).
				  /*easnew*/  mm = mm + STRING(doc_sn_last).
				  /*easnew*/  sanbr = "YS" + strYearChr + m.
				  /*easnew*/  pinbr = "1" + strYrChr + mm.
