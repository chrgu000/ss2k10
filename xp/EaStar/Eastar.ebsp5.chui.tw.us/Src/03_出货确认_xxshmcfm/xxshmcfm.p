...
def new shared var fm_effdate as date .  /*add-by-davild20051128*/
/*neil-060709*/ define variable addcimsodqty like sod_qty_ship.
...
/* SS - 091230.1 - B */
        field xxsq_so_line   like sod_line
/* SS - 091230.1 - E */
...
/*@@@@@@@@*/ vdisp_zero_qty label "Display Zero Quantity"
  /*begin-davild20051128*/
   /* skip */
    fm_site
    fm_effdate  label "Eff Date"
/*end-davild20051128*/
	WITH frame a side-labels width 80.
	 assign fm_effdate = today
   /*neil060711  fm_site = "EAST" .*/
   /*neil060711*/ fm_site = "SAMO".
	/* SET EXTERNAL LABELS */
...
/* SS - 091230.1 - E */
	...
	FORM 
        ...
        shad_ctn_line
	  	...
	  	shad_so_nbr
/* SS - 091230.1 - B */
        xxsq_so_line format ">>9" label "Ln"
/* SS - 091230.1 - E */
	  	shad_part
	  	vdec_shad_open_qty 	format "->,>>>,>>9.9<"	LABEL "Open QTY"
	  	xxsq_ship_qty	format "->,>>>,>>9.9<"	LABEL "Confirm QTY"
...
/*EAS043A1 */ vchr_CustFeedback = shah_feedback.
		...
		DO ON ERROR UNDO,RETRY:	
			UPDATE	
				...
				vchr_CustFeedback
/*@@@@@@@@*/    vdisp_zero_qty
fm_site fm_effdate   /*add-by-davild20051128*/
		    WITH FRAME a.
		find first glc_cal where glc_start <= fm_effdate and glc_end >= fm_effdate no-lock no-error.
		if not avail glc_cal then 
		DO:
	        	message "Invalid period/year " .
	            NEXT-PROMPT fm_effdate WITH FRAME a.
...
/*EAS043A1                    IF shah_consig = "C" THEN DO:			    		 */
			    		FIND FIRST shloc_hist where shloc_sanbr = shah_sanbr and shloc_sq = shad_sq 
			    			and shloc_ctn_line = shad_ctn_line and shloc_site = fm_site
...
/*@@@@@@@@*/                    IF (not vdisp_zero_qty and vdec_shad_open_qty > 0) or (vdisp_zero_qty and vdec_shad_open_qty >= 0) THEN
                    ...
                    assign xxsq_sanbr = shah_sanbr
                           ...
                           .
                            /* SS - 091230.1 - B */
                            find first snh_ctn_hdr where snh_sn_nbr = shad_snnbr and snh_sq_nbr = shad_sq and snh_ctn_status <> "d" no-lock no-error.
                            xxsq_so_line = if available snh_ctn_hdr then snh_so_line else  0 .
                            /* SS - 091230.1 - E */
                    END.
/*EAS043A1 add end.*/ 
...
/*EAS043A1 		    		
            ...
            assign
            /*fm_site = "EAST"  */    /*add-by-davild20051128*/
                   fm_loc = xxsq_shploc.
...
/* SS - 091230.1 - B */
find loc_mstr where loc_site = fm_site and loc_loc = vchr_loc no-lock no-error.
if available loc_mstr then do:
    if loc_type = "fg" then 
         assign fm_lotser = "" 
                fm_lotref = "".
    else assign fm_lotser = shah_sanbr 
                fm_lotref = if available shah_hdr then string(shah_etdhk) else "".
end.	
/* SS - 091230.1 - E */
...
/*@@@@@@@@*/    find first sod_det 
                        where sod_nbr = xxsq_so_nbr 
                /* SS - 091230.1 - B */
                         and sod_line = xxsq_so_line
                /* SS - 091230.1 - E */
                        AND sod_part = xxsq_part 
                no-lock no-error.
...
/*@@@@@@@@*/                    and vdec_shad_open_qty <> 0
                ...
                /* SS - 091230.1 - E */
			    	...
			    	/*if sod_qty_ord =  sod_qty_ship then next .  davild-20060708*/
			    	
						/**********************neil-060709b*************************/
						addcimsodqty = 0 .
						for each xcim_sod_det where xcim_sod_nbr = sod_nbr and xcim_sod_line = sod_line no-lock :
						    addcimsodqty = addcimsodqty + xcim_sod_qty_ship.
						end.    
			    	if sod_qty_ord - sod_qty_ship - addcimsodqty <= 0 then next.
			    	/**********************neil-060709e**************************/
			    	...
			    	ASSIGN
						...
						xcim_sod_inv_date	 	=	fm_effdate  /*add by davild20051214*/
						xcim_sod_line		 	=	sod_line
/*neil-060709 xcim_sod_qty_ship	 	=	min(sod_qty_ord - sod_qty_ship,vdec_shad_open_qty)*/
/*neil-060709*/	xcim_sod_qty_ship	 	=	min(sod_qty_ord - sod_qty_ship - addcimsodqty,vdec_shad_open_qty)
						xcim_sod_site		 	= fm_site
...
/*@@@@@@@@*/                    FOR EACH xxsq_tmp where xxsq_ship_qty <> 0:
                        ...
                        DO:
                            ...
                            /*shad_dte01 = fm_effdate .*/   /*add-by-davild20051128*/
/*EAS043A1                            IF shah_consig = "C" THEN DO:			    		 */
                                FIND FIRST shloc_hist where shloc_sanbr = shah_sanbr and shloc_sq = shad_sq 
                                    and shloc_ctn_line = shad_ctn_line and shloc_site = fm_site
...
/*EAS043A1*/ 					MESSAGE " S A confirmed." VIEW-AS ALERT-BOX.
					MESSAGE " ".
					MESSAGE " ".
