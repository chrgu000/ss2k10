...
repeat:
   ...
   editing:
      ...
      if frame-field = "wo_nbr" then do:
         ...
         if input wo_nbr =  "" then do:
            {mfnp01.i wo_mstr wo_lot wo_lot wo_nbr wo_nbr wo_lot}
         end.
         else do:
            nbr = input wo_nbr.
   ...
   if nbr <> "" and input wo_lot <> "" then
      find wo_mstr where wo_nbr = nbr using wo_lot no-lock no-error.
   ...
   then do:
      if not can-find(wr_route where wr_lot = wo_lot
         and wr_op = wo-op)
      then do:
   ...
   if not base and jp then do:
      find wo_mstr where wo_lot = parent_lot no-lock no-error.
      find pt_mstr where pt_part = wo_part no-lock no-error.
      if available pt_mstr then desc1 = pt_desc1.
   end.
