...
/* REVISION: eb sp5   LAST MODIFIED: 01/10/03 BY: *EAS023A* Jack Huang     */
...
/*eas001a         define shared variable addr as character format "x(38)" extent 6.*/
/*eas001a*/         define shared variable addr as character format "x(30)" extent 6.
         define shared variable billto as character format "x(38)" extent 6.
/*eas001a         define shared variable vendor as character format "x(38)" extent 6.*/
/*eas001a*/         define shared variable vendor as character format "x(30)" extent 6.
/*eas001a         define shared variable shipto as character format "x(38)" extent 6.*/
/*eas001a*/         define shared variable shipto as character format "x(30)" extent 6.
...
/*eas001a         define variable ext_cost like pod_pur_cost format "(z,zzz,zzz,zz9.99)".*/
/*eas001a*/         define variable ext_cost like pod_pur_cost format "$->,>>>,>>9.999".
         define variable desc1 like pod_desc.
         define variable desc2 like pt_desc2.
/*eas001a         define variable qty_open like pod_qty_ord format "->>>>>>9.9<<<<<<".*/
/*eas001a*/         define variable qty_open like pod_qty_ord format "->>,>>>,>>9.99".
...
/*eas001a*/ define shared variable title1# as character format "x(70)" extent 3.
/*eas001a*/ define shared variable title2# as character format "x(30)" .
/*eas001a*/ define shared variable vend_fax like ad_fax.
/*eas001a*/ define shared variable vend_city like ad_city.
/*eas001a*/ define shared variable rev# as character format "x(3)".
/*eas001a*/ define shared variable curr2# like po_curr.
/*eas001a*/ define shared variable cmmt1# as character format "x(75)".
/*eas001a*/ define shared variable page_yn as logical init yes.
/*eas001a*/ define shared variable um# as character format "x(5)".
/*eas001a*/ define shared variable amount# like pod_pur_cost format "->,>>>,>>9.99".
/*eas001a*/ define shared variable buyer# as character format "x(20)".
/*eas001a*/ define shared variable due_date as character format "x(9)".
/*eas001a*/ define shared variable rev_date as character format "x(9)".
/*eas001a*/ define shared variable ord_date# as character format "x(9)".
/*eas001a*/ define shared variable date1# as character format "x(9)".
/*eas001a*/ define shared variable date2# as character format "x(9)".
/*eas001a*/         define variable j as integer.
/*eas001a*/         define variable k as integer.
/*eas001a*/         define variable mline as integer.
/*eas001a*/         define variable m1 as integer.
/*eas001a*/         define variable m2 as integer.
/*eas001a*/         define variable m3 as integer.
/*eas001a*/ define variable cont# as character format "x(12)".
/*eas001a*/ cont# = "*** Cont ***".
/*eas001a*/ define shared variable line_yn as logical init no.
/*eas001a*/ define variable months as character format "X(3)" extent 12
          initial
          [
...
/*N09M* -- END INITIAL VALUES --------------------- */
         {po03b01.i}
/*eas001a /*J053*/ {po03d01.i} /* DEFINE FOR FRAME C */ */
/*eas001a*/ {xxpo03d01.i} /* DEFINE FOR FRAME C */
...
/*eas001a /*J20Y*/ with frame c width 80.*/
/*eas001a*/ with frame c width 110.
...
/*eas001a               then desc1 = pt_desc1 + " " + pt_desc2.
/*F0HF         else desc2 = pt_desc1 + " " + pt_desc2. */
/*F0HF*/       else desc2 = pt_desc2. *eas001a*/
/*eas001a*/ then do:
/*@@@@@@@@ /*eas001a*/     desc1 = pt_desc1.*/
/*@@@@@@@@*/ /*eas001a*/     desc1 = string(pt_desc1,"x(24)") + " " + pt_desc2.
/*@@@@@@@@ /*eas001a*/      desc2 = pt_desc2.*/
/*@@@@@@@@*/ /*eas001a*/      desc2 = string(pt__chr01,"x(24)") + " " + pt__chr02.
/*eas001a*/ end.
            end.
...
/*eas001a add***************************************************************/
          ...
          if page-size - det_lines - line-counter < 15 then do:
          put "----------------------------------------------------------------------------------------------------------" at 1.
	  put skip(5).
...
/*eas023a        if pod_vpart <> "" then put pod_vpart at 9.  */
/*eas023a */       if pod_vpart <> "" then put pod_vpart format "x(40)" at 9.
...
/*eas001a            {gpcmtprt.i &type=PO &id=pod_cmtindx &pos=5
...
/*eas001a* add*****************************/
	  ...
	  if mline > 0 then do:
	  if page-size - mline - line-counter < 15 then do:
          put "----------------------------------------------------------------------------------------------------------" at 1.
	  put skip(5).
	  put "EASTAR(H.K.)LIMITED" at 75.
	  put skip(4).
...
/*				PUT "** This line was created on " + string(DAY(xpv_mod_date),"99") + " " + months[month(xpv_mod_date)] +
		       " " + SUBSTRING(STRING(YEAR(xpv_mod_date)),3,2) at 9 format "x(60)" skip.*/
				PUT "** New order item line <" + string(pod_line) + "> has been added" at 9 format "x(60)" skip.
/*Leemy*/   END.
/*Leemy*/   ELSE DO:
/*****************leemy end****************************************/              
		    if xpv_pur_cost_new <> xpv_pur_cost_old then 
		       put "** The price from " + trim(string(xpv_pur_cost_old,"->>,>>9.99<")) + " change to " + trim(string(xpv_pur_cost_new,"->>,>>9.99<")) at 9 format "x(60)" skip.
		    if xpv_qty_ord_new <> xpv_qty_ord_old then 
		       put "** The order quantity from " + trim(string(xpv_qty_ord_old,"->>,>>>,>>9.99")) + " change to " + trim(string(xpv_qty_ord_new,"->>,>>>,>>9.99")) at 9 format "x(60)" skip.
		    if xpv_due_date_new <> xpv_due_date_old then do:
		      date1# = "".
		      date2# = "".
		      repeat m1 = 1 to 12:
			  ...
			  if m2 = m1 then do:
		       ...
		       put "** The delivery schedule change from " + date1# + " to " + date2# at 9 format "x(60)" skip.
		    END.
/*Leemy*/   END.			    
	  end. /*mline > 0*/
		end.
...
/*eas001a add*******************************************/
          if page-size - line-counter < 18 then do:
...
/***********************
                  do while page-size - line-counter > 18:
                     put skip(1).
                  end.
   put "  " at 1.
   put "----------------" to 106.
...
k = 0.
	      for each cmt_det  where cmt_indx = po_cmtindx and (lookup("PO",cmt_print) > 0 ) and cmt_seq = 1 no-lock:
           ...
           if k < 15 then do:
          if page-size - k - line-counter < 22 then do:
		  do while page-size - line-counter > 18:
                     put skip(1).
                  end.
          end.
                  do while page-size - line-counter - k > 22:
                     put skip(1).
                  end.
	  end.
	  else do:
   ...
   put "================" to 106.
          ...
          if page-size - k - line-counter < 19 then do:
                  do while page-size - line-counter > 14:
                     put skip(1).
                  end.
          put "----------------------------------------------------------------------------------------------------------" at 1.
...
/********************/
           if k < 15 then do:
                  do while page-size - k - line-counter > 19:
                     put skip(1).
                  end.
           end.
	  put "----------------------------------------------------------------------------------------------------------" at 1.
	  ...
	  put skip(1).
	  put "REMARKS:" at 1.
	  put "A Certificate of Compliance document shall be attached with each" at 13.
	  put "of delivery of your product by part number and send one copy with" at 13.
	  put "your invoice." at 13.
	  put "Please mark our PO#,P/N# & Country of origin on the invoice." at 13.
	  put skip(1).
	  put "EASTAR(H.K.)LIMITED" at 75.
	  put skip(4).
	  put "__________________________" at 1.
