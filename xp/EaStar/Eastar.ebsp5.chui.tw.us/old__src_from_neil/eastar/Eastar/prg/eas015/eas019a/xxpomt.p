...
/* $Revision: 1.13.2.13.2.4 $         BY:Anup Pereira         DATE: 07/10/00  ECO: *N059*   */
...
/* REVISION: 9.1     LAST MODIFIED: 04/10/02 BY: *M1WX* Deirdre O'Brien       */
/* REVISION: 9.1     LAST MODIFIED: 01/09/03 BY: *esa019a* jack       */
/*                                                                            */
...
&SCOPED-DEFINE pomt_p_1 "Ln Disc"
/* MaxLen: Comment: */

&SCOPED-DEFINE pomt_p_7 "Comments"
/* MaxLen: Comment: */

/*N0F3*
 * &SCOPED-DEFINE pomt_p_3 " Ship To "
 * /* MaxLen: Comment: */
 *
 * &SCOPED-DEFINE pomt_p_4 "Purchase Order"
 * /* MaxLen: Comment: */
 *
 * &SCOPED-DEFINE pomt_p_6 " Supplier "
 * /* MaxLen: Comment: */
 *N0F3*/
...
define new shared variable disc like pod_disc label {&pomt_p_1}.
...
define new shared variable pocmmts like mfc_logical label {&pomt_p_7}.
...
/*M0S8*/ define variable l_rebook_lines like mfc_logical initial no no-undo.
/*N13P*/ define variable poTrans as character no-undo.
/*N13P*/ define variable extKey as character no-undo.
/*N13P*/ define variable vend as character no-undo.

/*EAS019A **Add Begin ************* */
define new shared variable poc_po_pre# like poc_po_pre .
define new shared variable grp# as char.

  form                      
      poc_po_pre# format "x(3)" /* label ""               */
  with overlay frame a0 row 8 centered side-labels width 20 attr-space THREE-D.		
  setFrameLabels(frame a0:handle). 

/*EAS019A **Add End   ************* */


/*N13P*/ /* COMMON API CONSTANTS AND VARIABLES */
/*N13P*/ {mfaimfg.i}

/*N13P*/ /* PURCHASE ORDER API TEMP-TABLE, NAMED USING THE "Api" PREFIX */
/*N13P*/ {popoit01.i}

/*N13P*/ {icieit01.i}
/*N13P*/ {mfctit01.i}
/*N13P*/ if c-application-mode = "API" then do on error undo, return:

/*N13P*/    /* GET HANDLE OF API CONTROLLER */
/*N13P*/    {gprun.i ""gpaigh.p""
                     "(output ApiMethodHandle,
                       output ApiProgramName,
                       output ApiMethodName,
                       output apiContextString)"}

/*N13P*/    /* GET LOCAL PO MASTER TEMP-TABLE */
/*N13P*/    create ttPurchaseOrder.
/*N13P*/    run getPurchaseOrderRecord in ApiMethodHandle
                        (buffer ttPurchaseOrder).

/*N13P*/    run getpoTrans in ApiMethodHandle
                        (output poTrans).

/*N13P*/    run getPurchaseOrderCmt in ApiMethodHandle
                        (output table ttPurchaseOrderCmt).

/*N13P*/ end.  /* If c-application-mode = "API" */
...
MAINLOOP:
repeat:
/*N13P*/if c-application-mode = "API" and retry then
/*N13P*/   return error.
...
/*N13P*/if c-application-mode = "API" then
/*N13P*/do:
/*N13P*/   if poTrans = "GETPONUM" then
/*N13P*/   do:
/*N13P*/      {pxrun.i &PROC='get-input'}
/*N13P*/      assign ttPurchaseOrder.nbr = ponbr.
...
/*N13P*/   end.
/*N13P*/   else
/*N13P*/      assign ponbr = ttPurchaseOrder.nbr.
/*N13P*/end. /* if c-application-mode = "API" */
...
/* init Ctrl File */
  ...
  find first xxpoc_ctrl no-lock no-error.
  if not avail xxpoc_ctrl then do:
    create xxpoc_ctrl.
           assign xxpoc_po_pre2 = "TM"
                  xxpoc_po_nbr2 = 0
                  xxpoc_po_pre3 = ""
                  xxpoc_po_nbr3 = 0                  
            .       
  ...
  assign
      grp# = "OTHER"
      poc_po_pre# = xxpoc_po_pre3
  .

find first usr_mstr where usr_mstr.usr_userid = global_userid no-lock no-error.
if avail usr_mstr then do:
    if (LOOKUP("SUPERVISOR",usr_mstr.usr_groups) > 0 ) THEN DO:
          assign
             grp# = "SUPERVISOR"
             poc_po_pre# = ""
            .
    ...
    END. /* 1. SUPERVISOR  */
    ELSE IF (LOOKUP("BUYER-PRC",usr_mstr.usr_groups) > 0 ) THEN DO:
          assign
             grp# = "BUYER-PRC"
             poc_po_pre# = "CM"
            .
    ...
    END. /* 2. BUYER-PRC */
    ELSE IF (LOOKUP("BUYER-HK",usr_mstr.usr_groups) > 0 ) THEN DO:
          assign
             grp# = "BUYER-HK"
             poc_po_pre# = "TM"
            .
...
end. /* if avail usr_mstr */
  if grp# = "SUPERVISOR" then do:       
           CLEAR FRAME dtitle NO-PAUSE.             
           CLEAR frame a NO-PAUSE.
           CLEAR frame vend NO-PAUSE.
           CLEAR frame ship_to NO-PAUSE.
           CLEAR frame b NO-PAUSE.

     update poc_po_pre# 
     validate(poc_po_pre# = xxpoc_po_pre2 or poc_po_pre# = xxpoc_po_pre3
                            or poc_po_pre# = poc_po_pre,"PO Prefix Not Found !")
                            
     with frame a0.                            
  end. /* end if grp# = "SUPERVISOR" */
           CLEAR FRAME a0 NO-PAUSE.
           Hide FRAME a0  NO-PAUSE.
           view frame dtitle.
           view frame a.
           view frame vend.
           view frame ship_to.
...
/*eas019a del           
if (grp# = "OTHER") and (poc_po_pre# = "") then do:


          if global_user_lang = "tw" then do:
               message "你沒有權限訪問PO!" VIEW-AS ALERT-BOX error BUTTONS OK.                      
               undo , retry .
          end.
          else do:
               message "You Have Not Right To Access  PO !" VIEW-AS ALERT-BOX error BUTTONS OK.                      
               undo , retry .          
          end.
end.
**** */
...
/*EAS019A      if not available poc_ctrl then  */
/*EAS019A         create poc_ctrl.    */
...
/*N13P*/ /* Added to resolve compile-time problem */
/*N13P*/ recid(po_mstr).  /*Oracle Standards*/

/*N13P*/ /* Added to resolve compile-time problem */
/*N13P*/ recid(poc_ctrl).  /*Oracle Standards*/

/*N13P*/ if c-application-mode = "API" then
/*M1WX* /*N13P*/    {gprun.i ""poeqref.p""
                     "(input ttPurchaseOrder.eqRef,input po_nbr)"}  */

/*M1WX*/    {gprun.i ""poeqref.p""
                     "(input ttPurchaseOrder.extRef,input po_nbr)"}
...
/*N13P*/    if c-application-mode <> "API" then
/*N13P*/       undo mainloop, retry mainloop.
/*N13P*/    else /* if c-application-mode = "API"*/
/*N13P*/       undo mainloop, return error.
         end.
...
/*N13P*/
/*N13P*/if c-application-mode = "API" then
/*N13P*/   assign {mfaiset.i po__qadc02 ttPurchaseOrder.qadc02}.
...
/*N13P*/ if retry and c-application-mode = "API" then
/*N13P*/    return error.
/*N13P*/ if c-application-mode <> "API" then
/*N13P*/ do:
...
/*N13P*/ end. /* if c-application-mode <>"API" */
/*N13P*/ else
/*N13P*/ do: /* if c-application-mode ="API" */
/*N13P*/    assign vend = po_vend
/*N13P*/           {mfaiset.i vend ttPurchaseOrder.vend}.
/*N13P*/    for first vd_mstr no-lock
/*N13P*/        where vd_addr = vend:
/*N13P*/    end.
/*N13P*/    if not available vd_mstr then
/*N13P*/    do:
/*N13P*/       {pxmsg.i
                  &MSGNUM = 2
                  &ERRORLEVEL = 3}

/*N13P*/       undo, return error.
/*N13P*/     end.
/*N13P*/  end.
         /* DO NOT ALLOW MOD TO VENDOR IF ANY RECEIPTS */
         if (not new_po and po_vend <> old_vend) or
            (not new_po and po_vend entered) then do:
            {pxrun.i &PROC='validateSupplierReceipts' &PROGRAM='popoxr.p'
               &PARAM="(input old_vend,
                        input po_nbr)"
               &NOAPPERROR=TRUE &CATCHERROR=TRUE}
            if return-value <> {&SUCCESS-RESULT} then do:
/*N13P*/       if c-application-mode <> "API" then do:
...
/*N13P*/       end.  /* If c-application-mode <> "API" */
/*N13P*/       else
/*N13P*/          undo vendblk, return error.
            end.
/*M0S8*/    else
/*M0S8*/       l_rebook_lines = yes.
         end.
...
/*N13P*/ if c-application-mode <> "API" then
         ...
         if not new_po and input po_vend <> old_vend then do:
            {pxrun.i &PROC='validateScheduledOrder' &PROGRAM='popoxr.p'
                &PARAM="(input po_sched,
...
/*N13P*/ else do:    /*if c-application-mode = "API"*/
            {pxrun.i
...
/*N13P*/else    /*if c-application-mode = "API"*/
        do:
/*N13P*/    if ((not new_po) and
/*N13P*/        (ttPurchaseOrder.vend <> ?) and
/*N13P*/        (old_vend <> ttPurchaseOrder.vend))
/*N13P*/        then do:
/*N13P*/
/*N13P*/           {pxrun.i &PROC='validateSupplierCurrency' &PROGRAM='popoxr.p'
/*N13P*/              &PARAM="(input po_curr,
/*N13P*/                input ttPurchaseOrder.vend)"
/*N13P*/              &NOAPPERROR=TRUE &CATCHERROR=TRUE}
/*N13P*/
/*N13P*/           if return-value <> {&SUCCESS-RESULT} then do:
/*N13P*/              undo mainloop, return error.
/*N13P*/          end.
/*N13P*/    end.
/*N13P*/end.
...
/*N13P*/else
/*N13P*/   assign {mfaiset.i po_vend ttPurchaseOrder.vend}.
...
/*N13P*/ else    /*if c-application-mode = "API"*/
         do:
/*N13P*/    assign  {mfaiset.i po_ship ttPurchaseOrder.ship}.
...
/*N13P*/ else  /* if c-application-mode = "API" */
/*N13P*/ do:
/*N13P*/    for first ad_mstr where ad_addr = po_ship no-lock:
/*N13P*/    end.
/*N13P*/ end.
/*N13P*/                .
...
/*M0S8*/ if l_rebook_lines and
/*M0S8*/    not pBlanket
/*M0S8*/ then do:
/*M0S8*/    {gprun.i ""pomtrb.p"" "(input old_vend)"}.
/*M0S8*/    l_rebook_lines = no.
/*M0S8*/ end. /* IF L_REBOOK_LINES AND NOT BLANKET ORDER */
...
/*N13P*  if pocmmts = yes then do:*/
/*N13P*/ if pocmmts = yes then do on error undo mainloop, retry:
/*N13P*/    if c-application-mode = "API" then
/*N13P*/    do:
/*N13P*/       {gpttcp.i ttPurchaseOrderCmt
                         ttTransComment
                         "ttPurchaseOrderCmt.apiExternalKey = ttPurchaseOrder.apiExternalKey"}
/*N13P*/       run setTransComment in ApiMethodHandle (input table ttTransComment).
/*N13P*/    end.
...
/*eas015 add*******************************/
	for each xxpoa_det where xxpoa_nbr = po_nbr:
		    xxpoa_curr        = po_curr.
		    xxpoa_vend        = po_vend.
		    xxpoa_buyer       = po_buyer.
	end.
/*eas015 add*******************************/

      /* LINE ITEMS */
/*leemy*        {gprun.i ""popomta.p""}*/
...
/*N13P*/ /* RETURN SUCCESS STATUS TO API CALLER */
/*N13P*/ if c-application-mode = "API" then
/*N13P*/    return {&SUCCESS-RESULT}.
...
/*N1D7*/       /*Changed to mfnp05.i and last 2 parameters (yes yes) removed*/
...
/*J3Q4**   do transaction on error undo,retry: */
/*N13P*/ if c-application-mode <> "API" then
            find po_mstr where po_nbr = input po_nbr no-lock no-error.
/*N13P*/ else /*if c-application-mode = "API" */
/*N13P*/ do:
/*N13P*/    for first po_mstr where po_nbr = ttPurchaseOrder.nbr:
            end.
/*N13P*/ end.

/*EAS019A Add Begin *************** */
/* process "OTHER" */
IF grp# = "OTHER" THEN DO:
     if avail po_mstr then do:
         if (substr(po_nbr,1,2) = "CM") or (substr(po_nbr,1,2) = "TM") then do:
          if global_user_lang = "tw" then do:
               message "你沒有權限訪問這種類型的PO!" VIEW-AS ALERT-BOX error BUTTONS OK.                      
               undo , retry .
          end.
          else do:
               message "You Have Not Right To Access This Type PO !" VIEW-AS ALERT-BOX error BUTTONS OK.                      
               undo , retry .          
          end.                           
        end. /* if (substr   ) */
     end. /* if avail po_mstr */   
     else do:
         if ((substr(input po_nbr,1,2) = "TM" ) or (substr(input po_nbr,1,2) = "CM" ))
         and (input po_nbr <> "") then do:
             if global_user_lang = "tw" then do:
               message "你沒有權限建立這種類型的PO!" VIEW-AS ALERT-BOX error BUTTONS OK.                      
               undo , retry .
             end.
             else do:
               message "You Have Not Right To Create This Type PO !" VIEW-AS ALERT-BOX error BUTTONS OK.                      
               undo , retry .          
             end.             
         end. 
...
END.
/* process "TM" "CM" and "SUPERVISOR" */

ELSE DO:
  if avail po_mstr then do:
     if (substr(po_nbr,1,length(poc_po_pre#)) <> poc_po_pre# ) and (grp# <> "SUPERVISOR") then do:
          if global_user_lang = "tw" then do:
               message "你沒有權限訪問這種類型的PO!" VIEW-AS ALERT-BOX error BUTTONS OK.                      
               undo , retry .
          end.
     ...
     if (substr(input po_nbr,1,length(poc_po_pre#)) <> poc_po_pre# ) and (input po_nbr <> "") then do:
          if global_user_lang = "tw" then do:
               message "你沒有權限建立這種類型的PO!" VIEW-AS ALERT-BOX error BUTTONS OK.                      
               undo , retry .
          end.
...
/*N13P*/ if (c-application-mode <> "API" and input po_nbr <> "") then
            ponbr = input po_nbr.
/*N13P*/ else
/*N13P*/ do:
/*N13P*/    if (c-application-mode = "API" and ttPurchaseOrder.nbr <> "")
/*N13P*/    then
/*N13P*/       ponbr = ttPurchaseOrder.nbr.
/*N13P*/ end.

/*J3Q4*/ do transaction on error undo,retry:
/*N13P*/    if c-application-mode = "API" and retry then
/*N13P*/       return error.
/*N13P*/    if (c-application-mode <> "API" and input po_nbr = "") or
/*N13P*/        (c-application-mode = "API" and
...
/*EAS019A Add Begin **************************** */
/* EAS019A 02-11 ****************  loopNextNo:
repeat :
***** EAS019A 02-11 ************************************* */
               {pxrun.i
                  &PROC='getNextPONumber'
                  &PROGRAM='xxpopoxr.p'
                  &PARAM="(output ponbr,input poc_po_pre#)"
                  &NOAPPERROR=TRUE
                  &CATCHERROR=TRUE}
...
/*N13P*/ /* Line below is for Desktop 2*/
/*N13P*/ {gpbrparm.i &browse=dtlu001.p &parm=c-brparm1 &val=poNbr}
END PROCEDURE.
