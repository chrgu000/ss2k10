...
/*20050617*/	    if pc_curr <> po_curr then do:
   ...
   find last pc_mstr use-index pc_part where pc_part = pod_part 
                  and pc_list = "Q-PRICE"
/*20050617		  and pc_curr = po_curr */
		  and pc_amt_type = "L"
		  no-lock no-error.
...
/*20050617*/	    if pc_curr <> po_curr then do:
	       find first exr_rate where exr_curr1 = po_curr and
		    ...
		    exr_start_date <= today and exr_end_date >= today
	            no-lock no-error.
	       if available exr_rate then a_quote = exr_rate / exr_rate2 * pc_amt[1].
	    end. /*20050617*/
	end.
	else do:
	   q_yn = no.
	end.
	 find xxpoa_det where xxpoa_nbr = pod_nbr
