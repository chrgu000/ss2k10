...
/* Revision: 1.26        BY: Jean Miller       DATE:06/03/01    ECO: *M11Z*   */
/* $ Revision: 1.26.1.1 $        BY: John Lannon       DATE:10/09/01    ECO: *N13P*   */
/*                                                                            */
...
&SCOPED-DEFINE gpcmmt01_p_1 " Transaction Comments "
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_2 "Print On Shipper"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_3 "Print On Customer Reports"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_4 "Print On EMT PO"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_5 "Print On Quote"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_6 "Print On Rtn To Supplier"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_7 "Print On Bill of Lading"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_8 "Print On Packing List"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_9 "Print On Invoice"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_10 "Print On PO Receipt"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_11 "Print On Internal Reports"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_12 "Print On Schedules"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_13 "Print On Purchase Order"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_14 "Print On Distr. Order"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_15 "Print On I/S Request"
/* MaxLen: Comment: */

&SCOPED-DEFINE gpcmmt01_p_16 "Print On Sales Order"
/* MaxLen: Comment: */

/* ********** End Translatable Strings Definitions ********* */

define input parameter file_name as character no-undo.
/*eas024*/ define input parameter pchr_title as character no-undo.
...
define variable prt_on_quote    like mfc_logical label {&gpcmmt01_p_5}  no-undo.
define variable prt_on_so       like mfc_logical label {&gpcmmt01_p_16} no-undo.
define variable prt_on_invoice  like mfc_logical label {&gpcmmt01_p_9}  no-undo.
define variable prt_on_packlist like mfc_logical label {&gpcmmt01_p_8}  no-undo.
define variable prt_on_po       like mfc_logical label {&gpcmmt01_p_13} no-undo.
define variable prt_on_rtv      like mfc_logical label {&gpcmmt01_p_6}  no-undo.
define variable prt_on_rct      like mfc_logical label {&gpcmmt01_p_10} no-undo.
define variable prt_on_shpr     like mfc_logical label {&gpcmmt01_p_2}  no-undo.
define variable prt_on_bol      like mfc_logical label {&gpcmmt01_p_7}  no-undo.
define variable prt_on_schedule like mfc_logical label {&gpcmmt01_p_12} no-undo.
define variable prt_on_cus      like mfc_logical label {&gpcmmt01_p_3}
   initial yes no-undo.
define variable prt_on_intern   like mfc_logical label {&gpcmmt01_p_11}
   initial yes no-undo.
define variable prt_on_isrqst   like mfc_logical label {&gpcmmt01_p_15} no-undo.
define variable prt_on_do       like mfc_logical label {&gpcmmt01_p_14} no-undo.
...
/*N13P*/ {mfaimfg.i} /* Common API constants and variables */

/*N13P*/ {mfctit01.i} /* API transaction comment temp tables */

/*N13P*/ define variable validCmtPrintList        as character   no-undo.
/*N13P*/ define variable validPrintEntry          as character   no-undo.
/*N13P*/ define variable createTransComment     like mfc_logical no-undo.
/*N13P*/ define variable cmtPrint               like cmt_print   no-undo.
/*N13P*/ define variable commentTextProvided    like mfc_logical no-undo.
/*N13P*/ define variable masterCommentPageCount   as integer     no-undo.
/*N13P*/ define variable renumberIncrement        as integer     no-undo.
/*N13P*/ define variable newPageNumber            as integer     no-undo.
/*N13P*/ define variable cmtSeq                 like cmt_seq     no-undo.
...
/*N13P*/    run getTransComment in ApiMethodHandle
...
/*eas024 title color normal (getFrameTitle("TRANSACTION_COMMENTS",29))*/
/*eas024*/ title color normal (getFrameTitle(pchr_title,length(pchr_title) * 2))
...
do with frame cmmt01:
   if cmtindx <> 0 then
      for first cmt_det
         fields(cmt_cmmt cmt_indx cmt_lang cmt_print cmt_ref cmt_seq cmt_type)
...
repeat with frame cmmt01:
   ...
   if can-find (first cmt_det where cmt_indx = cmtindx) or
      first-time = no
   then do:
   ...
   do on error undo, retry:
      ...
      then do:
         for first cd_det
            fields(cd_cmmt cd_lang cd_ref cd_seq cd_type)
            using cd_ref
            and   cd_type
            and   cd_lang
            no-lock: end.
...
/*M0QX*/ if cd_seq:screen-value = "" then
/*M0QX*/ do:
/*M0QX*/    {pxrun.i &PROC='findMasterComment' &PROGRAM='gpcmxr1.p'
/*M0QX*/          &PARAM="(input cd_ref:screen-value,
/*M0QX*/                   input cd_type:screen-value,
/*M0QX*/                   input cd_lang:screen-value,
/*M0QX*/                   buffer cd_det)"
/*M0QX*/          &NOAPPERROR=True
/*M0QX*/          &CATCHERROR=True}
/*M0QX*/ end. /* IF CD_SEQ:SCREEN-VALUE = "" */
/*M0QX*/ else
/*M0QX*/ do:
            {pxrun.i &PROC='processRead' &PROGRAM='gpcmxr1.p'
                  &PARAM="(input cd_ref:screen-value,
...
/*M0QX*/ end. /* ELSE DO */
   ...
   then do:
      ...
      find next cmt_det no-lock
         where cmt_indx = cmtindx no-error.
      if not available cmt_det then
   ...
   form
      ...
      prt_on_po         colon 25 label {&gpcmmt01_p_4}
   ...
   form
      prt_on_intern     colon 30
      prt_on_po         colon 30 label {&gpcmmt01_p_4}
...
/*N13P* REPLACE mfmsg**.i WITH pxmsg.i *
.                     {mfmsg03.i 4667 3
.                        string(ttTransComment.apiSequence)
.                        ApiContextString
.                        """"
.                     }
*N13P*/
                  ...
                  */
                  ...
                  if commentTextProvided = no then do:
                     ...
                     if ttTransComment.masterCmtSeq <> ? then do:
                        ...
                        for first cd_det
                          where cd_ref  = ttTransComment.ref
