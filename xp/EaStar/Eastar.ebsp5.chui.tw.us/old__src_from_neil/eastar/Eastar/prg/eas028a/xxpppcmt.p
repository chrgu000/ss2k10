...
/* REVISION: eBSP5      LAST MODIFIED: 05/22/03   BY: *EAS028A*  Ricky Ho */
...
/*EAS028A*/ define variable have_right as logical init "no".
...
/*@@@ Ricky add begin **************************************
        define variable have_right as logical init "no".
	find usr_mstr where usr_userid = global_userid no-lock no-error.
	if available usr_mstr and index(upper(usr_groups),"Q-PRICE") > 0 then have_right = yes.
	else have_right = no.
@@@ Ricky add begin ***************************************/
...
/*@@@@@@@@ /*EAS028A*/ if recno <> ? and upper(pc_list) = "Q-PRICE" then do:*/
/*@@@@@@@@*/ /*EAS028A*/ if recno <> ? and (upper(pc_list) = "Q-PRICE" or upper(pc_list) = "L-PRICE") then do:
/*EAS028A*/ 	find usr_mstr where usr_userid = global_userid no-lock no-error.
/*EAS028A*/ 	if available usr_mstr and index(upper(usr_groups),"Q-PRICE") > 0 then have_right = yes.
/*EAS028A*/ 	else have_right = no.
/*EAS028A*/ end.
/*EAS028A*/ else have_right = yes.
/*EAS028A*/ if have_right then do:
           if recno <> ? then do:
...
/*EAS028A* /*Kaine*/ 		DEFINE BUFFER pc-mstr FOR pc_mstr.*/
...
/*Kaine*/ 					AND xpc_um = pc_um AND xpc_start = pc_start no-lock NO-ERROR.

/*EAS028A* /*Kaine*/ 			IF NOT AVAILABLE xpc_mstr THEN 
/*Kaine*/ 			DO:
...
*/
          display pc_list pc_curr pc_prod_line pc_part pc_um
            pc_start pc_expire pc_amt_type /*eas024*/ pc_user1 xxcmmt
            				/*Kaine*/	   /*eas024b*/ xpc_supp when available xpc_mstr xpc_adddate when available xpc_mstr with frame a.
...
/*EAS028A*/ end. /* if have_right */
/*EAS028A*/ else do:
/*EAS028A*/	clear frame a no-pause.
/*EAS028A*/	clear frame b no-pause.
/*EAS028A*/	clear frame c no-pause.
/*EAS028A*/ end.

        end.

/*@@@@@@@@ /*EAS028A*/ if upper(input pc_list) = "Q-PRICE" then do:*/
/*@@@@@@@@*/ /*EAS028A*/ if upper(input pc_list) = "Q-PRICE" or upper(input pc_list) = "L-PRICE" then do:
/*EAS028A*/ 	find usr_mstr where usr_userid = global_userid no-lock no-error.
/*EAS028A*/ 	if available usr_mstr and index(upper(usr_groups),"Q-PRICE") > 0 then have_right = yes.
/*EAS028A*/ 	else have_right = no.
/*EAS028A*/	if not have_right then do:
/*EAS028A*/		if global_user_lang = "us" then
/*EAS028A*/		message "Error: You don't have right to access this type of Price List, Please Re-enter!".
/*EAS028A*/		else message "錯誤 : 你沒有權限維護該類價格表﹐請重新輸入".
/*EAS028A*/		next-prompt pc_list.
/*EAS028A*/		undo main-loop,retry.
/*EAS028A*/	end.
/*EAS028A*/ end.
