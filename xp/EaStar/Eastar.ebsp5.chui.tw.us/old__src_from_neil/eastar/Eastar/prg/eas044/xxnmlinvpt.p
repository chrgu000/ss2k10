    ...
    define temp-table shad-det field shad-sanbr like shad_sanbr
     ...
     form
	    ...
	    if sanbr1 = hi_char then sanbr1 = "".
            update sanbr sanbr1 sort_yn charge_yn mar1 return_yn mar2 print_yn
            with frame a.
       ...
       /******************/
      update_yn = yes.
      message "Normal Sales Invoice Print accepted?" update update_yn.
      number# = first_number.
      if update_yn = yes then do:
       if sort_yn = yes then do:
        for each shad_det  where shad_sanbr >= sanbr and shad_sanbr <= sanbr1 and shad_line > 0 
	                     and shad_inv_qty > 0 EXCLUSIVE-LOCK break by shad_sanbr + shad_ponbr + shad_part:
        ...
        for each shad_det  where shad_sanbr >= sanbr and shad_sanbr <= sanbr1 and shad_line > 0 
	                     and shad_inv_qty > 0 EXCLUSIVE-LOCK break by shad_sanbr :
