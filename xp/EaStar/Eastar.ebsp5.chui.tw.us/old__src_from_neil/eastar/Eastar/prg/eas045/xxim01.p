 ...
 define variable k as integer.
    define variable months as character format "X(3)" extent 12
	 ...
	 for each shad_det where shad_sanbr >= sanbr and shad_sanbr <= sanbr1 and shad_line > 0 
	                     and shad_inv_qty > 0 no-lock break by shad_ponbr + shad_part:
	 ...
	 for each shad_det where shad_sanbr >= sanbr and shad_sanbr <= sanbr1 and shad_line > 0 
	                     and shad_inv_qty > 0 no-lock break by shad_sanbr by shad_sanbr + shad_ponbr + shad_part:
   ...
   if sort_yn = yes then do:
      ...
      each shad_det  where shad_sanbr = shah_sanbr and shad_line > 0 and shad_inv_qty > 0 no-lock 
