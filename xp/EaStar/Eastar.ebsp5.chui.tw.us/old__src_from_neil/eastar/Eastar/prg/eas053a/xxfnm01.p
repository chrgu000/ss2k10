	...
	if sort_yn = yes then do:
		...
		break by shad_sanbr + shad_ponbr + shad_part:
			...
			if last-of(shad_sanbr + shad_ponbr + shad_part) then do:
				...
				/*eas053a*  if page-size - line-counter < lines + 2 then page.  */
				...
				FIND first pt_mstr where pt_part = shad_part no-lock no-error.
				if available pt_mstr then do:
					/*eas053a*  IF pt_desc1 <> "" then put pt_desc1 at 14.  */
					/*eas053a*  if pt_desc2 <> "" then put pt_desc2 at 14.  */
					/*eas053a*  if pt__chr01 <> "" then put pt__chr01 at 14 format "x(24)".  */
					/*eas053a*  if pt__chr02 <> "" then put pt__chr02 at 14 format "x(24)".  */
				end.
				/*eas053a*  put skip(1).  */
				um = "".
				price = 0.
