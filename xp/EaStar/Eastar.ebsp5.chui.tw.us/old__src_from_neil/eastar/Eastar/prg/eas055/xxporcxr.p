...
/* Revision: 1.24.3.1 BY: Ashwini G.        DATE: 04/15/02  ECO: *M1XB*       */
/* $Revision: 1.24.3.3 $  BY: Ashwini G.        DATE: 05/24/02  ECO: *N1JY*       */
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
/*eas055      {gprun.i ""poporcb.p""*/
/*eas055*/      {gprun.i ""xxpoporcb.p""
               "(input pShipNbr,
                 input pShipDate,
                 input pInvMov,
                 input """",
...
/*eas055      if (pReceiverNbr <> "") */
/*eas055*/      if (length(pReceiverNbr) > 1)
...
History:
        Last change:  LAL  24 Apr 2000    8:51 am
------------------------------------------------------------------------------*/
   define variable returnData as logical no-undo.

   if pTransactionType = {&TAX-TR-TYPE-PO-RETURN-TO-SUPPLIER} then
      returnData = TRUE.
   else
      returnData = FALSE.
