...
/* Revision: 1.46.1.8      BY: Deirdre O'Brien    DATE: 04/05/01  ECO: *M1WX* */
/* Revision: 1.46.1.9      BY: Ashish M.          DATE: 06/10/02  ECO: *N1KW* */
/* Revision: 1.46.1.10     BY: Karan Motwani      DATE: 07/03/02  ECO: *N1N1* */
/* $Revision: 1.46.1.11 $            BY: Rajaneesh S.       DATE: 07/03/02  ECO: *M1BY* */
/* REVISION: eb sp3 us  LAST MODIFIED: 01/11/03 BY: *EAS019* Leemy Lee        */
/* REVISION: eb sp3 us  LAST MODIFIED: 03/19/03 BY: *EAS015* Apple Tam        */
...
/*EAS015*/   define variable a_log like xxpoa_log.
/*EAS015*/   define variable a_appr like xxpoa_appr.
/*EAS015*/   define variable a_last like xxpoa_last_price.
/*EAS015*/   define variable a_quote like xxpoa_quote.
/*EAS015*/   define variable q_yn as logical.
/*EAS015*/   define variable l_yn as logical.
/*EAS015*/   define variable l_curr like gl_base_curr.
/*EAS015-20051111*/   define variable q-date as date.

/*****************EAS003 begin**************************************/
DEFINE VARIABLE v_pod_pur_cost LIKE pod_pur_cost.
DEFINE VARIABLE v_pod_qty_ord LIKE pod_qty_ord.
DEFINE VARIABLE v_pod_due_date LIKE pod_due_date.
DEFINE VARIABLE vlog_changed AS LOGICAL.
DEFINE VARIABLE v_cmtindx LIKE cmt_indx.
DEFINE VARIABLE v_line LIKE pod_line.
DEFINE VARIABLE vchr_filename AS CHARACTER.
DEFINE VARIABLE vchr_message AS CHARACTER.
DEFINE VARIABLE j AS INTEGER.
DEFINE VARIABLE v_linecopy LIKE pod_line INIT 0.
/*EAS019*/ DEFINE VARIABLE vchr_pod_vpart LIKE pod_vpart.

DEFINE BUFFER cmtdet FOR cmt_det.
/*ss - 071019*/ define new shared variable re-price as logical initial no.
/*****************EAS003 end****************************************/
...
/*****************EAS019 begin**************************************/
				MESSAGE "Ctrl+V=Copy Line detail".
/*****************EAS019 end****************************************/               
/*EAS019*                 set line with frame c*/

               set line
               GO-ON (CTRL-V) with frame c
               editing:
                  /* FIND NEXT RECORD */
...
/*****************EAS019 begin**************************************/						
			IF LASTKEY = KEYCODE("CTRL-V") THEN DO:
				FIND LAST pod_det WHERE pod_nbr = po_nbr USE-INDEX pod_nbrln NO-LOCK NO-ERROR.			
				v_line = pod_line + 1.
				FIND FIRST pod_det WHERE pod_nbr = po_nbr AND pod_line = INPUT LINE NO-LOCK NO-ERROR.
				IF NOT AVAIL pod_det THEN DO:
					FIND LAST pod_det WHERE pod_nbr = po_nbr AND pod_line < INPUT LINE 
						USE-INDEX pod_nbrln NO-LOCK NO-ERROR.
				END.
				IF AVAIL pod_det THEN DO:
/*EAS019*/ 			vchr_pod_vpart = pod_vpart.				
					v_cmtindx = pod_cmtindx.
					vchr_filename = ".\ssi" + mfguser.
					OUTPUT TO value(vchr_filename).
						{xxcim.i}						
					OUTPUT CLOSE.
					INPUT FROM VALUE(vchr_filename).					
					vchr_filename = ".\sso" + mfguser.	
					OUTPUT TO VALUE(vchr_filename).		
...
/*EAS019*/ 			batchrun = NO.
					...
					OS-DELETE VALUE(".\ss" + mfguser).
					OS-DELETE VALUE(".\sso" + mfguser).
					FIND FIRST pod_det WHERE pod_nbr = po_nbr AND pod_line = v_line SHARE-LOCK NO-ERROR.
					IF AVAIL pod_det THEN DO:
						/*must first define buffer cmtdet*/
						IF v_cmtindx <> 0 THEN 
						{gpcmtcpy.i 	&old_index = v_cmtindx
		              					&new_index = pod_cmtindx
										&counter   = j}
						MESSAGE "Copy Accomplished".
						v_linecopy = pod_line.
/*EAS019*/ 				pod_vpart = vchr_pod_vpart.						
						NEXT.
					END.
					ELSE DO:
...
/*****************EAS003 begin**************************************/
		ASSIGN
			v_pod_pur_cost 	= 	pod_pur_cost		
			v_pod_qty_ord   = 	pod_qty_ord 		
			v_pod_due_date  = 	pod_per_date		
			.
/*****************EAS003 end****************************************/
...
/*ss-070726 add************************************************************/
       define variable ii as integer.
	       ...
	       find first pc_mstr where pc_part = pod_part                                  
       	                  and pc_list = po_pr_list2                                         
	       	  and pc_curr = po_curr                                             
	       	  and pc_um = pod_um                                                
	       	  and pc_start <= po_ord_date                                       
	       	  and (pc_expire >= po_ord_date or pc_expire = ?)                                    
	       	/*  and pc_amt_type = "P"*/                                         
	       	  no-lock no-error.                                                 
		     ...
		     if pc_amt_type = "P" then do:
	                ii = 1.                                                                
	            	repeat ii = 1 to 15.                                                   
	            	   if ii = 1 then pod_pur_cost = pc_amt[1].
			   if pc_min_qty[ii] <> 0 and pod__dec01 >= pc_min_qty[ii] then do:    
	            	      pod_pur_cost = pc_amt[ii].                                       
	            	   end.                                                                
	            	end.                                                                   
	             end.                                                                      
                  end.
		        ...
		        {mfmsg.i 2852 3}  /*未找到价格表*/                                    
       	                undo, retry. 
		     end.
		     if not new pod_det then do:
		        if re-price = yes then do:
		           {mfmsg.i 2852 3}  /*未找到价格表*/                                    
       	                    undo, retry. 
			end.
		     end.
...
/*ss-070726 add************************************************************/
            ...
            if sngl_ln = yes then do:
               assign continue = no
                  pod_recno = recid(pod_det).
/*eas023a               {gprun.i ""popomtd.p""}  */
/*EAS019*  /*eas023a */          {gprun.i ""xxpopomtd.p""}  */
/*EAS019*/      IF batchrun THEN DO:
/*EAS019*/ 			{gprun.i ""popomtd.p""}  
/*EAS019*/ 		END.
/*EAS019*/ 		ELSE DO:
/*EAS019*/ 			{gprun.i ""xxpopomtd.p""}
/*EAS019*/ 		END.
               if c-application-mode <> "API" and continue = no then
                  undo mainloop, next mainloop.
               else if c-application-mode = "API" and continue = no then
...
/*20050617		  and pc_curr = po_curr */
/*		  and pc_um = pod_um
		  and pc_start <= po_ord_date
		  and pc_expire >= po_ord_date*/
		  and pc_amt_type = "L"
		  no-lock no-error.
         if available pc_mstr then do:
	    a_last = pc_amt[1].
/*20050617*/	    if pc_curr <> po_curr then do:
	       find first exr_rate where exr_curr1 = po_curr and
		    exr_curr2 = pc_curr and
		    exr_start_date <= today and exr_end_date >= today
	            no-lock no-error.
	       if available exr_rate then do:
	          a_last = exr_rate / exr_rate2 * pc_amt[1].
		  /*message a_last view-as alert-box.*/
	       end.
	    end. /*20050617*/
	 end.
	 else do :
...
/*20051111****************************************************************/
    /*只有一种币别*/
 /*20051111*/  find first pc_mstr where pc_part = pod_part 
                  and pc_list = "Q-PRICE"
/*20050617		  and pc_curr = po_curr */
		  and pc_amt_type = "L"
		  no-lock no-error.
	     ...
	     a_quote = pc_amt[1].
	        ...
	        for each pc_mstr where pc_part = pod_part and pc_list = "Q-PRICE" and pc_amt_type = "L"  and pc_start <= today  no-lock:
		    if pc_expire = ? or pc_expire >= today then do:
		       if pc_start > q-date then do:
		          q-date = pc_start.
			  a_quote = pc_amt[1].
		       end.
		    end.
...
/*20051111****************************************************************/
 ...
 /*20051111  find last pc_mstr use-index pc_part where pc_part = pod_part */
 /*20051111*/  find first pc_mstr where pc_part = pod_part 
                  and pc_list = "Q-PRICE"
/*20050617		  and pc_curr = po_curr */
		  and pc_amt_type = "L"
		  no-lock no-error.
         if available pc_mstr then do:
/*20051111	   a_quote = pc_amt[1].*/
/*20050617*/	    if pc_curr <> po_curr then do:
	       find first exr_rate where exr_curr1 = po_curr and
		    exr_curr2 = pc_curr and
		    exr_start_date <= today and exr_end_date >= today
	            no-lock no-error.
/*20051111*	       if available exr_rate then a_quote = exr_rate / exr_rate2 * pc_amt[1].*/
/*20051111**/	       if available exr_rate then a_quote = exr_rate / exr_rate2 * a_quote.
	    end. /*20050617*/
	end.
	else do:
	   q_yn = no.
	 ...
	 find first xxpoa_det where xxpoa_nbr = pod_nbr
		    ...
		    xxpoa_part        = pod_part.
		    xxpoa_qty_ord     = pod_qty_ord.
		    xxpoa_um          = pod_um.
		    xxpoa_curr        = po_curr.
		    xxpoa_per_date    = pod_per_date.
		    xxpoa_vend        = po_vend.
		    xxpoa_buyer       = po_buyer.
	       if xxpoa_unit_cost <> pod_pur_cost then do:
		assign
/*		    xxpoa_log         = a_log
		    xxpoa_appr        = a_appr*/
		    xxpoa_part        = pod_part
		    ...
		    xxpoa_um          = pod_um
		    xxpoa_curr        = po_curr
		    xxpoa_unit_cost   = pod_pur_cost
		    xxpoa_last_price  = a_last
		    xxpoa_quote_price = a_quote
		    xxpoa_per_date    = pod_per_date
		    xxpoa_vend        = po_vend
		    xxpoa_buyer       = po_buyer
		    .

		   if l_yn = yes and pod_pur_cost > a_last and pod_pur_cost > a_quote then do:
		      xxpoa_log = "02".
		      xxpoa_appr = yes.
		   end.
		   if l_yn = yes and pod_pur_cost < a_last and pod_pur_cost > a_quote then do:
		   ...
		   if l_yn = yes and pod_pur_cost < a_last and pod_pur_cost < a_quote then do:
		      xxpoa_log = "03".
		      xxpoa_appr = no.
		      find first pc_mstr where pc_part = pod_part and pc_amt_type = "L" 
		                           and pc_curr = po_curr
		   ...
		   if l_yn = yes and pod_pur_cost > a_last and (pod_pur_cost < a_quote or q_yn = no) then do:
		      xxpoa_log = "01".
		      xxpoa_appr = yes.
		   end.
		   if q_yn = no then do:
...
/*****************EAS003 begin**************************************/
		vlog_changed = NO.
		IF new_pod AND po_rev > 0 THEN vlog_changed = YES.
		IF 		(v_pod_due_date	<>	pod_per_date
			OR v_pod_pur_cost	<>	pod_pur_cost
			OR v_pod_qty_ord	<>	pod_qty_ord )
			AND (pod_line <> v_linecopy OR v_linecopy = 0)
			THEN DO:
			IF (NOT new_pod) THEN vlog_changed = YES.
		END.

		IF vlog_changed THEN DO:
			IF po_print THEN DO:
			END.
			ELSE DO:
				po_rev = po_rev + 1.
				po_print = YES.   /*增加新版本后该标志应reset为yes*/				
			END.
		
			IF po_rev > 0 THEN DO:
				FIND LAST xpv_mstr WHERE xpv_pod_nbr = pod_nbr AND xpv_pod_line = pod_line
					AND xpv_po_rev = po_rev EXCLUSIVE-LOCK NO-ERROR.				
				IF NOT AVAIL(xpv_mstr) THEN DO:
					CREATE xpv_mstr.
					assign
						xpv_pod_nbr 		= 	pod_nbr
						xpv_pod_line    	= 	pod_line
						xpv_po_rev			= 	po_rev
						xpv_due_date_old	=	IF new_pod THEN ? ELSE v_pod_due_date	/*new_pod 表示新建的line*/	 
						xpv_pur_cost_old	=	IF new_pod THEN ? ELSE v_pod_pur_cost 	/*新建的line没有旧值，旧值就是现在的值*/
						xpv_qty_ord_old		=	IF new_pod THEN ? ELSE v_pod_qty_ord		
					.					
				END.
				ASSIGN
					xpv_due_date_new	=	pod_per_date
					xpv_pur_cost_new	=	pod_pur_cost
					xpv_qty_ord_new 	=	pod_qty_ord 
				.
				po__dte01 = TODAY.			
				xpv_mod_date = TODAY.			
			END.
