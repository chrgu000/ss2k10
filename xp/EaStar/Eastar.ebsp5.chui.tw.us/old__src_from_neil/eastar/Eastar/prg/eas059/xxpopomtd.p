...
/* Revision: 1.14.4.15.1.3    BY:Rajaneesh S.       DATE:06/19/02 ECO: *N1H7* */
/* $Revision: 1.14.4.15.1.4 $               BY:Rajaneesh S.       DATE:07/03/02 ECO: *M1BY* */
/* $Revision: eb sp5                      BY:Jack Huang DATE:17/03/03 *eas023a" */
...
/* eas023a Add Begin ********************************************** */            
  ...
  form                      
      skip
      pod_vpart  label "Supplier Item" format "x(40)"
  with overlay frame a00 /*row 8 centered */ ROW 8 COLUMN 10 side-labels width 60 attr-space THREE-D.		
   setFrameLabels(frame a00:handle).  
     
  def var error# as char.
  /*def var pod_vpart like pod_vpart. */
  error# = "Supplier Item is not exist !".
  if global_user_lang = "tw" then error# = "供應商零件不存在 ﹗".
  set_a00:
  do on error undo, retry:
  
  update pod_vpart
  /*  validate(can-find(first vp_mstr where vp_vend_part = input pod_vpart and vp_vend = po_vend,error#))
  */
   with frame a00 
            editing:
               if frame-field = "pod_vpart" then do:
...
/*Leemy*/ HIDE FRAME a00.            
     if (not can-find(first vp_mstr where vp_vend_part = input pod_vpart and vp_vend = po_vend) and (input pod_vpart <> "")) 
     then do:
       message error# VIEW-AS ALERT-BOX error BUTTONS OK.
       undo set_a00,retry set_a00.       
     end.
  end.
