...
/* $Revision: 1.15.1.2 $      BY:Manisha Sawant   DATE:07/30/01 ECO: *M1FV*       */
/*                                                                            */
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
       if can-find(first po_mstr where po_vend = pSupplierId) then do:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for each pod_det exclusive-lock
         ...
         if pod_type = "" and pBlanket = false then do:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      if can-find(iec_ctrl where iec_impexp = yes) then do:
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      do on error undo, return {&GENERAL-APP-EXCEPT}:
         ...
         assign
            ...
            po_ers_opt  = if poc_ers_proc then poc_ers_opt else "1".
...
/*EAS019A */ define input parameter pPrefix like poc_po_pre .
...
/*EAS019a */ if pPrefix = "NC" then do:      
/*EAS019A 11-02 Begin ***************************** */   
  pre = pPrefix.
  preL = length(pre).

 find last po_mstr where  (po_nbr begins pre) and
   (substr(po_nbr,(preL + 1),(length(po_nbr) - preL)) >= "0") and
   (substr(po_nbr,(preL + 1),(length(po_nbr) - preL)) < "a")
  no-lock no-error .
 if avail po_mstr then do:
    find first poc_ctrl exclusive-lock.
    if not avail poc_ctrl then create poc_ctrl.
    if poc_ctrl.poc_po_nbr > (integer(substr(po_nbr,(preL + 1),(length(po_nbr) - preL))) + 1) then
    do:
       poc_ctrl.poc_po_nbr = integer(substr(po_nbr,(preL + 1),(length(po_nbr) - preL))).
       poc_ctrl.poc_po_nbr = poc_ctrl.poc_po_nbr + 1.
    end.
 end.
...
/*EAS019A 11-02 Begin ***************************** */   
             ...
             (substr(po_nbr,(preL + 1),(length(po_nbr) - preL)) < "a")
              no-lock no-error .
             if avail po_mstr then do:
                find first xxpoc_ctrl exclusive-lock.
                if not avail xxpoc_ctrl then create xxpoc_ctrl.
                if xxpoc_ctrl.xxpoc_po_nbr2 > (integer(substr(po_nbr,(preL + 1),(length(po_nbr) - preL))) + 1) then                
                do:
                  xxpoc_ctrl.xxpoc_po_nbr2 = integer(substr(po_nbr,(preL + 1),(length(po_nbr) - preL))).
                  xxpoc_ctrl.xxpoc_po_nbr2 = xxpoc_ctrl.xxpoc_po_nbr2 + 1.
                end.  
             end.
...
/*EAS019A 11-02 Begin ***************************** */   
             ...
             if not avail xxpoc_ctrl then create xxpoc_ctrl.
             else if avail xxpoc_ctrl then do:
                pre = xxpoc_ctrl.xxpoc_po_pre3.
                preL = length(pre).
               ...
               find last po_mstr where  (po_nbr begins pre) and
                  ...
                  no-lock no-error .
               if avail po_mstr then do:
                 if xxpoc_ctrl.xxpoc_po_nbr3 > (integer(substr(po_nbr,(preL + 1),(length(po_nbr) - preL))) + 1) then                
                 do:
               
                   xxpoc_ctrl.xxpoc_po_nbr3 = integer(substr(po_nbr,(preL + 1),(length(po_nbr) - preL))).
                   xxpoc_ctrl.xxpoc_po_nbr3 = xxpoc_ctrl.xxpoc_po_nbr3 + 1.
                 end.   
               end.
            end. /* end if avail xxpoc_ctrl */
/*EAS019A 11-02 End  ****************************** */             
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      if pOldRevision = pRevision then do:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if return-value = {&SUCCESS-RESULT} then do:
         ...
         if po_inv_mthd = "n" or po_inv_mthd = "e" then do:
            po_print = no.
         end.
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      if pPOType <> "B" and pBlanket then do:
      ...
      if pPOType = "B" and not pBlanket then do:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if return-value = {&SUCCESS-RESULT} then do:
         if pTermsInterest <> 0 then do:
            if pTermsInterest <> masterTermsInterest then do:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if discListValidationFailed then do:
         {pxmsg.i
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      if not ({gppoers.v pERSOption}) then do:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      if can-find(first po_mstr where po_nbr = pPOId) then do:
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      if (pPrepaidAmt <> 0) then do:
         {gprun.i ""gpcurval.p"" "(input pPrepaidAmt,
            input pRoundMethod, output retval)"}
         if retval <> 0 then
            return error {&APP-ERROR-RESULT}.
      end.
   end.
