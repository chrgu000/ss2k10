...
/*F701*         label "To Location" no-undo.                            */
    ...
    DEFINE OUTPUT PARAMETER err_seq AS INTEGER NO-UNDO.
...
define shared variable fm_eff_date like tr_effdate.   /*add-by-davild20051128*/
...
/*K04X*/ {icshmtdf.i "new" }
/* ricky*/ define shared temp-table  xxsq_tmp
		...
		.
DEFINE shared VARIABLE fm_site    like si_site .
DEFINE shared VARIABLE fm_loc     like loc_loc .
DEFINE shared VARIABLE to_site    like si_site .
DEFINE shared VARIABLE to_loc     like loc_loc.
define shared variable fm_lotser like sr_lotser.
define shared variable fm_lotref like ld_ref.  
define shared variable to_lotser like sr_lotser.
define shared variable to_lotref like ld_ref.  
...
/*G0V9*/ transloop:
/* ricky         repeat  */
/* ricky /*F701*/ with frame a:  */
/*ricky*/ FOR EACH xxsq_tmp WHERE xxsq_select and xxsq_trf_qty > 0:
...
/*F0FH*/    /*eff_date = today*/   /*remark by davild20051214*/
            eff_date = if fm_eff_date <> ? then fm_eff_date else today .   /*add by davild20051214*/
/*F701*/    find pt_mstr where pt_part = xxsq_part no-lock no-error.
/*F701*/    if NOT available pt_mstr THEN DO:
                err_seq = 1.
                UNDO,RETURN.
            END.
...
/* /*F701*/          undo, retry.                                           */
/* /*F701*/       end.                                                      */
/*ricky*/           lotserial_qty = xxsq_trf_qty.
...
/*                   apply lastkey.                   */
/*                end.                                */
/*ricky*/         ASSIGN site_from = fm_site                
                         loc_from = fm_loc
                         lotser_from = fm_lotser
                         lotref_from = fm_lotref
                         site_to   = TO_site
                         loc_to = TO_loc
                         lotser_to = to_lotser
                         lotref_to = to_lotref.
...
/*                      end.                                                     */
/*                      else do:                                                 */
/*                         find first lddet1 where lddet1.ld_part = xxsq_part    */
...
/* /*J3KH**       display global_part @ xxsq_part with frame a. */  */
/* /*J3KH*/       display                                           */
