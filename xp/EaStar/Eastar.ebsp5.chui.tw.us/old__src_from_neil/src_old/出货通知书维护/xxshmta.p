...
/*			    if snh_sq_nbr <> sqnbr and snh_part = part3 then do:
	    ...
	    if lastkey = keycode("F5") or lastkey = keycode("CTRL-D") then do:
		 ...
		 for each snp_totals where snp_sn_nbr = snnbr2 and snp_sq_nbr = sqnbr exclusive-lock:
		     snp_status = "D".
		     snp_last_date = today.
		     snp_last_user = global_userid.
		 end.
		 for each snp_totals where snp_sn_nbr = snnbr2 and snp_sq_nbr = sqnbr no-lock:
		 end.
