...
for each snp_totals where snp_sn_nbr = snnbr2 and snp_status <> "D" use-index snp_nbr no-lock:
   create xxsnp2.
   assign
...
/*	   {mfnp.i xxsnp_tmp shipto3 xxsnp_shipto nbr3 xxsnp_nbr xxsnp_nbr}*/
	...
	width3       
	 ...
	 apply lastkey.
       ...
       find first snp_totals where snp_sn_nbr = snnbr2 and snp_shipto = shipto3 and snp_nbr = nbr3 exclusive-lock no-error.
       ...
       release snp_totals.
       find first snp_totals where snp_sn_nbr = snnbr2 and snp_shipto = shipto3 and snp_nbr = nbr3 no-lock no-error.
        line3 = xxsnp_line.
...
/*	       if snd_plt_nbr = 0 then total_grosswt = total_grosswt + snd_ext_grosswt.*/
	...
	for each snp_totals where snp_sn_nbr = snnbr2 and snp_status <> "D" no-lock:
