...
/* ss - 100622.1 by: jack */
/* ss - 100623.1 by: jack */
/* ss - 100630.1 by: jack */  /*增加工作中心 */
/* ss - 100714.1 by: jack */  /* 增加退回供应商方式*/
/* ss - 100809.1 by: jack */
/* ss - 100827.1 by: jack */  /* 维护时增加新增功能 */
/* SS - 101208.1  By: Roger Xiao */  /*增加退料原因备注(xxrt_remark,no-ECO),另:为保证打印结果与3.5.2一致,请不要在本程式目录下存放xxricrt11.p ! */
/******************************************************************************/
...
define var v_ptdesc  as char format "x(40)" label "零件说明" . /* SS - 101208.1 */
form
...
/* ss - 100809.1 -e */
   ...
   display  p-type rcvno  with frame a.
      ...
      repeat with frame b:
         ...
         FORM           
            xxrt_line        label "项"       format ">>"
            xxrt_wodpart     LABEL "子件"     FORMAT "x(14)"
            xxrt_wonbr       LABEL "工单"     FORMAT "x(10)"
            xxrt_relqty      LABEL "发料数"
            xxrt_qty_from    label "退料数"   FORMAT "->>,>>>,>9.9<"
            xxrt_lot_from    LABEL "批号"     FORMAT "x(8)"        
            /*
            xxrt_method      LABEL "方法"     FORMAT "x(4)"
            pt_desc1         no-label    space(1)
            pt_desc2         NO-LABEL
            v_method         NO-LABEL */ /* SS - 101208.1 */
            skip
            v_ptdesc         at 6   label "零件说明" format "x(40)"
            xxrt_remark      at 48  label "    退料原因备注" format "x(30)"

         with frame b 	5 down no-validate width 80   attr-space.
        ...
        /* ss - 100827.1 -e */
         ...
         editing:
            ...
            if recno <> ? then do:
        	    display xxrt_line xxrt_wonbr  xxrt_wodpart xxrt_qty_from   xxrt_lot_from  xxrt_remark   with frame b.               
        	    find first pt_mstr no-lock where pt_mstr.pt_domain = global_domain and pt_part = xxrt_wodpart no-error.
                       if available pt_mstr then
                          display  trim(pt_desc1) + trim(pt_desc2) @ v_ptdesc with frame b .
                       else
                          display "" @ v_ptdesc with frame b .
       ...
       display xxrt_line  xxrt_wonbr xxrt_wodpart xxrt_relqty xxrt_qty_from  xxrt_lot_from xxrt_remark with frame b .               
      ...
      IF newrec  THEN  DO:
        /* ss - 100827.1 -b */
           ...
           /* ss - 100622.1 -b
            ...
            ss - 100622.1 -e */
                       ...
                       find xxrt_det where xxrt_domain = global_domain and xxrt_site_from = site  and  xxrt_nbr = rcvno and xxrt_type = p-type and  xxrt_line = input xxrt_line   no-error.
                        ...
                        IF AVAILABLE usrw_wkfl  THEN DO:
                            ...
                            ASSIGN
                                ...
                                .
                        /* ss - 100630.1 -b */
                            FIND FIRST wo_mstr WHERE wo_domain = GLOBAL_domain AND  /* ss - 1008271 -b wo_nbr = xxrt_nbr ss - 100827.1 -e */  /* ss -100827.1 -b */  wo_nbr = xxrt_wonbr /* ss - 100827.1 -e */
                                 AND wo_lot = usrw_key4  NO-LOCK NO-ERROR .
             ...
             IF AVAILABLE xxrt_det THEN DO:
                    DISPLAY  xxrt_line  xxrt_wonbr    xxrt_wodpart xxrt_relqty xxrt_qty_from   /* ss - 100714.1 - b */   /* xxrt_rsn */   xxrt_lot_from   /* ss - 100714.1 -e */ xxrt_remark  with frame b .     
         ...
         DO ON ERROR UNDO,RETRY:
             ...
             PROMPT-FOR xxrt_qty_from   xxrt_lot_from  xxrt_remark GO-ON(F5 CTRL-D ) WITH FRAM b .
        ...
        END.  /* setd */
     assign  xxrt_qty_from     xxrt_lot_from   xxrt_remark .
 ...
 */
 /* ss - 100714.1 -b */
 {gprun.i ""xxricrt11.p"" "(input rcvno)"}
 /* ss - 100714.1 -e */
