...
define var v_to_po as logical label "ÊÖ¹¤ÐÞ¸ÄPO" initial no . /* SS - 101208.1 */
...
repeat on endkey undo, leave:
   ...
   do transaction on error undo, retry with frame a:
      ...
      display po_nbr po_vend po_ship v_to_po with frame a.  /* SS - 101208.1  */
      ...
      then do:
         l_taxflag = no.
         {gprun.i ""rqpobldc.p""
                  "(input-output l_taxflag)" }  /* SIMILAR TO popomtb.p */
         if l_taxflag = no
         then
   ...
   {gprun.i ""rqpobldd.p"" "(input using_supplier_consignment)"}
...
if v_to_po then do:
    {gprun.i ""xxpomt5218.p"" "(input false , input ponbr)"}

    for each rqpo_wrk where rqpo_copy_to_po = yes :
        open_qty = 0.
        {gprun.i ""rqoqty.p""
            "(input true,
            input rqpo_site,
        ...
        if open_qty > 0 then do:
            find first rqd_Det where rqd_domain = global_domain and rqd_nbr = rqpo_nbr and rqd_line = rqpo_line exclusive-lock no-error.
            if avail rqd_det then do:
                rqd_status = ""  .
                rqd_open   = yes .
                {gprun.i ""rqsetopn.p"" "(input rqpo_nbr)"} /* set rqd_status rqm_status */
            end.
        end.
        else do:
                {gprun.i ""rqsetopn.p"" "(input rqpo_nbr)"} /* set rqd_status rqm_status */
