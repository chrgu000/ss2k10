...
/* $Revision: 1.5.1.6 $  BY: Vinay Soman           DATE: 10/14/04 ECO: *P2PN* */
/******************************************************************************/
   ...
   repeat with frame {2}:
      ...
      if avail pt_mstr then do:
          find first cd_det 
              ...
              and cd_ref  = {9} 
              and cd_type = "PT"
              and (cd_lang = "" or cd_lang = global_lang )
          no-lock no-error .
          if avail cd_det then
              {15} = trim(pt_desc1) + "/" + trim(pt_desc2) + "/" + trim(cd_cmmt[1]) + trim(cd_cmmt[2]) .
          else 
              {15} = trim(pt_desc1) + "/" + trim(pt_desc2)  .
