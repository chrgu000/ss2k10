...
/* ss - 100423.1 -e */
 ...
 find first ad_mstr no-lock  where ad_mstr.ad_domain = global_domain and  ad_addr =
  global_domain and ad_type = "company"  no-error.
   ...
   if available ad_mstr
      then companyname = ad_name. else companyname = " SINCERE-HOME".

     find first xdn_ctrl where xdn_ctrl.xdn_domain = global_domain and xdn_type = "RN"  no-lock no-error.
 ...
 for each xxrt_det  no-lock where xxrt_det.xxrt_domain = global_domain and xxrt_nbr = rcvno use-index xxrt_line ,
   ...
   with frame c down width 85 no-box:
    ...
    if mm = 1 then do:
            ...
            if available loc_mstr then assign prodline = loc_loc
                                       ...
                                       proddesc = "".
             /* ss - 100809.1 -b
           PUT "工单/ID           零件号码      描述                          数量     实退           批号/部门         备注"  AT 1 .
             PUT "-------------------------------------------------------------------------------------------------------------" AT 1 .
              ss - 100809.1 -e */
              view frame phead1.  
  ...
  /*  view frame phead1.  */
   ...
   if page-size - line-counter < 12 THEN do:  /* 11 */ 
   ...
   /*     put "--------------------------------------------------------------------------------------------------------" at 1. */
      ...
      /* ss - 100809.1 -e */                                                        
             ...
             PUT "制单人:"      AT 1 .
	 	     put "第一联:"          at 11  xdnpage1  format "x(8)"   .
		     put "第二联:"          at 30 xdnpage2  format "x(8)"   .
		     put "第三联:"         at  50 xdnpage3  format "x(8)"   .
             /*
	   ...
	   put "Page: "    at 65  string(page-start) format "xxxx"  /* ss - 100810.1 -b */ SKIP /* ss - 100810.1 -e */ .
     ...
     PUT xxrt_dept  AT 97.
       ...
       /* ss - 100810.1 -b */  IF page-size - line-counter >    12  /* 11 */  then   /* ss - 100810.1 -e */  put "-------------------------------------------------------------------------------------------------------------" at 1 . 
    /* ss - 100810.1 -b */
     IF page-size - line-counter  =   12 THEN  PUT SKIP(1) .
    /* ss - 100810.1 -e */
 ...
 /*
    ...
    */
      PUT SKIP(PAGE-SIZE - LINE-COUNTER - 10 ) .  /* 9 */
/* ss - 100423.1 -e */
   /*   put "--------------------------------------------------------------------------------------------------------" at 1. */
      /* ss - 100809.1 -b
	             put skip(1). 
