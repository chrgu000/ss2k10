...
/* SS - 090908.1  By: Roger Xiao */  /*退出前,打印已审批的清单*/


/*----rev description---------------------------------------------------------------------------------*/

/* SS - 090828.1 - RNB
改成可以按照供应商分配比例,拆分成多PR的方式:
1.仅使用于grs模式下
2.先不自动产生请购单号,待找好供应商并拆分之后,才产生请购单号
3.其他相关逻辑见spec
SS - 090828.1 - RNE */

/* SS - 090831.1 - RNB
修改xxmrprapa.p写入DB前不用真实单号,改用模拟单号
SS - 090831.1 - RNE */
...
{xxmrprapa03v.i "new" }  /*shared temp-table temp1*/
...
/* SS - 090908.1 - B */
find first temp1 where t1_nbr_new <> "" no-lock no-error.
if avail temp1 then do:
    ...
    {mfphead.i}
    for each temp1 where t1_nbr <> "" break by t1_nbr by t1_rqdline with frame xxx width 200:
        ...
        disp    
            ...
            t1_rqdline       label "行"
            t1_part          label "零件号"
            pt_desc1         label "零件说明1" when avail pt_mstr 
            pt_desc2         label "零件说明2" when avail pt_mstr 
            pt_um            label "UM" when avail pt_mstr 
            t1_qty           label "请购数量"
            t1_vend          label "供应商"
            ad_name          label "供应商名称" when avail ad_mstr
            t1_due_date      column-label "截止日期"
            t1_rel_date      column-label "发放日期"
            global_userid    label "批准用户"
        with frame xxx.
    end. /*for each temp1*/
...
/* SS - 090828.1 - B */
    for each temp1 :
      ...
      for each temp1 no-lock break by t1_vend:
         if last-of(t1_vend) then
            l_cnt = l_cnt + 1.
      end. 
      {pxmsg.i &MSGNUM=4640 &ERRORLEVEL=1 &MSGARG1=l_cnt}

      for last temp1 use-index t1_nbr no-lock:
      end. 
      if  available temp1 and l_cnt > 0 then do:
         if l_cnt = 1
         then do:
            {pxmsg.i &MSGNUM=2765 &ERRORLEVEL=1 &MSGARG1=temp1.t1_nbr}
         end. /* IF l_cnt = 1 */
         else do:
            {pxmsg.i &MSGNUM=4641 &ERRORLEVEL=1 &MSGARG1=temp1.t1_nbr}
         end. /* ELSE DO */
      end. /* IF AVAILABLE temp1 AND l_cnt > 0 */
/* SS - 090828.1 - E */
