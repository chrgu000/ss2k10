...
/* $Revision: 1.32.2.1 $           BY: Archana Kirtane    DATE: 05/15/07  ECO: *P5WP*  */
...
/* SS - 090828.1 - B */
    define input parameter in_rqm_vend     like rqm_mstr.rqm_vend no-undo.
/* SS - 090828.1 - E */
...
/* SS - 090908.1 - B */
define output parameter v_rqd_line as integer no-undo.
/* SS - 090908.1 - E */
...
/* SS - 090828.1 - B 
   ...
   SS - 090828.1 - E */
/* SS - 090828.1 - B */
   for last temp1 no-lock use-index t1_nbr_new:
   end. 
   if available temp1 then get_rqmnbr = t1_nbr_new.
   else       get_rqmnbr = rqm_nbr.
/* SS - 090828.1 - E */
...
if available ptp_det then do:
   ...
   SS - 090828.1 - E */
/* SS - 090828.1 - B */
assign rqd_vend = in_rqm_vend .
find vd_mstr  where vd_mstr.vd_domain = global_domain and  vd_addr = in_rqm_vend no-lock no-error.
/* SS - 090828.1 - E */
...
/* SS - 090828.1 - B */
rqm_buyer       = if avail vd_mstr then vd_buyer else ""  .
/*
    rqm_rtto_userid = rqm_buyer .
    rqm_rtdto_purch = yes .
    rqm_aprv_stat   = APPROVAL_STATUS_APPROVED .


    define variable email_sent_to  as character no-undo.
    define variable action_type    like rqh_action no-undo.
    action_type = "21" .
    {gprun.i ""rqemsend.p""
