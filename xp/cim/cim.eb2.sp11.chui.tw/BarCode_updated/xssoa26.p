...
/*----rev history-------------------------------------------------------------------------------------*/
/* SS - 090630.1 By: Roger Xiao */ /*just modify: xssoa26u.i & xsso20s.i*/

/* the last version update by softspeed julie huang 08/11/25 */
/* SO Allocation */
/* Generate date / time  10/10/07 10:09:21 */
...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xssoa26wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
END FUNCTION.
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品備料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1003 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1003.
            ...
            THEN DO:
                  ...
                  IF recid(xxshm_MSTR) = ? THEN find first xxshm_MSTR where 
                              xxshm_inv_no >=  INPUT V1003
                               no-lock no-error.
                  ELSE find next xxshm_MSTR no-lock no-error.
                  IF AVAILABLE xxshm_MSTR then display skip 
                      xxshm_inv_no @ V1003 trim ( xxshm_cust ) @ WMESSAGE NO-LABEL with fram F1003.
            ...
            THEN DO:
                  IF recid(xxshm_MSTR) = ? THEN find first xxshm_MSTR where 
                               xxshm_inv_no <=  INPUT V1003
                               no-lock no-error.
                  ELSE find prev xxshm_MSTR no-lock no-error.
                  IF AVAILABLE xxshm_MSTR then display skip 
                   xxshm_inv_no @ V1003 trim ( xxshm_cust ) @ WMESSAGE NO-LABEL with fram F1003. 
                  else   display skip "" @ WMESSAGE with fram F1003. 
            END. 
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first XXSHM_MSTR where xxshm_inv_no = V1003  NO-ERROR NO-WAIT.
        IF NOT AVAILABLE XXSHM_MSTR then do:
                display skip "發票無效或被鎖!" @ WMESSAGE NO-LABEL with fram F1003.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
     ...
     END.
     PV1003 = V1003.
     /* END    LINE :1003  楷き瘍[Invoice]  */
    


   /* Additional Labels Format */
     /* START  LINE :1100  銷售訂單[SalesOrder]  */
     V1100L:
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.

                /* LABEL 1 - START */ 
            /********************Julie huang 08/11/25 del******************************** 
		L11001 = "銷售訂單?" .
                ...
                /* LABEL 2 - START */ 
                  L11002 = "" . 
            ********************Julie huang 08/11/25 del ********************************/ 
            /********************Julie huang 08/11/25 add ********************************/ 
                L11001 = "發票號:" + TRIM ( V1003 )  .
                display L11001          format "x(40)" skip with fram F1100 no-box.
                /* LABEL 1 - END */ 
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1100.
            ...
            THEN DO:
                  IF recid(SO_MSTR) = ? THEN find first SO_MSTR where 
                              ( SO_SITE = "" OR SO_SITE = V1002 ) AND so_conf <> ? and
                              SO_NBR >=  INPUT V1100
                               no-lock no-error. /*julie huang 08/11/28 add so_conf <> ? and*/
                  ELSE find next SO_MSTR where 
                              ( SO_SITE = "" OR SO_SITE = V1002 ) and so_conf <> ?  
                               no-lock no-error. /*julie huang 08/11/28 add so_conf <> ? */
	   ...
	   find first xxshd_det where xxshd_inv_no = V1003 and xxshd_so_nbr = V1100 NO-ERROR NO-WAIT.
	   IF NOT AVAILABLE xxshd_det then do:
                display skip  "不在發票內或被鎖!" @ WMESSAGE NO-LABEL with fram F1100.
                pause 0 before-hide.
                undo, retry.
            end.
	end.
     ...
     END.
     PV1100 = V1100.
     /* END    LINE :1100  銷售訂單[SalesOrder]  */


   /* Additional Labels Format */
     /* START  LINE :1105  客戶代碼[Customer Code]  */
     V1105L:
     REPEAT:
...
If AVAILABLE ( so_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1105 no-box.
     ...
     END.
     PV1105 = V1105.
     /* END    LINE :1105  客戶代碼[Customer Code]  */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE ( ad_mstr ) then
        ...
        /* PRESS e EXIST CYCLE */
        IF V1300 = "e" THEN  LEAVE V1300LMAINLOOP.
        ...
        IF NOT AVAILABLE SOD_DET then do:
                display skip "該成品不匹配,請查核!" @ WMESSAGE NO-LABEL with fram F1300.
                pause 0 before-hide.
                undo, retry.
        end.
            /********************Julie huang 08/11/25 add ********************************/ 
        else do:
	find first xxshd_det where xxshd_inv_no = V1003 and xxshd_so_nbr = V1100 and xxshd_so_part = ENTRY(1, V1300, "@") use-index xxshd_sonbr_line NO-ERROR NO-WAIT.
	   IF NOT AVAILABLE xxshd_det then do:
                display skip "該成品不在發票內﹐請查核!" @ WMESSAGE NO-LABEL with fram F1300.
                pause 0 before-hide.
                undo, retry.
        end.
        end.  
...
If AVAILABLE (sod_det) and string ( sod_line ) = V1305 then
        leave V1305L.
        /* LOGICAL SKIP END */
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1305 no-box.
        ...
        find first SOD_DET where SOD_NBR = V1100 AND sod_line = integer(trim(V1305)) and SOD_SITE = V1002 
	NO-ERROR NO-WAIT.
        /*****************************julie huang 08/11/28 add***************************************/
	if AVAILABLE sod_det and sod_confirm = no then do:
	        display skip "訂單項未確認!" @ WMESSAGE NO-LABEL with fram F1305.
                pause 0 before-hide.
                undo, retry.
        end.
	/*****************************julie huang 08/11/28 add***************************************/
	IF NOT AVAILABLE SOD_DET then do:
                display skip "無效或被鎖!" @ WMESSAGE NO-LABEL with fram F1305.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
...
If NOT AVAILABLE (lad_det) THEN
        leave V1309L.
        /* LOGICAL SKIP END */
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1309 no-box.

                /* LABEL 1 - START */ 
                L13091 = "ERR:已打DN,不能備料" .
        ...
        /* PRESS e EXIST CYCLE */
        IF V1309 = "e" THEN  LEAVE V1300LMAINLOOP.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP END */
                ...
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
                ...
                /* LABEL 3 - START */ 
                L14003 = "訂單/項:" + trim ( V1100 ) + "/" + trim ( V1305 ) .
	...
	NO-ERROR NO-WAIT.
        IF NOT AVAILABLE SOD_DET then do:
                display skip "訂單項無效或被鎖!" @ WMESSAGE NO-LABEL with fram F1400.
                pause 0 before-hide.
                undo, retry.
        end.
	if AVAILABLE sod_det and sod_confirm = no then do:
	        display skip "訂單/項:" + trim ( V1100 ) + "/" + trim ( V1305 ) + "未確認!" @ WMESSAGE NO-LABEL with fram F1400.
                pause 0 before-hide.
                undo, retry.
        end.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
...
If AVAILABLE ( ld_det ) then
                L15003 = "庫存:" + trim(string(ld_qty_oh)) .
                else L15003 = "" . 
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1500.
            ...
            THEN DO:
                  IF recid(LD_DET) = ? THEN find first LD_DET where 
                              LD_PART = V1300 AND LD_LOC = V1400 AND LD_QTY_OH <> 0 AND LD_SITE = V1002 AND ( LD_REF = "" OR LD_REF = V1003) AND   
                              LD_LOT >=  INPUT V1500
                               no-lock no-error. /* julie huang 08/11/25 chang v1100 to v1003 */
                  ELSE find next LD_DET where 
                              LD_PART = V1300 AND LD_LOC = V1400 AND LD_QTY_OH <> 0 AND LD_SITE = V1002 AND ( LD_REF = "" OR LD_REF = V1003)  
                               no-lock no-error. /* julie huang 08/11/25 chang v1100 to v1003 */
                  IF AVAILABLE LD_DET then display skip 
            LD_LOT @ V1500 (IF LD_REF = "" THEN "可" ELSE "已" ) + "備L/Q:" + LD_LOT + "/" + trim(string(LD_QTY_OH)) @ WMESSAGE NO-LABEL with fram F1500.
...
display skip "批號對應訂單不等於輸入訂單" @ WMESSAGE NO-LABEL with fram F1500.
                pause 0 before-hide.
                Undo, retry.
End.
*********julie huang 08/11/27 del**************************/
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
...
If AVAILABLE ( sod_det ) then
                L16002 = "訂單/已出:" + string ( sod_qty_ord ) + "/" + string (sod_qty_ship) .
                else L16002 = "" . 
                ...
                find first sod_det where sod_nbr = V1100 and string ( sod_line ) = V1305 no-lock no-error.
If AVAILABLE ( sod_det ) then
                L16003 = "已備/撿量:" + string (sod_qty_all) + "/" + string ( sod_qty_pick ) .
                else L16003 = "" . 
                ...
                /* LABEL 4 - START */ 
                find last ld_det where ld_part = V1300 and ld_loc = V1400 and ld_site = V1002 and ld_qty_oh <> 0 and ld_lot = V1500 and ( ld_ref = "" OR ld_ref = V1003 ) use-index ld_part_lot no-lock no-error. /* julie huang 08/11/25 chang v1100 to v1003 */
If AVAILABLE ( ld_det ) then
                L16004 = "批號/" + ( if ld_ref = "" then "可" else "已" ) + "備:" + V1500 + "/" +  string (ld_Qty_oh) .
                else L16004 = "" . 
    ...
    ld_site = V1002 and ld_lot = V1500 and  ( 
    (DECIMAL ( V1600 ) < 0 and ld_ref = V1003 and ld_qty_oh + DECIMAL ( V1600 ) >= 0)
  OR
  (DECIMAL ( V1600 ) > 0 and ld_ref = "" and ld_qty_oh >= DECIMAL ( V1600 ))
  or 
   (DECIMAL ( V1600 ) > 0 and ld_ref = v1003 and ld_qty_oh >= DECIMAL ( V1600 ))
  )  no-lock no-error. /* julie huang 08/11/25 chang v1100 to v1003 */
        IF NOT AVAILABLE LD_DET then do:
                display skip "可/已備數 <: " + string( V1600 ) @ WMESSAGE NO-LABEL with fram F1600.
                pause 0 before-hide.
                undo, retry.
        end.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
...
If AVAILABLE ( sod_det ) then
                L17004 = "訂單未決數量:" + ( if sod_qty_ord <= sod_qty_ship then "0" else ( string ( sod_qty_ord - sod_qty_ship ) ) ) .
                else L17004 = "" . 
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
...
IF 1 = 1 THEN
        leave V9005L.
        /* LOGICAL SKIP END */
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9005 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售成品備料限定訂單]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
...
If NOT AVAILABLE ( tr_hist ) then
        ...
        /* PRESS e EXIST CYCLE */
        IF V9010 = "e" THEN  LEAVE V9010L /*V1300LMAINLOOP*/ .
