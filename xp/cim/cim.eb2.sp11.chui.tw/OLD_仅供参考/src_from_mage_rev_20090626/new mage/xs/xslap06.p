...
/* SO-WO LABLE */
/* Generate date / time  5/16/2008 10:04:23 AM */
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售工單標簽]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1310 no-box.
...
/*mage del 08/11/08*******************************************************************************
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售工單標簽]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1320 no-box.
...
/*mage del 08/11/08*******************************************************************************
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售工單標簽]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1330 no-box.
        ...
        IF not length( trim ( V1330 ) ) = 1 THEN DO:
                display skip "長度不能隻能1位" @ WMESSAGE NO-LABEL with fram F1330.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
...
/*mage error delete 08/11/14   IF NOT (V1129="Y" ) THEN LEAVE V1336LMAINLOOP.  ****/
	 ...
	 If search(LabelsPath + "lap06" + trim ( V1336 ) ) = ? Then do:
	                 display skip "文件不存在,請重新輸入." @ WMESSAGE NO-LABEL with fram F1130.
                         pause 0 before-hide.
                         Undo, retry.
	 End.
Wtm_fm = V1336.
        IF not (V1336="l" or V1336="b" ) THEN DO:
...
/*mage error del 08/11/14    IF NOT (V1129="Y" ) THEN LEAVE V1336LMAINLOOP.  */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP END */
                ...
                /* LABEL 2 - START */ 
                L13362 = "S-機身  P-紙箱 C-卡板 T-Templet" .
                display L13362          format "x(40)" skip with fram F1336 no-box.
                /* LABEL 2 - END */ 
        ...
        IF not (V1336="s" or V1336="b"  or V1336="c"  or V1336="t"  ) THEN DO:
                display skip "Input Error" @ WMESSAGE NO-LABEL with fram F1336.
                pause 0 before-hide.
                undo, retry.
        end.
...
/*mage add 08/11/08*******************************************************************************/
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        if v1336 = "b" then v1335 =  pt__chr08 .
        if v1336 = "c" then v1335 =  pt__chr09 .
 ...
 /*mage add 08/11/08*******************************************************************************/
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售工單標簽]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* PRESS e EXIST CYCLE */
        IF V9010 = "e" THEN  LEAVE V1336LMAINLOOP.
...
/*mage add 08/11/08 */ 	v9020 = string( wo_qty_ord / integer(v9010), ">>>>>>>9") .
        Update V9020
...
/*mage add 08/11/14*********************************************************************************/
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[銷售工單標簽]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9025 no-box.
        ...
        /* PRESS e EXIST CYCLE */
        IF V9025 = "e" THEN  LEAVE V1336LMAINLOOP.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[銷售工單標簽]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9030 no-box.
...
/*mage add 08/11/08****************************************************************************/
   ...
   if available pt_mstr then do:
           if v1336 = "S" then  pt__chr07 = string(integer(v1335) + integer(wtm_num) , "999999").
           if v1336 = "C" then  pt__chr08 = string(integer(v1335)  + integer(wtm_num) , "9999" ).
           if v1336 = "P" then  assign pt__chr09 = string(integer(v1335) + integer(wtm_num), "9999"  )
	                         pt__chr08 = string(integer(v1335) +  integer(wtm_num)  * c_num, "9999"   ).
...
/*mage del 08/11/08***********************************************************************************
     ...
     INPUT FROM VALUE(LabelsPath + "lap0621"  ).
...
/*mage add  08/11/08***********************************************************************************/
     PROCEDURE lap069030l.
        /* Define Labels Path  Start */
        Define input parameter  lotnbr as character.
        Define variable LabelsPath as character format "x(100)" init "/app/mfgpro/bc/labels/".
        Find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="LabelsPath"no-lock no-error.
        If AVAILABLE(code_mstr) Then LabelsPath = trim ( code_cmmt ).
        If substring(LabelsPath, length(LabelsPath), 1) <> "/" Then 
     ...
     Do While True:
       ...
       END.
        ...
        av9030 = V1104 + "@" + string(D1110,"999999")  +  lotnbr  .
       IF INDEX(ts9030,"$B") <> 0  THEN DO:
       TS9030 = substring(TS9030, 1, Index(TS9030 , "$B") - 1) + av9030 
       + SUBSTRING( ts9030 , index(ts9030 ,"$B") + length("$B"), LENGTH(ts9030) - ( index(ts9030 ,"$B" ) + length("$B") - 1 ) ).
       END.
    ...
    do i = 1 to integer(wtm_num):
    labnum = string(wtm_num, "99999").
            
	   if v1336 = "s" then  labnum  = string(integer(v1335) + i, "999999"  ).
           if v1336 = "P" then  labnum  = "P" + string(integer(v1335) + i, "9999"  ).
           if v1336 = "c" then  labnum  = "C" +  string(integer(v1335) + i, "9999").
           if v1336 = "T" then  labnum  = "Templet" .
	                         
     run lap069030l(input labnum).
      find first PRD_DET where PRD_DEV = V9030 no-lock no-error.
       IF AVAILABLE PRD_DET then do:
         unix silent value (trim(prd_path) + " " + trim(wsection) + ".l").
         unix silent value ( "clear").
