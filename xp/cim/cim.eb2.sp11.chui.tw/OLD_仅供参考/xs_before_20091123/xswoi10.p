...
/* Generate date / time  7/15/2008 11:56:45 AM */
...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 no-box.
...
If AVAILABLE ( code_mstr ) then
        leave V1203L.
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1203 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
        ...
        IF NOT AVAILABLE WOD_DET then do:
                display skip "該料品不在WO清單內!" @ WMESSAGE NO-LABEL with fram F1300.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
...
If AVAILABLE ( ld_det ) then
                ...
                find first ld_det where ld_part = V1300 and ld_site = V1002 and ld_ref = "" and ld_loc = V1400 and ld_qty_oh <> 0 use-index ld_part_lot no-lock no-error.
If AVAILABLE ( ld_det ) then
                L15003 = "庫存:" + trim ( V1400 ) + "/" + trim(string(ld_qty_oh)) .
                else L15003 = "" . 
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find first wod_det where wod_part = V1300 and wod_lot = V1110 no-lock no-error.
If AVAILABLE ( wod_det ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1502 no-box.
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1502.
            ...
            THEN DO:
                  IF recid(WOD_DET) = ? THEN find first WOD_DET where 
                              WOD_LOT = V1110 AND WOD_PART = ENTRY(1, V1300, "@") AND  
                              string ( WOD_OP ) >=  INPUT V1502
                               no-lock no-error.
                  ELSE find next WOD_DET where 
                              WOD_LOT = V1110 AND WOD_PART = ENTRY(1, V1300, "@")  
                               no-lock no-error.
                  IF AVAILABLE WOD_DET then display skip 
            string ( WOD_OP ) @ V1502 string  ( WOD_OP ) @ WMESSAGE NO-LABEL with fram F1502.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first WOD_DET where WOD_LOT = V1110 AND WOD_PART = ENTRY(1, V1300, "@") and string (WOD_OP) = V1502  no-lock no-error.
        IF NOT AVAILABLE WOD_DET then do:
                display skip "工序有誤!" @ WMESSAGE NO-LABEL with fram F1502.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1503L.
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1503 no-box.
                ...
                find first pt_mstr where pt_part = V1300  no-lock no-error.
If AVAILABLE ( pt_mstr ) then
                L15033 = "保質期:" + ( if pt_avg_int = 0 or pt_avg_int = 90 then " " else string ( pt_avg_int ) ) + "月,通知期" + string ( WOISSUEBUFFERDAY ) + "天" .
                else L15033 = "" . 
...
If AVAILABLE ( ld_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1550 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
...
ld_lot = V1500 and  ld_QTY_oh >= DECIMAL ( V1600 )  no-lock no-error.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
                ...
                /* LABEL 4 - START */ 
                find first wod_det where wod_lot = V1110 and wod_part = V1300 and string ( wod_op ) = V1502 no-lock no-error.
If AVAILABLE ( wod_det ) then
                L17004 = "工單未發數量:" + ( if wod_qty_req <= wod_qty_iss then "0" else ( string ( wod_qty_req - wod_qty_iss ) ) ) .
                else L17004 = "" . 
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
...
If 1 = 1 then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9005 no-box.
...
IF 1 = 2 THEN
        leave V9010L.
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
...
If NOT AVAILABLE ( tr_hist ) then
   ...
   IF NOT (V9010 = "Y" AND V1700 = "Y" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工單發正常料]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
