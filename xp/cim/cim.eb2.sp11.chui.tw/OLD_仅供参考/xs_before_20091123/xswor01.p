...
REPEAT:
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1005 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
...
/*mage del ************************************************************************
            ...
            THEN DO:
                  IF recid(WO_MSTR) = ? THEN find first WO_MSTR where 
                              WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0 AND  ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" ) AND  
                              WO_NBR >=  INPUT V1100
                               no-lock no-error.
                  ELSE find next WO_MSTR where 
                              WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0 AND  ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" )  
                               no-lock no-error.
...
/*mage del ************************************************************************/
            ...
            THEN DO:
                  IF recid(WO_MSTR) = ? THEN find first WO_MSTR where 
                              WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0 AND   
                              WO_NBR >=  INPUT V1100
                               no-lock no-error.
                  ELSE find next WO_MSTR where 
                              WO_SITE = V1002 AND INDEX("AR",WO_STATUS) <> 0   
                               no-lock no-error.
                  IF AVAILABLE WO_MSTR then display skip 
            WO_NBR @ V1100 trim( WO_PART ) + "/" + trim (wo_lot) @ WMESSAGE NO-LABEL with fram F1100.
                  else   display skip "" @ WMESSAGE with fram F1100.
...
If AVAILABLE ( wo_mstr )  and  wo_lot = V1110 then
        leave V1110L.
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
...
/*mage del*****************************************************************
            ...
            THEN DO:
                  IF recid(wo_mstr) = ? THEN find first wo_mstr where 
                              wo_site = V1002 AND wo_nbr = V1100 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" ) AND  
                              wo_lot >=  INPUT V1110
                               no-lock no-error.
                  ELSE find next wo_mstr where 
                              wo_site = V1002 AND wo_nbr = V1100 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" )  
                               no-lock no-error.
                  IF AVAILABLE wo_mstr then display skip 
...
/*mage add*****************************************************************/
            ...
            THEN DO:
                  IF recid(wo_mstr) = ? THEN find first wo_mstr where 
                              wo_site = V1002 AND wo_nbr = V1100 AND INDEX("AR",WO_STATUS) <> 0   AND  
                              wo_lot >=  INPUT V1110
                               no-lock no-error.
                  ELSE find next wo_mstr where 
                              wo_site = V1002 AND wo_nbr = V1100 AND INDEX("AR",WO_STATUS) <> 0  
                               no-lock no-error.
                  IF AVAILABLE wo_mstr then display skip 
            wo_lot @ V1110 "成品: " +  trim (wo_part) @ WMESSAGE NO-LABEL with fram F1110.
                  else   display skip "" @ WMESSAGE with fram F1110.
...
/*mage add*****************************************************************/
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first wo_mstr where wo_site = V1002 AND wo_lot = V1110 AND INDEX("AR",WO_STATUS) <> 0 AND ( substring (WO_SO_JOB,1,2) = "SA" OR substring ( WO_SO_JOB ,1,2) = "RA" )  NO-ERROR NO-WAIT.
        IF NOT AVAILABLE wo_mstr then do:
...
If AVAILABLE ( code_mstr ) then
        leave V1203L.
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1203 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
    ...
    /* Additional Labels Format */
     ...
     REPEAT:
        ...
        IF V1410 <> "L" then
        leave V1501L.
	find first ld_det where ld_site = v1002 and ld_part = v1300 and ld_loc = v1400
	 and ld_lot = v1500 no-lock no-error .
	 if available ld_det then do :
	
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1501 no-box.
	...
	end . /* available ld_det */

        /* PRESS e EXIST CYCLE */
        IF V1501 = "e" THEN  LEAVE V1100LMAINLOOP.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
...
If AVAILABLE ( wo_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[成品入庫]"        + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
