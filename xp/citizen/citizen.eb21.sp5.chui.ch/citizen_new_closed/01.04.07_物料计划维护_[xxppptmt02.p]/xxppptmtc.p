...
/* Revision: 1.29       BY: Paul Donnelly (SB)  DATE: 06/28/03   ECO: *Q00K*  */
/* Revision: 1.30       BY: Jean Miller         DATE: 02/23/04   ECO: *Q063*  */
/* Revision: 1.31       BY: Max Iles            DATE: 10/05/04   ECO: *N2Y9*  */
/* $Revision: 1.32 $        BY: Shivaraman V.       DATE: 10/13/04   ECO: *P2N9*  */
/******************************************************************************/
...
/* SS - 101208.1  By: Roger Xiao */ /*if pt_vend changed ,modify pt__chr03 to default */
...
define var v_site  like ptp_site . /*xp001*/
...
do transaction with frame c on endkey undo, leave:
   ...
   display
      inmstr4.in_mrp when (available inmstr4) @ pt_mrp
      pt_cum_lead
   with frame c.
...
/* SS - 101208.1 - E */
   ...
   old_insp_rqd = pt_insp_rqd . /*xp001*/
   ...
   do on error undo, retry with frame c:
      ...
      set
         ...
         pt_routing
         pt_bom_code
      go-on ("F5" "CTRL-D") with frame c no-validate.
      ...
      if available soc_ctrl and soc_atp_enabled = no then do:
         ...
         then do:
            ...
            {pxmsg.i &MSGNUM=4095 &ERRORLEVEL={&WARNING-RESULT}
                     &PAUSEAFTER = true}
         end.
   ...
   /*xp001*/ if pt_insp_rqd <> old_insp_rqd then do:
				v_site = "" .
				{gprun.i ""xxppptmtinsp.p""
					     "(input pt_part,
						   input v_site ,
						   input pt_insp_rqd )"}
				/*xxppptmtinsp.p”Î1.4.17π≤”√*/
   /*xp001*/ end.

/* SS - 101208.1 - B */
    if pt_vend <> v_old_vend then do:
