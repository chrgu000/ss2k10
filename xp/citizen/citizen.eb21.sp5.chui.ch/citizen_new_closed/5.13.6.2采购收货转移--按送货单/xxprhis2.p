...
/* SS - 110301.1  By: Roger Xiao */ /*按批序号汇总,检查库存是否over-issue*/
/*-Revision end---------------------------------------------------------------*/
	       ...
	       and   tr1__log01 = no with frame e   :
               for each sr_wkfl
                  ...
                  sr_rev
                   ...
                   mfguser
                    and sr_lineid =  tr1_lot + string(tr1_line) + "::" + tr1_part
...
do on endkey undo mainloop, retry mainloop:
    ...
    define var v_qty_tmp like tr_qty_loc .

    for each sr_wkfl
        where sr_wkfl.sr_domain = global_domain 
        and  sr_userid = mfguser
        no-lock 
        break by sr_user2 by sr_site by sr_loc by sr_lotser by sr_ref :

        if first-of(sr_ref) then do:
            v_qty_tmp = 0 .
        end.

        v_qty_tmp = v_qty_tmp + sr_qty.

        if last-of(sr_ref) then do:
            find first pt_mstr where pt_domain = global_domain and pt_part = sr_user2 no-lock no-error.
            assign  trans_um   = if available pt_mstr then pt_um else  ""
                    trans_conv = 1.
           ...
           then do:
              assign
                        line = integer(sr_rev)
                        part = sr_user2.
              message "零件批序:"part "/" sr_site "/" sr_loc "/" sr_lotser ",累计转出量:" v_qty_tmp .
              next setd.
           end.
        end.
    end. /* FOR EACH sr_wkfl ...*/
...
each tr_hist use-index tr_trnbr
        where tr_hist.tr_domain = global_domain 
        and tr_trnbr = tr1_trnbr
        :
        assign tr__dec01 = tr__dec01 +	tr1_chg.
        if tr_qty_loc <=  tr__dec01 then tr__log01 = yes.
end. 

for each trhist1 no-lock where tr1_chg <> 0 :

        for each tr_hist  use-index tr_trnbr no-lock 
        where tr_hist.tr_domain = tr1_domain  
        and tr_trnbr = tr1_trnbr :

                if tr__log01 = yes then do:
         ...
         then do:
		  ...
		  lotserial_qty
	          ...
	          tr1_loc_to @ location1 
               with frame d.
      assign  cline       = tr1_lot + string(line) + "::" + part
         global_part = part.
...
location1 = tr1_loc_to .
cline = tr1_lot + string(line) + "::" + part .
global_part = part.
...
***************************/           
	 ...
	 and    tr1_lot2    =  nbr
	 and    tr1_chg <> 0
         and    tr1_lot + string(tr1_line) + "::" + tr1_part   = cline:
      end. 
