/* ppptiqb.p - ITEM MASTER INQUIRY SUBROUTINE                           */
...
/* Revision: 1.1.2.9.1.7  BY: Paul Donnelly (SB)   DATE: 06/28/03 ECO: *Q00K* */
/* Revision: 1.1.2.9.1.8  BY: Dayanand Jethwa      DATE: 10/14/04 ECO: *P2PS* */
/* Revision: 1.1.2.9.1.9  BY: Abhishek Jha         DATE: 12/23/04 ECO: *P2ZV* */
/* Revision: 1.1.2.9.1.10 BY: Vandna Rohira        DATE: 02/14/05 ECO: *P35T* */
/* $Revision: 1.1.2.9.1.11 $           BY: Preeti Sattur        DATE: 11/28/05 ECO: *P49K* */
...
form
   {xxppptmta1.i}
   pt_site colon 19
with frame a0 side-labels width 80 attr-space.
...
with frame a1 title color normal (getFrameTitle("ITEM_DATA",20))
...
form
   {ppptmta3.i}
with frame b1 title color normal
...
form
   {ppptmta5.i}
with frame d1 title color normal
...
form
   {ppptmta9.i}
with frame d0 1 down width 80 no-labels
   title color normal (getFrameTitle("TOTALS",16)) no-attr-space.

form
   {ppptmt10.i}
with frame bb title color normal
...
form
   pt_rp_bom           colon 35
   pt_rp_route         colon 35
   pt_rp_vendor        colon 35
   skip(1)
   pt_pvm_bom          colon 35
   pt_pvm_route        colon 35
   skip(1)
   pt_ins_bom          colon 35
   pt_ins_route        colon 35
with frame f title color normal (getFrameTitle("SERVICE_STRUCTURE_DATA",32))
side-labels width 80 attr-space.
...
no-error.
   ...
   find in_mstr  where in_mstr.in_domain = global_domain and  in_part = pt_part
   and in_site = pt_site
   no-lock no-error.
   if available in_mstr then do:
      ...
      display {xxppptmta2.i} with frame a1.
      ...
      then do:
         ...
         display {ppptmta3.i} with frame b1.
         hide frame b1.

         assign l_comm_code = "".
         for first comd_det  where comd_det.comd_domain = global_domain and
         comd_part = pt_part
         no-lock:
         end.
         ...
         display {ppptmt10.i} with frame bb.
         ...
         display {ppptmta4.i} with frame c.
         display in_mrp @ pt_mrp with frame c.

         if pt_bom_code <> "" then
         find bom_mstr no-lock
             where bom_mstr.bom_domain = global_domain and  bom_parent =
             pt_bom_code no-error.
         else
         find bom_mstr no-lock
             where bom_mstr.bom_domain = global_domain and  bom_parent =
             pt_part no-error.
         ...
         display {ppptmta5.i} with frame d1.
         ...
         on endkey undo, next mainloop:
            ...
            display
               ...
               pt_mfg_mttr
               pt_sttr
            with frame service-data.

            hide frame service-data.
         ...
         end. /* IF CAN-FIND(FIRST svc_ctrl ... */
