...
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2007/11/14  ECO: *xp001*  */
/*-Revision end---------------------------------------------------------------*/
...
/*xp001*/
define var code  like ps_par label "ps_par" .
define var code1 like ps_par label "ps_par" .
define variable ptdesc3  like pt_desc1.
define variable ptdesc4  like pt_desc2.
define var v_process like ps_process label "±ä¸üÆ±ºÅ" .
/*xp001*/
...
form
   ...
   effdate      colon 23
   part         colon 23 
				ptdesc1 at 45 no-label
   ptum         colon 23 
				ptdesc2 at 45 no-label
   code         colon 23  
				ptdesc3 at 45 no-label
   code1        colon 23  
				ptdesc4 at 45 no-label
   v_process    colon 23 
...
repeat:
 ...
 if code1 = "ZZZZZZZZZZZZZZZZZZZZ"  then code1 = "" . 
   ...
   editing:
	  ...
	  else if frame-field = "code" then do:
         {mfnp.i ps_mstr code  " ps_domain = global_domain and ps_par"  code ps_par ps_parcomp}
	  ...
	  end.  /*if frame-field = "code" then do:*/
	  else if frame-field = "code1" then do:
         {mfnp.i ps_mstr code1  " ps_domain = global_domain and ps_par"  code1 ps_par ps_parcomp}
	  ...
	  end.  /*if frame-field = "code1" then do:*/
      else do:
         readkey.
         apply lastkey.
      end.
   ...
   find pt_mstr where pt_domain = global_domain and pt_part = code  no-lock no-error.
   if available pt_mstr then do with frame a:
      display
         pt_desc1 @ ptdesc3 .
   end.
   ...
   find pt_mstr where pt_domain = global_domain and pt_part = code1  no-lock no-error.
   if available pt_mstr then do with frame a:
      display
         pt_desc1 @ ptdesc4 .
   end.
   ...
   if code1 = "" then code1 = "ZZZZZZZZZZZZZZZZZZZZ" . 
   
   if not can-find (first ps_mstr where ps_domain = global_domain 
							and ps_comp = part
						    and ( ps_par >= code and ps_par <= code1 )
					)
   then do:
   ...
   do transaction:
      ...
      if can-find (first ps_mstr where ps_domain = global_domain 
								 and ps_comp = part
								 and ( ps_par >= code and ps_par <= code1 )
            ...
            if not available psmstr then do:
               ...
               if action = "A" then do:
                  ...
                  assign psmstr except oid_ps_mstr ps_domain.
						 psmstr.ps_process = psmstr.ps_process + "," + v_process. /*xp001*/
               ...
               if action = "R" then do:
				  ...
				  ps_mstr.ps_process = ps_mstr.ps_process + "," + v_process. /*xp001*/
                  ps_mstr.ps_mod_date = today.
                  ps_mstr.ps_userid = global_userid.
