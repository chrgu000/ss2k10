...
define var line as integer label "Ïî" format ">>9" .   /*xp001*/
...
form
   ...
   ecmid      colon 31
   dbase      colon 51
   pt__chr01  colon 13 label "±ä¸üÆ±ºÅ" format "x(24)" /*xp001*/
...
form
   ...
   ps_ps_code
   iss_pol
with frame heading  width 90  /*xp001*/ no-attr-space.
...
repeat:
      ...
      then do:
         ...
         then do:
            ...
            display
			   ...
			   find first pt_mstr where pt_mstr.pt_domain = global_domain and  pt_part = parent
                         no-lock no-error .
               if available pt_mstr  then  display pt__chr01 with frame a.
   ...
   then do:
      ...
      display
		...
		find first pt_mstr where pt_mstr.pt_domain = global_domain and  pt_part = parent no-lock no-error .
		if available pt_mstr  then  display pt__chr01 with frame a.
