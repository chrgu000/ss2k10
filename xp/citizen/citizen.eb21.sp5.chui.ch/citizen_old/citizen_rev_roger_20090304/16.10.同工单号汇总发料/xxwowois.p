...
define variable dummy_gl_amt  like tr_gl_amt no-undo.
	...
	define temp-table temp 	field t_lot like wo_lot . /*xp001*/
...
form
   wo_nbr      colon 12
   /*wo_lot      colon 36  xp001*/ 
   wo-op       colon 40
   eff_date    colon 68
   wo_part     colon 12 wo_status column-label "状态"  fill_all    colon 68
...
repeat:
	...
	for each temp : delete temp . end .  /*xp001*/	
   ...
   editing:
      ...
      if frame-field = "wo_nbr" then do:
         ...
         if recno <> ? then do:
            nbr = wo_nbr.
            desc1 = "".
            find pt_mstr  where pt_mstr.pt_domain = global_domain and  pt_part = wo_part   no-lock no-error.
            if available pt_mstr then desc1 = pt_desc1.
            display wo_nbr  /*wo_lot xp001*/  wo_part wo_status desc1       with frame a.
         end.
         else do:
            nbr = input wo_nbr.
         end.
...
xp001*/ 
   ...
   if input wo_nbr <> "" then
		if not can-find(first wo_mstr using  wo_nbr where wo_mstr.wo_domain = global_domain ) then do:
	...
	**xp001*/ 
	...
	find first wo_mstr where wo_domain = global_domain and wo_nbr = nbr no-lock no-error . /*xp001*/  
	...
	find first wo_mstr where wo_domain = global_domain and wo_nbr = nbr and (lookup(wo_status,"A,R") <> 0 ) no-lock no-error . /*xp001*/  
	if not available wo_mstr then do:
		message "工单非备料/发放状态,不可发料." . 
		undo, retry.
	end.
...
for each wo_mstr where wo_domain = global_domain and wo_nbr = nbr  and (lookup(wo_status,"A,R") <> 0 )  no-lock :
	...
	/*xp001**     
		   ...
		   if wo_joint_type <> "" then do:
			  ...
			  if wo_joint_type = "5" then base = yes.
				 ...
				 find wo_mstr  where wo_mstr.wo_domain = global_domain and  wo_lot = base_id no-lock no-error.
				 if not available wo_mstr then do:
   ...
   **xp001*/
   ...
   then do:
	...
	for each wod_det
		fields( wod_domain  wod_lot wod_op wod_qty_req )
		where wod_det.wod_domain = global_domain 
		and ( wod_lot = wo_lot)
		and  (wod_op    = wo-op   or    wo-op     = 0)
	no-lock :
		if wod_qty_req < 0 then do:
			message "错误:不允许负需求的子零件." .
			undo,retry .
		end.
	end.
...
/*xp001*********up: disp the frame a ,with first wo_mstr.wo_nbr ********************************************/

for each sr_wkfl exclusive-lock  where sr_wkfl.sr_domain = global_domain and  sr_userid = mfguser:
	delete sr_wkfl.  
end. /*xp001 from wowoisb.p : sr_lineid = string(wod_part,"x(18)") + string(wod_lot,"x(8)")+ string(wod_op) */
...
for each temp :
		find first wo_mstr where wo_domain = global_domain and wo_lot = t_lot no-lock no-error .
		wo_recno = recid(wo_mstr).


	   find first wo_mstr where wo_domain = global_domain  and   recid(wo_mstr) = wo_recno  exclusive-lock no-error.
	   if available wo_mstr  then
		  recno = recid(wo_mstr).

	   {gprun.i ""xxwowoisb.p"" "(wo_recno, wo-op, fill_all, fill_pick, output undo-input)"} /*xp001*/

	   if undo-input then next mainloop.

end. /*for each temp :*/
/*xp001********up : create  issue items' sr_wkfl here*********************************************/


	find first wo_mstr where wo_domain = global_domain and wo_nbr = nbr  and (lookup(wo_status,"A,R") <> 0 ) no-lock no-error .
	wo_recno = recid(wo_mstr). /*xp001*/
	
	
	/*disp details & and check the issue items here*/
   {gprun.i ""xxwowoisc.p"" "(input wo-op, output undo-input)"} /*xp001*/ 

   if undo-input then next mainloop.


/*xp001********up : disp details & and check the issue items here*********************************************/
   ...
   run wip_lot_trace_processing
	...
	for each temp :  /*执行发料*/
		   ...
		   if is_wiplottrace_enabled() and
			  is_woparent_wiplot_traced(input wo_lot)
		   then do:
		   ...
		   {gprun.i ""xxwowoisa.p"" 
			...
			/*xp002*/
			for each wod_det 
				fields (wod_domain wod_lot wod__chr01)
				where wod_domain = global_domain 
				and wod_lot = t_lot :
				wod__chr01 = "" . /*过程中存放自编单号,发料时写入tr_rmks, 这里清空,*/
			end. /*xp002*/
