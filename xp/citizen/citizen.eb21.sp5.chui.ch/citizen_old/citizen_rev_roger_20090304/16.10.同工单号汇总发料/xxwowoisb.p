 ...
 */
   for each wod_det  where wod_det.wod_domain = global_domain and (  wod_lot = wo_lot and (wod_op = wo-op or wo-op = 0)) no-lock:
         ...
         if fill_all or fill_pick then do:
            ...
            lad_dataset = "wod_det"
               ...
               if ladqtychg <> 0 then do:
                  ...
                  assign
                     sr_userid = mfguser
					 /*sr_lineid = string(wod_part,"x(18)") + string(wod_op)*/  /*xp001*/ 
                     sr_lineid = string(wod_part,"x(18)") + string(wod_lot,"x(8)") /*xp001*/ + string(wod_op)
                     sr_site = lad_site
                     sr_loc = lad_loc
                     sr_lotser = lad_lot
                     sr_ref = lad_ref
