...
define new shared var v_loc like loc_loc .   v_loc = "100" . /*xp001*/
...
global_domain ) then do:
   ...
   then
      find wo_mstr
          where wo_mstr.wo_domain = global_domain and  wo_nbr = nbr using wo_lot
         no-error.

   if nbr = ""
   and input wo_lot <> ""
   then
find wo_mstr using  wo_lot where wo_mstr.wo_domain = global_domain  no-error.
...
if v_loc <> "" then do:
	find first loc_mstr where loc_domain = global_domain and loc_loc = v_loc no-lock no-error .
   ...
   then do:
      ...
      {gprun.i ""xxwoisrc01_test.p"" "(input Yes)"}  /*xp001*/

      /*V8-*/
      if (keyfunction(lastkey) = "end-error" or outta_here)
      then
         undo, retry.
      /*V8+*/
