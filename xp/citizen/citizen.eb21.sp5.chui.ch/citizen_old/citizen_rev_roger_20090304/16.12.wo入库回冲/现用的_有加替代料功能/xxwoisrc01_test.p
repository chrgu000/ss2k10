...
/* Revision: 1.31     BY: Manisha Sawant          DATE: 08/25/03 ECO: *N2GV* */
/* Revision: 1.32     BY: Ken Casey               DATE: 02/19/04 ECO: *N2GM* */
/* Revision: 1.33     BY: Dayanand Jethwa         DATE: 04/28/04 ECO: *P1YJ* */
/* Revision: 1.34     BY: Ed van de Gevel         DATE: 03/24/04 ECO: *Q06N* */
/* Revision: 1.35     BY: Max Iles                DATE: 07/01/04 ECO: *N2VQ* */
/* Revision: 1.36     BY: Bhagyashri Shinde       DATE: 07/14/04 ECO: *P29X* */
/* Revision: 1.36.1.1 BY: Priyank Khandare        DATE: 05/13/05 ECO: *P3L6* */
/* $Revision: 1.36.1.2 $       By: Chi Liu                 DATE: 07/07/05 ECO: *P3S7* */



/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/01/16  ECO: *xp001*  */ /*输入一个库位,材料默认回冲用*/
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/02/25  ECO: *xp002*  */ /*优先发放替代料:加临时table,F4时复原wod_det*/
/*-Revision end---------------------------------------------------------------*/
...
define new shared temp-table xx_Det  /*xp002*/
...
with overlay frame d side-labels width 80 attr-space row 16.
{&WOISRC01-P-TAG3}
...
title color normal (getFrameTitle("ISSUE_DATA_REVIEW",78)).

/* SET EXTERNAL LABELS */
setFrameLabels(frame e:handle).

find first clc_ctrl
 where clc_ctrl.clc_domain = global_domain no-lock no-error.
...
repeat:
   ...
   do transaction:
      {gprun.i ""xxwoisrc1a_test.p""}  /*xp001*/
   end. /* DO TRANSACTION */
   ...
   do transaction on error undo, retry:
      ...
      then do:
         ...
         repeat on endkey undo, leave:
            ...
            then do:
               ...
               {windowxt.i
                  ...
                  &framephrase = "size 80 by 9 "
				  &window-down = 5
                  &where1 = "where wod_domain = global_domain and wod_lot = wo_lot"
                  &where2 = "where wod_domain = global_domain and true"
                  &and         = "(wod_lot = wo_lot)"
                  &time-out    =  0
                  }
            ...
            do on error undo, retry with frame d:
               ...
               editing:
                  ...
                  then do:
                     ...
                     {mfnp01.i wod_det part wod_part wo_lot  "
                     wod_det.wod_domain = global_domain and wod_lot "
                        wod_det}
               ...
               repeat:
                  ...
                  assign
                     site      = "" 
                     /*location  = ""*/  /*xp001*/
					 location  = if v_loc = ""  then "" else v_loc 
                     lotserial = ""
                     lotref    = "".
                  ...
                  then do:
                     /*assign
                        ...
                        location  = lad_loc
                        lotserial = lad_lot
                        lotref    = lad_ref.*/  /*xp001*/

						if v_loc = "" then do:  /*xp001*/
							assign
								site = lad_site
								location  = lad_loc
								lotserial = lad_lot
                        ...
                        site     = wod_site .
                        if v_loc = "" then location = wod_loc .   /*xp001*/	
                  end. /* ELSE DO */
                  ...
                  do on error undo, retry
                     ...
                     then do:
                        ...
                        then do:
                           ...
                           for each lad_det
                              fields( lad_domain lad_dataset lad_line lad_loc
                              lad_lot
                        ...
                        then do:
                           ...
                           then do:
                              create lad_det. lad_det.lad_domain =
                              global_domain.
                              assign
                                 lad_dataset = "wod_det"
                                 lad_nbr     = wod_lot
         ...
         do on endkey undo setd, leave seta:
            ...
            /*
             * In Desktop, to give a chance to cancel the current transaction,
             * just like pressing endkey in CHUI.
             */
            if {gpiswrap.i} and yn = no then do:
               /* DO YOU REALLY WANT TO LEAVE AND UNDO CURRENT TRANSACTION */
               {pxmsg.i &MSGNUM=1346 &ERRORLEVEL=1 &CONFIRM=yn
                        &CONFIRM-TYPE='LOGICAL'}
               if yn = yes then do:
                  outta_here = yes.
         ...
         then do:
            ...
            for each wod_det
               ...
               for first pt_mstr
                  fields( pt_domain pt_desc1 pt_loc pt_lot_ser pt_part pt_um)
                   where pt_mstr.pt_domain = global_domain and  pt_part =
                   wod_part
                  no-lock:
			...
			do:
				for each wod_det 
					...
					:
					find first xx_det 
					where xx_lot = wod_det.wod_lot 
					and xx_part = wod_det.wod_part 
					and xx_op = wod_det.wod_op no-error.
					if not avail xx_det then do:
						delete wod_det .
					end.
					else do:
						assign 
							wod_det.wod_qty_req = xx_qty_req 
							wod_det.wod_qty_chg = xx_qty_chg
							wod_det.wod_bo_chg  = xx_bo_chg .
					end.
				end.  
