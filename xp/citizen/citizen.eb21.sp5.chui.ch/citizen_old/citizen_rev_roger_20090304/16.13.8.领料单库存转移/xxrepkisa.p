...
laddet-loop:
for each xic_det exclusive-lock  where xic_det.xic_domain = global_domain 
      and xic_nbr = nbr and xic_qty_from <> 0:

   for each sr_wkfl exclusive-lock  where sr_wkfl.sr_domain = global_domain and
    sr_userid = mfguser
         and sr_lineid = string(xic_line) + "::" + xic_part
         and sr_site = xic_site_from and sr_loc = xic_loc_from
         and sr_lotser = xic_lot_from and sr_ref = xic_ref_from:

      {&REPKISA-P-TAG4}

      global_part = xic_part.
      ...
      = xic_part
         and ld_site = xic_site_to
         and ld_loc = xic_loc_to and ld_lot = xic_lot_to
         and ld_ref = xic_ref_to no-error.
      ...
      find ld_det exclusive-lock  where ld_det.ld_domain = global_domain and
         ld_site = xic_site_to and
         ld_loc = xic_loc_to and
         ld_part = xic_part and
