...
/*xp001*/ /* ststatus = stline[3]. */
/*xp001*/ /* status input ststatus.*/
...
then do:
   ...
   then do:
      for first pt_mstr
         fields( pt_domain pt_part pt_prod_line)
          where pt_mstr.pt_domain = global_domain and  pt_part = wo_bom_code
          no-lock:
      end.
      l_joint_prod = yes.
   end.
   ...
   then do:
      ...
      then do:
         ...
         for first pl_mstr
            fields( pl_domain pl_prod_line pl_xvar_acct)
             where pl_mstr.pl_domain = global_domain and   pl_prod_line =
             pt_prod_line no-lock:
         end.
