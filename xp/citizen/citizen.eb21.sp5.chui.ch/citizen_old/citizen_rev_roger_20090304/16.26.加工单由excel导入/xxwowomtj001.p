...
*/
wocmmts = no .
if v_delete = no then do:
	assign  wo_qty_ord = v_qty_ord
			wo_status = v_status
			wo_ord_date = today 
			wo_rel_date = v_rel_Date
			wo_due_Date = v_due_Date
			wo_so_job = v_so_job 
			wo_var = no 
			.
end.
...
/*xp001*/ 
/*******************   
   if /* DELETE */ lastkey = keycode("F5") or lastkey = keycode("CTRL-D")
   then do:
...
if  v_delete = yes then do:
   ...
   if not del-yn then do:
    ...
    /*xp001*/ /*     display prev_routing @ wo_routing. */
         undo, retry.
      end.
      ...
      /*xp001*/ /*   display prev_bomcode @ wo_bom_code. */
         undo, retry.
      end.
         ...
         if not add_2_joint then do:
            ...
            then do:
               assign
                  ...
                  wo_base_id = ""
                  joint_label = "".
           /*xp001*/ /*    display joint_label with frame a. */ 
            end.
...
*/
     ...
     /*xp001*/ /*    if not batchrun then pause.
	 */
      end.
...
*/
         ...
         end. /*if bom_code_entered or rel_date_entered then*/
...
*/ 
      ...
      if prev_site <> wo_site then do:
         ...
         then do:
            ...
            next-prompt wo_site.
 /*xp001*/ /*           display prev_site @ wo_site.
 */
            undo, retry.
         end.
      ...
      if available clc_ctrl
         ...
         /*xp001*/ /* {gprun.i ""clatmt1.p"" "(wo_recno)"}  xp001*/
		 /*xp001*/ wo_lot_next = v_lot_next .
      end.
      ...
      {gprun.i ""xxwomtacct001.p""} /*xp001 */
	  /*run D:\000Workfiles\citizen200711\16.1\xxwomtacct001.p. xp001 */
