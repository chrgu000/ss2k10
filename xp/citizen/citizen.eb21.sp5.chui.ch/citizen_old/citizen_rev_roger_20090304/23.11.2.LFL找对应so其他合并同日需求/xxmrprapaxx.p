...
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/01/16  ECO: *xp001*  */ 
                            /*找数量刚好匹配的so,记录so_nbr到req_det or rqd_det*/

/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/04/22  ECO: *xp003*  */  
					        /*加记录so_rmks 到req_det or rqd_det*/
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/07/28  ECO: *xp004*  */  
					        /*找so,日期wod_iss_date是成品,wo_due_date是材料*/
/*-Revision end---------------------------------------------------------------*/
...
define new shared var v_sonbr  like sod_nbr.   /*xp001*/
define new shared var v_soline like sod_line .  /*xp001*/
define new shared var v_sormks like so_rmks  .  /*xp003*/
define var v_part like pt_part.                 /*xp001*/
define var v_qty_ord like wo_qty_ord .          /*xp001*/
define var v_due_date like wo_due_date .         /*xp001*/
define buffer twomstr for wo_mstr.     /*xp001*/
...
then do:
   ...
   for each tt-rqm-mstr
      where tt-nbr = ""
   use-index ttnbr
   exclusive-lock break by tt-vend  by tt-site by tt-loc /*xp002*/ :

      for first b-rqm-mstr
         where b-rqm-mstr.tt-vend = tt-rqm-mstr.tt-vend
         and b-rqm-mstr.tt-site = tt-rqm-mstr.tt-site /*xp002*/ 
         and b-rqm-mstr.tt-loc  = tt-rqm-mstr.tt-loc  /*xp002*/ 
                  ...
                  then do:
                     ...
                     then do:
                        ...
                        find first wod_det 
                            ...
                            and wod_qty_req = wo_qty_ord 
                            and (if wo_due_date > today 
                                 then wod_iss_date = wo_due_date 
                                 else wod_iss_date >= wo_due_date 
                                ) /*到期日小于今天,最迟今天发放*/ /*xp004*/
                            and wod_lot <> wo_lot
                            and can-find(first twomstr where twomstr.wo_domain = global_domain and twomstr.wo_lot = wod_lot and twomstr.wo_status = "P" no-lock)
                        no-error .
                        if avail wod_det then do:
                            ...
                            if available mrp_det then do:
                                v_part = mrp_part.
                                v_qty_ord = mrp_qty .
                                v_due_date = mrp_due_date.                                
                                find first mrp_det 
                                    where mrp_domain  = global_domain
                                    and   mrp_dataset = "sod_det"                             
                                    and   mrp_part    = v_part
                                    and   mrp_qty     = v_qty_ord
                                    and  (if weekday(v_due_date) < 6 
                                          then mrp_due_date = v_due_date 
                                          else (mrp_due_date = v_due_date or mrp_due_date = v_due_date + 1 or mrp_due_date = v_due_date + 2 )
                                         ) /*周六日的so_due_date会提前到wo_mstr的周五duedate*/
                                no-error.
                                if avail mrp_det then do:
                                    v_sonbr = mrp_nbr .
						            v_soline = integer(mrp_line) .

                                    repeat:
                                        find first rqd_det 
                                            use-index rqd_part
                                            where rqd_domain = global_domain 
                                            and rqd_part = mrp_part 
                                            and rqd__chr01 = mrp_nbr
                                            and rqd__chr03 = string(v_soline) 
                                        no-error .
                                        if avail rqd_det then do:
                                            ...
                                            find next mrp_det 
                                                ...
                                                and   mrp_part    = v_part
                                                and   mrp_qty     = v_qty_ord
                                                and   mrp_due_date = v_due_date 
                                            no-error.
                                            if avail mrp_det then do:
                                                v_sonbr = mrp_nbr .
                                                v_soline = integer(mrp_line) .
                                            end.
                                            else leave.  /*soline已转pr,且无其他soline可用*/
                                        end.
                                        else leave.  /*soline可用*/
                                    end. /*repeat:*/
                                    

                                    find first sod_det use-index sod_part  
                                            where sod_domain = global_domain 
                                            and sod_nbr  = v_sonbr
                                            and sod_line = v_soline 
                                            /*and sod__chr01 = "" */
                                    no-error.
                                    if avail sod_det then do:
                                        /*sod__chr01 = "A" . */ /*Approved to PR*/
                                        find first so_mstr 
                                            where so_domain = global_domain 
                                            and so_nbr = sod_nbr 
                                        no-lock no-error.
                                        v_sormks = if avail so_mstr then so_rmks else "" .   /*xp003*/
                                    end.  /*if avail sod_det then */
                                    /*else  message "d" view-as alert-box.*/
                                end.  /*if avail mrp_det */
                                /*else  message "c" view-as alert-box.*/
                            end. /*if available mrp_det */
                            /*else  message "b" view-as alert-box.*/

                        end. /*wod_det*/
                        /*else  message "a" view-as alert-box.*/
                           ...
                           no-lock:
                              ...
                              {gprunmo.i
                               ...
                               &param="""(input grs_approval_cntr,
                                          ...
                                          input wo_due_date,
                                          input remarks_text,
                                          output grs_return_code)"""}  /*xp001*/ /*<-- rqm__chr01 = v_sonbr*/
