...
/* Revision: xx , by  softspeed Roger Xiao 20071018  ECO: xp001               */
...
repeat with frame {&framename}:
   ...
   if available {&detfile} then do:
	  ...
	  {gprun.i  ""xxmppopoca.p""  "(input {&display3} ,input {&display4},input {&display2},input {&ext01})"}  /*xp001*/
   ...
   repeat on endkey undo, leave {&exitlabel} with frame {&framename}:
      ...
      then do with frame {&framename}:
		 ...
		 if frame-line <> frame-down then do:
            ...
            down.
			  /*xp001*/
			find next {&detfile} {&detkey} {&searchkey} {&other-search} {&index-phrase} no-lock no-error.
			if avail {&detfile} then do:
				{gprun.i  ""xxmppopoca.p""  "(input {&display3} ,input {&display4},input {&display2},input {&ext01})"} 
				find prev {&detfile} {&detkey} {&searchkey}	{&other-search} {&index-phrase} no-lock no-error.
			end.
			  /*xp001*/
 ...
 /*  xp001      if keyfunction(lastkey) = "return" then do with frame {&framename}:
            if frame-line <> frame-down then do:
               color display normal {&display1} with frame {&framename}.
               down.
               /*Test to see if at eof*/
		...
		leave sw-main-loop. /* xp001*/
      end.  /*end : RETURN or SPACE will toggle  */
