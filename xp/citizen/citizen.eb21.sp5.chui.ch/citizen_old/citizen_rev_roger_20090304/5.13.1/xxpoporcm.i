      ...
      then do:
         ...
         then do:
            ...
            display
               ...
               vndname
               "" @ ps_nbr
	       po_site @ v_site /*xp001*/
	       po_ship @ v_shipto  /*xp001*/
            with frame {&frame}.
         end. /* IF RECNO <> ? */
...
v_site   = po_site  .
v_shipto = po_ship .
disp v_site v_shipto v_from with frame {&frame} .

/*find last prh_hist where prh_domain = global_domain and prh_nbr = ordernum no-lock no-error . 
if avail prh_hist then do:
v_shipto = prh__chr01 .

disp v_shipto v_site with frame {&frame} .
...
/*xp001 begin********/
assign v_from v_type .

if not 
(not can-find(first code_mstr where code_mstr.code_domain = global_domain and  code_fldname = "prh__chr01" )
 or  can-find (code_mstr  where code_mstr.code_domain = global_domain 
			  and code_fldname = "prh__chr01" 
			  and code_value   =  v_from )
) then do:
  message "发货自,无效输入。" view-as alert-box .
  next-prompt v_from with frame {&frame}.
  undo,retry .
end.  

find first xdn_ctrl where xdn_domain = global_domain and xdn_site = v_site and xdn_ordertype = "RC" no-lock no-error.
if not avail xdn_ctrl then do:
	message "未启用按类别编号,由系统编号" .
end.
else do:
	find xdn_ctrl where xdn_domain = global_domain and xdn_site = v_site and xdn_ordertype = "RC" and xdn_type = v_type no-lock no-error.
	if not avail xdn_ctrl then do:
		message "无此收货单据类型,请先设定。" .
		next-prompt v_type with frame {&frame}.
		undo,retry .
	end.
end.
