...
define var v_shipto             as char    label "交运地址" . /*xp001*/
define var v_from               as char    label "发货自" . /*xp001*/
define new shared var  v_site like po_site .
define new shared var  v_type  like xdn_type label "单据类别".
...
form
   ...
   fill-all       colon 68
   v_type         colon 15  /*xp001*/
   v_from         colon 37  /*xp001*/
   cmmt_yn        colon 68
   v_site         colon 15   /*xp001*/
   v_shipto       colon 37     /*xp001*/
...
/* DELETE qad_wkfl RECORDS */
   ...
   do transaction:
      if porec
      then do:
         {xxpoporcm.i &frame=a1}  /*xp001*/ /*update v_from ,v_shipto=po_ship ,v_site = po_site, v_type here*/
      end. /* IF porec */
      else do:
         {poporcm.i &frame=a2}
      end. /* ELSE DO: */
...
/*xp001 begin********/
      for each prh_hist use-index prh_nbr 
			where prh_domain = global_domain 
			and prh_nbr = po_nbr  
			and prh_receiver = receivernbr 
			exclusive-lock :
	assign prh__chr01 = v_from .
      end.
...
------------------------------------------------------------------------------*/
   ...
   if proceed = yes then do:
      {pxrun.i &PROC='commitReceipt' &PROGRAM='xxporcxr.p'
               &PARAM="(input shipnbr,
                        input ship_date,
                        input inv_mov,
                        input ip_usage,
...
/* xxporcxr.p modified for create receivernbr from xdn_ctrl */ /*xp001*/
