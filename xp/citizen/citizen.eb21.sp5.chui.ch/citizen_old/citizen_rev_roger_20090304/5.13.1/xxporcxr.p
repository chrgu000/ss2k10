...
/* Revision: 1.39.1.1     BY: Puja Bajaj   DATE: 04/12/06  ECO: *P4P1*        */
/* Revision: 1.39.1.2     BY: Amit Kumar  DATE: 08/28/06  ECO: *P53J*       */
/* $Revision: 1.39.1.3 $   BY: Julie Milligan    DATE: 09/27/06 ECO: *P57P* */
...
------------------------------------------------------------------------------*/
   ...
   {gprun.i ""xxpoporcb.p""
      "(input pShipNbr,
        ...
        input 0,
        output pRctPOTrnbr)"}
/* xxpoporcb.p modified for create receivernbr from xdn_ctrl */ /*xp001*/
   end.
   return {&SUCCESS-RESULT}.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  do on error undo, return error {&GENERAL-APP-EXCEPT}:
   for first pod_det
      fields( pod_domain  pod_nbr pod_line pod_po_db)
       where pod_det.pod_domain = global_domain and  pod_nbr = pPurchaseOrderId
       no-lock:
   ...
   /* SO PREVENT THIS ERROR CONDITION IN THAT CASE. */
      ...
      if global_db <> pod_po_db and pod_po_db <> ""  then do:
...
------------------------------------------------------------------------------*/
  ...
  do on error undo, return error {&GENERAL-APP-EXCEPT}:
     chkpodsite:
     for each pod_det
        fields( pod_domain pod_nbr pod_line pod_site)
         where pod_det.pod_domain = global_domain and  pod_nbr =
         pPurchaseOrderId no-lock:
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for each bPod_det exclusive-lock
          where bPod_det.pod_domain = global_domain and (  pod_nbr =
          pPurchaseOrderId
         and   pod_status <> "c"
         and   pod_status <> "x"
...
------------------------------------------------------------------------------*/
   ...
   do transaction on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first po_mstr fields( po_domain po_fsm_type)
          where po_mstr.po_domain = global_domain and  po_nbr = pOrderId
          no-lock:
      ...
      for each prh_hist
         fields( prh_domain prh_curr_amt prh_line prh_nbr prh_rcvd
...
------------------------------------------------------------------------------*/
   ...
   do on error undo, return error {&GENERAL-APP-EXCEPT}:
      ...
      for first poc_ctrl
            fields( poc_domain poc_rcv_type)  where poc_ctrl.poc_domain =
