...
/*ss-min001 add******************************************************/
         ...
         then do:
            ...
            if recno <> ? then
               display
                  substring(prh_receiver,1,8) @ nbr
...
/***********************mage del**************
      if not can-find(first tr_det  where tr_det.tr_domain = global_domain
         and tr_site_from begins prod_site)
      then do:
...
/*mage*********************del ***********************
if loc-to <> "" then
for each tr_det where tr_det.tr_domain = global_domain and tr_receiver = nbr :
tr_loc_to = loc-to.
end.
****************************************************/
/*ss-min001 end**********/
...
/*ss-min001*/   {gprun.i ""xxprhisb.p"" "(nbr, prod_site, loc-to, picked, ponbr)"}
...
/*mage
         ...
         then do:
            ...
            {mfnp05.i
               trhist1
               tr1_part
              " trhist1.tr1_domain = global_domain and
                 tr1_qty_loc <> tr1__dec01 and tr1__log01 = no"
               tr1_part
               part}
...
/*mage del ****************************************************** 
	 ...
	 location1 = tr1_loc_to.
         ...
         for first loc_mstr
            fields( loc_domain loc_loc)
             where loc_mstr.loc_domain = global_domain and  loc_loc = tr1_loc_to
            no-lock:
         end. /* FOR FIRST loc_mstr */
      ...
      for first pt_mstr
         fields( pt_domain pt_desc1 pt_desc2 pt_lot_ser pt_part pt_um)
          where pt_mstr.pt_domain = global_domain and  pt_part = tr1_part
         no-lock:
      end. /* FOR FIRST pt_mstr */
