...
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/01/16  ECO: *xp002*  */ /*记录PO历史记录*/
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/01/16  ECO: *xp003*  */ /*添加限制:同销售订单才能转化为同一采购单. 并传递到PO */
...
define variable loc                   like loc_loc no-undo .  /*xp004*/
...
define new shared var v_sojob     as char label "销售订单" .    /*xp003*/
/*define new shared var v_sojob1    as char label "至" .  xp003*/
...
form
   ...
   v_sojob             colon 36    /*xp003*/
   /*v_sojob1            colon 49    xp003*/
...
repeat:
   ...
   /*if v_sojob1 = hi_char   then v_sojob1 = ''.   xp003*/
   ...
   /*if v_sojob1 = '' then v_sojob1 = hi_char.   xp003*/
   ...
   if rqpo_wrk_cntr = 0 then do:
      ...
      if info_correct then do:
         ...
         if can-find (first rqpo_wrk where rqpo_copy_to_po) then do:
            ...
            {gprun.i ""xxrqpobldb.p""}   /* SIMILAR TO pomt.p */   /*xp001*/  /*xp002*/ /*xp003*/
