...
/* $Revision: 1.5.3.9 $ BY: Paul Donnelly (SB) DATE: 06/28/03 ECO: *Q00L* */




/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/07/28  ECO: *xp006*  */ /*C开头的PR,币别取明细中供应商的币别*/
/*-Revision end---------------------------------------------------------------*/
...
then do:
   ...
   for each rqpo_wrk
      ...
      for first rqd_det
          where rqd_det.rqd_domain = global_domain and  rqd_nbr  = rqpo_nbr
      ...
      for first rqm_mstr
         fields( rqm_domain  rqm_cc rqm_curr rqm_email_opt rqm_end_userid
    ...
    if rqd_nbr begins "C" then do:
        ...
        wkrqd_curr         = if avail vd_mstr then vd_curr else rqm_curr .
    end.
    /*****xp006*/
