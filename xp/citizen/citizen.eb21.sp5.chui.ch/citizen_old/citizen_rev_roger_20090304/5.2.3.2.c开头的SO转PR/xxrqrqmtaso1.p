...
/* Revision: 1.64.1.1    BY: Tejasvi Kulkarni    DATE: 07/12/05  ECO: *P3T1*  */
/* Revision: 1.64.1.2    BY: Shivaraman V.       DATE: 08/18/05  ECO: *P3YM*  */
/* Revision: 1.64.1.3    BY: Somesh Jeswani      DATE: 11/28/05  ECO: *P49H*  */
/* $Revision: 1.64.1.4 $         BY: Amit Kumar      DATE: 07/21/06  ECO: *P4XX*  */
...
define input  parameter p_sngl_ln as logical no-undo.
...
line = 1.
find last rqd_det  where rqd_det.rqd_domain = global_domain and  rqd_nbr =
rqm_nbr no-lock no-error.
...
repeat:
   ...
   repeat transaction:
      ...
      find rqd_det  where rqd_det.rqd_domain = global_domain and  rqd_nbr =
      rqm_nbr and rqd_line = line
      no-lock no-error.
      ...
      do on error undo, retry:
         ...
         with frame b
         editing:
            {mfnp01.i rqd_det line rqd_line rqm_nbr  " rqd_det.rqd_domain =
         ...
         find first rqpo_ref
             ...
             rqm_nbr and rqpo_req_line = line
         no-lock no-error.
      ...
      if available rqd_det then do:
         ...
         then do:
            assign
               ...
               prev_ext_cost     = rqd_req_qty * net_price
      ...
      if new_rqd or rqd_part <> prev_item then do:
         ...
         if available pt_mstr then do:
            ...
            assign
               ...
               for first clc_ctrl
                  fields(clc_domain clc_active clc_polot_rcpt)
            ...
            then do:
               find first poc_ctrl  where poc_ctrl.poc_domain = global_domain
               no-lock.
            ...
            if available pl_mstr then do:
               ...
               vd_addr = rqd_vend no-lock no-error.
         ...
         then do:
            ...
            for each vp_mstr
               fields( vp_domain vp_curr vp_mfgr vp_mfgr_part vp_part vp_q_date
                ...
                where vp_mstr.vp_domain = global_domain and  vp_part = rqd_part
                 and vp_vend = vendor
            no-lock
      ...
      do on error undo, retry:
         ...
         then do:
            ...
            find first rqpo_ref  where rqpo_ref.rqpo_domain = global_domain and
             rqpo_req_nbr = rqd_nbr
            ...
            if del-yn then do:
               ...
               for each mrp_det exclusive-lock
                   where mrp_det.mrp_domain = global_domain and  mrp_dataset =
                   "req_det"
                    and mrp_nbr = rqd_nbr
               ...
               {gprun.i ""gpcurrnd.p"" "(input-output prev_ext_cost,
                                         input rndmthd)"}

               {gprun.i ""gpcurrnd.p"" "(input-output prev_max_ext_cost,
                                         input rndmthd)"}
         ...
         if available pt_mstr then do:
            ...
            do:
               ...
               if not (not can-find(first code_mstr
                                       where code_domain  = global_domain
                                         and code_fldname = 'rqd_um')
                  or can-find(code_mstr
                                 where code_domain   = global_domain
                                   and code_fldname  = 'rqd_um'
                                   and code_value    = rqd_um))
               then do:
               ...
               end.  /* IF NOT (NOT CAN-FIND(FIRST code_mstr */
      ...
      if sngl_ln then do:
         ...
         do on error undo, retry:
            ...
            /* CHECK TYPE */
            if not can-find(pt_mstr  where pt_mstr.pt_domain = global_domain
            ...
            if rqd_category > "" then do:
               ...
               find last rqcd_det
                   where rqcd_det.rqcd_domain = global_domain and
                   rqcd_category = rqd_category
               ...
               if not available rqcd_det then do:
                  ...
                  for last rqcd_det  where rqcd_det.rqcd_domain = global_domain
                  and
            ...
            do i = 1 to num-entries(messages):
               ...
               run p_ip_msg (input msgnbr, input 3).
               undo, retry.
      ...
      {gprun.i ""gpcurrnd.p"" "(input-output max_ext_cost,
                                input rndmthd)"}
...
PROCEDURE display_line_frame_c:
   ...
   for each rqpo_ref no-lock
       where rqpo_ref.rqpo_domain = global_domain and  rqpo_req_nbr = rqd_nbr
       and rqpo_req_line = rqd_line:
...
PROCEDURE get_site:
   ...
   do on error undo, retry:
      ...
      with frame b
      editing:
         {mfnp.i si_mstr rqd_site  " si_mstr.si_domain = global_domain and
...
PROCEDURE get_item:
   ...
   do on error undo, retry:
      ...
      editing:
         ...
         {mfnp.i pt_mstr rqd_part  " pt_mstr.pt_domain = global_domain and
...
PROCEDURE get_supplier:
   ...
   do on error undo, retry:
      ...
      editing:
         {mfnp.i vd_mstr rqd_vend  " vd_mstr.vd_domain = global_domain and
...
PROCEDURE get_remote_item_data:
   ...
   gl-site = si_site.
   for first in_mstr  where in_mstr.in_domain = global_domain and  in_part =
   p_part
                       and in_site = si_site
   no-lock: end.
...
PROCEDURE get_vendor_q_price:
   ...
   for each vp_mstr no-lock
       where vp_mstr.vp_domain = global_domain and  vp_part = p_part
        and vp_vend = p_vend
   break by vp_q_date descending:
...
PROCEDURE get_default_category:
   ...
   if not p_direct then do:
      find last rqcd_det
          ...
          where rqcd_det.rqcd_domain = global_domain and   rqcd_acct_from <=
          p_acct
...
PROCEDURE getPurchaseAcctDefault:
   ...
   then do:
      ...
      for first vd_mstr
         fields( vd_domain vd_addr vd_type vd_pur_acct)
          where vd_mstr.vd_domain = global_domain and  vd_addr = rqd_vend
          no-lock: end.
