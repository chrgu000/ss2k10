...
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2007/11/23  ECO: *xp001*  */  /*限制仅M类型PO*/
...
define frame getnbr .  /*xp001*/
define var v_site like si_site .
define var v_type like xdn_type label "单据类别" .
form 
   v_site     colon 10
   v_type     colon 30
with frame getnbr row 3 centered overlay side-labels.
...
{gprun.i ""xxhist001.p"" "(input ""PO"" ,input po_nbr ,input 1,output v_hist )"}
		...
		define input parameter v_todo   as integer  . /*todo: 1.程序前读入, 2.程式后检查 3.单据发行记录 */
		define output parameter v_hist as char  .     /*是否有做修改,或是否可发行*/
		****/
/*xp002*/
   ...
   */
   ...
   {gprun.i ""xxpopomtf.p""}  /*xp002*/ /*同5.7*/
...
------------------------------------------------------------------------------*/
 ...
 /*xp001*/ 
	...
	repeat with frame getnbr :
		...
		v_site = if avail icc_ctrl then icc_site else global_site .
		update v_site with fram getnbr .
