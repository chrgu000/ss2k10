...
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2007/11/23  ECO: *xp001*  */  /*限制仅可pod_type = M */
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/01/12  ECO: *xp002*  */  /*记录pod_So_job*/
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/03/28  ECO: *xp003*  */  /*pod_loc = in_loc */
/*-Revision end---------------------------------------------------------------*/
/*                                                                            */
/******************************************************************************/
...
define variable w_pod_so_job     like pod_so_job. /*xp002*/
      ...
      do transaction:
         ...
         if return-value = {&RECORD-NOT-FOUND} then do:
            ...
            if w_pod_req_nbr > "" then do:
						...
						if using_grs then do:
							find first rqd_det 
									...
									and rqd_nbr = w_pod_req_nbr 
									and rqd_line = w_pod_req_line
							no-lock no-error .
							w_pod_so_job = if avail rqd_det then rqd__chr01 else "" .
						end.
						else do:
							find first req_det 
									where req_domain = global_domain 
									and req_nbr = w_pod_req_nbr 
									and req_line = w_pod_req_line
							no-lock no-error .
							w_pod_so_job = if avail req_det then req__chr01 else "" .
						end .
						/*xp002*/
               assign
				  pod_so_job   = w_pod_so_job /*xp002*/
            ...
            {pxrun.i &PROC='getERSLineOption' &PROGRAM='aperxr.p'
                     ...
                     &NOAPPERROR=True &CATCHERROR=True}
			/*xp003*/
			find first in_mstr where in_domain = global_domain and in_site = pod_site and in_part = pod_part no-lock no-error .
