...
/* $MODIFIED BY: softspeed Roger Xiao         DATE: 2008/01/23  ECO: *xp001*  */
/*1.all function of this-is-rma = yes removed ,no ECOs for those delete */
/*2.输入成品和数量展一层bom,把子件新增进sod,成品不保存*/
...
/*xp001------------------------------------------------------------------------------------*/
define var v_site like sod_site label "地点".
define var v_part like pt_part .
define var v_qty_ord like sod_qty_ord .
define var v_um like sod_um .
define var v_um_conv like sod_um_conv initial 1 .
define var v_list_pr  like sod_list_pr .
define var choice  as logical initial yes .

define var v_price like sod_price .  /*for item price*/
define var v_pr_list like sod_pr_list  .  /*for item price*/
define            variable pics_type_key  like lngd_key1    no-undo. /*for item price*/
define            variable part_type_key  like lngd_key1    no-undo.  /*for item price*/
/* Set default types */
...
form
   line			column-label "行"
   v_part       label "部品/成品编号"
   v_qty_ord    label "   已订购量"         format "->>>>,>>9.9<<<<"
   v_um         label "UM"
   v_list_pr    label "   价目表价格"       format ">>>,>>>,>>9.99<<<"
   discount     label "    折扣"            format "->>>9.9<<<"
   v_price      label "          净价"

with frame x  
no-attr-space clines down 
width 80 title color normal frametitle.
...
repeat on endkey undo, leave:
   ...
   if not first_time then do:
		 hide frame c_site no-pause.   /*xp001*/
		 hide frame x no-pause.  /*xp001*/
   ...
   repeat with frame c down on endkey undo, next linefmt:
      ...
      {&SOSOMTA-P-TAG14}
      if not available sod_det then do:  /* if new line */
         if line = 0 then do:
            /* INVALID LINE NUMBER */
         ...
         if not new_order then reprice_dtl = yes.  /*This will cause line to be priced*/
...
do on error undo, retry:
		...
		clear frame d no-pause .
		 ...
		 display
			  ...
			  v_part v_um 
			  v_qty_ord discount		
			  v_list_pr when (not this-is-rma)
			  v_price   when (not this-is-rma)
		 with frame x.
         prompt-for v_part with frame x
         editing:
						...
						if not avail ps_mstr then do:
							...
							undo,retry .
							  ...
							  assign
								  ...
								  v_um = pt_um
								  v_um_conv = 1 
								  v_site = if so_site = "" then pt_site else so_site.
		...
		do on error undo, retry:
				pause 0 .
				disp v_site with frame c_site .
				set v_site with frame c_site .

				{gprun.i ""gpsiver.p""
				  "(input v_site, input ?, output return_int)"}
				if return_int = 0 then do:
					 {pxmsg.i &MSGNUM=725 &ERRORLEVEL=3}
					 next-prompt v_site with frame c_site.
					 undo setsite, retry.
				end. /* if return_int = 0 */
		...
		do on error undo, retry:
			...
			update v_qty_ord v_um with frame x.
			if v_qty_ord < 0 or v_qty_ord = 0 then do:
				message "订购数量无效,请重新输入" .
				undo,retry .
			end.
		   
		   find first pt_mstr where pt_domain = global_domain and pt_part = v_part no-lock no-error.
		   if available pt_mstr then do:
			  if v_um <> pt_um then do:
				   if pt_lot_ser = "s"  then do:
					  /* UM MUST EQUAL TO STOCKING UM FOR SERIAL-CONTROLLED  ITEM */
					  run p-pxmsg (input 367, input 3).
					  next-prompt v_um with frame x.
					  undo ,retry.
				   end. /*if pt_lot_ser = "s" */

				 {gprun.i ""gpumcnv.p"" "(input v_um,
										  input pt_um,
										  input v_part,
										  output v_um_conv)"}

				 if v_um_conv = ? then do:
					/* FREIGHT ERROR DETECTED - CHARGES MAY BE INCOMPLETE */
					run p-pxmsg (input 669, input 2).
					if not batchrun then pause.
					v_um_conv = 1.
				 end. /* IF L_UM_CONV = ? */
			  ...
			  v_qty_ord = v_qty_ord * v_um_conv . 
		end . /*setqty*/

	{pxmsg.i &MSGNUM     = 32 &ERRORLEVEL = 1 &CONFIRM = choice }
	if choice = no or choice = ? then do:
		undo loopcreate ,retry loopcreate .
	end.
	else do:  /*choice = yes */
			...
			no-lock :
			 ...
			 if available pt_mstr then do:
				assign
				   ptstatus = pt_status
				   substring(ptstatus,9,1) = "#".
				if not can-find(isd_det where isd_domain = global_domain
									and  isd_status = ptstatus
									and  isd_tr_type = "ADD-SO")
				then do:
					find first temp where t_part = ps_comp no-error.
					if not avail temp then do:
						create temp.
						assign 
							t_part = ps_comp
							t_qty_per = ps_qty_per
							t_um = pt_um 
							t_qty_ord = v_qty_ord * ps_qty_per .
					end.
					else do:
						assign t_qty_per = t_qty_per + ps_qty_per .
							   t_qty_ord = t_qty_ord + v_qty_ord * ps_qty_per .
					end.
				end.
				else do:
					/*如果某子件有效,却没产生sod,可查这里状态*/
				end.
			 end . /*if available pt_mstr*/
		...
		for each temp :
			 line = line + 1 .
				...
				for each pi_mstr
					...
					and  (pi_list = v_pr_list )
					and  (pi_cs_type = pics_type_key)
					and  (pi_cs_code = v_pr_list /* or pi_cs_code begins "qadall"*/)
					and  (pi_part_type = part_type_key)
					and  (pi_part_code = t_part /*or pi_part_code begins "qadall"*/)
					and  (pi_curr = so_curr )
					and  (pi_um = t_um )
					and  (pi_start  <= today  or pi_start = ?)
					and  (pi_expire >= today  or pi_expire = ?)
					no-lock 
					break by pi_start by pi_list_price : 
					
					if last-of (pi_list_price) then do:
						v_price = pi_list_price .
					end.
			 ...
			 assign
				...
				if so_req_date  <> so_due_date then   sod_req_date = so_req_date.
				if perform_date <> so_due_date then   sod_per_date = perform_date.
				if promise_date <> ?           then   sod_promise_date = promise_date.
				if soc_calc_promise_date = yes
					and soc_atp_enabled = yes
					and sod_confirm = no
					and sod_due_date = ?    then     sod_due_date  = today + soc_shp_lead.
			    ...
			    if sod_req_date = ? then
				 ...
				 if available pt_mstr then 
					assign  pl = pt_prod_line  desc1 = pt_desc1.

				 pt_recno = if available pt_mstr then recid(pt_mstr) else ? . 
				 sod_recno = recid(sod_det).
				 so_recno = recid(so_mstr) .
				 {gprun.i ""sosocom.p""}	 /* SET sod_comm_pct[] FROM SPD_DET IF AVAILABLE */
				 ...
				 if available pt_mstr then do:
						...
						run setTaxDefaults. /*sod_taxable ,sod_taxc, sod_prodline sod_um */
						{gprun.i ""soplsd.p""} /*from:sosomtlb.p : 
												根据pt_recno,so_recno,sod_recno,
												sod_prodline --> sod_acc,sod_sub等*/
			  ...
			  if using_cust_consignment and new_line then do:
				...
				view frame c .
				view frame d .
				display line sod_part sod_qty_ord sod_um sod_list_pr discount sod_price with frame c.
				display sod_qty_all sod_qty_pick sod_qty_ship sod_qty_inv sod_loc sod_site sod_serial sod_std_cost desc1
					base_curr sod_pricing_dt sod_req_date sod_per_date sod_due_date sod_promise_date sod_fr_list sod_acct
					sod_sub sod_cc sod_project sod_dsc_acct sod_dsc_sub sod_dsc_cc sod_dsc_project sod_confirm sod_type
					sod_um_conv sod_consume sod-detail-all sod_slspsn[1] mult_slspsn sod_comm_pct[1] sod_taxable sod_taxc
					(sod_cmtindx <> 0) @ sodcmmts sod_crt_int sod_fix_pr sod_order_category
				with frame d.
				 proc_id = "init".
				 {gprunmo.i
					&program=""socnsod.p""
			...
			outfile = "xxsomt_" + string(line) + ".prn".
			output to value(outfile ).
				put unformatted v_site skip .   /*site*/
				put unformatted " -  - " skip . /*qty ,um*/
				put unformatted " - " skip .    /*sod_pr_list*/
				put unformatted v_price .    /*sod_list_pr */
				put unformatted " - " skip . /*discount*/
				put unformatted " - " skip .   /*sod_price*/
			...
			if undo_all then  undo newloop, next newloop.
