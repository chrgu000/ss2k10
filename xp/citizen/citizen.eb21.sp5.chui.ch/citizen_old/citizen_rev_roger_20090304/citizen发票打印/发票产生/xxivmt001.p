...
define new shared variable shipfrom  like abs_shipfrom .  /*xp001*/
define new shared variable shipnbr  like so_nbr .  /*xp001*/ 
define new shared variable v_type   as char format "x(2)" label "发票类型".  /*xp001*/ 
...
{&SOSORP10-P-TAG23}
form
   shipfrom         colon 15 /*xp001*/
   shipnbr          colon 15 /*xp001*/
   v_type           colon 35 /*xp001*/
   skip(1)
   nbr              colon 15
...
*/ /*xp001*/
      ...
      if lookup(form_code,"1,11") = 0 then do:
         {&SOSORP10-P-TAG30} /* FORM CODE NOT INSTALLED */
      ...
      if not avail si_mstr then do:
        message "无效发货地点,请重新输入." .
        next-prompt shipfrom with frame a.
        undo,retry .
      end. /*xp001*/ 

      find first xdn_ctrl where xdn_domain = global_domain and xdn_ordertype = "IV" and xdn_type = v_type no-lock no-error .
      if not avail xdn_ctrl then do:
        message "无效发票类型,请重新输入." .
   ...
   do transaction on error undo, leave on endkey undo, leave:
      ...
      {gprun.i ""sorp10a.p"" }  /*确保每个域同时只有一个发票列印在执行,以避免soc_inv重号,用xdn_ctrl时需修改否? */  /*xp001*/
      {&SOSORP10-P-TAG7}

      /*find first soc_ctrl  where soc_ctrl.soc_domain = global_domain no-lock.
      next_inv_nbr = soc_inv - 1.   
      next_inv_pre = soc_inv_pre.  ***********xp001*/ /*xp001*/

      find first xdn_ctrl where xdn_domain = global_domain and xdn_ordertype = "IV" and xdn_type = v_type no-lock no-error .
      if avail xdn_ctrl then do:
          next_inv_nbr = inte(xdn_next) - 1 .   
          next_inv_pre = xdn_prev .
      end.   /*xp001*/
      ...
      {&SOSORP10-P-TAG36}
      {gprun.i " ""xxsorp10"" + run_file + "".p"" "}  /*xp001*/
      {&SOSORP10-P-TAG37}
      {&SOSORP10-P-TAG17}
      {&SOSORP10-P-TAG10}
      ...
      /* If the print run is accepted, update the control file ,更新下一发票单号*/
      /*{gprun.i ""sorp10b.p"" } */    /*xp001*/
      
      find first xdn_ctrl where xdn_domain = global_domain and xdn_ordertype = "IV" and xdn_type = v_type exclusive-lock .
      if avail xdn_ctrl then do:
          if next_inv_nbr >= inte(xdn_next) then
          xdn_next = string(next_inv_nbr + 1,"999999").
      end.   /*xp001*/
...
PROCEDURE p_mfquote:
   bcdparm = "".
   {&SOSORP10-P-TAG39}
   {mfquoter.i shipfrom} /*xp001*/
   {mfquoter.i shipnbr} /*xp001*/
   {mfquoter.i v_type} /*xp001*/
   {mfquoter.i nbr}
   {mfquoter.i nbr1}
   {mfquoter.i shipdate}
