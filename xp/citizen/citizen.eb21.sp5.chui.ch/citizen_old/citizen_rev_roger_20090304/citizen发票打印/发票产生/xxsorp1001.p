...
/* LAST MODIFIED: 2008/06/19   BY: Softspeed roger xiao   ECO: *xp001*      */  /*add 条件:同一张货运单的销售单产生同一张发票 , 发票号产生规则变化 */
/*-Revision end---------------------------------------------------------------*/
...
define shared variable shipfrom  like abs_shipfrom .  /*xp001*/
define shared variable shipnbr  like so_nbr .  /*xp001*/ 
define buffer absmstr for abs_mstr . /*xp001*/
   ...
   for first so_mstr
       where so_mstr.so_domain = global_domain
         ...
         and /*xp001*/ (can-find(first abs_mstr 
                        ...
                        and can-find(first absmstr 
                                    where absmstr.abs_domain = global_domain 
                                    and absmstr.abs_par_id   = "s" + shipnbr 
                                    and absmstr.abs_shipfrom = shipfrom
                                    and absmstr.abs_order    = so_nbr )
                        )
   no-lock: end.
