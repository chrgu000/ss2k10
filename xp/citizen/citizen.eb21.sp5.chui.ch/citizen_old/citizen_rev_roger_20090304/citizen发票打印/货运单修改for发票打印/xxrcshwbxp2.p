...
define var v_desc1 as char format "x(40)" .   /*xp002*/
define var v_desc2 as char format "x(40)" .  /*xp002*/
define var v_desc3 as char format "x(40)" .  /*xp002*/
define var v_desc4 as char format "x(40)" .  /*xp002*/
define var v_desc5 as char format "x(40)" .  /*xp002*/
define var v_date1 as date .  /*xp002*/
define var v_date2 as date .  /*xp002*/
define var v_inv_ok as logical . /*xp002*/

form 
abs__chr03  format "x(8)"    colon 15  label "贸易方式" 
    validate(can-find(code_mstr where code_domain  = global_domain and   code_fldname = "abs__chr03" and   code_value   =  abs__chr03 no-lock),
    "错误:无效贸易方式,请重新输入")
v_desc1 no-label 
abs__chr04  format "x(8)"    colon 15  label "起运港"  
    validate(can-find(code_mstr where code_domain  = global_domain and   code_fldname = "abs__chr04" and   code_value   =  abs__chr04 no-lock), 
    "错误:无效起运港,请重新输入")  
v_desc2 no-label 
abs__chr05  format "x(8)"    colon 15  label "目的港"  
    validate(can-find(code_mstr where code_domain  = global_domain and   code_fldname = "abs__chr05" and   code_value   =  abs__chr05 no-lock), 
    "错误:无效目的港,请重新输入")  
v_desc3 no-label 
abs__chr06  format "x(8)"    colon 15  label "香港仓"   
v_desc4 no-label 
v_date1                      colon 15  label "开船日期"
    validate(v_date1 <> ? ,"错误:无效日期")
v_date2                      colon 15  label "到达日期"
    validate(v_date2 <> ? ,"错误:无效日期")

skip space(1) "货柜号:" 
abs__chr07  format "x(36)"              no-label 
skip space(1) "货物描述:"
skip space(1) abs__chr08  format "x(60)" no-label 
skip space(1) "船名等:"   
skip space(1) abs__chr09  format "x(70)" no-label  
with frame inv centered row 8 overlay side-labels title color normal "发票数据" .  /*xp002*/
...
repeat with frame a:
   ...
   assign doc_type.
      ...
      if not avail abs_mstr then do:
          message "错误:货运单号不存在,请重新输入." .  /*xp002*/
          undo,retry .   /*xp002*/
      end.
      if avail abs_mstr and substring(abs_mstr.abs_status,2,1) <> "y" then do:
          message "错误:货运单未确认,请重新输入." .  /*xp002*/
          undo,retry .   /*xp002*/
      end.
   ...
   do transaction:
      ...
      if v_confirmed then do:
         ...
         /* run ip_msg (input 8146 , input 2).*/  /*xp002*/
      ...
      then do:
         ...
         run ip_msg (input 8103 , input 1).*/   /*xp002*/
      end.
...
repeat:
    v_inv_ok = yes .
    clear frame inv no-pause .

    v_date1 = if abs__dec03 <> 0 then date(integer(abs__dec03)) else ? .
    v_date2 = if abs__dec04 <> 0 then date(integer(abs__dec04)) else ? .
    assign v_desc1 = "" v_desc2 = "" v_desc3 = "" v_desc4 = "" .

    find first code_mstr 
            where code_domain  = global_domain 
            and   code_fldname = "abs__chr03"
            and   code_value   =  abs__chr03
    no-lock no-error .
    v_desc1 = if avail code_mstr then code_cmmt else "" .
    find first code_mstr 
            where code_domain  = global_domain 
            and   code_fldname = "abs__chr04"
            and   code_value   =  abs__chr04
    no-lock no-error .
    v_desc2 = if avail code_mstr then code_cmmt else "" .
    find first code_mstr 
            where code_domain  = global_domain 
            and   code_fldname = "abs__chr05"
            and   code_value   =  abs__chr05
    no-lock no-error .
    v_desc3 = if avail code_mstr then code_cmmt else "" .
    find first loc_mstr 
            where loc_domain  = global_domain 
            and   loc_loc     =  abs__chr06
    no-lock no-error .
    v_desc4 = if avail loc_mstr then loc_desc else "" .
    ...
    do on error undo,retry :
        ...
        update 
            ...
            v_date1  
            v_date2     
        with frame inv editing:
                      if frame-field="abs__chr03" then do:
                         {mfnp01.i code_mstr abs__chr03 code_value code_fldname "code_domain = global_domain and 'abs__chr03' " code_fldval}

                         if recno <> ? then do:
                            v_desc1 = code_cmmt .
                            display code_value @ abs__chr03 v_desc1 with frame inv.
                         end.
                      end.
                      else if frame-field="abs__chr04" then do:
                         {mfnp01.i code_mstr abs__chr04 code_value code_fldname "code_domain = global_domain and 'abs__chr04' " code_fldval}

                         if recno <> ? then do:
                            v_desc2 = code_cmmt .
                            display code_value @ abs__chr04 v_desc2 with frame inv.
                         end.
                      end.
                      else if frame-field="abs__chr05" then do:
                         {mfnp01.i code_mstr abs__chr05 code_value code_fldname "code_domain = global_domain and 'abs__chr05' " code_fldval}

                         if recno <> ? then do:
                            v_desc3 = code_cmmt .
                            display code_value @ abs__chr05 v_desc3 with frame inv.
                         end.
                      end.
                      else if frame-field="abs__chr06" then do:
                         {mfnp01.i loc_mstr abs__chr06 loc_loc global_domain loc_domain loc_loc}

                         if recno <> ? then do:
                            v_desc4 = loc_desc .
                            display loc_loc @ abs__chr06 v_desc4 with frame inv.
                         end.
                      end.
                      else do:
        ...
        if not avail code_mstr then do:
            message "错误:无效贸易方式,请重新输入" .
            next-prompt abs__chr03 with frame inv.
            undo,retry .
        end.
        ...
        v_desc2 = if avail code_mstr then code_cmmt else "" .
        if not avail code_mstr then do:
            message "错误:无效起运港,请重新输入" .
            next-prompt abs__chr04 with frame inv.
            undo,retry .
        end.
        ...
        v_desc3 = if avail code_mstr then code_cmmt else "" .
        if not avail code_mstr then do:
            message "错误:无效目的港,请重新输入" .
            next-prompt abs__chr05 with frame inv.
            undo,retry .
        end.
        if abs__chr06 <> "" then do:
            find first loc_mstr 
                    where loc_domain  = global_domain 
                    and   loc_loc     =  abs__chr06
            no-lock no-error .
            ...
            if not avail loc_mstr then do:
                message "错误:无效香港仓,请重新输入" .
                next-prompt abs__chr06 with frame inv.
                undo,retry .
            end.
        end. /*要么为空,要么受限*/
        ...
        if v_date1 = ? then do:
            message "错误:无效日期".
            next-prompt v_date1 with frame inv.
            undo,retry .
        end.
        if v_date2 = ? then do:
            message "错误:无效日期".
            next-prompt v_date2 with frame inv.
            undo,retry .
        end.
        
        abs__dec03 = inte(v_date1) .
        abs__dec04 = inte(v_date2) .
    ...
    if v_inv_ok = yes then leave .
end. /*invloop1:*/
hide frame inv no-pause .
/*xp002***********************************/
...
/*************************************xp002* 
   ...
   /*{gprun.i ""icshtrl.p"" "(abs_recid)"}
     {gprun.i ""icshprt.p"" "(abs_recid)" } */  /*xp002*/
