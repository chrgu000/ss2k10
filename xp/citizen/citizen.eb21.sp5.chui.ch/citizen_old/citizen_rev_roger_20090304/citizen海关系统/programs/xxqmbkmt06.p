...
define var v_form_type  like xximp_type initial "5" .
...
repeat:
    ...
    if avail xximp_mstr then do:
        if xximp_type <> v_form_type then do:
            message "错误:已存在,且非'成品返修'进口报关单,请重新输入" .
            undo,retry .
        end.
        ...
        do on error undo,retry :
            ...
            if not avail xximpd_det  then do:
                ...
                do on error undo,retry :
                    ...
                    find first xxccpt_mstr where xxccpt_domain = global_domain and xxccpt_ln = xximpd_cu_ln no-lock no-error .
                    if not avail xxccpt_mstr then do:
                        message "错误:无对应公司零件,请重新输入" .
                        undo,retry.
                    end.
                    ...
                    xximpd_cu_qty  = 0 .
                    xximpd_ctry    = if avail xxccpt_mstr then xxccpt_ctry else ""  .
                    xximpd_price   = xxcpt_price .
                    xximpd_amt     = xxcpt_price * xximpd_cu_qty .
