...
define var v_form_type  like xxexp_type initial "5" .
...
form
    ...
    xxexp_bk_nbr       label "进口报关单"  colon 13 xxexp_crt_date     label "维护日期"    colon 45  
...
repeat:
    ...
    do on error undo,retry :
        ...
        if not avail xxexp_mstr then do:
            ...
            find first xximp_mstr where xximp_domain = global_domain and xximp_type = v_form_type and xximp_stat = "" no-lock no-error .
            ...
            assign 
                ...
                xxexp_bk_nbr     = if avail xximp_mstr then xximp_nbr else ""                
                xxexp_amt        = 0
                xxexp_curr       = "USD"
            ...
            do on error undo,retry :
                update xxexp_bk_nbr with frame b editing:
                    if frame-field="xxexp_bk_nbr" then do:
                        {mfnp01.i xximp_mstr xxexp_bk_nbr xximp_nbr v_form_type "xximp_domain = global_domain and xximp_type" xximp_type }
                        if recno <> ? then do:
                            xxexp_bk_nbr = xximp_nbr .
                            disp 
                                xxexp_bk_nbr  
                            with frame b.
                        end.
                ...
                find first xximp_mstr 
                        ...
                        and xximp_type = v_form_type 
                        and xximp_nbr  = xxexp_bk_nbr
                no-lock no-error .
                if not avail xximp_mstr then do:
                    ...
                    find first xximpd_det 
                            ...
                            and xximpd_type     = v_form_type 
                            and xximpd_nbr      = xxexp_bk_nbr
                            and (xximpd_stat = "" )
                    no-lock no-error .
                    if not avail xximpd_det then do:
                        message "错误:进口报关单,无未结项次" .
                        undo,retry .
                    end.
            ...
            if not avail xxexpd_det  then do:
                ...
                do on error undo,retry :
                    ...
                    find first xximpd_det 
                            ...
                            and xximpd_type     = v_form_type 
                            and xximpd_nbr      = xxexp_bk_nbr
                            and xximpd_cu_ln    = xxexpd_cu_ln 
                            and (xximpd_stat = "" )
                    no-error .
                    if not avail xximpd_det then do:
                        message "错误:进口报关单无此商品的未结项次" .
                        undo,retry .
                    end.
                    else do:
                        xxexpd_cu_ln   = xxcpt_ln .
                        ...
                        xxexpd_cu_qty  = xximpd_cu_qty - xximpd_used .
                        xxexpd_ctry    = xximpd_ctry  .
                        xxexpd_price   = xximpd_price .
                ...
                do on error undo, retry:
                    ...
                    if xxexpd_cu_qty > (xximpd_cu_qty - xximpd_used  )  then do:
                        message "错误:报关数不可超过进口报关余数" .
                        undo,retry .
                    end.
                    xxexpd_amt     = xxexpd_price * xxexpd_cu_qty .
                    ...
                    if avail xximpd_det then do:
                        xximpd_used  = xximpd_used +  xxexpd_cu_qty .
                        if xximpd_used >=  xxexpd_cu_qty then xximpd_stat = "C" .
                        ...
                        undo,retry.
                            ...
                            if del-yn then do:
                                find first xximpd_det 
                                        ...
                                        and xximpd_cu_ln    = xxexpd_cu_ln 
                                no-error .
                                if avail xximpd_det then assign xximpd_used = xximpd_used -  xxexpd_cu_qty xximpd_stat = "" .
                                ...
                                find first xximp_mstr 
                                        ...
                                        and xximp_nbr      = xxexp_bk_nbr
                                no-error .
                                if avail xximp_mstr then xximp_stat = "" .
