...
define new shared variable prod_site  like lad_site.
...
repeat:
   ...
   do on error undo, retry with frame a:
      ...
      with frame a editing:
         ...
         then do:
            ...
            {mfnp01.i loc_mstr
                      loc-to
                      loc_loc
                      "global_domain and loc_site = input prod_site "
                      loc_domain 
                      loc_loc
                      }
      ...
      find first xxipr_mstr where xxipr_domain = global_domain and xxipr_nbr = nbr no-error .
      if not avail xxipr_mstr then do:
          message "错误:申报单不存在,请重新输入" .
          next-prompt nbr .
          undo,retry.
      end.
     ...
     for each xxiprd_det where xxiprd_domain = global_domain and xxiprd_nbr = nbr and xxiprd_rct_qty > xxiprd_used no-lock ,
         each tr_hist no-lock where tr_hist.tr_domain = global_domain and tr_nbr = xxiprd_rct_order and tr_lot  = xxiprd_rct_nbr
             and tr_line = xxiprd_rct_ln and tr_type = "rct-po" and tr__log01 = no and tr_qty_loc > tr__dec01 use-index tr_nbr_eff:
        transok = yes.
        ponbr = xxiprd_rct_order.
        create trhist1.
        assign tr1_domain = tr_domain
            ...
            tr1_ref    = tr_ref
            tr1_loc    = tr_loc
            tr1_chg    = xxiprd_rct_qty - xxiprd_used /*tr_qty_loc - tr__dec01*/.
     ...
     end. /*for each xxiprd_det*/


/*****************************
      for each  prh_hist where prh_hist.prh_domain = global_domain and prh_receiver = nbr and prh__log01 = no no-lock 
       use-index prh_receiver ,
       each tr_hist no-lock where tr_hist.tr_domain = global_domain and tr_nbr = prh_nbr and tr_lot  = prh_receiver
       and tr_line = prh_line
        and tr_type = "rct-po" and tr__log01 = no and tr_qty_loc > tr__dec01 use-index tr_nbr_eff:
      transok = yes.
      ponbr = prh_nbr.
      create trhist1.
      assign tr1_domain = tr_domain
   ...
   ********************************/   
      ...
      if loc-to <> "" then do:
         for first loc_mstr
            fields( loc_domain loc_loc)
             where loc_mstr.loc_domain = global_domain and loc_site = prod_site and  loc_loc = loc-to
            no-lock:
   ...
   location1 = loc-to .
   {gprun.i ""xxqminmt11b.p"" "(nbr, prod_site, loc-to, picked, ponbr)"}
...
for each trhist1 no-lock where tr1_chg <> 0 :
    ...
    for each xxiprd_det 
        ...
        and xxiprd_nbr = nbr 
        and xxiprd_line = tr1_line
        and xxiprd_rct_nbr = tr1_lot :
        xxiprd_used = xxiprd_used + tr1_chg .
        /*if xxiprd_used >= xxiprd_rct_qty then xxiprd_stat = "C" .*/
    end.
end.
/*
