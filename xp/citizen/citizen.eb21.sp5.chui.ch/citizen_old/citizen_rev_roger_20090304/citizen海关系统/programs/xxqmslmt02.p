...
repeat with frame h1 :
    ...
    prompt-for v_nbr with frame h1 editing:
         if frame-field = "v_nbr" then do:    
             ...
             if recno <> ? then do:
                v_nbr = xximp_nbr .
                disp  v_nbr   xximp_rct_date xximp_req_date xximp_sl_nbr  xximp_bk_nbr  with frame h1 .
    ...
    if avail xximp_mstr then do:
        if xximp_type <> v_form_type then do:
            message "错误:已存在,且非'转厂'进口报关单,请重新输入" .
            undo,retry .
        end.
        v_add = no .
        v_all = no .
        disp v_nbr v_add v_all xximp_rct_date xximp_req_date   xximp_sl_nbr  xximp_bk_nbr  with frame h1 .
        disp xximp_cu_nbr    xximp_pre_nbr xximp_dept  xximp_ship_via  xximp_ship_tool xximp_bl_nbr xximp_trade_mtd xximp_tax_mtd   
            xximp_tax_rate  xximp_license   xximp_appr_nbr  xximp_contract  xximp_container   xximp_from     
            xximp_to xximp_port  xximp_fob xximp_box_num xximp_box_type xximp_net xximp_gross xximp_curr xximp_amt xximp_stat xximp_use xximp_rmks1 xximp_rmks2 
        with frame h2 .
    ...
    do on error undo , retry:
        ...
        if not avail xximp_mstr then do:  
            ...
            assign  xximp_domain = global_Domain 
                    ...
                    xximp_sl_nbr     = if avail xxsl_mstr  then xxsl_nbr else ""   
        ...
        go-on ("F5" "CTRL-D") with frame h1 editing :
                if frame-field = "xximp_sl_nbr" then do:
                    {mfnp01.i xxsl_mstr xximp_sl_nbr xxsl_nbr "global_domain and xxsl_stat = '' " xxsl_domain xxsl_nbr }
                    if recno <> ? then do:
                        display  xxsl_nbr @ xximp_sl_nbr with frame h1.
                    end.
                end.
                else if frame-field = "xximp_bk_nbr" then do:
        ...
        find first xxsl_mstr 
                ...
                and xxsl_type     = v_sl_type 
                and xxsl_stat     = ""
                and (xxsl_expire  = ? or xxsl_expire >= xximp_rct_date )
        no-lock no-error .
        if not avail xxsl_mstr then do:
            message "错误:关封号无效或已结,请重新输入" .
            next-prompt xximp_sl_nbr .
            undo,retry .
        end.
        else do:
            find first xxsld_det 
                    where xxsld_domain = global_domain 
                    and xxsld_nbr      = xxsl_nbr 
                    and xxsld_bk_to    = xximp_bk_nbr 
                    and xxsld_stat     = "" 
                    and xxsld_qty_ord >= xxsld_qty_used 
            no-lock no-error .
            if not avail xxsld_det then do:
                message "错误:关封内无此手册的未结项次,请重新输入" .
                next-prompt xximp_bk_nbr .
                undo,retry .
            end.
        end. /*if avail xxsl_mstr*/

        find first xxsl_mstr where xxsl_domain = global_domain and xxsl_nbr = xximp_sl_nbr no-lock no-error .
        v_vend = if avail xxsl_mstr then xxsl_addr_from else "" .        
    end. /*  loopa: */
        ...
        for each prh_hist use-index prh_rcp_date 
            ...
            if not avail xxccpt_mstr then do:
                ...
                if not avail xxcpt_mstr then do:
                    ...
                    no-lock no-error .
                    if not avail xxsld_det then do:
                        message "警告:关封无未结零件" prh_part ".收货单号:" prh_receiver .
                        v_error = "*ERROR*" .
                    end.
                        ...
                        if round(t_rcvd  * round(t_um_conv1 * t_um_conv2,v_round2 + 2 ) , v_round2 ) > 
                           ( xxcbkd_qty_ord - xxcbkd_qty_tsf - xxcbkd_qty_sl - xxcbkd_qty_io  + xxcbkd_qty_rjct ) then do:
                            ...
                            if not avail xximpd_det then do:
                                ...
                                xxcbkd_qty_sl   = xxcbkd_qty_sl + xximpd_cu_qty .     /*扣减手册数量*/
                                xxsld_qty_used  = xxsld_qty_used + xximpd_cu_qty .     /*扣减关封数量*/
