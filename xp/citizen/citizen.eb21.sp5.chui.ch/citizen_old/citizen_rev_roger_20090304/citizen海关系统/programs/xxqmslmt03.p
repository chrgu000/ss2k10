...
repeat with frame h1 :
    ...
    prompt-for v_nbr with frame h1 editing:
         if frame-field = "v_nbr" then do:    
             ...
             if recno <> ? then do:
                v_nbr = xxexp_nbr .
                disp  v_nbr   xxexp_iss_date xxexp_req_date xxexp_sl_nbr xxexp_bk_nbr  with frame h1 .
    ...
    if avail xxexp_mstr then do:
        if xxexp_type <> v_form_type then do:
            message "错误:已存在,且非'转厂'出口报关单,请重新输入" .
            undo,retry .
        end.
        v_add = no .
        v_all = no .
    ...
    do on error undo , retry:
        ...
        if not avail xxexp_mstr then do:            
            find first xxsl_mstr 
                    ...
                    and xxsl_type     = v_sl_type 
                    and xxsl_stat     = "" 
                    and (xxsl_expire  = ? or xxsl_expire >= today )
            no-lock no-error .
            if avail xxsl_mstr then do:
                find first xxsld_det 
                        ...
                        and xxsld_stat     = "" 
                        and xxsld_qty_ord >= xxsld_qty_used 
                no-lock no-error .
                if avail xxsld_det then do:
                    find first xxcbk_mstr 
                                    where xxcbk_domain = global_domain
                                    and xxcbk_bk_nbr   = xxsld_bk_from
                                    and xxcbk_stat     = "" 
                    no-lock no-error . 
                end.
            end. /*if avail xxsl_mstr*/
            ...
            assign 
                ...
                xxexp_sl_nbr     = if avail xxsld_det then xxsld_nbr else "" 
        ...
        disp v_nbr  v_add v_all xxexp_iss_date xxexp_req_date xxexp_sl_nbr xxexp_bk_nbr  with frame h1 .

        v_recid = recid(xxexp_mstr) .
        update 
            xxexp_iss_date 
            xxexp_req_date 
            xxexp_sl_nbr when ( new_add )
            xxexp_bk_nbr when ( new_add )
            v_add when ( new_add )
            v_all when ( new_add ) 
        go-on ("F5" "CTRL-D") with frame h1 editing :
                if frame-field = "xxexp_sl_nbr" then do:
                    {mfnp01.i xxsl_mstr xxexp_sl_nbr xxsl_nbr "global_domain and xxsl_stat = '' " xxsl_domain xxsl_nbr }
                    if recno <> ? then do:
                        display  xxsl_nbr @ xxexp_sl_nbr with frame h1.
                    end.
                end.
                else if frame-field = "xxexp_bk_nbr" then do:
                    ...
                    if ( lastkey = keycode("F5") or lastkey = keycode("CTRL-D") ) then do:
                        if xxexp_stat <> "" then do:
                            ...
                            if avail xxexpd_det then do :
                                message "错误:明细非未结状态或已开始转厂,不允许删除" . 
                                undo,retry .
                            end.
                            else do:
                                del-yn = yes.
        ...
        find first xxsl_mstr 
                where xxsl_domain = global_domain 
                and xxsl_nbr      = xxexp_sl_nbr 
                and xxsl_type     = v_sl_type 
                and xxsl_stat     = ""
                and (xxsl_expire  = ? or xxsl_expire >= xxexp_iss_date )
        no-lock no-error .
        if not avail xxsl_mstr then do:
            message "错误:关封号无效或已结,请重新输入" .
            next-prompt xxexp_sl_nbr .
            undo,retry .
        end.
        else do:
            find first xxsld_det 
                    where xxsld_domain = global_domain 
                    and xxsld_nbr      = xxsl_nbr 
                    and xxsld_bk_from  = xxexp_bk_nbr 
                    and xxsld_stat     = "" 
                    and xxsld_qty_ord >= xxsld_qty_used 
            no-lock no-error .
            if not avail xxsld_det then do:
                message "错误:关封内无此手册的未结项次,请重新输入" .
                next-prompt xxexp_bk_nbr .
                undo,retry .
            end.
        end. /*if avail xxsl_mstr*/

        find first xxsl_mstr where xxsl_domain = global_domain and xxsl_nbr = xxexp_sl_nbr no-lock no-error .
        v_vend = if avail xxsl_mstr then xxsl_addr_to else "" .   
    ...
    if v_add then do:
        ...
        for each abs_mstr use-index abs_par_id 
            ...
            and abs_shipto   = v_vend /*限关封对应的供应商*/
            and abs_type     = "S"
            /*and abs__chr02   = "CHA" */
            no-lock :
            ...
            if not avail xxccpt_mstr then do:
                ...
                if not avail xxcpt_mstr then do:
                    ...
                    find first xxsld_det
                            ...
                            and xxsld_nbr =  xxexp_sl_nbr 
                            and xxsld_bk_from  = xxexp_bk_nbr
                            and xxsld_cu_ln  = xxcpt_ln
                            and xxsld_stat   = ""
                            and xxsld_qty_ord > xxsld_qty_used 
                    no-lock no-error .
                    if not avail xxsld_det then do:
                        message "警告:关封无未结零件" abs_item ".货运单号:" v_abs_id  .
                        v_error = "*ERROR*" .
                    end.
                    else do:
                        if round((abs_qty - abs__dec02) * round(integer(abs__qad03) * xxccpt_um_conv,v_round2 + 2 ) , v_round2 ) > 
                           ( xxsld_qty_ord - xxsld_qty_used ) then do:
                            message  "警告:关封余数不足,零件" abs_item ",商品序" xxccpt_ln ".货运单号:" v_abs_id . 
                            v_error = "*ERROR*" .
                        end.
                    end.
        ...
        if info_correct then do:
             ...
             for each temp where t_yn = yes :
                    ...
                    no-lock no-error .
                    if not avail xxsld_det then do:
                        message "警告:关封无未结零件" t_part ".货运单号:" t_nbr  .
                        next .
                    end.
                    else do:
                        if round(t_rcvd  * round(t_um_conv1 * t_um_conv2,v_round2 + 2 ) , v_round2 ) > 
                           ( xxsld_qty_ord - xxsld_qty_used ) then do:
                            message  "警告:关封余数不足:" t_cu_ln "零件" t_part ".货运单号:" t_nbr .
                            next.
                        end.
                    end.
                        ...
                        if round(t_rcvd  * round(t_um_conv1 * t_um_conv2,v_round2 + 2 ) , v_round2 ) > 
                           ( xxcbkd_qty_ord + xxcbkd_qty_tsf - xxcbkd_qty_sl - xxcbkd_qty_io  + xxcbkd_qty_rjct ) then do:
                            ...
                            if not avail xxexpd_det then do:
                                ...
                                xxcbkd_qty_sl   = xxcbkd_qty_sl + xxexpd_cu_qty .     /*扣减手册数量*/
