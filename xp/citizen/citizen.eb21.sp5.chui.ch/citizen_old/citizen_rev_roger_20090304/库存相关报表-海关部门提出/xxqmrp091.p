/* xxqmrp091.p 材料库存报表,进口数量,工单发料数,材料出货数          */
...
1.地点(上线前):1100-->1100,1300,1700 ;1200--> 1200,1400 .
  香港库位v_loclist 
2.发生产线数:所有iss-wo
3.出货数量: 所有(工厂库位的)iss-so 
4.进口数量: 非1700地点 :直接收货到工厂的, 收货到香港再转仓到工厂的 ; 1700 : 指定供应商的所有收货
*/
...
for each tr_hist 
    ...
    if (tr_type = "RCT-PO" or tr_type = "iss-so") 
    and( (tr_site = "1700" and  lookup(tr_loc , v_loclist) > 0 ) 
    or lookup(tr_loc , v_loclist) = 0 ) then do:
        find first xxptstkrp02 
...
/*tr_type end*/
    ...
    for each tta6ictrrp0303 
        field(tta6ictrrp0303_part tta6ictrrp0303_type tta6ictrrp0303_loc tta6ictrrp0303_nbr
              tta6ictrrp0303_trnbr tta6ictrrp0303_qty_dr tta6ictrrp0303_amt_dr tta6ictrrp0303_program )
        where tta6ictrrp0303_site <> "1700" 
        and lookup(tta6ictrrp0303_site,v_sitelist) > 0
        and lookup(tta6ictrrp0303_loc,v_loclist) > 0
        and tta6ictrrp0303_type = "rct-po" 
        break by tta6ictrrp0303_type by tta6ictrrp0303_trnbr :
                for each tr_hist 
                    ...
                    and tr_type = "RCT-TR"
                    and tr_effdate >= tta6ictrrp0303_effdate
                    and tr_part   = tta6ictrrp0303_part
                    and tr_nbr    = tta6ictrrp0303_lot
                    ...
                    find first tt where tt_trtype = "RCT-PO" no-error.
                    if avail tt then do:
                        assign xxptstkrp02_rctiss_qty[tt_integer] = xxptstkrp02_rctiss_qty[tt_integer] + tr_qty_loc .
                    end.
	...
	for each xxptstkrp02 break by xxptstkrp02_site by xxptstkrp02_loc  by xxptstkrp02_part :
        ...
        if xxptstkrp02_site = "1700" then do:
                for each tta6ictrrp0303 
                    field(tta6ictrrp0303_part tta6ictrrp0303_type tta6ictrrp0303_loc tta6ictrrp0303_nbr
                          tta6ictrrp0303_trnbr tta6ictrrp0303_qty_dr tta6ictrrp0303_amt_dr tta6ictrrp0303_program )
                    ...
                    and (tta6ictrrp0303_type = "rct-po" or tta6ictrrp0303_type = "iss-prv" )
                    break by tta6ictrrp0303_type by tta6ictrrp0303_trnbr :
                    find first tr_hist where tr_domain = global_domain and tr_trnbr = tta6ictrrp0303_trnbr no-lock no-error .
                    if avail tr_hist then do:
                        if (lookup(tr_addr ,v_vendlist) > 0 or (tr_addr = "93P007MH" and tr_part = "C6009-200#")) then do:
                            find first tt where tt_trtype = "RCT-PO" no-error.
                            if avail tt then do:
                ...
                for each tta6ictrrp0303 
                    ...
                    find first prh_hist where prh_domain = global_domain and prh_receiver = tta6ictrrp0303_lot and prh_line = tta6ictrrp0303_line no-lock no-error .
                    if avail prh_hist then do:
                        if prh__chr01 = "CHA" then do:
                            find first tt where tt_trtype = "RCT-PO" no-error.
                            if avail tt then do:
                                assign xxptstkrp02_rctiss_qty[tt_integer] = xxptstkrp02_rctiss_qty[tt_integer] + tta6ictrrp0303_qty_dr .
                                assign xxptstkrp02_rctiss_amt[tt_integer] = xxptstkrp02_rctiss_amt[tt_integer] + tta6ictrrp0303_amt_dr .
                            ...
                            if avail tr_hist then do:
                                find first tt where tt_trtype = "RCT-PO" no-error.
                                if avail tt then do:
                                    assign xxptstkrp02_rctiss_qty[tt_integer] = xxptstkrp02_rctiss_qty[tt_integer] + min(tta6ictrrp0303_qty_dr, tr__dec01) .
                                    assign xxptstkrp02_rctiss_amt[tt_integer] = xxptstkrp02_rctiss_amt[tt_integer] + min(tta6ictrrp0303_qty_dr, tr__dec01)  .
                                end.
                            end.                            
                        end.
                    end.
		...
		for each tta6ictrrp0303 field(tta6ictrrp0303_part tta6ictrrp0303_type tta6ictrrp0303_loc tta6ictrrp0303_nbr
    			...
    			if not avail tt then next .
                    ...
                    if xxptstkrp02_site = "1700" and tta6ictrrp0303_type = "ISS-SO" then do:
                        vvv = "" .
                        find first ptp_det where ptp_domain = global_domain and ptp_part = tta6ictrrp0303_part and ptp_site = xxptstkrp02_site  no-lock no-error  .
                        if avail ptp_det then do:
                            vvv = ptp_vend .
                        end.
                        else do:
                            find first pt_mstr where pt_domain = global_domain and pt_part =  tta6ictrrp0303_part  no-lock no-error  .
                            if avail pt_mstr then do:
                                vvv = pt_vend .
                            end.          
                        end. 
                        if lookup(vvv,v_vendlist) = 0 or (tta6ictrrp0303_part = "C6009-200#" and vvv <> "93P007MH") then next .
                    end.
...
for each temp99 : delete temp99 . end.
	...
	put unformatted "#def reportpath=$/Citizen/xxqmrp091" skip.
...
*/
	for each xxptstkrp02 ,
        ...
        if v_log1 = yes or v_log2 = yes then do:
            ...
            find first temp99 where t99_part = xxptstkrp02_part	no-error .
            if not avail temp99 then do:
                create temp99 .
                assign 
                    t99_part     = xxptstkrp02_part
                    t99_desc1    = pt_desc1
                    t99_desc2    = pt_desc2
                    t99_um       = pt_um
                    t99_start_qty_oh  = xxptstkrp02_start_qty_oh  
                    t99_end_qty_oh = xxptstkrp02_end_qty_oh		
                    .
                    if maxii >= 1 then do:
                        do ii = 1 to maxii:
                            t99_rctiss_qty[ii] =  xxptstkrp02_rctiss_qty[ii]	 .                               
                        end.   
                    end.
            end.
            else do:
                    assign 
                    t99_start_qty_oh  = t99_start_qty_oh + xxptstkrp02_start_qty_oh  
                    t99_end_qty_oh = t99_end_qty_oh + xxptstkrp02_end_qty_oh		
                    .
                    if maxii >= 1 then do:
                        do ii = 1 to maxii:
                            t99_rctiss_qty[ii] = t99_rctiss_qty[ii] + xxptstkrp02_rctiss_qty[ii] .                               
                        end.   
                    end.                
            end.
