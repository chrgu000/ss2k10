	...
	***** Remark By:  SS - 20061103.1 End */
    ...
    for each tr_hist FIELD( tr_rmks tr_nbr tr_line tr_site tr_prod_line tr_part tr_trnbr tr_addr tr_lot tr_effdate
         ...
         and (
              ...
              )  )NO-LOCK  BREAK BY tr_type BY tr_part :
	...
	PUT UNFORMATTED "#def REPORTPATH=$/Citizen/xxqmrp099" SKIP.
	...
	FOR EACH xxptstkrp02 ,
        ...
        v_rct_integer = 0.
        v_iss_integer = 0.
        FOR EACH tttr WHERE tttr_part = xxptstkrp02_part 
                        AND tttr_rct_loc <> ""  AND tttr_iss_loc <> "" 
                        AND tttr_rct_loc >= loc AND tttr_rct_loc <= loc1 
                        AND tttr_iss_loc >= loc AND tttr_iss_loc <= loc1 :
