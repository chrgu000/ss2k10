...
&SCOPED-DEFINE arcsrp2a_p_39 "区域国家       "
/* MaxLen: Comment: */
...
/*L02Q*   {wbrp02.i} */
          ...
          define shared variable age_days as integer extent 8
             label {&arcsrp2a_p_1}.
          ...
          define variable age_range as character extent 8 format "X(16)".
          ...
          define variable age_paid like ar_amt extent 8.
          define variable sum_amt like ar_amt extent 8.
...
/*L00S*BEGIN ADDED SECTION*/
          {etrpvar.i }
          {etvar.i   }
          define variable et_age_amt          like ar_amt extent 8.
          define variable et_age_paid         like ar_amt extent 8.
          define variable et_sum_amt          like ar_amt extent 8.
          ...
          define variable et_org_sum_amt      like ar_amt extent 8.
          ...
          define variable country like ad_country.
/*L00S*END ADDED SECTION*/
/* SS 090824.1 - B */
					define shared var sub like sb_sub.
					define shared var sub1 like sb_sub.					
/* SS 090824.1 - E */

/* SS 090923.1 - B */
					define variable et_age_totamt          like ar_amt extent 9 format "->>,>>>,>>9.99".
					et_age_totamt[1] = 0.
					et_age_totamt[2] = 0.
					et_age_totamt[3] = 0.
					et_age_totamt[4] = 0.	
...
/* SS 090923.1 - E */
          ...
          find first gl_ctrl where gl_domain = global_domain  no-lock. /*rounding after currency conversion*/

          /* CREATE REPORT HEADER */
      /*    assign
            age_range[1] = {&arcsrp2a_p_13} + string(age_days[i - 1],"->>>9").
          end.
          age_range[1] = " 0--30天"{&arcsrp2a_p_15} + string(age_days[1],"->>>9").*/
        ...
        /*  do with frame c down no-box:*/
             ...
             for each cm_mstr where cm_domain = global_domain and 
...
/* SS 090824.1 - B */
										and (ar_type = "P" or (ar_sub >= sub and ar_sub <= sub1))
/* SS 090824.1 - E */
                     ...
                     /* Store first customer information in a logical, since the
                      ...
                      if (acct_type <> "" or acct_type1 <> hi_char) or
                         ...
                         if ar_type <> "P" then do:
                            ...
                            u_amt = 0.
                            u_applied = 0.
                            for each ard_det where ard_domain = global_domain and ard_nbr = ar_nbr
...
/* SS 090824.1 - B */
																	and ard_sub >= sub and ard_sub <= sub1				
/* SS 090824.1 - E */
                                  then do:
                                  use_rec = true.
                                  u_amt = u_amt - /* + */ ard_amt.
                            ...
                            for each armstr where armstr.ar_domain = global_domain and 
                              ...
                              for each ard_det where ard_domain = global_domain and ard_nbr = armstr.ar_nbr
                                 no-lock:
...
/*L00S*/                    et_age_amt[1 for 4]
                         ...
                         form 
                            country no-label 
                            name no-label 
                            et_sum_amt[1 for 8] no-label
                            ar_amt  no-label 
                             with stream-io frame d width 256.   
...
/*L00S*/                    et_sum_amt[i] = 0.
                         ...
                         if new_cust then do:
                            ...
                            rec_printed = true.
                           name = "".
                            find ad_mstr where ad_domain = global_domain and ad_addr = ar_bill no-lock no-wait
                               no-error.
...
/* SS 090923.1 - B */
														et_age_totamt[1] = et_age_totamt[1] + et_age_amt[1].
														et_age_totamt[2] = et_age_totamt[2] + et_age_amt[2].
														et_age_totamt[3] = et_age_totamt[3] + et_age_amt[3].
														et_age_totamt[4] = et_age_totamt[4] + et_age_amt[4].
														et_age_totamt[5] = et_age_totamt[5] + et_age_amt[5].
														et_age_totamt[6] = et_age_totamt[6] + et_age_amt[6].
														et_age_totamt[7] = et_age_totamt[7] + et_age_amt[7].
														et_age_totamt[8] = et_age_totamt[8] + et_age_amt[8].
/* SS 090923.1 - E */
...
/*L00S*BEGIN ADD
 ...
 /*L00S*END ADD*/
                                  ...
                                  no-lock with frame c:
                                     find payment where payment.ar_domain = global_domain and payment.ar_nbr = ard_nbr
                                        no-lock no-error.
                   ...
                   if last-of(ar_bill) and rec_printed then do with frame d: 
                         rec_printed = false.
                         if summary_only = no then do:                           
                            displ country format "x(12)" no-label
                                  name   format "x(24)" no-label                   
/*L00S*/                        accumulate total by ar_bill (et_sum_amt[1]) @ et_sum_amt[1] format "->>,>>>,>>9.99" no-label
                                accumulate total by ar_bill (et_sum_amt[2]) @ et_sum_amt[2] format "->>,>>>,>>9.99" no-label
                                accumulate total by ar_bill (et_sum_amt[3]) @ et_sum_amt[3] format "->>,>>>,>>9.99" no-label
                                 accumulate total by ar_bill (et_sum_amt[4]) @ et_sum_amt[4] format "->>,>>>,>>9.99" no-label
                                 accumulate total by ar_bill (et_sum_amt[5]) @ et_sum_amt[5] format "->>,>>>,>>9.99" no-label
                                 accumulate total by ar_bill (et_sum_amt[6]) @ et_sum_amt[6] format "->>,>>>,>>9.99" no-label
                                 accumulate total by ar_bill (et_sum_amt[7]) @ et_sum_amt[7] format "->>,>>>,>>9.99" no-label
                                 accumulate total by ar_bill (et_sum_amt[8]) @ et_sum_amt[8]  format "->>,>>>,>>9.99" no-label                         
                                accum total by ar_bill (et_base_amt - et_base_applied)  @ ar_amt format "->>>>,>>>,>>9.99" no-label with frame d width 256 stream-io.
                         end.
...
/*L059*/          displ  {&arcsrp2a_p_41} @ country
/*L059*/               {&arcsrp2a_p_40} @ name 
...
/* SS 090923.1 - B */
						et_age_totamt[9] = et_age_totamt[1] + et_age_totamt[2] + et_age_totamt[3] + et_age_totamt[4] +
						                   et_age_totamt[5] + et_age_totamt[6] + et_age_totamt[7] + et_age_totamt[8].
						
						put    skip(1).
						put   	"合计" at 1
						        et_age_totamt[1] at 39 " " 
										et_age_totamt[2] " "
								    et_age_totamt[3] " "
										et_age_totamt[4] " "
										et_age_totamt[5] " "
