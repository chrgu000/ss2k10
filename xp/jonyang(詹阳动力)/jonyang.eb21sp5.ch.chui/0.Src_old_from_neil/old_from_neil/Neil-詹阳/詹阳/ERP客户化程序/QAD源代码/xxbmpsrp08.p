...
/*IFP*/  define variable qty_zlc like qty_oh column-label "短缺量".
/*IFP*/  define variable qty_avail like in_qty_avail column-label "可用量".
...
repeat:
            ...
            update part pline pline1 nbr site eff_date maxlevel assy_only with frame a.
...
/*IFP*/     find first wo_mstr where 
				wo_domain = global_domain and /*---Add by davild 20090205.1*/
				wo_part = part and wo_nbr = nbr no-lock no-error.
...
if not can-find(first wo_mstr  where wo_domain = global_domain and /*---Add by davild 20090205.1*/	
	wo_nbr = nbr and wo_part = part) then do:
...
/*GN61*/    end.
/*GN61*/    else do:
/*GN61*/       find wo_mstr where wo_domain = global_domain and /*---Add by davild 20090205.1*/
...
if not can-find(si_mstr  where si_domain = global_domain and /*---Add by davild 20090205.1*/
...
/*GN61*/    end.
/*GN61*/    else do:
/*GN61*/       find si_mstr where si_domain = global_domain and /*---Add by davild 20090205.1*/	
si_site = site  no-lock no-error.
/*GN61*/       if available si_mstr then display si_desc with frame a.
/*GN61*/    end.
end.
...
find si_mstr where si_domain = global_domain and /*---Add by davild 20090205.1*/
si_site = site  no-lock no-error.
...
/*IFP*/                 qty_zlc = 0.                        
/*IFP*/                 qty_avail = 0.
                        for each in_mstr no-lock where 
						in_domain = global_domain and /*---Add by davild 20090205.1*/
						in_part = ptmstr.pt_part
                        and in_site = site:
...
*****/
                           WITH STREAM-IO /*GUI*/ .
/*IFP*/                 for each wo_mstr where 
							wo_domain = global_domain and /*---Add by davild 20090205.1*/
							wo_part = pk_part and wo_nbr = nbr 
