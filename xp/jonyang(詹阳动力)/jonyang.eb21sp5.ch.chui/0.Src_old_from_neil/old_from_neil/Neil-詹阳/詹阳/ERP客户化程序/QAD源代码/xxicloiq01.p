...
/*K1HQ*/ (c-web-request begins 'data':u)) then do:
        ...
        if not can-find(first pt_mstr where 
			pt_domain = global_domain and /*---Add by davild 20090205.1*/
			pt_part = part) then do:
           {mfmsg.i 16  3}
...
/*K1HQ*/ end.
        ...
        and (can-find(first ld_det where 
...
/*F767/*F528*/    pause before-hide. */
		  ...
		  is_status  = ld_status no-lock no-error.
          find loc_mstr where 
		  loc_domain = global_domain and /*---Add by davild 20090205.1*/
		  loc_loc = ld_loc no-lock no-error.
