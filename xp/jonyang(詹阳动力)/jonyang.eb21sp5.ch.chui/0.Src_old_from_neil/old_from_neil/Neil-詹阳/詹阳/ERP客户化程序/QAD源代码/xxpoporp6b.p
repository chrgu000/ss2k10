/* poporp6b.p - PURCHASE ORDER RECEIPTS REPORT Sorted by Item                 */
...
/* REVISION: 9.1     LAST MODIFIED: 09/16/00 BY: *N0KM* Rajinder Kamra        */
/* REVISION: 9.1     LAST MODIFIED: 01/18/01 BY: *N0VP* Sandeep Parab         */
/* Revision: 1.8.1.14      BY: Steve Nugent       DATE: 04/17/02  ECO: *P043* */
/* Revision: 1.8.1.15      BY: Patrick Rowan      DATE: 05/24/02  ECO: *P018* */
/* Revision: 1.8.1.17      BY: Lena Lim           DATE: 06/14/02  ECO: *P07G* */
/* Revision: 1.8.1.18      BY: Hareesh V.         DATE: 06/21/02  ECO: *N1HY* */
/* Revision: 1.8.1.21      BY: Patrick Rowan      DATE: 08/15/02  ECO: *P0FH* */
/* Revision: 1.8.1.22      BY: Karan Motwani      DATE: 08/27/02  ECO: *N1SB* */
/* Revision: 1.8.1.23      BY: Dan Herman         DATE: 08/29/02  ECO: *P0DB* */
/* Revision: 1.8.1.24.1.1  BY: N. Weerakitpanich  DATE: 05/03/03  ECO: *P0R5* */
/* Revision: 1.8.1.24.1.2  BY: Paul Donnelly (SB) DATE: 06/28/03  ECO: *Q00J* */
/* Revision: 1.8.1.24.1.3  BY: Deepak Rao         DATE: 07/31/03  ECO: *P0T9* */
/* Revision: 1.8.1.24.1.4  BY: Manisha Sawant     DATE: 04/26/04  ECO: *P1YV* */
/* Revision: 1.8.1.24.1.5  BY: Bhagyashri Shinde  DATE: 11/23/04  ECO: *P2W5* */
/* Revision: 1.8.1.24.1.7  BY: Robin McCarthy     DATE: 01/05/05  ECO: *P2P6* */
/* Revision: 1.8.1.24.1.8  BY: Sukhad Kulkarni    DATE: 03/29/05  ECO: *P2Y8* */
/* Revision: 1.8.1.24.1.9  BY: Anil Sudhakaran    DATE: 08/11/05  ECO: *P2PJ* */
/* Revision: 1.8.1.24.1.9.1.1  BY: Steve Nugent   DATE: 09/07/05  ECO: *Q0LM* */
/* Revision: 1.8.1.24.1.9.1.2  BY: Shivaraman V.  DATE: 03/06/06  ECO: *P4KF* */
/* $Revision: 1.8.1.24.1.9.1.4 $ BY: R. McCarthy  DATE: 09/22/06  ECO: *P575* */
...
no-lock break by prh_part
/*SS 20090318 - B*/
/*
with frame d down width 132 no-box:
*/
with frame d down width 132 no-box:
/*SS 20090318 - E*/
   {&POPORP6B-P-TAG2}
   ...
   do with frame dhead:
      /* SET EXTERNAL LABELS */
      setFrameLabels(frame dhead:handle).
      ...
      for each tt_pvoddet no-lock:
         ...
         /* NO NEED FOR CONVERSION OF BASE_COST = DOC CURR UNIT COST */
         if base_rpt = ""
            and prh_curr <> base_curr
         then do:
         ...
         if first-of (prh_part) then do:
            if first (prh_part) then put skip(1).
            ...
            if available pt_mstr then
               descname = pt_desc1 + " " + pt_desc2.

            display
               prh_part
               descname no-label
            with frame dhead no-box width 132.
         end.
         ...
         if qty_open <> 0 or prh_ps_qty <> 0 then do:
            display
               prh_vend
                  column-label "Supplier!Project"
               prh_nbr
                  column-label "Order!ERS Opt"
               prh_receiver
                  column-label "Receiver!PS Nbr"
                  format "x(20)"
               prh_line
               prh_rcp_date
                  column-label "Receipt!Ship Dt"
               qty_open
      ...
      accumulate std_ext (total by prh_part).
      accumulate pur_ext (total by prh_part).
      accumulate std_var (total by prh_part).
      accumulate tax_amt (total by prh_part).
      ...
      then do:
         ...
         if (accum total by prh_part std_ext) <> 0 or
            (accum total by prh_part pur_ext) <> 0 or
            (accum total by prh_part std_var) <> 0 or
            (accum total by prh_part tax_amt) <> 0
         then
            display
               (if base_rpt = "" then
                   getTermLabelRtColon("BASE_ITEM_TOTAL",16)
                else
                   base_rpt + " " + getTermLabelRt("ITEM_TOTAL",11) + ":")
                   format "x(16)"              to 84
               accum total by prh_part std_ext
                  format "->>>>>>>>>>9.99<<<<" to 100
               accum total by prh_part pur_ext
                  format "->>>>>>>>>>9.99<<<<" to 116
               accum total by prh_part std_var
                  format "->>>>>>>>>>9.99<<<<" to 132
               accum total by prh_part tax_amt
                  format "->>>>>>>>>>9.99<<<<" to 116
               skip(1)
            with frame e width 132 no-labels no-box.
      end.
   end. /* DO WITH */
...
then
   display
      "--------------- --------------- ---------------" to 132
      (if base_rpt = "" then
         getTermLabel("BASE_REPORT_TOTAL",18) + ":"
      else
         base_rpt + " " + getTermLabelRt("REPORT_TOTAL",13) + ":")
         format "x(18)"                                 to 84
      accum total std_ext
         format "->>>>>>>>>>9.99<<<<"                   to 100
      accum total pur_ext
