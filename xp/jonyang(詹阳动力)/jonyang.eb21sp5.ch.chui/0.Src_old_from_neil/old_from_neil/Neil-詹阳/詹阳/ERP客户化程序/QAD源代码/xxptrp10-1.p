...
setFrameLabels(frame a:handle).
   ...
   form 
        tmp_site column-label "地点"  
        tmp_part column-label "库位"  
        tmp_loc  column-label "物料号"
        pt_desc1 
        pt_um
        tmp_beg_bal column-label "上期结存数量!上期结存金额 " 
        tmp_rct_qty column-label "本期入库数量!本期入库金额"
...
/* REPORT BLOCK */
   ...
   /* SELECT PRINTER */
          ...
          {mfphead.i}
          find first icc_ctrl 
		  where icc_domain = global_domain  /*---Add by davild 20090205.1*/
		  no-lock no-error.
