...
find first gl_ctrl 
where gl_domain = global_domain /*---Add by davild 20090205.1*/
no-lock.
   ...
   if (c-application-mode <> 'web') or
      ...
      if available vd_mstr then do:
         ...
         /*       {pxmsg.i &MSGNUM=mc-error-number &ERRORLEVEL=2}   */
            ...
            for each ap_mstr no-lock where 
...
/*ekDm1*/ and ((ap_entity = m_entity) or (m_entity = ""))
      ...
      if ap_type = "VO" then do:
         ...
         applied = base_amt.
         find ck_mstr where 
			ck_domain = global_domain and /*---Add by davild 20090205.1*/
			ck_ref = ap_ref
     ...
     /*           notes = getTermLabel("HOLD",8)   */
            ...
            if notes = "" then do:
               ...
               do while available ckd_det:
                  find ck_mstr where 
					ck_domain = global_domain and /*---Add by davild 20090205.1*/
					ck_ref = ckd_ref no-lock no-error.
                  if ck_status <> "Void" then do:
                     notes = ckd_ref.
                     leave.
                  end.
                  else
                  find prev ckd_det where 
					ckd_domain = global_domain and /*---Add by davild 20090205.1*/
					ckd_voucher = ap_ref
                  no-lock no-error.
               end.
