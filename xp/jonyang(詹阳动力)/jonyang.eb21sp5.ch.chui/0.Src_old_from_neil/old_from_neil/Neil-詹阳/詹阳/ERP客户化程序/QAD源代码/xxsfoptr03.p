/* sfoptr03.p - LABOR FEEDBACK BY WORK CENTER TRANSACTION                     */
...
/* REVISION: 4.0     LAST MODIFIED: 09/22/88    BY: flm *A449*                */
...
/* Revision: 1.15      BY: Anitha Gopal         DATE: 08/23/02  ECO: *N1RP*   */
/* Revision: 1.16      BY: Narathip W.          DATE: 04/29/03 ECO: *P0QN* */
/* Revision: 1.18      BY: Paul Donnelly (SB)   DATE: 06/28/03 ECO: *Q00L* */
/* Revision: 1.19      BY: A.R. Jayaram       DATE: 08/25/03 ECO: *P10N* */
/* $Revision: 1.20 $   BY: Marian Kucera   DATE: 07/14/04    ECO: *Q06H*  */
...
form
   op_wkctr       colon 13
   wc_desc        no-label format "x(20)"
   op_mch         colon 62 skip(1)
   emp            colon 13
   ...
   do transaction with frame a:
      ...
      editing:
         ...
         then do:
            ...
            {mfnp.i wc_mstr op_wkctr  " wc_mstr.wc_domain = global_domain and
            wc_wkctr "  op_mch wc_mch wc_wkctr}
            ...
            then do:
               display
                  wc_wkctr @ op_wkctr
                  wc_mch @ op_mch
                  wc_desc wc_dept @ dept.
            end. /* IF recno <> ? */

            wkctr = input op_wkctr.

         end. /* IF frame-field = "op_wkctr" */
         ...
         then do:
            ...
            {mfnp01.i wc_mstr op_mch wc_mch
               "input op_wkctr"  " wc_mstr.wc_domain = global_domain and
               wc_wkctr "  wc_wkctr}
               ...
               then do:
                  ...
                  for first wc_mstr
                     fields( wc_domain wc_dept wc_desc wc_mch wc_wkctr)
                      where wc_mstr.wc_domain = global_domain and  wc_wkctr =
                      wr_wkctr
                     and   wc_mch   = wr_mch
                     no-lock:
         ...
         end. /* ELSE IF frame-field = "wr_nbr" */
         ...
         then do:
            ...
            /* FIND NEXT/PREVIOUS RECORD */
            {mfnp01.i wr_route wr_op wr_op "input wr_lot"  " wr_route.wr_domain
      ...
      for first wc_mstr
         fields( wc_domain wc_dept wc_desc wc_mch wc_wkctr)
          where wc_mstr.wc_domain = global_domain and  wc_wkctr = input op_wkctr
      ...
      then do:
         ...
         for first emp_mstr
            fields( emp_domain emp_addr emp_dept emp_lname emp_status)
             where emp_mstr.emp_domain = global_domain and  emp_addr = input emp
            no-lock:
      ...
      then do:
         ...
         for first ea_mstr
            fields( ea_domain ea_earn ea_type)
             where ea_mstr.ea_domain = global_domain and  ea_earn = input earn
            no-lock:
         end. /* FOR FIRST ea_mstr */
      ...
      then do:
         ...
         then
            for first wr_route
               fields( wr_domain wr_desc wr_lot wr_mch wr_nbr wr_op wr_status
               wr_wkctr)
using  wr_nbr and wr_lot where wr_route.wr_domain = global_domain  no-lock:
         ...
         then
            for first wr_route
               fields( wr_domain wr_desc wr_lot wr_mch wr_nbr wr_op wr_status
               wr_wkctr)
using  wr_nbr where wr_route.wr_domain = global_domain  no-lock:
         ...
         then
            for first wr_route
               fields( wr_domain wr_desc wr_lot wr_mch wr_nbr wr_op wr_status
               wr_wkctr)
using  wr_lot where wr_route.wr_domain = global_domain  no-lock:
...
using  wr_lot and wr_op where wr_route.wr_domain = global_domain  no-lock:
      ...
      if project <> ""
         and can-find(first gl_ctrl
                          where gl_ctrl.gl_domain = global_domain and
                          gl_verify)
      then do:
