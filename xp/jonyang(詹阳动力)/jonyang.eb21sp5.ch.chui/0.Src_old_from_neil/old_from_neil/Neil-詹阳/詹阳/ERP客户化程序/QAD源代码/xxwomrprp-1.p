...
/*G2B9*/ define variable onlybom  like mfc_logical no-undo.
         ...
         define variable xxdataset like mrp_dataset label "类型".
...
&UNDEFINE PP_FRAME_NAME
         ...
         eff_date = today.
         site = global_site.
/*GN61*/ find si_mstr where si_site = site  
/*SS 20090207 - B*/
...
/*K12M*/    if c-application-mode <> "WEB":U then
						...
						update part part1 part2 part3 pline pline1 buyer buyer1 due_date due_date1 pm site
             eff_date maxlevel with frame a.
...
/*G2B9*/    /* BEGIN ADD SECTION */
   ...
   for each mrp_det where mrp_domain = global_domain and mrp_det.mrp_dataset = "fcs_sum" 
...
/*SS 20090207 - E*/
               ...
               if assy_only = yes then do:
                  find first ps_mstr where ps_par = comp 
/*SS 20090207 - B*/
									and ps_domain = global_domain
/*SS 20090207 - E*/                  
                  no-lock no-error.
                  if not available ps_mstr then get-next = yes.
               end.

               if not can-find(bom_mstr where bom_parent = comp and bom_domain = global_domain)
               then
                  get-next = yes.
...
/*SS 20090207 - E*/                        
                      ...
                      for each mrpdet where mrpdet.mrp_part = tmp_part 
/*SS 20090207 - B*/
											and mrpdet.mrp_domain = global_domain
/*SS 20090207 - E*/                      
                      break by mrpdet.mrp_part by mrpdet.mrp_dataset
                           with frame c width 500 down no-attr-space no-box:

                          if mrpdet.mrp_dataset = "wo_mstr" then do:
                         find wo_mstr where wo_lot = mrpdet.mrp_line 
/*SS 20090207 - B*/
...
/*SS 20090207 - E*/                         
                         ...
                         if last (mrpdet.mrp_part) then
                             display 
                             xxdataset column-label "类型"
                             due_date2 column-label "到期日"
                             ppart column-label "零件号(预测)"
                             mrp_qty1 column-label "预测数量" 
                             tmp_part column-label "零件号"
                             pt_desc1 column-label "摘要"
                             pt_desc2 column-label "摘要"
                             pt_um    column-label "单!位"
                             pt_buyer column-label "采!计"
                             pt_pm_code column-label "采!制"
                             qty_req1  column-label "需求量" 
