...
define variable TRAN_DATE like OP_TRAN_DATE .
...
define variable DUE_DATE like WO_DUE_DATE LABEl "加工单到期日".
define variable DUE_DATE1 like WO_DUE_DATE.
define variable desc2 like pt_desc1.
define variable desc3 as character format "x(5)".
define variable desc4 like pt_desc1.
...
vend vend1 wkctr wkctr1 mch mch1 op op1 start start1 due due1 OPDATE OPDATE1 rel_date rel_date1 due_date due_date1 sort_op with frame a.
...
/* SS 090924.1 - E */
            ...
            for each wr_route where 
                ...
                and (wo_due_date >= due_date and wo_due_date <= due_date1)
                and (wo_status = "R")
                no-lock break by wr_part by wr_op with frame c STREAM-IO width 132 no-labels no-box:
                ...
                desc3 = "".
                DESC4 = "".
                if substring(wr_route.wr_wkctr,1,2) = "23" then do :
                    DESC4 = wr_route.wr_DESC.
                    find FIRST wod_det where 
                    /* *ss_20090225* */  wod_domain = global_domain and
                    wod_lot = wo_lot and wod_qty_req <> 0 no-lock no-error.
                    if available wod_det then find pt_mstr where pt_part = wod_part no-lock no-error.
                    if available pt_mstr then desc2 = pt_desc1.
                    desc3 = "材料：".
                end.
