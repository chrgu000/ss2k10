...
/*IFP*/     if due_date = low_date then due_date = ?.
           ...
           update nbr nbr1 lot lot1 part part1 wkctr wkctr1 wkctr2 wkctr3 buyer buyer1 rel_date rel_date1 due_date due_date1 statu with frame a.
...
/*SS 20090206 - E*/    
    ...
    buyer2 = pt_buyer.
    find pt_mstr where pt_part = wod_part and (pt_buyer >= buyer3 and pt_buyer <= buyer4) 
/*SS 20090206 - B*/
			and pt_domain = global_domain
/*SS 20090206 - E*/    
    no-lock no-error.
...
/*SS 20090206 - E*/
         ...
         if page-size - line-counter < 2
             ...
             open_ref = max(wod_qty_req - max(wod_qty_iss,0),0).
               ...
               rowkctr column-label "首工序" WHEN OLD_LOT <> WO_LOT
               ro_wkctr column-label "尾工序" WHEN OLD_LOT <> WO_LOT
               wo_part column-label "父零件" WHEN OLD_LOT <> WO_LOT
               desc1 WHEN OLD_LOT <> WO_LOT
               buyer2 column-label "采购员" WHEN OLD_LOT <> WO_LOT
