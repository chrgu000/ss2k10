...
/*J242*/  for first en_mstr fields( en_domain en_name en_entity)
/*J242*/  no-lock  where en_mstr.en_domain = global_domain and  en_entity =
current_entity: end.
...
/*J242*/  for first co_ctrl fields( co_domain co_ret co_use_cc co_use_sub)
...
/*J242*/        for first glc_cal fields( glc_domain glc_end glc_per glc_start
glc_year)
/*J242*/        no-lock  where glc_cal.glc_domain = global_domain and  glc_year
= fiscal_yr and glc_per = 1: end.
...
/*J242* BEGIN ADD */
             if not budgflag and
             can-find (first glt_det  where glt_det.glt_domain = global_domain
             and
...
/*K1RK* start add section ***************/
          PROCEDURE print-comments:
            ...
            for each cd_det
            fields( cd_domain cd_cmmt cd_ref cd_type)
...
/*L017*/  /* Added following section */
          PROCEDURE p-euro1:
             ...
             for each bg_mstr fields( bg_domain bg_fpos bg_acc bg_code) no-lock
              where bg_mstr.bg_domain = global_domain and  bg_code = budgetcode
              and
             bg_acc = "" use-index bg_ind1:
                ...
                if available fm_mstr and fm_type = "I" then do:
                   for first ac_mstr fields( ac_domain ac_type ac_fpos)
                   no-lock  where ac_mstr.ac_domain = global_domain and
                   ac_fpos = fm_fpos: end.
                   if available ac_mstr and ac_type = "I" then do:
                      {glfmbg.i &fpos=fm_fpos &yr=fiscal_yr &per=per_beg
                                &per1=per_end &budget=balance &bcode=budgetcode}
