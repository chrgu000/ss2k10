...
/*J23W*/ for first en_mstr fields( en_domain en_name en_entity)
/*J23W*/ no-lock  where en_mstr.en_domain = global_domain and  en_entity =
current_entity: end.
...
/*J23W*/ for first co_ctrl fields( co_domain co_ret co_use_cc co_use_sub)
/*J23W*/  where co_ctrl.co_domain = global_domain no-lock: end.
...
/*J23W*/           for first glc_cal fields( glc_domain glc_end glc_per
glc_start glc_year)
/*J23W*/           no-lock  where glc_cal.glc_domain = global_domain and
glc_year = fiscal_yr[i] and glc_per = 1: end.
...
/*L01W*/           et_report_curr = rpt_curr.
             ...
             if cmmt-yn then do:
                put cname at 1 skip.
                for each cd_det
/*J23W*/            fields( cd_domain cd_cmmt cd_ref cd_type)
                     where cd_det.cd_domain = global_domain and  cd_ref =
                     cmmt_ref and cd_type = cmmt_type
                    no-lock:
                    do i = 1 to 15:
...
/*J23W* BEGIN ADD */
                   if not budget[i] and
                      can-find (first glt_det  where glt_det.glt_domain =
                      global_domain and  glt_entity >= entity and
                                           glt_entity <= entity1 and
                                           glt_sub >= sub and
                                           glt_sub <= sub1 and
                                           glt_cc >= ctr and glt_cc <= ctr1 and
