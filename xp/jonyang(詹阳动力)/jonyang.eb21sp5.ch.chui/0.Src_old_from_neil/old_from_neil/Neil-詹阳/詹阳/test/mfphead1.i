...
*/
define variable rtitle as character format "x(82)".
/* SS 090924.1 - E */
...
*/
then len_raw = {gprawlen.i &strng=substring(dtitle,21,50)}.
else len_raw = {gprawlen.i &strng=substring(dtitle,27,44)}.
/* SS 090924.1 - E */
...
if global_usrc_right_hdr_disp < 2
then rtitle = substring(dtitle,1,24) + fill(" ",6) +
              substring(dtitle,25,50 - (len_raw - 44)).
else rtitle = substring(dtitle,1,30) + fill(" ",6) +
              substring(dtitle,31,44 - (len_raw - 38)).
/* SS 090924.1 - E */
/*V8+*/
...
/*
   ...
   &ENDIF
*/
   getTermLabelRtColon("DATE",9) format "x(9)" to 97
   today           skip
   getTermLabel("PAGE",8) + ":" at 1
   string(page-number {2}) format "x(8)"
   co_name         at 34
   getTermLabelRtColon("TIME",9) format "x(9)" to 97
   string(Time,"hh:mm:ss")
...
*/
with frame phead page-top width 106 no-box.
/* SS 090924.1 - E */
...
*/
   with frame pwebhead page-top width 106 no-box.
/* SS 090924.1 - E */
