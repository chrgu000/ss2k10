...
FORM /*GUI*/ 
 ...
 SKIP(.4)  /*GUI*/
with overlay frame a side-labels NO-BOX THREE-D /*GUI*/.
...
/*K0PG*/ if c-application-mode <> 'web':u then
...
/*K0PG*/ {wbrp06.i &command = update &fields = "  LOC LOC1 nbr nbr1 trdate trdate1 part
...
FOR each tr_hist where (tr_nbr >= nbr and (tr_nbr <= nbr1 OR nbr1 = ""))
...
/*G028*/ and (tr_type <> "ORD-PO" or type = "ORD-PO"),
    EACH TRGL_DET WHERE TRGL_TRNBR = TR_TRNBR AND TRGL_GL_AMT <> 0 
...
/*FO38*/   use-index tr_nbr_eff */
   no-lock break by tr_nbr by tr_effdate by tr_part with frame b down width 620:
     ...
     /*put {&ictrrp02_p_1}*/display tr_nbr label "订单".
      ...
      FIND FIRST RO_DET WHERE RO_ROUTING = WO_PART AND SUBSTRING(RO_WKCTR,1,2) <> "23"  no-lock no-error.
      END.
...
/*K003*/ if available pt_mstr then desc1 = pt_desc1 + " " + pt_desc2.
      find IN_mstr where IN_part = tr_part AND IN_SITE = TR_SITE no-lock no-error.
  ...
  /*  FIND TRGL_DET WHERE TRGL_TRNBR = TR_TRNBR NO-LOCK NO-ERROR.*/
   if tr_type = "rct-po"   
      then do:
      find prh_hist WHERE TR_LOT = PRH_RECEIVER AND TR_NBR = PRH_NBR AND TR_PART = PRH_PART and tr_line = prh_line no-lock no-error.
      find ad_mstr where ad_addr = tr_addr no-lock no-error.
     end.
...
/*GUI*/ {mfguirpb.i &flds="  nbr nbr1 trdate trdate1 part part1 site site1 so_job so_job1 loc loc1 LINE LINE1 truserid truserid1 type "} /*Drive the Report*/
