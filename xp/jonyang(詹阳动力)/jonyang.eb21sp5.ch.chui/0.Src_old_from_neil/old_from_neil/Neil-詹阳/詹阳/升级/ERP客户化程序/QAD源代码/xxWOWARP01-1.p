...
/*K102*/ {wbrp06.i &command = update &fields = "  nbr nbr1 part part1 lot lot1 deliver deliver1 buyer buyer1 getall getall1" &frm = "a"}
...
/*B357*/       end.
           ...
           find pt_mstr where pt_part = wod_part and (pt_buyer >= buyer and pt_buyer <= buyer1) no-lock no-error.
/*           if page-size - line-counter < 2 and available pt_mstr
     ...
     find ld_det where ld_part = wod_part and ld_loc = loc no-lock no-error.
      if open_ref <> 0 
...
/*GUI*/ {mfguirpb.i &flds=" nbr nbr1 part part1 lot lot1 deliver deliver1 buyer buyer1 getall getall1"} /*Drive the Report*/
