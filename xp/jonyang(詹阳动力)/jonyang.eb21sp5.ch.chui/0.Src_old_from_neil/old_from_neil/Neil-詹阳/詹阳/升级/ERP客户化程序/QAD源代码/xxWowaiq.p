...
/* FIND NEXT/PREVIOUS RECORD */
          {mfnp.i wod_det nbr wod_nbr nbr wod_nbr wod_nbrpart}
...
/*K1K2*/       if c-application-mode = 'web':u then return.
        ...
        if part <> "" then do:
           find first wod_det where wod_part = part no-lock no-error.
           if not available wod_det then do:
              if can-find (pt_mstr where pt_part = part) then do:
...
/*K1K2*/ end.
        ...
        /* FIND AND DISPLAY */
        if nbr <> "" then
           for each wod_det where wod_nbr = nbr
              and (wod_lot = lot or lot = "" )
              and (wod_part = part or part = "" )
/*G656*/      and (wod_op = op or op = 0)
              and (wod_iss_date = issue or issue = ?) no-lock
              with frame b width 108 no-attr-space:
/*K1RZ*    {mfrpchk.i}                     /*G339*/ */
/*K1RZ*/      run mfrpchk in this-procedure.
              {mfwaiq-xx.i}
           end.
          else if lot <> "" then
           for each wod_det where wod_lot = lot
              and (wod_part = part or part = "" )
/*G656*/      and (wod_op = op or op = 0)
              and (wod_iss_date = issue or issue = ?) no-lock
              with frame c width 108 no-attr-space:
/*K1RZ*    {mfrpchk.i}                     /*G339*/ */
/*K1RZ*/      run mfrpchk in this-procedure.
              {mfwaiq-xx.i}
           end.
          else if part <> "" then
           for each wod_det where wod_part = part
/*G656*/      and (wod_op = op or op = 0)
              and (wod_iss_date = issue or issue = ?) no-lock by wod_iss_date
              with frame d width 108 no-attr-space:
/*K1RZ*    {mfrpchk.i}                     /*G339*/ */
/*K1RZ*/      run mfrpchk in this-procedure.
           ...
           for each wod_det where wod_iss_date = issue
/*G656*/      and (wod_op = op or op = 0)
              no-lock
              with frame e width 108 no-attr-space:
/*K1RZ*    {mfrpchk.i}                     /*G339*/ */
/*K1RZ*/      run mfrpchk in this-procedure.
              {mfwaiq-xx.i}
           end.
          else for each wod_det no-lock
/*G656*/      where (wod_op = op or op = 0)
              with frame f width 108 no-attr-space:
/*K1RZ*    {mfrpchk.i}                     /*G339*/ */
/*K1RZ*/      run mfrpchk in this-procedure.
             {mfwaiq-xx.i}
        end.
