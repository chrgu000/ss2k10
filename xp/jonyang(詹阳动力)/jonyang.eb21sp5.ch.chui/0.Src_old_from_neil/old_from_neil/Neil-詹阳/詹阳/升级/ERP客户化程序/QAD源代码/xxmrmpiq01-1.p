...
/*K10G*/ if c-application-mode <> 'web':u then
     ...
     if frame-field = "in_part" then do:
        ...
        {mfnp.i pt_mstr in_part pt_part in_part pt_part pt_part}
        ...
        /* FIND NEXT/PREVIOUS RECORD */
        {mfnp.i si_mstr in_site si_site in_site si_site si_site}
...
/*K10G*/ (c-web-request begins 'data':u)) then do:
      ...
      find pt_mstr where pt_part = input in_part no-lock no-error.
...
/*K10G*/ if c-application-mode = 'web':u then return.
    undo.
      ...
      find si_mstr no-lock where si_site = input in_site no-error.
...
/*J045*/ find first fsworkfile exclusive-lock no-error.
     ...
     find first mrp_det no-lock where mrp_part = part
     and mrp_site = site
     use-index mrp_site_due no-error.
     repeat:
        if not available mrp_det
        or mrp_dataset <> "fcs_sum" then leave.
        find next mrp_det no-lock where mrp_part = part
...
/*F816*/          if mrp_dataset = "pb_sold" then do:
             find sod_det where sod_nbr = mrp_nbr
...
/*F816*/             detail = {&mrmpiq01_p_14} + " " + mrp_line2.
          ...
          find wo_mstr where wo_lot = mrp_line
          ...
          find fc_det where fc_part = mrp_part
...
/*F816*/         end.
/*F816*/         find dsd_det no-lock where dsd_req_nbr = mrp_nbr
