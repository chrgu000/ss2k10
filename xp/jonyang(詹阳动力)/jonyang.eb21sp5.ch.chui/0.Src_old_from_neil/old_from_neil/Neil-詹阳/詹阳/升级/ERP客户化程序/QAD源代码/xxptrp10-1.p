...
define new shared temp-table tmp 
    ...
    field tmp_rct_qty1 like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_rct_amt1 like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_rct_qty2 like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_rct_amt2 like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_rct_qty3 like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_rct_amt3 like in_qty_oh format "->>,>>>,>>9.99"   
    
    
    field tmp_iss_qty like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_iss_amt like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_iss_qty1 like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_iss_amt1 like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_iss_qty2 like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_iss_amt2 like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_iss_qty3 like in_qty_oh format "->>,>>>,>>9.99"
    field tmp_iss_amt3 like in_qty_oh format "->>,>>>,>>9.99"    
...
FORM /*GUI*/ 
 ...
 SKIP(.1)  /*GUI*/
   ...
   form 
        ...
        tmp_rct_qty1 label "计外入库"
        tmp_rct_qty2 label "订单入库"
        tmp_rct_qty3 label "其它入库"
        
        
        
        tmp_iss_qty column-label "本期出库数量!本期出库金额"
        tmp_iss_qty1 label "计外出库"
        tmp_iss_qty2 label "订单出库"
        tmp_iss_qty3 label "其它出库"
   ...
   form 
        ...
        tmp_iss_qty column-label "本期出库数量!本期出库金额"
        tmp_iss_qty1 label "计外出库数量"
        tmp_iss_qty2 label "订单出库数量"
        tmp_iss_qty3 label "其它出库数量"
...
/*GUI*/   mainloop: do on error undo, return error on endkey undo, return error:
          ...
          for each in_mstr where in_site >= site and in_site <= site1 
       		and in_part >= part and in_part <= part1:
               find first pt_mstr where pt_prod_line >= line and pt_prod_line <= line1
               ...
               for each ld_det where (ld_site = in_site) and (ld_part = in_part)
         ...
         for each tr_hist where tr_effdate >= eff_date and tr_site >= site and tr_site <= site1
              ...
              , first pt_mstr where pt_part = tr_part and pt_abc >= abc and pt_abc <= abc1 no-lock:
              ...
              if tr_effdate >= eff_date and tr_effdate <= eff_date1 then do:
                 if tr_type = "RCT-UNP" then do:
                    assign tmp_rct_qty1 = tmp_rct_qty1 + tr_qty_loc
                           tmp_rct_amt1 = tmp_rct_amt1 + amt.
                 end.
                 else if tr_type = "RCT-WO" OR tr_type = "RCT-PO" then do:
                    assign tmp_rct_qty2 = tmp_rct_qty2 + tr_qty_loc
                           tmp_rct_amt2 = tmp_rct_amt2 + amt.
                 end.                 
                 else if tr_type begins "RCT" AND (tr_type <> "RCT-WO" AND TR_TYPE <> "RCT-UNP") then do:
                    assign tmp_rct_qty3 = tmp_rct_qty3 + tr_qty_loc
                           tmp_rct_amt3 = tmp_rct_amt3 + amt.
                 end.                                  
                 END.
             if tr_effdate >= eff_date and tr_effdate <= eff_date1 then do:
                 if tr_type = "ISS-UNP" then do:
                    assign tmp_ISS_qty1 = tmp_ISS_qty1 + tr_qty_loc
                           tmp_ISS_amt1 = tmp_ISS_amt1 + amt.
                 end.
                 else if tr_type = "ISS-WO" then do:
                    assign tmp_ISS_qty2 = tmp_ISS_qty2 + tr_qty_loc
                           tmp_ISS_amt2 = tmp_ISS_amt2 + amt.
                 end.                 
                 else if tr_type begins "ISS" AND (tr_type <> "ISS-WO" AND TR_TYPE <> "ISS-UNP") then do:
                    assign tmp_ISS_qty3 = tmp_ISS_qty3 + tr_qty_loc
                           tmp_ISS_amt3 = tmp_ISS_amt3 + amt.
                 end.                                  
                 END.
         ...
         if sort_by then          
             for each tmp where tmp_userid = mfguser ,
                 ...
                 break by tmp_site by tmp_loc by tmp_part 
                 with frame c stream-io width 300 no-box:
                  ...
                  display tmp_site tmp_loc tmp_part 
                          pt_desc1 pt_um
                          tmp_beg_bal 
                          tmp_rct_qty 
                          tmp_rct_qty1 
                          ...
                          (tmp_iss_qty1 * -1) @ tmp_iss_qty1
                          (tmp_iss_qty2 * -1) @ tmp_iss_qty2
                          (tmp_iss_qty3 * -1) @ tmp_iss_qty3
                  ...
                  display 
                    ...
                    tmp_rct_amt1 @ tmp_rct_qty1
                    tmp_rct_amt2 @ tmp_rct_qty2
                    tmp_rct_amt3 @ tmp_rct_qty3

                    
                    tmp_iss_amt @ tmp_iss_qty 
                    tmp_iss_amt1 @ tmp_iss_qty1 
                    tmp_iss_amt2 @ tmp_iss_qty2 
                    tmp_iss_amt3 @ tmp_iss_qty3 
                 ...
                 first pt_mstr where pt_part = tmp_part no-lock
                 break by tmp_site by tmp_part by tmp_loc
                 with frame d stream-io width 300 no-box:
                  if zeroflag then 
                      if tmp_beg_bal = 0 and tmp_rct_qty = 0 and tmp_iss_qty = 0 
                         and tmp_cnt_qty = 0 and tmp_end_bal = 0 then next.        
                  if last-of(tmp_site) then page.
...
/*GUI*/ {mfguirpb.i &flds="site site1 loc loc1 line line1 part part1 part_type part_type1 
part_group part_group1 eff_date eff_date1 abc abc1 grade grade1 assay assay1 stat stat1
zeroflag sort_by"} /*Drive the Report*/
