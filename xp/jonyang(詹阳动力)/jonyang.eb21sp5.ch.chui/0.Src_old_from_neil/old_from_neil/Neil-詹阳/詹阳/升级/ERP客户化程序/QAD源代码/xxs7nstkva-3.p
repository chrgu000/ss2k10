...
def var m_sumdet as log format "Summary/Detail" label "Summary/Detail" no-undo.
def var m_opt as char format "x(3)" label "Select transation type" no-undo.
...
for each w_file
    ...
    if w_type="1" then
    do:        
       find first pl_mstr where pl_prod_line = w_line no-lock no-error.
       Disp
