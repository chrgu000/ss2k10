...
/*LW*/ find first op_hist  where op_type = "labor" and (op_wo_lot = wr_lot and op_wo_op = wr_op)  no-lock no-error.
...
/*lw*/                 find FIRST wod_det where wod_lot = wo_lot and wod_qty_req <> 0 no-lock no-error.    
/*lw*/                 if available wod_det then find pt_mstr where pt_part = wod_part no-lock no-error.
