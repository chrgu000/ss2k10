...
/*IFP*/     buyer1         label {t001.i} colon 49 skip
	  ...
	  SKIP(.4)  /*GUI*/
with frame a side-labels WIDTH 200 /*GUI*/ NO-BOX THREE-D /*GUI*/.
...
/*GUI*/   mainloop: do on error undo, return error on endkey undo, return error:
	    {mfphead.i}
  if getall1 = "y" then
   for each sod_det where sod_user1 = "*":
   assign sod_user1 = "".
   end.
	    /* FIND AND DISPLAY */
	    for each wo_mstr no-lock where (wo_rel_date >= rel_date and wo_rel_date <= rel_date1)
...
/*	    AND ((SUBSTRING(WOD_PART,1,3) < "GYA") or (SUBSTRING(WOD_PART,1,3) > "GYZ"))*/
	    ...
	    if first-of(wod_part) then do :
                  FIND IN_MSTR WHERE IN_PART = wod_PART AND IN_SITE = "10000"  no-lock NO-ERROR.
	  ...
	  /*   bz2 = "".*/
	     qty_oh1 = 0.
   	    if wo_status = "r" then 
	       find first wr_route where wr_lot = wo_lot no-lock no-error.
	       if available wr_route then wkctr = wr_wkctr.
	       if not available wr_route or wo_status <> "r" then
	       find first ro_det where ro_routing = wo_part no-lock no-error.
...
/*B357*/       else open_ref = min(wod_qty_req - wod_qty_iss - wod_qty_pick,0).
	      ...
	      if available pt_mstr then  part3 = pt_part.
	       find last ro_det where ro_routing = pt_part no-lock no-error.
	       ...
	       find first ro_det where ro_routing = pt_part 
	       ...
	       if (pt_pm_code = "m" and available ro_det)
              ...
              FIND FIRST RO_DET WHERE ((RO_ROUTING = WOd_PART) AND (RO_WKCTR MATCHES "*95" OR RO_WKCTR MATCHES "*96" OR RO_WKCTR MATCHES "*99" OR RO_WKCTR MATCHES "*98" OR RO_WKCTR MATCHES "*97"))
                 no-lock no-error.
           ...
           IF SUBSTRING(RO_WKCTR,1,2) = "99" THEN OP = "外协". 
              ...
              FIND NEXT RO_DET WHERE ((RO_ROUTING = WOd_PART) AND (RO_WKCTR MATCHES "*95" OR RO_WKCTR MATCHES "*96" OR RO_WKCTR MATCHES "*99" OR RO_WKCTR MATCHES "*98" OR RO_WKCTR MATCHES "*97"))
                 no-lock no-error.
...
/*               qty_oh1 = decimal(in_user1) * -1.*/
    ...
    if qty_oh < 0 then
       ...
       if last-of(wod_part) then do:
             find first sod_det where sod_part = wod_part and sod_qty_ord > sod_qty_ship no-lock no-error.
             if available sod_det then do:
	    /*    down .*/
	       for each sod_det where sod_part = wod_part 
	      ...
	      if (last-of(wod_part)) and ((accum total by wod_part (qty_oh)) < 0 or qty_oh2 < 0) then do:
	          down 1.
              for each mrp_det where (mrp_dataset = "wo_mstr" or mrp_dataset = "pod_det")
                    ...
                    nbr3 = mrp_nbr.
                    lot3 = mrp_line.
                  find womstr where womstr.wo_lot = mrp_line no-lock no-error.  
...
/************************************	     
	        if last-of (wod_part) and getall = "y" then do:
                   find sod_det where sod_part = wod_part 
	           and (sod_due_date >= due_date and sod_due_date <= due_date1)
	           and (sod_req_date >= rel_date and sod_req_date <= rel_date1)
	           and (sod_qty_ord > sod_qty_ship and sod_user1 <> "*") 
	           no-error.
                   if available sod_det then do:
	                     rel_date3 = sod_req_date.
	                     due_date3 = sod_due_date.
	                     NBR3 = SOD_NBR.
	                     lot3 = string(sod_line).
	                     open_ref = max(sod_qty_ord - sod_qty_ship,0).
	                     assign sod_user1 = "*".
	                     bz1 = "*".
               in_user1 = string(decimal(in_user1) - open_ref). 
               if decimal(in_user1) < 0 and open_ref >= decimal(in_user1) * -1 
                  then qty_oh = decimal(in_user1) * -1.
               if decimal(in_user1) < 0 and open_ref <= decimal(in_user1) * -1 
                  then qty_oh = open_ref.
               if decimal(in_user1) > 0 then qty_oh = 0. 
	       ...
	       pt_buyer
	       wkctr1 label "交库类型" 
               max(in_qty_avail - in_qty_all,0) /*when first-of (wod_part)*/
	       open_ref 
	       qty_oh label "缺件数"
	       qty_oh1
	       op when pt_pm_code = "m"
	       space(2)  WITH STREAM-IO width 400 /*GUI*/ .
	                 ...
	                 end. /*end for available sod_date*/
	                end. /*end for last-of wod_part*/ 
...
****************************/
       ...
       if getall1 = "Y" THEN DO:
            for each sod_det where sod_user1 = "" and sod_qty_ord > sod_qty_ship
                   and (sod_nbr >= nbr4 and sod_nbr <= nbr5)
	          ...
	          IF FIRST-OF (SOD_PART) THEN DO:
                      FIND IN_MSTR WHERE IN_PART = sod_PART AND IN_SITE = "10000"  no-lock NO-ERROR.
                      if available in_mstr and in_qty_oh > 0 then qty_oh = max(in_qty_avail - in_qty_all,0).
                      if not available in_mstr or in_qty_oh = 0 then qty_oh = 0.
               if pt_pm_code = "m" then  
                         
        
              FIND FIRST RO_DET WHERE ((RO_ROUTING = sOd_PART) AND (RO_WKCTR MATCHES "*95" OR RO_WKCTR MATCHES "*96" OR RO_WKCTR MATCHES "*99" OR RO_WKCTR MATCHES "*98" OR RO_WKCTR MATCHES "*97"))
                 no-lock no-error.
                 if available RO_DET then op1 = substring(RO_wkctr,1,2).
              FIND NEXT RO_DET WHERE ((RO_ROUTING = sOd_PART) AND (RO_WKCTR MATCHES "*95" OR RO_WKCTR MATCHES "*96" OR RO_WKCTR MATCHES "*99" OR RO_WKCTR MATCHES "*98" OR RO_WKCTR MATCHES "*97"))
                 no-lock no-error.
...
/*GUI*/ {mfguirpb.i &flds=" nbr nbr1 lot lot1  nbr4 nbr5 parn parn1 part part1 deliver deliver1  deliver2 deliver3 rel_date rel_date1 due_date due_date1 buyer buyer1 xz PTPM getall1 getall "} /*Drive the Report*/
