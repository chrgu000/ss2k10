...
/*IFP*/  define variable zero_open like mfc_logical label "打印短暂量为零".
...
/*IFP*/	    exclusive-lock break by wo_nbr by wod_lot :
                if wod_deliver = "" then do:
                   if wo_routing <> "" then do:
                      find first ro_det where ro_routing = wo_routing and ro_op = wod_op no-lock no-error.
                      if available ro_det then assign wod_deliver = ro_wkctr.
                      else do:
                         find first ro_det where ro_routing = wo_routing no-lock no-error.
                         if available ro_det then assign wod_deliver = ro_wkctr.                   
                      end.
                   end.
                   else do:
                      find first ro_det where ro_routing = wo_part and ro_op = wod_op no-lock no-error.
                      if available ro_det then assign wod_deliver = ro_wkctr.
                      else do:
...
/*IFP*/     first wo_mstr where wo_lot = wod_lot and wo_part >= parn and wo_part <= parn1
/*IFP*/           and wo_rel_date >= rel_date and wo_rel_date <= rel_date1
/*IFP*/           and wo_due_date >= due_date and wo_due_date <= due_date1
...
/*LW*/     IF BUYER <> "" THEN DO :
/*LW*/         find pt_mstr where pt_part = wo_part AND PT_BUYER = BUYER no-lock no-error.
/*LW*/         END.
/*LW*/         IF BUYER = "" THEN DO :
/*LW*/         find pt_mstr where pt_part = wo_part  no-lock no-error.
/*LW*/         END.
...
/*IFP*/           display wod_deliver label "工作中心".
...
/*LW*/	       IF BUYER <> "" THEN DO:
/*LW*/	       find pt_mstr where pt_part = wod_part AND PT_BUYER = BUYER no-lock no-error.
/*LW*/	       END.
/*LW*/	       IF BUYER = "" THEN DO:
/*LW*/	       find pt_mstr where pt_part = wod_part  no-lock no-error.
/*LW*/	       END.
	       if available pt_mstr then desc1 = pt_desc1.
/*LW*/         find in_mstr where in_part = wod_part and in_site = "10000" no-lock no-error.
                if available pt_mstr and (pt_pm_code = "m" or pt_pm_code = "r")
                  then find last ro_det where ro_routing = pt_part no-lock no-error.  
...
/*IFP*/        if (open_ref <> 0 or zero_open) AND (available pt_mstr AND BUYER1 = "A") then do:
	              ...
	              display space(8) wod_nbr wod_lot wo_part label "父零件" 
/*LW*/	                        desc1_par WO_QTY_ORD LABEL "订货量"
	                        wod_part label "子零件" desc1 PT_BUYER
...
/*GUI*/ {mfguirpb.i &flds=" nbr nbr1 parn parn1 part part1 deliver deliver1 lot lot1 rel_date rel_date1
   due_date due_date1 iss_date iss_date1 STAT BUYER skpage zero_open"} /*Drive the Report*/
