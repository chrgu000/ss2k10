...
/*G656*/                 wod_lot >= lot and wod_lot <= lot1
                     ...
                     ,first wo_mstr where wo_lot = wod_lot and wo_part >= parn 
...
/*IFP*/                if wo_status = "C" then open_ref = 0.
                       ...
                       if first-of(wod_deliver) then do with frame c1 STREAM-IO side-labels:
                           find first wc_mstr where wc_wkctr = wod_deliver no-lock no-error.
             ...
             find sct_det where WOD_part = sct_part AND sct_sim = "Standard" and WOD_site = sct_site no-lock no-error.
...
/*F822*/             by lad_part
                  ...
                  find first wo_mstr where wo_lot = lad_nbr no-lock no-error.
...
/*IFP*/           find first wod_det where wod_lot = wo_lot no-lock no-error.
...
/*IFP*/           else next.
                  ...
                  if first-of(substring(lad_loc,1,2)) then do with frame d1 STREAM-IO side-labels width 255:
                       find first wc_mstr where wc_wkctr = lad_user1 no-lock no-error.
...
/*IFP*/           if wo_status = "C" then open_ref = 0.
                  ...
                  find first pt_mstr where pt_part = lad_part 
/*IFP*/               and pt_prod_line >= prod_line and pt_prod_line <= prod_line1
                      no-lock no-error.
/*IFP*/           if available pt_mstr then do:                      
...
/*G656*/             and lad_nbr >= lot and lad_nbr <= lot1
                     ...
                     and lad_user1 >= deliver and lad_user1 <= deliver1
/*IFP*/              and substring(lad_loc,1,2) >= loc and substring(lad_loc,1,2) <= loc1                     
                     ,first wod_det where wod_lot = lad_nbr and wod_part = lad_part
...
/*GUI*/ {mfguirpb.i &flds=" prod_line prod_line1 nbr nbr1 lot lot1 parn parn1 part part1 rel_date rel_date1
     due_date due_date1 iss_date iss_date1 loc loc1 deliver deliver1 skpage summary 
     only_pick sortoption only_all disp_zero"} /*Drive the Report*/
