...
/*K0WP*/ if c-application-mode <> 'web':u then
...
/*K0WP*/ {wbrp06.i &command = update &fields = " ptloc ptloc1 wkctr wkctr1 nbr nbr1 part part1 " &frm = "a"}
                ...
                if available ro_det then assign wod_deliver = ro_wkctr.                   
                    ...
                    if available pt_mstr then assign pt_routing1 = pt_routing.             
                      if pt_routing1 <> "" then do:
                        find first ro_det where ro_routing = pt_routing1 no-lock no-error.
                        if available ro_det then assign wod_deliver = ro_wkctr.                   
                      end.  
                end.
...
*******/
        ...
        no-lock break by wod_part by wod_user1 by wod_nbr by wod_deliver
        with frame b width 132 no-attr-space:
...
/*B357*/   else open_ref = min(wod_qty_req - wod_qty_iss,0).
           ...
           accumulate wod_qty_req (total by wod_part by wod_nbr by wod_deliver ).
           accumulate all_pick (total by wod_part by wod_nbr by wod_deliver ).
           accumulate wod_qty_iss (total by wod_part by wod_nbr by wod_deliver ).
           accumulate open_ref (total by wod_part by wod_nbr by wod_deliver ).
...
/* Only Release WO can be print */
           ...
           if last-of(wod_nbr) or last-of(wod_part) or last-of(wod_deliver) then do:
             put "---------- ---------- ---------- ----------" at 73.
...
/*GUI*/ {mfguirpb.i &flds="  ptloc ptloc1 wkctr wkctr1 nbr nbr1 part part1  "} /*Drive the Report*/
