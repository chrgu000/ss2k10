...
/* SS - 100623.1 - B */
define var v_go_on as logical .
define var v_full  as logical .
{xxsfoptrax01.i "new"}
/* SS - 100623.1 - E */
...
/* SS - 100623.1 - B */
   {gprun.i ""xxsfoptrax01.p"" "(input wr_lot, input wr_op)"}
/* SS - 100623.1 - E */
...
/* SS - 100623.1 - B */
      ...
      if qty_comp_this_op > wr_qty_ord then do:
         find prev wr_route no-lock
             where wr_route.wr_domain = global_domain and  wr_nbr = wrnbr and
             wr_lot = wrlot no-error.
         if not available wr_route or qty_comp_this_op > wr_qty_comp + wr_sub_comp then do:
            message "工序累计完成数量大于订购量,是否允许反馈?" update v_go_on.
            if v_go_on = no then undo set_ophist,retry set_ophist .
         end.
         find wr_route no-lock where recid(wr_route) = wr_recno.
      end.
      else if qty_comp_this_op < wr_qty_ord then do:
          v_full = no.  
      end.

      if v_full = no then assign wocomp = no compprev = no .
      set-comp-loop:
      do on error undo,retry :
          disp wocomp compprev with frame b .

          update 
              wocomp 
              compprev when v_full = yes 
          with frame b.

          if wocomp = yes and v_full = no then do:
              message "工序累计完成数量小于订购量,确定要关闭此工序?" update v_go_on.
              if v_go_on = no then undo set-comp-loop,retry set-comp-loop .
              else do:
                  message "是否同时关闭前工序?" update v_go_on.
                  if v_go_on = yes then compprev = yes .
              end.
          end.
...
/* SS - 100623.1 - B */
      if compprev = no then do:
         {gprun.i ""xxsfoptrax02.p"" }
      end.
/* SS - 100623.1 - E */
