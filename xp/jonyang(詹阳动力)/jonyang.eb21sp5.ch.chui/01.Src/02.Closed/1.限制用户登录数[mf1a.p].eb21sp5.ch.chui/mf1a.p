...
/* SS - 100426.1 - B */
define var v_number as integer .
define var v_allow  as integer .
define var v_tmp    as integer .

define buffer usrmstr for usr_mstr .
/* SS - 100426.1 - E */
...
c-mfgmgmt = getTermLabelCentered("MANUFACTURING_MANAGEMENT",78).
...
form
   ...
   c-mfgmgmt no-label skip(1)
   "QAD" at 38
   "6450 Via Real" at 33
   "Carpinteria, CA  93013" at 29
   skip(1)
   "(805) 684-6614" at 32
   "(805) 684-1890 FAX" at 32
   skip(1)
   "Copyright 1986-2004 QAD Inc. " at 15
...
with frame welcome side-labels width 80 no-attr-space.
...
form
   ...
   skip(1)
   skip(1)
with frame b side-labels overlay width 70 centered attr-space row 12.
...
/*V8-*/
assign
   token1    = "M F G / P R O " + trim(temp_vers)
   temp_vers = "                                                                              ".
...
/*查找所有登录的monitor,*/
for each mon_mstr where mon_userid = global_userid no-lock :
     v_number = v_number  + 1 .
    /*排除:已经退出系统但是系统依然monitor的部分,比如某些按ctrl+c退出的用户,
    因为:这部分记录,登录任意其他用户会自动删除,但是连续登录同一个用户不会删除*****
    find first cnt_mstr where cnt_userid = global_userid and cnt_application = "MFG/PRO" and cnt_sid = mon_sid no-lock no-error.
    if available cnt_mstr then do:
        v_number = v_number  + 1 .
    end.
    *********/
end .

/*if global_userid = "mfg" then message v_number v_allow view-as alert-box.*/

find first usrmstr where usrmstr.usr_userid = global_userid no-lock no-error .
if available usrmstr then do :

        /*1.检查异常字符:不是数字*/
        if usrmstr.usr_variant_code <> "" then do:
            do v_tmp = 1 to length(usrmstr.usr_variant_code):
                if index("0987654321", substring(usrmstr.usr_variant_code,v_tmp,1)) = 0 then do:
                    message "错误:用户可同时登录数超限:设定有误" .
                    pause 5.
                    undo,retry.
                end.
            end.
        ...
        /*2.无异常字符,取可登录数*/
        v_allow = int(usrmstr.usr_variant_code) .

        /*3.未设定的用户,默认可登录10次*/
        if v_allow = 0 then v_allow = 10 .   /*客户要求,默认先不限制,待正式库设定完再限制住*/


        /*4.校验可登录数,并报错 **/
        if v_number >= v_allow then do :
                    message "错误:用户可同时登录数超限,您只能同时登录个数:" v_allow .
                    pause 5.
                    undo,retry.        
        end.
end . /*if available usrmstr*/

/*if global_userid = "mfg" then message v_number v_allow view-as alert-box.*/

/*注意!!!
以上校验一定要放在原程式这句之前:"l_authorized = true." ,
否则按F4会跳过校验*/
/* SS - 100426.1 - E */
...
/* RUN THE MENU DRIVER */
{gprun1.i ""mfmenu.p"" "(output rstatus)"}
