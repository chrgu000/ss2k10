...
/* SS - 100527.1  By: Roger Xiao */  /*增加单据号,并保存在历史记录tr__chr01*/
/* SS - 100601.1  By: Roger Xiao */  /*限制发料数不可超过未结数,且不可输入*/
/* SS - 100803.1  By: Roger Xiao */  /**/
/* SS - 101020.1  By: Roger Xiao */  /*加领料单号到10位宽*/
/*-Revision end---------------------------------------------------------------*/
...
define var v_nbr as char format "x(10)" no-undo .
...
form 
    ...
    v_nbr  label "    领料单号"
    skip(1)
with frame xnbr side-labels overlay row 5 centered width 40 no-attr-space.
...
repeat:
    ...
    pause 0.
    view frame xnbr .
    update v_nbr with frame xnbr .
    if v_nbr = "" then do :
        message "错误:领料单号不允许为空,请重新输入" .
        pause .
        undo,retry.
    end.
...
/* SS - 100803.1 - B */
    ...
    if avail tt then leave mainloop.
/* SS - 100803.1 - E */  
