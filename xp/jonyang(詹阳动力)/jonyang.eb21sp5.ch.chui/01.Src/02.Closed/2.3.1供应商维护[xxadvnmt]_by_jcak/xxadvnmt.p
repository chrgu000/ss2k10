...
/* $Revision: 1.53 $   BY: Surajit Roy     DATE: 07/20/05 ECO: *P3TW* */
/*-Revision end---------------------------------------------------------------*/
...
/* guidelines.                                                                */
/* ************************************************************************** */
/* ss - 100423.1 by: jack */  /* 名称不能重复*/
/* ss - 100604.1 by:jack */  /* 分类别控制*/
...
/* ss - 100423.1 -b */
DEFINE VAR v_ok AS LOGICAL .
/* ss - 100423.1 -e */
/* ss - 100604.1 -b */
DEFINE VAR v_addr LIKE ad_addr .
/* ss - 100604.1 -e */
{&ADVNMT-P-TAG9}
{&ADVNMT-P-TAG10}
           ...
           {pxrun.i &PROC='processRead' &PROGRAM='adsuxr.p'
            ...
            do on error undo, retry:
               if c-application-mode <> "API" then
                  ...
                  /* ss - 100423.1 -b */
                  /* ss - 100604. -1 -b
                 {gprun.i ""xxvnadname.p"" "(input vd_addr , input ad_name,output v_ok)"}
                     IF v_ok = NO THEN DO:
                         MESSAGE "名称不能重复" VIEW-AS ALERT-BOX .
                         NEXT-PROMPT ad_name .
                         UNDO .
                     END.
                     ss - 100604.1 -e */
                  /* ss - 100604.1 -b */
                     {gprun.i ""xxvnadname.p"" "(input vd_addr , input ad_name,output v_ok,output v_addr)"}
                        IF v_ok = NO THEN DO:
                       
                         MESSAGE VIEW-AS ALERT-BOX BUTTONS YES-NO TITLE "名称已经存在:" + v_addr UPDATE v_logical AS LOGICAL .
                         IF v_logical = NO  THEN DO:
                               NEXT-PROMPT ad_name .
                         UNDO .
                         END.
