...
/* SS - 100803.1 - E */
    ...
    if first-of(ld_lot) and last-of(ld_lot) and ld_ref = "" then do:
        ...
        display 
            ld_site ld_loc ld_part desc1 ld_lot column-label {&iclorp_p_1}
            pt1_mstr.pt_um
            ld_qty_oh ld_date ld_expire ld_assay ld_grade
            ld_status is_avail is_nettable is_overissue 
            /* SS - 100803.1 - B */
            pt1_mstr.pt_article when (avail pt_mstr)
            pt1_mstr.pt_net_wt when (avail pt_mstr)
            pt1_mstr.pt_net_wt_um when (avail pt_mstr)
            /* SS - 100803.1 - E */
        WITH STREAM-IO /*GUI*/ .
        down 1.
        display desc2 @ desc1.
