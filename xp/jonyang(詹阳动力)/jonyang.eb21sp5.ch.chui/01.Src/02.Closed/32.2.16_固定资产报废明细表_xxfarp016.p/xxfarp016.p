...
define var l-retrsn       as char format "x(10)".
define var l-retrsn1      as char format "x(10)".
...
form
   ...
   v_date    colon 25      label "报废日期"
   v_date1   colon 42      label {t001.i}
   l-retrsn     colon 25      label "报废原因"
   l-retrsn1    colon 42      label {t001.i}
...
repeat:
    ...
    if l-retrsn1 = hi_char then l-retrsn1  = "".
    ...
    if l-retrsn1 = ""       then l-retrsn1  = hi_char.
    if l-retrsn1 < l-retrsn then l-retrsn1  = l-retrsn.
...
do on error undo, return error on endkey undo, return error:                    
    ...
    PUT UNFORMATTED "#def REPORTPATH=$/FA/xxfarp016" SKIP.
...
for each fa_mstr 
        ...
        and fa_disp_dt  <> ?
        and fa_disp_dt  >= v_date   and fa_disp_dt   <= v_date1
        and fa_disp_rsn >= l-retrsn and fa_disp_rsn  <= l-retrsn1
    no-lock
    break by fa_disp_dt by fa_id :
    ...
    l-yrper      = string(year(fa_disp_dt), "9999") + string(month(fa_disp_dt), "99"). /*报表基准期间*/ 
