/* xxfarp004.p 固定资产与累计折旧汇总表 --- 月固累                        */
/* All rights reserved worldwide.  This is an unpublished work.            */
/* REVISION: 1.0      LAST MODIFIED: 2010/04/30   BY: Apple Tam            */
...
define new shared temp-table temp_fa no-undo
   ...
   field temp_amt12m    as decimal format "->>,>>>,>>>,>>9.99<"
   field temp_amt13m    as decimal format "->>,>>>,>>>,>>9.99<"
   ...
   field temp_amt22m    as decimal format "->>,>>>,>>>,>>9.99<"
   field temp_amt23m    as decimal format "->>,>>>,>>>,>>9.99<"
...
do on error undo, return error on endkey undo, return error:                    
    ...
    PUT UNFORMATTED "#def REPORTPATH=$/FA/xxfarp004" SKIP.
...
for each fa_mstr 
        where fa_domain    = global_domain 
          ...
          /*and fa_id >= "110002" and fa_id <= "110002"*/
    no-lock
    break by fa_id :
    ...
    if avail faba_det then do:
        assign  v_acct = faba_acct v_sub = faba_sub v_cc = faba_cc .
          find first temp_fa where temp_fa_id = fa_id no-error.
          if available temp_fa then do:
             temp_sub = faba_sub.
          end.
       ...
       for each fab_det where fab_domain = global_domain
            ...
            if available temp_fa then do:
             ...
             if string(year(fa_startdt), "9999") = substring(l-yrper1,1,4) 
                and string(month(fa_startdt), "99") = substring(l-yrper1,5,2) 
                and fab_resrv = 0 then do:
                temp_amt12m    = temp_amt12m + fab_amt.
             end.
             /*当年报废(减少）---当月减少*/
             if fa_disp_dt <> ? then do:
                if string(year(fa_disp_dt), "9999") = substring(l-yrper1,1,4) 
                   ...
                   and fab_resrv = 0 then do:
                   temp_amt13m    =  temp_amt13m + fab_amt.
...
/*ss-20110329*/                if substring(temp2_yrper,1,4) <= l-yrper 
             ...
             /*累计折旧-当年增加(当月）*/
                if substring(temp2_yrper,1,4) = substring(l-yrper1,1,4) 
                   ...
                   find first temp_fa where temp_fa_id = fa_id no-error.
                   if available temp_fa then do:
                    temp_amt22m = temp_amt22m + temp2_peramt.
                   end.
                end.
...
for each temp_fa:
    ...
    temp_amt15 = temp_amt11 + temp_amt12 + temp_amt12m - temp_amt13 - temp_amt13m.
    ...
    /*累计折旧-年末数 */
    temp_amt25 = temp_amt21 + temp_amt22 + temp_amt22m - temp_amt23 - temp_amt23m.
    /*净值*/
...
/*        accum temp_amt10 (total by temp_facls_id).*/
        ...
        accum temp_amt12m (total by temp_facls_id).
        accum temp_amt13m (total by temp_facls_id).
        ...
        accum temp_amt22m (total by temp_facls_id).
        accum temp_amt23m (total by temp_facls_id).
...
/*           temp_facls_id
           ...
           temp_yrper   
           temp_disp_dt 
           accum total by temp_facls_id temp_amt10  */ 
           accum total by temp_facls_id temp_amt11   
           accum total by temp_facls_id temp_amt12   
           accum total by temp_facls_id temp_amt12m   
           accum total by temp_facls_id temp_amt13m   
           ...
           accum total by temp_facls_id temp_amt22m   
           accum total by temp_facls_id temp_amt23m   
