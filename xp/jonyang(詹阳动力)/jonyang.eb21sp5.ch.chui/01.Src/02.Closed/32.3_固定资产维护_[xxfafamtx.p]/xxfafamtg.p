...
/* SS - 100705.1  By: Roger Xiao */  /*允许输入负数的调整金额*/
...
with three-d overlay  3 down scroll 1 width 80.
   SS - 100505.1 - E */
/* SS - 100505.1 - B */
define var v_backward as logical format "Yes/No" label "追溯调整" initial No .
define frame f-1
   skip(.4)
   space(2)
   l-type-desc column-label {&fafamtg_p_1}
   ...
   faadj_amt
   faadj_yrper
   v_backward  label "追溯调整"
with three-d overlay  3 down scroll 1 width 80.
/* SS - 100505.1 - E */

/* SS - 110319.1 - B */
define var v_salv_amt    like fab_salvamt    no-undo.
define var v_perfrom     like fabd_yrper .
define var v_per_assign  as logical no-undo .
form 
   "   是否手工指定追溯期间?" v_per_assign "   "
   skip(1)
   "   追溯至期间:" v_perfrom   
with frame perfrom 
row 5 
...
overlay 
no-label.
setFrameLabels(frame perfrom:handle).
...
/* SS - 110319.1 - B */
         if l-error = no and l-type = "2" then do:
            
            /*调整残值*/
            {gprun.i ""xxfafamtgslv.p""
               "(buffer fa_mstr,
                 buffer fab_det,
         ...
         end. /* if l-error = no and l-type = "2" then do: */
/* SS - 110319.1 - E */
...
/* SS - 110319.1 - B */
    ...
    break by fabd_fa_id by fabd_yrper :
        if last-of(fabd_fa_id) then assign v_perfrom = fabd_yrper .
    end.

    view frame perfrom  .
    disp v_perfrom with frame perfrom .

    per-need-or-not:
    repeat with frame perfrom :
        update 
            v_per_assign 
        with frame perfrom.

        if keyfunction(lastkey) = "end-error" then undo per-need-or-not, leave per-need-or-not.

        if v_per_assign then do:
            ...
            repeat with frame perfrom :
                ...
                if keyfunction(lastkey) = "end-error" then undo per-input-loop, leave per-input-loop.

                find first fabd_det 
                    where fabd_domain   = global_domain  
                    and   fabd_fa_id    = fab_det.fab_fa_id  
                    and   fabd_fabk_id  = fab_det.fab_fabk_id 
                    and   fabd_yrper    = v_perfrom  
                    and   fabd_post     = yes        
                no-lock no-error.
                if not avail fabd_det then do:
                    message "错误:期间未过账或不存在,请重新输入" .
                    undo,retry.
                end.
                leave .
            end.
	...
	hide frame perfrom no-pause .
end.  /* if v_backward  = yes */
/* SS - 110319.1 - E */
