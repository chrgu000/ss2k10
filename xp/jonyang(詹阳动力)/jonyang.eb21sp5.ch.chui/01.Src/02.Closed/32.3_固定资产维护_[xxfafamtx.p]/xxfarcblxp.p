...
/* SS - 100505.1 - B */
        /*追溯调整,且指定追溯期间:直线折旧法,且全期间或下期间算法时,*/
        for first famt_mstr
            where famt_mstr.famt_domain = global_domain 
            and  famt_id = fab_famt_id
        no-lock:
        ...
        then do:
            ...
            define var v_mth_post  as decimal .   /*要追溯至的期间的折旧*/
            define var v_per_post  as char    .   /*要追溯至的期间*/
            define var v_mth_post2 as decimal .   /*已过账期间的折旧数*/
            define var v_per_post2 as char    .   /*实际已过账的最后期间*/
            ...
            no-lock 
            break by fabd_fa_id by fabd_yrper :
                if last-of(fabd_fa_id) then assign v_per_post2 = fabd_yrper .
            end.
...
*/ 
            ...
            /*所有四舍五入差异,以及之前期间的追溯计入未过账的第一个期间*/
            for each dep 
                where dep.fabd_yrper > v_per_post2 
                break by dep.fabd_yrper :
                find first fabd_det
                    where fabd_det.fabd_domain = global_domain 
                    and  fabd_det.fabd_fa_id   = fab_fa_id
                    and  fabd_det.fabd_fabk_id = fab_fabk_id
                    and  fabd_det.fabd_yrper   = dep.fabd_yrper 
                    and  fabd_det.fabd_post    = no 
                no-lock no-error .
                if avail fabd_det then do:
