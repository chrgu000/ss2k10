/* xxsoivrp001.p  INVOICES PRINT (BI) -- 限已過賬的發票                                               */
/*----rev history-------------------------------------------------------------------------------------*/
...
{mfdtitle.i "090707.1"}
define new shared variable dev as character format "x(8)" label "輸出".

define var inv              like so_inv_nbr label "發票".
...
define var v_inv_date       as date  label "發票日期".
define var v_inv_date1      as date  label "至".
define var v_group          like pt_group label "組別".
...
repeat:
	...
	do on error undo, return error on endkey undo, return error:   
            ...
            for each temp9 break by t9_inv_nbr :
                v_pause = v_pause + 1 .
                if v_pause > 1 then pause 8 .   /*一定要預留足夠的時間給每筆記錄刷新數据*/
