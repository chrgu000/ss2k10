/* xxsoivpst001.p   每張發票的明細資料 create by roger 090707.1 */
/*----rev history-------------------------------------------------------------------------------------*/
...
form
    ...
    sod_part       label "物料號"
    sod_um         label "UM"
    sod_qty_inv    label "發票數量"
    net_price      label "价格"
    vv_amt         label "金額"  
with frame b down width 80 
title  color normal "發票明細" .   /*發票明細*/
...
for each so_mstr where so_nbr  = v_nbr ,
    each sod_det where sod_nbr = so_nbr and sod_qty_inv <> 0 no-lock :
