...
/*----rev history-------------------------------------------------------------------------------------*/
/* SS - 100914.1  By: Roger Xiao */ /*ship qty limit*/
/*-Revision end---------------------------------------------------------------*/
...
/* SS - 100914.1 - B 
   ...
   SS - 100914.1 - E */
/* SS - 100914.1 - B */
               if c-application-mode <> "API" and l_sod_bo_chg <> 0 then do:
/* SS - 100914.1 - E */
      ...
      do on error undo, retry:
         ...
         then do:
            ...
            then do:
               ...
               {pxmsg.i &MSGNUM=622 &ERRORLEVEL=2}
                   SS - 100914.1 - E */
                /* SS - 100914.1 - B */
               {pxmsg.i &MSGNUM=622 &ERRORLEVEL=3}
                undo,retry.
                /* SS - 100914.1 - E */
            end.
         end.
        ...
        if sod_qty_ord > 0 then do:
            define var v_qty_wo like wo_qty_ord .
            define var v_qty_ld like ld_qty_oh  .
            define var msg-arg  as character format "x(16)" no-undo.
            ...
            for each wo_mstr 
                ...
                no-lock :
                v_qty_wo = v_qty_wo + (wo_qty_ord - wo_qty_comp - wo_qty_rjct) .
            ...
            no-lock no-error.
            v_qty_ld = if avail ld_det then ld_qty_oh else 0.

            if v_qty_wo + v_qty_ld - total_lotserial_qty * trans_conv  < 0
            then do:  /*not consider :same SO,same part,other lines ,this time ship qty */

                /* Quantity available this site/location: */
                msg-arg = string(v_qty_wo + v_qty_ld ).
