/* xxwowomta.p - WORK ORDER MAINTENANCE SUBROUTINE                      */
...
/*                   1. Modify xxwoworle.p to calculate release date    */
/*                   2. If wo_type = "R" then wo_rel_date = wo_due_date */
/* REVISION: 8.5     LAST MODIFIED: 12/19/02    BY: hkm *090815.1*        */
/* 1. Remove If wo_type = "R" then wo_rel_date = wo_due_date            */
/* 2. If wo_status = "F" or "R" then wo_rel_date = wo_due_date - MFG LT */
/* REVISION: 8.5     LAST MODIFIED: 02/10/03    BY: hkm *090815.1*        */
/* 1. If rel_dt = ? and status = F/R, rel_date = input due - (MFG LT+1) */
/* REVISION: 8.5     LAST MODIFIED: 03/18/03    BY: hkm *030318*        */
/* 1. Remove 02/10/03 changes (Ref#090815.1) rel_dt calculations          */
...
do transaction:
  ...
  if available wo_mstr then do:
    if (wo_status = "F" or wo_status = "R")
    and wo_rel_date = ?
    then do:
    find first ptp_det where ptp_domain = global_domain and ptp_part = wo_part
