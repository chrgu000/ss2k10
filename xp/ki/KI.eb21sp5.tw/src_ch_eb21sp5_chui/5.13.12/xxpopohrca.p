/* xxpopohrca.p - PURCHASE ORDER RECEIPT W/ SERIAL NUMBER CONTROL       */
/* xxpoporca.p - PURCHASE ORDER RECEIPT W/ SERIAL NUMBER CONTROL        */
...
/* SSIVAN 08021401 BY:Ivan Yang Date:02/14/08				*/
/* SSIVAN 08021501 BY:Ivan Yang Date:02/15/08				*/
/* SSIVAN 08021701 BY:Ivan Yang Date:02/17/08				*/
/*		1. Default L/S value for Audit Trail format "yyyymmdd-h999"          */
...
define variable chg_attr       like mfc_logical label "Chg Attr" no-undo.
...
/*SSIVAN 08021401 add*/ define variable xpart like pod_part.
/*SSIVAN 08021401 add*/ define variable desc1 like pt_desc1 format "x(49)".
/*SSIVAN 08021401 add*/ define variable record as integer.
/*SSIVAN 08021701 add*/ define variable yyyy as character format "x(4)".
/*SSIVAN 08021701 add*/ define variable mm   as character format "x(2)".
/*SSIVAN 08021701 add*/ define variable dd   as character format "x(2)".
/*SSIVAN 08021701 add*/ define variable seq  as integer format '999'.
/*SSIVAN 08021701 add*/ define variable lot  as character format "x(13)".

/*SSIVAN 08021701 add*/  define temp-table auto-ls no-undo
/*SSIVAN 08021701 add*/         field rct_line   like sr_lineid
/*SSIVAN 08021701 add*/         field rct_lotser like sr_lotser
/*SSIVAN 08021701 add*/  index rctlotser IS UNIQUE rct_lotser.
...
form
/*SSIVAN 08021401 rmk*/ /*  line           colon 15  */
/*SSIVAN 08021401 add*/ line colon 12
/*SSIVAN 08021401 rmk*/ /*   receipt_um     colon 36   */
/*SSIVAN 08021401 add*/  cancel_bo      colon 36     label "Cancel B/O"
   site           colon 54
   location       colon 68 label "Loc"
/*SSIVAN 08021401 rmk*/ /*   lotserial_qty  colon 15  */
/*SSIVAN 08021401 add*/ lotserial_qty  colon 12
   wolot          colon 36 label "ID"
   lotserial      colon 54 label "Lot/Ser"
/*SSIVAN 08021401 rmk*/ /*   packing_qty    colon 15 label "Packing Qty"  */
/*SSIVAN 08021401 add*/ packing_qty    colon 12	 label "Packing Qty"
   woop           colon 36 label "WO Op"
   lotref         colon 54
/*SSIVAN 08021401 rmk*/ /*   cancel_bo      colon 15 label "Cancel B/O"	*/

/*SSIVAN 08021501 rmk*/ /*   pod__qad04[1]  colon 54 label "Supplier Lot" format "x(22)" */
/*SSIVAN 08021401 rmk*/ /*   pod_part       colon 15 /*7.0*/ no-attr-space  */
/*SSIVAN 08021401 add*/   xpart          colon 12 no-attr-space
/*SSIVAN 08021401 add*/   receipt_um     colon 36
/*SSIVAN 08021501 rmk*/ /* /*SSIVAN 08021401 add*/   vendlot        colon 54 format "x(22)"  */
/*SSIVAN 08021501 add*/   pod__qad04[1]  colon 54 label "Supplier Lot" format "x(22)"
/*SSIVAN 08021401 add*/   pod_vpart      colon 14
/*SSIVAN 08021501 rmk*/ /*   multi_entry    colon 54   */
/*SSIVAN 08021501 add*/   multi_entry    colon 58
   chg_attr       colon 73  
/*SSIVAN 08021401 rmk*/ /*   pod_vpart      /*7.0 colon 54 */ */
/*SSIVAN 08021401 rmk*/ /*   colon 15			     */
   no-attr-space
/*SSIVAN 08021401 add*/   desc1          colon 12
/*SSIVAN 08021401 rmk*/ /*   cmmt_yn        colon 54 label "Cmmts"    */
/*SSIVAN 08021401 add*/   cmmt_yn        colon 73 label "Cmmts"
...
/*SSIVAN 08021701 add*/ /* INITIALIZE VARIABLES */
/*SSIVAN 08021701 add*/   yyyy = string(year(eff_date)).
/*SSIVAN 08021701 add*/   if int(string(month(eff_date))) < 10
/*SSIVAN 08021701 add*/      then  mm = "0" + string(month(eff_date)).
/*SSIVAN 08021701 add*/      else  mm = string(month(eff_date)).
/*SSIVAN 08021701 add*/   if int(string(day(eff_date))) < 10
/*SSIVAN 08021701 add*/      then  dd = "0" + string(day(eff_date)).
/*SSIVAN 08021701 add*/      else  dd = string(day(eff_date)).
...
/*SSIVAN 08021401 add*/                xpart	 
	 with frame d
            ...
            nppoddet:
            repeat:
/*SSIVAN 08021401 add*/            if frame-field = "line" then do:
               ...
               if recno <> ? then do:
                  ...
                  if not available si_mstr
                     ...
                     next nppoddet.
/* SS - 090917.1 - B 
    /*SSIVAN 08021401 add*/                   find first pt_mstr where pt_part = pod_part 
    /*SSIVAN 08021401 add*/                      no-lock no-error.
   SS - 090917.1 - E */
/* SS - 090917.1 - B */
                    find first pt_mstr where pt_domain = global_domain and pt_part = pod_part  no-lock no-error.                         
/* SS - 090917.1 - E */
                  run display-detail.
/*SSIVAN 08021401 add*/                  xpart = pod_part.
...
/*SSIVAN 08021401 add*/               end. /* if frame-field = "line" */ 
/*SSIVAN 08021401 add*/               else do:
/*SSIVAN 08021401 add*/                  readkey.
/*SSIVAN 08021401 add*/                  apply lastkey.
/*SSIVAN 08021401 add*/               end.
               leave.
            end.  /* NPPODDET: REPEAT: */
...
/*SSIVAN 08021401 add*/         assign
/*SSIVAN 08021401 add*/            vendlot = ""
/*SSIVAN 08021401 add*/            lotnext = ""
/*SSIVAN 08021401 add*/            newlot  = "".

         if (line = 0) then do:         /* NO PO LINE SELECTED */

/*SSIVAN 08021401 add*/         if xpart = "" then leave lineloop.
/*SSIVAN 08021401 add*/         else do:
/*SSIVAN 08021401 add*/              clear frame c all no-pause.
/*SSIVAN 08021401 add*/              view frame c.
/*SSIVAN 08021401 add*/              record = 0.
/*SSIVAN 08021401 add*/              for each pod_det no-lock where pod_det.pod_domain = global_domain 
/*SSIVAN 08021401 add*/			      and pod_nbr = po_nbr
/*SSIVAN 08021401 add*/                       and pod_part = xpart
/*SSIVAN 08021401 add*/                       and pod_status <> "C"
/*SSIVAN 08021401 add*/                       and pod_status <> "X"
/*SSIVAN 08021401 add*/              use-index pod_nbrln by pod_line:
/*SSIVAN 08021401 add*/                if porec then do:
/*SSIVAN 08021401 add*/                  if pod_rma_type <> "I" and
/*SSIVAN 08021401 add*/                     pod_rma_type <> "" then next.
/*SSIVAN 08021401 add*/                end.
/*SSIVAN 08021401 add*/                else do:
/*SSIVAN 08021401 add*/                  if pod_rma_type <> "O" then next.
/*SSIVAN 08021401 add*/                end.
/*SSIVAN 08021401 add*/              find si_mstr where si_mstr.si_domain = global_domain and si_site = pod_site no-lock no-error.
/*SSIVAN 08021401 add*/              if available si_mstr and si_db = global_db then do:
/*SSIVAN 08021401 add*/                 find pt_mstr where pt_mstr.pt_domain = global_domain and pt_part = pod_part
/*SSIVAN 08021401 add*/                 no-lock no-error.
/*SSIVAN 08021401 add*/                    if porec then
/*SSIVAN 08021401 add*/                       assign qopen = pod_qty_ord - pod_qty_rcvd.
/*SSIVAN 08021401 add*/                    else
/*SSIVAN 08021401 add*/                       assign qopen = - (pod_qty_ord - pod_qty_rcvd).

/*SSIVAN 08021401 add*/                    if pod_sched then do:
/*SSIVAN 08021401 add*/                       {gprun.i ""rsoqty.p"" "(input recid(pod_det), input eff_date, output qopen)"}
/*SSIVAN 08021401 add*/                    end.

/*SSIVAN 08021401 add*/                    if record = 0 then record = recid(pod_det).
/*SSIVAN 08021401 add*/                       display pod_line pod_part
/*SSIVAN 08021401 add*/                               pt_um when available pt_mstr
/*SSIVAN 08021401 add*/                               qopen
/*SSIVAN 08021401 add*/                               pod_um
/*SSIVAN 08021401 add*/                               pod_qty_chg
/*SSIVAN 08021401 add*/                               pod_rum
/*SSIVAN 08021401 add*/                               pod_project pod_due_date pod_type
/*SSIVAN 08021401 add*/                       with frame c.
/*SSIVAN 08021401 add*/                       if frame-line(c) = frame-down(c) then leave.
/*SSIVAN 08021401 add*/                       down 1 with frame c.
/*SSIVAN 08021401 add*/              end.    /* available si_mstr */
/*SSIVAN 08021401 add*/              end.    /* for each pod_det */
/*SSIVAN 08021401 add*/		     if record <> 0 then 
/*SSIVAN 08021401 add*/              find pod_det where recid(pod_det) = record no-error.
/*SSIVAN 08021401 add*/         end.         /* else do */
/*SSIVAN 08021401 rmk begin*/ /*
...
*/ /*SSIVAN 08021401 rmk end*/ 
         end.     /* IF LINE = 0 */
/*SSIVAN 08021401 add*/         else         /* not line = 0 */

/*SSIVAN 08021401 rmk*/   /*       assign      */
/*SSIVAN 08021401 rmk*/   /*            vendlot = "" */
/*SSIVAN 08021401 rmk*/   /*            lotnext = "" */
/*SSIVAN 08021401 rmk*/   /*            newlot  = "".  */
...
/*SSIVAN 08021401 rmk*/         /* if return-value = {&SUCCESS-RESULT} then do:	 */
/*SSIVAN 08021401 add*/           if (return-value = "" and available pod_det ) or return-value = {&SUCCESS-RESULT} then do:
/*SSIVAN 08021401 add*/            line = pod_line.
/*SSIVAN 08021401 add*/            xpart = "".
/*SSIVAN 08021401 add*/            /* WARNING: RECEIVED ITEM IS MEMO TYPE */ 
/*SSIVAN 08021401 add*/            if pod_type = "M" then do: 
/*SSIVAN 08021401 add*/               bell.
/*SSIVAN 08021401 rmk*/              /*  message "警告: 无库存收料.".	*/
/*SSIVAN 08021401 add*/		     {pxmsg.i &MSGNUM     = 9033 &ERRORLEVEL = 2}
/*SSIVAN 08021401 add*/            end.
/*SSIVAN 08021401 add*/            if pod_type = "H" then do:
/*SSIVAN 08021401 add*/               bell.
/*SSIVAN 08021401 rmk*/             /*  message "错误: 料品管制中, 采购单不允许收料.".  */
/*SSIVAN 08021401 add*/		     {pxmsg.i &MSGNUM     = 9034 &ERRORLEVEL = 3}
/***        CANNOT RECEIVE AN ON-HOLD ITEM !".**/
/*SSIVAN 08021401 add*/               undo, retry.
/*SSIVAN 08021401 add*/            end.
...
/*SSIVAN 08021401 add*/      desc1 = "".
/*SSIVAN 08021401 add*/      find first pt_mstr where pt_mstr.pt_domain = global_domain and pt_part = pod_part no-lock no-error.
/*SSIVAN 08021401 add*/      if available pt_mstr then do:
/*SSIVAN 08021401 add*/          if pt_desc1 <> "" then desc1 = pt_desc1.
/*SSIVAN 08021401 add*/                            else desc1 = pt_desc2.
/*SSIVAN 08021401 add*/      end.
      if c-application-mode <> "API" then
         display
            line
...
/*SSIVAN 08021701 add*/       if lotserial = "" then do:
/*SSIVAN 08021701 add*/        find last auto-ls no-lock no-error.
/*SSIVAN 08021701 add*/        if available auto-ls then do:
/*SSIVAN 08021701 add*/            seq = int(substring(rct_lotser,11,3)) + 1.
/*SSIVAN 08021701 add*/            if seq < 10
/*SSIVAN 08021701 add*/               then lot = "00" + string(seq).
/*SSIVAN 08021701 add*/            else if seq < 100
/*SSIVAN 08021701 add*/               then lot = "0" + string(seq).
/*SSIVAN 08021701 add*/            else lot = string(seq).
/*SSIVAN 08021701 add*/        end.
/*SSIVAN 08021701 add*/        else do:
/*SSIVAN 08021701 add*/         find last tr_hist use-index tr_eff_trnbr
/*SSIVAN 08021701 add*/         where tr_hist.tr_domain  = global_domain and tr_effdate = eff_date
/*SSIVAN 08021701 add*/         and tr_type = "RCT-PO"
/*SSIVAN 08021701 add*/         and tr_serial begins yyyy + mm + dd + "-h"
/*SSIVAN 08021701 add*/         no-lock no-error.
/*SSIVAN 08021701 add*/         if available tr_hist then do:
/*SSIVAN 08021701 add*/            seq = int(substring(tr_serial,11,3)) + 1.
/*SSIVAN 08021701 add*/            if seq < 10
/*SSIVAN 08021701 add*/               then lot = "00" + string(seq).
/*SSIVAN 08021701 add*/            else if seq < 100
/*SSIVAN 08021701 add*/               then lot = "0" + string(seq).
/*SSIVAN 08021701 add*/            else lot = string(seq).
/*SSIVAN 08021701 add*/         end.
/*SSIVAN 08021701 add*/         else lot = "001".
/*SSIVAN 08021701 add*/        end.
/*SSIVAN 08021701 add*/         display yyyy + mm + dd + "-h" + lot @ lotserial
/*SSIVAN 08021701 add*/         with frame d.
/*SSIVAN 08021701 add*/       end.
/*SSIVAN 08021701 add*/         if int(lot) > 999 then do:
/*SSIVAN 08021701 add*/            {mfmsg.i 2729 3} /* SEQUENCE NUMBER EXCEEDS MAX */
/*SSIVAN 08021701 add*/            next-prompt lotserial with frame d.
/*SSIVAN 08021701 add*/            undo, retry.
/*SSIVAN 08021701 add*/         end.
            ...
            set lotserial_qty
               packing_qty
               cancel_bo
/*SSIVAN 08021401 rmk*/     /*          receipt_um   */
               wolot
               woop
/*SSIVAN 08021401 add*/            receipt_um
...
/*SSIVAN 08021701 add*/         if lotserial = "" then do:
/*SSIVAN 08021701 add*/            {mfmsg.i 40 3}
/*SSIVAN 08021701 add*/            next-prompt lotserial with frame d.
/*SSIVAN 08021701 add*/            undo, retry.
/*SSIVAN 08021701 add*/         end.
/*SSIVAN 08021701 add*/       if newlot = "" or newlot <> lotserial then do:
/*SSIVAN 08021701 add*/         find first auto-ls where /** rct_line <> string(line)
/*SSIVAN 08021701 add*/                              and **/ rct_lotser = input lotserial
/*SSIVAN 08021701 add*/         no-lock no-error.
/*SSIVAN 08021701 add*/         if not available auto-ls then do:
/*SSIVAN 08021701 add*/           create auto-ls.
/*SSIVAN 08021701 add*/           assign rct_line = string(line)
/*SSIVAN 08021701 add*/                  rct_lotser = input lotserial.
/*SSIVAN 08021701 add*/         end.
/*SSIVAN 08021701 add*/         else do:
/*SSIVAN 08021701 add*/           {mfmsg.i 2720 3} /* RECEIPT EXISTS FOR THIS LOT */
/*SSIVAN 08021701 add*/           next-prompt lotserial with frame d.
/*SSIVAN 08021701 add*/           undo, retry.
/*SSIVAN 08021701 add*/         end.
/*SSIVAN 08021701 add*/       end.
/*SSIVAN 08021701 add*/         find last tr_hist
/*SSIVAN 08021701 add*/         where tr_hist.tr_domain = global_domain and tr_serial = input lotserial
/*SSIVAN 08021701 add*/         and tr_type = "RCT-PO"
/*SSIVAN 08021701 add*/         no-lock no-error.
/*SSIVAN 08021701 add*/         if available tr_hist then do:
/*SSIVAN 08021701 add*/           {mfmsg.i 2720 3} /* RECEIPT EXISTS FOR THIS LOT */
...
------------------------------------------------------------------------------*/
   ...
   for first po_mstr
      fields( po_domain po_nbr
             po_vend)
          where po_mstr.po_domain = global_domain and  po_nbr = pod_nbr no-lock:
   end. /* for first po_mstr where po_nbr = pod_nbr no-lock: */

   for first wo_mstr
      fields( wo_domain wo_lot
             wo_nbr
             wo_site)
          where wo_mstr.wo_domain = global_domain and  wo_lot = ip_wo_id
         and wo_nbr = "" no-lock:
...
/*SSIVAN 08021401 add*/                  @ xpart
      ...
      pod_det.pod_op      @ woop
/*SSIVAN 08021401 add*/                  pt_mstr.pt_desc1 when available pt_mstr @ desc1 
/*SSIVAN 08021401 add*/                  ""       when not available pt_mstr @ desc1
   with frame d.
...
PROCEDURE p_validateTransaction:
   ...
   then do:
      ...
      for first loc_mstr
         fields (loc_site loc_loc loc_status loc_domain)
      ...
      then do:
         ...
         for first si_mstr
            fields (si_site si_status si_auto_loc si_db si_entity si_domain)
   ...
   for first pt_mstr
      fields (pt_domain  pt_auto_lot pt_loc     pt_lot_grp pt_desc1 pt_um
              pt_lot_ser pt_part     pt_status  pt_desc2)
      where pt_mstr.pt_domain = global_domain
      and   pt_part           = p_part no-lock:
   end. /*FOR FIRST pt_mstr*/
   ...
   then do:
      ...
      then do:
         ...
         then do:
            ...
            {pxmsg.i &MSGNUM=7085 &ERRORLEVEL=2
                     &MSGARG1=p_transtype
                     &MSGARG2=p_site
                     &MSGARG3=p_location}
         end. /* IF p_multi_ent */
   ...
   then do:
      ...
      then do:
         ...
         then do:
            ...
            {pxmsg.i &MSGNUM=2707 &ERRORLEVEL=3}
            p_undotran = yes.
            undo, retry.
         end. /*IF p_ponbr <> ""*/
