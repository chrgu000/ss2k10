...
for each temp1 where t1_ok = yes no-lock,
    each ih_hist  where ih_domain = global_domain and  ih_inv_nbr  = t1_inv_nbr and ih_nbr  = t1_nbr no-lock,
    each idh_hist where idh_domain = global_domain and idh_inv_nbr = ih_inv_nbr and idh_nbr = ih_nbr no-lock
