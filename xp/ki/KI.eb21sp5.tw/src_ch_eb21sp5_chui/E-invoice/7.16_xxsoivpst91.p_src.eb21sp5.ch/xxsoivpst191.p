...
/* Revision: 1.117.2.1  BY: Dayanand Jethwa     DATE: 02/24/05  ECO: *P27M*   */
/* Revision: 1.117.2.2  BY: Karel Groos         DATE: 04/01/05   ECO: *P2BV*  */
/* Revision: 1.117.2.3  BY: Ken Casey           DATE: 02/22/05   ECO: *P38T*  */
/* Revision: 1.117.2.4  BY: Manjusha Inglay     DATE: 11/29/05   ECO: *P49C*  */
/* Revision: 1.117.2.5  BY: Dinesh Dubey        DATE: 01/09/06   ECO: *P3HZ*  */
/* $Revision: 1.117.2.6 $         BY: Nishit V            DATE: 02/02/06   ECO: *P4GY*  */
...
/* SS - 090707.1 - B 
   ...
   repeat:
      ...
      if w-first-pass then do:
         ...
         for first somstr
            fields (so_domain so_ar_acct so_ar_sub so_ar_cc
                    so_bill so_cr_card so_cr_init so_curr
                    so_cust so_disc_pct so_exru_seq so_ex_rate
...
for each temp1 where t1_ok = yes :
        ...
        find first somstr 
            ...
            where so_domain = global_domain and so_inv_nbr = t1_inv_nbr
            and  so_nbr = t1_nbr 
            and (so_invoiced = yes)
            and (so_to_inv = no)
        no-lock no-error.
            ...
            break by so_inv_nbr:
               ...
               if first-of(so_inv_nbr) then do:
                  ...
                  then do:
                     ...
                     for each somstr2
                        fields (so_domain so_inv_nbr)
                        where somstr2.so_domain  = global_domain
                        and   somstr2.so_inv_nbr = so_mstr.so_inv_nbr
                        no-lock :
                        if so_mstr.so_fsm_type = "FSM-RO"
                        then
                           l_tr_type = "38" .
               ...
               /* GET ORDER DETAIL  */
               for each sod_det
                  fields (sod_domain sod_acct sod_sub sod_cc sod_crt_int
                          sod_line sod_list_pr sod_nbr sod_part sod_price
                          sod_pst sod_qty_inv sod_qty_ord sod_qty_ship
                          sod_site sod_std_cost sod_taxable sod_taxc
                          sod_qty_item sod_qty_per sod_tax_in sod_um
                          sod_desc sod_sched)
                   where sod_domain = global_domain
                    and  sod_nbr = so_nbr
                    and  sod_line > 0
               ...
               break by sod_line with frame e width 132:
                 ...
                 /* INORDER TO MATCH THE INVOICE TOTAL WITH DR/CR MEMO TOTAL */
                  ...
                  {gprun.i ""soivpstb.p"" "(input-output l_addtax2)"}
               ...
               /* ADDED SEVENTH INPUT PARAMETER p_last_line TO ACCOMODATE */
               /* THE LOGIC INTRODUCED IN gpcurcnv.i FOR HANDLING         */
               /* ROUNDING ISSUES                                         */
               ...
               assign
                      ...
                      undo_all  = no
                      ord_amt   = if last-of(so_inv_nbr) then
                                     invtot_ord_amt
                                  else 0.
...
/*---------------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------------*/
   ...
   /* Capture and Authorize amount by checking whether socc_mstr exists     */
   /*   and cctr_hist.cctr_tx_type = "SALE"                                 */
   ccOrder = no.
   if can-find(first socc_mstr where socc_domain = global_domain
                                 and socc_nbr = cur-sonbr no-lock)
   then
      ccOrder = yes.
   if ccOrder then do:
      if can-find(first cctr_hist where cctr_domain = global_domain
                                   and  cctr_nbr    = cur-sonbr
                                   and cctr_tx_type = "SALE" no-lock)
      then
         ccOrder = yes.
      else
         ccOrder = no.
      ...
      if ccOrder then do :
         for first cctr_hist
            where cctr_domain = global_domain
             and  cctr_nbr    = cur-sonbr
             and cctr_tx_type = "SALE"
         exclusive-lock:
            cctr_inv_nbr = promo_inv.
         end.
      end.
   end.
   assign
      pCardProcessed = ccOrder
      pSuccess       = ccOrder.

   /* QAD CSS does not use soivccpy.p. Credit check is done in CSS.
      To avoid "Unreferenced" error, we have to leave the code in place. But
   ...
   */
   ...
   {mfnctrl.i "arc_ctrl.arc_domain = global_domain" "arc_ctrl.arc_domain"
      "ar_mstr.ar_domain = global_domain" arc_ctrl arc_memo ar_mstr ar_check
      vparnbr}
   ...
   for first invcmemo_ar_mstr
      fields (ar_domain ar_curr ar_nbr ar_acct ar_sub
              ar_cc ar_entity ar_type ar_amt ar_applied
   ...
   {mfnctrl.i "arc_ctrl.arc_domain = global_domain" "arc_ctrl.arc_domain"
      "glt_det.glt_domain = global_domain" arc_ctrl arc_jrnl glt_det glt_ref jrnl}
...
PROCEDURE p_sixref_balance:
   ...
   for first armstr
      fields( ar_domain ar_nbr ar_bill ar_amt)
       where armstr.ar_domain = global_domain
        and  armstr.ar_nbr = i_invnbr
      no-lock:
   ...
   for first ihhist
      fields( ih_domain ih_inv_nbr ih_nbr)
       where ihhist.ih_domain = global_domain
        and  ihhist.ih_inv_nbr = armstr.ar_nbr
        and not can-find(first sixref
           where sixref.six_domain  = global_domain
            and  sixref.six_bill    = armstr.ar_bill
   ...
   then do:
      ...
      for each idhhist
         fields( idh_domain idh_inv_nbr idh_qty_inv idh_nbr idh_line)
          where idhhist.idh_domain = global_domain
           and  idhhist.idh_inv_nbr = armstr.ar_nbr
      no-lock:
         l_idh_qty_inv = l_idh_qty_inv + idh_qty_inv.

         /* TOTAL ALL INCLUSIVE TAX AMOUNTS */
         for each tx2ddet
            fields( tx2d_domain tx2d_ref tx2d_nbr tx2d_line tx2d_tr_type
      ...
      for each sixref
         fields( six_domain six_bill six_inv_nbr six_amt six_order six_qty
...
PROCEDURE p-adjustments:
   ...
   then do:
      ...
      {glenacex.i &entity=l_glt_so_entity
                  &type='"CR"'
                  &module='"AR"'
                  &acct=acct
                  &sub=sub
      ...
      {glenacex.i &entity=l_glt_ar_entity
                  &type='"DR"'
                  &module='"AR"'
                  &acct=acct
                  &sub=sub
