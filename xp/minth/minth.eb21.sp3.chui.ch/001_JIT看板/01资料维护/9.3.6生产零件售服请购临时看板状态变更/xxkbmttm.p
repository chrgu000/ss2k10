/* xxkbmttm.p    mfg temp kb status batch change                            */
...
repeat:
    ...
    do on error undo, return error on endkey undo, return error:                    
        ...
        for each xkb_mstr  where xkb_domain = global_domain and xkb_site = site and ( xkb_part >= part and ( xkb_part <= part1 or part1 = "" )) 
                           and xkb_type = "S" and xkb_status = v_stat_from  and ( xkb_kb_id >= id_from and xkb_kb_id <= id_to )  exclusive-lock , 
            each pt_mstr   where pt_domain = global_domain and ( pt_part = xkb_part ) and ( pt_pm_code = "" or pt_pm_code = "m" or pt_pm_code = "l" )
                           and ( pt_part_type >= v_type and (pt_part_type <= v_type1 or v_type1 = ""))
                           and ( pt_prod_line  >= v_line and ( pt_prod_line <= v_line1 or v_line1 = "" )) 
                           and ( pt_group >= v_group and (pt_group <= v_group1 or v_group1 = "" )) no-lock  break by xkb_part:
            
            find first xmpt_mstr where xmpt_domain  = global_domain and xmpt_site = xkb_site and xmpt_part = xkb_part no-lock no-error .
            ...
            {xxkbhist.i &type="xkb_type"      &part="xkb_part"      &site="xkb_site"
                        &kb_id="xkb_kb_id"    &effdate=today        &program="'xxkbmttm.p'"
                        &qty="xkb_kb_qty"     &ori_qty="xkb_kb_Raim_qty" &tr_trnbr=0
                        &b_status="v_stat_from"       &c_status="v_stat_to"
                        &rain_qty="xkb_kb_raim_qty"}


            if last-of(xkb_part) then  disp pt_part pt_desc1 pt_desc2 /* i label "状态变更张数" */ with stream-io width 100 .
