/* xxkbptcr.p   Pur kb batch ctreate                                        */
...
repeat:
    ...
    do on error undo, return error on endkey undo, return error:     
        ...
        for each xppt_mstr where xppt_domain  = global_domain and xppt_site = site and ( xppt_part >= part and ( xppt_part <= part1 or part1 = "") ) exclusive-lock,
            ...
            if v_yn then assign v_qty_need = xppt_kb_suggest .
            v_total = 0 .
            v_kb_id = 0 .
            i = 0 .
            ...
            for each xkb_mstr where xkb_domain  = global_domain and xkb_site = xppt_site and xkb_part = xppt_part and xkb_type = "p"
                                      and ( xkb_kb_id <> 000 and xkb_kb_id <> 999 ) no-lock break by xkb_kb_id:
                v_total = v_total + 1 .
            ...
            xppt_kb_avail = i .
            xppt_kb_number = v_total .
            v_kb_old = xppt_kb_number .
            ...
            if v_total < v_qty_need  then do:
                    ...
                    for each xkb_mstr where xkb_domain  = global_domain and xkb_site = xppt_site and xkb_part = xppt_part and xkb_type = "p"
                        ...
                        if xkb_status = "D"  then do:
                            ...
                            {xxkbhist.i &type="xkb_type"      &part="xkb_part"      &site="xkb_site"
            			                &kb_id="xkb_kb_id"    &effdate=today        &program="'xxkbptcr.p'"
                   ...
                   if  v_total < v_qty_need  then do:
                    ...
                    assign xppt_kb_avail = v_qty_need
                           xppt_kb_number =   max(v_qty_need,v_kb_old)   .
            end.   /* if xppt_kb_avail < v_qty_need */
            else do:
                
                    /*有多则让序号从大到小的顺序把空的有效的看板进行失效*/
                    v_qty_chg = v_qty_need . 
                    for each xkb_mstr where xkb_domain  = global_domain and xkb_site = xppt_site and xkb_part = xppt_part and xkb_status = "A" and xkb_kb_raim_qty = 0 
                                      and ( xkb_kb_id <> 000 and xkb_kb_id <> 999 )  and xkb_type = "P" exclusive-lock by xkb_kb_id desc :
                        if v_qty_chg >= xppt_kb_avail then next .
                    ...
                    if v_qty_chg < xppt_kb_avail then do:
                        for each xkb_mstr where xkb_domain  = global_domain and xkb_site = xppt_site and xkb_part = xppt_part and xkb_status = "A" and xkb_kb_raim_qty <> 0 
                                          and ( xkb_kb_id <> 000 and xkb_kb_id <> 999 )  and xkb_type = "P" exclusive-lock by xkb_kb_id  :
                            if v_qty_chg >= xppt_kb_avail then next .
                            assign   xkb_status = "D"    v_qty_chg = v_qty_chg + 1.
                            {xxkbhist.i &type="xkb_type"      &part="xkb_part"      &site="xkb_site"
        			                &kb_id="xkb_kb_id"    &effdate=today        &program="'xxkbptcr.p'"
                    ...
                    end.  /* if v_qty_chg > xppt_kb_avail */
    
                    assign xppt_kb_avail = v_qty_need  .

            end.  /* if xppt_kb_avail < v_qty_need */

            v_add = if ( xppt_kb_number - v_kb_old ) > 0 then ( xppt_kb_number - v_kb_old ) else 0 .
