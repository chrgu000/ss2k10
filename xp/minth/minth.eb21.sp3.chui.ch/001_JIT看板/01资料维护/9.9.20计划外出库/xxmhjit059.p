...
define var v_qty_req like xkb_kb_raim_qty .
define var v_qty_tmp like xkb_kb_raim_qty .
define var v_qty_oh  like xkb_kb_raim_qty .
define var v_raim_qty like xkb_kb_qty .
...
/*xp001*/
find first xdn_ctrl where xdn_ctrl.xdn_domain = global_domain and xdn_type = "UO" no-lock no-error.
if avail xdn_ctrl then do:
    ordernbr  = "UO" + string(xdn_next,"999999") .
    find first tr_hist where tr_domain = global_domain and tr_nbr = ordernbr no-lock no-error .
    if avail tr_hist then do:
        repeat:
            ordernbr = "UO" + string(inte(substring(ordernbr , 3 , length(ordernbr) - 2 )) + 1 ,"999999") .
            find next tr_hist where tr_domain = global_domain and tr_nbr = ordernbr no-lock no-error .
            if not avail tr_hist then leave .
        end.
    end.
end.
else do:
    message "计划外出库单号控制档不存在,请维护" view-as alert-box.
    leave .
end.
...
repeat:
    ...
    update ordernbr with frame a .
    if ordernbr = "" or not( ordernbr begins "UO" )then do:
        message  "单号不允许为空,且仅限以'UO'开头." view-as alert-box .
   ...
   repeat on endkey undo mainloop, retry mainloop:
      ...
      do on error undo ,retry :
          ...
          if ( avail xmpt_mstr ) or  ( avail xppt_mstr ) then do:
              ...
              v_qty_oh  = 0 .
              for each xkb_mstr where xkb_domain = global_domain and xkb_site = site and xkb_part = pt_part and xkb_status = "U"
                                and xkb_loc = location and xkb_lot = lotserial and xkb_ref = lotref no-lock:
                  v_qty_oh = v_qty_oh + xkb_kb_raim_qty .
              end.
              if v_qty_oh >= lotserial_qty then do:

                  v_qty_oh = lotserial_qty .
                  v_qty_tmp = 0.
                  repeat:
                          if v_qty_oh <= 0 then leave .

                          update v_type v_id with frame a.

                          find xkb_mstr where xkb_domain = global_domain and xkb_site = site and xkb_part = pt_part and xkb_status = "U"
                                        and xkb_loc = location and xkb_lot = lotserial and xkb_ref = lotref
                                        and xkb_type = v_type and xkb_kb_id = v_id  no-lock no-error .
                          if avail xkb_mstr then do:
                              find first tmpkb where tmp_type = xkb_type and tmp_site = xkb_site and tmp_part = xkb_part and tmp_id = v_id no-lock no-error .
                              if avail tmpkb then do:
                                   message "不可重复输入同一看板" view-as alert-box .
                                   undo ,retry .
                              end.
                              else do:
                                  v_qty_tmp =  min(xkb_kb_raim_qty,v_qty_oh) .
                                  v_qty_oh = v_qty_oh - v_qty_tmp .
                              ...
                              if v_qty_oh > 0 then message "请继续输入下一张看板,欠缺量:" + string(v_qty_oh ) .
                             

                          end.
                          else do:
                              message "无看板明细记录." view-as alert-box .
                              undo ,retry .
                          end.    
                  end. /* repeat*/
                  ...
                  message "对应库位/批序号的看板总余量:" + string(v_qty_oh) + "不足本次出库数量."  .
                  undo mainloop, retry mainloop .
              end.

          end.   /* if ( avail xmpt_mstr ) or  ( avail xpp */
      end.  /*kbloop */
      /*xp001*/
...
/*                ordernbr */
   ...
   {gprun.i ""icintra.p"" "(shipnbr, ship_date, inv_mov, 0, true)" }
           ...
           do transaction:
                for each tmpkb exclusive-lock:
                    v_raim_qty = 0 .
                    find xkb_mstr where xkb_domain = global_domain and xkb_type = tmp_type  and xkb_site = tmp_site and xkb_part = tmp_part and xkb_kb_id = tmp_id and xkb_status = "U" exclusive-lock no-error  .
                    if avail xkb_mstr then do:
                        v_raim_qty =  xkb_kb_raim_qty .
                        assign xkb_kb_raim_qty = xkb_kb_raim_qty - tmp_qty  xkb_upt_Date = today .
        
                        find last tr_hist where tr_domain = global_domain and tr_type = transtype and tr_nbr = ordernbr and tr_part = xkb_part and tr_effdate = eff_date no-lock no-error.
                        v_trnbr = if avail tr_hist then tr_trnbr else 0 .
        
                        {xxkbhist.i &type="xkb_type"      &part="xkb_part"      &site="xkb_site"
                                    &kb_id="xkb_kb_id"    &effdate= today        &program="'mhjit059.p'"
                ...
                find first xdn_ctrl where xdn_ctrl.xdn_domain = global_domain and xdn_type = "UO" exclusive-lock no-error.
                xdn_next = if avail xdn_ctrl then string(inte(substring(ordernbr , 3 , length(ordernbr) - 2 )) + 1 ,"999999") else string(1 ,"999999") .
           end.
            /*xp001*/
