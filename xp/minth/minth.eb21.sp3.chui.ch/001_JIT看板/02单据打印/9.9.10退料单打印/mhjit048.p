...
define variable nbrr  as character format "x(10)" label "退料单号".
define variable nbrr1 like nbrr.
define variable effdate as date label "退料日期" .
...
form
   ...
   nbrr                        colon 20 column-label "退料单"
   nbrr1       label {t001.i}  colon 49 skip
   effdate                    colon 20 column-label "退料日期"
...
/*    if effdate1 = effdate then effdate1 = ? . */
  ...
  if available xdn_ctrl then do:
	   ...
	   xdnisover = xdn_isover.
    	...
    	message "退料单格式没有设置, 请重新设置后再打印!!"  view-as alert-box.
    	undo, retry.
	end.
...
for each tr_hist use-index tr_nbr_eff  where tr_domain = global_domain and tr_type = "rct-tr" 
                 ...
                 and tr_nbr >= nbrr and ( tr_nbr <= nbrr1 or nbrr1 = "" ) and tr_nbr begins "RT"
