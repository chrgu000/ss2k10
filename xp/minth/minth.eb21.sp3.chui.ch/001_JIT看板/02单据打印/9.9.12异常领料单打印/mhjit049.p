...
define variable nbrr  as character format "x(10)" label "异常领料单号".
...
define variable issued as character format "x(30)" extent 4 initial "__________".
...
form
   ...
   nbrr                        colon 20 column-label "异常领料单"
...
/*    if effdate1 = effdate then effdate1 = ? . */
  ...
  if available xdn_ctrl then do:
	   ...
	   xdnisover = xdn_isover.
    	...
    	message "异常领料单格式没有设置, 请重新设置后再打印!!"  view-as alert-box.
    	undo, retry.
	end.
...
for each tr_hist  use-index tr_type where tr_domain = global_domain and tr_type = "rct-tr" and tr_nbr begins "LY"
                 and (tr_effdate >= effdate or effdate = ?) and (tr_effdate <= effdate1 or effdate1 = ? )
                 and tr_nbr >= nbrr and ( tr_nbr <= nbrr1 or nbrr1 = "" ) and tr_addr > ""
