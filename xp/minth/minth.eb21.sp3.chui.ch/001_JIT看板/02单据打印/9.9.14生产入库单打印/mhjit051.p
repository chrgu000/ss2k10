...
define variable linne   like op_wkctr label "工作中心".
...
define variable issued as character format "x(30)" extent 4 initial "__________" label "".
...
form
   ...
   nbrr        label "入库单号"       colon 20 
   nbrr1       label {t001.i}         colon 49 skip
   effdate     label "入库日期"       colon 20 
...
/*    if effdate1 = effdate then effdate1 = ? . */
  ...
  find first xdn_ctrl where xdn_ctrl.xdn_domain = global_domain and xdn_type = "MI"  no-lock no-error.
  if available xdn_ctrl then do:
	   ...
	   xdnisover = xdn_isover.
    	...
    	message "入库单格式没有设置, 请重新设置后再打印!!"  view-as alert-box.
    	undo, retry.
	end.
...
for each tr_hist  use-index tr_nbr_eff  where tr_domain = global_domain and tr_type = "rct-tr" 
                 ...
                 and tr_nbr >= nbrr and ( tr_nbr <= nbrr1 or nbrr1 = "" ) and tr_nbr begins "MI"
    ...
    if first-of(tr_nbr) then do:
        ...
        find loc_mstr no-lock  where loc_domain = global_domain and loc_loc = tr_addr no-error.
        desc2 = if avail loc_mstr then loc_desc else "" .
        rcvno = tr_nbr .
        prodline = tr_addr .
