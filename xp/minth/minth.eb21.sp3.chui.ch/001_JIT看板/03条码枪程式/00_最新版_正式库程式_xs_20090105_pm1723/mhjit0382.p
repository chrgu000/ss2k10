/* mhjit0382.p  看板领料: 有领料看板无采购看板  for barcode                          */
...
define shared variable execname as character .  execname = "mhjit0382.p".
...
/*                 if v1100 <> ""  then do: */
                    ...
                    if not avail loc_mstr then do:
                        disp "无效库位" @ WMESSAGE NO-LABEL with fram F1100.  
                        pause 0.  
                        undo, retry.
                    end.
                    else if v1100 = v_loc then do:
                        disp "错误:与领料看板库位相同" @ WMESSAGE NO-LABEL with fram F1100.  
                        pause 0.  
                        undo, retry.
                    end.

                    find first ld_Det where ld_domain = global_domain and ld_site = site_from and ld_part = v_part
                                    and ld_loc = v1100 and ld_qty_oh > 0 no-lock no-error .
                    if not avail ld_Det then do:
                        disp "无对应库位/零件的库存记录" @ WMESSAGE NO-LABEL with fram F1100.  
                        pause 0.  
                        undo, retry.                        
                    end.
                    ...
                    ref_from = "" .
                    find first ld_Det where ld_domain = global_domain and ld_site = site_from and ld_part = v_part
                                    and ld_loc = loc_from and ld_lot = lot_from  and ld_qty_oh > 0 no-lock no-error .
                    if not avail ld_Det then do:
                        disp "无对应库位/批序的库存记录" @ WMESSAGE NO-LABEL with fram F1100.  
                        pause 0.  
                        undo, retry.                        
                    end.

                    for each ld_det where ld_domain = global_domain and ld_site = site_from and ld_part = v_part
                                    and ld_loc = loc_from  and ld_lot = lot_from and ld_ref = ref_from  and ld_qty_oh > 0 no-lock:
...
/*                 else do :   /*if v1100 = ""  then */ */
                    ...
                    if v_qty > v_qty_iss then do:
                        run xxmsg01.p (input 0 , input  "可发料数不足领料数:" + string(v_qty_iss)  ,input yes )  .
                                              ...
                                              and tmpkb2.tmp_loc = tmpkb.tmp_loc and tmpkb2.tmp_lot =tmpkb.tmp_lot and tmpkb2.tmp_ref = tmpkb.tmp_ref no-lock :
                                                    ...
                                                    if avail xkb_mstr then do : 
                                                        ...
                                                        {xxkbhist.i   &type="xkb_type"      &part="xkb_part"      &site="xkb_site"
                                                                    &kb_id="xkb_kb_id"    &effdate="effdate"        &program="'mhjit0382.p'"
