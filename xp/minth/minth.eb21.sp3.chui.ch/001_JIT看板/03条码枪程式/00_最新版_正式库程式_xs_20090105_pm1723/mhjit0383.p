/* mhjit0383.p  看板领料: 无领料看板有采购看板  for barcode                          */
/* Copyright 1986-2002 QAD Inc., Carpinteria, CA, USA.                               */
/* All rights reserved worldwide.  This is an unpublished work.                      */
/*V8:ConvertMode=NoConvert                                                           */
/* REVISION: 1.0      LAST MODIFIED: 10/08/07   BY: Softspeed roger xiao   /*xp001*/ */
/* REVISION: 1.0      LAST MODIFIED: 10/22/07   BY: Softspeed roger xiao   /*xp002*/ */
/* REVISION: 1.2      LAST MODIFIED: 10/26/07   BY: Softspeed roger xiao   /*xp003*/ */
/*-Revision end------------------------------------------------------------          */
      
define shared variable execname as character .  execname = "mhjit0383.p".
...
/*mage*/ define variable wcloc like loc_loc init "".
...
*mage del 08/08/07*******************************************************************************/
		...
		/*mage 08/08/08 add */ assign	wcloc = ""  v_loc = "".
        pause 0.
        leave V1000L.
    END.  /* END 1000    */
...
/* /*xp002*/ 
            display  skip WMESSAGE NO-LABEL with fram F1300.
             ...
             /*if v1300 <> "" then do: */
                ...
                find first pt_mstr where pt_domain = global_domain and pt_part = v1300 no-lock no-error .
                if not avail pt_mstr then do:
                    disp "错误:无此零件号"   @ WMESSAGE NO-LABEL with fram F1300.
                    pause 0 .
                    undo ,retry .
                end.
                ...
                if v_par = v_part then do:
                    disp "错误:父/子零件相同"   @ WMESSAGE NO-LABEL with fram F1300.
                    pause 0 .
                    undo ,retry .
                end.
                /* *************** */
...
*/ /*xp002*/
            ...
            if not avail loc_mstr then do:
                disp "错误:无此工作中心."   @ WMESSAGE NO-LABEL with fram F1300.
                pause 0 .
                undo ,retry .
            end.
	    ...
	    wcloc = v_loc.
            /* *************** */
/*mage 08/08/08 add */
       end.   /*** if wcloc = "" then do: **/
/*mage 08/08/08 add */
            V1400L:
            REPEAT:
                
                hide all no-pause .
                define variable V1400           like tr_qty_loc .
                ...
                if v1400 < 0 then do:
                   display "领料数量仅限正数" @ WMESSAGE NO-LABEL with fram F1400.
                   pause 0.
                   undo,retry .
                end.
            
                if v1400 = 0 then leave mainloop .
            
                v_qty_ls = v1400 .
                v_qty_ly = 0 .
                create tmpkb .
                assign  tmp_part  = v_part
                        tmp_par   = v_par
                        tmp_site  = site_from
                        tmp_loc   = v_loc
                        tmp_qty_ls = v_qty_ls
...
*/ /*xp002test*/
            ...
            for each tmpkb no-lock :
             ...
             REPEAT:
        	    ...
        	    V1100 = "" .
				v_part = if avail pkb then pkb_part else  "" .
                ...
                if v1100 <> ""  then do:
					...
					if v_part = "" then v_part = v13002 .  /*xp002*/
					else do:
						if v13002 <> v_part then do:
							disp "错误:仅限领料零件的看板." @ WMESSAGE NO-LABEL with fram F1100.
							pause 0.
							undo, retry.            
						end.
					end.
...
/*                                                                     &kb_id="xkb_kb_id"    &effdate="effdate"        &program="'mhjit0383.p'"                            */
