...
xdefsite = si_site.
find last si_mstr where si_domain = global_domain no-lock no-error.
...
/* FOR USER LEVEL */
find first CODE_MSTR where code_fldname = "BARCODEACCESSSITE" 
   and code_value = userid(sdbname('qaddb')) 
   and code_domain = global_domain no-lock no-error.
IF NOT AVAILABLE CODE_MSTR then DO:

   find first CODE_MSTR where code_fldname = "BARCODEACCESSSITE" 
      and code_value =  "*" and code_domain = global_domain no-lock no-error.
...
if aPASS = "Y" and wDefSite <> "" then do:
   ...
   find first si_mstr where si_site = wDefSite
      and si_domain = global_domain no-error.
   ...
   /*
   find first code_mstr where code_fldname = "BARCODEDEFSITE" 
      and code_value =  userid(sdbname('qaddb')) and code_domain = global_domain no-error. 
   if NOT AVAILABLE ( code_mstr ) then do:
      create code_mstr.
