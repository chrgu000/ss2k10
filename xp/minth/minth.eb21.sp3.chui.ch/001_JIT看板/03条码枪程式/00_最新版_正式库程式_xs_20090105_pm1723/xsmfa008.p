...
/*tx01*/ find first ls_mstr where ls_type = "company" and ls_domain = "008"
...
/*tx01*/ find first ad_mstr where ad_domain = "008"
/*tx01*/    and substring(ad_addr,2,7) = "reports" no-lock no-error.
   ...
   /* Register Clicense Code  Start*/	 
    find first usrw_wkfl where usrw_key1 = "BARCODE" and usrw_key2 = "LICENSEKEY" 
               ...
               and usrw_domain = "008" no-lock no-error.
...
/*			    
    ...
    for each code_mstr where code_fldname = "BARCODEUSERID"  and code_domain = "008" no-lock :
	...
	quit.
     ...
     do transaction on error undo:
         ...
         for each usr_mstr no-lock where usr_userid = psid  :
	       ...
	       /*    usr_passwd = encode ( substring( encode ( psid + "008" ),1,8) ). */
         end.
	 ...
	 find first usr_mstr where usr_userid = psid  no-lock no-error.
         if available usr_mstr   then do: 
		  ...
		  output to value( trim(usection) + ".i") .
		   /*  display  """" + trim(psid) + """"  + " " + """" + substring( encode ( psid + "008" ) ,1,8) + """" format "X(50)" skip  
		  */
/*tx01* commentted		
