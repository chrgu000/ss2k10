...
define variable cim_pwd as char format "x(12)".

define variable cim_dbname as char format "x(20)".
...
/*tx01*/ find first ls_mstr where ls_type = "company" and ls_domain = "01"
...
/*tx01*/ find first ad_mstr where ad_domain = "01"
/*tx01*/    and substring(ad_addr,2,7) = "reports" no-lock no-error.
   ...
   /* Register Clicense Code  Start*/	 
    find first usrw_wkfl where usrw_key1 = "BARCODE" and usrw_key2 = "LICENSEKEY" 
               ...
               and usrw_domain = "01" no-lock no-error.
...
/*			    
    ...
    for each code_mstr where code_fldname = "BARCODEUSERID"  and code_domain = "01" no-lock :
...
/*start tx01*/
                  cim_dbname = dbname.
		  if cim_dbname = "mfgtest2" then cim_pwd = "mfg".
		  if cim_dbname = "mfgprod" then cim_pwd = "19790614qm".
                  if cim_dbname = "mfgkb2" then cim_pwd = "mfg".
...
/*mage  chg*/   /*
                 for first mnd_det where mnd_nbr ="" and mnd_select = 1 :
		      oldmnd_canrun = mnd_canrun.
		      mnd_canrun = mnd_canrun + "," + trim ( psid ) .
		  end. 
		  release  mnd_det 
