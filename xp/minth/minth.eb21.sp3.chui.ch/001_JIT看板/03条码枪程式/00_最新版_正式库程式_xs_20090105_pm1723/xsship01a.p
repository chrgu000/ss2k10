...
/*
	find first xxbc_ctrl where xxbc_domain = global_domain 
	   and xxbc_site = V1002 and xxbc_cust = trim(V1100)
	   and today >= xxbc_start_dt and today <= xxbc_end_dt no-lock no-error.
	if not available xxbc_ctrl then do:
           display skip "条码SO出货控制有误." @ WMESSAGE NO-LABEL with fram F1100.
           pause 0 before-hide.
           undo, retry.
	end.
	else do:
	   model_yr = string(xxbc_yr).
	   nbr = xxbc_nbr.
	   if not xxbc_active then do:
              display skip "条码出货控制被无效." @ WMESSAGE NO-LABEL with fram F1100.
              pause 0 before-hide.
              undo, retry.
	   end.
	end.
...
/****************************** Start Commment **********************************
        ...
        for each xxbct_hist where xxbct_domain = global_domain 
             ...
             and xxbct_cust_part = (V1200) no-lock
        break by xxbct_serial:
	   if last(xxbct_serial) then assign
              last_ser = integer(substring(xxbct_lot,9,3))
	      count_page = integer(substring(xxbct_lot,12,3)).
        end.
	for each xxbcm_hist where xxbcm_domain = global_domain
	   ...
	   if last(xxbcm_serial) then do:
	      if integer(substring(xxbcm_serial,9,3)) > last_ser then
		 assign last_ser = integer(substring(xxbcm_serial,9,3))
	              count_page = integer(substring(xxbcm_serial,12,3)).
	   end.
	end.
...
****************************** End Commment **********************************/
        ...
        REPEAT:
		 ...
		 and lookup(xxbcd_stat,"R,A") > 0 use-index xxbcd_serial
