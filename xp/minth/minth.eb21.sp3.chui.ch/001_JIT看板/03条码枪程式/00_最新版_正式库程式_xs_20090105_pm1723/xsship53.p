...
debug*/
	...
	count_box = 0.
        ...
        for each xxbct_hist no-lock where xxbct_domain = global_domain 
             and xxbct_site = trim(V1002) and xxbct_cust = trim(V1100)
             and xxbct_cust_part = (V1200) 
	     and substring(xxbct_serial,9,3) = substring(V1400,9,3) 
	     ,each xxbcm_hist no-lock
	     where xxbcm_domain = xxbct_domain
	     and xxbcm_part = xxbct_part
	     and xxbcm_serial = xxbct_lot 
        break by xxbct_lot:
	   if last(xxbct_lot) then assign
              last_ser = integer(substring(xxbct_lot,7,3))
	      count_page = integer(substring(xxbct_lot,10,3)). 
        end.
...
/*	for each xxbcm_hist where xxbcm_domain = global_domain
	     and xxbcm_site = trim(V1002) and xxbcm_cust = trim(V1100)
	     and xxbcm_cust_part = (V1200) 
	     and substring(xxbcm_serial,9,3) = substring(V1400,9,3) no-lock
        break by xxbcm_serial:
	   if last(xxbcm_serial) then do:
	      if integer(substring(xxbcm_serial,7,3)) > last_ser then
		 assign last_ser = integer(substring(xxbcm_serial,7,3))
		 ...
		 and lookup(xxbcd_stat,"R") > 0 use-index xxbcd_serial
		 ...
		 if index(locdesc, "成品仓") = 0 then do:
		        ...
		        and xkb_site = xxbcd_site
			and xkb_part = xxbcd_part and xkb_type = "M" and xkb_cust = xxbcd_cust 
			and xkb_q1bc = xxbcd_serial no-lock no-error.
		 if not available xkb_mstr then do:
                    display skip "错误:看板未找到,请21出货!" @ WMESSAGE NO-LABEL with fram F1400.
                    pause 0 before-hide.
                    undo, retry.
		 end.
