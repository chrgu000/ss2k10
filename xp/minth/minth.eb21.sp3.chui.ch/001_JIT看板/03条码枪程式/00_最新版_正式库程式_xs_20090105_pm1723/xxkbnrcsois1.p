...
/* $Revision: 1.129.1.30 $ BY: Bill Jiang  DATE: 03/06/06 ECO: *SS - 20060306* */
/* By: Neil Gao Date: 07/04/29 ECO: *ss 20070429.1 * */
...
/*tx01*/ define variable trnbr like tr_trnbr.
/*tx01*/ define buffer xkbhhist for xkbh_hist.
...
/********* start tx01**********/
	    ...
	    for each xkb_mstr where xkb_domain = global_domain
	         and  xkb_shp_nbr = trim(substring(abs_id,2,10)) use-index xkb_shp_nbr:
	       
                if xkb_kb_id = 999 or xkb_kb_id = 000 then   xkb_kb_raim_qty = xkb_kb_raim_qty - integer(xkb_fldusr[1]). 
		   else  xkb_kb_raim_qty = 0.
/*mage            xkb_kb_raim_qty = 0. */ 

               if xkb_kb_raim_qty <= 0 then do:
               {xxkbhist.i &type="xkb_type"         &part="xkb_part"      &site="xkb_site"
		           &kb_id="xkb_kb_id"       &effdate=today        &program="'xxkbnrcsois1.p'"
		           &qty=0                   &ori_qty="xkb_kb_qty" &tr_trnbr=0
