...
dmdesc = "[看板库存转移]" + (if available dom_mstr then trim(dom_name) else "")
    ...
    REPEAT:
        ...
        if available xdn_ctrl then do:
            ...
            p-next = xdn_next.
                ...
                disp  "错误:无库存转移单号控制档"  @ WMESSAGE NO-LABEL with fram F1000.
                pause 0.
                undo, retry  .
        end.
    ...
    define var v13003          as char format "x(3)" .
     ...
     REPEAT:
        ...
        if not avail loc_mstr then do:
              display "库位不存在,请重新输入."  @ WMESSAGE NO-LABEL with fram F1100.
              pause 0 .
              undo ,retry .
        end.
     ...
     REPEAT:
        ...
        if v1200 < today  then do:
            display "日期不得小于今天,请重新输入." @ WMESSAGE NO-LABEL with fram F1200.
            pause 0  .
            undo ,retry .
        end.
        ...
        find first loc_mstr where loc_domain = global_domain and loc_loc = loc_to no-lock no-error .
        if avail loc_mstr then do:
            find si_mstr where si_domain = global_domain and si_site   = loc_site no-lock no-error .
     ...
     REPEAT:
        ...
        L13006 = if avail tmpkb then "请继续刷读,或直接确认执行" else "请刷读看板条码:" .
              ...
              if not avail xkb_mstr then do:
                  ...
                  undo, retry.
                    ...
                    if (loc_to = xkb_loc) then do:
                      disp "错误: 看板已在待转入库位" @ WMESSAGE NO-LABEL with fram F1300. 
                      pause 0.
                      undo, retry.
                    end.
                    ...
                    if xkb_kb_raim_qty = 0 then do:
                          disp "刷读失败:看板余量为零" @ WMESSAGE NO-LABEL with fram F1300. 
...
/*                               end.                                                                                                                        */
                    ...
                    if not avail tmpkb then do:   /*first scan */
                        ...
                        if (tmp_site <> xkb_site or tmp_loc <> xkb_loc) then do:
                          disp "错误: 仅限同库位看板" @ WMESSAGE NO-LABEL with fram F1300. 
                          pause 0.
                          undo, retry.
                        end.
                        else do:
                            ...
                            if avail tmpkb then do:
                                  disp "错误:仅限同一零件" @ WMESSAGE NO-LABEL with fram F1300. 
                                  pause 0.
                                  undo, retry.
                            end.
                            ...
                            if v_qty_req > v_qty_oh then do:
                                  disp "刷读失败:库存不足累计转移量" @ WMESSAGE NO-LABEL with fram F1300. 
                                  pause 0.
                                  undo, retry.
                            end.
                              ...
                              for each tmpkb where tmp_qty > 0  no-lock break by tmp_site by tmp_loc by tmp_lot by tmp_ref by tmp_part :
                                  ...
                                  if last-of(tmp_part) then do:
                                      ...
                                      if v_qty <= v_qty_oh then do:
                                          ...
                                          find tr_hist where tr_domain = global_domain and tr_trnbr = rct_trnbr no-error .
                                          if avail tr_hist then  assign tr_addr = tmp_loc .
                                          find tr_hist where tr_domain = global_domain and tr_trnbr = iss_trnbr no-error .
                                          if avail tr_hist then  assign tr_addr = tmp_loc .
                                           ...
                                           /*disp "有效库存不足,转移失败" @ WMESSAGE NO-LABEL with fram F1400. */
                                           /*pause 3 no-message  .                                             */
...
/*                                 Update V1400   WITH  fram F1400 no-label .                          */
                                ...
                                if v1400 then do:
                                    ...
                                    j = 0 .
                                    /*display "请重新刷读所有看板.." @ WMESSAGE NO-LABEL with fram F1400.*/
                                    /*pause 3 no-message .*/
