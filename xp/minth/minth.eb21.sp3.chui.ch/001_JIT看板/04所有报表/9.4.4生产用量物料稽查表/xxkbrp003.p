    ...
    do on error undo, return error on endkey undo, return error:   
        ...
        for each tmpkb where tmp_comp >= part and (tmp_comp <= part1 or part1 = "" )
            ...
            for each tr_hist 
                      ...
                      no-lock:
                     if tr_type = "rct-tr" and tr_nbr begins "Ls" then  v_qty_ls  = v_qty_ls  + tr_qty_loc .
                     if tr_type = "rct-tr" and tr_nbr begins "LY" then  v_qty_ly  = v_qty_ly  + tr_qty_loc .
                     if tr_type = "iss-tr" and tr_nbr begins "rt" then  v_qty_rt  = v_qty_rt  - tr_qty_loc .
            end.


            v_qty_err = v_qty_par - (v_qty_ls + v_qty_ly - v_qty_rt ) .
        ...
        for each tmpkb where tmp_comp >= part and (tmp_comp <= part1 or part1 = "" )
            ...
            if first-of(tmp_comp) then do:
                   ...
                   if v_qty_ls <> 0 or v_qty_rt <> 0 or v_qty_ly <> 0 then do:
                            export  delimiter ";"
                                 "未知父件" 
                                 ""
                                 tmp_comp 
