            ...
            if last-of(tr_part) then do:
                /* disp tr_part tr_lot  v_qty_par tr_trnbr with fram x. */
                    ...
                    assign level = 1.
                           ...
                           comp = tr_part.
                           eff_date = tr_effdate .
                    find first ps_mstr use-index ps_parcomp where ps_mstr.ps_domain = global_domain and ps_par = comp no-lock no-error .
                    repeat:        
                       if not avail ps_mstr then do:                        
                             repeat:  
                                ...
                                comp = ps_par.  
                                qty = save_qty[level].            
                                find next ps_mstr use-index ps_parcomp where ps_mstr.ps_domain = global_domain and ps_par = comp no-lock no-error.
                        ...
                        if (ps_end = ? or eff_date <= ps_end) then do :
                               ...
                               if not avail ps_mstr then do:
                                   ...
                                   find first tmpkb where tmp_part = tr_part and tmp_comp = ps_comp exclusive-lock no-error .
