...
if v_type = "P" then do:
        for each pt_mstr where pt_domain = global_domain and pt_site = site 
            ...
            if not avail xppt_mstr then next .  /*非看板流程零件则跳过*/
            ...
            if not avail xkb_mstr then next . /*没有A状态的看板则跳过*/

            find first xkb_mstr where xkb_domain = global_domain and xkb_type = v_type and xkb_site = xppt_site and xkb_part = xppt_part
                                and xkb_status = "U" no-lock no-error .
            if avail xkb_mstr then next . /*有U状态的看板则跳过*/
        ...
        for each pt_mstr where pt_domain = global_domain and pt_site = site 
            ...
            if not avail xmpt_mstr then next .  /*非看板流程零件则跳过*/
