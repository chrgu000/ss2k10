...
for each xkb_mstr where xkb_domain = global_domain and xkb_type + xkb_part + string(xkb_kb_id,"999") = v_sn no-lock with frame x width 300 :
    ...
    v_comb2 = if avail xppt_mstr then xppt_comb else no.
        ...
        find first ro_det where ro_domain = global_domain and  ro_routing = xkb_par 
                                             and ro_start <= today and (ro_end >= today or ro_end = ? ) no-lock no-error.
        if not avail ro_det then do:
         ...
         "(input xkb_site,
           ...
           if cumwo_lot = ?   then   next  .
            ...
            v_qty_tr = 0 .
            for each tr_hist where tr_domain = global_domain and tr_site = xkb_site and (tr_type = "RCT-TR" or tr_type = "ISS-TR" ) and tr_effdate >= v_start
                 and tr_part = xkb_part  and tr_loc = line and ( tr_rmks = xkb_par ) and tr_serial = xkb_lot and tr_ref = xkb_ref   no-lock:
