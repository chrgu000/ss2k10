...
/* SS - 090827.1  By: Roger Xiao */ /*取号逻辑修改*/
...
REPEAT: 
	...
	/* ss - 090717.1 -b */
	   ...
	   if loc_Loc = "" then do :
	     display "库位不能为空."  @ WMESSAGE NO-LABEL with fram F1100.
              pause 2 .
              undo ,retry .	    
	   end .
	end .
        ...
        if v1300 <> "" then do:
		 ...
		 if available xbcm_mstr then do :
		   if xbcm_type <> "M" then do :
		    disp "汇总条码类型错误" @ WMESSAGE NO-LABEL with fram F1300. 
                     pause 0.
                     undo, retry.
		   end .
		 end .
		 ...
		 for each xbcm_mstr where xbcm_domain = global_domain and xbcm_nbr = v1300 no-lock :
		   find first xkb_mstr where xkb_domain = global_domain and xkb_type = xbcm_type 
		    and xkb_kb_id = xbcm_id and xkb_part = xbcm_part and ( xkb_status <> "u" or xkb_kb_raim_qty <= 0 )no-lock no-error .
		     ...
		     disp "汇总条码中看板资料有变化" @ WMESSAGE NO-LABEL with fram F1300. 
                     pause 0.
                     undo, retry.
		     end .
		 end .
		   ...
		   find first xkb_mstr where xkb_domain = global_domain and xkb_type = xbcm_type and 
		    ...
		    if available xkb_mstr then do :
			 ...
			 if ld_expire <> ? and ld_expire < today then do: /*xp001*/  
                            disp "刷读失败:库存已过期" @ WMESSAGE NO-LABEL with fram F1300. 
                            pause 0.
                            undo, retry.
                         end . /*xp001*/ 
			 /* ss-090327.1 -e */
		    ...
		    /*xp001***begin*/
                    ...
                    if not avail pt_mstr then do:
                        disp "错误:零件不存在" @ WMESSAGE NO-LABEL with fram F1300.
                        pause 0 .
                        undo,retry .
                    end.
                    need_sn = no .
                    if pt__chr08 = "Y" then do:
                        ...
                        if avail xsn_mstr then do:
                            ...
                            for each xsn_mstr where xsn_domain = global_domain and xsn_kb = xkb_type + xkb_part + string(xkb_kb_id,"999") and xsn_stat = "U" no-lock :
                                ...
                                or xsn_ref  <> xkb_ref 
                                then do:
                                    disp "错误:与条码库位批序有差异" @ WMESSAGE NO-LABEL with fram F1300.
                                    pause 0 .
                                    undo,retry .
                                end.
                            ...
                            if xkb_kb_raim_qty <> v_qty_sn then do:
                                disp "错误:与条码余量有差异" @ WMESSAGE NO-LABEL with fram F1300.
                                pause 0 .
                                undo,retry .
                            end.
                            need_sn = yes .
		    ...
		    /* ss-090327.1 -e */
		     ...
		     /* ss - 090717.1 -b */
		    if xkb_Loc = "" then do:
		       disp "看板库位不能为空" @ WMESSAGE NO-LABEL with fram F1300. 
                               pause 2.
                               undo, retry.
		    end .
		    /* ss - 090717.1 -e */
