...
/* SS - 090827.1 - B */
form
   parent   colon 15 label "子零件"
   desc1    colon 51 label "说明"
   um       colon 15 label "单位"
   desc2    colon 51 no-label
   eff_date colon 15 label "截止"
   maxlevel          label "层次"
...
with frame a1 width 90 attr-space no-underline.
...
/* SS - 090827.1 - B */
if available pt_mstr then assign um = pt_um desc2 = pt_desc2.
disp um desc2 with frame a.
/* SS - 090827.1 - E */
            recno = ?.
...
/* SS - 090827.1 - E */
   ...
   repeat:
      ...
      with frame bm width 80 no-attr-space.
      ...
      form
         ...
         l_phantom format "x(3)"
         ps_ps_code
      with frame fm width 80 no-attr-space.
      ...
      end. /* IF eff_date = ? OR (eff_date <> ? AND... */
      else
         find next ps_mstr
            use-index ps_comp
            where ps_mstr.ps_domain = global_domain
