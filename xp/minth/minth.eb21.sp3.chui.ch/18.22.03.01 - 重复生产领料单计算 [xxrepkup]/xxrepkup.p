...
/* $Revision: 1.6.1.11 $ BY: Paul Donnelly (SB) DATE: 06/28/03 ECO: *Q00K* */

/* SS - 100305.1  By: Roger Xiao */ /*修改原因call:ss-434, 取号方式,由最后跳号改为,进程式取完号,就跳号*/
/*-Revision end---------------------------------------------------------------*/
...
/*mage*/  define new shared variable fromloc        like pt_loc.
...
form
 ...
 /*mage*/   fromloc   colon 22     label "原材材仓"
   netgr          colon 30
...
if not can-find(first rpc_ctrl where rpc_ctrl.rpc_domain = global_domain ) then
do transaction:
   create rpc_ctrl. rpc_ctrl.rpc_domain = global_domain.
   release rpc_ctrl.
end.
...
/* SS - 100305.1 - B 
   find first rpc_ctrl  where rpc_ctrl.rpc_domain = global_domain no-lock
   no-error.
...
/* SS - 100305.1 - E */
 ...
 /*mage*/      fromloc
   ...
   {gprun.i ""gpquote.p"" "(input-output bcdparm,17,
        site,site1,part,part1,comp1,comp2,wkctr,wkctr1,
        string(issue),string(issue1),string(reldate),string(reldate1), fromloc,
...
for first lad_det no-lock
 ...
 if available lad_det then do:
    message "单号已存在! 请重新输入!"  view-as alert-box.
    next-prompt nbr with frame a.
    undo , retry.
 end.

 find first tr_hist no-lock where tr_domain = global_domain and tr_nbr >= nbr and index(tr_nbr , nbr) = 1 no-error.
...
/*ss-min001        with frame b width 80 no-attr-space down:*/
 *mage add 08/08/05*************************************************************/


/*mage* add**************/
find first loc_mstr no-lock where  loc_domain = global_domain and loc_loc = fromloc no-error.
if not available  loc_mstr then do:
next-prompt fromloc with frame a.
undo, retry.
end.
/*mage* add**************/
...
/* SS - 100305.1 - B 
   ...
   then do transaction:
      ...
      {gprun.i ""gpnbr.p"" "(16,input-output nbr)"}
      find first rpc_ctrl  where rpc_ctrl.rpc_domain = global_domain
      exclusive-lock.
      rpc_nbr = integer(substring(nbr,length(rpc_nbr_pre) + 1)).
      release rpc_ctrl.
   end.
