...
/*minth*/ crtdate1 = today. 
   ...
   then do:
      ...
      if seloption < 1 or seloption > 3 then do:
         /* Invalid Option */
         {pxmsg.i &MSGNUM=712 &ERRORLEVEL=3}
         if c-application-mode = 'web' then return.
         else next-prompt seloption with frame a.
         undo, retry.
      end.

      if sortoption < 1 or sortoption > 4 then do:
         /* Invalid Option */
         {pxmsg.i &MSGNUM=712 &ERRORLEVEL=3}
         if c-application-mode = 'web' then return.
         else next-prompt sortoption with frame a.
         undo, retry.
      end.
