...
/*minth*/ define    variable sum_short_isswo          as decimal .   
 ...
 /*minth*/    define temp-table tmp_det 
                     ...
                     field   tmp_rctwo  as   decimal 
                     field   tmp_isswo  as   decimal 
                     field   tmp_size   as decimal.
...
/*ss - 090514.1 - b */
  ...
  if available wc_mstr  and prcss  and update_yn then 
  if  pt_comm_code = "dt" then do:
  /*电镀材料回冲******************************/
     if tag_rcnt_dt <> ?
      then
          ...
          then
            ...
            if qty_var <> 0 then do:
               for each ps_mstr where ps_domain = global_domain and ps_comp = tag_part 
                   and ps_start  <= eff_date - 20 and (ps_end  >= eff_date or ps_end = ? ) no-lock:
...
/*message "test002"   sum_rctwo view-as alert-box .*/
               ...
               for each tmp_det no-lock where  tmp_type = "1" and index(tmp_ps_comp, tag_part) >= 1  and tmp_site = tag_site 
                   ...
                   for first wo_mstr
                      ...
                      where   wo_domain = global_domain and wo_site = tag_site and  wo_part = tmp_ps_par and wo_line = tag_loc
...
*/
               ...
               for each tmp_det no-lock where  tmp_type = "2" and  tmp_ps_comp = tag_part and tmp_site = tag_site 
                   and tmp_loc = tag_loc  and tmp_size <> 0 and   tmp_rctwo <> 0 :
                    if  tmp_rctwo * tmp_size + tmp_isswo <> 0 then
                    {gprun.i ""xxdisrlis.p""
                     "(input tmp_wo_lot   ,
                       input eff_date    ,
                        input tag_part,
                       input  - (qty_var * ( tmp_rctwo * tmp_size  ) / ( sum_short_isswo - sum_isswo)  + tmp_rctwo * tmp_size + tmp_isswo ),
                       input tag_loc )"}
                    else  {gprun.i ""xxdisrlis.p""
                     "(input tmp_wo_lot   ,
                       input eff_date    ,
                        input tag_part,
                       input - ( qty_var * ( - tmp_isswo) / ( sum_short_isswo - sum_isswo))   ,
                       input tag_loc )"}
