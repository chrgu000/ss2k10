...
do on error undo, return error {&GENERAL-APP-EXCEPT}:
   ...
   if (bUsage.origin <> EDI) or (working_qty <> 0) then do:
      ...
      if sod_consignment
      then do:
/*mage  08/08/20 del         for first so_mstr exclusive-lock */
/*mage  08/08/20 add */         for first so_mstr  
...
/* SS - 090401.1 - E */
      ...
      {gprun.i ""xxsocncixx.p""
               "(input bUsage.so_nbr,
                 input bUsage.sod_line,
                 input bUsage.site,
                 input bUsage.cncud_ship_date,
                 input ( - working_inv_qty),               /*modified cncix_qty_stock*/
                 ...
                 input no,
		 input busage.cncud_cncix_pkey,      /*minth add* modifer error***/
                 input-output first_time_in)"}

 
       first_time_in = no.
/**message "process 1 "  busage.cncud_asn_shipper   view-as alert-box. ***/
/* message " xxsocnisx.p  call xxsocncixx.p"      view-as alert-box.   */
 ...
 * ------------------------------------------------------------------------- */
 ...
 /*mage del 08/08/08     for first sod_det
         where sod_domain = global_domain
         and   sod_nbr    = p_order
         and   sod_line   = p_line
      exclusive-lock: */
/*mage add 08/08/08 */     for first sod_det
         where sod_domain = global_domain
         and   sod_nbr    = p_order
         and   sod_line   = p_line
         : 
