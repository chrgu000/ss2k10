...
/* By: Neil Gao Date: 20070307 ECO: *ss 20070307* */
/******************************************************************************/
...
define variable xxsym              as character.
define variable xxsw_new           as character.
define variable xxsw_i             as integer.
define variable xxsw_first_display as logical.
define variable xxsw_found_recs    as logical.
define variable xxsw_recid         as recid.
define variable xxsw_temp_recid    as recid.
define variable xxsw_frame_recid   as recid extent 25.
define variable xxsw_temp_new      like xxsw_new.

status default stline[4]. /* Replace old way of doing status-line */

xxsw_first_display = true.
...
if available {&detfile} then do:
   xxsw_new   = {&scroll-field}.
   xxsw_recid = recid({&detfile}).
end.
...
repeat with frame {&framename}:
   ...
   if xxsw_recid <> ? then
      find {&detfile} where recid({&detfile}) = xxsw_recid  no-lock no-error.
   else
      find first {&detfile} where {&scroll-field} >= xxsw_new no-lock no-error.

   xxsw_found_recs = available({&detfile}).
   ...
   if available {&detfile} then do:
      ...
      repeat with frame {&framename}:
         ...
         xxsw_frame_recid[frame-line] = recid({&detfile}).
   ...
   if not xxsw_found_recs  and  {&exit-flag}  and xxsw_first_display
      then leave sw-main-loop.

   xxsw_first_display = false.

   do xxsw_i = frame-line to frame-down - 1 with frame {&framename}:
      down 1.
   end.
   ...
   repeat on endkey undo, leave {&exitlabel} with frame {&framename}:
      ...
      find {&detfile} where recid({&detfile}) = xxsw_frame_recid[frame-line]
      ...
      then do with frame {&framename}:
         ...
         if frame-line <> frame-down then do:
            ...
            if xxsw_frame_recid[frame-line] = ? then do:
               up.
         ...
         find {&detfile} where recid({&detfile}) = xxsw_frame_recid[frame-down]
         no-lock no-error.
         ...
         if available {&detfile} then do:
            ...
            do xxsw_i = 1 to frame-down - 1:
               xxsw_frame_recid[xxsw_i] = xxsw_frame_recid[xxsw_i + 1].
            end.

            xxsw_frame_recid[frame-down] = xxsw_recid.
      ...
      then do with frame {&framename}:
         ...
         find {&detfile} where recid({&detfile}) = xxsw_frame_recid[1]
         no-lock no-error.
         ...
         if available {&detfile} then do:
            ...
            do xxsw_i = frame-down to 2 by -1:
               xxsw_frame_recid[xxsw_i] = xxsw_frame_recid[xxsw_i - 1].
            end.

            xxsw_frame_recid[1] = xxsw_recid.
      ...
      then do with frame {&framename}:
         ...
         if xxsw_frame_recid[frame-down] <> ? then do:
            find {&detfile} where recid({&detfile}) = xxsw_frame_recid[frame-down]
            no-lock no-error.
            find next {&detfile} {&detkey} {&searchkey}
               {&other-search} {&index-phrase} no-lock no-error.
      ...
      then do with frame {&framename}:
         ...
         find {&detfile} where
            recid({&detfile}) = xxsw_frame_recid[1] no-lock no-error.

         /* count backward a screenful of records */
         do xxsw_i = 1 to frame-down:

            /* save the old recid in case the find prev doesn't work */
            xxsw_temp_new = {&scroll-field}.
            xxsw_temp_recid = recid({&detfile}).
            ...
            if not available {&detfile} then do:
               ...
               xxsw_new   = xxsw_temp_new.
               xxsw_recid = xxsw_temp_recid.
               next sw-main-loop.
            end.
      ...
      then do:
         ...
         if keyfunction(lastkey) = "return" then do with frame {&framename}:
            if frame-line <> frame-down then do:
               color display normal {&display1} with frame {&framename}.
               down.
               /*Test to see if at eof*/
