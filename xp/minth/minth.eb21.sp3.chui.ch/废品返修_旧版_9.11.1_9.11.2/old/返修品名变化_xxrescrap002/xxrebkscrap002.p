...
/* SS - 090622.1 - RNB
之前版本: 有更新生产计划rps_mstr,现修改为不更改rps_mstr: b_qc = "qc" xxrescrap002.p --> xxrebkscrap002.p --> xxreceive.p --> xxreupdscf.p --> xxreupdscb.p
SS - 090622.1 - RNE */
...
/*tx01*/     {gprun.i ""xxretrin3scrap002.p""  
                "(output undo_stat)"}
...
/*apple*/   {gprun.i ""xxrebkfli1scrap002.p""
            "(input cumwo_lot,
              input op,
              output undo_stat)"}
/*tx01*
...
********* tx01**********/
                  ...
                  do transaction:
                     ...
                     then
/*mage                        sr_loc = pt_loc. */
/*mage*/  /*xp001**        if pt_part > "1" and pt_part < "1x"  then
	      sr_loc = line1.
	      else  sr_loc = pt_loc.*xp001*/

/*xp001*/ if   pt_article = "wkctrloc" then  sr_loc = line1 .
          else  sr_loc = pt_loc.  
...
/*minth*/ {gprun.i ""xxrelbra.p""
      ...
      then do:
	    ...
	    if b_qc = "qc" then do:
            /*xp: xxreceivebcmt.p 入库库位为xkbc_qc_loc */
            /*SS - 090622.1 - B 
