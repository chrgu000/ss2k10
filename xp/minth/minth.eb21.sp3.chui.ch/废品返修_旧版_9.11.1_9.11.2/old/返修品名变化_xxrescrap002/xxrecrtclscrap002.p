...
/*J2DG*/     where wo_mstr.wo_domain = global_domain and  wo_lot = cumwo_lot
no-lock:
...
/*J2DG*/ assign
         ...
         if include_prior_nonmilestones then do:
            for each wr_route
/*J2DG*/       fields( wr_domain wr_lot wr_milestone wr_op wr_qty_inque
wr_qty_outque)
               no-lock
         ...
         {xxgpxpldpsscrap002.i &date=eff_date &site=wo_site &comp=bomcode &op=op &op_list=oplist}  /*----------> CREATE PK_DET RECORDS*/
...
/*J2DG*/          fields( wr_domain wr_lot wr_milestone wr_op wr_qty_inque
wr_qty_outque)
/*J2DG*/           where wr_route.wr_domain = global_domain and  wr_lot =
cumwo_lot
...
/*G1TH*/ /* BEGIN ADD SECTION */
         ...
         for each pk_det exclusive-lock
            where pk_det.pk_domain = global_domain and  pk_user = mfguser and
                 pk_qty = 0:
           delete pk_det.
         end.
...
/*J2DG*/              where loc_mstr.loc_domain = global_domain and  loc_site =
wo_site
...
/*H0Q8*/    end.
/*H0Q8*/    else do:
/*H0Q8*/      if can-find (loc_mstr  where loc_mstr.loc_domain = global_domain
and  loc_site = wo_site and
...
/*J2DG*/        where pt_mstr.pt_domain = global_domain and  pt_part = pk_part
no-lock:
