...
/* SS - 090622.1 - RNB
之前版本: 有更新生产计划rps_mstr,现修改为不更改rps_mstr: b_qc = "qc" --> xxrebkscrap001.p --> xxreceive.p --> xxreupdscf.p --> xxreupdscb.p
SS - 090622.1 - RNE */
...
/* SS - 091125.1 - B */
define var v_scrap_rsn   like rsn_code.
define var v_scrap_desc  as char format "x(24)".

form 
        v_scrap_rsn        colon 15 label "报废原因" 
        v_scrap_desc       colon 15 no-label 
with frame f_scrap side-labels overlay row 4 centered width 60  .
...
repeat with frame a:
    ...
    do on error undo, retry with frame a:    
        ...
        do on error undo, retry with frame a:
            ...
            /* SS - 091125.1 - B */
                if v_qty_s > 0 then do on error undo,retry:
                    pause 0 .

                    update v_scrap_rsn with frame f_scrap  editing:
                        if frame-field = "v_scrap_rsn" then do:
                            {mfnp01.i rsn_ref v_scrap_rsn rsn_code ""scrap"" "rsn_domain = global_domain and rsn_type" rsn_type}
                            if recno <> ?  then do:
                                disp rsn_code @ v_scrap_rsn rsn_desc @ v_scrap_desc with frame f_scrap  .
                            end.             
                        end.
                        else do:
                                   status input ststatus.
                    ...
                    find first rsn_ref where rsn_domain = global_domain and rsn_type = "scrap" and rsn_code = v_scrap_rsn no-lock no-error .
                    if not avail rsn_ref then do:
                        message "错误:无效报废原因代码,请重新输入." .
                        undo,retry .
                    end.
                    disp rsn_code @ v_scrap_rsn rsn_desc @ v_scrap_desc with frame f_scrap .
                    hide frame f_scrap no-pause .
                end.
                hide frame f_scrap no-pause .
            /* SS - 091125.1 - E */
