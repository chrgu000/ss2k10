...
form
   ...
   prodline	  colon 35 label "生产线(生产线仓库)"
   locdesc        no-label 
   loc-to         colon 25 label  "入库仓位" 
   locdesc1       no-label
	...
	display prodline p-type rcvno loc-to with frame a.
      ...
      repeat with frame b:
         ...
         FORM           xic_line        label "项"
			...
			xic_loc_to      label "转入库位" space(1)
			xic_lot_from    label "批号"   format "x(18)"
...
/*
     ...
     */ 
      ...
      prompt-for xic_part
         editing:
 /****领料单, 退料单, 返修单, 补货单 , 车间转仓单     /* FIND NEXT/PREVIOUS RECORD */
            {mfnp05.i ld_det  ld_part
 ...
 {mfnp05.i lnd_det  lnd_linepart
               "lnd_det.lnd_domain = global_domain and  substring(lnd_line,1,7)   = substring(prodline,1,7) "  lnd_part  "input xic_part"}

            if recno <> ? then do:
	    display lnd_part @ xic_part .
