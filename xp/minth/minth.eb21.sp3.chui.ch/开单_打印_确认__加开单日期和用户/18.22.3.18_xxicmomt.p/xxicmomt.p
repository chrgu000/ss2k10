...
form
   ...
   rcvno          colon 60 label "补货单号码"
   prodline	  colon 10 label "成品仓库"
   locdesc  format "x(10)"      no-label 
   loc-to         colon 55 label  "客户服务仓" 
  
   cust	  colon 10 label "客户"  cmsort no-label  locdesc1   format "x(18)"  no-label
...
/*mage	  prodline 
     ...
     /* FIND NEXT/PREVIOUS RECORD */
        ...
        if recno <> ? then do:
	   display xic_nbr @ rcvno xic_site_from @ site  xic_loc_from @ prodline xic_loc_to @ loc-to  xic_addr @ cust with frame a.
	   rcvno = xic_nbr  .
	end. /* if recno<>? */
...
if newxic then do:
 update 
      ...
      find loc_mstr where loc_mstr.loc_domain = global_domain and loc_site = site 
	   ...
	   message "错误：客户服务仓库不存在，请重新输入".
	   undo, retry.
	end.

      find cm_mstr no-lock  where cm_mstr.cm_domain = global_domain and
            cm_addr = input cust no-error.
            if available cm_mstr then do:
	       cmsort = cm_sort.
	       display cmsort with frame a.
	    end.
	    else do:  
	       message "错误：客户不存在，请重新输入".
	       undo, retry.	     
	    end.
	...
	if loc-to <> "" then do:
	   ...
	   message "错误：生产线仓库不存在，请重新输入".
	   undo, retry.
	end.
       end.
end. /*if newxic *********************/
else do: 
display xic_loc_from @ prodline xic_loc_to @ loc-to  xic_addr @ cust with frame a.
...
loc-to = xic_loc_to.
	...
	display prodline p-type rcvno loc-to cust cmsort with frame a.
      ...
      repeat with frame b:
         ...
         FORM           xic_line        label "项"
			...
			xic_loc_to      label "生产线库位" space(1)
