...
form
   ...
   prodline	  colon 35 label "仓库"
   locdesc        no-label 
   loc-to         colon 25 label  "生产线仓库" 
   locdesc1       no-label
...
/*
 ...
 /****领料单, 退料单, 返修单, 补货单     /* FIND NEXT/PREVIOUS RECORD */
            ...
            if recno <> ? then do:
	    find first pt_mstr no-lock where pt_mstr.pt_domain = global_domain and pt_part = xic_part no-error.
               display pt_part @ xic_part   pt_loc @ xic_loc_to pt_um @ xic_um with frame b .
                if available pt_mstr then
                  display  pt_desc1.
               else
                  display "" @ pt_desc1.
 ...
 {mfnp05.i ld_det  ld_loc_p_lot
            ...
            if recno <> ? then do:
	    display ld_part @ xic_part with frame b.
