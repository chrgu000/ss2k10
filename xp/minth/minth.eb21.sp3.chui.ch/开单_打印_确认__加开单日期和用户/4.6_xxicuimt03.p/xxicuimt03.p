...
form
   ...
   rcvno          colon 60 label "入库单号码"
   prodline	  colon 35 label "入库仓库"
   locdesc        no-label 
...
site = global_domain.
/* p_type = "A1" .   /* Jack modify initial 20080620 */  */

find first xxicm_mstr where xxicm_domain = global_domain and xxicm_confirm = 1 no-lock no-error .
...
/*mage	  prodline 
    ...
    /* find first xxicm_mstr where xxicm_mstr.xdn_domain = global_domain and xdn_type = p_type no-lock no-error. */
     find first xxicm_mstr where xxicm_domain = global_domain and xxicm_type = p_type and xxicm_confirm = 1 no-lock no-error .
	if available xxicm_mstr then do:
	   p-prev = xxicm_start.
	   p-next = xxicm_nbr.
	...
	if rcvno = "" then do:
	    ...
	    find first xxicm_mstr where xxicm_domain = global_domain and xxicm_type = p_type exclusive-lock no-error .
		   ...
		   xxicm_nbr = fill("0",length(p-next) - length(string(k + 1 ))) + string(integer(p-next) + 1 )  /* m2 */.
		  newxic = yes.
		end.
	...
	if p_type <> xic_type then do :
	 message "单号类型需与单号开头一致" view-as alert-box .
	 undo , retry .
        end .
      end .
     end. 
...
if newxic then do:
 update 
        ...
        /* FIND NEXT/PREVIOUS RECORD */
      if frame-field = "prodline" then do :
...
prodline = xic_loc_from .
	...
	display /* prodline */ p_type rcvno /* loc-to */ prodline   with frame a.  /* Jack */
      ...
      repeat with frame b:
         ...
         FORM           xic_line        label "项"
			...
			xic_qty_from    label "入库数量"  space(1)
...
/*
	 ...
	 and xic_type = p_type  :
	 display xic_line xic_part xic_qty_from xic_lot_from  format "x(17)" xic_ref_from  xic_loc_to xic_um  with frame b .
 ...
 {mfnp05.i ld_det  ld_loc_p_lot
  ...
  /*mage add ***************************************************************************************/
    if newrec then do:
      /*mage         find first ld_det no-lock where ld_det.ld_domain = global_domain and ld_site = site 
	       and  ld_loc = prodline 
...
/* assign xic_part xic_qty_from   xic_lot_from xic_ref_from xic_um . */
