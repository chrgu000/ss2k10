...
define variable del-yn like mfc_logical initial no.
 define new shared var lotserial_qty1 like xic_qty_from .
          ...
          define new shared var cr_acct1 like trgl_dr_acct .
          define new shared var cr_sub1 like trgl_dr_sub.
          define new shared var cr_cc1 like trgl_cr_cc.
	  define new shared var cr_proj1 like pj_project .
...
define new shared variable transtype as character format "x(7)" initial "rct-unp".
...
form
    ...
    v_nbr                     colon 12 label "入库单号码" validate ( can-find( first xic_det where xic_det.xic_domain = global_domain and xic_nbr = input v_nbr and xic__log01 = no),"此单号已经确认或非计划外入库资料" ) 
...
repeat with frame a:
            ...
            {mfnp.i xic_det v_nbr  "xic_det.xic_domain = global_domain and xic_nbr "  v_nbr xic_nbr xic_nbr }
		     ...
		     find first pj_mstr where pj_domain = global_domain and pj_project = xic__chr02 no-lock no-error .
		      v_desc6 = if available pj_mstr then pj_desc else "" .
                ...
                display xic_nbr @ v_nbr   eff_date2 xic_loc_from @ v_loc_from xic_type @ v_type v_acc v_desc3 v_accd  v_desc4  xic__chr01 @ v_dept  v_desc5   xic__chr02 @ v_project  v_desc5  with frame a .
                for each xic_det where xic_domain = global_domain and xic_nbr = input v_nbr :
                 ...
                 display xic_line label "项次" xic_part label "料号" v_desc1 + v_desc2 label "说明" xic_qty_from label "入库数量" xic_lot_from format "x(17)" label "批号"  xic_ref_from label "序号" xic_um  column-label "UM" with width 100 stream-io .   
                end .
               end.
           end .
          else do:
	  ...
	  v_desc3 = if available ac_mstr then ac_desc else "" .
	   ...
	   find first cc_mstr where cc_domain = global_domain and cc_ctr = xic__chr01 no-lock no-error .
          ...
          v_desc6 = if available pj_mstr then pj_desc else "" .
            ...
            display xic_nbr @ v_nbr  xic_loc_from @ v_loc_from xic_type @ v_type v_acc v_desc3 v_accd  v_desc4  xic__chr01 @ v_dept
	       v_desc5  xic__chr02 @ v_project v_desc6  with frame a .
   for each xic_det where xic_domain = global_domain and xic_nbr = input v_nbr :
        find first pt_mstr where pt_domain = global_domain and pt_part = xic_part no-lock no-error .
           v_desc1 =  if available pt_mstr then pt_desc1 else "" .
           v_desc2 = if available pt_mstr then pt_desc2 else "" .
   ...
   find first  xic_det where xic_domain = global_domain and xic_nbr = input v_nbr .
    ...
    if available xic_det and lookup(xic_type , v_type1) > 0 then do :
     ...
     find first xxicm_mstr where xxicm_domain = global_domain and xxicm_type = xic_type and xxicm_confirm = 1 /* and xic_type = v_type1 */ no-lock no-error .
     if not available xxicm_mstr then do :
       ...
       message "此菜单不允许确认此类型或单号不存在" view-as alert-box .
       clear frame a .
       undo mainloop , retry mainloop .
    end .
/* SS - 090915.1 - B */
 ...
 if choice = yes then do :
   ...
   for each xic_det where xic_det.xic_domain = global_domain and  xic_nbr = input v_nbr and xic__log01 = no :
     ...
     /*  if xic_qty_from <= 0 then do :
        message xic_nbr xic_line "入库数量需大于零!" view-as alert-box .
	clear frame a .
	undo mainloop,retry mainloop .
      end . */   /* jack 08/26/09 */
