...
/* SS - 100505.1  By: Roger Xiao */  /*取消相关表的锁定*/ /*直接抓数,不产生gl数据再undo*/ 
/* SS - 100519.1  By: Roger Xiao */  /*"tt1_cc = wo_line" 直接替换为"tt1_cc = wr_wkctr"*/
/* SS - 100609.1  By: Roger Xiao */  /*xxreuvupxp.p: 1.wc查找方式加mch 2.显示生产线wo_line(而非成本中心wr_wkctr)和ln_desc说明,
...
REPEAT :
    ...
    /* main programmer */
    /*
    ASSIGN v_ini_date = DATE(v_month + 1  , 1 , v_year) .
    */
    ...
    ASSIGN start_date  =   DATE(v_month  , 1 , v_year) .
     ...
     PUT UNFORMATTED "#def REPORTPATH=$/Minth/sssfcv2" SKIP.
...
/* SS - 100505.1 - B 
     FOR EACH ssvdef2 NO-LOCK :
    	...
    	{gprun.i ""ssreuvup.p"" "(input  ssvdef2_effdate ,           
                         input    ssvdef2_part  ,
                         input    ssvdef2_part ,
                         input    ssvdef2_site  ,
                         input    ssvdef2_site ,
...
/* SS - 100505.1 - B */
     FOR EACH ssvdef2 NO-LOCK :
    	...
    	{gprun.i ""xxreuvupxp.p"" "(input  ssvdef2_effdate ,           
                         input    ssvdef2_part  ,
                         input    ssvdef2_part ,
                         input    ssvdef2_site  ,
                         input    ssvdef2_site ,
