...
/* By: Neil Gao Date: 20080229 ECO: * ss 20080229 * */
/*By: Neil Gao 08/03/23 ECO: *SS 20080323* */
/*By: Neil Gao 08/04/29 ECO: *SS 20080429* */
/*By: XUSUQIN 09/01/04 ECO: 若货运单已结算,报表显示结束为YES*SS 200900104* */
/*By: hubo 09/06/10 ECO:增加到达日期*/
/*By: hubo 09/06/29 ECO:增加车辆标志*/
/*BY: hubo 09/07/28 ECO:判断到达日期为空*/
...
/* if var = hi then var = "" */
   ...
   PUT UNFORMATTED "#def REPORTPATH=$/Minth/xxsoivr6" SKIP.
   PUT UNFORMATTED "#def :end" SKIP.

   for each absmstr no-lock 
           where abs_domain = global_domain 
           and abs_shipfrom = global_domain  /*SS - 091029.1*/
           and abs_par_id = "" 
           and abs_id  >= "s" +  ship and abs_id <= "s" + ship1 
       ...
       each abs_mstr where abs_mstr.abs_domain = global_domain  
            ...
            and abs_mstr.abs_par_id = absmstr.abs_id  
            and abs_mstr.abs_order >= nbr and abs_mstr.abs_order <= nbr1 
            and abs_mstr.abs_item  >= part and abs_mstr.abs_item <= part1        
            no-lock,
       each pt_mstr where pt_domain = global_domain 
            and pt_part = abs_mstr.abs_item 
       no-lock:

/*SS 20080429 - B*/
        find first ad_mstr 
            where ad_domain = global_domain 
            and ad_addr = absmstr.abs_shipto
            and ( (ad_addr >= cust and ad_addr <= cust1) or (ad_ref >= cust and ad_ref <= cust1)) 
        no-lock no-error.
...
/*SS 20080429 - E*/
        ...
        for each xxabs_mstr 
                 use-index xxabs_par_id2
                 where xxabs_domain = global_domain
                 and xxabs_shipfrom = global_domain /*SS - 091029.1*/
                 and xxabs_par_id = abs_mstr.abs_par_id 
                 and xxabs_id = abs_mstr.abs_id 
            no-lock,
            each xxrqm_mstr 
                where xxrqm_domain = global_domain 
                and xxrqm_nbr = xxabs_nbr 
            no-lock break by xxabs_nbr :
...
/* ss 20070410.1 - e */       
                ...
                if sod_consignment then do:
                    find last tr_hist use-index tr_part_eff 
                        where tr_domain = global_domain  and tr_part = sod_part 
                        and tr_type = "iss-tr" 
                        and tr_so_job = sod_nbr and
                        ...
                        tr_ship_id = substring(abs_mstr.abs_par_id,2)
                    no-lock no-error.
                    if avail tr_hist then outloc = tr_loc.

                    find last tr_hist use-index tr_part_eff 
                        where tr_domain = global_domain  and tr_part = sod_part 
                        and tr_type = "cn-ship" and
                ...
                if avail ad_mstr then put UNFORMATTED   ad_name ";" .
                else put UNFORMATTED   " " ";".
                /*hubo 20090610*/            
                if absmstr.abs_arr_date = ? then put  UNFORMATTED "". else put UNFORMATTED string(absmstr.abs_arr_date,"9999-99-99") .
                put UNFORMATTED ";" substring(absmstr.abs__qad01,81,20) .
                put skip.
                /* ss 20070611 - e */
                ...
                if avail idh_hist then do:
                    ...
                    find first sch_mstr where sch_domain = global_domain and /*SS - 091029.1*/ sch_nbr = idh_nbr and sch_line = idh_line no-lock no-error.
                    if avail sch_mstr then do:
                        find last tr_hist use-index tr_part_eff 
                            where tr_domain = global_domain 
                            and tr_part = idh_part 
                            and tr_type = "iss-tr" and
                        ...
                        find last tr_hist use-index tr_part_eff 
                            where tr_domain = global_domain and tr_part = idh_part 
                            and tr_type = "cn-ship" and
                            tr_so_job = idh_nbr and
                            ( tr_program = "rcsois.p" or tr_program = "xxncsois.p" or tr_program = "xxrcsois.p"
                            /* ss 20080229 */  or tr_program = "xxkbncsois.p"
                    ...
                    /* ss 20070709 - b */
                    cust_part = "".
                    for each cp_mstr no-lock where cp_domain = global_domain and cp_cust = ih_cust and cp_part = pt_part by cp_cust_eco:
                        cust_part = cp_cust_part.
                    end.
                    /* ss 20070709 - e */
