...
for each wr_route exclusive-lock
   ...
   find wc_mstr
    where wc_mstr.wc_domain = global_domain and  wc_wkctr = wr_wkctr and wc_mch
    = wr_mch no-lock no-error.
...
/* SETUP Burden =
   ...
   {gprun.i ""gpcurrnd.p"" "(input-output cum_var_amt,
                             input gl_rnd_mthd)"}
   var_to_post = cum_var_amt - wr_sbvuse_post.
...
/*ss - 2008.03.23 - E*/
      ...
      if var_to_post <> 0 then do:
         ...
         assign
            sf_dr_acct = gl_bvar_acct
            sf_dr_sub  = gl_bvar_sub
            sf_dr_cc   = gl_bvar_cc.
         ...
         if available wc_mstr then do:
            ...
            if available dpt_mstr and dpt_bvar_acc <> "" then
               assign
                  sf_dr_acct = dpt_bvar_acc
                  sf_dr_sub  = dpt_bvar_sub
                  sf_dr_cc   = dpt_bvar_cc.
         end.
...
/*ss - 2008.03.23 - B*/
/*
   if last(wr_op) and displaycount <> 0
   then do:
