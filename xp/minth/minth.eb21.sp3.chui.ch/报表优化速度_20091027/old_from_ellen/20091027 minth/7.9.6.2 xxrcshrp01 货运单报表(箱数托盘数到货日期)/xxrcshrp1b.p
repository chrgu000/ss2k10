...
/*tx01*/ define variable l_box like abs__dec01 column-label "箱数!托盘数".
/*tx01*/ define variable l_bbox like abs__dec01.
/*tx01*/ define variable cust_part like cp_cust_part label "客户零件号".
/*tx01*/ define variable cust like so_cust.
/*tx01*/ define variable ls_date as char.
/*tx01*/ ls_date = string(year(today),"9999") + string(month(today),"99") + string(day(today),"99").
...
form
   indent_id  /*tx01*/ label "类型"
   l_absitem     column-label "运往!零件号!模型年" /*tx01* "Ship-To!Item Number!Model Year" */
   scx_po        column-label "ID!PO 号!客户参考"  /*tx01* "ID!PO Number!Customer Ref" */
   l_abssite     column-label "运自!地点" /*tx01* "Ship-From!Site" */
   l_absorder    column-label "库存移动!订单" /*tx01* "Inv Mov!Order" */
   l_abs_line     column-label "项" /*tx01* "Line" */
/*tx01*/ cust_part
   l_pt_um
   open_qty /*tx01*/ label "未结数量"
   due_date /*tx01*/ column-label "截止日期"
   ship-qty      format "->,>>>,>>9.9<<<" /*tx01*/ column-label "运输数量"
   l_shp_date  
/*tx01*/ column-label "发货日期!到货日期"
/*tx01*/ l_box
with frame c down width 210 no-box.
/*
/* SET EXTERNAL LABELS */
...
PROCEDURE rptprint :
   ...
   then do:
/*tx01*/ assign l_box = abs__dec01
/*tx01*/    l_bbox = abs__dec02
      l_pt_um = abs__qad02.
...
/*tx01*/    find first so_mstr where so_domain = global_domain and so_nbr = abs_order no-lock no-error.
/*tx01*/    cust = (if available so_mstr then so_cust else "").
/*tx01*/    find last cp_mstr where cp_domain = global_domain and cp_cust = cust and cp_part = abs_item
/*tx01*/      and (cp_cust_eco = "" or cp_cust_eco <= ls_date) no-lock no-error.
/*tx01*/    cust_part = (if available cp_mstr then cp_cust_part else "").
...
/*tx01*/    l_bbox @ l_box
/*tx01*/    ship_abs.ship_arr_date @ l_shp_date
         with frame c.
      end. /* IF indent_ct <> 0 */
