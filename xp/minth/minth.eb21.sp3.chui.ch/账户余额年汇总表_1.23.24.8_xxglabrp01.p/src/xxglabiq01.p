...
for first en_mstr
   fields( en_domain en_entity en_name)
...
for first co_ctrl
   fields( co_domain co_ret co_use_cc co_use_sub)
      ...
      then
         ...
         with frame a editing:
            ...
            then do:
               ...
               {mfnp.i ac_mstr acc  " ac_mstr.ac_domain = global_domain and
               ac_code "  acc ac_code ac_code}
               if recno <> ?
               then do:
                  acc = ac_code.
                  display
         ...
         then do:
            ...
            /* VALIDATE ACCOUNT */
            for first ac_mstr
               fields( ac_domain ac_code ac_curr ac_desc)
                where ac_mstr.ac_domain = global_domain and  ac_code = acc
               no-lock:
            ...
            for first glc_cal
               fields( glc_domain glc_end glc_per glc_start glc_year)
                where glc_cal.glc_domain = global_domain and  glc_year = yr
               no-lock:
...
/* SS - 100602.1 - B 
            ...
            then do:
               ...
               for first qad_wkfl
                  fields( qad_domain qad_decfld qad_key1 qad_key2)
