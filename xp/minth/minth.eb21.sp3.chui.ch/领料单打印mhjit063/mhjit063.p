...
/* SS - 100309.1  By: Roger Xiao */
/* SS - 100526.1  By: Roger Xiao */  /*add流水号*/
/*-Revision end------------------------------------------------------------          */
...
Minth.QinMai:
增加选择条件,可以汇总打印,
如果汇总,则不按单号打印,而是按发料日期,生产线,料号汇总数量打印
SS - 100309.1 - RNE */
...
define variable linne   like op_wkctr label "生产线".
define variable linne1  like linne  .
define variable nbrr  as character format "x(10)" label "领料单号".
define variable nbrr1 like nbrr.
define variable effdate as date label "发料日期" .
...
define variable qtyall as decimal format "->,>>>,>>9.9<<<"  .
define variable qty_oh like ld_qty_oh .
define variable lines as integer format ">>9" label "项次" .
define variable prodline like loc_loc .
define variable rcvno as character format "x(10)" .
define var blanks as char label "" format "x(55)" .



define variable print_more like mfc_logical.
define variable print-loop as integer label {&repkrp01_p_7}.
define variable issued as character format "x(10)" extent 4 initial "__________".
...
/* SS - 100309.1 - B */
define var v_yn2 like mfc_logical label "按生产线+物料汇总打印"  initial No .
/* SS - 100309.1 - E */

/* SS - 100526.1 - B */
define var v_seq as integer . 
define var v_nbr as char format "x(8)" label "汇总单据号".
/* SS - 100526.1 - E */

effdate = today.
effdate1 = today.
form
   linne                       colon 20 column-label "生产线"
   linne1      label {t001.i}  colon 49 skip
   nbrr                        colon 20 column-label "领料单"
   nbrr1       label {t001.i}  colon 49 skip
   effdate                    colon 20 column-label "领料日期"
   effdate1   label {t001.i}  colon 49 skip
   skip(1)
   v_yn                       colon 30 column-label "仅限未打印"
/* SS - 100309.1 - B */
   v_yn2      colon 30  column-label "按生产线+物料汇总打印"  
/* SS - 100526.1 - B */
   v_nbr      colon 30  column-label "汇总单据号"
/* SS - 100526.1 - E */
...
/*    if effdate1 = effdate  then effdate1 = ? . */
   ...
   if c-application-mode <> 'WEB' then update  linne linne1 nbrr nbrr1 effdate effdate1 v_yn v_yn2 with frame a .
   {wbrp06.i &command = update &fields = " linne linne1 nbrr nbrr1 effdate effdate1 v_yn  v_yn2  " &frm = "a" }

/* SS - 100526.1 - B */
   if v_yn2 then do:
        v_seq    = next-value(xrpt_sq01) .
        if v_seq = 0 then v_seq = next-value(xrpt_sq01) .       
        v_nbr = "HZ" + string(v_seq,"999999").
        disp v_nbr with frame a .
   end.
/* SS - 100526.1 - E */
  ...
  find first xdn_ctrl where xdn_ctrl.xdn_domain = global_domain and xdn_type = "LS"  no-lock no-error.
  if available xdn_ctrl then do:
	   ...
	   xdnisover = xdn_isover.
    	...
    	message "领料单格式没有设置, 请重新设置后再打印!!"  view-as alert-box.
    	undo, retry.
	end.
...
if available ad_mstr then
   co_name = fill(" ", max ((15 - integer ({gprawlen.i &strng=ad_name} / 2)), 1))
      ...
      form header                                  
           companyname          at 5     "表   号:"          at 96  xdnisonbr  format "x(14)"   
           xdnname              at 40   "版本/修订:"         at 96  xdnisover   format "x(4)"  getTermLabelRtColon("DATE",6) format "x(6)" to 117 
           today          
           skip
      with frame phead2 page-top width 132 no-label  no-box no-attr-space.
...
if c-application-mode = 'WEB' and c-web-request = 'DATA' then do:
    form header         
        skip (1)
        companyname          at 5   
        "表   号:"           at 96 xdnisonbr  format "x(14)" 
        xdnname              at 40  
        "版本/修订:"         at 96  xdnisover   format "x(4)"  
...
/* disp  linne linne1 nbrr nbrr1 effdate effdate1 v_yn with frame xxx . */

/* SS - 100309.1 - B */
if v_yn2 = no then do:
/* SS - 100309.1 - E */

for each tr_hist  use-index tr_type where tr_domain = global_domain and tr_type = "rct-tr" and tr_nbr begins "LS"
                 and (tr_effdate >= effdate or effdate = ? ) and (tr_effdate <= effdate1 or effdate1 = ? )
                 and tr_addr > ""
                 and tr_nbr >= nbrr and ( tr_nbr <= nbrr1 or nbrr1 = "" ) 
    ...
    if first-of(tr_nbr) then do:
        ...
        rcvno = tr_nbr .
        prodline = tr_addr .
        find first wc_mstr no-lock  where wc_mstr.wc_domain = global_domain and wc_wkctr = tr_addr no-error.
...
/*         view frame c . */
        disp rcvno prodline desc2 with frame c .
    ...
    if last-of(tr_part) then do:
        /*         find first ld_det where ld_domain = global_domain and ld_site = tr_site and ld_part = tr_part */
        /*                           and ld_lot = tr_serial and ld_ref = tr_ref no-lock no-error.                */
        /*         qty_oh = if avail ld_det then ld_qty_oh else 0 .                                              */
...
for each tr_hist  use-index tr_type where tr_domain = global_domain and tr_type = "rct-tr" and tr_nbr begins "LS"
                ...
                with frame x2 width 150 no-attr-space no-box down :
                setFrameLabels(frame x2:handle).
                {mfrpchk.i}

    if first-of(tr_addr) then do:
        lines = 0.
        rcvno = string(tr_effdate) .
        prodline = tr_addr .
        find first wc_mstr no-lock  where wc_mstr.wc_domain = global_domain and wc_wkctr = tr_addr no-error.
        desc2 = if avail wc_mstr then wc_desc else "" .
        form
...
/* SS - 100526.1 - E */
            ...
            prodline  " "         
            desc2                skip
        with frame x1 page-top width 150 no-box no-label no-attr-space .

/* SS - 100526.1 - B 
        disp rcvno prodline desc2 with frame x1 .
   SS - 100526.1 - E */
/* SS - 100526.1 - B */
        disp rcvno prodline desc2 v_nbr with frame x1 .
/* SS - 100526.1 - E */

    end. /*if first-of(tr_addr) */
...
for each tr_hist  use-index tr_type where tr_domain = global_domain and tr_type = "rct-tr" and tr_nbr begins "LS"
                 ...
                 and tr_nbr >= nbrr and ( tr_nbr <= nbrr1 or nbrr1 = "" ) 
                 and tr_addr >= linne and ( tr_addr <= linne1 or linne1 = "" )
                 and ( tr__log02 = no or v_yn = no  )  exclusive-lock:
