...
/* Revision: 1.24     BY: Geeta Kotian      DATE: 01/22/03 ECO: *N24Y*  */
/* $Revision: 1.24.3.2 $    BY: Subramanian Iyer DATE: 07/02/03   ECO: *P0W8*  */
/******************************************************************************/
...
repeat:
   ...
   do transaction  with frame a:
      ...
      if not available pi_mstr then do:
         ...
         l_recid = recid(pi_mstr) no-error .
         if l_recid  = 0
         or l_recid  = ?
         then
...
/* SS - 110104.1 - B */
    ...
    define var v_list       like pi_list       no-undo.
    define var v_cs_type    like pi_cs_type    no-undo.
    define var v_cs_code    like pi_cs_code    no-undo.
    define var v_part_type  like pi_part_type  no-undo.
    define var v_part_code  like pi_part_code  no-undo.
    define var v_curr       like pi_curr       no-undo.
    define var v_um         like pi_um         no-undo.
    define var v_start      like pi_start      no-undo.
    define var v_ii         as integer format ">>" no-undo.

    assign 
        v_list       = pi_list     
        v_cs_type    = pi_cs_type  
        v_cs_code    = pi_cs_code  
        v_part_type  = pi_part_type
        v_part_code  = pi_part_code
        v_curr       = pi_curr     
        v_um         = pi_um       
        v_start      = pi_start    
        v_ii         = 0 
    ...
    for each pi_mstr 
        where pi_list      = v_list
        and   pi_cs_type   = v_cs_type   
        and   pi_cs_code   = v_cs_code   
        and   pi_part_type = v_part_type 
        and   pi_part_code = v_part_code 
        and   pi_curr      = v_curr      
        and   pi_um        = v_um        
        and  (pi_start     = ? or pi_start < v_start )
        and   pi_expire    = ?
        :
        v_ii        = v_ii + 1 .
        pi_expire   = v_start - 1 .
    ...
    if v_ii > 0 then message "前" v_ii "笔价格表已自动关闭." .


    find pi_mstr where recid(pi_mstr) = pirecno exclusive-lock no-error.
...
/* SS - 110104.1 - E */
   ...
   do transaction with frame acct:
      ...
      if pi_amt_type = "8"
         ...
         or pi_accr_cc <> "" or pi_accr_proj <> "") then do:
