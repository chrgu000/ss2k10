...
/* SS - 110104.1 - B */
    define var v_pclist like pc_list      no-undo.
    define var v_curr   like pc_curr      no-undo.
    define var v_prodln like pc_prod_line no-undo.
    define var v_part   like pc_part      no-undo.
    define var v_start  like pc_start     no-undo.
    define var v_um     like pc_um        no-undo.
    define var v_ii     as integer format ">>" no-undo.

    assign 
        v_pclist   = pc_list     
        v_curr     = pc_curr     
        v_prodln   = pc_prod_line
        v_part     = pc_part
        v_start    = pc_start    
        v_um       = pc_um  
        v_ii       = 0 
    ...
    for each pc_mstr 
        where pc_list      = v_pclist
        and   pc_curr      = v_curr
        and   pc_prod_line = v_prodln 
        and   pc_part      = v_part
        and   pc_um        = v_um 
        and  (pc_start     = ? or pc_start < v_start )
        and   pc_expire    = ?
        :
        v_ii        = v_ii + 1 .
        pc_expire   = v_start - 1 .
    ...
    find pc_mstr where recid(pc_mstr) = recno exclusive-lock no-error.
