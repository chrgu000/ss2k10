...
define var v_type       as char label "准备类型"  extent 5.
define var v_drsn       as char label "停工原因"  extent 5.
define var v_time_type  as decimal format "->>>9.999" label "工时" extent 5.
define var v_time_rsn   as decimal format "->>>9.999" label "工时" extent 5.
form
...
/* SS - 110120.1 - B */
if v_type[1] <> "" or v_type[2] <> "" or  v_type[3]  <> "" or v_type[4]  <> "" or v_type[5] <> "" then do:
    find first usrw_wkfl
        where usrw_key1 = "SETUP-TYPE"
        and   usrw_key2 = string(op_trnbr) 
    no-error.
    if not avail usrw_wkfl then do:
        create usrw_wkfl .
        assign usrw_key1 = "SETUP-TYPE"  
               usrw_key2 = string(op_trnbr)      
               .
    end.
...
if v_drsn[1] <> "" or v_drsn[2] <> "" or  v_drsn[3]  <> "" or v_drsn[4]  <> "" or v_drsn[5] <> "" then do:
    find first usrw_wkfl
