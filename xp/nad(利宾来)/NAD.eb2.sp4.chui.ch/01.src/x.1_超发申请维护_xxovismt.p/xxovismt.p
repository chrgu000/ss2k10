...
/* SS - 110307.1  By: Roger Xiao */ /* 限制 wo_status = R */
/* SS - 110310.1  By: Roger Xiao */ /* 允许手工输入单号 */
/* SS - 110316.1  By: Roger Xiao */ /* 允许手工输入单号fix-bug: 手工单号不能输入地点 */
/*-Revision end---------------------------------------------------------------*/
...
define var v_newnbr           as logical .  /* SS - 110316.1  */
...
repeat with frame a:
    ...
    if not avail xovc_ctrl then do: 
        message "错误:超发申请控制未维护,按任意键退出." view-as alert-box.
        undo,leave mainloop.
    end.
...
do on error undo ,retry on endkey undo, leave:
    ...
    repeat on endkey undo, leave:
        ...
        repeat on endkey undo, leave:
            ...
            /* SS - 110307.1 - B */
            find first wo_mstr where wo_lot = v_wolot no-lock no-error.
            if avail wo_mstr and wo_status <> "R" then do:
                message "错误:仅限工单状态为R,请重新输入" .
                undo,retry .
            end.
            /* SS - 110307.1 - E */
