...
/*计算上个星期六已发货未确认的库存量*/ 
          abv_qty = 0.
          for each abs_mstr no-lock where abs_shipfrom = t1_site 
	     and abs_id >= shipid
	     and abs_arr_date <= last-sta and substring(abs_status,2,1) <> "y"  use-index abs_shipto,
	     each absmstr no-lock where absmstr.abs_shipfrom = abs_mstr.abs_shipfrom and
	     absmstr.abs_par_id = abs_mstr.abs_id and absmstr.abs_item =t1_part use-index abs_par_id:
	     abv_qty = abv_qty + absmstr.abs_qty.
         end.
         v_qty_avail = v_qty_avail - abv_qty .
/*计算上个星期六已发货未确认的库存量*/ 
...
*/
/*ss - 090627.1 - b*/
define new shared variable s_increment as integer.
  s_increment = month(today) * 1000000  + day(today) * 10000 + 1 .
/*ss - 090627.1 - e*/
...
/*090622 mage 删除 因为客户不需要，发货时不能自动减少
/*写入对应的MRP需求,供查询23.17*/
