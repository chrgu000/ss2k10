...
/* SS - 090616.1 - B */
define shared variable wosite like wo_site.
define shared variable woduedate like wo_due_date.
define shared variable woqtyord like wo_qty_ord .
/* SS - 090616.1 - E */
...
SS - 090616.1 - E */
   ...
   do on error undo, retry:
      ...
      do for wo_mstr1:
         ...
         find first wo_mstr1
            where wo_mstr1.wo_nbr  = wo_mstr.wo_nbr
              and wo_mstr1.wo_part = wo_mstr.wo_part
              and wo_mstr1.wo_joint_type <> ""
         no-lock no-error.
      ...
      if screen_used then do:
         ...
         do for wo_mstr1:
            ...
            then do:
               ...
               find first wo_mstr1 where wo_mstr1.wo_nbr = input wo_mstr.wo_nbr
               ...
               then do:
                  ...
                  find bom_mstr where bom_parent = wo_mstr1.wo_bom_code
                  no-lock no-error.
...
/* SS - 090616.1 - B */
         assign
            ...
            find first ptp_det where ptp_site = wo_site and ptp_part = wo_part no-lock no-error .

            if available ptp_det then assign leadtime = ptp_mfg_lead.
            else assign leadtime = pt_mfg_lead.
