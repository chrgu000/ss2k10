...
define output parameter v_undo     like mfc_logical no-undo.

define new shared variable wosite    like wo_site.
define new shared variable woduedate like wo_due_date.
define new shared variable woqtyord like wo_qty_ord .
define new shared variable wopart         like wo_part.

assign
wopart    = v_part 
wosite    = v_site
woduedate = v_due_date
woqtyord  = v_qty_ord
v_undo    = yes .
/* SS - 090616.1 - E */
...
do transaction:
    find first woc_ctrl no-lock no-error.
    if not woc_auto_nbr then undo mainloop, leave mainloop.
/*ss - 090627.1 -b*    
{mfnctrl.i woc_ctrl woc_nbr wo_mstr wo_nbr wonbr} 
ss - 090627.1 -e*/
/*ss - 090827.1 -b*/
define shared variable s_increment as integer.
  
for first wo_mstr
             
            no-lock  where    wo_nbr =
            string(s_increment,"99999999"):
         end. /* for first wo_mstr */

         repeat while available wo_mstr:
         ...
         wonbr  = string(s_increment,"99999999").
/*ss - 090827.1 -b*/	  
    {mfnxtsq.i wo_mstr wo_lot woc_sq01 wolot}
    /*message wolot wonbr view-as alert-box .*/
    if wonbr = "" or wolot = "" then undo mainloop, leave mainloop.

    find wo_mstr no-lock use-index wo_lot using wo_lot and wo_nbr no-error.
end.
/* SS - 090616.1 - E */
...
{gprun.i ""xxwowomtjxp.p""}
      if undo_all then undo mainloop, leave mainloop.
...
/* SS - 090616.1 - B 
            ...
            undo mainloop, retry mainloop.
SS - 090616.1 - E */  undo mainloop, leave mainloop.
