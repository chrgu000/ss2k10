...
/* Revision: 1.20     BY: Niranjan R.           DATE: 06/25/02 ECO: *P09L*    */
...
/* Revision: 1.26     BY: Narathip W.           DATE: 04/19/03 ECO: *P0Q7* */
/* $Revision: 1.26.1.1 $       BY: Vivek Gogte           DATE: 08/06/03 ECO: *P0Z2* */
/* $Revision: 1.26.1.1 $       BY: Mage Chen             DATE: 08/06/03 ECO: *seastar**/
...
/*pak*/ define new shared variable woweight like sr_qty no-undo.
...
/*I030802**/ define new shared variable price_0     like mfc_logical initial no.
...
repeat with frame a:
   ...
   */
   ...
   for first wo_mstr
      fields(wo_base_id wo_batch wo_joint_type wo_lot wo_lot_next wo_lot_rcpt
             wo_nbr wo_part wo_qty_chg wo_rmks wo_site wo_so_job wo_status
             wo_stat_close_date wo_stat_close_userid wo_type wo_wip_tot)
      where recid(wo_mstr) = wo_recno
      no-lock:
   end. /* FOR FIRST wo_mstr */

   for first pt_mstr
      fields(pt_abc pt_auto_lot pt_avg_int pt_cyc_int pt_desc1 pt_lot_ser
   ...
   do for base_wo_mstr, base_pt_mstr:
      for first base_wo_mstr
         fields(wo_base_id wo_batch wo_joint_type wo_lot wo_lot_next wo_lot_rcpt
                wo_nbr wo_part wo_qty_chg wo_rmks wo_site wo_so_job wo_status
                wo_stat_close_date wo_stat_close_userid wo_type wo_wip_tot)
         where wo_lot = wobaseid
         no-lock:
      end. /* FOR FIRST base_wo_mstr */
   ...
   undo_setd = no.
   /* ADDED OUTPUT PARAMETER l_recv_all, TO   */
   /* INDICATE SINGLE CO / BY-PRODUCT RECEIPT */
/*seastar   {gprun.i ""woworcd.p""
            "(output l_recv_all)"
   } */
/*seastar*/   {gprun.i ""xxwoworcd.p""
            "(output l_recv_all)"
   } 
   if undo_setd
   then
   ...
   then do:
      ...
      do transaction:
         {gprun.i ""reophist.p"" "(input 'RECEIPT',
                                   input wobaseid,
                                   input wobaseid_last_opnbr,
                                   input '',
                                   input wkctr,
   ...
   then do:
      ...
      /* ADDED TWO INPUT PARAMETER. THESE TWO PARAMETER WILL BE USED BY FLOW */
   ...
   then do:
      do transaction:
         for first womstr
            fields(wo_base_id wo_batch wo_joint_type wo_lot wo_lot_next
                   wo_lot_rcpt wo_nbr wo_part wo_qty_chg wo_rmks wo_site
                   wo_so_job wo_status wo_stat_close_date
         ...
         for each womstr
            where womstr.wo_nbr = wo_mstr.wo_nbr
              and womstr.wo_joint_type <> "5"
              and wo_mstr.wo_joint_type > ""
              and womstr.wo_base_id = wo_mstr.wo_lot:
            for first sr_wkfl
               fields(sr_lineid sr_loc sr_lotser sr_qty sr_ref sr_userid)
               where sr_userid = mfguser
         ...
         for first womstr
            fields(wo_base_id wo_batch wo_joint_type wo_lot wo_lot_next
                   wo_lot_rcpt wo_nbr wo_part wo_qty_chg wo_rmks wo_site
                   wo_so_job wo_status wo_stat_close_date wo_stat_close_userid
                   wo_type wo_wip_tot)
            where womstr.wo_nbr = wo_mstr.wo_nbr
            and womstr.wo_joint_type = "5"
            no-lock:
...
PROCEDURE wiplot_trace_receipt:
   ...
   for first wo_mstr
      fields(wo_base_id wo_batch wo_joint_type wo_lot wo_lot_next wo_lot_rcpt
             wo_nbr wo_part wo_qty_chg wo_rmks wo_site wo_so_job wo_status
             wo_stat_close_date wo_stat_close_userid wo_type wo_wip_tot)
      where wo_lot = ip_wo_lot
      no-lock:
   end. /* FOR FIRST wo_mstr */

   for first op_hist
      fields(op_qty_wip op_trnbr)
      where recid(op_hist) = ophist_recid
      no-lock:
