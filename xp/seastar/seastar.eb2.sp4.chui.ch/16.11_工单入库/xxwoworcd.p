...
/*pak*/ define variable pak_logical like mfc_logical initial no.
/*pak*/ define variable pak_bom_qty like wod_bom_qty.
...
with frame r side-labels overlay row 9 column 24
   title color normal
   (getFrameTitle("CO/BY-PRODUCTS_RECEIPT_OPTIONS",48)) width 50
   attr-space.
...
/*seastar begin***********************************************************************/
                  ...
                  if avail wo_mstr then do:                                                      
						...
						if (wo_mstr.wo_qty_comp + tot_units)  >  wo_mstr.wo_qty_ord then do:
							message "超过工单未结数量,不能入库." view-as alert-box.
							pak_logical = yes.
							undo_setd = yes.
							leave setd.
						end.

	                     for each wod_det no-lock where wod_nbr = wo_nbr:
		                           pak_logical = no.
		                           /*pak_bom_qty = round(wod_bom_qty,6).
		                           if pak_bom_qty > wod_bom_qty then pak_bom_qty = pak_bom_qty - 0.000001.
		                           if (wod_qty_iss + (wo_qty_ord * wod_bom_qty - wod_qty_req) < (wo_mstr.wo_qty_comp + tot_units) * pak_bom_qty) and wod_qty_iss < wod_qty_req then do:
		                              message wod_part "工单发放不足不能入库." view-as alert-box.
		                              undo_setd = yes.
		                              leave setd.
		                           end.*/
								   find first pt_mstr where pt_part = wod_part no-lock no-error .
								   if avail pt_mstr then do:
										if pt_group >= "1800" and pt_group <= "1902" then next . 
								   end.

								   if (wo_mstr.wo_qty_comp + tot_units) * wod_qty_req > wod_qty_iss * wo_mstr.wo_qty_ord then do:
								      message "子件" wod_part "发放不足,不能入库." view-as alert-box.
									  pak_logical = yes.
		                              undo_setd = yes.
		                              leave setd.
		                           end.
						 ...
						 find first wr_route  where wr_lot = wo_mstr.wo_lot no-lock no-error .
						 if not avail wr_route then do:
								 message  "工单ID" wo_mstr.wo_lot "尚未做回报,不能入库" view-as alert-box.
								 pak_logical = yes .
						 end.
						 else do:
								 for each wr_route no-lock where wr_lot = wo_lot :
										if wr_qty_comp < wo_mstr.wo_qty_comp + tot_units then do:
											 message  "工序" wr_op  "回报数量不够不能入库" view-as alert-box.
											 pak_logical = yes .
										end.
								 end.
						 end.
