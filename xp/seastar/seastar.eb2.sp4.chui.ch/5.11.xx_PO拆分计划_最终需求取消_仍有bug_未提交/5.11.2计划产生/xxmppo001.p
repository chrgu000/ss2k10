...
define variable outfile   as char format "x(40)" .
...
/*2需求: mrp每天材料需求累计 xreq_det : create in db, unique_index:  site + part + date */
/*3.采购单拆合讯息表 xchg_det :create in db , unique_index: c_site + c_date_to + (c_part) + c_nbr + c_line */
...
put "start run: " today ""  string(time , "HH:MM:SS") skip  . /**/
...
for each mrp_det no-lock use-index mrp_det 
	...
	if first-of(mrp_part) then do:
		v_pm = "P" .
		find first ptp_det where ptp_site = mrp_site and ptp_part = mrp_part no-lock no-error .
		if avail ptp_det then do :
			v_pm = ptp_pm_code .
		end.
		else do:
			find first pt_mstr where pt_part = mrp_part no-lock no-error .
		...
		for each in_mstr where in_site = mrp_site and in_part = mrp_part no-lock :
...
/**/ 
put "end mrp req : " string(time , "HH:MM:SS") skip(3) .
for each xreq_det no-lock 
break by xreq_site by xreq_part by xreq_date 
...
/**/ 
put "end mrp date_to_2year : " string(time , "HH:MM:SS") skip(3) .
for each xreq_det no-lock 
break by xreq_site by xreq_part by xreq_date 
with frame abcd width 300 :
disp xreq_site xreq_part xreq_date xreq_qty xreq_date_to with frame abcd .
end.
put skip(5) .
/**/
...
for each xreq_det exclusive-lock 
	...
	if first-of(xreq_part) then do :
		find first ptp_det where ptp_site = xreq_site and ptp_part = xreq_part no-lock no-error .
		if avail ptp_det then do :
			qty_min = ptp_ord_min .
		...
		for each pod_det where pod_site = xreq_site
		...
		disp xpod_part xpod_nbr xpod_line xpod_ord_date xpod_need xpod_due_date xpod_qty_ord xpod_qty_rcvd  with frame bcd .	
		end.
		put skip(3) .				 /**/
	...
	if last-of(xreq_date_to) then do:
		...
		for each xpod_det exclusive-lock 
			...
			end. /*if xpod_qty_rcvd <> 0 */
			else do: /*if xpod_qty_rcvd = 0 */
				if xpod_due_date <> xreq_date_to or xpod_qty_ord <> v_qty_oh then do:
						/***** not contrl : same req line 合并 ********/
					v_id = v_id + 1 . 
					create xchg_det .
					assign  c_site = xpod_site 
						c_id   = v_id  
					...
					if xpod_qty_ord > v_qty_oh then do:
						c_qty_to = v_qty_oh .
						xpod_qty_ord = xpod_qty_ord - v_qty_oh.
						v_qty_oh = 0 .
						if v_qty_oh = 0 then leave .
					end. /*xpod_qty_ord > v_qty_oh*/
					else do: /*xpod_qty_ord <= v_qty_oh*/
						c_qty_to = xpod_qty_ord .					
						v_qty_oh = v_qty_oh - xpod_qty_ord .
	...
	if last-of(xreq_part) then do:
		...
		for each xpod_det where xpod_site = xreq_site 
			...
			assign  c_site = xpod_site  
				...
				c_date_to = date(01,01,year(today) + 2 )
				c_qty_to  = xpod_qty_ord - xpod_qty_rcvd
				c_detail  = if xpod_qty_rcvd <> 0 then  "XR" else "XN" .
			find first pod_det use-index pod_nbrln 
						where pod_nbr = xpod_nbr 
						and pod_line = xpod_line 
			no-lock no-error .
...
/*MRP无需求的pod,有需求但库存就足够的, 也show在拆分计划*/
		for each po_mstr where po_stat = "" 
						and po_fsm_type = "" /*防止ReturnToSupplier PO = RTS*/
						and po_type = ""     /*防止ReturnToSupplier PO = P ;Blanketpo = B */ 
						no-lock,
			each pod_det where pod_nbr = po_nbr 
						...
						and pod_qty_ord > 0
						no-lock :
			find first xchg_det where c_site = pod_site and c_nbr = pod_nbr and c_line = pod_line and c_part = pod_part no-lock no-error .
			if avail xchg_det then next .
			else do: /*not avail xchg_Det*/
				find first mrp_Det where mrp_dataset = "wod_det" and mrp_site = pod_site and mrp_part = pod_part no-lock no-error .
				if not avail mrp_det then do: /*MRP无需求*/
							
							v_id = v_id + 1 .  
							create xchg_det .
							assign  c_site = pod_site 
								c_id   = v_id 
								c_nbr  = pod_nbr 
								c_line = pod_line 
								c_part = pod_part 
...
put skip(3) "end adjust : " string(time , "HH:MM:SS") skip(3) .   /**/
...
put skip(3) "end disp : " string(time , "HH:MM:SS") .  	/**/
output close .		/**/
