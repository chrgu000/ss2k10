...
if v_type = "" then do:
		...
		repeat while v_need1 >= v_effdate :
			find first hd_mstr where hd_site = site and hd_date = v_need1 no-lock no-error .
			if avail hd_mstr then do:
				v_need1 = v_need1 - 1 .
			end.
			else if weekday(v_need1) = 1 or weekday(v_need1) = 7 then do:
				 v_need1 = v_need1 - 1 .
			end.
			else leave .
		end.

		if v_need1 < v_effdate  then v_need1 = v_effdate .
		...
		case v_type :
			...
			when "C"  then assign v_week1 = 5  v_week2 = 3  v_week3 = 0 .
