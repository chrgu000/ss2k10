...
define var v_yn2   as logical label "全部审核" .
...
repeat with frame a:
    ...
    update site  part part1 nbr nbr1 /*date date1 v_yn1 */ v_yn2 with frame a .
...
v_counter  = 0 .
	...
	for each xchg_Det no-lock where c_site = site 
			...
			/*and c_date_from >= date and c_date_from <= date1 */
		...
		if first-of(c_part) then do:
			v_qty_ord = 0 .
			for each pod_det no-lock where pod_nbr = c_nbr 
									and pod_part = c_part 
									and pod_stat = "" 
									/*and pod_qty_rcvd = 0 */:
	...
	repeat :
		...
		for first temp where t_select = "*" exclusive-lock with frame b :
				...
				if avail pod_det then do :
					...
					if pod_qty_rcvd <> 0  and t_qty <> (pod_qty_ord - pod_qty_rcvd) then do:
						message "警告:采购单" t_nbr "/项"  t_line "已部分收货,不可修改数量".
						pause .
						t_qty = v_qty_tmp .
						undo,retry .
					end.
		...
		if choice then do :
			for each temp no-lock 
				break by t_nbr by t_part by t_line :
					find first xchg_det where c_site = site 
								and c_nbr = t_nbr 
								and c_line = t_line 
								and c_part = t_part
					...
					if avail xchg_Det then do:
							assign  c_date_to = t_date_to 
								c_qty_to  = t_qty 
								c_user01  = global_userid
								c_date01  = today
								c_stat01  = t_app .
					end.
					else do:
						/* create ? no, 未加新增/删除计划功能,不同意计划则,整个零件都不审核即可 */
					end .					
			end .   /*for each temp */
