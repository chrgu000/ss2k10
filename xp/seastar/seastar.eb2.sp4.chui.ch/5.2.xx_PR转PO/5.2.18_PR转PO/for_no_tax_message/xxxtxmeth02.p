/* txmeth02.p - qad EXCLUSIONARY MIN/MAX CONSTRAINTS                       */
...
/* REVISION: 8.6      MODIFIED:      11/12/96   By: rxm  *H0P3*            */
...
/* REVISION: 9.1      LAST MODIFIED: 09/13/00   BY: *N0W4* BalbeerS Rajput */
/* Old ECO marker removed, but no ECO header exists *F0PN*                   */
/* Revision: 1.11.2.7    BY: Manjusha Inglay    DATE: 04/26/02  ECO: *M1X7*  */
/* Revision: 1.11.2.8    BY: Samir Bavkar       DATE: 07/07/02  ECO: *P0B0*  */
/* $Revision: 1.11.2.9 $ BY: Ashish maheshwari  DATE: 10/16/02  ECO: *N1W5*  */
/******************************************************************************/
...
/*!
    ...
    IF THE LINE AMOUNT IS NOT WITHIN THE MIN/MAX RANGE OF THE TAX RATE
    RECORD THEN NO TAXES ARE LEVIED (THE TAXABLE AMOUNT BECOMES 0).

    txmeth02.p  qad Generic Tax Calculation routine
...
{cxcustom.i "TXMETH02.P"}
   ...
   define variable l_mintax                   like tx2d_totamt          no-undo.
   define variable l_maxtax                   like tx2d_totamt          no-undo.
...
/*                   {pxmsg.i &MSGNUM=868 &ERRORLEVEL=2} */
      ...
      /* DON`T APPLY THE TAX IF BELOW MIN OR ABOVE MAX */
      ...
      then
         assign
            ntax_amt     = ntax_amt + tax_base_amt
            tax_base_amt = 0.
