...
/* REVISION: 1.0         Last Modified: 2008/08/08   By: Roger   ECO:*xp001*  */
/* REVISION: 1.0         Last Modified: 2008/07/17   By: Roger   ECO:*xp002*  */
/* REVISION: 1.0         Last Modified: 2008/10/24   By: Roger   ECO:*xp003*  */  /*单价仍然=0的仍然可以过*/ 
/*-Revision end------------------------------------------------------------*/
...
define input parameter v_vend like po_vend .  /*xp001*/
define input parameter v_site like po_site  .  /*xp001*/
...
define var      dnbr                   like pod_nbr .
define var      dline                  as integer initial 0 .
define variable open_qty              like rqd_req_qty no-undo.
define variable qty_um                like rqd_um no-undo.

define temp-table temp 
   field t_nbr like rqd_nbr 
   field t_line like rqd_line 
   field t_ponbr like pod_nbr 
   field t_poline like pod_line 
   field t_aprv  as char 
   field t_rmks  as char
   field t_user  as char 
   field t_date  as date 
   field t_time  as integer .
...
dline = 0 .
for each pod_det
	fields ( pod_nbr pod_line pod_part pod_need ) 
	use-index pod_nbrln /*xp002*/
	where pod_nbr = dnbr 
    exclusive-lock break by pod_line  :

	if dline > 0 then next .

	find first rqpo_wrk where rqpo_copy_to_po 
						  and rqpo_site = v_site 
	                      and rqpo_supplier = v_vend 
						  and rqpo_item <= pod_part no-lock no-error .
	if avail rqpo_wrk then do :
		if rqpo_item < pod_part then do : 
		    if dline = 0 then dline = pod_line .
			next.
        end.
		else do :
		    if rqpo_need_date < pod_need then do:
				if dline = 0 then dline = pod_line .
				next.
			end.
		end.
...
if dline > 0 then do:
	find first pod_det no-lock no-error . /*xp002*/

	repeat :
		find next pod_det   use-index pod_nbrln  /*xp002*/ 
							where pod_nbr = dnbr and pod_line >= dline 
		                    and pod_stat = "" 
	    no-lock no-error .
		if avail pod_det then do:
			  /*find first temp where t_nbr = pod_req_nbr and t_line = pod_req_line exclusive-lock no-error .
				if not avail temp then do: 

				end.
			  */
					create temp .
					assign t_nbr  = pod_req_nbr
					       t_line = pod_req_line 
						   t_ponbr = pod_nbr 
						   t_poline  = pod_line 
						   /*t_aprv = pod__chr01*/ 
						   ...
						   t_user = pod__chr03
						   t_date = pod__dte01 
						   t_time = integer(pod__dec01) .

				/*仅使用popomth.p的删除执行语句,是否可删除语句未使用line 272 to 362 */
	...
	for each temp no-lock ,
        each rqm_mstr use-index rqm_nbr exclusive-lock    /*xp002*/ 
	        where rqm_nbr = t_nbr   /*xp002*/ 
			/*and rqm_rtdto_purch and rqm_open   and rqm_status = ""*/ ,
	    each rqd_det use-index rqd_nbr exclusive-lock   /*xp002*/ 
	        where rqd_nbr = t_nbr and rqd_line = t_line  
			/*and rqd_status = ""   and rqd_open*/ :

				find first rqpo_ref where rqpo_req_nbr = t_nbr 
									and rqpo_req_line = t_line 
									and rqpo_po_nbr = t_ponbr 
									and rqpo_po_line = t_poline 
	...
	end. /*for each temp no-lock*/

end. /*if dline > 0 then*/

release rqpo_wrk.
/* /*xp001*/ end 删除现有项次 ,以便新增后重新排序 --------------------- */

find first rqpo_wrk no-lock 
     where rqpo_supplier = v_vend /*xp001*/
...
{gprun.i ""xxrqpobldg.p""
         "(input true,
           input rqpo_site,
		   input v_vend )"}  /*xp001*/ /* create wkrqd_det with the lowest price of the rqpo_supplier */

find last pod_det use-index pod_nbrln   /*xp002*/ where pod_nbr = dnbr no-lock no-error .
line = if avail pod_Det then pod_line else 0 .  

release rqpo_wrk.


rqpoloop:
for each rqpo_wrk exclusive-lock use-index rqpo_index2 
      where rqpo_supplier = v_vend 
	  and rqpo_copy_to_po /*xp001*/ 
	  and rqpo_site = v_site 
      break by rqpo_supplier by rqpo_item by rqpo_need_date /*xp001*/ 
	  /*by rqpo_nbr by rqpo_line with frame rpt width 132 down*/ 
   :


message  "...正在处理...请购单号:" rqpo_nbr "项:" rqpo_line .

   /* FIND REQUISITION LINE IN TEMP-TABLE */
	find wkrqd_det where
		wkrqd_nbr = rqpo_nbr and
		wkrqd_line = rqpo_line exclusive-lock no-error .
	if not avail wkrqd_det then do :
		rqpo_copy_to_po = no .
		next .
	end.
	else do:
		if wkrqd_pur_cost = 0 then do:
			  for each pc_mstr use-index pc_part 
								where pc_part = wkrqd_part
								 and pc_list = v_vend
								 and pc_um = wkrqd_um 
								 and pc_curr = po_curr 
   ...
   assign
      var_sub    = wkrqd_sub
      var_cc     = wkrqd_cc
      l_netprice = decimal(wkrqd_qadc01).
  ...
  line = line + 1.
   ...
   if line >= 999 then do:
     /* run put_message (input 7418, input 4).
      run put_message (input 1304, input 4).*/
		...
		{pxmsg.i &MSGNUM=7418 &ERRORLEVEL= 4 } 
		{pxmsg.i &MSGNUM=1304 &ERRORLEVEL= 4 }
        next rqpoloop.
   ...
   do with frame site:
      ...
      if wkrqd_site = "" then do:
         find pt_mstr where pt_part = wkrqd_part no-lock no-error.
   ...
   if not available pod_det then do:
      ...
      for first pt_mstr
          fields (pt_part pt_site pt_taxable pt_taxc pt_um pt_prod_line)
          where pt_part = rqpo_item no-lock:
      ...
      assign
         ...
         /* REFERENCE: POPOMTI.P */
         pod_rev            = wkrqd_rev
         pod_per_date       = ? /*wkrqd_due_date20071106*/ 
		...
		/*xp001*/
		find first temp  where t_nbr = pod_req_nbr and t_line = pod_req_line no-lock no-error .
		if avail temp then do :
			/*pod__chr01 = t_aprv .*/
			pod__chr03 = t_user .
			pod__dte01 = t_date .
			pod__dec01 = t_time .
			if pod__chr02 = "" then pod__chr02 =  "项次重整," .
			else if  pod__chr02 begins "项次重整" then pod__chr02 =  t_rmks .
				else pod__chr02 =  "项次重整," +  t_rmks .
...
/*xp002 ---------------------------
      ...
      if wkrqd_curr <> po_curr then do:
         ...
         if wkrqd_curr <> base_curr then do:
            for first rqm_mstr
               fields (  rqm_curr rqm_ex_rate rqm_ex_rate2
                         rqm_fix_rate rqm_nbr )
               where rqm_nbr = wkrqd_nbr no-lock.
            end. /* FOR FIRST */
...
------------------------ xp002*/
      ...
      /* IF SUPPLIER HAS CHANGED SEE IF PURCHASES ACCOUNT SHOULD BE */
      /* RE_DEFAULTED                                               */

      po_tot_terms_code = vd_tot_terms_code.

      if po_vend <> wkrqd_vend then do:

         /* DEFAULT TO GL_CTRL VALUES */
         assign
            pod_acct = gl_pur_acct
            pod_sub  = gl_pur_sub
            pod_cc   = gl_pur_cc.

         /* IF VALID SUPPLIER, DEFAULT TO SUPPLIER'S PURCH ACCT */
         ...
         /* DETERMINE IF DEFAULT GL ACCOUNTS EXISTS IN ACCT_MSTR */
         /* OR PL_MSTR (IF NOT MEMO ITEM)                        */
         if available pt_mstr then do:
            ...
            assign
               pod_acct = dftPURAcct
               pod_sub  = dftPURSubAcct
               pod_cc   = dftPURCostCenter.
         end.  /* IF AVAILABLE PT_MSTR */
      end.  /* IF WKRQD_VEND <> PO_VEND */
...
/*xp001*/ /*         display
      ...
      /* CREATE REQ - PO CROSS-REFERNCE RECORD */
      {gprun.i ""xxrqporef.p""
         "(input true,
           input pod_site,
           input pod_req_nbr,
...
/*xp001*/ /*         display
      ...
      then do:
         ...
         then do:
            ...
            if linecost = var_pur_cost then do:
            ...
            if unitcost = var_pur_cost then do:
      ...
      if po_pr_list2 <> "" then do:
         ...
         * the Net Price */
         ...
         if dummy_cost <> ? then
            ...
            /*xp001  dummy_cost = dummy_cost * (1 - pod_disc_pct / 100).  */
			/*xp001*/ dummy_cost = var_pur_cost * (1 - pod_disc_pct / 100).
			/*xp001*/ /*根据popomta--popoxr1.p line: 1167 , 修改此处 dummy_cost*/ 
 ...
 /*xp001*/ /*        display
      ...
      if var_req_cost <> 0 then do:
         if dummy_cost <> 0 and
            var_req_cost > dummy_cost then do:
...
/*xp001*/ /*                     display
                  ...
                  if var_req_cost <> 0
                     ...
                     and var_req_cost > var_vend_pur_cost then do:
...
/*xp001*/ /*                     display
               ...
               if var_req_cost <> 0 and var_vend_pur_cost <> 0 and
                  var_req_cost > var_vend_pur_cost then do:
                  /* REQUISITION PRICE IS GREATER THAN VENDOR PRICE */
/*xp001*/ /*                  display
                     var_req_cost
                     var_vend_pur_cost @ dummy_cost
      ...
      if include_lcmmts and wkrqd_cmtindx <> 0 then do:
         ...
         for each wkcmt_det where
               wkcmt_det.cmt_indx = wkrqd_cmtindx no-lock:
            if i = -1 then do:
               ...
               if available(cmt_det) then
                  pod_cmtindx = cmt_det.cmt_indx + 1.
               else
                  pod_cmtindx = 1.
      ...
      if using_consignment and pod_consignment then
         display c-consigment-contract no-label. xp001*/
...
/*xp001*/
/*xp001 begin :单位转换率应该是pod_um与基本单位pt_um的转换率，而非报价单位与pod_um的
   for each vp_mstr no-lock
      where vp_part = p_part
        and vp_vend = p_vend
      break by vp_q_date descending:
