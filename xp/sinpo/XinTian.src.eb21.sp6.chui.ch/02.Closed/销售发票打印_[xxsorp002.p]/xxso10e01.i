...
if sod_fsm_type <> "FSM-RO" then v_qty_inv = sod_qty_inv .
if sod_fsm_type = "FSM-RO" and available itm_det then v_qty_inv = itm_qty_call .

if (sod_fsm_type <> "FSM-RO"
     or (sod_fsm_type =  "FSM-RO" and  sod_fix_pr   = no)) 
then v_price = sod_price .
if (sod_fsm_type =  "FSM-RO" and  sod_fix_pr)  then v_price = sod_fixed_price .
