...
/* Revision: 1.29.1.31.1.3 BY: Dayanand Jethwa     DATE: 02/24/05 ECO: *P27M* */
/* Revision: 1.29.1.31.1.3 BY: Bhagyashri Shinde   DATE: 07/20/04 ECO: *P2CR* */
/* Revision: 1.29.1.31.1.4 BY: Dinesh Dubey        DATE: 01/09/06 ECO: *P3HZ* */
/* Revision: 1.29.1.31.1.5 BY: Preeti Sattur       DATE: 08/29/06 ECO: *P531* */
/* Revision: 1.29.1.31.1.6 BY: Rafiq S.            DATE: 11/15/06 ECO: *P4YY* */
/* Revision: 1.29.1.31.1.7 BY: Ashwini G.          DATE: 12/22/06 ECO: *P5KF* */
/* $Revision: 1.29.1.31.1.8 $ BY: Ed van de Gevel   DATE: 02/21/07   ECO: *P5PD* */
/*----rev history-------------------------------------------------------------------------------------*/
...
define variable l_gst_id      like ad_gst_id              no-undo.
define variable l_prnt_tax_id like mfc_logical            no-undo.
define variable l_ret_flag    like mfc_logical initial no no-undo.
...
with frame sototeuro side-labels width 132 attr-space.
...
form
   ...
   vcCreditCardMsg[3]
with frame soccmsg no-labels width 132 attr-space.
...
find first sac_ctrl  where sac_ctrl.sac_domain = global_domain no-lock no-error.
...
for first qad_wkfl
   ...
   and   qad_key1   = "sosopm.p"
   and   qad_key2   = "ad_gst_id"
no-lock:
   l_prnt_tax_id = qad_logfld[1].
...
then do:
   ...
   for first so_mstr  where so_mstr.so_domain = global_domain and  so_nbr =
   l_so_nbr
   no-lock: end .
end.
...
repeat while available so_mstr:
   ...
   {gprunp.i "txincopl" "p" "empty-table"}
   ...
   do:
      ...
      then do:
         ...
         if can-find (lgs_mstr  where lgs_mstr.lgs_domain = global_domain and
         lgs_app_id = so_app_owner
            and lgs_invc_imp = yes no-lock) then do:
            /* Finish it up without a print */
            so_recno = recid(so_mstr).
            {&SORP1001-P-TAG34}

            /* ADDED OUTPUT PARAMETER 'l_ret_flag' TO INDICATE */
      ...
      if can-find(qad_wkfl  where qad_wkfl.qad_domain = global_domain and
      qad_key1 = "sosois.p" + so_nbr
                             and qad_key2 = "BATCH" )
      then
         leave blocka.
      {&SORP1001-P-TAG5}
      ...
      then do:
         ...
         for each sod_det
             fields(sod_domain sod_nbr sod_price sod_qty_inv
                    sod_rma_type)
      ...
      so_bill no-lock no-wait no-error.
      update billto = "".
      ...
      find ad_mstr  where ad_mstr.ad_domain = global_domain and  ad_addr =
      so_cust no-lock no-wait no-error.
      update soldto = "".
      if available ad_mstr then do:
         {&SORP1001-P-TAG17}
         assign
      ...
      hdr_call = "".
      if rmaso then do.
         find first rma_mstr  where rma_mstr.rma_domain = global_domain and
         rma_nbr = so_nbr
                               and rma_prefix = "C"
         no-lock no-error.
...
/* SS - 100726.1 - B 
   ...
   SS - 100726.1 - E */
/* SS - 100726.1 - B */
define var v_gst_id   as char .
define var v_addr     as char .
define var v_custacct as char .
define var v_custname like ad_name .
    ...
    v_gst_id   = if avail ad_mstr then ad_gst_id else "" .
    v_addr     = if avail ad_mstr then ad_line1 + ad_line2 + ad_line3 else "" .

    find first cm_mstr where cm_domain = global_domain and cm_addr = so_cust no-lock no-error.
    v_custname = if avail cm_mstr then cm_sort else "" .
    v_custacct = if avail cm_mstr then cm__chr01 + cm__chr02  else "" .

        put unformatted 
            "销售订单号:;" "'" so_nbr  ";"
            "发票号:;"     "'" so_inv_nbr ";"
            "发运日期:;"   string(year(so_ship_date),"9999") + "/" + string(month(so_ship_date),"99") + "/" + string(day(so_ship_date),"99") ";"
            "开票日期:;"   string(year(inv_date),"9999") + "/" + string(month(inv_date),"99") + "/" + string(day(inv_date),"99") ";"
            "推销员:;"     so_slspsn[1] ";"
            skip.
        ...
        put unformatted 
            "物料号;物料说明;计量单位;发票数量;欠交量;是否计税;价格;合计价格;备注" 
            skip.
   ...
   SS - 100726.1 - E */
      ...
      if rmaso then do:
         if available rma_mstr then do:
            ...
            find first ca_mstr  where ca_mstr.ca_domain = global_domain and
            ca_nbr = rma_ca_nbr
                                and ca_category = "0"
            no-lock no-error.
...
/* SS - 100726.1 - B 
   ...
   SS - 100726.1 - E */
      ...
      if conso
         or (not conso and l_inv_conso)
      then do:
         {gprunp.i "txincopl" "p" "create-record"
                   "(input recid(so_mstr),
                     input tax-tran-type)" }
         ...
         then do:
            for each somstr2
               fields ( so_app_owner so_ar_acct  so_ar_cc     so_ar_sub
                        so_bill      so_bol      so_cmtindx   so_cr_card
                        so_cr_init   so_cr_terms so_curr      so_cust
                        so_disc_pct  so_domain   so_due_date  so_fob
                        so_fsm_type  so_invoiced so_inv_nbr   so_lang
                        so_nbr       so_ord_date so_po        so_prepaid
                        so_prep_tax  so_print_pl so_print_so  so_quote
                        so_req_date  so_rev      so_rmks      so_sa_nbr
                        so_ship      so_shipvia  so_ship_date so_site
                        so_slspsn    so_stat     so_to_inv    so_trl1_amt
            ...
            undo_trl2 = true.
            {gprun.i ""soivtrl2.p"" "(input so_mstr.so_inv_nbr,
                                      input (if conso
                                            or (not conso and l_inv_conso)
                                            then
                                               '*'
...
/* SS - 100726.1 - E */
   ...
   if (execname <> "rcsois.p"  and
       ...
       not lgData)
   then do:
      find next so_mstr  where so_mstr.so_domain = global_domain and
...
/* THIS PROCEDURE PROCESSES CONSOLIDATIONS OF SALES ORDERS */
   ...
   for first somstr3  where somstr3.so_domain = global_domain and  so_nbr =
   l_so_nbr3
   no-lock: end.
...
PROCEDURE p_consotot:
   ...
   for each somstr4
      ...
      no-lock :
         ...
         undo_trl2 = true.
         {gprun.i ""soivtrl2.p"" "(input somstr4.so_inv_nbr,
                                   input '*',
                                   input col-80 /* REPORT WIDTH */,
                                   input '16',
                                   input tot_cont_charge,
...
PROCEDURE p_check_locked:
   ...
   then do:
      find next so_mstr
          where so_mstr.so_domain = global_domain and  (so_nbr >= nbr and
          so_nbr <= nbr1)
         and   (so_to_inv = yes)
         and   (so_ship_date >=shipdate and so_ship_date <= shipdate1)
         and   (so_cust >= cust and so_cust <= cust1)
