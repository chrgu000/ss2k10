...
/* REVISION: 1.20          Created: 11/17/2008    BY: xie yu lin        */
	 ...
	 define variable lot like ld_lot.
         ...
         define variable serial as char format "x(4)" label "Serial".
	 ...
	 define variable wsection as char format "x(16)".
	    ...
	    if lot1 = hi_char then lot1  = "".
            ...
            update part part1 loc loc1 lot lot1 with frame a.


            {wbrp06.i &command = update
                      &fields = " loc loc1 lot lot1"
                      &frm = "a"}
	    ...
	    for each ld_det no-lock
		 ...
		 and ld_qty_oh <> 0
	    break by ld_loc by ld_part:

               find first pt_mstr where pt_part = ld_part no-lock no-error.

	       if available pt_mstr then do:         
	          if /*pt_prod_line = "FS00" or pt_prod_line = "FP00" judy*/ 
                     pt_prod_line <= "Fs00"  /*judy*/ then do:

		     run prt_lbl(input ld_part, input pt_desc1, input pt_desc2, input ld_lot, input ld_qty_oh).
	       ...
	       DEFINE INPUT PARAMETER i_lot like ld_lot.
               ...
               INPUT FROM VALUE("/mfgeb2/script_bc/labels/tfs.txt").
