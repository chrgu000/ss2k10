...
/* Revision: 1.22.3.1   BY: Rajiv Ramaiah        DATE: 06/06/03 ECO: *P0V1*   */
/* $Revision: 1.22.3.2 $         BY: Vivek Gogte          DATE: 09/23/03 ECO: *P13R*   */
/******************************************************************************/
...
repeat with frame a:
   ...
   repeat with frame a:
      ...
      editing:
         ...
         {mfnp01.i wod_det wod_part wod_part lot wod_lot wod_det}
         ...
         if recno <> ? then do:
            ...
            if can-find(first lad_det where lad_dataset = "wod_det"
                                        and lad_nbr = wod_lot
      ...
      if not available wod_det then do:
         ...
         find pt_mstr where pt_part = input wod_part no-lock no-error.
      ...
      then do:
         ...
         if detail_all or (wod_qty_req entered
            and can-find(first lad_det where lad_dataset = "wod_det"
                                         and lad_nbr = wod_lot
                                         and lad_line = string(wod_op)
                                         and lad_part = wod_part))
         then do:
