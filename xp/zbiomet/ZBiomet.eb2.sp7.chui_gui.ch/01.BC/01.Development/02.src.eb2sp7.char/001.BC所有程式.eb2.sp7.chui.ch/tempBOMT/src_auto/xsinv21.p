...
batchrun = no . /*cimload,有些会改成yes,这里改回来*/
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1100 no-box.
            ...
            THEN DO:
               if v_recno = ? then
                  find first CODE_MSTR where 
                              CODE_FLDNAME = "ISS-UNPCODE" AND  
                              CODE_VALUE >  INPUT V1100
                   no-lock no-error.
               else
                  find next CODE_MSTR where 
                              CODE_FLDNAME = "ISS-UNPCODE"
                   no-lock no-error.
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        find first CODE_MSTR where CODE_FLDNAME = "ISS-UNPCODE" AND CODE_VALUE = V1100  no-lock no-error.
        IF NOT AVAILABLE CODE_MSTR then do:
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If available pt_mstr then do:
   if can-find(first isd_det  where isd_status = string(pt_status,"x(8)") + "#" and ( isd_tr_type = "ISS-UNP" ))
   then do:
...
End. /* available pt_mstr */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        PV1300 = PV1300 + "@@" .
        Find first ld_det where ld_site = v1002 and ld_part = v1300 and (ld_lot = ENTRY(2,PV1300,"@") or ENTRY(2,PV1300,"@") = "" ) and ld_stat = "Normal" and ld_qty_oh > 0  no-lock no-error .
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1500L.
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
                ...
                /* LABEL 2 - START */ 
                find first ld_det where ld_part = V1300 and ld_loc = V1400 and ld_qty_oh <> 0 and ld_site = V1002 and ld_ref = "" use-index ld_part_lot no-lock no-error.
...
If AVAILABLE ( ld_det ) then
        ...
        /* CHECK FOR NUMBER VARIABLE START  */
        /* CHECK FOR NUMBER VARIABLE  END */
        define var v_acc1 as char .    v_acc1 = "" .
        find first code_mstr where code_fldname = "ISS-UNPCODE" and V1100 = code_value no-lock no-error.
        If AVAILABLE ( code_mstr ) then v_acc1 = ENTRY(2,code_cmmt,"@").
        ...
        Find first ld_det 
            ...
            and ld_loc = v1400 
            and ld_part = v1300 
            and ld_lot = ( IF INDEX(V1500,"@" ) <> 0 then ENTRY(2,V1500,"@") else V1500 ) 
            and ld_ref = "" 
        no-lock no-error .
        If not avail ld_Det then do:
                display "无效库存批号" @ WMESSAGE NO-LABEL with fram F1500.
                pause 0 before-hide.
                Undo, retry.
        End.
        Else do:
            if ld_qty_oh <= 0 then do:
                display "数量不足" @ WMESSAGE NO-LABEL with fram F1500.
                pause 0 before-hide.
                Undo, retry.
            End.
            If ld_expire <> ?  And ld_expire < today and v_acc1 <> "600037" then do:
                display skip "库存过期" + string(ld_expire) @ WMESSAGE NO-LABEL with fram F1500.
                pause 0 before-hide.
                Undo, retry.                
            End.
        End.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
                ...
                /* LABEL 4 - START */ 
                find first ld_det where ld_part = V1300 and ld_loc = V1400 and ld_lot = V1500 and ld_qty_oh <> 0  AND LD_SITE = V1002 AND LD_REF = "" use-index ld_part_lot no-lock no-error.
If AVAILABLE ( ld_det ) then
        ...
        /* CHECK FOR NUMBER VARIABLE  END */
        find first LD_DET where LD_SITE = V1002 AND ld_loc = V1400 and ld_part  = V1300 AND  ld_lot = V1500  AND  ld_ref = "" and ( ld_QTY_oh >= DECIMAL ( V1600 )  AND DECIMAL (V1600 ) > 0 )  no-lock no-error.
        IF NOT AVAILABLE LD_DET then do:
...
If AVAILABLE ( code_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1610 no-box.
...
If AVAILABLE ( code_mstr ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1620 no-box.
        ...
        no-lock no-error .
        If avail ld_Det then do:
            If ld_expire <> ?  And ld_expire < today and V1620  <> "600037" then do:
                display skip "过期库存限定科目为:600037" @ WMESSAGE NO-LABEL with fram F1620.
                Undo, retry.                
            End.
        End.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1630 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.

                /* LABEL 1 - START */ 
                find last tr_hist where 
tr_date = today     and 
tr_trnbr > integer ( V9000 ) and 
tr_type = "ISS-UNP" and  tr_part = V1300     and
tr_SITE = V1002     and tr_serial = V1500   and 
tr_time  + 15 >= TIME 
...
tr_date = today     and 
tr_trnbr > integer ( V9000 ) and 
tr_type = "ISS-UNP" and  tr_part = V1300     and tr_serial = V1500   and tr_SITE = V1002     and
tr_time  + 15 >= TIME 
use-index tr_date_trn no-lock no-error.
If AVAILABLE ( tr_hist ) then
                ...
                tr_time  + 15 >= TIME 
                use-index tr_date_trn no-lock no-error.
                If NOT AVAILABLE ( tr_hist ) then L90103 = "交易提交失败" .
                Else do:
                    define var v_qty_tmp like ld_qty_oh .
                    V_qty_tmp = 0 .
                    For each in_mstr where in_site = v1002 and in_part = v1300 no-lock :
                        v_qty_tmp = in_qty_oh + v_qty_tmp .
                    End.
                    L90103 = "安全/剩余库存:" + string(v_qty_tmp) .
                    find first ptp_det where ptp_part = v1300 and ptp_site = v1002 no-lock no-error .
                    if avail ptp_det then do:
                        L90103 = "安全/剩余库存:" + string(ptp_sfty_stk) + "/" + string(v_qty_tmp) . 
                        if ptp_sfty_stk > v_qty_tmp then do:
                            L90104 = "警告:低于安全库存" .
                        end.
                    end.
                    else do:
                        ...
                        if avail pt_mstr then do:
                            L90103 = "安全/剩余库存:" + string(pt_sfty_stk) + "/" + string(v_qty_tmp) . 
                            if pt_sfty_stk > v_qty_tmp then do:
                                L90104 = "警告:低于安全库存" .
                            end.
                        end.
                    end.     
   ...
   IF NOT (V9010 = "#" AND V1700 = "#" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[计划外出库]"       + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
   ...
   /* Additional Labels Format */
     ...
     PROCEDURE inv219130l.
        ...
        vv_label1 = (LabelsPath + "inv21" + trim ( wtm_fm ) ). /*xp001*/
     INPUT FROM VALUE(LabelsPath + "inv21" + trim ( wtm_fm ) ).
...
If AVAILABLE ( pt_mstr )  then
     ...
     END PROCEDURE.
     run inv219130l  (output vv_part2 , output vv_lot2 ,output vv_qtyp2 ,output vv_label2 ,output vv_filename2 ,output vv_oneword2 ) .  /*xp001*/
