...
LD_SITE = V1002 AND LD_REF = ""
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find first ld_det where ld_site = V1002 and ld_part = V1300 and  ld_lot  = V1500 and ld_qty_oh <> 0  no-lock no-error.
If AVAILABLE ( ld_det ) then
        V1510 = ld_loc.
        V1510 = ENTRY(1,V1510,"@").
        /* --FIRST TIME DEFAULT  VALUE -- END  */
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* LOGICAL SKIP END */
                ...
                /* LABEL 5 - START */ 
                  L15205 = "" . 
                display L15205          format "x(40)" skip with fram F1520 no-box.
                /* LABEL 5 - END */ 
            ...
            THEN DO:
               if v_recno = ? then
                  ...
                  IF AVAILABLE LOC_MSTR then display skip 
                         LOC_LOC @ V1520 LOC_DESC @ WMESSAGE NO-LABEL with fram F1520.
        ...
        /* PRESS e EXIST CYCLE */
        IF V1520 = "*" THEN  LEAVE V1300LMAINLOOP.
        display  skip WMESSAGE NO-LABEL with fram F1520.
...
If AVAILABLE ( upd_det ) then
   ...
   /* Additional Labels Format */
     ...
     PROCEDURE inv239130l.
        ...
        vv_label1 = (LabelsPath + "inv23" + trim ( wtm_fm ) ). /*xp001*/
...
If AVAILABLE ( pt_mstr )  then
     ...
     END PROCEDURE.
     run inv239130l  (output vv_part2 , output vv_lot2 ,output vv_qtyp2 ,output vv_label2 ,output vv_filename2 ,output vv_oneword2 ) .  /*xp001*/
