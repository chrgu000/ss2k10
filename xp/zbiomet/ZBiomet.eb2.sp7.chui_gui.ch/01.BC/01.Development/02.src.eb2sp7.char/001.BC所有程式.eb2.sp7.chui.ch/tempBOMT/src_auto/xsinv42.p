   ...
   REPEAT:
     ...
     REPEAT:
        ...
        If available pt_mstr then do:
           if can-find(first isd_det  where isd_status = string(pt_status,"x(8)") + "#" and ( isd_tr_type = "TAG-CNT" ))
           then do:
        ...
        End. /* available pt_mstr */
              ...
              disp "零件不存在"   @ WMESSAGE NO-LABEL with fram F1300.
              undo, retry.
        End.


        Find first ld_det where ld_site = v1002 and ld_part = ENTRY(1, v1300, "@") and ld_lot = ENTRY(2, v1300, "@") and ld_stat = "normal" no-lock no-error.
        If not avail ld_det then do:
            disp "批号不存在"   @ WMESSAGE NO-LABEL with fram F1300.
            undo, retry.
        End.
         /*  ---- Valid Check ---- END */
...
If AVAILABLE ( tag_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                /* LABEL 5 - START */ 
                  L16055 = "" . 
                display L16055          format "x(40)" skip with fram F1605 no-box.
                /* LABEL 5 - END */ 


                /* LABEL 6 - START */ 
                  L16056 = "" . 
                display L16056          format "x(40)" skip with fram F1605 no-box.
                /* LABEL 6 - END */ 
                display "确认或按*退出 "      format "x(40)" skip
        ...
        /* PRESS e EXIST CYCLE */
        IF V1605 = "*" THEN  LEAVE V1300LMAINLOOP.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --DEFINE VARIABLE -- START */
        hide all.
        define variable V1607           as char format "x(50)".
        define variable PV1607          as char format "x(50)".
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        V1607 = "".
Find first tag_mstr where tag_type = "B" and tag_part = ""  and not tag_void and not tag_posted  and V1605 = "" no-lock no-error.
If AVAILABLE ( tag_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[初盘(BY ITEM)]" + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1607 no-box.

                /* LABEL 1 - START */ 
                L16071 = "BLANK TAG" .
                display L16071          format "x(40)" skip with fram F1607 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                  L16072 = "" . 
                display L16072          format "x(40)" skip with fram F1607 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                  L16073 = "" . 
                display L16073          format "x(40)" skip with fram F1607 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                  L16074 = "" . 
                display L16074          format "x(40)" skip with fram F1607 no-box.
                /* LABEL 4 - END */ 


                /* LABEL 5 - START */ 
                  L16075 = "" . 
                display L16075          format "x(40)" skip with fram F1607 no-box.
                /* LABEL 5 - END */ 


                /* LABEL 6 - START */ 
                  L16076 = "" . 
                display L16076          format "x(40)" skip with fram F1607 no-box.
                /* LABEL 6 - END */ 
                display "确认或按*退出 "      format "x(40)" skip
        skip with fram F1607 no-box.
        /* DISPLAY ONLY */
        ...
        /* PRESS e EXIST CYCLE */
        IF V1607 = "*" THEN  LEAVE V1300LMAINLOOP.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        Find first tag_mstr where not tag_void and not tag_posted and string ( tag_nbr ) = V1605  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
        V1608 = tag_type.
        V1608 = ENTRY(1,V1608,"@").
        /* --FIRST TIME DEFAULT  VALUE -- END  */
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                /* LABEL 5 - START */ 
                  L16085 = "" . 
                display L16085          format "x(40)" skip with fram F1608 no-box.
                /* LABEL 5 - END */ 


                /* LABEL 6 - START */ 
                  L16086 = "" . 
                display L16086          format "x(40)" skip with fram F1608 no-box.
                /* LABEL 6 - END */ 
                display "确认或按*退出 "      format "x(40)" skip
        ...
        /* PRESS e EXIST CYCLE */
        IF V1608 = "*" THEN  LEAVE V1300LMAINLOOP.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        find first ld_det where ld_site = v1002 and ld_loc = v1200 and ld_part = v1300 and ld_lot = v1500 and ld_ref = "" no-lock no-error .
        ...
        /* LOGICAL SKIP END */
                ...
                /* LABEL 4 - START */ 
                L16204 = "库位/库存:" + trim ( V1200 ) + "/" + string(v_qty_ld) .
                display L16204          format "x(40)" skip with fram F1620 no-box.
                /* LABEL 4 - END */ 


                /* LABEL 5 - START */ 
                L16205 = "标签号:" + ( if V1605 = "" then V1607 else V1605 ) + if V1605 = "" then "  **空白标签**" else "" .
        ...
        /* PRESS e EXIST CYCLE */
        IF V1620 = "*" THEN  LEAVE V1620LMAINLOOP.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP END */
                ...
                /* LABEL 2 - START */ 
                L17002 = "批号/库位:" + trim ( V1500 ) + "/" +  trim ( V1200 ) .
                display L17002          format "x(40)" skip with fram F1700 no-box.
                /* LABEL 2 - END */ 


                /* LABEL 3 - START */ 
                L17003 = "库存数/盘点数:" + trim (string(v_qty_ld)) + "/" + trim( V1620 ) .
                display L17003          format "x(40)" skip with fram F1700 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                L17004 = "标签号:" + ( if V1605 = "" then "[B]" + TRIM( V1607 ) else TRIM ( V1605 ) ) .
                display L17004          format "x(40)" skip with fram F1700 no-box.
                /* LABEL 4 - END */ 


                /* LABEL 5 - START */ 
                L17005 = if v_qty_ld <> decimal(v1620) then "**差异**" else  "" .
        ...
        /* PRESS e EXIST CYCLE */
        IF V1700 = "*" THEN  LEAVE V1620LMAINLOOP.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        batchrun = yes  . 
/*roger add : cimload pitcmt1.p 时,非bathrun的缺一个assign tag_part = input tag_part.: "305行: if batchrun then tag_part = input tag_part." */
...
If AVAILABLE ( tr_hist ) then
        ...
        /* PRESS e EXIST CYCLE */
        IF V9000 = "*" THEN  LEAVE V1620LMAINLOOP.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP END */
                ...
                /* LABEL 1 - START */ 
                find first tag_mstr where ( tag_nbr = integer(V1605) or tag_nbr = integer(V1607) ) and tag_part = trim ( V1300 )  and tag_cnt_qty = decimal (V1620)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
                L90101 = "交易已提交" .
                else L90101 = "" . 
...
If AVAILABLE ( tag_mstr ) then
        ...
        /* PRESS e EXIST CYCLE */
        IF V9010 = "*" THEN  LEAVE V1620LMAINLOOP.
...
If AVAILABLE ( upd_det ) then
   ...
   /* Additional Labels Format */
     ...
     PROCEDURE inv429130l.
        ...
        vv_label1 = (LabelsPath + "inv42" + trim ( wtm_fm ) ). /*xp001*/
...
If AVAILABLE ( pt_mstr )  then
     ...
     END PROCEDURE.
     run inv429130l  (output vv_part2 , output vv_lot2 ,output vv_qtyp2 ,output vv_label2 ,output vv_filename2 ,output vv_oneword2 ) .  /*xp001*/
   ...
   END.
   pause 0 before-hide.
     /* START  LINE :9140  for_end_v1300mainloop  */
     V9140L:
     REPEAT:
        ...
        leave V9140L.
        /* LOGICAL SKIP END */
                display "[初盘(BY ITEM)]" + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9140 no-box.
                ...
                /* LABEL 5 - START */ 
                  L91405 = "" . 
                display L91405          format "x(40)" skip with fram F9140 no-box.
                /* LABEL 5 - END */ 


                /* LABEL 6 - START */ 
                  L91406 = "" . 
                display L91406          format "x(40)" skip with fram F9140 no-box.
                /* LABEL 6 - END */ 
                display "确认或按*退出 "      format "x(40)" skip
     ...
     END.
     PV9140 = V9140.
     /* END    LINE :9140  for_end_v1300mainloop  */


   /* Additional Labels Format */
   /* Internal Cycle END :9140    */
   END.
   pause 0 before-hide.
end.
