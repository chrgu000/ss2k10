...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv44wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
batchrun = no . /*cimload,有些会改成yes,这里改回来*/
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1205 no-box.
            ...
            THEN DO:
               if v_recno = ? then
                  find first TAG_MSTR where 
                              NOT TAG_VOID AND NOT TAG_POSTED and ((not can-find(first ld_det where ld_site = tag_site and ld_loc = tag_loc and ld_part = tag_part  and ld_lot = tag_serial and ld_ref = "" no-lock))  
or can-find(first ld_Det where ld_site = tag_site and ld_loc = tag_loc and ld_part = tag_part  and ld_lot = tag_serial and ld_ref = "" and ld_qty_frz <> tag_cnt_qty no-lock)) 

 AND TAG_LOC = V1200  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = "" AND  
...
or can-find(first ld_Det where ld_site = tag_site and ld_loc = tag_loc and ld_part = tag_part  and ld_lot = tag_serial and ld_ref = "" and ld_qty_frz <> tag_cnt_qty no-lock)) 
 ...
 AND TAG_LOC = V1200  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = ""
                   no-lock no-error.
...
or can-find(first ld_Det where ld_site = tag_site and ld_loc = tag_loc and ld_part = tag_part  and ld_lot = tag_serial and ld_ref = "" and ld_qty_frz <> tag_cnt_qty no-lock)) 
 ...
 AND TAG_LOC = V1200  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = ""
                  ...
                  IF AVAILABLE TAG_MSTR then display skip 
                         string( TAG_NBR )  @ V1205 string ( TAG_NBR ) + "/" + trim ( TAG_PART ) + "/" + trim(tag_serial) @ WMESSAGE NO-LABEL with fram F1205.
 ...
 AND TAG_LOC = V1200  AND TAG_SITE = V1002 AND TAG_CNT_DT <> ?  AND TAG_REF = ""
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE ( tag_mstr ) then
        ...
        find first TAG_MSTR where TAG_NBR = integer ( V1205 ) AND ( ( V1300 = "" ) OR ( TAG_PART = ENTRY(1, V1300, "@") ) AND TAG_SERIAL = ( if ENTRY(2, V1300, "@") = "" then "" else ENTRY(2, V1300, "@") ) )  no-lock no-error.
        IF NOT AVAILABLE TAG_MSTR then do:
...
If AVAILABLE ( tag_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        leave V1600L.
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.

                /* LABEL 1 - START */ 
                L16001 = "复盘数量(" + trim(V1200) + ")?" .
...
If AVAILABLE ( tag_mstr ) then
                ...
                If AVAILABLE ( tag_mstr ) then do:
                    find first ld_det where ld_site = tag_site and ld_loc = tag_loc and ld_part = tag_part and ld_lot = tag_serial and ld_ref = tag_ref no-lock no-error .
                ...
                L16005 = "库位/库存数:" + trim ( V1200 ) + "/" + string(v_qty_ld).
                if 1 = 1 then
                L16005 = l16005 .
                else L16005 = "" . 
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
...
If AVAILABLE ( tag_mstr ) then
                ...
                find first tag_mstr where tag_nbr = integer(V1205)  no-lock no-error.
                If AVAILABLE ( tag_mstr ) then
                L17003 = "批号/库位:" + trim ( tag_serial ) + "/" + trim ( V1200 ) .
                else L17003 = "" . 
                ...
                /* LABEL 4 - START */ 
                L17004 = "库存数/盘点数:" + trim (string(v_qty_ld)) + "/" + trim( V1600 ) .
                display L17004          format "x(40)" skip with fram F1700 no-box.
                /* LABEL 4 - END */ 


                /* LABEL 5 - START */ 
                L17005 = if v_qty_ld <> decimal(v1600) then "**差异**" else  "" .
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.

                /* LABEL 1 - START */ 
                find first tag_mstr where tag_nbr = integer(V1205) and tag_rcnt_qty = decimal (V1600)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
                L90101 = "交易已提交" .
                else L90101 = "" . 
...
If AVAILABLE ( tag_mstr ) then
   ...
   IF NOT (V9010 = "#" AND V1700 = "#" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.
...
If AVAILABLE ( tag_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
...
If AVAILABLE ( tag_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --FIRST TIME DEFAULT  VALUE -- START  */
        find first upd_det where upd_nbr = "INV44" and upd_select = 99 no-lock no-error.
If AVAILABLE ( upd_det ) then
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[复盘(BY NBR)]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
   ...
   /* Additional Labels Format */
     ...
     PROCEDURE inv449130l.
        ...
        vv_label1 = (LabelsPath + "inv44" + trim ( wtm_fm ) ). /*xp001*/
     INPUT FROM VALUE(LabelsPath + "inv44" + trim ( wtm_fm ) ).
     ...
     Do While True:
       ...
       find first tag_mstr where tag_nbr = integer(V1205)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
        av9130 = trim(tag_part) + "@" + trim(tag_serial).
...
If AVAILABLE ( tag_mstr ) then
       ...
       END.
       find first pt_mstr where pt_part = V1410  no-lock no-error.
...
If AVAILABLE ( pt_mstr )  then
       ...
       find first tag_mstr where tag_nbr = integer(V1205)  no-lock no-error.
If AVAILABLE ( tag_mstr ) then
        av9130 = if length( trim ( Tag_serial ) ) >= 8 then substring ( trim ( tag_serial ),7,2) else "00".
...
If AVAILABLE ( pt_mstr )  then
     ...
     END PROCEDURE.
     run inv449130l  (output vv_part2 , output vv_lot2 ,output vv_qtyp2 ,output vv_label2 ,output vv_filename2 ,output vv_oneword2 ) .  /*xp001*/
