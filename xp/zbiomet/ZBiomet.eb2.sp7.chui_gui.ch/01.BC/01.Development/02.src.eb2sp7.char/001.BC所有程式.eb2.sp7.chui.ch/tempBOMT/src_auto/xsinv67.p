...
find first code_mstr where code_fldname = "BARCODE" AND CODE_value ="xsinv67wtimeout" no-lock no-error. /*  Timeout - Program Level */
if AVAILABLE(code_mstr) Then wtimeout = 60 * decimal(code_cmmt).
...
batchrun = no . /*cimload,有些会改成yes,这里改回来*/
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[WhatIn货架查询]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[WhatIn货架查询]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 no-box.
        ...
        IF NOT AVAILABLE LOC_MSTR then do:
                display skip "错误,重试." @ WMESSAGE NO-LABEL with fram F1200.
                pause 0 before-hide.
                undo, retry.
        end.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[WhatIn货架查询]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
                ...
                /* LABEL 2 - START */ 
                find first ld_det where ld_loc = V1200   and  ld_qty_oh <> 0   and ld_site = V1002 no-lock no-error.
If AVAILABLE ( ld_det ) then
                ...
                /* LABEL 3 - START */ 
                find last ld_det where ld_loc = V1200   and  ld_qty_oh <> 0   and ld_site = V1002 no-lock no-error.
If AVAILABLE ( ld_det ) then
                L13003 = "最大料号:" + ld_part .
                else L13003 = "" . 
		...
		kk = 0.
		for each ld_det where ld_loc = V1200 and ld_qty_oh <> 0 and ld_site = V1002 break by ld_part.
                   if first-of(ld_part) then kk = kk + 1.
		end.
                find first pt_mstr no-lock no-error .
		if Available (pt_mstr) then
                L13004 = "总共料品个数: " + string (kk) .
                else L13004 = "" . 
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[WhatIn货架查询]"  + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1510 no-box.

                /* LABEL 1 - START */ 
                L15101 = "*库存明细*" + trim (V1200) .
                display L15101          format "x(40)" skip with fram F1510 no-box.
                /* LABEL 1 - END */ 


                /* LABEL 2 - START */ 
                find first ld_det where ld_part >= V1300  and ld_loc = V1200   and  ld_qty_oh <> 0   and ld_site = V1002 no-lock no-error.
If AVAILABLE ( ld_det ) then
                L15102 = "料品:" + trim(ld_part) .
                else L15102 = "" . 
...
If AVAILABLE ( ld_det ) then
                ...
                find first ld_det where ld_part >= V1300  and ld_loc = V1200   and  ld_qty_oh <> 0   and ld_site = V1002 no-lock no-error.
If AVAILABLE ( ld_det ) then
                L15104 = "数量:" + trim(string(LD_QTY_OH))  + ( if ld_ref = "" then "" else "/" + trim ( ld_ref ) ) .
                else L15104 = "" . 
            ...
            THEN DO:
               if v_recno = ? then
                  find first LD_DET where 
                              LD_PART >= V1300 AND LD_QTY_OH <> 0 AND LD_LOC = V1200 AND LD_SITE = V1002 AND  
                              LD_PART >  INPUT V1510
                   no-lock no-error.
               else
                  find next LD_DET where 
                              LD_PART >= V1300 AND LD_QTY_OH <> 0 AND LD_LOC = V1200 AND LD_SITE = V1002
                   no-lock no-error.
                  ...
                  IF AVAILABLE LD_DET then display skip 
                         LD_PART @ V1510 "料品:" + trim(ld_part) @ L15102
		     "批号:" + trim(LD_LOT)  @ L15103
"数量:" + trim(string(LD_QTY_OH))  + ( if ld_ref = "" then "" else "/" + trim ( ld_ref ) )  @ L15104
  LD_LOT + "/" + trim(string(LD_QTY_OH))  + ( if ld_ref = "" then "" else "/" + trim ( ld_ref ) ) @ WMESSAGE NO-LABEL with fram F1510.
