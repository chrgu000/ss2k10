            ...
            THEN DO:
               if v_recno = ? then
                  ...
                  IF not AVAILABLE SO_MSTR then do: 
                      if v_recno <> ? then 
                          find SO_MSTR where recid(SO_MSTR) = v_recno no-lock no-error .
                      else 
                          find last SO_MSTR where 
                              ( SO_SITE = "" OR SO_SITE = V1002 )
                          no-lock no-error.
                  end. 
                  v_recno = recid(SO_MSTR) .
        ...
        display skip "^" @ WMESSAGE NO-LABEL with fram F1205.
            ...
            THEN DO:
               if v_recno = ? then
                  ...
                  find next SOD_DET where 
                              sod_nbr = V1100 and sod_site = V1002
                   no-lock no-error.
                  IF not AVAILABLE SOD_DET then do: 
                      if v_recno <> ? then 
                          find SOD_DET where recid(SOD_DET) = v_recno no-lock no-error .
                      else 
                          find last SOD_DET where 
                              sod_nbr = V1100 and sod_site = V1002
                          no-lock no-error.
                  end. 
                  v_recno = recid(SOD_DET) .
