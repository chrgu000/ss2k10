...
batchrun = no . /*cimload,有些会改成yes,这里改回来*/
     ...
     REPEAT:
        ...
        leave V1002L.
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1002 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1110 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1200 no-box.
...
If AVAILABLE ( code_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1203 no-box.
   ...
   REPEAT:
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1300 no-box.
...
If AVAILABLE ( wo_mstr ) then
            ...
            THEN DO:
               if v_recno = ? then
                  ...
                  IF AVAILABLE LAD_DET then display skip 
                         trim(lad_PART) + "@" + lad_lot + "@" + lad_loc @ V1300 "备料库位/数量:" + lad_loc + "@" + string ( lad_qty_pick) @ WMESSAGE NO-LABEL with fram F1300.
        ...
        If not avail wod_det then do:
                display skip "工单不需求此零件" @ WMESSAGE NO-LABEL with fram F1300.
                pause 0 before-hide.
                Undo, retry.            
        End.
        ...
        If not avail lad_det  then do:
                display skip "备料需求不匹配" @ WMESSAGE NO-LABEL with fram F1300.
                pause 0 before-hide.
                Undo, retry.
        End.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1400 no-box.
        ...
        find first lad_det where 
         ...
         no-lock no-error .
         If not avail lad_det then do:
             display "备料明细不存在" @ WMESSAGE NO-LABEL with fram F1400.
             undo,retry .
         End.
         /*  ---- Valid Check ---- END */
...
If AVAILABLE ( pt_mstr ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1410 no-box.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1500 no-box.
...
If AVAILABLE ( ld_det ) then
        ...
        find first lad_det where 
         ...
         no-lock no-error .
         if not avail lad_det then do:
             display "备料明细不存在" @ WMESSAGE NO-LABEL with fram F1500.
             undo,retry .
         end.
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1600 no-box.
        ...
        if decimal(v1600) = 0 then do:
                display skip "发料数不可为零" @ WMESSAGE NO-LABEL with fram F1600.
                pause 0 before-hide.
                Undo, retry.
        end.
        ...
        Find first lad_det where 
         ...
         no-lock no-error .
         If not avail lad_det then do:
             display "备料明细不存在" @ WMESSAGE NO-LABEL with fram F1600.
             undo,retry .
         End.
         Else do:
             if decimal(v1600) > lad_qty_pick  then do:
                 display "超过备料明细数量" @ WMESSAGE NO-LABEL with fram F1600.
                 undo,retry .
             End.
         End.
         /*  ---- Valid Check ---- END */
     ...
     REPEAT:
        ...
        if avail tr_hist then do:
            ...
            if avail ld_det then do:
                v1660 = ld_loc .
                l16606 = "当前库存:" + string(ld_qty_oh)  + "/" + v1660.
            end.
        end.
        else do:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1660 no-box.
        ...
        IF NOT AVAILABLE LOC_MSTR then do:
                display skip "无效库位,请重新输入" @ WMESSAGE NO-LABEL with fram F1660.
                pause 0 before-hide.
                Undo, retry.
        End.

        If v1660 = v1400 then do:
            display skip "转入库位不可与生产线相同" @ WMESSAGE NO-LABEL with fram F1660.
            pause 0 before-hide.
            Undo, retry.
        End.
         /*  ---- Valid Check ---- END */
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F1700 no-box.
                ...
                /* LABEL 3 - START */ 
                L17003 = "从/至:" + trim ( V1400 ) + "/" + trim(v1660) .
                display L17003          format "x(40)" skip with fram F1700 no-box.
                /* LABEL 3 - END */ 


                /* LABEL 4 - START */ 
                L17004 = "退料数量:" + trim( V1600 ) .
                ...
                find first ld_Det where ld_site = v1002 and ld_loc = v1660 and ld_part = v1300 and ld_lot = v1500 and ld_ref = "" no-lock no-error .
                  L17005 = if avail ld_det then "现有库存:" + string(ld_qty_oh) else "现有库存:0" .
...
If AVAILABLE ( tr_hist ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9000 no-box.
   ...
   /* Additional Labels Format */
     ...
     /*  Update MFG/PRO END  */ 
        ...
        pause 0.
     /* START  LINE :9005  修改备料量Allocation  */
     V9005L:
     REPEAT:
        ...
        leave V9005L.
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9005 no-box.
...
IF 1 = 2 THEN
        leave V9010L.
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9010 no-box.
...
If AVAILABLE ( tr_hist ) then
                ...
                Else do:
                    ...
                    V_qty_tmp = 0 .
                    L90103 = "产线剩余库存:" + string(v_qty_tmp) .
                    For each lad_det 
                        where lad_dataset = "wod_det"
                        and lad_nbr  = v1110 
                        and lad_part = v1300
                    ...
                    If avail ld_det then do:
                        L90104 = "仓库剩余库存:" + string(ld_qty_oh)  + "/" + v1660.
                    end.
                End.  
                If 1 = 1 then
                L90103 = L90103 .
   ...
   IF NOT (V9010 = "#" AND V1700 = "#" ) THEN LEAVE V9110LMAINLOOP.
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9110 no-box.
...
If AVAILABLE ( pt_mstr ) then
   ...
   /* Additional Labels Format */
     ...
     REPEAT:
        ...
        /* LOGICAL SKIP START */
        /* LOGICAL SKIP END */
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9120 no-box.
...
If AVAILABLE ( upd_det ) then
        ...
        /* --CYCLE TIME SKIP -- END  */
                ...
                display "[工单退料转仓]"      + "*" + TRIM ( V1002 )  format "x(40)" skip with fram F9130 no-box.
