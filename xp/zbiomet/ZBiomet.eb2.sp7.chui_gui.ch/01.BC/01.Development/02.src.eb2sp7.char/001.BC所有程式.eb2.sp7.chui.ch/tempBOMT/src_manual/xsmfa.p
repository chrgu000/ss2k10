...
/* Register Clicense Code  Start*/         
        ...
        if substring(summaryusr,length(summaryusr),1) = "|" then summaryusr = substring(summaryusr,1,length(summaryusr) - 1) .
        ...
        if loginuser > integer ( counter ) then do:     
            ...
            unix value (command).
            unix silent value ("rm -f " + trim(usection) + ".tmp"). /*xp001*/
            quit.
        end.
...
/* Register Clicense Code  End*/         
     ...
     if available usr_mstr   then do: 
            ...
            display  """" + trim(psid) + """"  + " " + """" + substring( encode ( psid + "99" ) ,1,8) + """" format "X(50)" skip
                   /*"-" skip   *这里是域,不移除,在eb2.0会cim栏位错*/
                   "." skip
                   "P" skip   /*如果这里cim栏位错,下面一定可以*/
                   "." skip
                   "P" skip  
