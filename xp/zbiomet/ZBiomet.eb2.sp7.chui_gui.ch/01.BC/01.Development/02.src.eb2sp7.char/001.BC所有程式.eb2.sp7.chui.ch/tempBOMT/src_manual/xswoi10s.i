...
define var v_qty_before4 like ld_qty_oh .
define var v_qty_before5 like ld_qty_oh .
...
if avail wod_det then do:
            ...
            find first lad_det 
                ...
                and lad_ref  = "" 
            no-lock no-error . 
            v_qty_before3 = if avail lad_det then (lad_qty_pick ) else 0 .  /*本次发料前,该批次在生产线库位的领料数量,发料后增加*/
            v_qty_before4 = if avail lad_det then (lad_qty_all  ) else 0 .  /*本次发料前,该批次在生产线库位的备料数量,发料后变0 */


            /*v_qty_before5本次发料前,该批次在其他库位(非本次和非产线库位)的数量,用于最后累加*/
            find first lad_det 
                where lad_dataset = "wod_det"
                and lad_nbr = v1110 
                and lad_line = string(v_op )
                and lad_part = v1300
                and lad_site = v1002
                and (lad_loc <> v1660 and lad_loc <> v1550 )
                and lad_lot  = v1500
                and lad_ref  = "" 
            no-lock no-error . 
            v_qty_before5 = if avail lad_det then (lad_qty_pick + lad_qty_all ) else 0 .  /*本次发料前,该批次在生产线库位的备料/领料数量*/
            ...
            */
            ...
            if v_qty_other + v_qty_iss  + v_qty_before3 + v_qty_before5  + decimal(V1600) <> v_qty_need then do:
               find first wo_mstr where wo_lot = v1110 no-lock no-error .
               if v_chg_bom then do:
                   v_qty_need = v_qty_other + v_qty_iss + decimal(V1600) + v_qty_before3  + v_qty_before5  .
                   v_qty_bom  = truncate(v_qty_need / wo_qty_ord,5) .
               end.
               else do:
                   if v_qty_other + v_qty_iss  + v_qty_before3  + v_qty_before5 + decimal(V1600) > v_qty_need then 
                   v_qty_need = v_qty_other + v_qty_iss + decimal(V1600) + v_qty_before3 + v_qty_before5   .
               end.
            end.
