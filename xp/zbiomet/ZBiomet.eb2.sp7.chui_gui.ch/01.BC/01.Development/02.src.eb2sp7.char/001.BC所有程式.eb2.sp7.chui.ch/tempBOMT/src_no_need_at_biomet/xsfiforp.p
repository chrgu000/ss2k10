...
repeat:
   ...
   /* ICRLRPD.P -- DETAIL  REPORT SORTED BY SITE/ITEM             */
	...
	display tr_userid  label "用户"  
		tr_effdate column-label "发料日期" 
		tr_trnbr   column-label "交易号"
		trim( tr_nbr )     label "工单" 
		tr_lot	   label "ID" 
		trim( tr_part )   label "料号" format "x(18)"
		tr_loc     column-label "发料库位"
		tr_serial  label "实发批号"  format "X(15)"
		( - tr_qty_loc ) label "实发数量"  
		substring(tr__chr01,1,18) label "最小批号" format "X(15)"
		substring (tr__chr01,19,20) label "批号数量" 
