...
define variable ciminputfile3   as char .
define variable cimoutputfile3  as char .
...
if available tr_hist then do:
   ...
   wsite = tr_site.
   wloc  = tr_loc.
   /*   Subcontract PO   Use woworc to Receipt the Goods  START */
	ausection = TRIM ( string(year(TODAY)) + string(MONTH(TODAY)) + string(DAY(TODAY)))  + trim(STRING(TIME)) + trim(string(RANDOM(1,100))) .
	output to value( trim(ausection) + ".swi") .
	 

	display 
	"- "  + trim ( V1302 ) + "  " +  trim ( V1203 ) format "x(50)" skip
	trim ( V1600 ) + "  - - - - -  - " +  trim( V1400 ) format "X(30)"  if trim( V1500 ) = "" then " - " else """" + trim(V1500) + """"  format "X(25)" skip
	trim ( V1100 ) skip
	"Y" skip
	"Y" skip
	"."
	...
	input from value ( ausection + ".swi") .
	output to  value ( ausection + ".swo") .
		{gprun.i ""woworc.p""}
	input close.
	output close.

	ciminputfile1  = ausection + ".swi".
        cimoutputfile1 = ausection + ".swo".
        {xserrlg1.i}

   /*   Subcontract PO   Use woworc to Receipt the Goods  END */
...
/*  Round to 2 Decimal  Start */
	...
	if available ld_det then  do:   
	   ...
	   if ld_qty_oh <> roundto2  then do:
		...
		input close.
		output close.
		ciminputfile2  = ausection + ".2di".
		cimoutputfile2 = ausection + ".2do".
                {xserrlg2.i}
...
/*  BLANK THE  Inventory Expire Date Start */
	...
	if ( available ld_det ) and pt_shelflife <> 0 then  do:   
		...
		input close.
		output close.
		ciminputfile3  = ausection + ".iei".
		cimoutputfile3 = ausection + ".ieo".
		{xserrlg3.i}
