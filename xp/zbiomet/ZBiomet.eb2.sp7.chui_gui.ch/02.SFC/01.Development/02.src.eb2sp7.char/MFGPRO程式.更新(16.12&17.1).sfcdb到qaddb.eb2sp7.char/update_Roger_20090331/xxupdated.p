...
v_tmp_path = "/mfgeb2/bc_test/tmp_sfc/" .  /*Roger:程式中的每个'v_tmp_path +',都是本次直接添加,未作ECO */
...
define var v_tmp_path as char format "x(30)" . /*默认输入输出路径*/
v_tmp_path = "./" .   /*Roger:未做eco,所以加本行(原来是默认当前路径,这样修改后依然是当前路径)*/
...
form
    ...
    skip(1)
with frame a 
title color normal "更新MFG/PRO数据BY(天) => 全部才更新"
...
/* SS - 090330.1 - E */    
	...
	PROCEDURE PickListComplete:

	     DEFINE INPUT  PARAMETER pwolot like xxwrd_wolot.
	     DEFINE INPUT  PARAMETER pop    like xxwrd_op.
	     DEFINE INPUT  PARAMETER pwrnbr like xxwrd_wrnbr.

	     DEFINE OUTPUT PARAMETER OkPickListComplete     AS logical init no.
	     OkPickListComplete = no.
	     for each yywrddet where yywrddet.xxwrd_wrnbr = pwrnbr and yywrddet.xxwrd_lastop = yes and yywrddet.xxwrd_status <> "D" 
				     no-lock break by yywrddet.xxwrd_wolot :
		 if first-of (xxwrd_wolot)  then do:
		    if yywrddet.xxwrd_qty_ord = yywrddet.xxwrd_qty_comp + yywrddet.xxwrd_qty_rejct then   OkPickListComplete = yes.
		    if debugOnly = yes then message "工序:" + pwolot + ",对应的成品ID=" + yywrddet.xxwrd_wolot + " ,其完工情况=" OkPickListComplete ",可以更新=" OkPickListComplete view-as alert-box .
	...
	For each xywrddet where xywrddet.xxwrd_close = no and xywrddet.xxwrd_lastop = yes  and 
		 ...
		 RUN PickListComplete (INPUT xywrddet.xxwrd_wolot , INPUT xywrddet.xxwrd_op ,INPUT xywrddet.xxwrd_wrnbr, OUTPUT OkPickListComplete). 
		 if OkPickListComplete = yes then do  :
			 ...
			 End.  /* IF OkToUpdate = yes then do: */
		 end.  /*   if PickListComplete = yes then do  :  */
