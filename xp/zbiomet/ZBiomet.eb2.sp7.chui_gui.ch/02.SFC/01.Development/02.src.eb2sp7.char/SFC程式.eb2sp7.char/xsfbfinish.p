...
repeat :
    ...
    do  on error undo,retry: /*update_loop*/
        ...
        /*首工序,可能改变后续订购量,所以这里锁定后续所有工序*/
        if v_prev = no then do: 
            ...
            for each ttt :      
                ...
                if not avail xxwrd_det then delete ttt .
                    ...
                    if not avail xxwrd_det then do:
                        if locked xxwrd_det then do:
                            message  "工单条码(" ttt_wolot "+" string(ttt_op) ")正在被使用,按任意键退出." view-as alert-box title "" .
        ...
        if v_qty_now > 0 then do:
            ...
            if v_yn4 then do:
                if v_qty_now > v_qty_check then do:
                    message "不可超过评审数量("  v_qty_check  ")请重新输入" .
                    undo,retry .
                end.
            end.
