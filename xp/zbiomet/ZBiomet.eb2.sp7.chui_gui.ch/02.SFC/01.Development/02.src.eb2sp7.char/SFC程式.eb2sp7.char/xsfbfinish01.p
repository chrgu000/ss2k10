...
form
    ...
    v_qty_check   colon 45 label "评审数量"
    skip(1)       
    v_qty_now     colon 18 label "本次评审数量"

    skip(5)
with frame a 
title color normal "评审反馈"
...
repeat :
    ...
    do  on error undo,retry: /*update_loop*/
        ...
        disp v_qty_ord2 v_qty_comp v_qty_rjct v_qty_rework v_qty_return  v_qty_check  v_part v_inv_lot v_wolot v_op with frame a .
        ...
        if v_qty_now = 0 then do:
            message "评审数量不可为零.请重新输入" .
            undo,retry .
        end. /**/

        if v_qty_rework - v_qty_return > 0 then do:
            message "是否返工件完工评审?" update v_yn3 .
        end.
        ...
        if v_qty_now > 0 then do:
            ...
            if  v_prev = yes then do:
                if (v_qty_now + v_qty_check + (v_qty_rework - v_qty_return)  + v_qty_comp + v_qty_rjct) * v_qty_bom > v_qty_prev then do:
                    message "累计数量不可超过前工序(" v_prev_wolot "+" v_prev_op ")合格量("  v_qty_prev  ")请重新输入" .
                    undo,retry .
                end.

                if v_qty_now + v_qty_check  + (v_qty_rework - v_qty_return) + v_qty_comp + v_qty_rjct > v_qty_ord2 then do:
                    message "累计数量不可超过订购量("  v_qty_ord2  ")请重新输入" .
                    undo,retry .
                end.
            end.

            if v_qty_now + v_qty_check  + (v_qty_rework - v_qty_return) + v_qty_comp + v_qty_rjct > v_qty_ord2 * (v_tol_pct / 100 ) then do:
                message "数量超限,请重新输入" .
                undo,retry .
            end.
        ...
        if v_qty_now < 0 then do:
            if (v_qty_check  < - v_qty_now)  then do:
                message "不可超过已评审数量,请重新输入" .
                undo,retry .
            end.
...
do  :  /*xxfb*/
    ...
    if avail xxwrd_det then do:
            ...
            assign xxwrd_qty_check = xxwrd_qty_check + v_qty_now .
