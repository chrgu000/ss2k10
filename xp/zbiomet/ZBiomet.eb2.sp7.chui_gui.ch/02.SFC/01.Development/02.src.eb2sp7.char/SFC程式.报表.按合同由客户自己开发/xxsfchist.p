      define shared variable v_wc like xwc_wkctr.
      define shared variable v_wolot like xwo_lot.
      define shared variable v_op like xwr_op.

      define variable l_userid as char label "操作员" no-undo.
      define variable wkctr like xwc_wkctr label "机器".
      ...
      repeat:
         ...
         on endkey undo, leave loopa with frame b 12 down width 80:
            ...
            find first xcode_mstr where xcode_fldname = "SFC"
                   and xcode_value = xxfb_type no-lock no-error.

            hrs = (xxfb_time_end - xxfb_time_start) / 60.

            display
               xxfb_wc
               (if available xcode_mstr then substring(xcode_cmmt,1,index(xcode_cmmt,"@") - 1) 
                else "") @ xcode_cmmt format "x(9)" label "交易类型"
               xxfb_date
