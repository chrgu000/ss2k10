...
/* SS - 101022.1  By: Roger Xiao */ /*库位权限通用代码: "ISS-341" + LOC + UserList */
/*-Revision end---------------------------------------------------------------*/
   ...
   repeat transaction:
      ...
      do for lddet on error undo, retry with frame a:
         ...
         do on error undo:
            ...
            /* SS - 101022.1 - B */
            define var v_canrun as logical initial no .
            {gprun.i ""xxicck341.p"" "(input loc_from, output v_canrun)"}
            if v_canrun = no then do:
                message "错误:您无权限从本库位转出库存,请重新输入" .
