...
define var unpqty like tr_qty_loc .
...
define var tot-unpqty like tr_qty_loc .
...
repeat:
    ...
    if v_loc = "" or (not can-find(first loc_mstr where loc_site = v_site and loc_loc = v_loc no-lock)) then do:
        message "错误:无效地点+库位,请重新输入" .
        next-prompt v_loc .
        undo,retry.
    end.
...
do on error undo, return error on endkey undo, return error:                    
    ...
    put "湖南长丰汽车沙发有限责任公司长沙分公司 总装车间材料核算报表"  at 35 skip.
    ...
    form
        ...
        rctqty                                                      column-label "本月领用"     format "->>,>>>,>>9.99<<"
        issqty                                                      column-label "本月实际用料" format "->>,>>>,>>9.99<<"
        unpqty                                                      column-label "本月内部损耗" format "->>,>>>,>>9.99<<"
    ...
    for each tmp_mstr no-lock break by tmp_type by tmp_part:
        if first-of(tmp_type) then do:
            qmqty = 0 .
            qcqty = 0 .
            rctqty = 0.
            issqty = 0.
        ...
        assign 
            rctqty  = rctqty + tmp_qty8 * price
            issqty  = issqty + tmp_qty10 * price
            unpqty  = unpqty + tmp_qty12 * price
