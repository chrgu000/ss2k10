...
/* Revision: 1.42.5.2    BY: Dorota Hohol      DATE: 08/06/03 ECO: *P0Z1* */
/* Revision: 1.42.5.3    BY: Dorota Hohol      DATE: 08/27/03 ECO: *P11B* */
/* $Revision: 1.42.5.4 $   BY: Jyoti Thatte      DATE: 09/05/03 ECO: *P106* */
/* $Revision: 110907.1$   BY: zhangyun      DATE: 09/07/11 add vd__chr03      */
/******************************************************************************/
      ...
      if c-application-mode <> "API" then do:
         ...
         do transaction with frame a on endkey undo, leave mainloop:
            ...
            if input vd_addr = "" then do:
               {&ADVNMT-P-TAG5}
               {mfactrl.i vdc_ctrl vdc_nbr ad_mstr ad_addr vdnbr}
               {&ADVNMT-P-TAG6}
               vdnbr = string(integer(vdnbr),"99999999").
            end. /* if input vd_addr = "" then do: */
            if input vd_addr <> "" then vdnbr = input vd_addr.
         end. /*newloop */
      end.  /* If c-application-mode <> "API" */
