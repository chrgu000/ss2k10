...
/* Revision: 1.67.1.8  BY: Max Iles             DATE: 07/05/04  ECO: *N2VQ*   */
/* Revision: 1.67.1.14 BY: Priya Idnani         DATE: 10/13/04  ECO: *P2PM*   */
/* Revision: 1.67.1.16 BY: Dan Herman           DATE: 11/04/04  ECO: *M1V1*   */
/* Revision: 1.67.1.17 BY: Tejasvi Kulkarni     DATE: 12/28/04  ECO: *P31T*   */
/* Revision: 1.67.1.21 BY: Shivanand H          DATE: 01/06/05  ECO: *P32W*   */
/* Revision: 1.67.1.22 BY: Bhagyashri Shinde    DATE: 01/25/05  ECO: *P35C*   */
/* Revision: 1.67.1.23 BY: Priyank Khandare     DATE: 02/02/05  ECO: *P35K*   */
/* Revision: 1.67.1.24 BY: Ajit Philip          DATE: 14/03/05  ECO: *P3BV*   */
/* Revision: 1.67.1.25 BY: Preeti Sattur        DATE: 06/23/05  ECO: *P3QJ*   */
/* Revision: 1.67.1.26 BY: Anitha Gopal         DATE: 08/16/05  ECO: *P3RL*   */
/* Revision: 1.67.1.27 BY: Rafiq S              DATE: 12/05/05  ECO: *P4B8*   */
/* Revision: 1.67.1.29 BY: Kirti Desai          DATE: 12/28/05  ECO: *P4CF*   */
/* Revision: 1.67.1.30 BY: Shridhar M           DATE: 02/19/06  ECO: *P4J2*   */
/* Revision: 1.67.1.31 BY: Munira Savai         DATE: 03/20/06  ECO: *P4K6*   */
/* Revision: 1.67.1.32 BY: SurenderSingh Nihalani DATE: 04/27/06  ECO: *P4PQ* */
/* Revision: 1.67.1.33 BY: SurenderSingh Nihalani DATE: 05/22/06  ECO: *P4S2* */
/* Revision: 1.67.1.37 BY: SurenderSingh Nihalani DATE: 06/23/06  ECO: *P4TZ* */
/* Revision: 1.67.1.38 BY: Munira Savai DATE: 09/11/06  ECO: *P55G* */
/* $Revision: 1.67.1.39 $ BY: Munira Savai DATE: 09/13/06  ECO: *P561* */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
...
repeat on endkey undo, leave:
   ...
   repeat:
      ...
      do transaction on error undo, retry:
         ...
         if c-application-mode = "API" then do:
            ...
            if not available ttPurchaseOrderTransDet then leave.
         end.
      ...
      do transaction:
         ...
         if available sr_wkfl and (not can-find(first sr_wkfl where
            recid(sr_wkfl) <> l_sr_wkfl_recid and sr_lineid = cline))
            then do:
               ...
               */
               ...
               no-lock:
                  ...
                  then do:
                     ...
                     for each sr_wkfl
                        fields (sr_site sr_userid sr_lineid sr_loc)
                        where   sr_site    = site
                        and     sr_userid  = mfguser
                        and     sr_lineid  = cline
                     no-lock:
                        if not can-find (first loc_mstr
                                            where loc_site   = sr_site
                                            and   loc_loc    =
                                               if sr_site <> pod_site
                                               then
                                                  pt_loc
   ...
   if not batchrun then do:
      do on endkey undo edit-loop, leave edit-loop:
         ...
         run p_msg_confirm (input        msgnbr,
      ...
      do on endkey undo edit-loop, leave edit-loop:
         ...
         if yn then do:
            ...
            for each sr_wkfl
             ...
             if (accum total cnsix_qty_consigned) < abs(sr_qty)
                     then do:
                        /* NO CONSIGNED INVENTORY CAN BE RETURNED FOR PO LINE */
                        run p_msg2 (input 6303,
                                 input 3,
...
PROCEDURE get_pt_description:
   ...
   for first pt_mstr
      fields(pt_auto_lot pt_loc  pt_lot_grp pt_desc1 pt_um
...
END PROCEDURE. /* create_t_sr_wkfl */
...
PROCEDURE p_msg:
   ...
   define input parameter l_err_msg  like msg_nbr     no-undo.
   define input parameter l_err_lvl  as   integer     no-undo.

   {pxmsg.i &MSGNUM=l_err_msg &ERRORLEVEL=l_err_lvl}
...
PROCEDURE p_msg_confirm:
   ...
   {pxmsg.i &MSGNUM=l_err_msg &ERRORLEVEL=l_err_lvl &CONFIRM=l_confirm}
...
PROCEDURE p_msg2:
    ...
    define input parameter l_msgarg2 as   character no-undo.

    {pxmsg.i &MSGNUM=l_err_msg &ERRORLEVEL=l_err_lvl
             &MSGARG1=l_msgarg1 &MSGARG2=l_msgarg2}
...
PROCEDURE p_validateTransaction:
   ...
   for first ld_det
      fields (ld_site ld_loc ld_lot ld_ref ld_part ld_status)
      where ld_site = p_site
   ...
   then do:
      ...
      then do:
         ...
         for first si_mstr
            fields (si_site si_status si_auto_loc si_db si_entity)
   ...
   for first isd_det
      fields (isd_tr_type isd_status isd_bdl_allowed)
      where isd_tr_type = p_transtype
