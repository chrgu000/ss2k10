...
/* Revision: 1.66.1.2 BY: Orawan S.              DATE: 05/26/03   ECO: *P0RG* */
/* Revision: 1.66.1.3 BY: Vinay Soman            DATE: 09/25/03   ECO: *P145* */
/* Revision: 1.66.1.4 BY: Nishit V               DATE: 01/27/04   ECO: *P1LB* */
/* Revision: 1.66.1.5 BY: Subramanian Iyer       DATE: 03/12/04   ECO: *N2QC* */
/* Revision: 1.66.1.6 BY: Sachin Deshmukh        DATE: 08/24/04   ECO: *P2GP* */
/* Revision: 1.66.1.7 BY: Dan Herman             DATE: 11/04/04   ECO: *M1V1* */
/* Revision: 1.66.1.8 BY: Tejasvi Kulkarni       DATE: 12/28/04   ECO: *P31T* */
/* Revision: 1.66.1.9 BY: Swati Sharma           DATE: 03/04/05   ECO: *P3B2* */
/* Revision: 1.66.1.10 BY: SurenderSingh Nihalani DATE: 04/27/06  ECO: *P4PQ* */
/* Revision: 1.66.1.11 BY: SurenderSingh Nihalani DATE: 05/11/06  ECO: *P4RG* */
/* Revision: 1.66.1.12 BY: SurenderSingh Nihalani DATE: 05/22/06  ECO: *P4S2* */
/* $Revision: 1.66.1.13 $ BY: SurenderSingh Nihalani DATE: 06/08/06  ECO: *P4TZ* */
/*-Revision end---------------------------------------------------------------*/
...
{&POPORCM-P-TAG3}
define variable ordernum        like po_nbr  initial "z122yd01"         no-undo.
{&POPORCM-P-TAG4}
...
repeat:
   ...
   do transaction:
      ...
      then do:
         ...
         for each pod_det
            fields (pod_nbr pod_status pod_part)
            where pod_nbr    = po_nbr
            and   pod_status = ""
         no-lock use-index pod_nbrln:

            for first pt_mstr
               fields (pt_part pt_lot_ser)
...
------------------------------------------------------------------------------*/
   ...
   if proceed = yes then do:
   	message "1" shipnbr "2" ship_date "3" inv_mov "4" ip_usage "5" ip_usage_qty "6" ip_trnbr view-as alert-box.
...
PROCEDURE p_validateTransaction:
   ...
   then do:
      ...
      for first loc_mstr
         fields (loc_site loc_loc loc_status loc_single loc__qad01)
      ...
      then do:
         ...
         for first si_mstr
            fields (si_site si_status si_entity)
