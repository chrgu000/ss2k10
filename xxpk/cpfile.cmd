set src=C:\qadguicli\garbage\src
set xxpk="D:\ss\trunk\showa\xxpk"
copy /y %xxpk%\xxrepkrp0\xx*.? %src%\ 
copy /y %xxpk%\xxrepkup0\xx*.? %src%\
copy /y %xxpk%\xxcomm\xx*.? %src%\
copy /y %xxpk%\xxlnmmt\xx*.? %src%\
copy /y %xxpk%\xxlnmrp\xx*.? %src%\
copy /y %xxpk%\xxlnwkmt\xx*.? %src%\
copy /y %xxpk%\xxlnwrp\xx*.? %src%\
copy /y %xxpk%\xxwaini\xx*.? %src%\
copy /y %xxpk%\..\usrw\*.* %src%\
copy /y D:\ss\trunk\showa\bc\xs\xspktr.p %src%\
copy /y D:\ss\trunk\showa\bc\xs\xspkis.p %src%\
copy /y D:\ss\trunk\showa\bc\xs\xspkc.p %src%\
copy /y D:\ss\trunk\showa\bc\xs\xsdfsite.i %src%\
copy /y D:\ss\trunk\showa\bc\xs\xspkpub.i %src%\
