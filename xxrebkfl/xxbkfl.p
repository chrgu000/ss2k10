/* xxbkfl.p   - REPETITIVE   BACKFLUSH TRANSACTION                            */
...
/* $Revision: 1.50.1.8 $ BY: zy              DATE: 02/12/12 ECO: *21.1*       */
/*-Revision end---------------------------------------------------------------*/
...
&SCOPED-DEFINE xxbkfl_p_1 "Load File"
...
/*21.1*/ define new shared variable flhload as character
								format "x(65)" label {&xxbkfl_p_1}no-undo.
{&REBKFL-P-TAG4}
...
/*21.1*/ form
/*21.1*/    space(2)
/*21.1*/    flhload colon 20
/*21.1*/    space(2)
/*21.1*/    /*V8-*/
/*21.1*/ with frame set_file row 20 width 80 centered overlay side-labels.
/*21.1*/ setFrameLabels(frame set_file:handle).
...
repeat:
   ...
   repeat:
      ...
      do with frame bkfl1 on error undo, retry:
         ...
         if mod_issrc
         then do:
/*21.1*/    update flhload with frame set-file.
/*21.1*/    IF SEARCH(flhload) = ? THEN DO:
/*21.1*/        {mfmsg.i 53 1}
/*21.1*/        PAUSE 5.
/*21.1*/    end.
            hide frame bkfl1 no-pause.
/*21.1*/    hide frame set-file.
