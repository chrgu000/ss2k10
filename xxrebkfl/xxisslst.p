/* xxisslst.p - REPETITIVE   SUBPROGRAM TO MODIFY COMPONENT PART ISSUE LIST   */
...
/*21.1*/ define shared variable flhload   as character   no-undo.
{&REISSLST-P-TAG1}
...
/*21.1*/ define temp-table tmp_data no-undo
/*21.1*/  fields t_d_part like pt_part
/*21.1*/  fields t_d_ref  like pk_reference
/*21.1*/  fields t_d_qty  like pk_qty
/*21.1*/  index t_d_det t_d_part t_d_ref t_d_qty
/*21.1*/  .
...
/* INITIALIZE TEMP-TABLE t_pk_det */
empty temp-table t_pk_det.

/* INITIALIZE TEMP-TABLE tmp_data */
for each tmp_data exclusive-lock: delete tmp_data. end.
...
/*21.1*/    input from value(flhload).
/*21.1*/        repeat:
/*21.1*/          create tmp_data.
/*21.1*/          import delimiter "~t" tmp_data.
/*21.1*/        end.
/*21.1*/    input close.
/*21.1*/  end.
...
/*21.1*/ if can-find(first tmp_data where t_d_qty > 0) then do:
/*21.1*/   for each pk_det
/*21.1*/     fields( pk_domain pk_loc pk_part pk_qty pk_reference pk_user)
/*21.1*/        where pk_det.pk_domain = global_domain
/*21.1*/        and   pk_user          = mfguser
/*21.1*/    exclusive-lock:
/*21.1*/          find first tmp_data where t_d_part = pk_det.pk_part
/*21.1*/                 and t_d_ref = pk_det.pk_reference no-error.
/*21.1*/          if available tmp_data then do:
/*21.1*/             if t_d_qty < pk_det.pk_qty then
/*21.1*/                 assign pk_det.pk_qty = t_d_qty.
/*21.1*/          end.
/*21.1*/          else do:
/*21.1*/               assign pk_qty = 0.
/*21.1*/          end.    
/*21.1*/    end.
/*21.1*/ end.  /* if can-find(first tmp_data where t_d_qty > 0) then do: */
   ...
   break by pk_user
      ...
      then do:
         ...
         create t_pk_det.
         t_pk_det.t_pk_domain = global_domain.
   ...
   then do on endkey undo , retry:
      ...
      {pxmsg.i &MSGNUM=161 &ERRORLEVEL=2 &PAUSEAFTER=true}
                     ...
                     do on error undo, retry
                        ...
                        then do:
                           ...
                           then do:
                              ...
                              if not available pk_det
                              then do:
                                 create pk_det. pk_det.pk_domain =
                                 global_domain.
                                 assign
                                    pk_user      = mfguser
                                    pk_part      = part
