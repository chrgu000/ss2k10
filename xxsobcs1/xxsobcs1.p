...
DEFINE TEMP-TABLE tt2
    ...
    FIELD tt2_desc2 LIKE pt_desc2
    FIELD tt2_per_qty LIKE pt_ship_wt
    ...
    FIELD tt2_soil AS CHARACTER /*shen110127*/
    /* ss - 111130.1 -b */
    FIELD tt2_style AS CHAR
    FIELD tt2_grade AS CHAR
    /* ss - 111130.1 -e */
...
do on error undo, return error on endkey undo, return error:
    ...
    FOR EACH xxsod_det NO-LOCK WHERE
             xxsod_cust >= cust AND
             xxsod_cust <= cust1 AND
             date(int(entry(2,xxsod_due_date1,"-")),
                  int(ENTRY(3,xxsod_due_date1,"-")),
                  int(ENTRY(1,xxsod_due_date1, "-"))) >= v_due_date AND
             date(int(entry(2,xxsod_due_date1,"-")),
                  int(ENTRY(3,xxsod_due_date1,"-")),
                  int(ENTRY(1,xxsod_due_date1, "-"))) <= v_due_date1 AND
             xxsod_due_time >= v_time AND
             xxsod_due_time <= v_time1
       ,EACH cp_mstr WHERE cp_cust = xxsod_cust AND
             cp_cust_part = xxsod_part AND
             cp_user1 = prt_color NO-LOCK
      ,EACH pt_mstr WHERE pt_part = cp_part AND
            pt_buyer MATCHES  xbuyer AND
           (pt_drwg_loc = model OR model = "") AND
            pt_part >= part AND
            pt_part <= part1 NO-LOCK:
                 /*
                 DISP

                     date(int(entry(2,xxsod_due_date1,"-")),
                          int(ENTRY(3,xxsod_due_date1,"-")),
                          int(ENTRY(1,xxsod_due_date1, "-"))) v_due_date ">>"
                     date(int(entry(2,xxsod_due_date1,"-")),
                          int(ENTRY(3,xxsod_due_date1,"-")),
                          int(ENTRY(1,xxsod_due_date1, "-"))) v_due_date1.
                */
                ...
                FIND FIRST cp_mstr WHERE cp_cust = xxsod_cust AND
                           cp_cust_part = xxsod_part NO-LOCK NO-ERROR.
                ...
                */
                ...
                IF NOT AVAIL tt1 THEN DO:
                    ...
                    IF INDEX(cp_user2,":") <> 0 THEN
                       ASSIGN
                        tt1_style =  ENTRY(  1 , cp_user2 , ":")
                        tt1_grade =  ENTRY(  2 , cp_user2 , ":") .
                    /* ss - 111130.1 -e */
            ...
            FOR EACH tt1 BY tt1_cust BY tt1_part
                         BY tt1_ship_date BY tt1_ship_time:
...
*******************************************************************/                
            ...
            IF option_list = "1" THEN DO:
                ...
                PUT UNFORMATTED "#def REPORTPATH=$/QAD Addons/BI Report/"
                    + SUBSTRING(execname,1,LENGTH(execname) - 2) SKIP.
                PUT UNFORMATTED "#def :end" SKIP.
                FOR EACH tt2 BY tt2_id:
                    EXPORT DELIMITER ";" tt2_id
                                         tt2_part
                                         tt2_color
                                         tt2_title
                                         tt2_cust_part
            ...
            ELSE DO:
                PUT UNFORMATTED "#def REPORTPATH=$/QAD Addons/BI Report/"
                  + SUBSTRING(execname,1,LENGTH(execname) - 2) + "_"
                  + upper(prt_color) SKIP.
                PUT UNFORMATTED "#def :end" SKIP.
                FOR EACH tt2 BY tt2_id:
                    EXPORT DELIMITER ";" tt2_id
                                         tt2_part
