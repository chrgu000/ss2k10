...
if length( trim ( V1500 ) ) >= 10 then do:
  DO w = 1 to 6.
     /* Error Format PASS */
     if index("0987654321", substring(V1500, w ,1)) = 0 then do :
        wPASS = "Y" .
     end.
...
if wPASS = "N" and ( lotyear <> "" or lotmonth <> "" or lotday <> "" ) then do:
  ...
  if  integer ( lotday ) > day ( date (
                 (if lotmonth = "12" then 1 else  integer(lotmonth) + 1  ),
                  1 ,
                 (if lotmonth = "12" then 1 + integer ( "20" + lotyear )
                                     else  integer ( "20" + lotyear )  )
              ) - 1 )  then do:
     wPASS = "Y".
     lotday = "01".
     lotmonth = "01".
...
*/
find first  in_mstr where in_site=ld_site and in_part=ld_part no-lock no-error.
if avail in_mstr then
wptavgint = in_avg_int.
else wptavgint=0.
...
if wPASS = "N" then do :
        wLotdate = date (integer(lotmonth)
                        ,integer ( lotday )
                        ,integer ( "20" + lotyear ) )  .
        waddyear  = 0.
        waddmonth = 0.
